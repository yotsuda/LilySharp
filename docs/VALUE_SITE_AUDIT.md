...
**`drummap` は第172第2便で「観測者を先に立てる」とユーザーが決めた（§7 ④）**——
**`@name(引数)` は第168 でユーザーが決めた（§9）＝第172第1便で `@fig` まで完走**。
⇒ ★ **⒯ に決定待ちの項は無い。** 次は「型を付けるか」の判断で、
**drummap にはその判断のための網が今ある**（無かったのが第172 までの状態）。
...
`@name.suffix` は点を黙って落とす。**~~ — **第168第1便で閉じた**（`d8c8c588`）。
...
6. ★★ **第167 の後に残る A3（＝この監査が次に名指すもの）**:
   ...
   - ~~**`drummap` 本体**（§7 ④）と **`@name(引数)` run** の 2 つが残り、**どちらもユーザー決定**~~
     ...
     **第172第1便で `@fig` まで完走**。**`drummap` は第172第2便で決まった**＝
     **型付けの前に観測者を立てる**（§7 ④ に実測と現状）。⇒ **決定待ちは無い。**
...
⚠️⚠️ ★★★ **第172第2便の実測＝型を付ける前に、この島には*観測者が 1 つも無かった***
（**ユーザー決定＝観測者を先に立てる**・`366d1e27`）:
- **書いている本 0 / 308 冊**（コーパス 80・フィクスチャ 219・その他の .lys も全部）。
- **テストプロジェクトに `drummap` の字が 1 つも無い**＝**網 0 本**。
- ⚠️ **それでも機能は生きている**——正しい drummap で**ドラム譜の SVG が 164 行動く**
  （`data-pos` を除いた比較）。**死んだコードではない。**
- ⚠️⚠️ **そして何を間違えても黙っていた**: **鍵・値・ドラム名・範囲が全部間違っている
  drummap** を渡すと **`check` は「No errors found」**・**SVG は drummap が無い場合と
  `data-pos` を除いて 0 行相違**＝**丸ごと無視されて、何も言わない。**
⇒ ★★★ **この状態では型を付けられない**——**「受理される綴りが変わった」の falsifier が
無い**。`@fig` は旧実装と新実装を 3,825 綴りで直接 diff できたが（§9.5.3 ⑴）、
**ここは diff できてもどちらが正しいかを言う本が 0 冊。**
⇒ **第172第2便は観測者だけを立てた**（RULES §5.0「点が先」）: **`DrummapTests` 32 本が
今日の受理集合を言明**し、**`DrummapValidator`＋LYS0024 が 4 つの黙った失敗を報告する**
（未知のドラム名・未知の鍵・範囲外・未知の値語。**受理は 1 つも動いていない**）。
⚠️ **`Entries` は名前・鍵・値の span を運ぶようになった**——読み手と validator が
**同じ 1 回の walk** を消費するため（別に歩くと §5.2.1② の 2 つ目の綴りになる）。
⚠️ ★★ **1 つだけ「無視」ではない振る舞いが見つかり、変えずに pin した**:
**未知の `mark` 語は mark を*消す***（`drummap { hhc: mark loud }` で閉じハイハットの
`stopped`（+）が落ちる）。**fall-through に見えるが決めていないので現状維持**＝
網 `DrummapTests.AnUnknownMarkWord_ClearsTheMark`・**型を付ける人が決めること。**
⚠️ **まだ報告できない形が 2 つある**（walk 自身が落とすので validator に届かない）:
**値の無い鍵**（`drummap { hh: position }`）と**最初の `name :` より前のトークン**
（`drummap { 6 hh: position 3 }`）。**どちらも今日は何も変えない**（実測）。
**報告するには walk に「どのトークンもエントリに届かなかった」と言わせる**＝
**読み手側の変更**であって validator の変更ではない。
...
（数え方も §9.5.2）。
⇒ ★★★ **`@chord` の 3 往復は第170第2便で 1 往復になった**（`4baa51a9`・§9.5.3 ⑴ の測定つき）。
`AnnotationValues.Chord` が **引数の Text を `ChordStructure.TryParseChordEntry` へ直接渡す**ので、
**「点で連結して split+join で戻す」2 ホップは消えた**。`ChordNameItem.ParseChordName` は
**2 つの `MarkName` 読みごと削除**。

### 9.4 `(` の曖昧性は第168第2便で閉じた（`ac3dca97`）
...
⚠️⚠️ ★★★ **この節の結論（「小さなパーサを書く」）は第172第1便で倒れた**——**書く必要は無かった。**
**部分言語が欲しいのは run でも値でもなく*トークン*で、それは mark ノードの子として最初から
在る**（`MarkName` が `.` で繋いでいたのが同じもの）。**run から切れ目を復元しようとすると
lexer の 2 つ目の綴りになる**（§5.2.1②）——**実測でそこは 34 綴りで食い違った**：
**`6s6` は切れて `6S0` は切れない**（小文字 `s` は音高トークン）。
⇒ **上の 2 綴り（`#6`・`6s`）という見積りも低かった**（run 版の実相違は **653 / 3,825**）。
**新しい住所は `MusicMarkSyntax.ArgumentTokens`・経緯と数は §9.5.3 ⑴。**
...
| `WarnUnknown` の `SourceSpelling`（**提案の側だけ**） | 1 | 表示用（最後まで残る） |

⇒ ★★★ **`MarkName` から「引数」が 1 つも読まれなくなった**（第172第1便）。
**残る 6 site は全部「点つき*名前*」か表示用**＝§9.5.3 ⑷ の状態に到達している。
⚠️ **`WarnUnknown` の *報告*の側は第172第1便で source へ移した**——`SourceSpelling` は
**再構成であって「書いたとおり」ではない**（`.`→` ` にするので `@fig(6.4)` を `@fig(6 4)` と
報告していた＝**有効な綴りを壊れた綴りとして名指していた**）。**残っているのは候補の描画だけ**で、
候補には source が無いので**そこは再構成が正しい用途**。

### 9.5.3 残っている作業（次便へ）

~~2. `@frame`~~ — **第169第5便で移した**（上の 8 家族目）。

~~1. ★ `@chord` の 2 ホップは綴りを変えずに消せるはず（先に測ること）~~ —
   **第170第2便で消した**（`4baa51a9`）。**測定の結果は「ほぼ真」で、外れた 1 系統は
   ユーザー決定で落とした**:
   - **コーパス 80＋フィクスチャ 219 の全数**: 引数つき `@chord(…)` は **14 site・8 綴り**、
     **全部 1 run で旧＝新（食い違い 0）**。裸 `@chord` が 17。
     ⚠️ **§9.1 の「chord 16」は*コメント内の 2 件*を数えていた**（`07-lead-sheet.lys:1` と
     `chords-funky-ignatzek.lys:4`＝**後者は `)` すら無い**）。**実ソースは 14。**
   - **受理される綴りの battery**（実ソースをパースして突合）: `g:7/b`・`f:/a`・`cis:m7`・
     `fis:m7b5`・引用形 `"G7#9"`／`"N.C."`・`@chord()`／裸・`c, m7` — **全部同一**。
   - ⚠️ ★★ **`Arguments[0].Text` では足りなかった**——**`@chord(c :m7)`（空白入り）は
     今日 Cm7 として通る＝引数 2 つ**。**全引数の Text を連結**しないと受理が狭まる。
     （14 site は全部 1 run なので**この綴りは未行使**＝コーパスからは見えない。）
   - ⚠️⚠️ ★★★ **食い違いは 1 系統だけ実在した＝括弧の中に書かれた `.`**。
     `MarkName` は Dot トークンを落とすので、**`@chord(b.es:7)` は B♭7・`@chord(.c:m7)` は
     Cm7 と鳴っていた**（点が黙って食われる）。run で読むと**その綴りは unknown**。
     **書いている本は 0 冊**（299 冊を木から書き戻して測った）。
...
~~1. ⚠️⚠️ **`@fig`＝ユーザー決定が要る**~~ — **第172第1便で移した**（**ユーザー決定つき**）。
   ★★★ **答は「小さなパーサ」ではなかった＝*トークンを読む***。引継ぎと §9.5.1 は
   「run の中を自分でパースし直す」前提で書かれていたが、**それは lexer の 2 つ目の綴り**
   （§5.2.1②）になる。**引数ノードは run と値しか持たないが、トークンは mark ノードの子として
   最初からそこに在る**——`MarkName` が `.` で繋いでいたのがまさにそれ。
   ⇒ **`MusicMarkSyntax.ArgumentTokens` を 1 軒だけ立て**（`Arguments` もそこから作る）、
   `ParseFigures` が**トークンを読む**。**文字列の往復は消え、lexer のコピーも書いていない。**
   - ⚠️⚠️ ★★★ **「run で読めるか」を全数で測ったら、引継ぎの見積りが 2 つとも外れた**
     （§5.0「引き継がれた処方箋は着手前に診断を測る」の 3 例目）:
     ⑴ **食い違う綴りは §9.5.1 が名指した 2 つ（`#6`・`6s`）ではなく、一族まるごと**——
     **3,825 綴りの全数比較で run 版は 653 相違**。⑵ **原因は「空白が区切りではない」こと**
     ——**区切りはトークン境界**なので **`@fig(6#6)` ≡ `@fig(6 # 6)`・`@fig(6_)` ≡ `@fig(6 _)`**。
     ⑶ ★ **文字クラスで切り直す試作でも 34 綴りで食い違った**——**`6s6` は切れて `6S0` は
     切れない**（小文字 `s` は**音高トークン**・大文字 `S` は識別子）。**この 34 が
     「run から復元するのは lexer の写しだ」の証拠。**
   - ★★ **トークンで読む版は 3,825 中 3,326 一致・相違 499 で、その 499 は全部
     「括弧の中に書かれた `.`」**。方向も**全部「今日は受理→新しくは unknown」**
     （**逆向き 0・両方受理で図が違う 0**）。⇒ **`@chord(b.es:7)` と同じ 1 系統**。
   - ⇒ **ユーザー決定＝落とす**（第170 の `@chord` と同じ判断）。落ちるのは
     **`@fig(6.4)`（今日 6/4 の 2 段）・`@fig(6.s)`（6♯）・`@fig(.6)`・`@fig(6.)`**。
     **書いている本は 0 冊**（ディスク上の .lys **308 冊全数**で `@fig(` は **13 site・4 綴り**＝
     `6`／`7`／`5 3`／`6 4` のみ。**点つきの `@fig.` は 0**）。網
     `FiguredBassTests.ADotWrittenInsideTheParentheses_…`。
   - ★ **MusicXML 取り込みが書き出す綴りは全部不動**（`LysWriter.FigAnnotation` は
     `@fig(6 s)`／`@fig(#)`／`@fig(_)` の空白形）。**この便で初めて、その往復に網が付いた。**
   - ★ **同じ島の 2 つ目の綴りも 1 つ消えた**: **MusicXML exporter が `StartsWith("fig.")` で
     独自 gate を持っていた**（`ProcessDirectionName` の default 腕）。**今は
...
4. **`MarkName` 自体をいつ消すか**——**第172第1便でこの状態に到達した**:
   **`MarkName` はもう引数を 1 つも運んでいない**（残る 6 site は点つき*名前*が 5・表示用が 1）。
   ⇒ **残る作業は 2 つで、どちらも「引数」の話ではない**:
   - ⒜ **名前を変える**（`MarkName` → 点つき名前であることが分かる名前）。
     ⚠️ **シンボルのリネームは RULES §5.1 の手順**（`.cs` 以外も横断 grep → 旧名で再 grep → 全緑）。
     **`MarkName` は `.md`・テスト名・remark に多数出るので、数えてから始めること。**
   - ⒝ **点つき名前の家族そのもの**（`@ds.al.fine`・`@ottava.bassa`）を語彙に載せるか。
     ⚠️ **これは「引数に型を付ける」ではなく「名前に構造を認める」話**なので、
     ⒯ の残件ではなく別項。**着手前にユーザー決定が要るかを判断すること。**

### 9.6 ⒯ の外で見つかった欠陥＝**数字なしの点**（第170第1便・`45f0b298`）

**`@name(引数)` の島を測っている最中にユーザーが起票した**（「`c4 g.` はエラーにすべきでは？」）。
- **LP は syntax error**（2.26.0 実測: `\new Staff { c'4 g'. a'4 }` →
  `syntax error, unexpected '.'`・**ファイルごと拒否**）。文法は `steno_duration: UNSIGNED dots`。
- **Lily# はどちらでもなかった＝黙って点を捨てていた**。点はどの規則にも届かず、
  music-item ループの skip 回復が**木から落として**いた ⇒ **`c4 g. a4` は `c4 ga4` と書き戻る**
  （＝**隣とくっつく**ので、第168第1便の点落ちより悪い）。
- ⇒ **LYS0023**（`UnclaimedDot`）。**点は木に残す**（round-trip 網＋陽性対照
  `AStrayTokenWithNoRule_IsStillDropped`＝規則の無い `?` は今も落ちる、という言明）。
- ⚠️ ★★ **原因は 1 つではなく 2 つだった**（第2便の測定・第3便で文言を直した）——
  **1,443 冊の .lys 全数で LYS0023 を数えると 23 冊**、**その全部が
  「注釈の旧・点つき綴り」**（`@finger.3`・`@chord.C`・`@mark.A`）で、
  **音価の書き損じは 1 冊も無かった**（コーパス 0・フィクスチャ 0・全部 `scratch/` と `output/`。
  うち 12 冊は元から別のエラーで落ちていた）。
  ⇒ ★ **「診断の文言は、原因を数えてから書く」**——最初の文言は音価しか名指しておらず、
  **読者の全部を間違った方向へ送るところだった。**
- ⚠️ **`@finger.3` などの点つき綴りは、この便より前から受理されていない**
  （点が落ちて `3` が LYS0016 になっていた）。**この便が変えたのは「黙って」の部分だけ。**
