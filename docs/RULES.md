...
#### ⚠️ 自動で止まるものが無い（2026-08-21 読解・未実測）

⚠️ **これは見落としではなく、理由のある判断**——`PreviewUpdateBench` 自身がこう書いている:
...
| `.github/workflows/ci.yml` | build / test / coverage のみ。**ベンチのジョブが無い** |

⇒ **忠実度の回帰は merge できず、10 倍の速度回帰は黙って通る。** 優先順位の正確な裏返しであり、
**忠実度側だけが「思い出さなくても止まる」形を持っている**のが非対称の正体。

#### 台帳に載せるべき数字（`PreviewUpdateBench.cs` のコメントに既に在る）

```
render        6051.2 -> 11.8 ms     (500x)
keystroke     4189   -> 162.2 ms    (26x)
100 bars keystroke        13.3 ms
この書籍の keystroke 床    258.4 ms   ← ⚠️ 最悪ケース
```

⚠️ **床 258.4 ms は、打鍵中に引っかかりを感じる領域**（体感の目安は 100 ms 前後）。
「リアルタイムプレビュー」を売りにする以上、**最悪ケースの床がどこにあるかが商品の質**。
守られていない以上、**今どこにあるかも誰も知らない**。ここが最初に見る場所。

#### 設計（未実装・ビルドできるセッションで）

**細かい A/B は今のまま手動でよい。** 足すのは**桁でしか落ちない門**ひとつ:

- 上限は実測値の **5〜10 倍**に置く（11.8 ms が 600 ms に戻ったら落ちる。
  13 ms が 18 ms になっても落ちない）。**ランナーのぶれでは絶対に落ちない幅**にすること——
  %単位の予算は共有ランナーで flaky になり、「無視される赤」に堕ちる。**それは無いより悪い**、
  というのが上の「NOT A TEST」の判断の正しい部分で、それはここでも生きている
- **下振れでは落とさない。** 忠実度台帳は改善でも落ちるが、あれは「乖離を意図的に記録させる」ため。
  速くなったことを記録させる価値は無いので、ここは片側でよい
- 対象は**最悪ケースの床**（下の 258.4 ms）。平均ではなく床が商品の質
