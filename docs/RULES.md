...
- ★★★ ⚠️⚠️ **検出器の*射程*は、その主張の一部。「every X」と名乗る網が母集団の一部しか
  ...
  **木の git 管理下 566 冊のうち 300 冊**しか読んでいなかった。**その外**（`audit\lpreg` 257 ＋
  ...
  ⇒ ★★ **判定法は引き算 1 回**: **その網が読む冊数を印字させ、`git ls-files '*.lys' | wc -l`
  と引き算する。**差がゼロでないなら、**主張はその差のぶんだけ小さい。**
  ⇒ ★ **広げたら床を上げる**（`books > 250` → `> 500`）。**上げないと、コーパスのパスが動いたとき
  射程が*黙って縮む***——§5.4 の空集合の罠が、母集団の側に出た顔。
