...
- ★★★ **ベンチ本は「自分が何を描いているか」を測ってから信じる。時間が超線形なのは
  ...
  **`c'`→C5・`d'`→D6・`e'`→E7 と 1 音ごとに上がっていく**のがその場で見える。
  ...
  ⚠️⚠️ ★★★ **ただし判定法 ⑶ の計器そのものが、多 part の本では*描かれる音楽と違う答え*を返す**
  （2026-08-16・第182セッション実測・**未修理／§2F に起票**）。
  **`--pitches` は RenderSpec 無しの素の `collector.Collect(tree)`** を回すので、
  **section 内の 2 つめの part block が 1 つめの相対鎖を引き継ぐ**——`check` 自身の
  validator が使う `SemanticValidation.TryCollect`（RenderSpec つき）とは別物。
  **実測 2 冊**: `test/multi-movement` と `test/grandstaff-high-bass` はどちらも
  **`--pitches` が低音部の頭を C6 と言う**が、**双子は `rh = \relative c'` /
  `lh = \relative c`＝各 part が自分の clef のアンカーを持つ**（＝正しいのは描画側）。
  ⇒ ★★ **単一 part の合成本ではこの計器は今までどおり効く**（漂流はその 1 本の鎖の話）。
  ⇒ ★★★ **多 part の本で `--pitches` が食い違いを見せたら、まず `lysc ly` の
  `\relative` アンカーを読む**。**計器のほうが間違っている側の例が既に 2 冊ある。**
