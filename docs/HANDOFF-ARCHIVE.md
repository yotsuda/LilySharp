...
> （完了したロードマップ）を **逐語のまま** ここへ落とした。原則・学びは汎化して
> `HANDOFF.md` §5 に、未了の作業は §2 に、決定は §3 に残してある。
>
> **読むのはこの 2 つの場合だけ**:
> - **同じ regime にもう一度触るとき** — その欠陥をどう測ったか、対をどう設計したか、
>   第2欠陥が何だったか。同じ罠を踏まないために。
> - **`HANDOFF.md` の記述を疑うとき** — ただし最終的な裏取りは常に実コード（`HANDOFF.md` §0）。
>
> ⚠️ **ここへ追記しない。** 引継ぎは `HANDOFF.md` §1 の書き換えで行う。
> ⚠️ **ここの「現在地」「次の一手」「未 push 数」は全て過去の時点のもの＝ stale。**
> 生きている次の一手は `HANDOFF.md` §1 にしか無い。
> ⚠️ 見出し番号（§1/§2/§3）は当時のまま。コードが参照する §4〜§8 は `HANDOFF.md` 側にある。
...
# 第132セッション以前の経緯（2026-08-11 に `HANDOFF.md` §1 から降ろした）

> 降ろした理由は分量ではなく**手順が止まっていたこと**——`HANDOFF.md` は最初から
> 「経緯は ARCHIVE へ」と書いていたのに、実行されたのは 2026-07-24 の 1 回きりで、
> **92 便ぶんが §1 に積まれて全体の 86% を占めていた**。以後は §7 の終了時
> チェックリストが毎回 1 便ぶんを落とす。

---

## 以下は第132セッションの経緯

最終更新 第132セッション＝**和音の指番号（§1 ⒜）を、点 6 つを先に起票してから移植した**。
コミット 872b366c（点）/ 984be4de（配置の移植）/ 2adeafda（散文の訂正）/ 48f3765d（予約の点と移植）。
**未 push 35 件**・**push はしていない**（ユーザー判断待ち）。
**テスト 4340 passed / 0 failed / 4 skipped**（開始時 4334＝引継ぎと一致・**+6 は新しい点**）。Core 0 warning。
**台帳 489 点**（+6）・**ss 非ゼロ 87**・**総和 3.613006474**（開始時 3.612874756）。**snapshot 1 枚**。

★★★ **6 点とも、移植の*前に*書いた着地条件どおりに着地した**——**0 が 4 つ・−0.000065859 が 2 つ**。
外側 2 点が 0 にならないことは点の `why` に予め書いてある（**0 になったら「積み上げを桁の高さから作っていない」証拠**）。
道具＝`audit/lp-geometry/probes/chord-fingering.ly`（**CFL＝低い和音・CFH＝4 度上**。
LP 側で**上下の決定者が入れ替わる**対にしてある）。

★★★ **予測を 1 つ外し、その falsifier が機構を名指した**。「staff は staff-padding の
clamp（**offset** に効く）でしか効かない」という模型から CFL の下側を **−2.045**（頭の鎖）と
予測したが、名指した falsifier **−2.550** が鳴った——そして**その falsifier 自身が
「なら CFL と CFH の下側は*同じ数*になる」と書いてあり、実際 15 桁一致**（頭の鎖は 2.0 違うのに）。
⇒ **staff は support skyline に入る**（`side-position-interface.cc:325-331 include_staff`）ので、
**digit の*インク*が staff のインクを padding 0.5 で越える**。clamp は効いていない。
**「offset を clamp する」と書いた移植はどちらの本も再現しない。**

★★★ **道具の穴を先に塞いだ**: `lysc ly` が**和音メンバの `@finger` を*無言で*落としていた**
（第96 の音符レベルの穴の 1 つ下。`if` から外れる枝には警告が無かった）。
`<g@finger(1) b@finger(3) d'@finger(5)>1` が `<g b d'>1` になる＝**コンパイルは通る別の音楽**で、
**Fingering grob の無い和音を測るところだった**。exporter は**メンバの post-event を書き**、
**認識できないメンバ節点は全て警告する**ようにした。

**移植で変わった項は 3 つ**（どれも点が後ろに居る）: ⑴ staff は support（上）⑵ support は
**和音の最上/最下の頭**であって各digit自身の頭ではない ⑶ **頭の半インクは 0.545**（`NoteheadHalfHeight`
の但し書きが「点が無いから直せない」と名指していた site）。**snapshot は `test/ossia-beams` の
y 属性 1 つだけ**（＝⑶ の 0.045 × ossia 縮尺）。

⚠️⚠️ **コーパス本が*まだ*負っている 0.190000 は測ってある**（`scratch/chordfinger/cr-probe.ly` の 3 本）:
`chord-repetition` の和音は LP で **2.740000**、同じ和音から**梁・スラー・強弱を外すと 2.550000**、
**梁だけ戻すと 2.740000**。梁の Y-extent は `(1.76 . 2.24)` なので **2.24 + 0.5 = 2.74**
⇒ 残りは **`add-stem-support #only-if-beamed`**（`define-grobs.scm:1543`・
`new-fingering-engraver.cc:186-190`）**だけ**。スラーも強弱も 1 mm も動かさない。
⇒ **この島の次の点は「梁の付いた和音」で、LP 側の数はもう手元にある。**

★★★ **第2便＝予約も入れた**（`48f3765d`。**ユーザーがレポートを見て 2 件目を目視で起票**——
「`1` と `p` が重なる。LP では重ならない」）。§5.0 の「配置と予約は 1 つの claim」そのもので、
**第1便が自分の commit message で名指していた穴**。**点 3 つ（DYF/DYN/DYU）を先に起票**:
LP は**下向き digit が居ると `\p` を 1.495999 下げ、上向き digit では 1 mm も動かさない**
（DYU は DYN と 15 桁一致）。**間隔は digit のインク下端から 0.460023＝outside-staff-padding**
（side-position の 0.5 ではない＝**pass だと名指せる**）。
移植＝`SkylineBuilder.AddFingeringsToSkyline`（**inside-staff skyline に入れる**。
Fingering は outside-staff-priority を持たない＝**動かされない側＝profile に居る側**）。
⇒ **1.496021780 → 0.004580035**・**対照 2 点は不動**（+0.000023272 のまま）・**snapshot 0 枚**。
★ **残差 0.004580035 は既知の項の和**: 光学サイズの 0.004556763 ＋ ラベルの 0.000023272（7 桁一致）
⇒ **この点は光学サイズ欠陥の 2 つ目の観測者**（配置側の 200 倍の大きさで見える）。

★ **副産物＝フォントの光学サイズ**: Lily# の fetaText digit は **design-20 の箱を拡縮**しているが、
LP は**要求サイズの光学面を選ぶ**（font-size −5 ⇒ `20 × 2^(−5/6) = 11.2246` ⇒ emmentaler-11）。
LP の実測 3 つを `2^(−5/6)` で割ると **2.024 / 2.000 / 2.004**＝**Lily# 自身の生成表の
design-11.22 行そのもの**。**≤0.0046**・figured bass も同じ経路なので**起票のみ**（点の `why` に算術つき）。

★★ **第3便＝ユーザーの 3 件目の目視を「点だけ」起票した**（移植は未着手）。
**`chord-repetition` の 2 つ目のスラーが最後の staccatissimo を突き抜ける**。
新プローブ `probes/slur-script.ly`（**SSC＝script あり / SSN＝対照**・最小の対）:
**LP は script が居ると bow の attachment をちょうど 1.000000 下げる**（−4.045 → −5.045）。
**Lily# は +1.000000000（対照は 0.000000000）**＝**規則がまるごと無い**。
⚠️ **1.0 は「script のインク＋padding」ではない**（script のインク下端 −4.797 に対し 0.248 空く）
＝**scorer の変分ステップ 1 ss**。⇒ **定数で埋めると この本だけ合って他が全部外れる**。
機構: **staccatissimo は `avoid-slur . inside`**（`scm/script.scm:386-389`）＝**script は動かず bow が避ける**。
Lily# は**逆向きだけ**移植済（`outside_slur_callback`＝script が bow に乗る）で、
`SlurScoringProblem` の extra-encompass は**点と連符数字だけ・script を見ていない**。
⚠️ **移植は「式」ではなく「順序」の変更**: `'inside` の script は bow に依存しないので bow より先に
置けるが、**Lily# は 2 か所ともスラー→script の順**（`LayoutEngine:901` 対 `:3007`・
MultiStaffLayouter の skyline pass も同順）。

**次の一手**:
▶ **⒜ スラーが script を避ける**（上の +1.000000）。**点は起票済**＝残りは順序の移植。
▶ **⒝ 梁を指番号の support にする**（上の 0.190000）。**点が先**＝梁付き和音の本を CFL/CFH に足す。
  **同じ本に残るもう 1 件＝`avoid-slur #'around`**（`chord-repetition` の下向き digit は今もスラーに触る。
  ⚠️ ただし**この本のスラーは LP 側の digit を 1 mm も動かさない**（scratch cr-probe.ly で実測）ので、
  **目視は起票であって測定ではない**——点は別の texture で作ること）。
▶ **⒝ 行末 courtesy 群を `BreakAlignSpacing` に通す**（§2H ⑷・第131 から据え置き。**定数で埋めない**）。
▶ ⒞ **fetaText の光学サイズ選択**（上）。⒟ **指番号の予約**（上）。

---

## 以下は第131セッションの経緯

最終更新 第131セッション＝**LP と Lily# を横に並べる目視レポートを作り、ユーザーが目で見つけた欠陥を 4 件直した**。
コミット `37a8fd97` / `874f229b` / `b93939c3`。**未 push 25 件**・**push はしていない**（ユーザー判断待ち）。
**テスト 4334 passed / 0 failed / 4 skipped**。

★★★ **道具＝`audit/lp-regression/lp-vs-lilysharp.html`**（生成器は `scratch/lpreport/*.ps1`・**未追跡**）。
LP 回帰コーパスの **82 冊**を LP と 1 枚の HTML に並べる。紙を揃え（LP 側に
`left-margin/right-margin 15mm・indent 0・tagline ##f` を被せる）、横は両者インクの和集合・
縦は**五線の最上線**で錨を打つので、同じ画素が同じ音楽を指す。重ね合わせ表示・Δ幅/Δ高・
五線の本数・訳注・両ソースつき。
⚠️ **82 冊は「.lys が残っている本」**であって、plain の exact/fixed/open は **133 冊**ある。
**残り 51 冊は双子が scratch にしか無く消えている**＝**過去の判定を目視で再検分できない**。
⚠️ 生成器の罠を 2 つ踏んだ（どちらも実際にレポートを嘘にした）: ⑴ LP の複数ページを
`"<名前>-*.svg"` で拾うと **`auto-beam-partial` が `auto-beam-partial-grace.svg` を掴む**
（被害 4 冊）。**数字だけ**受けること。⑵ SVG 内の `<style>` を丸ごと剥がすと
`.music` クラスへ音符フォントを当てる規則まで落ちる（音符が豆腐になる）。

★★★ **直した 4 件**（すべて LP 原典＋陽性対照つき実測。数字と分解は commit message に）:
⑴ **休符は梁の被覆 grob ではない** — `scm/define-grobs.scm:496-504 collision-interfaces` に
   `rest-interface` が無い。別声部の休符・spacer が梁を **1.810 ss 持ち上げていた**。
⑵ **最終小節に終止線を自動で足さない** — 3 か所が "music convention" とだけ書いて stamp していた。
   LP は完全な最終小節を細線で終え、不完全なら何も引かない。**ほぼ全冊が右端 0.9 ss 広かった**。
   併せて**末尾 clef が「書かれた小節線」を上書きして消していた**件（`|.` も `||` も）。
⑶ **tuplet 括弧は連桁が「同じ両端」のときだけ隠す** — `bracket-visibility = if-no-beam` は
   **define-grobs.scm に存在しない既定**で、走る規則は `tuplet-bracket.cc:100-115` の
   `!(par_beam && equally_long)`。位置は follow_beam 枝（`:495-519` ＋ `:633-637`）、
   数字の 2 つ目の綴りを撤去、`shorten-pair` も移植。
⑷ **梁の走りの端も beamlet** — LP は**辺ごと**に「符尾から遠い側か」を訊く（`beam.cc:589-624`）。

★★★ **この 4 件が共通して教えたこと（次に読む人へ）**:
- ⚠️⚠️ **検査器とテストが誤った規則を固定していた（3 例）**。台帳の `beam.grouping` 抽出器は
  **括弧の端フック（縦・太さ 0.16）を符尾と数え**、`TwoTupletsUnderOneBeam` は
  「連桁された tuplet は括弧を描かない」と主張し、`BeamedTupletNumber_…BeamEdgePlusPadding` は
  LP の量子化を欠いた式を主張していた。**3 つとも、自分が既に持っている LP 双子が反例だった。**
  ⇒ **落ちたテストの主張を、直す前に双子に訊く。**
- ⚠️ **踏む本が無い欠陥は緑のまま残る**。⑷ は **4333 テスト・218 snapshot が全部緑のまま**在り、
  修正しても **snapshot は 1 枚も動かない**。⇒ **観測者は形ではなく実測値を主張させる。**
- ★ **snapshot は承認前に種類別に数える**（今回 218 枚: 太線 307 本の消滅・括弧線だけ・番号の y だけ、
  と全部言えるまで当てなかった）。その過程で **`lyrics-volta` の volta 番号が旧 snapshot に
  1 つも無かった**＝**改行で分断された volta が番号を失う既存欠陥**を掘り当てた（未修正）。

**次の一手（★ ユーザー指示で起票・次セッションの本命が 2 件）**:
▶ **⒜ 和音の指番号を五線の外へ振り分けて積む**（2026-08-10・ユーザーが目視で起票）。
  **`chord-repetition` の最初の和音 `<c-1 e-3 g-5>` で、3 つの数字が符頭と符尾に重なって読めない。**
  実測（同じ紙・LP 五線 10.178..14.178／Lily# 五線 9.69..13.69）:

  | | LP x | LP y | Lily# x | Lily# y |
  |---|---|---|---|---|
  | 5（g） | 17.380 | **7.815**（五線の上・外） | 17.326 | **12.69**（符頭と同じ） |
  | 3（e） | 17.380 | **9.438**（五線の上・外） | 17.326 | **13.69**（符頭と同じ） |
  | 1（c） | 17.363 | **17.834**（五線の下・外） | 17.326 | **14.69**（符頭と同じ） |

  （和音の符頭は LP x=17.121 / Lily# x=17.126、符尾は LP 18.36 / Lily# 18.356。）
  ⇒ **Lily# は各指番号を自分の符頭の真横 +0.2 に置いているだけ**で、符頭にも符尾にも被る。
  **LP は `fingeringOrientations`（Voice の既定 `#'(up down)`）で上下に振り分け、五線の外へ積む**
  ——実測どおり**上の 2 音が上・最低音が下**。⇒ **定数のずれではなく、振り分けと積み上げの規則が
  丸ごと無い**。移植先は `lily/new-fingering-engraver.cc` と `fingeringOrientations`。
  ⚠️ **台帳に和音の指番号を測る点は無い**。§5.0 のとおり点が先。
  ⚠️ 混同しないこと: `chord-repetition-script-stack` の訳注が言う「**和音レベル**の指番号
  `-1-2-3` は Lily# で書けない」は**綴りの穴**で別件。こちらは**書けている指番号の置き場所**。
▶ **⒝ 行末 courtesy 群を `BreakAlignSpacing` に通す**（§2H ⑷ に実測を追記した）。
  **定数で埋めないこと**——§2H ⑷ の 4 件は同じ「paper column モデルの欠落」なので、
  1 件だけ定数化すると**同じ量の 2 つ目の綴り**を作る（今セッションで 2 回剥がした型）。
  着手順は §5.0 のとおり **点が先**: 台帳に「courtesy 拍子の*右側*」の点が無い。

**そのほか今セッションで起票だけしたもの**:
⑴ **不完全な最終小節に Lily# は細線を引く**（LP は何も引かない）。「線を引くか」という別規則。
⑵ **改行で分断された volta が番号を失う**（`test/lyrics-volta` が 2 段のとき）。
⑶ **51 冊の双子が残っていない**（上）。目視再検分の穴。
⑷ レポート HTML が**未追跡**。生成器を `audit/` へ入れるか、HTML を追跡するかの判断が要る。

probe 残置（第131）: `scratch/beamskip/` に `lp-ctl.ly`（梁と休符の陽性対照 3 score）・
`lp-bar.ly`（終止線 4 score）・`lp-tuplet.ly`（括弧の陽性対照 3 score）・
`lp-courtesy.ly` と `courtesy.lys`（**行末 courtesy の対＝次の一手の実測に使った**）・
`bars2.ps1`（書かれた小節線 6 綴りの一覧）・`classify-*.ps1`（snapshot の種類別分類）。
`scratch/lpreg/clef-change-at-end-nofinal.ly`（`\bar "|."` を外した対）。
`scratch/lpreport/` にレポート生成器一式（`render.ps1` / `render-lp.ps1` / `build-report.ps1` /
`rerender-ls.ps1` / `fix-manifest.ps1`）。
...
**第2便 `part-combine-relative`＝コード変更ゼロで LP 全一致（exact）**。
**第3便＝残り 1 冊 `silence-mixed` の LP 側を測り切り、枠が在ることを確かめた**（下の ⑴・**双子は未着手**）。
**第4便＝自己監査（三問）＝違反 2 訂正・挙動変更ゼロ**。
**第5便＝プレビュー経路（毎打鍵）の A/B ＝実退行なし。round 30 が測っていなかった経路で、
そこにこの便の唯一の費用が在った**（道具を新設・下）。
**未 push 16 件**（`git rev-list --count origin/master..master`）・**push はしていない**（ユーザー判断待ち）。

★★★ **本の主張はコード変更ゼロで成立していた**。`part-combine-tuplet-end` の texidoc は
「三連符の終了イベントは*開始した*文脈へ送られるので、切替の後でも三連符は正しく閉じる」。
この本の切替＝**第1小節は part1 単独（Solo・part2 は `R1`）／第2小節は両者 `g1`（a2）**で、
**2 つ目の三連符はちょうどその境界で終わる**。LP 実測（`pctend.ly`＋`pcdump.ily` に
`TupletBracket`/`TupletNumber`/`Beam` の記録を追加）＝**三連符 2 つ・どちらも閉じる**。
**ブラケットは 2 つとも x extent が空**（beamed＝`bracket-visibility` if-no-beam）で番号だけ、
`TUPNUM "3"` が **17.830355 / 25.342955 @ y+3.34**、beam 2 本、**第2小節へ届く破片ゼロ**。
Lily# も同数・同座標（17.83/25.34 @+3.34）。⇒ **この本の claim は測る前から通っていた。**

★★★ **本当の欠陥＝左頭精算の voice 境界**（`ee0a…` この便）。修理前は
**休符→第1音以降の全列が一様に +0.30**（休符 8.59 は一致・列間隔 2.50/4.79 も一致＝**ずれは 1 対だけ**で、
残りはその剛体並進）。在処: `note-spacing.cc:77` の
`ideal = base − increment + left_head_end` は **`spacing-spanner.cc:350-393` が
「左列の wish の `right-items` に右列が在る」ときだけ**走り、その `right-items` を埋める
`last_spacing_` は **`Note_spacing_engraver` のインスタンス変数**（`:75-78`）で、
この engraver は **Voice**（`engraver-init.ly:419`）。⇒ **鎖は Voice 文脈ごとで、
その文脈が黙っている間も持ち越される。**
⚠️⚠️ **推定で書かなかったのが効いた**——同じ本の中に **精算が落ちる対と落ちない対が両方在り**、
「文脈が違えば境界」と書いていたら**今 exact な対を壊していた**。LP に wish の両端を吐かせて解決:
**shared の休符列 `8.585` の wish の right=(31.403, 29.313)**＝shared が**次に engrave する音**と小節線で、
**`14.087` は入っていない** ⇒ この対だけ base のばね＝**LP 5.501955 対 合体器なし対照 5.801955＝差 1.5−1.2**
...
**Lily# の既存 doc の 2 つ目の ⚠️（「小節線は境界ではない・MEASURED」）と完全に整合。**

★★★ **修理＝述語を「cue 専用」から LP 自身の条件へ**。`SpacingRules.CrossesVoiceBoundary` は
`spacing-spanner.cc:352-358` から移植済だったが、**境界の材料が `IsCue` だけ**で、
**doc 自身が「cue が Lily# で唯一書ける逐次的な voice 変更だから」と書いていた**——
**第126 で `combinedStaff` が入った時点でその札は stale**（今セッションの「stale な札」2 例目）。
新: `MusicItem.VoiceContext`（合体器が One/Two/Shared/Solo を焼く）＋
**「両列の文脈集合が交わるか」**＝LP の「どれかの文脈が両列を占めるか」そのもの。
**cue の既存の答えは 1 つも変わらない**（main→cue・cue→main は互いに素／同時に食い違う声部は
**交わるので精算する＝LP と同じ理由に置き換わった**＝旧「諦めて精算する」特例が*消えた*）。
⚠️ 小節線・改変列・spacer は**文脈を出さない**ので空集合＝境界にならない（測定済の VBB-CUE と一致）。

★★★ **同じ規則が 2 冊目に届いた＝`part-combine-silence` の X 残差が閉じた**。第127 は
「LS の列は LP の `\voiceOne`/`\voiceTwo` 対照と 0.005 以内で一致し、**LP 自身の `\partCombine` 出力だけが
±0.10〜0.20 ずれる**（col3 +0.20・col4-6 −0.10）」と**事実として**記録していたが、**その全部がこの欠陥**だった。
**stash 対照で実測**: 修理前 **8.59/10.79/12.99/18.09/20.29/24.57** 対 LP
**8.585/10.785/13.185/17.985/20.185/24.475** → **修理後は 6 列とも 0.005 以内**。
⇒ **`part-combine-silence` は open のまま**（残るのは score2 bar1 の**重なり**＝§2A の島。X ではない）。

★★ **第2便 = `part-combine-relative` を exact で閉じた（コード変更ゼロ）**。LP は
**3 小節とも完全に同一**（pos −4/−6 → −3/−5・列 8.585/12.860 で以降 9.891 ごと・
`CombineTextScript` レコードは 1 つも無い＝両パートは apart）。Lily# も **−3.0/−2.0 → −2.5/−1.5 ss**・
列 **8.59/12.86/18.48/22.75** ＝ **LP 全一致**・ラベル 0。
⚠️ **枠外＝本の第2小節**。LP の `\relative` は音楽を**囲む**ので内と外が在り、主張は
「合体器は*外*を無視する」。**Lily# の `octave` は流れの切替**（部は既定で relative・
`octave absolute` が opt-out）なので**「無視されるべき外」が存在しない**。書けるのは残り半分＝
「絶対綴りと相対綴りが同じ小節を印字する」で、それは**読者が頁で見るもの**そのもの
（枠外込みで exact は `whole-note-tremolo-accidentals` と同じ扱い）。
⚠️⚠️ **この本は陽性対照が要る**——`e f` は**両モードで同じ 2 音**（だから 3 小節が同一になる）ので、
**`octave relative` が何もしない実装でも双子は通る**。対照 `pcrel-ctl-probe.lys`＝
**D4 の後の相対 `a` は A3（−4.0 ss）・絶対 `a` は A4（−0.5 ss）** ⇒ 実測 −4.00 で切替は効いている。
★ **対照は 2 回外した**（`b` after F4・`c` after F4 はどちらも両モードで一致）——
**規則は「4 度以内の step では両モードが一致する」**。
⚠️ ★ **第1便の枠検分メモは誤りだったので訂正した**: `pcrel-frame.lys` の「頭 2 つ・data-pos 同一＝
読めていない」は**読み違い**で、0.5/1.5 ss は **E5/C5 の 2 声部＝正しい出力**だった
（`octave relative` は Lily# の**既定**なので、そもそも「置けるか」は問いではなかった）。

★★★ **第4便 = 自己監査（三問「字面移植できたか／変なハックは／REF は付けたか」）＝違反 2 訂正・白 3**
（**挙動変更ゼロ**——監査後に `pctend-probe` を再レンダして**全 8 座標が監査前とバイト一致**）。
⚠️⚠️ **違反 ⑴ = 挿入位置で既存の doc が関数から剥がれた（2 か所・これがいちばん重い）**。
`InContext` を `WithVoiceDirection` の doc と本体の**間**に入れたので、
**`\voiceOne`/`\voiceTwo` の REF（`make-directed-part-combine-music`）が
「文脈を焼く関数」の上に付いた**＝**引用が別のコードを指す**状態（§7.6 がいちばん警戒する形）。
同じことを `SpacingRules` でもやっていて、**`CrossesVoiceBoundary` の測定記録一式（VB-VOICE・VBB-CUE・
1/14 の話まで）が `ContextOf` の上に移り、当の述語は doc ゼロ**になっていた。
⇒ 両方とも**新しい関数を doc ブロックの前へ出して**復元。**ビルドは 0 warning のまま通るので、
コンパイラは何も言わない**——読むしかない型。
⚠️ **違反 ⑵ = LP の住所を印無しで散文に書いた（2 か所）**。`note-spacing-engraver.cc:82-91` と
`:115-120` を括弧書きで置いていた＝**ラチェットは `LILYPOND-REF` を含む行しか見ない**ので
**永久に検査されない散文**（[[reference_lilysharp_ref_marker_makes_a_citation]] の型）。
⇒ `:88-91 acknowledge_rhythmic_grob` と `:115-120 stop_translation_timestep` として**印を付けた**。
★ **新規 REF 5 本を `(Get-Content f)[n-1]` で実照合した**（**commit 前にやっていなかった**）:
`engraver-init.ly:419`＝`\consists Note_spacing_engraver`・`note-spacing-engraver.cc:75-78`＝
`if (last_spacing_)` の `right-items` 追加・`spacing-spanner.cc:350-393`＝
`extract_grob_set(wish,"right-items")` から `merge_springs` まで——**5 本とも住所は正しかった**。
**白 ⑴ 新規チューニング定数ゼロ**（`unsourced engraving defaults: 0`）。
**白 ⑵ `HashContent` が反射＋除外リストで、除外は `SourcePosition` だけ**＝**実読して確認**（
`MeasureContentKey.cs:333-336`）。⇒ **`VoiceContext` は自動でアイテムハッシュに入る**＝
perf 台本と §1 に書いた主張は**推定でなく事実**。
**白 ⑶ 変なハックは 1 つも出なかった**が、**「字面移植」ではない箇所を 1 つ明示した**——
mask 規則は LP の条件の**導出**（Lily# に wish オブジェクトが無い）で、**成立には「対が隣接」という
前提が要る**。コードにその前提を書き足した（**書いていなかった＝主張が半分だった**）。
⚠️ 併せて mask のビット幅（`VoiceContextId` は 32 を超えられない・現在 6）を注記。

★★★ **第5便 = プレビュー経路の A/B（ユーザーの問い「プレビューの更新速度は？」）＝実退行なし**。
⚠️⚠️ **round 30 はこれを測っていなかった**——`lysc svg` の端から端なので、**毎打鍵の経路
（`IncrementalCompiler.RenderIncremental`）を分離していない**（第128 round 28 から続く同じ限界）。
**しかも `VoiceContext` の唯一の届き先がそこ**（`MeasureContentKey` は反射ハッシュで、
`IncrementalCompiler.Compile` が毎回呼ぶ）。⇒ **測っていない所に唯一の費用が在った。**
★ **道具を作った**: `LilySharp.Tests\PreviewUpdateBench.cs`（**テストではない**——
`LILYSHARP_PREVIEW_BENCH` に出力先を渡したときだけ走る。**base worktree にコピーして両方で走らせる**）。
測るのは ⑴ **打鍵 1 回 = `RenderIncremental`（同じ木を再レンダ＝最大再利用）** ⑵ **`MeasureContentKey.Compute`
だけ**（100 回/ラウンドで内側平均）。**SVG のハッシュも一緒に出す**（絵が同じでなければ時間比較は無意味）。

| 測る対象 | base 床（3 回の最小） | curr 床 | hash |
|---|---|---|---|
| 打鍵 1 回・plain1k（1000 小節） | 191.9 ms | **188.4 ms** | 同一 |
| 打鍵 1 回・comb300 | 82.5 ms | **77.3 ms** | 同一 |
| content key のみ・plain1k | 6.789 ms | **6.570 ms** | — |
| content key のみ・comb300 | 0.986 ms | 1.111 ms（+12.7%・自帯 18〜41%） | — |

⇒ **4 行のうち 3 行で curr の床が base の床を下回り、測定値は完全に噛み合っている**＝**実退行なし**。
**content key は打鍵 1 回の約 3.5%** なので、仮に comb300 の +12.7% が本物でも**打鍵では +0.4%**。
⚠️⚠️ ★ **最初の 1 対を信じかけた**——curr 431ms 対 base 209ms（**2 倍**）と出たが、
**それはビルド直後の 1 サンプル目**（コールドスタート）で、反復すると消えた。
**「A/B の 1 サンプル目は捨てる」**（この機械の帯は §5.3 のとおり広い）。
⚠️ **開示**: ⑴ bench は `[Trait("Category","Bench")]` だが**アサートしない Theory 2 件として suite に数えられる**
（テスト数 4324 → **4326**）。⑵ 測ったのは**同じ木の再レンダ**＝最大再利用の打鍵で、
**実際に 1 小節変える打鍵は再レイアウトを足すので、この項の比率はさらに下がる**（薄まる方向）。
⑶ 合体器側の 2 つ（`InContext` の複製・`ContextMask`）は**素の楽譜のプレビューには届かない**。

**台帳: plain 322 = fixed 73・exact 46・skip 190・open 13**（§0 の数え方で確認）。
⇒ **`part-combine-tuplet-end` が skip → fixed**・**`part-combine-relative` が skip → exact**。
**テスト 4326 passed / 0 failed / 4 skipped**（開始時 4315＝引継ぎと一致・**+9 は新しい観測者・
+2 は第5便の bench**＝アサートしない）。Core 0 warning・**snapshot 0 枚**。
**perf round 30**（`perf-ab30.ps1`・base `a70a0fd8` worktree `C:\MyProj\LilySharp-perfbase-a70a` 残置・
両順 median-of-5＋床）: **plain1k / comb300 / combmmr の 3 冊とも hash MATCH**・中央値は
両順で符号反転＝**実退行なし**。**comb300 の床だけ両順とも小さく正（+0.4 / +0.6%）＝合体器側の実費**。
⚠️ **開示 ⑴** `InContext` は `WithVoiceDirection` が既に複製した item を**もう一度**複製する
（record の arm 3 綴りを避けたため・上の +0.5% の中身）。**⑵** cue 側は `IsCue` からの**導出**のままで
stamp していない＝**隣接 cue 領域 2 つは今も 1 文脈に見える**（LP は CueVoice 2 つ・コーパスに 1 冊も無い）。
**⑶** 3 冊 hash MATCH は「**この修理が触るのはどの声部も跨がない対だけ**」の測定であって、
**素の合体譜に届かないことの証明ではない**（届く形＝休符が shared/solo を切り替える本）。

**次の一手（候補・ユーザー判断）**:
⑴ **台帳の続き**——part-combine の plain は**残り 1 冊 = `silence-mixed`**。★ **LP 側は測り終えて在る**
   （`pcsm.ly`／`pcsm.log`）ので、次は**双子を書いて突き合わせるだけ**＝言語の判断は要らない。
   ★★ **枠は在る**: `combinedStaff` は**part の中の `voice { } { }` span を受け付ける**
   （`pcsm-frame2.lys`・warning ゼロで描画あり）。これが LP の**同一 Voice 内の同時音楽**の受け皿で、
   `Fixtures/test/beam-skip.lys` が同じ理由で既に使っている（第113 の本）。
   **LP が印字するもの（実測・staff1。staff2 は両パートを入れ替えた鏡で、同じ答えを返す＝本が自分自身の対照）**:

   | 小節 | part1 対 part2 | LP が印字するもの |
   |---|---|---|
   | 1 | `R1^"R"` 対 `r1_"r"` | 休符 **1 つ・dir=()・+1.0 ss**、ラベルは **"r" だけ**（**"R" は event ごと落ちる**） |
   | 2 | `s1^"s"` 対 `R1_"R"` | **MMR 1 つ・−3.0 ss**（＝**声部 two のまま＝shared に落ちない**）＋"R"、skip の "s" は出る |
   | 3 | `r1^"r"` 対 `s1_"s"` | 休符 **1 つ・dir=+1・+2.0 ss**（＝**声部 one のまま**）、"r"(上)と"s"(下) |
   | 4 | `<< R1 s1 s4 >>` 対 `<< s4 s1 R1 >>` | **MMR 1 つ・dir=()・0.0 ss**（＝**skip が無視され両者 R1＝unisilence で shared へ**） |
   | 5 | `<< r1 s2 s4 >>` 対 `<< s4 s2 r1 >>` | 休符 **1 つ・dir=()・+1.0 ss**（同上） |

   ⇒ **主張の前半（別種は shared に落ちない）は第2・第3小節**、**後半（同じパートに休符と skip が
   在れば skip は無視される）は第4・第5小節**。⚠️ **skip が絡む対だけが one/two に留まり、
   `R` 対 `r` は無設定の声部へ行く**——ここが読みどころ。
   ⚠️ ★ **最初の当たりで乖離が 1 つ見えている**: `pcsm-frame2.lys` の第1小節は
   **Lily# が "R" と "r" を両方印字する**が、**LP は落ちたイベントのラベルごと落とす**。
   ⇒ **「イベントが null へ行くとき、その上の text も一緒に行くか」**が最初の問い。
⑵ **cue を stamp 側へ**＝`VoiceContextId.Cue` を導出でなく焼く。**隣接 cue 領域が 2 文脈になる**
   （LP の CueVoice 2 つ）。⚠️ **踏む本がコーパスに 1 冊も無い**ので、先に本を作るかの判断が要る。
⑶ **combinedStaff / condensedStaff の snapshot 資産を作るか**（第128 の札・2 機能ぶん）。
   `pcmsh-{a,b,c}-probe.lys`・`pctend-probe.lys` は LP と 1 対 1 で照合済＝そのまま snapshot になる。
⑷ **段③ = `condensedStaff` の中の `combinedStaff`**（スロット方向の能動的強制）。
⑸ **向きの粒度**＝§2A の島。**`part-combine-silence` score 2 の残差はここだけになった**。
⑹ **`]~` のタイを黙って落とす件**（第126 の起票）——言語の判断が要る。

probe 残置（第129）: `pcrel.ly`（**逐語 3 小節**）／`pcrel-ctl.ly`（**枠を揃えた 2 小節**）／
`pcrel-probe.lys`（**双子**）・`pcrel-ctl-probe.lys`（**陽性対照＝切替が効いていることの観測**・
**捨てないこと**）・`pcsm.ly`／`pcsm.log`（**残り 1 冊の LP 側は測り終えて在る**・上の ⑴ の表の出所）・
`pcsm-frame.lys`・`pcsm-frame2.lys`（**枠の検分＝`combinedStaff` は voice span を受ける**）・
`pctend.ly`／`pctend-probe.lys`（**双子**）・`pctend-ctl.{ly,lys}`
（**合体器なし対照**＝差が 1 対だけであることの根拠）・`perf-ab30.ps1`・`update-ledger-129.ps1`。
⚠️ `pcdump.ily` に **`TupletBracket`/`TupletNumber`/`Beam` の記録**と、
**`REST` の `headend=`**（＝`left_head_end`）、**`NoteSpacing` の `WISH left=… right=…`**、
**`MultiMeasureRestText`（`MMTXT`）**、**`BarLine`（`BAR`）** を追加した。
★ **`WISH` が第1便の鍵**——「どの対に wish が在るか」は**推定できない**（同じ本の中で答えが割れる）。
⚠️ ★ **計器の穴を 2 つ塞いだ（第3便）**: ⑴ **`R1^"R"` の text は `TextScript` ではなく
`MultiMeasureRestText`**——`silence-mixed` の最初の dump は **"s" と "r" を全部出して "R" を 1 つも
出さず**、「LP がラベルを落とした」と読める嘘になっていた（第128 の `TXT` 追加と同じ型の穴の 2 例目）。
⑵ **小節線が無いと記録を小節に帰属できない**——**MMR の x は左境界＝小節線**なので、
BAR 無しで読むと**全部の MMR を 1 小節後ろに置く**。

---

## 以下は第128セッションの経緯

最終更新 第128セッション＝**「書かれた休符 1 つ = MMR 1 本」を 3 便で移植して
`part-combine-mmrest-shared` を閉じ、4 便目で A/B**（第127の「次の一手 ⑴」＝いちばん筋がよいとされていた候補）。
...
★★★ **第4便 = A/B round 29（`perf-ab29.ps1`・base `ee1e8a52` worktree `C:\MyProj\LilySharp-perfbase-ee1e` 残置）。
実退行なし——ただし ★★ この機械では中央値が読めず、床（min）で読んだ**。
⚠️⚠️ **まず帯を測った**（`perf-ab29b.ps1`＝**同じ base バイナリを自分自身と 10 回比べる**）:
**plain1k の spread は 40%**（9617〜13669 ms）。⇒ **中央値の差は 40% 未満なら全部ノイズ**で、
round 29 の中央値表（plain1k が **−2.4% / +23.1%** と符号反転）は**何も言っていない**。
⇒ ★★★ **床（min）で読み直した**——**ノイズは時間を足すだけなので、最小値は最も頑健**:

| 入力 | 何を課すか | base 床 | curr 床 | 読み |
|---|---|---|---|---|
| plain1k（MATCH） | **反射ハッシュの bool 1 つだけ** | 8912 | 8928 | **+0.2% ＝ 床が一致。素の楽譜に計測可能な費用は無い** |
| comb300（MATCH） | ⑵ の**純オーバーヘッド**（休符ゼロの合体） | 2153 | 2167 | **+0.7%**・spread 6〜14% ＝ **ほぼゼロ**（この行は帯が狭いので**読める**） |
| mmr1k（MATCH） | ⑴ の walk と展開（**出力を動かさずに**） | 1309 | 1834 | ⚠️ **判定不能**。base は spread 55% で**中央値 1986 対 curr 1878＝curr が速い**・round 29 も **−16.0% / −1.9%**。⇒ **退行の証拠なし** |
| mmradj（DIFFER・**設計どおり**） | 1 本の run が **250 本**になる | 524 | 1259 | **絵が別物**（休符 1 個 → 250 個）＝**描く量の差であって退行ではない** |
| combmmr（DIFFER・**設計どおり**） | 折り畳みが大半の小節で発火 | 974 | 1371 | 同上 |

⇒ **要点 = 絵が変わらない 2 冊（plain1k・comb300）の床が一致する**＝**この移植は、絵が同じところでは
ただ働きをしていない**。DIFFER の 2 冊が遅いのは **LP 正解の絵が記号を桁違いに多く含むから**。
⚠️ **開示**: ⑴ これは `lysc svg` の端から端なので、**診断／incremental の経路だけを分離して測ってはいない**
（round 28 と同じ限界）——**素の楽譜が払う唯一の費用がそこに在る**ので、**在処を名指せるのは「数えた」ほう**。
⑵ **中央値ではなく床で読んだ**のはこの機械の帯が広かったからで、**帯の測定込みで開示する**のが条件。
...
- twin = scratch\lpreg\stempure.{lys,svg,-gen.ly,-lp.svg,-lp.log}（\stemUp で
  ...
  この本が初めて corpus で踏んだ。
- **読者を数えた結果 = 家ごと beamed 化が字面**: LP は 3 読者全部が
  pure_y_extent（= beamed 込み pure height）を読む——separation-item の skyline 箱・
  note-spacing.cc:272-273 stem_dir_correction・staff-spacing.cc:55-59
  optical_correction。「one house」は LP 側でも成立していた。
- **port 3枚**: ⑴ NoteItem/ChordItem に **PureBeamedStemTip** 新設
  ⑵ MeasureCollector.ResolveBeamStemDirections（beam 群を既に歩く家）で**同方向
  member の unbeamed tip の極値を焼込**——Stem::internal_pure_height（stem.cc:
  387-447）calc_beam 枝の字面: unite+逆側 overshoot clip は「共有 tip 1個 + 自分の
  begin」に畳める・cross-staff coords 項（:421-436）は LS beam 群が単一譜なので
  恒等 0 と開示・LP 自身の cache_pure_height（:449-458）が焼込の対応物
  ⑶ SpacingRules.StemSpacingInfo が tip で帯を置換＝AddStem／stem 補正／barline
  optical が一斉に beamed 化。
- **照合: 全列 LP exact**（♭ **13.61**・頭 9.34/11.59/14.76/17.02/19.77/22.03/
  24.78/27.03・stem 8 本全部）。観測者 =
  BeamedPureStemSpacingTests.AccidentalClearsTheBeamCarriedStem（LP 9点ピン）。
- **snapshot 17枚 census 済 = 全部 X drift 0.01-0.06 級・Y 不変**（beamed 本全般:
...
plain 322 / 処理済 **279**（fixed **53**・exact **35**・skip **174**・open **17**・
pending **43**。status.json 実数。数えたら state 別内訳も一緒に書くこと）。
frontier = アルファベット順の次の pending（stem-shorten 以降・明白なゲート無し＝
実評価が要る）。第2便の open（多 score 頁の鎖＝アーキ級）は focused session 候補。
slur-flag / slur-nice は文法宿題（slur 向き強制の綴りなし）の棚のまま——文法が
入ったら **slur-flag の追試を最初に**（第118 の理由そのまま）。

未 push は §0 のコマンドで開始時に数える（**⚠️ push しない**）・
テスト **4222 passed / 0 failed / 4 skipped**（第1便観測者+1・ApplyRods isinf 枝+1・
第4便観測者+1・全スイート確認済）・snapshot 第120 は **5枚（第1便）+17枚（第4便）**
（両方 census 済・上記）・Core (Debug) 0 warning・
base worktree = C:\MyProj\LilySharp-base（cc19cccc・残置）＋
C:\MyProj\LilySharp-perfbase-0c63（第118残置）＋
C:\MyProj\LilySharp-perfbase-ae2a（第119残置）＋
C:\MyProj\LilySharp-perfbase-87c4（第120残置）。
...
stempure.{lys,svg,-gen.ly,-lp.svg,-lp.log}（第53号の照合材）**＋
...
**sub-question ①（X 軸）は完全決着**、**島2（単音/グレース臨時記号）も決着**（courtesy paren は 2026-07-24 前半で**字面移植して完了**＝島2 完了）。次はどれも「点を対で起票→測って初めて見える」未測定領域（§3A の穴）：
...
- ✅ **beam の page/system スカイライン種は完了（続き×5・`8c35ba42`→`b287d444`）**: BSD/BSU を対で起票（LP 13.790000・開栓 +0.76 が予測に桁一致）→ base（`BuildSystemSkylines`）で suppress＋seed → **両点 0 exact**。§1 最新ブロック参照。
- ✅ **courtesy paren スカイライン＝`parenthesize` を字面移植して完了（2026-07-24 前半・下記ブロック）**。当初「単音は box と恒等・列 regime は書けない＝打ち切り」と判定したが、**ユーザー指示で覆った：「LP のロジックの完全模倣が目標。字面移植できる部分は出力が同じでもそうする」**。leftparen/rightparen の実 outline を bake・box `Merge` を paren 合成へ置換・flat 0.375 を runtime `!parenthesized` 分岐へ。出力 byte 不変・LP dump ピンのテスト 4 本。和音 courtesy を実装した暁には**追加作業ゼロで**列 nest も LP 一致になる。
- **clef の LILC-vs-skyline sliver（Y 4 点）** は SKPath 不可＝下記「保留になった一手」（LP instrument が先）。

**手順は 7 回連続で成立した型をそのまま**（点を対で起票→予測を why に先書き→種/移植→対の食い違いが第2欠陥を出す）。§1 末尾の 4 箇条を読むこと。

#### ✅ このセッション（2026-07-24 続き×7・最新）＝ **widest-key の譜集合を閉じた（TKC/TKT を起票 `c0ae2338` → 1 述語に統一 `fc892032`・既存 corpus byte 不変）**

- **起票**（`c0ae2338`・ハーネスのみ・出力不変）: `line-start.time-to-first-note.{tab-concert,tab-keyed}`（probe 名 TKC/TKT）。**対の設計＝LP 側が恒等になる対**（KCS/KCC と同型）: 2 スコアは**音符が完全に同じ**で、tab 譜の `\key` だけが C major / F# major（6 sharps）。LP は `TabStaff \remove Key_engraver`（`engraver-init.ly:1214`・DrumStaff も :297）で **KeySignature grob が存在しない**＝誰もその差を読まない → **13 grob 全て 15 桁一致**。Lily# の対の差は **7.050000000 exact**（= Clef→Key 0.82 + 6 sharp ink 6.6 + Key→Time 1.15 − Clef→Time 1.52）で、**dump 前に書いた予測が 15 桁で的中**。実害: 第1音が**メーターから 7.05 右**に飛ぶ（fixture 皆無だった）。
- **修正**（`fc892032` → 字面移植し直し `a41d11f0`）: **break-align 群そのものを字面移植**——`ContributesToKeyColumnWidth`（＝調号を**彫る**譜。tab/text は `\remove Key_engraver`）で staff 集合を選び、`WidestActiveKeyInk`→`EngravedKeyInkWidth` で**彫られた幅の union** を取る（`break-alignment-interface.cc:141-142` が union するのは幅）。**予約が KeySignature でなく幅を持つ**ようになったので、**ossia も LP どおり群の一員**として scaled stencil（OKN 実測 1.5558 = 2.2×magstep(−3)）で参加できる。⚠️ 初版（`fc892032`）は ossia を**両側から除外して一致させた**が、これは LP の字面ではなく都合＝**ユーザー指摘で差し戻し**（§5.2「出力が同じでも字面移植する」）。tab-keyed **+7.450000000 → +0.400000000＝control と同値**＝対が Lily# 側でも恒等になった（＝譜集合が差の全てだった検算）。**既存 corpus は byte 不変・snapshot 0 枚移動**（最広 key を tab/text/ossia が持つ fixture が 1 つも無かった）。fixture `test/tab-part-key` を新設（PNG 目視済・snapshot 1 枚新規）。
- ⚠️ **ossia は述語から除外＝LP の字面ではない**（既存挙動の据え置きを 1 箇所に明文化しただけ）。LP の ossia は普通の Staff で Key_engraver を持ち群に入る（OKN/OKNF）。ただし stencil が **scaled**（OKM dump: ossia key ink 1.5558 = 2.2×magstep(−3)）で、**予約も描画も KeySignature を持ち回っていて「scaled な幅」を表現できない**——非スケールで入れると LP より広く予約する。▶ のキュー (0) に起票条件つきで積んだ。
- ★ **開けたら別の regime が落ちた（§5.2.1④・8 回目）**: **control の TKC が最初から +0.400000000**。正体は**譜間 `merge_springs` の平均化**——LP の行頭 spring は譜ごとの wish を**平均**する（`spacing-spanner.cc:478-517`→`spring.cc:104`）。**通常譜 2 段では wish が等しく平均＝自分自身**なので 3.700000 のまま＝**既存の多段譜の点では原理的に見えなかった**。tab 譜が混ざって初めて wish が食い違う（TimeSignature semi-shrink 2.0 → 8.82 ／ TAB Clef minimum-fixed 5.0 → 6.0 が共有 min_dist 床で 8.02 ／ 平均 8.42）＝**LP は 3.300000**、KCS/KC2 の単一譜 3.700000 ではない。**この 2 点の +0.8 が LP 忠実度 total を 0.006267→0.806267 に増やしているが、回帰ではなく新規開栓の正直な残差**。→ 次の ▶。
- ✅ **本題の外で見つけた実バグも修正（`ee6f62aa`）**: `tab X as numbers` を含むスコアが **CLI で LYS1007「Undefined part: 'numbers'」で落ちていた**。`PartReferenceFinder` が tab の**最後のトークン**を part 名としていたので `as` セレクタの語を part 参照だと誤認（`RenderSpecParser.ParseTab` は `with chords` 尾部→`as` 尾部の順に落としてから `toks[^1]`＝**検証器だけが字面から外れていた**）。**コミット済み fixture `test/tab-as-numbers.lys` すら `dotnet run --project LilySharp.Cli -- svg` で exit 1** だった（テストが緑だったのは snapshot 経路が `SymbolReferenceValidator` を通さないから＝**§5.2.1② の「複製された側に住むバグ」と同じ形**）。改名も同じ誤りでセレクタ語を書き換えていたはず。`TabPartToken` を `ParseTab` の字面（guard 込み）に揃え、**修正前に落ちることを実証した 6 アサーション**を追加。**LP 忠実度とは無関係**（`as numbers` は Lily# 固有表記・幾何は 1 mm も動かない）。⚠️ **未修正で残した隣**: chords 行 / lyrics 行は `PartReferenceFinder` に**そもそも case が無く**、part 参照が検証も改名もされない。足すと「未定義の chord/lyrics part を参照するスコアを新たに弾く」挙動変更になるので別途判断。

#### ✅ このセッション（2026-07-24 続き×6）＝ **carve-out 3 件の LP 忠実性監査（ユーザー問い掛け）→ cross-staff 除外を字面移植（`72905813`・出力不変）**

ユーザーの「残り 3 点は LP 忠実にできるか。アーキ不利が無いならしろ」に LP 実ソースで判定：**3 点ともアーキ不利（1-pass/memo/perf）は無い**。実施と残件：

- ✅ **cross-staff beam ＝ LP の字面は「除外」だった**（`72905813`）。`axis-group-interface.cc:850-858` に LP 自身のコメント「**we just leave cross-staff grobs out of the skyline altogether**」（`skyline_spacing` は :921/:954 で skip）＋ `stem.cc:1278-1290`＝**beam が cross-staff なら stem も cross-staff→除外**。つまり Lily# の「固定 3.5 stem を残す」は**発明**で、忠実形は**削除**（suppress あり・seed なし・符頭のみ残る）。`BeamedItemsToSuppress` を `IsKnee && !IsCrossStaff` の skip に変更＋単体テスト 3 本（旧条件に戻すと cross-staff テストが落ちることを実証済み）。
- ⚠️ **ただし判明した機能ギャップ: `BeamGroup.IsCrossStaff` は現状**到達不能**——`BeamMember.TargetStaffIndex` を −1 以外にする producer が皆無**（`@cross` は `CrossStaffLayouts`＝描画側にしか流れない。grandStaff＋`@cross` beam の scratch fixture は符頭が動かず普通の seeded beam として描かれた＝gap 11.79 で実測確認）。よって今回の変更は **corpus 出力不変**で、cross-staff beam 機能が実装された時のための**方針のピン**。E2E の対 probe（XBD/XBU 級）は**機能が届いてから**起票（LP twin は `\change Staff`・Lily# twin は `@cross`——ただし現状は等価にならないので作れない）。
- ⏸ **同一譜 knee の実 ink seed は保留＝「LP の knee stencil 帯を dump してから」**。LP は同一譜 knee の Beam/Stem stencil を skyline に**入れる**（cross-staff だけが除外）。ただし Lily# の `OuterEdgeStaffSpaceAtX` は「primary beam 中心±(半厚+stack)」の**対称モデルで、stack の内側面を言えない**——ここから反対面を作るのは §5.2 の発明。knee の帯（多段 beam の stack がどちらへ伸びるか）を LP の after-line-breaking dump（Beam ext）で測ってから、両面 seed（up 面/down 面）を移植する。なお knee の実 ink は**内向き**（符頭が外縁）なので、staff/page gap には原理的に bind しない＝現行の固定 stem 維持は観測不能な非忠実（courtesy paren と同じ族＝将来 regime で欠陥化し得る）。
- ⏸ **system silhouette の edge 譜限定（`BuildSystemSkylines` が先頭/末尾譜のみ）＝真の LP 非忠実**。LP の `build_system_skyline`（`page-layout-problem.cc:1075-1124`）は**全譜**の skyline を offset 付きで union。内側譜の ink が edge 譜の silhouette を突き抜ける regime（例: 3 譜で中央譜の深い beamed run が末尾譜の帯を超える）で乖離する。忠実化＝全譜 loop＋per-staff offset（`sysStaffGroups` の `StaffLayout.Y` を渡す配線が必要）。**probe を対で起票してから**（3 譜・内側譜の ink を deliberately 突き抜けさせる TSU 型の設計）。アーキ不利なし（同じ場所・内容の純関数・O(譜数)＝隣の pass と同オーダー）。focused session 向き。
- ついで: `EdgeStaffBeams` の staff index 解決を列挙順から直接導出に（reference 照合と fallback 排除・`72905813` に同梱）。

#### ✅ このセッション（2026-07-24 続き×5）＝ **beam の page/system 対（BSD/BSU）を起票→種→両点 0 exact（`8c35ba42`→`b287d444`・snapshot 4 枚再ベース）**

- **起票**（`8c35ba42`・ハーネスのみ・出力不変）: `system.beam-{under,over}-notes`＝TSD/TSU の設計（床 12・音符 8 ss 外・各小節の**先頭と末尾**に素の全音符で clef/終止線から beam を隔離・2 声で方向強制＝BMD の device）。**LP 実測 BSD == BSU == 13.790000 exactly = 8 + 2.5 + 0.24 + 2.05 + 1＝全項 LP grob 定数**。⚠️ 予測 13.60（BMD の quant 符尾 2.31）は 0.19 外れ——**譜から遠い beam は staff-line grid の引力が消えて ideal そのもの（3.5 − shorten 1.0 = 2.5）に量子化**される。モデルは不変・quant を測定が訂正。開栓時 Lily# は両方 **14.550000 exactly＝残差 +0.760000（= 3.5 − 2.5 − 0.24）が予測に桁一致**。
- **種**（`b287d444`）: ⚠️ **計画（AugmentSkylinesForPaging に配線）は成立しない**——stale な固定 3.5 stem は **base（`BuildSystemSkylines`→`AddNoteBoxToSkylines`）に住み、skyline merge は max なので下流では消せない**。fix は staff 経路（`BuildStaffSkylines`）と同型に base へ：`BuildSystemSkylines` が edge 譜（先頭/末尾＝この builder が処理する 2 譜）の描画 beam を受け取り、`BeamedItemsToSuppress` で固定 stem を suppress・`AddBeamsToSkyline` で実外縁を seed。beam は `LayoutEngine` が **staff 経路と同一 helper**（`MultiStaffLayouter.StaffBeamLayouts`・internal 化）で計算し、**memo（`ComputeSystemSkyline`）の compute ラムダ内**で作る（beam は memo キーが畳む小節内容の純関数＝cache 健全）。**staff gap・page gap・第1システム Y・annotation pass が読む system silhouette の全てが beam 1 モデル**に。
- **結果**: BSD/BSU **0 exact**（描画 beam positions −10.5 = −8 − 2.5 が quant モデルの検算）。**LP 忠実度 54/70 exact・total 0.006267 ss（セッション開始 baseline）・3247 緑・Core 0 warn/err**。
- **snapshot 4 枚**（台帳キー `system.beam-{under,over}-notes` で正当化・機械照合＝テンプレート不変・X 不変・PNG 目視済）: `drum-groove` 単一 system が **−0.95 剛体上昇**（全変化数値が exactly −0.95×114＝譜上 up-beam の固定 stem→実縁）／`tempo-swing` **canvas 高のみ −0.45**／`tab-as-numbers` **canvas 高のみ −0.15**／`timesig-grandstaff` **canvas 高のみ +0.04 伸長**＝傾いた bass beam の外縁が head+3.5 を **0.04 超えていた＝固定モデルは過少予約の側もあった**（実 ink がキャンバス外に居た真陽性。種の方向は「小さく」でなく「実 ink」）。
- ★ **教訓（2 つ）**: ①「merge は max」——過剰予約は下流で足しても消えない。**suppress は予約が生まれる場所で**（tuplet/slur/tie が augmentation で済んだのは base に競合予約が無かったから。beam だけ base に stale モデルが居た）。② **report の数字はピン値の echo**——`Corpus_ReportsTotalDivergence` の一覧は台帳の記録値で、実測は per-entry テスト（`Geometry_MatchesLilyPondWithinTheRecordedResidual`）の失敗メッセージが持つ。「0.76 のまま」と 2 回誤読し、計装（downExt=8.74）が「種は効いている」を先に示した。
...
⚠️ **beam の残**:
- ✅ **page/system スカイラインの beam 種は完了（続き×5・`b287d444`）**: `BuildSystemSkylines` が edge 譜の drawn beam を suppress＋seed（`system.beam-{under,over}-notes` 両 0 exact・§1 最新ブロック）。
- **cross-staff / kneed beam** は forced-shorten の対象外（`ComputeBeamShorten` は knee で 0＝LP `calc_stem_shorten` line 1068 と一致）＝固定 3.5 のまま（staff/system 両経路とも suppress せず＝台帳点なしの意図的 carve-out）。
...
✅ **ページ跨ぎスラーの対 SSD/SSU は閉じた**（`0385b1fb` 起票→`3ac143e7` 種＋衝突修正。`system.slur-{under,over}-notes` −1.122500648479451 → **−0.007708667**・両側 13.114791981 で 15 桁一致→prefix port 2 本で **+0.000534361** まで縮小）。✅ **ページ跨ぎタイ TSID/TSIU も閉じた**（+0.000442474）。✅ **ページ跨ぎ beam BSD/BSU も閉じた**（`b287d444`・両 **0 exact**・§1 最新ブロック）＝**bow/beam/bracket 族の system 対は全て起票済み・全て閉じた。**
...
## 第23・24セッション（2026-07-28）— **ossia の島**（ばね・spec・spaceable 性、そして鎖の両端）

> `HANDOFF.md` §1 から落とした逐語（第25セッション末）。**閉じた島なので、読むのは同じ regime に
> もう一度触るときだけ。** 原則は §5 へ汎化済み、数は台帳の `why` にある。
> ⚠️ ここに書かれた「残り」の読みは**第25セッションで訂正された**ものがある——
> 頭の +0.073104 は「ossia の量ではない」と結んでいるが、実際は ossia のインクが scale
> されていなかった分で、`c2955ba3` で閉じた。**当時の読みとして残してある。**

<details><summary>第24セッション（2026-07-28・閉じた）＝ ossia は spaceable。3 つの量を同時に入れて島が閉じた</summary>

**commit 1 本**: `489ac6d7`（Core **+124 / −91**・`LILYPOND-REF` **+3**・
`LILYSHARP-OWN` **−1**〈発明が移植になったので消えた＝正しい向き〉／
snapshot **3 枚**・ユーザー承認済）。§7.5 は移植と同じ commit で走らせた。
**LP の `is_spaceable` は 1 つのことしか訊かない**——`staff-affinity` を宣言しているか
（`page-layout-problem.cc:1173-1177`）。ossia は `\new Staff` で宣言しないので **spaceable**。
Lily# はそれを **4 か所**で外していた。

- ★★ **3 つの量で、一番大きいのは台帳が名指していなかったほう**:
  ⑴ **ばね**（`StaffSprings` のスキップ・+0.212184）⑵ **spec**（`InterGroupSpec` が群側
  `staff-staff-spacing`（min 7）へ差し替えていた。LP は grouper が無ければ
  `default-staff-staff-spacing`（min 8）＝**圧縮強度 1**・`axis-group-interface.cc:1007-1027`）
  ⑶ ★★ **spaceable 性そのもの**（`ClassifySystem`）＝ **−6.850208**。
  **⑶ は台帳が「候補を振って落ちた id で決めよ」と書いていたとおり摂動で決めた**——
  この 1 行だけ外すと **ちょうど 3 つの id**（頭・system・足）が動き、コーパスの他は 1 つも動かない。
- ★★ ⚠️ **分割して当てると「悪化」する移植だった**（§5.0 へ汎化）。⑴ だけ入れると
  inside は **8.350000**・system は **17.350000** で**両方とも遠くなる**（鎖にばねができたのに
  配置は ossia を外に置いたまま＝フレームが割れる）。⇒ **「1 つずつ試して良くなったものだけ
  入れる」では絶対に到達しない。** 到達させたのは**前セッションが書いた falsifier**
  （「4 読みが揃って着地しなければ移植は終わっていない」）。
- ★★ **4 読みは揃った**: inside **−0.002309**／system **−0.009234**（＝ちょうど 4 倍・
  強度 1 と 4）／頭 **+0.073104**／足 **+0.010000**。
  **36 × 0.002309 = 0.083108 == 頭 0.073104 ＋ 足 0.010000 = 0.083104** ⇒
  **1 force ＋ 2 固定項**で、対照とまったく同じ形。
- ★★ **足は対照と六桁で同一になった**（+0.010000）。⇒ **足には最初から ossia 固有の量は
  無く、頭の誤りが鎖の反対端に出ていただけ**。§5.3 の「両端を同時に開ける」の**裏側からの実演**。
- ★ **⑵ 単独では何も動かない**（両 spec とも basic 9・違いは minimum 7/8 だけ）。
  ⚠️ **「動かないから入れない」ではない**——§5.2 の裏面（観測差が無いことは字面移植をしない
  理由にならない）。**圧縮ページでだけ割れる**ことは規則テストが実証している（比が 2 になる）。
- ★★ **撤回された 8.414200 が、別のものとして戻ってきた**（§5.0 へ汎化）。今回は
  **snapshot 3 枚のページ高の実測差**（33.08→41.49・24.94→33.36・24.63→33.04）で、
  **アンカーが「譜の名目半分 2.000000」から対の 9 を上がって「ossia 自身の scaled 半分
  1.414200」へ移る**とそうなる。⇒ **撤回したのは数ではなく導出**。数が正しく見えても
  「共通の混入を確かめる前の一致」は証拠にならない、という前回の教訓はそのまま生きている。
- ★ **目視も両側で取った**（§5.3）: `test/ossia` はセクション記号が**紙の端に貼り付いていた**
  ——ページが ossia の頭を予約していなかったから。今は room があり、ページがその分伸びる。
- ★ **網は規則で書いた**（§5.4）: `AnOssiaIsSpaceable_SoThePageSolvesItsPairAndReservesItsHead`
  ＝ **system の不足は staff の不足のちょうど 4 倍**（対の両側）＋**頭は上マージンより下**。
  **3 つの量それぞれについて「修正前なら落ちる」ことを実証済**（剛体=0／群 spec=**ちょうど 2.000000**／
  非 spaceable=**−8.23**）。⚠️ **自分が圧縮 regime に居ることを先に assert する**（§5.0 罠 7）。
- ⚠️ **`LpReferenceCitationTests` のラチェットが実際に落ちた**（748 対 747）。原因は
  **引用の行範囲を書いた行に記号名を置かず、次の行に書いた**こと（検査は**同じ行の後ろ**しか読まない）。
  ⇒ **REF を折り返すときは記号名を同じ行に置く。**
- ⚠️ **残ったものは ossia 固有ではない**: 頭 +0.073104 は **対照の +0.024000 ＋ 0.049104**
  ——Lily# は**両方の本で床 3.8+padding 1 に座り**、LP は top ばねをその下（対照 4.776000・
  ossia 側 4.726896）まで圧縮する。**閉じるのは top ばねの床の側**。
...
今回は数ではなく**理由**が stale だった）。

</details>

<details><summary>第23セッション（2026-07-28・閉じた）＝ ossia のばねの点を開けたら、ossia でない欠陥が出た</summary>

**commit 1 本**: `50dc27ba`（台帳点 **10 件**・**Core 差分ゼロ**）。books **OSSK / OSSKN**
＝ OSSU/OSSUN を **120 小節**にして**圧縮する紙**に載せた対。

- ★★ **▶1／▶2 が「先に作れ」と言っていた点はできた。** 力 0 では**剛体と可動が同じ 9.000000
  に見える**ので、前セッションの摂動が「何も動かない」と出たのは regime のせい（§5.3）。
  **その regime を自分で作るのが今回の作業だった。**
- ★★ **フォークは片方だけ鳴った**: **LP は ossia の距離を普通の譜ばねとして解く**——
  OSSK の inside は **8.787816**（＜ ideal 9）で、VAG は **`aff=()` ＝ spaceable**。
  しかも**同じ dump の中に falsifier がある**: `(9 − inside)/1 == (12 − system)/4` が
  **両冊とも六桁一致**＝ページごとに 1 つの force。⇒ **`StaffSprings` のスキップは移植ではなく
  欠陥**。Lily# は **9.000000000**（residual **+0.212184**）で、**予測が桁まで当たった**。
- ★ **ばねの圧縮強度は 1 で、JSK の 2 ではない。regime でなく spec の違い**:
  `calc_maybe_pure_staff_staff_spacing`（`axis-group-interface.cc:1007-1027`）は grouper が
  あれば `StaffGrouper.staff-staff-spacing`（min 7）、無ければ **`default-staff-staff-spacing`
  （min 8）へ落ちる**。**ossia は素の `\new Staff` なので後者**。⚠️ ところが
  `MultiStaffLayouter` は **ossia が絡むと `sp.StaffStaff`（群側）へ差し替える**
  （`:130-131`・`:222-223`）。**両 spec とも basic-distance 9 なので静止では一致する。**
  ⇒ **移植は 2 つの量**（ばねを作ること＋spec を正しく選ぶこと）。
- ★★ ⚠️ **同じセッションで自分の所見を 1 つ撤回した。そしてそれが今回一番役に立った作業。**
  最初の測定では **control が exact でなく**（inside −0.108167）、残差を強度で割ると
  **4 残差＝1 force ＋ 2 固定項**・**36 ×(force 差) = 3.894004 == 頭 3.884000 ＋ 足 0.010000**
  で閉じたので、「**Lily# は鎖の頭を 3.884 下げすぎ**」と結論した。
  ⚠️ **その 3.884 は Lily# の欠陥ではなく、プローブ自身の section mark だった**——
  Lily# 側だけが `form main { A }` と書いて**セクション記号を 1 個彫り**、`.ly` 側は
  **`\header` も title も markup も持たない**（page-vertical.ly の冒頭がそう明記している）。
  ⇒ **対の両側が同じ音楽ではなかった**（§5.0 の罠・**7 度目**）。`~A` と綴ると同じ読みが
  **−0.000945**。
  ⇒ ★★ **見つけ方は「動かないこと」だった**: その頭は**圧縮でも伸長でも 1 system ページでも
  14.350551 で不動**。**ばねなら regime で動く。動かないなら固定予約**（§5.3 の
  「摂動して動かない」の裏返しで、**今回は不動そのものが証拠になった**）。
- ★★ **訂正後の control は「ほぼ exact」で、これは前の結論よりずっと強い主張**:
  inside **−0.000945**・system **−0.003778**（比 3.998＝やはり 1 force）・
  **36 × 0.000945 = 0.034020 == 頭 +0.024000 ＋ 足 +0.010000**。
  残る頭の +0.024 は **Lily# が床 3.8 + padding 1 = 4.8 に座り、LP は top ばねをその下
  4.776 まで圧縮する**差。⇒ **Lily# は非群の圧縮ページを千分の一で解いている。**
- ★★ **そして本命が出た**: **ossia 側の頭は −6.850208**（Lily# 3.567239 ＝ 上マージン
  5.690551 より **2.123312 上**＝**ossia がマージンにはみ出して描かれている**）。
  対照が +0.024 なので**これは ossia 固有**。⇒ **`StaffSprings` のスキップ（+0.212184）と並ぶ
  第2の量で、しかもこちらのほうが大きい。**
  ⚠️ **今回は識別子の算術を出さない**——最初の「8.414200 = 9 − 2 + 2 × 0.7071」は
  **section mark 込みの数から出した幻**で、六桁一致していても**共通の混入を確かめる前の一致は
  証拠にならない**。**持ち主は摂動で決める**（§5.3）。
- ⚠️ **予測を 1 つ外し、外れとして記録した**: system 距離を「剛体の対が引き受けない分が出るので
  **LP より下**」と予測して、**上（+0.299732）** に出た。頭の固定項が予測した効果より一桁大きい
  ためで、⇒ **量を予測しないと決めたなら符号も予測してはいけない**（§5.0 の既出項の 2 例目）。
  ★ **この外しがあったから、鎖の両端の点を「後で」ではなく同じ commit で入れた**（§5.3 へ汎化）。
- ⚠️ **LP の最終ページは 1 つ前のページの force を印字する**（OSSKN の 3 ページ目は system 1 本・
  足の slack 144 なのに **page 2 の 8.728721719 を九桁そのまま**）。**どの点も読んでいない**し、
  説明もしていない。**page 1 だけ読むこと。**
