...
最終更新 2026-07-27（第14セッション・**hara-kiri 島の「借り」を返した＝台帳点 6 件を開いた**）/
HEAD は §0 で確認すること（⚠️ 自己参照。`dfb5af62`・origin より **122 ahead・未 push**）。
**3389 passed / 0 failed / 3 skipped**・Core 0 warn 0 err・
**LP 忠実度 109/135 exact・total |residual| 1.454788 ss / 120 distances・counts 13/15**。
...
1. ~~**hara-kiri 島**~~ — **閉じた**（アーカイブ §0）。~~この島の量は台帳の regime の外~~
   — **`dfb5af62` で台帳に入った**（`hara-kiri.*` 6 点）。網はテスト 4 本
   （`RemoveEmptyDeclaration_WithNothingEmpty_ChangesNothing` の 2 regime ＋
   `HaraKiriSystemHeight_*` ＋ `EachSystemIsSpacedByItsOwnInk_*`）**＋コーパス 6 点**。
   ⚠️ **残る regime は「圧縮ページで譜間インクが spec を超える」**（新しい 2 点は ragged のみ）。
