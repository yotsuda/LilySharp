...
最終更新 第324セッション＝**入り方は第298〜第323 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い、便の途中の口挟みは 0）。⇒ ★★★★ **本便は §1 の次の一手 ⑴＝T7 の比較の前提を直し（製品コード 0）、⑶＝F2（section 名の箱を RehearsalMark の位置へ・ユーザー決定済み）を閉じた（製品コード 1 commit）**。**骨は 4 つ**:

⚠️⚠️ ★★★ **⑴ T7 の指標は `KindMatched` の対だけになった**（`scratch/p321/parse321.ps1` の末尾に "KIND-MATCHED (the T7 metric)" 行を足した・§2 T7 の頭に数え方）: **第323 の木で 203 対中 SigMatch 126・小節数一致で段割れ違い 45・小節数違い 32**（第322 の木 194／124／42／28）。**代用対 384（SigMatch 256）は指標にしない**。⚠️⚠️ **第323 ⑶″ の「+2」は gate の得ではなかった**——**第322 → 第323 で 9 対が代用から一致へ移った**（ユーザーが 09-02 22:35〜23:03 に `She Bangs`・`君が思い出になる前に`・`Rose Garden` ×2・`Everything I Do`・`Close To You` の `.lys` を編集した＝`.lys` の mtime）**うち 7 対が SigMatch**、**KindMatched の中の得失は 得 0／失 5**＝`Alone Again [both]`（第323 ⒞・双子の break 欠落）・`She Bangs` ×2（入力変化）・`いとしのエリー [tab]`（第323 ⒠）・**`悲しみにさよなら [both]`（新規＝下 ⒣）**。**第323 の `kindmatched.ps1` は壊れていた**（lost 3 だけ刷って gained 0 なのに +2 と言った）→ **`scratch/p324/kindmatched2.ps1`**（key の一致 592/592 を自分で数える）。⚠️⚠️ **第323 ⑶′⒡ は撤回**: `悲しみにさよなら.ly` の `%{`（L90）は **L116 の `%}` で閉じる**（LP のブロックコメントは入れ子にならない・L142 の `%}` は行コメント）ので **staff＋tab の bass book は生きていて、LP は `lp-bass-page1..3.png` を描いている**。**parse321 の `Strip-LyComments` は最初から正しく、足すものは無い**。
★★★ **⒣ `悲しみにさよなら [both]` bar 73-78（LP 6 段・Lily# 3,3 は第323 の `38cb4dad` から）を第323 ⒜ の形で測った**（`scratch/p324/P324LineProbeTests.cs.txt`＝行の spring と解・`kanashimi-lys-scores.txt`＝段数ごとの得点・LP は `lp/dbg.err`（`-ddebug-page-breaking-scoring`）と `lp/jus.out`／`lp/rag.out`（`probe-bn-barx`）: **⒜ Lily# の行: ideal 123.82／min 92.66／幅 94.71→ 線形 −0.51（罰 −0.65）・解くと −0.834（46 本中 36 本 block・罰 −1.80）→ 得点 17 段 22.366 対 18 段 21.480 で 18 を採る**。**LP（bass book）: 17 段 14.607 対 18 段 15.600 で 17**。**⒝ LP は justified でその 6 小節を 90.32 に置く（自然幅 ≈128.9）＝bar 74/75/76/78 を Lily# の *min* より狭く**（bar 76: LP 17.64 対 Lily# min 20.08。min に居る spring は tab の 16 分列＝床 2.307 で剛体）。⇒ **Lily# の tab 数字（§3 F9・単数字幅 1.625）は LP の `font-size = 2`（≈1.25）より広く、床が LP の実配置より高い＝どんな gate でも 6 小節は置けない**。**旧 gate が置けたのは線形見積りが block を見なかったから**。⇒ **6 冊とも gate の後退ではない（第323 の 4 冊＋本便の 1 冊＋She Bangs は入力変化）＝`38cb4dad` は戻さない、が推奨**。

⚠️⚠️⚠️ ★★★★ **⑵ F2 を閉じた**（`MusicMarkEngraver.CalculateXPosition`: `if (lineStartSystem >= 0)` の腕を Rehearsal と SectionLabel の両方に＝行頭で `|:` が描かれていればその線・無ければ `BuildPrefixMarkAnchorX`＝key/clef の右端。LILYSHARP-OWN と宣言＝LP の SectionLabel は left-edge だが双子は `\mark \markup \box`）: **陽性対照 `scratch/p321/fx/fx4-mark-tempo.lys` → `scratch/p324/fx4-new.png`＝箱は key の右・♩=117 はその真下＝LP の絵と同じ**。**台帳**: **`mark.section-label.line-start.box-left-from-clef-left` を MKQ（RehearsalMark 2.565）に付け替えて exact**／**`mark.plain`・`mark.chord-row.staff-to-baseline` を MKP／MKC の RehearsalMark（2.85）に付け替え＝残差 +0.75 ×2（＝0.40 箱底→基線 ＋ 0.35 箱底 1.15 対 LP 文字底 0.8）**／**`lyrics.chord-row.marked.gap-second`・`.every-system` を RWM／RWMA（`\mark` 綴り・12.563793）に＝−0.108252 ×2（`@mark` 双子は +0.2196＝描画側・OPEN）**／**`page.section-label.first-staff-refpoint` を RWMN（12.284649）に＝+0.205902**（**RWMA は本便が `page-vertical.ly` に足した書・LP 3 冊は `scratch/p324/pv-subset.ly` で測った＝dump は同一**）。⚠️⚠️ ★★★ **`system.indent-floor.one-staff` が 12.0 → 13.999 に伸びたのは gate でも F2 でもなく双子の octave**——**`LpGeometryProbes.SystemIndentFloorScore` は `c'4 c' g'' g''`＝C5/G6（LP の `tuneO` は `\relative c'` で C4/G4）**。行頭の箱が音符の上に来て初めて露見した。**`c4 c g g`（C4/G4）に直して 12.0 exact**（LP の `tuneO` 後半の `c,` は 1 oct 下＝Lily# 双子は同じ register のまま・測る対は 1→2 段だけ）。**`FormNavigationTests` の 2 本**（「label は左端に留まる」「coda に持ち上げられない」）**は前提が反転**＝label は `|:` に乗り coda と同じ列で積む、に書き換え。**snapshot 225 枚再ベース**（`\mark` を持つ本ほぼ全部）・`docs/APPROXIMATIONS.md` は OWN 116。CHANGELOG 0.6.0 Engraving に 1 項。

★ **⑶ 踏んだ罠**: **⒜ ripple の Red console が `Select-String … | ForEach-Object` の後に `PS>`（パス無しの prompt）を刷って boundary 検出が外れ、以後ずっと busy**（Ctrl+C でも戻らず・新 console を `reason` 付きで起こして続行）＝ユーザー自作ツールなので報告価値あり／**⒝ `Measure-LilyPondPageGeometry.ps1 -Probe` は `probes/` 起点の相対**（`..\..\..\scratch\p324\pv-subset.ly`）／**⒞ 第323 ⒡ の「コメントの中」は入れ子と読んだ誤り**（ls をまず見る）。

★ **開始時裏取り**: HEAD `43566c6c`・**未 push 8**（`origin/master` は `dcf111bd`）・木 2（README.md と samples/README.md＝ユーザー手直し。本便も触っていない）・未追跡 0・Core 0 エラー 0 警告・追跡 `.lys` 594・snapshot 244・台帳 759 点（exact 594 / `OPEN:` 0）。
終了時: **commit 2 本**（`f91b9274`＝F2 一式（232 ファイル＝snapshot 225 込み）／この HANDOFF＋ARCHIVE）・**未 push 10**・作業ツリーは README.md と samples/README.md の 2 件だけ・未追跡 0（`scratch/p324` は git 管理外）・Core 0 エラー 0 警告・**Windows Debug 6915 / 0 / 4 / 6919**（**`scratch/p324/full3.trx`**。第323 比 ±0＝F2 はテストを足していない。`full1.trx` は inventory 1 赤＝`LILYSHARP_UPDATE_DOCS=1` で再生成・`full2.trx` は `HandoffArchiveContinuity` 2 赤＝HANDOFF を書き換えている最中に読まれた race・単独再走 3/3 緑）・**追跡 `.lys` 594 不動**・**snapshot 244 不動（225 枚動いた）**・**台帳 759 点不動・exact 594 → 593**（section-label 対照点 +1・marked gap 2 点 −2＝`\mark` 綴りの参照へ）・`OPEN:` 0・`docs/APPROXIMATIONS.md` OWN 115 → 116（`audit/magic_constants.csv` 不動）・**LSP/拡張は未配備**。⚠️⚠️ **WSL ubuntu Release は取れない**（第313 ⑶）——**本便は Core を触っているので `push` 後に `gh run list` で読むこと。**

⇒ ★★★★ **次の一手**（**第323 の並びの ⑴⑶ を本便が消費した**）: ✅✅ ★★★ **⑴ perf＝段数ループ＋DP の解き＝2026-09-03 に静かな窓（ユーザー許可）の Release で取った。数字は `audit/perf/editbench-p325.txt`**（生は `scratch/p325/perf/`＝管理外）。**要点だけ**: **base を 4 点に割った**（`a8ae0ad6` → `ccab465f` → `38cb4dad` → HEAD。**2 点 A/B では「解き」の値段が出ない**——間に rods と F2 が居る）。**EditKeystrokeBench（regime ⑶）の floor・3 走の最小**: **`ccab465f`（rods）は無料**（+3.4% / +2.8% / **−7.8%**＝v2bow1k はむしろ速い）／**`38cb4dad`（解き）が費用の全部**（**+7.9% / +8.4% / +73.6%**）／**そこから HEAD までは雑音**（+0.2% / +1.4% / +4.5%）。⇒ ★★★ **費用は平らな税ではなく `perf-v2bow1k` に集中している**＝**候補行あたりのばねが一番多い本**＝`SpringSolver` の仕事が一番多い所。**形が機構と一致する。** ★ **対照**: **`PreviewUpdateBench`（regime ⑴＝木が不変で layout 全体再利用＝DP が走らない）は 9 冊とも動かない**（最大の変化は head が僅かに速い）＝**費用は break gate に居て collect / key / render には居ない**。⚠️ **ただし `perf-v2bow1k` は regime ⑴ の棚に無い**ので、対照は「他の 9 冊」であって「退行を抱えた本の別 regime」ではない。⚠️⚠️ **1 走目だけ読むと `perf-plain1k` が +72% に見えた（3 走で +9%）**——**単サンプルは、主張を*大きく*する向きに間違えた。** ⇒ ⚠️ **これは第323 ⑶ の「戻すか」の*値段側*で、Release では一度も数えられていなかった。忠実度の議論とこの数字は、ユーザーが並べて量るもの（本便は推奨を出さない）。**／★★★ **⑵ T6＝スラー復元 138 冊**／★★ **⑶ T7 の C 37 冊を 1 冊ずつ**（指標は KindMatched の対で）／**⑷ 以下は第323 の並び**: §2 T6 の残り／`audit/lpreg` 取り直し（本便で `\mark` 本の絵が動く＝取り直し前に `REGENERATE.md`）／§2 U8c の島／§2 U8b／§2 U8 の残り／§2 A/B/D/E／§2 C⑴／§2 G／★ 低優先: `marks stacked` / `marks beside`（F2 が閉じたので着手可）。⚠️⚠️ ★★★ **⑸ §2 F ⒴＝(B) 実音入力／移調パート出力**（2026-09-03 にユーザーが「最終的には (B) まで実装したい。安全・堅実に進めて」と明示。**(A)＝preset が半音移調を持つ、は `259922de` で入った**）——**着手前に ⒴ の実測 3 点を読むこと**（調号の機械は既に在る／符号は (A) の逆／同じ量の綴りが 2 つになる罠）。▽▽ **⑹ §2 F ⒵ は 2026-09-03 に*三重に*縮んだ**（起票と同日に根・取り分・射程を全部測った）: **⒜ 素朴な緩和は不健全**（連鎖打鍵で `data-pos` が古くなる＝既存の網が捕らえた）／**⒝ 時間でも見えない**（floor と median が符号で食い違う＝分解能の下）／**⒞ 射程も「Δ≠0 の打鍵すべて」ではなかった**——**実本 3 冊では `perf-v2bow1k` だけが落ち、`perf-plain1k` と `perf-fingbeam1k` は落ちない**。⇒ **「増分の残りはここ」は撤回。⒵ は閉じてよい項目に近い。** **追うなら ⒵ ではなく「なぜ `perf-v2bow1k` だけ違うのか」（2 声・prefix resume も効いていない）。**⚠️ **承認待ちは §2 T2 の 1 件**（`Lambada Complicada.lys`）＋**§2 T5 の ⏸ 1 件**＋**第323 ⑶ の「戻すか」（第324 の推奨＝戻さない・⑴ ⒣。⚠️ 2026-09-03 に*値段側*が付いた＝`38cb4dad` は `perf-v2bow1k` の実編集打鍵を +74%・単声の 2 冊は +8%。上の ⑴）**。⚠️ **§2 F の開いている項目は第320 の ⒲ 1 件。** ⚠️ **リリース 0.6.0**: CHANGELOG に F2 を足した・版番号 7 か所の bump と tag は未着手（週末判断）。
...
- ★★ **⒴ (B)＝実音入力／移調パート出力（＝「コンサートピッチで書いて、パートは移調して刷る」）**（2026-09-03 起票・**未実装・ユーザーが「最終的には (B) まで実装したい。安全・堅実に進めて」と明示**）。**(A) は入った**（`259922de`）＝**`InstrumentDefaults.GetTransposition` が半音移調楽器にも答える**（clarinet / clarinet-a / trumpet / trumpet-c / horn / soprano-alto-tenor-baritone-sax）。**(A) は鳴る側だけ**——**書いた音高がそのまま刷られ、動くのは MIDI と tab のフレットだけ**（`InstrumentDefaults.ConcertPitchIsNotImplemented` に「何が足りないか」を書いてある）。**(B) はその逆向き。**
