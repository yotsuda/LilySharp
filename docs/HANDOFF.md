...
最終更新 第308セッション＝**§1 が名指した「次の一手」の 1 つめ＝§2 U8 の残りに着手し、⒜ の指定した宿題を先に片付け、そこから*和音*を閉じた**。**入り方は第298〜第307 と同じ**——**ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った**。⇒ ★★★ **本便の骨は「起票が*測ってから決めよ*と名指した宿題が、起票そのものの仕分けを 4 度目に割った」**: **⒜ は和音と休符ではなく、*梁に触らない半分*と*⒞ の梁の模型そのものである半分*だった（⑴）／その宿題の答えは「取りこぼしている」で、しかも*Lily# の側だけを見た測定*が外していた（⑵）／ページに和音を教えた瞬間に MIDI との 1 オクターブが露出した（⑶）。** **掃きは 1992 冊・両側 Release・4 出力。**

⚠️⚠️ ★★★ **⑴ ⒜ は「和音と休符」ではなかった。難所は*梁*で割れる。** **第302 は 2 つを「難所は住所ではなく*模型*」の 1 箱に入れた**が、**和音は「1 列が符頭を N 個持てる」だけで梁に 1 行も触らない**（**第302 自身の member 表がそう言っている——和音をどこに置いても polygon 2・path +4**）／**休符は「1 列が符頭を *0 個* 持てる」で、その瞬間に LP の梁は*先頭の連なりだけ*を覆う**（同じ表の `{ d'16 e'16 r16 f'16 }`＝**1 つの群が梁と旗を同時に持つ**）。⇒ ★★★ **つまり休符は ⒞ の*部分梁グループ*そのもので、⒜ ではない。** ⚠️ **この起票が仕分けを間違えたのは 4 度目**（第298 症状／第299 症状／第302 難所／本便 *難所の粒度*）——**毎回、実際に手を動かす便が 1 つ小さい単位を見つけている**（→ RULES §5.0）。★ **割る材料は新しく測っていない。第302 の LP 表を読み直しただけ。**

⚠️⚠️ ★★★ **⑵ ⒜ の宿題の答えは「取りこぼしている」で、第302 の測定は*自分の木だけ*を見ていた。** **起票は「`StaffAccidentalColumns` と `NoteCollision` は `grep -i grace` が 0 行＝装飾音符はその 2 軒を通らない」と書き、同じ段落で「今 grace の和音は 1 つも彫られないので空虚に真」と*自分で警告していた*。** ⇒ **LP に訊いたら 2 軒とも通っていた**（`scratch/p308/lp`）: **`<< { \grace { cis''16 } } \\ { \grace { bis'16 } } >>` の臨時記号は 16.2208 / 17.0831 に積まれ、*同じ 2 音を一声の和音で書いた本と 4 桁一致*。符頭も 0.9179 寄る。** ★★ **Lily# は今日その 2 つを同じ X に重ねて描く**（`scratch/p308/ab/g1_2vgrace`＝両方 x=7.39・y 差 0.5）——**和音を 1 つも要らない、前から在る欠陥**。⚠️ **ただし住所（`itemIndex`）を要るので ⒞⒟ と同じ族**＝**混ぜずに §2 U8b として起票した**。⇒ ★★ **`grep` の 0 行は「要らない」ではなく「まだ書いていない」の証拠で、2 つは同じ 0 行に見える**（→ RULES §5.3）。

⚠️⚠️ ★★★ **⑶ ページに和音を教えた瞬間に、MIDI との*1 オクターブ*が露出した——前から在って、比べる相手が居なかっただけ。** **`MidiExporter.ProcessGrace` の和音の腕は各構成員を*直前の音に対する相対*で解いていた**のに、**コメントは「matches ProcessChord / CreateChordItem」と書いていた**（**第178・第306・第307 に続く *`observed by: NOTHING` の兄弟*）。**実測**: **`grace { <c b>16 }` は 60 と 59 を鳴らし、同じ本の `<c b>4` は 60 と 71。ページと MusicXML はどちらも B4。** ⚠️ **さらに悪く、`<c' e'>` のように*狭い*和音でも主旋律のアンカーが 2 オクターブ上へ流れていた**（掃きで 11 冊の MIDI が動いたのはこれ）。⇒ **1 軒に寄せた**（`ResolveChordMemberPitch`・**XML 側はまだ食い違っていなかったが、grace の腕を書くのに主旋律の綴りが要るので同じ形にした**）。

⚠️ ★★ **⑷ LP 実測がフォントの規則を 4 桁目で名指した。** **`grace { <c' d'>16 }` の二度の寄せは 0.8530** で、**14 デザインの `1.298161 × magstep(-3) − 0.065 = 0.852938` には合い、20 デザインを縮めた `0.857209` には合わない**（**全サイズの `<c' d'>4` は 1.2392 ＝ 1.304200 − 0.065 で桁まで一致**）。⇒ **`GraceNoteItem.ScaleFactor` の doc が何便も警告していた 0.004270 が LP の印字で裏取りされた**——**縮尺ではなく*デザイン*を渡す**（`ChordHeadPositioning` の `headFont` overload）。⚠️ **cue の和音は今も scale を渡している＝同じ 0.0043 を抱えたまま**。**本便は触っていない**（cue の snapshot が動く・cue のフォントを LP に訊く実測が別に要る）＝**§2 U8c として起票**。

⚠️ ★★ **⑸ 実装は模型 1 語**（`GraceNoteItem.Notes`＝1 音の平らな列 → `Columns`＝1 列・符頭 N 個・`GraceHeadInfo` 新設）**で、幾何は 1 つも発明していない**——**二度の寄せは `ChordHeadPositioning`、臨時記号の積みは `AccidentalPlacement`、どちらも既にフォントを取り、住所を取らない**（**起票の予測が当たった**）。**規則の家は `Svg/Layout/GraceColumnHeads.cs` 1 軒**（予約・スカイライン・描画が同じ答えを読む）。⚠️⚠️ ★★ **`ImmutableArray` の `Equals`/`GetHashCode` は*配列の参照*で答える**ので、**それを持つ record struct は「値の計算できない値型」になる**——**`GraceColumnInfo` は自分で対を書いた**。**見つけたのは既存の網**（`SystemLayoutCacheTests` が幅不変の編集で reuse を 1 つ断った＝**ページは 1 度も間違っていない、reuse だけが落ちた**）。

⇒ ★★★ **次の一手**: **⑴ §2 U8 の残り＝⒞ の*スラーとタイ* と ⒟ *注釈の族***（**梁は本便が閉じたので、⒞ に残るのは住所を要る 2 つだけ**）。★★★ **⒞⒞ の残りと ⒟ と §2 U8b（二声の同時 grace）は*同じ 1 つの設計*を待っている**——**装飾音符が `ItemIndex` を名乗れないこと**。**3 つまとめて 1 便で設計するのが安い**（**⒟ は `ArticulationItem`・`DynamicItem`・`MusicMarkItem.AnchorItemIndex`、⒞ は `TieItem`/`SlurItem` の `Start/EndItemIndex`、U8b は `VoiceItemKey` と `(measure,voice,item,note)`——**5 つとも同じ形の鍵**）。⚠️ **正典 2.26.0 が要るかは*量ごとに*確かめる**（本便が示したとおり、既存の正典実測と 1 度突き合わせれば WSL で足りることがある）——**この機械の正典 exe は本便でも 17 分ブロックした**。／**⑵ §2 の X/Y 側**（§2 A/B/D/E）／**⑶ §2 C⑴ の多声 walk の moment 順への再設計**（**最大の構造負債・未着手**。**大手術なので踏む本が溜まってから**と §2 自身が書いている）。⚠️ **承認待ちは無い**（snapshot は 1 枚も動いていない）。⚠️ **§2 F は開いている項目が無い。**

⚠️⚠️ ★★★ **⑹ 後半で ⒜ の*休符*と ⒞ の*梁*を閉じた——⑴ の見立て（この 2 つは 1 つ）が実装で当たった。** **休符の模型は和音と同じ 1 語**（`Heads` が空＝符頭 0 個）**で、そこから出る梁の規則を LP に 12 冊で訊いた**（座標で。第302 は polygon の数で読んでいた）: **`{ d'16 e'16 r16 f'16 }` の梁は `{ d'16 e'16 }` 単独と*4 桁一致*し（span 1.4679・y も同じ）、`f'` は旗**。⇒ ★★★ **部分梁は「新しい梁」ではなく「既存の quanter に*接頭辞だけ*を渡す」**。**`IsBeamedRun`（bool）を `BeamedPrefix`（個数）にしただけで、4 人の読み手が全員*どの列か*を訊くようになった。**

⚠️⚠️ ★★★★ **⑺ 装飾の休符は *full size* で描かれる**（1 冊の中で並べて実測＝**休符 0.0040 対 符頭 0.0028**・path はバイト同一）。**機構は正典ソース**: **`general-grace-settings` は 9 つの grob に font-size を与え、*Rest を 1 度も名指さない*。** ⇒ ★★ **「装飾は小さい」は他の全部に当たるので、*一番もっともらしい実装が間違う*場所**——**grace のフォントで描くと LP より 1/4 狭く、列ごと狂う**（休符の次の列が広い理由でもある: LP 1.7000 対 1.4180、Lily# は 1.70 対 1.42）。

⚠️⚠️ ★★★ **⑻ 版の扱いが 1 段変わった。** **第302 の「WSL 2.27.3 は質的まで・座標は正典で」に対し、第300 が正典で測った 2 冊と本便が WSL で測った同じ 2 冊が*4 桁一致*した**（梁 span 1.4679 / 2.8859）。⇒ ★★ **「WSL は座標に使えない」ではなく「*その量について*一致するかを 1 度確かめれば使える」。** ⚠️ **確かめざるを得なかった**——**正典 exe は本便で 17 分ブロックし 1 バイトも書かなかった**（**CPU 1.4 秒 / WS 28 MB**＝第302 の署名）。★ **ユーザー依頼で*ブロック解除*（1203 → 0）と *Defender 除外*を入れたが、どちらも効かない**。**Smart App Control は既にオフ**＝**3 つとも犯人ではない**（→ §2 G）。

⚠️ ★★ **⑼ LYS4020 の Element の文面が*1 便のうちに 2 度*古びた**（「bare notes only」→和音が列になった／「notes and chords only」→休符が列になった）。⇒ **文面を `GraceBodySupport.CarriedKinds` にして `IsCarried` の隣へ置いた**——**メッセージの中の列挙は述語の第 2 の綴りで、述語が増えても赤くならない。** ★ **同じ理由で番人が 3 度*正しく*赤くなり、3 度*別の本*に貼り替えた**（`<d' f'>` → `r8` → `cue { }`）——**「閉じたら赤くなる網」が働いた記録として、その経緯をテストのコメントに残した。**


⚠️⚠️ ★★★★ **⑽ 掃きも番人も緑のまま出荷したものに、*測っていない発明*が 1 つ残っていた——`s`（spacer）の列幅。** **休符を入れたときに「spacer は列を保持して何も描かない」と*書いた*が、LP に訊いていなかった**——**commit してユーザーに「緑」と報告したあとで訊いた**（`scratch/p308/lp2/t1_spacermid`）:
- ★★ **構造は当たっていた。しかも偶然ではないことまで測れた**: **LP は `grace { d'16 s16 e'16 }` を*両方旗*で描き梁を出さない**（休符のときと同じ）。⇒ **梁を切るのは*インクの有無*ではなく*符頭の有無*。** **番人に 1 行足して固定した**（足すまでは `IsRest` が spacer を含んでいる偶然だった）。
- ⚠️⚠️ ★★★ **幅は 0.78 ずれている**: **`{ d'16 s16 e'16 }` の d'→e' は LP 2.5600・Lily# 3.3400。** **同じ形を*休符*で書くと 3.6387 対 3.64 で桁まで一致する**⇒ **列の機械は正しく、spacer の*ばね*だけが違う**（**LP の「grob を 1 つも宣言しない列」のばねは、Lily# が計算する床より狭い**）。**`departs from / goes away when / observed by` で名指した**（**`observed by: NOTHING`＝台帳に grace spacer の点は 1 つも無い**）。**射程はディスク 1997 冊で 2 冊、両方とも本便のプローブ。**
⇒ ★★★★ **教訓＝*緑の掃きは「設計どおり動いた」しか言わない。「設計が正しい」は言わない*。** **掃きも番人も LP を知らないので、この 0.78 について 1 字も言えない**（→ RULES §5.2）。⚠️ **本当の問題は順番**: **実装 → 掃き → commit → 報告 → *それから*測定、をやった。**


★ **開始時裏取り**: HEAD `07f8c408`・**未 push 50**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6805 / 0 / 4 / 6809**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 点 158（非ゼロ 2）／OPEN: 0**・追跡 `.lys` 581・snapshot 231——**第307 の終了時の数と 1 つも違わなかった**（**素朴に数えると `exact` 556・`OPEN:` 12 が出る。§0 のレシピは `-le 1e-6` と `why -like 'OPEN:*'`**）。
終了時: **commit 5 本**（**⒜ の和音 `d3fd657b`** ／ **§1・§2 U8・U8b/U8c の起票・RULES §5.0/§5.3・第306 をアーカイブへ `ba7051cc`** ／ **⒜ の休符 ＋ ⒞ の梁 `3b0b76a2`** ／ **§1・§2 U8・§2 G・RULES §5.2/§5.4 `1c1a26c6`** ／ **spacer の実測と、そこで見つけた自分の未測定の発明（この commit）**）・**未 push 55**⚠️ **数を閉じる commit が数を動かすので、最後の 1 本は*自分を数えて*「この commit」と書く**（第305〜第307 と同じ形）・作業ツリー 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6808 / 0 / 4 / 6812**（**開始 6809 から +3**）・**台帳 748／588／190／24.714570188／158 中 2／`OPEN:` 0・追跡 `.lys` 581・snapshot 231 は開始時と同じ**（**snapshot は 1 枚も動いていない**）。⚠️⚠️ **`docs/APPROXIMATIONS.md` は行番号*だけではない*――`UNWATCHED` が 49 → 50 に増えた**（**spacer の 0.78。定数は 1 つも足していない・`audit/magic_constants.csv` は行番号だけ・REF/OWN の増減 0**）。⚠️ **間の push は無し**（`origin/master` は `be16a770` のまま）。★ **base 用の worktree `lys-base308` / `lys-base308b` は両方消した。**
★★ **WSL ubuntu Release は Windows と一致を 2 度読んだ**: **`d3fd657b` で 6807 / 0 / 4 / 6811**、**`3b0b76a2` で 6808 / 0 / 4 / 6812**（build 8 秒 / test 32 秒）——**§0 の両脚を読んだ。**⚠️ **docs commit では ubuntu を回していない**（動いたのは `.md` だけ）。
⚠️ **計器は `scratch/p308/` に全部残っている**: **`lp/`**（**LP 16 冊＋`dump.py`**＝**和音の中の二度・和音の中の臨時記号列・二声の同時 grace**。**`<a xlink:href="textedit://…">` が grob を*ソースの列*に結びつけるので、どの符頭がどの音かを推測せずに読める**）／**`lp2/`**（**LP 15 冊＋`dump2.py`＋`measurements.md`**＝**休符と梁。⚠️ `dump2.py` は第302 の `count.py` の polygon 抽出を*直した*もの——あちらは points をカンマ区切りと決め打ちしていて、この build では*梁を 1 本も見つけない*。「梁ゼロ」と報告する計器で梁の規則を測るところだった**）／**`ab/`**（**プローブ 13 冊と `run4.py`**＝**4 出力を対照 2 冊と突き合わせる。着手前と着手後の両方で回した**）／**`sweep308.ps1`/`.json`**（和音・base `07f8c408`・1992 冊）と **`sweep308b.ps1`/`.json`**（休符・base `d3fd657b`・1993 冊）／**`canon/`**（**正典 exe が 17 分ブロックした跡**）。
...
- **U8. ▶ 容れ物は全部閉じた（第298 ＋ 第299 ＋ 第300 ＋ 第301 ＋ 第302）。残るのは*本当に grob を要る*ものだけ＝`grace { }` の本体は*歩かれない*。起票の「注釈を運ばない」より広かった**
  ...
  **描画（`SharedRenderer`）・spacing・MIDI・MusicXML・LilyPond export**（**この 5 つは今*脇の配列*を読むので、
  ...
  ⚠️⚠️ ★★★ **【⒝ の中にもう 1 つ分岐が在る。第309 が着手を見送るときに気づいた——上の段落は ⒝2 だけを書いている】**
  - **⒝1＝住所のためだけに項目にする**: **項目は `measure.Items` に入れるが、描くのは今までどおり
    `GraceNoteLayout`。脇の配列は*捨てずに、その項目から導出する***（**二度読まないので二重にならない**）。
    ⇒ **ページは動かない**＝**2007 冊の掃きで*バイト同一*を示せる**。**⒞⒟ は「住所を grace の幾何へ解決する」だけになる。**
  - **⒝2＝脇の配列を畳む**: **普通の engraver が grace のフォントで描く。** ⇒ **`GraceNoteEngraver` 645 行・
    `SharedRenderer.GraceNotes` 744 行・`GraceColumnHeads` 231 行が畳める**が、**ページを計算し直す。**
  ⇒ ★★ **どちらを採るかは「grace 固有の幾何がどれだけ残っているか」で決まり、それは第309 が 1 つ測った**:
  ★★★ **`GraceColumnHeads` は自分で「A TRANSLATION, NOT A SECOND MODEL」と書いており、
  *符頭の二度*も*臨時記号の積み*も、フル サイズの和音が通るのと同じ `ChordHeadPositioning`／`AccidentalPlacement` を
  grace のフォントで呼んでいるだけ**（`:32-33`・`:82`・`:106`・`:209-210`）。
  ⇒ **つまり符頭と臨時記号は ⒝2 でも*測り直しが要らない*——既に普通の規則。**
  ...
  ⚠️⚠️⚠️ ★★★★ **入口＝*まだ 1 度も測っていない LP の量*。第309 は LP に*ソースしか*訊いていない（座標 0）。**
  **⒞⒟ を実際に彫るには、この 3 つを LP に訊くところから始まる**（**どちらの分岐でも要る**）:
  **⑴ grace body の中のスラーの制御点と端点**／**⑵ grace body の中のタイの同上**／
  **⑶ grace の符頭に付く script（`@staccato` 等）の y と padding**（**`general-grace-settings` は Script に font-size −3 を*与えている***
  ——**休符と違って「小さい」が当たる側。`scm/music-functions.scm:636-650`**）。
  ⚠️ **正典 exe はこの機械で 13 分（第302）・17 分（第308）ブロックした前科がある**（→ §2 G）。
  **WSL 2.27.x は「その量について一度一致を確かめれば使える」**（第308 が梁 span で示した型）。
  ⇒ ★★ **だから ⒝ の便は「長く・遅延の読めない実測」から始まる**——**設計の話を抱えたまま入らないこと**
  （**第307 が U8 を見送った理由と同じ。第309 も同じ理由で見送った**）。
