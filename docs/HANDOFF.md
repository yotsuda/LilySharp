...
最終更新 第188セッション＝**「黙って消える」と起票した欠陥が、4 つの歩きのうち 2 つだけの
沈黙で、規則は既に木の中に*散文で*書いてあった便**（**実装 3 便**）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
開始時の裏取りは**全項目を走らせた**——**台帳 516 点・ss 非ゼロ 98・総和 3.609965521・
count 点 106/うち非ゼロ 2**・HEAD `d23f7b0a`（実装の最後 `688763c6`）・
**未 push 30・未追跡 0・作業ツリー項目 0**・suite **5208 passed / 0 failed / 4 skipped**
＝**引継ぎの数は全部合っていた**（**19 回連続**）。
終了時 **実装の最後は `8aca063c`**・**未 push は
`git rev-list --count origin/master..master` が言う**（**セッション中にユーザーが 30 を push した**）・
**未追跡 0**・suite **5239 passed / 0 failed / 4 skipped**（+31＝新しい網ちょうど）・
**台帳 516 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・**snapshot 0 枚**。
**全数 A/B は 1025 冊**（base 対 HEAD・**PID ごとに*両方の*バイナリを写した作業場**）：
**絵 0・data-pos 0・MIDI 0**（**1025/1025 描画成功・両側とも失敗 0**）。
**診断の全数掃きも 1025 冊で変化 0**（`scratch\p187-checksweep.ps1` を再利用。
**陽性対照＝342 冊が既に何か言っている**・第187 と同じ数）。
★ **ユーザー決定 1 件**＝**repeat の無い volta ending は「彫る＋警告」**（error にしない）。
★★ **環境＝コンソールを 3 本使う羽目になった**（`Sage` を**自分で閉じた**・下の ⑸）。

> ## ★★★ この便の骨＝**「黙って消える」は、*誰が*黙っているかを数えるまで欠陥の名前ではない**
> §2F ⑼ は「**repeat の無い volta ending が黙って消える**」＋「**規則の形が決定**」と起票していた。
> **どちらも半分だった。**
> ⇒ ★★★ **⑴ 消していたのは 4 つの歩きのうち 2 つだけ。** `form main { A [1. B] }` は
> **MusicXML と双子では最初から平の参照として鳴っており**、**ページと MIDI にだけ腕が無かった**。
> ⇒ **同じ本が、どの出力に訊くかで別の曲だった。**「消える」は*ページの*性質であって
> 言語の性質ではなかった。**4 出力を並べて 1 つの表にするまで、これは見えない。**
> ⇒ ★★★ **⑵ 規則は「決定」ではなかった——木の中に散文で書いてあった。**
> `LilyPondExporter:956-958` が **「A volta ending OUTSIDE a repeat block is just its
> section: there is no \repeat for an \alternative to hang on」**と*コメントで*書いており、
> **その通りに実装されていた**。**私は既に決まっている規則をユーザーに設問として出しかけた。**
> ⇒ ★★ **⑶ 起票が書いた述語は弱かった。** 「規則は『その form に repeat block が 1 つも無い』
> でなければならない」——**`|: A :| B [1. B]` は repeat block を持ちながら同じく落ちていた**。
> **正しい述語はエングレーバ自身の条件**（「repeat block の中に居ない `FormAlternative`」）で、
> **推測ではなく*その落とし主に訊く*と一発で出る。**
> ⇒ ★★★ **判定法**: **「黙って消える」と書く前に、その構文を読む歩きを全部並べて 1 行ずつ答えを書く。**
> **1 つでも答えが違えば、それは沈黙の欠陥ではなく*不一致*の欠陥**で、直し方も名前も変わる
> （沈黙なら「規則を決める」・不一致なら「多数派に合わせる」）。
> ⚠️ **そして「規則の形は決定だ」と引き継がれた項は、まずコードの*散文*を grep する。**
> **実装した誰かが理由を書いていることがある。**

> ## ★★ 2 つ目の骨＝**設問の前提を測ったら、LP の警告は*別の問い*への答えだった**
> RULES §5.0 は「**設問に出す前に、設問の前提を 1 冊で測る**」と書いている。**今日それが効いた。**
> **`\alternative { { … } }`（番号なし）を `\repeat` 無しで書くと LP は警告する**
> ——ここで止めれば「**LP も警告する**」と設問に書いていた。
> ⇒ ★★★ **番号を付けた `\alternative { \volta 1 { … } }` を測ると、LP は*何も言わず*
> 平の音楽と SVG バイト同一を出す。** 警告は**番号が無いこと**への警告で、repeat の話ではない。
> ⇒ **`[1. A]` は番号を*書いている*綴りなので、当たるのは後者。**
> **前者だけ測っていたら、ユーザーに出した 3 択の説明文が逆になっていた。**
> ⇒ ★★ **判定法**: **測る本は「設問の綴り」と同じ形にする。** 近い形ではなく同じ形。
> **「LP は警告する/しない」は、どの綴りについての話かを書いていなければ主張ですらない。**

> ## ★★ 3 つ目の骨＝**§7.5 の監査が「REF 0」と言ったので LP のソースを引いたら、1 grep で住所が出た**
> 3 便を書き終えて §7.5 を回すと **Core +176 行（うちコード 45）・REF 0 / OWN 0**。
> **⒟（既存の規則を別の器へ指し直しただけ）と分類して閉じるつもりだった**——
> 根拠は「規則は LP の**出力**を測って決めた」。⚠️ **§5.2 は逆を要求している**
> （「移植の前に読むのは実測ではなくソース」）。**引いたら 1 回の grep で出た**:
> **`lily/alternative-sequence-iterator.cc:83-84`＝`repeat-count` は既定 1**
> ——**囲む repeat が無ければ alternative はちょうど 1 回鳴る**。**規則そのもの。**
> ⇒ **分類は ⒝（LP 由来・字面でない）で、§7.6 は REF を*必須*にしている。** 3 か所に付けた。
> ⚠️ **これは §5.2 が和音記号の 2.6 で記録している欠陥と同型**——**LP の規則が真横にあるのに
> 「LP には無い」の札を貼る**形。**札の誤りは、次の人に「未移植」か「発明」を選ばせる。**
> ⇒ ★★ **判定法は §7.5 の数そのもの**: **REF 0 で閉じようとしたら、閉じる前に 1 回 grep する。**
> **「出力を測って決めた」は、ソースを読んでいないことの言い換えであることがある。**

- **⑴ ★★★ 第1便＝ページと MIDI を残り 2 つの歩きに合わせた**（`4dd1aedb`・**Core 2 ファイル**）。
  **`MeasureCollector.ProcessForm` と `MidiExporter.PlayForm` に `FormAlternativeSyntax` の腕が
  無かった**だけ（MIDI は**再生側 `RepeatFromTheBeginning` にも**＝同じ列を 2 回 switch している）。
  **測定後は 4 出力とも平の綴りとバイト同一**: `A [1. B]` ≡ `A B`／`[1. A]` ≡ `A`（**旧は 0 バイト**）／
  `|: A :| B [1. B]` ≡ `|: A :| B B`。**括弧は描かない**（LP と同じ）。
  ⚠️ ★★ **毒 ⒠ が緑で返り、それが収穫だった**——`!IsInsideRepeatBlock` を外しても
  **FormVolta 一族の網が 1 本も赤くならない**のに、**ガードは効いている**
  （`|: A [1. A] :| [2. B]` のインク `86F66A44` → `63559F68`＝両 ending が二度彫られる）。
  **その穴のために網を 1 本足した**（section の並びが `["A","A","B","A","B"]` になる）。
  **§5.4「毒はどの case が赤くなったかで読む」の、*どれも赤くならない*版。**
  ★ **第187 のピンが設計どおり鳴った**——`AVoltaEndingNoRepeatOpens_…AndStillEngravesNothing` は
  「閉じたらこの網は変わる。だから注記でなく網にした」と自分で書いており、**suite の赤 1 本がそれ**。
- **⑵ ★★★ 第2便＝LYS6008（警告）**（`bc168607`・**ユーザー決定＝彫る＋警告**）。
  **述語は 1 か所だけに綴った**——validator が collector と MIDI と同じ問いを訊く。
  ★ **`TheWarnedEndingsAreExactlyTheOnesTheEngraverDoesNotBracket`** が **1 冊
  （`|: A [1. A] :| [2. B] [3. B]`）で「彫る集合 {1.,2.}」と「警告する集合 {3.}」が
  *互いに素かつ網羅*** であることを主張＝**2 つ目の綴りが生えたらここが鳴る。**
  ★ **毒 ⒢＝起票の弱い規則に差し替えると*ちょうど 2 本*赤**（`|: A :| B [1. B]` の case と
  上の分割テスト）＝**起票の述語が弱いことの証拠がそのまま網になっている。**
  ⚠️ ★★ **メッセージが §5.0 の「引用せよ・再構成するな」を破りかけた**——初稿は
  **「drop the `'[1.]'`」**＝**section を真ん中から抜いた綴り**で、**誰も打てないし言語も受理しない**
  （`WarnUnknown` が `@fig(6.4)` を `'@fig(6 4)'` と報告していたのと同型）。
  **いま引用する綴りは全部 source の部分文字列**で、**候補（`|: … :|`）だけが例外**——
  `EveryQuotedSpellingIsOneTheAuthorWrote` が**候補を名指しで除いたうえで**それを主張する。
  ⚠️ **squiggle は `SyntaxNode.Span` を使っていない**——**leading trivia は落ちるが
  *trailing* は残る**ので `{ A [1. B] }` の `[1. B]` は `"[1. B] "` を張る（1 文字多い）。
  **トークンの span はインク**（実測：`]` は 104..105）なので `InkSpan` が最後の子から端を取る。
- **⑶ ★★ 第3便＝LP の住所を引いた**（`8aca063c`・**コメントのみ・挙動 0**）。上の 3 つ目の骨。
- **⑷ ★ §7.5 の監査**＝**Core +176 行（うちコード 45）・数値定数は `InkSpan` の添字だけ**。
  **分類は当初 ⒟ → 実際は ⒝**（第3便で REF 3 本）。**LP に対応物が無い部分**＝
  **`form` という編曲層そのもの**（LP は `\score` が音楽を直に持つ）。**ending の意味論は LP に在る。**
- **⑸ ★★ 道具で 2 回転んだ。どちらも RULES に*既に書いてある*罠**。
  ⚠️ **⒜ `Add-Type -Path <ビルド出力>` を打ったコンソールが DLL を掴み、以後ビルドが
  MSB3021 で倒れた**（.NET Core は既定 ALC から降ろせない＝**コンソードを閉じるしかない**）。
  ⇒ **§5.4 の「PID ごとの作業場」は掃きだけの規則ではない**——**in-process の計器にも要る**。
  **§5.5 に書いた。**（CLI を子プロセスで呼ぶ計器は安全。API を直に叩くものだけが持つ穴。）
  ⚠️⚠️ **⒝ `$a` と `$A` は同じ変数**（PowerShell は大小無区別・**§5.4 に第147 の実例で既出**）。
  `$A = ReadAllLines($a)` がパスを潰し、**2 ファイルの移動が半分だけ完了した**
  ——**§1 から 152 行を削る書き込みは通り、ARCHIVE へ足す書き込みだけが落ちた**＝
  **その 152 行がどちらのファイルにも無い状態**。**git に在ったから戻せた。**
  ⇒ ★★ **「落ちた＝まだ何もしていない」ではない。落ちたら両方の状態を数える。§5.5 に書いた。**
- **⑹ ★ perf（RULES §7 ⑼）＝3 便とも「足していない」。**
  第1便は switch のラベルが 1 つ増えただけ（新しい走査も確保も memo も無し）。
  第2便は**打鍵経路**（LSP の `PublishDiagnostics`）に乗るので明記した——
  **歩くのは form 自身の部分木**で、**すぐ上の LYS6007 が既に同じ部分木を歩いている**
  （`SectionReferenceFinder.AllSectionNameTokens`）。**木全体の `DescendantNodes()` は使っていない。**
  第3便はコメントのみ。
- **⑺ ★★★ §2F に残っているのは 8 つ**——**⑴ 一覧に構文を与えるか（決定であって作業ではない）
  ...
  残るのは木を faithful にするか否かの決定）⑻ `_ "text"` の空白（**ユーザー決定＝後回し**）**。
  **「repeat の無い volta ending」はこの便で閉じた**（4 出力のうち 2 つの不一致だった）。
  ⇒ ★★★ **「0.3.0 の前でなければ不可能」な項目は 0 のまま。**
  ⚠️ ★★ **「絵が出ないのに黙る」形は 46 形すべて塞がった**（第187 が 15 形・この便が残る 1 形）。
  **次にこの族を疑うなら、`GRAMMAR.md` の項を総当たりして engraver に通すこと**（46 形・約 27 秒）。
  ⇒ ★ **次に「黙って消える」を疑うときの手順が 1 つ増えた**：**綴りを engraver に通すだけでなく、
  *4 出力ぜんぶ*に通して表にする**（ページ・MIDI・MusicXML・双子）。**今日の欠陥は
  ページだけ見ていては「言語の穴」に見え、4 列並べた瞬間に「不一致」に変わった。**

---

## 以下は第187セッションの経緯

最終更新 第187セッション＝**引き継がれた「1 件」も、自分が数えた「4 綴り」も、
自分が書いた「網は無い」も、全部*的を絞った問い*の答えだった便**（**実装 3 便**）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
開始時の裏取りは**全項目を走らせた**——**台帳 516 点・ss 非ゼロ 98・総和 3.609965521・
...
**台帳 516 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・**snapshot 0 枚**。
**全数 A/B は `lysc check` を 1025 冊**（base 対 HEAD・**PID ごとにバイナリを写した作業場**）：
**診断が変わった本 1 冊**（陽性対照＝**base で 342/1025 冊が既に何か言っている**）。
★ **ユーザー決定 3 件**（**⑻ `_ "text"` は*後回し*・触らない**／**空 form は LYS6002 と同じ *error***／
**誰も出さない診断コードは 7 件とも*削除*して引退**）。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Sage`）で通せた**（第183・第185・第186 と同じ）。
⚠️⚠️ ★★★ **ただし「長い掃きの最中は Read/Grep/Edit だけ」の規律を*破った***——
**掃きの最中に `dotnet build` を 3 回打ち、掃きと bin を共有した**（下の骨 ⑵）。
**Edit は安全だが build は安全ではない。**
★ **この便の道具は 3 本、`scratch\` に置いた**（⚠️ **git 管理外＝2 台目には来ない**・RULES §5.5）:
**`p187-checksweep.ps1 -Label base|after`**（全 `.lys` の `lysc check` を丸ごと記録して
`scratch\p187-check-<label>.csv` に出す。**先頭でバイナリを `$env:TEMP\...-$PID` へ写す**・約 5 分・
**CLI は診断コードを印字しないので出力そのものを突き合わせる**）／
**`p187-formmatrix.ps1`**（`GRAMMAR.md` §StructureItem の項を総当たりして
「絵が 0 バイトなのに `check` が黙る」形を出す・**46 形・約 27 秒**）／
**`p187-emptysweep.ps1`**（git 管理下 566 冊を SVG にして 0 バイトを数える・約 5.5 分）。
★ **`scratch\p187-check-base.csv` は clean なバイナリで取った 1025 冊の診断 base**——
**診断を動かす便は、これと突き合わせれば掃きが 1 回で済む**（取り直しても約 5 分）。
⚠️ **3 本とも「何件拾ったか」を同じ run で印字する**（§5.0 罠21）。
**`-eq 'No errors found.'` で数えて 0/1025 を返した**のはこの便で、
**改行を `' | '` に潰したあとの `Trim()` に末尾の `|` が残る**のが原因。**前置き一致で数えること。**

> ## ★★★ この便の骨＝**「N 件」は、どの問いを誰に訊いたかまで書いていないと族の数ではない**
> §0 ★ は「**数を引き継ぐときは数え方も書く**」と 5 例を挙げている。**今日はその*コード側*の顔を
> 3 回、しかも 3 つの別の出所で踏んだ。**
> ⇒ ★★★ **⑴ 引き継がれた数**: §2F ⑼ は「**LYS1015 が誰も出さない**」＝**1 件**。
> **家（`DiagnosticCodes` の全メンバ）に訊いたら 7 件**だった——**96 宣言・89 に呼び手・
> 7 件は誰にも名指されず、リテラル参照も 0**。**「1 件」は grep の的の数**で、族の数ではない。
> **見えなかった理由は構造**: **既存の網は全部「1 つのコード」を見る**ので、
> **誰も名指さないコードは、誰の網にも名前が出ない。集合を見るものが無かった。**
> ⇒ ★★★ **⑵ 自分がその場で数えた数**: 空 form の綴りを **4 つ**（空・縦線・ナビ記号・`_"text"`）
> 数えて commit message に書きかけたが、**`GRAMMAR.md` §StructureItem の項を総当たりして
> engraver に通したら 46 形中 16 形が 0 バイト**（＋`break`/`nobreak`・`@mark("X")`）。
> **15 形が今日の網に入り、1 形だけが射程の外に残った**（`form main { [1. A] }`）。
> **綴りを思いつく作業をやめて機械に列挙させた瞬間に、4 が 16 になった。**
> ⇒ ★★★ **⑶ 自分が書いた数**: 第2便の commit message に「横断 grep 済み・**網なし**」と書いたが、
> **それを確かめたのは LYS1015 だけ**で、他 6 件に*一般化*していた。**ビルドが 1 件出した**
> ——`DocExamplesParseTests.FragmentCodes` が `UndefinedPhrase` を名指していた。
> ⇒ ★★ **しかもそれが本体より面白かった**——**その除外は何も除外していなかった**
> （`UndefinedPhrase` を出す validator が存在しない。phrase は検査されているが
> `UndefinedVariable` の本文 "Undefined variable or phrase" に畳まれている）。
> **死んだ除外は、無い被覆に見える。**
> ⇒ ★★★ **判定法**: **「N 件」を書く前に、その N が*どの集合*を数えたかを 1 行書く。**
> 書けないなら、それは**的の数**であって族の数ではない。
> ⚠️ **そして「一般化」は数の話だけではない**——⑶ は数ではなく**述語**（「網は無い」）を
> 1 件から 7 件へ広げた形。**1 件で確かめた述語を、族の全員について書かない。**

> ## ★★ この便の 2 つ目の骨＝**共有の作業場の罠を、今度は*自分のビルド*と踏んだ**
> §5.4 は第184 の実例で「**A/B ハーネスは PID ごとの作業場を持たせる**」を書いている。
> **今日は掃きの出力先ではなく*バイナリ*が共有だった**——1025 冊の `lysc check` を回している
> 最中に、毒の出し入れで `dotnet build` を 3 回打ち、**掃きの子プロセスが差し替え中の
> `LilySharp.Cli\bin` から起動していた**。⇒ **「114 冊が新しく報告」という測定でない数**が出て、
> しかも**もっともらしかった**（実 fixture `snappizzicato.lys` が名指されていた）。
> ⇒ ★★ **見破ったのは 1 冊を手で測ったこと**——**現バイナリで `snappizzicato.lys` は緑**。
> **PID ごとにバイナリを写して測り直すと 1 冊**（第185 の壊れたプローブで、**新しい診断は真**）。
> ⇒ ★ **予防は 1 行**（掃きの先頭で bin を `$env:TEMP\...-$PID` へ `Copy-Item`）。
> ⚠️ **規律の言い方も直す**: 「長い掃きの最中は Read/Grep/**Edit** ツールだけで待つ」は正しいが、
> **`dotnet build` / `dotnet test` はツールではなく*同じ木への書き込み***。**待つ対象に入れる。**

- **⑴ ★★★ 第1便＝section を名指さない form が、0 バイトの絵を「Created」と言って出していた**
  （`8c9d4dbe`・**LYS6007・ユーザー決定＝LYS6002 と同じ error**）。**⑻ の前提を測っている
  途中で出た。** **家は `SvgDocumentContext.Assemble` の `_pages.Count == 0` の裸の `return ""`**
  ——`lysc svg` は **exit 0 で「Created: … Font embedded: Yes」と言いながら 0 バイト**を書き、
  `lysc check` は `No errors found` だった。
  ★ **規則は既に在った**——**LYS6002 の doc が「空のページはレイアウト障害に見えるので
  出荷せず報告する」と書いている**。**器が 2 つあって、1 つにしか当たっていなかった**（§5.2.1②）。
  **結果は blank page より悪く、page が無い。**
  ★ **別コードにした理由は LYS6003 の先例**（LYS6002 の本文「its body declares no staff」は
  **この器の名も出口の名も指していない**。form は staff を足しても直らない）。
  ★ **「section を名指すか」は `SectionReferenceFinder` に訊いた**——**3 綴りを既に知っている家**
  （plain `A`・silent `~A`・volta `[1. A]`）。**4 つ目の写しを作らない。**
  ⚠️ ★★ **毒が*テストの弱さ*を暴いた**——plain だけ数える毒に替えたら、
  **volta の「緑であるべき」case が緑のまま**だった（その body に plain 参照も入っていた）。
  **綴りを分離する body は `|: [1. A] :| [2. B]`**（§5.4「その本がその枝を*踏む*ことを別に確かめる」）。
- **⑵ ★★★ 第2便＝誰も出さない診断コードを機械に落とさせた**（`8b365092`・**Core は 1 行も触っていない**）。
  **名前の一覧のラチェット**（数ではない——LpReferenceCitation の理由と同じで、
  **数だと古い 1 件が開いたまま新しい 1 件が入る**）。**ソースを読む**——`const string` は
  コンパイラが畳むので**反射では呼び出し側に残らない**。
  ★ **網は 3 つの毒で確かめた**: ⒜ 出す先を別コードに替えると**両向きに赤**
  ⒝ **コメント内の言及は「出す者」に数えない**（`StripComment`。この毒は*緑*が正解）
  ⒞ **その `StripComment` を外すと、同じコメント 1 行で赤**＝**ガードが生きていることを同じ run で示す**。
  ★ **空集合の罠は両側**（「60 以上のコードを反射できたこと」＋「60 以上が名指されているのを見たこと」を
  先に assert）。**テストは走査対象に入れない**——**網が名指すだけのコードにも出す者は居ない**から。
- **⑶ ★★★ 第3便＝7 件とも引退させ、何も除外していなかった除外を落とした**（`688763c6`・
  **ユーザー決定＝7 件とも削除**）。**番号は `DiagnosticCodes` に 1 か所だけ列挙**
  （**8 か所の削除跡に散らさない**＝次に番号を選ぶ人が一度に見るため）。
  **`LYS0001` `LYS0005` `LYS0013` `LYS1002` `LYS1003` `LYS1006` `LYS1015` `LYS2003`。**
  ★ **毒**: 出す者のいない定数を 1 つ足すと**その場で赤**＝**空になった一覧は守られている**。
  ⚠️ **横断 grep は今度こそ全部やった**（**1969 ファイル**・`.cs/.md/.ts/.json/.ps1/.py/.lys`）
  ——**7 シンボルとも 0 件・7 番号ともリテラル 0 件**。
- **⑷ ★ §7.5 の監査**＝**第1便 Core +92 行（うちコード 25）・第2便 0 行・第3便は削除＋ remarks 1 塊**、
  **REF 0 / OWN 0・数値定数 0**（**正しい**）。**分類は 3 便とも ⒟**——
  **既存の規則を 2 つ目の器へ指し直した／網を足した／死んだ宣言を削除した**。
  **LP に対応物が無い**: **LilyPond に「編曲層」に当たる `form` が無い**（`\score` が音楽を直に持つ）。
- **⑸ ★ perf（RULES §7 ⑼）＝3 便とも「足していない」。**
  第1便の検査は**意味解析の pass で form 宣言ごとに 1 回**走り、**その form 自身の部分木**を歩くだけ
  （**打鍵経路の `PublishDiagnostics` に乗る**ので、`DescendantNodes()` を*木全体*に掛けていない
  ことが要件＝第182 の教訓）。**`DeclarationBody.BraceSpan` は元から在ったループそのもの**で、
  ★ **消費者は 2 つとも診断**（`EmptyScoreValidator`・`FormDeclarationValidator`）＝
  **抽出は絵に届かない**（grep で確定・推論ではない）。第2・3便はテストと削除のみ。
- **⑹ ★★★ §2F に残っているのは 9 つ**——**⑴ 一覧に構文を与えるか（決定であって作業ではない）
  ⑵ 繰り返し縦線の ⑹（MIDI の片側 `:|`）⑶ `lyrics` の voice 束縛（要決定）
  ⑷ `lysc ly` が transpose を落とす件 ⑸ 「Guards X」と名乗る fixture の残り 13 冊
  ⑹ 入れ子の中の phrase 参照が双子で空になる件 ⑺ 注釈と記号の順序（**半分だけ閉じた**——
  残るのは木を faithful にするか否かの決定）⑻ `_ "text"` の空白（**ユーザー決定＝後回し**・
  **ただし前提が 1 つ増えた**＝空白を許すと*生きた対*が潰れる）
  ⑼ 新規＝**repeat の無い volta ending が黙って消える**（`form main { [1. A] }`）**。
  **「LYS1015 が誰も出さない」はこの便で閉じた**（7 件だった）。
  ⇒ ★★★ **「0.3.0 の前でなければ不可能」な項目は 0 のまま。**
  ⚠️ ★★ **「絵が出ないのに黙る」形は、今日の引き算で 16 形中 15 形が塞がった**——
  **残る 1 形は §2F の新規項**。**次にこの形を疑うなら、綴りを思いつく前に
  `GRAMMAR.md` の項を総当たりして engraver に通すこと**（46 形・約 27 秒）。

---

## 2. 開いている作業

> # ▶ **打鍵の首位＝script の profile が置く綴りで作られている**（2026-08-17・第190 実測・**未修理**）
>
...
>
> **値段**: **articulation 1 個あたり約 0.15 MB・約 30 µs**（1 打鍵あたり）。
> 目安は **1000 script で約 40 ms・4000 で約 130 ms**。最悪の合成本 `perf-fingstack1k`
> （11,988 script）は **1 打鍵 3.3 GB / 430 ms**。
>
> **⑴ phase 二分**: 打鍵の **98% が注釈パス 2 本**
> （fingstack: prelim 1776.5MB ＋ final 1472.8MB。対して fingbeam 53.5+8.2、plain 38.3+2.9。
> collect / keys / springs / break / systems / render はどの本もほぼ同じ）。
> ⚠️ **アロケーションで割ること**——**決定的**（同じ本でレンダごとに 0.1 MB まで同値）。
> **時間では割れない**: 同じ run で**同一コードの対照が 16% 揺れた**。
>
> **⑵ 何が高いかは書き分けて測った**（同じ骨格の本を 4 形 × 125〜1000 小節）:
> **素の音符 0.03 MB/小節・運指のみ 0.04・articulation のみ 2.05・両方 2.32**、
> **どれも倍化ごとに ×2.0＝線形**（二次ではない）。⇒ ★★★ **`fingstack` という名前に反して運指は無実**、
> **articulation の定数が 50 倍**。1 音符あたりの script 数を 1→6 に振ると
> **1 script あたり 0.143 → 0.317 MB**＝**支え連鎖の上乗せは副次的で、主項は script 1 個の定数**。
>
> **⑶ 住所と直し方**: `ArticulationEngraver.ScriptSkylines`（**置いた pair を返す綴り**）が
> 呼ばれるたびにグリフ輪郭を歩いて `VerticalSkyline` を 2 つ作る。呼び手は **10 か所**＝
> `ArticulationEngraver` :515/:519/:996/:1058/:1064（script column の積み）と
> `OutsideStaffStacker` :306/:356/:1265/:1430（outside-staff パス）。
> ⇒ ★★★ **直し方は既に隣に書いてある**——同ファイル :1603-1610 が
> 「**512-script のページで 295 MB 対 box 綴り 139 MB**。**(glyph, size, design, pad) で
> キャッシュして `VerticalSkyline.Merge` で流し込むと box の数 % 以内**」と*測って*記録している。
> **その安い綴りは skyline の*ビルダー*にだけ入っていて、*スタッカー*は今も置く綴りを呼ぶ**
> ＝§2A「同じ量の 2 軒目」の性能側の顔。**発明ではなく指し直し。**
> ⚠️ **10 か所を一度に指し替えないこと**——**置く綴りは pair を*返す*ので、消費者は返り値を
> 読んでいる**（距離を取る／Y を比べる）。**merge 版は target へ流し込む形**なので、
> **消費者ごとに「何が要るのか」が違う**。**1 か所ずつ・毎回 SVG 全数ハッシュで不動を確かめる。**
>
