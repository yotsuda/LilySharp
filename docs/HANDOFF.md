...
⇒ ★★★★ **⑹ 次の一手**: **⒜ push はユーザー**（第355 の 8 本＋第356 の 7 本＋この便 3 本）。**⒝ 承認待ち＝⑶ の patch**（`git apply scratch/p358/keep-inside-line-frame.patch` → build → snapshot 10 枚を LP 照合のうえ再ベース（`LILYSHARP_UPDATE_SNAPSHOTS=1`）→ full → commit。LP の数は上 ⑶ と番人に在る）。**⒞ §3 に「決定済み・未実装」は 0**。**⒟ 第356 ⑹⒟ の拾い箱はそのまま**（跨ぐ音符を LP のように割る／梁を跨いで折る／双子は LYS1037 の本で LP と違う絵／段末の head の末尾 spring）。**⒠ 第355 ⑹⒞⒠⒡ の並びの残り**（F-fonts 拾い箱・`time none` の縁 ⑵⑶⑷・MIDI の途中 `partial` は閉じた・`lymelmel` は閉じた・`lyrics-verses` bar 4 は patch へ・拍グリッド／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8b（列の合体・承認要）・U8／A/B/D/E／C⑴／G・段端の floor の regime）。⚠️ scratch は `p358`。
