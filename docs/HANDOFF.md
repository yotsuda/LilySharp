...
終了時: **commit 2 本**（**本体 `d6bf1e1b` ／ docs（この commit）**）＝**未 push 31**（**この commit を含む。本体だけの時点で 30 だったのを確かめてから書いた**）・木 0・未追跡 0。**suite 6684 合格 / 0 失敗 / 4 skip / 合計 6688**（開始 6674 から **+14**＝**本便の網 15 ケース − 番人から消えた 1 行**）。Core 0 エラー 0 警告。**台帳・追跡コーパス・snapshot は開始時と同じ**（748／588／190／24.714570188／158 中 2／`OPEN:` 0／581 冊／231 枚）。⚠️ **`docs/APPROXIMATIONS.md` も `audit/magic_constants.csv` も 1 行も動いていない**——**本便は定数を 1 つも足していない**（**足した `LILYPOND-REF` は*決定*の出所であって量ではない**）。
★★ **WSL ubuntu Release も同じ 6684 / 0 / 4 / 6688**（本体 commit の木で 1 回）。⚠️ **CI は `11f48202` までしか走っておらず、未 push 30 本は未実行。**
