...
終了時: **commit 3 本**（`f9ab3cbc` code ＋ `#2` §1 と §2 と第290 のアーカイブ ＋ **この 1 本＝両脚の緑**）＝**未 push 15**・木 0・未追跡 0。**suite 6559 合格 / 0 失敗 / 4 skip / 合計 6563**（開始 6561 から **+2**＝**番人 2 本。台帳の点は 1 つも足していない**——**crop は LP に対応物が無いので点で測れない**）。Core 0 エラー 0 警告。**追跡 `.lys` 579 冊**（不動）・**snapshot 229 枚**（不動＝**5 枚を*書き換えた***）。**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2＝すべて不動。**
...
★★ **緑は*コード commit のあと*に取った**。★★ **WSL ubuntu Release も同じ 6559 / 0 / 4 / 6563**（`f9ab3cbc` の木で・§0 の「この機械の Windows での緑でしかない」に両脚で答えた）。⚠️ **CI だけは未実行**（未 push 15）。
