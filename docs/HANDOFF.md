...
最終更新 2026-08-01（第64セッション＝**引継ぎ ▶ の ⑼ を実行した**——
**度数和音（`<1 3 5>`・`<d 3 5 7,>`）を exporter が具体的な音高に解決して書くようにした**
（`9c924efc`・**Core の描画は 1 行も触っていない**）。
★★★ **これで「黙って音楽を落とす」形の exporter の穴はゼロになった**——
**残る警告 183 件は全部 装飾か `DrumNote`**（⑤ に実測・**数え方つき**）。
★★★ **双子 199 本のうち 196 本が byte 不変**・**動いた 3 本が度数の本**（② の補正が
度数和音の無い本では恒等的に 0 だから＝**設計がそう作ってある**）。
⚠️ **引継ぎの予告「比較できる本が 1 冊増える」は外れた**——`chord-octave-marks` は
**4分音符の和音だけで梁が両側とも 0 本**（④ に数え方つき）。**予告は測定ではない。**
**作業は `9c924efc`（度数）＋`96459386`（⑥ の 2 件）**（＋この handoff commit）・**未 push 24**・
テスト **3747 passed / 0 failed / 3 skipped**（**+8 は exporter のテスト**）・
...
**⑨ を塞いだ影響だけを数える**: **196 本は byte 不変なので LP 出力も定義から不変**——
**測り直す必要があるのは動いた 3 本だけ**（これが「196/199 byte 不変」を先に測る理由）。
```
比較できた本   39   ← 一致 31 / 不一致 8      （第63セッションから不変）
bar check      7    ← 8 から 1 減（chord-octave-marks が抜けた）
両側とも梁なし 153  ← 152 から 1 増
                    合計 199
```
★★★ **`chord-octave-marks` は「比較できる本」にはならない**——**LP 側の Beam grob 0・
Lily# 側の sweep 行 0**（両方実測）。**4分音符の和音 4 つに梁は立たない。**
★ **残る bar check 7 冊**: `05-special-techniques`・`barcheck`（わざと短い）・`beamlets`・
`dense-chromatic`・`drum-groove`・`tab-staccato-beam-side`・`tuplet-lower-staff`。

## ⑤ 残りの warning を棚卸しした（**183 件・全 199 本で実測**）

**度数の警告はゼロ**（塞がった）。**多いものから**:
`DrumNote not exported` **24**／`@arpeggio` 10／`@ped` 9＋`@ped.off` 9／`@glissando` 8／
`@cue` 6／`@portato` 6／`@finger.N` 15／`@fig.N` 11／`@chord.X` 14／`@text."…"` 4／
`chord row 'prog'` 4／`lyrics row 'words'` 3／`Override/RevertDeclaration` 各 1／
custom key 1。⇒ **音楽ストリームを落としているのは `DrumNote` だけ**（`drum-groove` の
bar check の出所）。**残りは装飾で、双子の音符列は変わらない。**
★ **和音の中の drum メンバー（`<bd hh>`）も今回 warning にした**——**実測 0 件**
（24 件は全部素の drum note）。**`<>` を黙って書く形をもう作らない、が理由。**
...
★★ **⑥⒝ の三者三様を決める**（**Lily# 側の設計判断＝ユーザーに訊く**）。**測定は済んでいる**ので、
決まれば直すのは collector か MidiExporter のどちらか一方。**描画が動く側なら snapshot 再ベース。**
★ **`DrumNote` の exporter 移植**（24 件・**残る唯一の「音楽を落とす」穴**）。
**閉じれば `drum-groove` の bar check が消える**（＝**比較できる本が 1 冊増える**・
⚠️ **今度は先に梁の有無を確かめること**——④ の外し方をもう一度やらない）。
★ **⑺ の grace の音価の既定はまだ決まっていない**（**1 つの綴り `grace { a b }` に既定が 3 つ**＝
レイアウト 1/8・MIDI 1/32・双子は「直前の音価」）。**Lily# 側の設計判断なのでユーザーに訊いてから。**
...
★ **測定器の宿題**: **sweep に tab 譜の frame を教える**（第63セッション ② の警告）。
...
★ **`instrument` は今も読まない・これは変えていない**——`instrument bass` は
**「ベース記号 ＋ octave 3 ＋ 実音 −12」の束**（**−12 は意図的**）で、**オクターブだけ写すと
書いた音は動いて鳴る音は間違ったまま**＝「**正しく見える間違い**」になる。
...
## ⚠️ `test/tab-as-numbers` は双子で測れない（**欠陥ではない**）
