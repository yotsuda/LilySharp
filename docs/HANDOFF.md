...
**⑹ 終了時**: **full `scratch/p370/run2.trx` 7699 / 0 / 4 / 7703（開始時 run1 と合計一致・HANDOFF／ARCHIVE 込み・RunInfos は SKIP 4 だけ・4 m 26 s）**・§7.5 Core `+` 0／REF 0／OWN 0・台帳 821・snapshot 249・追跡 `.lys` 599 不動・**捨て計器は `scratch/p370/ZzP370ProbeTests.cs.txt` へ退避済み（Tests に残っていない）**。commit＝この HANDOFF＋ARCHIVE だけ（`scratch/p370/commit1.txt`＝`47317e59`）・Deploy 無し（Core 不変）。**第 2 便（ユーザー「有利なら着手」→ p370 の道具がそのまま効くので着手）＝⑵⒜ を列で突き合わせて閉じた（上）**: **full `scratch/p370/run3.trx` 7699 / 0 / 4 / 7703（run1・run2 と合計一致・RunInfos は SKIP 4 だけ・4 m 9 s）**→ commit（この HANDOFF だけ・`commit2.txt`）。捨て計器 2 本は `scratch/p370/Zz*.cs.txt` に退避済み。push はユーザー（未 push 5）。
