...
そこから `using` が沈黙の最後の 1 つだと分かった便**（**実装 4 便**）。
...
終了時 **実装の最後は `5d492d5f`**・**未 push は
`git rev-list --count origin/master..master` が言う**・**未追跡 0**・
suite **5046 passed / 0 failed / 4 skipped**（+12＝新しい網ちょうど）・
**台帳 516 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・
**コーパス 0/81**（**worktree の base ビルドと HEAD ビルドで同じ 81 冊を描いた真の A/B**・
**陽性対照つき**＝1 音変えた本でハッシュが動くことを確かめてある）・
...
- **⑸ ⚠️ ★★★ この便が*直していない*で起票したもの＝`lysc check --pitches` が
  多 part の本で描画と違う音高を言う**。**RULES §5.3 判定法⑶ が「合成した本は着手前に
  これを 1 回通せ」と名指している計器**なので、**RULES §5.3 に注意書きを足した**。
  **実測 2 冊**（`test/multi-movement`・`test/grandstaff-high-bass`）で
  **低音部の頭が C6**＝上の part の相対鎖を引き継ぐ。**双子は `\relative c'` / `\relative c`
  で各 part が自分の clef のアンカーを持つ＝描画側が正しい**。
  **原因は構造**: `--pitches` は RenderSpec 無しの素の `collector.Collect(tree)`、
  `check` の validator は RenderSpec つきの `SemanticValidation.TryCollect`＝**同じ量の 2 綴り**
  （§5.2.1②）。⇒ **§2F に起票**。
- **⑹ ★ 毒は 5 方向**（§5.4）。**⑵＝警告だけ落とす→3 本赤**（diamond と cycle は緑のまま
...
- **⑺ ★ §7.5 の監査**＝第2便 Core +95 行 REF 0 / OWN 0・第3便 Core +16 行 REF 0 / OWN 0・
  **数値定数 0**。**どちらも正しい**——**この島は量を 1 つも計算していない**。
...
- **⑻ ★★★ §2F に残っているのは 3 つ＋1**——**⑴ 一覧に構文を与えるか（決定であって作業ではない）
  ⑵ 繰り返し縦線の ⑹ ⑶ `lyrics` の voice 束縛（要決定）**、＋**⑷ 新規＝⑸ の `--pitches`**。
  **「parse しない fixture 2 本」は ⑷ で閉じた。**
...
- ⚠️ ★★★ **新規＝`lysc check --pitches` が多 part の本で描画と違う音高を言う**
  （第182 実測・**未修理**）。**RULES §5.3 判定法⑶ が「合成した本は着手前にこれを 1 回通せ」と
  名指している計器**なので、**RULES §5.3 に注意書きを入れてある**。
  **実測 2 冊**（`test/multi-movement`・`test/grandstaff-high-bass`）で**低音部の頭が C6**
  ＝**上の part の相対鎖を引き継ぐ**。**双子は `rh = \relative c'` / `lh = \relative c`
  ＝各 part が自分の clef のアンカーを持つ＝描画側が正しい。**
  ⇒ **原因は構造**: `--pitches` は **RenderSpec 無しの素の `collector.Collect(tree)`**、
  `check` 自身の validator は **RenderSpec つきの `SemanticValidation.TryCollect`**
  ＝**同じ量の 2 綴り**（§5.2.1②）。**単一 part の本ではどちらも同じ答え**なので
  今まで見えなかった。⇒ **直すなら `--pitches` を `TryCollect` 側へ寄せる**のが素直だが、
  **`--pitches` は「描く前に効く」のが取り柄**なので、**RenderSpec が無い本での挙動を先に決める。**
