...
⑷ `IsSpecSpacedRowBoundary` は**残した**——行は解かれるようになったが、**解く前の配置**と
**行の上下に立つ要素**は今も枠の選択を要る。消すには tab/ossia を refpoint 枠へ通す必要があり、
それは §1 が前から名指している別島（コード側にも明記済み）。
...
- ⑷ **silhouette の「選択」はまだ要素基準**——両端とも text row の system は譜が 1 枚も入らない。
