...
★★★ **⑻ 数**: **開始時**: HEAD `1e80a9b2`・未 push 9・木 clean・未追跡 0（**`Session-Check -Build -Test` は走らせていない**）。**終了時**: HEAD `9e96174d`・未 push 12・**台帳 843 点／ss 非ゼロ 210／総和 23.216135909／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249／追跡 `.lys` 599＝全部不変**（**絵は 1 枚も動かない**＝触ったのは LSP と test だけ・**Core は 3 便とも 1 行も触っていない**＝§7.5 の数えは 0 行）。**full `scratch/p375/run5.trx` 7825 / 0 / 4 / 7829**（第 1 便末 `run1.trx` 7812 / 0 / 4 / 7816＝第373 末 7810 ＋ 6・第 2 便末 `run4.trx` 7823 / 0 / 4 / 7827。**合計を 3 回とも読んでいる**）。⚠️ scratch は **`p375`**。⚠️ §7 3.5: 第372 を `-Archive 372` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝**第372 の天井 commit `9f6df2a3` が未 push のまま**。**push はユーザー（未 push 12）**。★ **LSP は 2 度配布した**＝`tools/Deploy-Lsp.ps1` が `0.6.0+9e96174d` を `yotsuda.lilysharp-0.5.0-win32-x64\server` へ（95 ファイル byte 一致）⇒ **ユーザーは Reload Window が 1 回要る**（package.json は動かしていないので 2 回は不要・記憶 `feedback_lilysharp_lsp_deploy_target`）。

⇒ ★★★ **⑼ 次の一手＝*枠の綴りを全数で数える*（この便が作った未計数）**。3 便で**同じ族の読み落としが 3 件**出た（`chords` トラック本体／`sings` 節／沈黙 `~`）。共通の原因は **`BlockFrame` が `{` の前の 2 語しか持たない**ことで、**それを超える綴りは `ReadFrame` に 1 つずつ教えるしかない**（今 跨ぐのは **引用表示名**と**tilde** の 2 つ）。⇒ **GRAMMAR の「`{` を開く宣言」を全部並べ、各ヘッダが 2 語に収まるかを表にする**のが次の一手（**LP を 1 回も回さない・読むだけ**）。★ 判定材料として**この便で確かめた綴りは**: `part x {`／`part x "表示名" {`／`lyrics {`／`lyrics w {`／`lyrics w sings m {`／`chords p {`／`section A {`／`section ~A {`／`form f {`／`score s {`／`score s "out" {`／`fonts|paper|layout [name] {`／staff group 5 種。**残りは未点検**。⒝ 第373 ⑼⒜（probe 24 冊の仕分け）もそのまま先頭級／⒞ 第369 ⑸⒝（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。★ **判定の記録（ユーザーの常設指示）**: **第 2 便はユーザー起票**（判定の対象外）。**第 3 便は着手した**——**この便の道具でしか安くない**（end-to-end ハーネスと `GetCompletionContext` の dump 計器が温まっていて、**1 回の実行で 8 綴りを対で読める**）うえ、**⑹ を書いた時点で初めて「枠の綴り」という問いが生まれた**＝**前の便には存在しなかった未計数**。**着手しなかったもの**: ⑼⒜ の全数表は**読む量が大きく道具の温度が要らない**／⒝⒞ は**別 regime**＝**次のセッションでやっても損しない**。
