...
最終更新 第351セッション（2026-09-08）＝**入り方は第298〜第350 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。**第350 ⑹⒡⑴「双子（和音・歌詞つき）と LP の対の調査」に着手した**——65 冊の一括計器を建てて掃き、**最大の族＝「行の最初／最後の音節が小節線を避ける」LILYSHARP-OWN の半分（LP には無い）を移植で閉じ**、対の列比較で **2 つ目＝狭い音節の左 reach の 0 clamp** も閉じた。**出力が動く（snapshot 14 枚）ので RULES §5.1 に従い製品は作業ツリーに置いたまま承認待ち**（第350 第 10 便と同じ形）。**commit 1 本＝HANDOFF＋ARCHIVE**。**骨は 6**:

★★★ **⑴ 計器（`scratch/p352/`・次便も同じ手順で始められる）**: `lp/pair.ps1 -Lys <lys…> [-Lyr]`＝1 冊を `lysc ly`→LP 2.26.0 と `lysc svg` に通し、段ごとの小節線 X と幅を並べる（`-Lyr` で LP の `LyricText` の ext）／`lp/settings.ly`＝第333 の列計器＋LyricText 計器＋**serif/sans の pin**（第350 ⒟ の罠）＋`ragged-right`／`sw/sweep.ps1 [-ParseOnly]`＝第350 の `lyricchord-books.txt` 65 冊を一括（Lily# 側は `paper { raggedRight }` を頭に足した写し＝両側 ragged）→ `sw/summary.txt`・`sw/NN-*-detail.txt`。⚠️ **3 つの罠**: ⒜ **LP の「非対応の形式を無視 (pdf)」警告が `PROBEBAR`／`LYR` 行の途中に割り込み、数が次の行へ落ちる**（行単位で読むと x=0 の偽の小節線＝maxD 17 の偽陽性が 2 冊出た。生テキストに正規表現で当てる＝両 ps1 に入れてある）／⒝ **反復小節線（`.|:`／`:|.`）は両エンジンで参照点が違う**（±1.1 が対で出て、隣 2 小節の和は等しい）／⒞ **行だけの lead sheet は LP に BarLine grob が無く PROBEBAR 0 本**（7 冊＝未測。やるなら ALLCOL の `starter=()` 列で読む）。**最初の 1 冊 `sings-chorus-row` は対を正しく組んだら 10.046 / 16.87 / 8.15 対 10.05 / 16.90 / 8.15＝0.03 以内**——第350 の「bar 1-2 の残差」は英語行だけ・未 pin の c.ly を 2 行の Lily# と比べた対の artefact だった。
★★★ **⑵ 正体（2 つ・どちらも `LyricSpacing.cs`）**: **⒜ 小節線の半分**——`ReserveLyricLine` は行の最初の音節に「左 reach＋0.4」を小節線まで、最後の音節に「右 reach＋0.4」を次の小節線まで予約していた（LILYSHARP-OWN・コード自身が「LP はここに何も予約しない。ただし未測なので発明の 0.4 を残す」と書いていた）。**LP は音節と小節線の間に何も予約しない**（`LyricText` の spacing box は Y で 0.2 ずつ退く＝`define-grobs.scm:2213-2218`・縛るのは次の音節の rod だけ）。**実測 `lyrics-after-rest-bar`（休符小節の次に "Twin-" で始まる行）: LP は "Twin" のインクが小節線より 1.11 左から始まり、その小節 18.147。Lily# 19.67＝+1.523＝半分の不足分そのもの（2.335+0.4−1.222）**。`lyrics-volta`（verse 2 の頭）+1.6・`lyrics`（"how" が 0.55 はみ出す）も同じ。**直し＝半分は `keepEdgeHalves`（lead sheet・2026-08-20 のユーザー決定）のときだけ**。段の端の floor（`LineStartLyricFloor`／`LineEndLyricReservation`＝改行を跨ぐ行）は**触っていない**（その regime は今も未測・多段の対は段割れが違って組めない）。**⒝ 符号付き reach**——`GetLyricLeftExtent` が `maxExtent = 0` から max を取り、**符頭の alignment extent（4 分＝0.652）より狭い音節の負の reach（"I"＝−0.157）を 0 に clamp していた**。LP の rod は符号付き算術（`lily/rod.cc:56-67 bounds_protrusion`＋`add_to_cols`）＝"How I" は 3.213+0.45−0.157＝**3.505**、Lily# は 3.67（+0.16）。**直し＝先頭要素で seed**（`GetLyricRightExtent` も同形に）。**直後の対**: `lyrics` **9.83 / 14.99 / 8.15 対 LP 9.836 / 14.989 / 8.15（exact）**・`lyrics-after-rest-bar` 18.00 / 12.46 / 15.98 / 15.19 対 18.147 / 12.465 / 15.955 / 15.142（残りは Schola 対 C059 の字幅・rod 1 本 0.05）・`lyrics-volta` bar 4 18.29 → 16.86（LP 16.695）。
★★ **⑶ 番人 3 本 `LyricBarEdgeTests`**（page 級・LP の数を名指し: 小節幅 4 本が 18.147 / 12.465 / 15.955 / 15.142 の ±0.25／"Twin" のインク左端が小節線より左／"How I" の rod＝右 reach＋0.45＋負の左 reach＝3.505±0.1）。**毒**（Core だけ stash・`scratch/p352/poison1.trx`）: **3 本とも赤**。**対照＝lead sheet は `LeadSheetLyricSpacingTests` が半分を pin したまま緑**。⚠️ `SpacingInvariantTests.BreakGate_PricesTheCrossBarLyricRod` は本の語を "mum"→"mummum" に（半分が消えて対の最小が縮み、窓の前提 min+excess>ideal が 0.6 だけ外れた＝機構は不変・remark に理由）。
★★ **⑷ 射程**: **掃き `scratch/p352/sweep352.ps1`（`svg --combined`・921 冊＝追跡 597＋ユーザー 324・base＝HEAD `7cad95db` の exe `exe-base`／head）: MOVED 17＝追跡 14＋ユーザー 3**（`She Bangs`・`tab-low`・`ぐるぐるワンダーランド`）。**snapshot 14 枚が赤＝掃きの追跡 14 冊と一致**（`lyrics-after-rest-bar`・`named-voice-lyrics`・`rit-across-systems`・`lyrics-below-marcato`・`08-chorale`・`chordrow-rit-second-system`・`tab-lyrics-inside-strings`・`lyrics-verses`・`lyrics-volta`・`lyrics`・`sings-chorus-row`・`sings-per-row`・`lead-sheet`・`lyric-break-pricing`。`lead-sheet` は ⒝ の分＝lead sheet の半分は残るが reach の符号で 0.4 の余りが消える）。**台帳 810 不動（全点緑）**。**65 冊の掃きの表（`sw/summary.txt`）**: 対が組めた 42 冊のうち **≤0.05: 23・≤0.25: 6・>0.25: 13**。>0.25 の内訳＝**この便で閉じた半分の族 4**（15/19/20/21）／**和音名の文字幅（plain text 対 LP の記号・§3 既決）3**（`chordnames` 1.9・`chord-names-bass` 14＝`@text.down` も双子が落とす・ユーザー `bench` 2.8）／反復小節線 artefact 1（`volta-chord-row`）／**未分類 5（下 ⑹⒠ の拾い箱）**。不一致 23＝行だけ 7＋段割れ違い 16（多段の本は両側 ragged でも段割れが揃わない＝段の中の幅は比べられない）。
★ **⑸ 数**: **開始時裏取り**: HEAD **`7cad95db`**・**未 push 29**・`origin/master`＝`21efb993`（間で push 無し）・木 clean・未追跡 0・`--no-incremental` build 0 エラー／Core 0 警告・**full 7263 / 0 / 4 / 7267**（`scratch/p352/run1.trx`＝引継ぎと一致・10:32）・`gh run list` 5 本緑（最新 34121145135）。**終了時**: **commit 1 本（この HANDOFF＋ARCHIVE）・未 push 30・木は製品未 commit で*汚い*（M 4＝`LyricSpacing.cs`・`SpacingInvariantTests.cs`・`CHANGELOG.md`・`docs/APPROXIMATIONS.md`（`LILYSHARP_UPDATE_DOCS=1` で再生成・3 行）／?? 1＝`LyricBarEdgeTests.cs`）・full 7252 / 14 / 4 / 7270（`scratch/p352/run3.trx`・+3＝番人・赤 14＝snapshot だけ）**・台帳 810／snapshot 247／追跡 `.lys` 597 不動。§7.5: Core `+` 71・LILYPOND-REF 2（`define-grobs.scm LyricText extra-spacing-height`・`rod.cc bounds_protrusion`）・LILYSHARP-OWN 3（残る lead-sheet の半分と段端 floor の名指し・新規の定数 0）。**perf: 計算は足していない**（分岐 2 つの削除と seed の変更）。⚠️ **scratch は `p352`**（`p351` は第350）。⚠️ **§7 3.5**: 第349 のブロックを `scratch/p352/archive-349.ps1`（バイト保存）で ARCHIVE の先頭へ・第350 の経緯は下の見出しの下に残した。⚠️ **この機械は今日遅い**（full 10:32／掃き 1 パス 7 分・第350 の 4〜5 分ではない）。

**終了時（第 2 便＝承認後）**: **commit 2 本（`6dea0ab5`＝製品＋番人 3＋`SpacingInvariantTests` の語＋CHANGELOG＋APPROXIMATIONS＋snapshot 14 枚（`LILYSHARP_UPDATE_SNAPSHOTS=1` で動いたのはその 14 枚だけ・`scratch/p352/rebase.trx`）／この HANDOFF）・未 push 32・木 clean・未追跡 0・full 7266 / 0 / 4 / 7270（`scratch/p352/run4.trx`・env 無し）**・台帳 810／snapshot 247（枚数不動・14 枚再ベース）／追跡 `.lys` 597 不動。

✅ ★★★ **第 4 便（ユーザー「手順と置き場所を工夫して、儀式にかかる時間を減らして」）＝儀式を道具に**: **⒜ `tools/Session-Check.ps1`**（git・§0 の全部の数・gh・`-Build`・`-Test -Scratch pNNN`＝trx の合計とホスト死の RunInfos・`-DiffBase`＝§7.5 の `+`/REF/OWN・`-Archive N`＝§7 3.5 の移動）／**⒝ `tools/Fold-ClosedHandoffItems.ps1`**＝§2 の「✅ 閉じた」項目の*本文*を ARCHIVE の「閉じた §2 の本文」へ逐語で落とし 1 行目をポインタに（冪等）——**この便で 64 項目・1,923 行を落とした（HANDOFF 4,300 → 2,301 行・617 → 407 KB）**／**⒞ §0 を 9,442 → 1,741 字に**（数え方 10 例は RULES §6.1 の表へ・罠は 1 行ずつ）／**⒟ 門 `HandoffArchiveContinuityTests.TheHandoffStaysReadable`**（HANDOFF 450 KB・§1 現在便 20,000 字。上げて通すのは禁止）。**次便の入り方＝§0 の 1 コマンド。**

⇒ ★★★★ **⑹ 次の一手**: **⒜ push は禁止のまま**（未 push は終了時の行）。✅ **⒝ ~~承認待ち＝この便の製品コード~~ → ユーザー「リベースは承認する」で第 2 便に再ベース・commit 済み（`6dea0ab5`・message に ⑵ の LP の数 18.147／3.505 と MOVED 17 を名指した。台帳キーは無い＝横の spacing は番人が観測者・第333/第350 と同じ）**。**⒞ 第350 ⑺⒝ の並びの残り**（~~⒡⑴ 双子と LP の対の調査~~＝この便／⒡⑵ 行の拍グリッド＝実需ゼロ・起票のみ／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8b・U8／A/B/D/E／C⑴／G）。**⒟ 段端の floor の regime**（`LineStartLyricFloor`／`LineEndLyricReservation`＝改行を跨ぐ行の inkL/R＋0.4）は**今も未測**——LP は `keep-inside-line` の rod（ink だけ・0.4 無し・Lily# は `InkReachPerColumn` で移植済み）だけのはず。測るには段割れを揃えた 2 段の対が要る（`lyric-break-pricing` は段割れが違う）。
**⒠ 拾い箱（掃きの未分類・着手していない）**: **⑴ melisma の左寄せ**——`lyrics-verses` の bar 4（`saved~`／`grace~`・8.44 対 LP 8.15）: Lily# は `~` の音節を左寄せ、双子の `\lyricmode` は melisma を持てず、LP は `\lyricsto`＋スラーでしか左寄せしない＝**対にならない**。page の規則（`~`＝左寄せ）が LP の規則（スラーの melisma だけ）と同じかは*文法の問い*→ ✅ **ユーザー決定 2026-09-08＝`~` は Lily# の規則として残す**（LP は `\lyricsto`＋スラー／タイ／`\melisma` から melisma を読む・Lily# は歌詞側の `~`＝意図的乖離・§3）。**対は双子側 (B)＝`~` の音節の前に `\once \override LyricText.self-alignment-X = #LEFT` を書く**（`EmitLyricTracks`・十数行）。`\lyricsto` 化 (A)・ページがスラーから melisma を導く (C) は採らない（ページ側の束縛 `sings` と melisma の LEFT 寄せ `LyricEngraver.cs:1926-1935` は既に LP の移植）。**⑵ `雪やこんこ`（ユーザー・3 譜）**: bar 2/4 の小節線→最初の列が LP 2.396 対 Lily# 1.40（−1.0）・歌詞ではなく他の譜の何か（未特定）。**⑶ `tab-with-chords`** bar 2 +1.1（tab＋和音行＝§2 T の島）。**⑷ `nav-below-clears-lyrics`** −1.04（LP が広い・双子の `\mark` 2 つが同じ moment で LP が片方を捨てる警告＝対が壊れている可能性）。**⑸ `rit-across-systems`** sys1 +0.6（双子が `@rit` を落とす・Lily# は text script を spacing に入れているか？ LP の TextScript は `extra-spacing-width (+inf . -inf)`＝spacing に参加しない。`chord-names-bass` の `@text.down` も同じ問い）。**⑹ `greensleeves`** 0.957（4 段・partial＋和音行＋歌詞・未分類）。✅ **⑺ 行だけの lead sheet 7 冊＝第 3 便（「有利なら着手」）で測った**（`sw/sweep.ps1` が PROBEBAR 0 本のとき ALLCOL の `mlen=#<Mom` 列＝小節の NonMusicalPaperColumn を段ごとに読む腕を足した・`-ParseOnly` で既存 log から）: **Lily# が一様に広い**——`lead-sheet-chords` 15.52 / 12.38 / 10.49 対 LP 9.561 / 9.26 / 5.798・`lead-sheet-lyrics` 23.96 / 13.18 対 18.881 / 11.04・`lead-sheet` 21.84 / 15 / 17.63 / 10.19 対 16.759 / 11.64 / 14.546 / 6.22・`lytie` 12.9 対 8.771（`rows-song-sheet`・`drunken-sailor` は段割れ違い）。**⇒ 欠陥ではなく設計**: 行だけの本の LP 双子は小節線の無い素の `ChordNames`／`Lyrics` の 1 行で、Lily# のグリッド（小節線が帯を貫く・`EnsureLeadSheetBarWidth` の床・`ApplyRowCommandColumnSprings` の 0.5・ユーザー決定の小節線の半分）に LP の対応物は無い（§3・RULES の LILYSHARP-OWN）。**この対は「差の量」を刷るだけで、縮める根拠にはならない**——起票を閉じる。**⑸ の補足**: Layout で `Advance` を横 spacing に入れている家は歌詞・和音名・tuplet 数字（縦 skyline）だけ＝**text script（`@rit`／`@text`）は Lily# でも横 spacing に参加していない**（grep 実測・LP と同じ）。`rit-across-systems` の +0.6 は未分類のまま。

★★★★ **⒡ ユーザー決定便（2026-09-08・第351 の後・製品 0 変更・commit 無し）＝§2 の「ユーザー判断待ち」9 件をユーザーが一括で決めた。** 決定は §3 の 2026-09-08 行 7 本、各項には ✅ を付けた。**次便の材料（並びは私の案・ユーザーの優先度は未指定）**: ⑴ T6 ピックアップ census 8 冊＝弱起を前 section へ戻す（コーパス側・`.ly` と同じ絵・§2 T6）／⑵ 途中の `partial` を許す＝`PartialScopeValidator` の緩和＋番人・パートごとに書く規則を GRAMMAR に（§2F ⒱）／⑶ 双子の `~` 音節に `\once \override LyricText.self-alignment-X = #LEFT`（上 ⒠⑴）／⑷ `time none` を彫る（§2F ⒤・fixture＋LP 双子の対から）／⑸ `removeEmpty` を `staff m as lines 1 removeEmpty all` へ・pedal は part のまま（§2F ⒣）／⑹ LYS1033 の文面（§2F ⒩）／⑺ `_ "text"` の診断文面（§2 の同項）／⑻ ⒵ は閉じた（§2 の同項）。⚠️ **T6⒞（`@mark` 438 札）は第332 第 3 便で apply 済みだった**——§2 の行が第351 まで「未 apply＝ユーザー判断」のまま stale で、今日ユーザーに「apply する」と答えさせてから ARCHIVE で気付いた（行は直した）。**§2 の「▶」を読むときは ARCHIVE の同じ便の「実行した」行も grep すること。**

## 以下は第350セッションの経緯

最終更新 第350セッション（2026-09-08）＝**入り方は第298〜第349 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。**第349 ⑺⒝ の並びを順に読み、先頭 2 つ（拾い箱 ⑺⒜＝ユーザーの本と手順待ち／`lysc ly` の ChordNames＝綴りの決定待ち）はユーザー待ちなので飛ばし、3 つ目「lyric row の slot の投票」（第333 ⑶⒟ が「未測」と起票し、第334〜349 の 16 便が並びに置いたまま運んだ項）に着手し、閉じた**——**出力が動く（snapshot 1 枚）ので RULES §5.1 に従い第 1 便は製品コードを作業ツリーに置いたまま承認を待ち、第 2 便で拾い箱 ⒟ を測って着手せず（下）、第 3 便＝ユーザー「リベースは承認する」で 1 枚再ベースして製品を 1 commit にした**（第333 第 1〜2 便と同じ形）。**第 4 便（ユーザー「このセッションでやる方が有利なら着手」の 3 度目）＝拾い箱 ⒟ に着手し、製品ではなく LP 側の計器の罠と分かって閉じた（下 ⑹⒟）**。**第 5 便＝ユーザーが「ユーザー待ち」2 件に答え、拾い箱 ⑺⒜ を閉じ、`lysc ly` の ChordNames を案A で実装した（下 ⑹⒠）。第 6 便＝inline `@chord` も双子へ（page の model に訊く・同 ⒠）。第 7 便＝歌詞も双子へ（同じ形・同 ⒠）。第 8 便＝65 冊を LP に通す網→和音行の弱起の欠陥を閉じた（同 ⒠）。第 9 便＝その網の残り 2 件＝section 境界で双子が音価を引き継ぐ欠陥を閉じた（同 ⒠）。第 10 便＝行の小節が音楽より長い page の欠陥（弱起）を直し、第 11 便でユーザー承認→commit（⑹⒝′）**。**commit 5 本＋第 5〜9 便の各 2 本＋第 10 便の 1 本＋第 11 便の 2 本**（HANDOFF＋ARCHIVE／HANDOFF（第 2 便）／`3e3d5890`＝製品＋番人 4＋CHANGELOG＋snapshot 1／HANDOFF（第 3 便）／この HANDOFF）。**骨は 6**:

★★★ **⑴ 正体**: `SpacingRules.CommonShortestDuration` は text row の spacer slot を「和音記号の grob の代役」として投票させていて、**lyric row（`IsLyricsTextRow`）も同じ腕を通っていた**（コード自身が「A LYRIC row's slot would not vote in LilyPond; it still does here, unmeasured」と*自白していた*）。**LP は `lily/spacing-engraver.cc:176-183 add_starter_duration` の冒頭で `lyric-syllable-interface` を持つ grob を追い返す**（`LyricText` は rhythmic-grob-interface を持つので acknowledge はされる＝`scm/define-grobs.scm:2213-2236`）。**投票していた量は音符の鳴らない音価**: 独立行は小節を音節数で等分した slot（5 語なら 1/5）、bound 行（`sings`）は旋律の骨格の写し——**旋律自身が投票するので無害に見えるが、旋律の `R1` を骨格は「全音符の spacer」に綴り直す**（`IsFullMeasureRest` は spacer を弾く）ので**休符小節ごとに全音符を投票していた**。**直し 1 行**: `voiceMeasures.Where(v => !v.IsLyricsRow)`（chord row は残す＝`ChordName` は rhythmic grob・lead-sheet の recipe がその投票で測られている）。
★★ **⑵ 番人 4 本 `CommonShortestDurationTests`**: bound 行 ＋ `c8×8 | R1*3`（行あり／なしとも 1/8。投票させると mode が全音符→3/16 の cap）／独立行 8 語 over 4 分（行あり／なしとも 3/16。投票させると 1/8）／対照＝chord row 8 cell は 1/8 のまま／**page 級**＝1 字音節の bound 行を足しても sax の小節幅が不動で、**LP の 9.836 / 13.336 / 8.150 に 0.01 以内**。**毒**（Core だけ stash・`scratch/p351/poison1.trx`）: 新番人 2 本が赤・対照は緑。
★★★ **⑶ LP 照合（`scratch/p351/lp/`・LP 2.26.0・`allcol-settings.ly`＝第333 の計器の写し・ragged-right）**: **a（sax だけ）／b（＋`\lyricmode` の 8 分音節）／d（＋Devnull の vocal に `\lyricsto`＝fixture の `sings` の形・1 字音節）の 3 冊とも `common-shortest-duration 3/16`・小節線 X の刻み 9.836 / 13.336 / 8.150＝*1 桁も動かない***（c＝d の英語音節版は bar 2 が 14.061＝音節幅の押し・投票ではない）。**Lily#**: `sax-only.lys` 9.84 / 13.33 / 8.15（LP と一致）／`sings-narrow.lys`（＝d の双子）**base 11.24 / 16.14 / 8.85 → head 9.84 / 13.33 / 8.15**。⚠️ LP のログに「プログラミングエラー: file name not normalized: …allcol-settings.ly」が出るが PROBEGS/PROBEBAR は刷られている（第333 も同じ計器で測っている・無害）。
★★ **⑷ 射程（`scratch/p351/sweep351.ps1`・`svg --combined`・921 冊＝追跡 597 ＋ ユーザー 324・base `56257065` の exe／head）**: **MOVED 1＝`test/sings-chorus-row` だけ**（sax 4 分の上に `sings` 2 行＝骨格の 8 分が投票していた本。base 11.29 / 18.31 / 8.85 → head 10.05 / 16.90 / 8.15・LP の c は 9.837 / 14.061 / 8.15＝**bar 3 exact・bar 1-2 に残るのは 2 行の音節幅（CJK 込み）の押し＝別の島**）。**ユーザー実コーパス 0 冊**（`lyrics … sings` を行として置く本は 5 冊在るが、旋律の隣に置いて折り畳まれる＝row ではない）。snapshot は 1 枚（承認待ち・未再ベース）・台帳 810 不動・追跡 `.lys` 不動。
★ **⑸ 数**: **開始時裏取り**: HEAD **`56257065`**・**未 push 10**・`origin/master`＝`21efb993`（間で push 無し）・木 clean・未追跡 0・`--no-incremental` build 0 エラー／Core 0 警告・**full 7235 / 0 / 4 / 7239**（`scratch/p351/run1.trx`＝引継ぎと一致）・`gh run list` 5 本緑（最新 34121145135＝`21efb993` の CI）。**終了時（第 1 便）**: commit 1 本（`93cf43e0`＝HANDOFF＋ARCHIVE）・未 push 11・木は製品未 commit で*汚い*（M 3）・**full 7238 / 1 / 4 / 7243（`scratch/p351/run3.trx`・+4＝番人 4・赤 1＝`SvgSnapshotTests … test/sings-chorus-row`）**。**終了時（第 3 便＝承認後）**: **commit 2 本（`3e3d5890`＝製品＋番人 4＋CHANGELOG＋snapshot 1 枚（`LILYSHARP_UPDATE_SNAPSHOTS=1` で動いたのはその 1 枚だけ・`rebase.trx`）／この HANDOFF）・未 push 14・木 clean・未追跡 0・full 7239 / 0 / 4 / 7243（`scratch/p351/run4.trx`・env 無し）**・台帳 810／追跡 `.lys` 597 不動・snapshot 247 枚（枚数不動・1 枚再ベース）。**終了時（第 4 便）**: commit 1 本（この HANDOFF）・**未 push 15**・木 clean・未追跡 0（捨て計器は `scratch/p351/` へ退避・製品 0 変更＝full は run4 のまま 7239 / 0 / 4 / 7243）。**終了時（第 5 便）**: **commit 2 本（`1332d870`＝exporter＋`ChordStructure`＋`ChordNameCollector`＋番人 8＋CHANGELOG＋`docs/APPROXIMATIONS.md` 再生成（行番号）／この HANDOFF）・未 push 17・木 clean・未追跡 0・full 7247 / 0 / 4 / 7251（`scratch/p351/run7.trx`・+8＝番人）**。**終了時（第 11 便＝承認後）**: **commit 2 本（`d5295916`＝`MeasureCollector`＋`RowBarLengthTests` 4＋CHANGELOG＋`docs/APPROXIMATIONS.md`／この HANDOFF）・未 push 28・木 clean・未追跡 0**（full は run13 のまま 7263 / 0 / 4 / 7267）。**終了時（第 10 便）**: **commit 1 本（この HANDOFF）・未 push 26・木は製品未 commit で*汚い*（M 3＝`MeasureCollector.cs`・`CHANGELOG.md`・`docs/APPROXIMATIONS.md`／?? 1＝`RowBarLengthTests.cs`）・full 7263 / 0 / 4 / 7267（`scratch/p351/run13.trx`・+4＝番人）**・§7.5: Core `+` 77・REF 1（`Timing_translator`）。⚠️ ラチェット `CitationsThatNameNothing` に 3 度目——名前は住所と*同じ行*（折り返しで次行に落ちた）。**終了時（第 9 便）**: **commit 2 本（`ce5c8bb3`＝exporter＋番人 1＋CHANGELOG＋`docs/APPROXIMATIONS.md`／この HANDOFF）・未 push 25・木 clean・未追跡 0・full 7259 / 0 / 4 / 7263（`scratch/p351/run11.trx`・+1）**・§7.5: Core `+` 18・REF 1（`optional_notemode_duration`）。**終了時（第 8 便）**: **commit 2 本（`ed912c23`＝exporter＋番人 1＋CHANGELOG＋`docs/APPROXIMATIONS.md`／この HANDOFF）・未 push 23・木 clean・未追跡 0・full 7258 / 0 / 4 / 7262（`scratch/p351/run10.trx`・+1）**・§7.5: Core `+` 29・REF 0。**終了時（第 7 便）**: **commit 2 本（`62656e52`＝exporter＋番人 5＋CHANGELOG＋`docs/APPROXIMATIONS.md`／この HANDOFF）・未 push 21・木 clean・未追跡 0・full 7257 / 0 / 4 / 7261（`scratch/p351/run9.trx`・+5＝番人）**・§7.5: Core `+` 188・REF 0（第 6 便と同じ理由）。**終了時（第 6 便）**: **commit 2 本（`1d4e6c90`＝exporter＋番人 5＋CHANGELOG＋`docs/APPROXIMATIONS.md` 再生成／この HANDOFF）・未 push 19・木 clean・未追跡 0・full 7252 / 0 / 4 / 7256（`scratch/p351/run8.trx`・+5＝番人）**。§7.5（第 5 便）: Core の `+` 行 436・LILYPOND-REF 7（`construct-chord-elements` ×3・`default-chord-modifier-list`・`new_chord`・`duration`／`steno_duration`・`ignatzek-chord-names`）・LILYSHARP-OWN 0。⚠️ **ラチェット `CitationsThatNameNothing_DoNotGrow` に 2 度捕まった**（`noChordSymbol`・`interpret-bass` は記号として数えられない＝hyphen は 3 節・underscore は 2 節が要る。§7 の規則どおり引用先を読んで囲む関数名を書いた）。**page/svg・snapshot・台帳は不動**（exporter だけ）。§7.5: Core の `+` 行 22・LILYPOND-REF 1（`add_starter_duration`）・LILYSHARP-OWN 0。⚠️ **Co-Authored-By は付けていない**（2026-07-06 のオーナー決定）。⚠️ **scratch は `p351`**（`p350` は第349）。⚠️ **§7 3.5**: 第348 のブロックを `scratch/p351/archive-348.ps1`（バイト保存）で ARCHIVE の先頭へ・第349 の経緯は下の見出しの下に残した（継ぎ目の番人は緑）。⚠️ **ripple の罠（3 度目）**: 別 console が busy のときの自動ルーティングで `Set-Location` だけ走り本体が落ちた（memory の既知・打ち直しで正常）。

⇒ ★★★★ **⑹ 次の一手**: **⒜ push は禁止のまま**（未 push は終了時の行）。✅ **⒝′ ~~承認待ち（第 10 便）~~ → ユーザー「承認する。commit して」で commit 済み（第 11 便・下 ⑸）**。
✅ **⒝ ~~承認待ち＝この便の製品コード~~ → 第 3 便で承認・再ベース・commit 済み（`3e3d5890`・message に ⑶ の LP の数を名指した。台帳キーは無い＝横の spacing は台帳ではなく番人が観測者・第333 と同じ）。****⒞ 第349 ⑺⒝ の並びの残り**（~~拾い箱の ⑺⒜~~＝**第 5 便でユーザーが閉じた**（下）／~~`lysc ly` の ChordNames~~＝**第 5 便で案A を実装**（下 ⒠）／~~lyric row の slot の投票~~／**⒡⑴ 双子（和音・歌詞つき）と LP の対の調査**／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8b・U8／A/B/D/E／C⑴／G）。
✅ ★★ **⒠ 第 5 便＝ユーザーが 2 つの「ユーザー待ち」に答えた**: **⑴ プレビューの拾い箱（第341 ⑺⒜）＝ユーザー「私がデプロイした。直っているように見える。そのまま閉じて」→ 閉じた**（第341 ⑵ の `.0` 名の修理と第342 ⑿ のタブ閉じの穴の 2 つで報告は説明がつく・再現の本と手順はもう要らない）。**⑵ `lysc ly` の ChordNames＝案A（§3 に決定を記録）を同便で実装した**: `EmitChordTracks`＝chords row の part ごとに `<part>Chords = \chordmode { … }` を part 変数の後に書き、`EmitScore` が行の位置に `\new ChordNames \<var>` を置く（interior 行が下の譜へ畳まれる Lily# の読みと LP の `<< >>` の順は同じ）。**form の歩きは音楽と共有**（`AppendFormItems`／`AppendSection` に `_chordTrack` の腕＝section の play は和音の小節だけ・`\mark`／key・meter の復帰は音楽 stream のもの・`FormWalk.Other` は `break` だけ通す）。**小節の綴りは page と同じ grouping**（`ChordNameCollector.SlotGroups` を public static に切り出し、`ForEachSlotGroup` もそれを読む＝1 綴り）→ `ChordBarMarker`（green に文字列・`SectionPlayMarker` と同じ理由＝`CreateEnding` の green 再構築を生き延びる）。**entry の綴り**＝`ChordStructure.ToChordMode(duration)`（Dutch root＋音価＋`ChordQualityRegistry.LilyPondModifier`＋`/bass`）・roman は section header の key か score の key で解く（`ChordKeyFor`）・未登録 quality は root だけ＋警告・`r`→`r`（LP の `noChordSymbol`＝"N.C."）・小節頭 `.`→`s`・音価は `1 2 4 8 …`／付点／それ以外 `1*n/d`。**番人 8 本 `LilyPondExporterTests.ChordRow_*`＋`EveryChordQuality_HasAChordmodeModifier`**。**LP 2.26.0 実測**（`scratch/p351/cm/run-lp.ps1`・`lp-names.ly`＝ChordName の文字列を刷る計器）: **全 29 quality の本 `qualities.lys`＝exit 0・警告 0**（`C7sus4`→"C7sus4"・`Cm7-5`→"Cø"・`Cadd9`→"Cadd9"・`Cm13`→"Cm9add13"＝LP の名前）／`bench.lys`・`Lambada Complicada.lys`・`lead-sheet-repeat`・`rit-across-systems`（roman の `Im7 VIImaj7` が `g:m7 f:maj7`）・`雪やこんこ`（score に行を置いていない＝双子も無し・page と同じ）**すべて exit 0**。✅ **第 6 便（「このセッションでやる方が有利なら着手」の 4 度目＝exporter の和音機構・LP の計器・`bench.lys` の対が全部この便に在る）＝inline `@chord` も閉じた**: 音価から組み直す読み手は要らなかった——**page の model に訊く**（`EmitInlineChordTracks`＝`@chord` を書く本だけ `MeasureCollector.CollectMultiStaff` で collect し、`MultiStaffScore.ChordNames` の `UseTiming=false` を part ごとに、小節長＝その part の primary voice の items の和（pickup は短い・空 placeholder は meter）で `s` を埋めた `\chordmode` 変数に。bare `@chord` は page と同じ認識・度数も page の解決）。**その part の最初の行（staff／full の tab／group の staff）の上に `\new ChordNames`**・numbers-only の tab には置かない（page が刷らない）・`EmitMark` は `@chord` を bare の音符として書く（"dropped" 警告は消えた）。**model の小節列と双子の小節列は一致する**（両者とも `\repeat volta`／ending を展開せず・再登場は 2 度書く）。番人 5 本 `InlineChord_*`。**LP 実測**: `bench.lys`（第334 が `bench-cn.ly` を*手で*作っていた本）→ `basslineInlineChords`＝`s1 | fis1:sus4 | e1:maj7 |` と `progChords`＝`c1:maj7 | d1:maj7 |` の 2 context・LP 名 "C D F♯sus4 E△"・exit 0／`07-lead-sheet` C Am F G7・exit 0。§7.5: Core `+` 164・REF 0（LP の計算は無し＝配置は page・綴りは `ToChordMode` の家）。
✅ **第 7 便（「有利なら着手」の 5 度目）＝歌詞も双子へ**（双子は添付分も含めて歌詞を 1 音節も持っていなかった＝fixture 23 冊・ユーザー 6 冊が盲・今日の dy/sing の対も手書き双子だった）: **第 6 便と同じ形＝page の model に訊く**（`EmitLyricTracks`＝`PageModel` を共有し、`MultiStaffScore.Lyrics` を (staff, voice, verse, row) の線ごとに `\lyricmode` 変数へ・各音節は置かれた Timing から次の音節（か小節末）までの音価を持つ＝melisma は長い 1 音節・隙間は `\skip`・連結は page の `--`／`__`・音節は英字なら裸・それ以外（アポストロフィ・数字・空白）は引用符）。**`\lyricsto` も voice 名も要らない**——3 種（添付・`sings` 行・独立行）が 1 つの読みで、LP は音価で語を置く（今日の b.ly がその形の probe）。添付は譜の直下（group の譜も）・行はその位置。⚠️ **LP の識別子は英字のみ**＝verse は `LyricsOne/Two`（`SanitizeVar` が数字を落として 4 本赤になった）。⚠️ **畳まれた添付行（`lyrics X sings P` を譜の直下に置いた形）は row spec に無い**ので、警告は `PageKeepsLyricsRow`（spec に行が残るときだけ）。番人 5 本 `AttachedLyrics_*`／`LyricsRow_*`。**LP 実測 7 冊 exit 0**（`07-lead-sheet`・`lyrics-verses`・`sings-chorus-row`・`lyrics-volta`・`named-voice-lyrics`・`雪やこんこ`・`ぐるぐるワンダーランド`。最後の 1 冊の bar check 1 件は音楽 stream 側＝その本の音価持ち越しの既存差・歌詞ではない）。**運ばないもの（CHANGELOG に明記）**: verse 2 以降の番号（`\set stanza`）・extender の終点（page は最後の held head・LP は次の音節まで）。
✅ **第 8 便（「有利なら着手」の 6 度目＝一括確認の道具が在るうちに網を掛けた）**: **歌詞か和音を書く 65 冊（追跡 54・ユーザー 11・`scratch/p351/cm/lyricchord-books.txt`）を `lysc ly` → LP 2.26.0 に通した（`run-lp.ps1`・15 分）: 65 冊 exit 0・エラー 0・bar check 4 件**。**うち 2 件が第 5 便の欠陥**（`samples/amazing-grace`・`greensleeves`＝`partial` の本で行が先頭 `|` で開く形）: 空の和音小節（bare `|` が閉じる小節・page の規則）を `\chordmode` に音価なしの `|` だけで出していた→**空小節は `s<拍子>`、play の先頭小節は section header の `partial` の長さ（`ChordPickupFor`）、書かれた和音がある弱起小節は slot を pickup/meter で縮尺**。番人 +1（`ChordRow_AnEmptyBar_IsSilent_AndAPickupBarIsAsShortAsThePartial`）・2 冊とも bar check 0 に。**残る 2 件は音楽 stream 側の既存差**（`ぐるぐるワンダーランド` L67・`Lambada Complicada` L110＝音価の持ち越し・歌詞／和音ではない）。⚠️ **stanza 番号は入れない判断**: page は「同じ system・同じ譜に verse が 2 つ以上積むときだけ・system ごと」に刷る（`StanzaNumberEngraver`）が LP の `\set stanza` は line の先頭 1 度だけ＝機構が違い、どちらに寄せても対にならない。✅ **第 9 便（「有利なら着手」の 7 度目）＝第 8 便の残り 2 件の bar check＝双子の実欠陥を閉じた**: §3「section 頭で音価を書かない音は 4 分」（2026-09-04）は collector（`MeasureCollector.Form.cs:281` が play ごとに `_defaultDuration` を 4 分へ）にだけ在り、exporter の `EmitSectionPlay` は音価の carry を戻していなかった→LP は前 section の最後の音価を引き継ぐ（`parser.yy:3503-3515`）ので、`<des' fes>1 |` の次の section の `aes aes'` が全音符 2 つになって bar check 1/2。**直し**: 境界で `_lastWrittenValue/_lastWrittenDots` を 4 分に戻し、直前が素の 4 分でなければ次の音に強制書き（`aes4`）。番人 1 本（＋対照＝4 分の後は書かない）。**射程＝`lysc ly` 全数掃き（`scratch/p351/ly/sweep351ly.ps1`・921 冊・base＝HEAD `ceacd190` の exe を stash で建てた）: MOVED 4＝全部ユーザー本**（`Locked out of Heaven`・`STILL LOVE HER`・`ぐるぐるワンダーランド`・`君の恋人になったら（リードシート）`）・追跡 0 冊・snapshot／page 不動。⚠️ 第 8 便の一括確認で「`_________________.ly:110`」と読んだ 17 字の本は Lambada ではなく `君の恋人になったら（リードシート）`（run-lp.ps1 が非 ASCII を `_` にする）＝MOVED に入っている方で辻褄が合う。
✅ **第 10 便（「有利なら着手」の 8 度目）＝第 8 便の拾い箱を測ったら page の欠陥で、直した（承認待ち）**: **「たぶん無害」は外れ**——`amazing-grace`（3/4・`partial 4`・行は `| G | …`）の弱起小節は**行あり 20.46 ／ 行なし 14.12 ／ LP 14.585**（`scratch/p351/pk/`・`ls-widths.ps1`）＝**行のせいで 6.34 ss 広く、後続の小節が縮んでいた**。**正体**: 行は score の拍子でグリッドを組む（`ChordRhythm`・even spread）ので、弱起小節でも**拍子の全長の spacer**を置き、その小節が全長で値付けされる。**拍子変更を含む section の行も同じ**（行は `_meta.TimeBeats` しか知らない）。**直し＝`MeasureCollector.FitRowsToMusicBars`**（行の収集後・音楽 voice の各小節の items 和を最大で取り、行の小節がそれより長ければ spacer を share で縮尺＝第 8 便で双子に入れた規則の page 版・REF `engraver-init.ly:756-759 \alias Timing / Timing_translator`＝Score の Timing 1 つ）。**直後の実測: 行あり ≡ 行なし（小節線 X バイト同一）**。番人 4 本 `RowBarLengthTests`（弱起・拍子変更・歌詞独立行の share 縮尺・page 級「行あり＝行なし」）。**射程**: `partial`＋行を持つ本は追跡 2 冊（`samples/amazing-grace`・`greensleeves`）・ユーザー 0（拍子変更＋行は掃きで数える・下）。**snapshot は動かない（samples に snapshot 無し）が page 出力が動く→RULES §5.1 の承認待ち＝製品は作業ツリーに置いたまま**。**掃き（`sweep351.ps1`・`svg --combined`・921 冊・base＝朝の HEAD `56257065`）: MOVED 3＝`sings-chorus-row`（第 1〜3 便の投票の分・承認済み）＋ `amazing-grace`・`greensleeves`＝この便の 2 冊ちょうど・拍子変更＋行の本は 0**。**承認後**: `git add LilySharp.Core/Svg/Collector/MeasureCollector.cs LilySharp.Tests/RowBarLengthTests.cs CHANGELOG.md docs/APPROXIMATIONS.md` → 1 commit（message に上の実測 20.46 / 14.12 / 14.585 を名指す）。戻すなら `git checkout -- LilySharp.Core CHANGELOG.md docs/APPROXIMATIONS.md; Remove-Item LilySharp.Tests/RowBarLengthTests.cs`。 ⚠️ **対象外のまま**: section header 以外の場所の `key`（part cell の中の転調は roman の解決に見えない・page は見える）・拍グリッドは score の拍子（collector と同じ）。**出力（page/svg）は 1 バイトも動かない＝exporter だけ**。
**⒡ 拾い箱（第 12 便＝「有利なら着手」の 9 度目で*着手しない*と判断・次便の材料）**: **⑴ 双子が和音行・inline `@chord`・歌詞を持つようになったので、歌詞／和音を書く 65 冊（`scratch/p351/cm/lyricchord-books.txt`）を LP と対で測る fidelity 調査が次の島**——道具は `run-lp.ps1`（LP 通し・`lp-names.ly` で和音名）・`lp/allcol-settings.ly`（列 x）・`ls-widths.ps1`（Lily# の小節線 X）・`sweep351ly.ps1`（双子文面の掃き）。**最初の 1 冊は `sings-chorus-row`（行 2 本の音節幅の押し・第 2〜4 便の bar 1-2）か lead-sheet fixture**。台帳の型（`audit/lp-geometry`）に横の点は無い＝番人で観測する（第 3 便の `ABoundLyricRow_LeavesTheStaffsBarWidths…` の形）。**⑵ 行の拍グリッドは score の拍子しか知らない**（`ChordNameCollector.CollectPart`／`LyricsCollector.CollectRow` に `_meta.TimeBeats`）: 3/4 の section の行 `| C . D |` は 4/4 のグリッドで 3 slot＝偽の LYS2009＋等分。第 10 便の `FitRowsToMusicBars` は*長さ*だけ音楽に合わせる。**掃きで動いた本 0＝実需ゼロ**なので起票のみ（やるなら小節ごとの拍子の timeline を row の grid に渡す）。**着手しなかった理由**: ⑴ は新しい台帳級の島で、道具は全部ディスクに残っており次便が同じ手順で始められる＝文脈の利点は小さい／⑵ は実需ゼロ／並びの残り（ps2・lpreg・U8）は文脈が効かない。
✅ **⒟ 閉じた（第 4 便）＝製品の欠陥ではなく *LP 側の計器*だった**: c.ly は `\paper { property-defaults.fonts.serif = "LilyPond Serif" }` を pin しておらず、**svg backend でこの機械の fontconfig が generic serif を別の顔に解決していた**（`audit/lp-geometry/probes/chord-lyric-run.ly:64-74` が同じ罠を書き、memory `reference_lilypond_svg_backend_generic_fonts` も在った——**読まずに踏んだ**）。**pin した `c2.ly`**: bar 3 **14.505**（Lily# 14.53・差 0.025）・`LyricText` の幅は "sing" 4.746 / "bo" 2.595 で **Lily# の advance と 9 桁一致**、"dy" だけ LP 2.697 対 Lily# 2.766（+0.068＝Schola 対 C059・§3 既決の永久非ゼロ族。regular 面は memory が「未測定」と書いていた通り、ここで初めて regular の差が 1 語出た）。**未 pin の c.ly は全語が 8〜16% 狭かった**（"sing" 4.097）＝顔が違う。⇒ ★★ **LP で文字を測るときは serif/sans を pin する**（§0 の計器 `run.ps1` の型で書くなら `\paper` に 2 行）。Lily# の字幅は `scratch/p351/ZzP351LyricWidthProbe.cs.txt`（捨て計器・退避済み）で `ScoreTextMetrics.Advance` を刷った。**下は第 2 便の経緯**（未 pin の数）: lyric row の音節幅が列を押す量（Lily# `ApplyLyricSpacing` 対 LP の `LyricText` extra-spacing-width／`LyricSpace` minimum-distance）。**対＝`scratch/p351/lp/sings-en.lys`（fixture の英語行だけ・head exe）と `c.ly`（Devnull vocal ＋ `\lyricsto`）**: 小節幅 **Lily# 9.84 / 14.53 / 8.15 対 LP 9.837 / 14.061 / 8.150**＝差は bar 3（`a4 g f e`・"bo- dy sing"）の **+0.47 だけ**、しかも**列の対 1 つ**: 符頭の刻み Lily# 3.00 / **4.20** / 3.01 / 3.23 対 LP（ALLCOL `c-log.txt`・9/4 の行は改行で割れている）3.002 / **3.728** / 3.002 / 3.239＝**"dy"（9/4）→"sing"（5/2）の rod が LP より 0.47 広い**（音節の中心は両者とも符頭中心＝配置は合っている・押す量だけ違う）。fixture の 2 行版が bar 1-2 でもっと広いのは日本語行（CJK の字幅）が加わるから＝同じ島。**ユーザー実コーパスで歌詞を書く本は 6 / 324**（`lyrics` ブロック grep）・追跡 fixture は多数（showcase 07/08 等・折り畳んだ添付歌詞も同じ `ApplyLyricSpacing` を通るはず＝未確認）。**着手しなかった理由**（ユーザー「このセッションでやる方が有利なら着手」の 2 度目の判断）: 作業ツリーに ⒝ の未承認の出力変更が乗ったまま、もう 1 つ出力を動かす島（歌詞 fixture の snapshot が広く動く）を重ねると、掃きの base と承認が絡む（第333 が承認を待ってから次便に入ったのと同じ形）。**次便の入り方**（⒝ は第 3 便で閉じたので、次便はここから入れる）: `ApplyLyricSpacing` の rod の式と LP（`scm/define-grobs.scm` の `LyricSpace` minimum-distance・`LyricText` word-space・`lily/lyric-hyphen.cc`／`separation-item.cc` の extent rod）を字面で並べ、0.47 が定数か字幅（Schola 対 LP の serif）かを 1 対で割る。計器は全部 `scratch/p351/lp/`（`run.ps1`・`ls-widths.ps1`・`cols.ps1`）。
...
- ▶ **F-fonts. `fonts { }` に*サイズとスタイル*を持たせる（2026-09-07・第342 起票。✅ 文法はユーザー承認済み——「その形で起票して。VS code editor の自動補完も、それに合わせて修正して」）**
...
> ## ▶ **⒩ 展開の予算はページだけのもので、`midi`・`xml`・`ly` は読まない＝1 冊の本に「どれだけ音楽が在るか」で 4 つの出力が食い違う**（第301 起票・**実測**・✅ **【ユーザー決定 2026-09-08: ⒜ 予算は絵だけのもの】残る仕事は LYS1033 の文面だけ**）
...
> ## ▶ **⒤ `time none`（senza misura）は描画側で効いていない＝半実装**（第280 起票・**実測**・✅ **【ユーザー決定 2026-09-08: 彫る】**）
...
> ## ▶ **⒱ 途中の `partial`＝【ユーザー決定 2026-09-08: 許す。パートごとに書く（`time` と同じ規則）】**（決定便で起票・未実装）
...
> ## ▶ **⒣ `removeEmpty`・`pedal` も「版面ものが part ヘッダに居る」同族＝別便で検討**（第217 起票・**ユーザー指示「別便で検討して」**・✅ **【ユーザー決定 2026-09-08: ⒝ removeEmpty だけ score 側へ・pedal は part のまま】**）
...
- ⚠️ ★★ **`_ "text"` の空白**（第185 起票・**第187 で前提を測り直した・ユーザー決定＝後回し**・✅ **【ユーザー決定 2026-09-08: ⒜ GLUED 維持・診断文面だけ直す】**＝`_` が宣言済み section でないとき「密着させると custom text」と名指す・未実装）。
...
| 決定 | 根拠（要点） |
|---|---|
| ★★★ **途中の `partial` は許す。パートごとに書く（途中の `time` と同じ規則）**（2026-09-08・**ユーザー決定**・未実装＝§2F ⒱） | LP の `\partial` は Timing（＝Score）に送られ 1 か所で全譜が動くが、Lily# は共有 Timing を持たず途中の `time` をパートごとに書き直す規則（`MeasureValidator.cs:281-286`）。walker と充足検査は既に途中の `partial` を読み、止めているのは `PartialScopeValidator` だけ。score 級 Timing（書き一度）は需要が出てから。 |
...
| ★★★ **`time none` は彫る**（2026-09-08・**ユーザー決定**・未実装＝§2F ⒤） | 「LP が彫るなら Lily# も」。LP は `\cadenzaOn`（小節長に数えない・自動小節線／連桁／改行が止まる）で、双子は既に `time none` をそう書いている＝今日はページだけが嘘。移植点は `MeasureBuilder` の境界を作らない腕・自動連桁停止・改行は次の `time` まで無し・小節番号不動。観測者 0 なので fixture＋LP 双子の対から。 |
| ★★★ **`removeEmpty` は score 項目へ＝`staff m as lines 1 removeEmpty all`（`as` 1 つに列挙）。`pedal` は part のまま**（2026-09-08・**ユーザー決定**・未実装＝§2F ⒣） | LP の `\RemoveEmptyStaves` は context mod で score の `\layout`／`\with` にしか書けない＝`lines` を移した論法（総譜では隠す・パート譜では隠さない）がそのまま当たる。`pedalSustainStyle` は `\set` で音楽にも書ける property＝家風で score ごとに変える需要が無い。2 つの綴りは持たない。 |
...
| ★ **`_ "text"` は GLUED のまま。診断文面だけ直す**（2026-09-08・**ユーザー決定**・未実装） | `_ "shown"` は section `_` への表示ラベル付き参照として生きており、空白を許すとその対が潰れる。`_` を予約語にする案は `@fig(_ 6 4)` の付け替えが要るので退けた。 |
