...
最終更新 第323セッション＝**入り方は第298〜第322 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い、便の途中の口挟みは 0）。⇒ ★★★★ **本便は §1 の次の一手 ⑴＝§2 T7 F12（`Alone Again` の 8|4）を閉じた——ただし根は第322 の仮説（bar の min）と*もう 1 つ*（gate の力）の 2 つで、製品コード 2 commit**。**骨は 5 つ**:

⚠️⚠️⚠️ ★★★★ **⑴ bar ごとの min を LP に揃えた（`ccab465f`）**——**LP 2.26.0 の圧縮行（`\paper { line-width = 60\mm system-count = 1 }`）で各 paper column の `minimum-distances` を読む計器 `scratch/p323/fx/probe-cols.ly`（第322 の `probe-barx.ly` は X だけ・rod は読めない）で F12 の bar `fis,,2 fis,,8 fis,, r cis,` を 1 spring ずつ対にした**: **⒜ note→bar line の min が head-only で *up-stem の flag* を見ない**（LP rod 2.3674＝stem 1.2392＋flag 0.8282＋0.3 対 1.6042＝**−0.86**。down-stem の flag（`m-gis.ly`）・4 分（`m-q.ly`）・`Flag.stencil = ##f`（`m-noflag.ly`）は全部 1.6042＝flag が項）→ `SpacingRules.NoteColumnToBarlineFloorPair`＝列の skyline 全部（flag・dot・半 tie）対 bar の箱（隣の列の reach ぶん staff の外へ最大 1.01＝`account-for-span-bar`）／**⒝ bar→note の rod 0.1 が無い**（LP 0.49＝0.19＋0.3 対 0.39）／**⒞ 描かれる休符が notehead 箱**（r8→note 1.6042 対 LP 1.30＝glyph 1.0＋0.3。`ItemSkylineFactory` の beamed/voiced 枝は `GlyphMetrics.NoteValueOf(item)`＝休符に 4 を返すので*全部 4 分休符の箱*だった＝同便で直した）／**⒟ 不揃いの頭の対を中心で測る**（half→8th 1.6408 対 LP 1.6774＝左頭の全幅）→ `CreateRightSkylineAtColumn`（列原点＝頭の左端）／**⒠ ideal**: `note-spacing.cc:264-266` の **flag 付き左音符は stem correction 無し**（未移植と宣言されていた簡略化）＝bar ideal の +0.10 の正体。**⇒ bar は min 9.0432・natural 15.8432＝LP と 4 桁一致**（`BarlineColumnRodTests` 8 本・snapshot 12 枚は flag 付き音符と休符の下流だけ動いた）。**⚠️ それでも掃きは 388 → 386 対＝min では段署名は動かない。**

⚠️⚠️⚠️ ★★★★ **⑵ gate の力（`38cb4dad`）**——**LP の得点を並べ直すと（plain 双子 `alone-intro8.ly`）2 段 1.648・3 段 1.274、Lily# は 1.222・1.276**。⚠️ **第322 の「LP は 2 段を inf」は `alone-intro8-def.ly` 双子の `\noBreak` ×7 の副作用**（plain 双子は 1.648）＝計器の取り違え。**根＝`KnuthPlassBreaker.CalculateLineForce` は per-measure の*和*で `compress_line` の線形部分だけ（自分の doc がそう断っていた）＝最初の spring が block した先を見ない**。8 小節の段は全 bar が flag 付き 8 分で終わり（block −0.17）、block した先は残りが払う。**LP は候補行ごとに `Simple_spacer` で解く**（`constrained-breaking.cc:127-152 space_line`）。⇒ **`MeasureSpringData.Springs`（値比較＝増分 gate の vector 比較と DP session の row 再利用がそのまま読む）を持たせ、fits する候補行を `SpringSolver`（range_solve 移植済み）で解く**。伸びる行で block > 0 が無ければ線形＝従来と同じ（解くのは圧縮行と rod 縛りの行だけ）。MMR run rod／歌詞 rod の行と手組みの test vector は線形のまま。**`alone-intro8`: 1.583（LP 1.648）／1.276（LP 1.274）→ 4|4|4**（番人 `SystemCountPageScoreTests.AloneAgainIntro_*`）。**解いて初めて露見した spring の欠陥 2 つも同 commit**: **⒜ 予約の `Widen`（chord 記号・tab 数字・wide script・歌詞）が 3 引数 ctor で spring を作り直し compress strength を ideal−min にリセットしていた**（`Freedom` bar 69-76 の r→note が −1.0 で block・LP −0.62）／**⒝ ただし rod が ideal に届いたら strength は 0**（LP の blocking 0 は compress 0 のときだけ。`compress_line` は block 0 の spring を「既に block 済み」と見て足さずに引く）。**perf: DP は Debug 単独で 170〜410 ms 対 115〜250 ms（`scratch/p323/perf-plain1k-dp-timing.txt`・伸びる行は線形のまま）＝静かな窓の Release bench は未**。

⚠️⚠️ ★★★ **⑶ 掃き（`scratch/p323/sig323.ps1`・第322 の署名比）＝388 → 382 対**（`structure323-abc.csv`＝⑴ だけ 386／`structure321.csv`＝最終）: **＋4＝`Livin' It Up` ×2（F12 のもう 1 冊）・`Lovely Day`・`Together Forever`／−10＝`Alone Again [both]`・`I Should Be So Lucky [tab]`（2 小節段 → 4）・`She Bangs` ×2・`いとしのエリー [tab]`・`歌うたいのバラッド [tab]`・`想い人` ×2・`悲しみにさよなら` ×2**（8 対が tab 種）。**負け方は「和では fits・解くと fits でない」行**（計器 `scratch/p323/P323LineProbeTests.cs.txt`＝行の spring と解を吐く。`想い人` bar 84-87: **line-start spring が ideal 1.95／min 1.10／invC 0＝床は ideal**、行の床が幅を 0.8 超える → 段が割れる。**旧 gate はその行を overfull に置いていた**）。⚠️ **ユーザー判断**: **移植は LP の形で gate と layout が同じ答えを出す（RULES §5.4）が、corpus は −6**。**戻すなら `git revert 38cb4dad`（`ccab465f` は独立）**。
★★★ **⑶′ ユーザー「進めて」で −10 対のうち 2 冊を LP 双子で測った（`scratch/p323/fx/probe-bn-barx.ly`＝PROBEBN＋BarLine/NoteColumn X を同時に出す・`-ragged.ly` は自然幅・`parse-bn-barx.ps1 -Bar N` で段を引く）——どちらも gate の欠陥ではない**: **⒜ `想い人` bar 84-87（LP 番号 83-86）**: **LP も justified で圧縮して置いている**（ragged にすると LP 自身が 2+2 に割る＝自然幅 > 幅）。**LP の book は「staff だけ」「tab だけ」の 2 冊で、Lily# の `score main` は staff＋tab の 1 score**——**Lily# の行は staff の prefix（9.9・LP tab book は ≈5）と tab の数字床（16 分列 7 本が 2.307 で剛体＝§3 F9）の*両方*を背負う**ので、**available 92.5 に対し床 92.45＋line-start の剛性 0.85**。**LP tab book は available ≈97 に 26.6＋24.7＋21.3＋24.7 で置き、staff book は床が無いので 22.8/20.9/22.8 まで縮める**。⇒ **`both` 構成の Lily# score を単譜の LP book と対にする T7 の比較は、この形で構造的に外れる**（memory「前提確認」の族）。**⒝ `I Should Be So Lucky`（2 → 4）**: **LP の 2 小節段は `\break` ×15（2 小節ごと）の強制**、`.lys` は 4 小節ごと（T6 ⒥＝break 数が違う 21 冊の 1 つ）。**以前の 2,2,… は 4 小節の強制区間を線形見積りが割った偶然で、解くと 4 小節は −0.43 で置ける**。**⒞ `Alone Again [both]`（bar 51-58 を 8 に併せる）**: **`.ly` の `\break` 19 本・`.lys` の `break` 17 本**＝bar 54 の後ろの強制 break が `.lys` に無い。LP は 19 段を inf と刷る（全段が強制＝20 段しか無い）ので LP の 4,4 は選択ではなく強制。Lily# は自由なので 8 小節を解いて置く（staff score: ideal 128.3・min 66.8・幅 91.5 → −0.54。LP の自然幅 ≈131.6 と一致）。⇒ **corpus の −10 は gate の後退ではなく、⑴ 比較の前提（both 対 単譜・⒜）と ⑵ 双子の break 欠落（⒝⒞＝T6 ⒥）**。**残り 4 冊も測った（ユーザー「このセッションが有利なら着手」）**: **⒟ `She Bangs`**: Lily# の小節数が p322 の 139 から 141 に変わっている（LP 署名の和は 139）＝**`.lys` 側の入力が変わった**（全段強制・spacing は無関係）／**⒠ `いとしのエリー [tab]`（bar 47-50 を Lily# は 4・LP bassTab は 2+2）**: **LP の bassTab は `\override TabNoteHead.font-size = 2` で数字が大きく、4 小節の自然幅 ≈148 を幅 ≈97 に圧縮しきれない。Lily# tab は床 93.5 < 幅 99.0 で置ける（−0.70・49 本中 32 本 block）**＝tab 数字幅の設計差（§3 F9 の族・向きは逆）／**⒡ `悲しみにさよなら [both]`**: `.ly` の staff＋tab book は `%{ %}` の中＝**parse321 の book 検出はコメントを剥がない**ので「both book 在り」と読む→ 実体は代用対（⒜ の形）／**⒢ `歌うたいのバラッド [tab]`**: 未測（both/tab 種・⒠ の形の疑い）。
★★★ **⑶″ 前提を直した指標（`scratch/p323/kindmatched.ps1`＝`KindMatched` 列で切る）**: **譜構成が一致する対だけ 124/194 → 126/203（+2）・代用対 264/393 → 256/384（−8）**。**−10 は代用対と入力変化と双子の break 欠落に尽きる**。⇒ **「戻すか」の判断材料**: 測った 6 冊とも移植側が正しい（gate の後退は 0 件）。

★ **⑷ 踏んだ罠**: **⒜ `\fixed c'` 双子の `a,`/`e,`/`d` は A3/E3/D4＝bass の中線より上＝stem DOWN**（「pitch の対照」のつもりが stem 向きの対照になっていた・memory `reference_lilysharp_twin_fixed_c_octave_reading`）／**⒝ 引用ラチェットの記号は*8 文字以上*で `_`/`-` を含む**（`boxes`・`finalize`・`add_rod` は無名に数える → `Separation_item::boxes` の形で）／**⒞ ripple の busy fallback が `cd` だけ走らせる**（第322 と同じ・2 回。idle console に打ち直し）。

★ **開始時裏取り**: HEAD `a8ae0ad6`・**未 push 2**（`origin/master` は `407b4f77`）・木 2（README.md と samples/README.md＝ユーザー手直し。本便も触っていない）・未追跡 0・Core 0 エラー 0 警告・追跡 `.lys` 594・snapshot 244・台帳 759 点（exact 594 / `OPEN:` 0）。
終了時: **commit 3 本**（`ccab465f`／`38cb4dad`／この HANDOFF＋CHANGELOG）・**未 push 5**・作業ツリーは README.md と samples/README.md の 2 件だけ・未追跡 0（`scratch/p323` は git 管理外）・Core 0 エラー 0 警告・**Windows Debug 6916 / 0 / 4 / 6920**（**`scratch/p323/full4.trx`＝一時 probe 3 本込み。probe を消した木は 6919＝第322 比 +9＝番人 8 ＋ `AloneAgainIntro` 1**）・**追跡 `.lys` 594 不動**・**snapshot 244 不動（13 枚動いた＝⑴ 12 ＋ ⑵ 1）**・**台帳 759 点不動**（exact 594・本便は台帳を触っていない）・`docs/APPROXIMATIONS.md`・`audit/magic_constants.csv` は行番号だけ再生成（`LILYSHARP_UPDATE_DOCS=1`・2 度）・**LSP/拡張は未配備**。⚠️⚠️ **WSL ubuntu Release は取れない**（第313 ⑶）——**本便は Core を触っているので `push` 後に `gh run list` で読むこと。**
⚠️ **計器は `scratch/p323/`**（git 管理外）: **`fx/probe-cols.ly`（`minimum-distances`）・`fx/probe-flag.ly`（Flag/Stem/BarLine の extent と esh）・`fx/m-*.ly`（bar の変種と LP の `.out`）・`P323SpringProbeTests.cs.txt`（spring の dump・DP timing・得点）・`P323LineProbeTests.cs.txt`（候補行の spring と解）・`lys-springs-w-h8.txt`・`alone-intro8-lys-scores.txt`・`freedom-line-69-76.txt`・`omoibito-line-84-87.txt`・`sig323.ps1` → `structure321.csv`（最終）／`structure323-abc.csv`（⑴ のみ）／`structure323-d1.csv`（⑵ の strength 修正前）・`full1..4.trx`**。

⇒ ★★★★ **次の一手**（**第322 の並びの ⑴ を本便が消費した。差し替えと追加**）: ★★ **⑴ T7 の比較の前提を直す**——**`both` 構成の Lily# score は単譜の LP book と段署名が構造的に合わない（⑶′⒜）**。**`parse321.ps1` の対付けを「LP book の譜構成と同じ Lily# score」に限る**か、**LP 双子に staff＋tab の book を足して比べる**（`lysc ly` の双子は tab を書かない＝memory「双子にTabStaff無し」）。**（⑶″ で −10 対は全部説明が付いた＝もう掃きの負けを追わない）。parse321 の book 検出に `%{ %}` 剥ぎを足し（⒡）、`KindMatched` の対だけを §2 T7 の指標にする**。⚠️ **line-start spring の compress 0（`LineStartColumn.cs:418/539/547`）は 0.85 の小物**＝LP の `Staff_spacing` も `ideal − fixed` で 0 になり得るので、測るなら台帳点で／★★ **⑵ perf＝段数ループ＋DP の解きを静かな窓の Release で**（`EditKeystrokeBench`／`PreviewUpdateBench`・base は `38cb4dad~2`＝`a8ae0ad6` の worktree・1 サンプル目は捨てる）／★★★ **⑶ F2＝section 名の箱を LP の RehearsalMark の位置へ**（ユーザー決定済み・§3 第322。**F12 の spacing は本便で閉じたので snapshot の再ベースは 1 回で済む**）／★★★ **⑷ T6＝スラー復元 138 冊**／★★ **⑸ T7 の C 37 冊を 1 冊ずつ**／**⑹ 以下は第322 の並び**: §2 T6 の残り／`audit/lpreg` 取り直し（**本便で段数・spacing が動く本は動くはず＝取り直し前に `REGENERATE.md`**）／§2 U8c の島／§2 U8b／§2 U8 の残り／§2 A/B/D/E／§2 C⑴／§2 G／★ 低優先: `marks stacked` / `marks beside`。⚠️ **承認待ちは §2 T2 の 1 件**（`Lambada Complicada.lys`）＋**§2 T5 の ⏸ 1 件**＋**本便 ⑶ の「戻すか」**。⚠️ **§2 F の開いている項目は第320 の ⒲ 1 件。** ⚠️ **リリース 0.6.0**（第322 ⑻ のユーザー決定＝週末判断）: **repo の `CHANGELOG.md` 0.6.0 に本便の 2 項を足した（F12 の既知制限の文は消した・拡張側の CHANGELOG は engraving を載せないので不動）・版番号 7 か所の bump と tag は未着手。**

## 以下は第322セッションの経緯

最終更新 第322セッション＝**入り方は第298〜第321 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い、便の途中の口挟みは 0）。⇒ ★★★★ **本便は §1 の次の一手 ⑴＝`Optimal_page_breaking::solve` の段数ループを移植して閉じた**（製品コード 1 commit・§2 T7 の B-eng 69 冊が射程）。**骨は 7 つ**:

⚠️⚠️⚠️ ★★★★ **⑴ 移植の形**（`LayoutEngine.SystemCount.cs` 新設・`ChooseSystemCount`＝`lily/optimal-page-breaking.cc:41-254 Optimal_page_breaking::solve` の転記）: **行 DP は表ごと返す**（`KnuthPlassBreaker.Solve` → `LineBreakSolutions`。`For(k)`＝`constrained-breaking.cc:262-315 line_details`＝k 段の最良解と **Σf²・Σbreak_penalty（Δf² 無し）**）→ **理想 N を配置**（`PlaceSystems`＝`Layout` の本体を切り出した・2 度呼べる）→ **その paging skyline を小節の X で切って per-measure の高さ推定**（`EstimateMeasureHeights`＝LP の `pure_height` の代役。⚠️ **Lily#-own 逸脱として宣言**: LP は列の pure extent から段を組まずに推定する。Lily# は理想配置の silhouette を切るので、その段の spacing と行頭 prefix（全段の最大）を運ぶ）→ **候補 N ごとに `EstimatedSystemDetails`（`PageLayouter.BuildSystemDetails`＝実配置と同じ 1 軒）→ `PageBreaker.BreakIntoPagesScored`（頁 DP ＋ 頁ごとの力・`Page_spacer::solve`）→ `PageBreaker.Demerits`（`page-breaking.cc:1548-1586 finalize_spacing_result`＝Σf² ＋ Σpenalty ＋ 10×Σmin(f²,BAD)・ragged は last を除く）**。**ループは LP と同じ順**: 理想 → 減らす（`min_sys_count`＝理想 − 最終頁 − 直前頁・BAD か「頁が減って平均力 > 0」で止まる）→ 増やす（`PageBreaker.MinPageCount`＝`:1186-1278 min_page_count` が理想の頁数を超えたら skip・BAD で止まる）。**選び直したら 2 度目の配置**（理想配置は捨てる。段数が同じ本は配置 1 回＝従来と同じ費用）。`PageBreakResult`（第321 ⑻ ⒞「どこからも使われていない」）に `PageCount`/`AverageForce` を足して初めて読んだ。**REF 17・OWN 1（推定器）・`+` 550 行**。
⚠️⚠️ ★★★ **⑵ 増分の門（第321 ⑻ ⒝）の決め**: **`precomputedLineSizes`（行のサイズ）は廃止し、`IncrementalCompiler._lineBreaks`＝DP の表そのものを gate 通過時に渡す**（表は gate の純関数。`SystemBreaker.RegroupBySizes` 削除）。**ループは gate 通過時も毎回走る**（理想配置＝cache hit・推定 O(小節)・候補の頁 DP は O(段²)）ので **増分 == full は構成で成り立つ**（ink だけの編集で頁の得点が動けば段数も動く＝full と同じ）。`ScoreLayout.LineBreaks` が表を運ぶ。**`IncrementalRenderEquivalenceTests` ほか増分の網は全部緑**。
★★★ **⑶ 番人**: `SystemCountPageScoreTests` 8 本（**`bis-v6` 相当が 2|4|2**・`bis-v8` 2|4|2・**陽性対照 `bis-v12` 4|4|2 不動・`bis-v3` 4|2 不動**・`LineBreakSolutions.For(3/4)` の Σf² が LP の 38.781/42.466 の ±0.6 に入る・`Demerits` の ragged 3 形＋overfull=BAD・`MinPageCount`・`BreakIntoPagesScored`）＋ **fixture/snapshot `test/system-count-page-score`**（`fixture.png` 目視＝2|4|2）。**既存 snapshot 243 枚は 1 枚も動かず**（fixture に「forced break 直後の underfull 段」が無い）。⚠️ **`IncrementalCompilerTests.RenderFragments_*` 2 本の fixture（8 分音符 18 小節・break 無し）が 4 段 → 3 段になった**——**LP の双子（`scratch/p322/fx/eighths18.ly`）も 6|6|6**＝Lily# が LP に寄った方。fixture を 36 小節にした。
⚠️⚠️⚠️ ★★★★ **⑷ 射程＝段署名の再掃き**（`scratch/p322/sig322.ps1`＝`lysc layout --all` 全冊 ＋ 第321 の `lp.out` 再利用 → `parse321.ps1` → `scratch/p322/structure321.csv`。**LP を描き直していない**）: **一致 356 → 388 対（＋41 / −9）・全対一致 170 → 183 冊・小節数一致で段割れ違い 69 → 65 冊**。**動いた本 56 冊**。**−9 対＝7 冊は §2 T7 の F12**（下 ⑸）。
⚠️⚠️ ★★★ **⑸ 失った 9 対の正体＝ループではなく gate の幅**（**LP の `-ddebug-page-breaking-scoring` と Lily# の `LayoutEngine.DebugPageBreakingScoring` フック（新設・production では null）で得点を並べた**・`scratch/p322/fx/*-dbg.err` / `*-lys-scores.txt`）: **⒜ `未来予想図Ⅱ`（8,8 → 16）は LP 27 段 53.70 / 26 段 54.06＝差 0.36・Lily# 56.94 / 56.44＝逆向き 0.50 ＝knife-edge**（tab 本は LP の差 0.06）。**⒝ `Alone Again`（4,4 → 8）**——**再現 `scratch/p322/fx/alone-intro8.lys`（LP 4|4|4・Lily# 8|4・`alone-again-head-page1.png` の 1 段目が詰まっている）**。⚠️⚠️ **便の末尾で測り直して、最初に書いた「LP は 8 小節を置けない（min 幅）」を撤回**（§2 T7 F12 の行に正しい根）: **LP はその 8 小節を natural 幅ぴったりに置ける（力 ≈ 0）**。**Lily# の ideal 和 133.3 対 幅 88.7（LP の natural は 102.2）＝Lily# の note spacing がこの内容で LP の 1.35 倍広く、min は 0.9 倍**——**gate も loop も自分の spacing に忠実で、乖離は `SpacingRules` の bar ごとの幅**。計器 `alone-intro8-gate.txt`（Lily# の gate の数）・`alone-intro8-{min,nat,def,sc2,nb45}-lp.out`（LP の BarLine X・`probe-barx.ly`）。**⒞ `Livin' It Up`（4,4 → 2,2,2,2）は Lily# の 22 段が Σf² 6.97・LP 1.89**＝末尾の 4 小節段が Lily# でだけ圧縮されている（凸罰 f−2f⁴ が割らせる）。⇒ **§2 T7 に F12 として起票**（閉じるなら gate に compress_line の blocking walk＝per-measure SUM の設計変更・F3 の門も読む）。⚠️ **推定の高さは LP より高め**（`未来予想図Ⅱ` で Lily# は 29 段を `min_page_count > 4` で skip・LP は 29〜36 を試す）——増やす側の刈り込みが早い。得点は単調に悪化するので今回は無害。
★★ **⑹ 射程 2＝絵の A/B と perf**（`scratch/p322/ab322.ps1`・Release・base は worktree `C:\MyProj\LilySharp-perfbase-7c43`＝HEAD `7c4315a8`・`svg --combined` 全 318 冊を 1 スレッドで base→head 交互・39 分・`scratch/p322/ab/ab322.csv`）: **MOVED 61 / SAME 257**（動いた本＝段数が変わった本＝⑷ の 56 冊＋scratch の本）。**壁時計は base 1117 s / head 1225 s＝比 1.10・冊ごとの中央値 1.03**——⚠️⚠️ **静かな窓を取らずに回した**（memory「bench前に一声」・ユーザー不在の自律便）ので**絶対値は疑う**: 外れ値（`Mandy` 17→42 s・`She` 4→14.7 s）は**打ち直すと両側 ±10%**＝機械の帯。**段数が変わった本は 2 度目の配置を払う**（`Lovely Day` 4.3/5.0 → 6.5/6.4 s＝+30〜50%・`Friend Like Me` は帯の中）。**ループ自体の費用は直接測った**（`LayoutEngine.DebugPageBreakingScoring` に stopwatch）: **`Mandy`（22 段）20 ms／`perf-plain1k`（200 段・16 頁）415 ms＝候補 79 個（減らす側 1・増やす側 73＝`min_page_count > 16` になるまで LP と同じだけ試す）×約 5 ms**（候補ごとに `EstimatedSystemDetails` 200 個＋頁 DP）——**5.2 s の cold layout の 8%・warm 1.7 s の打鍵なら 2 割級**。**`EditKeystrokeBench`（base/head・Release・14 round）は plain1k 313→490・fingbeam1k 581→461・v2bow1k 525→203 ms と両向きにばらけて読めない**＝機械の帯（`scratch/p322/edit-base.txt` / `edit-head.txt`）。⇒ **perf は静かな窓で取り直し**（次の一手 ⑵）。
★★★ **⑹′ 同便で削った（ユーザー指示「F2 以外で有利なものがあれば着手」）**: **候補の採点は LP の*頁数を制約しない* 1 次元 DP（`page-spacing.cc:147-181 Page_spacer::solve ()`＝`simple_state_`・`:298-405 calc_subproblem (VPOS, line)`）を使うべきところ、Lily# の `FindOptimalBreaks`＝(段, 頁数) の 2 次元表（LP の *forced page count* 用 `state_`）を解いて列で min を取っていた**＝頁数分の余計な仕事。**`PageBreaker.SolveUnconstrained` に転記**（`PageSpacing.Resize`＝`:43-48` の再 seat・first page は header 分だけ短い）。**`perf-plain1k` の loop 415 → 101 ms（cold）・warm 1038 ms 中 69 ms**。**再掃きは署名が 1 行も動かず（388 対・失った 9 対も同じ）**。番人 `BreakIntoPagesScored_AgreesWithTheTwoDimensionalTable`（8 冊・header 有無・force/forbid 入り）。⚠️ **実配置の頁割り（`BreakIntoPages`）は 2 次元表のまま**（tie の癖が corpus のもの。触るなら別便で A/B）。
★★ **⑻ ユーザー起票（エディタ・便の末尾）＝プレビューの配色設定 `lilysharp.preview.theme`**（`auto`＝VS Code のテーマに従う（既定・従来どおり）／`light`／`dark`）。**実査**: webview は `@media (prefers-color-scheme: dark)` 2 ブロックで editor テーマに追従するだけだった（= `auto` 固定）。⇒ **`<html class="theme-dark">` に畳み、head の nonce script が body より前に立てる（開いた瞬間の白フラッシュ無し）・CSS は `:root.theme-dark …` に書き換え・`getHighlightColor` も class を読む・`onDidChangeConfiguration('lilysharp.preview.theme')` → 開いている全 panel に `setTheme` を post（`applyTheme` が `auto` なら `matchMedia` の change も聴く）**。`package.json` に enum ＋ `enumItemLabels`/`enumDescriptions`。`npm run check-types` / `compile` 緑・**`tools/Deploy-Lsp.ps1` で配備済み（`out`・`package.json` も写す＝Reload Window 待ち）**。extension の CHANGELOG 0.5.0 Added に 1 行。
★ **⑺ 踏んだ罠**: **⒜ ripple の console が busy のとき、fallback console で pipeline が `cd` だけになって本体が走らないことが 2 度あった**（結果に `Set-Location` しか無ければそれ。同じ pipeline を idle な console に打ち直す）／**⒝ `LpReferenceCitationTests` のラチェット＝記号名は住所と*同じ行*・`(lily/x.cc:1-2)` の括弧書きも数える**（2 回落とした・RULES §7 の 7）／**⒞ `PlaceSystems` に切り出した本体が `headerHeight` を読んでいた**（1 回目のビルドで CS0103＝切り出しの引数漏れはコンパイラが拾う）。

★ **開始時裏取り**: HEAD `7c4315a8`・**未 push 21**（`origin/master` は `b7865029`）・木 2（README.md と samples/README.md＝第318 ⑸ のユーザー手直し。本便も触っていない）・未追跡 0・Core 0 エラー 0 警告・追跡 `.lys` 593・snapshot 243・台帳 759 点（exact 594 / `OPEN:` 0）。
終了時: **commit 6 本**（**⑴〜⑶ の移植＋番人＋inventory 再生成／HANDOFF 第322／F2 決定の HANDOFF／⑹′ の 1 次元 DP `46832b5f`／⑻ の preview.theme／この HANDOFF**）・**未 push 27**・作業ツリーは README.md と samples/README.md の 2 件だけ・未追跡 0（`scratch/p322` は git 管理外）・Core 0 エラー 0 警告・**Windows Debug 6906 / 0 / 4 / 6910**（**`scratch/p322/full4.trx`。+10＝第321 比: 番人 9 本 ＋ snapshot 1 枚**）・**追跡 `.lys` 594**（+1）・**snapshot 244**（+1・既存は不動）・**台帳 759 点不動**（exact 594・本便は台帳を触っていない）・`docs/APPROXIMATIONS.md`（409 行）と `audit/magic_constants.csv`（925）は行番号だけ再生成（`LILYSHARP_UPDATE_DOCS=1`・2 度）・**LSP と拡張は `Deploy-Lsp.ps1` で配備済み（`46832b5f` の Core）**。⚠️⚠️ **WSL ubuntu Release は取れない**（第313 ⑶）——**本便は Core を触っているので `push` 後に `gh run list` で読むこと。**
⚠️⚠️ ★★★ **便の途中でユーザーが畳んで push した**（§0 の「間で push」の形・第201 と同じ）: **上の「commit 6 本・未 push 27」は畳む前の記録**。**畳んだ後は 28 → 13 本・HEAD `fb79074d`＝`origin/master`・未 push 0**。**§1 が名指す本便の SHA 6 つは孤児になった**ので `HistoryCitationTests` の dead 天井を 495 → 501 に上げた（remarks に再編を名指し・旧 SHA は書かない＝memory「畳むと引用が孤児」の手順）。**この後の commit（`@reverseturn`・`to coda`／天井 501）は畳んだ木の上に 2 本＝ユーザー指示「CI を直して」で push 済み・CI は 4 脚とも緑（run 33644386917・畳んだ push の run 33640099106 は 4 脚とも dead 天井の 1 本だけで赤だった＝ubuntu も本便の Core で緑）。**
★ **リリース準備（ユーザー指示・2026-09-03 未明）**: **両 CHANGELOG を `## 0.6.0` に切り分けた**（0.5.0 出荷後に 0.5.0 の下へ溜まっていた項目を移し、第321 のタブ 2 件・第322 の段数・preview.theme・F12 の既知制限を足した。**0.5.0 以下は tag `v0.5.0` の本文と byte 一致**＝`git show v0.5.0:CHANGELOG.md` を cmd 経由でファイルに落として UTF-8 で読む。⚠️ **pwsh で `git show` の出力を変数に受けると em dash が `窶・` に化ける**（console の code page）——1 度踏んで直した）。**版番号 7 か所の bump と tag は未着手＝出す判断は週末（ユーザー決定「毎週末に判断・2 週間目安」）。F12 が閉じなければ既知の制限のまま出す。**
⚠️ **計器は `scratch/p322/`**（git 管理外）: **`sig322.ps1` → `structure321.csv`・`out/<本>/layout.txt`・`ab322.ps1` → `ab/ab322.csv`・`fx/`（`eighths18`・`alone-intro8`・`alone-8bars`・`mirai-dbg`・`*-dbg.err`＝LP の得点・`*-lys-scores.txt`＝Lily# の得点・`fixture.png`・`alone-again-head-page*.png`）・`full1.trx`（6 赤＝inventory 2・snapshot 新規 1・citation 1・fixture 縮小 2）・`full2.trx`（全緑）・`sc1/sc2.trx`**。

⇒ ★★★★ **次の一手**（**第321 の並びの ⑴ を本便が消費した。差し替えと追加**）: ★★★ **⑴ §2 T7 F12＝bar ごとの spring の min（圧縮の余地）が LP より大きい**（`Alone Again`・`Livin' It Up`。**再現 `scratch/p322/fx/alone-intro8.lys`＝LP 4|4|4 / Lily# 8|4・得点は 1.279（3 段・LP 1.274 と一致）対 1.208（2 段）の knife-edge**。**T7 F12 の行に 09-03 の測り直し**: ideal は一致（15.95 対 15.84）、**min は LP 9.04 対 Lily# 7.17〜8.35**。**次の一手は台帳 1 点**: `w-h8-bass-fis.lys`（`fis,,2 fis,,8 fis,, r cis,` ×8）の 1 小節の min を LP の `system-count = 1` 押し込み（`w-h8-min60.ly`）と対にし、Lily# の `MeasureSpringData.MinWidth` の内訳（2 分→8 分・8 分→休符・休符→8 分・小節線の各 min）のどれが短いかを名指す。LP 側の計器は `probe-barx.ly`（BarLine/NoteColumn の X）。⚠️ **`\noBreak` で LP の幅を測らないこと**（非 breakable の小節線は loose column になり幅が縮む＝09-03 に 1 度踏んだ）。⚠️ **F2 の直しの前に**: 段署名は spacing に敏感で、F12 を先に閉じないと F2 の snapshot 再ベースをもう一度やることになる）／★★ **⑵ perf＝段数ループを静かな窓で測り直す**（⑹′ で候補 1 個 5 ms → 1.3 ms に落ちた。残るのは `EstimatedSystemDetails` の 200 record 生成と `CalcLineHeights` の再走。`PreviewUpdateBench`/`EditKeystrokeBench` を base worktree `LilySharp-perfbase-7c43` と並べて、**1 サンプル目は捨てる**）／★★★ **⑶ F2＝section 名の箱を LP の RehearsalMark の位置へ**（**ユーザー決定済み・§3 第322**: 拍子記号の列＝`BuildPrefixMarkAnchorX` の Rehearsal 腕と同じ anchor・テンポはその下に縦積み。**台帳の SectionLabel 対照点（意図的 OPEN）を閉じる**・snapshot は `\mark` を持つ本が大量に動くので LP 双子で正当化・`fx4-mark-tempo` が陽性対照）／★★★ **⑷ T6＝スラー復元 138 冊**／★★ **⑸ T7 の C 37 冊を 1 冊ずつ**／**⑹ 以下は第321 の並び**: §2 T6 の残り／`audit/lpreg` 取り直し（**本便で段数が動く本は動くはず＝取り直し前に `REGENERATE.md`**）／§2 U8c の島／§2 U8b／§2 U8 の残り／§2 A/B/D/E／§2 C⑴／§2 G／★ **低優先: 表示オプション `marks stacked` / `marks beside`**（§3 第322・ユーザー決定済み・**⑶ を閉じてから**）。⚠️ **承認待ちは §2 T2 の 1 件**（`Lambada Complicada.lys`）＋**§2 T5 の ⏸ 1 件**（F2 は決定済みで待ちから外れた）。⚠️ **§2 F の開いている項目は第320 の ⒲ 1 件。**
...
- **T2. ⏸ 判断待ち（第317）＝`Lambada Complicada.lys` は移行できない。`.ly` が無く、和音トラックの割り方が決まらない**
...
- **T7. ▶ 起票（第321）＝絵の突き合わせ（286 冊全数）の結果＝族の表。計器は `scratch/p321/`（§1 ⑴）**
  ...
  | **F2** | section 名の箱が行頭寄り・key があると tempo が横滑り（LP は縦積み） | `fx4`（key 有/無）・LP `Billie Jean` | `\mark` 281 冊・`\tempo` 277 冊 | ▶ **ユーザー決定済み（第322・§3）＝LP に揃える**: 箱を拍子記号の列（clef 右端 3.365／key 右端 6.385＝Rehearsal 腕と同じ `BuildPrefixMarkAnchorX`）へ、テンポはその下。台帳 `mark.*` の SectionLabel 対照点を閉じる。次便の着手 ⑴。横並びは `marks beside`（§3・低優先） |
...
- ✅ **⑴ オクターブアンカー（絶対指定）構文は*既に在る*＝`octave absolute` / `octave N`**（2026-08-31・第306 に確認。**`GRAMMAR.md:75-82` と `OctaveDecl`**——**top-level・part ヘッダ・*楽中*のどこにでも書け、`octave 2` で絶対基準を貼り直せる**。**追跡 fixture でもユーザーの本でも実際に使われている**）。
  ...
  ★ **本当に残っているのは文法ではなく*増分*の問い**: **`octave absolute` が実際に再解決の波及を区間で止めているかは未測定**。**止まっているなら ⑴ は丸ごと閉じ、止まっていないなら課題は「アンカー構文を足す」ではなく「アンカーを増分が使う」。**
...
| 決定 | 根拠（要点） |
|---|---|
| ★★★ **section 名の箱は LP の RehearsalMark と同じ位置に立てる（拍子記号の列＝clef 右端 3.365／key があれば key 右端 6.385）。テンポはその下に縦積み。行頭配置は退役**（2026-09-02・第322・**ユーザー決定「F2 は選択肢 1 で」**・▶ 次便で実装） | §2 T7 F2＝第257 から据え置きの「SectionLabel を RehearsalMark と同じ左端に立てる」問題。**LP**: 箱の左端は拍子記号の列、`♩=117` はその真下（`scratch/p321/fx/fx4-mark-tempo-lp-staff.png`）。**Lily#**: 箱は行頭（indent+0.3）、key があるとテンポが箱の右に横滑り（`-lys-staff.png`）。第220 の「マーク＋テンポの縦積みは LP 忠実のまま」と同じ方針で箱も揃える。台帳の意図的 OPEN 点（`Indent+0.3` 対 LP `0.0`・第257 第 4 幕）はこれで閉じる側へ。射程は `\mark` 281 冊・snapshot 多数（行頭の箱が右へ 3〜6 ss）。**横並びは表示オプション `marks beside` として後日**（下の行）。 |
