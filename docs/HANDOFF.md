...
- ★★★ **⒮¹⁵ 第457 の census が*見ない*族＝括弧に何かある `new X<T>(…)`・`CreateBuilder<T>(…)`・`new StringBuilder(…)`
  ...
  **445 軒・生きている 228 軒で actual＋obj 774,764 B／打鍵＝21.0%**（scratch 169 軒 650,950）。**第463 が 8 軒 −204,092 を閉じた**。残り（`remaining.txt`・census 時点の住所）:
  ...
  `MeasureCollector.cs:232` 39,235 は**閉じていた**（第499 `_resolvedNotes`・第500 PitchTrace＝第526 が HEAD で確認）／⑶ `AugmentSkylinesWithScripts` の遅延経路の builder（`PagingAugmentProgram.cs:278-280` 17,542）は**閉じていた**（`t_pagingBuilders` の drawer＝第532 が HEAD で確認）／⑷ scratch の尾＝**第464 が HEAD で数え直して 6 軒 −80,083 を閉じた**（Lab `sessions/p464/joined.csv`・
  ...
  `new PriorityQueue`・配列など**どちらの census の綴りにも無い器**（E の 76,594 はそこに居た。`SlurScoringProblem` の候補 list は**第531 が problem ごと貸して閉じた** −22,325）。
