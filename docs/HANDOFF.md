...
最終更新 第291セッション＝**第288 が台帳に `OPEN:` で残した点を閉じ、そのあとユーザーが選んだ §2 の ▶ も閉じた（2 件・commit 6 本）**——**⑴〜⑹ が「部屋」（`35893e41`）、⑺〜⑽ が「閉じの距離を*外側*プロファイルへ」（点 `6e4f8e69` ＋ 移植 `2ce6f992`）**。**台帳 744 → 748・exact 584 → 588・ss 非ゼロ 190（不動）・総和 25.844537623 → 24.714570188。**

⚠️⚠️ ★★★ **⑴ 「部屋」は部屋ではなく*輪*だった**。**`BuildLooseChainEnds` はページ最後の loose-line 塊を `anchor - MarginBottom` で閉じていたが、それは*このページの crop*。**Lily# は単一ページを内容に合わせて裁つ**（意図的な乖離＝`page.height` の残差 −109.468268）**ので、裁つ元の量は*この塊の予約（alignment 最小）*そのもの**——**部屋を予約から読み戻していた**。**実測: 部屋 4.407001213 対 鎖の最小の和 4.369959965 ＋ 0.037041248 ＝ 4.407001213（9 桁）**。⇒ ★★★ **最小の和ちょうどに解かれた鎖は、バネが何であれ床から離れられない**——**予約が自分を真にしていた**。**だから「spec は floor ではなくバネ」という第288 の正しい観察は、答えを 1 mm も動かせなかった。** ⇒ ★★ **「短い部屋」を疑ったら、部屋の*出どころ*を辿ること。ある量を予約に使い、同じ量を制約として読み返していたら、それは測定ではなく同語反復。**

★★★ **⑵ LP の記録値そのものが「LP の部屋」を持っていた**——**移植を書く前に検算できた**。**null のバネの stretchability は HUGE_STRETCH 10e7、歌詞のバネは 1** なので、**台帳の `5.500001451282664` から force ＝ 1.451282664e-6・部屋 ＝ 6.5 ＋ f×(1＋1e8) ＝ 151.628268** が出る。**紙の印字域は 169.009370 − 2×5.690551 ＝ 157.628268 で、差の 6.000000 は `top-system-spacing` の basic-distance**（`ly/paper-defaults-init.ly:78`）。**移植式は Lily# の幾何で 151.628268 を返す**——**テストを 1 本も走らせる前に 7 桁で一致した。** ⇒ ★★★ **「6 桁目の尻尾」は誤差ではなく*情報*。バネの直列系では、1 つの読みから相手の部屋が復元できる。**

⚠️⚠️ ★★★ **⑶ exact だった隣の 2 点は「その regime が動かない」exact で、しかも両方の `why` がそう書いていた——LP についての文の顔をして**。**`lyrics.row.two-staff.{deep,flat}-ink...last-system` は「room is unbounded」と書いていたが、Lily# の部屋は同じ crop で、実測 11.500000**——**たまたま鎖の理想 5.5＋2.8＋1.0 より広かっただけ**。**記録されていた −8.9e-07 は「LP 自身の尻尾で、我々の乖離ではない」と書かれていた**（**「ここに 0.0 と書くのは誰も測っていない数だ」とまで**）。⇒ **尻尾については当たり、*誰の*かについては外れ**——**同じ鎖を同じ部屋に解けば、その尻尾はこちらにも出る**（今 2 点とも 0 と 2.3e-09）。⇒ ★★ **`why` に書く「相手はこうしている」は、*自分*についての文と混ぜないこと。混ざると、自分についての未実測の主張が相手の観測の顔で 3 便生き延びる。**

★★ **⑷ ユーザーの実コーパスが 0 冊なのは運ではなく、測れる理由がある**。**326 冊のうち歌詞を書くのは 9 冊、9 冊とも crop された単一ページでこの分岐に届いている**——**それでも 1 冊も動かないのは、どの本でも第 1 バネの*床*が spec の理想 5.5 より高いから**（**`max(min, ideal)` が床になり、部屋は答えを変えられない**）。**実測 3 冊: 6.119960（`Untitled-6`＝タブの一番下の弦）・5.864960（`Twinkle`）・6.290221（`ぐるぐるワンダーランド`）。** ⇒ ★★ **「射程 0 冊」は掃きだけでは*理由*を言わない**——**0 が「届かない」なのか「届くが動かない」なのかは別の実測。本便は後者だった。**

⚠️ ★★ **⑸ 代わりに crop が stale になった。名前を付けて棚に載せた**（`LayoutEngine.Pages.cs`・`LILYSHARP-OWN, DECLARED:`・**APPROX 棚 220 → 221**）。**ページ高はいまも*予約*から計算され、音節は*理想*まで下がる**ので、**crop されたページの下の余白がその分だけ縮む**（**TBL2 で 1.130041・`test/lyrics` で 0.139・5 枚の snapshot の最大歩幅 0.85**）。**切れることはない**——**伸びは第 1 バネの理想 5.5 − 自分の床で上に抑えられ、以降のバネは basic-distance 0 ＝理想が最小なので伸びない**。⚠️ **ここを「予約側を理想にする」で閉じてはいけない**——**同じ量は system-system 用の下スカイラインでもあり、そちらは LP も*最小*を予約する**（`page-layout-problem.cc:593-599`）。**閉じるには消費者を 2 つに割る＝別便。**

★★ **⑹ 番人は主張を 2 つ持たせた**——**残差 0 は間違った理由でも起きるから**。**この場合の「間違った理由」は*部屋を無限にすること*で、それも広いページでは理想を返す**（そして短いページで間違う）。**だから `TheLastBlockOnAPage_IsSolvedIntoThePaper_NotIntoThePagesOwnCrop` は ⒜ 既定の紙で理想を読み、⒝ crop 21.788103 に対し紙を 23.0 に詰めたとき床と理想の*あいだ*を読む、の 2 つを述べる**。**毒（部屋を戻す）で 7 本が赤**（**⒜ だけが赤で ⒝ は緑・台帳 1 点・snapshot 5 枚**）。
★★★ **⑺ 同じ鎖の両端が別々のプロファイルを読んでいた**（ユーザーが選んだ ▶）。**LP は loose line の最小を `min_offsets` の差で取り**（`page-layout-problem.cc:961-962`）、**その walk は各要素の `vertical-skylines` を読む**（`align-interface.cc:207`）——**VerticalAxisGroup のそれは `skyline_spacing`（`axis-group-interface.cc:860-985`）＝内側シルエット ＋ *配置済みの outside-staff grob 全部***。**Lily# は譜間の端（`ComputeBetweenStavesEnd`）では部屋の per-staff スカイライン（＝外側）を読み、システム間の端（`LeadingLinesOfSystem`）では*内側*を読んでいた**——**第286 が text spanner のインクだけ手で 1 つ merge して。** ⇒ **1 つの量に 4 つ目の綴り**（§5.2.1②）。**移植は「部屋の finished プロファイルを読む」1 行で、特別扱いはその中へ退役した。**

⚠️⚠️ ★★★ **⑻ 「何が欠けているか」を書いた注記は、誰も読み返さない側から腐る**。**この呼び出し地点の注記は「dynamics・scripts・beams が室の持ち出し表に無い」と書いていた**が、**⒜ 前提が別の表について**だった（**page パスが届かないのは*annotation* 文脈のコピーで、`LayoutSystems` は自分の分を渡している**）し、**⒝ 内側の半分は 2026-08-26 から実際に読まれていた**——**つまり scripts と beams は 3 日前から profile に入っていたのに、注記は 3 つ並べたまま**。**本当に欠けていた 1 つ（外側の層）は、その間に*手で 1 grob ずつ*答えられていた。** ⇒ ★★ **修理は名前を 1 つ閉じ、一覧は 3 つのまま残る。** ⇒ ★ **だから注記は消さずに*その場で*直した**（形のほうが履歴より価値がある）。

★★ **⑼ 対は「族」で設計し、番人も族で置いた**。**インクは Emmentaler のグリフ（`@f.up`）にした**——**第286 の対のインクは "rit." という*語*で、両engine のテキスト計量差が対に載る。グリフなら同じフォントファイルから来る。** ★★ **そして番人は台帳の点ではなく*別の supplier*（`@text(...).up`）で書き、符号だけを主張する**（**LP に訊いていない配置なので、量を書けば台帳の服を着た snapshot になる**）。⚠️ **これは歴史から来た設計**——**この欠陥が在ったのは前回の修理が特別扱いだったから**で、**`@f` を特別扱いする修理は台帳の点を全部通ってしまう。** ★ **⑽ 射程は 2 冊＝本便が建てたプローブだけ**（**1591 冊 SAME 1589 / MOVED 2・その対照 `crn.lys` は不動**）。**追跡 fixture も snapshot も samples もユーザーの 326 冊も 1 冊も動かない**——**点を置く前に flag 実験で 1588 冊 0 冊も測ってある**（**flag が生きていることはプローブ本で先に示した**）。


⚠️ **見つけて直していないもの**（前便から引き継いだ分に加えて）:
- **★★ crop が loose-line の伸びのぶん stale**（⑸・**APPROX 棚に 1 件・§2 D に起票**）。**閉じるには「予約」の消費者を 2 つに割る**——**下スカイラインは最小のまま、crop は描かれた量へ。**
- **★ プレリミナリ・パスの閉じプロファイルは薄いまま**（⑺の裏・`ClosingProfileWithoutTheRoom`）。**部屋がまだ無い段では dynamics も movers も chord band も取れない**——**ページを少し小さく見積もるだけで、本パスが直す。観測者ゼロ。**
（以下、前便からの一覧はそのまま）

⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 の ▶ は本便で 0 になった**（**部屋＝⑴〜⑹／外側プロファイル＝⑺〜⑽**）。**手元で証拠が一番揃っているのは本便が開けた ⑸ の crop**（**量も上限も測ってあり、閉じ方の設計まで書いてある・ただしユーザーの本への影響は 0 冊**）。★ **次に読み手の目に触れそうなのは §2 U2 の族の残り**——**本便は「行の下の譜」を閉じたが、`ClosingProfileWithoutTheRoom` の薄さと、ペダルの対応付けに譜／声部フィルタが無い件は開いたまま。** ★ **ユーザーの本で今 LYS4018 が出ているのは `Untitled-6.lys` の 1 冊だけ**（第288 便の `@rit`）——**本便の 2 件はその本を 1 バイトも動かしていない。**

★ **開始時裏取り**: HEAD `3bb0681b`・**未 push 4**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6551 / 0 / 4 / 6555**・台帳 **744／exact 584／総和 25.844537623／ss 非ゼロ 190／count 156 中 2**・追跡 `.lys` **579 冊**・snapshot **229 枚**——**§1 の終了時の数と 1 つも違わなかった**。
終了時: **commit 7 本**（`35893e41` 部屋の移植 ＋ `160987eb` §1 と §2 ＋ `71ef1008` 第289 ブロックをアーカイブへ ＋ `f77457c6` 1 件目の両脚の緑 ＋ `6e4f8e69` 外側プロファイルの点 ＋ `2ce6f992` その移植 ＋ `1d7e39b4` §1 と §2 ＋ **この 1 本＝2 件目の両脚の緑**）＝**未 push 12**・木 0・未追跡 0。**suite 6557 合格 / 0 失敗 / 4 skip / 合計 6561**（開始 6555 から **+6**＝**番人 2 本 ＋ 台帳の点 4**）。Core 0 エラー 0 警告。**追跡 `.lys` 579 冊**（不動＝**本を 1 冊も足していない**）・**snapshot 229 枚**（不動＝**5 枚を*書き換えた***）。**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2。**
⚠️ **掃きは 4 回**（**母集団はディスク上の全 `.lys`＝ユーザーの実コーパス込み・base と head は*本ごとに続けて***）: **⑴部屋＝1588 冊 SAME 1562 / MOVED 26**（**追跡 fixture 5 ＋ `samples/amazing-grace.lys` ＋ scratch 20・ユーザー実コーパス 0 冊**）／**⑵外側プロファイル＝1591 冊 SAME 1589 / MOVED 2**（**本便が建てたプローブ `dur.lys`・`crt.lys` だけ・その対照 `crn.lys` は不動・ユーザー実コーパス 0 冊**）。**どちらも `check` の差 0・レンダー失敗 0**。★★ **どちらも 2 度回した**——**1 度目のあとにコメントを足したので、第288・第290 の規則どおり最終の Release を建て直して回し直し、2 度とも MOVED 集合が同一**。
★★ **緑は*コード commit のあと*に取った**。★★ **WSL ubuntu Release は両件で Windows と同じ**——**1 件目 6552 / 0 / 4 / 6556（`71ef1008` の木）・2 件目 6557 / 0 / 4 / 6561（`1d7e39b4` の木）**（§0 の「この機械の Windows での緑でしかない」に両脚で答えた）。⚠️ **CI だけは未実行**（未 push 12）。★ **1 件目の `docs:` は木を一時的に赤くした**——**§1 から落とした第289 ブロックをアーカイブへ入れ忘れ、`HandoffArchiveContinuityTests` が次の全数実行で捕まえた**（`71ef1008` で解消）。**§1 の回転は「落とす」と「入れる」で 1 つの動作。**

## 以下は第290セッションの経緯

最終更新 第290セッション＝**第289 が残した ▶ を、第289 が書いた順のまま閉じた（2 便）**——**`b6c33de3` 点を 2 つ置き** ＋ **`b8fde5b4` 直して掃いた**。**テキストスパナの左端が「書き手の音符」になった**（**右端は第289 で既にそうなっていた**）。**台帳は 742 → 744・exact 582 → 584**（**新しい 2 点が exact で閉じた**）——**ss 非ゼロ 190 と総和 25.844537623 は 1 も動いていない**＝**この直しは自分が開けた 2 点しか閉じておらず、他の点に触れていない。**

⚠️⚠️ ★★★ **⑴ 欠陥は 1 つではなく 2 つで、対照が無ければ 1 つしか見えなかった**。**⒜ 列**（`PairTextSpanners` が `StartItemIndex: 0` を*定数*で渡す）と **⒝ 左の padding**（`Calculate` が `BoundPadding` を*右端でしか*払っていない——**同じファイルが LP の `bound-details` 0.25 として既に名前を付け引用していた定数**）。**対照の本 TXH は「Lily# の定数が既に指している所」で開く**ので、**⒜ を構造的に持てず ⒝ だけを見る**。⇒ ★★★ **⒜ だけ直していたら TXH は −0.25 に残り、主役の TXO もそこへ着地していた**——**「主役が exact にならない」という*正しいが原因を名指さない*状態**。**対照は「動かないことを確かめる網」ではなく、主役が隠している第 2 の量を出す装置**（第288 ⑺ と同じ形が、今度は*同じ便のうちに*出た）。⇒ ★★ **点は必ず対で置く。対の片方は「今の実装が既に正しく答える所」に置くと、残差がそのまま第 2 の量になる。**

⚠️⚠️ ★★★ **⑵ 第289 のこの項の射程見積りは低く、外し方に名前が付く**。**「小節頭以外に `@rit` を書く追跡本は `showcase/01-expressions` の 1 冊」**と書いてあったが、**`test/multi-staff-text-spanners` は両譜の 2 小節目の*2 番目の音符*に `@accel` を書いている**——**正しくは 2 冊**。**原因は族の 1 綴りで grep したこと**（`@rit` だけを見て `@accel`／`@rall`／`@textSpan` を見ていない）。⇒ ★★★ **糖衣が 4 つある族の在庫は、4 つとも掃くまで在庫ではない**——**そして第289 は*その族を 4 綴りに設計した便そのもの***。**自分が増やした綴りは、自分の grep には入っていない。**

★★ **⑶ 1 つの直しが 2 つ動かした、そして説明が付いた**。**`showcase/01-expressions` ではリハーサルマーク "B" が 1.68 下がった**——**その箱は x 58.47..60.75 に在り、旧 `rit.` のラベルは 59.31 から始まっていた**ので、**outside-staff の段が「そこに居るべきでなかったラベル」からマークを持ち上げていた**。**62.29 になって重なりが消え、マークが自分の高さへ戻った。** ⇒ ★★ **`startX` は*水平重なり判定の入力***なので、これは第 2 の変更ではなく帰結。**予期はしていなかったが、掃きの diff を*読んで*名前を付けた**——**「1 冊で 2 か所動いた」を「たぶん連動」で通さないこと。**

★★ **⑷ 左を直したことが右を読めるようにした**。**右端は束ねた列の*左端*を取っているが、LP は*中央*を取る**（`bound-details.right` が `attach-dir` を宣言しないので `linear_combination` は CENTER）——**同じ dump に 31.309760708228 対 head anchor 30.657660708228＝約 0.65**。**継続片も小節の原点のまま**（**LP の `left-broken` は `attach-dir` を RIGHT に反転**）。**両方 `NOT PORTED HERE` を立てて APPROX 棚に載せた**（**55 → 57**）。**どちらも台帳の点がゼロなので、閉じる便は点を置くところから。** ⇒ ★★ **1 辺を LP に寄せると、残りの辺の綴りが*初めて*読める**（それまでは 2 つの差が重なっていた）。

★★ **⑸ 読みは pen 対 pen にした＝計器の側の選択**。**測るのはラベルの原点どうし**（LP のテキスト stencil の X 箱は Pango の LOGICAL rect なので grob の X 左端が pen・Lily# は `TextAnchor.Start` で描く）。**破線の始まりにしなかったのは、その 2 辺が 0.05 ずれた定義になるから**——**LP 側は stencil の縁、Lily# 側は SVG の rule の端点**で、**`ottava.x.line-start-to-notehead` がその harness 項を抱えている**。⇒ ★★ **「どの辺を測るか」は、欠陥の話が始まる前に決まる。族の中に*両側が同じ物を指す辺*が在るなら、それを使う。**

★ **⑹ 計器を 1 段挟むと、挟んだものを測る**。**台帳 json を書き換えるスクリプトの anchor が 0 件で落ちた**——**`sed -n 'Np' file | cat -A` が `{$` を返したので LF だと読んだが、実際は全行 CRLF**（**Git Bash の `sed` がテキストモードで CR を落としていた。`cat -A` を*直接*当てると `^M$` と出る**）。⇒ ★ **EOL を確かめるときは生バイトを数える**（`\r\n` と `\n` の個数）。**そして書き戻したら `--numstat` が編集した行数と合うのを見る**（RULES §5.1・**本便の json は 14/0 と 4/4 で合った**）。

⚠️ **見つけて直していないもの**（前便から引き継いだ分に加えて）:
- **★★ テキストスパナの右端が列の*左端*・継続片が小節の原点**（⑷・**APPROX 棚に 2 件**）。**観測者ゼロなので、直す便は点を置くところから。**
（以下、前便からの一覧はそのまま）

⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 の ▶ は 1 つ減って残り 1 つ**（**loose-line の閉じの距離を部屋の外側プロファイルへ**）。**手元で一番証拠が揃っているのは第288 が残した「歌詞のバネが最小に座る」**（**台帳 `lyrics.tab.control.staff-to-lyric` に `OPEN:` で待っている・残差 −1.130041486・次の一手はその `why` に書いてある**）。★ **ユーザーの本で今 LYS4018 が出ているのは `Untitled-6.lys` の 1 冊だけ**（第288 便の `@rit`）——**本便の直しはその本を 1 バイトも動かしていない**（**閉じていない span は何も描かないから**）。

★ **開始時裏取り**: HEAD `91650d7d`・**未 push 0**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6547 / 0 / 4 / 6551**・台帳 **742／exact 582／総和 25.844537623／ss 非ゼロ 190／count 156 中 2**・追跡 `.lys` **579 冊**・snapshot **229 枚**——**§1 の終了時の数と 1 つも違わなかった**。⚠️ ★ **第289 が「CI 未実行・未 push 29」で閉じた分は、間にユーザーが push して解消していた**——**`gh run list` の最新は `91650d7d` で success（run 33251365995・5m17s）**。**開始時の未 push 0 は「何も無かった」ではなく「全部出た」。**
終了時: **commit 4 本**（`b6c33de3` 点 2 つ ＋ `b8fde5b4` 直しと掃き ＋ `fcc8f88f` §1 と §2 ＋ **この 1 本＝両脚の緑を §1 に入れるもの**）＝**未 push 4**・木 0・未追跡 0。**suite 6551 合格 / 0 失敗 / 4 skip / 合計 6555**（開始 6551 から **+4**＝**台帳の点 2 ＋ 対応付けの単体テスト 2**）。Core 0 エラー 0 警告。**追跡 `.lys` 579 冊**（不動＝**本を 1 冊も足していない**）・**snapshot 229 枚**（不動＝**4 枚を*書き換えた***）。**台帳 744 点／exact 584／ss 非ゼロ 190／総和 25.844537623（不動）／count 156 中 2。**
⚠️ **掃きは 1 回**（**母集団はディスク上の全 `.lys`＝ユーザーの実コーパス込み・base と head は*本ごとに続けて***）: **1588 冊 SAME 1581 / MOVED 7・`check` の差 0・レンダー失敗 0**。**7 は追跡 fixture 4（snapshot も 4 枚）＋ 第289 の scratch 3**・**ユーザー実コーパスは 0 冊**。
★★ **掃きは 2 度回した**——**1 度目のあとにコメントを 2 か所足したので、head の exe が最終の木のものでなくなった**。**第288 の「掃きの最中に建てない。建てたらやり直す」に従い、最終の Release を建て直して回し直した**（**2 度とも同じ 7 冊**）。⇒ ★ **コメントだけの編集でも、掃いた head と出す head は同じ物にしておくこと**（**IL は同じでも、それを*確かめた*のは回し直したから**）。
★★ **緑は*コード commit のあと*に取った**。★★ **WSL ubuntu Release も同じ 6551 / 0 / 4 / 6555**（`fcc8f88f` の木で・§0 の「この機械の Windows での緑でしかない」に両脚で答えた）。⚠️ **CI だけは未実行**（未 push 4）。
---
...
- **★★ crop が loose-line の伸びのぶん stale**（**第291 起票・`LayoutEngine.Pages.cs` に `LILYSHARP-OWN, DECLARED:` で宣言済み・APPROX 棚に 1 件**）。**単一ページの高さは*予約*（＝下の塊の alignment 最小）から計算されるが、その塊を描く鎖は第291 から*紙*に対して解かれ理想まで下がる**ので、**crop されたページの下の余白が (理想 − 床) だけ縮む**。**実測: TBL2 で 1.130041・`test/lyrics` で 0.139・動いた 5 枚の snapshot の最大歩幅 0.85。切れることはない**（**伸びは第 1 バネの理想 5.5 − 自分の床で上限が付き、以降のバネは basic-distance 0＝理想が最小なので伸びない**）。⚠️ **「予約側を理想にする」で閉じてはいけない**——**同じ量は system-system 用の下スカイラインでもあり、そちらは LP も*最小*を予約する**（`page-layout-problem.cc:593-599`）。⇒ **閉じるのは 1 つの量の消費者を 2 つに割る仕事**（**下スカイライン＝最小のまま／crop＝描かれた量へ**）で、**crop 自体は LP に対応物の無い Lily# 固有の選択**（`page.height` の −109.468268）。
