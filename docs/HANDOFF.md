...
## ▶ 次の一手＝**その境界がどちらの機構で決まっているかを割る**（①の道具をそのまま使う）
...
- ★★★ ⚠️ **「どちらが発散側か」も予測して書く。境界を挟む 2 点を開くと、外れたときに
  残差より強い言明が落ちてくる**（2026-08-01・第60セッション。**実際に逆だった**）。
  grace の register sweep で **J（譜の上）を発散・K（中央線の下）を対照**として開いたら、
  **J が九桁 exact で K が外れた**。⇒ 得られたのは「K の残差は −0.284」ではなく
  **「両 engine は自分の境界の両側では一致していて、境界の位置が 1 段違う」**——
  **Lily# は K に J の regime の答えを与えている**。★ **1 点だけ開いていたら
  「その register で発散する」までしか言えなかった**し、**J だけ開いていたら
  「島は閉じた」と誤って結論していた**。
