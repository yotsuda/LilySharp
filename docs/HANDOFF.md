...
✅ **⑵ `lysc --batch`＝第 2 便（⑾・`f4d64db4`）／✅ CLI_REFERENCE の同期＝第 3 便（⑿・`32216700`）。** ⇒ **lysc 側に残っているのは 2 つ**: **⒜ 掃きを `--batch` に載せ替える**——ただし **追跡コーパスだけ**（⑾⒟＝実コーパスは base/head を本ごとに交互に取る規則が要る）。**追跡 597 冊ぶんが 1 パス 2 回になる。** **⒝ バッチのプロセス内並列化**（⑾⒠＝先に `FontEmbedInfo` の 2 つのキャッシュに lock が要る）。
