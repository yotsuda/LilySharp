...
最終更新 第186セッション＝**「最後の島」と書いた起票が、最後でも 4 冊でもなく、
値段も別のところにあった便**（**実装 3 便**）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
開始時の裏取りは**全項目を走らせた**——**台帳 516 点・ss 非ゼロ 98・総和 3.609965521・
count 点 106/うち非ゼロ 2**・HEAD `51f0f413`（実装の最後 `90470c67`）・
**未 push 18・未追跡 0・作業ツリー項目 0**・suite **5176 passed / 0 failed / 4 skipped**
＝**引継ぎの数は全部合っていた**（**17 回連続**）。
終了時 **実装の最後は `56180f1e`**・**未 push は
`git rev-list --count origin/master..master` が言う**・**未追跡 0・作業ツリー項目 0**・
suite **5193 passed / 0 failed / 4 skipped**（+17＝新しい網ちょうど）・
**台帳 516 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・**snapshot 0 枚**。
**A/B は 1 回**（`git stash` 不要＝木が clean な base ビルド対 HEAD ビルド・**PID ごとの作業場**・
**563 冊**＝`lp-regression` ＋ `lpreg` ＋ `Fixtures` ＋ `samples`）：
**絵 0/563・data-pos 3/563**（**予測どおりちょうど**）。
**round trip のラチェットは 300 → 566 冊**＝**git 管理下の `.lys` 全数**（引き算が 0）。
★ **ユーザー決定 1 件**（§2F ⑺ は **⒝ 変換器の安全策を先に・⒜ faithful な木は別便**）。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Sage`）で通せた**——**長い sweep（1 回 5.5 分）の
最中は Read/Grep/Edit ツールだけで待つ**規律が今回も効いた（第183・第185 と同じ）。

> ## ★★★ この便の骨＝**検出器の射程は、その主張の一部**
> §2F ⑺ は「round trip を破る**最後の**島・**4 冊**・**幅は落ちない**・
> **data-pos も LSP のジャンプ先も滑らない**」と書いてあった。**4 つとも測り直した。**
> ⇒ ★★★ **「最後」ではなかった。** `EveryBook_SpellsItselfBackOutOfItsTree` は
> **`audit\lp-regression\lys` と `Fixtures` だけ**を掃いており、**木の git 管理下 566 冊のうち
> 300 冊**しか読んでいなかった。**射程の外**（`audit\lpreg` 257 ＋ `samples` 6）に
> **幅を落とす島がもう 1 つ座っていた**——**`lyrics L { la -- la }` が木で `la-- la`**（**24 → 22**）。
> **第183〈`using`〉・第185〈迷子トークン〉と同じ一族の 7 つ目の器**で、
> **この網はまさにそれを捕まえるために在る**のに、**誰も見ていない所に居たので何年でも緑だった。**
> ⇒ ★★★ **「4 冊」でもなかった。** 木の **.lys 1021 冊に通すと 55 冊**が破れ、
> **うち約 40 冊は `scratch\ベースタブLy` の*ユーザー自身の曲***。
> **綴りは 1 つに集中していた**——**`c,,1~@mark("A")`**（タイのあとにリハーサル記号）が **34 冊**、
> **`f,)\3`**（スラー閉じのあとに弦番号）ほかが 18 冊。
> ⇒ ★★★ **「幅は落ちない」は正しく、「位置は滑らない」は*半分*嘘だった。**
> **後続は確かに滑らない**（総幅が保たれる）。**が、並べ替わった 2 ノード自身が嘘の位置を持つ**
> ——`g4(@cresc` の **`At` トークンが「`@`」と綴りながら位置 36 を名乗り、そこの原文は `(`**。
> **⇒ 第183 の「報告するか／保つか」の、もう一段下**: **「幅を保つか／*位置*を保つか」も別の問い。**
> ⇒ ★★★ **そして値段は LSP の精度ではなかった。** `PartSectionLayoutConverter`
> （section⇔part を入れ替えるエディタコマンド）は **セルを `ToFullString()` で運んでいた**ので、
> **ユーザーのファイルを黙って書き換えていた**——`Top Gun Anthem` で実測、
> **`c,,1~@mark("A") c,,` → `c,,1@mark("A") ~c,,`**（**タイが記号を跨いで次の音符に付いて見える**）。
> **上書き保存するコマンドなので、これは「精度」ではなく*編集*。**
> ⇒ ★★ **起票の 4 つの断定のうち、当たっていたのは「幅は落ちない」だけ**
> （**それも今日の一族の話としてのみ**）。**「measured」の札が無い断定は、札が無いだけ全部同じ**（§5.0）。

- **⑴ ★★★ 第1便＝lyrics のハイフンが隣の空白を落としていたのを直した**（`afb6b5be`）。上の骨。
  **家は `ParseLyricSyllable` の 2 つの枝**で、**どちらも 2 トークンから 1 つを組み直し、
  対の*外側*の trivia しか持たない**——だから **`la` と `--` のあいだの空白はどちらにも入らず木を出た**。
  **`- -`（離れた対）を 1 つの `--` に融かす枝も同じ形**だった。**規則は「密着しているものだけ貼る」。**
  ★ **値段は実測**: `section A { lyrics L { la -- la -- } v { c4 d e f | } }` で
  **最初の音符は 49 に立つのに data-pos は 47**（落ちた空白 2 個ぶん）。
  **空白なしの双子 `la--la--` は 46 対 46 で一致**＝**対照が「元から正しく対応している」ことを言う。**
  ⇒ ★★★ **予測は 2 つとも当たり、しかも当たり方が機構の裏取りだった**——
  **⒜ 絵は 0/563 冊**（`LyricSyllableReader.Classify` が `la-`+`-` と `la`+`--` を
  **同じ音節の同じ Hyphen connector に畳む**ので、動けるのは木の字面だけ。**動いたら私の読み違い**と
  先に書いた）**⒝ data-pos は 3/563**（＝離れたハイフンを書く本ちょうど）。
  ★ **毒は枝ごと**: trailing-hyphen の隣接条件を外すと 6 本赤・`--` の融合ガードだけ外すと 1 本赤
  ＝**互いに覆っていない**。**既存の網は 1 本も赤くならなかった**（消費者は `Classify` の後ろしか見ない）。
- **⑵ ★★★ 第2便＝「every book」と名乗る網が 566 冊中 300 冊を読んでいた**（`ca7060b8`
  ＋**追い足し `2e70ca4b`**・**Core は 1 行も触っていない**）。
  `audit\lpreg`（257）と `samples`（6）を射程に入れて 563 冊へ——
  ⚠️⚠️ ★★★ **そこで止めたのが誤りで、引き算をすると*まだ 3 冊余っていた***
  （`audit\lilypond-ref\cases\*\case.lys`）。**「前より広い」は「全部」ではない。**
  **`audit\lilypond-ref` も入れて 566 冊＝git 管理下の全数**（3 冊とも元から round trip する）。
  **床の assert は 250 → 566**（＝掃くべき冊数そのもの。**コーパスのパスが動いたときに
  *黙って縮む*のを止める**・§5.4 の空集合の罠）。**毒**: 床を 567 にすると
  **「only 566 books found」と冊数を名指して赤**＝**射程が本当に 566 であることを網自身が言う。**
  ⚠️ **結果として既知一覧が 4 → 11 冊になったが、退行は 0**——**7 冊とも元から破れていて、
  誰も読んでいなかっただけ**。**7 冊が同じ島であることは 1 冊ずつ「最初に食い違う文字」で確かめた**
  （`e'4(@accent` ×4・`c''4(@staccato`・`g1)~@startTrillSpan`・`e8(@accent`）。
  ★ **毒**: `tupnumss.lys` を既知一覧から外すと**その名前を挙げて赤**＝**新しい射程は働いている**
  （§5.4 ⑸「毒が生きていることを同じ run で示す」）。
- **⑶ ★★★ 第3便＝変換器がユーザーの音楽を*運ぶ*かわりに*綴り直して*いた**（`56180f1e`・
  **ユーザー決定＝⒝ を先に**）。**家は 1 軒 `Verbatim(source, node)`**
  ＝`source.Substring(node.Position, node.FullWidth)` を **9 か所の `ToFullString()` と入れ替えた**。
  ★ **この切片が「打たれた文字そのもの」である理由は測ってある**: **1025 冊で
  `root.FullWidth == ソース長`・全ノードの span が範囲内**＝**並べ替えは幅を落とさないので、
  中身が入れ替わっていても*外枠*は正しい。**
  ⇒ **`Top Gun Anthem` は変換してもバイト同一で戻る**（前は書き換わっていた）。
  ★ **網は自分の前提を先に assert する**（`Assert.NotEqual(source, respelled)`）＝
  **「木が実際に綴り直す」ことを言ってから「変換器は綴り直さない」を言う**ので、
  **fixture が退屈なせいで通ることがない**（§5.4 の空集合の罠の、書き手側の顔）。
  ★ **毒**: 9 か所のうち 1 か所を `ToFullString()` に戻すと **4 本赤・対照 1 本は緑**。
  ⚠️ **これは §2F ⑺ の*安全な半分***。**木は faithful になっていない**ので、
  **2 ノードの data-pos / LSP ジャンプ先は嘘のまま**、既知一覧 11 冊もそのまま。
- **⑷ ★★★ §2F ⑺ の消費者を数え切った**（起票が「まず数えよ」と要求していた第 1 歩）。
  **本番 17 か所・生存 12・生存はすべて SEQUENCE 読み**で、**順序が効いているのは設計**
  ...
  **1021 冊で 0 冊**がそこに記号を置く。**陽性対照は同じ歩き**（seq の Slur 26,399・Tie 7,862・
  BeamMarker 319・articulation 110,282 を見ている）＝**0 は「見えていない」ではなく「無い」。**
  **faithful にすると*この 6 経路が目を覚ます***。
  ⚠️ **`editors/vscode/src/smartTyping.ts` に同じ順序契約の 2 つ目の実装がある**（TypeScript）。
  ⇒ ★★ **だから ⑺ は「作業」ではなく「木の形の決定」**（faithful か正規化か）＝**1 セッション以上。**
- **⑸ ★ §7.5 の監査**＝**第1便 Core +33 行（うちコード 4）・第2便 0 行・第3便 +35 行（うちコード 15）**、
  **REF 0 / OWN 0・数値定数 0**（**正しい**）。**分類は 3 便とも ⒟**——
  **既存のガードを締めた／射程を広げた／9 つの複製を 1 軒に畳んだ**。
  **LP に対応物が無い**: **LP には lossless な木が無い**ので lyric mode の空白はそもそも木に入らず、
  layout の入れ替えという機能も無い。
- **⑹ ★ perf（RULES §7 ⑼）＝2 便は「足していない」・1 便は*安くなった*。**
  第1便はハイフン 1 個あたり整数フィールド読み 2 つで、新しい走査も確保も memo も無し
  （**parse は打鍵経路なので、足していないことを書く**）。第2便はテストのみ。
  第3便は **`Substring` 1 回が `ToFullString()`（部分木を歩いて StringBuilder を組む）に置き換わった**
  ので**厳密に安い**——しかも **`Emit` に届くのは `Convert` だけ**で、
  **その非テスト呼び手は LSP の layout コマンド 1 つ**（`Commands.cs:532`）＝**描画経路でも打鍵経路でもない。**
- **⑺ ★★★ §2F に残っているのは 9 つ**——**⑴ 一覧に構文を与えるか（決定であって作業ではない）
  ⑵ 繰り返し縦線の ⑹（MIDI の片側 `:|`）⑶ `lyrics` の voice 束縛（要決定）
  ⑷ `lysc ly` が transpose を落とす件 ⑸ 「Guards X」と名乗る fixture の残り 13 冊
  ⑹ 入れ子の中の phrase 参照が双子で空になる件 ⑺ 注釈と記号の順序（**半分だけ閉じた**——
  実害は止まり、残るのは木を faithful にするか否かの決定）⑻ `_ "text"` の空白
  ⑼ LYS1015 が誰も出さない**。
  ⇒ ★★★ **「0.3.0 の前でなければ不可能」な項目は 0 のまま。**
  ⚠️ ★★ **round trip は parser の不変条件に「近い」まま**——**566 冊で破れるのは 11 冊**、
  **1 つの島**（§2F ⑺）。**幅を落とす一族はこれで 7 器とも塞がった**（§2F ⑺ は幅を落とさない別類）。
  **次に「黙って消える」を疑うなら、まず
  `SyntaxTree.Parse(src).GetRoot().ToFullString() == src` を*射程を確かめてから*通すこと。**
...
終了時 **実装の最後は `6baa4ff9`**（そのあと `90470c67` は文書と網のみ・
**Core は 1 行も触っていない**）・**未 push は
`git rev-list --count origin/master..master` が言う**・**未追跡 0・作業ツリー項目 0**・
...
- **⑺ ★★★ §2F に残っているのは 9 つ**——**⑴ 一覧に構文を与えるか（決定であって作業ではない）
  ...
  ⑺ 新規＝round trip を破る最後の島（注釈がスラー記号を跨ぐ・**幅は落ちない**・4 冊）
  ⑻ 新規＝`_ "text"` の空白（`_"text"` だけが通る。規則の形が決定）
  ⑼ 新規＝LYS1015 が宣言されているだけで誰も出さない**。
  **「`ParseList`／`ParseMusicBlock` の裸 `Advance()`」はこの便で閉じた**（器は 6 つだった）。
  ⇒ ★★★ **「0.3.0 の前でなければ不可能」な項目は 0 のまま。**
  ⚠️ ★★ **round trip は今日から parser の不変条件に近い**——**ディスクの本 300 冊で破れるのは 4 冊だけ**、
  **1 つの島**（§2F ⑺ に起票）。⇒ ★★★ **その 4 冊は長さが 1 バイトも変わらない**＝
  **今日塞いだ「幅が落ちる」一族とは別物で、位置は滑らない**（起票時に実測。
  §1 に「4 冊は 1 つの島」とだけ書いて §2F に住所を作らないと、2 便でアーカイブへ降りて消える）。
  **次に「黙って消える」を疑うなら、まず
  `SyntaxTree.Parse(src).GetRoot().ToFullString() == src` を合成した本に通すこと。**
...
- ✅✅ ★★★ **`lyrics` のハイフンが隣の空白を落とす件は第186 で閉じた**（`afb6b5be`）。
  ...
  **`EveryBook_SpellsItselfBackOutOfItsTree` の射程外**（`audit\lpreg` 257 ＋ `samples` 6）に
  居たから。**射程は 300 → 566 冊＝git 管理下の全数**（`ca7060b8` ＋ `2e70ca4b`。
  床の assert も 250 → 566）。
