...
最終更新 第306セッション＝**§1 が名指した「次の一手」の 1 つめ＝§2 F ⒯ を取り、そこから出た ⒰ と、第305 が残した ⒭ の後半まで閉じた（3 項目・commit 5 本）**。**入り方は第298〜第305 と同じ**——**ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った**。⇒ ★★★ **本便の骨は「起票が*確かめよ*と書いたことと、*決めよ*と書いたことの両方が、読み直すと違っていた」**: **⒯ の射程 proxy は偽陽性で（⑴）、⒭ の「決めることが 2 つ」は既に §3 で決まっていた（⑹）。** **掃きは 3 回・両側 Release・**4 出力**（`ly` は本便が足した）・1966〜1973 冊。**

⚠️⚠️ ★★★ **⑴ 起票が「まず確かめよ」と名指した 1 冊は白だった＝proxy の偽陽性。** **起票は「独立ヘッダを*後ろ*に置く 4 冊のうち 1 冊はユーザーの `(They Long To Be) Close To You.lys`」と書き、「双子の小節線 75 に対しページは 83 小節」を状況証拠にしていた。** ★ **開いたら独立ヘッダを 1 つも持っていない**（9 つの section は全部 `part bassline` の中）**双子は完全**。⇒ ★★ **8 の差は `\repeat percent 4 { r1 | r1 }`**（ページでは 8 小節に開くのに双子は `|` を 1 本しか書かない＝−7）**＋末尾の `s1` に閉じの小節線が無い（−1）＝ちょうど 8。** ⚠️ **起票自身が「小節線の数と小節の数は別の量」と警告していて、その警告のほうが正しかった**（→ RULES §5.3 に汎化）。

⚠️⚠️ ★★★ **⑵ ⒯ の原因は述語の食い違い 1 か所で、直したのは 1 行。** **`LilyPondExporter.OrderedMusic` の「単一 part の略記」の腕が `LooseSectionMusic(s).Any()` を訊いていた**——**その一覧は指示を音楽に数える**ので、**`section A { key g major }` が「鳴る宣言」として登録され、この辞書は*後勝ち*なので part の後ろに書いたヘッダが part の本物の cell を上書きした**。**前に書けば健全＝違いは行の順だけ。** ⇒ **コレクタが訊いている `SectionHasInlineMusic`（THE one spelling）に揃えた。** ⚠️ **`GRAMMAR.md` 自身の独立ヘッダの参考例が、壊れていたほうの綴り**（文法書は直していない——**綴りは正しく、直ったのは実装のほう**）。★ **form 無しの腕も一緒に閉じた**（ヘッダが `inOrder` の 2 つ目として「空の 2 度目の A」を鳴らしていた）。

⚠️⚠️ ★★★ **⑶ 掃きに 4 つ目の出力を足さないと、⒯ の掃きは*何も見えないまま全緑*だった。** **第305 の `sweep305.ps1` は svg / midi / xml / check を撮り、`.ly` は 1 度も描かない**——**⒯ が動かす唯一の出力がそれ**。⇒ ★★ **`ly` を足し、動いた本は双子の本文を JSON に残した**（掃き直さずに diff が読める）。**1966 冊で SVG 0 / MIDI 0 / XML 0 / `check` 0・LY が動いたのは 7 冊で全部 scratch のプローブ**（→ RULES §5.3 に汎化）。

⚠️⚠️ ★★★ **⑷ ⒯ を直すと隣に*前から在った*食い違いが露出したので、混ぜずに起票した＝⒰。** **`section A { key g major }` だけが A の宣言で、どの part も音楽を与えないとき、ページはその調を arm して次の section の小節に効かせ、双子は 1 つも書かない。** ⚠️ **⒯ の前は双子が 3 つ書いて*最後が違う調*だった**ので、**食い違いは前から在って ⒯ で*小さく*なった。**

⚠️⚠️ ★★★ **⑸ ⒰ は「ページ対双子」ではなかった——ページに §3 の則が届いていない穴だった。** **A が*小節を持つ*ときは、ページも双子も §3 の境界則どおり「A の調 → B で score の調へリセット」を出す**（`scratch/p306/b9` 実測）。**壊れているのは A が 0 小節のときだけで、そこでは境界そのものが発火しない。** ⇒ ★★★ **ユーザー決定は「どちらの沈黙が正しいかを選ばず、綴りを拒否する」**（§3・`LYS1036`）。★ **`LYS1005` の兄弟で、その差が規則**: **宣言が*無い*名前は既にエラー**なので、**これは宣言が在って全部ヘッダの場合**＝同じ検証器の 1 つ先の腕。⚠️ **判定は「名前が」であって「この part が」ではない**——**A の音楽が*この score が描かない別の part*に在る本は正しい**（`u3` 実測・2 小節・clean）。**規則を書く前に測って、番人の*黙る行*に留めた。**

⚠️⚠️ ★★★ **⑹ ⒭ の「先に決めることが 2 つ」は、読み直すと既に決まっていた。** **§3 に「`transpose` / `octave` を section スコープの機能として*足さない*」が在り（第305・ユーザー承認）、その理由は「`transpose` が欲しい場面は*参照側*の印であって宣言側ではない」。** **cell は宣言側なので理由がそのまま届く**⇒ **⑵ が No なら ⑴ も決まる**（ユーザー確認済み）。⇒ ★★ **「未決定」と書かれた項目でも、§3 を読み直すと決まっていることがある**——**起票の「決めること」を額面で受け取らない。**

⚠️ ★★ **⑺ ⒭ では*書き手*も見た**（第305 の規則「禁じる綴りは読み手だけでなく書き手を grep する」）: **`PartSectionLayoutConverter` は cell を `BetweenBraces` で写すので option を*黙って落としていた***——**源で拒否したことでその経路も消えた。**

★ **述語を 1 軒に寄せたのが本便で 2 回**: **`SectionHasInlineMusic`（⒯・エクスポータ→コレクタ）と `IsBareHeader`（⒰・変換器→`SectionSymbols`）。** ⚠️ **⒯ の原因がまさに「自前の問いを訊いていた」ことなので、⒰ で 3 人目の読み手を作らずに済ませたのは同じ教訓の適用。**

⚠️⚠️ ★★★ **⑻ letter を再利用した瞬間に、コード内の 14 本の参照が*違う項目を指すようになった***（**ユーザーの「次便は HANDOFF だけで始められるか」への答えを裏取りしていて出た**）。**§2 の letter は項目が閉じると再利用される**ので、**本便が新しい ⒯ と ⒭ を立てた瞬間に、古い `HANDOFF ▶ ⒯⑸`（値サイトの型付け）と `HANDOFF ▶ ⒭ ⑵/⑵′`（増分の flat list）を指していた 14 本が*今日の項目*に当たるようになった**。⇒ ★★★ **壊れ方が「宛先が無い」ではなく「*違う宛先に当たる*」**——**grep は当たり、文章は読め、話が噛み合わないだけなので、読んだ人は間違いに気づかない。** **14 本とも retired と明記した**（消さない＝アーカイブはその letter で引く）。⚠️ **`HANDOFF §1 ⒪` の 4 本は*別の壊れ方*で未修理**（§1 は毎便書き換えるので宛先が*存在しない*）——**§2 G に負債として記録**。★ **規則は RULES §5.1 へ**（**コードから引くのは主題であって letter ではない**）。

⚠️⚠️ ★★★ **⑼ ユーザーが「判断すべきことは何か」を訊いたので §2 の*札*を数えて 4 件渡したら、3 件が既に閉じていた。** **⒜ U6 の `@rit` 終端＝ユーザーが「もう実装した」と指摘**（`@!rit` は在り、文法は REQUIRED と書いている）／**⒝ §2 C⑺ の post-event 木順＝2026-08-30 に閉じていた**（**私が「未決定」と言う 1 日前**。ラチェット 2 本の既知一覧が*空*でそう明記してある）／**⒞ §2 D⑴ のオクターブアンカー＝`octave absolute` / `octave N` として実装済み**。⇒ ★★★ **D⑴ の外し方が一番痛い**——**同じ節の ⑶ が「台帳の『〜が無い』は Lily# 側の語彙を*検索してから*言う」という家訓を付けているのに、その ⑴ が違反していた**（第113 に次いで 3 例目）。⇒ ★★ **教訓＝「未決定」の札は、*貼った便の次に一度は実コードで確かめる***（→ RULES §5.3）。**生きていたのは §2 C⑶（record の同値性）だけ。**

⚠️ ★★ **⑽ その流れで `@rit` の強さを直した**（§3）。**ユーザーの提案は「小節末で自動終端」だったが、測ってから*断った***——**もっともらしく間違った長さを黙って描く**からで、**代わりに「文法が REQUIRED と言っているのに warning」という食い違いのほうを閉じた**。★ **`SpanPairingValidatorTests` が severity を*読み出しヘルパ*に埋めていたので 10 行が赤くなった**——**ペアリングの判断は 1 つも変わっていないのに**。**`Reports()`（severity 不問）に直し、severity は新しい 1 本にピン留めした**（→ RULES §5.4 の「主張は 1 か所に」の実例）。

⇒ ★★★ **次の一手**: **§2 U8 の残り**（`grace { }` 本体の和音・休符／スラー・梁・タイ／注釈。**第298〜第302 が 1 つずつ移植した続きで、残りは*本当に grob を要る*もの**＝レンダリング側の仕事。★★ **射程は本便が着手前に測って §2 に書いた＝ディスク 1973 冊で 47 冊・うち 45 冊は過去便の自分のプローブ・ユーザーの 326 冊は 0**⇒**snapshot の再ベースはほぼ起きない。数え直さなくてよい**）／**§2 の X/Y 側**。⚠️ **承認待ちは無い**（snapshot は 1 枚も動いていない）。⚠️ **§2 F は ⒫〜⒰ が全部 ✅ になった**——**開いている F 項目は無い。**

★ **開始時裏取り**: HEAD `270673af`・**未 push 29**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6773 / 0 / 4 / 6777**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 点 158（非ゼロ 2）／OPEN: 0**・追跡 `.lys` 581・snapshot 231——**第305 の終了時の数と 1 つも違わなかった**（**素朴に数えると `exact` 556・`OPEN:` 12 が出る。§0 のレシピは `-le 1e-6` と `why -like 'OPEN:*'`**）。
終了時: **commit 14 本**（**⒯ 本体 `10321fbd` ／ §1・§2・RULES とアーカイブ `0db665a0` ／ **⒰ 本体＝`LYS1036` `2833cb2b`** ／ **⒭ 後半＝`LYS1035` の 2 つ目の位置 `07c8fc87`** ／ §1・§2・§3 `3b943b27` ／ **U8 の射程を着手前に測って §2 へ記録 `abc254ab`**＝**着手はしていない** ／ 数の閉じ直し `645f42d1` `2fe03971` ／ **letter 再利用で腐った参照 14 本を retired 明記＋規則を RULES §5.1 へ `13817e2b`** ／ §1 に letter の件 `3b95158c` ／ **`@!rit` は実装済みだったという訂正 `ea94997d`** ／ **§2 C⑺ と D⑴ も閉じていたという訂正 `a81113c3`** ／ **未終端 span を error へ `6e9bf696`** ／ §3・§1（この commit）**）・**未 push 43**⚠️ **数を閉じる commit が数を動かすので、最後の 1 本は*自分を数えて*「この commit」と書く**（第305 も同じ形で 2 本使った）・作業ツリー 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6802 / 0 / 4 / 6806**（**開始 6777 から +29＝新しい番人 2 ファイル ＋ 既存 2 ファイルの行**＝`LilyPondStandaloneSectionHeaderTests` 12 ＋ `SectionPlaysNothingValidatorTests` 11 ＋ `PartSettingInSectionHeaderValidatorTests` +5）・**台帳 748／588／190／24.714570188／158 中 2／`OPEN:` 0・追跡 `.lys` 581・snapshot 231 は開始時と同じ**（**snapshot は 1 枚も動いていない**）。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけ**（**定数は 1 つも足していない**・`audit/magic_constants.csv` も不動・**REF/OWN の増減 0＝LP からの移植は 1 行も無い**）。⚠️ **間の push は無し**（`origin/master` は `be16a770` のまま）。★ **base 用の worktree `lys-base306` / `lys-base306u` は両方消した。**
★★ **WSL ubuntu Release は Windows と一致を 2 度読んだ**: **`10321fbd` で 6785 / 0 / 4 / 6789**、**最終 `07c8fc87` で 6801 / 0 / 4 / 6805**（build 7 秒 / test 30 秒）——**§0 の両脚を読んだ。**⚠️ **docs commit では ubuntu を回していない**（動いたのは `.md` だけ）。
⚠️ **計器は `scratch/p306/` に全部残っている**: **`sweep306.ps1`**（第305 の掃きに **`ly` の 4 つ目**と「動いた双子の本文を残す」を足したもの）**と掃き 3 本**（`sweep306.json` ⒯／`sweep306u.json` ⒰／`sweep306r.json` ⒰＋⒭）**と プローブ 15 冊**（`b1`〜`b9`＝⒯ と ⒰、**`b8` は陽性対照＝ヘッダを消した本**・**`b9` は境界リセットの対照**／`u1`〜`u3`＝⒰ の隣、**`u3` が述語の形を決めた**／`r5`〜`r7`＝⒭、**`r6` `r7` は正しい家の陽性対照**）。
