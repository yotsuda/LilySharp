...
最終更新 第273セッション＝**inline `@chord` の paging 課金を、観測者の起票から移植まで 2 幕で完済し、その先の一族（標 3 冊と volta）の射程まで測って置いた**——地図①（scratch/p272/sweep-map.txt の act 6 追記）。commit は points `843aaeb5`＋code `a685f494`＋docs。**Lily# 側は毎回 probe／コード変更の前に測定**（scratch/p273/predictions.txt）・**移植は全木 sweep 0/572・rerender 0/81 で出力同一**。

**⑴ まず定数の名前が間違っていた**——そしてそれが「誰も見なかった」理由そのものだった。地図は「chord row 3.0」と書いていたが、`EstimateAboveStaffExtents` の chord 腕に渡るのは **`inlineChordNames`**（`score.ChordNames` − text-row staff・LayoutEngine.cs:257）＝**裸の `@chord` だけ**で、**和音*行*は届く前に濾される**。だから **CHR1/CHR2・GCF/GCS＝本コーパスの和音行の書 4 冊とも構造的に盲目**で、572 冊で唯一噛む本（`chords-funky-ignatzek.lys`＝`@chord` で書かれた本）が SVG ハッシュ 1 本だけに見張られていた。

**第 1 幕（points `843aaeb5`・コード変更ゼロ・新 probe `inline-chord-page.ly`・台帳 4 点）**: **⒜ 毒で腕を分離し、読む所を決めた**——`upExtent`（頁アンカー）は**構造的に盲目**: 1 譜では床候補が `3.0 + 2.0 + 1 =` **ちょうど 6.000000**＝top-system-spacing の basic-distance との *tie* で、**上げれば 1:1 で動くが下げても 1 ミリも動かない**。`bandUp`（system 間の床・`InterSystemPairMinimum` の `inkBelowLastRefpoint + nextHalfFirst + bandUpNext`）は**両方向に動く**ので点はこちらへ。**⒝ 対は LP 側が恒等**: CIB（`@chord` あり）と CIBN（`@chord` を消すだけ・他は 1 語も違わない）で、**CIBN は両 engine 12.000000000 exact**（sys1 の `c,` が refpoint 下 8.045・8.045+2.0+1=11.045 が system-system-spacing の 12 に負ける）＝**gap の他項が全部 ink-true と分かるので、CIB の残差は丸ごと記号の課金**。⚠️ sys1 が ct 一族より深いのは意図的（`g,` の 5.045 では band が 12 の床に届かず何も縛らない）。**⒞ 予測は外れ、それが収穫**: 「LP は 13.6 まで広げ Lily# が −0.6 の*過小*」と書いたが、**LP は 0.593884 しか広げず Lily# は 13.045000000＝符号が逆で +0.451116000 の*過大*課金**。外れ方が真因を指した——**予測は行を skyline のインクとして値付けしていた**（8.045+4.548884+1=13.598884＝予測した数そのもの）が、**LP は loose line として*再配分*し、溢れた分しか押し広げない**（Lily# が持たない subsystem・§2 D⑵⑶）。**⒟ 残差は同じ run の毒で 2 項に分解**: **+0.451116000 ＝ 0.450000000（band）＋ 0.001116000（silhouette 腕の平坦 scalar）**——`bandUp` を 0 にすると同じ本が **12.595000000＝LP と 0.001116 差**、`AddMarkBox(…, cnY + 1.9, …)` の 1.9 を 1.95 にすると **12.595→12.645 が 1:1**。**⒠ 対応物も測った**（§5.0・第179 の「訳の置換は一致の裏に隠れる」）: LP に inline 和音記号は無いので**同じ音楽を `\chords`（loose line）と `^\markup`（TextScript）の両方で測った**——**一致しない**（12.593884 対 12.183464）。インクが答えを出す（1.9989/1.9073 対 1.6544/1.5885・Lily# は font-size 2.62 で ≈1.95）＝**対応物は ChordNames**。**コーパスが 62 便主張してきた対応づけが、初めて*測られた*。**

**第 2 幕（同委任で続行・code `a685f494`・Core 2 ファイル・出力同一の証明つき）＝chord 腕を両腕とも retire**。**band は第 3 の課金だった**: 入力は per-note のインクなのに**全 x に床を敷く**——X-aware silhouette が実位置で予約し、per-measure annotation extents が実インクで値付けし*済み*（§5.2.1②）。**歌詞 band の 2026-08-20 とまったく同じ話**で、**`CreatePages` 自身が発火条件を書いていた**（"the chord row keeps this shape until a point measures it the same way"）＝第 1 幕がその点。**`upExtent` は観測者を待たずに落とした**——上記の tie により**どの読みも下向きには見られない**から。**⚠️ 2 腕は別々に sweep して帰属した**（点は片腕しか見ないので、まとめて消すと未観測の変更を出荷する）: base→band-off **0/572**・band-off→both-off **0/572**・全体 **0/572**・rerender **0/81**・suite 全緑。**その 0 が読めるのは同じ run で台帳点が動いたから**（§5.0 ⑸）——`page.inline-chord.gap-first` **+0.451116000 → +0.001116000**（第 1 幕の毒が予告した値ちょうど）・対照 **12.000000000 不動**・count 2 点不動。**残る +0.001116000 は名指しで OPEN**（silhouette 腕の平坦 1.9 と Nimbus/Heros の面項が混ざっており、分離はその腕の移植の仕事＝**ここで詰めない**）。**★ 対はもう次の腕の計器になっている**（§5.0「恒等の対は片側が exact になった瞬間に第2の測定器へ変わる」）: 1.9→1.95 の毒で **6359 本中この 1 点だけ**が名指しで赤・**0.051116000** に着地→復旧緑。⚠️ **CIB は移植後も生きている**（12.595 は対照の 12.000 床を 0.595 上回る）——**下回らせる変更は赤ではなく*沈黙*を招く**ので、そのときは本を深く切り直すこと（対照の点がそれを言うために在る）。

**⑶ その先の一族（標 3 冊と volta）の射程も測った**（commit は docs のみ・コードは復旧済み・`git diff` 清）——**地図の読みを訂正する結果**。act 6 追記は「±0.5 で 0 冊・+3.0 で 3 冊＋snapshot 1 ＝ *生きているが dominated*、だから移植であって削除ではない」と書いたが、**その非対称は chord 腕と同じ tie の顔**で、**`Math.Max` される推定値は上げれば必ず可視化できる＝「上げて動く」は仕事をしている証拠にならない**。**答えを出すのは*下げ*のほう**: **rehearsal 3.0／section label 3.5／segno·coda 2.5／volta 2.0 を*同時に 0.0* にして 0/572・rerender 0/81・suite 全緑・台帳全点不動**。毒が生きていることは同日同ファイルで別に示してある（rehearsal 3.0→6.0 で有題・無題の標書 2 冊とも動く＝§5.0 ⑸ の順番）。⇒ **4 つとも 2026-07-30 に退治した下側 4 定数と同じ species**（dominated・削除側）で、**移植側ではない**。⚠️ **ただし退治にはまだ足りない**: あの 4 つが持っていた**床の議論**（「どんな texture でも実インクはこの数を下回らない」＝hairpin の 3.540000）が無く、**「今日のコーパスで dominated」は「binding し得ない」ではない**。⚠️ **削除は要ユーザー承認**（§5.1）なので本便は測って置くだけ。**記録は `EstimateAboveStaffExtents` の remarks**（次に触る人が最初に読む所）。

⚠️ **本便が作った死んだ配線を 1 つ残した**: `bandUp` の書き手は chord 腕だけだったので、**`AugmentExtentsWithLooseLines` → `CreatePages` の `BandUp` → `PageLayouter` → `InterSystemPairMinimum` の `bandUpNext > 0` 腕まで、誰も供給しない経路**になった。**独立したリファクタ便は立てない**（§5.1「独立したリファクタ便を立てると 1 島/1 関心のリズムを投機的な整理と交換することになる」）＝**次にこの島に触る幕に括り付ける**。**削除は要ユーザー承認**（§5.1 は出力同一でも*承認*は免除しない——免除されるのは*点*だけ）。⚠️ **第 2 幕の commit message はこれを「its own commit に残す」と書いており、§5.1 の形を取り違えている**。**コード側の札は docs commit で訂正済み**（message は履歴なので手を入れない）。

検証: Core 0 エラー 0 警告（--no-incremental）・**Windows Release 6355/0/4**・**WSL ubuntu Release も commit ごと実測**（`843aaeb5` 6355・`a685f494` 6355・いずれも 0 失敗 / 4 skip）・**全木 sweep 0/572 を 4 回**（band-off・both-off・全体・標/volta ゼロ化）・**rerender 0/81 を 3 回**・毒 6 種（band ±0.5／band 0／silhouette 1.9→1.95 を移植前後 2 回／rehearsal 6.0 の liveness／標·volta 同時 0.0）→各復旧緑・**snapshot 0 枚**（出力同一なので承認事項なし）・一時変更 0（毒と実験は全部 `git checkout` で復旧・`git diff` 清・測定と予測は scratch/p273 残置＝git 管理外）。**perf（§7-9）**: **本便の Core 変更は分岐の削除のみ**——新しい走査はゼロで、むしろ毎レンダの `score.ChordNames.Where(…).ToImmutableArray()` が 1 本消えた（**非コメントの `+` 行は 3 本**・全部「引数を 1 つ減らした」ことによる署名と呼び出しの調整・新しい数値定数ゼロ）。⚠️ **port の commit message にこの 1 行を書き忘れた**ので、ここに置く。

⚠️ **SAC にまた噛まれた**（§5.5・memory）——**実験ビルドの `LilySharp.Tests.dll` が 0x800711C7 でブロックされ、`dotnet test` が合否行を出さずに終わった＝偽の緑**（危うく「実験しても台帳は動かない」と読むところだった）。⚠️ **memory の回避策「`-c Release` か WSL」のうち、*Release 自体が塞がれた*のは今回が初めて**。**`--no-incremental` では解けず、効いたのは `rm -rf LilySharp.Tests/{obj,bin}/Release` してからの再ビルド**。⇒ ★★ **合否行が出ない run は緑ではない**（§5.5 の trap 21 と同じ顔）。実験の測定自体は WSL で取り直した。

⇒ ★★★ **次の一手＝和音記号の silhouette 腕の平坦 scalar `[cnY + 1.9, cnY − 0.3]`**（`AddMarkBox`）**——ただし本便が着地まで測った結果、自律側ではなく*要ユーザー判断*側に置く**。計器は armed（`page.inline-chord.gap-first` の +0.001116000・毒で 1:1 に動く）で、話は 4 回目（標 0.8＝第270・和音推定 1.9＝第271・custom text 1.8/0.6＝第272）、移植先も同じ `ChordNameEngraver.SymbolInk` の一つの家——**ここまでは前提どおり**。★★★ ⚠️ **ところが移植すると数が*悪くなる***（port を建てる*前*に実測・§5.0「建てる前に測る」）: `SymbolInk` に差し替えると台帳点は **+0.001116000 → +0.008366371**＝**LP から 0.00725 *遠ざかる***。**Lily# の TeX Gyre Heros は大文字を LP の Nimbus Sans より高くインクするので、平坦な 1.9 は*面の偶然*で近いだけ**。⇒ **その +0.008366371 は丸ごと面項**＝`page.chord-row.staff-to-chord-baseline` と同じ島・同じ決定（**Nimbus Sans 同梱**）。**§5.2「定数を出力に合わせるな」はこの取引を*する*側に立つが、取引するかはユーザーの判断**——**黙って移植しないこと**。射程も測ってある: +0.05 の毒で **1/572**（`samples/greensleeves`・**snapshot でも rerender 81 冊でもない**）。**この幕に上の死んだ `bandUp` 配線の撤去を括り付けるのが §5.1 の形**（どちらも要ユーザー承認）。**要ユーザー判断で開いているもの**（前便どおり）: 標·volta の 4 定数の削除（⑶・本便が測った）・silhouette chordNames scalar の一つの家への出荷（第271 第 5 幕）・和音語彙（CHL 3 点）・Nimbus Sans 同梱・CHANGELOG 版番号・SectionLabel の X（第257）・F【8】3・混在綴りの同名 section（第269）・標の箱そのもの（0.400000 の項）・`@text` の縦（第272 末で自律側から外した）。
⇒ ★ **語彙の島（`Cm⁷` 対 `Cm7`）は台帳 3 点＝要ユーザー判断のまま**（CHL1 中間・CHL4 中間・CHL2 閉じ——決定が落ちれば 3 点まとめて動く）。

── 前便（第272・custom text／dynamics の 5 幕）の経緯は**この下の「以下は第272セッションの経緯」に逐語で残し**、第271 の分を ARCHIVE 冒頭へ移した。
...
- **★ 双子 exporter は和音行と歌詞行を出さない**（和音行・歌詞行の LP 照合は手書きプローブのみ。verse-carry.ly も手書き）。⚠️ **inline `@chord` も落とす**——`lysc ly` が **"warning: @chord dropped (out of scope)"** と言う（第273 実測）ので、**inline 和音の LP 照合も手書きプローブだけ**（`inline-chord-page.ly` は octave を自分で決めず、exporter が出した `ixed c'` の枠をそのまま使っている）。
...
★ **開始時裏取り**: HEAD `77ba0ef8`・未 push 70（第272 終了時どおり・origin `9e3c06a9` 不動）・木 0・Core 0 エラー 0 警告（`--no-incremental` 実測）・**Windows Release 6351/0/4**（第272 終了時と同値）・台帳 720 点／|残差|総和 23.620116084／count 152 うち非ゼロ 2——**引き継いだ数は全部当たった**。
終了時: **本便の commit は points `843aaeb5`（第 1 幕・probe `inline-chord-page.ly`＋台帳 4 点・コード変更ゼロ）＋ code `a685f494`（第 2 幕・Core 2 ファイル・出力同一の証明つき）＋ docs（本便・⑶ の測定を `EstimateAboveStaffExtents` の remarks へ／§5.1 の札の訂正／本 §1）**＝**points 1 本・code 1 本**＋ handoff 系（総数は `git rev-list --count origin/master..master` − 開始時 70 で引くこと）**・origin 不動・木 0。suite 6355 合格 / 0 失敗 / 4 skip / 合計 6359**（台帳テスト +4・Windows Release・**WSL ubuntu Release も commit ごと実測**）・Core 0 エラー / 0 警告・**全木 sweep 0/572 を 4 回・rerender 0/81 を 3 回**・毒 6 種→各復旧緑・snapshot 0 枚・一時変更 0。忠実度計器の消費: **台帳 全点 724（+4）・|残差|総和 23.621232084・exact（|r|≤1e-6）574/724**・**count 点 154 うち residual 0 が 152**（第272 と同じ数え方）。⚠️ **総和は +0.001116 しか増えていない**——**起票が +0.451116 を可視化し、同じ便の移植が 0.450000 を返したから**。

---

## 以下は第272セッションの経緯

最終更新 第272セッション＝**custom text／dynamics の paging 一族を、観測者から移植まで 5 幕で完済**——地図 ③ の観測者起票（第 1 幕）・dynamics 側の観測者（第 2 幕）・frame の 2 段化（第 3 幕）・envelope の port（第 4 幕）・**X の対と port（第 5 幕）**。commit は points `fce0cb53`＋points `056d55c1`＋code `efabe51f`＋code `dc32003d`＋points `4e67d23c`＋code `91bf34b2`（Core は終始同じ 1 ファイル）。**Lily# 側は毎回 probe／コード変更の前に測定**（scratch/p272/predictions.txt）・**移植 3 本とも出力同一**（rerender 0/81・sweep 0/572 を各 3 回）。

**第 1 幕（points `fce0cb53`・コード変更ゼロ・新 probe `custom-text-page.ly`・台帳 16 点）**: **⑴ 正体の特定**——paging 2 腕（Enrich の `Add(ctY−1.8, ctY+0.6)`／silhouette の `AddMarkBox(…, ctY+1.8, ctY−0.6)`）の **[1.8/0.6] は旧 letter-class trio（TextAscentEm 0.75／TextDescentEm 0.25 × 旧 em 2.4）の化石**——stacker は実アウトライン化済み・draw の em は 2.2 で、死んだ定数の値の読者はこの 2 腕だけ。LP に envelope は無い（grob 自身の stencil skyline）＝標の 0.8・和音推定の 1.9 と同じ話の 3 回目。**⑵ 対と毒**: 大文字 1 変数の対（"meno mosso"／"Meno mosso"・両方 descender 無し＝baseline 共通）をページ錨（CTP/CTC）と gap（CTG/CTGC＋音符対照 CTGN）に 1 組ずつ・row-leading frame は ROWM 型 3 冊（CTW/CTWN/CTWO）。毒で腕を分離: Enrich ±0.03→**錨 2 点だけ**赤・silhouette ±0.03→**gap 3 点だけ**赤・2 段化実験→**CTW だけ** 14.342092602→12.000000000 exact。**⑶ LP 着地**: 恒等が両腕とも成立——**CTC−CTP＝CTGC−CTG＝0.554428623277563＝M のインク成長 15 桁一致・Lily# の Δ は両対とも 0.000000000＝scalar の平坦さがそのまま対の差**。baseline は両 engine 9.6e-6 一致・CTGN は 0 exact。**⑷ CTW の予測は外れて、それが収穫**: LP は床 12 でなく **12.570929**——text のインクが和音行を押し上げ、**LP は loose line を pair gap の中で再配分する**（第271 第 5 幕の遮断的発見と同じ regime・Lily# が意図して持たない subsystem）。残差 +1.771164 ＝ float +2.342093 − 再配分 0.570929 に分解して起票。

**第 2 幕（同委任で続行・points `056d55c1`・新 probe `dynamics-row-page.ly`・台帳 6 点）＝dynamics 腕の frame 観測者**（DYW/DYWN/DYWO・深い g, 行＋先導 row＋`@pp`）。**float の鏡像**: 下向き dynamic の箱が down silhouette から浮き上がり、**Lily# の DYW は音符対照とバイト同値の 13.090000000＝dynamic が gap から消える＝under-charge（重なり側）**。LP は予測どおり **DYW＝DYWN 恒等（15.442035・3e-15 一致）**＝row はこの pair gap に触れない（CTW と違い再配分項なし・残差 −2.352035 は全額 frame）。分解も閉じた: 15.442035 ＝ pp インク底 7.397035 ＋ a''' 天 7.045 ＋ padding 1。毒〈descent −0.03〉→ **DYWN の 1 点だけ**名指しで赤（DYW は float で dynamic が消えているので不動＝機構の再確認）→復旧緑。

**第 3 幕（同委任で続行・code `efabe51f`・Core 1 ファイル）＝ct・dynamics 両腕の frame を 2 段化**（mark 腕 2026-08-25 と同型・`ScoreGrobStaffTopUp` を第 1 歩に。dynamics 側は lower staff のオフセットも同時に正しくなる）。**着地は 2 点とも予測の桁どおり**: `page.custom-text.leading-row.gap-first` **+1.771164 → −0.570929000**（残りは LP の loose-line 再配分＝無い subsystem の値段・**名指しで OPEN・0 を追わない**）・`page.dynamics.leading-row.gap-first` **−2.352035 → −0.000035000**（pp bbox の face 項＝DYWN と同値）。対照 4 冊（CTWN/CTWO/DYWN/DYWO）は全点不動。

**第 4 幕（同委任で続行・code `dc32003d`・Core 1 ファイル・出力同一の証明つき）＝ct envelope の port**（地図の ⒜＝第270 の標・第271 第 4 幕の和音推定に続く同じ話の 3 回目）。paging 2 腕が読んでいた **[1.8/0.6]＝旧 letter-class trio（0.75/0.25 × 旧 em 2.4）を捨て、描画・stacker と同じ一つの家（文字列自身のインク＝`fonts.Ink` at TextScriptFontSize italic）**へ。LP には envelope が無い（TextScript は stencil で skyline に入る・define-grobs.scm:3818）ので**移植は「持つのをやめる」こと**。**着地は 4 点とも予測の桁ちょうど**: `page.custom-text.first-staff-refpoint` **+0.765955 → −0.000045**・`…capital.first-staff-refpoint` **+0.211526 → −0.000074**・`…gap.gap-first` **+0.765955 → −0.000045**・`…gap.capital.gap-first` **+0.211526 → −0.000074**（各々 face 項のみ＝baseline 0.0000096 ＋ インク天 0.0000354／0.0000640）。**大文字対の残差の差は 0.554429 → 0.000029 に潰れた**＝対が測っていた平坦さが無くなった。**出力は完全同一**（rerender 0/81・全木 sweep 0/572＝コーパスの届く範囲ではこの腕はどこでも dominated・**見たのは台帳 4 点だけ**・§5.1 の証明）。毒〈各腕の ink 読みに ±0.03〉→**その腕の 2 点だけ**名指しで赤→復旧緑。⚠️ **X 半分は名指しで温存**: silhouette の箱は今も**pen origin 中心**で、描画は start-anchored（第204 が mark 腕で閉じた defect の X 版）——**コーパスに観測者がゼロ**（どの本も text の下のインクが一様）なので、対になる本（前の system が 1 つの x にだけインクを持つ）が先。

**第 5 幕（同委任で続行・points `4e67d23c`＋code `91bf34b2`・出力同一の証明つき）＝X の対と port**（第 4 幕が「観測者ゼロ」として名指し温存した最後の一枚）。**箱は pen origin 中心・描画は start-anchored**＝箱がインクの **半 advance（この本で 6.04 ss）左**に立っていた。新 probe `custom-text-x.ly`・4 冊 8 点で**両方向**を測る: 深い `c,,` を**文字の 4 ss 左**（箱だけが在る所）に置くと **LP は無筆対照と完全同値 14.545000**・Lily# は 16.039000＝**+1.494000 の幻の課金**。同じ音を**実インクの下**へ 1 変数だけ動かすと符号が反転し、**LP は満額 16.039044・Lily# は 45° flank の残りだけ 15.970199＝−0.068845 の under-charge**（重なり側）。★★★ **対の収穫は恒等**: **LP の満額 16.039043639 と Lily# の満額 16.039000000 が face 項 0.000044 で一致**＝第 4 幕で高さが ink-true になった後、**残っていたのは純粋に位置だけ**（「正しい量を、間違った x で課金していた」）。port（箱を pen origin から `+advance`）で**0.000000000／−0.000044000 と予測どおりに着地**・対照 2 点不動・**rerender 0/81・sweep 0/572**。毒〈中心綴りの復元〉→**X の 2 点だけ**名指しで赤（⚠️ 左端 0.3 のずらしでは 1 点も赤くならない＝**この本が縛っているのは*ずれ*であって 0.2 の pad ではない**——pad は観測者ゼロのまま据え置き）。

検証: Core 0 エラー 0 警告（--no-incremental）・**Windows Release 6351/0/4**・**WSL ubuntu Release も commit ごと実測**（`fce0cb53` 6337・`056d55c1`／`efabe51f`／`dc32003d` 各 6343・`4e67d23c`／`91bf34b2` 各 6351・いずれも 0 失敗 / 4 skip）・**rerender 0/81 と全木 sweep 0/572 を各 3 回**（第 3・4・5 幕。基線は各修理の直前に stash して Release で取り直し——**この 0 が読めているのは、同じ run で台帳の該当点が予測値ちょうどに動いたから**＝liveness は台帳が持つ）・毒 7 種（Enrich ±0.03／silhouette ±0.03／dynamics descent −0.03／port 後の ink 読み 2 腕／X の中心綴り復元／**外れた毒 1 本も記録**＝X 左端 0.3 は 0 点赤＝届いていない）→各復旧緑・**snapshot 0 枚**（出力同一なので承認事項なし）・一時変更 0（temp 計装 4 本と 2 段化実験は commit 前に除去・`git diff` 清・測定と予測は scratch/p272 残置＝git 管理外）。

⇒ ★★★ **次の一手（自律で回せる側）の候補**（帳簿としての強制は無し。**第272 が毒 sweep で射程を測って置いた＝scratch/p272/sweep-map.txt 末尾の act 6 追記**）: **custom text／dynamics の paging 一族は本便で完済**（envelope・frame・X の 3 つとも移植済み・番人 30 点）。残る自律筋は **`EstimateAboveStaffExtents` の Prelim 5 定数**で、**射程は測ってある**——⑴ **chord row 3.0 だけが今日すでに生きている**（±0.5 で `audit/lp-regression/lys/chords-funky-ignatzek.lys` 1 冊が動く・**テストも台帳点も 1 つも見ていない＝SVG ハッシュだけ**＝直す便は rerender 0/81 が割れて再祝福が要る）⑵ **segno/coda 2.5・rehearsal 3.0** は ±0.5 で 0 冊・**+3.0 でようやく `probe-coda.lys`／`markprobe.lys`／snapshot `test/grandstaff-repeat`**（＝生きているが dominated・**消すのではなく測る側**）⑶ **section label 3.5・volta 2.0 は新しい本が先**。⚠️ **下向き 4 定数（lyrics/dynamics/hairpin/figbass・2026-07-30 に退治）とは species が違う**——あちらは dominated で削除できたが、**こちらは +3.0 で届く＝削除ではなく移植**。**要ユーザー判断で開いているもの**（前便どおり）: silhouette chordNames scalar の一つの家への出荷（第271 第 5 幕）・和音語彙（CHL 3 点）・Nimbus Sans 同梱・CHANGELOG 版番号・SectionLabel の X（第257）・F【8】3・混在綴りの同名 section（第269）・標の箱そのもの（0.400000 の項）。⚠️ **`@text` の縦（DynamicEngraver の FallbackAscent/Descent）は自律側から外した**（第272 末で判定）——**LP に対応 grob が無く**、Lily# は dynamics pipeline の em 2.0・italic・**中央揃え**で描く一方 LP の同等表現 TextScript は 2.2・start-anchored＝**「@text とは何か」を決める言語設計**であって移植ではない。**要ユーザー判断側**。
⇒ ★ **語彙の島（`Cm⁷` 対 `Cm7`）は台帳 3 点＝要ユーザー判断のまま**（CHL1 中間・CHL4 中間・CHL2 閉じ——決定が落ちれば 3 点まとめて動く）。

── 前便（第271・+0.7706 の島完済＋5 幕）の経緯は**この下の「以下は第271セッションの経緯」に逐語で残し**、第270 の分を ARCHIVE 冒頭へ移した。
...
- **★★★【ユーザー決定 2026-08-26・優先度高】リファクタ在庫は全部完済**——**⑨⑪⑬⑭ は第263・④②＋ファイル分割＋死物件は第264・§4a 小物の ①⑧⑩⑮ の返済確認と LSP Completion.cs 分割・CollectionCursor 束ねは第266・⑫ SectionHasInlineMusic は第268 で完済**（挙動判定と MIDI の可聴 drift の修理込み）。`CompletionContextScanner` 型出しは**便にしない**と判定済み（第266）。
- **★ ② で名指し温存した乖離 ⑴⑵ は第266 で実測・両方 inert**（台帳は `InterSystemPairMinimum` remark ⑴⑵ と `probes/rows-only-page.ly`）——**コード上の二腕は残っている**（⑴ は到達する本が無く・⑵ は値が等しいので、どちらも「直す」対象ではなく「触る便が remark を読む」対象）。**連鎖側の loose-line 再配分そのものは未移植のまま**（§2 D ⑵⑶——移植の便が来たら rows-only-page.ly がその審判）。
- **★ customText／dynamics の paging 箱は完済（envelope・frame・X）**（第272 第 3〜5 幕・番人 30 点）——`page.custom-text.leading-row.gap-first` −0.570929000 だけは**LP の loose-line 再配分の欠落**を名指しで OPEN（0 を追わない＝閉じるには subsystem を建てる話）。他は全部 face 項（−0.000035／−0.000044／−0.000045／−0.000074）か exact。⚠️ **残る小物 2 つ**: **silhouette 箱の 0.2 pad は観測者ゼロ**（第 5 幕の毒が届かなかったことでそう分かった）・**`@text` の縦は InkOf の保守的 fallback のまま**（実測インクは dynamic glyph だけ）。chord name の envelope は第271 第 4 幕で完済・silhouette chordNames scalar だけ要ユーザー判断側に parked。
- **★★ 第259 レビューの残件の束**（全文 `scratch/review-2026-08-26/code-review.md`・git 管理外）: **残り＝F【8】3 だけ**（要ユーザー判断）。
- **★★ CHL1/CHL2 の第 1 歩 −0.252403051（第260 から両書*同値*）**＝binding X の五線側インク——LP の同 X の down が 0.25 深い。stem/binding の内容差、未分解。
- **★★ TextScript（`^"Text"`）の高さが LP より約 0.35 高い**——**観測者が立った**（`lyrics.row-between.lyric-to-staff` +0.082995881、対を通した下からの読み）。標の箱の項 0.400000 と同族の可能性。
- **★ `Staff.TextRowVerses` のスコア全体最大は事実として残る**（MultiStaffLayouter:286・SharedRenderer:426・MeasureContentKey:319 の 3 軒）が、**第262 の RVC 対で「対の geometry には効かない」ことを実測**（対照＝詩 2 全削除が系 1 の 3 読みと 9 桁一致）。**残る読者は帯の高さ（extents/silhouette）だけ**＝効くとすれば純粋高さ・改ページ側。観測者が要るなら inter-system gap（台帳 why に名指し済み）。
...
★ **開始時裏取り**: HEAD `7be0befb`・未 push 59（第271 終了時の 51＋8・origin `9e3c06a9` 不動）・木 0・Core 0 エラー 0 警告（`--no-incremental` 実測）・**Windows Release 6321/0/4**（第271 終了時と同値）・CI は push 済み HEAD の run 緑（33025894215 success）——**引き継いだ数は全部当たった**。
終了時: **本便の commit は points `fce0cb53`（第 1 幕・probe `custom-text-page.ly`＋台帳 16 点＋Lily# 側 8 冊）＋ points `056d55c1`（第 2 幕・probe `dynamics-row-page.ly`＋6 点＋3 冊）＋ code `efabe51f`（第 3 幕・frame 2 段化）＋ code `dc32003d`（第 4 幕・envelope port）＋ points `4e67d23c`（第 5 幕・probe `custom-text-x.ly`＋8 点＋4 冊）＋ code `91bf34b2`（第 5 幕・X port）＝**points 3 本・code 3 本**＋ handoff 系（総数は `git rev-list --count origin/master..master` − 開始時 59 で引くこと）**・origin 不動・木 0。**suite 6351 合格 / 0 失敗 / 4 skip / 合計 6355**（台帳テスト +30・Windows Release・WSL も commit ごと実測）・Core 0 エラー / 0 警告・rerender 0/81 と全木 sweep 0/572 を各 3 回・毒 7 種（うち 1 本は「届かなかった」として記録）→各復旧緑・snapshot 0 枚・一時変更 0。忠実度計器の消費: **台帳 全点 720（+30）・|残差|総和 23.620116・exact（|r|≤1e-6）571/720**（第271 の 549/690 と同じ数え方）・**count 点 152 うち residual 0 が 150**（⚠️ 第271 の「141/143 counts」は本便の再集計で unit=count 135/137 だった＝出所不明の数。本便からは unit=count の全数で言う）。総和の推移: 26.774961（第 1 幕）→ 25.574796（第 3 幕）→ 23.620072（第 4 幕）→ **23.620116**（第 5 幕＝X の 2 点が 1.562845 分閉じ、新規 8 点の可視化分と相殺してほぼ同値）。
...
- ✅ **歌詞帯のスカラー床は X 盲目 — 閉じた**（第221・`785ade3c`。起票 `90833c84`＋対の修理 `053e2674`）。
  ...
  **露出して残った島**: row/sings の音節 X ドリフト・j-dot（→§1 第221 ⓐⓑ）。
  ✅ **上側の帯も閉じた**（第273・`843aaeb5` 起票＋`a685f494` 移植）——⚠️ **これは "chord-row 帯" ではなかった**: `EstimateAboveStaffExtents` の `bandUp` に渡るのは `inlineChordNames` だけで、**和音*行*は届く前に濾されていた**（だから和音行の書 4 冊が全部盲目だった）。
