...
★★★ **⒆ 数**: **開始時**: HEAD `1e80a9b2`・未 push 9・木 clean・未追跡 0（**`Session-Check -Build -Test` は走らせていない**）。**終了時**: **出荷物の HEAD は `de98c1b4` のまま**（第 13・14 便は**production を 1 行も触っていない**＝第 13 は「見つからなかった」・第 14 は**網だけ**）・未 push 35・**台帳 843 点／ss 非ゼロ 210／総和 23.216135909／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249／追跡 `.lys` 599＝全部不変**（**絵は 1 枚も動かない**＝触ったのは LSP と test だけ・**Core は 3 便とも 1 行も触っていない**＝§7.5 の数えは 0 行）。**full `scratch/p375/run20.trx` 8087 / 0 / 4 / 8091**（第 14 便の摂動 136 本ぶん増）・第 12 便末 `run19.trx` **7951 / 0 / 4 / 7955**（⚠️ **第 12 便で 9 本*減った*のは正しい**＝matrix の MIDI 行 3 綴り × 3 主張が言語から消え、代わりに「退役した綴りは断られる」2 本が入った。第 11 便末 `run16.trx` 7964・第 10 便末 `run15.trx` 7956・第 9 便末 `run14.trx` 7953・第 8 便末 `run13.trx` 7947・第 1 便末 `run1.trx` 7816・第 2 便末 `run4.trx` 7827・第 3 便末 `run5.trx` 7829・第 4 便末 `run6.trx` 7839・第 5 便末 `run8.trx` 7842・第 6 便末 `run10.trx` 7854・第 7 便末 `run12.trx` 7869。**12 便とも合計を読んでいる**＝新 test は差引 144 本（第 12 便は −9）（第 8 便の 78 ＝ 26 綴り × 3 主張）。⚠️ 第 5・6 便は既存 test を**書き換えている**ので増分はそれより小さい）。★ **第 7 便は 7 つの context の配線を丸ごと替えたのに既存 7854 本が全部緑**＝**行の読みが 1 つになっても、読めていた綴りの答えは変わっていない**。⚠️ scratch は **`p375`**。⚠️ §7 3.5: 第372 を `-Archive 372` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝**第372 の天井 commit `9f6df2a3` が未 push のまま**。**push はユーザー（未 push 12）**。★ **LSP は 10 度配布した**＝`tools/Deploy-Lsp.ps1` が `0.6.0+de98c1b4` を `yotsuda.lilysharp-0.5.0-win32-x64\server` へ（95 ファイル byte 一致）⇒ **ユーザーは Reload Window が 1 回要る**（package.json は動かしていないので 2 回は不要・記憶 `feedback_lilysharp_lsp_deploy_target`）。

⇒ ★★★ **⒆ 次の一手＝⒜ *摂動網の射程を広げる*（第 14 便が建てた機械に、まだ載せていない語彙を足す）**。**今載っているのは 136**（`@` 98・layout 全値・clef・staff selector・part 属性 5・drummap 4）。**未収載で価値が高い順**: ⑴ **drum 名 83**（`GetDrumCompletions`＝打楽器 part で 1 音ずつ。**同じ position＋notehead の別名クラスタ**が出るはずで、それ自体が読み物）／⑵ **instrument preset 全数**（`InstrumentDefaults.KnownInstruments`＝clef・octave・tuning・MIDI 音色が動くはず）／⑶ **tuning 7・tab style 2・chord display 2**／⑷ **paper キー全数**（寸法は SVG が直に動く）／⑸ **chord quality 35**（`layout { chordQualities }` の両値 × 35）。⚠️ **足すたびに「フィクスチャが差を表現できるか」を先に確かめること**——第 14 便の inert 4 件は全部それだった。⒝ **✅ 第 11 便で閉じた**（上 ⒂）——**名前を書く項目は全数見た**（score／form／lyrics／chords／part／section／phrase／drummap／template／staff・lyrics の retrigger 群）。**残りは drummap の `${1:hh}` だけで、それは欠陥ではなく UX 案件**（83 語の drum 語彙を出すと親切。`drummap { ▮ }` の本体に語彙の家が無いので context が 1 つ要る）。⒞ **第373 ⑼⒜（probe 24 冊の「量を測ったか／問いに答えたか」の仕分け）**も先頭級のまま。**ヘッダの綴りの島は閉じた**——⑻ の 25 綴りが全数で、**残る未点検はゼロ**（`{` を開く構文は score／staff group 5／part／phrase／form／lyrics／chords／section／fonts・paper・layout とその入れ子／音楽の容れ物で尽きる）。⚠️ **閉じたのは*補完の*読み手だけ**: 同じ「2 語しか読まない枠」を持つ道具が他に居ないかは見ていない（`SemanticTokens`・折り畳み・`BarCountFix` は別の木＝**実構文木**を読むので原理的に無縁、という*予想*まで）。⒟ 第369 ⑸⒝（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。

★★ **⒇ 判定の記録（ユーザーの常設指示「次便は、このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）**: **第 2・5・6・7・8・9・12・13・14 便はユーザー起票／指示**（判定の対象外）。**判定で着手したのは 5 脚**（⑺ tilde・⑻ 掃き・⒁ MIDI 行・⒂ 名前の監査・⒅ 摂動網）で、**5 つとも「直前の脚が*何で*断定したか」を道具に採った的**。★★★ **基準はこの 1 行＝「どの道具が温まっているか」ではなく*直前の的をどうやって見つけたか***（第 3 便で 1 度書き間違え、第 13 便で 1 度守り損ねて 40 分落とした——**2 度とも同じ形**）。⚠️ **仕掛け線を張ったら必ず毒で赤を見る**（第 8・14 便は入れた。入れない網は「在るつもり」になる）。⚠️⚠️ **冷えた便が §1 の見出しだけで着手すると*逆に壊す***——⒁ は「穴」ではなく死語だった（埋めていたら何もしない綴りを補完に足した）／⒂ は `form` が*逆向き*だった（在ってはいけない名前を書く側）。★ **1 便だけ shell 切断で「有利／不利」ではなく*実行可能か*が判定を決めた**。**着手しなかったもの**: ⒆⒜ は**機械が在るので冷えていても安全に進む**（語彙を 1 つ足してフィクスチャを確かめる繰り返し）／⒆⒝ は UX 案件／⒆⒞⒟ は**別 regime**（LP を回す／設計級）＝次のセッションでやっても損しない。★ **便ごとの逐語は `HANDOFF-ARCHIVE.md`「第374セッションの『判定の記録』逐語」**（20,000 字の門で出した）。
