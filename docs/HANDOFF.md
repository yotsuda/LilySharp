...
- ~~**同じ測定が名指した残り 3 件**~~ — **3 件とも閉じた（第376 第 4 便・ユーザー承認）**＝⑴ bracket `802dfcc4`・⑵ 縦線 `e9fb8a06`・⑶ 譜線の左端（§1 ⑼）。
  **まだ開いているもの**: **譜線の右端**（LP は `staff-symbol.cc:84` で右端も太さの半分引くが、Lily# の右端は LP と未測定）・**入れ子の delimiter**（下の ★）。以下は起票時の記録:
