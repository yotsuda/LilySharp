...
最終更新 第336セッション＝**入り方は第298〜第335 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。第335 の次の一手＝**「Express Yourself の 5 ss を縦の台帳で起票する（題付き 1 頁目の yoff・staff＋tab の段の tallness）」**に入り、**台帳 15 点（専用プローブ 2 本）を起票して commit した。製品コードは 1 行も触っていない（出力不動・snapshot 0 枚）。Express Yourself の 5 ss は 2 つの島に割れ、どちらも*出力を変える移植*なので承認待ち（下 ⑺）。骨は 7 つ**:

★★ **⑴ 計器**: `audit/lp-geometry/probes/titled-page.ly`（TTL 題＋作曲者／TTT 題のみ／TTC 作曲者のみ／TTN 対照）と `staff-tab-page.ly`（STBN 自然 3 段／STBK 15 段・max-systems-per-page 8／STB8 8 段ちょうど）＝`Measure-LilyPondProbe.ps1 -Probe … -Prefix PROBET|PROBEST`（各 10〜20 秒・自分で first-ref／inside／system を刷る）。Lily# 側は `RenderedGeometry.StaffTabPair*`（5 線の五線＋1.5 間隔 5 線の tab の対を読む・**tab の弦線は数字の周りで途切れて描かれるので線分を Y で束ねて全長で判定**＝最初の版は 28 本と数えた）。捨て計器 `scratch/p336/ZzP336ProbeTests.cs.txt`（段数ループの採点を吐く・出力 `probe2.txt`）。
⚠️⚠️ ★★★★ **⑵ 島 1＝題の帯**: LP は書名を*top-aligned な paper system* として頁の鎖に入れる（`paper-book.cc:443 book_title` の `align_to UP`・`:570-580 get_system_specs`）。top-markup-spacing 4（床＝padding 1＋上インク 0 は負ける・**title の Y-offset は 4.000000 ちょうど＝予測どおり**）→ markup-system-spacing（床＝題の深さ＋段の上インク＋0.5 が縛る）。**TTL 20.073246＝5.690551＋4＋6.106695（列の深さ）＋0.5＋3.776（ト音記号の頂）**。Lily# は題の baseline を MarginTop に描き（ascent は余白の中）、baseline 以下 3.974 だけを top-system-spacing の床に足す＝14.440551。**残差 −5.632695／TTT −5.555091／TTC −3.930450／TTN exact**<!-- ledger: page.titled.first-staff-refpoint = -0.001565 --><!-- ledger: page.title-only.first-staff-refpoint = -0.001981 --><!-- ledger: page.composer-only.first-staff-refpoint = -0.00005 --><!-- ledger: page.untitled-control.first-staff-refpoint = 0 -->。3 項＝(a) 題の上端が LP は margin＋4・Lily# は baseline が margin (b) 列の深さ 6.107（4.56 ss の題＋baseline-skip 3.5 の作曲者行）対 3.974 (c) padding 0.5 対 1。**設計差**（Lily# に title system も top-markup／markup-system のばねも無い）＝題付きの本全部の 1 段目が 4〜6 ss 下がり頁が割れ直る→承認事項。
★★★ **⑶ 島 2 の対照＝staff＋tab の自然本 STBN**: 4 点 exact（first-ref 11.690551／譜間 9／段間 12／6 staves）。
⚠️ ★★ **⑷ 最初の版は*行割り*が違った**（LP 8 小節/段・Lily# 7 小節/段＝T7 spacing 族）→ 両側 `\break`／`break` で 8 小節行を強制（Express Yourself 自身が `break` 付きで行が揃っていた理由）。**縦の対は行を揃えてから。**
⚠️⚠️⚠️ ★★★★ **⑸ 島 2＝頁ブレーカーの staff＋tab の tallness**: STBK は **LP 8,7（1 頁目 f＝−0.200517・top 6＋6f／譜間 9＋f／段間 12＋4f が 6 桁一致・rod に乗るばね無し）／Lily# 7,7,1（1 頁目 +0.028 で伸び）**。STB8 は **LP 1 頁／Lily# 7,1**。**正体＝ブレーカーの最小積み**: Lily# は staff＋tab の段を tallness 18.000 で積み、LP は 16（譜間*最小* 8＋段間最小 8・インク則 3.05＋1＋4.3＝8.35 が縛って 16.35）。**2 項とも `PageLayouter.BuildSystemDetails`**: (a) `RefpointExtentDown = −(staffHeight − 2.0)`＝tab の refpoint を nominal 半譜で読む（実は外弦から 3.0）——**そこの LILYSHARP-OWN 札が「頁の*段数*を測る点が出たら消える」と書いていた札で、`page.staff-tab.compressed.staves-on-first-page`（−2）と `page.staff-tab.eight-systems.*`（−2／+1）がその点**<!-- ledger: page.staff-tab.compressed.staves-on-first-page = 0 --><!-- ledger: page.staff-tab.eight-systems.staves-on-first-page = 0 --><!-- ledger: page.staff-tab.eight-systems.page-count = 0 -->／(b) staffHeight＝見積りの body で譜間が*自然* 9、LP の `Line_details` は pure minimum translations（`constrained-breaking.cc:562 pure_refpoint_extent` → `system.cc:864` → `align-interface.cc:137-143 get_pure_minimum_translations`＝staff-staff-spacing の minimum-distance 8）。各 1.0 × 7 段＝14 ss ＞ 8 段に要る圧縮 8.45。**移植は多段・tab 本の頁割りを全部変える→承認待ち。** 圧縮側の距離 4 点は「違う頁を読んでいる」数として記録（why に明記）。
★ **⑹ 名指しだけの未解明 1 つ**: `page.staff-tab.compressed.last-staff-to-foot` +0.49<!-- ledger: page.staff-tab.compressed.last-staff-to-foot = 0 -->＝foot の rod（LP＝tab 外弦インク 3.05＋padding 1）が Lily# で 4.54。tab の下端インク（Lily# の bottom 0.540 対 LP 0.05）か padding か。頁の段数には効かない（ブレーカーの選択が違う本で読んだ数）。✅ **第 4 便が閉じた（⑽⒜＝幻のト音記号）**。
★ **⑺ 数**: 台帳 770 → 785 点・exact 603 → 608・ss 非ゼロ 197 → 204（総和 26.785 → 46.712＝足した点が可視化した分）・count 点 164／うち非ゼロ 5・OPEN 0。full **7143 / 0 / 4 / 7147（`scratch/p336/run1.trx`・2 分 35 秒・+15＝台帳 15 点）**。

✅ ★★★★ **⑻ 第 2 便（ユーザー「島 2 の移植を承認する。着手して」）＝ブレーカーの純粋枠を移植して commit した**: `BreakerRefpointFrame(ToFirst, ToLastAtMinimum, StaffCompression)`＝`LayoutEngine.BreakerFrame`（`PageAnchorOffsets` の実配置 refpoint 対＋`StaffSpringCompression`＝各 staff spring の 描画 refpoint 距離 − `MinimumDistance` の和）を `PageLayouter.BuildSystemDetails` に通し、body・Height・InverseHooke・refpoint 対を最小枠で建てる（LP `constrained-breaking.cc:562 fill_line_details` の `pure_refpoint_extent` → `system.cc:864` → `align-interface.cc:137-143`）。呼び手は 2 つとも（実配置の頁割り・段数ループの見積り＝小節ごとに置かれた段の枠を持ち、最も高い body の小節の枠を採る）。配置側は `SystemDetails.StaffCompression`（LILYSHARP-OWN・札あり）で描画 body を戻す（`DrawnBody`）。**結果**: STBK の tallness 18.000 → 16.000・**段数 5 点が exact**（`page.staff-tab.compressed.staves-on-first-page`／`eight-systems.*`／**hara-kiri 2 点 `lyrics.hara-kiri.*.staves-on-first-page`（2026-08-19 から −2）も頼まれず閉じた**＝2 段譜の同じ項）・圧縮距離 3 点は −0.070／−0.0117／−0.0467＝**1 つの力の差 0.01115（6×・1×・4×＝強度は全部正しい）× 強度和 43 ≈ 0.49＝`last-staff-to-foot` の +0.49 が唯一の残り**（why に連鎖を書いた）。**snapshot 1 枚**（`test/system-count-line-start-ink`＝Le Freak staff＋tab 11 段: 頁割り 7,4 → 8,3・**LP 双子 `scratch/p335/lsi.ly`＋`pages-when.ly` も題＋8／3＝同じ割り**）を再ベース・番人 `LineStartInk_IsPricedPerCandidateLine` の 11 段不動・`PageLayouterTests.BuildSystemDetails_PricesTheBodyAndRefpointsInThePureFrame` 新設。CHANGELOG Engraving 1 項・`APPROXIMATIONS.md`／`audit/magic_constants.csv` 再生成。**full 7143 / 1 / 4 / 7148（`scratch/p336/run2.trx`・4 分 34 秒・赤 1＝定数棚卸しの行番号・再生成で緑）**・`--no-incremental` build 0 エラー・Core 0 警告（102 秒）。

✅ ★★★★ **⑼ 第 3 便（ユーザー「島1も移植して」）＝題の帯を LP の book-title column として移植して commit した**: `HeaderBand`（`ly/titling-init.ly` の `bookTitleMarkup`＝top-aligned な列・題 `\huge \larger \larger \bold`＝**2.2 × 2^(4/6)＝3.49＝Lily# が元から描いていた寸法（magstep は 2^(1/6)/段・1.2 ではない）**・作曲者は text size 2.2 で**立体（italic をやめた）**・baseline-skip 3.5＝`stack-lines` の「基線は 3.5 以上離す／インクは重ねない」の max・深さは `ScoreTextMetrics` のインク）。**頁の鎖**: 題で開く頁は top-markup-spacing（`VerticalSpacingParameters.TopMarkup`・4/0/1・`LayoutUtilities.TitleTopSpring`＝床 1 は負けて 4）→ markup-system-spacing（`TitleToSystemSpring`＝床＝深さ＋段の上インク＋0.5）。**ブレーカー**: 題は line 0（`PageLayouter.BuildTitleDetails`＝Prob の Line_details・inverse_hooke 1.0・Forbid＝LP の `compress_lines` の代わり）・min/max-systems-per-page の数に入れない・`MinWhitespaceAtTopOfPage` は題行なら top-markup を読む・段数ループも題行を前に置いて採点。`CreateBreaker` の headerHeight は 0（LP の header_height_ は頁ヘッダ）。描画は `PageLayout.Header/HeaderTop`（鎖が解いた位置）から基線を取る（`DrawHeader`）。**結果**: `page.titled.first-staff-refpoint` −5.632695 → **−0.001565**・title-only −0.001981・composer-only −0.00005<!-- ledger: page.titled.first-staff-refpoint = -0.001565 --><!-- ledger: page.title-only.first-staff-refpoint = -0.001981 --><!-- ledger: page.composer-only.first-staff-refpoint = -0.00005 -->（残りは題文字列のインク高さの Skia outline 対 Pango の差＝字形の欠片）・対照 exact。**Express Yourself は 8＋3 段（11 段）＝LP と同じ頁割り**（`scratch/p336/exy-after.svg`・第 1 便の 7,7（14 段）から）。**代価: snapshot 187 / 245 枚**（題か作曲者を持つ fixture 全部＝帯ぶん 5.6〜6.1 ss 下がる・無題は byte 同一）を再ベース（台帳 3 点を鍵に・RULES §5.2.1③）。番人: `LeadSheetHeaderClearanceTests`（`HeaderBand` から帯の下端を読む・新 `HeaderBand_StacksTheComposerUnderTheTitle_BySkipOrByInk`）。**full 7144 / 1 / 4 / 7149（`scratch/p336/run3.trx`・3 分 53 秒・赤 1＝`LpReferenceCitationTests` の無名引用ラチェット＝`.ly`/`.scm` の 2 節ハイフン名は記号に数えない→命名し直し・番人 8 クラス 825 本再走で緑）**・`--no-incremental` build 0 エラー・Core 0 警告（61 秒）。⚠️ **`LooksLikeLilyPondSymbol` はハイフン名 3 節以上・下線名 2 節以上・8 文字以上**＝`stack-lines`・`baseline-skip`・`bookTitleMarkup` は名前にならない。行範囲を散文に下げるか、C++ の下線名を挙げる。

✅ ★★★ **⑽ 第 4 便（ユーザー「次便がこの便で有利なら着手」→ 計器が温かい foot rod と掃きに着手）**: **⒜ foot rod +0.49 の正体＝幻の音部記号**: `SkylineBuilder.SeedClef` は `ClefGlyph(staff.Clef)` で字形を選び、tab の `Clef` は `ClefType.Tab` → switch の既定腕＝**ト音記号の輪郭（中線の下 3.55・上 3.80）が全 tab の skyline に播かれていた**。6 弦（±3.75）は隠し、5 弦ベース（±3.0）は下 0.55・上 0.80 が突き出る＝`page.tab-only.*` や TBL 本が見なかった理由。直し＝tab は `GlyphMetrics.ClefTabOutline`（clefs.tab の LILC 箱 ±2.888・`ly/engraver-init.ly:1218 clef::print-modern-tab-if-set`）を中線に播く。**結果**: `last-staff-to-foot` +0.49 → 0・圧縮距離 3 点も ≤6e-7（頁の力が LP の −0.200517 と 6 桁一致）<!-- ledger: page.staff-tab.compressed.first-staff-refpoint = -0.000000571 --><!-- ledger: page.staff-tab.compressed.staff-to-tab-inside = -0.000000429 --><!-- ledger: system.staff-tab.compressed-distance = 0.000000286 -->。snapshot 18 枚（tab 本 17 枚＝1 頁の高さが 0.55〜0.65 縮む・Le Freak fixture は 8,3 のまま ≤0.2 の再配置）。**⒝ 実コーパス掃き**（`scratch/p332/relayout332.ps1 -Csv scratch/p336/structure-after336.csv`・837 秒・2 島の後）: **T7 KindMatched 209 / SigMatch 149 / 段違い 41 / 小節数違い 19＝第335 後と同数**・全対 SigMatch 443 → 445・SystemsMatch 481 → 483・全対一致の本 208 → 209。**得た 4＝Express Yourself ×2・Nothing's Gonna Stop Us Now ×2（両枠とも LP と一致）／失った 2＝Freedom ×2（37 段 → 39 段: 先頭 8 → 4,4・後半の 4 → 2,2・LP は 37）**／残酷な天使のテーゼ 30 → 26 段（LP 20・近づいた）。⚠️⚠️ **この掃きは頁割りに盲**（CSV の `LysPages` は全冊 0＝`lysc layout --all` の dump が頁を持たない）＝2 島の頁の効果は段数ループ経由でしか見えていない。**頁数を比べるには layout dump に頁を足す**（LP 側 `LpPages` は在る）。

✅ ★★★ **⑾ 第 5 便＝Freedom の「退行」は 2 島と幻の音部記号の*間*の木で測った数だった**: 掃き ⑽⒝ は clef seed を直す**前**の bin で回っていた。直した木で `scratch/p336/ZzP336bProbeTests.cs.txt`（Freedom.lys の段数ループ採点＝`freedom-probe.txt`）を吐くと **37 段＝LP の署名と一致**（`8,4,4,4,4,4,4,2,…`）。得点も LP（`scratch/p336/freedom-lp-dpbs.txt`・`-ddebug-page-breaking-scoring`・bassTab book）と 1% 以内で並ぶ: 40 段 224.90 対 222.27／39 段 197.71 対 195.91／38 段 205.39 対 203.85／**37 段 178.19 対 177.49**／36 段 ∞ 対 inf。⚠️ **LP の staff book は 39 で止まる**（40 → 39 198.89 → 38 206.14 で降下の出口が鳴り 37 を試さない）＝同じ `.ly` の 2 冊で LP 自身が 39 と 37 に割れる。CSV の LP 側「staff 37」は第332 の掃き（`scratch/p332/out/Freedom/lp.out`・9/05 1:23）の数で、今日の LP 出力の staff book と食い違う＝**LP 側の基線が古い可能性**（`.ly` の編集か 2.26.0 の再現性か＝次便が `lp.out` の日付と `.ly` の mtime を比べる）。**掃きを clef 修正後の bin で取り直した（`scratch/p336/structure-after336b.csv`・413 秒）: T7 209 / 149 / 41 / 19 不動・全対 SigMatch 443 → 447・SystemsMatch 481 → 485・全対一致の本 208 → 210・退行 0**。動いた本＝Express Yourself ×2（14 → 11 段＝LP）・Nothing's Gonna Stop Us Now ×2（21 → 20＝LP）・That's The Way ×3（21 → 25・LP 24・近づいた）・残酷な天使のテーゼ（30 → 26・LP 20・近づいた）。**Freedom は 37 のまま一致**（⑽⒝ の 39 は clef 修正前の数）。

★ **開始時裏取り**: HEAD `1ef2e097`・**未 push 60**（第335 の 58＋第 4・5 便の HANDOFF 2 本）・木 clean・未追跡 0・`bin\Debug` は net10.0 のみ・solution build 0 エラー。
**終了時（第 1 便）**: **commit 2 本（台帳 15 点＋プローブ 2 本＋`RenderedGeometry`／この HANDOFF＋ARCHIVE＝第334 の経緯を先頭へ逐語）・未 push 62・木 clean・未追跡 0**・`--no-incremental` build 0 エラー・Core 0 警告（64 秒）・台帳の既存点／snapshot／`.lys`（追跡）不動。
**終了時（第 2 便）**: **commit 2 本（島 2 の移植＝Core 4＋番人＋snapshot 1＋台帳 8 点更新＋CHANGELOG＋APPROXIMATIONS＋magic_constants／この HANDOFF）・未 push 64・木 clean・未追跡 0**。
**終了時（第 3 便）**: **commit 2 本（島 1 の移植＝Core 9（新 `HeaderBand.cs`）＋番人 3 ファイル＋snapshot 187＋台帳 3 点更新＋CHANGELOG＋APPROXIMATIONS＋magic_constants／この HANDOFF）・未 push 66・木 clean・未追跡 0**。
**終了時（第 4 便）**: **commit 2 本（tab の clef seed＝Core 1＋snapshot 18＋台帳 4 点更新＋CHANGELOG＋再生成 docs／この HANDOFF）・未 push 68・木 clean・未追跡 0**・full 7145 / 0 / 4 / 7149（`scratch/p336/run4.trx`）・`--no-incremental` build 0 エラー・Core 0 警告（130 秒）。
**終了時（第 5 便）**: **commit 1 本（この HANDOFF）・未 push 69・木 clean・未追跡 0**・製品不変。
⇒ ★★★★ **次の一手**: ✅ ⑴ 島 2（⑻）・✅ ⑵ 島 1（⑼）・✅ ⑶ foot rod（⑽⒜）＝**STBK の 9 点全部 exact**・✅ ⑷ Freedom は退行ではなかった（⑾）。**⑷′ 掃きの LP 基線の鮮度**（`scratch/p332/out/*/lp.out` は 9/05 1:23 の第332 掃き。Freedom の staff book は今日の LP が 39 で止まるのに CSV は 37＝`.ly` の mtime と `lp.out` の日付を全冊で比べ、古い本は LP 側だけ取り直す。25 分の sweep332 の型）。**⑸ 掃きに頁を足す**（`lysc layout --all` に頁ごとの段数を出させ、parse321 の `LysPages`/`PagesMatch` を生かす＝2 島の頁の効果を全冊で数える）。⑹ 以下は第335 の並び（`lysc ly` の ChordNames／lyric row の slot の投票／ps2 の 0.017・beam-over-stem bar 1 の +0.33／push 後 `gh run list`／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／A/B/D/E／C⑴／G）。⑷ 以下は第335 の並び（⑵′ `lysc ly` の ChordNames＝ユーザーと綴りを決めてから／lyric row の slot の投票／ps2 の 0.017・beam-over-stem bar 1 の +0.33／push 後 `gh run list`／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／A/B/D/E／C⑴／G）。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
