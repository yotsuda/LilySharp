...
★★ **テキストの advance が LP と 1 段あたり ~0.032 ずれている**（**ottava の島が測って出した副産物**・
**点は ottava.x.line-start-to-notehead の中にしか無い＝専用の book が要る**）。
```
"8"   単一グリフ  Lily# 1.212200000 対 LP 1.213302362   −0.001102362  ← ほぼ一致
"8va" 3 グリフ    Lily# 3.907200000 対 LP 3.842337008   +0.064862992  ← 累積している
⇒ bbox の緩さではない（確認済み）。**グリフ間の advance/kerning**。
```
**どのテキスト grob にも効く**（歌詞・TextScript・和音記号・マーク）ので、
**直す前に「どの文字列で何段ずれるか」を測る book を作ること**。一つの定数では無い可能性が高い。
