...
⇒ ★★★ **次の一手＝§2 F ⒫ の残る前提条件 ⑵（構造だけを担う section にラベルを刷らない既定）**。**⑴ は本便で入った。** ⚠️ **⒫ 自体は「方向には合意した・決定ではない」ままなので、⑵ が済んだらユーザーに訊く項。** ★ **独立した小粒は 2 つとも手つかず**: **⒭（section ヘッダの `clef` が効かない）と ⒮（`form` の repeat ＋ music の volta で `.ly` が余分な `\bar ":|."` を出す）。** ⚠️ **§2 U8 の残り**（`grace { }` 本体の和音・休符／スラー・梁・タイ／注釈）は**本便では 1 行も触っていない**。
...
終了時: **commit 6 本**（**本体 `b06f6968` ／ 文法書 `96a965bf` ／ §1・§2・§3 とアーカイブ `b8954d85` ／ ubuntu 脚 `47ef56c3` ／ RULES への汎化 `515039e5` ／ 数の閉じ直し（この commit）**）・**未 push 6**・木 0・未追跡 0。**Windows Debug 6730 / 0 / 4 / 6734**（開始 6705 から **+29**＝**本便の網 29 ケース**）。Core 0 エラー 0 警告。**台帳・追跡コーパス・snapshot は開始時と同じ**（748／588／190／24.714570188／158 中 2／`OPEN:` 0／581 冊／231 枚）。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけが 6 行動いた**——**定数は 1 つも足していない**（`audit/magic_constants.csv` も不動）。
★★ **WSL ubuntu Release も同じ 6730 / 0 / 4 / 6734**（`b8954d85` の木で・build 7 秒 / test 26 秒）——**§0 の「この機械の Windows の緑は Windows の緑でしかない」の両脚を、本便も両方読んだ。** ⚠️ **CI は `be16a770` までしか走っておらず、未 push 4 本は未実行。**
...
⚠️ **計器は `scratch/p304/` に全部残っている**: `poison.py`（**17 本**）＋`runpoison.ps1`＋`poison-results.txt`・`sweep304.ps1`＋`sweep.json`／`sweep2.json`／`sweep3.json`（**掃きは 3 回。引くのは 3 回目＝出荷した木**）・A/B のプローブ（`u*` 相対と絶対の対・`w*` ending・`y*` phrase 本体・`x*` slash）。⚠️ **`scratch/` は git 管理外。** ⚠️ **`C:\MyProj\lys-base304` の worktree は次便が消してよい**（`git worktree remove`）。
...
> ✅ **前提条件 ⑴ ＝ 引き継ぎの記法**（下の ⒬）は **2026-08-31・第304 で入った**（`~B'` / `~B,` / `[1. B']`）。**残るのは ⑵ だけ。**
