...
**3432 passed / 0 failed / 3 skipped**・Core 0 warn 0 err・
**LP 忠実度 125/157 exact・total |residual| 2.506328 ss / 135 distances・counts 20/22**。
...
- ⑴ **群間ギャップの上側高さが名目 `staffHeight`**（`LayoutStaffGroups` の `StaffGap`）。
- ⑵ **`IsSpecSpacedRowBoundary` の門**（消すには残りを refpoint 枠へ通す）。
- ⑷ **silhouette の「選択」はまだ要素基準**——両端とも text row の system は譜が 1 枚も入らない。
  ここは**中央線と clef も placement から採る**必要があり、**そこが 19 枚の出どころ**。

⚠️★★ **「この島は snapshot が ~20 枚動く」は粗すぎた**（何セッションも引き継がれ、私も昨日
繰り返した）。**扉ごとに値段が違う**: ⑶ は 1 枚、⑷ が 19 枚。⇒ **島を「何枚動く」で見積もらない。
どの量を変えるかまで割ってから数える。**
