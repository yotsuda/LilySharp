...
> ## ▶ **⒫ `|:` `:|` `[N. …]` を form 限定にする＝【ユーザー決定 2026-08-31・第304】** ← **第303 起票・第304 で決定・未実装**
> **決定**（2026-08-31・第304・ユーザー）: **音楽ブロックには `|:` `:|` `[N. …]` を書けなくし、`form {}` の中だけで書けるようにする。**
> **狙いは `.lys` の可読性**——構造が 1 か所に集まる。**第303 の「方向には合意した・決定ではない」は決定に変わった**（§3）。
> ⚠️⚠️ ★★★ **着手前に残っている問いは 1 つ＝*移行*。** **禁止すると*ユーザーの 326 冊のうち 115 冊（35%）が通らなくなる***（2026-08-31・第304 実測＝**music に `|:`/`:|` か `[N. …]` を書く本**。**`[N. …]` だけなら 77 冊**）、**追跡側も 581 冊中 20 冊**。✅ **答えは ⒜ 即エラー**（2026-08-31・第304・ユーザー決定。猶予も自動書き換えも置かない）。⚠️ **私は ⒞ を勧めたが、数を見たうえでユーザーが ⒜ を選んだ**——**「実装コストや移行コストは選択肢の並べ方に混ぜない」の適用**。⇒ **⒫ の着手を止めるものは*もう無い*。**
...
> ⚠️⚠️⚠️ ★★★★ **着手前の数を第304 が測った。⚠️ 3 度間違えてから収束した＝20 → 14 → 12 → **11 冊**。**
> **⒜ 1 度目は `//` コメントを読んだ**（`break.lys` のヘッダの「段の割りは `[1-4][5][6]`」）。
> **⒝ 2 度目は行頭アンカーの regex が*1 行の* `form { … }` を剥がせなかった**（`tocoda-volta-clearance.lys` は `|:` を form に書いている）。
> **⒞ 3 度目は lyrics の `[1. …]` を数えていた**——**あれは `LyricVoltaGreen` という別ノードで、繰り返しではなく*何番の歌詞か***（`ParseLyricVolta`）。**⒫ が禁止してはいけない。**
> ⇒ ★★★ **正しい数え方**: **`//` を除去 → `form <name> { … }` を*波括弧の対応*で除去 → 残りに `|:` / `:|` / `[N.` があるか → lyrics の volta を除く。**（計器は `scratch/p304/count.py`。）
> | | 冊数 | 何が要るか |
> |---|---|---|
> | **追跡本ぜんぶ** | **11** | |
> | うち **snapshot 持ち** | **3** | `grandstaff-repeat` / `lead-sheet-repeat` / `rehearsal-marks-inside-containers`。**インクが同一に切り直せれば再ベースは 0 件**——そこを狙って測ること |
> | うち **LP 回帰コーパス（81 冊）** | **1** | `repeat-volta-initial-grace.lys` のみ。**ユーザー決定 ⒝＝切り直して LP に対して測り直す** |
> | うち **`audit/lpreg/`** | **5** | LP コーパスではない（旧セッションの作業場・`.lys` 257 冊）。`VoltaBracketSkylineTests` の fixture |
> | うち **showcase** | 2 | snapshot 無し |
> ⚠️ **禁止の射程に注意**: **lyrics の `[N. …]`（歌詞の番）と `repeat percent` / `repeat unfold` / `tremolo` は残す。** `chords` 行の `|:` は繰り返しなので対象。
...
> ⇒ ★★ **残るのは網の張り直し**: **`grandstaff-repeat` と `grammar-tour` の主張（1 パートだけの繰り返し）は ⒫ の後*書けない*ので観測できなくなる**——**黙って別のテストに変えないこと**（§5.4）。
> ★ **ユーザーのコーパスには当たらない**——**`|:` を music に書く 115 冊は全部 1 パート**（この 115 は 3 通りの数え方で同じ数だった＝頑健）。
> ⚠️ **掃きは 126 冊級の差を出す**（115 ＋ 11）が、**読むのではなく*機械で*確かめられる**——**head 側の新しい診断がその 1 つだけであることを確かめる**形にすること。
> ⚠️ **⑵ が解く問題**: **今は `~name` と*参照ごとに*書かないとページにラベルが増えるので、繰り返しの縁を運ぶためだけに作った section が全部ラベルを持つ**（**ユーザーの本の form 本体で、素の名前 2309 個 対 `~` つき 260 個**＝第304 実測）。**⒫ は section を切り直す本を増やすので、先にここが要る。**
...
> ## ▶ **⒮ `.ly` 双子は music の inline volta を*書き出さない*（起票時の見立て「form の repeat と混ぜたときの余分な `\bar ":|."`」は誤りだった）** ← **第303 起票・2026-08-31・第304 が測り直して書き換えた**
...
> ⇒ ⚠️⚠️ ★★★ **着手の前にユーザー判断が要る**（**第304 は判断待ちと見て着手しなかった**）: **⒫（`\|:` `[N. …]` を form 限定にする）が入れば、この綴りは*書けなくなる*ので双子に教える必要も消える。** **「双子に inline volta を教える」と「⒫ で綴りを禁止する」は同じ 1 つの決定の裏表**で、**⒫ は「方向には合意した。決定ではない」のまま。**
