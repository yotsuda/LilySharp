...
最終更新 第413セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `8474de18`・**未 push 3**＝第412 の 3 本・**full `sessions/p413/run1.trx` 8718 / 0 / 3 / 8721＝第412 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 131 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、**第412 が「次便が見てもそれは第412 の欠陥ではない」と予告したもの**）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 411`＝19 行 4,914 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」が名指していた ★★★ **⒜″ `GetOrComputePagingAugment` の鍵を作り直す**を採った＝**第412 が 3 便かけて測り切り「次便は実装から始められる」と書いた当のもの**。★ **ベンチは着手前に AskUserQuestion で静かな窓をもらった**。根拠は割当バイトと**回数**（CLAUDE-OPERATIONS §2）。道具は pwsh MCP（Bash 0 回）:

★★★ **⑴ 実装＝「system 番号ごとに 1 枠」をやめ、*配置ごとに 1 部屋*にした**（`SystemLayoutCache.GetOrComputePagingAugment`・製品コードの変更はこの 1 メソッドと持ち物だけ）。**2 という数は調整つまみではなく count loop 自身の上界**＝`Layout` は `PlaceSystems` を**最大 2 回**しか呼ばない（ideal の見積もりと、`ChooseSystemCount` が別の数を選んだときの本番）。⇒ **店は「今まで通りいちばん広い段数 × 2」で頭打ち**＝世代退避はやはり要らない。**古い部屋で当たった照会はその部屋を*昇格*させる**ので 2 つの配置が 1 部屋ずつに落ち着き、**miss が落とすのは古い方＝今の配置が使っていない側**。⚠️ **鍵の検査は 1 バイトも変えていない**（`Serves` に畳んだだけ＝参照同一性 2 本と `program.Matches`）＝健全性の議論は第141〜142 のまま。

⚠️⚠️ ★★★ **⑵ 最初の測定は「効果なし」と答え、外れていたのは*母集団*だった。** 先頭 20 冊で A/B を回したら **forward で打鍵の 0.15%・miss 393 → 391（14 冊中 1 冊しか改善しない）**。**全 292 冊で回したら 8.82%**。⇒ **効いたのは 231 冊のうち 44 冊だけで、その 44 冊では打鍵の 27.70% が消え、その冊の miss の 80.7% が消える**（第412 が「P2 発火は 292 冊中 46 冊」と測った数とほぼ同じ）。⇒ ★★★ **効果が少数の冊に集中している島では、部分コーパスは「効果なし」と答える**（§5.3 へ）。**アルファベット順の先頭 20 冊には、その 44 冊がほとんど入っていなかった。**

⚠️⚠️ ★★★ **⑶ もう 1 つ交絡があり、こちらは harness の側だった＝「編集して元に戻す」と、2 つ目の部屋が*もう一方の配置*ではなく*もう一方の本文*を持って当たる。** 第412 と同じ alternating harness では **14.04%** 出るのに、**戻らない編集列（forward）では 8.82%**。★ **合図は「全冊ぴったり 7 miss 減」**＝`8 打鍵 − 1`＝**冊の大きさと無関係な数**で、これは配置の衝突ではなく undo が前の本文の entry に当たっているだけ。⇒ ★★★ **memo の枠数を変える A/B は、戻らない編集列でも測る**（§5.3 へ）。**採る数は forward の側。**

★★★ **⑷ 実測**（**forward・実コーパス 231 冊 × 8 打鍵**・`RenderIncrementalPages`・**Release**・割当バイトと照会回数・計器は Lab `sessions/p413/Zz413Probe1.cs.txt`＋一時の `ZzOneRoom` 継ぎ目（**commit しない**）・全文 `two-rooms-ab.txt`）:

| | 打鍵あたり B | hit | miss | hit 率 |
|---|---|---|---|---|
| 1 枠（前） | 10,890,100 | 39,702 | 18,228 | 68.5% |
| 2 部屋（後） | 9,929,310 | 49,564 | **8,366** | **85.6%** |

⇒ ★★★ **miss は 18,228 → 8,366＝54.1% が消え、打鍵は −8.82%**。⇒ ★★★ **第412 の予測（賞金 8.3%＝無条件 3.8% ＋「前の打鍵の P2 の枠が生きていれば」の 4.5%）は当たり、*未検証だった側も出た***。★ **雑音の大きさも測った**＝同じ 1 枠構成を 2 回回して **0.0001% 差**（miss は完全一致）＝**8.82% は雑音の 8 万倍**。⚠️ **割当の通貨での話**（§5.3・`Matches` は 1 バイトも割り当てない）。

★★★ **⑸ 出力同一は「思った」のではなく*その場で*測った。** 同じ harness に全ページの SHA-256 を足して 1 枠と 2 部屋を突き合わせた＝**forward 231 冊・alternating 292 冊の *8 打鍵ぜんぶ*で、絵が動いた本 0 冊**（＝4,184 打鍵ぶんのページ）。証明 ⑴ は **絵が動いた本 0 / 81**（⚠️ **Lab に baseline が無かった**＝変更を `git stash push` して Release を建て直し baseline を取り、pop して比べた。§5.1 の手順どおり）。⚠️ **証明 ⑴ のコーパスは全冊単一システム＝count loop が 2 度置く本を 1 冊も含まない**＝**この変更にとって効いている証拠は上の SHA-256 のほう**で、⑴ は儀式として通した。

⚠️ **⑹ 2 分 15 秒を溝に捨てた＝`dotnet build` が落ちたのに `dotnet test --no-build` が*旧 dll* で走り、「成功!」と言った**（`IncrementalHash.Create`／正しくは `CreateHash`）。⇒ **`--no-build` の前は build の exit code で止める**（§5.5 の「`--no-build` は偽緑」の、*コンパイルエラー*版）。

★ **⑺ 網 2 本**（`SystemLayoutCacheTests`）＝**1 つの system 番号の 2 つの配置が両方とも当たり続ける**（**1 枠なら 0 hit / 8 miss と読む**）と、**3 つ目の照会は古い部屋を落とし、落ちた側は*同じ silhouette* に計算し直る**。⚠️ **これは liveness の網で、それ以上にはなれない**——**何も返さない memo も*正しい*ので、incremental==full の網は死んだ store の上でも緑のまま**。

★★★ **⑼ 第 2 便＝残った miss は 97.9% が*上流で強制されたもの*で、この店の鍵に取り返せるのは 0.04%。⒜‴ はここで閉じる**（**製品コードの変更 0**・継ぎ目 ZZ413B・計器 `Zz413Probe2.cs.txt`・生データ `why-misses-remain.txt`・forward 231 冊 × 8 打鍵）。⚠️ **まず計器が第 1 便の数を訂正した**＝`PagingAugmentStats` は compiler の誕生から数えるので **1 回目の描画（全 system が新規）が混ざっており、最初の表は cold が miss の 69.8%** と出た。引くと **打鍵あたり miss 2,160 / 1,848 打鍵＝hit 率 95.8%**（⑷ の 85.6% は priming で薄まった数。**差と −8.82% は両側同じ定数なので影響なし**）。**区分は 4 つで尽きる**: **`baselineRebuilt` 2,114＝97.9%（打鍵の 1.66%）／`baselineOther` 41＝1.9%（0.04%）／`programDiffers` 4／`cold` 1**。⇒ ★★★ **`baselineRebuilt` ＝「skyline store がその baseline をこの打鍵で*建てた*」＝どんな鍵でも当たりようがない**⇒ **天井は skyline store の hit 率 96.5%、augment は 95.8%＝天井まで 0.7 ポイント。** ⇒ ★★★ **第412 の「取れない床は打鍵の 1.6%」が、別の方法で 1.66% と出た**（予測は「編集された system」・実測は「baseline を建て直された照会」＝同じものの 2 つの顔）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: **⒜‴ ✅ 第413 第 2 便が閉じた**（取り返せるのは 0.04%）⇒ **次の梃子はこの店ではなく `_skylines`**（miss 1,871・hit 率 96.5%＝**augment の天井そのもの**。⚠️ 何度も削られた島なので、着手前に §2 と ARCHIVE を grep すること）／★★★ **⒵″ `h6e.everythingElse` の残り 20 家族に名前を付ける**＝**打鍵の 5.1% が無名**・**同じ harness に家族 1 つにつき継ぎ目 1 本**（`Zz412Probe1` は Lab に在る）／★ **⒴″ `b.scriptsFingerings`（`ComputeFingeringsAndScripts`）は予備と最終の 2 人合わせて打鍵の 5.0%＝名前の付いた最大の葉**・未分割／★★ **⒴′ 2 度目の注釈パスを要らなくする**（打鍵の 7.7%・第411 ⑶）／⒜ **R13⒝ の実機確認**（第404 ⑵＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械）＝上限は red を 14〜20% だけ実体化＝fingbeam1k で 26.2 MB → 約 10 MB。⚠️ **未着手・未見積もり＝validator ではなく土台の変更**／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った（採るならそれを知って採る）／**R13⒦ ⒤ ✅ 第410 が「測って、やらない」で閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅ 第407 が測って「やらない」と決めた**／**⒟ ✅ 第407 で閉じた（R2R）**／LP 双子が要る R7〜R11／**R15 ✅ 削除は第407 で完了**（残り＝「2 綴り」一族のみ＝統合・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑽ 終了時**: commit 3 本（第 1 便 code `9059999f`・docs `ddb49c10`・第 2 便の docs 1 本。**第 2 便は製品コード 0 行**）＋Lab 2 本・**最終 full `sessions/p413/run4.trx` 8720 / 0 / 3 / 8723**（⚠️ **第412 最終の 8718 / 8721 から +2＝今便が足した liveness の網ちょうど 2 本**・計器と継ぎ目は全部外してあり `Zz413`／`ZzOneRoom` の grep 0 件）・§7.5（対 `8474de18`＝スクリプトの出力）**Core `+` 64 行／REF 0／OWN 0**（★ **数値定数を 1 つも足していない**＝「2 部屋」は定数ではなくフィールド 2 本〈`Recent`／`Older`〉で、**数ではなく*形*が count loop の上界を言っている**・LP 由来の式なし）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。⚠️ **棚卸し `docs/APPROXIMATIONS.md` と `audit/magic_constants.csv` は再生成した**＝`MultiStaffLayouter.cs` に註を 2 行足したので**行番号が 2 つずれただけ**（**数は 929／254 とも 1 つも動いていない**＝手順どおり diff を読んでから回した）。§7 4（恒久知識）＝**RULES §5.3 に 2 項**（「**効果が少数の冊に集中している島では、部分コーパスは『効果なし』と答える**」＝20 冊 0.15% 対 292 冊 8.82%／「**memo の枠数を変える A/B を『編集して元に戻す』harness で測ると、新しい枠が*もう一方の本文*を持って当たる**」＝14.04% 対 8.82%・合図は冊の大きさと無関係な一定の減り）／「**memo の hit 率を『打鍵あたり』で語るなら 1 回目の描画を先に引く**」＝priming だけで miss の 69.8%・引くと 85.6% → 95.8%・**差は汚れないが率と件数は汚れる**）＋**§5.5 に 1 項**（`--no-build` の偽緑の **4 例目＝エラー行を*刷っていた*のに分岐しなかった**）。計器と全文は Lab `sessions/p413/`（`Zz413Probe1.cs.txt`／`Zz413Probe2.cs.txt`・継ぎ目 `zz413-with-seam.diff.txt`／`zz413b-seam.diff.txt`・生データ `two-rooms-ab.txt`／`why-misses-remain.txt`・`summary.txt`）。§7 3.5 は**セッションの頭で**済（第411 を `-Archive 411`＝19 行 4,914 chars）⇒ **HANDOFF は 439.5 KB → アーカイブ後 434.6 KB → 2 便ぶん書いて 447.2 KB＝天井 450,000 まで約 2.7 KB**。⚠️⚠️⚠️ **残りは 2 便ぶんの 1 段落も入らない。次便は §1 を 1 行も書く前に `-Archive 412` を回すこと**（**第412 のブロックは 14,449 B＝それだけ戻る**）（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で 3 回使って残っていない。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412・第413 の欠陥ではない。

## 以下は第412セッションの経緯

最終更新 第412セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `9f5cd52a`・**未 push 0**＝⚠️ **ユーザーが便の外で 52 commit を 11 本に畳んで push 済み**・**full `sessions/p412/run1.trx` 8717 / 1 / 3 / 8721**・CI は HEAD の run が赤・`Session-Check -Build -Test` 131 秒）、⚠️ **裏取りが 1 つ赤を出した＝畳みが SHA 引用を孤児にした**（下の ⑴）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」が名指していた ★★★ **⒵′ `RunPreliminaryAnnotationPass` の*中*を段別に割る**を採った。★ **ベンチは着手前に AskUserQuestion で静かな窓をもらった**。根拠は割当バイト（CLAUDE-OPERATIONS §2）。道具は pwsh MCP（Bash 0 回）:

★★★ **⑴ 第 0 便＝dead citation の天井を上げた**（`9f1d74a9`・**変更は註と定数 1 つ**）。**第402〜411 の未 push 52 commit が 11 本に畳まれて push された**ので、それらの便が自分の commit を名指した文の SHA が全部死んだ＝**dead 699 → 730**。⚠️ **順序が逆だったことは書いておく**＝**census を先に読み**（開始時 full が赤）、**予測はその後**（引用されている畳み前 commit 38 本 − `LooksLikeCitation` が見ない 7 本＝**31**＝実測の増分にちょうど一致）。**答えを知ってから当てた予測は、知る前のものより値打ちが低い**＝確かめたのは「増分は畳みで全部説明が付き、他に何も混じっていない」ことだけ。木は**同一 tree オブジェクト・diff 空**・live は 858（床 564）で落ちていない。⚠️ **`origin/master` の CI もこれで赤**＝この commit を push するまで緑にならない（RULES §5.4 の既知の型）。

★ **⑵ §7 3.5 は*セッションの頭*で回した**（`e7ad703c`＝第410 のブロックを ARCHIVE へ・17 行 12,154 chars）⇒ **HANDOFF は 437.6 KB → 424.6 KB＝天井 450,000 まで約 25 KB**。第410 が途中で回して 3 本赤くした手順を、正しい位置で回しただけ。

★★★ **⑶ 第 1 便＝注釈パスは 1 つの島ではなく、*ほぼ同じ大きさの 2 つ*だった**（**コード変更 0**・全文は Lab `sessions/p412/summary.txt`・計器 `Zz412Probe1`＋`ZzStage`（**commit しない**）・実冊 **292 冊 × 8 打鍵**・`RenderIncrementalPages`・**Release**・バイト）。★ **外側は第411 と一致**（打鍵 6,898,084 B・`PlaceSystems` 32.2%・注釈パス 25.0%／置きの 77.8%・P2/P1 0.45x・P2 発火 368）＝**継ぎ目は測る対象を乱していない**。★★★ **内訳**: **`h8.augment`（`AugmentSkylinesForPaging`）42.7%＝打鍵の 10.7%・筆頭は 41 冊／`h6.annotations`（`CalculateAnnotationLayouts`）40.1%＝打鍵の 10.0%・筆頭は 196 冊／`h2.staffLoop`（予備の梁・タイ・スラー）16.4%＝4.1%・筆頭 53 冊**。残る 4 段（`h7.enrich`・`h3`・`h1`・`h5`・`h4`）は**合わせて 0.8%＝雑音**。⇒ ★★ **バイトでは 6% しか違わないのに*形*が違う**＝**h6 は 3 分の 2 の冊が払う典型的な費用・h8 の総額は少数の大きい冊が担いでいる**。**どちらを採るかは「どの問いに答えるか」で決まる。**

★★★ **⑷ h8 は 94.5% が replay で、その下の memo は冷たい**（`h8b.replay` 打鍵の 10.1%／`h8a.program` 0.6%）。★ **註が謳う速い道は 1 度も通らない**＝「step を 1 つも受けない system は元の skyline のまま」は **59,032 / 59,032 冊すべてで program を持った＝100%**。★★★ **`GetOrComputePagingAugment` の実測**: **P1 hit 84.4%（42,944／miss 7,960）・P2 hit 27.0%（2,192／miss 5,936）**、そして **miss の理由は 13,896 件*全部*が「baseline の参照不一致」で、「entry が無い」も「program が違う」も 0 件**。⇒ ★★★ **鍵の高いほうの項（program の照合）は 1 度も判定に効いていない。** ⇒ **store は「system *番号*ごとに 1 枠・miss で上書き」**（註が自分でそう書いている）＝**2 度目の配置は違う段数で置き直すので同じ番号が別の小節範囲になり、miss して P1 の枠を潰し、次の打鍵の P1 がまた miss する**＝**count loop が段数を変える冊では、この store は温まらない。**

★★★ **⑸ そして h8 は「2 度目の配置で潰れない唯一の段」**（P2 は P1 の 0.16x の system を置く）: **`h8.augment` 0.77x（`h8b.replay` 0.83x）／`h6.annotations` 0.37x／`h2.staffLoop` 0.08x／`h2b.beamGroups` 0.00x（検出 memo が 2 度目に全部返す）**。⚠️ **絶対比は交絡している**（P1 は 292 冊・P2 は発火した 46 冊＝第410 第 6 便が踏んだ穴）**ので「1 系あたり」とは読まない**——**ただし交絡の係数は全行に共通**（同じ冊・同じ run）**なので*順位*は健全**＝**replay は staff loop の 10 倍、2 度目に残る。**

★★★ **⑹ 同じ関数が 1 打鍵に 2 度走り、2 人目はパスの外に居た**。`CalculateAnnotationLayouts` は**予備パスと最終パスの両方**が呼ぶ＝**最終側がさらに打鍵の 5.8%** ⇒ ★★★ **2 人合わせて打鍵の 15.8%＝⑶ のどちらの島より大きい**。**形は両者そっくり**（`e.everythingElse` 59.9%／44.7%・`b.scriptsFingerings` 28.1%／38.5%＝**2.8% ＋ 2.2% ＝ 打鍵の 5.0%**＝いま名前の付いた最大の葉）⇒ **片方に効く手はもう片方にも効く。** ⚠️ **`e.everythingElse` は*まだ 85.5% が無名***（829 MB＝打鍵の 5.1%・約 20 の engraver 家族）＝名付けたのは 4 つだけ（`musicMarks` 0.8%・`chordNames` と `tupletBrackets` は端数・**`figuredBass` は全コーパスで*ちょうど 0 バイト***）。

⚠️⚠️ **⑺ 計器が先に間違え、*負の残り*で自白した**（§5.3 へ 2 項）: 中の段を 1 組の名前で数えたので**予備と最終の両方が予備の取り分に入り**、部分の和が全体を超えて残りが **−932,642,264 B（−57.7%）**と刷られた。**直したのは数ではなく名前**（飛んでいるパスで接頭辞を `h6…`／`f6…` に）⇒ 残りは **+0.6%** と **ちょうど 0**。★★★ **そしてその誤測定がそのまま ⑹ ＝この便の最大の数になった。** ⚠️ **見えたのは「全体の中に部分を入れ子にした」からで**、部分だけ並べた表は超過しても何も言わない。

★★★ **⑻ 第 2 便＝その未測を埋めたら、警告のほうが外れていた**（**コード変更 0**・同じ harness に継ぎ目 3 本・計器は `Zz412Probe1` に同居）。`h8b.replay` の内訳＝**`h8b3.execute`（miss のときだけ）97.3%＝打鍵の 9.9%／`h8b1.build`（毎回）1.4%＝1 照会あたり 399 B／`h8b2.guard`（`program.Matches` を含む hit 判定）＝*ちょうど 0 バイト***。⇒ ★★★ **hit 399 B 対 miss 115,157 B＝289 倍**＝**⑷ の「84.4% hit は 84.4% 節約ではない」は、測ったら*外れ*だった**——**hit はほぼ無料なので hit 率はほぼそのまま節約率で、完全な鍵の賞金は 9.9% 全部**。★ **それでも測ったのは正しかった**＝**外れる側に賭けても、賭けを外したことが分かるのは測ったときだけ**（⑺ と同じ日に 2 度目の「自分の推論を疑う」）。⚠️ **これは*割当*の通貨での話**＝`Matches` は 1 バイトも割り当てずに比べるので、**0 バイトは 0 CPU ではない**（ms で測れば別の答えが出うる・RULES §5.3）。★ **miss の内訳＝P1 7,960 件（打鍵の 5.7%）／P2 5,936 件（4.2%）**で、**P2 の分が「番号ごとに 1 枠・上書き」が作っている分。**

★★★ **⑼ 第 3 便＝その残りの未測も埋めた。床は 1.6% で、鍵が届く先は 8.3%**（**コード変更 0**・slot に「どちらの配置が書いたか」を 1 つ持たせ、miss をその区分で値付けしただけ）。**miss は 1 件残らず「entry は在るが baseline が別物」**なので、区分は 3 つに尽きる:

| miss の区分 | 件数 | `Execute` バイト | 打鍵比 |
|---|---|---|---|
| P1 が P1 の枠で外す（＝**編集された system**） | 2,024 | 257,590,992 | **1.6%** |
| P1 が **P2 に潰された枠**で外す | 5,936 | 609,708,288 | **3.8%** |
| P2 が **P1 の枠を潰して**外す | 5,936 | 732,940,808 | **4.5%** |

★★★ **2 つの 5,936 がぴったり同じ数なのが、擦り切れの署名**＝**P2 が潰した枠の数だけ、次の打鍵の P1 が外す**。⇒ ★★★ **取れない床は打鍵の 1.6%（編集した system は必ず計算し直す）で、残り 8.3% は枠の奪い合いが作っている**。⇒ **内訳の確度は 2 段**: **P1 側の 3.8% は無条件**（同じ計算を、潰されたせいでやり直している）。**P2 側の 4.2〜4.5% は「前の打鍵の P2 の枠が生きていれば」**＝**部屋を配置ごとに分ければ引けるはずだが、そこは未検証**（P1 の非潰し hit 率 84.4% から見て妥当、というだけ）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: ★★★ **⒜″ `GetOrComputePagingAugment` の鍵を作り直す＝測定は全部そろった。次便は実装から始められる**（第 2・第 3 便＝⑻⑼）。**「system 番号ごとに 1 枠・miss で上書き」をやめ、配置ごとに部屋を分ける**（または内容で引く）。**賞金＝打鍵の 8.3%**（**うち 3.8% は無条件＝P2 に潰された枠を P1 が引き直しているだけ**／**4.5% は「前の打鍵の P2 の枠が生きていれば」＝そこだけ未検証**）。**取れない床は 1.6%**（編集された system）。**hit はほぼ無料**（399 B 対 115,157 B）なので hit 率がそのまま節約率。⚠️ **割当の通貨での賞金**＝`Matches` は割り当てずに比べるので **0 バイトは 0 CPU ではない**。⚠️ **土台の変更**＝`PagingAugmentEntry` の持ち方と `_pagingAugments` の鍵を触る（振る舞いは変えない＝出力同一の証明 ⑴ と台帳で見る）／次に ★★★ **⒵″ `h6e.everythingElse` の残り 20 家族に名前を付ける**＝**打鍵の 5.1% が無名**・**同じ harness に家族 1 つにつき継ぎ目 1 本**（`Zz412Probe1` は Lab に在る）＝**いま最も安く測れる島**／★ **⒴″ `b.scriptsFingerings`（`ComputeFingeringsAndScripts`）は 2 人合わせて打鍵の 5.0%＝名前の付いた最大の葉**・未分割／⒜ **R13⒝ の実機確認**（第404 ⑵＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械）＝上限は red を 14〜20% だけ実体化＝fingbeam1k で 26.2 MB → 約 10 MB。⚠️ **未着手・未見積もり＝validator ではなく土台の変更**／⒵ **collect 17.8% と `S1.prelim` 17.0%**（何度も削った島だが実冊では今なお合計 35%）／⒥ は第409 が上限 4.7 ms と測った（採るならそれを知って採る）／**R13⒦ ⒤ ✅ 第410 が「測って、やらない」で閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅ 第407 が測って「やらない」と決めた**／**⒟ ✅ 第407 で閉じた（R2R）**／LP 双子が要る R7〜R11／**R15 ✅ 削除は第407 で完了**（残り＝「2 綴り」一族のみ＝統合・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑽ 終了時**: commit 3 本（第 0 便 code `9f1d74a9`・§7 3.5 の docs `e7ad703c`・第 1〜第 3 便の docs 1 本。**製品コードは 1 行も触っていない**）＋Lab 1 本・**最終 full `sessions/p412/run5.trx` 8718 / 0 / 3 / 8721**（第411 最終と同じ＝**網の増減なし**。計器は全部外してあり、この run は §1・§2・RULES を書いた*後*の最終ツリーで取った）・§7.5（対 `9f5cd52a`＝スクリプトの出力）**Core `+` 0 行／REF 0／OWN 0**（**Core の差分そのものが空**＝継ぎ目は全部 revert 済み・残存 grep 0 件）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。棚卸し `docs/APPROXIMATIONS.md` は**不変**。§7 4（恒久知識）＝**RULES §5.3 に 2 項**（「**呼び手が 2 人いる関数の中に継ぎ目を入れると両方を 1 つの名前で数える＝合図は*負の*残り**」「**memo の hit 率を語る前に hit のときに何を払っているかを分けて測る**＝**この件では用心のほうが外れたので、外れたと書いた**」）。計器と全文は Lab `sessions/p412/summary.txt`（`Zz412Probe1`・`ZzStage`・継ぎ目は `zz412-seams.diff.txt`・生データ `inside-annotation-pass.txt`）。§7 3.5 は**セッションの頭で**済（第410 を `-Archive 410`＝17 行 12,154 chars）⇒ **HANDOFF は 424.6 KB から §1・§2 を書いて 441.9 KB＝天井 450,000 まで約 8 KB**。⚠️⚠️ **残りが薄い＝この便が 3 便回して使い切った**。次便は**§1 を 1 行も書く前に `-Archive 411` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で 3 回使って残っていない。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412 の欠陥ではない。
