...
最終更新 第339セッション＝**入り方は第298〜第338 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。第338 の次の一手 **⑴「Lily# 自身の鎖を計器にする」**に入り、⒜⒝ を建てて ⒞ の入口まで進んだ。**製品は inert な debug hook 1 本**（出力不動・snapshot 0 枚）・台帳 0。**骨は 7 つ**:
...
⚠️⚠️ ★★★★ **⑹ その矛盾の在処＝LP の DP が最小化する量と、報告される demerits が違う**: `lily/page-spacing.cc:148-181 Page_spacer::solve ()` は `ret.demerits_ += cur.force_ * cur.force_` を**全頁**について足す。一方 `lily/page-breaking.cc:1571-1577 finalize_spacing_result` は `for (i = ragged () ? size−1 : 0; i < size − (is_last () && ragged_last ()); i++)`＝**`ragged-last-bottom = ##t`（既定）なら最終頁の力は 1 度も課金されない**。**⇒ 最終頁に 1 段だけ置く解は報告側では*ただ*で、DP 側では課金される（逆も同じ）＝DP の argmin は報告される点数の argmin ではない。** Lily# 側は `PageBreaker.Demerits` が finalize を逐語（最終頁の除外を含む）・`SolveUnconstrained` が `Page_spacer::solve()` の移植。**どちらの目的関数で Boogie が割れるかはまだ数で分けていない。**
...
⇒ ★★★★ **次の一手**: ✅ 第338 ⑺ mark・✅ ⑻ 括弧・✅ ⑾ 2.311・✅ ⑿ 段の高さ・✅ **⑵ 段間の rod（engine 自身の数で確定）**・✅ **⑷ 段数と line breaking（LP と一致）**・✅ **⑺ spec**。**残るのは 1 つ: 題の頁に 7 段か 8 段かを決める*採点*。**
**⑴ ⑹ の 2 つの目的関数を Boogie の数で分ける**: **⒜** Lily# の `PageBreaker.SolveUnconstrained` が DP の中で**最終頁の力を課金しているか**を読む（LP は課金する＝`Page_spacer::solve ()` の `ret.demerits_ += cur.force_*cur.force_` に頁の除外は無い。**Lily# の doc は「:357-358 ragged な最終頁が伸びるなら力 0」とだけ書いていて、*圧縮*の側は書いていない**）。**⒝** Boogie「both」24 段について 3 頁解と 4 頁解の **(i) DP の Σf²（全頁）** と **(ii) finalize の Σf²（最終頁を除く）** を両方吐き、**LP がどちらで 4 を選び Lily# がどちらで 3 を選ぶか**を突き合わせる（`DebugPageBreakingScoring` は既に `pages 9,8,8 forces …` を刷る＝**4 頁解の力も見るには候補ごとの dump を足す**）。**⒞** 差が (i)/(ii) の取り違えなら**移植は `SolveUnconstrained` の数行**＝**出力が動く（頁が割れ直る本が出る）＝承認事項**。
⚠️ **もう疑わなくてよい**: 段数の選択・line breaking（⑷）・spec の値（⑺）・題の値付け（第338 ⑸）・括弧（第338 ⑻）・段ごとの上下インク（第338 ⑾⒠）・段間の rod（⑵）。
