...
最終更新 第402セッション（2026-09-18・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `ea2114ef`・未 push 29・**full `sessions/p402/run1.trx` 8659 / 0 / 3 / 8662＝第401 最終と一致**・CI は直近 5 run が赤＝未 push のまま・build 0 エラー・`Session-Check -Build -Test` はこの PC で 45 秒）、指示「HANDOFF を読んで着手」→ §1「次の一手」の **⒜ section の GreenSite gather を採った**（⒝ 診断 pass の残りは「実仕事」か fresh tree の red 実体化で、walk ではない）。道具は pwsh MCP（Bash 0 回）。**閉じたのは 1 本**:

★★★ **⑴ 「採用しても払う gather」は、checkpoint に *slot path* を持たせて walk を*その場所から*始めることで閉じた**（`0e19c0d1`）。**先に計器**（`sessions/p402/Zz402Probe.cs.txt`＝第400 の段階割り＋part block の `MusicSitesLazy` を list に取る費用・min-of-5・Debug）: base＝**gather plain1k 2.89 ms / 9000 site・fingbeam1k 12.18 ms / 33000 site・v2bow1k 0 / 1 site**（block が `<< \\ >>` 1 個＝1 site・以後この本は射程外）、無変更打鍵の collect 16.8／34.5／16.0 ms。**病名**＝`ProcessNodes` は checkpoint を flat list の*添字*で名指す（`WalkCheckpoint.NodeIndex`）ので、`ProcessMusicContainer` は採用する bar の site まで**全部 gather してから**添字を読んでいた＝green walk とはいえ block 全体・毎打鍵。しかも同一本の打鍵（Δ=0・空の窓）は planner が*最後の* checkpoint を prefix 側に選び、そこから splice する＝**「全部採用」ほど gather が無駄**。**直し＝3 点**: ⒤ `GreenSite.PathFrom(root)`＝spine 鎖から root（container）までの slot 列（preset red＝marker・form の小節線・inline の直接子、別 root＝phrase 本体の site は null）を `TryCaptureWalkCheckpoint` が `WalkCheckpoint.GatherPath` に記録／⒥ `SyntaxNode.TryGreenSiteAt(rule, path)`＝slot を下りながら**手前の slot の FullWidth を足すだけ**（rule 呼び出し 0・skip した物の site 0）で site と frame stack を返し、**`GreenSitesLazyFrom(rule, frames)`＝従来の iterator 本体を static にして同じ loop で続ける**（`GreenSitesLazy` はその root frame 版＝1 綴り）／⒦ `Svg/Collector/MusicSiteList.cs`＝ProcessNodes が読む flat list（`Preset`／`Eager`／`Lazy` の 3 形・`TryGet(i)` で必要なぶんだけ pull・`TrySeek(index, path)` で resume の prefix 側が walk に*その site から*入る・`PathOf(i)`）。**記録側（full collect）は従来どおり eager**＝全 site をどうせ消費する上、**expansion 予算の課金順は「gather 順（container の phrase 参照を先に全部）」で full collect の打ち切り点（`ExpansionBudgetExceededAt`）が定義されている**。lazy は課金を walk 自身の課金（unfold repeat・多小節休符）と*交互*にするので、それが観測不能なのは **resume mode だけ**（予算が尽きたら `ChargeExpansion` が full collect へ abort する＝描かれる前）。⇒ lazy は `WalkProbe is { IsRecording: false }` のときだけ。住所検査（位置・kind・添字）は seek の結果に対して従来どおり効く＝seek は再利用の hint であって真実ではない。**後（無変更打鍵・collect 段・min-of-5）**: **plain1k 16.8 → 13.7／fingbeam1k 34.5 → 22.2 ms＝gather の全額**・打鍵全体 24.2 → 21.1／56.1 → 39.6・v2bow1k 不動。**数（回数）**: 最後の checkpoint からの resume で gather された site **9000 → 1／33000 → 1**（`CollectWalkProbe.GatherSeeks`／`GatherSitesMaterialized`）・net 259 冊 2671 checkpoint のうち **2430 が path 持ち**（残り 241＝marker・phrase 本体・inline）。**網 `GatherSeekTests` 5 本**（RULES §7.7 差分の網）: 全 net 冊＋perf 3 冊の全 gather root（root・part block・section 宣言）で spread した site から seek → **green の同一 instance・位置・red の同一 instance・以後の列が full walk の残りと 1 site ずつ一致**／Δ=0 で全 spread checkpoint から resume＝full と model 同一＋**path 持ちなら seek 回数ちょうど 1・無ければ 0**／drift した path（空・範囲外・token・rule が collect しない葉）は拒否・marker は path 無し／perf 2 冊の最後の checkpoint は本の 1% 未満しか gather しない。既存の網 `CollectResumeTests`／`CollectEditResumeTests`／`MusicSitesEquivalenceTests`／`PreviewCollectSharingTests`／`ExpansionBudget*` 57 本も緑。⚠️ 踏んだ罠 4 つ＝pwsh MCP の `execute_command` の引数名は `command` ではなく **`pipeline`**／新規ファイルは LF で出るので CRLF に直す（今回は raw string literal 無し）／**Zz 計器は full run の前に木から出す**（throw で数を刷る設計＝full を赤くする）／`ApproximationInventoryTests.TheInventoryIsNotStale` は行番号が動くだけで赤くなる＝再生成（RULES §6）。⚠️ ベンチは一声かけずに回した（数秒の in-process 計器・ユーザー不在の autonomous 便）＝絶対値ではなく **gather の額と一致する差・site 数の 9000 → 1** を根拠にしている（CLAUDE-OPERATIONS §2 の但し書きの側）。

★ **⑵ 終了時**: commit 1 本＋docs 2 本（第 2 便は docs のみ）・**最終 full `sessions/p402/run4.trx` 8664 / 0 / 3 / 8667**（+5＝⑴ の網 5 本・run2 は APPROXIMATIONS 再生成前で `TheInventoryIsNotStale` 1 本赤・run3 は §1 書き換え前で継ぎ目の網 3 本赤）・§7.5（対 `ea2114ef`・スクリプトの出力）**Core `+` 419 行／REF 0／OWN 0**（数値定数なし・LP 由来の式なし＝監査対象は「同じ答えか」だけ＝⑴ の 2 つの差分の網）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・APPROXIMATIONS.md 再生成（行番号のみ）・CHANGELOG 0.8.0 Editor に 1 項。§7 3.5 は済（第400 を `-Archive 400`＝11 行 4,210 chars）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。**第 2 便（同日・指示「このセッションが有利なら着手」）＝着手したのは Release 計測だけ**（ユーザーに一声かけて回した・同じ計器を `-c Release`・HEAD `4262c45a`・`sessions/p402/release-stages.txt`）: **無変更打鍵＝plain1k collect 2.3／keys+gate 7.2／render 3.5／計 13.6 ms・fingbeam1k 5.3／2.5／7.4／18.7・v2bow1k 5.1／0.7／2.3／8.2**。⚠️ **plain1k は keys+gate 7.2 ms が最大の段になった**（collect の 3 倍）——しかも「edit end」の打鍵では 0.9 ms＝*同一本の打鍵のほうが edit より高い*＝R13⒟「`MeasureContentKey.Compute` が無変更打鍵でも全 fold（reference-identity memo）」の顔。**なぜ Δ=0 だけ高いか（採用した measure が copy で memo を外すのか、別の門か）は読んでいない**＝次の便が計器で割ってから。gather 単体は Release でも 2.94 ms（Debug 2.89 と同じ＝delegate 呼び出しが主で JIT は効かない）。診断 pass や R13⒟ の実装は別のコードを読み直す仕事なので、この便では着手しなかった。⇒ **次の一手**: ⒜ **R13⒟＝plain1k の keys+gate 7.2 ms（Release・無変更打鍵）を段で割る**（`Compute` の memo hit 率を*回数で*数える＝RULES §7 9・Δ=0 と Δ≠0 で何が違うか）／collect 段の残りは Release で 2〜5 ms（defs walk・restore・v2bow の extra voice live walk）＝小さい／⒝ 診断 pass の残り＝`AnnotationName` 14（実仕事）／`MeasureValidator` 10／`LyricSings` 6.5（fingbeam1k・warm）と fresh tree の red 実体化 ~19 ms／⒞ R13⒝〜⒤（⒟ `MeasureContentKey.Compute` の無変更打鍵 fold＝keys+gate 3.5〜4.7 ms が打鍵の次に大きい固定費）／LP 双子が要る R7〜R11／R15（要承認）。
