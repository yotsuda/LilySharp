...
最終更新 第389セッション（2026-09-15）＝**入り方は第298〜第388 の型**（ユーザーは `docs/HANDOFF.md` を読ませて「作業に着手して」だけ）。**§0 の裏取りを走らせた**（`Session-Check -Build -Test -Scratch p390`）。道具は pwsh MCP。**結果＝第388 ⑷ の次の一手「小節頭の前置きの後ろから測る中央」を移植した＝MMR は bounding column の *break alignment* の間で中央（commit `c4e2dbef`・snapshot 2 枚はユーザー承認のうえ再ベース）**。**骨は 4**:

★★ **⑴ 開始時の数は第388 末（⑸）と一致**: HEAD `f7db1cd6`・未 push 14（`origin/master` は `2554d59a`）・木 clean・未追跡 0・**台帳 851 点／ss 非ゼロ 215／総和 22.584727806／count 点 180 うち非ゼロ 0／exact 671／OPEN 0・snapshot 249・追跡 `.lys` 608**・lysc は net10.0 だけ・build 0 エラー・**full `scratch/p390/run1.trx` 8577 / 0 / 4 / 8581**（RunInfos は SKIP 4 だけ）・`2554d59a` の CI は success。

★★★ **⑵ 機構と移植**: LP の MultiMeasureRest は `spacing-pair (break-alignment . break-alignment)`（`scm/define-grobs.scm:2376`）⇒ `multi-measure-rest.cc:44-61 bar_width` の各辺は `paper-column.cc:167-218 break_align_width` の column 全体の extent（column は system のもの＝他の譜の key でも動く）。**Lily#** `MultiMeasureRestEngraver.BarWidth`＝左は `BoundaryColumn.Build` の grob 右端・右は `SpacingRules.BoundaryClefAllowance`・system の頭の小節は触らない・rod は不変。**実測（中央からのずれ・LP / ページ）**: `v3.ly` +1.17 / +1.17・`keysigspace` +5.14 / +4.58・`cue-clef-manually` −1.16 / −1.38（残差は ⑷ ①②）。

★★★ **⑶ 射程と LP 照合**: 掃き 937 冊で 5 冊（インクだけ・実コーパス 0）。snapshot `mmr-key-change-bound`・`mmr-meter-and-time-change` は `lysc ly` 双子＋`\compressMMRests`（⚠️ 双子は compress を出さない）で LP と 0.01 以内＝**ユーザー承認のうえ再ベース**。**網** `MultiMeasureRestLayoutTests.AnMmrCentresBetweenTheBreakAlignments_NotTheBarLines`・毒 3 本とも網だけ赤。⚠️ 引用ラチェットはハイフンの名前を 3 節以上しか数えない。

★★ **⑷ 数と次の一手**: **終了時**: HEAD＝この §1 を書く commit（親 `c4e2dbef`）・未 push 16・**full `scratch/p390/run3.trx` 8578 / 0 / 4 / 8582**（+1＝網・run2 は 8575 / 3 / 4 / 8582＝snapshot 2＋引用ラチェット）・台帳 851／総和 22.584727806・snapshot 249（2 枚は内容更新）・追跡 `.lys` 608 は不変・§7.5（working tree 対 `f7db1cd6`・commit 前に手で数えた）: **Core `+` 86 行／REF 3／OWN 0**＝**監査した**: 数値定数なし・column を voice ごとに建てて reach の max で束ねるのは ⒝（LP は譜をまたいで symbol ごとに 1 group を align）・perf: 足した計算＝MMR 記号ごとに各 voice の先頭項目の走査＋key／time で開く小節だけ `BoundaryColumn` 1 本＝**読んで数えた・測っていない**・`APPROXIMATIONS.md` 不変・§7 3.5: 第387 を `-Archive 387`（済）。★ **LSP の配布は要る**（ページが動く）。⚠️ **残した差（この量ではない）**: ① **key 変更の予約幅が描画のインクより 1.10 短い**＝`keysigspace` の −0.56 の全量（描画は LP と ±0.01＝小節線右端から ナチュラル 1.00 / 1.82 / 2.79 / 3.60・シャープ 4.77 / 5.87 / 6.97 / 8.07 / 9.17）＝⑸／② **cue clef は full size で描かれ予約される**（`cue-clef-manually` の −0.22・小節幅 13.19 対 LP 11.25）／③ 第388 の残り（run の外の R の spacing・count の Y 9.29 対 9.24・`pcsm-order-probe` 1 本目の小節線）。⇒ **次の一手**: ⑸ の判断／②／第379 ⑺ の並び（⒝・⒠ drummap・⒢）／第379 ⑻ ② `CreateTimingSpring` の床。

★ **⑸ 第 2 便＝⑷ ① の住所を読んで設計の問いで止めた → ユーザー「可能な限り LP に寄せる・安全か」→ 第 3 便で実装（⑹）**。

★★★ **⑹ 第 3 便＝キー変更は描画の歩きで予約し、clef は項目が運ぶ（commit `dff11a41`・snapshot 4 枚承認済み）**。collector が `KeySignatureChangeItem.Clef` を刻む（読み手 6 軒へ clef を通さない・content key は反射で自動）／`KeyChangeGeometry(change)`／`MeasureBuilder` が同じ瞬間の clef で restamp／`BoundaryColumn` は KeyCancellation と KeySignature の 2 grob。実測（`scratch/p390/keyw`）: clef 依存は 210 通り中 102・key と clef の順は LP バイト同一。射程 7 冊・全部 LP に近づいた。⚠️ 毒の後の incremental build が 2 回目に腐った（記憶を更新）。full `run7.trx` 8582 / 0 / 4 / 8586・Core `+` 108／REF 5／OWN 0。

★★★ **⑺ 第 4 便（常設指示「このセッションが有利なら着手」）＝⑹ の残り +0.19 は小節線の光学補正の誤適用（commit `f0945c82`・snapshot 7 枚はユーザー承認のうえ再ベース）**。**住所**: `Staff_spacing::get_spacing` の `next_notes_correction` は**最後の break-aligned grob** の `bar_y_positions` を読み、小節線以外では空＝**key／time 変更が小節線の後ろに立つと下向き符尾の補正は 0**（`staff-spacing.cc:72-93`）。`BarlineToFirstColumnSpring` は何が最後でも足していた（`LineStartColumn` は既に staff bar で門）⇒ `boundary.HasValue ? 0 : …`。**実測（小節線右端から第 1 音・LP／前／後）**: kn-b（key・下向き）9.00／9.19／9.00・kn-u（上向き・対照）9.00 不変・kt-d（time・下向き）4.35／4.54／4.36・kt-u 不変・kn-a／kn-c も 0.01 以内。**射程**: 掃き 937 冊で 22 冊＝snapshot 7（LP 照合は commit message）＋`beam-auto`＋**実コーパス 14 冊**（TabStaff で双子と行が揃わず LP 直比較なし・動いたのは time 変更直後の小節と行の再配分）。**網** `KeySignatureChangeTests.ADownStemAfterAKeyChange_GetsNoBarLineOpticalCorrection`・毒＝この網だけ赤。§2 の台帳名 2 つに tag。**数**: full `scratch/p390/run9.trx` **8583 / 0 / 4 / 8587**・台帳不変・snapshot 7 枚更新・Core `+` 9／REF 1／OWN 0。★ LSP 配布要。⇒ 次の一手は ⑧。

★ **⑧ 第 5 便＝`keysigspace` 2 小節目 +0.55 の住所を測って止めた → ユーザー「悪影響がなければ移植」→ ⑨**。

★★★ **⑨ 第 6 便＝小節頭の spring を譜ごとの Staff_spacing wish にして merge（commit `b5522efb`）**。`spacing-spanner.cc:478-536 breakable_column_spacing` の `merge_springs`（ideal は平均・min は最大）。**譜ごと**＝last break-aligned grob と space-alist／**列で共通**＝min_dist と符尾の光学補正（`right-items` は音楽 PaperColumn）。`MultiStaffLayouter.CollectStavesOfMeasuresAtIndex` → `MeasureLayouter.StaffItemsAt` → `BarlineToFirstColumnSpring` の `Wish` を `Spring.MergeSprings`。⚠️ 初版は補正を譜ごとに読み 6 冊が LP から遠のいた。射程 938 冊で 2 冊（|幅−LP| の和 1.093 → 0.007）・snapshot 0。**網** `KeySignatureChangeTests.AKeyChangeOnOneStaffOnly_IsAveragedWithTheOtherStaffsBarLineWish`。full `run12.trx` 8584 / 0 / 4 / 8588・Core `+` 189／REF 15／OWN 0。

★★★ **⑩ 第 7 便（常設指示）＝装飾音符で始まる小節の小節線 spring を column 原点から 0.8 倍（commit `38996141`・snapshot 7 枚はユーザー承認のうえ再ベース）**。**LP**: `spacing-spanner.cc:519-527` は右 column に grace 部があると `spring *= 0.8`＝**column 原点から**（小節線の幅は縮尺の内側）・grace run の幅は縮尺の後に足す。grace の符尾は上向き固定（`music-functions.scm:652-656`）＝下向き符尾の光学補正は掛からない。**Lily#**: `SpacingRules.MidMeasureChanges` の rigid な `graceApproach` 分岐を消し、merge 後に `origin = EngravingDefaults.BarlineDrawnWidth(leftBound)` だけずらして最も広い run で `AdjustSpringForGraceNotes` → 戻す・`Wish` の補正は `startLeadGrace > 0` で 0（`leftBound` は `MeasureLayouter.CreateBarlineToFirstSpring` と `MeasureSprings` が渡す）。**実測（小節線右端から grace 頭／主音・LP／前／後）**: `kg1`〜`kg3`（ragged）主音 2.62／2.74／**2.62**・`grace-notes` 0.68 2.62／0.80 2.74／**0.68 2.62**・`grace-rest` 1 小節目 一致・`grace-lower-staff` 3.52／3.63／3.51・`02-ornaments` 1 小節目 0.45 2.19／0.80 2.74／0.47 2.41（同じ段の 5〜9 小節目は再配分で 0.01〜0.07）。**射程**: 938 冊で 26 冊（追跡 25＝snapshot 7＋`grammar-tour`／`feature-tour`＋audit 16・実コーパス `青い珊瑚礁`）＝`verify-grace.txt`: 小節幅の |w−LP| 和で CLOSER 1（`grace-lower-staff`）・FARTHER 1（`grace-unfold` 1.46 → 2.18＝**小節線→主音は LP へ（2.74 → 2.83 対 2.96）だが、既存の最後の音→小節線 +0.72 が段の stretch を行頭の小節から奪う＝藁**）・same 20・比較不能 5。**網** `GraceSpacingTests.AGraceRunOpeningABar_ScalesTheBarLineSpringFromTheColumnOrigin`・毒 2（補正を戻す／column 枠を外す）＝各この網だけ赤。**数**: full ``dotnet test` 直（trx なし）**8585 / 0 / 4 / 8589**（+1＝網・run13 は 8577 / 8 / 4 / 8589＝snapshot 7＋`ApproximationInventoryTests`）`・台帳不変・snapshot 7 枚更新・Core `+` 52／REF 2／OWN 0（数値定数なし）・`APPROXIMATIONS.md` 再生成（CRLF）・perf: 小節頭に grace がある時だけ run 数ぶんの `AdjustSpringForGraceNotes`＝読んで数えた。HEAD＝この追記の commit（親 `38996141`）・未 push 26・★ LSP 配布要。⚠️ **残した差**: ① `grace-dot-flag-column` の grace 頭 1.48 対 LP 0.68（付点・旗の grace＝別の量）／② `grace-unfold` の最後の音→小節線 +0.72（stretch 行）／③ 未測: 小節線の前の clef・主音の臨時記号・複数譜の grace。⇒ **次の一手**: ①／②／cue clef の大きさ／第379 ⑺⑻。

## 以下は第388セッションの経緯

最終更新 第388セッション（2026-09-15）＝**入り方は第298〜第387 の型**（ユーザーは `docs/HANDOFF.md` を読ませて「作業に着手して」だけ）。**§0 の裏取りを走らせた**（`Session-Check -Build -Test -Scratch p389`）。道具は pwsh MCP。**結果＝第387 ⑷「MMR が第 1 voice に居ない小節」を閉じ、掃きで見えた後退をユーザー選択「範囲を広げる」で閉じた＝①run の判定 ②描画の Y ③run の外の R も MMR 記号（commit `e09af71e`）**。**骨は 4**:

★★ **⑴ 開始時の数は第387 末（⑶）＋第 2 便の HANDOFF commit と一致**: HEAD `0ebf6796`・未 push 10（`origin/master` は `2554d59a`）・木 clean・未追跡 0・**台帳 851 点／ss 非ゼロ 215／総和 22.584727806／count 点 180 うち非ゼロ 0／exact 671／OPEN 0・snapshot 249・追跡 `.lys` 608**・lysc は net10.0 だけ・build 0 エラー・**full `scratch/p389/run1.trx` 8573 / 0 / 4 / 8577**（RunInfos は SKIP 4 だけ）・`2554d59a` の CI は success。

★★★ **⑵ 閉じた**。**① run**: `MultiMeasureRestEngraver.FindRuns` が各譜の**全 voice** を読む＝「どれかの voice が R を書き、他の voice は skip だけ（空の小節＝span の外も可）」（`StaffRests`・`IsSkipOnly`・`OpensNewRun` も全 voice）。入口は 2 つで生の形は 1 つ＝`FindRuns(MultiStaffScore)`（`MmrRunMap.ForScore` 経由で spacing・breaker・`MeasureContentKey`・incremental）と描画の `Calculate`（`ctx.VoicesByStaff` を受ける・以前は `measuresByStaff`＝primary だけ）。**② 描画**: **LP 2.26.0 は `Multi_measure_rest_engraver` を Voice に consist**（`ly/engraver-init.ly:374`・Lily# の旧コメント「Staff context」は誤り）⇒ R を書いた voice ごとに 1 記号・`MultiMeasureRestLayout.VoiceDirection`。向きは `RestItem.VoiceDirection` に stamp（`ResolveVoiceStemDirections` と `PartCombiner.WithVoiceDirection` の `IsMultiMeasure: false` を外した＝`scm/music-functions.scm:617-634 direction-polyphonic-grobs` に MultiMeasureRest が在る）・**`ItemSkylineFactory` の voiced 分岐は MMR を除外**（stamp が箱を voiced 位置へ動かさない＝spacing 不変・MMR は従来の placeholder）。★★★ **church_rest の Y は override 腕**: `multi-measure-rest.cc:254-266` は symbol ループの**前に** staff-position を set する ⇒ 各 symbol の `staff_position_internal` は `rest.cc:53-74` の position_override 腕＝**semibreve は pos+2・breve 以上は pos**（pos＝1 小節なら semibreve の voiced 位置 −2・それ以外は minim の voiced 位置）。dir 0 では旧表と同一。**予測を外して見つけた**: 最初は各 symbol に新しい voiced 位置を与えて書き、`v2.ly` の voice two `R1*3` が **breve 13.69・semibreve 12.69**（新しい voiced 位置なら両方 13.69）で反証された。**実測（`scratch/p389/mmr/`・中線 11.69）**: voice two `R1` 13.69・voice one 9.69・`{ R1 } { R1 }` は両方・count は譜の上のまま。**ページ対 LP**（`compare.ps1`）: 小節幅 7.89×4・run 12.66・休符の小節内 x 3.29・Y 全部一致（base は `s1`/`R1` 7.69・`s2 s2`/`R1` **6.69**）。⚠️ `.lys` の綴りの罠 2 つ: `voice { s1 s1 s1 } { R1*3 }` は LYS2002（voice の中も小節線が要る）・`voice { … | } { R1*3 | } |` は `}` の後ろの `|` が空小節 5.51 を足す。

★★★ **⑶ 掃き（第 1 回・`sweep.txt`）の後退 → ユーザー選択「範囲を広げる」**。①② の後の掃きで動いた 4 冊のうち **`pcomb-cond` 3 小節目（第 1 voice `R1`・第 2 voice `g,2 g,`）が後退**: 旧コードは第 1 voice しか見ずに run 扱い＝**偶然** LP どおり小節中央に描いていた（x 34.70）が、①で run から外れ、Lily# は **run の外の R を拍頭の普通の休符**として描く（31.63）。LP（`pcombine-ctl.ly`・`scratch/p389/cond/pctl.svg`）は中央**かつ** voice の高さ（y 9.69）。⇒ **③ run の外の R も MMR 記号**: 全小節を回り、run に入らない小節で**全小節ぶんの** `R` を持つ voice ごとに 1 小節の記号（count なし・その小節の小節線の内側で中央・voice の向き）＝`MultiMeasureRestEngraver.Calculate` の後半。`MultiMeasureRestLayout` に `StaffIndex`・`VoiceIndex`・**`SharedRenderer.IsMmrCovered` を譜×voice ごとに**（以前は小節番号だけ＝run の記号がその小節の全譜・全 voice の休符を消していた。run の外の記号でそれをやると他の譜の `r4` が消える）。**実測 LP 2.26.0（`scratch/p389/mmr/v3.ly`・下の譜が演奏）**: `R1 | R1*3` は小節ごとに中央の全休符 4 つ・count なし／`{ R1 } { g2 g }` 中央・中線 −2／`{ c''2 c'' } { R1 }` 中央・+2／3/4 の `R2.` は全休符（Lily# の旧描画は付点 2 分を拍頭）。**ページ対 LP**: 中央からのずれは両方 +0.09〜0.10（小節線の半分）・Y 一致・下の譜の `r4` は残る。**`pcmsh-a-cond`（condensed の 8|16|4 対 16|8|4）**: LP（`cond/cond-mmr.ly`＝`pcmsh-a.ly` の `\partCombine` を `<< \\ >>` に）は **和集合の境界 8・8・8・4 で両 voice に MMREST**・voice one +2／voice two −2＝ページと一致（旧は voice two の休符が**消えていた**）。**網** `AnRAgainstPlayingMusic_DrawsItsOwnCentredSymbolEveryBar`（記号 7 個の譜・voice・向き＋SVG の中央＋他の譜の `r4` が残る）。**毒**: ⒜ run の外の記号を出さない＝この網だけ赤／⒝ `IsMmrCovered` を小節番号だけに戻す＝この網だけ赤。⚠️ **残した差（未移植）**: ⑴ **count の二重**＝2 voice が同じ run に R を書くと Lily# は数字を同じ座標に 2 つ描く（インク同一）・LP は 1 つ（`cond-mmr.svg` の "8" は区間ごとに 1 個・機構は `multi-measure-rest-engraver.cc` を読んでも未特定）／⑵ **小節頭に time 変更がある小節**の中央: LP は拍子記号の右から測る（`v3.ly` の `R2.` が中央より +1.17）・Lily# は小節全体の中央（run でも同じ既存の差）／⑶ run の外の R の **spacing** は据え置き（Lily# は拍頭に列を立てる・LP の MMR は列を作らず rod だけ）。

★★ **⑷ 数と次の一手**: **網** `MultiMeasureRestLayoutTests.AnMmrOutsideTheFirstVoice_FormsItsRunAndDrawsInItsVoice`（run 6 本・記号 7 個の voice 向き・陽性対照 `voice { c'1 } { R1 }` は run 無し）・`AVoicedChurchRest_MovesWithTheStaffPositionItSetFirst`（SVG の Y）。**毒で確かめた**: run を第 1 voice に戻す毒＋描画の毒＝2 本とも赤／**描画の毒だけ**（semibreve に新しい voiced 位置）＝後者だけ赤＝網は 2 本とも自分の量を見ている。**snapshot**: ①② の時点では 0 枚（run2）・③ で **1 枚**＝`test/multi-measure-rest-grandstaff`（トレブル `R1*4` 対 バス＝全休符 4 つが拍頭 → 中央・LP 双子 `mmr/gs.ly` と小節内 x **LP 5.97 / 5.975 / 4.855 / 4.405 対 5.97 / 5.98 / 4.85 / 4.41**・**ユーザー承認のうえ再ベース**）・台帳 851／総和 22.584727806 は不変。**射程＝掃き 937 冊**（追跡 608＋実コーパス・`scratch/p389/sweep/`・第 1 回 `sweep.txt`＝①② で 4 冊・第 2 回 `sweep2.txt`＝③ 込みで **13 冊・実コーパス 0**）: 小節線が動いたのは `pcsm-probe`（2 小節目 7.69 → 7.89＝LP）と `pcmsh-a-cond`（3 → 4 区間＝LP の和集合）だけ・残り 11 冊は全休符が中央（と voice の高さ）へ＝`mmr-voice-probe`・`pcomb-cond`・`multi-measure-rest-grandstaff`・`grammar-tour`・`feature-tour`・`canon-in-d`・`pcmmas-x4`・**LP 原典と照合した 3 冊**（`scratch/p389/lpchk/`）: `chord-names-rests`＝小節内 x 3.29 で一致（旧 2.09）／`keysigspace`（＝`key-signature-space`）＝1 小節目 LP 9.27 対 9.28（旧 15.06）・**2 小節目は小節頭の 5♯ の後ろから中央を測る LP とずれたまま**／`cue-clef-manually`＝Y は LP の +2 に一致・**x は LP が中央より 1.16 左（小節頭の cue clef）**。`## 0.8.0` の `### Engraving` に項目。**commit はユーザー承認のうえ**。**終了時**: HEAD＝この §1 を書く commit（親 `e09af71e`）・未 push 12・**full `scratch/p389/run5.trx` 8576 / 0 / 4 / 8580**（+3＝網 3 本・run2 8572 / 1 / 4 / 8577＝`ApproximationInventoryTests` の行番号／run4 8575 / 1 / 4 / 8580＝上の snapshot＝再生成と承認済み再ベースで閉じた・`APPROXIMATIONS.md` と `audit/magic_constants.csv` は CRLF で commit に含めた）・§7.5（working tree 対 `0ebf6796`・commit 前に手で数えた）: **Core `+` 185 行／REF 8／OWN 0**＝**監査した**: 数値は church_rest の `−2` と rest.cc の `+2` だけで REF 付き・「他の voice は skip だけ」は ⒝（MMR grob は voice ごと・skip は grob を作らない、から導出）・run の外の記号は `Multi_measure_rest_engraver` の住所・perf: 走査を足した＝各小節で staves × voices（`FindRuns` は `MmrRunMap.ForScore` で score ごとに 1 回＋描画 1 回・run の外の記号は描画で 1 回）＝**読んで数えた・測っていない**・§7 3.5: 第386 を `-Archive 386`（済）。★ **LSP の配布は要る**（ページが動く）。⚠️ **残した差**: ⑶ の 2 つ（**小節頭の clef / key / time / cue clef の後ろから測る中央**＝`Multi_measure_rest::bar_width` の読解から・run の外の R の spacing）＝count の二重と big_rest の voiced Y は ⑸ で片付いた／count の Y ページ 9.29 対 LP 9.24（既存）／第387 ⑶ の `pcsm-order-probe` 1 本目の小節線 14.18 対 14.99。⇒ **次の一手**: 小節頭の前置きの後ろから測る中央（`keysigspace` 2 小節目・`cue-clef-manually`・`v3.ly` の `R2.` が計器）／それ以外の上の差／第379 ⑺ の並び（⒝・⒠ drummap・⒢）／第379 ⑻ ② `CreateTimingSpring` の床／第379 ⑽。

★★ **⑸ 第 2 便（常設指示「このセッションが有利なら着手」）**: **判定**: 「小節頭の前置きの後ろから測る中央」は**着手しない**（run の中の出力も動く＝承認要・break-alignment の regime でこの便の文脈は効かない）。**着手した 2 つ**（計器が `scratch/p389` に残っていた・描画コードを触った直後）: **⒜ big_rest の voiced Y＝差なし**（LP `mmr/big.ly` の dump: 単独・voice two・voice one の `R1*12` とも staff-position 未設定・Y 範囲同一＝中線／ページも中線）＝⑷ の「未実測」を消す。**⒝ count の二重を閉じた**（commit `3cb94b64`）: LP は **`Merge_mmrest_numbers_engraver`**（`scm/scheme-engravers.scm:354-370`・**Staff に consist** `ly/engraver-init.ly:98`）が同じ timestep の同じ text の MultiMeasureRestNumber を 2 つ目以降 suicide する（`cond/num.ly` の dump: `<< { R1*8 } \\ { R1*8 } >>` で after-line-breaking に届くのは 1 つ）⇒ `MultiMeasureRestLayout.DrawsCount`＝譜ごとに run を休む最初の voice だけ true（run の中の voice は count が等しいので「等しい text」は構築上成り立つ）・church_rest と big_rest の数字が読む。`Merge_rests_engraver`（休符を 1 か所へ寄せる）は既定で consist されない＝voice ごとの位置のまま。**網** `TwoVoicesRestingOneRun_PrintOneCount`（毒＝全 voice が数字を描く→この網だけ赤）。**射程** 掃き 937 冊（`sweep/sweep3.txt`・base＝`e09af71e` の描画）: **1 冊**＝`audit/lpreg/pcmsh-a-cond`（二重だった count が 1 つに＝LP `cond-mmr.svg` と同じ）・実コーパス 0。⚠️ `HANDOFF.md` は 448,876 バイト＝**450 KB の門の直前**＝次便は `tools\Fold-ClosedHandoffItems.ps1` か §1 の圧縮から。**数**: full `scratch/p389/run6.trx` **8577 / 0 / 4 / 8581**（+1）・§7.5（working tree 対 `bdeedce5`）Core `+` 15 行／REF 2／OWN 0（数値定数なし）・`APPROXIMATIONS.md` 再生成（CRLF）・台帳・snapshot 不変・perf: 足した計算なし（既存ループのフラグ 1 つ）。**第 2 便の終了時**: HEAD＝この §1 を書く commit（親 `3cb94b64`）・未 push 14・commit はユーザー承認のうえ。
...
| 決定 | 根拠（要点） |
|---|---|
| ★★★★ **撤去した綴りに移行ヒントも Did you mean も付けない＝その語が受ける汎用メッセージだけ。0.x を公開した後も同じ**（2026-09-15・第385・0.7.0 準備中・**ユーザー「lily# のユーザーは、今はまだほとんどいないから後方互換性は気にしなくて良い」「後方互換を気にした固有の警告があれば削除して」**・✅ `b5f112a8`） | 移行ヒントは守るべき既存ユーザーのための装置で、その利用者がまだいない＝**診断文は資産ではなく負債**（2026-07-31 の `version` 撤去と同じ判断の延長・記憶 `feedback_lilysharp_prerelease_no_back_compat`）。⚠️ **typo 提案候補が撤去語を正しい語へ誘導するなら候補から外す**（`@feather(rit)`→`right` は逆向きへの誘導だった）。**残すもの**＝LP の綴りを書いた人への案内（LYS0006／0007／0008・`\` コマンド）と現在形の typo 助言＝`GRAMMAR_AUDIT.md` §1.1 の線引き。 |
...
| ★★★ **LP が綴れない形を含む本は、そのままでは LP と比べない。比べる形に直してから比べる**（2026-09-14・第381 の後・**ユーザー決定**） | 綴れない形（LILYSHARP-OWN の配置など）を含む本の双子は、どう書いても別の楽譜で、残差が何の差か読めない。**比較の側で記号を除外・補正するのではなく、本の側を LP が綴れる形の変種にする**（追跡コピーに作り、何を直したかを header に書く）。例: bench の和音行は列がぶつかる記号を持ち上げる（下の行）＝ぶつからない変種で比べる。 |
...
| ★ **タブの弦は小節の中で継承する。明示 `\N` も継承する**（ユーザー判断・2026-08-16 明示） | **LP と違うことを測ってから決めた**: `c( g'\2) g g4` をベースで書くと Lily# は g を 3 つとも 2 弦 5 フレットに置き、**LP は無印の 2 つを 1 弦開放へ戻す**（2.26.0 実測・双子）。**1 つの音高は 1 小節のあいだ同じ押さえ方に見えるほうが読める**、が決定の理由。⚠️ **この resolver はもともと LP と別の模型**（`TabFingeringPlanner`＝左手の位置を声部全体で計画する LILYSHARP-OWN・2026-09-14 に 1 音ずつの `Tunings.CalculateFret` から置き換え）なので、乖離はその延長。**2026-09-14 に絞った: 小節を通して継承するのは*書いた* `\N` だけ**（選ばれた弦は計画が音ごとに選び直す・Amanda section A2 7 小節目）。**「LP と違う＝バグ」で消さないこと**（理由は `TabResolver.ResolveTabStrings` の remarks にも書いてある） |
