...
⚠️⚠️ ★★★ **便の途中でユーザーが畳んで push した**（§0 の「間で push」の形・第201 と同じ）: **上の「commit 6 本・未 push 27」は畳む前の記録**。**畳んだ後は 28 → 13 本・HEAD `fb79074d`＝`origin/master`・未 push 0**。**§1 が名指す本便の SHA 6 つは孤児になった**ので `HistoryCitationTests` の dead 天井を 495 → 501 に上げた（remarks に再編を名指し・旧 SHA は書かない＝memory「畳むと引用が孤児」の手順）。**この後の commit（`@reverseturn`・`to coda`）は畳んだ木の上に 1 本。**
