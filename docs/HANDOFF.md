...
★★★★ **㉒ 第 18 便＝ユーザー決定で LP の打楽器 33 種と調弦 25 種を入れた**（「(1) (2) 両方とも入れて。」・commit `851e5469`・**逐語は ARCHIVE「第374 の ㉒」**）。**移植は機械的でも、決めることが 4 つ在った**: ⑴ **どの style 表から行を取るか**（新しい 33 種は kit に 1 行も無い⇒ **kit → その楽器の対表 → `percussion-style` → LP がどこにも置かない 8 種は中央線**、という順序を registry の remark に規則として書いた）／⑵ **mark は 2 語では足りない**（`guiro` と `longguiro` は行も符頭も GM 74 も同じで LP は `tenuto` だけで分ける⇒ `MarkWords`／`MarkArticulation` を 1 軒建てた）／⑶ **`tt` は入れない**（LP の唯一の宙ぶらりん＝canonical 63・alias 64 の余り 1 つで、位置も音も持たない）／⑷ **調弦の綴りの規則**（LP の symbol から `-tuning` を落とし `<n>-string` を数字にしハイフンを詰める＝既存の `bass5` がそうなっていた）。⚠️⚠️ ★ **名前→`TuningType` の switch が 3 軒に写されていた**＝**25 種を 3 箇所に足す作業**になるところを `Tunings.Parse` へ寄せた。★★★ **副産物＝phrase 名前空間の実測**（remark は「NOT MEASURED」と書いていたが、**`LYS1030` が宣言の側で断っていた**＝コストは「phrase に使えない語が 33 増えた」だけ）。
...
★★★ **㉓ 数**: **開始時**: HEAD `1e80a9b2`・未 push 9・木 clean・未追跡 0（**`Session-Check -Build -Test` は走らせていない**）。**終了時**: HEAD `286a4ce3`（**出荷物の最後の変更は第 24 便 `bf39967d`＝双子が羽根梁を書く。第 13・14・16・17・19〜22・25・26 便は production 無変更**。**最後の 1 本はこの §1 を書く doc commit**）・未 push 60・**台帳 843 点／ss 非ゼロ 210／総和 23.216135909／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249／追跡 `.lys` 599＝全部不変**（⚠️ **第 18 便は Core を 7 ファイル触ったのに、絵は 1 枚も動かない**——**既存の綴りの振る舞いを 1 つも変えていない**からで、**台帳・snapshot・追跡 `.lys` が全部不変なのがその測定**。第 13・14・16・17 便は Core を 1 行も触っていない）。**full `scratch/p375/run40.trx` 8455 / 0 / 4 / 8459**（第 26 便＝**本数は変わらず、3 件が「固定」から「動く」に変わった**）・第 25 便末 `run39.trx` 同数（+31＝paper 94 件）・第 24 便末 `run37.trx` 8424 / 0 / 4 / 8428（+5＝双子の主張）・第 23 便末 `run35.trx` 8419 / 0 / 4 / 8423（+5＝羽根梁の幾何。⚠️ **snapshot 249 のうち 1 枚が動いた**＝showcase `05-special-techniques`・**台帳 843 点は不変**）・第 22 便末 `run33.trx` 8414 / 0 / 4 / 8418（+21＝注釈オペランド。⚠️ `run32` は 1 本赤＝**私が足した LP 引用の綴りが `lily/` 前置きで「名前を挙げていない引用」に数えられた**＝`CitationsThatNameNothing_DoNotGrow`。既に通っている同じ引用の綴りに揃えた）・第 21 便末 `run31.trx` 8393 / 0 / 4 / 8397（+40＝和音品質 40 綴り）・第 20 便末 `run30.trx` 8353 / 0 / 4 / 8357（+127＝drum 126 綴りの掃き＋mark の網 1 本）・第 19 便末 `run29.trx` 8226 / 0 / 4 / 8230（+33＝調弦語 32 の摂動ほか）・第 18 便末 `run27.trx` 8193 / 0 / 4 / 8197（+68＝打楽器 3 theory・調弦 1 theory の case が大半。⚠️ **`run24` は 1 本赤**＝`ApproximationInventoryTests` は**生成物 `docs/APPROXIMATIONS.md` の行番号**を見張るので、exporter が 11 行縮んだだけで赤くなる⇒ `LILYSHARP_UPDATE_DOCS=1` で再生成・差は 4 行）・第 17 便末 `run23.trx` 8125 / 0 / 4 / 8129・第 16 便末 `run22.trx` 8119 / 0 / 4 / 8123（drum 照合 31 本）・第 15 便末 `run21.trx` 8088 / 0 / 4 / 8092（表の番人 +1）・第 14 便末 `run20.trx` 8087 / 0 / 4 / 8091（摂動 136 本ぶん増）・第 12 便末 `run19.trx` **7951 / 0 / 4 / 7955**（⚠️ **第 12 便で 9 本*減った*のは正しい**＝matrix の MIDI 行 3 綴り × 3 主張が言語から消え、代わりに「退役した綴りは断られる」2 本が入った。第 11 便末 `run16.trx` 7964・第 10 便末 `run15.trx` 7956・第 9 便末 `run14.trx` 7953・第 8 便末 `run13.trx` 7947・**第 1〜7 便末は `run1`→`run12` で 7816→7869**。**13 便とも合計を読んでいる**＝新 test は差引 144 本（第 12 便は −9）（第 8 便の 78 ＝ 26 綴り × 3 主張）。⚠️ 第 5・6 便は既存 test を**書き換えている**ので増分はそれより小さい）。★ **第 7 便は 7 つの context の配線を丸ごと替えたのに既存 7854 本が全部緑**＝**行の読みが 1 つになっても、読めていた綴りの答えは変わっていない**。⚠️ scratch は **`p375`**。⚠️ §7 3.5: 第372 を `-Archive 372` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝**第372 の天井 commit `9f6df2a3` が未 push のまま**。**push はユーザー（未 push 60）**。★ **LSP は 13 度配布した**＝`0.6.0+3740810d`（第 19〜22 便は Core も Lsp も無変更なので配布していない）＝`tools/Deploy-Lsp.ps1` が `0.6.0+851e5469` を `yotsuda.lilysharp-0.5.0-win32-x64\server` へ（95 ファイル byte 一致）⇒ **ユーザーは Reload Window が 1 回要る**（package.json は動かしていないので 2 回は不要・記憶 `feedback_lilysharp_lsp_deploy_target`）。

⇒ ★★★ **㉔ 次の一手＝✅ ⒜（LP の語彙をどこまで入れるか）は第 18 便のユーザー決定「両方とも入れて」で閉じた**（打楽器 30→63・調弦 7→32 語）。✅ **⒜‴ `@feather` はユーザー決定「幾何を実装して」で閉じた**（第 23 便・下 ㉒″）。⚠️⚠️ ★★★ **⒜″ 摂動網は paper まで届いた（第 25 便・commit `6cfd86d7`）＝語彙の掃きは全部終わり、残ったのは*11 鍵の原因未確定*だけ**。**問いの形を変えた**: **同じ鍵の 2 つの値が違うページになるか**（`indent 5mm` 対 `25mm`）＝**既定値を知らずに済み、「既定を書くと inert」の罠も踏まない**。**陽性対照は `size a4`**（既定の紙なので inert でなければならない）。⇒ **94 件中 83 件が動く**。★★★ **残り 11 件は「死語」と呼ばず一覧にした**——**原因が未確定で、2 つの試みがどちらも決めなかった**: ⑴ **フィクスチャを厚くしても 1 件しか解けなかった**（`staffGroupStaffSpacing`＝group の*後ろ*の staff が要った）／⑵ **読み手の grep は決定的でない**——**`staffStaffSpacing` も `markupSystemSpacing` も grep で 0 なのにページは動く**（間接参照）＝**0 は何も証明しない**。⇒ **残るのは page/staff spacing engine の鍵ごとの読解＝別 regime**。★★★ **第 26 便でその「最初に試すこと」を実行し、11 → 8 になった**（commit `d7ee87b6`）: ⚠️⚠️ **`RaggedLastBottom` の既定は true**（LP 自身の既定「短い曲に最適」）＝**最終ページは自然な間隔のまま**なので、**1 ページの本は*自分が最終ページ*で、ばねが 1 つも圧縮されない**。**2 ページに伸ばして page 1 を均させる**と **`topSystemSpacing`・`lastBottomSpacing`・`raggedBottom` が動いた**（＝フィクスチャの性質であって鍵の性質ではなかった）。⇒ ★★★ **恒久の教訓 3 つ目**: **ばねが inert に読めたら、まず「フィクスチャがそのばねを圧縮することが在るか」を訊く**（既存の 2 つ＝「フィクスチャはその鍵を表現できるか」「計器は本が通る枝を読んでいるか」に続く）。⚠️ **残り 8 件のうち 2 件は問いが狭まった**: **`topSystemPadding` は ragged でも justified でも inert**（他 3 件を説明した理由では説明できない）／**`stretchability` は*実際に伸びるページ*でも inert**（同じブロックの隣 2 つはそこで動き出す）＝**どちらももう engine の読解**。⚠️ **やり方は 4 便で確立した**: **⑴ 陽性対照を 1 本置く**／**⑵ フィクスチャがその鍵を表現できるか先に確かめる**／**⑶ 毒を入れ、外れたら毒ではなく*計器と経路*を疑う**／**⑷ 決められなかったものは「今日の答え」を固定して*未確定と名指す*——死語と呼ばない**。**残った札は 3 枚**: **⒜′ `InstrumentDefaults.GetTuning` は preset 5 族しか答えない**（`violin`／`cello`／`viola`／`mandolin`／`banjo` の preset を書いた part を tab で描くと**ギターに落ちる**）＝**調弦は在るのに preset が繋がっていない**。**触っていない理由は 1 つ**——**既存の本の tab の絵が変わる**（`instrument violin` ＋ tab の本があれば 6 弦→4 弦）ので、**射程を数えてから**（第 12 便の型）。⒝ **摂動網の残り**（paper キー・chord quality 35・`@` の引数つき形）。⒠ **drummap の `${1:hh}`＝UX 案件**（drum 語彙を出すと親切。`drummap { ▮ }` の本体に語彙の家が無いので context が 1 つ要る）。⒡ **第373 ⑼⒜（probe 24 冊の「量を測ったか／問いに答えたか」の仕分け）**も先頭級のまま。**ヘッダの綴りの島は閉じた**——⑻ の 25 綴りが全数で、**残る未点検はゼロ**（`{` を開く構文は score／staff group 5／part／phrase／form／lyrics／chords／section／fonts・paper・layout とその入れ子／音楽の容れ物で尽きる）。⚠️ **閉じたのは*補完の*読み手だけ**: 同じ「2 語しか読まない枠」を持つ道具が他に居ないかは見ていない（`SemanticTokens`・折り畳み・`BarCountFix` は別の木＝**実構文木**を読むので原理的に無縁、という*予想*まで）。⒢ 第369 ⑸⒝（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。
