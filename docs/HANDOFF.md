...
★ **⑸ 終了時**: commit 3 本（`c8cb106a` 天井・`e1e64006` R3・この §1）・HEAD＝この §1 の commit・**未 push 3**（push はユーザー・push で CI が緑に戻るはず）・**最終 full `sessions/p397/run4.trx` 8624 / 0 / 4 / 8628**（+9＝`TupletArmTests`）・§7.5（対 `184b80d0`・スクリプトの出力）**Core `+` 203 行／REF 2／OWN 0**（新しい数値定数なし・2 本は `tuplet` と `bracket_basic_visibility`）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 608・§7 3.5 は済（第395 を `-Archive 395`＝24 行 5,302 chars）。**残る道具**: `LilySharp-Lab/sessions/p397/`＝`exe-base/`（`c8cb106a` の Debug lysc）・`sweep/sweep.ps1`（p395 の型・母集団を Lab の corpora に向けた）・`sweep/base/` 938 枚（**HEAD の絵と同一なので次便もそのまま base に使える**・moved 0 の証明つき）・`Zz397ProbeTests.cs.txt`・`commit-r3.txt`。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（ARM64 RID と PATH 再読込・触っていない）。⇒ **次の一手**: §2 R の上から＝R1（exporter の `alternative`＝言語判断・ユーザーに訊く）／R2（MusicXML の section header `partial`＝出力が動くのは XML だけ・掃き不要）／R13⒜（診断の collect 共有）。LP 双子が要る R7〜R11 もこの PC で進められる（⑴ の lilypond は解決済み）。
...
- **R5 `cue { }` 内の phrase 参照が黙って落ちる**（`ProcessCueRegion :1796-1799` は bare `GreenSite`・他の容れ物は `GatherMusicSite`）。
