...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第428セッション（2026-09-20・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `b800d7e5`・**未 push 41**・**full `sessions/p428/run1.trx` 8757 / 0 / 3 / 8760＝第427 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・128 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第427 が予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 426`＝20 行 7,481 chars）。指示は「**HANDOFF を読んで着手**」＝§1 の「次の一手」の先頭 ★★★＝**⒴¹¹**。★ ベンチは着手前に静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第427 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、**before の打鍵 8,048,052／8,048,072 B は第427 最終の 8,048,053 B と一致**。⇒ **島を 1 つ閉じ、計器の罠を 1 つ記録した**:

★★★ **⑴ 脚 1 ＝ 政策 4 つを 1 つの run で値付けした**（構成で数える counter＝**各 skyline が政策ごとの capacity を持ち歩く**ので、headroom を捨てる政策には「次の merge が払う分」が請求される。⚠️ 模型の検算＝**第427 の実測を再現した**: m+p の climb **29,946,160 B／73,254 walk** 対 第427 の 29,912,072 B／73,231 walk）。`MergeInternal` は **70,874 walk（38.4／打鍵）・in 1,743／打鍵・out 1,604・out/in 0.920** で、**払うのは 19.8%（14,061 walk）だけ**。政策の値（打鍵の %）: **今日の梯子 0.192／入力の上界を walk の前に 1 発予約 0.159／exact を全部に 0.372／`EnsureCapacity(R)` 0.113**。⇒ ★★★ **「exact を全部に」は今日の約 2 倍払う**＝第427 の警告が実測で裏付いた（**今日 1 バイトも払わない 17,474 walk に 41.8 MB を新しく払う**）。

★★★ **⑵ 島の 92.4% は「その skyline の *1 回目*の merge」**（**26,439,832 / 28,610,688 B**・12,769 / 14,061 walk）。**1 回目は capacity 0 から 4-8-16-…-R と登るので約 2R 払う**。2 回目以降は headroom が効き、**idx8+ は 43,139 walk で 1.5 MB しか払わない**。

★★★ **⑶ 直し＝walk は*常に*貸し buffer に書き、結果の list は R が判ってから寸法を決める**（`66ec1a95`・**Core 1 ファイル**・網 1 本・**数値定数 0・LP 由来の式 0・REF 0／OWN 0**）。**2 つの政策は List 自身のもの**＝構築は `Capacity = R`、**累積は `AddRange` が取る 1 段の成長に任せる**（**空の list には R ちょうど・育った list には倍々**）。★ **`EnsureCapacity(R)` を明示で書くのは `AddRange` の 2 綴り目なので書かない**（§5.2.1②・一度書いてから消した）。⇒ **A/B ＝ −0.0793%**（8,048,052／8,048,072 → **8,041,673／8,041,699**・床 26 B）＝**模型の予測 −6,337 B に対し実測 −6,379 B**。

⚠️⚠️⚠️ **⑷ 罠を踏んだ＝`Copy-Item` の書き戻しは*コピー元の更新時刻*を引き継ぐので `dotnet build` が黙って再コンパイルを飛ばす。RULES §5.5 が第285 から名指している罠の 2 例目で、私はそれを引けずに踏んだ**（§0 の「まず grep」の実例）。毒を戻したあとに取った after 2 本（8,064,239／8,064,216）は**毒 ⑵（exact を全部に）の dll を測っていた**——しかも ★★ **+16,187 B は政策 P2 の予測（+14,500 B）と整合するので「直しが悪化させた」という*筋の通った物語*が読めてしまう**。気づけたのは**仮説を割当時計で測ったから**＝`rent cold 0 / warm 83,538` で「貸し buffer が使い回されていない」が死に、同じ run の合計が予測どおりの 8,041,689 を出した。⇒ ★★★ **A/B の符号が予想と逆に出たら、コードを疑う前に「今測ったバイナリは今のソースか」を訊く**（RULES §5.5 に 2 例目として追記した。`git stash pop` は書き込み時刻が今になるので安全）。

★★ **⑸ 網 1 本＋毒 2 つ**。`AFirstMergeIntoASkyline_DoesNotClimbToTheSizeOfTheAnswer`（300 棟の profile を**冷えた** skyline に 1 度 merge・天井 15,000 B）。毒 ⑴「walk を in-place に戻す」→ **新網 1 本だけが赤で、実測 32,680 B は予測した梯子（8+16+…+512 slot）と 1 バイト違わず一致**／毒 ⑵「exact を全部に」→ **第421 の `AMergeIntoALargeSkyline_DoesNotCopyItToReadIt` だけが赤**（10,264 B）。**2 本は構成上お互いを覆わない。**

★★★ **⑹ 出力同一は 3 つとも**: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline は `b800d7e5` の木から取り直した）／⑵ **実コーパス全ページ SHA-256＝5,824 行・0 行差**（第427 の `hashes-after.txt` と突き合わせ）／⑶ suite 全緑・snapshot 249 枚不動・台帳 851 点／総和 22.584727806 不変。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: **⒴¹¹ ✅ 第428 が閉じた**／★★ **⒴¹² 新規＝resolve walk の*中身*が 0.269%**（第428 脚 2 の割当時計・**83,538 walk・40,172,952 B／sweep＝21,739 B／打鍵**。⚠️ **未分解**＝`ResolveScratch` 3 本／walk と `MergeOverlapping` の `AddRange`・**まず中を割ること**。出口側＝`_buildings` の寸法は 15,643 B／打鍵で、その内訳はもう ⑴ が持っている）／★ **⒴¹⁰ tab fork の残りは `TabSoundingNotes` の iterator ちょうど 64 B × 34,717 items＝0.015%**（⚠️ `fret.ToString`・`FretGlyphWidth`・`CalculateFret` は 3 つとも 0 B）／★ **⒴⁷ `BeamedItemsToSuppress` は beams が空でも `HashSet` 0.028%**／★ **⒴⁶ 残った包み＝`SeedClef` の `PlaceGlyphOutlineCached`**（天井 `ink.clef` 0.021%）／⒴⁸ **batch でない `Merge` 147,417 回／打鍵は未値付け**／★★★ **⒡′ bow を*staff 自身の枠*で採点して offset は描画時に足す**（0.074% ＋ 1 ULP の尾・土台の変更・要設計）／★★ **⒢′ 拒否された staff の前処理＝検出器 0.023%＝`StaffSpannerItemsOf` の memo を最終パスにも通すだけ**（一番安い・出力同一が構成で言える）。⚠️ **まず回数を測ること**／★★ **⒵⁴ `prefixMarkAnchorX` の解き直し 0.338%**＝**memo は反証済み**（hit 率 0.53%）・土台の変更／★ **⒵⁵ `mm.g.split` 0.163%**（⚠️ `OrderBy` は*安定*）／★ **⒵⁶ 文字の 318,784 回／81 綴り 0.130%**／★ **⒴⁵ artic と trill の remap 辞書は作って捨てている**（天井 0.174%）／★ **⒳″ `Padded` の複写 0.028%＋`SortAndResolve` 0.15%**（起票のみ）／★★ **⒴⁗ 天井 2.03%＝安くすると `アゲハ蝶.lys` 1 冊が 24→26 系になる＝perf ではなく*忠実度の判断*。開けるならユーザー決定が先**（実装は Lab `sessions/p423/zz423-deferred-prelim.diff.txt`）・**先に LP 双子**／⚠️ **`g4.core` 1.21% と `p1.s1.beams` 5.538% は*もう実仕事*＝この 2 島には戻らないこと**／⒜ **R13⒝ の実機確認**（第404 ⑵）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／★★★ **LP 双子が要る R7〜R11 は*今日から着手できる*＝`lilypond.exe` の hang は 2026-09-20 に解決**（MCP コンソールのプロンプトの入力読み取り待ち・`cmd /d /s /c "… < NUL > log 2>&1"` で通る＝RULES §5.5）／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑺ 終了時**: commit 2 本（code `66ec1a95`・docs 1 本＝§1 と RULES §5.5。**この文を含むので SHA は書かない**＝§5.4）・**未 push 43**（開始時 41）・**最終 full `sessions/p428/run3.trx` 8758 / 0 / 3 / 8761**（**第427 最終 8760 から +1＝今便の網ちょうど 1 本**。`Zz428`／`ZZPOISON` の grep は**コードに 0 件**）・§7.5（対 `b800d7e5`）**Core `+` 52 行／REF 0／OWN 0**（**算術を 1 つも足していない**＝capacity だけ・§7.6 ⒟）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・棚卸しは **`APPROXIMATIONS.md` も `magic_constants.csv` も行の増減 0**（行番号だけ）。計器と全文は Lab `sessions/p428/`（`exit-nonbatch-leg1.txt`＝脚 1 の政策 4 つ・`Zz428.cs.txt`＋`Zz428Probe.cs.txt`＝脚 1 の計器・**`Zz428b.cs.txt`＋`Zz428Leg2.cs.txt`＝脚 2 の割当時計**・`Zz428Ab.cs.txt` と `Zz428Hash.cs.txt`＝再利用可の A/B と証明 ⑵ の harness・`hashes-after.txt`・`ab-{before,after}-{1,2}-tc0.txt`）。⚠️ **脚 1 の計器は*直した後*は嘘をつく**（rung 模型で 1 発確保を数える）＝**after の数は A/B の通貨で読むこと**。⇒ ⚠️⚠️ **HANDOFF は第427 の記録 447,742 B → アーカイブ後 440,261 B → 今便を書いて 445,843 B＝天井 450,000 まで 4,157 B**（**`Fold-ClosedHandoffItems` の弁は第426 の時点で既に 0 件**・**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない**）。⚠️ **次便は §1 を 1 行も書く前に `-Archive 427` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**そして §1 はこの便の長さ以下に保つ。** **作業ツリーは空**。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第428 の欠陥ではない。
