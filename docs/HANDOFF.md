...
⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: ⒜ **R13⒝ の実機確認**（第404 ⑵ のとおり＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item（第404 ⑵′・実機の 2 行を見てから）／⒝′ frame 変更の `applyFrame`（実機で頻度を見てから）／**⒞⒤ ✅ 終わった（今便）**＝この機械の現在値は `sessions/p409/validators-head2.txt`（fingbeam1k pass **30.9 ms fresh／7.5 warm**・床 0.05 を超えるのは **4 本だけ**＝`AnnotationName` 3.48・`MeasureValidator` 2.78・`RehearsalMarkEngraved` 0.66・`Duration` 0.54）。⚠️⚠️ **`RehearsalMarkEngravedValidator` 0.66 は畳む余地が無い＝*すでに*索引に訊いている**（`DescendantNodes<MusicMarkSyntax>()`・:88）。**この 0.66 は walk ではなく、会った mark 1 つずつに対する `AnnotationValues.Rehearsal` の実仕事**（fingbeam1k は注釈が 24,000 ある本＝第408 ⑴）。⇒ **尾部に「まだ畳める validator」は 1 本も残っていない。**（⚠️ この段落は当初「`DescendantNodes()` を使っておらず別の形＝次に見るならここ」と書いていた＝**誤り**。便の終わりに実物を読んで訂正した——§5.0 の「起票の住所は仮説」がそのまま自分の書いた §1 に当たった。）⇒ ★★★ **⒥ は第 3 便が測り直した（上の ⑶）＝「約 23 ms の red 実体化＋index 建て」は誤り**。**pass の中で建てているのは `DescendantIndex` の 7.0 MB／~4.7 ms だけ**（28 本全部が同じバイト数を払う＝他に per-tree 構造は無い）・**red 実体化 19.3 MB は parse ＋ collect の島**・残りは建てではなく初回タッチ。⇒ **⒥ を採るなら上限は 4.7 ms と知って採る。** ★★★ **そして同じ条件で collect は pass の約 3 倍（~89 対 ~27 ms）＝次に測るならそちら**（バイトで測ること・実コーパスでは未測定）。**validator 側を削る島は尽きた**＝残るのは count loop の page DP 5.3 ms（R13⒦・census で hit 率）／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝fingbeam1k の `fs.walk` 0.41 ms／1.5 MB と `fs.assemble` 0.29 ms／4.0 MB（24,000 digit の実仕事＝「何を返すか」を変える別の島）／**⒞″ ✅ 第407 第 2 便が測って「やらない」と決めた**／**⒟ ✅ 第407 で閉じた（R2R）**／LP 双子が要る R7〜R11／**R15 ✅ 削除は第407 で完了**（残り＝「2 綴り」一族のみ＝統合・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑷ 終了時**: commit 7 本（第 1 便 code `310f5ea9`・第 2 便 code `b0298b08`・第 3 便はコード変更 0・docs `065054be`・RULES `1cb72a0c`・§1 の手直し 2 本・第 3 便 docs 1 本）＋Lab 3 本（`b166649`〜）・**最終 full `sessions/p409/run6.trx` 8715 / 0 / 3 / 8718**（第408 最終 8704 / 0 / 3 / 8707 に対し**網 +11**＝`LyricSingsWalkTests` 8 本＋`TailValidatorKindsTests` 3 本）・§7.5（対 `b6026a2d`＝スクリプトの出力）**Core `+` 216 行／REF 0／OWN 0**（**数値定数なし・LP 由来の式なし**＝同じ述語をより少ない節点に向けただけで計算は 1 つも動いていない＝監査対象は「同じ答えか」＝**census 760 冊 435 行 73,495 バイトが 2 便ともバイト同一**＋毒 10 腕＋snapshot 249 不動）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。棚卸し `docs/APPROXIMATIONS.md` は**不変**（`TheInventoryIsNotStale` は緑のまま）。§7 4（恒久知識）＝**RULES §5.3 に 2 項**（「2 綴り目の代償は filter の形で決まる＝1 型なら kind 表は要らない」「木ごとの CWT は打鍵ごとに払っている」）。計器と全文は Lab `sessions/p409/summary.txt`（第 3 便は `freshtree-summary.txt`）。§7 3.5 は済（第407 を `-Archive 407`＝21 行 5,998 chars）。⚠️ **それでも §1 を書いたら 449,895＝天井まで 105 バイトだったので、§3「決定済み」の根拠セルを 22 個 ARCHIVE へ畳んだ**（`Fold-ClosedHandoffItems.ps1 -Session 409 -Before 2026-09-10`・18,444 バイト・**決定・日付・★ は §3 に残る**＝表の仕事は「蒸し返さない」ことで、それは主張が残れば足りる。§2 は畳む物が 0 だった）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️⚠️ **天井 450,000 バイトに対し今便の終わりで約 438.7 KB＝残り約 11 KB**（§3 を畳んで空けた）＝**次便も §1 を書く*前*に §7 3.5（`-Archive 408`）を回す**（天井は上げない）。

## 以下は第408セッションの経緯

最終更新 第408セッション（2026-09-18・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `e34b0a33`・未 push 25・**full `sessions/p408/run1.trx` 8693 / 0 / 3 / 8696＝第407 最終と一致**・CI は `851e5538` の run が緑（それより前の 4 run は赤のまま・`16f5157a` 以降は未 push）・build 0 エラー・`Session-Check -Build -Test` 82 秒）、指示「HANDOFF を読んで着手」→ §1「次の一手」のうち ⒜ 実機確認・⒝／⒝′ は実機の 2 行が要る、⒟ と `RestCollisionsOf` は閉じている／要ユーザー判断なので、**⒞ の「診断 pass の残り」の筆頭＝`AnnotationName` を採った**。根拠は回数と割当を主に（ユーザー指示・CLAUDE-OPERATIONS §2）。道具は pwsh MCP（Bash 0 回）。**閉じたのは 1 本＝R13⒩**:

⚠️ **第407 §1 の「最終 full `run4.trx` 8700 / 0 / 3 / 8703」は*古い数*だった**——`run4` は便の途中で、その後の第 5 便（R15 の削除）が網 −7 したので**第407 の最終は `run26.trx` 8693 / 0 / 3 / 8696**。今便の開始時の数がそれと一致した。**⑽ の段落の数だけが直前の run を写したまま残っていた**＝§7 の終了時に「最後の run の数を写す」ことを、途中の便の数で上書きしない。

★★★ **⑴ `AnnotationName` の費用は「述語の鎖」ではなく*建て*だった**（`3b12cbd6`・**全文と数は §2 R13⒩**・probe は `sessions/p408/`）。打鍵あたり: 234,030 節点を歩いて 24,000 の注釈に会い・`MarkName` を 24,000 本建てて*報告する枝しか読まない*・`Arguments` を 24,000 回建て直す（1 個 656 B）。⇒ **18.82 MB → 6.92 MB／6.81 → 3.13 ms**。★★★ **per-node cache は counter で「当たらない」と決めた**（cold pass 72,000 回でも**実 keystroke の render は 48 回**）＝**覚えるのではなく建てを軽くする**。⚠️ **validator に安い述語を書くのは禁じ手**（VALUE_SITE_AUDIT §9.3）。健全性＝census 435 行一致＋毒 4 腕＋網 2 本。

★★★ **⑵ 第 2 便＝尾部 11 本の「全木 scan」を*綴りを 1 つも増やさずに*軽くした**（`a743726e`・**全文は §2 R13⒪**）。★★★ **まず一覧を取り直したのが効いた**——§1 が名指していた「`MeasureValidator` 10／`LyricSings` 6.5」は**第401 が*前の機械*で測った数**で、この機械では順位が違った。⇒ **古い機械の一覧で validator を割らない。** 尾部＝**11 本が 234,030 節点を舐めて一握りに会う＝打鍵あたり約 2.6M 節点訪問**。bucket 化は 11 本の 2 綴りが要るので**やらず**、**`DescendantNodes()` の戻り値を構造体にした**（root では既に*配列*を `IEnumerable` に入れて返していた）＝**呼び出し側を 1 箇所も変えずに** pass 44.4 → 39.9 ms・warm 21.0 → 17.3・割当 72 → 40 B。★★★ **対照が内蔵**＝第 1 便で bucket に移した `AnnotationName` だけ**動かない**。健全性＝census 一致・毒 309 本・⚠️ `DescendantIndexTests` の「built once」が**戻り値に `Assert.Same`** していて唯一の実害を捕まえた。

★★★ **⑶ 第 3 便（ユーザーの問い「再帰を繰り返しに書き直したら良い形になるの?」→ 有利と判断して着手）＝第 2 便が「畳めない」と書いた再帰を畳んだ**（`c168b220`・**全文は §2 R13⒫**）。⑴ **明示スタックの繰り返しは*遅い*（1.44 → 1.68 ms）**＝**再帰という「形」は費用ではない**。⑵ ★★★ **刈る walk は 22 ns/節点・平坦配列は 2.2 ns/節点**＝**「全部見る 234,030」のほうが「刈って 65,009 だけ見る」より速い**＝費用は*slot* あたり。⑶ **65,009 節点を歩いて着くのは 2 節点**。⇒ **仕事をする 4 kind を index に訊き、刈り込みは*祖先*に訊く**＝**whole 3.39 → 2.33 ms・walk 2.12 → 1.06**。網 9 本（旧再帰をテスト側の reference に）・毒 8／8 本・census 一致。⚠️⚠️ ★★★ **census の base を*前の便のファイル*と比べて「875 行違う」と誤警報を出した**——**Lab の corpora はユーザーが*今*編集している作業場**＋`WriteAllLines` の 435 行を `Get-Content` が 440 行に読む（メッセージ内の改行）。⇒ **base はその場で stash して取り直す**。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: ⒜ **R13⒝ の実機確認**（第404 ⑵ のとおり＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item（第404 ⑵′・実機の 2 行を見てから）／⒝′ frame 変更の `applyFrame`（実機で頻度を見てから）／⒞ **診断 pass の残り**＝⚠️ **第401 の一覧（`MeasureValidator` 10／`LyricSings` 6.5）は前の機械の数で使えない**。この機械の現在値は `sessions/p408/validators-after.txt`（fingbeam1k pass 39.9 ms fresh／17.3 warm・`MeasureValidator` **4.48 → 第 3 便で 2.33**・`AnnotationName` 3.66・`LyricSings` 2.40・尾部 9 本が 0.5〜0.8）＝**残るのは ⒤ kind 表を 1 本ずつ足して尾部を bucket に移す**（**第 3 便の型がそのまま使える**＝仕事をする kind を index に訊き、条件は祖先に訊き、旧 walk をテスト側の reference にして差分の網を張る）（1 本あたり ~0.5 ms・代償は 2 綴り＋網。**一番安いのは `LyricSings` の第 1 walk**＝`PartReferenceFinder` に第401 の*網付き*kind 表が既に在る）か **⒥ fresh tree の red 実体化＋index 建て（39.9 のうち ~22 ms）**＝第401 が「validator が red を読む限り要る」と書いた所・count loop の page DP 5.3 ms（R13⒦・census で hit 率）／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝fingbeam1k の `fs.walk` 0.41 ms／1.5 MB と `fs.assemble` 0.29 ms／4.0 MB（24,000 digit の実仕事＝「何を返すか」を変える別の島・**今便の残り 288 B もこの島**）／**⒞″ ✅ 第407 第 2 便が測って「やらない」と決めた＝残りは畳む価値が無い**／**⒟ ✅ 第407 で閉じた（R2R）**／LP 双子が要る R7〜R11／**R15 ✅ 削除は第407 で完了**（残り＝「2 綴り」一族のみ＝統合・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑷ 終了時**: commit 6 本（第 1 便 code `3b12cbd6`＋docs `64487311`・第 2 便 code `a743726e`＋docs `4d78c53a`・第 3 便 code `c168b220`＋docs）・**最終 full `sessions/p408/run6.trx` 8704 / 0 / 3 / 8707**（第407 最終 8693 / 0 / 3 / 8696 に対し**網 +11**＝`AnnotationKindsTests` 2 本＋`MeasureValidatorWalkTests` 9 本。第 2 便は網 0 本＝既存の `DescendantIndexTests` を 1 行向け直しただけ）・§7.5（対 `e34b0a33`＝スクリプトの出力）**Core `+` 354 行／REF 0／OWN 0**（第 1 便 137＋第 2 便 117＋第 3 便 100）（**数値定数なし・LP 由来の式なし**＝建てる物を減らしただけで計算は 1 つも動いていない＝監査対象は「同じ答えか」＝census 760 冊 435 行の順序込み完全一致＋毒 4 腕＋snapshot 249 不動）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。棚卸し `docs/APPROXIMATIONS.md` は**行番号 1 行だけ**（`:444` → `:474`・`LILYSHARP_UPDATE_DOCS=1` で再生成し、行番号を伏せて機械で一致を確認）。§7 3.5 は済（第406 を `-Archive 406`）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️⚠️ **天井 450,000 バイトまで残り僅か**＝**次便は §1 を書く*前*に §7 3.5（`-Archive 407`）を回す**（~6 KB 空く・天井は上げない）。⚠️ **`-Archive` は 1 便ぶんだけ**＝§1 は「現在便＋直前の 1 便」ちょうど（2 つ回して網 3 本を落とした）。
