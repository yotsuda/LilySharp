...
**3463 passed / 0 failed / 3 skipped**・Core 0 warn 0 err・
...
### 第25セッション（2026-07-28）＝ **代理表現 A・ossia のインク・glyph skyline・X の単位**

**commit 15 本**。⚠️ **数と経緯は台帳の `why` と commit message にある**——ここには
**次に手を動かすために要ることだけ**（§4）。閉じたもの:

| commit | 何を | 値段 |
|---|---|---|
| `2feb6021` | `is_spaceable` の**5 綴り**（引き継ぎは「3」）→ property lookup | byte 不変 |
| `c2955ba3` | **ossia のインクを scale**（`VerticalSkyline.Scale`） | snapshot 3 |
| `6c6be1af` | **glyph skyline＝アウトライン**（ただし**grob ごとの宣言**） | snapshot 74 |
| `94705160` | **譜のサイズはメトリクスに載る**＝ seed は掛け算しない。`Scale` 消滅 | 出力不変 |

- ★★★ **§2C の clef 族が丸ごと閉じた**（12 点が 1e-7・3 点 exact）。
  **`system.stretched-distance` の「未説明の 0.005＝フォント量」は符頭ではなく clef だった。**
- ★★★ **引き継ぎの誤りを 4 つ訂正した**（全部 §5 へ汎化済み）: 綴りは 3 でなく 5／
  「もう ossia の量ではない」は誤り／「clef の下向きは 8e-5 で一致」は**薄まった残差の誤読**／
  **「未特定の 0.27%」は単位の取り違えで定数ですらなかった**。
- ★★ **総和は上がった**（2.5577 → 2.6070）が**後退ではない**——上がったのは lyrics 3 点だけで
  **打ち消しの解除**（§5.3）。**exact は 137 → 140。**
- ★ **自分で入れた代理表現を 1 つ起票した**＝`StaffSize.Of` の `IsOssia` 列挙（棚卸し ⑸）。

（**第21〜24セッションの節は落とした**。第23・24＝**ossia の島**は逐語で
`HANDOFF-ARCHIVE.md` の末尾へ。⚠️ そこの「残りは ossia の量ではない」という読みは
**第25セッションで訂正済**——実際は ossia のインクが scale されていなかった分で `c2955ba3` で閉じた。
第21・22 は commit message と台帳の `why` に。**原則は全部 §5 へ汎化済み。**）

### ▶ 次の一手

★★★ **推奨順（第25セッション末に棚卸し・全部「点か決定が先」）**

1. ★★ **`lyrics.*.staff-to-lyric` の閉じ方 −0.105961＝LP の `Skyline::distance` の当たり点**。
   **今セッションで instrument の型が確立した**（`probes/glyph-skyline.ly` に
   `after-line-breaking` の callback を足して `cmd /c "... < NUL"` でデタッチ実行）。
   ⇒ **同じ手で「LP がどの点で当てているか」を dump できるはず**＝**もう BLOCKED ではない可能性が高い**。
   ⚠️ ただし**確かめてから**書くこと（今回 §2C が「instrument が要る」と言いながら
   **既にプローブが repo に在り前任者が最後の一歩を残していた**のと同じ形かもしれない）。
...
3. **annotation pass の magnification**（今セッションが名指しで残した「1 フレーム外の同じ問い」）。
...
**非ゼロで残っている台帳点は 33 点**（36 から −3＝glyph skyline 移植で 3 点が exact になった）。
⚠️ **下の表は第24セッション時点の並びで、今回動いた 15 点は行の中に追記してある**——
**次に触る人は「残差」列を信じる前に §0 でテストを走らせること**（§5.2）。
★ **今回の最大の変化は「clef 由来の固定項が全部消えた」こと**で、残る最大の族は**歌詞の書体量**:

| 残る族 | 量 | 状態 |
|---|---|---|
| **歌詞書体（up-extent 1.459200 対 1.187880）** | **+0.271310** が 5 点＋`staff-to-lyric` 3 点の中にも | ⚠️ **0 にしない。**埋めるなら書体メトリクス側（⚠️ **CJK の受け皿が先**） |
| `lyrics.*.staff-to-lyric` の**閉じ方** | **−0.105961**（clef の箱が隠していた分 0.034 が剥がれた） | **LP の `Skyline::distance` 当たり点**＝ ▶ の instrument 対象 |
| `lyrics.*.system-gap` | +0.207200 ×2 | フォント量＋小節番号 cap-height |
| Pango 量子化・符頭 metric | 5e-6〜1.4e-3 | 閉じる予定の無い名前付き残差 |
