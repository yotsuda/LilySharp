...
⇒ ★★★ **⑹ 次の一手**: **⒜ 今便が起票した 1 件＝双子の per-score `fonts NAME` が `PageModel` に届かない**（⒝ の穴の fonts 側・直すと per-score fonts を使う本の `.ly` が動く＝射程を測ってから）。**⒝ `layout {}` の 2 つ目以降のキー（上 ⑴ の ②〜⑤）はユーザー決定**——私の推し順は ② `accidentals`（LP に `\layout` の前例が在る）→ ④ `partCombineText` → ③ `sectionLabels` → ⑤ 和音綴り。**⒞ 第369 ⑸⒝ の並びはそのまま**（tab の wish の merge＝設計級・`lily/spacing-spanner.cc generate_springs` の wish の本数を読むところから）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。**push はユーザー**。

**⑺ 終了時**: **full `scratch/p371/run3.trx` 7740 / 0 / 4 / 7744（run2 の赤 2 を直した後・RunInfos は SKIP 4 だけ・5 m 28 s・HANDOFF／CHANGELOG／APPROXIMATIONS 込み）**・commit 1 本（Core 3 新規＋Core 16 変更＋Lsp 3＋Tests 6＋docs 4＋tmLanguage＋CHANGELOG＋この HANDOFF＋ARCHIVE）・`Deploy-Lsp.ps1`（Core と Lsp が動いたので server）。**push はユーザー（未 push 8）**。
...
| 決定 | 根拠（要点） |
|---|---|
| ★★★ **score 全体の*表示スイッチ*は `layout [NAME] { }` ブロックに集める＝`marks stacked\|beside`・`barNumbers lines\|none\|every N`。裸の `marks` directive は撤去**（2026-09-11・第370・**ユーザーの問い「トップレベルに書く今の規則は妥当か。`layout {}` を足しては。ほかに入れる語は」→ 私の分析→「良い。着手して」**・✅ 同便実装＝`Semantics/LayoutPlan.cs`／`LayoutPlanReader.cs`／`LayoutValidator.cs`・LYS9101〜9110・§1 第370） | 裸 `marks` の 3 根拠（override にしない・paper にしない・`pitch` と同形）は生きていたが、**⒜ 音楽の設定の列に混じった唯一の表示専用の裸グローバル**で、**⒝ `fonts { marks "…" }` の font グループと同じ語**＝裸形にはどちらの意味か言う場所が無い。ブロックなら「marks の*どの面*か」をブロック自身が言う。**`layout {}` の値打ちは名前付きブロックの再利用**（`fonts NAME`／`paper NAME` と同じ 2 段）なので**キー 1 つでは裸語より悪い**＝2 つ目（`barNumbers`）と同時に作った。**入る条件**＝表示専用・score 全体・単位なしの閉じた語彙・grob 単位でない。**paper との線引き**＝単位のある量（mm・ss・justify のフラグ）は paper、描き方の切り替えは layout（`indent`／`raggedRight` は LP 自身が `\paper` に置くので paper に残す）。`layout` は LP の `\layout` からバックスラッシュを落とした綴り（`break`／`pageBreak` の決定と同じ規則）。**未実装の候補（ユーザー決定待ち）**: `accidentals`・`sectionLabels`・`partCombineText`・和音記号の綴り。 |
