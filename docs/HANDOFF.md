...
最終更新 第418セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `2e0ab899`・**未 push 14**・**full `sessions/p418/run1.trx` 8732 / 0 / 3 / 8735＝第417 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 69 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第417 が予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 416`＝44 行 5,715 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」の ★★ 4 つを AskUserQuestion に出し、**ユーザーが ⒴‴「上がった 17 冊の機構を測る」を選んだ**。★ ベンチは着手前に AskUserQuestion で静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第417 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト）:

★★★ **⑴ 機構を探しに行く前に床を取ったら、床のほうが問いより大きかった。** 同じビルド・同じ側・2 プロセスで **231 冊中 1 冊も一致しない**（**1,848 打鍵が全部違う**）・**0.05% 以上動く冊 55**・**最大 +0.49%**＝**第411〜第417 が冊ごとに引用したどの数より大きい**（合計のほうは 0.0079% しか動かない）。★ **別スレッドへの取りこぼしではない**（`GC.GetTotalAllocatedBytes`＝全スレッドの数は当スレッドの数と **0.006% 差**）。⚠️ **第417 が Lab に残した `ab-before/after.txt` 2 枚は「167 冊が上がった・合計 +0.07%」と言っており**、§1 ⑹ の「17 冊・−0.22%」とは別の run だった＝**同じ変更の同じ問いに 3 通りの答えが出ていた**。

★★★ **⑵ 予測を先に書いて、当たった**（§5.0②）＝**正体は tiered JIT で、第403 が*時間*について記録した罠の*割当*版**（**tier 1 は inline と escape 解析がヒープ割当をスタック割当に変える場所**＝まだ tier 0 で回った冊は多く割り当て、**どの冊がそれかは壁時計**（`TC_CallCountingDelayMs` の 100 ms）で決まる）。★★ **署名は「前詰めの散らばり」**（位置ごとの平均 |Δ|＝**0〜39 冊目 0.15〜0.21%・80 冊目以降 0.010〜0.015%**）。**`DOTNET_TieredCompilation=0` で 25 倍縮む**（合計の run 間差 0.0079% → **0.00002%**・0.05% 以上動く冊 55 → **0**・最大 0.49% → **0.02%**）。⚠️ **TC=0 は絶対値を 0.86% 上げる**（dynamic PGO が消えて inline が減る）＝**TC=0 どうし・tiered どうしで比べ、混ぜない**。

★★★ **⑶ 計器を直すと、上がった冊は 0 だった。** 第417 の直し（1 部屋 → 2 部屋）を**両側 TC=0** で取り直し: 打鍵 **8,810,889 → 8,797,893 B＝−0.147%**（§1 ⑹ は −0.22%）・**最良 `himawari.lys` −2.54%**（§1 の −2.60% と一致）・**−1% 以上 13 冊／−0.5% 以上 16 冊／0.05% 以上動いた 30 冊**（§1 は 168 冊と書いていた）・**不動 199 冊**・★★★ **+0.02% 以上上がった冊は 1 冊も無い**（最も正で **+0.014%**）。⇒ ★★★ **島 ⒴‴ の*主語*が存在しなかった＝機構は無く、直すものも無い**（§5.0 の「N を数え直す」の **10 例目**で、外れたのは *N が存在するかどうか*だった）。

★★ **⑷ 名指された候補も、測ると小さすぎた。** `StackBelowStaffMemoized` の Filter/remap/Rebuild に継ぎ目を入れると（TC=0・全掃き）: **`below.filter` 0.074%**・**`below.rebuild` 0.100%**・**`below.groups` 0 B**（line group の remap は**このコーパスで 1 度も発火しない**）＝**この経路の全部で打鍵の 0.174%**。**呼び出し 4,010 回／1,848 打鍵のうち hits>0 は 2,572 回**（16,050 hit / 16,574 系＝96.8%）＝★ **「hit 1 件で門が開く」なら、その門は第417 より前から開いていた**。⇒ **0.65% の犯人ではありえない**（天井が 0.174%）。

★★ **⑸ 古い便の何が死ぬか**: **合計は生き残る**（run 間 0.008% に対し、主張されていた効果は 0.15〜5%）。**死ぬのは第411〜第417 の*冊ごと*の内訳**（早い冊で 0.5%・遅い冊で 0.05% より小さい主張）。§7 4（恒久知識）＝**RULES §5.0 に 10 例目**（「N を数え直すは、N が 0 でありうることを含む」＋「**『未計測』と書いて宿題を残すなら、*床*も一緒に書く**」）と **RULES §5.3 に 1 項**（「**割当バイトが機械に依存しないのは*合計*だけ。冊ごとの A/B は tiered JIT の床の下＝`DOTNET_TieredCompilation=0` で両側を取る**」）。⚠️ **第417 が §5.3 に入れた 3 項のうち 1 項（「hit を増やすと費用が増える経路＝17 冊が上がった」）は実例が消えたので*その場で書き換えた***＝残したのは「A/B は冊ごとに割る」「読む前に床を出す」「推論と測定を混ぜない」の 3 つ（§5.4 の dead citation と同じ理由＝**覆った実例を残すと、次便がそれを根拠に探しに行く**）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: **⒴‴ ✅ 閉じた**（★ **次に冊ごとの A/B を読む便は TC=0 で両側を取ること**）。★ **⒴⁵ Filter/Rebuild の無駄＝artic と trill の remap 辞書は作って捨てている**（上の ⑷ の内側・**天井が 0.174% と測れている＝「やっても 0.1% の島」で、安い日の仕事**）／★★ **⒴″ `b.scriptsFingerings` 4.2%**・未分割（**第417 で fing の miss が 73% 減ったので取り直すこと**）／★★ **⒳′ resolve の*複写*＝`EndBatch` の入力複写 0.17% ＋ `MergeInternal` 自身の複写（未計測・推定 0.7%）**＝call をまたいで生きる buffer が要る（instance か static）／★ **⒵‴ `e4.musicMarks` 1.22%**・未分割／★ **⒴ `inkFirst`＋`inkLast` 0.53%**／⚠️ **`g4.core` は repair 後 1.21% で*もう実仕事*＝この島には戻らないこと**／★★ **⒴⁗ 1 度目の配置を*安くする*＝`p1.prelim` は打鍵の 13.96%** で、**数が変わる 17.9% の打鍵ではその全部が捨てられる**（`ChooseSystemCount` が読むのは高さの見積りだけ＝**「見積り専用の安い prelim」が要るかは未測**・土台の変更）／⒜ **R13⒝ の実機確認**（第404 ⑵＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・⚠️ 未着手・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／LP 双子が要る R7〜R11／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑹ 終了時**: ★★★ **製品のコードは 1 バイトも動いていない**（この便は計器の便＝commit は docs 1 本だけ・**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 1 本・**最終 full `sessions/p418/run3.trx` 8732 / 0 / 3 / 8735**（§1・RULES を書いた*後*の最終ツリーで取った）（⚠️ **第417 最終と同じ＝網を 1 本も足していない**・`Zz418`／`ZzStage` の grep は**コードに 0 件**・継ぎ目は `git checkout` で戻した）・§7.5（対 `2e0ab899`）**Core `+` 0 行／REF 0／OWN 0**（§7.6 ⒟「何も足していない」）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。⚠️ **棚卸し `docs/APPROXIMATIONS.md` と `audit/magic_constants.csv` は不変**（Core が無変更なので再生成不要）。計器と全文は Lab `sessions/p418/`（`summary.txt`・`ab-{a1,a2,tc0-1,tc0-2,one-t1,one-tc0-1,one-tc0-2}.txt`・`named-path.txt`・`Zz418Probe{1,2}.cs.txt`・`ZzStage.cs.txt`・`zz418-seams.diff.txt`）。§7 3.5 は**セッションの頭で**済（`-Archive 416`＝44 行 5,715 chars）⇒ **HANDOFF は 440,367 B → アーカイブ後 434,652 B → 今便を書いて 444,339 B＝天井 450,000 まで 5.7 KB**。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 417` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第418 の欠陥ではない。
