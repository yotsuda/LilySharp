...
最終更新 第411セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `9329a196`・未 push 49・**full `sessions/p411/run1.trx` 8718 / 0 / 3 / 8721＝第410 最終と一致**・CI は `851e5538` の run が緑（それより前の 4 run は赤のまま・以降は未 push）・`Session-Check -Build -Test` 121 秒）、⚠️ **裏取りが 1 つ赤を出した＝Core の 0 警告が破れていた**（下の ⑴）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」が名指していた ★★★ **⒴ `PlaceSystems` の*中*を段別に割る**を採った——**第410 第 7 便が「天井が律速」で見送った当のもので、§7 3.5 を*頭で*回して 6,368 バイト空けたので入る**。★ **ベンチは着手前に AskUserQuestion で静かな窓をもらった**。根拠は割当バイト（CLAUDE-OPERATIONS §2）。道具は pwsh MCP（Bash 0 回）:

★★★ **⑴ 第 0 便＝Core の 0 警告を戻した**（`6097601a`・**変更は註だけ**）。第410 第 2 便が `SemanticVoiceKinds` の表を **`SemanticVoices` の doc ブロックと本体の*あいだ*に差し込んだ**ので、`<param>` 3 本と `<paramref name="root"/>` が**引数を持たないフィールドに束縛された**＝`CS1734` 1 本・`CS1572` 3 本。表を本体の doc の*上*へ移しただけ。⚠️ **1 便生き延びた理由が肝**＝**build は exit 0 を返し**、solution 全体の 31 警告（Tests の analyzer＝想定内）に紛れて流れる。**見張りは `Session-Check` の build 節の `LilySharp.Core` の行だけ**＝§0 の「Core の 0 警告は XML doc の健全性を含む」は正しく、**落ちたのは読む手順のほう。**

★★★ **⑵ 第 1 便＝`PlaceSystems` を段別に割ったら、*置いている*のは 4 分の 1 以下だった**（**コード変更 0**・全文は Lab `sessions/p411/summary.txt`・計器 `Zz411Probe1`＋一時 `ZzStage` 継ぎ目（**commit しない**）・実冊 **292 冊 × 8 打鍵**・`RenderIncrementalPages`・**Release**・バイト）。★★★ **1 回の置きの内訳**＝**`h.prelim`（`RunPreliminaryAnnotationPass`）77.7%／`f.layoutSystemsLoop` 21.3%／system 0 の支度 a〜e ＋ `g.looseLines` で合わせて 0.9%**。⇒ ★★★ **打鍵に対しては注釈パスが 25.0%＝*打鍵の単独最大*（`S0.collect` 17.8% より大きい）・292 冊のうち 249 冊で首位**。**置き*そのもの*は 7.2%。** ⇒ **⒴「`PlaceSystems` は L2＋L4＝32.2%＝単独最大」は合計としては正しく、その 4 分の 3 は尾部の注釈パスであって置きではない。**

★★★ **⑶ そして 2 度目の置きは、ほぼその注釈パスだけ**。P2/P1 は **a 0.00／b 0.00／e 0.03／f 0.06／d 0.15／c・g 0.16 倍**と全部潰れるのに **`h.prelim` だけ 0.45 倍**＝**P2 の 1,312,570,568 B のうち 1,241,899,752 B＝94.6% が注釈パス**。⇒ ★★★ **第410 第 6 便の案 ⑴「2 度目を要らなくする」は打鍵の 8.1% で、賞金の 94.6% は「注釈パスを 2 度走らせない」こと。案 ⑵「per-system より*下*を memo する」が狙う配置ループは *memo が既に 0.06 倍に潰している*＝覆われていないのは `h` のほう。**

⚠️⚠️ **⑷ 計器を先に疑い、疑いのほうが外れた**（§5.3 の作法が*他人の表の読み*に当たった）: `RunPreliminaryAnnotationPass` は **呼び手が 1 箇所＝`PlaceSystems` の尾部**なので、第410 第 5 便の表が `L2.placeSystems#1 24.0%` と `S1.prelim 17.0%` を**同じ分割の別の行**に並べているのは二重計上に見えた。**違った**——★★★ **「prelim」という名の*別のパス*が 2 つある**: **`S1.prelim` は collect と layout の*あいだ*の compile 段**（spring ベクタと break gate の prefix 幅）、**`h.prelim` は `PlaceSystems` の尾部の注釈パス**。**同じ 1 run で第410 の分割を建て直したら合った**＝`L1` 38,573,184（**バイト一致**）・`R2` 390,761,664（一致）・`L3`・`L5`・`L6`・`R1`・`S0` も一致し、**`S1.prelim` は 2,741,893,784 対 第410 の 2,741,892,672＝掃き全体で 1,112 バイト差**。**S1 に継ぎ目を与えた瞬間、未計上が 17.2% → 0.2% に落ちた。** ⇒ **第410 の分割は正しく、直交していた。**

⚠️ **⑸ Debug と Release は絶対値を動かし、順位を動かさない**（同じ掃きで打鍵 **7,399,593 対 6,892,000 B**・`h.prelim` 25.9% 対 25.0%・首位冊数 273 対 249）＝**第410 の表とバイトで突き合わせられるのは Release だけ**（第 3 便の「Release・TC=0」）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: ★★★ **⒵′ `RunPreliminaryAnnotationPass` の*中*を段別に割る**＝**今便が割ったのは `PlaceSystems` であって注釈パスではない**・**同じ harness に 1 段足すだけ**（`Zz411Probe1` は Lab に在る）・打鍵の **25.0%**・**249/292 冊で首位**＝**いま最も大きく、最も安く測れる島**／次に ★★ **⒴′ 2 度目の注釈パスを要らなくする**（打鍵の 7.7%。⚠️ **P2 は P1 の 0.16 倍の系に 0.45 倍のバイトを払う＝1 系あたり約 2.8 倍**——**これは答えではなく問い**で、2 度目が 1 度目を*やり直している*のか*別の仕事*なのかは未測）／⒜ **R13⒝ の実機確認**（第404 ⑵＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械）＝上限は red を 14〜20% だけ実体化＝fingbeam1k で 26.2 MB → 約 10 MB。⚠️ **未着手・未見積もり＝validator ではなく土台の変更**（前提だった ⒮ は第410 で片付いている）／⒵ **collect 17.8% と `S1.prelim` 17.0%**（何度も削った島だが実冊では今なお合計 35%）／⒥ は第409 が上限 4.7 ms と測った（採るならそれを知って採る）／**R13⒦ ⒤ ✅ 第410 が「測って、やらない」で閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅ 第407 が測って「やらない」と決めた**／**⒟ ✅ 第407 で閉じた（R2R）**／LP 双子が要る R7〜R11／**R15 ✅ 削除は第407 で完了**（残り＝「2 綴り」一族のみ＝統合・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑹ 終了時**: commit 3 本（第 0 便 code `6097601a`・§7 3.5 の docs `4be00cc7`・第 1 便 docs 1 本。**製品コードを触ったのは註の移動だけ**）＋Lab 1 本・**最終 full `sessions/p411/run4.trx` 8718 / 0 / 3 / 8721**（第410 最終と同じ＝**網の増減なし**。計器は全部外してあり、この run は §1 と RULES を書いた*後*の最終ツリーで取った）・§7.5（対 `9329a196`＝スクリプトの出力）**Core `+` 16 行／REF 0／OWN 0**（**数値定数なし・LP 由来の式なし**＝註の移動のみ＝振る舞いは 1 つも動いていない）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。棚卸し `docs/APPROXIMATIONS.md` は**不変**。§7 4（恒久知識）＝**RULES §5.3 に 2 項**（「**他人の分割表の行を読み替える前に、その継ぎ目を自分の 1 run で建て直す**」「**Debug と Release は絶対値を動かす＝比べる相手が取られた構成で測る**」）。計器と全文は Lab `sessions/p411/summary.txt`（`Zz411Probe1`・`ZzStage`・継ぎ目は `zz411-seams.diff.txt`・生データ `inside-placement.txt`）。§7 3.5 は**セッションの頭で**済（第409 を `-Archive 409`＝17 行 6,368 chars）⇒ **HANDOFF は 443.9 KB → 437.6 KB に落ちてから §1 を書いた＝天井 450,000 まで約 6 KB**。⚠️ **§3 の圧力弁は第409・第410 で 3 回使って残っていない**＝次便も**頭で `-Archive 410` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。
