...
**⑴ `PositionSystemsOnPage` の段間の床を吐いて LP と対にする**: ⑿ が示したのは「LP の段間は幅 3.49 で内容ごとに動く＝床が縛っている／Lily# は幅 1.54 でほぼ一定＝床が縛っていない」。**やること**: ⒜ 1 頁目の各ばねについて **(spec の basic/min, skyline から出した床, 解いた距離, 力)** を吐く（`scratch/p338/ZzP338cProbeTests.cs.txt` に足すのが早い）。⒝ **LP 側の床は手で出せる**——上の段の下インク ＋ 下の段の上インク ＋ padding 1（両方 `scratch/p338/bare/lp.out` の `PROBEG` 行に在る。⚠️ **括弧は頁の値付けに入らない**ので下インクは括弧を除いた値を使う＝⑻）。⒞ 食い違う段を 1 本選び、**`VerticalSkyline.Distance` が LP の `Skyline::distance` と同じ X で測っているか**（`skyline-horizontal-padding` 1.0・system 原点フレーム）を確かめる。⒟ 台帳点は `system.staff-tab.inter-system-floor` の型で LP と対 → 直す（**出力を変える＝承認事項**）。
