...
book → ⑴ 丸め → ⑵ カーニングまで通し、7 点が EXACT・1 点が「フォントに無い項」として残った**）。
...
★★★ ⚠️ **1 点だけ動かなかった＝`text.width.aa` の −4px。これが最大の収穫**（外れた予測）。
**HarfBuzz は A·A に調整を返さない**（shape 結果 704/704）。**フォントにも無い**（legacy `kern`
テーブル自体が無い＝CFF GPOS GSUB OS/2 cmap head hhea hmtx maxp name post）。
**推測でなく本を 4 冊足して絞った**（`TM1`–`TM4`）:
```
"An" 84 = 45+39 ／ "nA" 84 = 39+45   ⇒ 「A を含むペア」ではない
"AAAA" 192 = 4×45 + 3×4              ⇒ **ペアごと**（run の中で A が太るのではない）
\concat{\italic"A" \italic"A"} = 90  ⇒ **原子を分けると消える＝shaping の項**
cpsp なら "VV" も動くはずだが VV はちょうど 90 ⇒ capital spacing でもない
```
⇒ **次の一手は「Pango が HarfBuzz に何を渡しているか」を読むこと**（script/language/feature 一式）。
⚠️ **A·A に 4px を足して閉じない**——1 フォント 1 ペアの当てはめになる。
★★★ **同じ commit で `text.width.va` を開いた＝LP 側が恒等の対**（LP は "AV" も "VA" も **85px**）。
**フォントのペア値は非対称**（A·V −90／V·A −95）なので、**カーンをグリフと一緒に丸める規則では両立しない**
（Lily# は 84）。⇒ **「カーンをどこで丸めるか」は未解決**で、**その言明は誰も気づかなくても点が言う**。

**未 push 124**（この引継ぎ commit まで）・テスト **3895 passed / 0 failed / 4 skipped**（**+10**）・
...
★★★ **`text.width.aa` の +4px＝「Pango が HarfBuzz に何を渡しているか」**（§1・**残る最大の未解明**）。
**先に読むのは LP の設定**（`pango-font.cc` の `PangoAttrList`／script・language・feature）。
⚠️ **Lily# 側は `GuessSegmentProperties` ＋ 既定 feature で shape している**。ここが違う可能性が本命。
★★★ **カーンをどこで丸めるか**（`text.width.va` が −1px で開いている・**LP 側は AV≡VA=85 の恒等**）。
**候補は 2 つ**: ⑴ カーンを単独で 1px へ丸める ⑵ サブピクセルで累積して最後に 1 回丸める。
⚠️ **どちらも 85/85 を再現しうる**ので、**半ピクセルを跨ぐペアの本を先に作ってから**決めること。
**片方だけ満たす規則は `av` を壊す**（2 点はそのために並べてある）。
