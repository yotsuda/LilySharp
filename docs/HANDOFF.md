     ...
     **だからこの便では着手しなかった**）。**5 つに割れる。まとめて 1 便に入れないこと**:
     ...
     ⒟ **式の文法そのもの（1 件）**: `@name(引数)` run（`Music:875`）は token を無差別に
        ...
        **run 1 つ／点 2 部**＝切れ目が一致しない・監査 §9.5.1）。
        ⇒ ★ **残るのは `@fig` / `@chord` の 2 部分言語＋`@mark("A")` のラベル
        ＋点つき名前の家族**（`ParseMarkName`）。**着手順は監査 §9.5.3。**
        ⚠️⚠️ **`@fig` と `@chord` はユーザー決定が要る**（受理される綴りが動きうる）。
        ★ ただし **`@chord` の「点で連結して split+join で戻す」2 ホップは、
        綴りを 1 文字も変えずに消せる可能性がある**——`Arguments[0].Text` が既に
        `"c:m7"` だから。**先に測ること**（監査 §9.5.3 ⑴）。
