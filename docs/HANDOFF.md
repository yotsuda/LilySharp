...
最終更新 第326セッション＝**入り方は第298〜第324 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い、便の途中の口挟みは 0）。⚠️ **第325 のブロックは無い**——第325（2026-09-03・perf `438df048`／(A) `259922de`／F ⒵ の格下げ）は §1 を書き換えず**第324 のブロックの「次の一手」段落に書き足した**。本便はその形のまま第324 ブロックを下へ落とした（第325 の記録は下の第324 ブロックの ✅✅ ⑴・⑹ と §2 F ⒴⒵ に在る）。⇒ ★★★★ **本便は §1 の次の一手 ⑸＝§2 F ⒴ (B)＝実音入力／移調パート出力を実装した**（ユーザー明示「最終的には (B) まで実装したい。安全・堅実に進めて」・製品コード 1 commit）。**骨は 7 つ**:

⚠️⚠️⚠️ ★★★★ **⑴ 綴り＝便の後半でユーザーと対話して決めた（§3 に行を足した）**: **`pitch concert` | `pitch written` を `transpose` と同じ 3 つの家に**——**top-level**（ファイル既定・`octave absolute|relative` と同じ GlobalSetting・**既定は `written`＝ディスクの本の意味は 1 冊も変わらない**。corpus: 追跡 594 冊に半音移調 `instrument` 0 冊・実コーパス `scratch\ベースタブLy` 0 冊・`pitch` をコメント外で使う本 0 冊）／**part header**（その part の上書き＝own > file。**ユーザーの要件「サックスは移調済みパート譜から写し、ピアノは実音譜から写す、を 1 冊で」**。第 2 便で足した＝commit 3 本目）／**score header `score F pitch concert { … }`**（`transpose d` と同じ棚・順不同＝その score を実音で刷る＝指揮者譜）。**keyword `pitch` を予約**（Lexer・SyntaxKind.PitchKeyword・3 文書の予約語表・tmLanguage・LSP 補完）。★ **ユーザーの読み（実測どおり）**: 「`instrument` を指定しない場合 `pitch` は noop」＝T=0 の part ではどこに書いても何も動かない（番人の行に）。**「楽器名は付けたまま書いた音高で鳴らす」切替は今は不要**（楽器名を書かなければよい）＝ユーザー判断。**代案の検討は済み**（`instrument alto-sax concert` 修飾語案・part だけの案を提示 → 案 1＝3 つの家、を採択。理由＝`transpose`/`octave` と同じ優先順位で新しい規則を足さない）。
⚠️⚠️ ★★★ **⑵ 機構＝shift は `transpose` の水路だけを通る**（第2の綴り 0）: **`ConcertPitch`**＝(file, score) の 4 セルを 1 つの c→target 区間に畳む（**file concert → −T を `PartTranspose.Read(root, name)` の中で合成**＝page/MIDI/MusicXML/LP 双子の 4 consumer が同じ読者・**score concert → +T を `RenderSpec.ScoreConcert` → `MeasureCollector.ScoreConcert` → `GetPartDefaults` で合成**（LP exporter は `EffectiveTranspose` で同じ形）・両方なら identity → `PitchTransposer.NullIfIdentity` で null＝双子に `\transpose c c` を書かない）。**⒴ の 3 点はそのまま当たった**: 調号は `transpose` の機械で動く（新しい調号コード 0）／符号は (A) の逆＝`PitchTransposer.IntervalFromSemitones(−T)`（綴りは flat 側＝c→a／c→ees, で調号 ±3）／**相殺は 1 度**＝MIDI は printed ＋ `SoundingShift` のまま（番人 `ConcertPitchTests.AConcertPitchFile_SoundsTheLetterThatWasWritten`＝alto-sax 72・対照 flute／無 instrument 72・written 行 63）。
★★★ **⑶ 決定（仮）＝オクターブ移調楽器は `pitch concert` でも記譜不動**（`PartHeaderDefaults.ConcertShiftSemitones`＝`T % 12 == 0 → 0`。bass/piccolo/`transposition 8vb`/bass tuning）。**理由＝印刷された C スコアの慣習**（piccolo・contrabass・guitar はオクターブのまま）。**ユーザーが「bass も実音で」を望むならこの 1 行**。MIDI 側の帰結: concert 本の `instrument bass` は従来どおり printed −12。
⚠️⚠️ ★★★ **⑷ 副産物＝pre-existing 欠陥を同じ行で閉じた**: **part-major 本（`part x { transpose d section A { … } }`）の `transpose` は MIDI に届いていなかった**（`MidiExporter.PlayInPart` が sounding shift だけ armed・section-major の `x { }` 腕は合成していた）→ 両腕が `PartPlaybackShift(partName)` 1 つを訊く。**陽性対照＝旧 MidiExporter を stash して番人 `APartMajorBooksTranspose_ReachesTheMidiAsTheSectionMajorOneDoes` が赤（1/1）を確認してから戻した**。CHANGELOG 0.6.0 Language に 1 項。
★★ **⑸ MusicXML／LP 双子**: `<transpose>` は file 方式に依らず alto-sax −9 不動（⒴ の「測り直せ」）・`<pitch>` は written（concert 本は A5・fifths 3）／双子は `x = \transpose c a \fixed c'`（score concert のみなら `\transpose c ees,`・両方なら wrapper 無し）。✅ **双子の目視は第 3 便で済んだ**（`scratch/p326/fx/sax.lys`＝concert 本・alto-sax／clarinet／flute（対照）・`c' d' e' f' | g'2 bes'2 | c''1`・parts score と `pitch concert` の full score を `sax-parts.lys`／`sax-full.lys` に割って `lysc ly`＋`lysc png --crop` と LP 2.26.0 cairo png（`*-lp.png`）: **parts＝Lily# と LP が同じ絵**——alto-sax は A 調 3♯で a b cis d／e g♮（実音 b♭）／a''、clarinet は D 調 2♯で d e fis g／a c♮、flute は C 調で b♭のまま。**full＝3 段とも C 調で同じ音**。双子の text は `sax = \transpose c a \fixed c'`／`cl = \transpose c d \fixed c'`／full 側は wrapper 無し）。
★ **⑹ 番人 56 本＝`ConcertPitchTests`**（page 4 セル＋preset 13 行＋transpose 合成 3 行／MIDI 7 行＋score 側は届かない＋part-major／MusicXML 2 行／双子 4 行／綴り＝両 mode・第3語拒否（round trip 保持）・欠落・part header 拒否・score option 順不同／`IntervalFromSemitones` 8 行／増分＝file 切替 2 行＋score option 追加＝full と byte 一致）。**既存テストは 1 本も触っていない**（`InstrumentTranspositionMidiTests` は doc の参照だけ）。
★ **⑺ 罠**: XML doc（`/// <param>`）を `//` コメント行の*後ろ*に置くと CS1587＝Core の 0 警告が赤（`MeasureCollector.Definitions.cs`）／`RenderDeclarationSyntax.Transpose` は「最初の PropertyAssignment」を返していた＝2 つ目の option を足すと取り違える → 名前で引く（`HeaderOption`）。
**文書**: `GRAMMAR.md`（Keyword・§2.3 `PitchDecl`/`TransposeDecl`（後者は未記載だった）・`ScoreOption`・transposition 注）・`GRAMMAR_FOR_LLM.md`（Pitches 節・transpose の gotcha・予約語）・`SYNTAX_REFERENCE.md` 表・`README.md` 1 行・`CHANGELOG.md` 0.6.0 Language 2 項・`editors/vscode/syntaxes/lilysharp.tmLanguage.json`・LSP 補完（`pitch` ＋ GlobalSingletonKeywords）・**`scratch/site-showcase/manual-body.html`（管理外）の Transposing instruments 節・template・予約語**。

★ **開始時裏取り**: HEAD `e67671f2`・**未 push 24**（`origin/master` は `407b4f77`＝第323 の開始時と同じ。⚠️ 第324 の §1 は「`dcf111bd`・未 push 8→10」と書いていたが、本便の `git rev-list --count origin/master..master` は 24＝**第324 の数え方か origin の読みが違う。両端をこの便から取り直す**）・木 clean（README.md/samples の手直しは commit 済み）・未追跡 0・Core 0 エラー 0 警告・追跡 `.lys` 594・snapshot 244・台帳 759 点（exact 593 / `OPEN:` 0）。
終了時: **commit 4 本**（`3423a277`＝(B) 一式・製品 21 ファイル＋文書＋番人／`f349d53c`＝HANDOFF＋ARCHIVE／第 2 便＝part header の `pitch`（製品＋文書＋番人）／その HANDOFF）・**未 push 28**・作業ツリー clean・未追跡 0（`scratch/p326` は git 管理外＝`run1.trx`・`run2.trx`・`fail.txt`）・Core 0 エラー 0 警告・**Windows Debug 7037 / 0 / 4 / 7041**（**`scratch/p326/run2.trx`**。第 1 便の `run1.trx` は 7021 / 1 / 4 / 7026＝赤 1 は `ApproximationInventoryTests.TheInventoryIsNotStale`＝`InstrumentDefaults.cs` の const 撤去で `LilyPondExporter.cs` 側の行番号が動いた → `LILYSHARP_UPDATE_DOCS=1` で再生成）。**第324 の 6919 比 +122＝本便 71（第 1 便 56 ＋ 第 2 便 15） ＋ 第325 の 51**（第325 の数はどこにも書かれていない＝逆算）・**追跡 `.lys` 594 不動**（fixture は足していない）・**snapshot 244 不動（0 枚動いた＝既定 written は絵を変えない）**・台帳 759 点不動（exact 593 / `OPEN:` 0）・`docs/APPROXIMATIONS.md` は行番号だけ再生成・**LSP/拡張は未配備**。★ **第 2 便の罠**: `GRAMMAR.md` の production のコメントに `;` を書くと `DocKeywordListTests` の `Body()` がそこで production を切る（`(* … *)` を剥ぐ前に `;` で切っている）＝2 度踏んだ／part header の値検査は SEMANTIC pass（`SemanticValidation.Run`）＝`tree.Diagnostics` には出ない。

⇒ ★★★★ **次の一手**: ⚠️ ★★ **⑴ ユーザー確認が残るのは 1 点＝⑶「オクターブ移調楽器は `pitch concert` でも記譜不動」**（綴り 3 つの家は本便でユーザー決定済み＝§3）。**否なら `ConcertShiftSemitones` 1 行**。✅ **⑵ 双子の目視は第 3 便で済んだ**（⑸）／★★★ **⑶ T6＝スラー復元 138 冊**（**ユーザー指示「この便で有利なら着手、次便が有利なら着手するな」→ 新しい流れなので次便へ**）／★★ **⑷ T7 の C 37 冊を 1 冊ずつ**（指標は KindMatched の対で）／**⑸ 以下は第324 の並び**: §2 T6 の残り／`audit/lpreg` 取り直し（第324 で `\mark` 本の絵が動く＝取り直し前に `REGENERATE.md`）／§2 U8c の島／§2 U8b／§2 U8 の残り／§2 A/B/D/E／§2 C⑴／§2 G／★ 低優先: `marks stacked` / `marks beside`／**「なぜ `perf-v2bow1k` だけ落ちるか」（第324 ⑹）**。⚠️ **承認待ち**: §2 T2 の 1 件（`Lambada Complicada.lys`）＋§2 T5 の ⏸ 1 件＋第323 ⑶ の「戻すか」（第324 推奨＝戻さない・値段は第324 ⑴＝`38cb4dad` は `perf-v2bow1k` +74%・単声 +8%）＋**本便 ⑴**。⚠️ **§2 F の開いている項目は第320 の ⒲ 1 件**（⒴ は本便で閉じた・⒵ は第325 が格下げ）。⚠️ **リリース 0.6.0**: CHANGELOG に本便 2 項を足した・版番号 7 か所の bump と tag は未着手（週末判断）。⚠️⚠️ **WSL ubuntu Release は取れない**（第313 ⑶）——**本便は Core を触っているので `push` 後に `gh run list` で読むこと。**
...
- ✅ **⒴ は第326 で入った**（§1 第326 ⑴〜⑶＝`pitch concert|written`・score header の `pitch concert`・`Semantics/ConcertPitch.cs`・番人 `ConcertPitchTests` 56 本。**綴りと「オクターブ移調楽器は記譜不動」はユーザー未承認の仮決定**＝§1 次の一手 ⑴）。**下の 3 点の実測はそのまま当たった**——読み返すのは綴りを差し替えるときだけ。
...
| 決定 | 根拠（要点） |
|---|---|
| ★★★ **実音入力の綴りは `pitch concert` / `pitch written` を `transpose` と同じ 3 つの家に置く**（2026-09-03・第326・**ユーザー決定**・✅ 同便実装＝`3423a277`＋part header の commit） | top-level＝ファイル既定（既定 `written`）／part header＝その part の上書き（own > file。「サックスは移調済みパート譜から、ピアノは実音譜から写す」を 1 冊で書くため）／score header＝その score を実音で刷る（指揮者譜）。**両方に書いたら part が勝つ**＝`transpose`・`octave`・`key`・`time` と同じ規則なので混乱しない、が採択理由。代案（`instrument alto-sax concert` の修飾語・part だけで既定無し）は提示のうえ退けた。**`instrument` を指定しない part では `pitch` は noop**（T=0）。**「楽器名は付けたまま書いた音高で鳴らす」切替は今は実装しない**（楽器名を書かなければよい）。⚠️ **オクターブ移調楽器（bass/piccolo/`transposition 8vb`）が concert でも記譜不動なのは実装側の仮決定**＝`PartHeaderDefaults.ConcertShiftSemitones`・未確認 |
