...
最終更新 第396セッション（2026-09-16）＝§0 の裏取りから入り（HEAD `d1073407`・full 8609 / 0 / 4 / 8613＝第395 の数と一致・CI 緑）、第395 が「掃き不要」と残した **§2 R の R6（resume の門 3 本）＋ R6′（非既定 accidental style で resume 全滅）を閉じた**。道具は pwsh MCP。**出力不変**（変わるのは増分セッションの再利用だけ＝掃きなし・snapshot 0 枚・台帳不動）。**骨は 5**:

★★★ **⑴ 4 件とも「再現するテスト → 赤 → 直す → 緑」**（`CollectEditResumeTests` の session 396 節・`ResumeAcross` 1 軒＝記録→計画→再開→full と `ModelDeepDiff`）: ⒜ **`~Name` 腕が `!_suffixSpliced` を見ない**（`MeasureCollector.ProcessForm`）＝**モデルには出ない**（末尾を越えた start には何も置かれない）が、**2 part の本では 2 本目の walk 入口の journal 検査（`StartSectionStartLogCount`）が bail する**＝編集の後ろに `~Name` がある 2 part の本は毎打鍵 full collect だった。⚠️ 1 part で書いた最初のテストは緑のまま通った＝症状は「読み手」で変わる／⒝ **`BuilderCheckpoint` に `AtScopeStart`**（Capture/Restore）＝`c1 | |: d1 :| e1 |` が full 4 小節・増分 3 小節（fresh builder の `_atScopeStart=true` を restore が触らず `|:` が scope start と読む）／⒞ **`SuffixStateMatches` の builder 比較を record 等価に**（`LastMeasure`・2 位置を伏せた `with`）＝名指し 11/15 → 全項目。`time none` の前を `c4`→`c8` で編集すると `Measure.UnmeteredPosition` が古い読みのまま splice されていた／⒟ **accidental memory を `WalkCheckpoint.Accidentals` として capture/restore/compare**（`AccidentalMemorySnapshot`＝(step, octave, alter, barsBack) を刻印順に。**絶対 bar と order 計数は持たない**＝規則は差と順序しか読まないので、0 から数え直す resume walk でも音符 1 つ挿入した tail でも一致する）。`WalkCarriesNothing` から `_measureAccidentals.Count == 0` を外す。**`AccidentalStyleSpec.MemoryHorizon`**（最も怠惰な規則の届く bar 数）で `AdvanceAccidentalBar` が読めない entry を落とす＝observationally 同一（`modern` は 1 小節で編集の痕跡が消えて splice が再収束・`noReset` は消えない＝tail が触った entry を書き直すまで断る）。

★★★ **⑵ 計器 `VoiceResumePlan.LastSpliceDecline`（恒久）＋ opt-in `SpliceDeclines`（`CollectWalkProbe.RecordSpliceDeclines`）**: splice の decline は設計上無言＝「全部断る門」が見えない。`TrySpliceSuffix`／`SuffixStateMatches` の `return false` 全部を `DeclineSplice("…")` 1 軒に通した（理由は literal＝打鍵ごと境界ごとに出るので整形しない）。**⒟ のテストが「splice 0」で赤のまま**になり、捨て計器（`scratch/p396/Zz396ProbeTests.cs.txt`＝old/new 記録の checkpoint 対照＋decline 一覧）で ⑶ を見つけた。

★★★★ **⑶ 見つけた別の穴＝suffix parse agreement の過剰 decline**（`CollectResumePlanner.GreenSuffixAgrees`）: **pitch 文字ごとに token kind が違う**（`PitchA`≠`PitchF`）ので、**空白が続く文字置換**（`g4 a b d'` の `a`→`f`）は token の trailing trivia が suffix 境界をまたぎ、kind 比較で false＝**その walk の splice が全部断られていた**。網（`SyntheticEdits`）の文字置換は「文字＋数字」だけ選ぶ＝token が境界ちょうどで終わり比べられない＝15 冊床が気づかない。直し＝**text が limit より前に収まる token は両側とも window の業務**（trivia は構造ではない）。テスト `LetterSwapBeforeASpace_StillSplicesTheSuffix`。出力不変・プレビューの再利用が増える方向。

★★ **⑷ 汎化**: 「断る門は無言でよい（正しさに関わらない）」は「断る門が全部断っても誰も気づかない」の裏返し＝網の床（15 冊）は*何かが*splice することしか言わない。**decline の理由を 1 本残すだけで、編集の最頻形（空白付き文字置換）が 1 度も splice していなかったことが同じ日に見えた**。R6′ の「memory が空でないと checkpoint を取らない」も同じ族（gate が満たせない本は静かに full collect）。

★ **⑸ 終了時**: commit 3 本（code `cc61e0c3`・§1 `bebc67fd`・第 2 便＝網の編集クラス＋この §1）・HEAD＝第 2 便の commit・未 push 20・**full `scratch/p396/run2.trx` 8614 / 1 / 4 / 8619＝赤 1 は `ApproximationInventoryTests.TheInventoryIsNotStale`（行ずれ＋私の remark の「unobserved」が UNWATCHED の正規表現に当たった＝言い換えて再生成）→ 最終 full `scratch/p396/run3.trx` 8615 / 0 / 4 / 8619**（+6＝新テスト 4 本＋theory 2 例）・§7.5（対 `d1073407`・スクリプトの出力）**Core `+` 256 行／REF 0／OWN 0**（LP の計算は移植していない＝増分の門と計器だけ・新しい数値定数なし）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 608・§7 3.5 は済（第394 を `-Archive 394`＝17 行 2,970 chars）。⇒ **次の一手＝本便は着手しない**: §2 R の残りで掃き不要なのは R13⒜（診断の collect 共有＝LSP の設計）だけ、R3（tuplet 腕）は掃きと承認が要る＝次便が §0 から入るほうが有利。**第 2 便（同日・ユーザー指示「このセッションが有利なら着手」）＝`SyntheticEdits` に空白付き文字置換（` x ` の最後の 1 つ）を足した**（`LilySharp.Tests/CollectEditResumeTests.cs`・commit は §1 の下）。数（捨て計器 `scratch/p396/Zz396ProbeTests-leg2.cs.txt`＝この編集クラス単独で net 258 冊を回す）: **適用 249／resume 160／prefix 採用 125／splice 23／bail 0／planless 89／full との不一致 0**。planless 89 は「最後の ` x `」が `key c major` 等の top-level に落ちる本（`WindowRespectsTopLevel` が断る＝正しい）。網 38 本は新クラス込みで緑・**最終 full `scratch/p396/run4.trx` 8615 / 0 / 4 / 8619**（テスト本数は不変＝yield が 1 つ増えただけ）。⚠️ 「直す前は 0 冊 splice」の対照は取っていない（Core を戻す 15 分の再ビルド×2）＝陽性対照は `AccidentalMemory…` テストが直す前に `the parse agreements do not hold` で断られた事実（本便 ⑵）。残る道具は resume の門の regime にだけ効く＝続けるなら「`noReset` の再収束条件」。**第 3 便は着手しない（同日・同じ指示）**: `noReset` は `Same(Forever)` 1 規則なので比較は (step, octave)→alter だけで足りるように見えるが、「alter が今の key と同じ entry は落とせる」は tail の途中で key が変わると読みが変わる（C major で落とした c♮ の entry が D major の `c` で natural を出す／出さないに効く）＝健全性の設計問題で、style ごとの canonical form を決める仕事。観測 0（`noReset` の本はコーパスに無い・prefix 採用は既に効く）で、本便の道具は捨て計器 1 本しか使わない＝次のセッションでやっても損が無い。
...
- **R1 exporter の `alternative` 節**（`repeat TYPE N { } alternative { }`・`SyntaxNodes.Expressions.cs:42`）**は MIDI だけが鳴らし、page（`ProcessRepeatExpression`）・twin（`EmitRepeat` は `\alternative` を書かない）・XML は黙って落とす**。言語判断（validator で断るか、3 読み手に通すか）。同根＝MIDI の余分な pass は*最後*の ending を選ぶ（`MidiExporter.cs:2281`）が LP の `\volta` 無しは*最初*を繰り返す（repeats.itely:277-280）。
- **R2 MusicXML が section header の `partial` を落とし、header だけの section で空の `<part>` を出す**（schema 違反・importer が round-trip できない）＝`EmitSection :381-390` は key/time だけ登録、`EmitPartMajorSection :769-775` が `"Part 1"` に空 part を作る。MIDI は `_sectionHeaderPartials`・twin は名前鍵の registry を持つ＝3 綴り目を同じ形に。
