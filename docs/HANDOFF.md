...
最終更新 第283セッション＝**前便が残した gate の lead を測り直したら、その内訳が引き継ぎと違い、そこで見つけた*別の*欠陥を直した（1 便）**——commit は `0802a6d0`（**1 パートを 2 譜に彫る本で、tie・slur・beam の警告が譜ごとに 1 本ずつ出ていた**）。**snapshot 0 枚・台帳不動・origin 不動。**
...
⚠️⚠️ ★★★ **その網は最初 3 本で、1 本足りなかった**（同便で気づいて足した・`8884080b`）。**scanner は 3 本だが sink は 4 本**——**cue 境界の list は tie 走査と slur 走査の*両方*が埋める**ので、**「scanner を数える」と 1 つ落ちる**。**落ちていたのは `SpanCrossesCueBoundary`＝この一族で唯一の *error***（warning ではない）。⇒ ★★ **数えるのは「呼ぶ側」ではなく「書かれる側」**。**同じ便で「観測者が居たのは tie だけだから 3 本とも縛る」と書きながら、その 3 が既に間違った母集団だった**（§0 ★「N 件はどの集合に訊いたかまで書く」のコード側の再来）。
...
⚠️ **要ユーザー判断で開いているものは前便どおり**（標·volta の 4 定数の削除・silhouette chordNames scalar・和音語彙 CHL 3 点・Nimbus Sans 同梱・CHANGELOG 版番号・SectionLabel の X・F【8】3・混在綴りの同名 section・標の箱そのもの・`@text` の縦・`bandUp` の死んだ配線の撤去）。★ **本便は 1 つも増やしていない**——**直した欠陥は「同じ警告を 2 度出す」で、判断の余地が無かった**（読み手は 1 つしか間違えていない）。⚠️ **ただし据え置いた `check` の重複 ⒝ は、直すなら住所が変わる＝そのときは要ユーザー判断**（下の先頭項）。
...
- **✅ LP の第 3 分岐（拍スラッシュ）は第282 で移植した**（`31b89488`）——**ただし 1 小節未満の body だけ**。**整数小節の body は Lily# の小節ごと `%` のまま**で、これは*宣言された乖離*（`docs/APPROXIMATIONS.md` の APPROX・`PercentRepeatItem` の remark）。**理由は §1 ⑶**（LP は裸のスラッシュ 1 本と空小節で、両 slash grob の description が「1 小節未満のための記号」と自称している）。⚠️ **残っているのは記譜の問い 1 つ**＝上の「次の一手 ⒜」。**`audit/lpreg/slashprobe.lys` が今も審判**で、今は LP と同じ絵を出す。
...
最終更新 第282セッション＝**引き継いだ「全緑」が実は赤で、それを直してから LP 第 3 分岐を*測ってから*半分だけ移植し、その最初の試作が自分の小節を白紙にしたことで 4 便前から在った別の欠陥が出た（1 便）**——commit は `d8b82627`（**ハンドオフの最後の 1 本が suite を赤にしていた**）／`31b89488`（**拍スラッシュと、「percent がどの小節を隠すか」を訊いていなかった 4 人目の読み手**）。**snapshot 0 枚・台帳不動・origin 不動**。

**⑴ 引き継いだ「6437 全緑」は赤だった**（`d8b82627`）。木のままの実測は **6436 / 1 失敗**——`HistoryCitationTests.DeadCitationsDoNotGrow` が **470（天井 469）**。⚠️ **超えた 1 個は第281 自身のハンドオフ commit `c56ca7e5` が入れた**。**suite を回したあとに最後の 1 本を積んだので、その 1 本は誰にも読まれていない。** ⇒ ★★ **セッション最後の commit は suite の外に居る。** **中身は「amend が孤児にした commit の SHA を §1 に書いた」**——`git rev-parse` は答えるが branch から到達不能＝**dead citation の定義そのもの**。★ **同じ形をこの天井が捕まえるのは 2 度目**で、**テスト自身の remark にそう書いてある**（「その §1 は amend が孤児にした 2 つを『どこにも引用されていない』と、両方を引用する文の中で書いた」）。⇒ **直すのは天井ではなく散文**（remark の指示どおり）。**孤児の名前は書いた瞬間に dead citation。** 必要なのは SHA ではなく **amend の回数**で、それは reflog が持っている。

**⑵ 起票の想定と実際の観測者が食い違っていた**（着手前に測った）。**コレクタに毒を入れて 899 冊の `repeat percent` の body をすべて分類**した: **1 小節 = 73 冊 / 2 小節 = 39 冊 / 3 小節以上の*整数*小節 = 32 冊**（ちょうど 4 小節が 170 site・8 小節が 22）**／1 小節未満 = ちょうど 1 冊**——**`audit/lpreg/slashprobe.lys`、第278 がこの起票のために作ったプローブそのもの**。⇒ ★★★ **起票が書かれた分岐には観測者が 1 人も居らず、観測者 32 冊を持つのは同じ `else` の*別の形*だった。**

**⑶ その別の形に LP が何を描くかを測って、移植を半分で止めた**。`\repeat percent 2 { c'1 d'1 e'1 f'1 }` を 2.26.0 に通すと、**書かれた 4 小節・裸のスラッシュ 1 本・そして完全に空の小節 3 つ**（`scratch/p282/wholebody.ly`）。**スラッシュの grob は小節ぶんの広がりを持たないので、繰り返しの残りには何も置かれない。** ⚠️ **LP 自身がその範囲を宣言している**——**両方の slash grob の `description` が「repeating patterns *shorter than a single measure*」**（`scm/define-grobs.scm`）。⇒ **1 小節未満だけを移植し、整数小節の body は Lily# の小節ごと `%` を保つ**。**「未移植」ではなく*宣言された乖離*に変えた**（`docs/APPROXIMATIONS.md` の APPROX 56 → 55）。

**⑷ 移植は「隠す」ではなく「置き換える」**（第278 の見立てどおり）。**LP の iterator は body の代わりに `RepeatSlashEvent` を渡す**ので、繰り返しは **grob 1 個と音符 0 個**しか出さない。⇒ **collector が 2 回目以降の pass を body の長さの spacer 1 個に差し替える**。**隠すと間隔が残る**（`{ c16 d e f }` なら 16 分 4 つぶんの空きにスラッシュが 2 本浮く）。⚠️ **再生は無傷**——`MidiExporter` は**構文木**を歩く（`ProcessRepeat`）ので、MusicXML と `.ly` の exporter ともども item 列を読まない。**変わるのは彫られたページだけ。** 本数は `calc-repeat-slash-count` のまま（**全部同じ音価なら `max(duration-log − 2, 1)`・違えば 0**）で、**0 は「0 本」ではなく*別の grob***（`DoubleRepeatSlash`＝点つき・slope 1.0・kern 1.6／`RepeatSlash`＝点なし・1.7・0.85）。**数字は全部 LP 自身の SVG から読んだ**（`scratch/p282/slashprobe-lpsvg.svg`）。★ **`beat_slash` だけは `align_to (X_AXIS, CENTER)` を*しない***ので、**群の左端が拍の列に乗る**（`double_percent` は中央）。

**⑸ 試作が自分の小節を白紙にして、4 便前からの欠陥が出た**。`PercentRepeatItem.FirstCoveredMeasure` は「percent がどの小節を隠すか」の**一軒**で、remark は「**3 つの pass が訊く**」と書いていた。**4 人目が訊いていなかった**——`DrawBeams` が `MeasureIndex` だけから自前の集合を作っていた。⒜ **拍スラッシュではその小節を丸ごと消した**（スラッシュが繰り返している*書かれた拍*ごと）。⒝ **そして二重 percent では、ずっと前から壊れていた**——**二重の錨は対の*2 小節目***なので、**1 小節目は符頭だけ隠されて梁と符幹が残っていた**。**ユーザーの `1stbarline.lys` の 4・6・8 小節目に裸の梁が印刷されている**（絵で確認）。⇒ **その site も同じ範囲を歩くようにした。** ★ **remark の「3 つ」は、4 人目が訊いていなかったあいだ*ずっと正しかった*** ——一軒に寄せた札を貼っても、読み手が増えたことは誰も数え直さない。

**⑹ 掃きを並列化した**（第275 以来の申し送り・**独立した便にはしない**の指示どおり、掃きを回すこの便に括り付けた）。`scratch/p282/sweep.ps1`＝**逐次 18 分 → `-ThrottleLimit 8` で 148 秒**。**第279 が書き残した 3 点をそのまま実装**（冊ごとの一時ファイル名・`$using:`・集計は最後）、**罠 2 つも据え置き**（パスをクォート／「ファイル無し」と「0 バイト」を分ける）。

**⑺ そのユーザー決定が同じ便で返ってきて、⒥ を実装した**（`b6b76ce7`・LYS2014）。**下の「次の一手」で ⒤⒥⒦ を並べたところ、ユーザーが ⒥（診断を出す・ページは動かさない）を選んだ**。⇒ **警告が安全なのは「コレクタが小節ごとに署名するのと*ちょうど同じ*場所で鳴る」ときだけ**——別の署名をされる body に鳴る警告は、鳴らないより悪い。**規則を `PercentRepeatShape` の一軒に出し**、コレクタもそこを通すようにした。⚠️ **長さの*測り方*は 2 つのまま**（コレクタは `MeasureBuilder` から読み、検証器は自分で body を歩く——その層に builder が居ない）。**共有したのは規則**で、**2 つの測定は構造ではなく*掃き*で突き合わせる**。

⚠️⚠️ ★★ **その掃きが 2 回効いた**（どちらも「もっともらしい実装」を数が否定した）。**⒜ 初版は 30 冊中 6 冊にしか鳴らなかった**——flow accounting の gate を借りたせいで、**値段の付かない項目を 1 つでも含む body を拒む**。**コーパスの percent body 472 件のうちgate を落ちる 114 件は、全部が `break` か tie**——**どちらも時間を占めないので長さを変えない**。⇒ **その 1 呼び出しにだけ opt-in で緩め、flow 側は厳しいまま据え置いた**（緩めると 114 件がopaque 枝から flow 枝へ移り、既存の小節診断が動く＝別の変更）。**⒝ 次は 28 冊**——**この層の lead-in は*書かれた*小節をまたぐ running sum** なので、**小節線を省いた本（`r1 r1 r1 r1 break repeat percent 2 { … }`）では拍子より長い値で届く**。コレクタ側の対応物は auto-complete 済みで**常に開いている小節の中の位置**。**小節に畳み込んだら、残り 2 冊がちょうどその書き方の 2 冊だった。**

★ **開始時裏取り**: HEAD `c56ca7e5`・**未 push 65**・木 0・未追跡 0・Core 0 エラー 0 警告（`--no-incremental`）・**Windows Debug 6436 合格 / 1 失敗 / 4 skip / 合計 6441**——⚠️ **引き継ぎの「6437 全緑」は当たらなかった**（⑴。**赤にしたのは引き継ぎ commit 自身**）。台帳・コーパス・snapshot は引き継ぎどおり。
終了時: **commit 5 本**（`d8b82627` ハンドオフ修復・`31b89488` 拍スラッシュ＋梁・`3ea1b95e`／`29cdd779` ハンドオフ・`b6b76ce7` LYS2014）・**origin 不動（未 push 70・⚠️ ユーザー指示「プッシュはしないで」は生きている）**・木 0・未追跡 0。**suite 6469 合格 / 0 失敗 / 4 skip / 合計 6473**（開始の合格 6436 から **+33**＝⑴ の復旧 1 ＋ 新しい網 32）。Core 0 エラー 0 警告。**台帳全点不動・snapshot 不動・新規 0**。**SVG 全数掃きは 3 回**（899 冊）: **拍スラッシュ＋梁の便が SAME 883 / MOVED 16**、**LYS2014 の便が SAME 899 / MOVED 0**（＝**⒥ の「ページは動かさない」を実測で示した**）、**レンダー失敗 0 / BOTH-NONE 0**。**`lysc check` の全数掃きは 4 回**: **LYS2014 は 30 冊 94 site に鳴り、コレクタの census と 1 冊も違わない**（取りこぼし 0・偽陽性 0）。**終了コードの変化 0**、**他の診断の変化 0**（動いたのは「`No errors found.` が消えた」7 冊だけ＝全部もともと無警告）。**7.5 の数え上げ**: Core の `+` 行 260 に対し **`LILYPOND-REF` 1 本・`LILYSHARP-OWN` 3 本**（形は LP のもので測る場所に引用、**報告するという決定は Lily# のもの**＝APPROXIMATIONS の OWN 114 → 117）。
★★ **緑は*最後の commit のあと*に取った**（⑴ の再発防止）——**⑺ より前の区切りでは Windows Debug と WSL ubuntu Release の両方で 6450/0/4/6454**（`29cdd779` の木）、**終局は Windows の Debug と Release の両方で 6469/0/4/6473**（SAC が切れたので Release が使えるようになった・下記）。⚠️ **WSL 側は `git checkout <sha>` では測れない**（**detached HEAD で一度赤を見た**）: **`HistoryCitationTests` は到達性を *`master` から* 数える**ので、**detach すると自分の新しい commit が「master から到達不能・`refs/remotes/win/master` だけが生かしている」**ように見え、**`CitedCommitsAreNotHeldAliveByAnotherRef` と `DeadCitationsDoNotGrow` が 2 本とも赤くなる**（470 → 472）。⇒ **`git checkout master && git reset --hard <sha>` で*ブランチごと*動かすこと。** ★ **これは第236 の「書き換えの取りこぼしは commit ではなく *ref* の集合で数える」の裏面**——同じ計器が、同じ理由で、今度は*正しい木*を赤と言う。

★★★ **本便の途中でユーザーが Smart App Control をオフにした**（2026-08-29）。**直前に `lysc.dll` が塞がれて `CliParserTests` など 21 本が赤**だったのが、**切った直後に Windows の Debug も Release も 6469 全緑**。⇒ **第250 以来の回避策一式（WSL へ逃がす・`-p:Deterministic=false`・obj/bin 消し）は、この機械ではもう常用しなくてよい**（memory を書き換えた）。⚠️ **失敗の*読み方*だけは残る**——**`dotnet test` が合否行を 1 行も出さずに終了コード 0 で返ったら、それは緑ではない**。SAC と無関係に正しい規則。
...
> ## ✅ **⒧ 3 小節以上の整数小節 `repeat percent` の `%`＝【ユーザー決定 2026-08-29: 診断を出す】**（第282 起票・同便で実装・`b6b76ce7`・LYS2014）
