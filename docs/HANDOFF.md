...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第420セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `5874a5af`・**未 push 19**・**full `sessions/p420/run1.trx` 8734 / 0 / 3 / 8737＝第419 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 127 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第419 が予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 418`＝19 行 5,336 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」の 4 つを AskUserQuestion に出し、**ユーザーが ⒵‴「`e4.musicMarks` 1.22% を割る」を選んだ**。★ ベンチは着手前に AskUserQuestion で静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第419 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、**before 側の打鍵 8,451,068 B は第419 最終の 8,451,074 B と一致**（＝母集団も harness も同じという証拠）:

★★★ **⑴ 島は `MusicMarkEngraver.Calculate` 1 本で、打鍵の 1.37%**（継ぎ目込み 1.427%＝**計器は対象を 0.06% しか乱していない**）。4,010 呼び出し・マーク 46,138・group 42,328。段に割ると（`mm.x`＝X の算出／`mm.g.*`＝group ループ／`mm.sysIdx`・`mm.sysBot`・`mm.lyric`＝呼び出しごとに建てる 3 つの表）: **`mm.g.all` 34.5%**（`.split` 11.4／`.below` 10.3〈うち `.pedrow` 9.5〉／`.above` 9.0／`.rest` 3.8）・**`mm.x` 30.8%**・**`mm.sysBot` 14.0%**・**`mm.sysIdx` 10.2%**・`mm.group` 3.2・`mm.build` 2.0・`mm.finish` 1.5。

★★★ **⑵ カウンタが 2 つ、島の正体を言った。** ⒜ **`mm.n.below` は全コーパスで合計 16**（42,328 group 中 **42,312 が below 0 本**）——なのに **`mm.sysBot`（14.0%）は毎回・全システムの全小節**を歩いて表を建て、読み手は 1 か所・`belowMarks.Count > 0` の裏にしか居ない。⒝ **`mm.n.pedalRows` は 42,328 回すべてで 0**——`Where→Select→Distinct→OrderBy` ＋ Dictionary が **1 行も作らずに 9.5%** を払っていた（第419 ⑵「空を返す装置」の 2 例目）。★ さらに **`mm.sysIdx` は `SpannerBreakSubstitution.BuildMeasureToSystemMap`(:88-92) の*手写し*で、呼び手（`LayoutEngine.Annotations`＝製品側の唯一の呼び手）は同じ `systems` から同じ地図を既に建てている**。第407 は「pass に 1 枚」を**数えて**14〜18 枚を畳み、`prebuiltMeasureToSystem` を既定 null で配ったのに、**この家にだけ扉が無かった**（⇒ RULES §7.6 の「『N 箇所を 1 軒にした』は『全部を数えた』ではない」の実例が 13 便後に出た）。

★★ **⑶ 最大の葉は擦り切れではなく*解き直し*で、memo 仮説はその場で反証された。** `mm.x` の 30.8% はほぼ全部 `CalculateXPosition` の label 腕（1 呼び出し **1,493 B**）で、その中身は **`prefixMarkAnchorX` の Invoke ＝52,939,232 B／37,838 呼び出し＝1 回 1,399 B＝島の 22.2%・打鍵の 0.338%**（`BuildPrefixMarkAnchorX` の lambda が**マークごとに** `SpacingRules.MaxClefWidth` と `MultiStaffLayouter.SolveLineStartPrefix` を走らせる）。⚠️ **「答えは `sysIdx` だけで決まる＝memo が効く」と予測して測ったら、外れた**: **distinct（call, sysIdx）は 37,838 中 37,638＝hit 率 0.53%**——**行頭ごとに別の system を訊いている**ので call 寿命の memo は何も返さない。⇒ **この葉を取るには行頭 prefix を配置から持ち回る土台の変更が要る。** ★ 文字のシェイピングも測った: **318,784 回で distinct な綴りは 81**（`advance` 171,299／`ink` 147,485）——ただし **1 回 64 B ＝合計 8.6%** で、memo の賞金は小さい。★ 行頭 system の線形走査は **0 B**（12.9 歩／呼び出し）。

★★★ **⑷ 直し＝「誰も読まないものを建てない」3 本**（`047ec326`・Core 2 本・網 1 本・**数値定数 0・LP 由来の式 0**）: ⒜ `prebuiltMeasureToSystem` を引数に足し、手写しの walk を `BuildMeasureToSystemMap` へ**指し直す**／⒝ `measureToSystemBottom` を**唯一の読み手の後ろへ**（局所関数で遅延生成）／⒞ pedal row の表を **`belowMarks.Count > 0` の裏へ**。⚠️ **健全性は測定ではなく*構成*で言える**: ⒜ は同じ配列を同じ順に歩く同じ walk＝渡される地図が*その*地図／⒝ は読み手が 1 か所でそこで建てる／⒞ は表を埋める式が `belowMarks.Where(IsPedal…)` 自身なので、**below が空なら門が在ろうと無かろうと表は空**。

★★★ **⑸ A/B ＝ −0.4803%**（**8,451,068 → 8,410,475 B**・両側 TC=0・各側 2 回・床 0.0003%）。★★ **継ぎ目からの予測 0.480%（`sysIdx` 0.145 ＋ `sysBot` 0.199 ＋ `pedrow` 0.136）と実測 0.480% が一致**＝継ぎ目が正しいものに値段を付けていた。冊ごと（床 0.0157%）: **−1% 以上 12 冊**・最良 `Can't Get You Out Of My Head.lys` **−1.685%**・★★★ **上がった冊は 0**（**最も正の冊で −0.0002%＝231 冊が 1 冊残らず下がった**）。

★★ **⑹ 出力同一は 3 つとも走らせた**（RULES §5.1）: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline はこの便で `5874a5af` の木から取り直した＝製品の 2 ファイルだけ `git stash push`）／⑵ **全ページの SHA-256**＝**231 冊 × 8 打鍵＝5,824 ページで差 0 行**（ファイル全体の SHA も一致）／⑶ suite 全緑・snapshot 249 枚不動・台帳不変。

★★★ **⑺ 網 2 本——毒が「観測者が 1 人も居ない腕」を 2 つ見つけたから。** 毒 ⒞（pedal row の表を殺す）も 毒 ⒝（system bottom の表を空にする）も、**8,737 本のうち 1 本も赤くしなかった**。⚠️ **台帳の `mark.pedal.rows.*` は pedal row を見ているが、見ているのは `solvedPedalRowUp`（skyline 時の解）側で、その下の*legacy の段*は無観測だった**（毒 ⒜＝地図を空にすると 17 本赤くなる〈snapshot 9・台帳 6・単体 2〉ので、地図のほうはよく見られている）。⇒ `MusicMarkBelowStaffRowTests` 2 本（**差分の網**＝⒜ 2 つの pedal 族が 1 group に来たら別の段に立ち una corda が内側／⒝ 2 譜表の system では下の譜表を越えて落ちる）。**毒 2 本がちょうど自分の網だけを赤にする**（⒞ → ⒜ 赤・⒝ 緑／⒝ → ⒝ 赤・⒜ 緑）。★ **below-staff のマークが実コーパスに 16 本しか無い**のが、この穴が 420 便もったからくり。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: **⒵‴ ▶ 3 分の 1 だけ閉じた**（1.37% → 約 0.89%）。**残り**: ★★ **⒵⁴ `prefixMarkAnchorX` の解き直し 0.338%**＝**memo は反証済み**（hit 率 0.53%）・**要るのは「行頭 prefix を配置から持ち回る」土台の変更**／★ **⒵⁵ `mm.g.split` 0.163%**＝group ごとの `Where→OrderBy→ToList` 2 本（**マークは group あたり 1.09 本**・⚠️ `OrderBy` は*安定*ソートなので畳むなら安定性を保つこと）／★ **⒵⁶ 文字の 318,784 回／81 綴り 0.130%**（安い日の仕事）／★★ **⒳′ resolve の*複写*＝`EndBatch` の入力複写 0.17% ＋ `MergeInternal` 自身の複写（未計測・推定 0.7%）**＝call をまたいで生きる buffer が要る／★ **⒴ `inkFirst`＋`inkLast` 0.53%**／★ **⒴⁵ artic と trill の remap 辞書は作って捨てている**（天井 0.174%）／⚠️ **`g4.core` は repair 後 1.21% で*もう実仕事*＝この島には戻らないこと**／★★ **⒴⁗ 1 度目の配置を*安くする*＝`p1.prelim` は打鍵の 13.96%** で、**数が変わる 17.9% の打鍵ではその全部が捨てられる**（未測・土台の変更）／⒜ **R13⒝ の実機確認**（第404 ⑵）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・未着手・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／LP 双子が要る R7〜R11／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑻ 終了時**: commit 2 本（code `047ec326`・docs 1 本＝**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 1 本・**最終 full `sessions/p420/run3.trx` 8736 / 0 / 3 / 8739**（⚠️ **第419 最終の 8734 / 8737 から +2＝この便の網ちょうど 2 本**・`Zz420`／`ZzStage`／`ZZPOISON` の grep は**コードに 0 件**）・§7.5（対 `5874a5af`＝スクリプトの出力）**Core `+` 61 行／REF 0／OWN 0**（**うち註 31 行**＝足したコードは引数 1 本・局所関数 1 つ・門 1 つで、**数値定数は既存の `4.0` が局所関数へ移っただけ**・§7.6 ⒟「発明の削除と指し直し」）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。⚠️ **棚卸し `docs/APPROXIMATIONS.md` は*行番号のみ*動いた**（17 行・**数字を伏せた差 0**＝RULES §5.4 の既知の型）。計器と全文は Lab `sessions/p420/`（`musicmarks-split.txt`／`-final.txt`・`ab-{before,after}-tc0-{1,2}.txt`・`hashes-{before,after}.txt`・`Zz420{Probe,Ab,Hash}.cs.txt`・`ZzStage.cs.txt`・`zz420-seams.diff.txt`・`zz420-change.diff.txt`）。§7 3.5 は**セッションの頭で**済（`-Archive 418`＝19 行 5,336 chars）⇒ **HANDOFF は 442,054 B → アーカイブ後 436,718 B → 今便を書いて 448,3xx B＝天井 450,000 まで 1.7 KB しかない**（今便のブロックが 11.5 KB＝表を持たない代わりに段が 8 つあるぶん厚い）。⚠️⚠️⚠️ **次便は §1 に 1 字も足せない。`-Archive 419` を回すまで書き始めないこと**（回せば今便のブロック約 11.5 KB が空く）。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 419` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第420 の欠陥ではない。

## 以下は第419セッションの経緯

最終更新 第419セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `f4e9f4aa`・**未 push 15**・**full `sessions/p419/run1.trx` 8732 / 0 / 3 / 8735＝第418 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 67 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第418 が予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 417`＝32 行 6,192 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」の 4 つを AskUserQuestion に出し、**ユーザーが ⒴″「`b.scriptsFingerings` 4.2% を取り直す」を選んだ**。★ ベンチは着手前に AskUserQuestion で静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第418 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト）で、**第418 の教訓どおり A/B は両側 TC=0**（before 側の打鍵 8,797,90x B は**第418 が TC=0 で測った 8,797,893 B と一致**＝母集団も harness も同じという証拠）:

★★★ **⑴ 起票の 4.2% は裏取りできた。答えは大きさではなく*内訳*だった。** 段を割ると（`fs.gate`／`fs.maps`／`fs.plan`／`fs.units`〈`.group`／`.probe`／`.match`／`.miss`〉／`fs.walk`／`fs.assemble`・prelim と final の両 pass・TC=0）**`fs.all` は P 2.543% ＋ F 2.110% ＝ 打鍵の 4.653%**。内訳は **`fs.walk`（script 側の実仕事＝`ArticulationEngraver.CalculateWithFingerings`）2.824%＝block の 61%** と、**fingering 側の一式（`maps`＋`plan`＋`units`＋`assemble`）1.792%＝38.5%**。

★★★ **⑵ その 38.5% が返していたものは、空の配列だった。** **`fs.out.digits` は全 2,170 呼び出しで 0**＝**ユーザーの実コーパス 231 冊に指番号が 1 つも無い**。なのに第406 の門は「**script *も* digit も無い**」ときしか開かないので、**script を持つ本（P 1,402／F 1,160 呼び出し）は fingering の一式を丸ごと走らせていた**——小節ごとの beam／slur 表・unit plan・**62,542 の probe**（`MeasureIndices`／`Layouts`／`StaffOffsets` の 3 配列 ＋ beams／slurs の List を unit ごとに作って、hit なら捨てる）・**6,577 回の miss walk**（`CalculateWithTips`＝中身 0 を返す）。⚠️ **予測（§5.0②・probe のヘッダに先に書いた）は ⑴⑶ が当たり、⑵ は「miss は 0 のはず」で外れた**——**miss は 0.633% 払って空を返していた**。

★★★ **⑶ 直し＝門をもう 1 つ、*出力*の軸で入れる**（`5c3ea285`・Core 1 本・網 2 本・**数値定数 0・LP 由来の式 0**）。`bool anyFingering = AnyFingering(score, voicesByStaff)` を 1 度だけ取り、**digit が無ければ memo を `null` にし、島も `ImmutableArray<FingeringLayout>.Empty` で通す**＝**script の半分だけを走らせる**。⚠️ **健全性は測定ではなく*構成*で言える**: `AnyFingering` が歩く一覧（`FingeringStaffScores`）は `ComputeFingeringIslands` が消費する一覧*そのもの*で、`FingeringEngraver.Calculate` はその各 score の `score.Voice` しか読まない。さらに `CalculateWithFingerings` は空の `fingerings` をそのまま返す（1 行目 `adjustedFingerings = fingerings`）。⇒ **どの経路も空を返すことが型で言える。**

★★★ **⑷ A/B は −1.60%**（素の harness・継ぎ目なし・**両側 TC=0・各側 2 回**）: 打鍵 **8,797,902 → 8,657,440 B ＝ −1.5965%**（＝第417 の直し −0.22% の **7 倍**）。**床は同側 2 run で最大 0.0154%**。冊ごと: **−1% 以上が 140 冊・−2% 以上が 83 冊・−5% 以上が 3 冊**（最良 `Dream Come True.lys` **−5.67%**）・**不動 86 冊**（門が前から開いていた本＝script も無い）・★★★ **上がった冊は 0**（最も正で **+0.0086%**＝床の下）。

★★ **⑸ 出力同一は 3 つとも走らせた**（RULES §5.1）: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline はこの便で `f4e9f4aa` から取り直した）／⑵ **全ページの SHA-256**＝**231 冊 × 8 打鍵＝5,824 ページで差 0 行**（ファイル全体の SHA も一致）／⑶ suite 全緑・snapshot 249 枚不動・台帳不変。

★★★ **⑹ 網 2 本——ただし 1 本目は*毒見で作り直した*。** ⒜ `FingScripts_BookWithScriptsButNoDigit_NeverProbesTheMemo_AndMatchesFull`（memo 0 hit 0 miss ＋ full と同一）／⒝ `FingScripts_ADigitOnTheSecondStaffStillReachesThePage`（**第 2 譜表にだけ digit を置いた 2 譜表の本**＝門の射程）。**毒 2 本がちょうど自分の網だけを赤にする**（毒1＝門を外す → ⒜ 赤・⒝ 緑／毒2＝門が `voicesByStaff` を見ない → ⒝ 赤・⒜ 緑）。⚠️⚠️ **⒜ の最初の版は全小節に script を置いた本で、門を外す毒でも緑だった**——**script を持つ unit は memo を*断る***ので、門が在ろうと無かろうと probe は 0 ＝**「0」を構造的に保証する本を選んでいた**（1 小節目だけ script にして赤くなった）。**⒝ も最初は `data-pos` を伏せずに比べていて毒で緑だった**（綴りの長さが違うので、digit を 1 つも描かなくても差が出る）。⇒ **RULES §5.4 に 2 項**。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: **⒴″ ✅ fingering 側は閉じた**。**⒴⁶ ✅ 第 2 便が閉じた**（script の半分は **3.27% → 0.91%**。残るのは `w.rest` 0.40%・`w.ties` 0.14%・`w.order` 0.13% の実仕事で、★ **「script 側に memo が無い」は島として起票しない——賞金がもう 1% 未満**）／★ **⒴⁵ artic と trill の remap 辞書は作って捨てている**（第418 ⑷ の内側・**天井 0.174%**＝安い日の仕事）／★★ **⒳′ resolve の*複写*＝`EndBatch` の入力複写 0.17% ＋ `MergeInternal` 自身の複写（未計測・推定 0.7%）**＝call をまたいで生きる buffer が要る／★ **⒵‴ `e4.musicMarks` 1.22%**・未分割／★ **⒴ `inkFirst`＋`inkLast` 0.53%**／⚠️ **`g4.core` は repair 後 1.21% で*もう実仕事*＝この島には戻らないこと**／★★ **⒴⁗ 1 度目の配置を*安くする*＝`p1.prelim` は打鍵の 13.96%** で、**数が変わる 17.9% の打鍵ではその全部が捨てられる**（**「見積り専用の安い prelim」が要るかは未測**・土台の変更）／⒜ **R13⒝ の実機確認**（第404 ⑵＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・⚠️ 未着手・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／LP 双子が要る R7〜R11／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★★★ **⑺ 第 2 便**（同日・指示「**このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない**」→ ★ **有利と判断した根拠を先に書いて着手**＝⑴ 次の A/B の *before* 側が手元に在る（この便の after を TC=0 で 2 回取ってある）／⑵ 証明 ⑴ の baseline が今の出力で取ってある／⑶ 計器と継ぎ目の当て方が生きている）＝**⒴⁶ を割ったら、`fs.walk` の 76% は walk ではなく*入口の 2 つの全曲 fold* だった**。**`w.tips`（`BuildBeamedStemTips`）1.474% ＋ `w.groups`（`BuildBeamGroupMap`）1.023%** に対し、**優先度順の walk 本体 `w.rest` は 0.397%**（`w.ties` 0.138・`w.order` 0.130・`w.slurs` 0.024・`w.map` 0.010）。★★ **どちらの表も鍵は必ず script 自身の (staff, voice, measure, item)**＝**訊かれる鍵は `articulations` から先に全部わかる**のに、tip 表は **1 呼び出しで 561 entry を建てて 24.7 件しか訊かれない**（23 倍）。★★★ **tab 表は 1,848 打鍵で *0 回*しか訊かれていない**（numbers-only tab の script はこの分岐の手前で落ちる＝**tab 本ばかりのコーパスでも届かない**）。⇒ **直し**（`00755ef4`・Core 1 本・網 0 本）＝**tip 表は script の鍵だけで建て、tab 表は最初の tab の問いで建てる**（`wanted` は両 builder の省略可能な引数＝`FingeringEngraver` の呼びは無変更）。**A/B −2.38%**（**8,657,440 → 8,451,074 B**・両側 TC=0・各側 2 回・床 0.023%）・**−1% 以上 122 冊／−5% 以上 24 冊／最良 −8.89%**（`想い人.lys`）・**上がった冊 0**（最大 +0.011%）。★ **ask/hit は両側で同一**（34,676 / 25,546）＝**walk は同じ問いをして同じ答えを得ている**。出力同一は 3 つとも（絵 **0/81**・**5,824 ページ差 0 行**・suite 全緑・snapshot 不動）。⇒ ★★★ **便の合計は 8,797,902 → 8,451,074 B ＝ −3.94%**。

★★ **⑻ 網は 1 本も足していない——代わりに*観測者を毒で確かめた***（§5.3 の無観測の罠）: tip 表を空にする毒は **`test/fermata-down`・`test/scripts-stem-support`・`test/drum-groove`** を、tab 表を空にする毒は **`test/tab-beam-script`・`test/tab-staccato-beam-side`・`test/tab-beam-slope`** を赤にする（**6 つの名前をコードの註に書いた**）。⚠️ **掃きは tab 側について何も言っていない**（0 ask）＝**snapshot がその半分の証拠の全部**。⚠️ **棚卸し 2 本は*行番号ずれ*で赤くなる**＝`LILYSHARP_UPDATE_DOCS=1` で再生成し、**130 行が動いて「数字を伏せた差 0」**を確かめた（RULES §5.4 の既知の型・第339 が記録している）。

★ **⑼ 終了時**: commit 3 本（code `5c3ea285`・code `00755ef4`・docs 1 本＝**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 1 本・**最終 full `sessions/p419/run5.trx` 8734 / 0 / 3 / 8737**（⚠️ **第418 最終の 8732 / 8735 から +2＝第 1 便の網ちょうど 2 本。第 2 便は 0 本**・`Zz419`／`ZzStage`／`ZZPOISON` の grep は**コードに 0 件**）・§7.5（対 `f4e9f4aa`＝スクリプトの出力）**Core `+` 73 行／REF 0／OWN 0**（**大半は註**＝足したコードは第 1 便が `anyFingering` の 1 行と三項 2 つ、第 2 便が `wanted` の 4 行と両 builder の 2 行ずつ・§7.6 ⒟「既存の門を増やし、既存の表を細くしただけ」）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。⚠️ **棚卸しは*第 2 便で動いた*＝行番号のみ 130 行**（第 1 便では不変だった）。計器と全文は Lab `sessions/p419/`（`summary.txt`・`fingscripts-{before,after}.txt`・`scriptwalk-{before,after}.txt`・`ab-*-tc0-{1,2}.txt`・`hashes-{before,after,leg2-after}.txt`・`Zz419{Probe,bProbe,Ab,Hash}.cs.txt`・`zz419{,b}-seams-and-change.diff.txt`）。§7 3.5 は**セッションの頭で**済（`-Archive 417`＝32 行 6,192 chars）⇒ **HANDOFF は 439,550 B → アーカイブ後 433,358 B → 2 便ぶん書いて約 447,000 B＝天井 450,000 まで約 3 KB**。⚠️⚠️ **次便は §1 に 1 字も足せない**＝`-Archive 418` を回すまで書き始めないこと（回せば第418 のブロック約 10 KB が空く）。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 418` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第419 の欠陥ではない。
