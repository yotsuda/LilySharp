...
★★ **未 push は 4 便続けて両端が繋がった**——**第196 の終了時 35 と開始時 35 が一致**（間で push は無い）。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 44**（★★★ **この数は 3 度書き直した。40 → 42 → 44。**
**第196 の手順「commit を作る*前*に数えて +1 して書く」は 1 度目に当たったが、
そのあとユーザーが ⒡ を承認して便が 2 本続き、訂正 commit 自身がまた 1 本増やした。**
⇒ ★★★ **手順の抜けは「+1」ではなく*順序*だった**——**引継ぎ commit を最後にするか、
さもなくば「訂正した回数だけずれる」。この行を書く commit を含めて 44。**）・
suite **5338 passed / 0 failed / 4 skipped**（+13＝新しい網ちょうど。**⒡ は網を足していない＝本の修理**）・
...
- **⑽ ★★★ 次に触るならここ＝ ⒢ の 1 件だけ**（詳細と最小対は §2F ⒢）。
  **⒜⒝⒞⒟⒡ は全部閉じたので、§2F に*実装待ち*の項は他に無い。**
  ...
  ⇒ **決める形は「score が名指す part が 1 つなら素の section はその part のもの／
  2 つ以上なら今のまま＋警告」で、後者は将来のための定義。見積り 1 便。**
  ⚠️ **19 冊の MIDI が動く**ので、**全数 A/B で「動いた本がページと一致した」ことを見せること。**
...
- ⚠️ ★★★ **⒢ 素の section には part の lane が 1 つも掛からない**
  （2026-08-17・第197 実測・**未修理**）。
  ...
  - ★★★ **射程を数えた**（2026-08-17・第197。**綴りの grep＝下の数え方ごと残す**）:
    ...
    **うち part が既定でない（`clef`/`instrument`/`tuning`/`octave`/`transposition` を書く）本＝ 19 冊。**
    ...
    ```
  - ⇒ ★★ **だから決める形はこう**: **score が名指す part が 1 つなら素の section はその part のもの／
    2 つ以上なら今のまま＋警告**（**射程 0 冊なので、後者は将来のための定義**）。
  - **見積り**: **1 便**。**`MidiExporter` は既に `Form` を受け取る**（第197 `ab755a71`）ので
    **`RenderSpec` を足す配管は同じ形**、**arm するコードは `PartBlockSyntax` の枝がそのまま使える**。
    ⚠️ **19 冊の MIDI が動く**（それが直すということ）ので、**`pitches` の全数 A/B で
    「動いた本がページと一致するようになった」ことを見せること**。
