...
最終更新 第206セッション＝**オーナーが自分の本を読んで見つけた欠陥を 7 件閉じた便**
（**実装 6 便**。**perf は測っていない＝走査を 1 本も足していないため**。§7.9 の
「足していない例」・commit ごとに理由を 1 行書いた）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **第205 の §1 ⑹ ⓪ は「次は 0.3.0 を出すかの判断」だったが、
ユーザーが「残債の返済を中断して、*私に見えている*リリースブロッカーを直せ」と指示した。**
**以後この便は全部「ユーザーが本を開いて指した欠陥」で駆動されている**（下の骨 1）。
⇒ ★★★ **0.3.0 を出すかは*まだ決まっていない*。次便も同じ設問から始まる**（下の ⑹）。
★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 525 点・ss 非ゼロ 99・
総和 3.644108828・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 25**・
**snapshot 218 枚**・suite **5449 passed / 0 failed / 4 skipped**・**コーパス 567 冊**・
**Core 0 警告**＝**引継ぎの数は全部合っていた**（**36 回連続**）。
⚠️ **`origin/master` は `9830873f`＝開始時と同じなので、便の途中の push は無かった**（§0 の最後の確かめ方）。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 32**（**この行を書く commit を含めて 32**。
⚠️ **この行を*書いた時点*では作業ツリー項目 2**＝`docs/HANDOFF.md` と `docs/HANDOFF-ARCHIVE.md`）・
suite **5449 → 5480 passed / 0 failed / 4 skipped**（**+31**）・
**台帳 525 → 529 点**（**ss 非ゼロ 99 → 100・総和 3.644108828 → 3.794108828**
——⚠️ **総和が増えたのは悪化ではない**。**増えた 0.15 は「今まで一度も測られていなかった量」が
可視化されただけ**＝README の「比較は同じ点集合の中でだけ意味を持つ」の 5 例目）・
**count 点 106/2 は不動**・**snapshot 218 枚で不動＝中身は 1 枚だけ動いた**
（`test/key-change-linebreak`・**ユーザー承認済み**）・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Turmeric`）で通せた。**

> ## ★★★ この便の骨＝**7 件とも、追跡コーパスにも suite にも見えていなかった**
> **オーナーが自分の本を読んで 1 件ずつ指した。** 射程を測ると理由がはっきりする:
>
> | 欠陥 | 追跡 snapshot | ディスクの本 |
> |---|---|---|
> | `@mark` が 1 小節後ろ／最終音符で消える | **0 枚**（該当 fixture は不動） | **98 / 241** |
> | 行末カラテシーの右余白 0 | 1 枚（承認済み） | 3 / 12 |
> | to coda がボルタ括弧の中 | 0 枚 | 1 / 2 |
> | 調号列が段数ぶん膨らむ | 0 枚 | 3 / 12 |
> | 下向き強弱記号がシルエットに無い | 0 枚 | **1 / 316** |
> | コーダの行頭整列＋ラベル浮き | 0 枚 | 2 / 4 |
> | una corda の段順 | 0 枚 | **0 / 1123** |
>
> ⇒ ★★ **「suite が緑」は「その site に本が届いていない」と区別できない。**
> **7 件とも、欠陥を入れたまま suite が緑で通った**（毒で確認済み）。
> ⇒ ★★★ **`@mark(` を書く本は追跡 2 冊に対しディスク 241 冊（うち 237 冊がオーナーのベースタブ）。**
> **コーパスの偏りがそのまま「見えない領域」の地図になっている。**

> ## ★★★ 2 つ目の骨＝**毒が「死んだコードへの修正」を 2 度止めた**
> ⑴ **調号列**: 明らかな現場に見えた `MeasureChangeColumn` を先に直したら**何も動かない**。
> **幅を 10 倍にする毒を当てても動かない**——**報告の本の key 変更は小節の頭に立つので
> その walk に届いていなかった。** 生きていたのは `BoundaryChangePrefix`。
> ⑵ **網の初稿が 3 度、欠陥に対して合格した**（本が 1 段に収まって行頭が無い／
> 1 段の grandStaff は staff group を建てない／`LiveRender.Svg` が単声部の経路）。
> ⇒ ★★ **「直した」と「その site に届いた」は別。毒は後者を測る。**

> ## ★★★ 3 つ目の骨＝**計器が 3 回、測定そっくりの顔で嘘をついた**（全部 §5.3 の形）
> ⑴ **LP プローブが `KeySignature` を override し忘れ**——「LP はここにカラテシー調号を出さない」と
> 読みかけた。**五線の位置は動いていないので*もっともらしい*ゼロ**だった。
> ⑵ **`\codaMark` は Score コンテキストの grob**——`\new Staff` の override では何も出ず、
> **「LP はそこにコーダを描かない」と*そっくり同じ*空の出力**になった。
> ⑶ **LP のペダル「Ped.」はテキストではなくグリフ**——`<tspan>` を数える計器は
> **2 段を 3 段と報告しかけた。**
> ⇒ ★★★ **対照（control）を置くと嘘が割れる**: ⑵ は `SectionLabel` が同じ改行で
> 新しい行に出る（x 0.0）ことを見せて初めて「出ない」が測定になった。
> ⑶ は単独ペダル 3 スコアがラベルを同定した。

> ## ★★ 4 つ目の骨＝**私の修正が作った退行を、ユーザーが即座に指した**
> **コーダを小節線へ移した瞬間、隣の `E` ラベルが 4.96 ss 浮いた**（積まれない位置は 2.28）。
> **マークのグループ鍵 `(measure, position, timing)` は「同じ x」の*代理*で**、
> **行頭の印が全部同じアンカーだった間だけ成り立っていた。** 私がその前提を壊した。
> ⇒ **鎖は与えられた x を実際に見る**ようにした（インクが重なる相手の上にだけ積む）。
> ⚠️ **最初「別の既存欠陥」と報告しかけて、グループ化のコメントを読んで訂正した。**

★ **この便の値段**（**perf は測っていない**＝**走査を 1 本も足していない**。
**足したのは辞書 1 本の書き/読み・alist の項目 2 つ・列ごとの小さな走査・
既に置いた印の数個を見る内側ループ**。**pass も rebuild も新しい鍵も無い。**）:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ 印の小節 ＋ 行末カラテシーの `right-edge` | Core 6 ファイル ＋ 網 13 | 98 冊 / 3 冊 |
| ⑵ to coda がボルタ括弧を避け続ける | Core 2 ＋ 網 1 | 1 冊 |
| ⑶ 変更列は kind ごとに 1 grob | Core 1 ＋ 網 4 ＋ 台帳 1 点 | 3 冊 |
| ⑷ 強弱記号を系間シルエットへ | Core 1 ＋ 網 2 | 1 冊 |
| ⑸ コーダの行頭整列 ＋ ラベルを浮かせない | Core 3 ＋ 網 2 | 2 冊 |
| ⑹ una corda の段順 | Core 1 ＋ 網 1 | **0 冊**（推測→測定） |

- **⑴ `44f73630`**。**印は音符に書かれるのでその音符の小節に属する**（`builder.CurrentMeasureIndex` は
  「builder は今どこか」に答えていた）／**break-align 群は grob より 1 つ多い**——`right-edge`。
  **⚠️ 裸の `+ 0.4` を 1 つ削除**（この群で **5 個目**の無名 0.4。2026-08-03 の 4 個目と同じ形）。
- **⑵ `d244e880`**。**外側配置は正しく避けていて、`CoPlaceToCodaWithLabels` が取り消していた。**
  ⚠️ **1 手目は行き過ぎ**（障害物の無い本でもラベルが共通線から浮いた）——
  **スタック前の配列を渡して「そもそも動かされたのか」を訊く**ようにした。
- **⑶ `c798faa9`**。**列の item 列は段をまたいで集められる**ので、同じ key 変更が段の数だけ入り
  **足し算されていた**（+3.300030／段）。**LP は 1/2/3 段で 12 桁一致。**
- **⑷ `e4a175f0`**。**スカラー extent は最初から数えていた。X を見るシルエットに無かった**——
  **和音記号の腕が 3 行上で同じことを書いている。**
- **⑸ `016cbfc1`**。**`CodaMark` は `(staff-bar key-signature clef)`、`SectionLabel` は `(left-edge staff-bar)`。**
  ⚠️⚠️ **LP は改行位置のコーダを*新しい行に描かない***（`begin-of-line-invisible`・前の行の末尾に出る）。
  **ユーザーが「新しい行に残して小節線へ」を選択**したので、**配置そのものは LILYSHARP-OWN**。
- **⑹ `e6eeb280`**。**§2G が名指していた唯一の「未実測の推測」**。**una corda は最も*近い***
  （LP 2.7775 / 4.7387 / 7.1813）。⚠️ **計器が知らない数を再現した**——
  sustain−sostenuto が **2.442600**（第204 がペアだけから測った 2.443）。
- **⑺ ★★ ユーザー決定 3 件**: **⒜ snapshot 1 枚の再ベース承認**／
  **⒝ コーダは「新しい行に残して小節線に合わせる」**（LP 完全準拠＝前の行の末尾、ではなく）／
  **⒞ 便の最後に「残債のうち*いま*やるのが有利なものだけ」の委任**——
  **§2 を通して 1 件だけ着手し（⑹）、他 5 項は「不利」として着手しなかった**（理由は §2 の各項）。
- **⑻ ★★★ 次に触るならここ**——**⓪ まず 0.3.0 の判断**（**`Directory.Build.props` は `0.3.0`・
  タグは 1 つも無い・`v0.3.0` を push すると `.github/workflows/release.yml` が
  test → CLI 4 RID → VSIX を回す**）。**correctness の門は空のまま**（§2F に「決定済み・未実装」は無い）。
  ⚠️ **出す前に片付ける候補は第205 から変わらず 2 つ**: **⒤ `CHANGELOG` が無い**
  （`release.yml` は本文を生成しないので**リリースノート無しで出る**）／
  **⒥ 未 push 32 便ぶん**（**タグだけ先に push すると CI が古い木を建てる**ので **`master` を先に**）。
  ／**⒜ ★ 今便が開いた台帳点 `courtesy.key.key-to-line-end` の残差 0.15**——
  **`right-edge` ではなく*予約は上界／描画は実インク*のたるみ**で、
  **閉じ方は `SharedRenderer` 自身が名指している**（「予約幅を描画幅にして `SolveColumns` に渡す」）。
  **今便の網が初めてそれを観測している。**／
  **⒝ ★ ペダルの段間隔**（LP は 1.961 → 2.443＝各段の実インク・Lily# は一律 2.46。**台帳点は無い**）／
  **⒞ ★ `CoPlaceToCodaWithLabels` の鏡像ケース**（**ラベルが標識*以外*で持ち上げられていた場合**は
  今も落ちる。**直すにはこの装置をスタック前へ動かす**＝削除済み `CoPlaceTempoWithLabels` と同じ形）／
  **⒟ §2 の頭の ▶ perf**（第193 の地図のまま・門の外）／
  **⒠ ★ SVG の同梱 sans のずれ**（第205 のまま。**出力についてのユーザー判断が要る**）／
