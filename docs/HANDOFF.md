...
- **R14 exporter の残り**＝MIDI の歌詞 meta が 1 tick に全部（`ProcessLyrics :2567`）／~~twin が `CarryFrameInto/Back` で `_lastWrittenValue/Dots/_forceNextDuration` を運ばず tuplet/repeat/cue の後で音価がずれる~~（✅ 第398 第 3 便＝R4 と同じ commit で往復させた。**phrase 参照の body だけは従来どおり fresh "4"・force なし**＝ページの fresh frame と LP の lexical のどちらが正しいかは未判定・twin 項の残り）／`\time 3+2/8` の綴り（LP は `#'((3 2) . 8)`・要実測）／`_pendingGraceSteal` が lane 跨ぎ／`c4 r4 ~ c4` で前の音が休符を跨いで延びる／`tempo 2 = 60` の beat unit（XML は quarter 固定・MIDI は 4 分 BPM＝`Bpm` の契約を確認）。
