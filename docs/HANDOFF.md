...
終了時: **本便は 18 commit**（**実装 9**＝`bfe79e4c`／`c2a34cee`／`a74f4eb4`／`2febe24c`／`dc23f788`／`6265ed6b`／`a6c846e8`／`92ffee25`／`b46ed1e9`、**handoff 9**＝`0991f108`／`9ef417c0`／`ee5f4a5b`／`f2e2081f`／`cadb7dfc`／`e07413a3`／`8e0d04b0`／`13cd1fdf`／この行）・**未 push 20**・未追跡 0/木 0・**5940/0/4**（＋66・全部新規）・**台帳 573／ss 非ゼロ 115／総和 3.957938014／count 109・非ゼロ 2／snapshot 222／追跡コーパス 572 —— 6 つとも不動**・Core 0 警告（`--no-incremental`）。⚠️ **本便は出力を動かす変更を 3 つ入れている**（五線なし × section を 2 度名指す／form の `|: :|`／4.0 でない帯の繰り返し点）が、**追跡 572 冊・snapshot 222 枚のどれもその形を綴らないので再ベースは 1 枚も要らなかった**。⚠️ **§7.5 の数え上げ**＝①②③ は Core の `+` 行 157・`LILYPOND-REF` 0・`LILYSHARP-OWN` 0——**これは正しい 0**: `form { A B A }` は Lily# 自身の構文で LP に対応物が無く、**この島が要る唯一の LP の事実**（囲いの無い alternative は 1 度だけ鳴る＝`alternative-sequence-iterator.cc:83-84`）は**既に `ProcessForm` が住所を持っている**。新アームは**その家を指しただけ**（§7.6 ⒟）。**⑤ は逆で、REF が本体**（`scm/bar-line.scm:360-368`）。数値定数は ⑤ の `0.5` が 1 個で、**それは前から在った 2 個を 1 個に畳んだもの**。
...
- **★★ 次の一手の候補＝`make-colon-bar-line` の `dist` 探索が未移植**（⑤ で名指した）。**staff symbol を持たない帯は LP では ±0.45 ss**（「dots should go outside」枝）**で、Lily# は ±0.5 のまま**。⚠️ **これを移すには renderer が `line-positions` を持つ必要がある**（今は `StaffHeight` の定数だけ）。⚠️ **そして Lily# のリードシート行は既に意図的乖離**（拍子を刷る・§3 第226）なので、**「LP に合わせる」の意味を先に決めること**（§5.0「何を主張できるかを測る前に決める」）。**器を作る島であって、定数を動かす島ではない。**
