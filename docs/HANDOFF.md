...
そこから沈黙と、それを測る計器の欠陥が芋づるで出た便**（**実装 5 便**）。
...
終了時 **実装の最後は `ad358143`**・**未 push は
`git rev-list --count origin/master..master` が言う**・**未追跡 0**・
suite **5053 passed / 0 failed / 4 skipped**（+19＝新しい網ちょうど）・
**台帳 516 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・
**コーパス 0/81**（**worktree の base ビルドと HEAD ビルドで同じ 81 冊を描いた真の A/B を
2 回**・**陽性対照つき**＝1 音変えた本でハッシュが動くことを確かめてある）・
...
- **⑹ ⚠️ ★★★ この便が*直さずに起票*した 2 件**（どちらも §2F・実測つき）:
  ...
  **⑵ score 単位の `transpose` が 3 通りの答えを返す**——part 側 `transpose d` は c→D4（長2度）、
  score 側は c→E4（長3度・♯4 つを絵で確認）、**同じ score 綴りを別の score と並べると c→D4**。
  ⚠️ **fixture は score 単位 transpose を 1 冊も書いていない**ので誰も落ちない。
- **⑺ ★ 毒は 8 方向**（§5.4）。**⑵＝警告だけ落とす→3 本赤**（diamond と cycle は緑のまま
...
- **⑻ ★ §7.5 の監査**＝第2便 Core +95 行・第3便 +16 行・第5便 +119 行、**REF 0 / OWN 0・
  幾何の数値定数 0**（第5便の 1 件は `int collected = 0` ＝ループ計数器）。
...
- **⑼ ★★★ §2F に残っているのは 3 つ＋2**——**⑴ 一覧に構文を与えるか（決定であって作業ではない）
  ⑵ 繰り返し縦線の ⑹ ⑶ `lyrics` の voice 束縛（要決定）**、＋**⑷⑸ 新規＝
  `grandstaff-high-bass` が主張を守っていない件（要承認）と score 単位 transpose の 3 通り**。
  **「parse しない fixture 2 本」と「`--pitches`」はこの便で閉じた。**
...
- ⚠️ ★★★ **新規＝score 単位の `transpose` が 3 通りの答えを返す**（第182 実測・**未修理**）。
  **`part m { clef treble transpose d }` は c→D4**（長2度＝`test/transpose-down.lys` が
  「Verified against LilyPond \transpose c bes,」と書いている規約どおり）。
  **`score main "up" transpose d { staff m }` は c→E4**（長3度・♯4 つ＝ホ長調で描かれる。絵で確認）。
  **同じ score 綴りを、別の score と並べた本では c→D4**。
  ⇒ **1 つの構文に 3 つの読み**。⚠️ **fixture は score 単位 transpose を 1 冊も書いていない**
  ので誰も落ちない。**着手するなら双子で LP に訊くところから**（`\transpose c d` は長2度）。
