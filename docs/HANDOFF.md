...
最終更新 第215セッション＝**リリースブロッカー 2 件を閉じ、そのままユーザーと文法を 2 島設計・実装して「文法を完全にしてから出す」を満たした便**（**実装 5 便＋決定 6 件**）。
⚠️ **既存の彫版は 1 冊も動いていない**——⒠ の SVG family 属性（11 枚・属性のみ）と、fixture の `sings` 追記による **source offset のみの 14 枚**（正規化 diff 空を機械で確認）、新規 snapshot 2 枚。**台帳 529 点・総和 3.644109 は全便通して不動。**
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」——この便で 2 島を
実装したが、**便の最後にユーザーが `with` 除去（§2F ▶▶）を決定し、タグはその後ろに回った**。
★★★ **着手の理由**: ⒠・⒦ はユーザーの名指し。2 つの文法島は「このセッションで着手が有利なら着手」の委任——**設計の全ニュアンスが会話内にしか無く、引継ぎで最も失われやすい**ので有利と判断した（リズム島は判断済みで即着手・sings 島は未決 3 点を 1 問で閉じてから着手）。

★ **開始時裏取り（§0 通し）**: HEAD `45f9cf10`・未 push 5・suite 5596/0/4・CI 直近 2 本緑＝引継ぎと全一致。
終了時 **未追跡 0・作業ツリー項目 0・未 push 13**（**この行を書く commit を含めて 13**。
**数えたのは commit の*あと***）・suite **Windows 5629 / 0 / 4**・**Linux（この機械の WSL）5629 / 0 / 4**
——**両 OS 完全緑を維持**・**台帳 529 点・exact 454・総和 3.644109・count 110/112**（不動）・
**snapshot 220 枚**（＋`test/rhythm-slashes`・`test/sings-chorus-row`）・**Core 0 警告**。

★ **この便の値段**:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ ⒠ SVG sans 名指し（`f1676e5e`） | Core 1・snapshot 11 | 属性のみ・§2F ✅ |
| ⑵ ⒦ 決定＝未署名で出す | §3 | コード 0 |
| ⑶ リズム記法 `/`＋裸 duration（`c6b9fb32`,`f279565c`） | Core 27 file・網 25 本 | 新スペルのみ（旧本は字句的に到達不能） |
| ⑷ `sings` 束縛＋刷らないメロディの行（`5fc6f788`） | Core 8 file・網 7 本・fixture17 書換 | 新スペル＋**unbound `with lyrics` は即エラー**（意図的破壊・初タグ前） |

- **⑶⑷ の設計と代償は §3 の上 3 行が正典**（LP 準拠の実測ピン・LYS0016 の網の行き先・
  スペーサー骨格の仕掛けまでそこに）。実装の家: 裸 duration は `q` の写し
  （`Music.BareDurations`＋各 walker の記録二段）・スラッシュは `@notehead(slash)` 配管の再利用・
  sings 行は「メロディをサブ収集→全 item を同長スペーサーに→既存の note-bound aligner に
  `asRow` で流す」（`MeasureCollector.CollectMelodyFor`／`SpacerSkeleton`／
  `LyricsCollector.CollectBoundRow`）。
- **perf（§7.9）**: 打鍵 alloc は 3 商とも桁まで不動（162.6／45.2）・全体 ≤0.13%
  （音符ごとの記録辞書）。sings は render 経路に 0・診断経路に validator 1 本
  （既存 DescendantNodes 族と同形）。
- **twin**: 裸 duration は素通し（LP 2.20+ が読む）・スラッシュは `\improvisationOn`＋clef 中央線
  音高（LP 側フレームだけ進める）・歌詞行は従来どおり「twin に行は無い」警告のまま。
- **⑸ ★★★ 次に触るならここ**——**⓪ `with lyrics`/`with chords` の除去＝「score は帯の縦列」**
  （**ユーザー決定・この便の最後に起票**。設計・残作業 4 点・probe の観察 2 件は **§2F ▶▶**。
  構文破壊なので**タグより先**）。**① そのあと `v0.3.0`**: push（未 push 13・`master` を先に）
  → CI 緑 → タグ。CHANGELOG は 3 機能とも反映済み・最上段=0.3.0 確認済み・署名の行は残す（§3）。
  ⚠️ workflow の初実走はタグの瞬間。**タグ後の残債**: ⒝ ペダル段間隔／⒞ CoPlaceToCodaWithLabels
  鏡像／⒟ §2 ▶ perf／⒡ 配管 6 site／小粒: sings の LSP 補完（`sings`＋part 名）未実装・
  twin の歌詞行未対応（従来から・警告は出る）・`lines` 属性は twin 未輸出（従来から）。

> ## ★★ 骨 1＝**「ユーザー判断待ち」は 2 種類**（判断だけで閉じる／資産が要る）——起票時に書き分ける。

> ## ★★★ 骨 2＝**文法拡張の最安の家は先人の自分が建てている**
> スラッシュ符頭は `@notehead(slash)`、反復は `q` の resolver、sings 行は「行＝スペーサー声部」
> という既存の骨格が、それぞれ実装前から全配管を持っていた。**新設の発明は 0**
> （`LILYSHARP-OWN` 新規 0 本・REF は music_embedded／improvisationOn／CHORD_REPETITION）。
> ⇒ **設計は「何を書くか」の前に「どの家が半分やっているか」を数える。**

> ## ★★ 骨 3＝**網を消費する変更は、消費した網の行き先を 3 箇所で同じ言葉にして初めて閉じる**
> `c 4` の LYS0016 も unbound `with lyrics` も、閉じた扉と黙って通るようになったものを
> §3・文法書・テスト remark に同文で残した。**「エラー→機能」は互換では安全・網では負債。**

> ## ★★★ 骨 4＝**source を動かす一括変更の検算は「正規化して diff が空」を機械に言わせる**
> fixture 17 冊への `sings` 追記で snapshot 14 枚が動いたが、data-pos を正規化した diff が
> 全冊空＝ink 不動を 1 コマンドで確認してから承認した（⒠ の属性 11 枚も同じ手）。
> **「見た目同じ」を目で言わない。**
...
最終更新 第214セッション＝**§1 ⑸ の残債を 3 つ、両 OS の緑のまま閉じた便**（**実装 2 便＋doc 1 便**）。
...
（⒜ は台帳が閉じ方まで名指し済み・⒢ は「小さい」と起票済み）。**第 3 便はユーザーの
「次便、有利ならこのセッションで着手」委任で ⒤（CHANGELOG）に着手**——環境が温態
（両 OS 緑を今測ったばかり）で検証費用が最小、かつ自律で閉じられる残りがこれだったため。
...
終了時 **未追跡 0・作業ツリー項目 0・未 push 5**（**この行を書く commit を含めて 5**。
**数えたのは commit の*あと***）・suite **Windows 5596 / 0 / 4**（**doc 便の commit 後にも
取り直して同数**）・**Linux（この機械の WSL）5596 / 0 / 4**
...
| ⑶ CHANGELOG ＋ Release への配線 | doc 1・workflow 1 | **リリースの門のみ**（彫版 0・テスト 0） |
...
- **⑶ `9e7c82a8`＝⒤ CHANGELOG を書き、Release に配線した**（doc＋workflow のみ・テスト不変）。
  **`CHANGELOG.md` は 0.3.0 を*差分ではなく製品として*記述**（最初のタグなので前がない。
  数は全部この便の実測: 台帳 529・snapshot 218・テスト 5,600 両脚緑）。
  **`release.yml` の両 upload job が最上段セクションを awk で抜いて `body_path` で添える**
  （**両方に置いた理由**: 先に走った方が Release を作るので、片方だけだと勝者がノートの
  有無を決める）。⚠️ **タグを打つ便への 2 つの宿題を残した**——
  **⒜ 最上段セクションが打つ版と一致していることを確認**（抽出は「最上段＝打つ版」を仮定）／
  **⒝ Known limitations の「未署名」行は ⒦ を追跡している**＝署名するならその行を消す。
  ⚠️ **awk 抽出はローカルで実測済み・YAML は pyyaml で構文検証済み**だが、
  **workflow 自体はタグを打つまで走らない**（初回リリースが初回実走）。
- **⑷ ★★★ 次に触るならここ**——**⓪ 0.3.0 の判断**（**10 便続けて未決だが、材料は初めて揃った**:
  **push 済み・CI 緑・両 OS 完全緑・CHANGELOG 済み・§2F に「決定済み・未実装」無し**）。
  **出す前候補は 2 つに減り、どちらもユーザー判断**:
  **⒦ 未署名 exe を SAC が弾く**（`release.yml` に署名は 1 行も無い。**CHANGELOG の
  Known limitations がこれを明記済み**＝署名するならその行も直す）／
  **⒠ SVG の同梱 sans のずれ**（和音記号の `font-family="sans-serif"`）。
  ...
  ⚠️ **未 push 5（この便）**——push はユーザーの手癖に合わせて残してある。
...
> # ▶▶ **`with lyrics`/`with chords` の除去＝「score は帯の縦列」への統一（ユーザー決定・第215・次便着手・v0.3.0 はこの後）**
> **終着点は LilyPond と同型**: 関連は定義（`sings`／chords は無名ブロックの同時書き）・
> **位置は score 内の並び順**・譜への吸着は **staff-affinity**。`sings`（§3）で半分は済んでいる。
> **残る作業 4 つ**（第215 が A/B probe で測って名指した・probe は scratch/p216/ab2-{attach,row}.lys）:
> - **⑴ staff-affinity の移植**——行を「独立の帯」から「関連譜に吸着する帯」へ。
>   LILYPOND-REF: staff-affinity ＋ nonstaff-relatedstaff-spacing（LP の Lyrics は
>   これで譜に張り付く）。**LayoutEngine の row 分岐に「HANDOFF §3 の意図的乖離・
>   見直す日が来たらこの分岐も一緒に行く」と注記済み**（`IsLyricsRow` の縦 extent 分岐）。
>   これで行が**譜下の優先順位スタック**（ペダル・強弱・nav）に参加する——
>   ピンは `pedal-below-lyrics`・`nav-below-clears-lyrics`・`lyrics-below-marcato`。
> - **⑵ グループ内の行**——`StaffGroupBody` は今 `staff` しか取れない。コラール
>   （choirStaff の譜間に歌詞）を並び順で書くには、行をグループ本体に許す。
> - **⑶ ハラキリ連動**——添付歌詞は譜と一緒に消える（removeEmpty）。吸着行にも同じ連動を。
> - **⑷ chords 側の残り**——行構文は `chords prog as roman` を既に持つ。**無名 `chords {}`
>   の自動添付形の扱いを決める**（そのまま残すか、これも縦積みに畳むか）。
> **移行**: `with lyrics`/`with chords` の全 fixture・docs・samples・XML importer 出力を
> 並び順へ書き換え（第215 の sings 一括の再演・正規化 diff 空の検算を同じ手で）。
> **縦座標は歌詞系 fixture 全冊で動く**——snapshot は ink 差が本物なので、承認は目視の上で。
> ⚠️ **probe が拾った観察 2 件**（統一と独立に欠陥の可能性・再測から）:
> **⒜ 添付形でペダル括弧が音節を貫通する**（同一譜に @sustainOn＋歌詞）／
> **⒝ `@f` が音節に重なる**（添付・行の両形で「ho」に f が乗った）。
...
| 決定 | 根拠（要点） |
|---|---|
| ★★★ **`with lyrics`/`with chords` は除去し、score は「帯の縦列」に統一する**（2026-08-19・第215・**ユーザー決定**・**次便着手・v0.3.0 のタグはその後**） | ユーザー起案「縦に積むだけで意図する楽譜を書けるはず」。関連は定義（`sings`）・位置は並び順・吸着は staff-affinity＝**LilyPond 自身のモデルに収束**（LP に `with lyrics` は無い）。構文破壊なので**初タグ前が唯一の無料期間**——タグより先にやる。設計と残作業 4 点は §2F ▶▶ |
