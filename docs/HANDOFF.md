...
**終了時**: **commit 2 本**（`e7ffe059`＝製品 4〔`RenderSpecParser.cs`／`DuplicateScoreNameValidator.cs`／`LilySharpLanguageServer.Commands.cs`／`LspProtocolDtos.cs`〕＋拡張 1〔`editors/vscode/src/extension.ts`〕＋番人 1〔`LilySharp.Tests/Lsp/ScorePickerTests.cs`〕＋`CHANGELOG.md`／この HANDOFF＋ARCHIVE＝第339 の経緯を ARCHIVE の先頭へ逐語）・**未 push 28**・木 clean・未追跡 0。⚠️ **配布はしていない**（`pwsh tools/Deploy-Lsp.ps1` ＋ Reload Window はユーザーの判断＝今動いている LSP を落とすため）。

## 以下は第340セッションの経緯

最終更新 第340セッション＝**入り方が第298〜第339 と違う**（ユーザーが `docs/HANDOFF.md` と `scratch/ベースタブLy/bench.lys` を読ませて「**`chords prog` の `Cmaj7` と `@chord` の `F♯sus4` の y を揃えて。x が重なった場合に限り縦に積んで、そうでない場合には y を揃えて**」と*的を名指し*・background job）。⚠️ **第334 と同じ本・同じ 2 小節**（あちらは X の余白・こちらは Y）。**製品 6 ファイル・番人 7 本＝`704ce303`（✅ ユーザー承認「全て承認する。コミットして」を得てから commit・RULES §5.1）**。**骨は 10**。
⚠️⚠️⚠️ ★★★★ **この便の形そのものが教訓**: **ユーザーが便中に 6 回口を挟み、そのうち 3 回は「私が緑にした実装の*絵が違う*」という指摘だった**（向きが逆・帯が空・既存の本が壊れた）。**番人も full も掃きも毎回緑だった。** ⇒ ★★★ **「揃った／積んだ」を測る番人は*どちらが動いたか*を測らない。読み手は絵を見る。** 下 ⑷⒟・⑸・⑹ が 3 件の中身:

★★★ **⑴ 再現**: `bench.lys` を `lysc svg` すると **`Cmaj7`（chords 行）は y 11.70・`F♯sus4` と `Emaj7/D♯`（inline `@chord`）は y 14.68**＝**2 本の線**（`scratch/p340/bench.svg`）。**X は重なっていない**（`Cmaj7` 8.70 で幅 ~7.1・`F♯sus4` は 21.89）ので、報告どおり揃うべき配置。

★★★ **⑵ 正体＝2 つの経路が 2 つの帯を持っている**（`ChordNameEngraver.Calculate` の 2 本の腕）: **行**（`IsChordRow`）は**自分の staff（テキスト行）を持ち**、`YUp = −(band top + RowTextBaseline)` で置かれ、**loose-line の解**（`LayoutEngine.ApplySolvedRowPositions`）が最後に動かす。**inline `@chord`** は staff を持たず `YUp = −(0.6 + linePeak) + staffOffset`＝**譜の上 0.6＋出っ張り**。⇒ **両者が同じ本に居ると必ず違う y になる。** ⚠️ **LP にこの配置は綴れない**（`ChordNames` context が唯一の綴りなので 2 種類は必ず 2 本の線）＝**LILYSHARP-OWN の規則を 1 つ決める仕事**であって移植ではない。

★★★★ **⑶ 直し＝「その記号はどの*線*に刷られるか」を 1 つの札にし、規則を 1 か所に置いた**（製品 6 ファイル・`704ce303`）: **⒜ `ChordNameLayout` に `RowStaffIndex`**（行の記号は自分の行・行に載った `@chord` はその行・それ以外は −1）。**⒝ `ChordNameEngraver.ChordLineOfSystem` が規則の唯一の家**＝**⑴ `@chord` は行の baseline に載る／⑵ 行の記号のうち `@chord` の箱＋`SymbolGap` に触れるものだけが*持ち上がる*／⑶ 持ち上げ幅は 2 つの記号の*インク*から出し、**線ごとに 1 つ**（下 ⑹⒜）／⑷ **共有できるものが 1 つも無ければ何もしない**（下 ⑹⒝）。**新しい定数は 1 つも足していない。** **⒞ どの行かは `ChordRowAboveStaff`＝*staff 表*から**（解いた Y からではない＝配置と部屋の門が同じ線を名指せる・行が黙っている段でも同じ答え）。**⒟ 譜は自分の和音帯を返す**（`StaffKeepsItsOwnChordLine`・下 ⑸）。**⒠ 下流 4 軒の門が `IsChordRow` から `RowStaffIndex >= 0` ／ `ChordRowAbove` に変わった**——`ApplySolvedRowPositions`（**行が動けば載った `@chord` も動く**）・`OutsideStaffStacker` の inline seed（**行の線に載った記号は*その譜の*上インクではない**）・`ChordRowSupport`（**volta と mark はそれを避ける**）・`RowSkylines`（**載った `@chord` と持ち上がった記号が*行のインク*になる**＝帯を返した以上、部屋を主張するのはこれだけ）。

⚠️⚠️⚠️ ★★★★ **⑷ ユーザーが便中に 4 度足した。3 つは同じ規則に落ちたが、4 つ目は*向きが逆*だと言っていた**:
**⒜「`score {}` に複数の `chords` を積んだら必ず y を違えた方がよい。妥当な実装になるか」** ⇒ **なる。行の記号は行から降りない**（動くのは `@chord` と、持ち上がる行の記号だけ）。**実測**（`tworows.lys`＝`as roman` の上に `as names`）: **`Imaj7` 11.66 / `Cmaj7` 14.76 / `@chord` 2 つとも 14.76**＝**行は 2 本のまま・`@chord` は*一番近い*行に載る**。
**⒝「`@chord` は、どこか任意の `chords` で未記載の小節があればそこにはめて描く」** ⇒ **箱の規則がそれを含んでいる**（記号の無い小節には触れる相手が居ない）。**小節単位の規則にはしていない**——⒞ がその理由。
**⒞「一拍目と三拍目のような場合も y が揃うように」** ⇒ ★★★ **これが「小節」ではなく「箱」で測る理由**。**小節単位だと同じ小節の 1 拍目と 3 拍目は*書かれた小節*なので落ちる**。**実測**（`beats2.lys`）: **両方 11.70＝1 本**。
⚠️⚠️ **⒟「下の帯に `Cmaj7 F♯sus4 Emaj7` が並び、`Dmaj7` だけが上にスタックしてほしい。実際は `Cmaj7 Dmaj7 Emaj7` が上で `F♯sus4` が下。見にくい」** ⇒ ★★★★ **1 度目の実装は*積む向きが逆*だった**（行が線を保ち `@chord` が下へ降りた）。**読みの 3 記号が 2 本の線に割れる**。**正しい向きは「譜に近い線が共有線・持ち上がるのは*行*の記号」**——**`@chord` はその音符が実際に綴る和音を名乗るので音符の傍を離れない／行の記号は名目上の和音なので浮かせられる**。**実測**（`bench.lys`・下 11.99 に `Cmaj7 F♯sus4 Emaj7`・上 8.60 に `Dmaj7` だけ）。
⇒ ⚠️⚠️ ★★★★ **教訓: 1 度目は番人 6 本も full も全部緑で、それでも絵が違った。**「揃った／積んだ」だけを測ると*どちらが動いたか*を測らない。**⇒ 番人は「差がある」ではなく「*どの記号が*動いたか」を表明すること**（`RowSymbolOnAnInlineChordsColumn_IsTheOneThatLifts` がその形）。

★★★ **⑸ 2 つ目の報告＝*部屋*の半分**（⒟ の 1 つ前）: 「y は揃ったが**コード名の下に無駄な余白**がある。上段に名前・下段が y を予約したまま空欄」。**正体は `MultiStaffLayouter.ReserveChordRowBand`**——**譜が「自分の和音行」のために上に予約する帯**で、記号が行へ上がっても予約は残る。⇒ **`StaffKeepsItsOwnChordLine`（行が上に在り・`with chords` トラックが無ければ false）で帯を返す**。**実測**: `bench.lys` の頁 **27.56 → 24.85 ss**・譜が 2.12 上がる。⚠️ ★★ **これは私が 1 度目の remark に「名前を付けたがやらない」と*書いた*項だった**——**読み手はその remark を読まない。絵を見る。**

⚠️⚠️ ★★★★ **⑹ 3 つ目の絵で*既存の本*が壊れた（`scratch/site-showcase/chord-axes.lys`＝「1 つの進行を 4 通りに綴る」showcase・3 行＋全音符に `@chord`）**——**掃きで MOVED に出たので中を見て気づいた**。**2 つ欠陥が出た**:
**⒜ 持ち上げ幅を記号ごとに出すと*ガタつく***: 4 つの名前が **18.50 / 19.11 / 19.02 / 18.50** に散った。⇒ **線ごとに 1 つの段**（必要な最大）にした。
⚠️⚠️ **⒝ 全部が衝突する本では、併合しても*何も共有されない***: `chord-axes` は `@chord` が**行の記号と全部同じ列**なので、併合すると**行が丸ごと上へ**＝**元と同じ 2 本の線を、ガタついて描き直しただけ**（しかも 2 行目に食い込んだ）。⇒ ★★★ **「共有される記号が 1 つも無いなら併合しない」**（`ChordLineOfSystem` の `Merged` false＝**その段は元のまま・帯もそのまま**）。**結果 `chord-axes` は base と*バイト同一*に戻った**（頁 36.48・4 本の線 15.42/18.52/21.62/24.59）。
⇒ ★★★ **規則の目的は「1 本の線に*載せる*」ことであって「積み方を変える」ことではない。載せられないなら本に触らない。**

★★ **⑺ 番人 7 本 `ChordRowInlineAlignmentTests`**（⚠️ **観測者が 1 冊も居ない**——`@chord` と `chords` ブロックを両方持つ追跡 `.lys` は**木に 1 冊も無い**）: 行の線に載る／**同じ列なら*行の*記号が持ち上がる**（⒟ の形・X が同一であることも表明）／**全部衝突なら併合しない**（⑹⒝ の形）／1 拍目 vs 3 拍目／積んだ 2 行／**部屋（`@chord` の有無で譜間距離が動かない）**／**陰性対照（行の無い本＝この島が 1 行も走らない・`RowStaffIndex` −1・0.650000）**。⚠️ **毒 2 つ**: 「持ち上げない」で **`IsTheOneThatLifts` だけ赤**・「常に帯を予約」で **`BooksNoBandForItsOwnChordLine` だけ赤**＝**2 つの半分が別々に測られている**。
⚠️ **罠 2 つ**: **⒜ 陽性対照を staff 相対で測る**（行を落とすと系から 1 本消えるので `YUp` は 0.650000 と −5.870344 に化ける）。**⒝ 2 小節の本では持ち上げを測れない**——**`@chord` 1 個がその 1 個だけ衝突する本は ⑹⒝ の「併合しない」側**なので、**3 小節（衝突する 1 個＋抜けている 1 個）が要る**＝`bench.lys` 自身の形。

★★ **⑻ 射程＝`scratch/p340/sweep340c.ps1`（母集団を*名指し*・下 ⑽⒞）**: **母集団 923 冊**（target 6 ＝ `@chord` と `chords` を両方持つ本／control 917 ＝ 追跡 `.lys` 597 ＋ ユーザー実コーパス）・**svg は全数・midi/xml/ly/check は 156 冊の標本**（target 6 冊全部＋種を固定した抽選 150）・**32.7 分**（`scratch/p340/sweep340d.txt`・csv は `sweep340c.csv`）:
**svg MOVED 4 / SAME 919**——**動いたのは `bench.lys`・`overlap.lys`・`tworows.lys`・`beats2.lys` だけ**。**`chord-axes.lys` と `beats.lys` は ⑹⒝ の「併合しない」で*バイト同一に戻った***。**midi / xml / ly / check は標本 156 冊すべて SAME**（target 本ですらバイト同一＝この変更は縦の配置しか触っていない）。**control は 1 冊も動いていない。**
⚠️ **落とした集合の名前**: `scratch/` の使い捨てプローブ（ディスク上の `.lys` は 3482 冊ある）。**script の頭にそう書いてある**——「全部測った」と書かないため。

★ **⑼ 数**: full **7172 / 0 / 4 / 7176**（`scratch/p340/full4.trx`・1 分 57 秒・**+7＝番人**・第339 の 7165 ちょうど＋7）・`--no-incremental` build 0 エラー・Core 0 警告。**snapshot 247・追跡 `.lys` 597・台帳 800／exact 632＝全部不動**。生成物 2 つ（`docs/APPROXIMATIONS.md`・`audit/magic_constants.csv`）は **`LILYSHARP_UPDATE_DOCS=1` で再生成済み＝行番号だけ**。
★ **開始時裏取り**: HEAD **`e9bfef7b`**・**未 push 17**・木 clean・未追跡 0・`bin\Debug` は net10.0 のみ。
**終了時（第 1 便）**: **commit 2 本**（`704ce303`＝製品 6＋番人 1＋CHANGELOG＋生成物 2／`99a2cc9e`＝この HANDOFF＋ARCHIVE＝第338 の経緯を ARCHIVE の先頭へ逐語）・**未 push 19**・木 clean・未追跡 0。**snapshot・台帳・追跡 `.lys` は全部不動**（動いたのは ⑻ の 4 冊＝すべて未追跡）。
**終了時（第 2 便）**: **commit 2 本**（`f4d64db4`＝`--batch`（CLI 2＋番人 1＋CHANGELOG＋CLI_REFERENCE）／この HANDOFF）・**未 push 21**・木 clean・未追跡 0。**製品の絵は 1 バイトも動いていない**（⑾⒞）。

⚠️⚠️ ★★★★ **⑿ 第 3 便＝「出荷する lysc を R2R に」はユーザーが選んだが、*既に入っていた*（`32216700` は別件）**: **`.github/workflows/release.yml:59-65` が 4 つの RID すべてで `--self-contained -p:PublishReadyToRun=true`** で publish していて、**測定つきの決定がコメントに書いてある**（single-file+compressed 1694 ms／素のディレクトリ 1184／ディレクトリ＋R2R **862**＝「固定費の半分は JIT ではなく bundle」。**single-file の下では R2R は逆に悪化**＝`--version` 545 → 1093 ms・ペイロード 36.8 → 50.7 MB）。LSP 側（`:161-167`）も同じ。⇒ ★★★ **私が「次の一手」に書いたのは閉じている項目だった——出荷形態を 1 度も確認せずに書いた**（§5.2 の stale の族・**自分で書いた次の一手にも当てはまる**）。⚠️ **本便で測った「`check` は R2R で 245 → 363 ms と遅くなる」は like-for-like ではない**（`bin/Release` の build 対 `publish` の R2R＝build と publish の差が混ざっている）。**publish 同士で 11 コマンド測り直すのが残っている問い**だが、ユーザーの判断で保留。
★★★ **⒜ 代わりに閉じたのが CLI_REFERENCE の同期**（ユーザー選択）。**「古い」ではなく数えた**（`scratch/p340/docsync.ps1`＝`lysc <cmd> --help` を全コマンドに打ち、options を正規表現で拾って doc と突き合わせる）: **11 中 4 コマンドが節ごと無い**（`vsqx` `ly` `import` `harmonize`）＋ `svg --all/--combined`・`png --crop`・`midi --score/--all`・`xml --score/--all`・`check`/`layout` は options 表そのものが無い。**どれも「CLI に足したが doc を触らなかった」**——**ヘルプは Program.cs の生文字列 14 か所・doc はその隣の散文・両者を繋ぐものが 1 つも無かった。**
⚠️⚠️ ★★★★ **⒝ 番人の 1 版目は効いていなかった。毒だけがそれを言った**: 「その節のどこかに現れる」で書いたら、**`--combined` の表の行を消しても緑**——**下の Examples 行に同じ字面が在る**から。⇒ **表の行だけを見るように締めた**（失敗メッセージが「表に行を足せ」と言っている以上、見る場所もそこ）。**毒り直して赤・`svg: --combined` と名指す。** ⇒ ★★★ **番人を書いたら毒る。「書いた」と「効く」は別。**
★ **⒞ ヘルプが正典**（doc ではなく）＝**端末で読まれる側で、parser が受ける flag について嘘を書けない**（書けばコマンドが目に見えて落ちる）。**doc がヘルプより*多く*語るのは自由**で、そこは落とさない。**グローバル オプション（`--batch`／`--verbose`）はどのコマンドの Options 節にも出ない**ので 3 本目の番人で別に見る。**full 7181 / 0 / 4 / 7185**（`scratch/p340/full7.trx`・2 分 57 秒・**+3**）・製品コード 0。
**終了時（第 3 便）**: **commit 2 本**（`32216700`＝CLI_REFERENCE＋番人 1／この HANDOFF）・**未 push 23**・木 clean・未追跡 0。

⚠️⚠️ ★★★★ **⒀ 第 4 便＝⒜⒝ 両方（`48396648`）。⒝ は*動いたが割に合わない*と分かったのが収穫**:
★★★ **⒜ 掃きの追跡半分を `--batch` に載せた＝11.1 分 → 2.6 分（4.3 倍・MOVED どちらも 0/597）**（`scratch/p340/sweep340f.ps1` 対 対照 `sweep340g.ps1`＝同じ本・同じ exe・同じ並列度 10 で「1 冊 1 プロセス」を測った）。⚠️ **対照を取るまで「速くなった」と言わない**——1 度目は base 側が `-j` を知らずに全冊エラーし、**base パス 0.3 秒・`MOVED 2`** という*もっともらしい嘘*を出した（**0.3 秒に気づかなければ通っていた**）。⚠️ **実コーパスは載せない**（§2 U1＝掃き中に編集されるので base/head を本ごとに交互に取る規則が要る。バッチは build ごとに 1 パス）。
⚠️⚠️ ★★★★ **⒝ `-j N`（1 プロセス内並列）は動くが `svg` には割に合わない**（200 冊・16 プロセッサ・実測）: **`check` 0.6 → 0.7 s（1.0 倍・並列化する仕事が無い）／`ly` 8.5 → 3.8 s（2.2 倍・CPU 6.5 → 24.1）／`svg` 9.7 → 7.6 s（1.3 倍・CPU 9.1 → 37.6）**。⇒ ★★★ **天井は描画側の*プロセス全体の shaping ロック***（HarfBuzz の shaping は thread-safe でないので `TextFontMetrics` が font を掴む）。**コマンドを分けて測って切り分けた**（parse だけの `check`・書き出しだけの `ly`・描画する `svg`）。⇒ ★★★★ **プロセスを分ければロックも分かれる＝「複数の `--batch` プロセス」が正解で、`-j N` ではない。** **ヘルプ・CLI_REFERENCE・CHANGELOG の 3 つとも表ごと載せた**（黙って期待を下回るフラグは、無いフラグより悪い）。
★★ **⒞ `pdf` は `-j` を拒否**（理由つき）。**`FontEmbedInfo` の 2 つのキャッシュを ConcurrentDictionary に**——⚠️ **これは投機ではない: 素の `Dictionary` に lock 無しで書いていて、テストは第315 から並列**（競合書き込みは corrupt か無限ループになりうる）。**Core に残っていた非同期の static はこの 2 つだけ**（export 経路の全 mutable static を監査＝他は全部 content-keyed な `ConcurrentDictionary` か score/tree ごとの `ConditionalWeakTable` か lock 済み）。
⚠️ **⒟ 1 文書の*中*の並列化は依然禁止**: `PageLayouter` の対距離メモが**2 フィールドの check-then-write を無同期**でやっている（skyline の参照 identity がキー）。**文書をまたぐなら別オブジェクトなので裂けない**が、1 文書を 2 スレッドで組むと裂ける。`Batch.cs` の remark に名指した。⚠️ **LSP は PDF の同時書き出しに対して無防備**（今は単スレッドなので出ない）。
★ **⒠ 数**: full **7183 / 0 / 4 / 7187**（`scratch/p340/full8.trx`・2 分 15 秒・**+2**）・build 0 エラー・Core/Cli 0 警告。**並列と逐次の出力はバイト一致**（番人 ＋ 追跡 597 冊で `-j 1` と `-j 0` を比較）。
**終了時（第 4 便）**: **commit 2 本**（`48396648`／この HANDOFF）・**未 push 25**・木 clean・未追跡 0。

⚠️⚠️⚠️ ★★★★ **⑽ 掃きの費用の形を間違えた（ユーザーから「フルテストが 6 時間超は許容できない」と指摘・*テストではなく私の掃き*だった）**: **full test は 2〜5 分**（並列化は第315 のまま生きている＝`xunit.runner.json` の `maxParallelThreads: 0`）。**6 時間走っていたのは `sweep340.ps1`**。**⑴ 母集団をディスク上の全 `.lys` で取って 3482 冊**（過去便の掃きは 1519 冊。増えた分は `scratch/` の使い捨てプローブ）**× 5 出力 × base/head ＝ 34,820 起動**・**⑵ しかも Debug の exe**。**41.2% で打ち切り**（位置は走っている `lysc` のコマンドラインから割り出せる）。
★★ **⒜ 空いた機械での固定費**（median of 7・`tiny.lys` は 1 小節）: **`lysc svg` 1352 ms・`check` 245 ms・`svg -n` 1049 ms**（フォント埋め込みが ~300 ms）。**`bench.lys`（3 頁）2390 ms** ⇒ **実仕事 ~1.0 s・残りは全部固定費**。
★★ **⒝ ReadyToRun publish**（`-p:PublishReadyToRun=true`・第329 が LSP に当てたのと同じ手）: **`svg` 1352 → 838 ms（−38%）**。⚠️ **`check` は 245 → 363 ms で*遅くなる***ので「R2R で速くなる」と一般化しないこと。
⇒ ★★★★ **⒞ 掃きの費用は「冊数」ではなく「`lysc` の起動回数」で決まる**。**`sweep340c.ps1` はそれで組み直した**: 母集団を*名指し*（target ＋ 追跡 `.lys` 597 ＋ ユーザー実コーパス。**落としたのは `scratch/` の使い捨てプローブ**——落とした集合の名前を script に書いてある）・**svg は全数／他 4 出力は 156 冊の標本**（種を固定）・exe は R2R。**3,094 起動**。
⚠️ ★★★ **⒟ 隣で掃きを回しながら測った数は「効き」ではなく「取り合い」**（第315 の教訓の再演）——**同じ exe・同じ本が 9.5 s と 1.35 s になった**。**計器を当てる前に機械を空けること。**
⇒ **残る無駄は「1 冊 1 プロセス」だけ**（固定費 ~1.3 s × 3,094 ≒ 67 分ぶんのスレッド時間）。**消すには `lysc` にバッチ入力（1 プロセスで N 冊）が要る＝製品の変更**——**✅ ユーザー承認を得て第 2 便で入れた（下 ⑾）。**

✅ ⚠️ ★★★★ **⑾ 第 2 便＝`lysc --batch <list>`（`f4d64db4`・ユーザー承認 2 件目「2 をやって」）**: **`--verbose` と同じくディスパッチの*手前*で取って剥がす**ので、**全サブコマンドに一度に効く**（何も書かない `check`・`layout` も含む）。**オプション配列はそのまま渡して各冊の positional だけ足す**＝**バッチの意味と単発の意味が定義上ずれない**（オプションの第 2 の解釈を持たない）。リストは 1 行 1 冊・`#` と空行を飛ばし・**TAB**（空白ではない——実コーパスのファイル名に空白が在る）で出力を名指す・`-` で標準入力。**`-o` とは排他**（1 つのパスは多数を名指せない）。**1 冊落ちても止まらない**（生きたコーパスは parse しない本を含む・止めると残り 900 冊が無駄になる）が**終了コードに出る**＝**バッチが N 回の run と違う唯一の点で、意図的**。
★★ **⒜ 効きは実測、しかも母集団で桁が変わる**（同じ機械・`svg -n`・出力はバイト一致を確認）: **小さい fixture 120 冊 995 → 45 ms/冊＝22.2 倍**／**実コーパスのタブ本 40 冊 1149 → 231 ms/冊＝5.0 倍**。**除かれる固定費はどちらも ~950 ms/冊で同じ**で、**倍率が違うのは実仕事の量の差**。⚠️ **片方だけ書くと過大（過小）に言うことになる。両方書く。**
★★★ **⒝ 唯一の危険は測って潰した＝PDF のフォント リゾルバ**（`PdfDocumentContext` の頭注が「PdfSharpCore はプロセスに 1 度しか設定できない／`fonts { }` の違う 2 文書は互いの face を上書きする」と*自分で*書いている）。**1 ファイル 1 プロセスの間は到達不能で、`--batch pdf` が初めて到達可能にする。** ⇒ **漏れない**（`fonts { title "TeX Gyre Heros" }` の本と素の本を**両方の順で**バッチしても、単独で埋める face と完全一致）。番人 `CliBatchTests`。⚠️ **陽性対照を*先に*表明する**——2 冊が単独で同じ face なら計器は漏れを見分けられず、緑に意味が無い（**最初の probe がまさにそれで、`fonts { serif … }` では face が変わらず「全部違う」と出た＝サブセット接頭辞 `ABCDEF+` を落とし忘れた計器**）。⚠️ **PDF はバイト比較できない**（作成日時・2 回の run が不一致。PNG は一致）。
★ **⒞ CLI は他に 1 バイトも動いていない**（`clineutral.ps1`＝この commit の直前の build と本 build で fixture 120 冊を 1 冊 1 プロセス: **svg 0/120 差・`check` は stdout と終了コードが 0/40 差**）。**番人 6 本 `CliBatchTests`**（バッチ＝単発のバイト一致／PDF の face／落ちても続く／コメントと TAB／`-o` 排他／`check` でも効く）。**full 7178 / 0 / 4 / 7182**（`scratch/p340/full6.trx`・2 分 12 秒・**+6**）。
⚠️⚠️ ★★★ **⒟ この便の掃き自身は `--batch` に*していない***: **base と head を本ごとに続けて取る**規則（§2 U1 で実測した「掃きの最中にユーザーがコーパスを編集して偽の差が出る」）と、**バッチ＝build ごとに 1 パス**が両立しない。**追跡コーパスは編集されないのでバッチにできるが、実コーパスの半分は交互のまま残す**必要がある。**名指しただけで、やっていない。**
⚠️ **⒠ バッチは逐次**。描画パスの `FontEmbedInfo.ShapeCache`／`Cache` は**素の `Dictionary` を lock 無しで書く**ので 1 スレッドなら安全・複数なら安全でない。**プロセス内の並列化はそこを先に直す必要がある**（呼ぶ側が複数バッチを並べるのは自由）。

⇒ ★★★★ **次の一手**: ✅ **⑴ この便の製品は*承認を得て commit 済み*（`704ce303`・ユーザー「全て承認する。コミットして」）。作業ツリーに未 commit の製品は無い。**
✅ **⑵ `lysc --batch`＝第 2 便（⑾・`f4d64db4`）／✅ CLI_REFERENCE の同期＝第 3 便（⑿・`32216700`）。** ✅ **⒜ 掃きの追跡半分を `--batch` に（第 4 便・11.1 → 2.6 分）／✅ ⒝ `-j N`（第 4 便・ただし `svg` には割に合わないと*測って*分かった）。** ⇒ **lysc 側で残るのは 1 つだけ**: **shaping のロックを外す**（`TextFontMetrics` が HarfBuzz font を掴む＝`svg -j N` の天井）。**スレッドごとの font が要り、第317 が `0xC0000005` と戦った native ハンドルの寿命の話に戻る**＝別の島・重い。**やる前に「誰がそれを待っているか」を先に決めること**（今は掃きが複数プロセスで足りている）。
★★★★ **⑶ ここが次便の 1 手目＝第339 の路線（頁の島）**——**残るのは 1 つ: breaker が 1 行を値付けする 3 つの数（`full_height` / `tallness` / `inverse_hooke`）**。⒜ Lily# 側は取ってある（第339 ⒜ の表）・⒝ **LP 側は `ragged-bottom = ##t` の `yoff` 差分から引く**のが一番安い（第339 ⒝(iii)）。⚠️ **`⒁` の「最小の枠」は着手の根拠にしない**（第338 ⒂⒜＝この本では inert）。
⚠️⚠️ ★★★ **第340 はこれに*着手しなかった*（ユーザー「次便がこの便で有利なら着手して。有利でないなら着手してはいけない」→ 有利でないと判断）**。**理由は第339 第 4 便とまったく同じで、そのとき書いた一行がそのまま当たる**: 「**LP 半分は設計仕事が要り、それはこの便の文脈の重さより新しい頭のほうが安い**」。**第340 の文脈はさらに遠い**——和音記号の Y・CLI の引数解析・バッチ並列・コンソール横取り・フォント キャッシュ・掃きの費用で埋まっていて、**頁の島に効く暖まりは 1 つも無い**（要るのは LP の probe・Boogie の本・`Line_details` の読みで、全部冷えている）。**温かいのは R2R の exe と掃きの道具だけで、それはこの島が使わない物。**
⑷ 以下は第339 の並び（`lysc ly` の ChordNames＝ユーザーと綴りを決めてから／lyric row の slot の投票／ps2 の 0.017・beam-over-stem bar 1 の +0.33／push 後 `gh run list`／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／A/B/D/E／C⑴／G）。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
