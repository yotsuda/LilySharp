...
最終更新 第210セッション＝**部品ヘッダの値検証を入れた便**（**実装 4 便**）。
**perf は §7.9 に従って数で測った**（下の ⑹）——**検証は打鍵ごとに走る `PublishDiagnostics` の経路**なので。
⚠️ **彫版のコードは 1 式だけ動いた**（`DrawingTransform.Identity`・下の ⑷）——**⑴〜⑶ は 1 行も触っていない。**
**その 1 式は*誰も読んでいなかった*ので出力は不変**（**snapshot は 218 枚で不動＝中身も 1 枚も動いていない**）。
⚠️ **「彫版を触っていない」と丸ごと書かないこと**——**この便は 3 便それを書いてから 4 便目で触った。**
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **§1 ⑹ ⓪ が「まず 0.3.0 の判断」だったので裏取りのあと訊いた**
（**前便の延期理由「文法を続ける」が第209 で一区切りついたので、同じ設問でも答えが変わりうると添えた**）。
**ユーザー決定は「⒜ §2F の新チケット 2 件を決める」**（選択肢は ⓪ 0.3.0 へ舵／⒜ チケット／
⒝ 小さい残債／⒞ ▶ perf、の 4 つ）。
⇒ ★★★ **0.3.0 を出すかは*まだ決まっていない*。次便も同じ設問から始まる**（下の ⑺）。
⇒ ★★ **そして 2 件目の設問「5 つをどうするか」にユーザーは「よくわからない、お勧めは？」と答えた**
——**推薦を 1 つ出して着手した**（**⒜ 5 つとも error**。理由は下の ⑴）。

★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 529 点・ss 非ゼロ 100・
総和 3.794108828・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 52**・
**snapshot 218 枚**・suite **5552 passed / 0 failed / 4 skipped**・**追跡コーパス 567 冊**・
**Core 0 警告**＝**引継ぎの数は全部合っていた**（**39 回連続**）。
⚠️ **`origin/master` は `9830873f`＝開始時と同じなので、便の途中の push は無かった**。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 60**（**この行を書く commit を含めて 60**。
⚠️ ★★★ **1 度ここを間違えた**——**55 と書いたが、それは引継ぎ commit の*前*に数えた値で、
括弧では「この commit を含む」と主張していた**。**§0 が何度も書いている形をこの便自身が踏んだ**ので、
`07a5487f` で訂正した。**数えるのは commit の*あと*。**）・
suite **5552 → 5576 passed / 0 failed / 4 skipped**（**+24**）・
**台帳 529 点で不動**（ss 非ゼロ 100・総和 3.794108828・count 106/2 も不動）・
**snapshot 218 枚で不動**・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Turmeric`）で通せた。**

> ## ★★★ この便の骨＝**チケットが数えていたのは*事例*で、*族*ではなかった**
> **起票は「部品ヘッダの 2 つの値が検証されていない」で、その 2 つは第209 が実測して裏取りしていた。**
> **着手時に validator の `switch` を数えたら、検査の枝が在るのは 9 property のうち 4 つ**
> ——**無検証は 5 つ**（`removeEmpty`・`lines`・`octave`・`transpose`・`transposition`）。
> ⇒ ★★★ **2 つだけ塞いでも報告は「消える」のではなく「移動する」**——
> **第209 が*色づけ*で書いた骨と同じ形が、1 便あとに*検証*で出た。族は 1 つで顔が 2 つあった。**
> ⇒ **§5.0 罠22 に汎化して置いた**（「族の大きさは*検査する側*を数えて出す」）。
> ⇒ **網もその形にした**: `EveryPartProperty_RefusesAValueItCannotRead` は**公開語彙の上を回る**ので、
> **検査を持たない property を後から足すと赤で着く**（5 本並べるだけだと 6 つ目は誰も見ない）。

> ## ★★★ 2 つ目の骨＝**「読み手に訊く」は原則だが、読み手が言語より甘いことがある**
> **4 つの検査は読み手そのものに訊いた**（`transpose` は `PartTranspose.ReadProperty`・
> `lines` は `StaffSpec.MinLines/MaxLines` を `RenderSpecParser` と共有）ので**語彙の 2 つ目の綴りは 1 つも増えていない。**
> ⚠️⚠️ ★★★ **`transposition` だけは*わざと*読み手に訊いていない**——
> **`ParseTranspositionSemitones` は引数を小文字化するので、訊くと `8VB` を*受理する*。**
> **初稿は実際にそう書いて、この便が拒もうとしていた唯一の綴りを通した**（probe で気づいた）。
> ⇒ ★★ **しかも同じ初稿のコメントは「これで 8VB は拒まれる」と主張していた**——
> **commit していたら、コードとコメントが逆を言う状態が入っていた**（第209 の ⑸ と同じ族）。

> ## ★★ 3 つ目の骨＝**doc が 3 箇所で同じ嘘を綴っていて、どれも機械に縛られていなかった**
> **`GRAMMAR.md` は `octave` が `( 'absolute' | 'relative' | Integer )` を取ると書いていた。**
> **その 2 語は `OctaveDecl`（最上位・section の directive）のもので、部品ヘッダでは
> *何もしない*——変更前の木で測って MIDI が「octave 無し」とバイト同一**
> （⚠️ **対照 `octave 2` vs `octave 5` が*違う*ことを先に示してから測った**——
> **最初 SVG で測って両方 IDENTICAL になり、計器が盲だと分かった**。
> ⚠️ **SVG は `data-pos` にソースのバイト位置を埋めるので、字数の違う対照とは必ず差が出る**）。
> ⇒ **同じ嘘が LSP の補完文脈と KeyCompletionTests の 1 行にも在った**——**理由はどこにも書かれていない。**
> ⇒ ★★★ **エディタは*コンパイラが拒む語*を提案していた**ので、`IsInsidePartBlock` で門を付けた。
> ⇒ **doc の inline な値の羅列を名前付き production にして `DocKeywordListTests` に縛った**
> （`RemoveEmptyValue`・`PedalStyleName`）——**縛りが無い alternation は、誰も見ないから腐る。**

★ **この便の値段**（**彫版のコードは 1 行も動いていない**。**動いたのは validator・レクサー・
LSP・doc・テストだけ**。**台帳 529・snapshot 218 とも不動。**）:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ 無検証の値 5 つ ＋ レクサーの ottava 大小 | Core 5 ＋ doc 1 ＋ 網 3（+21 本） | **部品ヘッダを書く本すべて**（追跡 503 宣言） |
| ⑵ `octave` のモード語（doc・補完・網） | LSP 1 ＋ 網 2 | **エディタのみ**（出力不変） |
| ⑶ `1..5` の出所注記 | Core 1 | **注記のみ**（挙動不変） |
| ⑷ `DrawingTransform.Identity` が単位変換でなかった | Core 1 ＋ 網 3（+3 本） | **出力不変**（snapshot 218 不動）。**観測は 0 本 → 562 本** |

- **⑴ `b6482657`**。**ユーザー決定＝5 つとも error**（warning でも文言だけでもなく）。
  **推薦の理由 3 つ**: **⒤ 同じヘッダで重さを 2 種類にしない**（`clef`/`tuning`/`pedal`/`instrument`
  は最初から error）／**⒥ 今日の挙動は「黙って既定に落ちる」＝書き手に嘘をつく側**
  （`lines 9` は 5・`transposition banana` は 0 半音）／
  **⒦ ★ 決定的な非対称は*時期***——**タグは 1 つも無く 0.3.0 は未リリースなので、
  厳格化が破壊的変更にならない最後の機会。**
  - ★ **検証の形は「読み手が読めることだけを要求する」に統一**（勝手な範囲を発明しない）。
    **`octave` に上下限を入れなかったのはそのため**——**`octave 99` は*読まれて使われる*ので、
    間違っているのは「数でないこと」だけ。**
  - ★ **射程は実測**: **追跡 567 冊の全数 `lysc check` でエラー 1 冊**、
    **それは既存の `repeat volta` probe**（`audit/lpreg/voltagrace-probe.lys`）＝**新検査で落ちた本は 0 冊。**
  - ⚠️ ★★ **起票の射程の数は raw grep だった**（コメント中の語を含む）。**part ヘッダの中だけを
    brace 対応で数え直すと `removeEmpty` は `true` 13/`all` 1（5 冊）・`lines 1` は 2 件（2 冊）**
    ——起票は「`true` 14・`all` 2」「`lines 1` が 5 件」。**どちらの数え方でも違反は 0 冊。**
  - **レクサーの ottava 接尾辞を大小無視にした**——**受理する集合は 1 語も変わらない。**
    **`transposition 8VB` を拒む主体がレクサーから validator へ移り、診断が 3 行から 1 行になった**
    （**うち 2 行は誰も書いていない `VB` という property についてだった**）。
    **実測: 追跡 567 冊に「数字＋非小文字 ottava」は 0 site。**
  - **毒は 6 つ当てた**（5 つの枝＋レクサーの大小）——**それぞれ*別の*テストを赤にする。**
- **⑵ `2b66808b`**。**LSP の門は 1 行**（`IsInsidePartBlock` は既に在り、`instrument`/`removeEmpty` が
  *逆の向き*で使っている）。**既存テスト 2 本が古い挙動を名指ししていたので、実測に合わせて直した。**
- **⑶ `8c76075d`**。**§7 の項 7・7.5 を実際に走らせて出た**——**Core に 130 行足して出典注記 0。**
  **`MinLines`/`MaxLines` は*移した*だけだが、名前が付いた時点で初めて問える。**
  **答えは非自明**: **既定の 5 は LP のもの**（`scm/define-grobs.scm:3396`）だが、
  **範囲は LP に無い**（`Staff_symbol::calc_line_positions` は line_count 回まわすだけ）＝`LILYSHARP-OWN`。
  ⚠️ **初稿は `Staff_symbol::line_positions` と書いた（実在しない）**——**引用先を開いて直した。**
- **⑷ `6cbe39d9`**。**§2G の `DrawingTransform.Identity`**——**第208・第209・第210 が読んで見送った項。**
  ★★★ **着手の理由**: **ユーザーが「有利なら着手／不利なら着手するな」を*2 度*出した。**
  **1 度目は断った**（引継ぎを閉じ切った直後に開け直す不利のほうが大きく、
  **チケットは自己完結するよう書いてあった**ので次便が損をしない）。
  **2 度目に着手した理由は温度ではなく*スロット***——**次便は ⓪ 0.3.0 の分岐で開き、
  「出す」ならその便は丸ごとリリース作業になるので、この項は 4 度目の見送りになる。**
  **3 便が見送ったのは*理由があったから*ではなく*毎回もっと大きい仕事が走っていたから*で、
  いまその理由だけが消えている**（この便の冒頭で 0.3.0 の設問に使ったのと同じ論法）。
  - ★★★ **引継ぎの「1 行・出力不変」は正しかったが、*形*は 1 段深かった**——
    **`ScaleX = 1` は主コンストラクタの既定値なので `new(x, y)`（＝`Translate`）では効く。
    struct の `new()` だけが主コンストラクタを通らず全フィールドを 0 にする。**
    ⇒ **欠陥は「数の 1」ではなく「どのコンストラクタが走るか」**なので、
    **網は `Translate`/`Scale` も主張する**（症状ではなく形が見えるように）。
  - ★★ **本当の値打ちは §7.7 の匂いのほう**——**`Identity` を使おうとした 2 人が
    どちらも `new(0, 0, 1, 1)` を手書きし、「これを使うな」というコメントを添えていた**
    （片方は「もう 2 人捕まえた」と書いている）。**回避策が 2 つ目・3 つ目の綴りを産んでいた。**
    **直したのは property で、2 軒ともそれを読むようにした。**
  - ★★★ **だから*観測*が変わった**: **`new()` に戻す毒は、以前は何も赤にしなかった**が、
    **いまは 562 本赤になる**（実測）。**依存が増えたのではなく、3 つの写しが 1 軒に寄った。**
  - ⚠️ **`default(DrawingTransform)` は今も縮退したまま**（struct とはそういうもので、
    **尺度を 1 だけずらして持たない限り直せない**）。**そう書いた網を 1 本置いた。**
- **⑸ ★ この便が*起票した*もの**（§2F 末尾・**エディタのみ・出力不変**）:
  **LSP の値補完が Core の語彙の 2 つ目・3 つ目の綴りを持っている**
  （`GetRemoveEmptyCompletions` は `true/all/false` を自前で持つ＝**検証が入った今、ずれた瞬間に
  エディタがコンパイラの拒む語を提案する**／`Clefs` は 5 語しか無いので**11 語を取る部品ヘッダで
  6 語出せない**＝向きが逆で実害は小さい）。**第209 が閉じたのは色づけで、補完は数えていなかった。**
- **⑹ ★ perf の判定**（§7.9・**計算を足したので測った**）:
  **検証は打鍵ごとの `PublishDiagnostics` を通る**が、**足したのは既に訪れている `switch` への枝だけ**
  ——**新しい走査も `DescendantNodes()` も無い**。**回数で測った: 追跡コーパスで最も部品 property の
  多い本でも 7 個**（1 パスあたり最大 7 回の O(1)）。
  **レクサー側は呼び出し回数が不変**で、**変わったのは 1 回あたり最大 2 文字の比較の種類だけ**
  （`8`/`15` トークン最多の本で 8000 回＝`audit/lpreg/perf-fingbeam1k.lys`）。
  ⚠️ **⑷ は §7.9 の「足していない例」**（**`Identity` は 4 フィールドを書くようになったが、
  製品コードはこの property を 1 箇所も読まない**——**読むのは記録用コンテキスト 2 軒＝テストだけ**）。
  ★ **その 1 行は commit message に書き漏らした**——**§7 を 4 便目に通したのは便を閉じたあとで、
  そこで出た**（§7.7 の「無いと即答せず上から当てる」が 2 便続けて本物を出した形）。
- **⑺ ★★★ 次に触るならここ**——**⓪ まず 0.3.0 の判断**（**6 便続けて未決**。
  **`Directory.Build.props` は `0.3.0`・タグは 1 つも無い・`v0.3.0` を push すると
  `.github/workflows/release.yml` が test → CLI 4 RID → VSIX を回す**）。
  **correctness の門は空のまま**。⚠️ ★★ **§2F に「決定済み・未実装」の項は 1 つも残っていない**
  （第209 の 2 チケットはこの便で閉じた）。
  ⚠️ **出す前に片付ける候補は変わらず 2 つ**: **⒤ `CHANGELOG` が無い**（**リリースノート無しで出る**）／
  **⒥ 未 push 60 便ぶん**（**タグだけ先に push すると CI が古い木を建てる**ので **`master` を先に**）。
  ...
  **⒞ ★ `CoPlaceToCodaWithLabels` の鏡像ケース**／**⒟ §2 の頭の ▶ perf**（門の外）／
  **⒠ ★ SVG の同梱 sans のずれ**（**出力についてのユーザー判断が要る**）／
  **⒡ ★ 配管の残り 6 site**（`TabStaffGeometry` 3・`FetaTextRun` 3）／
  ✅ **旧 ⒢ `DrawingTransform.Identity` はこの便で閉じた**（`6cbe39d9`・上の ⑷。**letter は空いた**）
  ／**⒢ ★ ⑸ の LSP 補完の綴り**（**この便が起票・§2F 末尾**）。

---

## 以下は第209セッションの経緯

最終更新 第209セッション＝**エディタの色づけを言語の語彙に合わせ切った便**（**実装 3 便**）。
**perf は 1 件だけ判定が要った**——**`semanticTokens/full` は打鍵ごとに走る経路**なので
§7.9 に従って**数で測った**: **`tempo` は追跡コーパス全体で最大 4 個/冊**（82 冊が持つ）で、
**足したのは既に訪れている節点への分岐だけ＝新しい走査も `DescendantNodes()` も無い**。
彫版のコードは 1 行も動かしていない。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **§1 ⑹ ⓪ が「まず 0.3.0 の判断」だったので裏取りのあと訊いた。
ユーザー決定は「⒜ まだ出さない・文法を続ける」**（選択肢は ⓪ 出す方向へ舵／⒜ 文法／
⒝ 小さい残債／⒞ §2 を読んで提案、の 4 つ）。
⇒ ★★★ **0.3.0 を出すかは*まだ決まっていない*。次便も同じ設問から始まる**（下の ⑹）。

★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 529 点・ss 非ゼロ 100・
総和 3.794108828・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 47**・
**snapshot 218 枚**・suite **5536 passed / 0 failed / 4 skipped**・**追跡コーパス 567 冊**・
**Core 0 警告**＝**引継ぎの数は全部合っていた**（**38 回連続**。**第208 が §1 を書いたので、
第207 のときのような「数え直し」は要らなかった**）。
⚠️ **`origin/master` は `9830873f`＝開始時と同じなので、便の途中の push は無かった**。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 52**（**この行を書く commit を含めて 52**）・
suite **5536 → 5552 passed / 0 failed / 4 skipped**（**+16**）・
**台帳 529 点で不動**（ss 非ゼロ 100・総和 3.794108828・count 106/2 も不動）・
**snapshot 218 枚で不動＝中身も 1 枚も動いていない**・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Turmeric`）で通せた。**

> ## ★★★ この便の骨＝**規則は在ったが、誰も到達していなかった**
> **開始時に文法を `#comments` で grep したら、どこからも include されていなかった。**
> **`//` も `/* */` も、全ファイル・全位置で無色**——**第208 の `8f7620c6` が 1 便前に入れた退行。**
> **`#fonts-block` を*足す*つもりで `#comments` を*置換*した**（ルート／`#lyrics-content`／
> **新しいブロック自身**の 3 箇所とも。おかげで `#fonts-block` は**自分自身を include** していた）。
> ⇒ ★★★ **suite は 1 便まるごと緑のままだった。** **同じ commit が `Patterns()` に
> include を辿らせた**のに——**その歩きは「暗くなった規則を誰の語彙リストも訊いていない」ときは黙る。**
> **コメントは語ではない。** **この網の検査は全部「語彙 → その語に色が付くか」だった。**

> ## ★★★ 2 つ目の骨＝**到達可能性は必要条件で、十分条件ではなかった**（**毒が緑で返した**）
> **⑴ ルートの `#comments` だけ落とす毒は*緑*だった**——**`#lyrics-content` 経由でまだ到達可能**。
> **その間、歌詞ブロックの外のコメントは全部無色。** ⇒ **到達（reach）は位置（position）ではない。**
> **この 2 つの規則では位置こそが振る舞い**（自分の文字列を他の規則に取られる前に読まれる必要がある）。
> ⇒ **不変条件はリストごと**: **引用文を守るリストは、コメントも守り、その 2 つが先頭に来る。**
> **⑵ 形の検査 2 本は、`comments` 項目を*消す*と両方緑**（存在しない規則は「未到達」でも「遅い」でもない）
> ⇒ **3 本目は直接訊く。** ⇒ ★★ **3 本とも独立に効く**（毒 3 種がそれぞれ*別の* 1 本を赤にした）。

> ## ★★★ 3 つ目の骨＝**「その語に色が付いているか」は「その語彙が塗られているか」ではない**
> **§2F のチケット（無色 17 語）を閉じた。** **色が付いていた語は*全部*「別の理由で予約語だから」。**
> **`pedal text` は `text` だけ色が付く**（`text` は *fonts* の鍵）／**tuning 7 語のうち `bass` だけ**
> （**clef だから**）／**`clef treble` は付いて `clef treble^8` は付かない。**
> ⇒ ★★★ **そして*チケットの数え方*にも穴があった＝6 つ目の集合**——
> **`transposition` は「無色の property 名」として数えられていたが、その値 `8va 8vb 15ma 15mb` も
> 4 語とも無色**。**鍵だけ塗るのは、報告された欠陥の左右を入れ替えただけ。**
> ⇒ **語彙の家を 1 軒にした**（`SymbolCaseValidator` の 5 集合＋`InstrumentDefaults.TranspositionMarkers`）。
> **`private static readonly` で網から数えられなかったのが、部分的な写しが放置された理由そのもの。**

> ## ★★ 4 つ目の骨＝**1 つの production が 2 つの位置を名乗っていた（実測で割れた）**
> **部品本体は clef 11 語を取る／音楽中の `clef` と score の `staff`/`ossia` は 5 語で残り 6 語を拒む。**
> **tuning 7 語には*第 2 の位置*がある**（`tab guitar melody`・7 語とも受理）。
> ⇒ **`GRAMMAR.md` は 4 箇所ずれていた**（`transposition`/`lines`/`pedal` が PartProperty に無い・
> `ClefName` が 5 語）。**`ClefName`（5）と `PartClefName`（11）に割り、doc を Core に縛った。**
> ⇒ ★★ **片方だけ塗ると報告は「消える」のではなく「移動する」**——だから `#tab-tuning` が要る。

★ **この便の値段**（**彫版のコードは 1 行も動いていない**。**動いたのは文法・LSP・Core の
語彙公開・doc・テストだけ**。**台帳 529・snapshot 218 とも不動。**）:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ `#comments` の到達不能（第208 の退行） | 文法 1 ＋ 網 1（+3 本） | **コメントを書く本すべて**＝実質全冊 |
| ⑵ 部品本体の 6 語彙・45 site | Core 2 ＋ 文法 1 ＋ doc 1 ＋ 網 3（+4 本） | **`part … { }` を書く本すべて**（追跡 505 宣言） |
| ⑶ tempo の feel 語 2 語 | LSP 1 ＋ 網 1（+8 本） | `swing`/`shuffle` を書く本 |

- **⑴ `f5bbe7bf`**。**3 本立てた**（**それぞれ*別の*毒で赤になる**＝どれも独立に効く）:
  **⒜ repository の項目はすべて到達可能**（規則についての主張・どの語彙リストからも導けない）／
  **⒝ 引用文を守るリストはコメントも守り、2 つが先頭**（到達≠位置）／
  **⒞ 両方のコメント形が塗られる**（項目ごと消す毒は ⒜⒝ を素通りする）。
  ⚠️ **⒞ は `Assert.Equal` で書いた**——**「どれかの規則が覆う」だと、行末まで届く別の規則で
  緑になりうる**（第208 の `bass_8` × 歌詞コネクタと同じ族）。
- **⑵ `e0b957af`**。**着手手順はチケットの通り**（⑴ 全語を `lysc check` に通す → ⑵ 文脈 → ⑶ 毒 → ⑷ 網）。
  - ★ **測ってから決めた**: **この文脈が塗る 22 語のうち 18 語は部品名に使える**
    （`part lines { }`・`part standard { }`・**`part true { }` すら通る**）。
    **予約語は clef 4 語だけ**（`soprano mezzosoprano baritone percussion`）——
    **その 4 語が既に色付きだったのは、この理由ではなく*別の*理由。**
    ⇒ **裸の alternation はここでは使えない。文脈だけが、書き手の部品名について嘘をつかずに塗れる。**
  - ★ **`#part-body` は追跡コーパスの `part … {` 505 件*すべて*で開く（0 件外さない）**。
    **ブレースを次行に置く本は 0 冊**なので、`#fonts-block` が書いている 1 行制約は今どこにも当たらない。
  - **値は必ず*鍵と一緒に*読む**（`tuning treble` が tuning に見えないように）——`#directive-value` と同じ device。
- **⑶ `5473f7ce`**。**`swing`/`shuffle` は正規表現で持てない**（第208 が 3 通り試して棄却・理由は §2F）。
  **`tremolo`/`percent`/`unfold` と同じ device で木から塗った**（`RepeatExpressionSyntax` の隣）。
  ⚠️ **読みは `TempoValue` 自身のもので、2 つ目の写しではない**——
  **feel 語の枝が marking の枝より*前*にあるので、裸の `tempo swing` は必ず feel 語。**
- **⑷ ★ この便が*起票した*もの**（§2F の頭・**どちらも「書ける本を拒む」側の変更なので要ユーザー判断**）:
  **⒤ `removeEmpty banana` と `lines banana` が今日通る**（実測。前者は `false`・後者は既定に読まれる）／
  **⒥ 大小文字を無視するのは `removeEmpty` ただ 1 つ**（`removeEmpty TRUE` が通る）。
  ★★★ **この 2 項は一度「`transposition` だけが大小文字を無視する」と書いて commit した**。
  **§7 の終了時チェックリストを走らせて自分の記述を実測し、逆だと分かって直した**（下の ⑸）。
- **⑸ ★★★ 便を閉じるときに §7 を*実際に走らせた*ら、本物の残件が 2 件出た**
  （**「無い」と即答せずに上から当てる**と §7.7 が書いているとおりになった）:
  - **§7.5 の数え方**: **Core に 72 行足して出典注記 0・数値定数 0**。
    **これ自体は妥当**（LP 移植ではなく Lily# 自身の語彙）だが、
    ★★ **§7.7 の匂い「同じ量の 2 つ目の綴り」に 1 件当たった**——
    **`RemoveEmptyValues` は `RenderSpecParser` の `is "true" or "all"` と同じ量の 2 つ目**で、
    **`TranspositionMarkers` には縛りを入れたのにこちらには入れていなかった**。
    ⇒ `HaraKiriTests.TheRemoveEmptyVocabulary_IsWhatRenderSpecParserReads` を足した。
    ⚠️ **しかも初稿は既存の `RemoveEmptyPartProperty_MapsToStaffFlags` と同じ 3 行を持っていた**
    （**自分で 3 つ目の綴りを作りかけた**）。**削って、その網が*見られない*方向
    （公開リストに語を足しても読み手が覚えない）だけにした**。
  - **§1 に書いた事実が 1 件逆だった**——**上の ⑷ ⒥**。
    ★★★ **commit 済みのソースコメントにも同じ嘘が入っていた**
    （`InstrumentDefaults.TranspositionMarkers` の remarks）。**3 箇所とも直した。**
  ⇒ ★★★ **どちらもテストでは見つからない**（suite はずっと緑だった）。
  **チェックリストを「確認した」と書くのと実際に走らせるのは別のこと。

- **⑹ ★★★ 次に触るならここ**——**⓪ まず 0.3.0 の判断**（**5 便続けて未決**。
  **`Directory.Build.props` は `0.3.0`・タグは 1 つも無い・`v0.3.0` を push すると
  `.github/workflows/release.yml` が test → CLI 4 RID → VSIX を回す**）。
  **correctness の門は空のまま**。⚠️ ★★ **文法側は*この便で*一区切りついた**——
  **§2F に「エディタの色づけ」の未了項目はもう無い**（無色 0 語・到達不能 0 項目）。
  ⚠️ **出す前に片付ける候補は変わらず 2 つ**: **⒤ `CHANGELOG` が無い**（**リリースノート無しで出る**）／
  **⒥ 未 push 52 便ぶん**（**タグだけ先に push すると CI が古い木を建てる**ので **`master` を先に**）。
  ／**⒜ ★ 台帳点 `courtesy.key.key-to-line-end` の残差 0.15**（第206 が開いた。
  **閉じ方は `SharedRenderer` 自身が名指している**＝予約幅を描画幅にして `SolveColumns` に渡す）／
  **⒝ ★ ペダルの段間隔**（LP は 1.961 → 2.443・Lily# は一律 2.46。**台帳点は無い**）／
  **⒞ ★ `CoPlaceToCodaWithLabels` の鏡像ケース**／**⒟ §2 の頭の ▶ perf**（門の外）／
  **⒠ ★ SVG の同梱 sans のずれ**（**出力についてのユーザー判断が要る**）／
  **⒡ ★ 配管の残り 6 site**（`TabStaffGeometry` 3・`FetaTextRun` 3）／
  **⒢ ★ §2G の `DrawingTransform.Identity` が単位変換でない**（**`new()` は record struct を
  全ゼロにするので `ScaleX/ScaleY = 0`・`Identity.IsIdentity` 自身が false**）。
  **第208 に続き第209 も読んで着手しなかった**——**1 行・出力不変・
  falsifier は `Assert.True(DrawingTransform.Identity.IsIdentity)`。**
...
> ## ▶ **LSP の値補完が Core の語彙の 2 つ目・3 つ目の綴りを持っている**（2026-08-19・第210 が測って起票）
> **第209 が閉じたのは*色づけ*（TextMate 文法）で、*補完*は数えられていなかった。**
> **⑴ `GetRemoveEmptyCompletions` は `true`/`all`/`false` を自前で持つ**——
> **`SymbolCaseValidator.RemoveEmptyValueVocabulary`（Core）と `GRAMMAR.md` の
> `RemoveEmptyValue`（第210 が名前付き production にして拘束済み）に続く 3 つ目。**
> **今は 3 つとも一致しているので現物の欠陥は無い。危険なのは*検証が入った*こと**——
> **ずれた瞬間、エディタがコンパイラの拒む語を提案する**（第210 が `octave` でまさにそれを直した）。
> **⑵ `Clefs`（LSP）は 5 語**＝**`ClefName`（音楽中・score の `staff`/`ossia`）の語彙**。
> **ところが補完の文脈は位置を見ていない**（`if (prevWord == "clef")` だけ）ので、
> **11 語を取る部品ヘッダでも 5 語しか出さない**（★ **実測済み**——起票時は「`Clefs` が 5 語」と
> 「文脈に門が無い」を読んで*導いた*だけだったので、`GetCompletionContext("part vln { clef ", …)` が
> `AfterClef` を返すことを毒行で確かめた。**部品ヘッダ・音楽中とも `AfterClef`**）——**⑴ と向きが逆で、「拒む語を出す」ではなく
> 「出せる語を出さない」。**⚠️ **こちらは実害が小さいので、⑴ と同じ便で直す必要は無い。**
> ⇒ **閉じ方は 1 つ**: **`GetRemoveEmptyCompletions` を `RemoveEmptyValueVocabulary` から作り、
> `Clefs` を位置で 2 つに割る**（`instrument` の補完が既に `InstrumentDefaults.KnownInstruments`
> から作られていて、doc コメントに「never drifts」と書いてある——**その形が正解の見本**）。
> ⚠️ **`tuning`/`pedal` には値の補完文脈がそもそも無い**（`AfterTuning`/`AfterPedal` は存在しない）。
> **足すかどうかは別の判断**で、この項に混ぜないこと。
> ★ **射程**: **エディタのみ・出力不変・追跡 567 冊は 1 冊も動かない。**
