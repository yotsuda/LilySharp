...
★★★ **⒀ 第 9 便＝入れ子を「書いたとおり」に一般化（ユーザー決定: ①「staffGroup の中の grandStaff は中かっこが左…意図通り？」→ 調べて LP・Dorico・MuseScore とも*子は親の外（左）*と報告 ②「数が少ないグループを左に寄せる方が自然」③「**なるべく、ユーザーが書いた lys テキストを尊重する形で動作できるようにしたい**」→ 案「どの group もどの group の中に・深さ無制限・書いた順に描く」を出して「良い。それで進めて」）**。★ **② と ③ は矛盾しない**: 子は親の譜の部分集合＝段数は必ず子 ≤ 親⇒「子を外に」＝「少ない方を左」（同数の時だけ書いた順で決まる）。**LP で先に測った**（`scratch/p377/nest/deep.ly`＝`-dbackend=null`・indent 0・10 冊／`sd3,6,7,8,9.ly`＝svg で span bar）: ⒜ **位置＝各 delimiter は*親の*インク左端に対して bracket 0.8・brace 0.3**（最上位は縦線に対して）・**同じ深さの兄弟は列に揃わない**（D1: 深さ 1 の brace −1.61 と深さ 1 の bracket −2.56..−2.11 の子 brace −2.86）・**brace の中の bracket は brace の幅（高さ依存）の外**（D4: −1.49−0.8）⒝ **間隔＝最内のグループが同じなら 9・違えば 10.5**＝**第 8 便の grouper 規則がそのまま全深さで正しかった**（コード不変・コメントだけ）⒞ ⚠️ **span bar＝「両方の譜を含むどれか 1 つのグループが grandStaff／staffGroup なら通す」**＝**choirStaff の中の隙間も外の grandStaff／staffGroup が通す**（sd6・sd7。外の engraver が子孫の小節線を全部拾う）。⚠️ **null backend の SpanBar は Y が ±inf で読めない**＝svg の rect で数えた。⚠️ **LP を MCP コンソールから直に起動して 9 分固まった**＝記憶 `lilypond-mcp-console-hang` の `cmd /c … < NUL` で 1.7 秒（**記憶を先に引け、の 2 度目**）。**実装**: **model＝`OuterStaffGroup` に親 `Outer`**（鎖・`SelfAndOuters`）・`StaffGroup.Outer` は最内。**`RenderSpec.BuildStaffGroups` は再帰の `Leaves`**（入れ子を持たない group は今までどおり leaf 1 個）。**`OuterRuns` は全 outer を外側から順に**。★ **delimiter の X は描く時に 1 箇所で**（`SharedRenderer.PlacedDelimiters`＝親から順に置き、子は親のインク左端に対して `SystemStartBracketCentreAgainst`／`SystemStartBraceRightEdgeAgainst`）＝**親が brace だと幅が高さで段階的に変わる**ので、頁の再配置の後にしか決まらない（layouter の入れ子 leaf の X は仮置き・最上位 leaf は layouter の値のまま＝入れ子の無い本は同じ式）。楽器名の左端・描画・span bar が同じ関数を読む。**span bar は「最内の共通グループが 1 回だけ描く」**（choir の leaf は外に通す group があれば自分で描く）・`HairpinEngraver` も鎖を見る。**キャッシュ鍵 2 本は鎖全体**。**パーサはどの group 本体でも 3 語を受ける**（LYS6011 の文面・回復ループの止まり語）・**「2 本未満の group は落とす」は段数で数える**（`staffGroup { grandStaff { a b } }` は 2 つの delimiter を描く＝LP も同じ）。**LSP は `BracketGroupBlock` を消し、グループ本体の補完は常に 7 語**。**双子・歌詞行の検査は元から再帰**（文面を "group" に）。**GRAMMAR の `StaffGroupBody`**・§2 E の範囲外（残るのは LP の `SystemStartSquare` の形だけ）。**数値は全部 LP の実測と ±0.02 で一致**（`NestedGroupDepthTests` 13 本＝D1/D2/D4/D5/D9/D10/sd3/sd6/sd7/D8・双子）。**毒 3 本（1 本ずつ・出力を見て・ハッシュで復元）**: ⑴ 子の X を親でなく縦線に対して → 8 本赤 ⑵ span を group 自身だけで判定 → sd7 の 1 本赤 ⑶ 段数でなく member 数 → 2 本赤。★ **射程は実測**: **snapshot 動いたもの 0・台帳不変**（入れ子の本はコーパスに 0 冊）・既存 test は拒否 theory を「受ける」に・LSP の期待 3 本に 3 語。**数**（`Session-Check -Build -Test -Scratch p377 -DiffBase 3bf509fa`）: **full `scratch/p377/run15.trx` 8519 / 0 / 4 / 8523**（+17＝`NestedGroupDepthTests` 17 case。第 8 便 run14 の赤 2 本＝生成物は再生成で解消）・Core 0 警告・**台帳 843 点／ss 非ゼロ 210／総和 22.584587806 不変**・snapshot 249・追跡 `.lys` 599 不変・§7.5 は未 commit で 0 と出る。★ **LSP は配布する**（Core と LSP が動いた）⇒ **ユーザーは Reload Window が 1 回要る**。
