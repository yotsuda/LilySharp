...
⇒ **次の一手**（⒜⒝⒝′⒟ は第406 から変わらず）: ⒜ **R13⒝ の実機確認**（第404 ⑵ のとおり＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item（第404 ⑵′・実機の 2 行を見てから）／⒝′ frame 変更の `applyFrame`（実機で頻度を見てから）／⒞ 第403 の残り＝count loop の page DP 5.3 ms（R13⒦・census で hit 率）・診断 pass の残り（`AnnotationName` 14／`MeasureValidator` 10／`LyricSings` 6.5）・⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（⑺⑴）＝提案しない**／⒞′ **prelim の残り**＝fingbeam1k の `fs.walk` 0.41 ms／1.5 MB と `fs.assemble` 0.29 ms／4.0 MB（24,000 digit の実仕事＝「何を返すか」を変える別の島）・尾部の `t.mmr` 247 KB・`t.ledger`／**⒞″ ✅ 測って「やらない」と決めた（第 2 便・下の ⑸）＝残りは畳む価値が無い**／**⒟ ▶ LSP 起動直後の tier-0＝ユーザーが「起動が遅いと感じることがある」と確認（⑺⑵）＝問題は実在する。次は「まず cold start を実測」から**（ReadyToRun・`TieredPGO` の knob は packaging 側なので変更は承認後）／LP 双子が要る R7〜R11／**R15 ✅ 削除分は第 3 便で実施**（残り＝「2 綴り」一族・`IsInside*` 6 本・テストだけが読む 3 件）。⚠️ **⒜ と push は「後回し」＝催促しない（⑺⑸）。**

★★ **⑸ 第 2 便（指示「このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）＝⒞″ を採り、*数えた結果「やらない」と決めた*（コード変更 0 行・詳細は R13⒨″ と `sessions/p407/leg2-residue.txt`）。** 残り 4 箇所に計器を戻し、**2 staff の `perf-hairpingrand1k.lys` を 4 冊目に足した**。打鍵あたり 8 本の内訳＝**注釈 pass 4（床）＋`PagingSkylines` 2 ＋ 梁のある本なら `Prelim.LayoutBeamsMemoized` 1 と `CalculateRestShifts` 1（二声なら `Prelim.MeasureToSystemOf` 2）**。⇒ **畳める余地は 1 冊 2 本**に対し代償は per-staff ヘルパ 3 つ＋public 署名＋hoist＝**配管のほうが大きい**。★★★ **`PagingSkylines` は畳めない＝別の量**（`Math.Min(perSystemExtents.Count, systems.Length)` 等の**狭い表**で、full を渡すと後ろの system の小節が解決するようになる）＝**「同じ表か」は字面でなく*境界*で確かめる**。⚠️ **留保**: 前 2 者は per-staff ループの中なので構造上は staff 数で増えるが、**梁のある多 staff の本が木に無く測れない**（hairpingrand1k の 0 は「4 分音符だけで梁が無い」ため＝**「測って 0」ではない**）。⇒ **⒞″ は閉じる。この島に戻らないこと。**

★★ **⑹ 第 3 便（**ユーザー決定 5 件**・⑺）＝R15 の dead code 削除（`c4c03056`・詳細は §2 R15）。** ⚠️★★★ **一覧は過半が古かった**——呼び手を 1 件ずつ grep したら**削除できたのは 8 件だけ**で、残りは生きていた。★★★ **最大の収穫＝`BeamScoringProblem.stemPositions` は param doc の「⚠️ NO CALLER PASSES IT」を信じて dead に載っていたが、`TabStaffGeometry.SolveTabBeam` が全 tab 梁で渡している＝消していれば tab 梁の quanting が壊れた**（doc を訂正）。⚠️ **副産物＝`magic_constants.csv` の生成則の欠陥**: 定数 1 つ消して行が 4 行動いたら `BeamTranslation` の REF が変わった → census は ±5 行の窓を**上から**走査して**最初の** REF を採るので、**定数が 5 行以内に並ぶと後の定数が前の定数の REF を貰う**（色は Green のまま＝§7.6 の「実在する出所を消す」）。**近い方（上→下）に直して 16 行が改善**（`SlurMidThickness` が **Tie**、`ThickBarlineThickness` が **hair-thickness 1.9**、`HorizonPadding` が **right-padding**、`EdgeHeight` が **bound-padding** を名乗っていた＝全部第407 より前からの誤り）。**full 8700 / 0 / 3 / 8703 不変・台帳・snapshot 249 不動・Core 0 警告。**

★ **⑻ 第 4 便＝新しい島は開かず、承認済み R15 の「要 grep」2 件を verify し切った**（`bdf4abd0`・§2 R15 ⒞）。**判断**: 残る大物はこのセッションの文脈が効かない——診断 pass の `AnnotationName` 14 ms 等は §2 が「**実仕事**」＝今日の 3 便の「二重をやめる」型ではなく「何を返すか」の設計仕事／⒟ は別サブシステムでベンチの窓が要る／⒜・push・D⑶・テストだけが読む件はユーザー待ち。**一方 R15 を半分 verify で残すのは今日まさに事故りかけた形**（doc を信じて `stemPositions` を消しかけた）ため閉じた。**結論＝R15 は項目ごとに当たり外れがある**（⑹ は過半が古く、⑻ は正確）＝**一覧を信用せず 1 件ずつ読む。**

★★★ **⑺ ユーザー決定（2026-09-18・第407 第 3 便の冒頭）**: ⑴ **`RestCollisionsOf`／`RestDotOffsetsOf`（旧 ⒞ の保留分）は「今はやらない。着手はずっと後だ」** ⇒ §2 R13⒧ の残件として寝かせる・次便は提案しないこと。⑵ **⒟ LSP 起動直後の tier-0 は「起動が遅いと感じることがある」＝問題は実在する**（ユーザー確認）⇒ **live な項目になった**が、着手の指示はまだ出ていない。**次にやるなら順序は「まず cold start を実測（何秒・どの段が占めるか）→ ReadyToRun／`TieredPGO` の可否をユーザーに出す」**（packaging 側を触るので勝手に変えない）。⑶ **R15 は承認＝⑹ で実施**。⑷ **§2 D⑶（voice スパンの遅入り）は説明を求められた段階で、可否は未決**（説明は §2 D⑶ の本文どおり＝提案の半分は既に実装済み `s1*3`、残りは表現力寄りで、意図的な誤り回復 `RepeatedVoiceKeyword` と衝突する）。⑸ **⒜ 実機確認と push は「後回し」**（⇒ **次便が勝手に催促しない**。ただし push しない限り他の PC には何も届かない事実は変わらない）。

★★ **⑼ 第 5 便（ユーザー規則「テストだけが使う製品コードは*テストの質*に依る。良いテストができるなら残して。役に立たないテストなら製品コードを削除して」）＝R15 の最後の分岐を決着（`6832f67a`・§2 R15 ⒟・判定の全文は RULES §5.3）。削除 3・残留 2。** 効いた問い＝⑴ **2 綴り目でないか** ⑵ **実装していない機能を宣言していないか** ⑶ **既に覆われた理由でしか落ちないのでは**。**full 8693 / 0 / 3 / 8696**（網 −7）・台帳・snapshot 249 不動・census 931 → 930（**消えたのは発明の `0.1` 1 行だけ**）。

★ **⑷ 終了時**: commit 4 本（code `c18ca01a`＋docs＋第 2 便 docs＋第 3 便 code `c4c03056`＋docs）・**最終 full `sessions/p407/run4.trx` 8700 / 0 / 3 / 8703**（第406 最終 8697 / 0 / 3 / 8700 に対し網 +3）・§7.5（対 `cee2c758`＝スクリプトの出力）**Core `+` 106 行／REF 0／OWN 0**（数値定数なし・LP 由来の式なし＝既存の辞書を建て直さずに渡す配管と辞書 1 本の削除＝監査対象は「同じ答えか」＝snapshot 249 不動＋毒 205／148 本＋差分の網 3 本）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。§7 3.5 は済（第405 を `-Archive 405`＝15 行 5,290 chars）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。
