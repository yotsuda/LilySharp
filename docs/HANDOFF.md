...
★ **⑶ 終了時**: docs のみ（commit 1 本）・**最終 full `sessions/p403/run3.trx` 8664 / 0 / 3 / 8667＝開始時と同数**（run2 は §1 の数字記入前で同数）・§7.5（対 `567355ef`・スクリプトの出力）**Core `+` 0 行／REF 0／OWN 0**＝監査対象なし・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。§7 3.5 は済（第401 を `-Archive 401`＝11 行 5,271 chars）。RULES §5.3 に「数秒の in-process 計器は tier-0 を含む」を汎化して置いた。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⇒ **次の一手**: ⒜ **R13⒝＝render 段（Δ=0 打鍵の最大の段・plain1k 2.3 ms／8.4 MB・fingbeam1k 7.7 ms／33.6 MB を*打鍵ごとに*割当）**＝SVG 全文の 7 回コピー（`Assemble`→JSON→…）を差分応答に／⒝ **edit 打鍵の layout 段（plain1k 16・fingbeam1k 19・v2bow1k 10 ms＝Δ≠0 の最大）を段で割る**（R13⒡ per-system memo の無い `LayoutStaffGroups`／`StaffSprings`・⒢ `PlaceSystems` 2 回）／⒞ 診断 pass の残り＝`AnnotationName` 14／`MeasureValidator` 10／`LyricSings` 6.5（fingbeam1k・warm）／⒟ LSP 起動直後の数秒は `Compute` 等が tier-0 で 6 倍＝ReadyToRun 発行や `TieredPGO`／`TieredCompilation` の knob は packaging（ユーザーの `tools/Package-And-Install.ps1`）の側＝**要ユーザー判断**／LP 双子が要る R7〜R11／R15（要承認）。
