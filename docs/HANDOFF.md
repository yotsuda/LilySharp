...
最終更新 第133セッション＝**第132 が起票した ⒜「スラーが script を避ける」を移植した**。
コミット `5ea9073e`（移植の全部）＋ §1 のコミット 2 つ（この記述とレポートの結果）。
**未 push 41 件**（§1 のぶんを含む）・**push はしていない**（ユーザー判断待ち）。
**テスト 4346 passed / 0 failed / 4 skipped**・Core 0 warning・**snapshot 0 枚**。
**台帳 494 点**（増減なし）・**ss 非ゼロ 91 → 90**・**総和 4.617633053 → 3.617633053**
（**ちょうど −1.000000000**）。
⚠️ **引継ぎの数は開始時点で 3 つとも stale だった**（テスト 4340・台帳 489 点・未 push 35 に対し、
実測は **4346 / 494 点 / 38 件**）——**§1 は自分の便より前に書かれ、その後 2 コミット進んでいる**。
§0 のとおり**開始時に実測して直した**。この欄の数も同じ性質のものと思って再測すること。
...
**次の一手**:
▶ **⒜ 梁を指番号の support にする**（0.190000）。**点が先**＝梁付き和音の本を CFL/CFH に足す。
  LP 側の数はもう手元にある（下の第132 の記述）。
  **同じ本に残るもう 1 件＝`avoid-slur #'around`**（`chord-repetition` の下向き digit は今もスラーに触る。
  ⚠️ **この本のスラーは LP 側の digit を 1 mm も動かさない**ので、**目視は起票であって測定ではない**——
  点は別の texture で作ること）。
