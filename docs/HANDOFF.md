...
最終更新 第294セッション＝**§1 が「次に読み手の目に触れそう」と名指していた §2 U4 の残債に着手し、着手前の測定でその残債が*存在しない*ことを確定した**。**代わりに §2 U9 の原因を測り直して §2F ⑺ の島へ合流させ、その島の検出器の*欠けていた半分*をコーパス規模で建てた**（**commit 2 本＝`12599c12` の test ＋ この 1 本の docs**）。**production コードは 1 行も動かしていない**——**台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188・snapshot 230・追跡 `.lys` 580 は全部不動。**

⚠️⚠️ ★★★ **⑴ 「タブの `as full` に連桁が無い」という残債は無かった**（§2 U4 の末尾。**§5.0「引き継がれた処方箋は、着手する前にその*診断*を 1 回測る」がそのまま効いた**）。**タブの連桁は主梁も副梁も切れ端も在り、しかも LP の量子器を通っている**——**`TabBeamQuant.Compute` が `BeamScoringProblem` へ*弦の線*を stem position として渡し、TabStaff の 2 つの override（`Beam.beam-thickness 0.32`・`Beam.length-fraction 0.62`）で解く**。**実測 30 秒**（`scratch/p294/b16.lys`・2/4 の 16 分 8 個＋混在）: **タブ側 polygon 8 本＝主 4・副 4**、**`staff` を外したタブ*だけ*の本でも同じ**。⇒ ★★★ **反証は起票の 12 行上に最初から在った**——**同じ §2 U4 が「fixture 7 冊の主題（*連桁*・付点・…）は numbers では消えるので `as full` を明示した」と書いている**。**`test/tab-beam-slope` も `0238f5b1`「Slope the tab beam along the fret contour」も在った。** ⇒ ★★ **1 つの量について同じ節が 2 つのことを言っていたら、grep ではなく*その節を通しで*読む。**

⚠️⚠️ ★★★ **⑵ §2 U9 の「`GreenNode` の span の取り方の問題・第286 U3 と同じ族」も両方外れ**（第293 が起票時にそう書いた）。**`GreenNode` は正しい**——**犯人は `ParsePostEvents` の並べ替え**: **記号（`~ ( ) [ ]`）の*後ろ*に書いた post-event は音符の上へ持ち上げられ、記号は `_pendingPostEventMarkers` から*その後ろ*へ再生される**ので、**木の順序が原文と逆になり、入れ替わった 2 ノードが両方とも嘘の住所を名乗る**。⇒ ★★ **U3 と見分ける問いは 1 つ**: **「幅は正しいが `Span` が trivia を食う」（`GreenNode`）か、「`Span` の取り方は正しいが、そのノードが木の別の場所に居る」（parser）か**。**症状はどちらも「data-pos が 1 手前」で、同じ顔をしている。** ⇒ **U9 は §2F ⑺ へ合流させた**（**2 つ目の綴りを作らない・§5.2.1②**）。

★★★ **⑶ RULES §5.0 が「2 つで 1 組」と書いていた検出器の、片方しか無かった**。**和の検査（`root.ToFullString() == src`）はコーパス 580 冊に在り、*各ノードの*検査は inline 2 本と狭い 3 ファイルにしか無く、本に 1 度も当たっていなかった**。**建てた**（`AnnotationRoundTripTests.EveryNodeInEveryBook_StandsWhereItSaysItStands`）——**⒜ `FullSpan`（`Position` と `ToFullString`）と ⒝ `Span`（trivia を食っていないか）の 2 つの主張**。★★ **⒝ は「幅から導かない」ことが値打ち**——**ノード自身のテキストを切り出す書き方は同じ `GetLeadingTriviaWidth` から来るので、U3 の誤答と一緒にずれて*一致してしまう*。**動かないのは trivia の定義のほう**（trivia-free な span は空白でもコメントでも始まらず終わらない）。

⚠️⚠️ ★★★ **⑷ 予測を 2 つ書いて 2 つとも初回の run に訂正された**（**先に測っていれば要らなかった 2 往復**）。**⒳「2 つの一覧は違うはず」→ 同じ 11 冊**（**この島では入れ替わる 2 つが `A+M == M+A` を満たしえないので、和の検査も必ず落ちる**）／**⒴「`Span` 側は 0 のはず」→ 同じ 11 冊**（**再生された記号は音符の後続空白の*後ろ*に落ちるので、`(` と綴るノードが `" "` を含む span を名乗る**）。⇒ ★★ **どちらも「網を書いてから remark を書く」順なら出なかった**——**remark を先に書くと、測定が*確認*になって読み飛ばされる。**

★★ **⑸ 毒は 2 つの半分を弁別した**（§5.0「毒は量ではなく量の*綴り*を狙う」）。**`SyntaxNode.Span` を U3 の綴り（`Green.LeadingTriviaWidth`）に戻すと ⒝ が 569 冊で赤・和の掃きは*緑*のまま**。⇒ **和の掃きは U3 に対して 8 月と同じだけ盲目で、2 本は 1 つの主張の 2 度書きではない。** ★ **⒜ の陽性対照は既知一覧そのもの**（11 冊が今日も赤で、名前が書いてある）。

★ **⑹ 射程を 1 軒に寄せ、床を上げ、島を数え直した**。**2 つの掃きが各自ディレクトリ一覧を持っていた**ので `CorpusBooks()` に統合（§5.2.1②）、**床 566 → 580**（**引き算の実測＝追跡 580 に対し射程 580・差 0**）。**島の census（ディスク 1629 冊。2026-08-16 は 1021 冊で 55）＝62 冊・sum と span で同一集合**——**ユーザーの 326 冊のうち 40 冊 63 か所**（**`~@mark` 53・`~@trill` 1・`)@fall` 3・スラー閉じ後の弦番号 `)\4` `)\2` `)\3` 各 2**）／**追跡 11 冊**／**scratch 11 冊**。⚠️ **ユーザー側の 40 冊は 8/16 から動いていない＝この島は増えても減ってもいない。**

★ **⑺ §0 に 10 例目の「数え方」を足した**＝**台帳の `OPEN:` 0**。**素朴に json を grep すると 12 になる**（**`OPEN:` は `why` の*接頭辞*で、台帳自身の説明文 1 行と、本文中で語として使う点 11 個が当たる**）。⇒ ★★ **接頭辞で意味が決まる札は、grep ではなく*その場所*で数える**——**「先頭に在るか」と「含まれるか」は別の述語で、後者は*増やす側*に外れるので、数がもっともらしいと気づけない。**

⚠️ **見つけて直していないもの**（前便から引き継いだ分に加えて）:
- **★★ 本便は新しい欠陥を 1 つも開いていない。** 開いたのは**決定 1 つ**（下）と、**倒した札 2 枚**（⑴⑵）だけ。
（以下、前便からの一覧はそのまま）

⇒ ★★★ **次の一手＝ユーザーに訊く。問いは §2F ⑺ の 1 つ**——**木を原文の順に忠実にするか、それとも「木は意味の順・住所は原文の切片で読む」を明文の設計として残すか。** **前者の代償は数え切ってある**（**生存 12 経路の順序契約の書き換え ＋ 死んでいた 6 経路の覚醒 ＋ `editors/vscode/src/smartTyping.ts` の同期**）／**後者は `343d755d` が実害を止めた形をそのまま規則に昇格させ、本便の 2 本の網が既知一覧つきでその設計の観測者として残る**。★ **射程はユーザーの 40 冊 63 か所で、インクは 1 mm も動かない**（**動くのは data-pos＝クリック／ハイライトの的**）。★ **▶ で残るのは §2 U8**（**装飾音符が注釈を 1 つも運ばない・射程はユーザーの本 0 冊・閉じる便は設計から**）。★ **ユーザーの本で今 LYS4018 が出ているのは `Untitled-6.lys` の 1 冊だけ**（第288 便の `@rit`）——**本便はどの本の絵も 1 バイト動かしていない。**

★ **開始時裏取り**: HEAD `fba98ca1`・**未 push 19**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6578 / 0 / 4 / 6582**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2・`OPEN:` 0**・追跡 `.lys` **580 冊**・snapshot **230 枚**・**CI は最後の push（`91650d7d`）で success**——**§1 の終了時の数と 1 つも違わなかった**（**ただし `OPEN:` だけは数え方を書き足した＝⑺**）。
終了時: **commit 2 本**（`12599c12` test ＋ **この 1 本＝§1・§2 U4・§2 U9・§2F ⑺・§0**）＝**未 push 21**・木 0・未追跡 0。**suite 6579 合格 / 0 失敗 / 4 skip / 合計 6583**（開始 6582 から **+1**＝**本便の fact 1 本**）。Core 0 エラー 0 警告。**追跡 `.lys` 580 冊・snapshot 230 枚・台帳 748 点＝すべて不動**（**production コードが動いていないので、掃きも再ベースも要らない**）。
★★ **WSL ubuntu Release も同じ 6579 / 0 / 4 / 6583**（**`12599c12` の木**・§0 の「この機械の Windows での緑でしかない」に両脚で答えた）。⚠️ **CI だけは未実行**（未 push 21）。
...
- **U9. ▶ `note~@mark("X")` の印は自分の住所に `~` を含む＝§2F ⑺ の島そのもの。単独の項ではない**
  （**第293 起票・第294 が原因を測り直して合流**）。**絵は正しい**が、**`data-pos` が書かれた `@` より
  1 手前**になるので、**エディタのクリック／ハイライトの的が tie を巻き込む。**
...
- ⚠️ ★★★ **⑺ 記号の*後ろ*に書いた post-event が木では*前*に出る＝実害は第186 で止めたが、
  ...
  **読み手がプレビューをクリックして失う量そのものを名指す。**
  ...
  ⇒ ★★★ **決定はまだユーザーのもの**。**問いは 1 つ**: **木を原文の順に忠実にするか**
  （**代償＝生存 12 経路の順序契約の書き換え ＋ 死んでいた 6 経路の覚醒 ＋ `smartTyping.ts` の同期**）、
  **それとも「木は意味の順・住所は切片で読む」を明文の設計として残すか**（**`343d755d` が
  実害を止めた形をそのまま規則に昇格させる**）。**後者を選ぶなら、上の 2 本の網は
  *既知一覧つきで永久に*残る**——**それは負債ではなく、選んだ設計の観測者になる。**
