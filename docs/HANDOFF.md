...
最終更新 第354セッション（2026-09-08）＝**入り方は第298〜第353 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。**第353 ⑹ の並びには「決定済み・未実装」が無かったので、§2F で唯一そうだった F-fonts（第342 起票・文法はユーザー承認済み「その形で起票して。VS code editor の自動補完も、それに合わせて修正して」・11 便放置）を実装した**＝`fonts { }` の entry に `as`／`step`／`size`／`bold`／`italic`／`regular`。**掃き 922 冊 MOVED 0**（`fonts` ブロックを書く本は追跡・ユーザーとも 0 冊＝既存出力は 1 バイトも動かない・§2F の見立てどおり）・台帳 810 不動・snapshot 248 不動（再ベース 0・新規 0）。**製品は commit 済み（下 ⑸）・push は禁止のまま**。**骨は 6**:

★★★★ **⑴ 文法と読み手（§2F F-fonts の起票どおり・変えた綴りは転送 1 つ）**: entry＝キー＋属性（順不同）で**次のキーで終わる**（区切り無し・キーも属性語も閉じた語彙・face は必ず引用符）。**`chordName serif` は拒否**（裸の語は次のキー＝空の `serif` entry になる→ LYS8006 に「`chordName as serif` と書け」のヒント）。**`Entries`（syntax）は「どこで entry が終わるか」だけ**（`as` 直後の家族語は operand として entry に留める＝最初の版はここが穴で `as serif` の `serif` が次のキーになった・DocExamples 3 本が赤で教えた）。**`FontPlanReader.ReadAttributes` が意味を読む**: `as FAMILY`／`step [±]N`／`size N`／style 語（`bold italic` は合成・`regular` で消す・最後が勝ち）・重複は LYS8005・**家族キーへの属性は LYS8015**（家族は face 表で role ではない）・数値なし／範囲外（step ±12・size 0.5..20＝LILYSHARP-OWN 2 本・言語の限界で幾何ではない）は LYS8016・step と size 両方は LYS8017（どちらも捨てる）。**`TextFontPlan.Binding(Names, Redirect, Step, Size, Style)`**＝1 entry 1 binding（後の同キーは全部置き換え）・`SizeOf(role, default)`（leaf に size/step があればそこで終わり→ group → default **そのもの**を返す＝掃き 0 の根拠）・`StepOf(role, default)`＝`6·log2(size/default)`（size 絶対でも glyph の連れが同じ歩で動けるように）・`StyleOf(role, default)`＝**書いたスタイルは彫版の既定を置き換える**（`tempo italic` は italic で bold-italic ではない・GRAMMAR に明記）・`Signature` に step/size/style を含めた（増分 collector と fragment memo が打鍵を見る）。**`ScoreTextMetrics.Size`／`Style`／`StepOf`** が全読み手の 1 口。

★★★ **⑵ 描画の付け替え（Core 25 ファイル・約 60 か所・「描く側と予約する側が同じ 1 口を読む」を role ごとに）**: **従う role 13＝`TextRoles.PlanReachOf`（Size|Style）**: title/composer（`HeaderBand.TitleEm/Style`＋`DrawHeader`）・lyricText（`LyricEngraver.LyricFontSize(fonts)`／`LyricStyle`＝9 読み手＋`LyricSpacing`＋`DrawLyrics`）・chordName（`ChordNameGlyphRun.Em/Style/AccidentalStep`＝**臨時記号グリフは名前の歩に連れて動く**・`GlyphFor(alt, step)`）・tempo（`MetronomeMarkGeometry.Em/TextStyle/PlainStyle/NoteScale(fonts)`＝**`\smaller` の音符も歩に連れる**・`StemTopAboveCentre/StemThickness/DotX/NoteRight/NoteTop` 全部 fonts 引数に）・mark（`LabelEm(fonts,type)`／`LabelStyle`／**`LabelBoxMargin(fonts,type)` は box-padding×magstep(自分の step＋plan の step)**）・pedal/navigation（`PlainMarkEm(fonts,type)`／`TextStyleOf(fonts,type)`＝1 引数版は既定として残る）・text（`CustomTextEngraver.Em/Style`＋`TextSpannerEngraver.TextEm/TextStyle`）・dynamics（`DynamicEngraver.LabelEm(fonts)`／`LabelStyle(fonts,expressive)`＝**`DrawDynamics` の `FontSize*0.5` という 2 つ目の綴りをここで畳んだ**）・barNumber（`Em/Style`＝Marks・PagingSkylines×3・Stacker×2）・tuplet（`NumberEm/NumberStyle`＝Overlays・Stacker・SkylineBuilder・ElementCoordinator）・volta（`NumberEm/NumberStyle`）。`SharedRenderer` の `DrawLyrics/DrawDynamics/DrawTupletBrackets/DrawBarNumbers/DrawCustomTexts/DrawTextSpanners` に `score.TextMetrics` を通した。**従わない role 12**（instrument・stanza・fretFrame・figuredBass・fingering・partCombine・ottava・bend・tabTechnique・clefOctave・meter・tabFret＝描画点の裸リテラルか Emmentaler 数字）**は LYS8018 警告**（group は全 leaf が従わないときだけ）＝**「黙って無視」を作らない**（起票の ⑶）。

★★★ **⑶ 番人＝`FontAttributeTests` 44 本（Theory 展開で +107）**: 読み手 11（合成・順不同・`as`・leaf/group 独立・size は step で割らない・StepOf・Signature・**既定はそのもの**）／診断 10（8015〜8018・8005・ヒント文面・**`TheNotFollowedWarning_IsAboutTheTable`＝validator と表を全 role で突き合わせ**）／**ページ `TheReachTable_IsWhatThePageDoes`＝全 25 role を `step +6`（＝2 倍）で描き、表の role は sample 文字列の `font-size` が 2 倍・表外の role は data-pos 伏せで byte 同一**（両方向）＋`TheStyleReach`（`bold italic`／`regular`）＋group step＋**予約の証明**（`lyricText step +6` で 1 段目末尾の小節線が右へ動く＝描くだけでは動かない量）／双子 4／LSP 4。⚠️ **Book の罠 3 つ**（全部実機で潰した）: `_"rit."` は form 項目（music では LYS0030）・`fine` は裸で小節頭（`@fine` は LYS1022）・`@sostenuto` は `@!sostenuto` で閉じないと描かれず、**`pedal text` を part に書かないと "Sost. Ped." は出ない**・末尾 section を D と名付けると section 札 "D" が和音 "D" の sample と衝突（Z に）。**docs**: GRAMMAR §2.4（production `FontAttribute`＋規則 5 段落）・GRAMMAR_FOR_LLM・SYNTAX_REFERENCE（属性表）・CHANGELOG 0.7.0 Language・APPROXIMATIONS／`audit/magic_constants.csv` 再生成（行番号のみ +134/−134・+36/−36）。**双子**: `\layout { \context { \Score printInitialRepeatBar … \override Grob.font-size = #n / font-series / font-shape } }`（role→grob 表 `TwinGrobsOf`＝近似と明記・mark は RehearsalMark＋SectionLabel・text は TextScript＋TextSpanner・pedal は Sostenuto／UnaCorda）・title/composer は `\markup { \fontsize #n \normal-text \bold "…" }`・navigation は `EmitNavMark` の markup 内（RehearsalMark を汚さない）・**`size` は警告「no LilyPond spelling; write step」**。plan は `PageModel(tree, render).Fonts`（collector と同じ物を読む＝2 つ目の解決を持たない）。**LSP**: `AfterFontRoleKey` は引用符＋属性 6（裸の serif/sans は**外した**＝拒否綴りを補完しない）・`AfterFontAs`／`AfterFontNumber`（空）／`FontEntryOpen`（`LastFontKeyBefore` で開いた entry を判定・家族 entry は除く）。**TextMate**: `as FAMILY`／属性語／数値・`EditorColouringTests` は属性語も両方向。

★★ **⑷ 射程（証明）**: **掃き `scratch/p355/sweep355.ps1`（922 冊＝追跡 598＋ユーザー 324・base＝HEAD `bfb45a55` の Release exe `exe-base`／head・`svg --combined`）: MOVED 0**／台帳 810 全点不動／snapshot 248 不動。**perf: 計算は足していない**（既定経路は `SizeOf` の辞書 2 回の TryGetValue が増えただけ・`ScoreTextMetrics.Face` のキャッシュは不変・plan 無しの本は `Bundled` のまま）。⚠️ **測っていないこと**: 打鍵経路の alloc（`fonts.Size` の呼びは全部 O(1) で alloc 0 のはずだが bench は回していない・memory「perf は回数で」）。

★ **⑸ 数**: **開始時裏取り**（`tools\Session-Check.ps1 -Build -Test -Scratch p355`）: HEAD **`bfb45a55`**・**未 push 46**・`origin/master`＝`21efb993`・木 clean・未追跡 0・build 0 エラー／Core 0 警告・**full 7298 / 0 / 4 / 7302**（`scratch/p355/run1.trx`＝引継ぎと一致）・`gh run list` 5 本緑（最新 34121145135）。**途中**: `run2.trx` 7403 / 2 / 4 / 7409＝赤 2 は目録 2 本（APPROXIMATIONS・magic_constants＝行番号のみ・再生成）／`run3.trx` 7404 / 1 / 4 / 7409＝赤 1 は APPROXIMATIONS（step/size の範囲に LILYSHARP-OWN 2 本を付けた分＝OWN 126 → 128・再生成）。**終了時**: **commit 2 本（`850bf455`＝製品 Core 25 ファイル＋Lsp 3＋TextMate 1＋番人 5 ファイル（新規 1）＋CHANGELOG＋GRAMMAR／GRAMMAR_FOR_LLM／SYNTAX_REFERENCE／APPROXIMATIONS／magic_constants（2 度 amend＝OWN タグと目録）／この HANDOFF＋ARCHIVE）・未 push 48・木 clean・未追跡 0・full 7405 / 0 / 4 / 7409（`scratch/p355/run4.trx`・+107＝`FontAttributeTests` 44 本の Theory 展開）**・台帳 810／snapshot 248／追跡 `.lys` 598 全部不動。§7.5（`-DiffBase bfb45a55`・commit 後）: Core `+` 1342・LILYPOND-REF 2（`scm/lily-library.scm magstep`・`define-grobs.scm BarNumber (font-series . bold)`）・LILYSHARP-OWN 2（step/size の範囲＝言語の限界）——**1342 行に REF 2 は監査対象**（RULES §7.5）: 読み直した結果、幾何の式は 1 本も足していない（全部「既定を 1 口に通す」配管と、既に REF を持つ家（magstep・box-padding・`\smaller`）に plan の歩を*足し込む*変更）。⚠️ **scratch は `p355`**（`p354` は第353）。⚠️ **§7 3.5**: 第352 のブロックを `Session-Check.ps1 -Archive 352` で ARCHIVE の先頭へ・第353 の経緯は下の見出しの下に残した。⚠️ ripple: main が掃きで busy のとき standby へ落ちた命令は今回は 1 回目で走った（memory の症状は再現せず）。

✅ ★★★ **第 2 便（ユーザー「有利なら着手」）＝F-fonts 段階 ⑵ のうちテキスト face の 4 role を表に足した（commit `94e4a315`）**: **instrument**（`SharedRenderer.Connectors.InstrumentNameEm/Style`＝右端の測りと描画が同じ 1 メソッド・既定 3.0＝`FontSize×0.75` は LP の 2.2 ではない＝動かさず remark に明記）／**stanza**（`StanzaNumberEngraver.Em/Style`・既定 2.4 も LP の 1.96 ではない＝同じく明記・予約無し）／**partCombine**（`PartCombineAnalyzer.LabelEm/LabelStyle`・描画だけ）／**ottava**（`OttavaBracketEngraver.LabelEm/LabelStyle`＝描画・自分の skyline 対・`LabelInkCentre`・`LineStartX`・Stacker の 5 か所）。**`PlanReachOf` は 17 role**。**残る 8**＝Emmentaler 数字の fretFrame／figuredBass／fingering／tabFret（glyph run＝`StepOf` で歩を連れる chordName 臨時記号の形が要る）と bend／tabTechnique／clefOctave／meter（描画点のリテラル・notation）。**Theory の本**: main Book に `part melody "Vln."` と `@ottava … @!ottava`（"8va"）を足し、**stanza は 3 節の別 Book**（"1." "2." "3." は volta の "N." と同じ文字列＝1 頁に同居できない）、partCombine は `combinedStaff { fl ob }` の別 Book（"a2"）。**掃き head2: 922 冊 MOVED 0**・full `run5.trx` 7405 / 0 / 4 / 7409（本数不動＝Theory の行は role 数で決まる）・目録 2 本再生成（行番号のみ +16/−16）・§7.5 Core `+` 1434／REF 2／OWN 2・未 push 49・木 clean。docs 3 本と CHANGELOG の role 一覧を 17 に。

✅ ★★ **第 3 便（同じ指示の 3 度目）＝テキスト face の残り 3 role を表に足し、数字 4 role は*着手しないと判定*した（commit `5039541f`）**: **tabTechnique**（`ArticulationEngraver.TabTechniqueEm/Style`＝予約の ink box と描画の 2 読み手）／**bend**（`SharedRenderer.BendLabelEm 1.6`・描画 1 か所）／**fretFrame**（`FretFrameLabelEm 1.1`＝"Nfr"・描画 1 か所）——後 2 つの裸リテラルに名前と LILYSHARP-OWN（Lily# 固有の装置・予約無し）を付けた。**`PlanReachOf` は 20 role**。**着手しなかった理由（判定の記録）**: fingering／figuredBass は `FingeringGlyphRun`／`FiguredBassGlyphRun` の `Em`・`Design`・`Font` が static で、`DigitRun` のメモ表（10 桁）・`BassFigureAlignment.RowOffsets(columns)`・`FiguredBassEngraver.ColumnUpSkyline/RowInkBelowStaff`・`MultiStaffLayouter:3069`・`PagingSkylines 432/888` など **fonts の無い静的経路 25 か所級**を通る＝取りこぼしは Theory（描画）では見えず予約側に残る。1 便の文脈利点より広さが勝つので別便（着手時は `DigitRun(fonts, n)` でメモを step 0 に限る形）。tabFret は tab 幾何の島。clefOctave／meter は notation の「broad binding を受けない」規則を size/style にも敷くかの決定が先。Theory は `GuitarBook`（`@hammeron`＝"H"・`@bend(full)`＝"full"・`@frame(x57565)`＝"5fr"）を足した。**掃き head3: 922 冊 MOVED 0**・full `run6.trx` 7405 / 0 / 4 / 7409・目録再生成（magic_constants に新 2 行＝命名した 2 定数・OWN 128 → 130）・§7.5 Core `+` 1477／REF 2／OWN 4・未 push 51・木 clean。

✅ ★★★ **第 4 便（同じ指示の 4 度目）＝第 3 便で「着手しない」と判定した fingering／figuredBass を*このセッションで*表に足した（commit `f4c34d84`）**。**判定を覆した理由**: 第 3 便は「fonts の無い静的経路 25 か所級＝取りこぼしが予約側に残る」を広さの根拠にしたが、**`Em`・`Design`・`Pieces`・`Width`・`InkTop`・`InkBottom` の*署名*に fonts を足せば、取りこぼしはコンパイラが全部列挙する**（静的な既定を残す設計だけが黙って取りこぼす）＝広さは文脈利点の側に回る。**形**: `FingeringGlyphRun.Step(fonts) = FontSizeStep(−5) + fonts.StepOf(Fingering, EngravingEm)`・`FiguredBassGlyphRun.Step(fonts) = FontSizeStep + fonts.StepOf(FiguredBass, EngravingDefaults.FiguredBassFontSize)` の**1 つの歩**が `GlyphMetrics.AtFontSize(step)`・`EmmentalerDesignSize.ForFontSizeStep(step)`・em を一緒に連れる（chordName の `AccidentalStep` と同じ）。**PlanReach は Size だけ**（グリフ列にスタイルは無い・`fingering bold` は LYS8018＝表と validator と Theory が同じ答え）。**付け替えた家（実測で列挙されたぶん）**: `FingeringEngraver`（`DigitRun(fonts, n)`＝メモは彫版の歩のときだけ・`Calculate`×3・`CalculateWithTips`・`BuildLayouts`）／`ArticulationEngraver`（`Calculate`・`CalculateWithFingerings`・`InsideSlurScriptLayouts`・`FingeringScriptLayout`）／`SkylineBuilder.AddFingeringsToSkyline`（static → instance）／`FiguredBassEngraver`（`Calculate`・`StackRows`・`ApplySkylineDrop`・`ColumnUp/DownSkyline`・`RowInkBelowStaff`・`FigureInkTop`）／`BassFigureAlignment.RowOffsets/ColumnDepth`／`LayoutEngine` の Annotations・PagingSkylines（432・888）・Finishing（`ComputeFingeringIslands`）・Prelim（`LayoutPreliminaryStaffSlurs`）／`MultiStaffLayouter` 4 か所（**譜ごとの `new Score(...)` は plan を運ばないので MultiStaffScore の `score.TextMetrics` を渡す**＝ここが static 既定なら黙って取りこぼした場所）／`SharedRenderer.Marks` の `DrawFingerings/Live`・`DrawFiguredBass`・**`FoldFingeringPage` は `fonts.Plan.Signature` をハッシュに入れた**（打鍵で歩が変わればページが差し替わる）。**`PlanReachOf` は 22 role**。Theory の sample は `EmmentalerGlyphs.FingeringDigit3`／`FigBassDigit6`（`@finger(3)`・`@fig(6)` を main Book に足した）・`FiguredBassTests`／`RenderedGeometry` の直呼びは `ScoreTextMetrics.Bundled`。**掃き head4: 922 冊 MOVED 0**（`FingeringGlyphRun.Step(Bundled)` は −5 そのもの・`DigitRun` のメモも既定経路で生きる）・対象 1048 本＝赤 1（`fingering step +1` が「無視される」側の InlineData に居た＝表が変わった証拠・`fingering bold`／`figuredBass italic`／`tabFret step +1` に差し替え、`fingering step +1`／`figuredBass size 3` は「従う」側へ）・full `run7.trx` **7409 / 0 / 4 / 7413**（+4＝差し替えた InlineData の純増）・目録 2 本再生成（行番号のみ +87/−87・OWN 130 不動）・§7.5 Core `+` 1664／REF 2／OWN 4（+187 行は全部 fonts 引数の配管・幾何の式は 0）・未 push 53・木 clean。

✅ ★★★ **第 5 便（ユーザー決定 ⒜ 2026-09-09「3 つとも従わせる」）＝clefOctave と tabFret を表に足し、meter は理由を名指して外に残した（commit `b2fde398`）**。**問いの訂正**: 第 4 便 ⑹ は「broad binding の扉をどうするか」をユーザー判断としたが、**size／style には家族層が無い**（`serif step +1` は LYS8015）ので、`tabFret step +1`／`notation bold` という**名指しの扉しか存在しない**＝face の規則が求めた狭い扉に既に居る。ユーザーへはそう訂正して 3 択（⒜ 3 つとも／⒝ 従わせない／⒞ 文字 2 つだけ）を出し、⒜ を得た。**tabFret**: `TabConstants.FretEm(fonts)`／`FretStyle(fonts)` を 1 口に、`FretDigitHeight`・`FretBaselineDrop`・`FretGlyphWidth`・`StemBeginOffset` が fonts を取り、`TabChordColumns.FretWidth/Offsets` と **`TabStaffGeometry`（コンストラクタで `Fonts` を受ける＝11 か所の生成点をコンパイラが列挙）** が読む＝描く数字・列の予約（`TabItemHalfExtent`→`ApplyTabChordSpacing`）・弦線の噛み跡・skyline 箱・符尾の近端・タイの clearance・スラーの障害物・grace 数字が**一緒に**動き、**弦間隔は動かない**（LP の `TabNoteHead.font-size` が `staff-space` を触らないのと同じ・ユーザーに前提として明示）。plan の配管: `ElementCoordinator.LayoutTies/LayoutSlurs`（public・internal 両 overload）・`BuildTieSpecification`・`BuildTabSlurLayout`・`TupletBracketEngraver.Calculate`（tab 梁の geom）・`SkylineBuilder` の tab seed 2 本（static→instance・`_fonts`）・renderer の Tab／GraceNotes／Beams。**clefOctave**: `SharedRenderer.ClefModifierEm(fonts, change)`／`ClefModifierStyle(fonts)`＝描画 1 か所・予約無し（clef が skyline を seed する）。**meter を外に残した理由（`PlanReachOf` の remark に明記）**: 複合拍子の「+」は feta run 内の Lily# 固有 fallback 文字で、**その advance は TimeSignature の X extent＝spacing core の列算術（`CalculateNoteheadRightExtent`・change-column の walk・`BoundaryColumn.Build`）が読む**。そこに plan は届いておらず、通すには SpacingRules 静的族＋BoundaryColumn＋CalculatePrefixWidth 3 overload＋SystemBreaker＋テスト約 40 か所に fonts を通す別便になる。**描く側だけ従わせると「+」が予約の枡を越える**（step +6 で 2.8 em 対 2.2 の枡）＝この表が禁じる黙った割れ方なので、**meter は LYS8018 のまま**（doc 4 本にその理由を書いた）。⚠️ **観測**: 「+」の advance は em 4.0 regular で測り、グリフは 2.4 bold で描く＝同じ量の 2 つ目の綴りが既に在る（LILYSHARP-OWN・MOVED 0 のため触っていない・meter の便で一緒に畳む）。**`PlanReachOf` は 24 role**。Theory: `GuitarBook` に `clef treble_8` と `tab gtr`・`b'\2`（8vb 下で B3＝2 弦開放 "0"）・sample "8"／"0"・LYS8018 の InlineData は meter に差し替え、`notation bold` は「従う」側へ。**掃き head5: 922 冊 MOVED 0**（`FretEm(Bundled)` は 3.0 そのもの・`FretDigitHeight` は同じ face を同じ em で測る）・対象 784 本緑・full `run8.trx` 7410 / 1 / 4 / 7415＝赤 1 は `LpReferenceCitationTests.CitationsThatNameNothing_DoNotGrow`（`ClefModifierEm` の REF 行が `font-size`＝2 節のハイフン語しか名指さず「無名の引用」が 681→682＝ratchet・**3 節の `clef-modifier-interface` を名指して直した**・ついでに自分が書いた「font-size −2」という数は読まずに書いた数だったので消した）→ full `run9.trx` **7411 / 0 / 4 / 7415**（+2＝InlineData の純増）・目録 2 本再生成（行番号のみ +136/−136・OWN 130 不動）・§7.5 Core `+` 1895／**REF 3**（+1＝`ClefModifierEm` の ClefModifier 引用＝grob の font 属性の話で幾何の式ではない）／OWN 4・未 push 55・木 clean。

✅ ★★★ **第 6 便（ユーザー「通して」2026-09-09）＝meter を spacing core の奥まで配管し、F-fonts 段階 ⑵ を閉じた（commit `c89a2a2d`）**。**形**: `MeterGlyphRun.PlusEm(fonts)`（既定 `PlusEngravingEm` 2.4）／`PlusStyle(fonts)`（既定 Bold）が描く側の 1 口、**advance 側は run の em 4.0 を同じ step で歩かせる**（`FallbackEm(fonts) = Em·magstep(StepOf(Meter, 2.4))`）＝「+」の 2 つの em（advance 4.0 regular・グリフ 2.4 bold）は**畳んでいない**（畳めば複合拍子の本が全部動く＝別件・remark に名指し）。描く側の縦位置 0.55 も em に比例。`Pieces/Width(fonts, text)` → `GetTimeSigWidth(fonts, …)` → **その読み手全部**: `SolvePrefixColumns/CalculatePrefixWidth`（BreakAlignSpacing＋SpacingRules 3 overload）・`TimeCourtesySuffixWidth`・`GetTimeSignatureChangeWidth`→`ChangeItemColumnWidth`→`CalculateNoteheadRightExtent`・change-column の walk 10 本（`BoundaryChangePrefix`・`MidMeasureChangeGaps`・`MeasureChangeColumn`・`MidMeasureChangeRightGap`・`MidMeasureChangeOffsetWithin`・`ChangeColumnGlyphAdvance`・`IsLooseChangeColumn`・`LooseChangeColumnHangDistance`・`ChangeColumnItemSpring`・`BarlineToFirstColumnSpring`）・`BoundaryColumn.Build` とその読み手（`BoundaryClefAllowance`・`MmrRodMinimumDistance`・`SkipOpenedBarFirstSpring`・`EmptyBarSprings`・renderer の `BoundaryClefX`）・その上の spring（`CreateSpring`・`CalculateSkylineDistance`・`SeparationRodDistance`・`NoteColumnToBarlineFloorPair`・`CreateSpringsForMeasure`・`CalculateMeasureIdealWidth`・`MusicalInkOverhangsPerColumn`・`MeasureLayouter.CreateTimingSprings/LayoutColumns/LayoutItems`）・`SystemBreaker`（gate 幅 2 本・greedy・MMR rod）・`LineStartColumn`・`SkylineBuilder.KeyChangeSeedX`・**`MeasureContentKey`（畳む clef allowance）**・renderer の `DrawTimeSignature` 9 か所と `EnumerateStaffItems`。Core は全部 `score.TextMetrics`（MultiStaffScore）を渡す＝静的既定は 1 つも残していない。**テストは 115 か所 16 ファイルを `scratch/p355/fix-tests-fonts.ps1`（正規表現 1 本・改行保存）で `ScoreTextMetrics.Bundled` に**。`PlanReachOf` は **25 role＝全 role**（meter は Size|Style＝「+」はテキストなので style も効く）。Theory: `MeterBook`（`time 3+2/8`）・sample "+"・LYS8018 の Warns データは Emmentaler 2 role の style だけに。**掃き head6: 922 冊 MOVED 0**（`FallbackEm(Bundled)` は 4.0·magstep(0)＝4.0 そのもの・複合拍子の本も 1 バイト動かず）・対象 521 本緑・full `run10.trx` **7410 / 0 / 4 / 7414**（−1＝Warns の InlineData 2 本を外し DoesNotWarn に 1 本足した差）・目録 2 本再生成（+72/−71＝行番号の移動・行番号を伏せた集合比較で増減 0）・§7.5 Core `+` 2117／REF 3／**OWN 5**（+1＝`PlusEngravingEm` 2.4＝「+」は LP が markup で綴る Lily# 固有の文字）・未 push 57・木 clean。

⇒ ★★★★ **⑹ 次の一手**: **⒜ push は禁止のまま**。**⒝ F-fonts は段階 ⑴⑵⑶ 全部入った＝§2F は閉じた**。残る拾い箱は ⒞。**「+」の 2 つの em を畳むか**（advance 4.0 regular 対 グリフ 2.4 bold・LILYSHARP-OWN・複合拍子の本が全部動く）は、畳むなら**先に LP 双子で `\compoundMeter` の「+」幅を測る**（観測者 0・低優先）。**⒞′ 第 3 便→第 4 便→第 5 便→第 6 便の教訓（RULES に足す候補）**: 「広い付け替え」を別便に回す根拠は「取りこぼしが黙る」ことであって数ではない——署名に通す設計なら数はコンパイラが数える（第 4 便・第 6 便＝spacing core 約 60 メソッドでも 3 回のビルドで閉じた）。静的な既定を残す設計だけが黙る（第 5 便が meter を警告で止めた理由も同じ）。**⒞ F-fonts の拾い箱（観測者 0）**: sustain の "Ped." グリフ列は `pedal step` に従わない（doc に明記・LP は `SustainPedal.font-size` で縮む＝乖離）／双子の role→grob 表は近似（`@text` の expressive は Lily# では dynamics role・双子は TextScript）／`size` を双子に書く手は無い／instrument 3.0・stanza 2.4 の既定は LP と違う（台帳点を作ってから動かす）。**⒟ 第353 ⑹ の並びはそのまま**（`time none` の縁 3 つ・`lymelmel.lys` stale／`lyrics-verses` bar 4 の 0.19／MIDI の途中 `partial`／拍グリッド／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8b・U8／A/B/D/E／C⑴／G）。**⒠ 段端の floor の regime**（第351 ⑹⒟）は今も未測。

## 以下は第353セッションの経緯

最終更新 第353セッション（2026-09-08）＝**入り方は第298〜第351 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。**第352 ⑹⒝ の残る決定 ⑤ `time none` を彫った**（§2F ⒤・§3 2026-09-08 行）——観測者 0 なので **fixture＋LP 双子の対を先に建てて LP 2.26.0 の挙動を 4 つ実測し**、その数へ製品を合わせた。**掃き 921 冊 MOVED 0**（`time none` を書く本は追跡・ユーザーとも 0 冊＝既存出力は 1 バイトも動かない）・台帳 810 不動・snapshot は再ベース 0（新規 1 枚）。**製品は commit 済み（下 ⑸）・push は禁止のまま**。**骨は 6**:

★★★★ **⑴ LP の実測（`scratch/p354/lp/`・`pair.ps1 -Lys <lys> [-Ly <手直し.ly>]`＝第351 の型＋`settings.ly` に BarNumber／TimeSignature／Beam の刷り出し `PROBEBN`／`PROBETS`／`PROBEBEAM`）**: **⒜ `\cadenzaOn` は `\set Timing.timing = ##f` の 1 行**（`ly/property-init.ly:283-284`・`\cadenzaOff` は `##t` に戻すだけで measurePosition を触らない）。**`timing` が偽のあいだ `Timing_translator::start_translation_timestep`（`lily/timing-translator.cc:478-507`）は measurePosition を進めず `currentBarNumber` を増やさない**。**⒝ cadenza の中の素の `|` は bar check で小節線を描かない**（最初の双子＝`lysc ly` HEAD 版は `\cadenzaOn` の後も `|` を書いていて **LP の絵は cadenza に小節線 0 本・`\time 4/4` の後も 0 本**（`\cadenzaOff` を書かないので数え続けない）・bar check 警告 1）。**手直し `senza-fixed.ly`（`\bar "|"`×2＋`\cadenzaOff \time 4/4`）で小節線 5 本＝20.357 / 49.089 / 75.919（1 段目）・14.645 / 22.795（2 段目）**。**⒞ 2 段目の BarNumber は 2**（bar 1 が 4/4、cadenza の 2 小節と次の `c1` が全部「2」、`d1` が 3）。**⒟ cadenza に自動連桁は無い**（8 分音符 8 つで Beam grob 0・陽性対照 `senza-control.ly`＝同じ 8 分を 4/4 で Beam 2 本）——機構は `scm/auto-beam.scm:82-90 default-auto-beam-check` の end 規則 `(zero? pos)`＝measurePosition が凍っているので毎 stem で「ここで終われ」。**⒠ `\time` は event ごとに刷る**（`senza-reprint.ly`: `\time 4/4 … \time 4/4` で PROBETS 2 本・3/4 も同じ）＝cadenza から戻る `\time 4/4` は印字される（Lily# が毎 `time` を描くのと同じ）。⚠️ **fixture の最初の綴りは相対オクターブで `c' d' e' …` と書いて 1 音ごとに 1 オクターブ上がり**（LP も同じ絵・幅は一致したが番人の絵として不適＝目視で気付き順次進行に書き直した。**2 段が 2 頁に割れて見えたのはその高さのせい**で、書き直したら 1 頁）。

★★★ **⑵ 製品（Core 12 ファイル＋Cli 0・全部 `time none` の腕で既存経路は不変）**: **⒜ `MeasureBuilder`**＝`_senzaMisura`（ctor／`SetMeasureLength(length, senza)`／`TimeSignatureChangeItem` で切替）・**凍った時計**（`AddItem`／`AddDuration` は `_currentDuration` を進めず auto-complete しない＝LP の measurePosition）・`HasMeasureContent`／`AtPieceOpening` は新設 `_hasMeasureContent`（duration>0 の item が入ったか）で答える（凍った時計では `_currentDuration > 0` が嘘になる）・`AtMeasureBoundary` は senza では「item 0」だけ・**閉じた measure に `Measure.Unmetered`**（`EmitMeasure`／`EmitEmptyMeasure`／`FinalizeMeasures`）・`| |` の spacer は最後の拍子の長さ（`MeasureModel` は Zero と言う＝両者を読む検査は senza では走らないので未統一・remark に書いた）・checkpoint に `SenzaMisura`。**⒝ `MeasureCollector`**＝2 か所の `new MeasureBuilder(…, _meta.TimeSenzaMisura)`／`MusicWalk` の `time` 腕が `IsSenzaMisura` を `TimeSignature` に運び **senza の `TimeSignatureChangeItem` は `Blanked = true`**（LP は grob を作らない＝幅もインクも無し・`Blanked` の doc に追記）／`Form.cs` の section header time と score 復帰も同じ（復帰の比較に `builder.SenzaMisura != _sectionResetTimeSenzaMisura` を足した＝senza の section の次は 4/4 を描き直す）／`Measure` を組み直す 5 か所（`MeasureCollector.cs`×2・`Stems.cs`×2・`StaffAccidentalColumns.cs`）に `unmetered:` を運ぶ。**⒞ 小節番号**＝`BarNumberEngraver.NumberMeasures(measures, offset)`（Unmetered を数えない表）を `Annotations` が渡し `Calculate` の `displayedNumbers` が index+1 に優先。**⒟ 連桁**＝`BeamDetector.DetectBeamGroupsInMeasure` が senza では Phase 0（書かれた連桁）の後で返る・`AddDetectionInputs`／`MeasureContentKey` に `Unmetered`・`BeamingPattern` の LILYSHARP-OWN 文を「観測者は手動連桁の subdivision だけ」に直した。**⒠ 報告**＝`TimeSignature.ToString()` が senza で `none`（`lysc layout` が `time 4/4 -> none (bar 2) -> 4/4 (bar 4)`）。**⒡ 双子 `LilyPondExporter`**＝`_timeSenza`／`_homeTimeSenza`（frame buffer にも）・`EmitTime` が senza から戻るとき `\cadenzaOff \time N/M`・section 復帰の条件に senza・**`|` は senza の中では `\bar "|"`**（`EmitBarline`）。

★★★ **⑶ 番人**: **`SenzaMisuraTests` 10 本**（5 小節・Unmetered の並び・senza 変更は Blanked／戻りは印字・**番号 [1,2,2,2,3] と page の "2"**・自動連桁 0＋陽性対照 2＋手書き連桁 1・top-level `time none`・section header と復帰・**小節線 5 本＝LP の 20.357 / 48.501 / 73.533・14.645 / 22.795 の ±0.05**（raggedRight 両側・Lily# 20.36 / 48.50 / 73.53・14.65 / 22.80＝cadenza の幅は音符だけで決まり両エンジン一致）・双子の綴り・validator 無音・`| |` placeholder）／**snapshot `test/senza-misura` 新規 1 枚**（目視済み・`SvgSnapshotTests` の一覧に理由つき）／`MeasureContextChainTests` の fixture 一覧に追加。**docs**: GRAMMAR §2 `TimeDecl`（`'time' , 'none'`＋規則 12 行）・§8.1 `MidMusicCommand`・GRAMMAR_FOR_LLM・CHANGELOG 0.7.0 Language・APPROXIMATIONS 再生成（行番号）。⚠️ **引用の門 `CitationsThatNameNothing_DoNotGrow`**（行範囲の引用は記号名を添える・`cadenzaOn` は記号に数えられない）＝`property-init.ly` は範囲なし・`timing-translator.cc:478-507 Timing_translator::start_translation_timestep`／`auto-beam.scm:82-90 default-auto-beam-check`／`auto-beam-engraver.cc:171-182 Auto_beam_engraver::consider_end` を名指しに直した。

★★ **⑷ 射程（証明）**: **掃き `scratch/p354/sweep354.ps1`（921 冊＝追跡 597＋ユーザー 324・base＝HEAD `10bdbe08` の exe `exe-base`／head・`svg --combined`）: MOVED 0**／台帳 810 全点不動（full の `LpGeometryLedgerTests` 緑）／snapshot 247 → 248（新規 1・再ベース 0）。**perf: 計算は足していない**（builder の分岐・番号表 1 本・beam の早期 return）。

★ **⑸ 数**: **開始時裏取り**（`tools\Session-Check.ps1 -Build -Test -Scratch p354`）: HEAD **`10bdbe08`**・**未 push 41**・`origin/master`＝`21efb993`・木 clean・未追跡 0・build 0 エラー／Core 0 警告・**full 7284 / 0 / 4 / 7288**（`scratch/p354/run1.trx`＝引継ぎと一致）・`gh run list` 5 本緑（最新 34121145135）。**途中**: `run5.trx` 7295 / 2 / 4 / 7301＝赤 2 は引用の門と APPROXIMATIONS（上 ⑶・両方直した）。**終了時**: **commit 2 本（`8d36d8aa`＝製品 Core 17 ファイル＋番人 3＋fixture 1＋snapshot 1（新規）＋CHANGELOG＋GRAMMAR／GRAMMAR_FOR_LLM／APPROXIMATIONS（`LILYSHARP_UPDATE_DOCS=1`・行番号 16 行）／この HANDOFF＋ARCHIVE）・未 push 43・木 clean・未追跡 0・full 7297 / 0 / 4 / 7301（`scratch/p354/run6.trx`・+13＝番人 10・snapshot 1・chain 1・+1 は引用の門の再計数）**・台帳 810／snapshot 248（+1 新規・再ベース 0）／追跡 `.lys` 598（+1 fixture）。§7.5（`-DiffBase 10bdbe08`・commit 後）: Core `+` 242・LILYPOND-REF 8（`timing-translator.cc:478-507 start_translation_timestep`×3・`auto-beam.scm:82-90 default-auto-beam-check`・`auto-beam-engraver.cc:171-182 consider_end`・`property-init.ly cadenzaOn` 範囲なし×3）・LILYSHARP-OWN 0（BeamingPattern の既存文を直しただけ）。⚠️ §7.5 は commit 前に打つと 0 と出る（作業ツリーを見ない）。⚠️ **scratch は `p354`**（`p353` は第352）。⚠️ **§7 3.5**: 第351 のブロックを `Session-Check.ps1 -Archive 351` で ARCHIVE の先頭へ・第352 の経緯は下の見出しの下に残した。⚠️ **ripple の main が掃きで塞がっているとき待機コンソールへ落ちた 1 回目は `cd` だけが走る**（memory どおり）＝同じ命令をもう 1 度打つ。

✅ ★★ **第 2 便（ユーザー「有利なら着手」）＝拾い箱 ⒞ の小物 2 件を閉じた（commit `48f5d03f`）**: **⑴ MIDI**＝`MidiExporter.ProcessTimeSignature` は senza で何も書かず走っている拍子を保つ・section header の `time none` も境界で何も戻さない（LP の `\cadenzaOn` は `timing` を触るだけで `timeSignature` は動かず、`Time_signature_performer::process_music`（`lily/time-signature-performer.cc:102-115`）は `\time` event か分数の変化でしか書かない）・番人 `SenzaMisuraTests` +1（3/4 → `time none` → 3/4 と header `time none` で conductor は 3/4 だけ）／**⑸ `MeasureModel` の `| |`**＝senza でも最後の拍子の長さ（collector と同じ答え・読む検査は senza では走らない）。**SVG のコードは触っていない**ので掃きは `8d36d8aa` のまま・full `run7.trx` 7297 / 1 / 4 / 7302（赤 1＝APPROXIMATIONS の行番号・再生成して門は緑）・未 push 45・木 clean。

✅ ★★ **第 3 便（同じ指示の 3 度目）＝拾い箱 ⑵⑷ を LP で測った（製品 0・記録だけ・`scratch/p354/probe-*.lys`）**: **⑵ 小節途中の `time none`**（`c'4 d time none e8 f g a b c d e f4 g | time 4/4 c1 |`）＝**LP も自動連桁 0**（PROBEBEAM 無し）＝Lily# の「小節ごと」の規則と絵は同じ。残る差は `c8 d time none e8 …` のように**同じ小節で `time none` の前に 8 分がある**ときだけ（LP は前半を梁で結ぶ・Lily# は結ばない・観測者 0）。⚠️ **LP は小節途中の `\cadenzaOff \time 4/4` を「小節の続き」と読む**（measurePosition 1/2 のまま→ warning "mid-measure time signature without \partial"・`c1 |` で bar check 1/2）＝**戻りも小節途中なら双子は `\partial` が要る**（Lily# は `time 4/4 c1 |` を満杯の 1 小節と読む＝絵は Lily# が正で、双子の側の穴）。**⑷ cadenza の反復**＝inline `|:` は LYS1034 で拒まれる（form に書け）ので、**form 反復 `form main { A |: B :| C }` で B が `time none`**＝双子は `\repeat volta 2 { \cadenzaOn … \bar "|" … }` を書き **LP は cadenza の中でも反復小節線を描く**（小節線 4 本 20.357 / 43.636 / 55.149 / 65.849）。**幅は cadenza の小節だけ違う**（LP 23.279 対 Lily# 20.99＝−2.29・`:|` の後の `c1`＋4/4 再印字 10.7 対 12.6＝+1.9）——**陽性対照 `probe-formrepeat-ctrl.lys`（B を `time 2/4` に）は反復記号の参照点の ±1（第351 ⒝）以外 exact**（11.25/11.24・8.641/8.65・13.302/13.3）＝**残差は「反復小節線に接した cadenza」固有・未分類・観測者 0**。

⇒ ★★★★ **⑹ 次の一手**: **⒜ push は禁止のまま**。**⒝ 第352 の決定 9 件は全部入った**（⑤ が最後）。**⒞ 拾い箱（`time none` の縁・⑴⑸ は第 2 便で閉じ、⑵⑷ は第 3 便で測った＝残るのは上の 3 つの未分類）**: ~~⑴ MIDI~~／⑵ **小節の途中の `time none`**（`c4 d time none e8 f g a |`）は連桁の判定が小節単位（`effectiveTimeSig`）なので `c4 d` 側も自動連桁が止まる・LP は位置ごと＝未測・観測者 0／⑶ **senza の中の `partial`** は時計が凍っているので効かない（LP も `\partial` は Timing の measurePosition＝同じく無意味）・診断無し／⑷ **双子の `|:`／`:|` は `EmitInlineRepeat` 経由で cadenza の腕を通らない**＝senza の中の反復は未測／~~⑸ `MeasureModel` の `| |`~~（第 2 便で統一）。**⒟ 第352 ⑷ の拾い箱はそのまま**（`lymelmel.lys` stale／`lyrics-verses` bar 4 の 0.19／MIDI の途中 `partial`）。**⒠ 第351 ⑹⒞ の並びの残り**（拍グリッド＝実需ゼロ／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8b・U8／A/B/D/E／C⑴／G）。**⒡ 段端の floor の regime**（第351 ⑹⒟）は今も未測。
