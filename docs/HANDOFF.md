...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第240セッション＝**第239 が測って断った 7 件目を、再現からではなく機構から着手して閉じた。五線を持たない score が form の演奏順を無視していたのを、`ProcessForm` と同じ順序で歩かせた**（1 commit＝`bfe79e4c`・4 ファイル・+348/−28）。⚠️⚠️ ★★★ **追跡 572 冊は 0 冊動いた**——**しかしそれは「届いていない」ではない**: 毒（Core 3 ファイルを `git stash`）で**新テスト 9 中 8 が赤**になる。**届く形をコーパスが 1 冊も綴らない**（第238② ・第239① と同じ形の **3 例目**）。⚠️ **台帳 573／ss 非ゼロ 115／総和 3.957938014／count 109・非ゼロ 2／snapshot 222／追跡コーパス 572 は 1 つも動いていない。**

**① 欠陥は歌詞ではなく `EnsureSectionStartsForRows`＝`ProcessForm` の 2 つ目の綴りにあった**（`MeasureCollector.cs`）。**score のトラックが全部「行」のとき**（`lyrics N` / `chords N` だけで staff が無い）**collect は `ProcessForm` を 1 度も通らない**——`CollectMultiStaff` の行分岐が `continue` で音楽の walk を丸ごと飛ばすので、代わりに立っているのがこの関数。**そこに欠陥が 3 つ**:
- **⒜ `|: … :|` を丸ごと跨いでいた。** 全アームが `!IsInsideRepeatBlock` で門番されており、**ブロック自身を受けるアームが無かった**⇒ 繰り返しの中の section は**小節を 1 つも取らず**、その後ろが前の上に重なる。
- **⒝ form が 2 度名指す section を 1 度しか記録しない。** `ContainsKey` の門で、しかも else 側が `cur = StartMeasure[name] + secBars`＝**カーソルを 1 度目の位置へ*巻き戻す*。**
- **⒞ `AllStarts` を 1 度も書かない。** ⇒ 行の collector 2 軒（`LyricsCollector` / `ChordNameCollector` の `StartsFor`）が**単一アンカーの fallback へ落ち**、再現部のセルが 1 度しか組まれない。
⇒ **`RecordSectionStart`（＝`ProcessForm` が呼ぶのと同じ 1 軒）を通し、カーソルは各パスの幅ぶん前へ進める**。足りていなかったアーム＝**繰り返しブロック**（`ProcessRepeatBlock` の slot walk を鏡写し・`:|:` 込み）・**囲いの無い alternative**・**`~Name`**。

**② ★★ 測っていて出た 4 つ目＝`LyricSyllableReader.CountBars` が `[N. …]` の節を 0 と答える**。**直下の `LyricMeasure` 子だけを数えていた**ので、**全部が角括弧で書かれた section は小節が 1 段下に居て 0**。**それが報告された本の綴り**で、2 つのパスの*あいだ*の section が幅 0 になっていた。⇒ **`[N. …]` は同じ小節の*別の連*なので、答えは「最も広い連」（総和でも 0 でもない）**。⚠️ **`VoltaMeasures` は `LyricSyllableReader` へ 1 軒に寄せた**——**グリッドが数える小節と `LyricsCollector` が置く小節が同じ物であること**が要点（2 つ目の綴りを増やさない）。

**③ ★★★ 網は「差分」にした＝`RowsOnlyFormOrderTests`**。**同じ本を 2 度組む——`score` に `staff melody` を入れた版と入れない版**——**`lyrics verse` はどちらでも同じ `CollectRow` が組む**ので、**違うのは section の start がどこから来たかだけ＝それがまさに接ぎ目**。⚠️ **2 つの綴りは*畳めない***: **part を持たない chord グリッド**（`chords X { section A { … } }`）は**section をトラックの中に宣言する**ので、`ProcessForm` が歩く音楽がどこにも無い。**畳めない接ぎ目には、差分の網を張る**（骨 2）。

★ **開始時裏取り**（第239 の閉幕数と全一致）: HEAD `7e25178e`・未 push 2・未追跡 0/木 0・Windows **5874/0/4**・台帳 573／ss 非ゼロ 115／総和 3.957938014／count 109・非ゼロ 2・追跡コーパス 572・snapshot 222・Core 0 警告。
終了時: **本便は 2 commit＝`bfe79e4c` 実装／この行＝handoff**・**未 push 4**・未追跡 0/木 0・**5883/0/4**（＋9・全部新規）・**台帳 573／ss 非ゼロ 115／総和 3.957938014／count 109・非ゼロ 2／snapshot 222／追跡コーパス 572 —— 6 つとも不動**・Core 0 警告（`--no-incremental`）。⚠️ **本便は出力を動かす変更**（五線なし × section を 2 度名指す本）だが、**追跡 572 冊・snapshot 222 枚のどれもその形を綴らないので再ベースは 1 枚も要らなかった**。⚠️ **§7.5 の数え上げ＝Core の `+` 行 157・`LILYPOND-REF` 0・`LILYSHARP-OWN` 0**——**これは正しい 0**: `form { A B A }` は Lily# 自身の構文で LP に対応物が無く、**この島が要る唯一の LP の事実**（囲いの無い alternative は 1 度だけ鳴る＝`alternative-sequence-iterator.cc:83-84`）は**既に `ProcessForm` が住所を持っている**。新アームは**その家を指しただけ**（§7.6 ⒟）。数値定数は 0 個。

★ **この便の値段**:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ① 演奏順の walk | `EnsureSectionStartsForRows` に 3 アーム＋`RecordSectionStart` | **五線なし score が form どおりに鳴るようになった** |
| ② `[N. …]` の幅 | `CountBars` が volta を見る・`VoltaMeasures` を 1 軒に | **報告された本の section が幅 0 でなくなった** |
| ③ 差分の網 | `RowsOnlyFormOrderTests` 9 本 | **畳めない 2 綴りに、初めて食い違いを見る機械が付いた** |

> ## ★★★ 骨 1＝**掃きの「0 冊」は、毒を先に当てていなければ意味が決まらない**
> `cmp base after` は **0 of 572** と言い、そのうえで自分から
> **「⚠️ NOTHING MOVED — the poison missed」**と書く。**この道具は
> 「射程外だから動かない」と「届いていないから動かない」を区別できない。**
> ⇒ ★★★ **本便が 0 冊を「射程外」と読めたのは、*その前に*毒で新テスト 8 本を
> 赤にしていたから**——**逆順なら、同じ 0 冊が何も言っていない。**
> ⇒ ★★ **順番が値打ち**: **①網を書く ②毒で網が赤くなることを見せる ③掃く。**
> **③ から始めると、緑も 0 冊も両方「測っていない」の顔をする**（第239 骨 2 ⑷ の掃き版）。
> ⚠️ **そして赤くならなかった 1 本が地図になる**——**生き残ったのは `A B`**、
> **つまり追跡 572 冊が唯一綴る形。「なぜ全部緑だったか」がテスト名で読める。**

> ## ★★ 骨 2＝**2 つ目の綴りは、消せないことがある。そのときは*差分*の網を張る**
> §5.2.1② は「1 量 2 綴り」を返済せよと言うが、**この 2 つは畳めない**——
> **part を持たない chord グリッドは section をトラックの中に宣言する**ので、
> **`ProcessForm` が歩く音楽が本のどこにも無い。**
> ⇒ ★★ **畳む代わりに、両方に同じ問いを出して答えを比べる**:
> **同じ本を `staff` あり／なしで 2 度組み、行の着地小節が一致することを見る。**
> **行を組むのはどちらでも同じ `CollectRow`** なので、**違いは接ぎ目 1 点に絞られている。**
> ⇒ ⚠️ **差分の網は「正しい答え」を知らなくても書ける**——これが値打ち。
> **本便は正しい小節数を 1 つも手で書いていない**（1 本だけ独立にピンした＝
> 両方が同じ向きにずれる退行を捕まえるため）。

⚠️ ★ **「診断が拒んだ綴りが他所のテストに在る」は stale の証拠ではない**（本便が 1 度そう書きかけた）。
フィクスチャに `form main { A @segno B }` と書いて `lysc check` に拒まれ
（**ナビ記号は form では裸・`@` は音符に付く**）、**同じ綴りを持つ
`SectionOrientedParserTests:223` を「stale」と commit message に書きかけた**——
**あれは拒否を*検査している*負のテスト**だった。⇒ **疑うのは正しいが、疑った結果も読むこと。**

⚠️ **見つけて直していないもの**（全部測定済み・**次便の候補**）:
- **★★★ 次の一手＝form に書いた `|: :|` は、五線なし score の小節線に届かない。** **本便でグリッドの*位置*は正しくなったが、その周りの*線*は出ていない。**実測: `A |: B :| A "A2"` と `A B A "A2"` は**五線なしでバイト同一・五線ありで別物**。⚠️ **原因は別の機構**——`PartHasStructure` は**part の*音楽*の中**の繰り返し小節線を探すので、**form に書かれたそれは見つからず `HarvestOmittedStructure` が 1 度も走らない**。⚠️⚠️ ★★ **切り分けは済んでいる**: **⑴ 行の*セル自身*に書いた `|: :|` は今も正しく描かれる**（第226 の決定・実測で確認）＝**`LyricsCollector.CollectRow` の `barEnds`/`repeatOpens` という機械は既に在る**。**⑵ 足りないのは「form の繰り返しをその機械へ渡す線」だけ。** ⇒ ★ **`PartHasStructure` の門を広げる案は勧めない**——**打鍵ごとに隔離 collect が 1 本増える**（§7.9・要実測）うえに、**part を持たないグリッドには omitted part が無いので半分しか直らない**（§5.2.1② の「1 本の正道と 1 本の予備」）。**rows-only の walk から `barEnds`/`repeatOpens` を埋めるほうが、両方の形に届く。**
- **音楽を持つが行のセルを持たない section は、rows-only グリッドで今も 0 小節**（`RowGridSectionBars` に音楽の fallback が無い）。**本便より前から在り、本便は動かしていない。**
- 第239 が残した 5 件（stray-item メッセージ・`MidiPartRenderSyntax` の stale doc・`form main`/`score main` の無色・`NEW_SCORE_TEMPLATE` の 2 綴り・CHANGELOG）は**どれも本便が触っていない**。下の第239 の欄がそのまま生きている。

---

## 以下は第239セッションの経緯

最終更新 第239セッション＝**ユーザーが画面で見ている編集体験の欠陥を、報告された順に 6 件直した。engine は 1 行も動いていない**（1 commit＝`03b584a9`・18 ファイル・+1706/−158・**ユーザー指示で 6 島を 1 commit に**）。⚠️ **6 件は 1 つの形の別々の顔**——**compiler が既に知っていることを、画面が言っていなかった**（どのテンプレートがファイル全体なのか／どの名前が解決するのか）。⚠️ **snapshot 222・台帳 573 点・追跡コーパス 572 冊は 1 つも動いていない**——本便が触ったのは LSP・文法・拡張と、Core の*述語*だけ。⚠️⚠️ ★★★ **末尾で 7 件目（歌詞の重なり）を測ったうえで着手を断った**（⑦）。**次便の最初の一手はそれ**で、**再現からではなく「どこで form が捨てられているか」から始められる**ように切り分けが済んでいる。

**① テンプレートはカーソルに挿さるのではなく、ファイルを置き換える**。`template-twinkle` / `template-twinkle-piano` は普通のスニペットで、**選ぶと「完成した 2 曲目」——自前の `title`・`form main`・`score main` ごと——が書きかけの本のカーソル位置に落ちていた**。⇒ 項目は**何もしない編集**（打ちかけの語をそのまま打ち直す）として受理され、`lilysharp.applyScoreTemplate` を持つ。置換は拡張側がやり、**消す前に訊く**。⚠️ **no-op なのが要点**——問いは受理の*あと*に出るので、「いいえ」が書き手の手元を（探すために打った `temp` ごと）そのまま残せる。**空文字を挿す実装だと、問う前にそれを食べる。**⚠️ **中身の無いファイルには訊かない**（消える物が無い）。
⚠️ ★★ **1 本も赤くならなかった理由**＝**補完の網は全部「一覧に出るか・正しい文脈か」を見ており、*選んだら文書に何が起きるか*を読む網が 1 本も無かった**。5807 本はこの形について*ただで*一致していた（第238 ② と同じ形）。

**② ★★★ その修正がタブを持ち込んだ＝欠陥を閉じずに*動かした***（骨 1）。`\t` は**スニペットでは正しい綴り**——VS Code はスニペットのテキストを editor の `insertSpaces`/`tabSize` に合わせて打ち直す——が、**エディタが逐語で書くようになった瞬間、literal のタブになった**。**タブでも parse は通る**ので誰も赤くならない。⇒ **コーパスが決めた**: **字下げのある追跡 `.lys` 548 冊のうち 545 が 2 スペース・タブは 1 冊も無い**（実測）。**他の補完項目の `\t` はそのまま**（まだスニペット）。⚠️ **フォーマッタは規範ではない**——`Format` は client の `DocumentFormattingParams` から字下げを受け取るので、入れた物を書き換える側。
⚠️ **テンプレート本文は「1 行のエスケープ文字列」から「行配列 ＋ `string.Join("\n", …)`」へ**。raw string literal は**この `.cs` の改行**（Windows で CRLF）を運ぶが、`.gitattributes` は snapshot の `data-pos` がバイトオフセットなので `*.lys` を全 OS で LF に固定している。**字下げが目で見えるようになったのも同じ変更の効果**——1 行のエスケープだったから `\t` が誰の目にも留まらなかった。
**新規ファイル**（File ▸ New File… ▸ Lily# Score）は **42 行 → 18 行**。Twinkle 全曲（歌詞・2 番・`|: :|`・6 段落の解説）から**最小の完成した 1 曲**へ——**新規ファイルで最初にやることが「40 行消す」だった**。手本は `template-…` 補完へ移っており、残した 1 行のコメントがそこへの導線。**`title "Untitled"` の `Untitled` が選択状態で開く**ので、最初の打鍵が曲名になる（範囲は**文書を読んで**求める。行と桁の定数はテンプレートの形の 2 つ目の綴り）。

**③ part の中の section に色が付き、part body が早く終わるのが止まった**。報告は「`part melody { section A { … } }` の `A` に色が付かない」。`#part-body` は part *ヘッダ*の語彙しか持たず section が居ないので、`section` は総称キーワード規則に落ち、名前は何にも当たらなかった。⚠️⚠️ ★★★ **見えていなかった半分＝`#part-body` の `end` は `\}` で中の波括弧を誰も釣り合わせていない**⇒ **part 本体が「最初の section の閉じ括弧」で終わっていた**。修正前の実測: **A の名前は素・その音楽はヘッダ規則で読まれ**（`|` が素・`4` がただの数）、**B は——既にトップレベルへ落ちていたので——完全に色が付いていた**。⇒ ★★ **この形の目印は「2 つ目が正しく見える」こと**（第238 骨 1 の親戚。正しく見える方が「直っている」ではなく「別の経路に落ちている」）。⇒ `#part-inner-section`（`#lyrics-inner-section` の兄弟）＋ 再帰する `#nested-braces`。
⚠️ **測っていて見つけた別件（未報告・同 commit）＝裸の duration に 1 度も色が付いていなかった**。`\b(1|2|4|…)` の**先頭 `\b` は `d` と `2` の間で偽**なので、**オクターブ記号を持つ音符だけが足場を持っていた**（`c'1 d2 e4 f8 g16 a32 b64` で **7 中 2**、その 2 つが `'` 付き）。付点は末尾 `\b` がドットの*後ろ*にあるので常に落ちていた。⇒ `(?<![0-9_])(128|64|32|16|8|4|2|1)\.{0,3}(?![0-9A-Za-z_])`。⚠️ **交替を長い順に直したのが要点**——旧版は `16` が「`1`」に当たってから**末尾の `\b` のおかげで**やり直していた。**あれを外して並べ替えないと `16` が黙って `1` になる。**

**④ ★★★ 固定してあった色のうち 2 つは 1 度も発火していなかった**（骨 3）。`package.json` は `section` #4FC1FF と `phrase` #DCDCAA を最初から `[*]` に固定していたが、**サーバの legend が 9 型でそのどちらも宣言していない**⇒ **その 2 行は誰も発行しない型に色を塗っていた**。section 名の色は TextMate 経由でテーマ任せ——**2026 既定テーマではオレンジ、Dark+ では*色なし***（出荷テーマファイルを直接読んで実測）。⇒ **`part` #4FC1FF・`section` #FFA657・`phrase` #D2A8FF** を legend に足して木から発行。⚠️ **旧 `phrase` #DCDCAA は articulation が既に持つ hex**——発火していたら `@staccato` と見分けが付かなかった。**死んでいたおかげで助かっていた。**

**⑤ 名前は「解決したときだけ」色が付く**（ユーザー決定）。form の section 名（`A` と `~A` の 2 綴り）、score の part 参照**8 綴り全部**（staff／表示名つき staff／ossia／tab／grandStaff／condensedStaff／combinedStaff／裸の名前＝MIDI のみ）、lyrics・chords のトラック名とその行、そして lyric トラックが `sings` する part。**診断が未定義と言う名前は素のまま**——⚠️ **これは行単位の正規表現には原理的に書けない**（`form main { A }` に A が在ることは見えても、それに意味を与える宣言が在るかは見えない）。⇒ **述語は `SectionSymbols` と `PartReferenceFinder` に 1 軒だけ置き、ノード 1 個に対して訊く**ので、**診断（LYS1005/1007/6011）と色が同じ答えを読み、semantic token の walk は 1 周のまま**（RULES §7.9）。⚠️ **`sings` の相手はより広い集合**（part **または**名前付き voice）なので voice も同じ家から取る——**part だけで解決すると、診断が受理する名前を素のままにする**（誤って色を付けるより静かで、見つけにくい側）。⚠️ **トラックの名前空間は分けたまま色だけ共有**（混ぜると `staff prog` が通り、それは LYS1007 が捕まえるための空の五線）。
⚠️ ★★ **1 ビルドだけ、まさにその「半分色」を自分で作った**——`sings` の相手は**トラック名と同じノードに乗っている**ので、`else if` の連鎖で宣言として先に掴んだノードからは二度と探しに行かず、**`verse` 青・`melody` 素**になった。**一つのノードが二つの名前を持つとき、else-chain は片方を黙って落とす。**

**⑥ トップレベルの Ctrl+Space に `chords` が出る**。**score の中では出ていた**（`chords NAME` の行）ので語としては知られて見え、**その行が名指す先を作る宣言だけがどのポップアップからも辿れなかった**＝**置けるが宣言できないトラック種**。⚠️ **スニペットは `lyrics` からコピーせず実測**——chord トラックは `sings` を取らず、**名前は省略できない**（`chords { … }` は LYS0032）。

**⑦ ✅ 第240 で閉じた**（`bfe79e4c`。**以下は逐語の記録で、開いている項目ではない**——第240 の欄が現在地。⚠️ **残っているのは「form の `|: :|` が五線なし score の*小節線*に届かない」だけ**で、それは §1 冒頭の欄に在る）**＝五線を持たない score が form を展開しない**（`scratch\ベースタブLy\Untitled-6.lys`・**第239 は着手を断った**）。報告は「歌詞が重なる」。**黒箱で 1 変数ずつ切った結果、変数は staff でも歌詞の番数でもなく「form が同じセクションを 2 度以上名指すこと」**:

| 本 | form | 結果 |
|---|---|---|
| 五線あり・2 番 | `A \|: B :\| A "A2"` | 正しい（12 小節・A B B A） |
| 五線なし・1 番 | `A \|: B :\| A "A2"` | **壊れる** |
| 五線なし・2 番 | `A \|: B :\| A "A2"` | **壊れる**（報告の本） |
| 五線なし・2 番 | `A B` | **正しい**（6 小節） |
| 五線なし・2 番 | `A B A` | ⚠️ **6 小節。2 度目の A が*丸ごと消える*** |

⇒ **五線を持たない score は各セクションを 1 回だけ組み、form の演奏順を無視する。**`A B A` では 2 度目が消え、`|: B :| A "A2"` では**繰り返しの分の歌詞が、1 度しか組まれていない同じ小節へ降ってくる**——それが「重なり」の正体。⚠️ **歌詞を動かして直そうとすると、消えている 2 度目の A が残る。**
⚠️ **なぜ全部緑か**＝**追跡の score ブロック 575 のうち五線を持たないものは 11**（lead-sheet 族・chord grid 族）で、**その 11 は全部、form が各セクションを 1 回しか名指さない**。⇒ この形をコーパスが 1 冊も綴らない（第238 ② と同じ）。
⚠️⚠️ ★★★ **最初の切り方「五線なし × 複数セクション」は*対照が反証した***——`samples/greensleeves.lys` の `greensleeves-grid`（五線なし・`form main { Verse Chorus }`）は**正しく出る**。**強い言明を書く前に対照を 1 枚描いたから助かった**（RULES §5.0）。⚠️ **その対照は `lysc png --score greensleeves-grid` で名指して描く**——既定は最初の score ブロックで、そちらは五線を持つ。
★ **次便への引き渡し**: 上の表は再現手順そのもの（本は `$env:TEMP` ではなく作り直せる。`score main { lyrics verse }` だけの本＋form を差し替える）。**始める場所は「staffless の経路がどこで form の演奏順を捨てているか」**——`score` に staff が無いときに collector が通る道が、staff が在るときと違う。⚠️ **snapshot は動かない見込み**（追跡の 11 冊はどれもこの形を綴らない）ので、**新しい fixture が要る**＝この島は「出力を動かす変更」として扱うこと。

★ **開始時裏取り**（第238 の閉幕数と全一致）: HEAD `347c5e09`・未 push 0・未追跡 0/木 0・Windows **5816/0/4**・台帳 573／ss 非ゼロ 115／総和 3.957938014／count 109・非ゼロ 2・追跡コーパス 572・snapshot 222・Core 0 警告。
終了時: **本便は 2 commit＝`03b584a9` 実装／この行＝handoff**・**未 push 2**・未追跡 0/木 0・**5874/0/4**（＋58・全部新規）⚠️⚠️ ★★ **handoff commit の SHA を §1 に書いてから `--amend` したら、その SHA が孤児になり `HistoryCitationTests` が即座に赤くなった**（第238 骨 4「番人は*建てた側*を捕まえる」の 3 便連続 3 例目）。**天井を上げるのは番人が名指しで禁じている**ので直したのは散文——⇒ ★ **handoff commit は自分の SHA を書かない。`この行` と書く**（この repo の綴りはそのためにある）。・**台帳 573／ss 非ゼロ 115／総和 3.957938014／count 109・非ゼロ 2・追跡コーパス 572・snapshot 222 —— 5 つとも不動**・Core 0 警告（`--no-incremental`）・`tsc --noEmit` 0。⚠️ **本便は engine を 1 行も動かしていない**ので、§5.1 の「出力同一の証明」3 点は**掛ける対象が無い**（絵を作る経路に触れていない）。

★ **この便の値段**:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ① テンプレート全置換＋確認 | LSP の項目＋拡張のコマンド＋テスト 2 ファイル | **「選んだら何が起きるか」に初めて計器が付いた** |
| ② タブ／新規ファイル／タイトル選択 | テンプレート本文の綴り・`extension.ts` | **①が持ち込んだ退行を閉じた**（骨 1） |
| ③ 文法 2 件 | `.tmLanguage.json` の 2 規則＋duration 正規表現 | **part body が早く終わるのを止めた**・**裸の duration が初めて色を持った** |
| ④ 名前の semantic token | legend 3 型・固定色 3 つ | **死んでいた固定色 2 つが生きた**（骨 3） |
| ⑤ 解決したときだけ色 | 述語 1 軒・walk は 1 周のまま | **診断と色が構造的に食い違えなくなった** |
| ⑥ `chords` の補完 | 項目 1 つ | **置けるが宣言できない**を閉じた |
| ⑦ ★★★ **断った 7 件目** | 変更なし（黒箱 8 枚・国勢調査 1 回） | **次便が再現からではなく機構から始められる** |

> ## ★★★ 骨 1＝**値を*誰が*適用するかを変えると、その中のエスケープの意味が変わる**
> `\t` は**スニペットでは正しい綴り**だった——client が editor の設定で打ち直すから。
> **カーソル挿入をやめて WorkspaceEdit で書くようにした瞬間、同じ 1 文字が literal のタブになった。**
> ⇒ ⚠️ **欠陥を閉じたのではなく、動かした。**しかも**タブでも parse は通る**ので、
> 5816 本のどれも赤くならない。**症状だけが別の場所へ移った。**
> ⇒ ★★ **判定法**: **その文字列を消費するのは誰か**を変える変更では、
> **中の全エスケープを「新しい消費者にとって何を意味するか」で読み直す**（`\t`・`$0`・`\n`）。
> **本便は `$0` には気づき（テストで pin した）`\t` を落とした**——**同じ文字列の中で。**

> ## ★★★ 骨 2＝**「毒を戻した」は、測るまで戻っていない。1 便で 4 回踏んだ**
> ⑴ **過剰置換**: `{ "include": "#comments" },` で戻したら**ファイル全体の 7 か所**に当たった
> （意図は 1）。⑵ **もう 1 度**: doc コメントの中の `title ""` を巻き込んで、
> **それ自体が矛盾した散文**を残した。⑶ **stale なバイナリ**: `Copy-Item` の復元は
> **タイムスタンプが古い**ので増分ビルドが再コンパイルを飛ばし、**毒入り DLL が生き残って
> 無関係のテストを 3 本赤にした**（§5.5 の族）。⑷ **空振り**: here-string の改行が合わず
> **anchor 数 0＝毒が当たっていないのに suite は緑**。
> ⇒ ★★★ **毒は 3 点セットで回す**: **⑴ anchor 数が 1 であることを両方向で確かめる
> ⑵ 復元をハッシュで照合 ⑶ 復元後にタイムスタンプを現在時刻へ。**
> ⚠️ **⑷ が一番危ない**——**当たらなかった毒は「壊れていない」ではなく「測っていない」**
> （§5.4 ⑸ の「動く本を 1 冊見せる」の毒版）。**緑を根拠にする前に、毒が生きていることを示す。**
> ⚠️ **⑶ が捕まえにくいのは、赤くなるのが*関係ないテスト*だから**——
> 「なぜこれが赤い？」が唯一の入口で、そこで止まらなければ誤った結論を持ち帰る。

> ## ★★ 骨 3＝**固定した値と、それを読む口が無いことは、固定した側のファイルからは同じに見える**
> `package.json` は `section`/`phrase` の色を最初から書いていた。**サーバの legend が
> その型を宣言していないので、その 2 行は誰も発行しない型に色を塗っていた。**
> **エラーはどこにも出ない**——色は「テーマ由来」に静かに落ちるだけ。
> ⇒ ★★ **2 つのファイルが名前で握手する所には、握手を見る網を置く**
> （`EveryPinnedColour_NamesATokenTypeTheServerEmits`＝**両方向**。
> 固定はあるが型が無い＝死んだ規則／型はあるが固定が無い＝テーマ任せ）。
> ⚠️ **同じ形は本便に 3 つ在った**: 拡張のコマンド名（C# が名指し TypeScript が実装）・
> テンプレートのタイトル欄（テンプレートが提供し拡張が選択）・この色。
> **3 つとも接ぎ目に網を置いた**——**ビルドが繋がない握手は、テストが繋ぐしかない。**

⚠️ **見つけて直していないもの**（次便の候補・全部測定済み）:
- **file レベルの stray-item メッセージが 5 つの合法な宣言を名指していない**——`lyrics`/`chords`/`fonts`/`paper`/`drummap`（`ParseTopLevelItem` は受理する）。**書き手が間違えたその瞬間に読む唯一の一覧**で、`DocKeywordListTests` は **score 本体の同じ一覧**には機械を持っているが**ファイル本体には無い**。⑥ と同じ形の 1 段下。
- **`MidiPartRenderSyntax` の doc が stale**（`partName channel:1 instrument:25`——`:` オプションは 0.4.0 で消えた）。**そこから書いたフィクスチャが `lysc check` に拒否された。**
- **`form main` / `score main` 自身の名前が無色**（part/section/phrase は色を持つようになった）。
- **`extension.ts` の `NEW_SCORE_TEMPLATE` と LSP のテンプレートが同じ曲の 2 つ目の綴り**。用途は別だが必ずずれる。
- **CHANGELOG 未更新**——`RELEASING.md` が「章はリリース時に書く」としているので意図的。⚠️ **本便は 6 件とも*利用者に見える*変更**なので、次のリリースで拾うこと。
⚠️ ★ **`lysc check` がフィクスチャを 5 回止めた**（`midi main { … }` は存在しない綴り／`condensedStaff { 1 つ }` は拒否／`p` は予約語／`voice` はスパンを 1 度だけ開く）。**記憶から書いたフィクスチャは、記憶ごと間違う**——**書き下ろす前に 1 冊通すのが、この repo では毎回元が取れている。**
