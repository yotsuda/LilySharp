...
**⑴ 段間の rod を「最小の枠」で組み直せるか測る**（⒁ の機構を数量で確かめる）: **やること** ⒜ Lily# 側で、段間の床を**譜を整列の最小に置いた skyline**から出したらいくつになるかを試算する（`MultiStaffLayouter.StaffSprings` の `MinimumDistance` と `StaffSpringCompression` が既に在るので、配置済み skyline を「最小の枠」へ戻す量＝各譜ばねの圧縮和は計算できる。第336 ⑻ の `BreakerRefpointFrame` がブレーカー側で同じ変換をしている）。⒝ その値で ⒀ の表の「床」列を置き換え、**Lily# のばねが rod に乗っているか**を確かめる（同一 spec・同一の力なら自由なばねは全部同じ長さになるので、6 本が違えば 5 本以上は rod 上＝⒁⒜ の論法）。⒞ 一致したら**台帳点**を `system.staff-tab.inter-system-minimum-frame` の型で LP と対にして起票 → 移植（**出力を変える＝承認事項**）。⚠️ **譜対そのものも Lily# 8.326／LP 8.927**＝1 段の差 3.05 のうち 0.60 はこちら。**同じ枠の話の裏表**である可能性が高い（rod を最小の枠で測れば頁の力が変わり譜対の伸びも変わる）ので、**別項として数えつつ一緒に検算する**。⚠️ **LP の床を「両端インクの単純和」で組むのは誤り**（⒁⒜ で自己矛盾を出した）。組むなら最小の枠の skyline 距離。
