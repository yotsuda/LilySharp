...
最終更新 第202セッション＝**§2F ⒣「拍子欄を誰も描かない本が*中途の*拍子変更の欄を予約する」を
閉じた便**（**実装 3 便**。**perf は測った＝pass を 1 本足したため。alloc は base と平坦**）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **第201 の §1 ⑹ が候補を 3 つ挙げ、
「0.3.0 の門という観点では correctness の残りは ⒝（＝§2F ⒣）だけになった」としていた。**
**ユーザーの指示は「読んで作業に着手して」で、着手先の選択はこちら。⒜（描画側の shaping）は
第201 が測って「門の内側ではない」と書いており、⒞（perf）は correctness ではない。**
★★★ **ユーザー決定 0 件**——**この便は 1 度も訊いていない。訊く前に測ったら、
決めるべきことが 2 つとも*事実で*決まったため**（下の骨 ⑴ ⑵）。
★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 518 点・ss 非ゼロ 98・
総和 3.609965521・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 5**・
**snapshot 217 枚**・suite **5400 passed / 0 failed / 4 skipped**
＝**引継ぎの数は全部合っていた**（**32 回連続**）。
★★ **第201 の「間で push が 1 回あった」も裏が取れた**——**`origin/master` は
`9830873f`（第201 の第1便）を指しており、開始 5 は「61 →（push）→ 5」の右端**。
**§0 が要求する「両端＋間の push」はこの形で成立する。**
 終了時 **未追跡 0・作業ツリー項目 0・未 push 9**（**この行を書く commit を含めて 9**。
⚠️ **この行を*書いた時点*では作業ツリー項目 2**＝`docs/` の `HANDOFF.md`・`HANDOFF-ARCHIVE.md` が
未 commit で、**commit 後の実体は 0**）・
suite **5400 → 5410 passed / 0 failed / 4 skipped**（**+10**＝**台帳 3 ＋ 単体 6 ＋ snapshot 1**）・
**台帳 518 → 521 点**（**足した 3 点は全部 exact で閉じた**ので
**ss 非ゼロ 98・総和 3.609965521・count 106/2 はどれも不動**）・
**snapshot 217 → 218 枚**（**再ベース 0 枚**＝**増えた 1 枚は観測者として*新しく*足した本**）・
**LP 回帰コーパス 0 / 81**・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Turmeric`）で通せた。**

> ## ★★★ この便の骨＝**起票が名指した 2 つの regime のうち、片方は*直っていない*のではなく*届かない***
> **第198 の起票は「`tab … as numbers` だけの本と、譜が 1 つも無い本（コード/歌詞のみ）」
> の両方が中途の拍子欄を予約する、と書いていた。**
> **後者をレンダして測ると、2/4・8/16・16/32 でジオメトリが*厳密に同一*だった。**
> ⇒ ★★★ **`chords` は自分の `time` を取らない**（`'chords' takes chord entries and barlines`）
> **ので、中途の変更は score が描かない part にしか住めず、列の一覧に一度も入らない。**
> ⇒ ★★ **だから正しい文は「もう直っている」ではなく「その経路が無い」**——
> **前者だと網を書きたくなり、後者だと注記になる**（書いたのは注記）。
> ⚠️ **その 0 は毒で裏を取ってある**——**同じ本の score を `staff melody` に替えると
> 2/4 と 16/32 で動く**（§5.3「0 冊を証拠に引く前に、届くことを見せる」）。

> ## ★★★ 2 つ目の骨＝**LP は grob を*外して*いない。*空白にしている*。だから欄は「幅 0」ではなく「無い」**
> **`ly/engraver-init.ly:1214-1220` の TabStaff ブロックは `\remove Key_engraver` の 5 行下で
> `\override TimeSignature.stencil = ##f` と書く**——**Time_signature_engraver は残っている。**
> **grob は非音楽列に*在って*、X extent が空**（**プローブの dump が `ext=(+inf.0 . -inf.0)` と
> 印字する＝LP の空区間**）。**そして extent を読む 2 つの walk がそれを飛ばす**:
...
> `last_grob` にもならない**）。
> ⇒ ★★★ **移植の形がここで変わった**——**「`ChangeItemColumnWidth` に 0 を返させる」実装は
> *間違い*で、それでも `(first-note . (semi-shrink-space . 2.0))` を払い続ける。**
> ⇒ ★★★ **バイトで裏を取った**（§5.0 の「受理は対照とバイトで決まる」）:
> **`\set Timing.measureLength = #1/2` で*同じ小節格子*を作った本**（拍子 grob は 1 つも作られない）
> **と `\time 2/4` の本が SVG バイト同一**（`29AD0B45…`・8394 バイト）。
> ⚠️ **毒も同じ run に入れてある**——**`\tabFullNotation` の 2 本（FN/FW）は差が出る**
> （11810 対 12674 バイト）。

> ## ★★★ 3 つ目の骨＝**identity だけでは足りなかった。2 点目が*別の site* を出した**
> **点を 2 つ開けたのは §5.0 の「対称な誤りは identity から定義により消える」だけが理由だった**
> ——**幅が digit に依らない欄を予約していたら、2/4 対 16/32 の identity は exact のまま嘘をつく。**
> ⇒ ★★★ **そして実際に出た**: **2 つの列 walk を移植した時点で identity 2 本は 0 になり、
> `change-bar-vs-plain-bar` だけが +0.150000 で残った**——
> **`BarlineToFirstColumnSpring` の min_dist walk が `ClefChangeItem` *だけ*を飛ばしていた。**
> **それまで exact だったのは「key/time が在ると必ず別の枝へ行く」から**で、
> **空白の拍子はその枝へ行かない**ので、**音符として測られていた。**
> ⇒ ★★ **`IsChangeItem` に直すのは、それ以前のどの本にとっても no-op**
> （**そこへ来られる change item は clef しか無かった**）。

> ## ★★★ 4 つ目の骨＝**score 単位の答えを、9 つの署名ではなく item に載せた**
> **問いは score 単位**（`AnyStaffEngravesTime`＝paper column が全譜を束ねるから）**なのに、
> 譜は collect が声部を建てるまで存在しない**（`RenderSpec.ToStaffGroups` は collect 済みの
> 声部を取る）——**walk の内側では誰も答えられない。**
> ⇒ **代替は `bool` を spacing の入口 9 つと呼び手 6 軒へ通すこと**で、
> **どれか 1 つを忘れられる形**（§5.2.1②「本経路と fallback」）。
> ⇒ ★★★ **`TimeSignatureChangeItem.Blanked` に載せた**——**LP 自身の機構が
> *grob のプロパティ*なのだから、これは発明ではなく移植。**
> ⚠️ **`IsChangeItem` は*わざと*変えていない**——**空白の拍子も*非音楽列の* item ではある**ので、
> **`MeasureLayouter.ItemStartingAt` は今までどおり飛ばさねばならない**
> （さもないと零長 grob がその瞬間の音符としてスカイラインへ渡る）。**問いは 2 つで、
> LP も 2 つに分けて答えている。**
> ⚠️ **pass は*参照同一性*で memo する**——**`RenderSpec.ToStaffGroups` は notation 譜と
> tab 譜に*同じ* `Voice` オブジェクトを渡す**ので、譜ごとに書き換えると模型が倍になり
> `ReferenceEquals` が壊れる（`MidMeasureChangeOffsetWithin` がそれで列内の位置を引く）。

> ## ★★ 5 つ目の骨＝**門を置く場所を、既存の網が訂正した**
> **最初 `SvgGenerator.CollectScore` に置いた**が、**第198 が書いた行頭側の網
> （`TabOnlyKeyPrefixTests`）は `new MeasureCollector().CollectMultiStaff(...)` を*直接*呼ぶ。**
> ⇒ **そのままだと、網はレンダ経路が決して作らない模型を測ることになる**（§5.2.1② の最悪形）。
> ⇒ **`CollectMultiStaff` 自身の最後の 1 行へ移した**（単一譜の包みだけ `SvgGenerator` に残る）。
> ★ **「door は 1 つ」と思ったら、既に居る呼び手を grep すること。**

★ **この便の値段**（**perf は測った**＝§7 の「足した例：pass を 1 本増やした」。
**stash した base との min-of-3 A/B で `perf-plain1k` full 693.9→694.0・打鍵 45.2→45.2、
`perf-lyrplain1k` full 256.3→256.3・打鍵 75.8→75.8**＝**すべて計器の帯の内側。**
⚠️ **厳密な対照になる本は無い**——**`AnyStaffEngravesTime` の walk は全 collect で走る**。
**ただし最初の notation 譜で返るので、コーパスのどの本でも O(1)。**
⚠️ **これは*時間*については何も言っていない**——**この機械は 5〜15ms を判定できない**（§7）。）:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ 台帳 3 点の起票＋LP プローブ | `audit/` 2 ＋ Tests 2（**Core 0 行**） | — |
| ⑵ 移植（`Blanked` ＋ 2 walk ＋ pass） | Core 7 ファイル ＋ 網 6 本 | **0 / 81 冊・snapshot 0 枚** |
| ⑶ コーパスの観測者 1 冊 | Fixtures 1 ＋ Snapshots 1 ＋ Tests 1 | **snapshot 217 → 218** |

- **⑴ ★★★ 第1便＝穴を測ってから開ける**（`66b0b639`・**Core 0 行**）。
  **プローブ `audit/lp-geometry/probes/tab-numbers-meter.ly`＝7 score**
  （**TN 素の tab に中途 2/4／TW その identity 双子 16/32／TL 同じ格子を `measureLength` で／
  MN・MW 中途を*小節の中*に／FN・FW `\tabFullNotation` の対照**）。
  **台帳 3 点を開いた**: `mid-piece.tab-numbers.meter-identity` **+1.229159055**／
  `mid-piece.tab-numbers.change-bar-vs-plain-bar` **+3.454735433**／
  `mid-measure.tab-numbers.meter-identity` **+1.229159055**。
  ★★★ **`+1.229159055` は幅の欠陥ではなかった**——**LP が拍子を*描く*ときに払う幅と 9 桁一致**
  （FN/FW の bar が 41.34217208065051 対 42.571331135768624＝差 1.229159055118114）。
  **Lily# の幅の模型は正しく、欄が在ることだけが誤りだった。**
  ★★ **`+3.454735433` は第198 が*行頭で*測った 3.4548 と同じ数**＝**1 つの量が 2 か所で
  予約されていて、198 が片方だけ閉じていた。**
  ⚠️ ★★★ **3 点目は最初 `0.000000000` を出し、それは何も言っていなかった**——
  **`BarlineRightToNextFret(1)` を測っていたが、小節線から自分の最初の音符までの間隔は
  *局所*なので、上流の欄が広がっても動かない**（§5.3「0 は『測れていない』の顔で出る」）。
  **変更を*またぐ*歩幅（`RenderedGeometry.TabFretStep`）に替えて +1.229159055 が出た。**
- **⑵ ★★★ 第2便＝移植**（`36c2e6f2`）。**`TimeSignatureChangeItem.Blanked`（`##f` の移植）／
  `SpacingRules.ChangeItemHasInk`（2 つの skip の移植）／`Svg/Collector/MeterStencil.Blank`
  （collect の最後・`AnyStaffEngravesTime` が false のときだけ書き換え・参照同一性で memo）。**
  **3 点とも exact。**
  ★★ **網 6 本は*機構*を主張する**（台帳点はジオメトリなので「欄が幅 0 で在る」実装でも満足する）:
  **`BoundaryChangePrefix` が null／`MidMeasureChangeGaps` が null／
  それでも `IsMidMeasureChangeColumn` は true**、ほか。
  ★★★ **毒 3 つを当てて、それぞれ*別の*集合が赤くなった**——
  **⒜ `ChangeItemHasInk` を常に true＝機構 2 本＋台帳 3 点／⒝ pass を効かなくする＝それに旗の網が加わる／
  ⒞ min_dist walk を元に戻す＝境界の網と 2 点目*だけ***。
- **⑶ ★★ 第3便＝コーパスの観測者**（`ebba7523`）。**`test/tab-numbers-meter-change`**。
  ★★★ **snapshot は 1 枚も再ベースしていない**——**baseline は*直したあと*に取った**ので、
  **間違った絵を承認する機会が 1 度も生まれていない**（§5.2.1③ の裏返し）。
  ★★ **その本が「自分の名乗る機構を観測している」ことを毒 2 つで確かめた**（§5.0）——
  **どちらの毒でも第3小節の最初のフレット数字が 33.62 → 37.08 へ動く**（＝3.46 の欄）。
  ⚠️ **中途の変更はこの本に入れていない**——**中途の拍子変更はどちらかの拍子と必ず食い違う**
  （**Lily# は LYS2001・LP は bar check 失敗**）ので、**規則を観測するために恒久の警告を
  コーパスへ足すことになる**。**その半分は台帳点と単体網が観測している。**
- **⑷ ★★ §2 の頭（perf）は 1 バイトも触っていない。地図と次の島は第193 の §1 ⑷ のまま。**
- **⑸ ★★★ 次に触るならここ**——**§2F ⒣ が閉じたので、0.3.0 の門の内側に
  correctness の項は 1 つも残っていない。** **候補は 2 つとも門の外**:
  **⒜ 描画側の shaping 2 軒**（**第201 の §1 ⑹ の記述がそのまま生きている**。
  **PNG は Skia に family 名で引き直す／PDF は `embedded` なしだと stand-in を描く**。
  **予約は正しく、ずれるのは shaping ぶん**＝**門の内側ではない**。
  **1 手目は「PNG/PDF のテキスト配置をどう観測するか」で、観測者は今も 0 本**）／
  **⒝ §2 の頭の ▶ perf**（**第193 の地図のまま**）。
  ⇒ ★★★ **だから第199 の見立てが今そのまま現在地になった**——
  **「次に何を直すかは*ユーザーが自分の本で見つけるもの*」。**
  **0.3.0 を出す判断そのものがユーザーのものなので、次便は§0 の裏取りを済ませたら
  まずそこを訊くのが筋**（**⒜/⒝ はどちらも門の外の磨きで、優先は「文法を完全にしてから出す」**）。
- **⑹ ★★ この便が見つけて動かさなかったもの**: **第201 ⑺ の「予約と描画でスタイルが食い違う
  site 3 つ」は今も未修理**（§2F・**直すと版面が動く**）。**この便は 1 バイトも触っていない。**

---

## 以下は第201セッションの経緯

最終更新 第201セッション＝**§2F の最後の本体「名指し face は*描かれる*が*測られない*」を閉じた便**
（**実装 5 便**。**perf は測った＝キャッシュ鍵を増やしたため。alloc は base と平坦**）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **第200 の §1 ⑻ が「次に触るならここ」として ⒜ を挙げ、
「0.3.0 の門に一番近い／`font { }` を出荷した以上*出荷済みの挙動*になった」としていた。**
**ユーザーの指示は「読んで作業に着手して」で、着手先の選択はこちら。**
★★★ **ユーザー決定 1 件、そしてそれは*過去の決定の上書き***:
**⒟「埋め込むときだけ実測」→「解決できたら常に実測。`embedded` は PDF 埋め込みだけ」。**
**訊く前に前提を測ったら ⒟ の前提が半分しか正しくなかった**（下の骨 ⑴）。
★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 518 点・ss 非ゼロ 98・
総和 3.609965521・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 61**・
**snapshot 217 枚**・suite **5387 passed / 0 failed / 4 skipped**
＝**引継ぎの数は全部合っていた**（**31 回連続**）。
★★ **未 push は 8 便続けて両端が繋がった**——**第200 の終了時 61 と開始時 61 が一致**。
⚠️⚠️ ★★★ **そして*セッションの途中でユーザーが push した***——**`origin/master` はいま
`9830873f`（この便の第1便）を指す**。**だから終了時の未 push はセッション中の commit 数ではない**:
**開始 61 →（第1便で 62）→ push →（第2〜5便）→ 終了時 5**（**この行を書く commit を含めて 5**。
⚠️ **この行を*書いた時点*では作業ツリー項目 3**＝`docs/` の `HANDOFF.md`・`HANDOFF-ARCHIVE.md`・
`RULES.md` が未 commit で、**commit 後の実体は 0**）。
⇒ ★★ **次便が引き算するときは「間で push が 1 回あった」を見ること**——
**§0 が要求する「両端を書く」は、間の push を書かないと成立しない。**
・suite **5387 → 5400 passed / 0 failed / 4 skipped**（**+13**＝**漏れ止め 3 ＋ face 鍵 8 ＋
端から端まで 2**）・**台帳 518 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・
**snapshot 217 枚で不動＝再ベース 0 枚**・**LP 回帰コーパス 81 冊も 0 / 81**・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Turmeric`）で通せた。**

> ## ★★★ この便の骨＝**設問の前提を測ったら、取ってあったユーザー決定のほうが倒れた**
> **⒟ は「`font "X" embedded` のときだけ実測する（そこだけは決定的）」だった。**
> **実コードに当たると layout は 1 本**——**PDF も SVG も `new LayoutEngine().Layout(multiScore)`**
> ——**なので `embedded` が買うのは*PDF の自己整合*だけで、機械間の決定性は買わない。**
> ⇒ ★★★ **つまり ⒟ は「1 つのキーワードに 2 つの仕事」（PDF 埋め込み＋版面の切替）を
> させる形で、しかもその 2 つ目は約束を果たさない。**
> ⇒ ★★ **§5.0「設問に出す前に前提を 1 冊で測る」の 2 例目**（第185 が 1 例目）。
> **測らずに ⒟ を実装していたら、濁った文法を出荷していた。**

> ## ★★★ 2 つ目の骨＝**測らないほうが Lily#-own の逸脱だった。LP は名指し face を測っている**
> **`lily/font-select.cc:193-217 select_font`** が **`font-name` の文字列を
> `pango_font_description_from_string` で記述子にして `find_pango_font` へ渡す**
> ——**LP のテキスト extent は fontconfig が解決した face から出る。**
> ⇒ ★★★ **`TextFontMetrics` の「THERE IS NO SYSTEM-FONT FALLBACK, deliberately」は
> *台帳を守るため*の判断で、製品の挙動として LP に従っていたわけではなかった。**
> ⇒ ★★ **§7.5 の監査がもう 1 つ出した**——**「同梱を機械より先に見る」も LP 自身の順序**
> （`lily/font-config.cc:43-78 make_font_config` が datadir の `00-lilypond-fonts.conf` を
> fontconfig の既定 conf より*前*に積む）。**REF を書こうとしたから読んだ。**
> ⇒ ★ **逆に、機械に無い face で*throw* するのは LP に対応物が無い**（fontconfig は必ず
> 何かを返す）＝**`LILYSHARP-OWN`。§7.5 は 2/0 で始まり、監査して 3/1 になった。**

> ## ★★★ 3 つ目の骨＝**「台帳に漏れない」はアーキテクチャの性質だった。開ける前に*述べて検査する*性質へ移した**
> **`score.Fonts` は `SharedRenderer.cs:98` の `doc.Fonts = score.Fonts` で*描画コンテキストにだけ*
> 渡っていた**——**`Svg/Layout` の 54 の測定呼び（15 ファイル）は 1 つも plan を見ていない。**
> ⇒ **だから今日まで台帳 518 点は漏れようがなかった。**
> ⇒ ★★★ **穴を開ける前に、その保証を*網*に移した**（第1便・コード 0 行）——
> **入口（台帳 probe が `font` を書かない）と出口（既定 plan の全 role が同梱に解決する）の 2 本で、
> 落ちる理由が違うので 2 本。** **毒も 2 つ当てて分離した。**

> ## ★★★ 4 つ目の骨＝**端から端までの網を、毒が 2 回書き直させた**
> **⑴ 1 度目は `font-family` だけ剥いで比べていた**——**毒の下で*緑*。**
> **見ていたのは幾何ではなく `data-pos`**（束縛の 1 行ぶんソース位置が全部ずれる）。
> **⑵ `data-pos` も剥いだら、今度は*機能が入っていても*緑**——**束ねていたのが `title` で、
> タイトルは中央揃えで幅を誰も予約していない。**
> ⇒ ★★★ **実測した**（geometry-only）: **`lyricText` は動く／`title`・`chordName`・`barNumber` は動かない**
> （**短い和音記号は `SymbolWidth` の 2.0 の床の下・その本は小節番号を刷らない**）。
> ⇒ ★★ **「実測するようにした」が*ページで*観測できるのは、幅が spacing に入る役どころだけ**
> ——**コードを読んでも出てこない事実で、訊いたのは毒。**

★ **この便の値段**（**perf は測った**＝§7.9 の「足した例」——**測定ごとに face の辞書引きが 1 回増える**。
**base commit との `alloc` A/B で `perf-lyrplain1k` full 256.6→256.6・打鍵 74.6→74.7、
`perf-plain1k` full 694.6→694.5・打鍵 45.2→45.1**＝**すべて計器の帯の内側。**
⚠️ **対照になる本は存在しない**——**鍵は全テキスト測定で共有される**ので、
**テキストのある本は全部届き得る**（§5.3・第191 ⑵ の形）。
⚠️ **これは*時間*については何も言っていない**——**この機械は 5〜15ms を判定できない**（§7.9）。）:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ 台帳への漏れ止めの網 | Tests 1 ファイル（**コード 0 行**） | — |
| ⑵ `TextFace` 鍵＋名指し face の解決 | Core 7 ファイル ＋ 網 8 本 | **0 / 81 冊** |
| ⑶ layout に plan を通す（前半 38 site） | Core 15 ＋ Tests 9 | **0 / 81 冊** |
| ⑷ 残り 10 site ＋ 通らない 6 の理由 | Core 12 ＋ Tests 8 | **0 / 81 冊** |
| ⑸ 端から端までの網 ＋ 文書 3 冊 ＋ 嘘になった注記 | Core 4 ＋ Tests 1 ＋ `docs/` 3 | — |

- **⑴ ★★★ 第1便＝穴を開ける前に保証を網へ**（`9830873f`・**コード 0 行**）。
  **毒 2 つ**: **入口**（`TimedScore` に `font { title "Georgia" }`）→ **その 3 本ちょうどが赤**／
  **出口**（`Resolve` が `LyricText` に名前を返す）→ **出口の 1 本だけが赤で `lyricText` を名指し**。
  ⚠️ **最初の出口毒は `Default` の辞書を触ったので*両方*赤くなり、半分を分離できなかった**
  ——**「毒は*どの* case が赤くなったかで読む」（§5.4）の当日版。**
- **⑵ ★★ `TextFontMetrics` の鍵が `TextFace` に**（`8313bc33`）。**`Name` が null＝同梱。**
  **解決順は同梱ファイル→機械（LP の順序・REF つき）**、**機械に無ければ throw**。
  ★★ **falsifier は*同梱 sans を名前で*指す**——**どの機械でも決定的**で、
  **fallback family を serif にしておくと「名前が決めた」と「flag がたまたま合った」が分離できる。**
  ⚠️ **`(sans, style)` の綴りは*移行のアダプタ*として残っている**（1 行で forward）。
  **最後の呼び手が移ったら消す。**
- **⑶⑷ ★★★ layout が draw と同じ問いを訊くようになった**（`2ed165a5`・`350f349a`）。
  **`ScoreTextMetrics`（1 score 1 個・`(role, style)` を memo）を `Score`/`MultiStaffScore` に。**
  ⚠️ **`LayoutEngine._skylineBuilder` は readonly をやめて `Layout()` の頭で座り直す**
  ——**ctor は options しか渡されず score を知らないから。1 engine 1 score であることを
  production 6 軒すべて読んで確かめた**（注記に書いた）。
  ★★ **`MusicMarkEngraver.TextRoleOf` を 1 軒作った**（描画側も読む）。
  ⚠️ **`IsPedalMark` は既に 2 軒ある**（§5.2.1⑤ の匂い・既存）——**3 軒目を作らず、直しもしていない。**
- **⑸ ★★ 嘘になった注記を 5 つ直した**（`a7b28e58`）——**`TextFontPlan`／`TextFontFamily`／
  `FontDirectiveTests` の頭／PNG・PDF の「測っていないから shaped にしない」。**
  **後ろ 2 つは*経路は変えず理由だけ*書き直した**（§5.1「札の理由が他所にあるなら、
  直した日に札が倒れる」）。**文書 3 冊も揃えた**（`embedded` は PDF 埋め込みだけ）。
- **⑹ ★★★ 次に触るならここ**——**§2F の ⒜ は閉じたので、候補は 3 つ**:
  **⒜ 描画側の shaping 2 軒**（**PNG は Skia に family 名で引き直す／PDF は `embedded` なしだと
  stand-in を描く**。**`embedded` があれば shaped 経路に届く＝解決済みの `TextFace` を
  描画側へ通すところから**。**ずれるのは shaping ぶんで face ぶんではない**）／
  **⒝ §2F ⒣**（**`as numbers` と譜なしの本が*中途の*拍子欄を予約する**・射程 0 冊・
  **先に LP を測って台帳点を作るところから**）／**⒞ §2 の頭の ▶ perf**（**第193 の地図のまま**）。
  ⇒ ★★ **0.3.0 の門という観点では、correctness の残りは ⒝ だけになった。**
  ⇒ ★★★ **⒜ は「予約は正しく、shaping ぶんだけずれる」ので*門の内側ではない*。**
  **⒝ も射程 0 冊なので、次に何を直すかは第199 の見立てどおり
  「ユーザーが自分の本で見つけるもの」になる可能性が高い。**
- **⑺ ★★ 予約と描画で*スタイル*が食い違う site を 3 つ見つけて記録した**（未修理・上の §2F）。
  **直すと版面が動くので、配管の便に混ぜていない。**
- **⑻ ★★ §2 の頭（perf）は 1 バイトも触っていない。地図と次の島は第193 の §1 ⑷ のまま。**
- **⑼ ★★★ 第201 は「この便で ⒜ に着手すべきか」を訊かれ、見送った**
  （ユーザーの「有利なら着手／不利なら着手するな」の委任）。**不利と判断した根拠**:
  ⇒ ★★★ **⒤ 温まっている資産は ⒜ の*安い半分*にしか効かない。** **この便は
  `PdfDrawingContext` と `PngDrawingContext` のテキスト経路を両方読んで注記まで書いた**ので、
  そこは確かに次便が失う。**しかし ⒜ の 1 手目は*観測者を作ること*で**（第172 のユーザー決定＝
  「観測者ゼロの機能に型を付けようとしない。先に観測者を作る」）、**その設計はこの便が
  1 行も見ていない＝次便と同じ冷たさ。** ⇒ **第197 が「有利」と言えた条件の裏。**
  ⚠️ ★★★ **そして「観測者ゼロ」は*測って*言っている**（2026-08-18 実測）:
  **PNG/PDF の*テキスト配置*を観測している網は 0 本**——`SharedRendererPngTests` 3 本と
  `SharedRendererPdfTests` 4 本が主張するのは「valid な PNG が出る／stub より大きい／
  両方の譜が収まる／音楽 face の design が正しい」で、**どこに glyph が落ちたかを読む網は無い**。
  **PNG snapshot は 1 枚**（SVG は 217 枚）。**「どう観測するか」自体が島。**
  ／**⒥ ⒜ は 0.3.0 の門の内側ではない**——**この便が測って ⑹ にそう書いた**
  （予約は正しく、ずれるのは shaping ぶん）。**ユーザーの優先は「文法を完全にしてから出す」**で、
  **門の外の磨きにセッション末を使うと次便の頭も半開きの項に使うことになる**／
  **⒦ この §1 は §7 どおり最後の commit で確定させてあるので、着手すると
  suite 5400・台帳 518・snapshot 217・未 push 5 が 4 つとも崩れる。**
  ⚠️ ★★ **温かいがやらなかったものをもう 1 つ**: **⑺ の 3 つのスタイル食い違い**は
  **この便が見つけたので温かい**が、**版面が動く＝ LP 照合＋ snapshot 承認が要る**。
  **セッション末に承認を取りに行くのは一番悪い時機**（第200 ⒥ が名指しした罠）。
  ⚠️ ★★ **急ぐ理由がある側も書く**: **⒜ を放置すると「予約は名指し face・PNG/PDF の描画は
  shaping なし」が出荷済みの挙動になる。** **ただし射程 0 冊**（`font` を書く本は 566 冊に 0）**で、
  ずれるのは shaping ぶん（kern・合字）＝この便が閉じた桁（−2.05〜+3.61 ss）より 1 桁以上小さい。**
  ⇒ ★ **次便へ**: **⒜ を採るなら 1 手目は「PNG/PDF のテキスト配置をどう観測するか」**——
  **そこが決まらないと、`TextFace` を描画側へ通す打鍵は検算できない。**
...
- ✅ **`font "NAME"` を指定すると、予約と描画が別の face になる**（2026-07-27 起票／
  ...
  **残っているのは*描画バックエンド 2 軒*の shaping だけで、それは下の ⑸ に名前が付いている。**
  ...
  - ★★★ **ずれの大きさは測ってある**（2026-08-18・`scratch\p200\facegap`。
    ...
    **face の差が桁で勝っている。**
  - 🔶 ★★★ **残っているのはこれだけ＝*描画側の shaping* が 2 軒**（第201 が名前を付けて残した。
    **どちらも予約は正しく、ずれるのは shaping ぶん（kern・合字）で face ぶんではない**）:
    **⒜ PNG は描画 typeface を Skia に family 名で引き直す**ので、**予約が開いたファイルと
    同じ物とは限らない**（`PngDrawingContext` の `reserved` の注記）／
    **⒝ PDF は `embedded` なしだと同梱 stand-in を描く**ので、**stand-in の字を名指し face の
    位置に並べるのは*悪化***（`PdfDrawingContext` の注記）。**`embedded` があれば本物の
    program が在るので shaped 経路に届く＝そこが次の一手**で、**解決済みの `TextFace` を
    描画側へ通すところから。**
  ...
  - ⚠️ ★★ **配管の残り 6 site を名前で残す**（**測るのは同梱 face のまま**）:
    ...
    **`FetaTextRun` 3**（feta run の中の*非 feta 文字*の fallback。消費者 3 軒が
    それぞれ static の包みを持つ）。
  - ⚠️ ★★ **予約と描画で*スタイル*が食い違う site を 3 つ見つけて記録・未修理**
    （**直すと版面が動くので配管の便には混ぜなかった**）:
    **To-Coda の `"To "` は Bold で測って BoldItalic で描く**／**swing の `"="` は Bold で測って
    Regular で描く**／**dynamic のラベルは upright で測って italic で描く**。
    **3 つとも今日の数を保存し、site にスタイルを明示して残した。**
