...
最終更新 第367セッション（2026-09-11）＝**入り方は第298〜第366 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・口挟み 0）。第365 ⑺⒞ の並びから **③「title 残差 +0.21」を測り（欠陥ではなかった・下 ⑴）**、**第361 ⑺⒞⑤「`slashprobe` の +1.1」を LP から読んで移植した（下 ⑵〜⑸）**。**製品は作業ツリー＝承認待ち**（**snapshot 1 枚が動く**: `test/tab-percent-blank-bars`・patch `scratch/p368/beat-slash.patch`＝Core 11＋Tests 3＋台帳＋CHANGELOG・**未追跡の probe `audit/lp-geometry/probes/beat-slash-spacing.ly` は patch の外＝`git add` が要る**）。**骨は 5**:

★★ **⑴ title +0.21 は偽（pin 無しの svg）**: 第363 の双子 `scratch/p364/pageBreakRB.ly` は `\paper` の font pin を書かず、svg backend で serif が generic 名に落ちていた（memory `reference_lilypond_svg_backend_generic_fonts`＝第350 が probe で踏んだ罠を双子で踏んだ）。pin して描き直す（`scratch/p368/pageBreakRB-pin.ly`）と **title baseline LP 12.2642 対 Lily# 12.26＝exact**。**残る差は最初の譜の上線 LP 16.4107 対 Lily# 16.27＝−0.14**で、正体は **section label の箱の高さ**: Lily# は LP `SectionLabel` の font-size 1.5（em 2.616256）で 'A' の箱 2.60、双子は `\mark \markup \box`＝`RehearsalMark` の font-size 2（em 2.771822）で 2.744。**箱の幾何は第344 が移植済み（インク＋0.2×magstep＋0.1・底 0.85）、箱と bold は Lily# の決定と remark に明記**＝残るのは**どちらの grob の em を真似るか**の決定（⑺⒝①）。memory `reference_lilysharp_mark_box_height_is_nominal_not_glyph` は古い本文だったので書き直した。**第363 ⑺⒝③ は閉じた**。

★★★ **⑵ `slashprobe` +1.1 の正体＝beat slash の列を skip と読んでいた**。LP: body が 1 小節未満（3 小節以上の整数 body も同じ else 枝）の `\repeat percent` は反復ごとに **RepeatSlash／DoubleRepeatSlash 1 個・音符 0**（`percent-repeat-iterator.cc:75-92`・`slash-repeat-engraver.cc:56-66`）。両 grob は rhythmic-grob-interface なので **列は used**（skip の unused 列と違う）、**Note_spacing_engraver が wish を出す**（`note-spacing-engraver.cc:87-91 acknowledge_rhythmic_grob`）、**`shortest-playing-duration` は body の長さ**。wish の `left_head_end` は左の *note column* から読むので slash では 0 → **ideal = 4.8 − 1.2 = 3.6**（`note-spacing.cc:77`・gs 1/16 の 4 分）。Lily# は spacer＝wish 無しの hemiola 枝（`spacing-spanner.cc:380-391`）で **4.8**。さらに **slash 群のインクは paper column の separation box に入る**（default extra-spacing-width 0.1）ので bar line への **rod＝群 3.757645 + 0.3 = 4.057645** が ideal 3.6 を上回って効く。Lily# は spacer の placeholder box（黒符頭 1.3042）で 4.8。**2 つで +1.2 と +0.74＝bar +1.73**（第361 の「+1.1」は 1 群だけの読み）。**probe `audit/lp-geometry/probes/beat-slash-spacing.ly`（BSL）**: f16→slash 2.504200・slash→g8. 3.600000・DoubleRepeatSlash→bar 4.057645。**3 小節以上の body の先頭小節も同じ機構**（`scratch/p368/bs3.ly`＝`test/tab-percent-blank-bars` の双子）: **LP 6.890・Lily# 前 7.09 → 後 6.89**（bar→slash は 0.9＋full-measure-extra-space 1.0＝LP と同じ 2.09）。

★★★ **⑶ 形（Core `+` 139／REF 12／OWN 0＝式は足していない・集合と箱の話）**: **`RestItem.RepeatSlashCount`（int?・`IsRepeatSlash`）**＝beat slash の列を**開く** spacer だけが LP の slash-count を運ぶ（collector の `WriteRepetitionAsSpacers(length, slashCount)` が最初の piece に刻む・bar 跨ぎは `MeasureBuilder.AddItem` の split が head 側に運ぶ・`PercentRepeatItem.BeatTiming` と同じ collector ステップで書くので同じ列を名指す）。読み手 9 軒: `SpacingRules.IsMusicalColumn` true／`MeasureLayouter.ItemStartingAt` は wish の端点に／`ApplyLeftHeadWidth` は head 0（＝−1.2）／`ComputeShortestPlayingAt` は実 duration／`ItemSkylineFactory.ColumnParts` は群の箱（`PercentRepeatEngraver.Geometry`＝描画と 1 軒・`NoteColumnMember = false`＝rod だけ・wish の minimum は見ない＝LP と同じ）／`CalculateNoteheadRightExtent` は群幅・`CalculateLeftExtent` は 0／`MeasureSprings` の `kept`・`BarHoldsOnlySkips`・`PruneSpacerOnlyOnsets` は slash spacer を落とさない（**PruneSpacerOnlyOnsets の `PercentRepeats` ループは item flag に置換＝綴り 1 つ**）。⚠️ **tab は ss 1.5 で群が 1.5 倍だが箱は notation frame（ss 1）＝未観測・remark に明記**（⑺⒝②）。

★★★ **⑷ 番人・台帳**: **台帳 +3＝全部 exact**（`percent.beat-slash.note-to-slash` 2.504200＝対照・前後とも exact／`slash-to-next-note` 3.600000＝前 +1.200000／`slash-to-barline` 4.057645＝前 +0.742355・`RenderedGeometry.BeatSlashGroupLeft`＝高さ 2.0 の水平端の quad を群にまとめる）→ 台帳 819・exact 643。`PercentRepeatTests` +2（spacer が count を運ぶ＋bar 跨ぎで head だけ／2 spring 系統の ideal 一致＋3 脚の形 2.5042・3.6・閉じ脚の min 4.057645）。`SpacingInvariantTests`・`SkipColumnSpacingTests`・`TabPercentBlankBarsTests`・補完系は不変で緑。APPROXIMATIONS／magic_constants 再生成（`LILYSHARP_UPDATE_DOCS=1`）。⚠️ 踏んだ罠 2: `LILYPOND-REF` の記号名を次行に折り返して `CitationsThatNameNothing_DoNotGrow` が落ちた（住所と同じ行に）／2 spring 系統の **min** は昔から別の枠（column chain は rod・estimate は skyline min）＝ideal だけ突き合わせる。

★★★ **⑸ 射程（掃き 925 冊・base 65d384a3 Release＝`scratch\p368\wt-base`・`sweep368.ps1`・svg バイト比較・`compare-svg.txt`）: MOVED 33**＝**追跡 2**（`audit/lpreg/slashprobe.lys`＝snapshot 無し・`test/tab-percent-blank-bars`＝**snapshot 1 枚**）＋**ユーザー 31**（percent の 3 小節以上 body を持つ本＝先頭小節が 0.2 縮む）。**段数が変わった本 2**（`Can't Get You Out Of My Head` 48→45 段・`Holiday - OctabeDown` 46→48）・**頁数変化 0**。

★★★ **⑹ 数**: **開始時**（`Session-Check -Build -Test -Scratch p368`）: HEAD **`65d384a3`**・未 push 24・`origin/master`＝`ba70753f`・木 clean・未追跡 0・`gh run list` **赤**（34416221197＝第361 ⑷⒝ の赤・以後 push 無し）・build 0 エラー／Core 0 警告・**full `scratch/p368/run1.trx` 7688 / 0 / 4 / 7692（第366 末 run6 と一致・RunInfos は SKIP 4 だけ・3 m 15 s）**・台帳 816・snapshot 249・追跡 `.lys` 599・ディスク 21843。**終了時＝下 ⑻**。§7.5（作業ツリー・`git diff --numstat -- LilySharp.Core`）**Core `+` 139／REF 12／OWN 0**（読み直し: REF は全部 LP の同じ関数を指す citation・式は 1 本も無い＝集合に列を足し、箱を群に替えた）。台帳 816 → **819**（+3 exact）・snapshot 249 不動（**1 枚が赤＝承認待ち**）・追跡 `.lys` 599 不動。CHANGELOG 1 項（Fixed）。⚠️ `git worktree`: p362／p363／p364 に加えて **`scratch\p368\wt-base`** を足した（`git worktree remove` で外す）。⚠️ scratch は **`p368`**（p367 は第366）。⚠️ §7 3.5: 第365 のブロックを `-Archive 365` で ARCHIVE の先頭へ（済）。⚠️ 記憶: 新規無し・更新 2（`reference_lilypond_svg_backend_generic_fonts` に双子の罠・`reference_lilysharp_mark_box_height_is_nominal_not_glyph` を em の決定に書き直し）。

⇒ ★★★ **⑺ 次の一手**: **⒜ ~~承認待ち~~＝第 2 便でユーザー「承認する」→ `tools\Approve-Snapshots.ps1 -Name test/tab-percent-blank-bars`（**LP 照合済み**: bar 4 の幅 6.89＝LP 6.890・`scratch/p368/bs3.ly`）→ full → commit → `Deploy-Lsp.ps1`（下 ⑻）**。**push はユーザー**（未 push 25）。**⒝ 拾い箱（この便が測って名指しただけ・未起票）**: ① **section label の em**＝LP `SectionLabel` の 1.5（今）か双子の `RehearsalMark` の 2 か（−0.14・section label を持つ本の絵が全部動く＝ユーザー決定）／~~②~~ **tab の beat slash の箱＝第 3 便で閉じた（下・第 4 便で承認・commit）**／③ **`lysc ly` 双子に `\paper { property-defaults.fonts.serif/sans }` を書くか**（svg で測るときだけの罠・pdf では無害・全双子の出力が動く＝ユーザー判断）。**第 5 便は着手しない**（同じ指示の 3 度目・第 4 便は承認の出荷）: ①③はユーザー決定、§7.6 ⒝ の負債（slash の箱の 2 軒）は列 skyline に譜ごとの frame を持たせる**設計級**のモデル変更で、測る残差が無い（台帳点は両 frame とも exact）＝p368 の道具（probe 雛形・base exe・staged 925 冊）で得るものが無く、次のセッションでやっても損が無い。⒞ の第361 ⑺⒞②③④⑥（drhs の臨時記号側・bos bar 1・ps2 の 0.017）は第361 自身が「観測者 1 冊か別 regime」と判定した並びのままで、beat slash の probe 雛形は流用できても機構が別。

★★★ **第 3 便（ユーザー「有利なら着手」→ ②は probe 雛形・staged 925 冊・base exe・温かい code path がそのまま効くので着手）＝tab の beat slash の箱**。**先に LP を測った**（`scratch/p368/bstab.ly`＝BST staff+tab／BTT tab 単独・bass tuning・`\tabFullNotation`）: **DoubleRepeatSlash → BarLine は 5.936468＝tab の群 5.636468 + 0.3、staff+tab でも tab 単独でも同じ**（separation box は譜ごと・列対の rod は最も広い譜のもの＝`spacing-spanner.cc:228-297`）。第 1 便の notation frame（4.057645）はユーザーのコーパス（全冊 staff+tab）で **−1.88** 短かった。slash → 次の音の脚は tab では TabNoteHead の左半分を読む＝Lily# の数字は自前の大きい cut（§3 F9）なので**台帳には載せない**（remark に理由）。**形（Core `+` 60／REF 7／OWN 0＝`MeasureSprings.cs` の腕 51 行＋ユーザーの §7.5 監査で足した REF 3 本（`kept`・`BarHoldsOnlySkips`＝`paper-column.cc is_used`／`CalculateLeftExtent`＝`beat_slash`）・APPROXIMATIONS 再生成）**: `ApplyTabChordSpacing`（tab の fret 数字を shared column に予約する既存の 1 軒＝layout と line-break gate が同じ表を読む）に slash spacer の腕: `slashRight[t]`＝`PercentRepeatEngraver.Geometry(…, TabStringSpace)` の群幅 → 次の列へ `群 + 0.1 + 0.1 + 0.1 + left[t+1]`（次が bar line なら `群 + 0.3`）を `Widen`。**台帳 +2＝exact**（`percent.beat-slash.tab-pair.slash-to-barline`／`tab-only.slash-to-barline` 5.936468・双子は `BeatSlashTabScore`＝`c,16 d, e, f,`）→ 台帳 821・exact 645。番人 `PercentRepeatTests.TabBeatSlash_ClosesOnTheTabGroupsRod`（Theory 2 形・閉じ脚の min 5.936468）。**3 小節 body の先頭小節（fixture の双子 `scratch/p368/bs3tab.ly`）: LP 8.0265・Lily# 前 6.89 → 後 8.03**。**掃き（base2 749a4b92 Release＝`scratch\p368\wt-base2`・`compare-svg-leg3.txt`）: MOVED 21＝追跡 1（`test/tab-percent-blank-bars`＝snapshot 1 枚・また）＋ユーザー 20・段数／頁数が変わった本 0**。⚠️ 踏んだ罠: `if (…) for (…) if (…) {…} else if` の `else` は内側の `if` に付く（C# のぶら下がり else）＝CS0136 で気付いた・波括弧で囲む。**製品は作業ツリー＝承認待ち**（patch `scratch/p368/beat-slash-tab.patch`＝Core 1＋Tests 2＋台帳＋probe＋CHANGELOG）。**⒞ 第365 ⑺⒝（chord quality 35 種）・第363 ⑺⒝④（`raggedLastBottom`）・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥ の並びはそのまま（⑤は閉じた・③は偽だった）**。

**⑻ 終了時**: **full `scratch/p368/run2.trx` 7692 / 1 / 4 / 7697（+5＝台帳 3＋番人 2・HANDOFF／CHANGELOG／APPROXIMATIONS／magic_constants 込み・RunInfos は SKIP 4＋その赤 1 だけ・6 m 15 s）・赤 1＝`SvgSnapshotTests.TestSample_MatchesSnapshot("test/tab-percent-blank-bars")`＝⑸ の snapshot＝承認待ち**。その時点では **commit 無し・Deploy 無し**（出力を変える変更は承認前に出荷しない＝RULES §5.1）。**第 2 便（ユーザー「承認する」）**: snapshot 1 枚を再ベース（動いたのは意図した 1 枚だけ）→ **full `scratch/p368/run3.trx` 7693 / 0 / 4 / 7697（RunInfos は SKIP 4 だけ・5 m 10 s）**→ commit（Core 11＋Tests 3＋snapshot 1＋probe 新規＋台帳＋APPROXIMATIONS＋magic_constants＋CHANGELOG＋この HANDOFF＋ARCHIVE）→ `Deploy-Lsp.ps1`（server だけ＝Reload 1 回）→ commit **`749a4b92`**。push はユーザー（未 push 25）。**第 3 便（tab の箱・上 ⑺）**: **full `scratch/p368/run5.trx` 7696 / 1 / 4 / 7701（+4＝台帳 2＋番人 2・RunInfos は SKIP 4 だけ・2 m 51 s）・赤 1＝`test/tab-percent-blank-bars` の snapshot（LP 照合済み: bar 4 8.03＝LP 8.0265）＝承認待ち**（run4 は REF の記号名を次行に折り返した赤がもう 1 本＝直した）。run5 の後にコメント 3 行（REF）と APPROXIMATIONS の再生成だけ＝build 0／inventory・citation・provenance 13 本緑。**第 4 便（ユーザー「承認する」）**: snapshot 1 枚を再ベース（動いたのは意図した 1 枚だけ）→ **full `scratch/p368/run6.trx` 7697 / 0 / 4 / 7701（RunInfos は SKIP 4 だけ・2 m 41 s）**→ commit（Core 3＋Tests 3＝snapshot 1 込み＋台帳＋probe＋APPROXIMATIONS＋CHANGELOG＋この HANDOFF）→ `Deploy-Lsp.ps1`。push はユーザー（未 push 26）。⚠️ **§7.6 ⒝ の負債 1 つ**: slash 群の箱が notation frame（`ItemSkylineFactory`・voice 単位で譜を知らない）と tab の予約表（`ApplyTabChordSpacing`）の **2 軒**に在る＝LP は譜ごとの separation item 1 機構。字面にするには列 skyline に譜ごとの frame（staff space）を持たせるモデル変更が要る。承認後の手順は第 2 便と同じ: `Approve-Snapshots.ps1 -Name test/tab-percent-blank-bars` → full → commit（Core 1＋Tests 2＋snapshot 1＋台帳＋probe＋CHANGELOG＋この HANDOFF）→ `Deploy-Lsp.ps1`。

## 以下は第366セッションの経緯

最終更新 第366セッション（2026-09-10）＝**入り方が違う**（ユーザーは HANDOFF を読ませたうえで**起票 1 件**: 「LYS2007（ある part のある section が別 part の同じ section より barline が足りない警告）と VS Code の quick fix（適切な数の barline を自動追加）が `lyrics {}` に対して未実装。lyrics にも同じ機能を」）。**製品＋番人＋CHANGELOG＋この HANDOFF を 1 commit（snapshot 0・出力を変える変更 0＝承認ゲート無し）・push はユーザー**。**骨は 4**:

★★★ **⑴ 裏取り＝起票は本物・ただし「lyrics は voice ではない」は設計で、片側だけ足す**。`SectionBarCounts` の remarks（第363）が「A lyrics track's cell is NOT a voice: a lyrics section longer than its music is a stacked verse by design（`LyricsCollector` の auto-wrap＝2 倍の小節は 2 番）」と明記し、`CrossPartMeasureValidator.ValidatePartMajorSections` は `SemanticVoices` だけ（part と named chords）・section-major の `ValidateSectionCrossPart` は `PartBlockSyntax`／`ChordPartBlockSyntax` の子だけを拾っていた＝**lyrics は長くても短くても無言**。⇒ **足すのは「短い側」だけ**: lyrics cell は section の長さを決めない（canonical＝`Max` に入れない・exporter の `SemanticIndex` にも入れない）が、voice の最大より**少ない**ときは LYS2007。lyrics だけの section（voice 0）は比べる相手が無い＝無言。小節の数え方は既存の `LyricSyllableReader.CountBars`（LyricMeasure 1 つ＝1 小節・`[N. ]` は最も広い verse・先頭の裸 `|` も 1 小節＝2026-08-28 のオーナー決定）。**quick fix は既存機構がそのまま通る**: `ScopeAnchoredAt` の section ループが part-major の `lyrics w { section A { … } }` の name span を既に解決する（足したのは section-major の `LyricsBlockSyntax`＝NameToken・無名なら keyword）。`la la |` の末尾に ` |` を足すとパーサは**空の LyricMeasure** を作る（`ParseLyricMeasure`＝syllable 0 個＋barline）＝row は「an empty bar (bare `|`) is skipped」で何も描かない。`{ la la }` は synthetic の幅 0 barline で閉じる＝`ClosingBarToken` null＝**trailing open**＝quick fix の「もう 1 本」が効く（chords の `{ Dm7 }` と同型）。

★★★ **⑵ 形＝Core 3・Lsp 1**: `LyricSyllableReader.CountBars(container, out bool trailingOpen)`（最後の plain LyricMeasure に閉じ barline が無ければ open）／`SectionBarCounts.SemanticVoice` に **`IsLyrics = false`**（positional の既定値）・**`LyricsCells(root)`**（part-major の `lyrics w { section A {…} }` だけ・`SemanticVoices` は不変＝exporter と page の canonical は 1 バイトも動かない）・`LyricsCell`／`LyricsLabel`（`lyrics 'w'`・無名は `lyrics`）／`LyricsAnchor`（NameToken ?? LyricsKeyword）／`CrossPartMeasureValidator`: `SectionVoice` に `IsLyrics`・part-major は `SemanticVoices` の後に `LyricsCells` を同じ byName に足す・section-major は `case LyricsBlockSyntax lb when !lb.HasSections` を chordVoices の列に足す・**`ReportBarCountMismatch` は `counted = !IsLyrics` の Max を section の長さにし、`Bars >= max` は全部無言**（lyrics の長い側＝stacked verse）・tail は「the track sings nothing over the remaining bar(s)」（message の頭「spans N bar(s) in lyrics 'w' but M in part 'p'」は据え置き＝`BarCountMessage` の regex がそのまま読む）。Lsp: `ScopeAnchoredAt` に `LyricsBlockSyntax` の 1 ループ（title は `lyrics w`）。

★★★ **⑶ 番人＝+7**: `CrossPartMeasureValidationTests` +5（part-major 短い→cell の `A` に anchor・message が `lyrics 'words'`＋`part 'melody'`＋「sings nothing」／長い→無言／lyrics 2 本だけ→無言／section-major `lyrics words sings vocal { la la | }` 短い→NameToken に anchor／sings-chorus-row の縮図＝一致→無言）・`BarCountQuickFixTests` +2（part-major「Add 1 bar line to section A (|)」→ `section A { la la | | }`・0 警告／section-major `{ la la }`＝open→「Add 2 bar lines to lyrics words (| |)」→ `{ la la | | }`）。`SectionVoicePaddingExportTests`（SyntacticCanonical ≡ Semantic の網）は不変で緑＝canonical に触れていない陽性対照。

★★★ **⑷ 数と射程**: **開始時**（`Session-Check -Scratch p367`・build／full は第365 末 run2 の数を引き継ぐ）: HEAD **`2eebab25`**・未 push 17・`origin/master`＝`ba70753f`・木 clean・未追跡 0・`gh run list` **赤**（34416221197＝第361 ⑷⒝ の赤・以後 push 無し）・台帳 816・snapshot 249・追跡 `.lys` 599・ディスク 21827。build 0 エラー／Core 0 警告（`CountRun` の cref 曖昧 CS0419 を 1 つ出して直した・Lsp `Editing.cs(193)` CS8602 と Tests の 7 本は既存）。的中 3 クラス 47 / 0（+7）→ **full＝下 ⑻**。§7.5（作業ツリー・`git diff --numstat -- LilySharp.Core`＝46+13+47・commit 前なので `-DiffBase` は 0 と出る）**Core `+` 106／REF 0／OWN 0**（式は 1 本も無い＝LP から導いた量は無い・半分は remarks）。台帳 816／snapshot 249／追跡 `.lys` 599 不動（**出力を変える変更は無い**＝validator と quick fix だけ・掃きは回していない）。CHANGELOG 1 節（Editor の quick fix 項の直後に 1 項）。⚠️ `git worktree`: p362／p363／p364 の wt-base が残ったまま（この便は足していない・`git worktree remove` で外す）。⚠️ scratch は **`p367`**（p366 は第365）。⚠️ §7 3.5: 第364 のブロックを `-Archive 364` で ARCHIVE の先頭へ（済）。⚠️ 記憶: 新規無し。

⇒ ★★★ **⑺ 次の一手**: **⒜ push はユーザー**（未 push 18）。**⒝ この起票の縁（起票に無いので触っていない・ユーザー判断）**: ① lyrics が voice の**倍数でない**長さ（4 小節の section に 5 小節＝2 番が 1 小節で切れる）は今も無言＝「2 番が短い」を言うなら `Bars % max` の話で、[N. ] verse 毎の比較になる／② part の本文に**同居する** `lyrics { }`（`Parser.Music` の co-written 形）は section-major の子として拾われるが、part-major cell の中では voice が 0 で無言＝意図（note-bound で短い＝残りの音に語が無いだけ）／③ 無名 `lyrics { section A {…} }`（top-level）は label `lyrics`・anchor は section 名で通る。**第 2 便は着手しない**（同じ指示が来たら）: ①②はユーザー決定、scratch/p367 には trx しか無い。**⒝′ 第 2 便（ユーザー報告・未再現）**: 「`scratch\ベースタブLy\tooLongChords.lys` の part melody section A の最後の小節 `c` が VS Code プレビューに出ない」→ **4 経路とも c は在る**: `lysc svg`／`png`（`data-pos=72` 1 個・PNG 目視）・`IncrementalCompiler`（`la la`…ではなく A 行を 1 字ずつ打鍵＝全打鍵で full と同一）・`LilySharpLanguageServer.GetSvg`・**隔離 VS Code（インストール済み拡張＋配布直後の server を丸ごと複製・CDP で ` c |` を打鍵）の webview DOM と描画**（`scratch/p367/shot-zoom.png`）。道具は `scratch/p367/prep.ps1`・`drive*.js`・`Zz366ProbeTests.cs.txt`（捨て計器・Tests から退避済み）。**一番ありそうな原因は Deploy-Lsp が殺した server を client が諦めた窓（Reload 前）**＝と第 2 便は書いたが**間違い**。**⒝″ 第 3 便＝再現して直した**: ユーザーの追記「bar 3 に e を書くと c が出る・e を消すと c も消える」＝**セッションの履歴が要る欠陥**。捨て計器 `scratch/p367/Zz366FuzzProbeTests.cs.txt`（1 セッションに乱数の編集列を流し、毎打鍵で full と比較＝seed 8 本中 2 本が数十手で乖離・`| | |` の真ん中に `e` を入れる 1 手で fresh session でも乖離）→ `ResumeTrace` で正体: **`TrySpliceSuffix` の「窓に跨る小節は採用しない」判定が `SourceEnd > Prefix`（厳密）で、閉じ barline の位置ちょうどへの挿入（空小節 `[61,63)` の 63 に `e ` を挿入）を「触れていない」と読み、最初の境界から old の尾を 7/7 小節そのまま採用**＝小節数は同じで音符だけ消える・次の編集で窓が動くと戻る。**直し 1 文字**: `MeasureCollector.Resume.cs` の `>=`（suffix 側 `SourceStart < SuffixStart` は据え置き＝suffix 先頭で始まる小節は不変の barline で開く）。番人 `CollectEditResumeTests.InsertionAtAMeasuresClosingBarLine_DoesNotSpliceThatMeasure`（e・space・削除の 3 打鍵）。**CollectEditResumeTests の完全性網（synthetic edits）は境界ちょうどの挿入を打っていなかった**＝fuzz（1 セッション・乱数列・毎手 full 比較）が捕まえた。**lysc／export は無傷**（毎回 full collect）。**第 2 便の「4 経路で再現せず」は fresh session で測ったから**＝**プレビューの報告は「編集の履歴」を再現しないと当たらない**（memory `feedback_preview_reports_need_edit_history_fuzz`）。**⒝‴ 第 4 便（「有利なら着手」＝着手した・射程を数えた）**: 網 `ResumedCollect_MatchesFullCollect_AcrossSyntheticEdits` の `SyntheticEdits` に **barline の直前 `" c4"`・直後 `" c4 "` の 2 挿入**を足して全 fixture で数えた → **直した穴は 0 冊・別の穴が 1 冊**（`tocoda-volta-clearance.lys`・直後の挿入＝section A の閉じ `|` の後に音符＝form 行より上の Δ=+4）: `score.MusicMarks[0].SourcePosition` が full 977／resumed 973＝**form 行の `segno` の位置が古いまま**。機構: `ProcessForm` の nav mark／`_"text"` は form 行（ファイル末尾）の位置を `_musicMarks` に焼くが、**`MaxSourceRead` にも `HeaderReads` にも入っていなかった**＝prefix 復元が側表を unshifted で採用。直し: 両 arm で `_probeRecording != null` なら `_walkHeaderReads.Add(node.FullSpan)`（section header と同じ扱い＝planner が span 毎に content＋position を検証・Δ≠0 で form 行より上の編集は prefix を諦める・Δ=0 は据え置き。`MaxSourceRead` に畳むと `segno` で始まる本の全 checkpoint が死ぬので選ばない）。網 90 / 0（CollectEditResume＋IncrementalCompiler＋CollectResume）。捨て計器 `scratch/p367/Zz366TocodaProbeTests.cs.txt`。**⒝⁗ 第 5 便（ユーザー「同じような穴がほかに残っていないか、徹底的に監査して」）＝掃き＋読み**: 捨て計器 `scratch/p367/Zz366SweepProbeTests.cs.txt`（全 net 本 258 冊 × 編集の**種類** 36＝barline の前後／削除／空小節追加／空白、音符の臨時記号／入替／削除／複製／休符化、`@` 注釈の追加／削除、空白の増減、コメント行／空行の挿入、`{`直後・`}`直前への音符と barline、phrase 本文の先頭挿入／先頭 token 削除、form 行の空白・記号・text・前後コメント、ファイル頭尾、宣言名の後の空白＝約 3 万編集・resumer は LSP と同じ `WithChange` の木・full は fresh parse・`ModelDeepDiff`）→ **最初 88 乖離／258 冊**（`brace.open.*` 54・`note.dup` 17・`brace.close.*` 14・`bar.extra` 3）→ **`Zz366TraceProbeTests.cs.txt`**（1 件ずつ baseline／resumed／full の小節列と plan を dump）で機構を 5 つに分けて直した → **0 乖離**（`scratch/p367/audit/sweep.txt`）: ① **候補が窓より前に立つ splice**（m=0 型＝old の尾が窓を跨ぐ）は「token 列が同一（trivia だけの編集）」か「窓が parallel span の 2 声目以降の中」のときだけ採用（`WindowIsTriviaOnly`＝green の token 列比較を probe に memo／`WindowInsideParallelExtraVoice`）＝**item span の straddle 判定では見えない隙間**（本文末尾の `}` 前・先頭の `{` 後・phrase 参照の間）を原理で閉じた。straddle 判定も隙間込み（前の小節の閉じ `|` から次の小節の頭／最後の小節は walk の最遠読みまで）。② **phrase 参照の marker が position 0**＝address 検証が 1 個ずれた node を見抜けず（本文先頭に `|` を足すと後ろの index が 1 つずれる）→ marker を参照の `Span` に置き（reset＝Start・end＝End・隣接 marker が同位置にならないよう Span）・checkpoint に `NodeKind`／`NodeIsPhraseEnd` を持たせて kind も検証・address は窓の写像で照合（`TryShift`）・参照本文は **value read**（`HeaderRead.ValueOnly`＝内容不変なら位置は動いてよい・`MaxSourceRead` に畳むと上に宣言した phrase の本すべてが死ぬ＝liveness の番人 `IncrementalCompiler_EditedKeystroke_…` が捕まえた）・peek の fold は marker を除外。③ **journal の復元が old の walk 1 の値を書き戻す**（`RestoreKeyLog`／`RestoreSectionStartLog` が 0 から replay＝先行 part の section start が編集で動いた後に旧値へ戻り、chord／lyrics 行が新旧両方に置かれた）→ 記録に `StartKeyLogCount`／`StartSectionStartLogCount`・walk 入口で件数検証・復元は自分の範囲だけ replay・splice の比較も自分の範囲だけ。④ **section 直下（part block の外）に打った node**（`| }` の `|`・`c4`）は section を inline にして prologue の挙動が変わるのに読みに無い → `HeaderRead.Structure`（section の直下の子 kind 列＝music の連なりは 1 つに畳む）を構造で照合。⑤ **form 行の nav mark**（第 4 便）。番人: `CollectEditResumeTests` +4（`NoteTypedAfterABodysLastItem_…`／`BarLineTypedBeforePhraseReferences_…`／`AnEmptyBarTypedIntoTheChordRow_…`／`ABarLineTypedAtSectionLevel_…`）＋網の `SyntheticEdits` に `{` 直後・`}` 直前の音符（各冊 +2）。**読みの監査**（手で全部見た）: `TryShift`（`p < Prefix`／`>= SuffixStart`・0 は番兵）・`SpanStable`／`ValueStable`／`WindowTouchesPhraseOrVariable`／`WindowRespectsTopLevel`／`ParsePrefixAgrees`／`ParseSuffixAgrees`（窓の中の node は両方とも見ない＝①の理由）・`ShiftMeasure`／`ShiftItem`／`ShiftSideEntry`（側表 21 種＝drift net が守る）／`MetaMatchesShifted`（6 位置）／`SuffixStateMatches`／walk 入口の `StartTableCounts`／nested channel（同じ planner）／form の `|:` `:|` は合成 barline node として fold 済み／`break` は位置を焼かない／lyrics 行と chord 行は walk 後に新木から／`MidBarBreaks` は checkpoint 外。**読みで名指した縁「walk 順が file 順と違う form（`form main { B A }`）で A の phrase 参照の隙間に打つ挿入＝候補が窓より後に見えて①が効かない」は第 6 便で計った＝再現しない**: 参照が value read になったので参照を書き換える編集（`pa`→`pb`・`pa'`）は先行候補を header-read floor が全部落とし、隙間への挿入は phrase の縁が小節を閉じるので必ず小節数が変わり `CanonicalBarsMatch` が落とす。7 形とも full と一致（再利用 0）＝番人 `ReversedFormOrder_EditsInAPhraseReferenceSection_MatchFull`。**⒞ 第365 ⑺⒝（chord quality 35 種）・第363 ⑺⒝③④・第362 ⑹⒝②〜④・第361 ⑺ の並びはそのまま**。

**⑻ 終了時**: **full `scratch/p367/run1.trx` 7676 / 0 / 4 / 7680（+7＝番人・第365 末 run2 7669 + 7・HANDOFF／CHANGELOG 込み・RunInfos 空・8 m 9 s）**→ commit（Core 3＋Lsp 1＋Tests 2＋CHANGELOG＋この HANDOFF＋ARCHIVE）→ `Deploy-Lsp.ps1`（server だけ＝Reload 1 回）。push はユーザー（未 push 18）。⚠️ 第365 の commit には `Co-Authored-By`／`Claude-Session` trailer が付いている（2026-07-06 のオーナー決定「付けない」に反する・履歴は書き換えない）。この便は付けていない。**第 3 便（⑺⒝″）**: **full `scratch/p367/run2.trx` 7677 / 0 / 4 / 7681（+1＝番人・RunInfos 空・4 m 4 s）**→ commit（Core 1＝`MeasureCollector.Resume.cs` の `>=` 1 文字＋Tests 1＋CHANGELOG（Fixed 1 項）＋この HANDOFF）→ `Deploy-Lsp.ps1`。Core `+` 1／REF 0／OWN 0。未 push 20。**第 4 便（⑺⒝‴）**: full `run3.trx` 7676 / 1 / 4 / 7681＝落ちた 1 本は `ApproximationInventoryTests.TheInventoryIsNotStale`（`MeasureCollector.cs` に 16 行足して引用行番号がずれた＝`LILYSHARP_UPDATE_DOCS=1` で再生成・差分は行番号 1 行）→ **full `run4.trx` 7677 / 0 / 4 / 7681（RunInfos 空・4 m 5 s・APPROXIMATIONS 再生成込み）**→ commit（Core 1＝`MeasureCollector.cs` の 2 arm＋Tests 1＝`SyntheticEdits` の 2 挿入＋`docs/APPROXIMATIONS.md`＋CHANGELOG＋この HANDOFF）→ `Deploy-Lsp.ps1`。Core `+` 16（コメント 11・コード 5）／REF 0／OWN 0。未 push 21。**第 5 便（⑺⒝⁗・監査）**: 掃き＝fixture 258 冊 → 88 乖離 → 0／**ユーザーの実コーパス込み 584 冊・64,953 編集・planned 45,513・resumed 8,838・bailed 11,962・乖離 0**（`scratch/p367/audit/sweep-with-corpus.txt`・40 分）。網 93 / 0（+4）。**full `run5.trx` 7681 / 0 / 4 / 7685（+4＝番人・RunInfos 空・2 m 45 s・APPROXIMATIONS 再生成 1 行）**→ commit（Core 7 ファイル `+` 378＝planner 166・probe 44・Resume 54・Collector 62・Form 22・marker 29・MusicWalk 1／REF 0／OWN 0＋Tests 1＋APPROXIMATIONS＋CHANGELOG＋この HANDOFF）→ `Deploy-Lsp.ps1`。未 push 22。捨て計器 3 本は `scratch/p367/*.cs.txt`。**第 6 便（読みの縁を計った・Core 0）**: 番人 +1（Theory 7 形）→ **full `run6.trx` 7688 / 0 / 4 / 7692（RunInfos 空）**→ commit（Tests 1＋この HANDOFF・Core 不変＝Deploy 無し）。未 push 23。捨て計器 `scratch/p367/Zz366EdgeProbeTests.cs.txt`。**第 7 便（ユーザー起票「クリップボードから `score{}` を貼るとプレビューに追加されないことがある」・着手＝CDP 計器と LSP 経路の理解が要る）**: ⑴ エンジン側は無傷＝捨て計器 `scratch/p367/Zz366PasteProbeTests.cs.txt`（貼り付け 7 形 × 位置 7 × 履歴 4＝196 通り、`WithChange` の増分 parse を fresh parse と `FindAll`／top-level kind 列で比較 → 全一致）。⑵ **正体は拡張 host の dedup**: `updatePreviewContent` が「SVG＋error が前回と同じなら webview に post しない」＝key に **picker の一覧が入っていない**ので、描画中の score の下に貼った 2 つ目の score は絵を 1 バイトも変えず → post されず → picker が古いまま（絵が動く次の編集まで）。直し: key に renders 一覧と drawnRender を畳む（`editors/vscode/src/previewCore.ts` の `svgPostKey`＝純関数・`test/previewCore.test.ts` +4・npm test 98 / 0）。⑶ **実 VS Code で確認**（`scratch/p367/drive4.js`＝tlc-full.lys を開き preview を開き picker を読む → Ctrl+End → `Input.insertText` で `score sub "S" {…}` を貼る → picker が `main`,`S` になった）。Deploy-Lsp は out/ も配る＝配布済み。Core 不変＝C# full は回していない（第 6 便の run6 のまま）。⚠️ Monaco の `.view-line` は空白を NBSP で描く＝`textContent.includes('a b')` は当たらない・1 token で探す。**第 8 便は着手しない**（同じ指示が来たら）: 残りは ⑺⒝①〜③（ユーザー決定）と第365 以前の並びで、p367 の道具（掃き・トレース・CDP）と噛み合う未計数の宿題は無い。bail の増分（縁の kind 検証・journal 件数）を再利用の損として数えるなら掃きの表で足りる＝perf の宿題としてユーザー判断。⚠️ bail は縁で増えた（address 検証が kind まで見る・journal 件数）＝**再利用の損はあるが不健全ではない**・数は掃きの表に。
