...
最終更新 第302セッション＝**§2 U8 の ⒜ から tuplet を閉じた＝あれは grob ではなく*容れ物*で、しかも落としていたのは*読み手 3 人全部*だった**。**入り方は第298〜第301 と同じ**——**ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った**ので、**§1 の「次の一手＝残りの U8」を取りに行き、その §1 自身が命じている「着手する便は、まず自分の取る 1 つを測ること」を先にやった**。⇒ ★★ **前便が「まだ測っていない」と*名指しで*書き残した 1 つ**（**⑻⒝＝「MIDI で tuplet の比を `_tupletStack` に積むべきか*は 1 度も測っていない*」**）**を測ったら、答えは LP に在り、しかも*ページと音の食い違いまで 1 つの機構から出ていた***。 **commit 8 本**（**本体 `d6bf1e1b` ／ docs `b296ed7e` ／ 数を閉じる `4093ffe5` ／ 射程の取り直し `ef6f430b` ／ 委任への答えと自己監査 `e8476165` ／ 2 度目の委任と難所の測定 `959658bd` ／ 3 度目の委任と `IsBeamedRun` の畳み込み `3d890da7` ／ 4 度目の委任と LP への設計質問（この commit）**）。 **掃きは 2 回・1761 冊・両側 Release・`svg --combined` ＋ `midi` ＋ `xml` ＋ `check`＝動いた本は 4 冊で、4 冊とも「grace 本体に tuplet を書く scratch のプローブ」**（第298・第300・第301・本便）。**追跡本は 1 冊も動かず、snapshot も 0 枚**。**台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188／追跡 `.lys` 581／snapshot 231 はすべて不動。**
...
- **U8. ▶ 容れ物は全部閉じた（第298 ＋ 第299 ＋ 第300 ＋ 第301 ＋ 第302）。残るのは*本当に grob を要る*ものだけ＝`grace { }` の本体は*歩かれない*。起票の「注釈を運ばない」より広かった**
  ...
  **`ProcessGraceNotes` が主旋律の `_defaultDuration` を共有していた**ため。
  ...
  梁の側は 1 行も変わらない。**
  ⇒ ★★★★ **⑵ 休符は run を*隣り合う符頭の極大部分列*に割る**——
  **`{ d'16 e'16 r16 f'16 }` が決定的**: **polygon 2（d'–e' の梁）＋ path +5（符頭 3・休符 1・
  *旗 1*）**＝**1 つの grace 群が「梁つき部分列」と「旗つき単独音」を同時に持つ**。
  ...
  ⚠️⚠️ **外れ値が 1 つ残っている**: **`{ r16 d'16 e'16 }` は隣り合う符頭を 2 つ持つのに
  両方とも旗**（`{ d'16 e'16 r16 }` は梁）。**先頭の休符だけが違う理由は測っていない**
  ——**LP の auto-beam を読むか、もう 1 冊測るまで、この 1 例に設計を合わせないこと。**
