...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第415セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `db189e73`・**未 push 8**＝第412〜第414 の 8 本・**full `sessions/p415/run1.trx` 8722 / 0 / 3 / 8725＝第414 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 128 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、**第412〜第414 が「次便が見てもそれは欠陥ではない」と予告したもの**）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 413`＝32 行 7,115 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」が ★★★ で名指していた ⒵″ **`h6e.everythingElse` の残り 20 家族に名前を付ける**（「いま最も安く測れる島」）を採った。★ **ベンチは着手前に AskUserQuestion で静かな窓をもらった**。根拠は割当バイトと**回数**（CLAUDE-OPERATIONS §2）。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**）:

★★★ **⑴ 検算が先に合った**＝打鍵 **9,502,886 B** 対 第414 最終 9,500,938 B＝**0.02% 差**＝**継ぎ目 22 本は測る対象を乱していない**。★ **雑音も測った**＝同じ構成を 2 度回して打鍵 13 B・段 16 B 差（≈1e-7）＝以下の数は雑音の 10^5〜10^7 倍。

★★★ **⑵ 「約 20 家族」は 1 家族だった。** 継ぎ目 22 本で `e.everythingElse` を尽くした（**残り +0.3%＝*正***＝二重計上なし・第412 ⑸）:

| 家族 | h6+f6 バイト | 段の | 打鍵の | 筆頭 |
|---|---|---|---|---|
| **`e18.stackAbove`（`OutsideStaffStacker.StackAboveStaff`）** | 854,887,456 | **71.9%** | **4.87%** | **228 / 231** |
| `e4.musicMarks` | 213,632,656 | 18.0% | 1.22% | 1 |
| `e13.stackBelow` | 57,785,568 | 4.9% | 0.33% | 0 |
| 残り 19 本 | 合計 63,882,768 | 5.4% | 0.36% | 2 |

⇒ ★★★ **上位 2 本で 89.9%。「無名の N 家族」は*費用の分布*について何も言っていない**（§5.3 へ）。⚠️ **5 家族は全コーパスで*ちょうど 0 バイト***（arpeggio・trill・custom text・figured bass とその下拵え）＝**0 は「小さい」ではなく「この母集団では測っていない」の顔**。

★★★ **⑶ もう 1 段割ると、費用は memo の miss だった**（`StackAboveStaffMemoized` が自分で 1..6 と番号を振っている段に継ぎ目）: **`g4.core`（miss した system の実 stacking）74.5%**／`g2.program` 11.8%／`g1.partition` 6.9%／`g5.rebuild` 3.7%／`g3.filter` 2.5%／`g6.store` 0.4%。**4,010 呼び出しの全部が memo 経路**（live 経路に落ちたものは 0）。

★★★ **⑷ 犯人を名指したのは「22 家族のうち 1 家族だけ比が外れている」。** 予備側と最終側を家族ごとに並べると **21 家族は 0.72〜1.00x で、`e18` だけ 0.30x**＝**音楽が原因なら全家族が同じ方向に振れる**⇒ その家族が持っている店の話。**予備側を*配置*で割ったら出た**:

| 誰が店に訊いたか | 呼び出し | 照会 system | hit | miss | hit 率 |
|---|---|---|---|---|---|
| 予備・**1 度目**の配置 | 1,840 | 44,121 | 37,307 | 6,814 | **84.6%** |
| 予備・**2 度目**の配置 | **330** | 7,315 | 2,095 | 5,220 | **28.6%** |
| 最終（2 度目の走りが無い） | 1,840 | 43,572 | 41,489 | 2,083 | **95.2%** |

⇒ ★★★ **第412 が paging-augment 店で測った 84.4% / 27.0% と、別の店で同じ 2 つの数。** どちらも「system *番号*ごとに 1 枠・miss で上書き」で、どちらも**配置 2 つ**に訊かれていた。**枠を最後に書いた者で割ると**（第412 ⑺ と同じ問い）: **h6 が h6 の枠で外す 1,009＝取れない床**／**h6 が h6b の枠で外す 5,805**／**h6b が h6 の枠で外す 5,216**（★ **2 つがほぼ同数＝擦り切れの署名**）／最終の床 2,079。**1 miss は run の中で測って 43,636〜47,945 B** ⇒ **賞金 2.81%**。

★★★ **⑸ 註が病名を書いて、一段手前で止まっていた。** `AboveStackMemo` の class 註は「**注釈パスは打鍵に 2 回走る（予備と最終）ので 2 つの pass は別 instance を持つ。さもないと毎打鍵潰し合って一度も当たらない**」と*書いてあった*。**分けたのは *pass* で、*配置* ではなかった**——予備パス自身が count loop の打鍵で 2 回走る。**註の文が、註の一段下でそのまま起きていた**（§5.3 へ）。

★★★ **⑹ 直し＝第413 と同じ形**（`cc85c104`・製品の変更はこの 1 クラスだけ）＝**2 部屋・LRU・古い部屋で当たったら昇格**。**2 は count loop 自身の上界**（`Layout` は `PlaceSystems` を最大 2 回呼ぶ）＝**定数ではなくフィールド 2 本**、店は「最大段数 × 2」で頭打ち。⚠️ **`Matches` は 1 バイトも触っていない**＝健全性の議論は不変。

★★★ **⑺ A/B（同じ harness・231 冊 × 8 forward 打鍵）**:

| | 前 | 後 | 差 |
|---|---|---|---|
| 打鍵あたり B | 9,503,446 | 9,268,889 | **−2.47%** |
| `e18.stackAbove` | 855,855,152 | 422,349,432 | **−50.7%** |
| └ `g4.core` | 637,716,280 | 207,793,216 | **−67.4%** |
| memo の miss（全体） | 14,117 | **4,227** | −70.1% |
| hit 率 1 度目／2 度目 | 84.6% ／ **28.6%** | 95.8% ／ **96.0%** | |
| hit 率 最終 | 95.2% | **95.2%** | **動かないのが正しい** |
| 床（h6 が自分の枠で外す） | 1,009 | **1,005** | **動かないのが正しい** |

⇒ **予測 2.81%・実測 2.47%**。★★ **対照が 2 つ*測定の中に*入っている**＝最終側は書き手が 1 人だから hit 率が動いてはいけない（動かなかった）・編集された system は建て直すのが正しいから床が動いてはいけない（動かなかった）⇒ **動いたのは擦り切れだけ**。残り 858+104 は**3 つ以上の配置が回る冊**で、**3 部屋は count loop の上界が正当化しない**。

★★★ **⑻ 出力同一は 3 つとも走らせた**（RULES §5.1）: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（⚠️ **baseline はこの便で stash した木＝`db189e73` から取り直した**＝前便のものを引き継いでいない）／⑵ **全ページの SHA-256**＝**231 冊 × 8 打鍵で差 0 行**（`hashes-{before,after}.txt`。before は製品変更だけを `git stash push -- AboveStackMemo.cs` して Release を建て直して取った＝ユーザーの未 commit `tools/*.ps1` には触っていない）／⑶ suite 全緑・snapshot 249 枚不動。

★ **⑼ 網 2 本**（`OutsideStaffStackMemoTests`＝**2 つの配置が交互に来ても両方当たり続ける**・**3 つ目の配置が古い部屋を落とし、落ちた側は同じ答えに計算し直る**）＋**毒 2 種でそれぞれちょうど 2 本赤**（1 部屋／昇格なし。既存 3 本は両方で緑＝新しい網だけが新しい性質を見ている）。★★ **後者は*件数*ではなく*答え*で赤くなる**＝`Get` が `Recent` を返すので、**昇格しないと「当たった部屋ではないほう」の出力を返す**＝昇格は LRU の作法ではなく正しさそのもの（§5.3 へ）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: **⒵″ ✅ 閉じた**（名前を付けたら 1 家族で、その 1 家族は擦り切れていた）。**残る島**: ★★★ **⒳ `resolve`（`VerticalSkyline.EndBatch`）＝打鍵の 1.15%・skyline miss の 19.1%＝第414 が「いま miss の中で最大」と書いたもの**（**未着手・未計測の内側**）／★★ **⒴′ 2 度目の注釈パスを要らなくする**（打鍵の 7.7%・第411 ⑶）／★ **⒴″ `b.scriptsFingerings`（`ComputeFingeringsAndScripts`）＝打鍵の 4.2%（今便の実測・第412 の 5.0% は当時の分母）＝名前の付いた最大の葉**・未分割／★ **⒵‴ `e4.musicMarks` 1.22%**（`e.everythingElse` の第 2 位・未分割）／★ **⒴ `inkFirst`＋`inkLast`＝0.53%**／⚠️ **`g4.core` は repair 後 1.21% で*もう実仕事***（編集された system の stacking）＝**この島には戻らないこと**／⒜ **R13⒝ の実機確認**（第404 ⑵＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械）＝上限は red を 14〜20% だけ実体化＝fingbeam1k で 26.2 MB → 約 10 MB。⚠️ **未着手・未見積もり＝validator ではなく土台の変更**／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った（採るならそれを知って採る）／**R13⒦ ⒤ ✅ 第410 が「測って、やらない」で閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅ 第407 が測って「やらない」と決めた**／**⒟ ✅ 第407 で閉じた（R2R）**／LP 双子が要る R7〜R11／**R15 ✅ 削除は第407 で完了**（残り＝「2 綴り」一族のみ＝統合・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。** ★★★ **そして「同じ病名の店を grep する」が今便の一番安い道具だった**——第413 の直しがそのまま形になり、網の形まで揃っていた。**`Dictionary<systemIndex, Entry>` の店を見たら「訊く主体はいくつか」を数えること**（§5.3）。

★ **⑽ 終了時**: commit 2 本（code `cc85c104`・docs 1 本＝**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 1 本・**最終 full `sessions/p415/run3.trx` 8724 / 0 / 3 / 8727**（§1・§2・RULES を書いた*後*の最終ツリーで取った）（⚠️ **第414 最終の 8722 / 8725 から +2＝今便の網ちょうど 2 本**・計器と継ぎ目は全部外してあり `ZzStage`／`Zz415`／`ZZ415`／`ZzWriter` の grep は**コードに 0 件**）・§7.5（対 `db189e73`＝スクリプトの出力）**Core `+` 63 行／REF 0／OWN 0**（★ **数値定数を 1 つも足していない**＝「2 部屋」はフィールド 2 本で、**数ではなく*形*が count loop の上界を言っている**・LP 由来の式なし・63 行の大半は註）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。⚠️ **棚卸し `docs/APPROXIMATIONS.md` と `audit/magic_constants.csv` は再生成不要**＝`AboveStackMemo.cs` はどちらにも 1 件も載っていない（grep 0 件・`TheInventoryIsNotStale` は緑）。§7 4（恒久知識）＝**RULES §5.3 に 4 項**（「**『約 N 家族が無名』の N を*名前の数*として読まない**」＝22 本入れたら 1 本が 71.9%・上位 2 本で 89.9%・5 家族は全コーパス 0 バイト／「**2 人の呼び手で*費用の比だけ*が外れているなら疑うのは memo。比は呼び手ではなく*配置*で割ると答える**」＝21 家族 0.72〜1.00x に対し 1 家族 0.30x／「**『X が 2 つあるから分けた』という註は、X 自身が 2 回走るようになっていないか確かめてから信じる**」＝註が病名を書いて一段手前で止まっていた／「**LRU の昇格は作法ではなく*正しさ*のことがある。毒で赤くなるのが件数か*答え*かを見る**」）。計器と全文は Lab `sessions/p415/`（`summary.txt`・`inside-everything-else.txt`・`ab-{before,after}.txt`・`hashes-{before,after}.txt`・`Zz415Probe{1,2}.cs.txt`・`ZzStage.cs.txt`・`zz415-seams-and-change.diff.txt`）。§7 3.5 は**セッションの頭で**済（第413 を `-Archive 413`＝32 行 7,115 chars）⇒ **HANDOFF は 444,675 B → アーカイブ後 431,608 B → 今便を書いて 448,878 B＝天井 450,000 まで 1.1 KB しかない**（今便のブロックは約 17 KB＝表を 3 枚置いたぶん厚い）。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 414` を回すこと**（**第414 のブロックが丸ごと戻る**）（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で 3 回使って残っていない。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第415 の欠陥ではない。
