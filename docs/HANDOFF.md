...
⇒ ★★★★ **次の一手**: ✅ ⑺ が mark を名指しし（Lily# は正しく予約）・✅ ⑻ が括弧を閉じた（頁に効かない・番人 4 点）。**残るのは ⒟ ただ 1 つ**。
**⑴ ⒟＝*静かな段の上 extent が定数 2.311***（⑽）。**LP は段ごとに違うインクを読む（tab の音部記号 2.880／符尾 4.500／譜線 2.05）のに、Lily# はどの本でも 2.311 を返す**——**tab book（原点＝上弦）でも both book（原点＝bass 5 線譜の上線）でも同じ値**なので、**staff の種類に依らない床**。**Boogie の tab では +1.68 ss（12 段中 3 段）。** **やること**: ⒜ `perSystemExtents[i].upExtent` の出所を追う（`LayoutEngine.Pages.cs` の `SysHeight`／`BuildLineShapes` → `SkylineBuilder.BuildSystemSkylines` か `EstimateAboveStaffExtents` か）——**2.311 を返している 1 行を名指しする**（`scratch/p338/ZzP338bProbeTests.cs.txt` が段ごとの top を吐くので、毒を入れて切り分けられる）。⒝ 名指しできたら **`probes/tab-stem.ly` か新プローブに「上向き符尾の無い tab 段の上インク」の book を足して LP と対**（`page.tab-full.quiet-system.up-ink` の型）→ 台帳点 → 直す（**出力を変える＝承認事項**）。⚠️ **「F 音部記号を中央線の枠で読んでいる」説は外れ済み**（⑽）。
⚠️ **Lily# の段の高さを LP と並べるときは必ず ⑴（枠）の `ToFirst` を足す。** ⑵ 以下は第337 の並び（`lysc ly` の ChordNames＝ユーザーと綴りを決めてから／lyric row の slot の投票／ps2 の 0.017・beam-over-stem bar 1 の +0.33／push 後 `gh run list`／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／A/B/D/E／C⑴／G）。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
