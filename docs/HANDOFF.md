...
終了時 **未追跡 0・作業ツリー項目 0**・suite **5245 passed / 0 failed / 4 skipped**
（+4＝新しい網ちょうど）・**台帳 516 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・
**snapshot 217 枚で不動＝再ベース 0 枚**・**Core は 0 警告**。
**全数 A/B は 5 回**（毎回 `git ls-files "*.lys"` の **566 冊**・**絵は毎回 0 冊動いた**）＋
**陽性対照 5 回**（6 冊／1 冊／37 冊／103 冊／2 冊）。
★ **ユーザー決定 0 件**——**着手判断は 2 回ともユーザーの委任（「有利なら着手／不利なら着手するな」）に
従って私が決めた**。⚠️ **2 便目を「有利」と判断した根拠は書いておく**＝
**住所の内訳・`alloc` の使い方・基準の掃き `p191-rb3` がディスクに在ること・memo 枠の
奪い合いと消費者 3 つという罠**が、**この便に在って次便には無い**（今日は住所を割るのに約 40 分かかった）。
...
★★ **値段は 4 状態を*同じ計器で*取り直した表で読むこと**（Release・**アロケーション**・
**3 回の最小値**。⚠️ 時間では割れない＝同一コードの対照が 16% 揺れる）:

| 1 打鍵 | 便前 | 第1便後 | 第2便後 | **第3便後** |
|---|---|---|---|---|
| `perf-fingstack1k` | 3304.5 MB | 2815.6 | 2191.1 | **1362.0**（**−58.8%**） |
| `perf-scripts1k` | 1117.8 MB | 932.6 | 697.8 | **403.9**（**−63.9%**） |
| `perf-v2bow1k` | 157.1 MB | 157.1 | 152.9 | 152.9 |
| `perf-plain1k` | 95.9 MB | 95.9 | 95.1 | 95.1 |

**フルレンダの `perf-fingstack1k` は 7355.1 → 3978.0 MB。**
⚠️ **対照の読み方は便ごとに違う**（骨 ⑶）: **第1便は plain1k も v2bow1k も ±0.00＝真の対照**／
**第2便は器が全 merge なので対照が存在しない**（両方が受益者）／
**第3便は plain1k が対照**（articulation 0 冊で早期 return）**・v2bow1k は届き得るが元から 0 MB**。
...
- **⑸ ★ §7.5 の監査**＝**Core +150 行（うち*コード 74*）・削除 −44（コード 42）・
  数値定数 0・REF 0 増減なし・OWN 0**（⚠️ **`git --no-pager diff --no-color` で数えた**・骨 ⑷）。**分類は 3 便とも ⒟**
  （何も足していない＝既存の家の指し直し／複製の削除。**LP の意味論に 1 つも触っていない**——
  出力が 566 冊バイト同一であることがその証明）。**§7.7 の匂い一覧はどれも当たらない。**
- **⑹ ★★ 残っている項＝**打鍵の残り 1362.0 MB のうち、**`ComputeFingeringsAndScripts` が 974 MB＝71%**
  （`ArticulationEngraver.CalculateWithFingerings`。**`ScriptSkylines` の置いた複製は
  そのうち 40.5 MB＝engraver 内でも小さい**ので、**中を割るところから**）。
  **`AugmentSkylinesWithScripts` は 11.5 MB＝1% まで落ちた**ので、**この島は閉じた**
  （⚠️ **memo は入れていない**——遅延で十分だったため。**行が在る本では今も毎打鍵組み直す**ので、
  **歌詞つきで script の多い本が出たら memo が次の一手**。⚠️ **`_pagingAugments` は
  system index だけで引く 1 枠なので、同じ memo を共用すると枠を奪い合って毎回 miss する。別の辞書が要る**）。
  **⒝ `MergeInternal` は今も merge ごとに全長の合成リストを 1 本作り、全体を sort する**
  ＝**1 つの system の script 数について二次のまま**。**器が全 merge なので射程は広く、危険も広い。**
  ⚠️ **`List.Sort` は不安定**なので、**同じ `Start` の building の順に現在の出力が依存している**。
  **アルゴリズムを触る前にそこを測ること。**
...
> | 1 打鍵 | 第190 時点 | ⑴ 後 | ⑵ 後 | **⑶ 後** |
> |---|---|---|---|---|
> | `perf-fingstack1k`（11,988 script） | 3304.5 MB | 2815.6 | 2191.1 | **1362.0**（**−58.8%**） |
> | `perf-scripts1k` | 1117.8 MB | 932.6 | 697.8 | **403.9**（**−63.9%**） |
> | `perf-v2bow1k` | 157.1 MB | 157.1 | 152.9 | 152.9 |
> | `perf-plain1k`（script 0） | 95.9 MB | 95.9 | 95.1 | 95.1 |
>
> **フルレンダの `perf-fingstack1k` は 7355.1 → 3978.0 MB。**
...
> **⇒ 第191 後は `AugmentSkylinesWithScripts` が 1%（11.5 MB）まで落ち、
> 残りは `ComputeFingeringsAndScripts` の 974 MB＝打鍵の 71%**
> （＝`ArticulationEngraver.CalculateWithFingerings`。**次に割るならここ**）。
>
...
>
> **⑶' 残っている 2 段（着手するならここから）**:
> ★★★ **⒜ `ArticulationEngraver.CalculateWithFingerings` が打鍵の 71%**（fingstack で 974 MB）。
> **中はまだ割っていない**——`ScriptSkylines` の置いた複製は **40.5 MB** なので**そこではない**。
> **region スコープを中に置いて 2 分割するところから**（§1 の骨 ⑴ の手順で 30 分）。
