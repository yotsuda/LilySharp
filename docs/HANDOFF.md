...
最終更新 第331セッション＝**入り方は第298〜第330 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。第330 の並びの先頭「push 後 `gh run list`」は push が私の権限外なので飛ばし、**「T6 の残り」の ⒦ から入った**。⇒ ★★★★ **本便は §2 T6 の ⒝（拍子欠落）・⒠（`break` 前の自動小節線）・⒦ を*コーパス側*で閉じた（製品コード 0 変更・commit は HANDOFF だけ）**。**骨は 6 つ**:

⚠️⚠️ ★★★★ **⑴ ⒦「`repeat percent N { 1 小節 } |` の綴りが LYS2002 を出す」は誤診だった**（§5.0「引き継がれた処方箋は着手前に診断を 1 回測る」の 2 例目）。**probe `scratch/p331/probes/k1`（`} | d,8×8 |`）は 3 小節・無警告**。**`Get Back` 186/96 の正体は拍子**＝`.ly` は `\time` を書かず（LP 既定 4/4）終盤で `\time 2/4 r2 | \time 4/4`、**旧変換器がその 2/4 をヘッダに持ち上げて 4/4 を落としていた**ので Lily# は全編を 2/4 で読み（8 分 8 個の小節が毎回 2 つに割れて）186 小節。**同型 9 冊**（Desperado／Get Back／Hello, my friend／Real Gone／We're All Alone／You Make Me Feel Brand New／You're So Vain／瞳をとじて／銀河鉄道999）＝**T7 の C「`.lys` が 2 倍」族そのもの**。fx5 の残りの警告は `a×8 break repeat percent` の **`break` 前の `|` 欠落（⒠）**。
★★★ **⑵ 計器＝`scratch/p331/t6bk/RestoreMeter`**（第328 の RestoreSlurs と同じ tokenizer＋LCS・`dotnet run -c Release -- meter|bars`・`verify.ps1 -Mode`（原本 vs patched の `lysc check` 差＋`lysc layout` の小節数）・`apply.ps1 -Mode`（`backup-<mode>/` に原本））。**`meter`**: `.ly` の各 event の「効いている拍子」（変数ごとに 4/4 から・`\time` で更新）と、揃った `.lys` event の拍子を比べ、違えば `time N/M ` を event の前（小節頭＝`|`／`{`／`}`／`break` の直後のみ・**`repeat percent N {` の中ではなく `repeat` の前**）に書く。ヘッダは最初に揃った対で直す。**`bars`**: `.lys` を歩いて小節位置を積み（音価解決・tuplet は D/N 倍・grace は 0・percent／section／volta の `{ }` で 0）、**`|` の無い `break` の手前が拍子の整数倍なら `| ` を書き、その run の内側の小節境界にも書く**（`r1 r1 r1 r1 break` → `r1 | r1 | r1 | r1 | break`）。**同じ run の直しは `|`・section 末・percent／volta body 末でも**（`repeat percent 4 { r1 r1 }` → `{ r1 | r1 }`・k4 で綴りを裏取り）。
★★★ **⑶ 実測（`meter`）**: **285 対・`.ly` の拍子変更 147・ヘッダ直し 9・`time` 挿入 121・37 冊**。**門: error 0→0・warning 627→113（37 冊）・小節数 10 冊が動いた**＝**Get Back 186→96（LP 96）・Hello, my friend 173→90（LP 89）・We're All Alone 120→66（LP 66）・Desperado 89→55・Real Gone 161→98・You Make Me Feel Brand New 100→64・You're So Vain 67→37・銀河鉄道999 149→76・瞳をとじて 155→98・I Love You 78→79**。**手で 4 冊**（計器が「小節頭でない」「揃わない」と報告した箇所）: `Crocodile Rock`／`世界に一つだけの花`（`r1 r2 |` → `r1 | time 2/4 r2 |`＋次の `time 4/4`）・`夏の終わりのハーモニー`（`r1 r1 r4 |` → `r1 | time 5/4 r1 r4 | time 4/4`）・`Are You Gonna Go My Way`（`.lys` が 2/4 の 8 分 4 つを 4/4 の 4 分 4 つに書き換えていた → `time 2/4 fis,,8 ×4 | time 4/4`）。原本は `backup-hand/`。
★★★ **⑷ 実測（`bars`・2 パス）**: **`|` の無い `break` 277（103 冊）→ `| ` 410 本（break の前 277 ＋ その run の内側）・`|`／section 末の内側 601 本・percent／volta body の内側 62 本・`} time` の間 1 本**。**門: パス 1＝190 冊 error 0→0・warning 788→280・新規 1（Viva La Vida の percent body＝パス 2 で閉じた）・小節数が動いたのは Reelin' In the Years 138→139 だけ／パス 2＝20 冊 68→18・新規 0・小節数不動**。**手に残した 10**（拍子の整数倍でない run の手前の `break`＝ピックアップ小節の形: Disco Inferno 1/2・Express Yourself／Holiday／Venus／もう恋なんてしない 1/4・You're So Vain 1/2・銀河鉄道999 7/8 ×2・If You Leave Me Now と 夏の終わり は 0＝`break break`）。⚠️ **`Boogie Oogie Oogie.lys` はユーザーが本便中に編集していたので当てていない**（apply が mtime で弾いた）。**終了時に `bars` を回し直したら patched 0**（Boogie も含めて＝計器は当て済みコーパスに対して冪等・**ユーザーの編集が先に同じ所を直していた**）。
⚠️⚠️ ★★★★ **⑸ 罠 3 つ（全部「新規診断 0」の門が捕まえた）**: **⒜ section 内の `time` はその section 限り**（k9・`Semantics/ScoreHomeMeter.cs` の doc＝collector は section 境界で home meter に戻す・**LP には無い意味論**）——1 度目の `bars` は 2/4 を次の section へ持ち越し、Endless Story／Reelin'／ホーリー&ブライト に「1/2 is less than 4/4」を 26／77／15 本出した（`|` を 100 本入れかけた）→ 両 scanner とも section の `{` `}` で冒頭値にリセット。／**⒝ `repeat percent N { … } time 1/4 r4 |`（`}` と `time` の間に `|` 無し）は `MeasureValidator` が新しい拍子を*閉じたばかりの body* に当てる**（k7: percent の `r1` に「1 exceeds 1/4」・k8 `} | time` なら無警告・**layout は両方正しい**＝validator と collector の不一致・**第 2 便で修理済み**）→ 計器は `| time` を書く。／**⒞ `time` を percent の `{` の内側に置くと body にしか効かない**（I Love You が 2→39 警告）→ `repeat` の前へ。
★★★ **⑹ 射程（同じ exe・コーパスの直しだけの差）**: **`lysc check` 全 285 対＝warning 1420（204 冊）→ 346（75 冊）・error 0→0・増えた本 0**（`scratch/p331/check331.ps1` → `check-before.csv`／`check-after.csv`）。**段署名の掃き `run331.ps1`**（`sig331.ps1` before＝`corpus-before/`（backup-* から組み直した原本）／after＝実コーパス・LP 側は第321 の `lp.out`・`layout --all` 533 s／986 s → `compare331.ps1`）: **T7 の指標（KindMatched）207 対 SigMatch 131／小節数一致で段違い 47／小節数違い 29 → 209 対 136／50／23**。**BarsMatch +19 対（Get Back・Desperado・Real Gone・We're All Alone・You Make Me Feel Brand New・銀河鉄道999・瞳をとじて＝全部 LP と同数に・I Love You 78→79＝LP 79・夏の終わり 84→83＝LP 83）・SigMatch +13 対**。**−2 対＝Reelin' In the Years 138→139**（LP 138）＝**⑸ の 4 つ目の罠: `.lys` は `… e,\3 break |` と*break の後ろ*に小節線を書いていて、計器がその前にも `|` を置き `| break |` の空小節を作った**→ 手で戻し（**計器は `break` の直後の `|` を同じ小節線と読むように直した**・`| break |` を元から持つのは `Arthur's Theme` 1 冊＝ユーザーの綴り・触らず）。⚠️ **`compare331.ps1` の 1 度目は全部 0 を刷った＝`param([string]$Before)` に `Import-Csv` の結果を代入すると `[string]` の型制約が残って文字列に潰れる**（PowerShell の罠・別名の変数に読む）。

★★★ **第 2 便＝ユーザー「手元の `.lys` の手入れは私がする。本便が有利な残債があれば着手」→ ⑸⒝ の validator を直した（製品 1 commit）**: `MeasureValidator.ValidateMeasures` は written bar（`|` の間）の `time` を*全部先に*採用してから repeat の cut を歩いていたので、`repeat percent 9 { r1 } time 1/4 r4 |` の body が 1/4 で検査されていた（collector は順に歩くので絵は正しい）。**直し＝拍子の採用を item 順に**（`AdoptMeters(from, to)` を各 cut の前と末尾の segment で呼ぶ・bar 自身の fill 検査は従来どおり最後の拍子）。**番人 `RepeatBodyMeasureValidationTests` +2**（`} time 1/4 r4 |` と `} | time 1/4 r4 |` が両方 clean／前に書いた `time` は body に効き・後ろの `time` は残りの `r2` に効く＝対照）。**焦点 80 / 0・full 7106 / 0 / 4 / 7110（`full1.trx`・2 分 24 秒・+2＝新番人）・k7 は `lysc check` で無警告に**。CHANGELOG（Diagnostics）1 項。

★ **開始時裏取り**: HEAD `70ac97e9`＝**未 push 29**（第330 §1 の「26」は第 10 便＋HANDOFF の後の数・`origin/master` は `f30cdfeb` のまま＝間で push 無し）・木 clean・未追跡 0・`bin\Debug` は net10.0 のみ・`xunit.runner.json` は並列のまま・**solution build 0 エラー・Core 0 警告（`--no-incremental`・1 分 58 秒）**。
終了時（第 1 便）: **commit 1 本（`6c22fae9`＝この HANDOFF＋`HANDOFF-ARCHIVE.md`＝第329 の経緯を末尾へ逐語）・未 push 30・製品コード 0 変更・台帳／`.lys`（追跡）／snapshot 不動**（動いたのは git 管理外の `scratch\ベースタブLy` だけ＝**meter 37 冊＋手 4 冊・bars 209 冊（193 の原本は `scratch/p331/t6bk/backup-bars/`・37 は `backup-meter/`・4 は `backup-hand/`・組み直した原本一式は `scratch/p331/corpus-before/`）**）。**full suite は回していない**（Core 0 変更）——`HandoffArchiveContinuityTests`＋`HistoryCitationTests` だけ（下の commit の前に緑）。⚠️ **ユーザーの指摘 1 件（memory 更新済み）: ripple／pwsh console では `.ps1` を直接実行する（`pwsh -NoProfile -File` は不要）**。
**終了時（第 2 便）**: **commit 1 本（`MeasureValidator`＋番人 2＋CHANGELOG＋この HANDOFF）・未 push 31・木 clean・未追跡 0・Core 0 エラー 0 警告・full 7106 / 0 / 4 / 7110**。**手元の `.lys` の手入れはユーザーが引き取った**（下の次の一手 ⑴⑵ はユーザー側）。

⇒ ★★★★ **次の一手**: **⑴ コーパスが編集されたらまた `dotnet run --project scratch\p331\t6bk\RestoreMeter -c Release -- meter` → `bars`**（順に・各 `verify.ps1 -Mode` → `apply.ps1 -Mode`・終了時は両方 patched 0＝冪等）。**⑵ 手に残した 10 の `break`**（⑷）＝ピックアップの綴り（`partial`）をユーザーに訊く。**⑶ §2 T6 の残り＝⒞ 記号（`\mark` 21 対 0 の `I Will Always Love You`）・⒟ LP の同時 2 声・⒥ break 21 冊（ユーザーに訊いてから）**。✅ **⑷ 製品候補＝⑸⒝ の validator**（`} time N/M` を前の body に読む）**＝第 2 便で閉じた**。**⑸ 以下は第330 の並び**: push 後 `gh run list`（`extension-tests` job 込み）／T7 の C の残り（⑹ の掃きで数え直した）／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／§2 A/B/D/E／§2 C⑴／§2 G／`:|.|:` の幅 0.08／smart typing の小穴／第330 ⑸ の overlay 群。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
...
- **T7. ▶ 起票（第321）＝絵の突き合わせ（286 冊全数）の結果＝族の表。計器は `scratch/p321/`（§1 ⑴）**
  ...
  **上の 388／382 は代用対（both の Lily# score を単譜の LP book と比べた 384 対）を含む数で、もう指標にしない**（第323 ⑶′⒜）。
  ...
  ▶ ★★★★ **半分は第332 ⑽ が閉じた（構造）＝覆われた反復は spacer で書く。SigMatch 140 → 143・失った対 0**。**残る半分＝空小節のばね**（`s1` だけの小節・`| |` プレースホルダ・MMR が同じ家を読んでいるかを先に測る）。**LP の式は `lily/spacing-basic.cc:41-83`**——`space = spacing-increment × (mlen / global_shortest) × 0.8`、`Spring(min_dist + space, min_dist)`、**`inverse_stretch_strength = space`**。**Lily# は本の最短音価に依らず 6.39 ss 固定で伸びない**（第332 ⑽⒠）。**射程は percent より広く別の承認が要る＝次便の入り口**。
