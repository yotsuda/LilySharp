...
★★★ **⒄ 数**: **開始時**: HEAD `1e80a9b2`・未 push 9・木 clean・未追跡 0（**`Session-Check -Build -Test` は走らせていない**）。**終了時**: HEAD `de98c1b4`・未 push 31・**台帳 843 点／ss 非ゼロ 210／総和 23.216135909／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249／追跡 `.lys` 599＝全部不変**（**絵は 1 枚も動かない**＝触ったのは LSP と test だけ・**Core は 3 便とも 1 行も触っていない**＝§7.5 の数えは 0 行）。**full `scratch/p375/run19.trx` 7951 / 0 / 4 / 7955**（⚠️ **第 12 便で 9 本*減った*のは正しい**＝matrix の MIDI 行 3 綴り × 3 主張が言語から消え、代わりに「退役した綴りは断られる」2 本が入った。第 11 便末 `run16.trx` 7964・第 10 便末 `run15.trx` 7956・第 9 便末 `run14.trx` 7953・第 8 便末 `run13.trx` 7947・第 1 便末 `run1.trx` 7816・第 2 便末 `run4.trx` 7827・第 3 便末 `run5.trx` 7829・第 4 便末 `run6.trx` 7839・第 5 便末 `run8.trx` 7842・第 6 便末 `run10.trx` 7854・第 7 便末 `run12.trx` 7869。**12 便とも合計を読んでいる**＝新 test は差引 144 本（第 12 便は −9）（第 8 便の 78 ＝ 26 綴り × 3 主張）。⚠️ 第 5・6 便は既存 test を**書き換えている**ので増分はそれより小さい）。★ **第 7 便は 7 つの context の配線を丸ごと替えたのに既存 7854 本が全部緑**＝**行の読みが 1 つになっても、読めていた綴りの答えは変わっていない**。⚠️ scratch は **`p375`**。⚠️ §7 3.5: 第372 を `-Archive 372` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝**第372 の天井 commit `9f6df2a3` が未 push のまま**。**push はユーザー（未 push 12）**。★ **LSP は 10 度配布した**＝`tools/Deploy-Lsp.ps1` が `0.6.0+de98c1b4` を `yotsuda.lilysharp-0.5.0-win32-x64\server` へ（95 ファイル byte 一致）⇒ **ユーザーは Reload Window が 1 回要る**（package.json は動かしていないので 2 回は不要・記憶 `feedback_lilysharp_lsp_deploy_target`）。

⇒ ★★★ **⒄ 次の一手＝⒜ *同じ「死語」を全数で数える*（第 12 便が作った未計数）**。**この便で見つけた死語は 1 つ（MIDI 行の option）だが、見つけ方は偶然だった**——**行の補完を直そうとして「誰がこれを読むのか」を追った**だけ。⇒ **パーサが受けるのに*どの consumer も読まない*綴りが他に無いか**を、同じ 3 点測で数える: ⑴ syntax node が値を公開しているか ⑵ exporter／collector が読むか ⑶ 綴りを変えて出力が変わるか。★ **`ConsumeRejectedColon` の呼び手 4 箇所**（part property・time・tempo）は**コロン形式を*断る*ために在る**ので生きている＝**出発点は「green に積まれるのに公開プロパティが無いノード」**（今回の `MidiPartRenderSyntax` がまさにそれ）。⚠️ **これは「構文が嘘をつく」を潰す掃きで、`.lys` を 27,095 冊 parse する計器はこの便がもう書いた**（第 12 便の probe＝2 分で再生できる）。⒝ **✅ 第 11 便で閉じた**（上 ⒂）——**名前を書く項目は全数見た**（score／form／lyrics／chords／part／section／phrase／drummap／template／staff・lyrics の retrigger 群）。**残りは drummap の `${1:hh}` だけで、それは欠陥ではなく UX 案件**（83 語の drum 語彙を出すと親切。`drummap { ▮ }` の本体に語彙の家が無いので context が 1 つ要る）。⒞ **第373 ⑼⒜（probe 24 冊の「量を測ったか／問いに答えたか」の仕分け）**も先頭級のまま。**ヘッダの綴りの島は閉じた**——⑻ の 25 綴りが全数で、**残る未点検はゼロ**（`{` を開く構文は score／staff group 5／part／phrase／form／lyrics／chords／section／fonts・paper・layout とその入れ子／音楽の容れ物で尽きる）。⚠️ **閉じたのは*補完の*読み手だけ**: 同じ「2 語しか読まない枠」を持つ道具が他に居ないかは見ていない（`SemanticTokens`・折り畳み・`BarCountFix` は別の木＝**実構文木**を読むので原理的に無縁、という*予想*まで）。⒟ 第369 ⑸⒝（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。

★★ **⒅ 判定の記録（ユーザーの常設指示「次便は、このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）**: **第 2・5・6・7・8・9・12 便はユーザー起票／ユーザー指示**（判定の対象外）。★★★ **第 12 便のユーザー決定には*恒久の規則*が入っている**——**「後方互換の問題であれば、除外すべきだ。lily# のユーザーはまだほとんどいない」**＝記憶 `feedback_lilysharp_prerelease_no_back_compat` の再確認。**⇒ 出所が「古い綴りの受け皿」だと分かった時点で、迷わず外す**（ただし**射程を数えてから**——27,095 冊 0 件を確かめたのがこの便）。★★★ **第 10 便は着手した——判定の根拠は「⒂⒜ を*誰が*見つけたか」**: **私の dump 計器**（第 8 便）なので、**同じ手続きの続き＝温まっている**（下の訂正で立てた基準そのもの）。**結果は着手して正解だった**——**穴だと思って直しに行ったら死語で、直すべきは別の 1 件だった**。**「欠落を埋める」で着手していたら、何もしない綴りを補完に足していた**＝**冷えた次便が同じ § だけ読んで着手すると、まさにそれをやる**。★ **第 9 便は「間違い」ではなく「不便」の起票**——**網は 318 本あったのに、並び順を主張する 1 本も無かった**（`Preselect` と sort キーはコメントで根拠まで書かれていて、誰も測っていない）。**⇒ UX の決定も主張しておく**（この便で足した）。**第 3 便（沈黙 `~`）と第 4 便（全数の掃き）は着手した**——**どちらもこの便の道具でしか安くない**（end-to-end ハーネス＋`GetCompletionContext` の dump 計器で**1 回の実行が 25 綴りを対で読む**。**高いのは「どの読み手が何を訊いているか」で、それは⑷⑹を書いた便の頭の中にしか無い**）。★★★ ⚠️ **私はこの判定を 1 度間違えて書いた**——第 3 便の時点の §1 に「全数表は**読む量が大きく道具の温度が要らない**ので次便が有利」と書いたが、**⑺ の tilde は*その計器で*見つけた**のだから、掃きは同じ操作の N を増やすだけだった。**⇒ 判定を書くときは「どの道具が温まっているか」だけでなく「*直前の発見をどうやって*見つけたか」を見る**——**同じ手続きの拡大なら、それは温まっている**。★★ **第 11 便も着手した**——**⒃⒝ は「読むだけ」に見えて、実際には*判別*が本体**（「名前かどうか」ではなく「その物は既に在らねばならないか」）で、**それはこの便が 11 脚で積んだ知識**。**結果、同じ名前を書く 2 項目が逆向きに間違っていた**＝**冷えた便が「placeholder を本から埋める」と読んで着手すると、`form` の方を*逆に*直す**（`form` は在ってはいけない名前を書く側）。**着手しなかったもの**: ⒄⒜（死語の全数）は**第 12 便の終わりに生まれたばかり**——⚠️ ただし**計器はもう在る**（27,095 冊を parse する probe）ので、**次便が自分で 1 度回してから決めること**。⒄⒝ の残り（drummap の語彙）は**UX 案件**。★ **第 8 便の形＝「統一しない」も立派な決着**——**書き下し（基準）＋仕掛け線（毒で発火を確認）**なら、**正しい 3 行を書き換えずに、壊れる日を見逃さない**。⚠️ **仕掛け線を張ったら必ず毒を入れて赤を見ること**（この便は入れた。入れない網は「在るつもり」になる）。⚠️ **判定の材料は「どの道具が温まっているか」ではなく「直前の的をどうやって見つけたか」**（上の訂正）——**⑼⑽⑾ は 3 つとも*ユーザーが*見つけた**（掃きの計器ではない）。⇒ **次便は自分で 1 度 dump を取ってから決めること**（`GetCompletionContext` を綴りの対で叩く計器は**毎回 2 分で書き直せる**＝第 4・6・7 便で 3 度作って 3 度捨てた。**捨てるのが正しい**＝`reference_lilysharp_throwaway_probe_leaves_tests_project`）。⒃⒞⒟ は**別 regime**（LP を回す／設計級）で、p375 の道具が 1 つも効かない＝**次のセッションでやっても損しない**。★★ **この便の 12 脚のうち、判定で着手したのは 4 脚（⑺ tilde・⑻ 掃き・⒁ MIDI 行・⒂ 名前の監査）で、4 つとも*その前の脚が残した計器・知識*で見つけた的だった**——**「温まっている」の実体は道具ではなく*問いの出所***。★★★ **そして 4 脚のうち 2 脚は「直しに行ったら、直すべき物が違った」**（⒁＝穴ではなく死語／⒂＝`form` は逆向き）＝**冷えた便が §1 の見出しだけで着手すると逆に壊す**という、この便でいちばん再現した形。
