...
**3462 passed / 0 failed / 3 skipped**（＋2＝今回の規則テスト 2 本）・Core 0 warn 0 err・
**LP 忠実度 137/177 exact・total |residual| 2.557670 ss / 151 distances・counts 24/26**
（−0.082027＝**ossia 3 点ちょうど**。exact 数は増えていない）。
...
- ~~★★ **top ばねの床**~~ — **1 個でなく 2 個だった**（第25セッション）。**⑴ は閉じた**
  ...
  **残るのは ⑵ だけ**: **対照側 +0.024000 ＝ 字形の量**。LP の treble clef は中央線の上
  **3.776000**、Lily# の LILC bbox は **3.800000**。⚠️ **既知の clef sliver 族ではない**
  （あちらは 4e-5〜8.3e-4・しかも**同じ字形の下向きは 8e-5 で一致**＝一様スケール差でもない）。
  **§2C（LP を instrument）行き。**
