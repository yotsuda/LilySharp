...
同じ会話の続き（第483 のすぐ後）。ユーザー指示「続けて」＝§1.0 の ⒳¹⁴（下向きスラーが LP より平たい）。
★ **`-Start p484` の 1 コマンドで §0 が全部済んだ**（HEAD `2bd3f868`・未 push 49・full 8824 / 0 / 3 / 8827・`-Archive 482` も自動）。

★★★ **⑴ ⒳¹⁴ は欠陥ではなかった**＝LP は下向きの弧の**外側の縁を先に**、Lily# は**内側を先に**書く（0.118 は太さ 0.12）。**両方の縁を組で比べると一致**（普通のスラー 13.638/13.518 対 13.52/13.64、`.down` のフレージング 13.790/13.670 対 13.67/13.79）。切り分けの順＝LP の `debug-slur-scoring`（`\layout` の変数）で選んだ候補の内訳「L edge=2.00, R edge=3.20 TOTAL=5.20 idx=63」を出し、Lily# に一時の計器（全候補の内訳）を入れると**同じ idx・同じ合計**＝採点は一致→曲線の生成を疑う→生成も同じ→**SVG の path の読み方**だった。網 `ADownPhrasingSlur_IsLilyPondsShape` は 2 本の縁を順不同で比べる。⚠️ **LP の SVG の弧は外側と内側の 2 本の曲線の塗り**＝1 本目だけを読む比較は向きで縁が入れ替わる。
