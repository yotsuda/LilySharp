...
最終更新 第212セッション＝**214 便ぶりに CI の門を読んだ便**（**実装 2 便**）。
⚠️ **彫版のコードは 1 行も動いていない**（**snapshot 218 枚・台帳 529 点とも不動**）。
**動いたのはテスト 2 ファイルと doc だけ。**
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **§1 ⑹ ⓪ が「まず 0.3.0 の判断」だったので裏取りから始めたが、
その裏取りで suite が赤かった**ので、**設問は「0.3.0 か否か」ではなく「この赤は何か」から始まった。**
**ユーザー決定は 3 つ**——**⑴ テストを apphost 非依存にする／⑵ 0.3.0 はまだ出さず 7 件を片付ける／
⑶ 残り 51 件は原因を 1 つだけ測って起票。**
⇒ ★★★ **0.3.0 を出すかは*まだ決まっていない*＝8 便続けて未決**（下の ⑸）。
⚠️ **ただし今回は「未決」の意味が変わった**——**前便までは「決めていない」だったが、
いまは「決められない」**。**§2F ▶▶ が閉じるまで `v0.3.0` を push しても CI が赤で止まる。**

★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**HEAD `9126e161`・未 push 66・
`origin/master` `9830873f`（＝開始時と同じなので便の途中の push は無し）・未追跡 0・作業ツリー項目 0・
台帳 529 点・ss 非ゼロ 100・総和 3.794108828・count 点 106/うち非ゼロ 2・snapshot 218 枚・
追跡コーパス 567 冊・Core 0 警告**＝**ここまでは全部合っていた**。
⚠️⚠️ ★★★ **合わなかったのは 1 つだけ＝ suite が `5577 passed / 7 failed / 4 skipped`**
（引継ぎは `5584 / 0 / 4`）。**「引継ぎの数が全部合っていた」の連番は 40 便で途切れた。**
 終了時 **未追跡 0・作業ツリー項目 0・未 push 72**（**この行を書く commit を含めて 72**。
**数えたのは commit の*あと***）。
⚠️ ★★★ **この数は 3 度書き直した**——**初稿 70（実際は 69＝数え違い）→ 修正して 70 →
*閉じたあとに 4 本目の run が乗って* 71 → この行の commit で 72。**
**第210・第211・第212 と 3 便続けて同じ所で外しており、外し方も 3 便とも違う**
（**第210 は commit の前に数え、第211 は閉じたあとに commit が乗って古くなり、
第212 は数える前に書いた**）。⇒ ★★★ **共通の根は 1 つ＝「閉じの数は*予告*ではなく*観測*」。**
**そして「閉じたあとにもう 1 本走るなら、閉じ自体を取り直す」**（memory がこれを警告していた）。
★ **4 本目の run に対して §7 のチェックリストを走らせ直した**（**第210 はそれを怠った便**）・
suite **5584 passed / 0 failed / 4 skipped**（**復帰**）・
**台帳 529 点で不動**（ss 非ゼロ 100・総和 3.794108828・count 106/2 も不動）・
**snapshot 218 枚で不動**・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Turmeric`）で通せた。**

> ## ★★★ この便の骨 1＝**引継ぎの数は「書いた時点では本当」だった。嘘にしたのは、それを書いた commit 自身**
> **第211 は緑を測り、そのあと handoff の commit を 3 本積んで閉じた。**
> **その commit が HEAD を変え、HEAD が `lysc.exe` の中身を変え、Windows がそれを弾いた。**
> ——**SDK は `InformationalVersion`（`0.3.0+<git sha>`）を apphost の版数リソースに焼く**ので、
> **`lysc.exe` の SHA-256 は commit ごとに新品**になり、**Smart App Control（この機械は Enforce）が
> 「見たことのないファイル」として `Process.Start` を拒む。**
> ⇒ ★★★ **つまり「便を閉じたあとに commit する」こと自体が、その便が報告した緑を偽にしていた。**
> **測った時刻と報告した時刻のあいだに、測った対象が変わっている**——
> **§5.0「測定と判断を混ぜない」の*時刻*版。**
> ⚠️ **裏取りは 4 本とも取った**: 同じビルドの `LilySharp.Probe.exe` は通る（そのハッシュは既知）／
> **08-17・08-18 に弾かれた Benchmarks と Probe は今は通る＝ブロックは時間で解ける**／
> **弾かれた exe を別パスへ複製しても弾かれる＝判定はパスでなく内容**／
> **`dotnet lysc.dll` は通る＝管理コードは無傷**。**RULES の SAC の族の 3 例目**で、
> **本体スイートに出たのは初めて。**

> ## ★★★★ この便の骨 2＝**その 7 件を追ったら、CI の門が 214 便ぶん誰にも読まれていなかった**
> **`gh run list` は直近 8 push すべて failure。**
> **最後に push した `9830873f` で ubuntu Release は 5331 合格 / 59 失敗**
> ——**そして windows-latest Release は完走して ✓**（他脚の `X` は fail-fast の巻き添え）。
> **引継ぎにも RULES にも「CI」「ubuntu」「Linux」は 1 行も無い。**
> ⇒ ★★★ **「§0 の緑」は*この機械の Windows での緑*でしかなかった**——
> **§0 は 214 便のあいだ、門の半分しか見ていなかった。** **§0 に `gh run list` を足した。**
> ⇒ ★★ **そして 59 件のうち 7 件は、今日 Windows で赤くなったのと*同じ 7 件*だった**
> ——**Linux には `lysc.exe` という名前が存在しない**ので、**ずっと "not beside the tests" で落ちていた。**
> **同じ 1 行が、2 つの OS で 2 つの別々の理由で落ちていた。**

★ **この便の値段**（**彫版のコードは 1 行も動いていない**。**台帳 529・snapshot 218 とも不動。**）:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ `lysc.exe` → `dotnet lysc.dll` | 網 1 | **Windows の SAC ＋ ubuntu 両脚の 7 件** |
| ⑵ `Path.Combine` に `\` 入りリテラル | 網 1 | **ubuntu 両脚の 1 件**（Windows は挙動不変） |
| ⑶ 起票と §0 | doc 2 | **観測のみ** |

- **⑴ `5ac02423`**。**apphost は*名前*も*中身*もプラットフォーム依存で、両方が既に赤を出していた**（骨 1・骨 2）。
  - **`dotnet lysc.dll` で起動する。muxer は `RuntimeEnvironment.GetRuntimeDirectory()` から導出**
    ——**`DOTNET_HOST_PATH` は同じ答えを返すが（testhost の中で実測）、置くのは MSBuild なので
    テスト dll を直接走らせると無い。**
  - **毒を 2 種当てた**: **`Program.cs` の best-effort を戻すと*誤り 5 件だけ*が赤くなり
    正常系 2 件は緑のまま**（＝終了コード・stderr・書かれたファイルの 3 本とも本当に見ている）／
    **muxer のパスを 1 段浅くすると*黙って通らず*読める文言で落ちる。**
  - ⚠️ **失われた被覆は「apphost が起動すること」ただ 1 つ**で、**Linux では元から誰も見ていなかった。**
    **それはリリースの問題であってこのクラスの問題ではない**——**下の ⑸ ⒦ に置いた。**
- **⑵ `dd1ef1c1`**。**`Path.Combine(root, @"LilySharp.Core\Svg\EngravingDefaults.cs")` は
  Linux では `\` が*ファイル名の中に*残る**ので、`LpProvenanceTests` は ubuntu で
  「did it move?」を出し続けていた。**Windows では `Path.Combine` が同じ文字列を作るので挙動不変。**
  ⚠️ **族は grep で数え切った＝テストプロジェクトに他の site は無い**
  （`.exe` の直打ちも、`@"…\…"` のパスも、これで 0 件）。
- **⑶ ★ 起票＝§2F の ▶▶「CI の ubuntu 脚が赤い」。** **§1 ⑸ ⓪ はそこを経由しないと答えられない。**
  - ★★★ **測ったことと、外した当たりの両方を書いた**——
    **「落ちるのは文字を描く本」という当たりは*空振り*だった**（**snapshot 218 枚は全部 `<text>` を含む**）。
    **台帳で分けたら見えた形は「縦に積み上げて距離が累積する族」**
    （`lyrics` 10/38・`staff` 9/35・`script` 5/8・`ottava` 4/8・`textscript` 3/6・`barnumber` 2/2 …
    **一方 `fingering` 22/22・`figbass` 13/13・`text` 9/9・`dynamic` 4/4 は文字を測るのに全部緑**）。
  - ⚠️⚠️ **原因はまだ測っていない**。**フォント側は白**（同梱・システムフォント不使用・分岐ゼロ）で、
    **残る筋は「同じフォントを測る*道具*が OS ごとに別ネイティブ」**（SkiaSharp / HarfBuzz）。
    **筋であって測定ではない、と §2F に明記した。**
- **⑷ ★ perf の判定＝計算を足していない。** **触ったのはテスト 2 ファイルだけで、
  Core にも LSP にも 1 行も入っていない**（§7.9 の「足していない例」）。
- **⑸ ★★★ 次に触るならここ**——**⓪ §2F ▶▶ の CI（観測 59・第212 の修正が効けば 51）**。
  **0.3.0 の前に必ず通る門で、次の一手は「まず WSL2 に .NET を入れて Linux をローカルで走らせる」**
  （**Ubuntu 24.04 は在るが .NET が無い**）。
  ⚠️ ★★ **その最初の 1 回で「51」の裏も取れる**——**第212 の 8 件は Windows でしか緑を見ていない。**
  ／**① そのうえで 0.3.0 の判断**（**8 便続けて未決**。**`Directory.Build.props` は `0.3.0`・
  タグは 1 つも無い**）。⚠️ **出す前に片付ける候補は 3 つに増えた**:
  **⒤ `CHANGELOG` が無い**／**⒥ 未 push 70 便ぶん**（**`master` を先に**）／
  **⒦ ★ 新規＝未署名 exe を SAC が弾く**（**`release.yml` に署名は 1 行も無い**ので、
  **SAC 既定 ON の Windows 11 では利用者が今日 私が踏んだ壁に当たる**。**ユーザー判断が要る**）
  ／**⒜ ★ 台帳点 `courtesy.key.key-to-line-end` の残差 0.15**（第206 が開いた。
  **閉じ方は `SharedRenderer` 自身が名指している**＝予約幅を描画幅にして `SolveColumns` に渡す）／
  **⒝ ★ ペダルの段間隔**（LP は 1.961 → 2.443・Lily# は一律 2.46。**台帳点は無い**）／
  **⒞ ★ `CoPlaceToCodaWithLabels` の鏡像ケース**／**⒟ §2 の頭の ▶ perf**（門の外）／
  **⒠ ★ SVG の同梱 sans のずれ**（**出力についてのユーザー判断が要る**。
  ⚠️ **§2F ▶▶ と同じ島かもしれない＝先に ▶▶ の ① を測ること**）／
  **⒡ ★ 配管の残り 6 site**（`TabStaffGeometry` 3・`FetaTextRun` 3）。
...
> # ▶ **同名のシステムフォントが同梱フェイスを隠す＝エディタが「エンジンが使わないフェイス」を勧める**
> （2026-08-19・第213 起票。**この機械の WSL でだけ赤い 2 件がこれ**。**直していない**）
>
> ⚠️ **これは「Linux の赤」ではなく「TeX Gyre が*インストールされている*機械の赤」。**
> **この機械の WSL は LilyPond のビルド依存で TeX Gyre をシステムに持つ**（`fc-list` 312 本）。
> **GitHub の ubuntu-latest は持たないので CI は緑**、**この Windows も持たないので緑**
> ——**つまり門を 2 つ通ってなお出る、機械依存の欠陥。**
>
> ## 測ったこと
> - **落ちるのは `FontBlockCompletionTests` の 2 件**:
>   `EveryFaceTheSnippetPreFillsIsAlsoOffered`（**`"TeX Gyre Heros"` の sortText が
>   `!…` ではなく `11…`**＝**同梱の段ではなくシステムの段に居る**。Detail も
>   `embeddable - license unverified`）／
>   `AGenericFamilyBindingOffersOnlyThatShape`（**serif の束に sans の `TeX Gyre Heros` が出る**）。
> - ⇒ **face の一覧は「同梱 2 つを先頭に置き、Skia が列挙したインストール済みを後ろに足す」**
>   形で、**同名を畳んでいない**。**同梱されているフェイスがインストールもされていると、
>   同じ名前が 2 つ出て、後ろのほう（システム）が分類と並びを持っていく。**
>
> ## ⚠️ なぜ「テストの都合」ではなく製品の欠陥か
> **`TextFontMetrics` は「束を機械より先に見る」**（`BundledPathForName`）ので、
> **エンジンが測って描くのは*同梱*のフェイス**。
> ⇒ **エディタは「システムの TeX Gyre Heros」を勧めているのに、
> 選ぶとエンジンは*同梱の*同名フェイスで組む**——**同じ名前だから気づけない。**
> ⚠️ **`TextFontMetrics` の doc が言う「1 つの問いに 3 人の読み手」の 4 人目。**
> **`f7e18024` と `EveryFaceTheSnippetPreFillsIsAlsoOffered` がその 3 人を揃えたが、
> *重複*は誰も見ていなかった。**
>
> ## 閉じ方（**未着手・小さい**）
> **列挙した家族名から同梱の 2 つを畳む**のが素直だが、**「畳む」と「システム側を別の
> 段に見せる」は別の判断**（**ユーザーが自分の入れた TeX Gyre を指したいことは在り得る**）。
> ⇒ **決めてから書くこと。** ⚠️ **再現は `scratch/linux-repro/README.md` の環境でだけ。**
