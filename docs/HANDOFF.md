...
⚠️ **見つけて直していないもの**:
- **★★★【ユーザー決定 2026-08-26・優先度高】リファクタ在庫の残り**——**③ は第 1 段（第261）に続き第 2 段も第262 で決着**（⒜ 供給源は寄せない＝測定済み・⒝ verse-hole 台帳 8 点＝粒度統一の器械。**残るは「合成帯の現役読者の特定」＝挙動を変えない読み調査・上の ⇒**）／**②（2 ページ経路の二綴り）④（−2.0 直書き）はコーパス A/B 必須の専用便**／**⑨（greedy fallback＝挙動差あり）⑪（Reset()/3 重列挙）⑬（form 再生順序 3 実装）⑭（LSP 言語リファレンス三重）**／**ファイル分割（partial 縫い目・レビュー §4c）は分割禁止ルール非公認をユーザー確認済み・専用便＋証明 3 点セットで**／死物件の残り（CalculatePureSystemHeight・KnuthPlass 単譜オーバーロード＝テスト網の付け替えが要る）。
...
終了時: **本便のコード commit は 2**（`4417f44c` LooseLineSpacer remark＝⒜ の答えの記録／`9c91c4c6` verse-hole 対＝probe＋台帳 8 点＋remark 更新）＋ handoff 系（総数は `git rev-list --count origin/master..master` − 開始時 43 で引くこと）・origin 不動＝間で push なし・未追跡 0/木 0・**Windows Release 6212 合格 / 0 失敗 / 4 skip / 合計 6216**（+8＝台帳 8 点）・**WSL ubuntu Release も 6212 / 0 / 4 実測**・Core 0 エラー / 0 警告（`--no-incremental`）・台帳 **658／exact 526／総和 24.390110433／非ゼロ 160／count 129・非ゼロ 2**（§0 の式で数えた・増分 0.278653506 は新 8 点の自前の字面項の総和と桁まで一致＝**既存 650 点は 1 点も動いていない**）**／snapshot 222（再ベース 0）／追跡コーパス 572 不動**（RVH の .lys はテスト内文字列で、.lys ファイルは足していない）。**絵側の Core 変更ゼロ**（本便の Core 差分はコメント 2 箇所のみ・一時計器は revert 済み）＝sweep 不要・suite＋台帳が計器。
