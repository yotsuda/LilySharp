...
> ⇒ **詰まったら語りを削るのではなく、まずここの ✅ と重複を落とす**（第453 −979 字・第454 約 −850 字
> ＝✅ 2 件と、尾しか残っていない ⒩⁴ の一覧）。

**⒜ 今すぐ手が動く（計器も直し方も分かっている）**
- ★★ **⒮⁶ 「建てない」の島の残り＝約 36,900 B／打鍵 0.82%**（**第455 が census を回し直した**＝Lab
  `sessions/p455/site-prices-before.txt`・分母 **4,520,635**・脚は同 `instrument.ps1`＋`Zz455.template.cs`
  ＝1 run で全サイト。⚠️ **7 軒すべて p451 と 1 B も違わなかった**＝第454 はこの島を動かしていない。
  そこから**第455 が閉じた 4 軒 4,869** を引いた。**次にこの島へ戻る便はまた回し直すこと**）。
  ⚠️⚠️ **`MeasureLayouter.cs` と `MeasureContentKey.cs` の行番号は第455 が動かした**＝この節の住所は
  census 時点のもの。⚠️ **門前払いと実証済みの 3 軒 16,058 は除く**
  （`ItemSkylineFactory.cs:465` 9,118／`HorizontalSkyline.cs:63` 4,171／`BeamSubdivision.cs:253` 2,769＝
  どれも器が呼び手へ出る）。⚠️⚠️ **その「門前払い」は*この欄*（hold1＝1 件を局所で持つ）についてだけ**
  ＝**⒮¹⁰ の欄（器ごと park）では 3 軒とも scratch**で、`ItemSkylineFactory.cs:465` は**第457 が閉じた**
  （残り 2 軒 6,940 は ⒮¹⁰ の一覧に居る。RULES §5.3）。
  ⇒ **手が動く残りは約 20,800 B／打鍵 0.46%**で、うち 6,409 は下の ⒮⁸。
  ...
  **781**（⚠️ **1 件で 1,928 B＝`Entry` が大きく capacity 3 が 3 枠取る。直すなら `Entry` を class にするか
  ...
  ⚠️⚠️ **zero 欄も one 欄も「drain 時点の最終 `Count`」**＝`Clear` で使い回す器は*両方に*化けて出るので、
  **`actual` と*コード*の両方と突き合わせて読む**（例と規則は RULES §5.3 末尾）
- ★★ **⒮⁸ 器を消せない残り 2 軒＝6,409 B／打鍵 0.142%・どちらも*値段ではない理由*で残った**（第455）。
  ...
  ＝**struct の値にすると損**: 鍵 `(int,int)` の辞書（同 `:84` actual 2,267）の entry が 24→40 になって
  **+1,511** 払い、one 27.1% の賞金 **約 911** しか返らない。**効く直しは「1 本の flat な list を
  (挿入順位, Left) で並べる」**だが、`foreach (var ((sysIdx,staffPos), entries) in perSystem)` が
  **辞書の挿入順**で builder に足すので、**並べ替えると `LedgerLineSpan` の配列順＝SVG が動く**
  ⇒ 順位を `Dictionary<(int,int),int>` で憶えてから安定ソートする形なら順序は保てる（未実装・未計測）。
...
- ★★ **⒳⁵ tie の outline の*旗の箱*は和音に建っていない**（第452 起票）。
  **`TieColumnParts.Flag` を `[]` にしても 41 冊 0 差だが、同じ箱を `(-1000,1000,x,x+50)` にすると
  ...
  は **`item is NoteItem` にしか旗の箱を建てず**、**単音の tie は必ず符尾の*反対側*に出る**
...
- ★★★ **⒮¹⁰ 「器ごと憶える」の島の残り＝336 軒 254,879 B／打鍵 6.37%・*全部が liveness を通っている***
  ...
  **生きている 356 軒の actual＋obj は 481,208＝打鍵の 12.03%**で、**escape はたった 16 軒 102,619**
  ...
  そこから**第457 が閉じた 4 軒 123,703 と第458 が閉じた 6 軒 98,901** を引いた（残り 329 軒
  155,978＝**今の打鍵 約 3,776,500 B の 4.13%**）。
  ⇒ **次の軒は値段順に並んでいるが、頭の 6 軒は全部 `ImmutableArray` の Builder**
  （`LedgerLineSpannerEngraver.cs:79` **7,822**／`LayoutEngine.Prelim.cs:402` **5,388**／
  `BarNumberEngraver.cs:256` **5,232**／`MusicMarkEngraver.cs:680` **4,032**／
  `MeasureLayouter.cs:947` **1,687**／`PageLayouter.cs:516` **1,366**＝約 25,500）。
  ⚠️⚠️ **Builder を park する前に 1 つ測ること＝`Builder.ToImmutable()` は `Count == Capacity` のとき
  *自分の配列を渡す*ことがあるか**。渡すなら、park した Builder を `Clear` した瞬間に**呼び手が持って
  いる `ImmutableArray` が壊れる**＝プローブ 1 本で 6 軒ぶんが決まる（第458 はこれを測らずに Builder を
  避けた）。Builder でない次点は `OutsideStaffStacker.cs:1146` **4,275**／`BeamSubdivision.cs:253` **4,379**／
  `HorizontalSkyline.cs:63` **4,170**／`LayoutEngine.Prelim.cs:493` **3,831**／`TieChordOutline.cs:191` **3,638**。
  ...
  **liveness は必要条件しか言わない**（RULES §5.3）。
  ★ **第458 の実測＝会計 98,901 に対し −98,875（99.97%）**＝**give 点が同じメソッドの中にあり、
...
- ★★★ **⒮¹² splice の「trivia だけの窓」の門は、どちらの母集団にも観測者が居ない**（第458 起票・実測）。
  ...
  **その答えを常に true にする毒＝7,016 本の splice を全部通す毒は、スイート 8,783 緑・コーパス
...
- ★★★ **⒮¹³ `ArticulationEngraver` の tie-bound 地図には*スイートに*観測者が居ない**（第458 起票・実測）。
  ...
  2 枚の地図で観測者の数が 104 対 0
- ★ **⒩⁴（寸法を言う）は*尾しか残っていない*＝着手先としては最後**（311 軒 150,948 B／打鍵 2.341%・
  現在地は Lab `sessions/p442/site-prices-after.txt`）。**top10 の 1.619% のうち 1.144% は「寸法を言えない」
  と実証済みの 2 軒**（第439・第441 が histogram で反証）で、**その下の上位 4 軒は第457・第458 が
  *器ごと* park した**＝この島の頭の値段は、寸法を言う前にもう建たなくなっている
...
`/clear` 直後の新セッション。指示は「**HANDOFF を読んで着手**」で、**着手先は §1.0 ⒜ の ⒮¹⁰**（ユーザーが
選んだ・⒮¹⁰ と ⒮⁹ と ⒮⁸ と ⒮⁶ を並べて訊いた）。★ **`-Start p457` の 1 コマンドで §0 が全部済んだ**
（HEAD `0c94e544`・未 push 5・full `sessions/p457/run1.trx` 8783 / 0 / 3 / 8786・台帳 851 点／
総和 22.584727806・snapshot 249・追跡 `.lys` 609・`-Archive 455` も自動＝moved 89 行 5,361 字）。
**裏取りは 1 つも赤を出さなかった。**

★★★★ **⑴ 国勢調査に *liveness* の欄を足したら、起票が「門前払い」と書いた軒が島で最大の scratch だった。**
⒮⁶ は `ItemSkylineFactory.cs:465` ほか 3 軒を「**器が呼び手へ出ると実証済み**」として除いていたが、
それは**第447 の直し方（1 件を局所で持つ＝hold1 欄）についての判定**で、⒮¹⁰ が訊いているのは別の文＝
**「*render* から出るか」**。⇒ **「メソッドから出る」と「render から出る」は失格にする直し方が違う**。
計器は**弱ハンドル 1 本／建てるたび＋render 境界で強制 gen2**で、356 軒中 **340 軒が scratch
（378,582 B／打鍵 9.47%）・escape は 16 軒（102,619）だけ**。★★ **その 16 軒は手で名指せるものばかり**
（memo・cache・layout・⒮⁸ ⑵ が「唯一器が呼び手へ出る」と書いた軒）＝**一致そのものが較正**。
⚠️ **sweep 時に呼び手が前の render のページを握ったままにする**（実機の条件）。握った版と捨てた版を
2 本取って**差 0 軒**＝出力の中に器は 1 つも住んでいない。規則は RULES §5.3。

★★★★ **⑵ 直し方は 9 便前から木の中に在った**（第455 の 17例目の 2 例目）。`VerticalSkyline.cs:985` が
**第421 から `[ThreadStatic]`＋「抜く」rent＋Give で `Clear`** を持っていて、**nesting も thread も例外も
「miss であって mix ではない」**理由まで註に書いてある。**4 軒ともその形のまま**＝新しい型は 1 つも足していない。

★★★ **⑶ 4 軒・A/B −123,518 B／打鍵 3.09%（会計 123,703 の 99.85%＝*予告どおり「下」*）。**
`ItemSkylineFactory.cs:465` **97,154**（311.05 回／打鍵・2.12 件・max 9・alive1 **0 / 574,829**）／
`Parser.Declarations.cs:136` **15,368**（`[.. items]` が写す）／`LayoutEngine.Pages.cs:867` **7,708**
（**waste 0＝actual だけの最も純粋な行**＝寸法は常に正しく、ただ 35.69 回建てて 1 回ずつ読むだけ）／
`MultiStaffLayouter.cs:2213` **3,473**。★ **parse 半分は −15,170（会計の 98.7%）だが、同じ側を 2 回
取った振れが約 250 B／打鍵＝雑音の中**。**render 半分は振れ 17 B で −108,348＝会計 108,335 の 100.01%**。

★★★★ **⑷ 毒 6 本のうち色が合ったのは 4 本で、外れた 2 つのほうが高かった。**
⑴ **毒 3（C を汚して park）は*恒等写像*だった**——`StaffSprings` と `StaffGroups` は**同じ system**から
来るので、spring が訊く鍵はすべて 3 行上のループが*今*書いた鍵。**門で裏を取った**（`continue` を
`throw` に：**スイート 8,783 緑・コーパス 231 冊完走**＝guard は両方の母集団で 0 回）。**事後条件を
1 行読めばタダで済んだ**（第453 の 3 つ目の顔）。⑵ **緑を予測した毒 5・6 がどちらも「赤 1」で返り、
その 1 赤は毒のものではない**＝どちらも `ItemSkylineFactory.cs` から**1 行消す**ので
`ApproximationInventoryTests` が行番号で赤くなる。**第449 の罠の 3 例目で、宿主が*毒*になった形**
＝ベースラインは 0 赤なので言い分けられない。規則は RULES §5.4。

★★★ **⑸ 毒 1 は「返らない毒」だった。** `ColumnParts` の buffer を空にせず park すると skyline が
**単調に膨らみ**、full run が **7 分・CPU 9,905 秒**（clean は 114 秒）で 1,853 本までしか進まない
（うち 14 赤）。RULES §5.0 の第430 の記述の **2 例目**＝「full が返らない」は環境ではなく毒。

★ **⑹ 出力は同一。** 実コーパス 231 冊 × 8 打鍵の**全ページ SHA-256＝5,824 行・0 差**（p439／p451／
p455／p456 と同じ絵）。**網は 1 本も増えていない**＝full **8783 / 0 / 3 / 8786** のまま。
⚠️ 増やさなかったのは、**毒 1・2・4 が「汚れた buffer」を既に 8,780 本で観測している**から
（第456 の「出力に映らない最適化には網が要る」は、ここでは*映る*）。**観測者が原理的に居ないのは
毒 3 の軒だけ**で、そこには**門の結果を註に書いた**。

★ **⑺ 終了時**: commit 2 本（code `0f57f595`＝Core 4 ファイル＋再生成した `APPROXIMATIONS.md`／
`magic_constants.csv`、**最後の docs＝この文を含むので SHA は書かない**＝§5.4）。
**最終 full 8783 / 0 / 3 / 8786**（`sessions/p457/run3.trx`＝docs を閉じた*あと*に `-End` が回した）・
§7.5（対 `0c94e544`）**Core '+' 228 行／REF 0／OWN 0**＝⚠️ **LP に対応物が無い**（貸し buffer は LP に無い）。
**コード中の新しい数は 0**＝数値を含む追加行 34 本のうち 33 本はコメント内の実測値で、残る 1 本は
`ColumnParts(item, 0, staffY)` の**既存の実引数**（§7.6 ⒟）。**§7.7 の匂いは 2 つ**＝
⑴ **give を忘れても何も投げない**（賞金が黙って消えるだけ）＝観測者は網ではなく A/B で、
**毒 6 がその形**（緑・註に書いた）／⑵ **C の `Clear` には観測者が原理的に居ない**＝門の結果
（両母集団 0 回）を註に書いた。台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・
追跡 `.lys` 609・未追跡 0。全文は Lab `sessions/p457/`（`prediction.txt`＝会計と方向と毒の色・
`site-prices.txt`／`site-liveness.txt`／`site-liveness-dropped.txt`／`joined.csv`＝⑴ の 2 本・
`instrument.ps1`＋`Zz457.template.cs`＋`Zz457Leg.cs.txt`・`ab-*-tc0.txt`＝A/B 5 本・
`hashes-after.txt`＝⑹・`poisons.ps1`／`poisons.txt`＝毒 6）。**push はユーザー**（Lab も）。
**`-Start p458` の 1 コマンドから入る**。

★★ **⑻ 「有利なら次便ぶんも着手して」には着手しなかった＝第453〜第456 と同じ理由で、材料ではなく口座。**
⚠️ **この判定はユーザーに 2 度訊かれて 2 度とも同じ答え**（2 度目は便の終わりに測り直した）。
**§1 現在便は残り 1,000 字を切り**、**HANDOFF の残りも 7,400 B（≒2,400 字）を切っている**
——★ **口座の数は*それを書く編集で動く*ので、上限ではなく*下回らない境*で書く**（第457 が 3 字ずらした）。**1 便の語りは 3,800〜4,900 字**＝**やっても書く場所が無い**。
⚠️ **材料の側は逆に今が最も有利**——⒮¹⁰ の残り 336 軒は**値段も liveness も全部付いていて**、
次の軒（`MusicSiteList.cs:83` **55,484**）に要るのは*直し方の設計*だけ（返る器の give 点）。
⇒ **`-Start p458` が第456 のブロックを ARCHIVE へ出せば §1 は約 12,700 字に戻る**＝
**次便は削らずに 1 便まるごと書ける**。★ **静かな窓も長く使った**（毒だけで 25 分・うち 13 分は
「返らない毒」2 本）。ここで閉じる。
