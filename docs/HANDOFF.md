...
★★ **未 push は 4 便続けて両端が繋がった**——**第196 の終了時 35 と開始時 35 が一致**（間で push は無い）。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 42**（★ **最初にこの引継ぎ commit を含めて 40 と書き、
その*あと*にユーザーが ⒡ を承認したので 2 本増えた**——**第196 が手順に落とした
「commit を作る*前*に数えて +1 して書く」は当たったが、*引継ぎのあとに便が続くと*ずれる。
⇒ ★★ 引継ぎ commit は最後にすること、でなければ最後に数え直すこと**）・
