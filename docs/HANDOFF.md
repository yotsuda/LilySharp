...
- ★★★ **⒮¹⁰ 国勢調査を `waste` ではなく `actual` 欄で読み直す＝*再利用*の島**（第456 起票）。
  第456 が ⒮″ で取ったのは waste 27,355 ではなく **actual 506,766＝18.5 倍**＝**`waste` は「正しい寸法で
  建てる」の値段で、`actual` は「建てない＝器ごと憶える」の値段**（1 render の中で死ぬ scratch なら効く）。
  生きている 363 軒の合計は **actual 953,499／ideal 737,957／waste 215,542**（p455 census の 3 行目）で、
  ⒮″ を引いた残りは **約 446,700 B／打鍵 9.9%**。⚠️ **上位 3 軒は「器が呼び手へ出る」と実証済み**
  （`VerticalSkyline.cs:76` 92,716／`ItemSkylineFactory.cs:465` 87,200／`MusicSiteList.cs:83` 55,273）
  ⇒ **次便の仕事は「どの器が*1 回の render の中で死ぬ scratch* か」を数えること**＝値段はもう付いている
- ★ **⒮¹¹ `Svg/Collector/CollectResumePlanner.cs:286/287` の `Stack` 2 本＝8,838 B／打鍵 0.21%**
  （220.96 件／回・max 708）は**寸法を言えるか未調査**（⒮″ から切り出した。⒮¹⁰ の網にも掛かる）
...
- **`docs/RULES.md` は 236,606 / 250,000 B・1,869 / 2,000 行**（第456 が規則を 5 本足した）。
  ⇒ **次に詰まったら、割るのではなく*規則そのもの*を畳む**（印のほうが高くつく）。

### 1.1 第456セッション（2026-09-21・YT-DELL2）

`/clear` 直後の新セッション。指示は「**HANDOFF を読んで着手**」で、**着手先は §1.0 ⒜ の ⒮″**（ユーザーが
選んだ・⒮″ と ⒲ と ⒮⁹ と ⒮⁸ を並べて訊いた）。★ **`-Start p456` の 1 コマンドで §0 が全部済んだ**
（HEAD `fe984af4`・未 push 3・full `sessions/p456/run1.trx` 8780 / 0 / 3 / 8783・台帳 851 点／
総和 22.584727806・snapshot 249・追跡 `.lys` 609・`-Archive 454` も自動＝moved 92 行 5,345 字）。
**裏取りは 1 つも赤を出さなかった。**

★★★★ **⑴ 起票が名指した値段は国勢調査の `waste` 欄で、同じ行の `actual` 欄はその 18.5 倍だった。**
⒮″ は「27,355 B／打鍵 0.637%」と書いてあった。同じ行の `actual` は **506,766＝render の 12.1%**。
**`waste` 欄が値付けているのは「器を*正しい寸法で*建てる」直し方だけ**で、**`actual` 欄は
「器を*建てない*」直し方の値段**——そして 1 つの render の中で死ぬ scratch なら、後者が効く。
⇒ **起票の「寸法は前のページが知っている」は、読みは合っていたが直し方が小さすぎた**
（第455 の 17例目の再演）。規則は RULES §5.3。

★★★★ **⑵ 「寸法を憶える」より「器を憶える」ほうが安く、しかも機械に優しい。**
起票どおり `new StringBuilder(前ページ長)` にすると、**76,047 字＝152 KB の配列が LOH（85 KB 境界）へ毎打鍵
3.15 本**落ちる＝台帳は 27,355 減って機械は悪くなる。**器ごと憶えると `actual` が丸ごと 0 になり、
LOH の配列は*一度だけ*建って住み続ける**。⇒ `SvgPageBuffers`（新規・session が持つ・**page 索引ごとに 1 枠**）。

★★★ **⑶ 設計の前に機構を測った。** .NET の `StringBuilder.Clear()` は**chain を持つ器では head へ歩いて
容量ぶんの配列 1 本に畳み、単一 chunk の器では即 return で 1 バイトも割らない**（`Length` setter の
`m_ChunkPrevious == null` 枝）。実測（pwsh・.NET 10.0.12・76,000 字）＝fresh 625,072／clear#1 152,888／
**reuse 473,032・clear+reuse 473,448**（差 152,040＝配列ちょうど 1 本、残りは PS の雑音で両側同じ）。
⇒ **畳む代金は「伸びた render」が 1 回払い、2 回目以降は 0。** だから `Clear` は `Take` ではなく `Give` に置いた
（`Take` で防御的に消すと ⑹ の毒 1 が黙る）。

★★★ **⑷ A/B −508,076 B／打鍵（render 4,174,634 → 3,666,576＝−12.17%・打鍵全体で −11.3%）。**
会計 505,894 に対し **100.43%＝上**。★ **その「上」は実測の*前*に書いた**——`actual` は国勢調査の `odd` 腕
（64%）が**最後の chunk しか数えない下限**で、chain を模すと 509,400。**第455 が潰した AMENDED を復活させた**
（`prediction.txt` §5b・450 上／451 下／454 上／455 下／**456 上**）。

★★★ **⑸ 唯一の当て推量を計器が 50 倍直した。** 会計は「頁が溢れて建て直す率 10%」と置いたが、実測は
**repark 0.19%・served 100.00%・fresh 0.00%・3.15 頁／打鍵**（Lab `counts-pool.txt`＝国勢調査の
builds/key と桁まで一致）。⇒ 会計の +53,900 は +1,023 だった。**`used/8 + 1024` の余白が steady state を
無料にしている**（余白 0 の毒 6 は緑＝絵には映らない＝**観測者は網ではなく計器**）。

★★★★ **⑹ 毒 8 本すべて予測どおりで、うち 2 本は*赤くなる本数*まで当たった。**
赤＝毒1 `Clear` を外す 116／毒2 枠を空けずに共有 29／毒3 ToPages の前に返す 17／毒4 ToSvg の前に返す 142。
**緑を予測して緑は 2 本**（毒6 余白なし・毒8 `_pages` を残す）。★★ **毒5「一度も park しない」と
毒7「1 枠ずれて park」は「`SvgPageBufferTests` の 2 本*だけ*が赤」と予測し、どちらも baseline 3 → 5**。
⇒ **出力に映らない最適化には liveness の網が要る**——絵の網は「pool が黙って働かなくなる」形を
1 枚も見ていない。規則は RULES §5.4。

★★ **⑺ baseline が 3 赤だった。原因はコードではなく §7 3.5** ——`-Start` が第454 をアーカイブした後、
**§1 に「## 以下は第455セッションの経緯」を立てる前に毒を回した**＝`HandoffArchiveContinuityTests` の 3 本
（第449 の「毒の赤は自分の変更からも来る」の 2 例目で、あちらは `APPROXIMATIONS.md`）。
**baseline 行を刷っていたので 1 秒で割れた**。⇒ 規則は RULES §5.4。

★ **⑻ 出力は同一。** 実コーパス 231 冊 × 8 打鍵の**全ページ SHA-256＝5,824 行・0 差**（p439／p451／p455 と
同じ絵）。**網は 3 本増えた**（`SvgPageBufferTests`＝等値の半分と liveness の半分）＝full **8783 / 0 / 3 / 8786**。

★ **⑼ 終了時**: commit 2 本（code `f4539ce2`＝Core 3 ファイル＋新規 `Rendering/Svg/SvgPageBuffers.cs`＋
`LilySharp.Tests/Rendering/SvgPageBufferTests.cs`、**最後の docs＝この文を含むので SHA は書かない**＝§5.4）。
**最終 full 8783 / 0 / 3 / 8786**（`sessions/p456/run3.trx`＝docs を閉じた*あと*に `-End` が回した）・§7.5（対 `fe984af4`）**Core '+' 196 行／REF 0／
OWN 0**＝⚠️ **LP に対応物が無い**（LP に render session は無い）。**コード中の新しい数は 2 つだけ**
＝`MinSlack = 1024` と `used / 8`＝**容量の目分量で、絵には映らない**（余白を 0 にする毒 6 が全緑＝
観測者は網ではなく計器）。**§7.7 の匂いは 1 つ**＝`ReleaseBuffers` は呼び手が憶える手順で、
**忘れると賞金が黙って消え、早く呼ぶと投げる**。⇒ **投げるほうは `ThrowIfReleased` が言い、忘れるほうは
liveness の網が言う**——両方に観測者を置いたうえで残した（器の所有権を型で移す設計は、
`IDrawingContext` の寿命ごと変えることになる＝⒮¹⁰ の島で考える）。
台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・未追跡 0。
全文は Lab `sessions/p456/`（`prediction.txt`＝会計と AMENDED・`counts-pool.txt`＋`Zz456Count.cs.txt`＝⑸ の計器・
`ab-*-tc0.txt`＝A/B 4 本・`hashes-after.txt`＝⑻・`poisons.ps1`／`poisons.txt`＝毒 8）。
**push はユーザー**（Lab も）。**`-Start p457` の 1 コマンドから入る**。

★★ **⑽ 「有利なら次便ぶんも着手して」には着手しなかった＝第453 ⑽・第454 ⑼・第455 ⑽ と同じ理由で、
材料ではなく口座。** 判定時点で **§1 現在便 17,808 / 20,000＝残り 2,192 字**・**HANDOFF 442,218 / 450,000
＝残り 7,782 B（≒2,600 字）**。**1 便の語りは 3,804〜4,925 字**（本便 3,804・第454 4,925・第455 4,728）
＝**やっても書く場所が無い**。⇒ 空けるには **§1.0（13,973 字）の ✅ と「戻らない島」の表を削るしかなく、
それは*永続の索引*を 1 便と交換すること**——`-Start p457` が第455 のブロックを ARCHIVE へ出せば
（前便の実績で 92 行 5,345 字）§1 は **約 12,500 字**に戻る＝**次便は削らずに 1 便まるごと書ける**。
⚠️ **材料の側も不利**: 残りは ⒮¹¹（0.21%＝小さい）と ⒮¹⁰（9.9% だが*数える*便＝書く量が最も多い）で、
**直近 6 便すべてが起票に無い発見を出している**（450 上・451 下・454 上・455 直し方・456 18.5 倍）
＝**「短く済むほう」に賭ける根拠が無い**。★ **静かな窓も約 1 時間使った。ここで閉じる。**
...
★★ **⑻ 途中でユーザーが push した。** `-Start` 時点の未 push 25 は消え、**`origin/master` は
`b22561d0`**（第454 の最後の docs）を指している＝この便の分だけが ahead。

★ **⑼ 終了時**: commit 3 本（code `329e16b7`＝Core 8 ファイル＋新規 `Svg/Layout/OneOrMany.cs`＋
`LilySharp.Tests/TabOnlyMeterColumnTests.cs`＋再生成した `APPROXIMATIONS.md`／`magic_constants.csv`、
docs `0c31ac70`＝§1 と §1.0 と RULES 5 本、**最後の docs＝この文を含むので SHA は書かない**＝§5.4）。
**最終 full 8780 / 0 / 3 / 8783**（`sessions/p455/run3.trx`＝docs を閉じた*あと*に回した）・§7.5（対 `b22561d0`）**Core '+' 284 行／
REF 0／OWN 0**＝⚠️ **数値を含む追加行 7 本はすべてコメント内の実測値**で、式も定数も 0（§7.6 ⒟。
`APPROXIMATIONS.md`・`magic_constants.csv` は**行番号だけ**が動き、**毒の前に `LILYSHARP_UPDATE_DOCS=1`
で再生成した**＝第449 の罠を踏んでいない。⚠️ **どちらも生成時に LF になるので CRLF に入れ直した**）・
**§7.7 の匂いは 1 つ**＝`ItemColumn.Append` の「外から来た list は*写して*から足す」枝は**今は呼び手が
居ない**（`Append` を受けるのは `BuildTimingColumns` が `default` から育てる列だけ）。**消さないのは、
消すと呼び手の list を黙って伸ばす道が開くから**——観測者が無いことは註に書いた。
台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・未追跡 0・作業ツリー 0。
全文は Lab `sessions/p455/`（`prediction.txt`＝会計と方向・`site-prices.txt`／`-before.txt`＝回し直した
census・`instrument.ps1`＋`Zz455.template.cs`・`poisons.ps1`／`poisons.txt`＝毒 11・`Zz455Count.cs.txt`＋
`counts-corpus.txt`／`counts-suite.txt`＝⑸ の計器・`green-to-corpus.ps1`／`.txt`＝毒 10 を実コーパスに・
`Zz455Ab.cs.txt`／`ab-*.txt`・`Zz455Hash.cs.txt`／`hashes-after.txt`・`SlurBucketCoverageTests.cs.txt`＝
取り下げた網・`p455-core.patch`）。**push はユーザー**（Lab も）。**`-Start p456` の 1 コマンドから入る**。

★★ **⑽ 「有利なら次便ぶんも着手して」には着手しなかった＝第453 ⑽・第454 ⑼ と同じ理由で、材料ではなく口座。**
判断時点で **§1 現在便 18,004 / 20,000＝残り 1,996 字**、**HANDOFF 442,508 / 450,000＝残り 7,492 B**。
次便ぶんの語りは 4,700〜4,900 字（本便 4,728・第454 4,925）＝**和文 3 B/字でファイル天井のほうが先に当たる**
（7,492 B ≒ 2,400 字）。⇒ **測っても書けない。** この文脈の利点も 0——脚は Lab に commit 済み・§1.0 は
`-Start` が逐語で刷る。⚠️ **唯一「今が安い」に見えた回し直した census も、⒮⁸ ⑴ は島に手を入れるので
どのみち回し直しが要る**（§1.0 にそう書いた）。
★ **⚠️ 第454 と違い、§7 3 も回していない——今回は詰まっていないから。** `-Start p456` が第454 の
ブロックを ARCHIVE へ出すと（前便の実績で 95 行 5,431 字）、§1 現在便は **§1.0 13,276 ＋ 語り**に戻り、
**語りに 6,724 字**（本便が使った 4,728 より多い）・ファイル天井も約 19,500 B まで戻る。
⇒ **次便は §1.0 を削らずに 1 便まるごと書ける。削るのは、届かなくなってからでよい。**
