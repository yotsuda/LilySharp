...
⇒ ★★★ **⑹ 次の一手**（第 1 便時点の記録。第 2 便は下 ⑻＝自分の穴を塞いだ）: **⒜ `layout {}` の候補 5 つは全部出荷した**（第370 ⑴ の ①〜⑤）＝この道は閉じた。**次に `layout {}` へ足す語が要るなら、まず「表示専用・score 全体・単位なしの閉じた語彙・grob 単位でない」の 4 条件に当ててから**（§3 の行）。**⒝ 和音記号の上付き＋三角**（上 ⑸ の起票）＝設計級・LP の `make-super-markup` と `\triangle` の移植。**⒞ 第369 ⑸⒝ の並びはそのまま**（tab の wish の merge＝設計級・`lily/spacing-spanner.cc generate_springs` の wish の本数を読むところから）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。**push はユーザー**。
