...
終了時: **commit 4 本**（**本体 `49886b42` ／ 緑歩き `c3451bda` ／ docs `243d83d8` ／ ⑽ の書庫移し 1 本**）＝**未 push 10**・木 0・未追跡 0。**suite 6637 合格 / 0 失敗 / 4 skip / 合計 6641**（開始 6610 ではなく **6617** から **+24**＝**本便の網 24 ケース**）。Core 0 エラー 0 警告。**追跡 `.lys` 581 冊・snapshot 231 枚**（**書き換え 0・追加 0・削除 0**）・**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2／`OPEN:` 0＝すべて不動。**
⚠️⚠️ ★★★ **⑽ WSL の ubuntu 脚が 1 本赤くした＝`HandoffArchiveContinuityTests.TheArchiveResumesWhereTheHandoffStops`**——**第296 の経緯を §1 から落としたまま `HANDOFF-ARCHIVE.md` へ移していなかった**（終了時チェックリスト 3.5）。**移して緑**（43 行・逐語・書庫の先頭）。⇒ ★★★ **Windows が緑だったのは*嘘ではなく古かった*から**——**最後の全数を回したのは §1 を書き換える*前*で、この網が見るのは §1 そのもの**。⇒ ★★ **§1 と §2 を書き換えたら、全数をもう一度回す**。**docs は「テストの外」ではない**（この repo は §1 の形・`APPROXIMATIONS.md` の行番号・`docs/*.md` の例をテストで縛っている）。★ **ubuntu 脚が先に鳴ったのは偶然**——**Windows で回し直しても同じ 1 本が鳴る**（回して確かめた）。
