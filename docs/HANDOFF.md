...
最終更新: 2026-07-22 / master `075277ff` の次（§0 で裏取りすること）
...
### ▶ 次のセッションの最初の一手 ＝ **まだ 1 点も無い領域に点を開く（ビーム or スラー/タイ）**
...
### ✅ このセッション（2026-07-22）でやったこと — **タプレット括弧に部屋を与えた**（`caa0f239`）

| commit | 内容 |
|---|---|
| `ba892daf` | **赤で始まっていた HEAD を戻した**。`26afa9fe` が**同一コミット内で**フィクスチャのコメントを +155 文字書き換えながら、snapshot を**その編集前**に再ベースしていた。`data-pos` だけが 155 ずれた状態がコミットされていた |
| `a5c0b6a8` | **タプレット括弧に届く点を 2 つ起票**（probe TU/TD）。出力不変。**欠陥 2 件を検出**（予約漏れ・幻の符尾） |
| `caa0f239` | **括弧を譜間スカイラインへ＋幻の符尾を止めた**。`−1.700225 → −0.520225`（2 点とも）。**snapshot 9 件再ベース**（分類・承認済）＋**新規フィクスチャ 1 件** |
| `bc756fd1` | §1 書き換え＋**逆算による分解の訂正** |
| `5271df10` | **ページ側に届く点を 2 つ起票**（probe TSU/TSD）。出力不変。**起票中に probe が clef を測っていたのを検出して直した** |
| `075277ff` | **括弧をページスカイラインへ＋prelim パスの声部/譜スコープ漏れ**。`−1.295225 → −0.520225`（2 点とも）。**snapshot 14 件再ベース**（分類・承認済） |

**テスト 3172 passed / 0 failed / 3 skipped。** Core 0 warn / 0 err。
**LP 忠実度 28/40 exact, total |residual| = 2.104753 ss over 36 distances ＋ counts 4/4。**
⚠️ **ss 合計の増加は悪化ではない**——タプレット 4 点が誰も測っていなかった乖離を可視化した分
（起票時 −1.700225 ×2・−1.295225 ×2 → 現在 **−0.520225 ×4**）。§5.2.1④。

#### ★ 4 点とも、起票時の予測も実装後の予測も桁まで的中した

| 点 | 起票 | 実装後 |
|---|---|---|
| `staff.staff.tuplet-bracket-{up,down}` | −1.700225（2 点同値） | **−0.520225**（2 点同値） |
| `system.tuplet-bracket-{up,down}` | −1.295225（2 点同値） | **−0.520225**（2 点同値） |
...
| シード＋符尾ガード後 | −0.520225 | **−0.520225000**（TU/TD 同値） |
...
**origin より 88 ahead で未 push**（HEAD = `075277ff`）。push はユーザー判断・コミットは可。
⚠️ **push は明示的に「まだしないで」と言われている**（2026-07-22）。解除まで push しないこと。
⚠️ **未 push にはフォント差し替えと紙面定数、snapshot 再ベース 14 回**
（186・192・2・2・80・82・3・14 件 ＋ `programmatic/hara-kiri-paged` 1 件 ＋ `test/tab-percent-repeat` 1 件
＋ 強弱 **16 件** ＋ `test/above-dynamics` の data-pos 1 件 ＋ タプレット **9 件** ＋ ページ側 **14 件**）
**が含まれる。**
別ブランチ `fix/vscode-extension`（`7291531a` から）と `fix/note-bang-diagnostic` は
**どちらも master に取り込み済み**（ユーザー）。**走っている別ブランチはもう無い。**
**テスト 0 failed / 3172 passed / 3 skipped。** Core build 0 warn / 0 err。
**LP 忠実度 28/40 exact, total |residual| = 2.104753 ss over 36 distances ＋ counts 4/4**
（**2.26.0 基準**。X 22点＋Y 7点＋譜間 **5点**＋**system 間タプレット 2点**＋**ページ本数 4点**）。
⚠️ **本数の点は距離ではないので ss の総和に入れない**（`unit` フィールドで分離。`page.height` を
台帳から落としたのと同じ理由——1 system を 2.104753 ss に足すと指標が意味を失う）。
⚠️ **ss 合計が 0.023853 → 2.104753 に増えたのは悪化ではない**——タプレットの 4 点が
**誰も測っていなかった乖離**を可視化した分（起票時 −1.700225 ×2・−1.295225 ×2 → 現在 −0.520225 ×4）。
§5.2.1④ と同じ形。
**タプレット 4 点は全て −0.520225**（`caa0f239` `075277ff`。TupletNumber のテキストインク＝
LP の斜体 font-size −2 を通常テキストフォントで組む経路で、TimeSignature の −0.004735 と同族。
**閉じる予定の無い名前付き残差**）。
...
**現状 28/40 exact, total |residual| = 2.104753 ss over 36 distances ＋ counts 4/4**
...
**タプレットの 4 点は全て −0.520225**（`caa0f239` `075277ff`。TupletNumber のテキストインク）、
...
⚠️ **ss 合計を過去の値と直接比べないこと**（点集合が違う）。0.023853 → 2.104753 は悪化ではなく、
タプレットの 4 点が**誰も測っていなかった乖離を可視化した**分。§5.2.1④。
...
⚠️ **未測定の疑い（新規・最大）**: **スラー/タイ・ビーム・臨時記号配置には台帳の点が 1 つも無い。**
タプレットで点を開いたら**欠陥が 4 つ**出た（うち 1 つは snapshot に構造的に現れない種類だった）。
着手手順は §1 の「次の一手」。**必ず対で作ること。**
