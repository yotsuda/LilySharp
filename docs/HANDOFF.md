...
最終更新 第282セッション＝**引き継いだ「全緑」が実は赤で、それを直してから LP 第 3 分岐を*測ってから*半分だけ移植し、その最初の試作が自分の小節を白紙にしたことで 4 便前から在った別の欠陥が出た（1 便）**——commit は `85406c45`（**ハンドオフの最後の 1 本が suite を赤にしていた**）／`85406c45`（**拍スラッシュと、「percent がどの小節を隠すか」を訊いていなかった 4 人目の読み手**）。**snapshot 0 枚・台帳不動・origin 不動**。

**⑴ 引き継いだ「6437 全緑」は赤だった**（`85406c45`）。木のままの実測は **6436 / 1 失敗**——`HistoryCitationTests.DeadCitationsDoNotGrow` が **470（天井 469）**。⚠️ **超えた 1 個は第281 自身のハンドオフ commit `85406c45` が入れた**。**suite を回したあとに最後の 1 本を積んだので、その 1 本は誰にも読まれていない。** ⇒ ★★ **セッション最後の commit は suite の外に居る。** **中身は「amend が孤児にした commit の SHA を §1 に書いた」**——`git rev-parse` は答えるが branch から到達不能＝**dead citation の定義そのもの**。★ **同じ形をこの天井が捕まえるのは 2 度目**で、**テスト自身の remark にそう書いてある**（「その §1 は amend が孤児にした 2 つを『どこにも引用されていない』と、両方を引用する文の中で書いた」）。⇒ **直すのは天井ではなく散文**（remark の指示どおり）。**孤児の名前は書いた瞬間に dead citation。** 必要なのは SHA ではなく **amend の回数**で、それは reflog が持っている。
