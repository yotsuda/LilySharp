...
最終更新 第363セッション（2026-09-10）＝**入り方はユーザー起票 3 つ**（⑴「`scratch\ベースタブLy\pageBreak.lys` が LP 双子と同じか。タイトルと五線の間が広すぎる」→ ⑵「`raggedBottom` を足して。`lysc ly` も直して」→ ⑶「`raggedRight`／`raggedBottom` を VS Code の補完に。ほかに補完に出していない機能が無いか監査」）。**⑴ は欠陥ではなかった**（下 ⑴）。**製品＋番人＋CHANGELOG＋この HANDOFF を 1 commit（snapshot 0＝承認ゲート無し）・push はユーザー**。**骨は 6**:

★★★ **⑴ 起票 ⑴ の答え＝双子と exact・広いのは LP の既定**。`lysc ly` → LP 2.26.0（svg＋cairo png・`scratch/p364/`）: **p1 五線上端 LP 91.1181／Lily# 91.12・p2 23.5307／23.53＝exact**（title baseline だけ 21.284 対 21.49＝**+0.21 は元から在る title の残差**・間隔の印象には効かない）。理由: `pageBreak` で 2 頁の本になり、**p1 は最終頁でないので `ragged-bottom = ##f`（LP 既定）が 1 段を頁の底まで伸ばす**（`PageLayouter.cs:415` の 4 通りは移植済み）。**LP の答えは `\paper { ragged-bottom = ##t }` で、Lily# は `LayoutOptions.PageBreaking.RaggedBottom` を持ちながら `.lys` の綴りが無かった**（`paper { }` の bare flag は `raggedRight` だけ）。

★★★ **⑵ `paper { raggedBottom }`（起票 ⑵ 前半）**: `PaperPlanReader.FlagKeys = [raggedRight, raggedBottom]`（`Canonical` で引く・`AllKeySpellings` は `size, ScalarKeys, FlagKeys, SpecKeys`）→ `options.PageBreaking with { RaggedBottom = true }`（REF 1＝`ly/paper-defaults-init.ly` ragged-bottom）。**`raggedLastBottom` は足していない**（LP 既定が true＝bare flag では既定の再宣言にしかならず、OFF には値付きの綴りが要る＝remark に理由）。`LanguageVocabulary.PaperFlagKeys` を公開し、**補完・色付けテストがそれを読む**。文法書 3 冊（GRAMMAR §2.5・SYNTAX_REFERENCE・GRAMMAR_FOR_LLM）・tmLanguage。番人 `PaperBlockTests` +3（flag が breaker に届く・`raggedBottom 1` は LYS9003・**pageBreak.lys の形で p1 の五線が 91 → 16 に上がる end-to-end**）。**対**: raggedBottom の p1 五線上端 **Lily# 16.27 対 LP（`ragged-bottom = ##t` を手で挿した双子）16.043＝+0.23＝⑴ の title 残差 +0.21 と同じ島**（この便は追わない）。**双子は `ragged-bottom` を書かない**（`raggedRight` も書いていない前例に揃えた）。

★★★★ **⑶ `lysc ly` の form-level `:|`（起票 ⑵ 後半）＝病名は「読み手 4 軒で 3 通り」**。`form main { A :|: B :| }` を **頁は A `:|`・`|:` B `:|`（A A B B）・双子は A を素で B だけ `\repeat volta 2`（A B B）・MIDI と XML は末尾の `:|` だけ巻き戻し（A B A B）**。⚠️ **top-level の `:|:` は `FormWalk.Other`（BarlineSyntax RepeatBothBar・分類されない）**で、双子は `EmitMusicStream` がそこで repeat を*開く*だけ＝`:|` 半分が消えていた。MIDI の `PlayFormItem` は Other を無視、XML も同じ（`RepeatBackward` は lone `:|` だけ）。**この便は双子だけ直した**（起票の射程）。**形**: `EmitMusicStream` 先頭で `LastRewind(items)`（depth 走査＝collector の対付けと同じ: `|:` +1・`:|` は開いていれば閉じ、さもなくば**巻き戻し**・`:|:` は depth 0 なら巻き戻し＋開く・**最後の巻き戻しが最外の body**）→ `EmitRewindRepeat`＝手前を `\repeat volta N { … }`（N＝`:|*N`）に包んで再帰・`:|:` なら残りの先頭に合成 `|:`。**`printInitialRepeatBar` は「書かれた `|:` で開く本」だけ `##t`**（`_rewindOpensThePiece`＝巻き戻しの body には `|:` が無く頁も描かない・`form { |: A :| B :| }` は `##t` のまま）。chord track は `:|:` だけ通す（`||`／`|.` を通すと `\chordmode` に `\bar` が出て 2 冊が動く＝掃きで戻した）。**2 重巻き戻し（`A :| B :|`）は入れ子になり warning**（LP は内側を外側の pass でもう一度弾く・Lily# は書かれた区間を 1 回＝`RepeatFromTheBeginning`）。**LP 実測**: 新双子は `\repeat volta 2 { A } \repeat volta 2 { B }`・`##f` → **p1 A の後ろに `:|`・p2 `|:` B `:|`＝頁と同じ絵**（`scratch/p364/lp2-page*.png`）。EmitBarline の lone-`:|` warning は「手前に何も無い `:|`」だけに残る。

★★★ **⑷ 掃き 925 冊（base 81f43f3c Release＝`scratch\p364\wt-base`・`sweep364.ps1 -Kind ly`）が第 2 の欠陥を拾った**: 第 1 カット MOVED 6＝予測 4（voltasky・Danger Zone・Disco Inferno・pageBreak）＋**volta-chord-row／Lambada Complicada（chord track に `\bar "|."`＝上の chord track 通し過ぎ・戻した）**。そして **voltasky／Danger Zone／Disco Inferno は `|: A [1. B] :| [2. C] :| [3. D]` の「ending の間の `:|`」（parser の furtherAlternatives）を巻き戻しと読んで本 1 冊を丸ごと包んでいた**——base はそれを `\bar ":|."` の stray として `\alternative` の後ろに書き、3 番目の ending を repeat の外に置いていた＝**どちらも頁（ending 2 を閉じて 3 を括る）と違う**。**形**: `IsEndingDivider`（`:|` の直後が `InlineVoltaSyntax`）を `EmitInlineRepeat` の closed 走査が吸収し `LastRewind` も巻き戻しに数えない → `\repeat volta 3 { } \alternative { {1}{2}{3} }`。**第 2 カットの MOVED は下 ⑸ に写す**。番人 `FormRepeatBarlineTests`: 旧「twin は弾けないと言う」を撤去し **+6**（巻き戻し＝`\repeat`＋`##f`・`:|*3`・`:|:` 2 repeat・書かれた `|:` は `##t`・2 重は warning・chord track 同形）＋ **divider 1 本**。

★★ **⑸ 補完（起票 ⑶）**: **`raggedRight` は最初から `PaperBlock` 文脈で出ていた**（`GetPaperBlockCompletions` の手書き 1 項目）——**それが唯一の手書きで、reader の表を読んでいなかった**ので `raggedBottom` は届かなかった。`LanguageVocabulary.PaperFlagKeys` から生成（`PaperKeyDetail` に 2 行）。番人 `PaperBlockCompletionTests`（新規 +8: 6 caret が PaperBlock・**`PaperPlanReader.AllKeySpellings()` 全部が label に在る**・flag 2 つは Keyword で InsertText 無し）。**監査（Explore agent・`CompletionVocabulary.cs` 対 GRAMMAR／Lexer／reader 表）の拾い箱は ⑺ ⒝**。

★★★ **⑹ 数と射程**: **開始時**（`Session-Check -Test -Scratch p364 -DiffBase 81f43f3c`＝起票再現を先にしたので `-Build` は回していない・solution build 0 エラー）: HEAD **`81f43f3c`**・未 push 12・`origin/master`＝`ba70753f`・木 clean・未追跡 0・`gh run list` **赤**（34416221197＝第361 ⑷⒝ の赤・以後 push 無し）・台帳 816・snapshot 249・追跡 `.lys` 599・ディスク 20303（p364 staged 込み）。**full `scratch/p364/run1.trx` 7609 / 1 / 4 / 7614（+16＝番人・赤 1＝`ApproximationInventoryTests.TheInventoryIsNotStale`＝`LILYSHARP_UPDATE_DOCS=1` で再生成・APPROXIMATIONS は行番号だけ）→ 再生成＋⑷ の第 2 カット＋この HANDOFF の後に full `run2.trx`＝下 ⑻**。§7.5（作業ツリー・`git diff -- LilySharp.Core`）**Core `+` 207／REF 1／OWN 0**（REF＝ragged-bottom・双子の変更は LP の式ではなく双子自身の構造＝式は 1 本も足していない）。台帳 816／snapshot 249／追跡 `.lys` 599 不動。CHANGELOG 3 節（Language・Editor・MIDI/XML/twin）。⚠️ `git worktree`: `scratch\p364\wt-base`（81f43f3c）を足したまま・p362／p363 のも残っている（`git worktree remove` で外す）。⚠️ scratch は **`p364`**（p363 は第362）。⚠️ §7 3.5: 第361 のブロックを `-Archive 361` で ARCHIVE の先頭へ（済）。⚠️ 記憶: `reference_lilysharp_form_level_repeat_both_four_readers`。

✅ ★★★★ **第 2 便（ユーザー「このセッションでやる方が有利なら着手」→ ⒝① は p364 の base worktree・staged 925 冊・掃き・FormWalk の把握がそのまま効くので着手・②〜④は道具が残らないので見送り）＝MIDI／XML の form-level `:|:` を頁に揃えた**。**ユーザー決定は要らなかった**: 文法（GRAMMAR §8・`RepeatPairingValidator` の remark）が「`:|:` は `:|` then `|:`・lone `:|` は巻き戻し・`|:` は次の `:|` が閉じる」と既に言っており、MIDI／XML はそれを読み落としていただけ。**形＝`FormWalk.GroupDividerRepeats`**（読み手 3 軒の唯一の入口＝1 綴り）: top-level の `:|:`（`Classify` で `DividerBar`＝内部型）を **`LoneRepeatEnd`（合成 `:|` node・divider の position）＋`Repeat(Node: null, [RepeatStart(divider token) … RepeatEnd(閉じる `:|` の token)])`** に読み替え・閉じる `:|` の `*N` が count・後続の `[2. Y]`／`:| [3. Z]` は block の子・**2 本目の `:|:` は閉じて開くだけ（巻き戻さない）**・閉じない `:|:` は RepeatEnd 無し（LYS4017 の頁の判断に任せる）。`Repeat.Node` を nullable に（消費者は誰も読まない＝grep）。双子の chord track の Other arm は breaks だけに戻した（divider は届かない）。**対**: MIDI `A :|: B :|` 60 60 62 62（前 60 62 60 62）・`A :|: B :|: C :|` 60 60 62 62 64 64・`:|*3` 62×3／XML bar 1 backward・bar 2 forward＋backward／双子は不変。**掃き 925 冊（base 81f43f3c）: midi MOVED 1・xml MOVED 1＝起票本だけ（top-level `:|:` を書く本は 925 冊中それだけ＝陽性対照）・ly head→head2 MOVED 0**。番人 `FormRepeatBarlineTests` +3（MIDI 3 形・XML・reader の形＝`Assert.Collection`）。CHANGELOG（MIDI/XML/twin）。§7.5（作業ツリー・第 1 commit から）Core `+` FormWalk 約 100／REF 0／OWN 0（式は無い・分類の話）。⚠️ **MIDI の `PlayRepeatBlock` は block 内の `BothBar` を読まない**（`|: B :|: C :|` を body 丸ごと N 回＝B C B C・XML は `EmitPlainRepeatBlock` が run を割る）＝既存・未計数→ ⑺⒝⑤。

✅ ★★ **第 3 便（同じ指示の 3 度目→ ⒝⑤ は base の midi 925 冊が描いてあり FormWalk／MIDI が温かいので着手・②〜④は見送り）＝MIDI の `PlayRepeatBlock` が block 内の `:|:` で run を割るように**。`|: B :|: C :|` を頁・双子・XML は `|: B :| |: C :|`（B B C C）、MIDI だけ body 丸ごと N 回（B C B C）。**形**: children を `BothBar` で run に割り `PlayRepeatRun`（block の `:|*N` は全 run に＝双子が各 run の閉じに書くのと同じ・ending は直前の run のもの）。**対**: `|: A :|: B :|` 60 60 62 62・`:|*3` 3＋3・`|: A :|: B [1. C] :| [2. D]` A A B C B D。**観測本 0**（925 冊中 block 内 `:|:` を書く本は無い＝掃き midi head→head2 MOVED 0・陽性対照は番人 `FormRepeatBarlineTests` +1）。CHANGELOG（MIDI/XML/twin）。**第 4 便は着手しない**: 残り ②〜④はこの便の道具が効かない。

⇒ ★★★ **⑺ 次の一手**: **⒜ push はユーザー**。**⒝ 拾い箱（この便が測って名指しただけ・未起票）**: ①~~MIDI と XML の form-level `:|:`~~＝第 2 便で閉じた。②★★ **補完の未露出（Explore 監査・要検証のものは `?`）**: TopLevel に `using "…"`・`drummap { }`・`transpose PITCH`／AfterTime に `time none`／AfterTempo に `shuffle`（`swing` だけ）／MusicBlock に `cue`・nav mark（segno/coda/fine/dc/ds＝form 文脈にしか無い・`GetArticulationCompletions` の remark は music にあると言う）・phrase 参照名・`q`／`voice { } { }`（section 本文）／SectionBlock に `lyrics NAME`・`chords NAME`／ScoreBlock に素の part 名（MIDI-only）・`staff CLEF PART`／`tab TUNING PART` の clef／tuning／`score X transpose P`／`@arpeggio(bracket)`・`@8va` 族・`@feather(rit|accel)`・`@bend(N)`／chord quality（`ChordStructure` 35 種のうち補完は 7 度×4 形）／lyrics 行の `[N. …]`。**手書き写しの表（drift 元）**: `size` 項目・`KeyModes`（Lexer の 9 語）・`GetKeyTonicCompletions` 15 語（文法は任意の PitchBase）・`GetScoreBlockCompletions`・`SectionHeaderDirectiveItems`・`RenderedGrobProperties`（`SupportedGrobOverrides` の写し）・`GetArticulationCompletions` 100+ 語（`ArticulationRegistry` と同期テスト有り・mark alias 4 つは外）。③ ⑵ の title 残差 +0.21（LP 21.284 対 21.49・raggedBottom の p1 五線 +0.23 と同じ島）。④`raggedLastBottom`（OFF にする綴り＝値付き flag の形が要る）。⑤~~MIDI の `PlayRepeatBlock` が block 内の `BothBar` を読まない~~＝第 3 便で閉じた（観測本 0＝番人だけ）。**⒞ 第362 ⑹⒝②〜④（rit-across-systems の fixture・XML の unfold／R1*N・双子の in-music `repeat volta`）と 第361 ⑺ の並びはそのまま**。

**⑻ 終了時**: 第 1 便＝full `scratch/p364/run2.trx` 7610 / 1 / 4 / 7615（+1＝divider の番人・赤 1＝APPROXIMATIONS が ⑷ の第 2 カットで再び stale）→ 再生成後 **full `run3.trx` 7611 / 0 / 4 / 7615（RunInfos 無し）**→ commit **`9b64457b`**（Core 3＋Lsp 1＋Tests 4＋docs 5＋tmLanguage＋CHANGELOG＋この HANDOFF）→ `Deploy-Lsp.ps1`（server＋syntaxes）。**第 2 便＝full `run4.trx` 7613 / 1 / 4 / 7618（+3・赤 1＝APPROXIMATIONS がまた stale＝Core を触った*最後*に再生成すること）→ 再生成後 full `run5.trx` 7614 / 0 / 4 / 7618（RunInfos 無し）**→ commit **`77239c5c`**（Core 2＋Tests 1＋CHANGELOG＋APPROXIMATIONS＋この HANDOFF）→ `Deploy-Lsp.ps1`。**第 3 便＝full `run6.trx` 7615 / 0 / 4 / 7619（+1・RunInfos 無し・APPROXIMATIONS は full の前に再生成）**→ commit（Core 1＋Tests 1＋CHANGELOG＋APPROXIMATIONS＋この HANDOFF）→ `Deploy-Lsp.ps1`（server だけ＝Reload 1 回）。push はユーザー（未 push 15）。
