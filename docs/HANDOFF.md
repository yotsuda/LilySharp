...
最終更新 第215セッション＝**ユーザーの「残債の返済を中断して、見えているリリースブロッカーを直す」で、門の 2 択を両方閉じた便**（**実装 1 便＋決定 1 件**）。
⚠️ **彫版は動いていない**——**動いたのは SVG の family 属性ただ 1 つ**（snapshot 11 枚が
その 1 属性で動き、枚数 218 で不変・台帳 529 点・総和 3.644109 で不動）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: ユーザーの「私に見えているリリースブロッカー」は
**§1 ⑷ の出す前候補 2 つ＝⒠・⒦ そのもの**（⒦ の SAC の壁は第212 でユーザー自身が踏んだ・
CHANGELOG にも明記済み）。**⒠ は委任がそのまま「出力についての決定」を与える**ので自律で閉じ、
**⒦ は判断ではなくユーザーの資産（証明書・本人確認）が要る**ので、選択肢を 3 つに絞って
訊いた——**ユーザー決定＝未署名のまま出す**（§3 の 1 行目へ）。

★ **開始時裏取り（§0 を通しで）**: HEAD `45f9cf10`・**未 push 5**・未追跡 0・作業ツリー項目 0・
**Core 0 警告**・suite **5596 passed / 0 failed / 4 skipped**・`gh run list` 直近 2 本 success
＝**引継ぎの数は全部合っていた**。台帳は便中に取得: **529 点・exact 454・
total |residual| 3.644109 ss・count 110/112 一致**（レポートの印字のまま）・**snapshot 218 枚**
＝第214 終了時と同値。
終了時 **未追跡 0・作業ツリー項目 0・未 push 7**（**この行を書く commit を含めて 7**。
**数えたのは commit の*あと***）・suite **Windows 5596 / 0 / 4**（doc commit 後にも取り直して同数）・
**Linux（この機械の WSL）5596 / 0 / 4**（`f1676e5e` で実測。以後の commit は doc のみ）
——**両 OS 完全緑を維持**・**snapshot 218 枚**（11 枚動いた・枚数不変）・**Core 0 警告**。

★ **この便の値段**:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ ⒠ SVG の sans が名指しになった | Core 1・snapshot 11 | **SVG の和音記号の属性のみ**（彫版 0・幾何 0） |
| ⑵ ⒦ の決定＝未署名のまま出す | §3 に 1 行 | **リリースの門のみ**（コード 0） |

- **⑴ `f1676e5e`＝⒠ を閉じた。** **`SvgDrawingContext.FamilyAttributeFor` が
  `"TeX Gyre Heros, sans-serif"` を出す**——**root が serif に与えているのと同じ
  「名指しが先・generic がフォールバック」の形**（SvgDocumentContext.WriteHeader と対）。
  読み手が Heros を持てば予約どおりに描かれ、持たなければ自分の sans に劣化する
  （従来の裸 `sans-serif` が言えたのは元からそこまで）。
  - ★★ **動きが「それだけ」であることを機械に言わせた**: 動いた 11 枚の diff を
    両綴りの正規化後に取り直すと**残りが空**＝x/y/font-size はバイト同一。
    **構造の主張も添える**——この属性は SVG 出力段でしか読まれず、layout は見ない
    （だから 567 冊の幾何 A/B は回していない）。
  - **perf は計算 0**——`const string + literal` は C# がコンパイル時に畳む
    （§7.9 の「足していない例」・commit に明記）。
  - **7.6 の分類は ⒟**（既存の家 `TextFontMetrics.SansFamily` への指し直し・新 REF 0 本）。
- **⑵ ⒦＝コードでは閉じられないので、決定として閉じた**（§3 の 1 行目）。
  この機械に code-signing 証明書は 0 枚（`Cert:` を実測）・`release.yml` に署名は 1 行も無い・
  どの経路（Azure Trusted Signing／SignPath OSS／OV 証明書）も**ユーザーの資産＋本人確認で
  日〜週単位**——ので**「未署名のまま 0.3.0 を出す」をユーザーが決定**。
  **CHANGELOG の Known limitations の行は残す**（第214 ⑶ 宿題 ⒝ の答え＝「消さない」）。
- **⑶ ★★★ 次に触るならここ**——**⓪ タグを打つ。門は全部開いた**
  （push 済みにする → CI 緑を見る → `v0.3.0` を打って push。**`master` を先に**）。
  **宿題 ⒜ は済み**（CHANGELOG 最上段 `## 0.3.0` ＝ `Directory.Build.props` の 0.3.0・今便実測）・
  **宿題 ⒝ は確定**（上の ⑵）。⚠️ **workflow の初実走はタグを打った瞬間**（第214 ⑶ のまま）。
  push とタグはユーザーの手癖に合わせて残してある。
  **タグの後の残債は従来どおり**: **⒝ ペダルの段間隔**（LP 1.961 → 2.443・Lily# 一律 2.46。
  **台帳点が無い＝§5.0 の対の起票から**）／**⒞ `CoPlaceToCodaWithLabels` の鏡像ケース**
  （装置をスタック前へ動かす再設計・専用の便）／**⒟ §2 の頭の ▶ perf**／
  **⒡ 配管の残り 6 site**（`TabStaffGeometry` 3・`FetaTextRun` 3）。

> ## ★★ この便の骨＝**「ユーザー判断待ち」は 1 種類ではなかった**
> **⒠ と ⒦ は 10 便のあいだ同じ棚（判断待ち）に並んでいたが、種類が違った**——
> **⒠ は「決めてくれれば 1 便で閉じるコード」・⒦ は「決めても資産（証明書・本人確認）が
> 無いと閉じないもの」。** 委任が来た瞬間、前者はその日に閉じ、後者は**選択肢を絞った 1 問**に
> 変換したら決定として閉じた（コードは 1 行も動いていない）。
> ⇒ **「ユーザー判断が要る」と起票するときは「判断だけで閉じる」か「ユーザーの資産が要る」かを
> 書き分ける**——委任が来た日に、どちらを手に取りどちらを訊くかが起票を読むだけで決まる。
