...
そこから沈黙と、それを測る計器の欠陥が芋づるで出た便**（**実装 6 便**）。
...
終了時 **実装の最後は `de4acbd0`**・**未 push は
`git rev-list --count origin/master..master` が言う**・**未追跡 0**・
suite **5057 passed / 0 failed / 4 skipped**（+23＝新しい網ちょうど）・
...
- **⑹ ★★★ 第6便＝score の `transpose` が「ファイルの既定」として数えられていたのをやめた**
  ...
  **それがこの欠陥が生き延びていた理由。**
- **⑺ ⚠️ ★★★ この便が*直さずに起票*した 2 件**（どちらも §2F・実測つき）:
...
- **⑻ ★ 毒は 9 方向**（§5.4）。**⑵＝警告だけ落とす→3 本赤**（diamond と cycle は緑のまま
...
- **⑼ ★ §7.5 の監査**＝第2便 Core +95 行・第3便 +16 行・第5便 +119 行・第6便 +25 行、
  **REF 0 / OWN 0・幾何の数値定数 0**（第5便の 1 件は `int collected = 0` ＝ループ計数器）。
...
- **⑽ ★★★ §2F に残っているのは 3 つ＋2**——**⑴ 一覧に構文を与えるか（決定であって作業ではない）
