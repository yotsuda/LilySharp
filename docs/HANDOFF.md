...
最終更新 第289セッション＝**span の終端を言語全体で 1 つの答えに寄せた（3 便）**——**`7b0df578` テキストスパナ（`@!X`）** ＋ **`1115f42e` オッターヴァ（`@!ottava`・`@loco` 退役）とペダルの診断** ＋ **`225ff9ec` ペダルの綴り（`@sustain` … `@!sustain`）**。**これで `@!` は全族に及んだ。** **台帳は 1 点も動いていない**（742／exact 582／ss 非ゼロ 190／総和 25.844537623／count 156 中 2）——**終端が動かしたのは*長さ*であって*高さ*ではない。**

⚠️⚠️ ★★★ **⑴ この便の一番大きい収穫は「§2 が LP について書いていたことが偽だった」こと**。**§2 は「閉じられなかった span に LP は答えを 1 つしか持たない＝warning ＋ `suicide()`」と書いていた**が、**それは `Text_spanner_engraver` *1 つ*から書いた一般化**だった。**実際は**: **`Ottava_spanner_engraver::finalize`（`lily/ottava-engraver.cc:220-226`）は warning も `suicide()` も出さず**、**`Piano_pedal_engraver::finalize`（`:425-443`）は RIGHT 境界を `currentCommandColumn` に置いて typeset する**——**どちらも*曲の最後まで描く*、無言で**。**ソースを読んだうえで LP 2.26.0 で実測もした**（`scratch/p289/lp/openott.ly`: **閉じない `\ottava #1` は 49.98 幅の破線を引き、warning は 0**）。⇒ ★★★ **§5.2.1 の「description ではなく分岐の在るファイルを読む」は*自分が前便で書いた要約*にも効く。** **「LP はこうしている」を引き継ぐときは、その族の engraver を*自分で*開くこと。**

★★★ **⑵ だから「描かない」は*言語の*答えで、LP の移植ではない**——**両方の engraver に `NOT PORTED HERE` を立てて宣言した**（**APPROX 53 → 55**）。**ユーザーは第288 で「決め手は LP 忠実性ではなく言語の一貫性」と決めており、本便もその延長**。⇒ ★★ **LP と違う道を選ぶのは構わないが、*違うと知って*選ぶこと。第288 は「LP と同じ」と思って決めていた。**

⚠️⚠️ ★★★ **⑶ プローブが私の設計ミスを捕まえた**。**オッターヴァにテキストスパナの第 3 の欠陥（「開いている最中の START は拒否」）を*類推で*与えた**が、**LP はそうしない**——**`process_music`（`:122-136`）は*どの* ottava イベントでも開いている span を閉じ、`\ottava #0` でなければ新しく開く**ので、**`8va` から `8vb` への直結は「2 本の括弧」**。**`audit/lpreg/ottcons.lys`（LP の `ottava-consecutive.ly` の双子・「連続 ottava は結合されない」を主張する本）が全数掃きで動いて発覚**。⇒ ★★★ **「同じ形だから同じ規則」は族をまたぐと外れる**——**テキストスパナは*入れ子を拒否*し、オッターヴァは*状態を変更*し、ペダルは*踏み直す*。3 つとも「開いている最中の START」だが、答えは 3 つとも違う。**

★★ **⑷ `@loco` は「刷る語」ではなかった**（ユーザーの前提を実測で訂正した）。**`@loco` を書いた本の SVG に `loco` の text 要素は 1 つも無い**——**変わるのは括弧の右端だけ**（`@loco` 有り 12.01→20.36／無し 12.01→33.64）。**`GetMarkText(Loco) => "loco"` は在ったが `IsSpannerHandled` が `Loco` を含むので誰も描かない死んだ綴り**。**LP にも `loco` は無い**（**2.26.0 の `ly/` `scm/` `lily/` で当たるのは C++ コメントの `in loco` 1 件と drum の `loconga` だけ・music glossary に*用語として*だけ載る**）。⇒ **ユーザーの条件「LP に loco という綴りがあるなら糖衣が欲しい」は満たされないので、綴りは `@!ottava` 1 つ。**

★★ **⑸ 診断は族ごとに違う言葉で、規則は 1 つ**（`SpanPairingValidator` が `SpanKind` × `SpanPairingFault` を読む・**族は 3 つ・LYS4018 は 1 本**）。**2 本目のコードは 2 つ目の答えが漂流する場所になる。** ⚠️ **失う物が族ごとに違うので、そこは言い分ける**——**テキストスパナは*語*を線と一緒に失い、オッターヴァは*移調*を括弧と一緒に失う**（後者は紙を見ても分からない）。

⚠️ ★★ **⑹ ペダルは「インクを動かさず、黙っていたのをやめた」だけ**。**閉じられない `@sustainOn` は*前から*何も描いていない**（実測 `scratch/p289/ped.lys` 対 `pedopen.lys`——**閉じれば括弧＋両端のかぎ爪、閉じなければ何も無い**）。**診断を足した便ではペダルの本は 1 冊も動いていない**——**動いたのは*綴りを寄せた 3 便目*で、そこでも動いたのは「本文を書き換えた 15 冊」だけ**（**追跡 6 冊の snapshot は `data-pos` のみの差・ペダルの台帳点は全部記録値のまま**）。⇒ ★★ **綴りの移行はインクを動かさない**——**それを*言える*のは、snapshot を「`data-pos` 以外 0 行」で確かめ、台帳が通ったから。** ⚠️ **ペダルの対応付けには譜／声部のフィルタが無い**（**別の譜の `@sustainOff` が閉じてしまう**）——**直すとインクが動くので本便では触っていない**。**実測: ディスク上でペダルを書く本は 15 冊、うち複数譜の score は 1 冊（`showcase/03-piano`）で、そこでも全ペダルが同じ part に在る**＝**その配置に届く本は 0 冊。**

★★ **⑺ 掃きの計器を 2 度直した**。**⒜ 並列版を*自己検査*した**（**base と head に同じ exe を渡すと、並列化が持ち込みうる唯一の欠陥＝ワーカーが互いの一時ファイルを潰して*他の本のページを読む*が MOVED として出る**）——**1582/1582 SAME・MOVED 0**。**⒝ レンダー失敗に 1 回のリトライを入れた**——**24 ワーカーだと書き込みが稀に負け**、**単体では exit 0 で描ける本が NO-OUTPUT と出た**（実際に 1 度出た）。**「head の engine がこの本を描けない」という*偽の*警報は 1 セッションを潰す**。**本物の失敗はリトライでも失敗する。** **⒞ base の exe が無いとき投げるようにした**——**worktree を消したあとに回したら 「TOTAL 0 / SAME 0 / MOVED 0」を返した**。**それは*1 冊も動いていない*と読めるが、実際には 1588 冊を 1 冊も描いていない**。⇒ ★★★ **偽の警報は 1 時間を潰し、偽の全クリアは欠陥を出荷する。**
⇒ ★★ **速くした計器は、速くしたことで増える失敗の型を先に塞ぐこと**——**そして「何も起きなかった」と「何もしなかった」を区別できない計器は、計器ではない。**

⚠️ **見つけて直していないもの**（前便から引き継いだ分に加えて）:
- **★★ スパナの*左端*が小節の頭に釘付け**（§2 の ▶・第289 起票）。**オッターヴァは既に START の音符から引いている**（`StartItemIndex: mark.AnchorItemIndex`）ので、**非対称なのはテキストスパナ側だけ**——**同じ engine の中に 2 つの答えが在る形。**
- **★★ ペダルの対応付けに譜／声部フィルタが無い**（⑹）。**観測者ゼロなので、直す便は本を書くところから。**
（以下、前便からの一覧はそのまま）

⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 の ▶ で「ユーザー決定・優先度高」だったものは 2 つとも閉じた**。**残っているのは ▶ 2 つ**（**スパナの左端**・**loose-line の閉じの距離を部屋の外側プロファイルへ**）**と、第288 が残した「歌詞のバネが最小に座る」**（台帳に `OPEN:` で待っている・証拠が一番揃っている）。★ **ユーザーの本で今 LYS4018 が出ているのは `Untitled-6.lys` の 1 冊だけ**（第288 便の `@rit`）。

★ **開始時裏取り**: HEAD `672994e9`・**未 push 22**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6480 / 0 / 4 / 6484**・台帳 **742／exact 582／総和 25.844537623／ss 非ゼロ 190／count 156 中 2**・追跡 `.lys` **579 冊**・snapshot **229 枚**——**§1 の終了時の数と 1 つも違わなかった**（**ただし §1 自体が 2 便古く**、**第288 が §2・§3 に 4 本足していた**）。
終了時: **commit 7 本**（`7b0df578` テキストスパナ ＋ `98198e9d` ハンドオフ ＋ `e0736dca` loco の実測 ＋ `1115f42e` オッターヴァとペダルの診断 ＋ `fd7995a9` ハンドオフ ＋ `225ff9ec` ペダルの綴り ＋ **この 1 本**）＝**未 push は 開始 22 ＋ 本便 7**・木 0・未追跡 0。**suite 6547 合格 / 0 失敗 / 4 skip / 合計 6551**（開始 6484 から **+67**）。Core 0 エラー 0 警告。**追跡 `.lys` 579 冊・snapshot 229 枚**（**どちらも不動**——**本を足さず、6 冊を*移行*した**）。**台帳全点不動。**
⚠️ **掃きは 3 回**（**便ごとに 1 回・母集団はディスク上の全 `.lys`・base と head は*本ごとに続けて***）: **テキストスパナ＝1582 冊 SAME 1554 / MOVED 28**（**追跡 fixture 4 ＋ scratch 23 ＋ ユーザー実コーパス 1**）／**オッターヴァ＝1586 冊 SAME 1581 / MOVED 5**（**移行した 2 冊 ＋ scratch 3・ユーザー実コーパスは 0 冊**）／**ペダルの綴り＝1588 冊 SAME 1573 / MOVED 15**（**ペダルを書く本ちょうど＝追跡 6 ＋ scratch 9・ユーザー実コーパスは 0 冊**）。**どれもレンダー失敗 0・エラーが増えた本 0。**
★★ **緑は*コード commit のあと*に取った**。★★ **WSL ubuntu Release も同じ 6547 / 0 / 4 / 6551**（`225ff9ec` の木で）。⚠️ **CI だけは未実行**（未 push 29 本）。
...
> ⚠️ ★★ **オッターヴァは既に正しい**（`StartItemIndex: mark.AnchorItemIndex`・第289 で確認）——
> **だからこれは「LP と違う」ではなく「同じ engine の中に 1 つの問いへの答えが 2 つ在る」**。——
