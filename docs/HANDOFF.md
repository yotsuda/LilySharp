...
最終更新 2026-07-30（第46セッション・**★★★ figured bass の「第3の綴り」＝`EstimateLooseLineExtents`
...
★★ **対が第2の欠陥を出した（OPEN）**: `figbass.page.deep.systems-on-first-page` が
**Lily# 10 対 LP 12**。**cap に当たっていない唯一の本＝ページブレーカを読む唯一の点**で、
深い figure 行のとき **system 間を 1 対あたり約 2.85 過剰予約**している（LP は一様 12.811124
...
⇒ 次は**幅でなくフレーム**（どこから測るか／`Distance()` が declined してスカラー和に落ちていないか）。
...
**HEAD は 3574 passed / 0 failed / 3 skipped**（台帳 **243 点**全緑・第43セッションで
...
- ★★★ **予約を箱でなく実アウトラインで**（第46セッションが点を開けた）。台帳
  `dynamic.page.quiet` **−0.412774041**／`dynamic.page.deep` **−0.390488638**／
  `hairpin.page.quiet` **−0.543200181**＝**Lily# は dynamic を 1.2/0.3 の平箱・hairpin を ±0.34 で
  予約する**（`LayoutEngine` の annotation-protrusion pass）が、LP は grob の**実 vertical-skyline**。
  ⇒ **一貫して過小予約**で、最下インクが下余白へ 1.5 ss 食い込む。⚠️ **配置の点は全部緑のまま**
  （既存 dynamic 点は「置いた場所」しか見ていない＝README の片側点の予約版）。
  ★ **2 冊の差が 0.022285 しか無い**＝債務は**箱の性質**でテクスチャに依らない。
...
- ★★ **`systemSkylines` を読む残り 2 家族**（元は 3・⑶ は第43セッションで閉じた。
  ...
  **次の figured bass は 3 つ**（どれも「点が先」）:
  - ~~★★★ **⒜ ページ高の点**~~ — **済（第46セッション・起票 `40efa06b`／移植 `5edd9481`・
    ...
    **深い figure 行の system 間を 1 対あたり約 2.85 過剰予約**。**seed の箱幅は反証済**
    （半分にしても全テスト・全点不動）⇒ **見るのはフレーム**（測る起点／`Distance()` の decline）。
