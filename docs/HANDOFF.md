...
同じ会話の続き。★ `-Start p571`（HEAD `a637d374`・full 8988 / 0 / 2 / 8990・第569 を ARCHIVE へ）＝HANDOFF の 450 KB 天井で便を切った。作業ツリーの `samples/*` はユーザーの手。

★ **⑴ R12⒟（`365f460b`）**: `eses`/`ases`（LP の `define-note-names.scm` nederlands で E𝄫/A𝄫）が識別子に落ちていた。lexer・`PitchSyntax.NameOf`・`KeySpelling.TonicFifths` の 3 か所で受け、`GRAMMAR_FOR_LLM` の誤記「`eses`=Dbb」を `deses` に直した。網は既存 3 本の Theory に行を足した（毒: lexer を戻すと 2 本赤）・双子を LP がコンパイル成功・full 8998 / 0 / 2。
  ⇒ R12 の残りは ⒜⒝⒠⒡。

★ **⑵ R12⒡（`d7feeec9`）**: 起票の「死んだ枝」は半分が生きていた。`ExpectMarkName` の Integer/`s`/`f` 腕は `@!f`・`@6` を validator の警告へ通す回復路で、`@!mf`/`@!p`/`@!r` だけが生の構文エラーだった＝接した語をすべて名前に（間を空けた `@! r4` は従来どおり）。`IsArticulationName` の Coda/Segno/Fine は直前の航行記号の枝が先に取るので撤去。full 9006 / 0 / 2。
  ⇒ R12 の残りは ⒜⒝⒠。

★ **⑶ R12⒜**: 増分再パースで LYS0020 が消える件。再現＝`{ c4 }` `phrase zz`… `{ d4 }` で先頭を消す（隣接だと damage 幅で再パースされ再現しない）。直し＝`_topLevelMusicReported` が偽で `IsTopLevelMusicStart()` の位置は adopt しない（switch だけ＝打鍵経路の値段は無視できる）。full 9007 / 0 / 2。
  ⇒ R12 の残りは ⒝（phrase 参照の小節充足の 2 綴り）⒠（予算の単位）。

★★ **⑷ R12⒝（`9f70c6f8`）＋掃きで見つけた落ち（`d0caef28`）**: ⒝ は §2 の行のとおり。掃きの途中 Lab `probes/s70b/cue-accidentals.lys` で `lysc check` が**スタック溢れで落ちた**（HEAD でも）。原因＝`phrase t { c4 cue { t } }`：cue は歩く時に本体を集めるので、外側の展開の循環ガードが既に無く、t を無限に展開した（grace/tuplet 経由は正しく循環エラー）。直し＝reset/end marker に phrase 名を持たせ、walk が開いている phrase を積み、cue の収集がそれでガードを種付けする（`_openPhrases`・`Reset` で空に）。網 `CueRegionTests.APhraseReachingItselfThroughACue_RendersItsAcyclicPrefix`（毒＝テストホストごと落ちる）。full 9013 / 0 / 2。
  ⇒ R12 の残りは ⒠ だけ。⒠ は予算 50,000 を超えた本の切り詰め位置が validator と頁で違い得るという話で、超えた時点で LYS1033 が「頁は切り詰め」と言っている＝病的な本だけ・提案の優先度は低い。
...
★★★ **⑴ 歌詞のメリスマを LP の `\lyricsto` に（ユーザー決定「LP に合わせて」・§3 に行・実装は第569 の末尾の `9049fb78`）**。`c'4( d e) f` に `la __ la` で Lily# は 2 つ目を e に置いていた（全音符が音節を取り、歌詞側の印だけがメリスマを作る・`__` も 1 音取る）。LP 実測（Lab `sessions/p569/melisma-assign.ly`＝各音節の列の `when`）: スラー 0／3/4・タイ 0／3/4・裸の `la __ lb lc ld` 0 1/4 1/2 3/4・`la _ lb lc` 0 1/2 3/4・小節をまたぐスラー 0 1/2 3/2。実装＝`LyricsCollector.BuildNoteIndices` が busy（`slurMelismaBusy`＝スラーの 2 音目〜閉じる音／`tieMelismaBusy`＝タイの着く音・休符はタイを切りスラーは跨ぐ）を印し、`LyricCollector` は busy でない音符にだけ音節を置き、busy は直前の音節のメリスマ（左寄せ・`MelismaEnd`）に。**`__` は線だけで音符を取らない・`_` と離れた `~` は 1 つ取る**。`LyricHyphen` の最後の延長線の連なりもスラーの閉じまで追う（旧＝開く音の次の 1 音まで）。旧綴りの本は LP の綴りへ: `audit/lpreg/lymelmel.lys`（`__ ~ ~`→`__`・描画 byte 同一）・台帳 LMS/LMN の Lily# 側（`mumum mum`＝LP のプローブと同じ語・2 点不動）・テスト 2 本。**追跡 40 冊で絵が動いたのはその双子 1 冊・snapshot 0**・コーパスに歌詞 0 冊。網 `LyricSlurMelismaTests` 6 本・毒 8 赤。文書: GRAMMAR の LyricSyllable・GRAMMAR_FOR_LLM・SYNTAX_REFERENCE（メリスマの段落と例）・CHANGELOG 0.8.0 Breaking。
  ⇒ R10 の残りは ⒜（aligned_side の 3 綴り＝大）だけ。

★★ **⑵ R7⒝⒠・R9⒝（小さい忠実度 3 件・どれも出力不変＝網で守る）**: R7⒝ `6bbcb389`＝改行 DP の自然長（ragged の罰と線形見積りの起点）を `max(Σideal, Σmin)` から LP の `configuration_length(0)`＝ばねごとの `max(min, ideal)`（`Spring::length` は `max(0, blocking_force)`）へ・網 `LineNaturalLengthTests`・コーパス 333 冊 svg 不変。R7⒠ `9c06618a`＝`StrictNoteSpacing`（min を ideal まで上げる＝発明・偽 REF・本番で立てる者 0）を撤去＝LP の `strict-note-spacing` は `float_nonmusical_columns_`（別物）。R9⒝ `8b706df4`＝梁の衝突 padding に**梁自身の** length-fraction²（grace 0.224・tab 0.1345）・網 `BeamScoringTests` 1 本（0.25 上の物体が通常の梁は動かし grace は動かさない）・毒で赤・コーパス不変。full **8985 / 0 / 2 / 8987**。
  ⇒ 判定: R7 の残り ⒜（item-spring の構築順）⒞（`ApplyRods` の閉形式）・R8（頁 DP の重み）・R9 の ⒜⒞⒟⒠⒡⒢・R12（言語層）。R9⒠ は `5133b49f` が閉じた（LP でも読まれない 2 値を撤去）。R9⒢（tremolo の `height_of_my_trem`）は**梁付きの単音トレモロの斜線そのものが未描画**（`SharedRenderer.Marks.cs` の註）＝描画と一緒の中項目・実コーパス 0 件・repo 8 冊。この便の文脈に依らない＝(c)。

★★ **⑶ R12⒞（part header の key・`f08ddf4c`）**: Lab `sessions/p569/partkey.lys`（part header `key bes major`・section B だけ D）で頁は B♭→D→B♭、双子は曲頭の `\key bes` を書かず C で始まり、section C の B♭ 復帰は `ScoreHomeKey` が part header を score home と読んだ偶然だった。`partkey2/3` の実測で phrase 自動移調の基準はファイル先頭の key だけ（part header は表示のみ）と確認。直し＝`ScoreHomeKey.Read/Declaration` から part header を除き（`PartHeaderDeclaration` を新設）、双子は part 頭で part header の key を書き、section 境界の復帰先を「part header、無ければファイル先頭」にし、ambient はファイル先頭に戻す。part header の `time` は LYS1026 なので meter 側は不要（一度書いて撤去）。網 `PartHeaderKeyTwinTests` 3 本（毒: 双子を戻すと曲頭の key の 1 本が赤）・LilyPond で双子がコンパイル成功・full 8988 / 0 / 2。MIDI の `ScoreHomeKey.Sharps` は part header を含むまま（回帰を避け別問）。
  ⇒ R12 の残りは ⒜⒝⒟⒠⒡。
...
- **R7 横 spacing の残り**＝⒜ item-spring builder（`SpacingRules.Springs.cs:81-118`）が LP の `Spring(fraction*len, fraction*min)` → `set_min_distance` → `set_ideal_distance` の順序と違い、column builder（`MeasureLayouter.CreateInterColumnSpring`）は合っている＝2 系統の 2 綴り（§2 A の spring2系統）／⒝ ✅ 第570 が閉じた（`NaturalWidthOf`＝ばねごとの `max(min, ideal)`＝`configuration_length(0)`・網 `LineNaturalLengthTests`・出力不変）／⒞ `SpringSolver.ApplyRods` の閉形式＋10 回ループ（LP は `range_solve`）／⒟ ✅ 第569 が閉じた（`StaggerEntries`＝音名の ape・`ape_less`・`stagger_apes`・台帳 `chord.accidental.flat-stagger-span` EXACT）／⒠ ✅ 第570 が閉じた（読み手 0 の発明＝`StrictNoteSpacing` を撤去）。
- **R8 頁 DP の重みの 2 綴り**（`PageBreaker.DemeritsOf :1389-1412` は f²×weight を clamp 前に掛け penalty は無重み・LP は `min(f²,BAD)` を page ごと足し `page_weighting` は `finalize_spacing_result` で 1 度）＋ `BreakPenalty` は line DP の項／⒝ overfull の判定が paper 高でなく printable 帯／⒞ `SystemDetails` の scalar extent と shape の max の食い違い（`Height`/`InverseHooke`/`MinWhitespace*` が scalar を読む）／⒟ 単一頁 stack が `PositionSystemsOnPage` の 2 綴り（`LayoutEngine.Pages.cs:393-589`）／⒠ ✅ 第570 が閉じた（`TightSpacing`＝代入 0 の発明・LP の `tight_spacing_` とは別物＝撤去）。
- **R9 梁・弓の残り**＝⒜ `shift_region_to_valid` の衝突半分（`beam-quanting.cc:810-889`・`Interval_minefield`）未移植＝seed が床の下なら LP は `v[DOWN]+2.0`・Lily# は床に clamp／⒝ ✅ 第570 が閉じた（梁自身の length-fraction²・grace 0.224／tab 0.1345・網 `BeamScoringTests`・出力不変）／⒞ `TieFormattingProblem :1000-1002` の line-centre が `|pos| ≤ 4` で ledger parity を切る（LP `on_line` は ledger も）・`:1065-1086` dot collision は X を読まず flat／⒟ `SlurScoringProblem :1061-1094` の slur 対 slur が発明（LP は相手の弓を k/2 で標本化して extra-object）／⒠ ✅ 第570 が閉じた（起票は半分古かった: `SlurTieExtremaMinDistance*`・`AccidentalCollision` は読まれている。`ExtraEncompassCollisionDistance`・`BetweenLengthLimit` は **LP でも代入だけで誰も読まない**＝撤去）／⒡ `TieVariantEngraver` は全部発明（LP は `from_semi_ties` で同じ scorer）／⒢ tremolo の `height_of_my_trem` 無し。
