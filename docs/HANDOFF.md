...
最終更新 第296セッション＝**§2 の項目ではなく*ユーザーが画面で見ていた欠陥*に着手＝`scratch\ベースタブLy\Untitled-6.lys` の「五線とコード名と rit が重なる」**（第295 の §1 が「次は訊く」で終わり、訊いたら *これ* が返ってきた）。**ユーザーの指示は直し方まで指定していた＝「ロジックを発明すると、もぐらたたきのような修正が収束しない。LP のコードを模倣して」**。**重なりは 1 つの欠陥ではなく 2 つで、どちらも LP に既に在る答えを 1 つ移すだけで閉じた**。**commit 2 本**。**ディスク 1645 冊・4 出力の全数掃きで動いたのは svg 1 冊＝報告されたその本だけ**（midi/xml/ly は 0・`check` の差 0）。**台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188・snapshot 230・追跡 `.lys` 580 は全部不動。**
...
終了時: **commit 2 本**（本体＋網 ＋ **この 1 本**）＝**未 push 29**・木 0・未追跡 0。**suite 6603 合格 / 0 失敗 / 4 skip / 合計 6607**（開始 6600 から **+7**＝**本便の網 7 ケース**）。Core 0 エラー 0 警告。**追跡 `.lys` 580 冊・snapshot 230 枚**（**書き換えも追加も削除も 0 枚**）・**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188＝すべて不動。**
