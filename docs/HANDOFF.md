...
（**実装 5 便＋道具 2 便**。**合成本の打鍵は 3304.5 → 244.2 MB＝−92.6%**。
...
**掃きは 20 本**（毎回 `git ls-files "*.lys"` の **566 冊**）＝**比較 13 回とも 0 冊動いた**＋
**毒つきの掃き 7 本**。**陽性対照は 7 回**（ScriptUp **6 冊**／ScriptDown **1 冊**／
箱・bow・tuplet **37 冊**／`RebuildKeepingHighest` **103 冊**／遅延 augment **2 冊**／
`extraPad` **11 冊**／ページ下界 **61 冊**）。⚠️ **数え方**——**比較には「同じ状態を取り直した分」と
「案A/案B を別々に測った分」が含まれる**（`audit/probe-out/p191-*.csv` を数えると 20 本。
...
| 1 打鍵 | 便前 | 第1便後 | 第2便後 | 第3便後 | 第4便後 | **第5便後** |
|---|---|---|---|---|---|---|
| `perf-fingstack1k` | 3304.5 MB | 2815.6 | 2191.1 | 1362.0 | 524.9 | **244.2**（**−92.6%**） |
| `perf-scripts1k` | 1117.8 MB | 932.6 | 697.8 | 403.9 | 336.8 | **87.5**（**−92.2%**） |
| `perf-v2bow1k` | 157.1 MB | 157.1 | 152.9 | 152.9 | 152.9 | **87.9**（−44.1%） |
| `perf-plain1k` | 95.9 MB | 95.9 | 95.1 | 95.1 | 95.1 | **66.3**（−30.9%） |

**フルレンダの `perf-fingstack1k` は 7355.1 → 2557.5 MB。**
...
- **⑺ ★ §7.5 の監査**＝**Core +249 行（うち*コード 110*）・削除 −65（コード 59）・
  ...
  **分類は 5 便とも ⒟**（何も足していない＝既存の家の指し直し／複製の削除／順序の確定／
  捨てられる計算の回避。**LP の意味論に 1 つも触っていない**——**566 冊・snapshot 217 枚・
  台帳 516 点がどれも不動**であることがその証明）。**§7.7 の匂い一覧はどれも当たらない。**
- **⑻ ★★ 残っている項＝**打鍵の残り **244.2 MB**（fingstack）／**87.5**（scripts1k）／
  **66.3**（plain1k）。**この便で潰した 5 段の内訳はもう当てにならないので、次は測り直しから。**
  **最後に測った形**（第5便の直前・fingstack 524.9 MB のとき）は
  **`3-layout` 93%・その内 `CreatePages` 54%・注釈パス 33%・`4-render` 5%・`1-collect` 1%**。
  ⇒ **`CreatePages` が消えたので、次の首位は注釈パスか render のどちらか**＝**未測定**。
...
> # ▶ **打鍵の首位＝注釈パスが system の silhouette を「1 個足すたびに」複製する**
> （2026-08-17・第190 起票／**第191 が住所を取り直して 3 段返済＝合成本の打鍵 −58.8%。
> 残るのは engraver の中と、全 merge が通る `MergeInternal`**）
>
...
>
> | 1 打鍵 | 第190 時点 | ⑴ 後 | ⑵ 後 | ⑶ 後 | **⑷ 後** |
> |---|---|---|---|---|---|
> | `perf-fingstack1k`（11,988 script） | 3304.5 MB | 2815.6 | 2191.1 | 1362.0 | **524.9**（**−84.1%**） |
> | `perf-scripts1k` | 1117.8 MB | 932.6 | 697.8 | 403.9 | **336.8**（**−69.9%**） |
> | `perf-v2bow1k` | 157.1 MB | 157.1 | 152.9 | 152.9 | 152.9 |
> | `perf-plain1k`（script 0） | 95.9 MB | 95.9 | 95.1 | 95.1 | 95.1 |
>
> **フルレンダの `perf-fingstack1k` は 7355.1 → 2838.2 MB。**
> ⚠️ **対照は便ごとに違う**——**⑴ は plain1k も v2bow1k も ±0.00＝真の対照／⑵ は全 merge が通る器なので
> 対照が存在しない（両方が受益者）／⑶⑷ は plain1k が対照**。
> **「対照が動いた」ではなく「これは対照ではない」と読むこと**（§1 の骨 ⑶）。
>
> **⑴ phase 二分**: 第190 時点で打鍵の **約 87% が注釈パス**（`CalculateAnnotationLayouts` を
> region で囲って実測。plain1k では同じ region が 6%）。**その中の内訳が下の ⑶。**
> ⇒ **第191 後は `AugmentSkylinesWithScripts` が 1%（11.5 MB）**、
> **`Distance` の padding は消えた**。**この島に首位はもう無い。**
>
...
>
> **⑶' 残っている段（着手するならここから）**:
> ★★ **打鍵の残りは 244.2 MB（fingstack）／87.5（scripts1k）／66.3（plain1k）。**
> **次の首位は未測定**——**第5便で `CreatePages` が消えたので、残るのは注釈パスか render**。
> **region スコープを置いて二分するところから**（§1 の骨 ⑴ の手順で 30 分）。
> ⚠️ **もう違うと分かっている所**: `AugmentSkylinesWithScripts` 1%・
> `LayoutSystems`／`looseChain`／`LayoutAllSpanners` は 0.0〜0.4 MB。
