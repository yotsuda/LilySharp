...
- ★★★ **⒩′ 残りの 355 サイト＝同じ道具で*回数から*絞れる**（第439 が ⒩ の頭を閉じた。脚は Lab
  `sessions/p439/instrument.ps1`＝`new Dictionary/HashSet/List<>()` を機械で pass-through counter に
  包む・`sites.txt`・`site-counts-before.txt`。**519 サイトのうち live 358・1 打鍵 2,875.8 建て**）。
  **次に大きいのは `Svg/Model/MeasureContentKey.cs:531` 98.70・`Svg/Layout/VerticalSkyline.cs:76` 67.87・
  `BreakAlignSpacing.cs:342` 64.51・`ArticulationEngraver.cs:394` 61.46 回／打鍵**＝**回数だけ既知・未値付け**
  （値段は `Count` と `Capacity` の会計で 1 脚＝第439 ⑵ の型）
...
- ⚠️ **`ColumnParts` の器は*寸法を言えない*（何を足すかが枝で決まる）＝残り 37,457 B／打鍵 0.48% は
  List の 4 枠の粒度そのもの。`FromBox` の 1 枚の器も、1 で建てると呼び手が `Merge` した瞬間に
  建て直す**（第439 実測）＝**この 2 つには戻らない**
- ⒥ は第409 が上限 4.7 ms と測った／**Ⓑ ⒢′ ⒳‴ ⒳⁗ ⒞″ ⒟ R13⒦ ⒤ ⒴⁵ ⒴⁷ ⒵⁵ ⒵⁶ ✅ 閉じた**

### 1.1 第439セッション（2026-09-20・YT-DELL2）
...
`/clear` 直後の新セッション。指示は「**HANDOFF を読んで着手**」で、**着手先は §1.0 ⒜ の ⒴⁷**（ユーザーが
選んだ・⒴⁷ と ⒵⁴ と ⒨ と LP 双子を並べて訊いた）。★ **`-Start p438` の 1 コマンドで §0 が全部済んだ**
（HEAD `3a0f7b29`・**未 push 61**・**full `sessions/p438/run1.trx` 8774 / 0 / 3 / 8777＝第437 最終と一致**・
台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・§7 3.5 の `-Archive 436` も自動で回って
68 行 4,476 chars）。★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` の
ままだから＝第412〜第437 が予告したもの）。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第437 と
同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、
**before 7,812,003 B／打鍵は第437 最終の 7,812,021 と 18 B 違い**（＝同じ木・同じ母集団の証拠）。
⇒ **起票は*腕*ではなく*形*を外しており、名指した形は島の 0.6% で、島の 42% は「建てて次の行で捨てる」だった。**

★★★ **⑴ 起票が名指した「空でも `HashSet`」は島の 0.6% しかなかった**（Lab `beam-counts-before3.txt`）。
⒴⁷ は「beams が空でも `HashSet`＝0.028%」だが、実測は **`bits.body` 6,536 回／打鍵 3.54・8,267,776 B
＝4,474 B／打鍵＝0.057%**（起票の 2 倍）で、その内訳は **空の腕 `bits.b-mt` が 712 回 × ちょうど
64.00 B ＝45,568 B＝25 B／打鍵＝0.00032%**、**空でない腕 `bits.b-full` が 8,222,208 B＝4,449 B／打鍵
＝島の 99.4%**。⚠️ **腕も 2 本あった**（`e.edge` 0.029%・`e.inside` 0.029%＝第435 の規則の 3 例目）。
★★★ **そして 42.4% は*捨てられていた***: `AddStaffToSkylines` は text row と tab 譜で即 return するのに、
**呼び手 2 人が*その 1 行手前で*集合を建てていた**——**6,536 回のうち 2,774 回（`d.tab` 2,766・
`d.textrow` 8）・101,644 鍵のうち 43,096 鍵**。⚠️ **`bits.grew` は `bits.items` と同数＝重複 `Add` は
1 回も無い**＝1 回 17.45 鍵で 1,411.68 B ということは、**1 鍵 31 B を超える分は全部*建て直し***
（3 → 7 → 17 → … の再成長）。

★★★ **⑵ 直しは 3 つで、どれも算法に触らない**（`01ef6012`・Core `+` 40 行〈**うち註 25 行**〉・
**数値定数 0・LP 由来の式 0・REF 0／OWN 0**＝§7.6 ⒟）: ⑴ **呼び出しを `AddStaffToSkylines` の*中*の
2 つの return の後ろへ移す**（＝**この量の家は「読む側」1 軒**になり、2 つの綴りが 1 つに畳まれ、
捨てる 2,774 回は*建たなく*なる）／⑵ **詰める前に寸法を言う**（`Members.Length` の和＝上限・割当 0・
`IsKnee` を訊き直さない）ので再成長しない／⑶ **何も抑制しない譜には共有の不変な空集合**を返し、
**戻り値の型を `IReadOnlySet` にして書き込みをコンパイラに禁じさせた**。

★★★ **⑶ A/B ＝ −0.041%**（**7,812,003 / 7,811,958 → 7,808,716 / 7,808,766 B／打鍵**・before はこの便で
取り直した・**before は 2 回で 45 B 差・after は 2 回で 50 B 差**）。継ぎ目は **`bits.body` 4,474 → 1,240 B／打鍵
（0.057% → 0.016%）**・**回数 6,536 → 3,762**・**鍵 101,644 → 58,548**・**B／回 1,411.78 → 609.25**・
**`bits.b-mt` 45,568 → 0**。**冊ごとは 231 冊中 223 冊が改善**・**中央値 −0.034%・最良 −0.136%・最悪 +0.007%**
で、**同じ木を 2 回回した対照帯は −0.018%〜+0.017%＝悪化側は 1 冊も帯の外に出ていない**。

★★★ **⑷ 予測は 6 つとも当たった——そして*当たると事前に分かっていた*のが今便の所見。**
第436・第437 は 2 便続けて「良い側に外れ、外れ方が所見」だったが、**今便は外れなかった**。理由は
**計器そのものが先に答えていた**: **呼び手 3 本の継ぎ目の和が `bits.body` と*ちょうど*一致した**
（4,127,936 ＋ 11,392 ＋ 4,128,448 ＝ 8,267,776）。⇒ ★★★ **和が閉じたら、そのメソッドの*入口*には
何も建っていない**（閉包の display class も iterator の state machine も無い）**＝内側の継ぎ目を
信じてよい**。第437 が 32 B ぶん外したのは、まさにこの和が**閉じていなかった**腕だった。
**実際、島は 3,234 B／打鍵減り、打鍵の総和は 3,238 B 減った（差 4 B）**（Lab `prediction.txt`
＝実装前の予測・P1 回数 3,762・P2 鍵 58,548 は**数字ちょうど**・P4 B／回 450〜700・P5 島 0.012〜0.018%・
P6 net −0.040〜−0.050%）。

★★ **⑸ 出力同一は 3 つとも**: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline `17fb650e`）／
⑵ **実コーパス全ページ SHA-256＝5,824 行・0 行差**（第437 の `hashes-after.txt` と突き合わせ）／
⑶ suite 全緑・snapshot 249 枚不動・台帳 851 点／総和 22.584727806 不変。

★★★ **⑹ 毒 4 つ・全部見分けつき——そして*緑を予測した毒*が 1 つ。**
⑴ **常に空集合を返す** → **18 本赤**／⑵ **`IsKnee && !IsCrossStaff` の skip を落とす** → **ちょうど 1 本**
（`SkylineStaffSpacingTests.BeamedItemsToSuppress_SameStaffKnee_KeepsFixedStems`）／⑶ ★ **edge の呼びに
`default` を渡す**（＝**`beams` が引数になって初めて可能になった間違え方**・第437 ⑹ の型）→ **5 本赤**
（snapshot 3 枚＋台帳 `system.beam-{under,over}-notes` 2 点）＝**この腕には観測者が居る**ので網は要らない／
⑷ ★★★ **`room - 1` で寸法を 1 つ足りなくする** → **0 赤**——**これは予測して当てた緑**で、
**「容量は*正しさ*ではなく性能の性質だ」を毒で*言わせた***（RULES §5.0「註の『安全だ』は毒で確かめるまで
予測」の裏返し＝**緑であることに意味がある毒**）。

★ **⑺ 終了時**: commit 2 本（code `01ef6012`・docs 1 本＝この §1 と RULES §5.0 ×1・§5.3 ×1・§7.7 ×1。
**この文を含むので SHA は書かない**＝§5.4）・**未 push 63**（開始時 61）・**最終 full
`sessions/p438/run3.trx` 8774 / 0 / 3 / 8777**（**第437 最終と同じ＝今便は網を 1 本も足していない**
——毒 ⑶ が既存の観測者 5 つで赤くなったので**網は要らない**。`Zz438`／`ZZPOISON` の grep は**コードに 0 件**）・
§7.5（対 `3a0f7b29`）**Core `+` 40 行／REF 0／OWN 0＝算術を 1 つも足していない**・台帳 851 点／
総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・**棚卸しは `LILYSHARP_UPDATE_DOCS=1` で
再生成し、`APPROXIMATIONS.md` はバイト同一・`magic_constants.csv` は 34 行が動いて*行番号を伏せると差 0***。
計器と全文は Lab `sessions/p438/`（**`prediction.txt`＝実装前の予測**・**`beam-counts-before.txt`／
`-before2`／`-before3`／`-after.txt`＝継ぎ目を 3 段に割った前後**・`Zz438.cs.txt`／`Zz438Leg1.cs.txt`＝計器・
**`Zz438Ab.cs.txt`＝再利用可の A/B**・`Zz438Hash.cs.txt`＝証明 ⑵ の harness・`ab-before-1-tc0.txt`／
`ab-before-2-tc0.txt`／`ab-after-1-tc0.txt`／`ab-after-2-tc0.txt`・`zz438-seams.diff.txt`／
`zz438-fix-plus-seams.diff.txt`）。
⇒ ⚠️⚠️ **HANDOFF は `-Archive 436` 後 438,207 B → 今便を書いて 447,6xx B＝天井 450,000 まで
残り 2,400 B 級**（`Fold-ClosedHandoffItems` の弁は第426 の時点で 0 件・§3 の圧力弁は第409・第410 で
使い切り。**天井は上げない**）。⚠️⚠️⚠️ **次便は §1 に 1 字も足せない＝`-Archive 437` を回すまで
書き始めないこと**（回せば今便の下のブロック約 9 KB が空く。**途中で回すと
`HandoffArchiveContinuityTests` が 3 本赤くなる**＝第410 ⑾）。
⇒ **次便は `-Start p439` の 1 コマンドから入る**（裏取り → `-Archive 437` 自動 → §1.0 の一覧）。
⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、
それは第412〜第438 の欠陥ではない。**作業ツリーは空**。**push はユーザー**（Lab も）。
