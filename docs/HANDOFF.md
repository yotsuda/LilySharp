...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第416セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `b3880b9a`・**未 push 10**・**full `sessions/p416/run1.trx` 8724 / 0 / 3 / 8727＝第415 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 68 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第415 が予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 414`＝49 行 6,823 chars）。指示「HANDOFF を読んで着手」→ §1 が ★★★ で名指していた **⒳ `resolve`（`VerticalSkyline.EndBatch`）＝第414 が「未着手・未計測の内側」と書いた島**を採った。★ ベンチは着手前に AskUserQuestion で静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414・第415 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト）:

★★★ **⑴ 検算**＝`resolve` **211,453,472 B** 対 第414 の 211,063,968 B＝**0.18% 差**＝継ぎ目は測る対象を乱していない。⚠️ **打鍵の分母は継ぎ目の重さで便ごとに違う**（この便 9,183,372 B）＝**便をまたいで比べる量はバイトのほう**。

★★★ **⑵ 予測を先に probe の頭へ書いて、当たった**（§5.0②）＝**費用は算術ではなく、overlap 1 件ごとに捨てられる 3 本の List**（tail の取り出し・境界点・組み直した segment）。内訳は **`a.sort` 0 B**・**`a.coalesce` 0 B**・入力の複写 13.9%・**walk 93.4%**＝**1 overlap が 398 B**（system 側 474 B）。

★★★ **⑶ そして resolve は*二次ではなかった***。新しい 1 本が merge に引き戻す tail は**平均 1.52 本・最大 18 本**（入力は最大 596 本）＝**線形**。⚠️ **「O(n²) に見える形」は形であって測定ではない**（§5.3 へ）。

★★★ **⑷ 島は名指されたものより大きかった。** 同じ walk（`RebuildKeepingHighest`）の門は 2 つ（`EndBatch` 10,284・`MergeInternal` 73,254）:

| | バイト | 打鍵比 | n |
|---|---|---|---|
| **walk 全体** | 673,848,928 | **3.97%** | 83,538 |
| └ `s.walk`＝§1 の島 ⒳ | 197,417,072 | 1.16% | 3,742 |
| └ `o.walk`＝それ以外 | 476,431,856 | **2.81%** | 79,796 |
| `Padded`（中に walk 2 本） | 457,849,992 | 2.70% | 7,185 |

⇒ ★★★ **§1 が名指した島は同じ機構の 29% でしかなかった**＝**費用の表を*呼び手*で割ると島の大きさを取り違える。*機構*で割り直すこと**（§5.3 へ）。

★★★ **⑸ 直し＝scratch を walk に 1 組**（`VerticalSkyline.ResolveScratch`＝3 本の List を *per overlap* から *per walk* へ）。**static でも field でもなく局所**＝2 つのスレッドが何も共有せず buffer は walk と共に死ぬ・遅延生成なので overlap 0 本の walk は 0 B。⚠️ **算術は 1 バイトも触っていない。**

★★★ **⑹ 同じ継ぎ目で前後を測ったら、*回数が 1 件も動かなかった***（overlap 1,691,308・tail 合計 2,570,996・境界 8,621,195・interval×building 29,327,868・buildings in 898,016 / out 324,555 が**前後で同一**）＝**動いたのは割当だけ**という対照が測定の中に入っている:

| | 前 | 後 | 差 |
|---|---|---|---|
| `resolve`（島 ⒳） | 211,453,472 | 16,572,016 | **−92.2%** |
| walk 全体 | 673,848,928 | 70,398,080 | **−89.6%** |
| `Padded` | 457,849,992 | 108,029,856 | −76.4% |
| 1 overlap あたり | 398 B | **42 B** | |

★★★ **⑺ A/B（継ぎ目を全部外した素の harness）＝打鍵 9,184,028 → 8,725,056 B＝−5.00%。**

★★★ **⑻ 出力同一は 3 つとも走らせた**（RULES §5.1）: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（⚠️ baseline はこの便で `b3880b9a` から取り直した）／⑵ **全ページの SHA-256**＝**231 冊 × 8 打鍵で差 0 行**（before は製品の 1 ファイルだけ `git stash push` して取った＝ユーザーの未 commit `tools/*.ps1` には触っていない）／⑶ suite 全緑・snapshot 249 枚不動。

★★ **⑼ 網 3 本、そして毒が*予想を外した***。**答えを書く buffer の `Clear` を抜くとちょうど 2 本赤**（新しい profile の網 2 本だけ・既存 13 本は緑）。**ところが入力側 2 本の `Clear` を抜いても全網が緑**——**max に食わせる使い回しは冪等**（一度負けた building をもう一度差し出しても答えは変わらず、余分な境界は連続判定が畳み直す）。⇒ **その 2 本を見る観測者は費用しかない**ので**割当の門を 1 本足した**（12 箱の resolve が **2,184 B**・per-overlap に戻すと **7,000 B** でちょうど 1 本赤）。★★ **絵が正しいまま費用だけが戻る変更は profile の網では一生捕まらない**（§5.3 へ・第414 ⑺ の「黙っている fallback」と同族）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: **⒳ ✅ 閉じた**（1.25% → 0.10%）。⚠️ **分母が 5% 動いた**＝以下の比は第415 までの分母で書かれている。**残る島**: ★★ **⒳′ resolve の*複写*＝`EndBatch` の入力複写 0.17% ＋ `MergeInternal` 自身の複写（未計測・推定 0.7%）**＝**今便の scratch とは別の機構**（call をまたいで生きる buffer が要る＝instance か static。**今便は局所で済ませたので触っていない**）／★★ **⒴′ 2 度目の注釈パスを要らなくする**（7.7%・第411 ⑶）／★ **⒴″ `b.scriptsFingerings` 4.2%**・未分割／★ **⒵‴ `e4.musicMarks` 1.22%**・未分割／★ **⒴ `inkFirst`＋`inkLast` 0.53%**／⚠️ **`g4.core` は repair 後 1.21% で*もう実仕事*＝この島には戻らないこと**／⒜ **R13⒝ の実機確認**（第404 ⑵＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・⚠️ 未着手・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／LP 双子が要る R7〜R11／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑽ 終了時**: commit 2 本（code `b1a4db5b`・docs 1 本＝**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 1 本・**最終 full `sessions/p416/run3.trx` 8727 / 0 / 3 / 8730**（§1・RULES を書いた*後*の最終ツリーで取った）（⚠️ **第415 最終の 8724 / 8727 から +3＝今便の網ちょうど 3 本**・`ZzStage`／`Zz416`／`ZZ416` の grep は**コードに 0 件**）・§7.5（対 `b3880b9a`＝スクリプトの出力）**Core `+` 77 行／REF 0／OWN 0**（★ **Core に数値定数を 1 つも足していない**＝足したのは buffer 3 本を持つ private class 1 つと引数 1 本で、**LP 由来の式も逸脱も無い**＝77 行の大半は註。門の 3000 は Tests 側）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。⚠️ **棚卸し `docs/APPROXIMATIONS.md` と `audit/magic_constants.csv` は再生成した**＝**行番号が動いただけで行の増減 0**（+4/−4・+1/−1。手順どおり diff を読んでから回した）。§7 4（恒久知識）＝**RULES §5.3 に 3 項**（「**費用の表を*呼び手*で割ると島の大きさを取り違える＝*機構*で割り直す**」／「**『O(n²) に見える形』は形であって測定ではない＝引き戻す長さを数える**」／「**使い回す scratch の `Clear` が*正しさ*に要るのは答えを書く buffer だけ。残りを見る観測者は費用の門しかない**」）。計器と全文は Lab `sessions/p416/`（`inside-resolve-{before,after}.txt`・`ab-{before,after}.txt`・`hashes-{before,after}.txt`・`Zz416Probe{1,2}.cs.txt`・`ZzStage.cs.txt`・`zz416-seams-and-change.diff.txt`）。§7 3.5 は**セッションの頭で**済（`-Archive 414`＝49 行 6,823 chars）⇒ **HANDOFF は 443,920 B → アーカイブ後 437,097 B → 今便を書いて 446,965 B＝天井 450,000 まで 3.0 KB しかない**（今便のブロックは約 9.8 KB＝表 2 枚に絞った）。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 415` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第416 の欠陥ではない。
