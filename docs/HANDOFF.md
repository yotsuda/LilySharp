...
終了時: **commit 6 本**（**タブの stencil 表＋網 ／ docs ／ 段間の rit＋網＋fixture ／ docs ／ `as numbers` スイッチ＋網の書き換え ／ この 1 本**）＝**未 push 33**・木 0・未追跡 0。**suite 6606 合格 / 0 失敗 / 4 skip / 合計 6610**（開始 6600 から **+10**＝**本便の網 10 ケース**）。Core 0 エラー 0 警告。**追跡 `.lys` 581 冊**（**+1＝`test/rit-across-systems`**）・**snapshot 231 枚**（**+1＝同じ本。既存の 230 枚は 1 枚も書き換わっていない**）・**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188＝すべて不動**（**`page.inline-chord.gap-first` を含む＝⑿ の広い直しならここが 2.5 動いていた**）。
★★ **WSL ubuntu Release も 3 つの code の木すべてで Windows と同じ**——**`fa57384c` で 6603 / 0 / 4 / 6607、`5e50d9cb` で 6605 / 0 / 4 / 6609、`c92dd851` で 6606 / 0 / 4 / 6610**。⚠️ **CI だけは未実行**（未 push 33）。
