...
★★ **⑴ 開始時の数は第375 末の記録と全部一致**: HEAD `0680bfe4`・未 push 65（第375 の記録 64 ＋ 第 3 便の doc commit）・木 clean・未追跡 0・**台帳 843 点／ss 非ゼロ 210／総和 23.216135909／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249・追跡 `.lys` 599**・Core 0 警告・**full `scratch/p377/run1.trx` 8469 / 0 / 4 / 8473**。

★★★ **⑵ 仕分けの結果＝24 冊のうち「量を測ったのに固定しなかった」は 1 冊だけ（`instrument-name-x`）**。**母数を先に数え直した**: 24 冊とも台帳の `"probe":` は今も 0（`beam-column-spacing` は第373 ⑺ で点が付いたので実質 23）。**仕分けは「ヘッダが答えを書いているか」＋「答えの読み手（コード・test・台帳の why）が居るか」の 2 問**:
- **問いに答えて役目を終えた・読み手あり（18）**: `accidental-skyline`（`AccidentalPlacementTests`）／`beam-grace-score`・`jn-line-forces`・`cue-grace-spacing`（台帳の why）／`beam-least-squares`・`beam-stem-x`（`BeamScoringProblem`）／`brace-ladder`（生成物）／`brace-name-clear`（`BraceLadderTests` が幅 1.3734 を 6 桁で固定）／`coda-line-start`（`FormNavigationTests`）／`compressed-line-force`（`SpringRodModelTests`）／`dynamic-text-x`（`DynamicLabelWidthTests` の 20 本）／`glyph-skyline`（`StaffSkylineFrameTests`）／`line-start-key-change`（`SpacingInvariantTests`）／`notehead-stem-attachment`（定数を消してフォントを読む）／`pedal-three`（`PedalTests`）／`rows-only-page`（未移植の loose-line 再配分を待つ審判・ヘッダが理由を書く）／`skyline-binding`（`VerticalSkyline`・台帳の why）／`ties-slurs-breaks-ragged`（撤回済み・`compressed-line-force` へ）。
- **答えはあるが*記録の置き場所*が壊れていた（4）＝直した**: `figbass-accidental-x`（**ヘッダに答えが無く**、コードは `scratch/figbass-accidental-x.ly` と*存在しないパス*を引いていた）／`grace-accidental-font-size`（コードは `scratch acc-size.ly` を引いていた）／`beam-over-stem-book`（**どこからも引かれず、ヘッダに答えが無い**＝答えは ARCHIVE にだけ在った）／`rest-column-x`（ヘッダが「台帳に置かない理由」を書いている＝壊れていない・数に入れただけ）。⇒ **前 3 冊は LP 2.26.0（`-dbackend=null`）で再実行して数を確かめてから**、ヘッダに答えを書き、コードの引用を `audit/lp-geometry/probes/` に張り替えた（**production のコードはコメントだけ**）。
- ⚠️ **量を測ったのに固定していない（1）＝`instrument-name-x`**: **brace は indent ではなく `SystemStartBar` に対して置かれる**（LP の brace 右端 8.175827 ＝ bar 左端 8.475827 − 0.3）。**Lily# は今も `MultiStaffLayouter` の `braceX = CurrentIndent − 0.3`＝LP より 0.06 右**。**台帳にも §2 にも無かった**（ARCHIVE の第88 と probe ヘッダにだけ）⇒ **§2 E に名指しで起票した**（出力が動く＝**要承認**・点を置いて射程を数えてから）。

⚠️⚠️⚠️ ★★★ **⑶ 仕分けの途中で*commit 済みの撤回が逆向きだった*のを実測で割った**。`brace-name-clear.ly` の「RE-MEASURED 2026-08-04」は旧値（Soprano 幅 8.365・余白 0.385）を「再現しない」として 7.307／0.915 に書き換えていた。**同じ本を今日 2 通りで回した**（`scratch/p377/probes`）: **`-dbackend=null` は 8.365110／0.385358、`-dbackend=svg` は 7.306668／0.914580**＝**撤回された旧値が正典で、撤回した側が svg の generic serif（機械のフォールバック書体）**だった（記憶 `reference_lilypond_svg_backend_generic_fonts` の罠の 3 度目＝*commit 済みのヘッダと test の入力*に住んでいた・記憶に追記）。**`InstrumentNamePlacementTests` の幅 6 行も svg の値**。★ **ただし test は無事**——**規則は幅に線形で、null の右端は幅の差のちょうど半分だけ動く**（Soprano +0.529221 対 +1.058442・Contrabassoon は indent 超えで不動）＝**規則の test としては正しく、「LP の幅」という*ラベル*だけが違った**⇒ **test の remark とヘッダに出典を書き、数は変えていない**。★ **同じヘッダの「唯一の未説明＝残差 0.028110」も割れた**: **indent を 15mm から 8.503937 と換算していた**（1.763889 mm/ss）が、**LP 自身の `indent` は 8.535827**（dump）⇒ **残差は 0.060000 ＝ indent − bar 左端**＝`instrument-name-x` が既に説明していた量。

★★ **⑷ 数**: **開始時**は上 ⑴。**終了時**: HEAD＝この §1 を書く commit（親 `0680bfe4`・**probe ヘッダ 4 冊・コメント 2 軒・test の remark 1 軒・HANDOFF**＝**出力も数値も不変**）・未 push 66・**台帳・snapshot・追跡 `.lys` は全部不変**・Core 0 警告・**full `scratch/p377/run2.trx` 8469 / 0 / 4 / 8473**（**本数不変**＝test は remark を足しただけ）。§7.5（`-DiffBase 0680bfe4`）: **Core `+` 0 行／REF 0／OWN 0**（変えたのはコメントのパス 2 本＝スクリプトはコメントを数えない）。§7 9: 計算は足していない。⚠️ scratch は **`p377`**。⚠️ §7 3.5: 第374 を `-Archive 374` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝**push はユーザー**。**LSP は配布していない**（Core の変更がコメントだけ）。
...
★★★ **⑼ 第 4 便＝ユーザー承認「承認する」で §2 E の残り 3 件（bracket・縦線・譜線の左端）に着手**。**1 件ずつ直して commit**（diff が混ざらないように）。**⑴ bracket**: LP の extent は stroke だけ（`staff_bracket` の `Interval (0, thickness)`・tips は X に入らない＝`system-start-delimiter.cc:54-61`）で、**右端 ＝ 縦線左端 − 0.8** ⇒ Lily# が中心で描く stroke の中心は **indent − 1.085**（旧 indent − 0.8＝**0.285 右**）。**tips は stroke の左端に揃う**（LP も Lily# も）ので stroke を動かせば tips も動く。**式は ⑻ と同じ縦線の helper から**（`MultiStaffLayouter.SystemStartBarLeftEdge`／`SystemStartBracketCentre`・太さ 0.45 は `EngravingDefaults.SystemStartBracketThickness` に 1 本化＝layout と renderer が同じ数を読む）。**射程は実測**: snapshot **3 枚**・変わった行は **bracket の線 3・tips 6・楽器名 3、全部 x が −0.285**（丸めて .28/.29）・他は不変。⚠️ **引用の gate を 1 度また踏んだ**（`EngravingDefaults` の REF の住所の行に `_`／`-` 入りの名前が無い）＝⑻ と同じ規則。**⑴ の数**（`Session-Check -Build -Test -Scratch p377 -DiffBase 2071489e`）: **full `scratch/p377/run6.trx` 8473 / 0 / 4 / 8477**（+2＝bracket と縦線左端の test）・Core 0 警告・**台帳 843 点／総和 22.584587806 不変**・snapshot 3 枚（`choir-staff`・`sings-per-row`・`staff-group`）・追跡 `.lys` 599 不変。commit `802dfcc4`。**⑵ 左端の縦線**: Lily# は indent を**中心**に描いていた（ink indent±0.08）＝LP の indent−0.06 〜 +0.10 より **0.02 左**。⇒ **描画も楽器名の基準も `SystemStartBarLeftEdge` から**（`SharedRenderer.Connectors` の 2 箇所＝描く位置と名前が避ける左端が同じ式）。**射程は実測**: snapshot **70 枚**・変わった行は **縦線 85 本（x1/x2 +0.02）と楽器名 62 個（x +0.02）だけ**・brace／bracket／音符は 1 行も動かない（名前が動くのは縦線が最左 delimiter の系＝brace も bracket も無い複数譜）。**⑵ の数**（`-DiffBase 802dfcc4`）: **full `scratch/p377/run7.trx` 8473 / 0 / 4 / 8477**（本数不変）・Core 0 警告・台帳・追跡 `.lys` 不変・snapshot 70 枚。commit `e9fb8a06`。**⑶ 譜線の左端**: LP は `staff-symbol.cc:84` の `span_points[d] -= d * t / 2` で**線の両端を自分の太さの半分だけ内側へ**引く⇒ **左端のインクは始点 + t/2**。**LP で実測**（`scratch/p377/staffline/staffline.ly`）: Staff・TabStaff とも indent 0 で 0.05、既定 indent で 8.585827、**太さ 2 倍で 0.10**（＝定数ではなく太さの半分）。⇒ `SharedRenderer.StaffLineInkLeft`（五線・tab の弦線・ossia の 3 箇所）。⚠️ **ossia を 1 度間違えた**: ossia は `OssiaScale` で縮めて描くので、**縮める前の太さの半分を足すと ossia の座標で +0.07** になった（分類が 15 行だけ flag した）⇒ **「その線*自身*の太さ」＝太さ × `OssiaScale`** に直して全行 +0.05 に揃った。⚠️ **右端は触っていない**（LP は右端も t/2 引くが、Lily# の右端は LP と測っていない）＝§2 E に残す。**射程は実測**: snapshot **244 枚**・変わった行は **2039 本、全部 `<line>` の x1 だけが +0.05**（flag 0）。⚠️⚠️ **最初の full は赤 17 本、1 本も LP の値から外れていない＝全部 test 側の計器**: **16 本は譜線を SVG から `<line x1="0.00" …>` の完全一致で探していた**（`TablatureTests`・`GrandStaffRenderTests`・`CombinedStaffTests`・`CondensedStaffTests` 3・`FormNavigationTests` 2・`PaperBlockTests`・`PedalTests`・`RestAvoidNoteTests` 2・`RestPitchedBeamTests` 2＝`x1="0.00"` 系・`PercentRepeatTests` は `x1 < 0.05`（**インクがちょうど 0.05 に来て `<` で落ちた**）・`SharedRendererKeySignatureTests` は `|X1| < 1e-6`）⇒ **インクが 0.05 から始まって 0 本**。**1 本は数が 0.05 ずれた**（`GlissandoBoundTests`＝譜線インクの左端を「LP の座標の原点」として引いていた⇒ **原点 ＝ インク左端 − 太さ/2** に直した）。★ **教訓: 「譜線は x=0 から」を SVG の文字列で覚えている test が 16 本在った**＝**譜線の始点を動かす変更は、snapshot より先にこの族を割る**。**⑶ の数**（`-DiffBase e9fb8a06`）: **full `scratch/p377/run9.trx` 8476 / 0 / 4 / 8480**（+3＝`StaffLineInkLeft` の theory 3 case）・Core 0 警告・**台帳 843 点／総和 22.584587806 不変**（`RenderedGeometry` は譜線の Y と長さ ≥10 しか読まない）・追跡 `.lys` 599 不変・snapshot 244 枚。★ **LSP は ⑴⑵⑶ の最後に 1 回だけ配布**⇒ **ユーザーは Reload Window が 1 回要る**（配布済み `0.6.0+f970e86a`）。⑶ は commit `f970e86a`・未 push 71。

★★ **⑽ 第 5 便の判定＝「測って承認用に出す」まで着手・製品は触らない**（ユーザーの常設指示「次便は、このセッションでやる方が有利なら着手して…」）。**着手の根拠**: 残る候補のうち **譜線の右端だけが第 4 便の道具にそのまま乗る**（左端と同じ `staff-symbol.cc:84` の 1 行・`scratch/p377/staffline` の LP 計器・snapshot の行単位の分類器・「譜線を座標で探す test の族」の知識）＝**次の便なら全部作り直し**。**着手しなかった部分**: **直すことは出力と台帳 3 点を動かす＝承認要**（記憶の「着手しない側」⑴）。⇒ **測定と射程の見積もりだけをやり、§2 E に承認に要る中身を書いた**（見えるのは予告記号付きの行末だけ・台帳 3 点は +0.05 の補正を外す・x2 で譜線を探す test の grep が先）。★ **入れ子の delimiter** は **Lily# の model に入れ子が無い**（第 4 便で grep）＝**言語の設計判断**で道具の温度は効かない＝次のセッションでやっても損しない。**⒝⒠⒢** も同じく別 regime。★ **この便で最初に測りを間違えた 1 箇所**: LP の dump に `ly:item-get-column` を全 grob へ当て、**Spanner（StaffSymbol）で Guile が落ちて 4 冊とも空**だった（`Items` と `Spanners` を分けて呼ぶ）。

★★★ **⑾ 第 6 便＝ユーザー承認「承認する。右端も直して」で譜線の右端を直した**（出力と台帳が動いた便）。**直し方は左端の対**＝`SharedRenderer.StaffLineInkRight`（右端 − 太さ/2）を五線・tab の弦線・ossia（ossia 自身の太さ）の 3 箇所に。**台帳 3 点**（`courtesy.meter.meter-to-line-end` 2.95→2.90・`…double-bar-numeral` 2.854735→2.804735・`courtesy.key.key-to-line-end` 7.7498→7.6998）は **+0.05 の補正を外し、残差 0 のまま**（**台帳は全体が緑**＝他に右端を読む点は無かった）・why と `probes/courtesy-meter.ly` の注記も書き換えた。**射程は実測**: snapshot **244 枚**・変わった行は **2039 本、全部 `<line>` の x2 だけが −0.05**（flag 0）。**赤は `CourtesyMeterTests` の 5 本だけ＝予想どおり**（予告記号の後ろの余白を**描いた譜線の端**から測っていたので 0.45 と読んだ）⇒ **測る相手を「描いた端 + t/2 ＝行の端」に直す helper `LineEdge` を 1 本にまとめ、0.5 の規則はそのまま主張**。★ **左端のときに学んだ「譜線を座標で探す test の族」を先に grep した**ので、`x2` を読む test を事前に 4 ファイルに絞れ、赤は予想した族だけで済んだ。⚠️ **full の 1 回目は赤 1 本＝`HandoffLedgerCitationTests`**: **第 5 便の §2 E の記録が台帳の点の名前 2 つを書いたのに `<!-- ledger: NAME = VALUE -->` の札を付けていなかった**（第 5 便は HANDOFF 用の gate 3 本しか回さず、この test を通っていなかった）⇒ **札 2 枚を付けた**（値は現在の残差 0）。★ **HANDOFF に台帳の点の名前を書いたら、その commit の前に `HandoffLedgerCitationTests` も回す**。**⑾ の数**（`-DiffBase 0f9d0ae0`）: **full `scratch/p377/run11.trx` 8478 / 0 / 4 / 8482**（+2＝`StaffLineInkRight` の theory 2 case）・Core 0 警告・**台帳 843 点／総和 22.584587806 不変**（3 点は値を書き直して残差 0 のまま）・追跡 `.lys` 599 不変・snapshot 244 枚。commit `cc7d60a8`。

★★★ **⑿ 第 8 便＝入れ子を実装（ユーザー決定: ①「意味があるのは condensedStaff／combinedStaff」②「安全に実装できるなら grandStaff も子にできるように」）**。**2 段に分けた**: **段階 A＝`grandStaff`／`staffGroup`／`choirStaff` の中に `condensedStaff { … }`／`combinedStaff { … }`**（どちらも*1 本の譜*を彫る＝グループは平らなまま）／**段階 B＝`staffGroup`／`choirStaff` の中に `grandStaff` を 1 段**（別 commit）。**段階 A の中身**: パーサが受ける（LYS6011 の文面も更新）／**`GrandStaffSpec.Staves` → `Members`**（staff／condensed／combined の item。**staff だけの見方は作らなかった**＝condensed を数え落として譜番号がずれるから。**構文側の `GrandStaffRenderSyntax.Staves` も `Members` に**＝コンパイラに全消費者を列挙させた）／`GetVoiceBindings`・`BuildStaffGroups` は**上位と同じ 1 本の switch** を通る（`BindingsOf`・`OneStaff`）／**combined の addressing は「グループ内の位置」を持ち `ToStaffGroups` が群の先頭を足す**（上位は 0＝不変）／歌詞行は**素の staff の下にだけ畳む**（condensed/combined の下は LYS6012 の新文面）／LilyPond 双子は**上位と同じく condensed/combined を書かず報告**／LSP のグループ本体の補完に 2 語。⚠️⚠️ ★★★ **危険は予想どおりの所に在った**: `RenderSpecParser.Parse` は `DescendantNodes` を歩くので、**ガード無しでは condensed がグループの中と外に 2 度彫られ、エラーは出ない**⇒ `IsInsideGrandStaff` を condensed／combined／grandStaff の 3 腕に付け、**毒（ガードを外す）で `NestedGroupMemberTests` の 5 本が赤、戻して 8 本緑・ハッシュ一致**。**射程は実測**: **snapshot 0 枚・台帳不変**（入れ子を書いた本はコーパスに 0 冊＝純粋な追加）。⚠️ **既存 test の直し**: `Staves` を読む 5 箇所（型の変更）と、**LSP の補完 4 本が「グループ本体は staff と lyrics だけ」を主張していた**（旧文法の網＝狭さの主張は残し、語を 2 つ足した）。**段階 A の数**（`-DiffBase cc7d60a8`）: **full `scratch/p377/run13.trx` 8486 / 0 / 4 / 8490**（+8＝`NestedGroupMemberTests`）・Core 0 警告・**台帳 843 点／総和 22.584587806 不変・snapshot 249 のうち動いたもの 0・追跡 `.lys` 599 不変**・生成物 `docs/APPROXIMATIONS.md` は行ずれのみ再生成。commit `a66f68cb`。**段階 B＝`staffGroup`／`choirStaff` の中に `grandStaff` を 1 段**。**model は平らなまま**: 入れ子を持つ bracket は**メンバーごとの leaf グループ**（素の staff は Single・入れ子の grandStaff は braced）に分かれ、**全 leaf が同じ `OuterStaffGroup`（参照で比べるクラス）を持つ**（`StaffGroup.Outer`・`StaffGroupLayout.Outer`）。**入れ子の無い本は何も変わらない**（`Outer` は null）。**LP の 3 規則をそれぞれ 1 箇所に**: ⒜ **間隔**＝`SelectInterGroupSpec` を「上の grouper（自分の delimiter か `Outer`）が無ければ既定／下と同じ grouper なら staff-staff／違えば staffgroup-staff」に一般化（平らな本では旧規則と同じ答え）⒝ **delimiter**＝入れ子の brace 右端は bracket のインク左端 − 0.3 ＝ indent − 1.61（`SystemStartNestedBraceRightEdge`・LP 実測 6.925827）、外側の bracket は**描く時に置かれた譜から作る**（`OuterDelimiters`＝段の再配置で古くならない）・楽器名の `totalLeft` にも入る ⒞ **span bar**＝外側の staffGroup は**直接の子どうしの隙間だけ**（入れ子の delimited leaf の中は leaf 自身が描く）・choirStaff は無し・`HairpinEngraver.SpanBarBelowOnSystem` も外側を見る。**キャッシュ鍵 2 本に外側を足した**（`MeasureContentKey`・`SvgSystemFragmentCache`）。**双子は `\new StaffGroup << … \new GrandStaff << … >> >>`**（`EmitStaffGroup` 再帰）。**パーサは bracket の本体でだけ・1 段だけ受ける**（grandStaff の中の grandStaff・staffGroup の中の staffGroup・2 段目は LYS6011）。**LSP は bracket の本体に `BracketGroupBlock`**（`grandStaff` を足す）。⚠️⚠️ ★★★ **危険はまた「子孫を歩く読み手」**: `LilyPondExporter.RenderRows` は `DescendantNodes` を歩くので、**ガード無しでは双子に grandStaff を 2 度書く**（段階 A の `RenderSpecParser` と同じ形）⇒ ガードを置いた。**毒 3 本（1 本ずつ・出力を見て・ハッシュで戻した）**: ⑴ 双子のガード → `TheTwinWritesTheGrandStaffOnce` 赤 ⑵ grouper 規則 → 境界 10.5 の 2 case 赤 ⑶ spec のガード → 7 本赤。⚠️ **1 度目の毒は `| Out-Null` で出力ごと捨てて何も見ていなかった**＝やり直した（**毒の結果は目で見るまで無い**）。**射程は実測**: **snapshot 0 枚・台帳不変**（入れ子の本はコーパスに 0 冊）・既存の test は `AGroupInsideAGroup` を「残りの group-in-group は断る」4 case に、LSP の theory を brace／bracket に分けただけ。**段階 B の数**（`-DiffBase a66f68cb`）: **full `scratch/p377/run14.trx` 8500 / 2 / 4 / 8506**（+16＝`NestedGrandStaffTests` 12・拒否 theory 3・LSP 1。**赤 2 本は生成物の行ずれ**＝その後に再生成、ソースは毒の復元でバイト一致のまま）・Core 0 警告・台帳 843 点／総和不変・snapshot 動いたもの 0・追跡 `.lys` 599 不変。

★★★ **⒀ 第 9 便＝入れ子を「書いたとおり」に一般化（ユーザー決定: ①「staffGroup の中の grandStaff は中かっこが左…意図通り？」→ 調べて LP・Dorico・MuseScore とも*子は親の外（左）*と報告 ②「数が少ないグループを左に寄せる方が自然」③「**なるべく、ユーザーが書いた lys テキストを尊重する形で動作できるようにしたい**」→ 案「どの group もどの group の中に・深さ無制限・書いた順に描く」を出して「良い。それで進めて」）**。★ **② と ③ は矛盾しない**: 子は親の譜の部分集合＝段数は必ず子 ≤ 親⇒「子を外に」＝「少ない方を左」（同数の時だけ書いた順で決まる）。**LP で先に測った**（`scratch/p377/nest/deep.ly`＝`-dbackend=null`・indent 0・10 冊／`sd3,6,7,8,9.ly`＝svg で span bar）: ⒜ **位置＝各 delimiter は*親の*インク左端に対して bracket 0.8・brace 0.3**（最上位は縦線に対して）・**同じ深さの兄弟は列に揃わない**（D1: 深さ 1 の brace −1.61 と深さ 1 の bracket −2.56..−2.11 の子 brace −2.86）・**brace の中の bracket は brace の幅（高さ依存）の外**（D4: −1.49−0.8）⒝ **間隔＝最内のグループが同じなら 9・違えば 10.5**＝**第 8 便の grouper 規則がそのまま全深さで正しかった**（コード不変・コメントだけ）⒞ ⚠️ **span bar＝「両方の譜を含むどれか 1 つのグループが grandStaff／staffGroup なら通す」**＝**choirStaff の中の隙間も外の grandStaff／staffGroup が通す**（sd6・sd7）。**実装**: **model＝`OuterStaffGroup` に親 `Outer`**（鎖・`SelfAndOuters`）・`StaffGroup.Outer` は最内。**`RenderSpec.BuildStaffGroups` は再帰の `Leaves`**（入れ子を持たない group は今までどおり leaf 1 個）。**`OuterRuns` は全 outer を外側から順に**。★ **delimiter の X は描く時に 1 箇所で**（`SharedRenderer.PlacedDelimiters`＝親から順に置き、子は親のインク左端に対して `SystemStartBracketCentreAgainst`／`SystemStartBraceRightEdgeAgainst`）＝**親が brace だと幅が高さで段階的に変わる**ので、頁の再配置の後にしか決まらない（layouter の入れ子 leaf の X は仮置き・最上位 leaf は layouter の値のまま＝入れ子の無い本は同じ式）。楽器名の左端・描画・span bar が同じ関数を読む。**span bar は「最内の共通グループが 1 回だけ描く」**（choir の leaf は外に通す group があれば自分で描く）・`HairpinEngraver` も鎖を見る。**キャッシュ鍵 2 本は鎖全体**。**パーサはどの group 本体でも 3 語を受ける**（LYS6011 の文面・回復ループの止まり語）・**「2 本未満の group は落とす」は段数で数える**（`staffGroup { grandStaff { a b } }` は 2 つの delimiter を描く＝LP も同じ）。**LSP は `BracketGroupBlock` を消し、グループ本体の補完は常に 7 語**。**GRAMMAR の `StaffGroupBody`**・§2 E の範囲外（残るのは LP の `SystemStartSquare` の形だけ）。**数値は全部 LP の実測と ±0.02 で一致**（`NestedGroupDepthTests` 13 本＝D1/D2/D4/D5/D9/D10/sd3/sd6/sd7/D8・双子）。**毒 3 本（1 本ずつ・出力を見て・ハッシュで復元）**: ⑴ 子の X を親でなく縦線に対して → 8 本赤 ⑵ span を group 自身だけで判定 → sd7 の 1 本赤 ⑶ 段数でなく member 数 → 2 本赤。★ **射程は実測**: **snapshot 動いたもの 0・台帳不変**（入れ子の本はコーパスに 0 冊）・既存 test は拒否 theory を「受ける」に・LSP の期待 3 本に 3 語。**数**（`Session-Check -Build -Test -Scratch p377 -DiffBase 3bf509fa`）: **full `scratch/p377/run15.trx` 8519 / 0 / 4 / 8523**（+17＝`NestedGroupDepthTests` 17 case。第 8 便 run14 の赤 2 本＝生成物は再生成で解消）・Core 0 警告・**台帳 843 点／ss 非ゼロ 210／総和 22.584587806 不変**・snapshot 249・追跡 `.lys` 599 不変・§7.5 は未 commit で 0 と出る。★ **LSP は配布する**（Core と LSP が動いた）⇒ **ユーザーは Reload Window が 1 回要る**。⚠️ **第 10 便（判定で着手）＝commit 後の §7.5 が「Core `+` 280 行／REF 0／OWN 0」＝監査対象だった**: 鎖と span bar の規則は **LP から導出した（§7.6 ⒝）のに REF が無かった**（実測のコメントだけ）⇒ **LP のソースを読んで住所を確かめてから** REF を 4 本（`system-start-delimiter-engraver.cc:108-114 set_nesting_support`＝子の delimiter の支えが親の delimiter／`span-bar-engraver.cc:61-66 acknowledge_bar_line`＝子孫の小節線を拾う／`engraver-init.ly:546-549` ChoirStaff の remove）。**コメントだけ・出力不変**。
...
★★★ **⑴ 開始時の full は赤 1 本だった＝`DeadCitationsDoNotGrow` 652／天井 651**（`scratch/p376/run1.trx` 8454 / 1 / 4 / 8459）。**畳み（squash）は起きていない**（HEAD・未 push は第374 末の記録どおり）。⇒ **原因は第374 第 26 便の §1 ㉓**＝「終了時 HEAD」に**その doc commit 自身の SHA を書いてから amend した**ので、書いた SHA が到達不能になった（reflog で確認）。⚠️⚠️ **計器を 1 度間違えた**: `git cat-file -e` は reflog の残骸を*解決できる*ので「死んでいない」と出る——**テストの定義は到達可能性**（`merge-base --is-ancestor`）で、class remark が既にそう書いている。⇒ **直し方は天井を上げずに文を張り替えた**（㉓ の「終了時 HEAD」を「この §1 を書く doc commit・親は `d7ee87b6`」に）。★ **規則: §1 に自分の doc commit の SHA を書かない**（記憶 `reference_lilysharp_squash_orphans_doc_citations` に amend 版として追記）。

★★★★ **⑵ 第374 ㉔⒜′ を閉じた＝string preset が tab で自分の弦を使う**（commit `b6df1676`）。**射程を先に数えた**: ディスクの `.lys` 27,049 冊中、**弓奏 preset（violin／viola／cello）＋ tab を書く本は 0 冊**（**陽性対照**＝同じ述語で bass／guitar／ukulele ＋ tab は追跡分 16 冊）＝**既存の絵は 1 枚も動かない**（台帳・snapshot・追跡 `.lys` 全部不変）。⚠️ **ただし射程とは別に、`violin → null`「bowed: not a tab instrument」を固定した test が 2026-06-27 から在った**（**violin の調弦が表に無かった時代の主張**。contrabass は既に `bass` 調弦に繋がっていて規則は不整合）＝**言語の判断なのでユーザーに訊いた**。⇒ ✅ **ユーザー決定「mandolin/banjo も足す」**: `GetTuning` に violin／viola／cello／mandolin（→ LP の mandolin-tuning＝violin の弦）／banjo（→ `banjoopeng`・**LP に素の banjo 調弦は無い**）を足し、**`KnownInstruments` に mandolin・banjo**（補完にも出る）。**clef は推測せず MuseScore の instruments.xml（3.x）を読んだ**＝**mandolin は clef G・移調なし／banjo は G8vb（guitar と同じ `treble_8`）**・MIDI の timbre は既存の部分文字列判定が既に拾っていた。

★★★ **⑶ 網は 3 層で主張し、毒で確かめた**: **page**（`RenderSpecParser`＝`StringPreset_SuppliesItsOwnTabTuning` 5 case）・**双子**（`stringTunings = #violin-tuning`）・**`GetTuning` の表と補完**。**violin の腕を `null` に毒すると*ちょうどその 4 本*が赤**。★ **実アプリでも確かめた**（`scratch/p376/tab-*.lys`・`data-pos` 伏せ）: **`instrument violin` ＋ tab は `tuning violin` ＋ tab とハッシュ一致（線 4 本）、guitar 対照（6 本）と不一致、banjo は 5 本**。

⚠️⚠️⚠️ ★★★ **⑷ 毒の後始末で*毒入りのバイナリが残った***——**退避を `Copy-Item` で戻すと mtime が退避時の古い値**なので、**続く incremental build が compile を飛ばした**。症状は 2 つ: **lysc の violin が guitar 対照とバイト一致**（unit test は毒の前に緑だった）／**その後の full の赤 4 本が*毒の赤とちょうど同じ 4 本***。⇒ **`--no-incremental` で建て直して両方消えた**。★ **規則: 毒の後始末の build は必ず `--no-incremental`**（記憶 `reference_lilysharp_dotnet_incremental_stale` に追記）。⚠️ **もう 1 つの自分の罠**: **page 側の新 test のフィクスチャで part 名を `p` にしたら 5 case とも Guitar**＝**`p` は強弱記号の綴り**で、part が束ならず preset が届かなかった（`str` に変えて緑）。✅ **第 2 便で測った＝穴ではない**（`scratch/p376/name-p.lys` 対 `name-str.lys`・判定で着手＝下 ⑹）: **言語は最初のエラーで正しく断る**（`LYS0002`「a reserved word — pick another name」）。**絵は「パースできた部分」から書かれ、guitar 対照とハッシュ一致**。⇒ **黙っていたのは私の計器**＝`InstrumentPresetTests.TabTuningOf` が parse エラーを見ずに spec を読んでいた。⇒ **helper が「本がパースすること」を先に主張するようにし、`part p` を戻す毒で*ガードの文面で* 5 case 赤を見た**（commit は下 ⑸）。★ **既存フィクスチャの `score { }`（名前なし）は validator の指摘で parse エラーではない**のでガードを通る。

★★★ **⑸ 数**: **開始時**（`Session-Check -Build -Test -Scratch p376`）: HEAD `d1c82c3a`・未 push 60・木 clean・未追跡 0・**台帳 843 点／ss 非ゼロ 210／総和 23.216135909／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249・追跡 `.lys` 599**・Core 0 警告・**full `run1.trx` 8454 / 1 / 4 / 8459**（上 ⑴）。**終了時**: HEAD＝この §1 を書く doc commit（親 `35ffd089`＝第 2 便のガード。第 1 便の出荷物は `b6df1676`）・未 push 64・**台帳・snapshot・追跡 `.lys` は全部不変**・Core 0 警告・**full `scratch/p376/run4.trx` 8469 / 0 / 4 / 8473**（第 2 便末＝**本数不変**・ガードは test ではなく主張）・第 1 便末 `run3.trx` 同数（+14＝`GetTuning` theory 1→7 case で +6・page theory +5・mandolin/banjo の clef +2・双子 +1）。⚠️ `run2.trx` は 8465 / 4 / 4＝**毒入りバイナリで走った run**（上 ⑷）。§7.5: **Core の `+` 行に REF/OWN 0**＝足したのは switch の腕 7 本と一覧 2 語で**数値定数も LP の式も無い**（出典は腕の隣のコメントに LP の `ly/string-tunings-init.ly` と MuseScore を書いた）。§7 9: **計算は足していない**（既存の switch に腕を足しただけ）。⚠️ scratch は **`p376`**。⚠️ §7 3.5: 第373 を `-Archive 373` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝**push はユーザー（未 push 62）**。★ **LSP は配布した**＝`tools/Deploy-Lsp.ps1` が `0.6.0+b6df1676` を `yotsuda.lilysharp-0.5.0-win32-x64\server` へ（95 ファイル byte 一致）⇒ **ユーザーは Reload Window が 1 回要る**（package.json は無変更）。

⇒ ★★★ **⑹ 次の一手＝第374 ㉔ の残り**: ✅ **⒜′ は上 ⑵ で閉じた**。**⒝ 摂動網の残り**（paper 8 鍵の engine 読解・chord quality 35・`@` の引数つき形）／**⒠ drummap の `${1:hh}`＝UX 案件**／**⒡ 第373 ⑼⒜（probe 24 冊の「量を測ったか／問いに答えたか」の仕分け）＝先頭級のまま**／**⒢ 第369 ⑸⒝・第368 ⑺⒝ の並び**はそのまま。★ **判定の記録**（ユーザーの常設指示「次便は、このセッションでやる方が有利なら着手して…」）: **第 1 便はユーザー指示「着手して」＋ユーザー決定（⒜′ の範囲）**。**第 2 便は判定で着手＝上 ⑷ の `part p`**——**この便が置いた札で、症状（page test の 5 case Guitar）も計器（`p376` の tab 本と線数・伏せハッシュ）も温まっていた**。**結果は言語の穴ではなく*この便が足した網の計器の穴***＝**「自分が今作ったものの検証が独立か」は作った便にしか問えない**（第374 第 19 便と同じ形）。**着手しなかったもの**: ⒡ は読むだけで道具の温度が要らない／⒝ は別 regime／⒠ は UX の判断＝**次のセッションでやっても損しない**。**第 3 便は判定で*着手しない***——**この便が作った札は全部閉じた**（`part p` は第 2 便・pwsh MCP の二重起動はユーザーの保守セッション向けに `scratch/p376/pwsh-mcp-feedback.md` へ）うえ、**残る ⒡⒝⒠⒢ のどれにも `p376` の道具（tab 本・preset の網・毒の手順）が効かない**。⇒ **次便は ⒡ から**（読むだけ・LP 不要）。
