...
最終更新 第414セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `96c9fc52`・**未 push 6**＝第412 の 1 本＋第413 の 3 本＋その前の 2 本・**full `sessions/p414/run1.trx` 8720 / 0 / 3 / 8723＝第413 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 128 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、**第412・第413 が「次便が見てもそれは欠陥ではない」と予告したもの**）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 412`＝35 行 7,746 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」が名指していた ★★★ **次の梃子＝`_skylines`**（第413 が「augment の天井そのもの」と書いた店）を採った。★ **ベンチは着手前に AskUserQuestion で静かな窓をもらった**。根拠は割当バイトと**回数**（CLAUDE-OPERATIONS §2）。道具は pwsh MCP（Bash 0 回）:

★★★ **⑴ 第 1 便＝`_skylines` も鍵の床に着いていた。しかし島は打鍵の 6.02% あった。** 継ぎ目 ZZ414 で miss を 4 区分に割った（**区分は構成上これで尽きる**・実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・計器 Lab `sessions/p414/Zz414Probe1.cs.txt`）。**hits 51,708 / shifted 27 / misses 1,871＝hit 率 96.5% は第413 第 2 便の三つ組と 1 件も違わない**（＝継ぎ目は測る対象を乱していない・打鍵 9,929,762 B 対 9,929,334 B）:

| なぜ miss したか | 件数 | miss 比 | compute B | 打鍵比 |
|---|---|---|---|---|
| `editedInk`（編集が触った小節を含む＝床） | 1,765 | 94.3% | 1,039,687,768 | **5.67%** |
| `evicted`（上限 1024 が落とした） | **0** | 0.0% | 0 | 0.00% |
| `heightOnly`（同じ内容・別の `systemHeight`） | **0** | 0.0% | 0 | 0.00% |
| `newRun`（行分割が動いて未経験の小節列） | 106 | 5.7% | 64,450,712 | 0.35% |

⇒ ★★★ **鍵で取り返せるのは 0.35% だけ**＝`systemHeight` を鍵に入れている代償は**ちょうど 0**、**1024 の上限は一度も噛んでいない**。⇒ ★★★ **だから問いは「miss を減らす」から「*miss の中身は何か*」に変わる**（1 miss 590,132 B・1.01 miss/打鍵）。

★★★ **⑵ そして miss の 71.8% は skyline を建てていなかった**（継ぎ目 ZZ414B＝compute lambda の中を段別に。**段は全体の中に入れ子**なので**負の残りが二重計上の合図**＝第412 ⑺）:

| 段 | miss 比 | 打鍵比 |
|---|---|---|
| **`edgeBeams`（縁の譜の梁を量子化し直す）** | **71.8%** | **4.32%** |
| `buildSystemSkylines` | 28.2% | 1.69% |
| └ `resolve`（EndBatch の解決） | 19.1% | 1.15% |
| └ `inkFirst` / `inkLast` / `staffSymbols` / `rows` | 6.6 / 2.2 / 0.2 / 0.0% | 0.40 / 0.13 / 0.01 / 0.00% |
| 残り（全体 − 段・**2 段とも**） | 0.0% | **0 と +239,488 B** |

★★★ **⑶ 二度手間は「数えた」＝推論していない。** `LayoutBeams` は**1 打鍵に 3.5 回**走り（6,528 回／1,848 打鍵＝**打鍵の 9.35%**）、`(staff, system, 先頭小節, 長さ)` で数えると **6,528 中 3,264＝ちょうど 50.0%** が**その打鍵が既に置いた鍵の二度目**で、**791,131,392 B＝打鍵の 4.31%**。⚠️⚠️ **その「同じ答えか」の 1 回目は*計器のほうが*間違えた**＝`==` で比べたら「3,264 中 360 しか等しくない」と出た。**`BeamLayout` は `ImmutableArray` を持つ record で、record の等値はその配列を*参照*で比べる**。数値を要素ごとに比べ直すと **3,264 / 3,264 が一致・differ 0**（§5.3 へ）。

★★★ **⑷ 実装＝部屋が既に作った梁を、部屋から持ち出すことにした**（`MultiStaffLayouter.StaffSkylineSet.Beams`）。**これは新しい発明ではなく、この型が slur・tie・tuplet bracket に対して既にしている move**（型自身の註が「部屋から*持ち出す*のがこの型の存在理由」と書いている）＝**譜のスカイラインはその梁から建てられている**ので、`LayoutEngine` の `EdgeStaffBeams` は**量子化の 2 回目ではなく list の添字**になった。⚠️ **`ShiftStaffSkylines` は梁も再刻印する**（`ShiftBeams`）。**system 刻印は*わざと*動かさない**＝この店に system 刻印の関数は無く、部屋の梁の唯一の読み手（silhouette の種）は `SystemIndex` を読まない。**描かれる梁**は注釈パス自身の店（`GetOrComputeStaffSystemBeams`・そちらには shift が在る）から来る。理由は shift の場所に書いた。

★★★ **⑸ A/B（同じ harness・同じ 231 冊 × 8 forward 打鍵）**:

| | 前 | 後 | 差 |
|---|---|---|---|
| 打鍵あたり B | 9,929,762 | 9,500,938 | **−4.32%** |
| `LayoutBeams` 回数 | 6,528 | 3,264 | **−50.0%**（うち二度目 3,264 → **0**） |
| miss の中の `edgeBeams` | 793,194,248 B | **0 B** | |
| 島（skyline miss 全体） | 打鍵の 6.02% | 打鍵の 1.77% | |
| skyline store の hit/miss | 51,708 / 27 / 1,871 | **同一** | **変わらないのが正しい**＝miss を*安く*した |

★★★ **⑹ 出力同一は 3 つとも走らせた**（RULES §5.1）: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（⚠️ baseline は第413 が `8474de18` で取ったもの＝**第413 の変更も同時に再確認された**）／⑵ **全ページの SHA-256**＝**231 冊 × 8 打鍵で差 0 行**（`hashes-before.txt`／`hashes-after.txt`。before は製品変更だけを `git stash push -- <5 パス>` して取った＝ユーザーの未 commit `tools/*.ps1` には触っていない）／⑶ suite 全緑・snapshot 0 枚。

★ **⑺ 網 2 本**（`SystemLayoutCacheTests`・**毒を入れて赤を見た＝毒ごとにちょうど 1 本**・RULES §5.4）＝**部屋に乗った梁は shifted hit で再刻印される**と、**silhouette の縁の梁は部屋から来る**（`LayoutEngine.EdgeBeamSource` が liveness の計器）。⚠️ **後者が要るのは fallback が*黙っている*から**——部屋が梁を持たなくなっても**絵は同じで全網が緑のまま**、量子化だけが 2 回走る（打鍵の 4.31%）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: **`_skylines` の鍵は ✅ 閉じた**（取り返せるのは 0.35%）。**残る島は 2 つで、どちらも「miss を安くする」側**: ★★★ **⒳ `resolve`（`VerticalSkyline.EndBatch`）＝打鍵の 1.15%・miss の 19.1%＝いま miss の中で最大**（25 up / 19 down buildings を 3.9 小節から解決している。**未着手・未計測の内側**）／★ **⒴ `inkFirst`＋`inkLast`＝0.53%**／★★★ **⒵″ `h6e.everythingElse` の残り 20 家族に名前を付ける**＝**打鍵の 5.1% が無名**・**同じ harness に家族 1 つにつき継ぎ目 1 本**（`Zz412Probe1` は Lab に在る）＝**いま最も安く測れる島**／★ **⒴″ `b.scriptsFingerings`（`ComputeFingeringsAndScripts`）は予備と最終の 2 人合わせて打鍵の 5.0%＝名前の付いた最大の葉**・未分割／★★ **⒴′ 2 度目の注釈パスを要らなくする**（打鍵の 7.7%・第411 ⑶）／⒜ **R13⒝ の実機確認**（第404 ⑵＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械）＝上限は red を 14〜20% だけ実体化＝fingbeam1k で 26.2 MB → 約 10 MB。⚠️ **未着手・未見積もり＝validator ではなく土台の変更**／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った（採るならそれを知って採る）／**R13⒦ ⒤ ✅ 第410 が「測って、やらない」で閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅ 第407 が測って「やらない」と決めた**／**⒟ ✅ 第407 で閉じた（R2R）**／LP 双子が要る R7〜R11／**R15 ✅ 削除は第407 で完了**（残り＝「2 綴り」一族のみ＝統合・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑻ 終了時**: commit 2 本（code `b2a0939c`・docs 1 本＝**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 1 本・**最終 full `sessions/p414/run4.trx` 8722 / 0 / 3 / 8725**（§1・RULES を書いた*後*の最終ツリーで取った。この 1 文だけが後から入った＝run4 の緑はこの文を除いて同一のツリーのもので、文書の網は入れたあとに回し直して緑）（⚠️ **第413 最終の 8720 / 8723 から +2＝今便の網ちょうど 2 本**・計器と継ぎ目は全部外してあり `Zz414`／`ZzOn`／`ZzStage` の grep 0 件）・§7.5（対 `96c9fc52`＝スクリプトの出力）**Core `+` 89 行／REF 0／OWN 0**（★ **数値定数を 1 つも足していない**＝足したのは list 1 本と添字・LP 由来の式なし）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。⚠️ **棚卸し `docs/APPROXIMATIONS.md` と `audit/magic_constants.csv` は再生成した**＝**行番号が動いただけで行の増減 0**（手順どおり `-U0` の diff を読んでから回した）。§7 4（恒久知識）＝**RULES §5.3 に 3 項**（「**鍵の床に着いた memo を見たら、次に割るのは*理由*ではなく*中身***」＝床 94.3% なのに島は打鍵の 6.02%、その 71.8% は skyline ではなく梁だった／「**`record` の等値は `ImmutableArray<T>` を*参照*で比べる**」＝値同一の 3,264 件を「2,904 件が違う」と刷った計器／「**『同じものを二度やっている』は鍵で数えて初めて主張になる**」＝ちょうど 50.0%）。計器と全文は Lab `sessions/p414/`（`summary.txt`・`why-skyline-misses.txt`・`hashes-{before,after}.txt`・`Zz414Probe{1,2}.cs.txt`・`zz414-seam-and-change.diff.txt`）。§7 3.5 は**セッションの頭で**済（第412 を `-Archive 412`＝35 行 7,746 chars）⇒ **HANDOFF は 432.9 KB → アーカイブ後 425.2 KB → 今便を書いて 444,194 B＝天井 450,000 まで 5,806 B しかない**（今便のブロックは約 19 KB＝表を 3 枚置いたぶん厚い）。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 413` を回すこと**（**第413 のブロックが丸ごと戻る**）（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で 3 回使って残っていない。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第414 の欠陥ではない。
