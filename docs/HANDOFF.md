...
⇒ ★★★★ **次の一手**: ✅ ⑺ mark（Lily# は正しく予約）・✅ ⑻ 括弧（頁に効かない・番人 4 点）・✅ ⑾ 静かな段の 2.311（＝小節番号・LP 基線が消していただけ）。**段ごとの項は*全部*合った**（⑾⒠）。**残るのは 1 頁目の鎖の解そのもの、1 つだけ**。
**⑴ Boogie「both」の 1 頁目の*鎖*を LP と並べる**: LP は題＋7 段で最後の段の下に slack 9.28 ss（8 段目に要る ~23 ss には遠い＝LP は確かに 8 段を載せられない）、Lily# は同じ段の高さで題＋8 段を載せている。**段の高さでは説明がつかない以上、差は頁の鎖の解＝ばねと床と題の行の並び**にしかない。**やること**: `PageLayouter.PositionSystemsOnPage` の解（各ばねの長さ・力・どの床が縛ったか）を 1 頁目について吐き、**LP の `yoff`（`scratch/p338/bare/lp.out` の PROBEG 行に全段ぶん在る）と 1 段ずつ差し引く**。⚠️ **題の値付けと括弧はもう疑わなくてよい**（台帳 8 点が exact）。⚠️ **インクを比べるときは `probe-settings.ly` を混ぜず `scratch/p338/lp-bare.ps1`**（⑾⒞）。⚠️ **Lily# の段の高さを LP と並べるときは `ToFirst` を足す**（⑴）。
