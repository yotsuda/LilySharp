...
最終更新 2026-08-01（第64セッション＝**exporter の穴を 2 つとも塞いだ**——
⑴ **度数和音（`<1 3 5>`・`<d 3 5 7,>`）を具体的な音高に解決して書く**（`9c924efc`）、
⑵ **ドラムキットを `\drummode` ＋ `DrumStaff` で書く**（`508b3849`＝⑦）。
**Core の描画は 1 行も触っていない。**
★★★ **これで「音楽を落とす」exporter の穴はゼロになった**——**残る警告 159 件は全部 装飾**
（⑤ に実測・**数え方つき**）。
★★★ **双子 199 本のうち 195 本が byte 不変**・**動いた 4 本が度数 3 冊＋`drum-groove`**
（② の補正が度数和音の無い本では恒等的に 0 だから＝**設計がそう作ってある**）。
★★★ **`drum-groove` は初回で対になった**（⑦・**梁 5 本が多重集合で一致**）＝
**比較できる本 39 → 40・一致 31 → 32**。
⚠️ **引継ぎの予告「⑼ を閉じれば比較できる本が 1 冊増える」は外れた**——`chord-octave-marks` は
**4分音符の和音だけで梁が両側とも 0 本**（④）。**増やしたのは度数ではなくドラムだった。**
**作業は `9c924efc`（度数）＋`96459386`（⑥ の 2 件）＋`508b3849`（ドラム）**
（＋handoff commit 2 本）・**未 push 26**・
テスト **3750 passed / 0 failed / 3 skipped**（**+11 は exporter のテスト**）・
