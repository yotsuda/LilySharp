...
最終更新 第384セッション（2026-09-14〜15）＝**入り方は型と違う**（ユーザーが `scratch\ベースタブLy\tab-fret.lys` の弦選択の誤りを報告し、対話で仕様を詰めた・§0 の裏取りは走らせていない・開始時 HEAD `c6ac2369`）。道具は pwsh MCP。**結果＝tab の弦選択をフレーズ全体の運指計画（Viterbi）に作り直した（ユーザー承認・製品 commit 4 本）**。**骨は 4**:

★★★ **⑴ 入ったもの**: `1549c837`（1 音ずつ決める貪欲版 v2＝ユーザー仕様の基準点）→ `17d49843`（評価の土台: `audit/tabfingering/` にユーザー実本 7 冊のバイトコピー＋`TabFingeringExpectationTests`＝ユーザーが指定した 21 箇所）→ `31dbea62`（`Core/Tablature/TabFingeringPlanner.cs`＝状態 (弦, フレット, 人差し指の位置)・移動は時間で重み・ストレッチ・弦とび（オクターブの形は除く）・スラー・タイ・高さ・開放弦。貪欲版と `Tunings` の手の位置ヘルパー群（`HandPosition`・`MoveCost`・`SkipCost`・`Place`）は削除・snapshot 5 枚は fret 数字だけ）→ `a1161e78`（静的最適化＝平坦な状態配列・音ごとのシフト表。**コーパス・fixture・audit の 158,942 音の弦がバイト一致**・Release A/B で resolve 10〜30% 減・確保 4〜6 割減）。**LSP 配布済み（`0.6.0+a1161e78`）**。

★★★ **⑵ 重みは機械探索**（Shift 1・FirstFret 0.75・MaxShiftFrets 6・Stretch 0.5・Skip 1・High 1.5・HeightPerFret 0.1・Free 0.35・Slur 5・OpenPerHandFret 0.05）。⚠️⚠️ **期待値 21 箇所だけで探すと過学習する**＝最初に選んだ重みは第 1 ポジションの曲を 5 ポジションで、ギター fixture を 19〜22 フレットで弾いた（snapshot の fret 数字列を before/after で比べて発覚）。⇒ **repo の tab fixture 41 冊で「\N の無い、第 1 ポジションに収まる小節で 4 より上を弾いた音数」を護りに併用**（435→1）。**重みを動かすときは両方を測り、snapshot の数字列を読む**（`TabFingeringWeights` の remarks）。結果: 21 箇所中 19・**Known 2＝tab-fret section B 2／4 小節目**（休符の後は第 1 ポジションの方がずらしもストレッチも少ない）。

★★ **⑶ ユーザー決定（2026-09-14）**: 「さっきより良くなっている。奏者の好みもあるし、これ以上追及しなくても良い」＝**運指の自動提案はここで打ち止め・好みは `\N` で書く・蒸し返さない**。perf は旧貪欲版比で resolve 2〜6 倍（1000 音で 1〜3 ms・collect 全体に対して小）→ ⑴ の最適化で縮めた。⚠️ ベンチは全冊を先に温める（先頭の本が JIT の途中で遅く出て、初回は順序で逆転した）。

★★ **⑷ 数と次の一手**: **終了時**: HEAD＝この §1 を書く commit（親 `a1161e78`）・未 push 24（`origin/master` は `cc16687a`）・**full `scratch/p385/trx/p385-perf.trx` 8559 / 0 / 4 / 8563**（第383 末 8538 から +21＝期待値テストと tab 単体テストの入れ替え）・§7.5（`-DiffBase c6ac2369`）: **Core `+` 595 行／REF 0／OWN 7**（7 本とも tab 運指の LILYSHARP-OWN＝LP の `determine-frets-and-strings` とは意図的に別模型・§3）・台帳 851 点（不変）・snapshot 249 枚（枚数不変・5 枚の中身）・**追跡 `.lys` 608**（+7＝`audit/tabfingering`）・§7 3.5: 第382 を `-Archive 382` で ARCHIVE へ（済）。計器 `scratch/p385/`（`ZzP385Tab{Bench,Dump}ProbeTests.cs.txt`・`search*.txt`）（A/B の worktree 2 つはユーザー指示で削除済み・before は commit `17d49843`／`31dbea62` から建て直せる）。⇒ **次の一手は第383 ⑸・⑼ のまま**（和音行 −0.0131 以降）。**次便判定「着手しない」**（常設指示）: 残る候補は和音行の regime で、この便の tab 運指の道具・文脈は効かない＝次のセッションで始めても損が無い。

## 以下は第383セッションの経緯

最終更新 第383セッション（2026-09-14）＝**入り方は第298〜第382 の型**（ユーザーは `docs/HANDOFF.md` を読ませて「作業に着手して」だけ）。**§0 の裏取りを走らせた**（`Session-Check -Build -Test -Scratch p384`）。道具は pwsh MCP。**着手したのは第382 ⑸ の筆頭＝a2 の −0.11**。**結果＝第 1 便は機構を特定（3 つの量の和・LP の式で全部説明がつく・製品 0）、第 2 便（ユーザー承認）は台帳 3 点を起票して移植＝3 点とも残差 −0.000010（ink 底の桁）・動いた 8 冊のラベルは全部 LP と一致**（⑷）。**骨は 5**:

★★ **⑴ 開始時の数は第382 末と一致**: HEAD `e8f42c11`・未 push 9（`origin/master` は `cc16687a`）・木 clean・未追跡 0・**台帳 846 点／ss 非ゼロ 210／総和 22.584587806／count 点 180 うち非ゼロ 0／exact 671／OPEN 0・snapshot 249・追跡 `.lys` 601**・lysc は net10.0 だけ・build 0 エラー・**full `scratch/p384/run1.trx` 8532 / 0 / 4 / 8536**（RunInfos は SKIP 4 だけ）・`cc16687a` の CI は success。

★★★ **⑵ regime を 3 つに割って測った**（`scratch/p384/a2/`＝1 小節の `combinedStaff` 5 冊・`measure.ps1`＝`lysc svg`／`lysc ly --pin-fonts`／LP 2.26.0 を `cmd /c … < NUL`）。**Lily# は捨て計器 `Zz383A2ProbeTests.cs.txt`（`PartCombineLayouts.YUp` を全桁・計器は Tests から scratch へ戻した）、LP は SVG 4 桁**。値は a2 の baseline を譜の最上線から上へ（ss）:

| 本 | a2 を決めるもの | Lily# | LP | Lily# − LP |
|---|---|---|---|---|
| `F-floor`（`f,1`） | 譜の上端の床 | 0.5500 | 0.5830 | **−0.0330** |
| `A-stem`（`c2`）・`C-floor`（`f1`）・`D-head`（`e'1`） | 譜の上の符頭 | 2.038／3.538／6.538 | 2.078／3.578／6.578 | **−0.0400**（3 冊とも） |
| `A2-stem`（`c,2`＝bench の 1 音目） | 上向き符尾（最上線 +1.0・両エンジンで同じ） | 1.42351 | 1.5330 | **−0.1095** |

⚠️ **予測（各 `.lys` の header に先に書いた）は半分外れた**: 床は 0 ではなく −0.033、符尾は符頭の −0.04 とは別の regime。⚠️ **最初の 3 冊は音高を取り違えた**（`octave absolute` の `f` は bass 譜の加線 2 本上）＝名前は floor でも中身は符頭の regime。双子は同じ音楽を描いているので点としては有効。

★★★ **⑶ 機構＝LP の CombineTextScript の skyline は*箱*で、Lily# は*輪郭*で置いている**。LP の dump（`A2-stem-dump.ly`＝`after-line-breaking`・CombineTextScript は既定を持たないので何も置き換えない）: **文字の DOWN skyline は x 0〜2.594891（＝advance）で平らな −0.033010**、support は **NoteHead（箱・上端 0.545）と Stem（±0.065・上端 3.0）**。**出所**: `scm/define-grobs.scm:1077-1105` CombineTextScript は `vertical-skylines` を宣言しない → `lily/grob.cc:81-85` が既定 `Grob::simple_vertical_skylines_from_extents`（`lily/stencil-integral.cc:813`）＝extent の箱。**TextScript は `:3818` で `grob::always-vertical-skylines-from-stencil` を宣言する**＝輪郭。⇒ `OutsideStaffStacker.PlacePartCombineTexts` のコメントは「TextScript の 450 と同じ形」と書いて `TextOutlineSkylines.Place` を使う＝**LP に無い輪郭**。Lily# の "a2" の DOWN 輪郭は符尾の x（文字原点から 1.25〜1.38＝'a' の右の尾と '2' の隙間）で baseline より +0.02〜+0.15 上。
⇒ **差は 3 つの量**: ① **padding 0.04**＝LP は `side-position-interface.cc:265-370 aligned_side` で support（`part-combine-engraver.cc:102-119` が符頭と符尾を足す）に `padding` 0.5、Lily# は support pass が無く staff profile に outside-staff-padding 0.46（`OutsideStaffStacker.cs:2288` の LILYSHARP-OWN bridge が既に宣言している差）。② **箱の底 0.033**＝LP の床は `:323-330 include_staff` で staff extent を dim の最小値にして*箱の底*から 0.5（0.05＋0.5＋0.033）、Lily# は *baseline* を 0.55 に置く（refpoint 床 `:433-453` は LP では既に満たされて効かない）。③ **輪郭 0.0695**＝符尾の上だけ。**LP ＝ max(support 上端, 譜の上端 0.05) ＋ 0.5 ＋ 0.033 が 5 冊すべてを説明する**。

★★★ **⑷ 第 2 便（ユーザー承認「進めて良い」）＝台帳の起票 → 移植 → LP 照合まで閉じた**。**① 起票 `29f4580c`**（製品 0）: probe `audit/lp-geometry/probes/part-combine-text.ly`（PCF／PCH／PCS・LP の dump は SVG の 4 桁と一致＝陽性対照）・`LpGeometryProbes` に 3 冊・`RenderedGeometry.PartCombineTextUpFromStaffTop`・台帳 `part-combine.text.{staff-floor,head,stem}` を −0.033010／−0.040010／−0.109502 で OPEN（予測 0 を why に）。**② 移植 `8d09cab2`**: `PartCombineAnalyzer.AlignedSideBaselineUp`（`DynamicEngraver.ColumnSupportSkylines`＝符頭の箱・実の符尾・譜の床を読み直すだけ・advance の下の最高点 − ink 底 ＋ padding 0.5・refpoint 床）を `PartCombineLayout.AlignedBaselineUp` に持たせ、`OutsideStaffStacker.PlacePartCombineTexts` はそこから **`PartCombineAnalyzer.LabelBox`（extent の箱）** で 0.46 の pass、`InkAboveStaff` も同じ箱・同じ出発点。LILYSHARP-OWN の bridge は消えた。⚠️ 宣言した狭さ 2 つ（UNWATCHED）: support は第 1 voice の column だけ・beam 付き符尾は beam 無しの長さ。**③ 台帳 3 点とも −0.000010**＝**予測どおり**（残りは ink 底の桁: Lily# の "a2" −0.033 対 LP の Pango 輪郭 −0.0330096）。**④ snapshot 0 枚**・掃き（`scratch/p384/sweep/sweep.ps1`・base＝`29f4580c` の worktree Release）: combinedStaff の追跡 24 冊＋実コーパス 2 冊のうち **8 冊が動いた**＝全ラベルを双子で LP 2.26.0 と照合（`verify.ps1`・SVG 桁）: bench 変種の a2 1.42／1.43 → 1.53／1.54（LP 1.533）・床のラベル 0.55 → 0.58（LP 0.583）・**`pcomb-probe` の Solo 0.81 → 1.03（LP 1.0331＝移植前は −0.22 だった）**・`pctend-probe` の Solo 2.46 は不動で LP 2.4607。**実コーパス `bench.lys` の a2 1.43 → 1.54**。⚠️ **移植と無関係の既存の差を 1 つ見つけた**: `pcsil-b-probe` は LP が 1 小節目にも "a2"（Solo II の後）を刷り、Lily# は刷らない（base も同じ 3 ラベル・高さは全部一致）。**LSP は配布済み**（`Deploy-Lsp.ps1`・`0.6.0+29f4580c`＝未 commit の木から建てた・95 ファイル一致）＝**VS Code の Reload Window が要る**。以下は第 1 便の設計（実装はこのとおりに入った）:
★★ **⑷' 移植の設計**: `PlacePartCombineTexts` を aligned_side にする＝my_dim は [X, X＋advance]×[ink 底, ink 上] の箱／dim はそのラベルの時刻の符頭・符尾（両 part）の up skyline ＋ 譜の上端の床／＋padding 0.5／refpoint 床／そのあと outside-staff pass を同じ箱で 0.46。**読み手は 3 軒**: `OutsideStaffStacker.PlacePartCombineTexts`（置く）・`PartCombineAnalyzer.InkAboveStaff`（和音行のための予約＝既に箱だが床は baseline 0.55・padding 0.46）・`LayoutEngine.PagingSkylines.cs:334-344`（ink の extent＝置いた Y に従うだけ）。**予測**: 5 冊とも LP と一致・bench 変種の a2 1.42 → 1.53。**射程**: 追跡 `.lys` で `combinedStaff` を書くのは 24 冊（全部 `audit/lpreg`・fixture／snapshot は 0 冊）＋ユーザー実コーパス 2 冊（`bench.lys`・`beam-slur.lys`）。⚠️ **「その時刻の符頭・符尾」を support に組む器が在るかは未確認**（先例は `TrillSpannerEngraver.cs:298-331`＝aligned_side を pointwise に写し、support は note column）。⚠️ **台帳に a2 の点はまだ 0**＝§5.0 の起票が先（`RenderedGeometry` に part-combine の読み手が無い）。

★★ **⑸ 数と次の一手**: **終了時**: HEAD＝この §1 を書く commit（docs だけ・製品 0）・**full `scratch/p384/run2.trx` 8532 / 0 / 4 / 8536（不変・RunInfos は SKIP 4 だけ）**・§7.5（`-DiffBase e8f42c11`）: **Core `+` 0 行／REF 0／OWN 0**・台帳 846／総和 22.584587806・snapshot 249・追跡 `.lys` 601 は不変・未 push 10（以上は第 1 便末）・§7 3.5: 第381 を `-Archive 381` で ARCHIVE へ（済）。**第 2 便末**: HEAD＝この §1 を書く commit（親 `8d09cab2`）・**full `scratch/p384/run4.trx` 8535 / 0 / 4 / 8539（+3＝台帳 3 点・RunInfos は SKIP 4 だけ）**（移植直後の run3 は 8533 / 2 / 4 / 8539＝失敗 2 は棚卸し 2 本の行番号・`LILYSHARP_UPDATE_DOCS=1` で再生成し LF → CRLF に戻して `8d09cab2` に含めた）・§7.5（`-DiffBase e437afc1`）: **Core `+` 106 行／REF 6／OWN 0**・**台帳 849 点（+3）／ss 非ゼロ 213／総和 22.584617806（+0.000030＝3 点の ink 底の桁）／exact 671**・snapshot 249（不変）・追跡 `.lys` 601・未 push 13。★ **LSP は配布済み＝Reload Window**。⇒ **次の一手**: ⒜ **`pcsil-b-probe` の 1 小節目の "a2"**（LP は Solo II の後の f 対 f で a2 を刷る・Lily# は刷らない・移植前から・`PartCombiner` の文言の判定）／⒝ a2 の support の狭さ 2 つ（第 1 voice だけ・beam 付き符尾の長さ＝観測する点が無い）／和音行 −0.06・Intro −0.05（2 桁の限界）／第381 ⑷ の voice span の綴り／第379 ⑺ の並び／`CreateTimingSpring` の床／第379 ⑽ の範囲外。

★★ **⑹ 第 3 便＝判定「着手する」→ ⑸ の ⒜ を閉じた（ユーザー承認・commit `01b6d262`）**（常設指示「次便は、このセッションでやる方が有利なら着手して…」）。**着手の理由**: `scratch/p384/sweep` の掃き（`sweep.ps1`）と LP 照合（`verify.ps1`・双子）がそのまま使え、`PartCombiner` の標の組み立ては第 2 便で読んでいた。**原因**: `BuildMarks` がラベルの錨を「その瞬間に音符を持つ最初の part」で探した＝part two の solo の後の unisono（`scm/part-combiner.scm:994-1013` の Promoted）では part one は null voice で*刷られない*音符を持つ⇒錨が landed に無く保留⇒次の標（2 小節目の Solo）が上書き。LP は刷られた符頭に付ける（`audit/lpreg/pcsil-b.log`: a2 は part two の f・part one の f は NOINK）。**直し**: `PartCombiner.LandedAnchorAt`＝音符を持ち*かつ* landed の part。網 `PartCombineSilenceTests.AUnisonoAfterAPartTwoSoloHangsItsLabelOnPartTwosHead`（LP の Solo II／a2／Solo／a2）。**射程**: 掃き 26 冊（base＝`e4e92d12` の lysc を `scratch/p384/exe-head1` に写したもの）で動いたのは `pcsil-b-probe` 1 冊・4 ラベルとも LP と SVG 桁で一致・snapshot 0・台帳不変。棚卸し `APPROXIMATIONS.md` は行番号で再生成（run5 の失敗 1 はこれ）。**終了時**: full `scratch/p384/run6.trx` **8536 / 0 / 4 / 8540**（+1＝網・RunInfos は SKIP 4 だけ）・§7.5（`-DiffBase e4e92d12`）: **Core `+` 17 行／REF 1／OWN 0**（1 本＝`LandedAnchorAt` の出典・17 行はその関数と remark だけ）・台帳 849・snapshot 249・追跡 `.lys` 601・未 push 15・**LSP 配布済み＝Reload Window**。⇒ **次の一手**は ⑸ の ⒝ 以降。

★★ **⑺ 第 4 便＝判定「着手する」→ ⑸ ⒝ の片方（beam 付き符尾）を閉じた（ユーザー承認・commit `5c20fb64`）**。**着手の理由**: `scratch/p384/a2/measure.ps1` と LP の dump の型で 5 分で測れた未測の狭さ。**実測**（`E-beam.lys`＝C3 の 8 分 8 つを 1 本の beam）: Lily# 1.55／LP 1.583。LP の dump（`E-beam-dump.ly`）: **beam 付き Stem の support skyline は中央線から 3.05**（beam 無しは 3.0）。**直し**: `AlignedSideBaselineUp` に beam の地図（`DynamicEngraver.BuildBeamMembers`＝強弱・trill・ottava が読む同じもの）を渡す＝`LayoutEngine.PartCombineLayoutsOf`（score の beams・大域 staff 番号）と `InkAboveStaff`（staff 自身の beams・staff 0）の 2 軒とも。**台帳** `part-combine.text.beamed-stem`（PCB・LP 1.583010 を probe で実測）＝−0.000010・850 点。**射程**: 掃き 26 冊で 0 冊（beam 付き符尾の上のラベルを持つ本が無い）・snapshot 0・既存 5 プローブ不変。**残る狭さは 1 つ**（support は第 1 voice の column だけ・観測する点なし）。★ **ユーザーの問い（この便の途中）**: `pcsil-b-probe` の 2 つの a2 と Solo／Solo II は意図どおりか・書いていない文字がなぜ出るか⇒ **`combinedStaff` の自動の文字**（GRAMMAR.md 1263 付近: 同じ音＝a2・片方だけ＝Solo／Solo II・状態が変わった瞬間だけ・`layout { partCombineText off }` で消せる）で、LP の `\partCombine` と同じ 4 つを同じ位置に刷る（`audit/lpreg/pcsil-b.log`）と答えた。**終了時**: full `scratch/p384/run8.trx` **8537 / 0 / 4 / 8541**（+1＝台帳の点・RunInfos は SKIP 4 だけ）・§7.5（`-DiffBase a267808f`）: **Core `+` 33 行／REF 0／OWN 0**（0 本＝監査した: 既存の家 `DynamicEngraver.BuildBeamMembers`／`ColumnSupportSkylines` への指し直しで、出典はその家が持つ＝RULES §7.6 ⒟）・台帳 850・snapshot 249・追跡 `.lys` 601・未 push 17・**LSP 配布済み＝Reload Window**。

★★ **⑻ 第 5 便（ユーザー指示「台帳の点を足して」）＝残る「狭さ」は狭さではなかった＝番人の点を 1 つ足した（製品の出力 0）**。**調べて分かったこと**: ① LP の `Part_combine_engraver` は **Voice context に consist**（`ly/engraver-init.ly:406`・`\name Voice` は :359）＝support は*ラベル自身の voice* の符頭・符尾だけ。② Lily# でラベルが付くのは solo1／solo2／unisono だけで、その音符は `PartCombiner.PlaceItems` が **Solo／Shared voice＝常に slot 0** に置く（slot 1 は apart の voice Two だけ・文字を刷らない）⇒ `AlignedSideBaselineUp` が第 1 voice を読むのは **LP と同じ模型**。remark を書き直し（UNWATCHED −1）。**番人**: 台帳 `part-combine.text.own-voice-support`（PCV: part one `c8 d8`・part two `r8 g'8`＝"Solo" の advance の中に voice Two の高い符頭）＝LP は衝突 pass の 0.46 でだけ避ける・全 voice を support に入れる誤りなら +0.04（padding からの導出・実行はしていない）。**Lily# 7.538000／LP 7.5381**（残差 −0.000100＝SVG 4 桁の読み）。⚠️⚠️ **probe の読み方を直した**: `part-combine-text.ly` の dump（after-line-breaking）は**衝突 pass の前**の値＝PCV で 2.078010 を出した（記憶 `reference_lilypond_dump_y_offset_is_pre_collision` の罠をもう一度踏みかけた）。**台帳の値は SVG で読む**と header に明記。同じ SVG で既存 4 点も 4 桁で一致＝陽性対照（衝突の相手が無いので dump と同値だった＝*偶然*）。**終了時**: full `scratch/p384/run9.trx` **8538 / 0 / 4 / 8542**（+1＝台帳の点・RunInfos は SKIP 4 だけ）・台帳 851 点／ss 非ゼロ 215／総和 22.584727806（+0.0001＝SVG 読みの桁）・snapshot 249・ページ出力 0（Core は remark だけ）・★ **LSP の再配布は不要**。⇒ **a2 の縦は台帳 5 点で閉じた**。次の一手は ⑸ の「和音行 −0.06・Intro −0.05」以降。

★★ **⑼ 第 6 便＝判定「着手しない」（測っただけ・製品 0）**。**測った**: a2 の移植の後に bench 変種 `audit/lpreg/combined-staff-chord-row-label-names.lys` を全桁で（捨て計器 `scratch/p384/leg6/Zz383ChordRowProbeTests.cs.txt`＝`RenderedGeometry` の Texts・最上線は a2 の Y ＋ `PartCombineTextUpFromStaffTop` で出す）。LP は p383 の畳んだ双子 `vB-fold.svg`（4 桁）: **和音行 3.637693 対 3.6508／sus4 4.826900 対 4.8400／Intro 6.650908 対 6.6640＝3 つとも −0.0131**・**a2 1.533000 対 1.5330＝0**。⇒ **第382 の −0.06／−0.05 の大半は a2 だった。残るのは「和音行が a2 の上に乗る距離」の 1 量 −0.0131**（Intro は和音行の上に積むので同じ量を継ぐ）。**着手しない理由**: 移るのは和音行の regime（ChordName の skyline・loose line の配置＝既存の `lyrics.chord-row.*` 族の台帳と probe が家）で、この便の a2 の道具・文脈は効かない＝次のセッションで始めても損が無い。機構は未特定（候補: 和音行の "C" の ink 底を sans の箱で読むか輪郭で読むか／行の padding／`PartCombineAnalyzer.InkAboveStaff` の箱が行の予約に入る経路）。
