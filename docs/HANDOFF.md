...
最終更新 第387セッション（2026-09-15）＝**入り方は第298〜第386 の型**（ユーザーは `docs/HANDOFF.md` を読ませて「作業に着手して」だけ）。**§0 の裏取りを走らせた**（`Session-Check -Build -Test -Scratch p388`）。道具は pwsh MCP。**結果＝第386 ⑶ の別件（combined part の span で 5 小節目が LP 原典より +1.0）を閉じた。機構は combined の特殊事情ではなく「skip が小節の途中で*終わる*列を見ていない」で、普通の譜にも在った（commit `718b2e31`・動いた本 2 冊）**。**骨は 3**:

★★ **⑴ 開始時の数は第386 末（⑷）＋第 2 便の HANDOFF commit と一致**: HEAD `2181ed5a`・未 push 7（`origin/master` は `2554d59a`）・木 clean・未追跡 0・**台帳 851 点／ss 非ゼロ 215／総和 22.584727806／count 点 180 うち非ゼロ 0／exact 671／OPEN 0・snapshot 249・追跡 `.lys` 608**・lysc は net10.0 だけ・build 0 エラー・**full `scratch/p388/run1.trx` 8572 / 0 / 4 / 8576**（RunInfos は SKIP 4 だけ）・`2554d59a` の CI は success。

★★★ **⑵ 閉じた＝`MeasureLayouter.CreateTimingSprings` の `droppedOnsetFollows` が skip の*始まり*しか見ていなかった**。**LP は event の始まりにも*終わり*にも列を立てる**（`Simultaneous_music_iterator::pending_moment`＝子の pending moment の最小）⇒ 小節頭から始まる skip も終わる場所に未使用の列を残し、`Spacing_spanner::fills_measure` は `!is_used (next)` で偽＝full-measure-extra-space 1.0 が付かない。**実測 LP 2.26.0（`scratch/p388/fm/`）**: `end.ly` の `<< { r1 } \\ { s2 } >>`・`<< { r1 } \\ { s4 } >>` は小節線 → 休符 1.09・小節 6.688／素の `r1` は 2.09・7.688／**Lily# は 3 小節とも 7.69 → 直して 6.69・6.69・7.69**。**陽性対照** `span-fm0.ly`（`NonMusicalPaperColumn.full-measure-extra-space = 0`）: 素の `r1` と MMR の小節だけちょうど 1.0 縮み、span の 5 小節目は動かない＝1.0 はその量。**始まりが後ろに在る skip**（`scope.ly` の `<< { r1 } \\ { s2 s2 } >>`）は**直す前から正しかった**（既存の門）。⚠️ 第386 の「combiner に渡るのは第 1 voice だけ」は外れ＝part の第 2 voice 以降は `extraVoices` として譜に載っている（`RenderSpec.cs` の `CombinedStaffSpec`）＝**起票の機構候補は予測であって観測ではない（RULES §5.0）**。⚠️ LP を MCP コンソールの子で直に起動すると Guile 初期化で固まる＝`cmd.exe /d /s /c "… < NUL > log 2>&1"`（memory `reference_lilypond_mcp_console_hang`・本便も 9 分踏んでから引いた）。

★★ **⑶ 数と次の一手**: **射程＝掃き 937 冊**（追跡 608＋実コーパス・svg・`data-pos` 伏せ・`scratch/p388/sweep/`）: **動いたのは 2 冊・どちらも休符の前で −1.0**＝`audit/lpreg/pcsm-probe`（5 小節目・LP 原典 6.688）・`pcsm-order-probe`（1 小節目・LP 双子 6.68）／実コーパス 0。**網** `SkipColumnSpacingTests.ASkipEndingInsideTheBar_DefeatsFullMeasureExtraSpace`（普通の譜 2 小節＋combined 1 小節・門を戻すと赤）。`## 0.8.0` に `### Engraving` を立てて項目。**終了時**: HEAD＝この §1 を書く commit（親 `718b2e31`）・未 push 9・**full `scratch/p388/run3.trx` 8573 / 0 / 4 / 8577**（第386 末 8576 から +1＝網 1 本・run2 は 8572 / 1 / 4 / 8577＝失敗 1 は `MagicConstantInventoryTests` の行番号 3 行＝`LILYSHARP_UPDATE_DOCS=1` で再生成し CRLF に戻して `718b2e31` に含めた）・§7.5（`-DiffBase 2181ed5a`）: **Core `+` 14 行／REF 2／OWN 0**＝**監査した**: 門の式は上の 2 住所から導出（⒝ではなく⒜＝LP は列の有無を問うだけで、Lily# は列の一覧を持たないので「残った列に無い終わりの moment」で同じ問いを立てた）・数値定数 0・台帳 851／総和 22.584727806・snapshot 249・追跡 `.lys` 608 は不変・§7 3.5: 第385 を `-Archive 385` で ARCHIVE へ（済）。★ **LSP の配布は要る**（ページの spacing が動いた）。⚠️ **ついでに見えた既存の差（未調査・今回の量ではない）**: `pcsm-probe` の 2 小節目（`s1` 対 `R1`）ページ 7.69 対 LP 7.89（第386 ⑶ と同じ）／`pcsm-order-probe` の 1 本目の小節線がページ 14.18 対 LP 14.99（前置き）。⇒ **次の一手**: 上の 2 つの既存の差／第379 ⑺ の並び（⒝ 摂動網の残り・⒠ drummap・⒢）／第379 ⑻ ② `CreateTimingSpring` の床／第379 ⑽ の範囲外。

★★★ **⑷ 第 2 便（常設指示「このセッションが有利なら着手」→「着手する」→ 住所と設計の制約まで測って止めた・製品 0）**: ⑶ の既存の差 `pcsm-probe` 2 小節目（ページ 7.69 対 LP 7.89）。**計器は全部 `scratch/p388/mmr/`**（LP 双子 `.ly`＋ページ `.lys`＋SVG・LP は `cmd /c … < NUL`）。**⒜ text は無関係**（`text.ly`: `s1` 対 `R1` の 4 小節とも text の有無に関わらず LP 7.890／ページ 7.69）。**⒝ 住所＝MMR が第 1 voice に居ない小節**: `comb.ly` で `R1` 対 `R1`・`R1` 対 `s1` は一致・**`s1` 対 `R1`（MMR は part 2 → 合成後の voice two）だけ 7.69**／**普通の譜でも同じ**（`voice.ly`: `voice { s1 } { R1 }` LP 7.890 対 7.69・`voice { R1 } { s1 }` と素の `R1` は 7.89 一致）＝**combined の話ではない**。**⒞ 機構**: `MultiMeasureRestEngraver.FindRuns(MultiStaffScore)` が各譜の `staff.PrimaryVoice.Measures` だけを `IsMmrMeasure` に渡す ⇒ 第 1 voice が `s1` の小節は run にならず、`MultiStaffLayouter`（:1667）と `SystemBreaker`（:292）の `MmrRodDistance` が付かない（LP の rod は `Multi_measure_rest::calculate_spacing_rods`＝MMR grob ごとで voice を問わない）。**⒟ 描画も同じ穴で、直すときの制約**（LP 対ページ・page frame・譜の中線 y 11.69）: `{ s1 } { R1 }` の MMR＝**LP x 17.47 / y 13.69（voice two・小節の中央）対 ページ 16.27 / 10.69（普通の休符として拍頭）**／`{ R1 } { s1 }`＝**LP y 9.69（voice one）対 ページ 10.69**（run の描画 `MultiMeasureRestLayout` は Y を譜の中線から取り voice の変位を持たない＝**幅が合っている側も Y が違う**）／素の `R1`＝両方 10.69。⇒ **直しは 2 つを同時に要る**: ① run の判定を「全 voice のうち MMR を持つ voice があり、他の voice は skip だけ」に広げる（`FindRuns` の読み手は 5 軒＝spacing・breaker・`MeasureContentKey`・incremental・描画）② MMR の描画に voice の Y 変位を運ぶ（LP の MultiMeasureRest の staff-position を voice one/two で測ってから）。**判定「ここで止める」**: ①② は run の模型と MMR 描画の regime で、この便の spacing（fills_measure）の文脈は効かない・計器はディスクに残る＝次便でも損しない。**射程（未計数）**: 第 1 voice 以外に `R` を書く本＝追跡・実コーパスとも数えていない。

## 以下は第386セッションの経緯

最終更新 第386セッション（2026-09-15）＝**入り方は第298〜第384 の型**（ユーザーは `docs/HANDOFF.md` を読ませて「作業に着手して」だけ）。**§0 の裏取りを走らせた**（`Session-Check -Build -Test -Scratch p387`）。道具は pwsh MCP。**結果＝第383 ⑼ は欠陥ではなく決定済みの書体差だった（製品 0）／ユーザーが選んだ第381 ⑷「voice span の綴り」を閉じた（commit `cc073457`・ページ出力 0）**。**骨は 4**:

★★ **⑴ 開始時の数は第385 末（⑸）と一致**: HEAD `f9da51a4`・未 push 4（`origin/master` は `2554d59a`）・木 clean・未追跡 0・**台帳 851 点／ss 非ゼロ 215／総和 22.584727806／count 点 180 うち非ゼロ 0／exact 671／OPEN 0・snapshot 249・追跡 `.lys` 608**・lysc は net10.0 だけ・build 0 エラー・**full `scratch/p387/run1.trx` 8570 / 0 / 4 / 8574**（RunInfos は SKIP 4 だけ）・`2554d59a` の CI は success。

★★★ **⑵ 第383 ⑼（和音行が a2 の上で −0.0131）＝`page.chord-row.staff-to-chord-baseline` と同じ量＝Heros の C のはみ出し（§3・2026-09-12 決定・追いかけない）**。LP 2.26.0 の dump（`scratch/p387/row/vB-fold-dump.ly`＝ChordName は既定の after-line-breaking を鎖でつなぐ）: **ChordName `Cmaj7` の箱 Y (−0.060184 . 2.159653)・CombineTextScript "a2" の箱 (−0.033010 . 1.557655)・両方とも skyline は extent の箱**。**LP ＝ a2 基線 1.533 ＋ 1.557655 ＋ padding 0.5 ＋ 0.060184 ＝ 3.650839**（SVG 3.6508 と一致）。Lily# も同じ「箱＋0.5」（`ChordNameEngraver.RowSkylines`・`PartCombineAnalyzer.InkAboveStaff`）で、違うのは C の底だけ＝Heros −0.047093 対 Nimbus −0.060184 ＝ **0.013091**・残り約 5e-5 は SVG 4 桁と "a2" の上端の書体差（Lily# 側は未計測）。⇒ **bench の縦の残差は a2（第383 で移植済み）と決定済みの書体差だけ**。

★★★ **⑶ 第381 ⑷ を閉じた（ユーザー選択「combined だけ 1 voice」）**。**ページの読みは譜の種類で 2 通り**（`scratch/p387/voice/`）: **普通の譜／condensed**＝各ブロックが voice（`MeasureCollector.ResolveVoiceStemDirections`＝`\voiceOne`/`\voiceTwo`・`voice { a4 b c' d' } { s1 }` は素の本と SVG が違う＝skip だけの声部でも符尾を向ける）／**combinedStaff の part**＝1 Voice の同時音楽（`PartCombiner.ChooseSilenceWithinPart`・`pcsm-probe` は 4・5 小節を素の `R1`/`r1` にした本と SVG バイト同一）。**双子は両方 `\\`** だった ⇒ `\partCombine` の中で voicify され "too many colliding rests"。**直し**: `LilyPondExporter._combinedPart`（combinedStaff だけが演奏する part・`CarryFrameInto` で入れ子へ運ぶ）で `EmitParallel` が `<< { a } { b } >>`。combined と他の譜の両方で使う part は `\\` のまま警告。**照合**: 新しい双子は手書きの `<< R1 s1 s4 >>`（LP 原典の綴り）と LP の SVG が**バイト同一**（`-dno-point-and-click`）・警告 0。**射程**: 追跡で combined part に span を書くのは 4 冊（`pcsm-probe`・`pcsm-order-probe`・`pcsm-frame`・`pcsm-frame2`）＝4 冊とも span の行だけ動き LP で警告 0／実コーパス 0 冊。**網** `LilyPondExporterTests` +2（combined の網は分岐を毒すると赤）。`## 0.8.0` に twin の項。⚠️ **見つけた別件（第 2 便で住所まで測った・機構は未特定）**: **5 小節目だけ**の差（`scratch/p387/voice`・LP は `-dno-point-and-click` の SVG 4 桁・ページは group の translate 8.54 ＋ SVG x）。小節線 → 休符／小節幅: **LP の span 双子（`<< r1 s2 s4 >>` 対 `<< s4 s2 r1 >>`）1.09／6.688＝LP 原典 `pcsm.ly` と同じ**（`audit/lpreg/pcsm.log`: bar 37.651・休符 38.741・bar 44.339）／**LP の素の本（`r1` 対 `r1`）2.09／7.688／ページ（2 冊ともバイト同一）2.09／7.69**。⇒ **ページは skip の列が無い LP と同じ絵を描く＝原典に対して +1.0**。差は**全部休符の前**（休符 → 小節末は LP の 3 枚とも 5.6）。1〜4 小節目の小節線は LP の 2 枚で一致（22.7189／30.6089／38.2969／46.1869）。⚠️ ついでに見えた既存の差: 2 小節目（`s1` 対 `R1`）はページ 7.69 対 LP 7.89（LP の 2 枚とも同じ＝skip の話ではない・未調査）。**機構の候補（未測定）**: combined part の span の skip は `ChooseSilenceWithinPart` の入れ替え後も part の第 2 voice 以降に残るが、combiner に渡るのは第 1 voice だけ＝ページの列にならない／LP は 1 Voice の events なので 1/4・1/2 に列が立ち、小節線 → 休符の spacing が変わる。

★★ **⑷ 数と次の一手**: **終了時**: HEAD＝この §1 を書く commit（親 `cc073457`）・未 push 6・**full `scratch/p387/run4.trx` 8572 / 0 / 4 / 8576**（第385 末 8570 から +2＝網 2 本・run2 は 8571 / 1 / 4 / 8576＝失敗 1 は棚卸しの行番号 3 行＝`LILYSHARP_UPDATE_DOCS=1` で再生成し CRLF に戻して `cc073457` に含めた・+2＝網 2 本）・§7.5（`-DiffBase f9da51a4`）: **Core `+` 54 行／REF 0／OWN 0**＝**監査した**: 双子の綴りの分岐で LP から導出した計算も数値定数も無い（出典はページ側の家 `ChooseSilenceWithinPart` が持つ）・台帳 851／総和 22.584727806・snapshot 249・追跡 `.lys` 608 は不変・§7 3.5: 第384 を `-Archive 384` で ARCHIVE へ（済）。★ **LSP の配布は不要**（`lysc ly` だけ）。⇒ **次の一手**: ⑶ の別件（combined part の span の skip 列＝LP の横幅 1.0）／第379 ⑺ の並び（⒝ 摂動網の残り・⒠ drummap・⒢）／第379 ⑻ ② `CreateTimingSpring` の床／第379 ⑽ の範囲外。**次便判定（第 2 便）「着手する」→ 住所まで測って止めた**（常設指示）: 対の本と LP の SVG が手元にあり、容疑者 `ChooseSilenceWithinPart` をこの便で読んでいた＝5 小節目・休符の前・原典に対して +1.0 まで確定（⑶）。**残りは「着手しない」**: 次は LP の paper column と小節線 → 休符の spacing を dump で読む横の spacing の regime で、この便の exporter／silence 選択の文脈は効かない。
