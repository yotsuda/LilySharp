...
最終更新 第297セッション＝**§2 U11 を閉じた（`bb6932e3`）＝laissez-vibrer と repeat tie の半弓が住所を持たなかった件**。**入り方は第295・第296 と同じ「まずユーザーに訊く」**で、**返ってきたのは「以前報告した問題は、ほかの修正の副作用ですでに直っているようだ。ほかの残債を直して」**——**着手先の選定そのものが私に委ねられた**。**§2 の ▶ 4 つのうち ⒤ と ⒣ は本文自身が「着手はユーザー決定から」と書いているので開けず、U8 は設計から＝一便まるごと**。**U11 を選んだ理由は「工事が小さい」ではなく、**⚠️⚠️ ★★★ **起票が「要決定 1 問」と書いていたその問いを、ユーザーが U10 で既に答えていたから**（⑵）。**commit 2 本**。**掃きは 1 回・1646 冊 4 出力・両側 Release＝svg MOVED 12／midi 0／xml 0／ly 0／`check` の差 0／レンダー失敗 0。** **台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188／追跡 `.lys` 581／snapshot 231 はすべて不動**（**snapshot は 1 枚が*書き換え*で、追加も削除も 0 枚**）。

⚠️⚠️ ★★★ **⑴ ユーザーは「画面で見えている欠陥を出す」を選んだあと、*その欠陥はもう消えていた*と答えた**——**第296 の 4 件の直しの副作用**。★ **私はその返事を待つあいだに第296 の本（`Untitled-6.lys`）と実コーパス 8 冊を描き直しており、`rit.` の破線が短すぎると*一度書いた*が、ソースを読んだら `c4@rit c@!rit`＝4 分音符 1 つぶんの span で、破線は正しかった**（⇒ **自分の見立てを 1 つ引っ込めた**）。⇒ ★★ **「レンダを見て変だと思った」は診断ではない。**⚠️ **絵の中の量を疑ったら、まずそれを書いた行を読むこと**——**この便でそれをせずに書いたのは 1 度きりだが、1 度は書いた。**

⚠️⚠️⚠️ ★★★★ **⑵ 起票の「これが決まらないと閉じられない」が*また*古かった＝U10 に続いて 2 例目**。**U11 は「注釈が書いた弓は `@laissezVibrer` を名乗るか、それが付いた音符を名乗るか——要決定 1 問」と書いていた**。**が U10 でユーザーが決めた規則は「弓は*それを書いた文字*を名乗る」**で、**半弓を書くのは注釈**なのだから**答えは既に出ている**。⇒ ★★★ **起票がそれを未決だと思った理由は、`TieVariantLayout.SourcePosition` が*既に在って*音符の住所を持っていたこと**——**それは「規則の別の読み」ではなく、*たまたまそこに転がっていたフィールド***。⇒ ★★ **だから工事は「drawer を包む」ではなく「注釈の住所をそこまで運ぶ」で、包むのは最後の 1 行だった**（起票は「工事は小さい」と書いていて、それは*包む部分についてだけ*正しい）。⇒ ★★★ **§5.0 の「引き継がれた処方箋は着手前に診断を 1 回測る」の*ブロッカー版*は、第295 が汎化したのに第297 でまた同じ形が出た＝1 例では足りない規則。**

★★ **⑶ 機械が先に鳴った、2 度とも**（U10 と同じ 2 か所）。**⒜ `CollectTailShifter` の drift net `ShifterInventory_CoversEveryPositionField` が新フィールド 6 つを本を 1 冊も描く前に名指した**（`NoteItem`／`RestItem`／`ChordItem` の 2 つずつ ＋ `ChordNoteInfo` の 2 つ）。**⒝ `SystemLayoutCacheTests.MultiStaff_ReusesSystems_AndStaysByteIdentical` が `data-pos="2836"` 対 `2835` で赤**——**キャッシュされた半弓は「それを*計算した*編集の住所」を持つ**（`SystemLayoutCache` は measure の*内容*で memo し、内容キーは source offset を全部除外している）。

⚠️⚠️ ★★★ **⑷ その直しは `ResolveBows` では*ない*、そしてそれが判ったのは「1 つの和音が住所の違う半弓を複数持ちうる」から**。**メンバー毎の `@laissezVibrer` は頭ごとに違う `@` を名乗る**ので、**note locator が表せる「item 1 つ → 住所 1 つ」ではメンバーが 1 つに潰れる**。⇒ **`SharedRenderer.ResolveSemiTies` は `Calculate` が出す*順序*で対応づける**（`SemiTiesOf` を生きた item に訊き直して歩調を合わせる）。⇒ ★★ **既存の解決器の形が合わないときは、合わない理由がそのまま新しい器の設計になる。**

★★ **⑸ 和音レベルがメンバーレベルに勝つ——これは*発明ではなく既存の先取権の続き***。**`LaissezVibrerUp`（強制方向）が既に「chordLv ? chordLvUp : memberLv?.ForcedAbove」で和音を先に読む**（LP の `acknowledge_note_head` は heard event を articulation より先に読む）ので、**住所も同じ順にした**。**実測**: `<c@laissezVibrer e g>4@laissezVibrer` は**3 本とも和音の `@`(52) を名乗り、メンバーの `@`(32) は 1 本も名乗らない**。

★★★ **⑹ 毒 5 本・赤くなる集合が違う**（§5.4）: **⒜ `Source` スコープを外す → 7（新ケース 6 ＋ snapshot）／⒝ note 腕で音符の住所を打つ → 4（note 綴り 3 ＋ snapshot。**和音の腕は緑のまま＝**この 1 本が「住所は注釈のもので音符のものではない」を*測定*にしている**）／**⒞ 先取権を逆に → 1（先取権のケースだけ）／⒟ `ResolveSemiTies` を外す → 1／⒠ content key の除外を外す → 1。** ⚠️ ★★ **⒟ と ⒠ は*同じ網*が赤くなる**——**が主張が違う**: **⒟ は L264 のバイト同一性（`data-pos` の文字列不一致）、⒠ は L262 の `cache.Count` 9→10（memo churn）**。⇒ ★★ **「同じ網が赤い」で毒を同一視しない。どの assert が落ちたかまで読む。**

⚠️ ★★★ **⑺ §5.2.1③ への答えは第295 と同じ形＝名指せる台帳キーは無く、無いことが正しい**（**台帳の点は属性を観測できない**）。**代わりに置いたのはマスクの機械的証明**: **書き換えた snapshot 1 枚も、掃きで動いた 12 冊も、`data-pos` を伏せると base と*バイト同一***（**追加された属性は snapshot 1 個 ＋ 掃き 34 個**）。★ **加えて「その住所が本当に `@` か」も測った**——**`data-pos="64"` の 64 は `@`、音符 `c'` は 62**（fixture で実測。**「住所が付いた」だけなら音符を指していても通る主張**）。

★★ **⑻ 掃きの 12 冊は*確かめた*、仮定しなかった**＝**12 冊すべてが `@laissezVibrer`／`@repeatTie` を書く本**（起票が測った射程 12 冊とちょうど一致）。**内訳＝audit 7・追跡 fixture 3（`grammar-tour`・`feature-tour`・`lv-meterchange`）・scratch のプローブ 2。** **ユーザーの実コーパス 326 冊は 0 冊**（起票のとおり）。

★ **⑼ perf は測った**（§7.9＝**`MusicItem` と `ChordNoteInfo` が item ごとに int 2 つ太り、shifter が per-item で 2 回余分に回る**）。**`audit/LilySharp.Probe alloc`・3 run の床・full render MB**（**base ／ head**）: **perf-plain1k 471.3 / 471.4**（打鍵 26.3 / 26.3 で不動）・**perf-slur1k 1028.0 / 1028.1**・**perf-fingbeam1k 961.7 / 962.5**。⚠️ **fingbeam1k の +0.8 は計算ではなく record が太った分**——**この 3 冊は半弓を 1 つも持たない**ので、**増えたのは寸法だけ**。**返す手は今日は無い**（フィールドは要る）。**+0.08%。**

⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 の ▶ は U8（装飾音符が注釈を 1 つも運ばない・射程はユーザーの本 0 冊・閉じる便は設計から）と、⒤（`time none` の半実装）・⒣（`removeEmpty`/`pedal`）の 3 つ**。★ **⒤ と ⒣ は本文自身が「着手はユーザー決定から」と書いている**ので、**残る「私の裁量で開ける項目」は U8 1 つだけ**——**その U8 も射程 0 冊で設計から**。⇒ **だから次は「残債を直して」より一段細かく、*どれ*を開けてよいかを訊くのが素直。**

★ **開始時裏取り**: HEAD `126ae312`・**未 push 3**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6606 / 0 / 4 / 6610**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2・`OPEN:` 0**・追跡 `.lys` **581 冊**・snapshot **231 枚**——**§1 の終了時の数と 1 つも違わなかった**（**10 個の数え方レシピは全部そのまま再現した**）。
終了時: **commit 3 本**（**本体 `bb6932e3` ／ docs `c0f5190b` ／ ⑽ の閉じ直し 1 本**）＝**未 push 6**・木 0・未追跡 0。**suite 6613 合格 / 0 失敗 / 4 skip / 合計 6617**（開始 6610 から **+7**＝**本便の網 7 ケース**）。Core 0 エラー 0 警告。**追跡 `.lys` 581 冊・snapshot 231 枚**（**1 枚が書き換え・追加も削除も 0 枚**）・**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2／`OPEN:` 0＝すべて不動。**
★★ **WSL ubuntu Release も同じ 6613 / 0 / 4 / 6617**（`bb6932e3` の木。**3 本目は docs だけなので建てていない**）。⚠️ **CI は `11f48202` までしか走っておらず、本便の 3 本を含む未 push 6 本は未実行。**

⚠️⚠️ ★★★ **⑽ §1 を閉じたあとに 3 本目が在る＝ユーザーが委任を出した**（「U8 は、このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）。**答えは「着手しない」**。⇒ **理由は 3 つとも*非対称性*の話で、熱の話ではない**: **⒜ 温まっているのは `MeasureCollector` の注釈を*読む*側だが、U8 の難所は `GraceNoteInfo` のアンカリング＝本便が 1 行も触っていない場所**／**⒝ base 側 exe は U11 の base のもので、U8 の base は別＝建て直し 4 秒＝非対称ではない**／**⒞ *次便のスロットは空いている***——**U11 を閉じた今 §2 の ▶ は U8・⒤・⒣ で、⒤ と ⒣ は本文自身が「着手はユーザー決定から」と書いている**ので、**U8 は次便が自力で開ける唯一の項＝スロット不足でまた繰り延べられることはない**。⇒ ★★★ **委任で取るべきは「*理由ではなくスロット不足で*飛ばされた項」で、U8 は*理由*で飛ばされている——しかもその理由を私はこの便の §1 で再確認したばかり。** ⇒ ★★ **そして代わりにやったのが、この規則が名指す「派生ではなく*読まれた*数に直す安い測定」**＝**U8 の射程**（§2 U8。**0/326 かつ 0/581・当たる 2 冊は起票者自身のプローブ**）。⇒ ★ **測ったら、着手しない理由のほうが強くなった。**
⚠️ ★★ **閉じ直しは第210 が数を間違えた場所**（未 push を commit の*前*に数えた）。**この 3 行の数は commit の*あと*に測り直してある。**
⚠️ **`docs/APPROXIMATIONS.md` が 7 行動いた**——**中身ではなく行番号**（`SharedRenderer.Marks.cs` に 10 行入ったため）。**`LILYSHARP_UPDATE_DOCS=1` で再生成した**（編集ではなく再生成が正しい直し方だと、そのテスト自身が言っている）。

## 以下は第296セッションの経緯

最終更新 第296セッション＝**§2 の項目ではなく*ユーザーが画面で見ていた欠陥*に着手＝`scratch\ベースタブLy\Untitled-6.lys` の「五線とコード名と rit が重なる」**（第295 の §1 が「次は訊く」で終わり、訊いたら *これ* が返ってきた）。**ユーザーの指示は直し方まで指定していた＝「ロジックを発明すると、もぐらたたきのような修正が収束しない。LP のコードを模倣して」**。**重なりは 1 つではなく *3 つ* の欠陥で、3 つとも LP に既に在る答えを 1 つ移すだけで閉じた**（**3 つ目はユーザーが 1 便目のレンダを見て「まだ不完全」と*その場で*報告した分**）。**続けてユーザーが *4 つ目* を出し、そこで基準そのものを名指した**（⒁——**タブの注釈は「タブか」ではなく `as numbers`／`as full` で決まる**）。**commit 6 本**。**掃きは 3 回**（**変更ごとに 1 回・各 1645〜1646 冊 4 出力**）＝**1 回目 svg 1 冊／2 回目 1 冊（どちらも報告されたその本）／3 回目 91 冊（全部分類・方向も確認）**。**midi/xml/ly は 3 回とも 0・`check` の差 0。** **台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188 は不動**（**snapshot は 230→231・追跡 `.lys` は 580→581**＝**3 つ目の網の fixture 1 冊。既存の snapshot は 1 枚も書き換わっていない**）。

⚠️⚠️ ★★★ **⑴ 「重なる」は 1 つの欠陥ではなく 2 つで、症状が同じ場所に出ていただけ**。**⒜ タブが上の五線と同じ注釈を*全部*描いていた**（`rit.` も `@f` も `@text` も 2 回）／**⒝ コード名の行が 1.75 高すぎた**。**⒜ だけ直しても ⒝ の重なりは残る**（実測・途中の PNG で確認）。⇒ ★★ **「重なっている」は座標の話で、原因の話ではない。動いている量を 1 つずつ名指すまで、直したつもりが半分になる。**

⚠️⚠️ ★★★ **⑵ ⒜ の答えは LP に表として在った＝`ly/engraver-init.ly:1277-1285`**（`TabStaff` 文脈の最後のブロック）。**`TupletBracket`／`TupletNumber`／`DynamicText`／`DynamicTextSpanner`／`TextSpanner`／`Hairpin`／`Script`／`TextScript` の 8 つを `\override X.stencil = ##f` する**。**Lily# は 8 つのうち 5 つを描いていた**（tuplet 2 つは既に抑止済み・`DynamicTextSpanner` は Lily# に綴りが無い）。⇒ **新しい置き場所 `TabStaffStencils.cs` はその表の*転記*で、判断はしていない。** ★ **重複の出所は正常**——**`staff m` と `tab m` は同じ part を 2 回 collect するので注釈は StaffIndex 違いで 2 つ在る**（`MeasureCollector.CollectMultiStaff` の binding ループ）。**LP でも grob は 2 つできる。違うのは stencil を空にするかどうかだけ。**

⚠️⚠️⚠️ ★★★★ **⑶ 空の stencil は「見えない」ではなく「場所を取らない」で、*この件はその差のほうが本体*だった**。**LP は stencil が `##f` の grob の extent を空で返す**ので**skyline に入らない**（`lily/grob.cc Grob::extent`／`axis-group-interface.cc:864-989 skyline_spacing`）。**Lily# はタブの `rit.` に outside-staff の帯を*予約*していて**、**コード行がそのタブに畳まれている本ではその帯が上の五線の*中*に着地した**。⇒ ★★★ **だから抑止は描画時ではなく layout 時**。**そして*予約の半分*は別の家に在る**——**インク側は `LayoutEngine.Annotations`、予約側は `MultiStaffLayouter.BuildAllStaffSkylines` が読む `ScoreSideTables` の staff 別バケツ**。**両方に入れるまで終わっていない**（毒で実測：予約側だけ戻すとインクは正しいまま帯だけ残る）。⇒ ★★ **LP にも「1 か所の filter」は無い**——**`stencil = ##f` は文脈プロパティで、stencil か extent を訊かれた場所すべてが読む。表は 1 つ、訊く場所は複数、が忠実な形。**

⚠️⚠️ ★★★ **⑷ `Script` は blank しない、と*機械が先に言った***。**表のとおり 8 つ全部を入れたら snapshot 7 枚 ＋ `TabScriptStemClearanceTests` が赤**——**タブの上の記号は Lily# が*意図して*描いている機能で、フレット数字に合わせた専用の配置コードまで在る**。**タイ（`Tie`/`RepeatTie`/`LaissezVibrerTie`、同ブロックの 4 行上）も同じ立場で `test/tab-tie` を持つ。** ⇒ ★★★ **「LP に忠実」と「Lily# の製品決定を消す」は別で、後者はインクを動かす＝別便の仕事。** ★ **その線引きを `TabStaffStencils` の remarks に、消さなかった理由と赤くなった網の名前つきで書いた**（次に表を見た人が同じ 8 行を全部入れないように）。

⚠️⚠️ ★★★ **⑸ ⒝ は 1.75 ちょうどで、その 1.75 は*既に書かれていた***。**`chords … as names` を五線とタブの間に書くと `RenderSpecParser.FoldAdjacentRows` が*下のタブ*に畳む**ので、**その行は「タブの上に立つ loose line」になる**。**alignment の walk 自身は正しい答えを出していた**（実測 `where=5.9523`＝五線の refpoint から下へ、baseline 19.50）**のに、描画は 17.75**。**差は `AttachedChordBaselineAboveTop` の frame 変換が `_options.StaffHeight / 2.0`＝*公称*五線の半分 2.0 を使っていたこと**——**6 弦タブの高さは 7.5 で半分は 3.75**。⇒ ★★★ **`StaffHeightOf` の注記が「SkylineBuilder と placed height が*タブの下で 1.750000 食い違っていた*」と既に書いている**。**同じ量・同じ数・1 か所あとの呼び出し元**。⇒ ★★ **「一つ家（one home）」を作った便の注記は、*その家をまだ通っていない呼び出し元*の一覧ではない。数が書いてあったら grep すること。** ★ **直しは placed `StaffLayout.Height` を読む**（タブも ossia も既に正しい）＝**呼び出し元が誤った値を渡せる引数そのものを消した。**

★★ **⑹ 掃きは 4 出力・1645 冊・両側 Release で svg MOVED 1／midi 0／xml 0／ly 0／`check` の差 0／レンダー失敗 0**。**動いた 1 冊は報告されたその本。** ⚠️⚠️ ★★★ **だがこれは「射程 1 冊」ではない**——**掃きは各ファイルの*既定 score* しか描く**。**射程を別に数えた＝タブを持つ本 341・タブと五線の両方 320・そのうち blank される族を書く本は 2 冊**（**どちらもユーザーの `ベースタブLy`**）。**2 冊目 `奏（かなで）.lys` が動かなかったのは既定 score にタブが無いからで、`--score both` と `--score tab` を明示すると*両方動く***（実測）。⇒ ★★★ **全数掃きの「0 冊」は、その掃きが選んだ score についてしか言っていない**（§5.0 の「掃きの 0 冊はその掃きが見た*出力*についてしか言っていない」の score 版）。

⚠️⚠️ ★★★ **⑺ 値段を 1 つ払っている＝タブ*だけ*の score は注釈を丸ごと失う**。**LP の blank は文脈プロパティなので五線が隣に在るかを見ない**——**`奏（かなで）.lys` の `"both"` score は `@text("人差し指で")` を 2 回描いていたのが 1 回になり（欠陥）、`"tab"` score は 1 回描いていたのが 0 回になる（値段）**。**タブを読む奏者に宛てた運指の指示が消える。** ⇒ **忠実ではある**（LP でも `\new TabStaff { c4\f }` は何も出さない）**が、これはユーザーの決定であって私の決定ではない**ので、**網 `ATabOnlyScorePrintsNoBlankedMarkupEither` で*現在の答えを固定*し、変えるなら 1 条件で足りることを両方の remarks に書いた**（`Blanks` を「同じ score で五線にも出ている part か」で gate する＝**LP からの逸脱**）。★ **未決＝§2 U12 に起票した。**

★★★ **⑻ 毒 6 本・赤くなる集合が全部違う**（§5.4）: **⒜ dynamics のインク側 → 2 赤／⒝ hairpin のインク側 → 1 赤**（**楔は文字ではないのでインクの網が数えられない＝だから本に `@cresc` を入れて layout の網に読ませた**）／**⒞ text spanner のインク側 → ⒜ と同じ 2／⒟ text spanner の予約側 → 2（バケツ ＋ 段間の room）／⒠ dynamics の予約側 → 1（バケツだけ。**page geometry の本では*どうやっても*届かない**——五線の下に置くと `default-staff-staff-spacing` の basic-distance 9 に埋もれ、歌詞行の下に置くと*五線自身の* dynamic でも動くので blank された写しと区別できない。**2 通り試して両方測ってから、主張はバケツで述べた**）／**⒡ frame 変換を公称に戻す → 1（15.845551 対 17.050551＝報告された欠陥そのものの数）。**

⚠️⚠️ ★★★ **⑼ その毒の 1 本が一度*嘘の赤*を返した**。**バケツの網が、ソースは既に正しい木で赤くなり、`--no-incremental` で建て直したら同じソースのまま緑**。⇒ ★★ **増分ビルドがここでは古い Core を配る**（§0 の警告の実例）。**毒の結果が意外だったら、説明を考える前に建て直す。** ★ **この 1 件を網の poison 表に書いた**（次に同じ驚き方をしないように）。

★ **⑽ perf は測った**（§7.9）。**`audit/LilySharp.Probe alloc`・3 run の床・full render MB**（**base ／ head**）: **plain1k 471.3 / 471.3**（打鍵 26.3 / 26.3）・**hairpin1k 314.6 / 314.6**。**不動**——**タブの無い score では `Blank` が入力の配列をそのまま返して 1 バイトも確保しない**（2 パスにしてある）。⚠️ **1 run だけ見たときは hairpin1k が +0.8 に見えた。床を取ること。**

⚠️⚠️ ★★★ **⑾ 3 つ目の欠陥はユーザーが*直したレンダを見て*出した**——**「まだ不完全だ。A2 の右付近で rit と歌詞の Like が重なっている。2 段目と 3 段目の改行幅を少し広くする必要があるように見える」**。**診断はそのとおりで、理由はこう**: **rit. の墨は*スカラー*の per-system extents には入っていた**（`EnrichExtentsWithAnnotationProtrusions`）**が、それを読むのは*ページ分割器*だけ**。**段と段の間隔は `LayoutUtilities.InterSystemPairMinimum` で、skyline が在るときそれは X を見る `Distance()` *だけ*を読む**（`nextUpExtent` は staff の無いリードシート専用の落とし穴）。⇒ **スカラーに在って X 認識の輪郭に無い族は、どれだけ背が高くても段間のバネから*見えない***。**直しは dynamics が既に持っている腕を 1 つ足すだけ**（`AugmentSkylinesForPaging`・**その dynamics の注記自身が 2026-08-28 に同じ形を直したと書いている**）。

⚠️⚠️⚠️ ★★★★ **⑿ そして最初に書いた直しは*広すぎて、台帳が止めた***。**`SkylineBuilder.BuildSystemSkylines` の行の歩きは最初の spaceable staff で break するので、上端 staff の*外側*の墨がページに届かない**——**そこで「上端 staff の skyline を丸ごと merge する」と書いた。重なりは消える。** **が `page.inline-chord.gap-first` が LP から 2.500000 離れ**（**15.095000 対 LP 12.593884**）、**snapshot 6 枚と `ArticulationPlacementTests` 2 本と `SlurScoringTests` 1 本が赤**。⇒ ★★★ **その台帳エントリの*記録された原因*が、この失敗を先に名指していた**——**「*少数の x* にしか無い墨のために*全部の x* の下で距離を床にする charge は、他の 2 本の腕が既に価格を付けているものへの 3 本目だ」**（2026-08-28 に retire された chord band そのもの）。**私は同じ band を別の綴りで作り直していた。** ⇒ ★★ **狭い腕（ラベルの箱と破線の箱を*分ける*）ならその点は 1 ミリも動かない。** ⇒ ★★★ **「重なりが消えた」は直しが正しい証拠ではない。台帳は消えた重なりの下で何が壊れたかを言う。**

⚠️⚠️ ★★★ **⒀ この網の本は 4 回作り直した＝*ページに余白があると何も試していない***。**ページは縦に justify されるので、余白のある本では段が広がり、バネの床は一度も効かない**——**2/3/5/6/7 段の合成本を作ったが、*5 つとも* 直しの前後で rit の位置が 1 ミリも変わらなかった**（**全部「緑だが何も見ていない」網になるところだった**）。⇒ **fixture `test/rit-across-systems` は題名・コード行・タブ・2 番まで の歌詞を*全部*持つ**——**どれもページを閉じておくための部品**で、**1 つでも薄くすると網は黙って引退する**。**その旨を fixture の登録行に書いた。** ⇒ ★★ **「この本はまだ問いを立てているか」を網自身に測らせること**（この網は X 帯の重なりを陽性対照にしている）。

⚠️⚠️⚠️ ★★★★ **⒁ そして ⑺ の値段はユーザーが即座に払い戻させ、*基準まで名指した***——**「タブに staff がない場合には `@text` がタブに表示されるようにして」＋「`as numbers` のタブに `@accent` が出ているが表示しないで」**、続けて **「staff がないというのは、つまり `as full` のタブには `@text` を描画するという解釈で良いのかな」**。**答えは yes で、しかも私が §2 U12 に書いた条件より良い。** ⇒ ★★★ **その基準は既にこの木に在った**——**`RenderSpecParser.StaffRenderedParts` の `as` 既定規則（2026-08-29・同じユーザーの決定）が「五線と並ぶタブは数字だけでよい。上の五線が拍子も休符も付点も符尾もタイも運んでいるから。単独のタブは全部自分で運ばなければならない」と書いている。** **`TabNumbersOnly` は最初から「この part は五線にも出ているか」を意味し、既定もそうで、`as` で上書きできる。** ⇒ **注釈の族はその表の続きなので `Blanks` を `IsTab` から `TabNumbersOnly` に変えるだけ**——**要望 2 つが 1 つのスイッチに畳まれ、U12 は閉じた。** ⇒ ★★★ **私が起票で提案した「part が五線にも出ているかで gate する」は、その既存規則の*再発明*だった**。**「1 条件で足りる」と書いたとき、その 1 条件が既に名前を持って隣の file に在るかを見ていない。**

⚠️⚠️ ★★★ **⒂ Script を表に足すとき、素朴にやると*その族のための fixture を丸ごと消す***。**`as numbers` のタブで記号を落とす——だが `test/tab-technique-letters` は `staff gtr`＋`tab gtr`（＝numbers）で、`@tap`/`@hammeron`/`@pulloff`/`@pluck` の T・H・P・p-i-m-a の*ためだけに*在る本**（**2026-08-28 に「文字が自分の符頭に食い込む」とユーザーが報告して生まれた**）。**T/H/P はギタリストが*タブを*読むための墨で、上の五線が運んでいる markup ではない。** ⇒ **だから `BlanksScript` は型ではなく *item* で訊く**（`TabTechniqueLetterOf` ＝既存の一つ家。`@pluck` は文字を item に載せている）。**区分は 3 つになった＝bend-after/breath（そもそも Script でない）／技法文字（タブの墨）／それ以外（accent・staccato・fermata…）。** ⇒ ★★ **「LP の表を移す」の*移し方*にも判断が要る。表の行を全部入れるのが忠実とは限らない。**

★★ **⒃ 掃きは 91 冊動いた**（1646 冊中・midi/xml/ly は 0・`check` 0）——**これまでの 2 変更の「1 冊」とは桁が違うので*全部*分類した**: **91 冊すべてが「五線＋タブで `as full` の明示が無い」＝numbers タブで、記号を落とした本**。**方向も確かめた、仮定しなかった**＝**91 冊を両側で描き直して head の music glyph が*厳密に少ない*のが 91／増えたのは 0**。⚠️ **「full タブに markup が戻った」側は掃きでは 0 冊**——**掃きは既定 score しか描かず、コーパス唯一のタブ単独 score は*名前つき*ブロックだから**（`奏（かなで）.lys --score tab` が 0→1、`--score both` は 1 のまま。直接測った）。

⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 の ▶ は U8（装飾音符が注釈を運ばない・射程はユーザーの本 0 冊）と U11（l.v./repeat tie の半弓の住所・射程は追跡 1 冊）の 2 つだけで、どちらも読み手の目には 0 冊。** ★ **U12 はこの便で開いてこの便で閉じた。**

★ **開始時裏取り**: HEAD `b95c0e90`・**未 push 27**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6596 / 0 / 4 / 6600**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2・`OPEN:` 0**・追跡 `.lys` **580 冊**・snapshot **230 枚**——**§1 の終了時の数と 1 つも違わなかった**（**10 個の数え方レシピは全部そのまま再現した。`exact` と `snapshot` は素朴に数えて 556／0 を出し、§0 を読んで直した＝レシピが在ることの値打ちがそのまま出た**）。
終了時: **commit 7 本**（**タブの stencil 表＋網 ／ docs ／ 段間の rit＋網＋fixture ／ docs ／ `as numbers` スイッチ＋網の書き換え ／ docs ／ この 1 本**）。**suite 6606 合格 / 0 失敗 / 4 skip / 合計 6610**（開始 6600 から **+10**＝**本便の網 10 ケース**）。Core 0 エラー 0 警告。木 0・未追跡 0。**追跡 `.lys` 581 冊**（**+1＝`test/rit-across-systems`**）・**snapshot 231 枚**（**+1＝同じ本。既存の 230 枚は 1 枚も書き換わっていない**）・**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188＝すべて不動**（**`page.inline-chord.gap-first` を含む＝⑿ の広い直しならここが 2.5 動いていた**）。
★★ **WSL ubuntu Release も 3 つの code の木すべてで Windows と同じ**——**`fa57384c` で 6603 / 0 / 4 / 6607、`5e50d9cb` で 6605 / 0 / 4 / 6609、`c92dd851` で 6606 / 0 / 4 / 6610**。
⚠️⚠️ ★ **未 push は 3 で、31 ではない**——**ユーザーが*本便の途中で* `11f48202` まで push した**（`git reflog show origin/master` が `update by push`。私は一度も push していない）。**残る未 push は `c92dd851`・`8e36dbb5`・この 1 本の 3 つ。** ⚠️ **CI は `11f48202` までは走り得るが、その 3 本については未実行。** ⇒ ★★ **§0 の開始時裏取りは §1 の終了時の数と突き合わせる決まりなので、「私が push しなかった」から未 push 数を*推論*すると次便が偽の食い違いを踏む。数は最後に実測すること。**
...
- **U8. ▶ 装飾音符は注釈を 1 つも運ばない**（**第293 が LYS4019 を作る過程で実測・起票**）。
  ★★★ **`MeasureCollector.CollectGraceNotes` は音高と音価しか読まない**——**`grace { }` の中の
  音符に書いた `@mark`・`@staccato`・`@text` は 1 つも collect されない**（**3 つとも実測で消える**）。
  **`CollectArticulations` を通らないので、これは記号 1 族の話ではなく*注釈の全族*。**
  ★ **射程は*読んだ*（2026-08-30・第297。それまでは括弧書きで理由だけが書かれていた）**:
  **ユーザーの実コーパス 0/326・追跡 `.lys` も 0/581**——**ディスク 1646 冊で当たるのは
  `scratch\p293\g1.lys`（`grace { d'8@staccato }`）と `scratch\p293\v2.lys`（`grace { d8@mark("P") }`）の
  2 冊だけで、どちらも*この項を起票した第293 自身のプローブ***。⇒ ★★ **読み手も fixture も 0。**
  **数え方**（`grace {` の対応する `}` まで取って中に `@` が在るかを見る。⚠️ **これは engine では
  なく文字列の走査**なので、§2 ⒥ の「文字列の走査は嘘をつく」の側——**閉じる便は engine に
  訊き直すこと**。ただし**当たった 2 冊は目で確かめてある**）:
```powershell
foreach ($f in (Get-ChildItem . -Recurse -Filter *.lys -File |
                Where-Object { $_.FullName -notmatch '\\(bin|obj|artifacts|output)\\' })) {
  $t = [Text.Encoding]::UTF8.GetString([IO.File]::ReadAllBytes($f.FullName))
  foreach ($m in [regex]::Matches($t, 'grace\s*\{')) {
    $i = $m.Index + $m.Length; $d = 1; $body = ''
    while ($i -lt $t.Length -and $d -gt 0) {
      $c = $t[$i]; if ($c -eq '{') { $d++ } elseif ($c -eq '}') { $d--; if ($d -eq 0) { break } }
      $body += $c; $i++ }
    if ($body -match '@') { $f.FullName; break } } }
```
  ⇒ ★★★ **この数は「急いで直す理由」ではなく「落ち着いて設計する理由」**——**0 冊の項に
  合わせた設計は、あとから来る本のために作るのであって、今日の赤を消すために作るのではない。**
  ⚠️ **いまは無言ではない**——**`@mark` については `LYS4019` が言う**（**それがこの項の起票理由**）。
  **`@staccato`・`@text` はまだ無言。**
  ⚠️ **直すなら「装飾音符は measure の item ではない」ところから**——**`itemIndex` で
  アンカーする既存の注釈経路がそのままでは使えない**。**閉じる便は設計から。**
  ★ **番人は既に在る**: `RehearsalMarkEngravedValidatorTests.AMarkOnAGraceNote_IsReported` が
  **「警告が出る」と「`@staccato`/`@text` も届かない」を両方述べている**ので、
  **穴を塞いだ人はこのテストに出迎えられる**（**塞いだら緑にする向きが正しい**）。
