...
（**実装 8 便＋道具 3 便**。**合成本の打鍵は 3304.5 → 171.8 MB＝−94.8%**。
★ **最後の 1 便だけがページ経路で、1 ページを超える*実ファイル*にも届く**——
`perf-plain1k` 95.9 → 60.2 MB＝**−37.2%**）。
...
| 1 打鍵 | 便前 | ⑴ | ⑵ | ⑶ | ⑷ | ⑸ | ⑹ | ⑺ | **⑻** |
|---|---|---|---|---|---|---|---|---|---|
| `perf-fingstack1k` | 3304.5 MB | 2815.6 | 2191.1 | 1362.0 | 524.9 | 244.2 | 177.1 | 171.7 | **171.8**（**−94.8%**） |
| `perf-scripts1k` | 1117.8 MB | 932.6 | 697.8 | 403.9 | 336.8 | 87.5 | 80.4 | 77.5 | **77.2**（**−93.1%**） |
| `perf-v2bow1k` | 157.1 MB | 157.1 | 152.9 | 152.9 | 152.9 | 87.9 | 87.9 | 83.7 | **83.7**（−46.7%） |
| `perf-plain1k` | 95.9 MB | 95.9 | 95.1 | 95.1 | 95.1 | 66.3 | 66.3 | 62.7 | **60.2**（**−37.2%**） |
...
- **⑹''' ★★★ 第8便＝梁の小節が「刻印 1 個につき 1 回」作り直されていた**
  （`MeasureCollector.ResolveBeamStemDirections`・**ユーザー指示＝`1-collect` を割れ**）。
  bake は 3 種類の刻印（方向＋BeamId／純ステム先端／休符の押し量）を書くが、
  **その 1 つごとに `Items.SetItem(...)` と `new Measure(...)`**——**8 音符の小節は第1パスで 8 回、
  第2パスでまた 8 回**作り直され、最後の 1 つ以外は即座に捨てられていた。
  **小節ごとの作業配列に貯め、全グループが書き終えてから 1 回だけ確定する**形にした。
  ★ **値段**: **`ResolveBeamStemDirections` 5.8 → 3.3 MB・collect 8.5 → 6.0・
  打鍵 plain1k 62.7 → 60.2（−4.0%）**。**梁の無い fingstack は前後とも 0.0＝真の対照。**
  ⇒ ★★ **本による差が住所の裏取り**——**3 冊が同じだけ動いたら別の何かを測っている。**
  ★ **566 冊 A/B で 0 冊**・**陽性対照は「作業配列を確定しない」毒で 104 冊**
  （梁はこのエンジンで最も見えやすい）。
  ⚠️⚠️ **読みも動かす必要があった**——**純ステム先端のパスは第1パスが書いた方向を読む**ので、
  **読みは `measures` ではなく `ItemAt` を通し、確定はループの外**。
  **ここで `measures` を直接読み直すと、刻印前の項目を静かに見る。**
  ⚠️ **再開機構は元から極めてよく効いている**（1000 小節のうち 999 が再利用＝採用 500 ＋ 継ぎ 499・
  計画は 0.0 MB）。**それでも 8.5 MB だったのは、再利用が*歩き*を省いても*刻印の作り直し*は
  省かないから。**
- **⑹'' ★★★ 第7便＝文書を builder に組んでから string へ写していた**
  （`62455900`・**Core 1 ファイル**・**ユーザー指示＝`4-render` を割れ／掘れ**）。
  `Assemble` が**各ページ本文を 2 つ目の `StringBuilder` に積んでから `ToString()`** していた
  ——**本文は既に組んである**（`EndPage` がページごとの builder を保持）ので、
  **完成した文書が 2 本存在していた**。`string.Create` で断片を直接結果へ書くようにした。
  ★ **値段**: **`Assemble` 7.2 → 3.6 MB**（**3.6 は床**＝文書自体が 3.76 MB）。
  **打鍵 plain1k 66.3 → 62.7（−5.4%）／v2bow1k 87.9 → 83.7（−4.8%）／
  fingstack1k 177.2 → 171.7（−3.1%）。**
  ⇒ ★★★ **本の形によらず全部の本に届く**——**費用は SVG の大きさで決まり、中身では決まらない。**
  ⚠️⚠️ **文字を 1 つずつ再現する変更**なので、**566 冊のバイト A/B が形式ではなく本体の検査**。
  **0 / 566 冊**・**陽性対照は最大級＝文書末尾の改行を 1 つ落とすと 566 冊全部が動く。**
  ⚠️ **実装は断片 1 列**（単一ページと複数ページの違いは「どの断片を並べるか」だけ）——
  **コピーのループを 2 本持つとそこがずれる。**
...
- **⑼ ★★★ 残っている項＝打鍵の残りと、その内訳**（**第6便の*直前*に実測**・単位 MB）:
  ...
  **減らすなら `(j, k − minLines[j])` で持つ帯状の表**にするしかなく、それは別の仕事。
  ...
  ⚠️⚠️⚠️ ★★★ **帯状化も測って倒れた＝この表は縮まない。2 案とも実測で却下したので、3 度目をやらないこと。**
  **やったこと**: 行ごとに `minLines[j]..maxLines[j]` だけを持つ（`bandBase[j]` 基準）。
  **行 j への書き込みは外側ループが j に居る間に全部起きる**ので、密なスクラッチ 1 行に貯めて
  最後に右サイズへ確定する——**設計としては正しく、行数は 125 のまま**（出力不変）。
  **結果**: **確保 19.23 → 9.06 MB（−52.8%）だが、時間 11.10 → 14.06 ms（+26.6%）。**
  ⇒ ★★★ **帯の幅の見積りが外れていた**——**9.06 MB ÷ 20 B ÷ 1001 行 ＝ 1 行あたり約 475 列**。
  **「幅 14」は第135 の k-iteration 数からの推定だったが、k ループに*到達する対*が少ないだけ**で、
  **到達したときの帯は全幅の約半分**。だから節約は 2 倍止まりで、
  **行ごとの確保（3 本 × 1000 行）と複製が時間を 27% 押し上げた。**
  ⇒ ★★ **結論＝この 19.23 MB は「無駄」ではなく「この DP の作業領域」**。
  **減らすには表の意味を変える（探索する k を減らす）しかなく、それは出力が変わる話。**
  ⚠️ **ユーザーが重視するのはプレビュー速度**（RULES §7 ⑼）なので、
  **確保 −53% と引き換えの時間 +27% は取らない。**
  ⚠️ **測り方は §1 の骨 ⑴ の手順**（`AllocProbe` の region を置いて二分・30 分）。
  ⚠️ **`3b-LayoutSystems`・`3e-looseChain`・`3f-LayoutAllSpanners` は 0.0〜0.4 MB**なので
  そこは見なくてよい。

  ★★★ **「もう余地は無いのか」を測って答えた**（第191 の最後）。**答えは「まだある」。**
  **`perf-plain1k`（実ファイルの形・打鍵 66.3 MB）の現在地**:

  | 段 | MB | 状態 |
  |---|---|---|
  | `L1-break`（DP） | **19.3（29%）** | **本質的**＝2 案とも測って却下（上） |
  | **`4-render`** | **12.5（19%）** | ⚠️ **一度も割っていない** |
  | **`1-collect`** | **8.5（13%）** | ⚠️ **一度も割っていない** |
  | **`3-layout` の未帰属分** | **約 10（15%）** | ⚠️ **一度も割っていない** |
  | 最終注釈パス | 2.7 | 済 |
  | prelim の注釈パス | 2.7 | 済（fingstack では 52.3＝30%） |
  | prelim の beams / ties / slurs | 0.0〜0.3 | **memo が効いている**（見なくてよい） |
  | `CreatePages` | 1.0 | 済（第5便） |

  ⇒ ★★★ **66.3 MB のうち約 31 MB＝47% が、まだ一度も二分されていない。**
  **次に着手するならこの 3 つのどれか**で、**どれも「割ってみる」から始まる**
  （どこに何が在るかは未知＝この便の 6 段はどれも割ってから見つかった）。

  ★★★ **全部割った。`perf-plain1k` の打鍵 60.2 MB の完全な地図**（第8便の後・単位 MB）:

  | 段 | MB | % | 状態 |
  |---|---|---|---|
  | `L1-break`（DP） | 19.3 | 32% | **閉じた**＝本質的・2 案とも測って却下 |
  | `4-render` | 8.5 | 14% | **床に近い**（構造上の下限 約 7.5・第7便） |
  | `1-collect` | 6.0 | 10% | 梁の bake は返した（第8便）。残りは未分解 |
  | **`F4a-edgeBeams`** | **5.0** | **8%** | ⚠️ **梁検出の 1 軒目**（下） |
  | **`P35`＝`DetectBeamGroups(prelimScore)`** | **4.4** | **7%** | ⚠️ **梁検出の 2 軒目**（下） |
  | `P4-prelimAnnotations` | 2.7 | 5% | 済 |
  | 最終注釈パス | 2.7 | 5% | 済 |
  | `CreatePages` | 1.0 | 2% | 済（第5便） |
  | `P1-beams` / `P0` / `P2` / `P3` / `F1`〜`F3` / `L3` | 0.0〜0.3 | — | memo が効いている。見なくてよい |

  ⇒ ★★★ **次の島＝梁の検出が別々の家で 2 回走る**（§2A「同じ量の 2 軒目」）。
  **合わせて 9.4 MB＝plain1k の打鍵の 16%**。
  **⒜ `MultiStaffLayouter.StaffBeamGroupsOf`**（`F4a`。**system 0 の縁の梁**を得るために譜全体を検出。
  `P1-beams` が 0.3 で済むのは**この 1 軒目と memo を共有している**から）／
  **⒝ `ElementCoordinator.DetectBeamGroups(prelimScore)`**（`P35`。**memo を共有していない**）。
  ⚠️⚠️ **畳む前に読むこと**——**2 軒は*違う Score*を見ている**。
  ⒜ は**その譜の全声部＋その譜の連符だけ**（`StaffBeamScoreOf` の remark が理由を書いている：
  連符境界で梁が切れるので他譜の連符が混ざると別の位置で切れる）、
  ⒝ は **prelim の主声部**。**単一譜の本では答えが一致するが、構成上の同値ではない。**
  ⇒ ★ **畳むなら「memo の鍵に Score の同一性を入れる」ところから**。
  **鍵を間違えると静かに別の梁を返す**——赤が出ない種類。
  ⚠️ **v2bow1k では ⒜⒝ とも 0.1〜0.2 MB**（梁が少ない）。**代わりに prelim の弓が首位**
  （`P3-slurs` 9.9・`P2-ties` 4.8＝打鍵 83.7 の 18%）——**本の形で首位が変わる。**

  ★★★ **`4-render` は割って、第7便で半分返した（下の ⑹''）。いまは*床に近い*。**
  **割った内訳**（plain1k・第7便の前）: **`R1-RenderTo` 5.3 ／ `R15-Dispose`（文書組み立て）7.2 ／
  `ToSvg` 0.0**（⚠️ **`ToSvg` は 0 MB で 188 万文字を返す**＝**組み立ては `Dispose` に居る**）。
  **`R1` の中**: `D3-pages` 4.9 ／ `D2-prep` 0.4 ／ `D1-resolveDataPos` 0.0（fingstack では 1.5）。
  ⇒ ★★★ **第7便のあと render は 8.5 MB で、床は約 7.5**——
  **本文を組む 3.76 ＋ それを string にする 3.76 は、この API が string を返す限り避けられない**。
  **残る無駄は約 1 MB なので、ここはもう掘らない。**
  ⇒ ★★ **断片キャッシュは効いている**——**ページ走査の「その他」は約 1.1 MB しかない**
  （無変更 system は再描画されず、記録した文字列を貼っているだけ）。
