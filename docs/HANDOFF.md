...
★★★ **㉓ 数**: **開始時**: HEAD `1e80a9b2`・未 push 9・木 clean・未追跡 0（**`Session-Check -Build -Test` は走らせていない**）。**終了時**: HEAD `31273028`（**出荷物の最後の変更は第 18 便 `851e5469`＝LP の打楽器 33 種と調弦 25 種。第 13・14・16・17・19〜22 便は production 無変更**）・未 push 51（うち 3 本は doc）・**台帳 843 点／ss 非ゼロ 210／総和 23.216135909／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249／追跡 `.lys` 599＝全部不変**（⚠️ **第 18 便は Core を 7 ファイル触ったのに、絵は 1 枚も動かない**——**既存の綴りの振る舞いを 1 つも変えていない**からで、**台帳・snapshot・追跡 `.lys` が全部不変なのがその測定**。第 13・14・16・17 便は Core を 1 行も触っていない）。**full `scratch/p375/run33.trx` 8414 / 0 / 4 / 8418**（第 22 便＝+21＝注釈オペランド。⚠️ `run32` は 1 本赤＝**私が足した LP 引用の綴りが `lily/` 前置きで「名前を挙げていない引用」に数えられた**＝`CitationsThatNameNothing_DoNotGrow`。既に通っている同じ引用の綴りに揃えた）・第 21 便末 `run31.trx` 8393 / 0 / 4 / 8397（+40＝和音品質 40 綴り）・第 20 便末 `run30.trx` 8353 / 0 / 4 / 8357（+127＝drum 126 綴りの掃き＋mark の網 1 本）・第 19 便末 `run29.trx` 8226 / 0 / 4 / 8230（+33＝調弦語 32 の摂動ほか）・第 18 便末 `run27.trx` 8193 / 0 / 4 / 8197（+68＝打楽器 3 theory・調弦 1 theory の case が大半。⚠️ **`run24` は 1 本赤**＝`ApproximationInventoryTests` は**生成物 `docs/APPROXIMATIONS.md` の行番号**を見張るので、exporter が 11 行縮んだだけで赤くなる⇒ `LILYSHARP_UPDATE_DOCS=1` で再生成・差は 4 行）・第 17 便末 `run23.trx` 8125 / 0 / 4 / 8129・第 16 便末 `run22.trx` 8119 / 0 / 4 / 8123（drum 照合 31 本）・第 15 便末 `run21.trx` 8088 / 0 / 4 / 8092（表の番人 +1）・第 14 便末 `run20.trx` 8087 / 0 / 4 / 8091（摂動 136 本ぶん増）・第 12 便末 `run19.trx` **7951 / 0 / 4 / 7955**（⚠️ **第 12 便で 9 本*減った*のは正しい**＝matrix の MIDI 行 3 綴り × 3 主張が言語から消え、代わりに「退役した綴りは断られる」2 本が入った。第 11 便末 `run16.trx` 7964・第 10 便末 `run15.trx` 7956・第 9 便末 `run14.trx` 7953・第 8 便末 `run13.trx` 7947・**第 1〜7 便末は `run1`→`run12` で 7816→7869**。**13 便とも合計を読んでいる**＝新 test は差引 144 本（第 12 便は −9）（第 8 便の 78 ＝ 26 綴り × 3 主張）。⚠️ 第 5・6 便は既存 test を**書き換えている**ので増分はそれより小さい）。★ **第 7 便は 7 つの context の配線を丸ごと替えたのに既存 7854 本が全部緑**＝**行の読みが 1 つになっても、読めていた綴りの答えは変わっていない**。⚠️ scratch は **`p375`**。⚠️ §7 3.5: 第372 を `-Archive 372` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝**第372 の天井 commit `9f6df2a3` が未 push のまま**。**push はユーザー（未 push 51）**。★ **LSP は 12 度配布した**（第 19〜22 便は Core も Lsp も無変更なので再配布していない）＝`tools/Deploy-Lsp.ps1` が `0.6.0+851e5469` を `yotsuda.lilysharp-0.5.0-win32-x64\server` へ（95 ファイル byte 一致）⇒ **ユーザーは Reload Window が 1 回要る**（package.json は動かしていないので 2 回は不要・記憶 `feedback_lilysharp_lsp_deploy_target`）。

⇒ ★★★ **㉔ 次の一手＝✅ ⒜（LP の語彙をどこまで入れるか）は第 18 便のユーザー決定「両方とも入れて」で閉じた**（打楽器 30→63・調弦 7→32 語）。⚠️⚠️⚠️ ★★★ **⒜‴ ユーザー決定が 1 件立った＝`@feather` を*実装する*か*退役させる*か**（第 22 便が証拠を揃えた）。**文法にも GRAMMAR §8.4 にも popup にも在り、33 冊が書いているのに、ページには 1 滴もインクが出ない**（上 ㉒′）。⚠️ **MIDI 行の前例（0 冊＝迷わず除去）は使えない**——**書かれている**ので、退役させるなら 33 冊に「やめてくれ」と言うことになる。★ **実装側の仕様は `FeatheredBeamTests` に書いてある**（grow-direction RIGHT は二次梁が左で収束し右で開く・LP `beam.cc:1039-1082`）。**着手はユーザーの承認待ち。** ★★★ **⒜″ 摂動網の残りは *paper キー* だけになった**（和音品質は第 21 便・`@` の引数は第 22 便で閉じた）。⚠️ **やり方は 3 便で確立した**: **⑴ 陽性対照を 1 本置く**（同義の 2 綴りがページを変えないこと）／**⑵ フィクスチャがその鍵を表現できるか先に確かめる**／**⑶ 毒を入れ、外れたら毒ではなく*計器と経路*を疑う**（表が 2 つあるかもしれない）。**残った札は 3 枚**: **⒜′ `InstrumentDefaults.GetTuning` は preset 5 族しか答えない**（`violin`／`cello`／`viola`／`mandolin`／`banjo` の preset を書いた part を tab で描くと**ギターに落ちる**）＝**調弦は在るのに preset が繋がっていない**。**触っていない理由は 1 つ**——**既存の本の tab の絵が変わる**（`instrument violin` ＋ tab の本があれば 6 弦→4 弦）ので、**射程を数えてから**（第 12 便の型）。⒝ **摂動網の残り**（paper キー・chord quality 35・`@` の引数つき形）。⒠ **drummap の `${1:hh}`＝UX 案件**（drum 語彙を出すと親切。`drummap { ▮ }` の本体に語彙の家が無いので context が 1 つ要る）。⒡ **第373 ⑼⒜（probe 24 冊の「量を測ったか／問いに答えたか」の仕分け）**も先頭級のまま。**ヘッダの綴りの島は閉じた**——⑻ の 25 綴りが全数で、**残る未点検はゼロ**（`{` を開く構文は score／staff group 5／part／phrase／form／lyrics／chords／section／fonts・paper・layout とその入れ子／音楽の容れ物で尽きる）。⚠️ **閉じたのは*補完の*読み手だけ**: 同じ「2 語しか読まない枠」を持つ道具が他に居ないかは見ていない（`SemanticTokens`・折り畳み・`BarCountFix` は別の木＝**実構文木**を読むので原理的に無縁、という*予想*まで）。⒢ 第369 ⑸⒝（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。
