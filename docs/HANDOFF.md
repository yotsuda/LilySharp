...
- **T3. ▶ 残り 11 冊の error**（`scratch/p317/tab/check-final.csv` が名前を持つ）。
  **内訳は長い尾で、もはや 1 族ではない**: **和音の綴り（大文字ルート）26 が最大の族で機械的に直せる／
  複数 part 3 冊（`Untitled-3` / `がくふ` / `ぐるぐるワンダーランド`）は移行器が対象外にしている／
  残りは grandStaff・lyrics の対応づけ。** ⚠️ **T1 の前にこれを片付ける必要はない**——
  **T1 の射程はスラーを持つ本で、error 本とは別の集合。**
