...
⇒ ★★★★ **⑹ 次の一手**: **⒜ push は禁止のまま**。**⒝ F-fonts 段階 ⑵ の残り 5 role**（上 第 3 便の判定）——fingering／figuredBass は `fonts.StepOf(role, default)` で `GlyphMetrics.AtFontSize(step+plan)` を引く形（chordName の臨時記号 `AccidentalStep` と同じ）＝`FingeringGlyphRun`／`FiguredBassGlyphRun` の `Em`・`Design`・`Font`・`Pieces`・`Width`・`InkTop`・`InkBottom` に fonts（か step）を通し、`FingeringEngraver.DigitRun` のメモは step 0 だけに限る。**着手前に fonts の無い経路を全部数える**（上の一覧が起点・RULES §5.0 の「起票の数は数え直す」）。tabFret は `TabConstants.FretFontSize 3.0`（tab 数字の em は tab 幾何の島＝T 系の判断が要る）。notation 3 つ（clefOctave・meter・tabFret）は「serif/sans の broad binding を受けない」規則があるので、size/style も同じ扉（`notation` か leaf を名指したときだけ）で足りるか先に決める＝**ユーザー判断**。**⒞ F-fonts の拾い箱（観測者 0）**: sustain の "Ped." グリフ列は `pedal step` に従わない（doc に明記・LP は `SustainPedal.font-size` で縮む＝乖離）／双子の role→grob 表は近似（`@text` の expressive は Lily# では dynamics role・双子は TextScript）／`size` を双子に書く手は無い／instrument 3.0・stanza 2.4 の既定は LP と違う（台帳点を作ってから動かす）。**⒟ 第353 ⑹ の並びはそのまま**（`time none` の縁 3 つ・`lymelmel.lys` stale／`lyrics-verses` bar 4 の 0.19／MIDI の途中 `partial`／拍グリッド／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8b・U8／A/B/D/E／C⑴／G）。**⒠ 段端の floor の regime**（第351 ⑹⒟）は今も未測。
...
- ✅ **F-fonts. `fonts { }` に*サイズとスタイル*を持たせる（2026-09-07・第342 起票。✅ 文法はユーザー承認済み——「その形で起票して。VS code editor の自動補完も、それに合わせて修正して」・✅ **第354 で実装＝段階 ⑴＋⑶（§1 第354）**）**
  ★★★ **入ったもの（第354）**: 文法どおり（`as` 転送・`step ±n`・`size n`・`bold`/`italic`/`regular`・順不同・次のキーで終わる）。**読み手＝`FontPlanReader.ReadAttributes`／`TextFontPlan.Binding(Step,Size,Style)`／`SizeOf`・`StepOf`・`StyleOf`（leaf→group→既定・属性ごと独立）／`ScoreTextMetrics.Size(role, default)`・`Style(role, default)`・`StepOf`**。**従う role の表＝`TextRoles.PlanReachOf`**（20＝title・composer・instrument・lyricText・stanza・chordName・fretFrame・tempo・mark・pedal・navigation・text・dynamics・partCombine・barNumber・tuplet・volta・ottava・bend・tabTechnique。第 1 便 13・第 2 便 +4・第 3 便 +3）——**validator（LYS8018 警告）とページの Theory `FontAttributeTests.TheReachTable_IsWhatThePageDoes`（全 role を `step +6` で描いて表と両方向に突き合わせ）が同じ表を読む**。診断 LYS8015（家族への属性・キー前の属性）／8016（数値なし・範囲外 step ±12・size 0.5..20）／8017（step と size 両方）／8018。**双子**＝`\layout { \context { \Score \override Grob.font-size = #n / font-series / font-shape } }`・title/composer は `\markup \fontsize`・navigation は markup 内（RehearsalMark を汚さない）・`size` は warning。**LSP**＝キー後に属性・`as` 後に家族・`step`/`size` 後は空・開いた entry では属性＋キー。**TextMate**＝`as FAMILY`／属性語／数値。**残り＝段階 ⑵ の後半 5 role**: figuredBass・fingering（Emmentaler 数字＝文字 em の意味が無い・glyph の magstep を `StepOf` で連れる形・fonts の無い静的経路 25 か所級＝§1 第354 第 3 便の判定）・tabFret（tab 幾何の島）・clefOctave・meter（notation＝broad binding の扉をどうするかはユーザー判断）——表に足すときは全読み手を `fonts.Size` に付け替えてから（Theory が赤で迎える）。instrument・stanza・partCombine・ottava は第 2 便、tabTechnique・bend・fretFrame は第 3 便で足した。⚠️ **sustain の "Ped." はグリフ列で `pedal step` に従わない**（doc に明記）。
