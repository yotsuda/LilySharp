...
- **⑼ ★★ 残っている食い違いが 1 つ、測って置いてある**＝**双子の入れ子**（§2F ⑸）。
  `form { |: A … }` の A 自身も `|:` で始まる綴り（**あなたの本 2 冊**）で、
  **ページは 1 つの反復に畳む**（ドット 4 個で不変）のに**双子は `\repeat volta` を 2 重にする**
  ＝**LP は 4 回鳴らす**。⇒ **直すには双子が「この項目は小節内容か」を再実装することになる**
  （collector の `HasMeasureContent` の 2 つ目の綴り＝§5.2.1②）。**設計判断なので置いた。**
- **⑽ ★ 仕様書の網は `GRAMMAR_FOR_LLM.md` しか読んでいない**（第174 実測）。第173第9便が
  ...
  **網ではない**（§2F に起票）。
- **⑾ 未追跡 0・未 push 24**（第171 の 2 便＋第172 の 4 便＋第173 の 12 便＋この 6 便）。
...
- ★★ **繰り返し縦線の島＝第174 で 4 便入れた。残っているのは 2 つだけ**（2026-08-15）。
  ...
  **残っているのは 2 つ**:
  ⑸ ★★★ **双子が「二重の `|:`」を入れ子にする**（**あなたの本 2 冊**＝`Addicted To Love`・
     `青い珊瑚礁`）。**`form { |: A … }` の section A 自身も `|:` で始まる**綴りで、
     **ページは 1 つの反復に畳む**（`HandleBarline` が `_pendingStartBarline` を立て直すだけ＝
     **反復ドット 4 個で不変**・LYS4017 も沈黙＝**正しく対**）のに、
     **双子は `\repeat volta 2` を 2 重に出す**＝**LP は 4 回鳴らす**（実測・第174）。
     ⚠️ **直すには `EmitInlineRepeat` が「直前に小節内容があったか」を判定することになり、
     それは collector の `HasMeasureContent` の 2 つ目の綴り**（§5.2.1②）。
     ⇒ ★ **設計判断が先**: **双子が再実装するか、collector の答えを読むか。**
     **後者は「4 本を 1 本へ畳む」の第一歩そのもの。**
  ⑹ ★ **section 音楽中の片側 `:|` を MIDI が鳴らさない**（**133 冊中 0 冊**が書く）。
     ...
     **鳴らすなら MIDI が collector の平らな列を読む形にすること**（＝⑸ と同じ設計判断）。
