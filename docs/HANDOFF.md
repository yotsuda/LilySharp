...
- ★★ **⒬ `IReadOnlyList<T>` を `foreach` する腕はまだ他に在る**（第443 起票・**値段は未測定**）。
  第443 が測ったのは `SkylineMath` の 1 本だけで、**そこだけで 918,863 B／打鍵（旧打鍵の 14.3%）だった**
  ——**箱はちょうど 64.0 B で、動的 PGO を有効にしても消えない**（両方の構成で同一の実測）。
  **母集団は `Svg`＋`Rendering` で `IReadOnlyList<...>` を引数に取るメソッド 135 本**（`foreach` は全体で 1,418 箇所）。
  ⇒ **値段は第443 の脚がそのまま使える**＝`GC.GetAllocatedBytesForCurrentThread` で呼びの scope を測る
...
### 1.1 第443セッション（2026-09-20・YT-DELL2）
