...
最終更新 第355セッション（2026-09-09）＝**入り方は第298〜第354 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。⚠️ **開始時に状況が 1 つ変わっていた**: ユーザーが第348〜354 の 57 commit を `cf6edb6f` に**まとめて push 済み**（`origin/master` ＝ HEAD・未 push 0・CI 緑 34286219497）＝§1 第354 の「未 push 57」は過去の数。**第354 ⑹ の並びで「決定済み・未実装」は §3 の `marks stacked / beside`（2026-09-02 第322 ユーザー決定・前提の F2 は第324 で閉じ済み・低優先と書かれて 31 便放置）だけだったので、それを実装した**＝section 名の箱と同じ小節の tempo の並べ方を選ぶ表示オプション。**掃き 922 冊 MOVED 0**（`marks` を書く本は追跡・ユーザーとも 0 冊＝既定 `stacked` は今までの絵そのもの）・台帳 810 不動・snapshot 248 不動。**製品は commit 済み（下 ⑸）・push はユーザー**。**骨は 5**:

★★★★ **⑴ 文法（§3 の決定どおり・置き場は `fonts`/`paper` と同じ 2 段）**: **top-level `marks stacked|beside`＝ファイル既定／score 本文の項目 `score main { marks beside  staff m }`＝その score（後勝ち・ファイル既定を置き換える）**。node は `pitch` と同じ `PropertyAssignment`（新 node 種 0）・3 語目は語の上で error（`ParseMarksDirective`）・top-level 2 度目は LYS4005（`DuplicateGlobalSettingValidator`）・part ヘッダと music では既存の網が拒む（実測 `scratch/p356/mk3`/`mk4`）・**歌詞の音節 "marks" は生きている**（`mk5`＝描かれる）。**家は `Semantics/MarkArrangement.cs`**（`Modes`＝[stacked, beside]・`ReadProperty`・`FileIsBeside`（green finder・後勝ち）・`ScoreArrangement`）。配管: `RenderSpec.MarksBeside (bool?)` → `MeasureCollector.MarksOverride` → `CollectDefinitions` 末尾で `_meta.MarksBeside = override ?? file`（`MetaMatchesShifted` が比べる・**`Reset()` は Fonts/Paper と同じく消さない**）→ `ScoreContent` → `Score`/`MultiStaffScore.MarksBeside`。⚠️ **踏んだ穴 1 つ＝`MultiStaffScore.FromScore`**（単譜の本が必ず通る wrap）が score-global の bit を写しておらず、**最初の版は単譜で beside が効かなかった**（番人が赤で教えた・Fonts/Paper は写していた＝同じ表に 1 行足す型）。**`IncrementalCompiler` は `_paper` と同じ門で `_marksBeside` の変化にキャッシュを捨てる**（番人 `AnArrangementEdit_RendersIdenticalToFullRecompile_BothWays`）。

★★★ **⑵ 幾何（LILYSHARP-OWN・LP に chart pair は無い＝2026-07-29 の tempo 移植で退役した `CoPlaceTempoWithLabels` の並びを*オプションとして*戻した）**: **`beside`＝箱の左端は行頭 `Indent + 0.3`**（`CalculateXPosition` の boxed-label 腕で `lineStartBarlineX`/`prefixMarkAnchorX` を飛ばす・**mid-line の箱は小節線中心のまま**）、**tempo の ink 左 = 箱の右端 + 0.8（`BesideTempoGap`・LILYSHARP-OWN）・baseline = 箱の文字の baseline（`BesideTempoBaselineUp`＝`labelYUp − LabelBaselineBelowCentre`）**。**対は 1 つの規則 `BesidePair`**（小節頭の tempo＝Beginning かつ `AnchorItemIndex <= 0`・箱は priority 順の最初＝SectionLabel が Rehearsal より先）を **`Calculate`（配置）と `BoxedLabelXWindows`（和音行の予約＝tempo の右端まで窓を広げる）が同じく読む**。**小節途中の `tempo` は音符列のまま**（番人 `AMidMeasureTempo_KeepsItsNoteColumn_UnderBeside`）。**stacker は対を 1 union で置く**: tempo layout が `BesideOfSourceIndex`（箱の SourceIndex＝memo の per-system filter で index がずれても生きる鍵）を持ち、`PlaceMusicMarks` が箱の box を tempo の ink 右・音符の天まで広げて置き、動いた分だけ tempo も動く（to-coda 対と同じ形・番人 `ANoteUnderEitherMember_LiftsThePairTogether`＝tempo の列だけに g'2. を置いて両方が同じ量上がる）。**pre-stack でも tempo 自身の chord ceiling が対を持ち上げる**。描画側は変更 0（箱は中心 X・tempo は左 X＋baseline のまま）。

★★ **⑶ 読み手**: **双子 `lysc ly` は `beside` を書けないので warning**（"marks beside is not exported: the twin stacks…"・`fonts … size` と同じ規則・stacked は無音で双子は不変）。**LSP**: `AfterMarks`（top-level と score 本文だけ・part/music/文字列では出さない）・`GetMarkArrangementCompletions`（語は `LanguageVocabulary.MarkArrangements`＝compiler の順）・top-level と score 本文の keyword 一覧に `marks`（`$0`＋再表示）・`GlobalSingletonKeywords`。**TextMate**: `#directive-value` に `marks (stacked|beside)`・keyword 列に `marks`。**docs**: GRAMMAR §2.3 `MarksDecl`＋§7 ScoreItem＋Keyword production・GRAMMAR_FOR_LLM（骨組み例・keyword fence・score 節）・SYNTAX_REFERENCE（keyword 表・Music Marks 下に新節・score 節）・CHANGELOG 0.7.0 Language・APPROXIMATIONS／magic_constants 再生成（OWN 131 → 133・UNWATCHED 51 → 52＝`BesideTempoGap` の "observed by: no ledger point"）。

★★ **⑷ 番人＝`MarkArrangementTests` 17 本**（語彙・両位置と score 優先の Theory 7・3 語目・重複・**`marks stacked` 明示 ≡ 無記入（data-pos 伏せでバイト同一）**・beside の幾何 3 本・双子の warning・増分≡full）＋ `EditorColouringTests.TheMarksRule_ColoursExactlyTheCompilersArrangements`＋補完 3 本（`ValueContextCompletionTests`）＋`CompletionVocabularyTests` 2 本（description・両 home で compile）。**目視 1 回**（`scratch/p356/mk1-beside.png`／`mk0-stacked.png`・`lysc png --scale 4`）: beside は "[Intro] ♩=117" が 1 行・箱は行頭、stacked は箱が key 列でテンポの上＝§3 の決定文どおり。**⒞′ の RULES 候補（第354 ⑹）は RULES §5.1 に 1 項として足した**（「広い付け替えを別便に回す根拠は取りこぼしが黙ることであって数ではない」）。§3 の stale 2 行（途中の `partial`・`_ "text"` の「未実装」）を第352 実装済みに直した。

★ **⑸ 数**: **開始時裏取り**（`tools\Session-Check.ps1 -Build -Test -Scratch p356`）: HEAD **`cf6edb6f`**・**未 push 0**・`origin/master`＝HEAD・木 clean・未追跡 0・build 0 エラー／Core 0 警告・**full 7410 / 0 / 4 / 7414**（`scratch/p356/run1.trx`＝第354 の終了時と一致）・`gh run list` 緑（最新 34286219497）。**途中**: `run2.trx` 7435 / 2 / 4 / 7441＝赤 2 は目録 2 本（APPROXIMATIONS・magic_constants＝`LILYSHARP_UPDATE_DOCS=1` で再生成）。**終了時**: **commit 2 本（`b3353c06`＝製品 Core 20 ファイル（新規 1）＋Lsp 3＋TextMate 1＋番人 6 ファイル（新規 1）＋CHANGELOG＋GRAMMAR／GRAMMAR_FOR_LLM／SYNTAX_REFERENCE／APPROXIMATIONS／magic_constants・2 本目＝この HANDOFF＋ARCHIVE＋RULES）・未 push 2・木 clean・未追跡 0・full 7437 / 0 / 4 / 7441（`scratch/p356/run3.trx`・+27＝`MarkArrangementTests` 17（Theory 展開込み）＋色 1＋補完 3＋語彙 2＋その他の Theory 行）**・台帳 810／snapshot 248／追跡 `.lys` 598 全部不動。§7.5（`-DiffBase cf6edb6f`・commit 後）: Core `+` 473・LILYPOND-REF 0・LILYSHARP-OWN 2——**REF 0 は監査対象**（RULES §7.5）: 読み直した結果、LP の式は 1 本も足していない＝全部が「1 bit の配管」と LP に対応物の無い並び（OWN 2＝`BesideTempoGap` と `CalculateXPosition` の beside 腕）で、既定経路の幾何は 1 行も変えていない（掃き MOVED 0 がその証明）。⚠️ **scratch は `p356`**（`p355` は第354）。⚠️ **§7 3.5**: 第353 のブロックを `Session-Check.ps1 -Archive 353` で ARCHIVE の先頭へ・第354 の経緯は下の見出しの下に残した。⚠️ 掃きの base exe は `git worktree add --detach scratch\p356\wt-base HEAD` から Release build（`scratch/p356/exe-base`）・終了時に worktree は外した。⚠️ ripple: main が busy のとき standby へ落ちた 1 回目は `cd` だけが走る（memory どおり・今便 3 回）。

✅ ★★ **第 2 便（ユーザー「有利なら着手」）＝⒟ の 2 つを測った（製品 0・記録＋コード内 remark・commit 3 本目）**: **⑴ `@mark("A")` と section 名が同じ小節**（`scratch/p356/mk7`）＝対の相手は SectionLabel、"A" の箱はその上に積まれる＝規則どおり。**⑵ staffless で beside**（`mk6`）＝**割れる**: tempo の ink（右端 ≈ 15.6）が 2 本目の小節線 13.90 を跨ぎ、`C` は 16.82 に押し出される。**ただし根は `marks` ではない**——**`BoxedLabelXWindows` の窓は記号を*動かす*だけで行の spacing を 1 も広げない**（第251 から）: 札無し／`Intro`／`IntroductionLong` で bar 1 は 3 冊とも 13.90、`C` は 4.50 / 8.61 / **25.92（bar 3）**（`mk8`/`mk6s`/`mk9`）。**§2 A の先頭に起票**（直し方＝窓を line-start の wish にする・LILYSHARP-OWN・出力が動くので承認要）。`BoxedLabelXWindows` と `ChordNameEngraver` の remark に数を書いた（「reserves」は shift の意味だと明記）。**着手しなかった理由**: 出力が動く（staffless で札を持つ全冊の snapshot＋ユーザーの chords-only 本）＝承認ゲート・この便の文脈利点は測定側に在って修理側には無い。

✅ ★★★ **第 3 便（同じ指示の 3 度目）＝§2 A 先頭の spring 化を試作し、動く本を全部数えて patch として置き、ユーザー承認（「広げて良い。patch をあてて」）の後に当てた（commit `c0c29692`）**: 出力が動く変更なので先に承認ゲート（RULES §5.1）＝数を出してから訊いた（掃き MOVED 8＝追跡 8・ユーザー 0／snapshot 4 枚／台帳不動）。**形**: `MusicMarkEngraver.StafflessLabelLineStartReach`（箱の右端・beside なら tempo の右端・＋`ChordNameEngraver.SymbolGap`＝window の shift と同じ 1 数）を `LineStartSpringForLine` の `ownFixedFloor` に max で足す＝gate と layout は同じ 1 実装。番人 `StafflessLabelSpacingTests` 4 本。**このセッションでやった理由**: 窓の幾何と `LineStartSpring` の frame を今読んでいた＝試作 76 行・掃き基盤が温かい。**終了時の数**: full **7441 / 0 / 4 / 7445**（`scratch/p356/run4.trx`・+4＝番人 4 本）・§7.5（`-DiffBase cf6edb6f`）Core `+` 567／REF 0／**OWN 4**（+2＝`StafflessLabelLineStartReach` と `LineStartSpringForLine` の床）・台帳 810 不動・**snapshot 248 枚のうち 4 枚再ベース（承認済み・要素数不変）**・未 push 6・木 clean。⚠️ **上の ⑸ の「snapshot 248 不動」は第 1 便の数**——最終は 4 枚動いている。

✅ ★★ **第 4 便（同じ指示の 4 度目）＝第 3 便が「未計測」と名指した staffless の `|:` 開きの縁を測って閉じた（commit `229bce70`）**: `scratch/p356/mk10`（staffless・`form { |: Intro :| }`）＝箱は `|:` の線 3.50 に乗る（`lineStartBarlineX`）のに床は 0.3 起点だったので、`C` は窓に 11.81 まで押されて後ろの余白が 6.2（他の bar は 9.4）。**さらに beside の腕が `|:` の anchor まで飛ばしていた**＝staffless では箱が反復記号の上に刷られる形（実書 0・番人が先）。**直し 2 行**: `CalculateXPosition` の boxed 腕は描かれた `|:` を両 arrangement で採る（beside が飛ばすのは key/clef の anchor だけ）／`LineStartSpringForLine` は `prefix.Columns.HasBar ? BarX : 0.3` を箱の左端として reach に渡す（`BarX` ≡ `lineStartBarlineX`＝mk10 で両方 3.50）。**掃き 922 冊: `c0c29692` 対 head MOVED 0**（`|:` で行を開く staffless の札本は追跡・ユーザーとも 0）・snapshot 0 枚・番人 `StafflessLabelSpacingTests` +2 行（箱の左端が縁より右・記号は bar の中・後ろの余白は札を隠した本と同じ）・full **7443 / 0 / 4 / 7447**（`run5.trx`）・§7.5（`-DiffBase cf6edb6f`・commit 後）Core `+` 578／REF 0／OWN 4。GRAMMAR §2.3 と `MarkArrangement` の remark に「`|:` の後ろ」を 1 句。

⇒ ★★★★ **⑹ 次の一手**: **⒜ push はユーザー**（この便の commit 8 本が未 push）。**⒝ §3 に「決定済み・未実装」は 0**（`marks` が最後だった）。**⒞ F-fonts の拾い箱（第354 ⑹⒞・観測者 0）はそのまま**: 「+」の 2 つの em／sustain "Ped." が `pedal step` に従わない／双子の role→grob 表は近似／instrument 3.0・stanza 2.4 の既定は LP と違う（台帳点を作ってから）。**⒟ §2 A 先頭＝staffless の札の窓を spring にする**（第 2 便で測った・承認要・`marks beside` を staffless で使う本はそれまで割れる＝doc には書いていない・着手時に GRAMMAR の `marks` 節へ 1 行）。**⒠ 第353 ⑹ の並びはそのまま**（`time none` の縁 3 つ・`lymelmel.lys` stale／`lyrics-verses` bar 4 の 0.19／MIDI の途中 `partial`／拍グリッド／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8b・U8／A/B/D/E／C⑴／G）。**⒡ 段端の floor の regime**（第351 ⑹⒟）は今も未測。
