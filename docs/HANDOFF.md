...
⚠️⚠️ ★★★ **⑸ 失った 9 対の正体＝ループではなく gate の幅**（**LP の `-ddebug-page-breaking-scoring` と Lily# の `LayoutEngine.DebugPageBreakingScoring` フック（新設・production では null）で得点を並べた**・`scratch/p322/fx/*-dbg.err` / `*-lys-scores.txt`）: **⒜ `未来予想図Ⅱ`（8,8 → 16）は LP 27 段 53.70 / 26 段 54.06＝差 0.36・Lily# 56.94 / 56.44＝逆向き 0.50 ＝knife-edge**（tab 本は LP の差 0.06）。**⒝ `Alone Again`（4,4 → 8）は LP が 19 段を inf**（Intro＋A の 8 小節は LP では 1 段に*置けない*）**のに Lily# は置ける**＝**break gate の min 幅と `CalculateLineForce` の compress_line 線形近似**（remarks が既に自白している「per-measure SUM・最初の spring が block するまでは正確」）。**再現 `scratch/p322/fx/alone-intro8.lys`（LP 4|4|4・Lily# 8|4・`alone-again-head-page1.png` の 1 段目が詰まっている）**。**⒞ `Livin' It Up`（4,4 → 2,2,2,2）は Lily# の 22 段が Σf² 6.97・LP 1.89**＝末尾の 4 小節段が Lily# でだけ圧縮されている（凸罰 f−2f⁴ が割らせる）。⇒ **§2 T7 に F12 として起票**（閉じるなら gate に compress_line の blocking walk＝per-measure SUM の設計変更・F3 の門も読む）。⚠️ **推定の高さは LP より高め**（`未来予想図Ⅱ` で Lily# は 29 段を `min_page_count > 4` で skip・LP は 29〜36 を試す）——増やす側の刈り込みが早い。得点は単調に悪化するので今回は無害。
...
終了時: **commit 2 本**（**⑴〜⑶ の移植＋番人＋inventory 再生成／この HANDOFF**）・**未 push 23**・作業ツリーは README.md と samples/README.md の 2 件だけ・未追跡 0（`scratch/p322` は git 管理外）・Core 0 エラー 0 警告・**Windows Debug 6905 / 0 / 4 / 6909**（**`scratch/p322/full2.trx`。+9＝第321 比: 番人 8 本 ＋ snapshot 1 枚**）・**追跡 `.lys` 594**（+1）・**snapshot 244**（+1・既存は不動）・**台帳 759 点不動**（exact 594・本便は台帳を触っていない）・`docs/APPROXIMATIONS.md`（409 行）と `audit/magic_constants.csv`（925）は行番号だけ再生成（`LILYSHARP_UPDATE_DOCS=1`）。⚠️⚠️ **WSL ubuntu Release は取れない**（第313 ⑶）——**本便は Core を触っているので `push` 後に `gh run list` で読むこと。**
...
⇒ ★★★★ **次の一手**（**第321 の並びの ⑴ を本便が消費した。差し替えと追加**）: ★★★ **⑴ §2 T7 F12＝break gate の min 幅／compress_line の blocking walk**（`Alone Again`・`Livin' It Up`。**再現 `scratch/p322/fx/alone-intro8.lys`＝LP 4|4|4 / Lily# 8|4**。**設計**: gate は per-measure SUM（`MeasureSpringData`）しか運ばず `CalculateLineForce` は線形——LP の `simple-spacer.cc:232-287 compress_line` は spring を blocking 順に落として Hooke 和を減らす。**per-spring の min/ideal を gate に運ぶか、SUM のまま「min 和 > 幅」の判定だけ厳しくするか**は corpus に訊く（Alone Again の 8 小節線の min 和と幅を先に数える）。⚠️ F3 増分の門は SUM の vector を比較している＝運ぶ量を変えれば門の key も変わる）／★★★ **⑵ perf＝段数ループを静かな窓で測り直し、増やす側の候補 1 個 5 ms を削る**（⑹。**LP と同じ数を試すのは仕様**——削るのは候補ごとの定数: `EstimatedSystemDetails` の 200 record 生成と頁 DP の再走。`PreviewUpdateBench`/`EditKeystrokeBench` を base worktree `LilySharp-perfbase-7c43` と並べて、**1 サンプル目は捨てる**）／★★★ **⑶ F2＝section 名の箱を LP の RehearsalMark の位置へ**（**ユーザー決定済み・§3 第322**: 拍子記号の列＝`BuildPrefixMarkAnchorX` の Rehearsal 腕と同じ anchor・テンポはその下に縦積み。**台帳の SectionLabel 対照点（意図的 OPEN）を閉じる**・snapshot は `\mark` を持つ本が大量に動くので LP 双子で正当化・`fx4-mark-tempo` が陽性対照）／★★★ **⑷ T6＝スラー復元 138 冊**／★★ **⑸ T7 の C 37 冊を 1 冊ずつ**／**⑹ 以下は第321 の並び**: §2 T6 の残り／`audit/lpreg` 取り直し（**本便で段数が動く本は動くはず＝取り直し前に `REGENERATE.md`**）／§2 U8c の島／§2 U8b／§2 U8 の残り／§2 A/B/D/E／§2 C⑴／§2 G／★ **低優先: 表示オプション `marks stacked` / `marks beside`**（§3 第322・ユーザー決定済み・**⑶ を閉じてから**）。⚠️ **承認待ちは §2 T2 の 1 件**（`Lambada Complicada.lys`）＋**§2 T5 の ⏸ 1 件**（F2 は決定済みで待ちから外れた）。⚠️ **§2 F の開いている項目は第320 の ⒲ 1 件。**
...
- **T7. ▶ 起票（第321）＝絵の突き合わせ（286 冊全数）の結果＝族の表。計器は `scratch/p321/`（§1 ⑴）**
  ...
  | **F12** | 段数ループが**LP より安く**段を併せる／割る本（`Alone Again` bar 1-8 を 1 段に併せる・`Livin' It Up` 末尾の 4,4 を 2,2,2,2 に割る・`First Love` tab／`にんじゃりばんばん`／`未来予想図Ⅱ` の 8,8 → 16） | **得点を並べた**（第322 ⑸）: `未来予想図Ⅱ` は LP 27 段 53.70 / 26 段 54.06（差 0.36）に対し Lily# 56.94 / 56.44（逆向き 0.50）＝knife-edge。`Alone Again` は **LP が 19 段を inf**（Intro＋A の 8 小節は 1 段に*置けない*）とするのに Lily# は置ける＝**break gate の min 幅／`CalculateLineForce` の compress_line 線形近似**（remarks 参照）。再現 `scratch/p322/fx/alone-intro8.lys`（LP 4\|4\|4・Lily# 8\|4）。`Livin' It Up` は Lily# の 22 段が Σf² 6.97 で LP の 1.89 の 3.7 倍＝末尾 4 小節段が Lily# でだけ圧縮 | 7 冊 9 対（−9 の全部） | ▶ **ループではなく gate の幅**。閉じるなら compress_line の blocking walk を gate に（per-measure SUM の設計を変える＝F3 増分の門も読む） |
