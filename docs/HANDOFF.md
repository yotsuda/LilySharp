...
⇒ ★★★★ **次の一手**（**第323 の並びの ⑴⑶ を本便が消費した**）: ★★ **⑴ perf＝段数ループ＋DP の解きを静かな窓の Release で**（`EditKeystrokeBench`／`PreviewUpdateBench`・base は `a8ae0ad6` の worktree・1 サンプル目は捨てる）／★★★ **⑵ T6＝スラー復元 138 冊**／★★ **⑶ T7 の C 37 冊を 1 冊ずつ**（指標は KindMatched の対で）／**⑷ 以下は第323 の並び**: §2 T6 の残り／`audit/lpreg` 取り直し（本便で `\mark` 本の絵が動く＝取り直し前に `REGENERATE.md`）／§2 U8c の島／§2 U8b／§2 U8 の残り／§2 A/B/D/E／§2 C⑴／§2 G／★ 低優先: `marks stacked` / `marks beside`（F2 が閉じたので着手可）。⚠️ **承認待ちは §2 T2 の 1 件**（`Lambada Complicada.lys`）＋**§2 T5 の ⏸ 1 件**＋**第323 ⑶ の「戻すか」（本便の推奨＝戻さない・⑴ ⒣）**。⚠️ **§2 F の開いている項目は第320 の ⒲ 1 件。** ⚠️ **リリース 0.6.0**: CHANGELOG に F2 を足した・版番号 7 か所の bump と tag は未着手（週末判断）。
