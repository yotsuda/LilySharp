...
セッション終了時: 未追跡 0（scratch/p226 は git 外）・commit 9 本（`e3a190dc` ⒥ 移植 → `7dc67cc7` リードシート機能 → `6449a827` handoff → `8c13f304` 行頭 bar＋クリアランス → `ec79dc69` handoff → `6db864eb` stanza → `da61e52f` handoff → `3650fd19` ⒦ per-voice edge → 閉幕 handoff）・suite **Windows 5711/0/4・Linux（WSL）5711/0/4＝両 OS 完全緑**（総数 5715＝−2 は退役台帳点）・snapshot 220 中 **計 18 枚動き承認**（⒥ 便 2＋機能便 10＋行頭 bar 便 5＋⒦ 便 1・全部帰属付き）・台帳 **560 点・ss 非ゼロ 111／総和 3.950175326・count 点 107／非ゼロ 2**・追跡コーパス 569 冊・Core 0 警告。**CI は `e3a190dc`＝32365572494・`7dc67cc7`＝32368962646・`6449a827`＝32369111798・`8c13f304`＝32370832384・`ec79dc69`＝32370848636 の success を全部本便で読んだ**。`6db864eb`（stanza 便）と閉幕 handoff の CI は次便が読む（commit は計 7 本＝⒥・機能・handoff・行頭 bar・handoff・stanza・閉幕 handoff）。
...
- **⑸ ★★★ 次に触るなら＝残債**: ⒞ CoPlaceToCodaWithLabels 鏡像／▶ perf（歌詞打鍵の章はほぼ完了＝55.1 vs 非歌詞 45.3。残り ~10 MB は hyphen／apply／非歌詞 L5/L9 等の小粒）／⒡ 配管 6 site／⒣ removeEmpty/pedal の score 移行検討（別便＝ユーザー指示）／✅ **⒦ 歌詞 sliver 族は第226 続き便 5 で閉じた**（`3650fd19`＝per-voice edge の 1 provider・予測 3 本桁着地）。**残った 1 問＝多声列の自然 spring の最太頭課金 +0.0732**（`no-bind.skip-gap` が観測者のまま。LP の ideal は外声の太頭に盲目＝**note-spacing モデルの問い**——多声自然間隔の regime で対を開いてから・lyric とは別族）／小粒: twin の歌詞行・`lines` twin 未輸出・マークの X・chord-row の上帯スカラー・**lead-sheet 音節×縦線の対（新規 2026-08-20・ユーザー目視起票。before/after 同一＝既存 regime。LP twin で点を開くのが先）**・**lead-sheet の mid-piece `time` 変更の表示（新規 2026-08-20＝§3 の決定の残。行 voice に change item が無く幅もインクも出ない——表示するなら row 経路に change item を通すところから）**。Marketplace は PAT 待ちのまま（第220 ①）。
