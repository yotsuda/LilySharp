...
最終更新: 2026-07-23 / master `b36db266` の次（§0 で裏取りすること）
...
### ▶ 次のセッションの最初の一手 ＝ **輪郭が読めるようになったので、それで閉じる残差を取りに行く**

`b36db266` で **Skia 経由でフォント輪郭を実行時に読む配線**（`TextFontMetrics`）が入った。
これで**「Lily# に無いパイプライン」を理由に保留していた残差 2 種が、初めて手が届く**:

| 残差 | 点数 | 保留理由（当時） | 今 |
|---|---|---|---|
| `barline.next.time-change-to-notehead` −0.004735 | 1 | 「テキストレイアウト経路が要る」 | **その経路がある** |
| clef の LILC bbox 3.550 vs LP skyline 3.540〜3.545 | 4 | 「輪郭ベースの skyline 生成が要る／C# に OTF パーサ無し」 | **後半は誤り。`SKPath` で輪郭が取れる** |

**推奨は TimeSignature を先に**（1 点・機構は今作ったものそのもの・規模が小さい）。
§2 の記録によれば LP は `(markup #:number "4")` 経路で **1.604735**、
`ly:font-get-glyph "fattened.four"` では **1.600000**。つまり**同じ Emmentaler を
LILC でなく輪郭で測った差**の可能性が高い。⚠️ **推測で直さない** ——
まず `TextFontMetrics` と同じ要領で Emmentaler の `fattened.four` を輪郭で測り、
**1.604735 と突き合わせてから**実装すること。一致しなければその差が次の欠陥。

clef の 4 点はその後。⚠️ **こちらは bbox でなく本物のスカイライン**（輪郭の描画プリミティブから
生成）が要るので、`SKPath` のセグメント走査が必要＝規模が一段大きい。

### ▶ その次 ＝ **まだ 1 点も無い領域に点を開く（ビーム or スラー/タイ）**
...
#### ✅ 完了: フォント metric を実行時に測るようにした（`daff1b1b` `b36db266`）
...
**同じ `.lys` がマシンによって違う組版になる**。これは `b69c73e6` で直したプローブのバグと**同型**
...
| `caa0f239` | **括弧を譜間スカイラインへ＋幻の符尾を止めた**。`−1.700225 → −0.547717`（2 点とも）。**snapshot 9 件再ベース**（分類・承認済）＋**新規フィクスチャ 1 件** |
| `bc756fd1` | §1 書き換え＋**逆算による分解の訂正** |
| `5271df10` | **ページ側に届く点を 2 つ起票**（probe TSU/TSD）。出力不変。**起票中に probe が clef を測っていたのを検出して直した** |
| `075277ff` | **括弧をページスカイラインへ＋prelim パスの声部/譜スコープ漏れ**。`−1.295225 → −0.547717`（2 点とも）。**snapshot 14 件再ベース**（分類・承認済） |
| `b69c73e6` | **プローブの serif フォントを固定**。タプレット 4 点の LP 参照値が SVG backend の fallback フォント（fontconfig 任せ・マシン依存）で測られていたのを訂正。**出力不変・snapshot 0 件** |
| `daff1b1b` | **TeX Gyre Schola / Heros を同梱＋輪郭で測る `TextFontMetrics`**。呼び出しゼロ・**出力不変**。単体テスト 7 件 |
| `b36db266` | **レイアウト 37 箇所と 3 backend を実フォントへ**。タプレット 4 点 `−0.547717 → +0.0000208`。**total 2.214721 → 0.023937 ss**。**snapshot 194 件再ベース**（分類・承認済）＋テスト 5 件更新 |

**テスト 3180 passed / 0 failed / 3 skipped。** Core 0 warn / 0 err。
**LP 忠実度 28/40 exact, total |residual| = 0.023937 ss over 36 distances ＋ counts 4/4。**
⚠️ **ss 合計の増加は悪化ではない**——タプレット 4 点が誰も測っていなかった乖離を可視化した分
（起票時 −1.700225 ×2・−1.295225 ×2 → 現在 **−0.547717 ×4**）。§5.2.1④。
...
⚠️ **途中段階の −0.547717 ×4 は `b36db266` のフォント配線で消えた。** 残るのは Pango 量子化の半分。
...
**origin より 95 ahead で未 push**（HEAD = `b36db266` の次）。push はユーザー判断・コミットは可。
...
**テスト 0 failed / 3180 passed / 3 skipped。** Core build 0 warn / 0 err。
**LP 忠実度 28/40 exact, total |residual| = 0.023937 ss over 36 distances ＋ counts 4/4**
（**2.26.0 基準**。X 22点＋Y 7点＋譜間 **5点**＋**system 間タプレット 2点**＋**ページ本数 4点**）。
⚠️ **本数の点は距離ではないので ss の総和に入れない**（`unit` フィールドで分離。`page.height` を
台帳から落としたのと同じ理由——1 system を 0.023937 ss に足すと指標が意味を失う）。
...
**現状 28/40 exact, total |residual| = 0.023937 ss over 36 distances ＋ counts 4/4**
