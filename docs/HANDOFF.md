...
★★★ **⑵ 機構と移植**: **LP `scm/define-grobs.scm:2376` の MultiMeasureRest は `spacing-pair (break-alignment . break-alignment)`**（`staff-bar` ではない＝Lily# の旧 REF の読みは誤り）⇒ `multi-measure-rest.cc:44-61 bar_width` の各辺は `paper-column.cc:167-218 break_align_width` の**column 全体の extent**＝左辺は開く column の右端（小節線＋その後ろの key／time）・右辺は閉じる column の左端（小節線の前の clef）。**column は system のもの**＝他の譜の key 変更でも動く。**Lily#**: `MultiMeasureRestEngraver.BarWidth`（run の記号と run の外の記号の両方が読む）＝左は各譜の各 voice で `BoundaryColumn.Build` の grob 右端 − staff-bar 右端（小節頭が key／time で開くときだけ建てる）・右は `SpacingRules.BoundaryClefAllowance`（spacing と同じ量）・**system の頭の小節は触らない**（prefix が X を払っている）。rod（`calculate_spacing_rods`）は `minimum_distance` で既に column 全体＝触っていない。**実測（中央からのずれ・LP / ページ）**: `v3.ly` 7 小節目（time 3/4）**+1.17 / +1.17**・`keysigspace` 2 小節目（他の譜の 5♯）**+5.14 / +4.58**・`cue-clef-manually` 2 小節目（3 小節目の前の cue clef）**−1.16 / −1.38**（残差 2 つは ⑷ ①②＝この量ではない）。

★★★ **⑶ 射程と LP 照合**: **掃き 937 冊**（`scratch/p390/sweep`・base＝`p389/exe-head3`＝`3cb94b64` の Core・head＝Release の複製）: **5 冊・全部インクだけ（小節線不動）・実コーパス 0**＝`key-signature-space`（追跡 2 冊・40.39 → 44.97）・`cue-clef-manually`（28.18 → 26.80）・snapshot 2 枚。**snapshot は `lysc ly` 双子＋`\compressMMRests`**（`scratch/p390/lpfix`・⚠️ 双子は compress を出さない＝そのままでは LP が小節ごとの全休符を描いて比べられない）で run の左小節線からの x: **`mmr-key-change-bound`** longa / count / whole＝LP 7.07 / 9.04 / 10.85 対 ページ 5.76 / 8.00 / 10.07 → **7.07 / 9.05 / 10.86**・**`mmr-meter-and-time-change`** breve / count / whole＝LP 6.32 / 7.96 / 9.43 対 4.85 / 6.79 / 8.56 → **6.32 / 7.96 / 9.44**（前置きの無い 3/4 の run は 3.93 対 3.92 で不動）＝**ユーザー承認のうえ再ベース**。**網** `MultiMeasureRestLayoutTests.AnMmrCentresBetweenTheBreakAlignments_NotTheBarLines`（他の譜の key・次の小節頭の clef・inline の time 3/4・前置きの無い辺 3 つが 0）。**毒 3**（左の reach を 0／右の clef を 0／自分の譜の voice だけ）＝**3 本ともこの網だけ赤**。⚠️ 引用ラチェットは**ハイフンの名前を 3 節以上しか数えない**（`spacing-pair` は英語扱い＝681 → 682 で赤）。
...
★★★ **⑧ 第 5 便（常設指示「このセッションが有利なら着手」）＝`keysigspace` 2 小節目 +0.55 の住所を測って止めた（製品 0）**。**切り分け**（`scratch/p390/ks`・第 1 音と小節幅・LP／ページ）: ⒜ 単譜・上向き符尾 26.66／26.66（一致）・⒝ 第 2 譜に普通の `r1`＝LP 26.11（第 1 音 12.22）／ページ 26.66・⒞ 第 2 譜 `R1`（原本）同じ・ⓓ **key 変更を第 2 譜にも書く**＝LP 26.66（12.77）／ページ 26.66 ⇒ **MMR ではなく「譜ごとに小節頭の列が違う」**。**LP**（`spacing-spanner.cc:478-536 breakable_column_spacing`）: 非音楽列 → 音楽列は**譜ごとの `Staff_spacing::get_spacing` を集めて `merge_springs`**（`spring.cc:104-129`＝**ideal は平均・min は最大・ideal ≥ min＋0.3**）。`ly:paper-column::print` の実測（2 小節目頭の列 rank 9・`ksb-dbg.ly`／`ksd-dbg.ly`）: ⒝ min 12.15／ideal **12.90**・ⓓ min 12.15／ideal **13.45**＝差 0.55 がそのまま＝**key の無い第 2 譜の spring（最後の grob は自分の小節線）が平均を引き下げる**（逆算 12.35・`min_dist`＋0.3＝12.45 との 0.1 は未測）。**Lily#**: `MeasureLayouter.cs:218 BuildTimingToItemsMap` が**全譜の項目を 1 つの列に集め**、`:419 BarlineToFirstColumnSpring` が**spring を 1 本**だけ作る（key がどこかの譜にあれば全譜が key の spring）。**判定＝着手しない**: ⒜ 字面の移植＝小節頭の spring を譜ごとに作って merge＝**列の模型（2 系統＝column layouter と item estimator・`SpacingInvariantTests`）と改行 gate にまたがる設計**／⒝ 動くのは「譜によって小節頭が違う」全ての多譜の小節（key／time／clef が片方の譜だけ・臨時記号・装飾音符…）＝射程が広い＝承認要／⒞ 計器（⒜〜ⓓ の `.ly`／`.lys`・`-dbg.ly`・`bars.ps1`）は小さく次便で作り直せる＝損しない。⇒ **要ユーザー判断**（譜ごとの Staff_spacing＋merge_springs を移植するか）。**次の一手**: ⑧ の判断／cue clef の大きさ／第379 ⑺⑻。
