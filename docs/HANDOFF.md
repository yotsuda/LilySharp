...
**終了時**: **commit 2 本**（`704ce303`＝製品 6＋番人 1＋CHANGELOG＋生成物 2／この HANDOFF＋ARCHIVE＝第338 の経緯を ARCHIVE の先頭へ逐語）・**未 push 19**・木 clean・未追跡 0。**snapshot・台帳・追跡 `.lys` は全部不動**（動いたのは ⑻ の 4 冊＝すべて未追跡）。
...
⇒ **残る無駄は「1 冊 1 プロセス」だけ**（固定費 ~1.3 s × 3,094 ≒ 67 分ぶんのスレッド時間）。**消すには `lysc` にバッチ入力（1 プロセスで N 冊）が要る＝製品の変更なので別の島**。**次の一手 ⑵ に置いた。**
...
**⑵ `lysc` にバッチ入力を足す（⑼⒞ の残り）**——**1 プロセスで N 冊**。**掃き 1 回の固定費 67 分ぶんのスレッド時間がまるごと消える**し、**同じ固定費はプレビューの cold にも乗っている**（第329 の R2R と同じ棚）。⚠️ **製品の変更なので承認が要る。** ⚠️ **`check` は R2R で*遅くなる***ので、速さの根拠に R2R だけを挙げないこと。
