...
- ⚠️ ★★ **新規＝round trip を破る最後の島＝スラー／タイの印の*後ろ*に書いた注釈が、木では
  *前*に出る**（第185 実測・**未修理**。`AnnotationRoundTripTests` の既知一覧に残る 4 冊）。
  ⇒ ★★★ **今日の一族とは別の欠陥類**——**4 冊とも長さが 1 バイトも変わらない**
  （628→628・1033→1033・593→593・4620→4620）。**幅は落ちていない。順序だけが入れ替わる。**
  だから **data-pos も LSP のジャンプ先も滑らない**——**壊れるのは「木から `.lys` を書き戻す」経路だけ**で、
  そこでは**注釈がスラー記号を跨いで移動する**。**実測（4 冊とも同じ 1 規則）**:
  ```
  empty-chord             e8( g <>)@text("sul D").up@f  →  e8( g <>@text("sul D").up@f )
  script-stack-order1     e'4(@accent@finger(0)@downbow →  e'4@accent@finger(0)@downbow (
  slur-vertical-skylines  g1)~@startTrillSpan           →  g1@startTrillSpan )~
  feature-tour            g4(@cresc a b c … g)@f |      →  g4@cresc (a b c … g@f )|
  ```
  ★ **家は `_pendingPostEventMarkers`**（`ParseMusicItem` 冒頭が「`ParsePostEvents` が順序を崩して
  消費したスラー/タイ/梁の印をここで再生する」と自称している＝**再生の位置が注釈の後ろになる**）。
  ⚠️ **「音楽が変わるか」は測っていない**——**同じトークンが別の順で木に居る**だけなので、
  **順序を読む消費者が居るかどうかが先**（居なければ書き戻しの見た目だけの問題、
  居れば今日の `check --pitches` と同じ形）。**着手するならそこを数えるのが第 1 歩。**
