...
最終更新 第176セッション＝**cue の島を閉じた便**（**実装 2 便＋文書 1 便**）。
...
- **⑼ ⚠️⚠️ ★★★ ▶ の打鍵順位は「取り直せない」と実測で分かった**——**ユーザーが静かな窓を
  ...
  **第159 の ms は別の機械の別ランタイムの数＝並べない**（§5.3）。
  ⇒ **要るのは窓ではなく ⑴ perf 本の再構成 ⑵ 生成器の置き場所の決定＝島 1 つ**（▶ 冒頭に詳述）。
  **順位に依存する判断は、それまで全部保留のまま。**
  ⚠️ ★ **汎化して RULES §5.5 に出した**: **窓・承認・ユーザーの時間を使う作業は、
  着手を決める前に「入力が全部この機械に在るか」を 1 コマンドで確かめる。**
...
> ## ⚠️⚠️ ★★★ **打鍵の順位は「窓が要る」より前に「本が無い」で止まる**（2026-08-15・第176 実測）
> **ユーザーが静かな窓を許可してくれたのに、ベンチが 1 つも走らせられなかった。**
> `PreviewUpdateBench` と `EditKeystrokeBench` が読むのは **`C:\MyProj\LilySharp\scratch\lpreg\perf-*.lys`**
> ——**`scratch\lpreg` はこの機械に存在せず、`perf-*.lys` はリポジトリ全体で 0 個、生成器も 0 個**。
> ⇒ **第171 が `rerender-ls.ps1` で踏んだ §5.5 の債務の 2 例目**。あちらは作り直されたが、
> **perf の本は誰も確認していなかった**（＝第159 以降の順位は**この機械では一度も取り直せない**）。
...
> ⇒ ★★ **これは「窓が空いたらやる」項目ではなく、島 1 つ**（本の再構成＋生成器の置き場所の決定）。
