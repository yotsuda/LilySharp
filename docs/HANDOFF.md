...
開始時の 11 と引き算できた**（＝間で push は無い）。**この形を続けること。**
 終了時 **未追跡 0・作業ツリー項目 0・未 push 15**（★ **この引継ぎ commit を含む数**）・
⚠️ ★★ **ここは 14 と書きかけた**——**引継ぎ commit を*作る前*に数えた数**だった。
**「この commit を含む」と書く数は、commit したあとに数え直すこと**（§0 ★ の当日版）。
...
- ★ **`font "NAME"` を指定すると、予約と描画が別の face になる**（2026-07-27・▶0 の
  ...
  **`font "NoSuchFontFace"` はいまも `No errors found.`** ＝**項は生きている。**
  ...
  破壊的変更**。**`font` を書く本はツリーに 2 冊**（実測）なので**射程は小さく、決めるのは今。**
