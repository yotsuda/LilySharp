...
最終更新 第333セッション＝**入り方は第298〜第332 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。第332 の次の一手 ⑹＝**「空小節のばね」の移植**に入り、**移植は終わった。ただし出力を変える（snapshot 6 枚が動く）ので、RULES §5.1「出力を変える変更はユーザー承認前に出荷しない」に従い*製品コードは commit していない*——作業ツリーに置いたまま**（下 ⑻）。**骨は 8 つ**:

⚠️⚠️⚠️ ★★★★ **⑴ 機構の住所が第332 ⑾⒞ と違った＝LP は skip の列を *prune* するのではなく *is_used* で落とす**（`lily/paper-column.cc:115-136 Paper_column::is_used`＝elements／bounded-by-me／breakable／used／labels のどれも無い列・`lily/system.cc:751-777 System::used_columns_in_range` がそれを filter・`spacing-spanner.cc:42-58 get_columns` はその list を受け取る）。skip は grob を作らないので**列そのものが消えて小節線の対が隣り合い**、`breakable_column_spacing`（`:478-517`・dt≠0 なので Staff_spacing の wish 無し＝`springs.empty()`）→ **`standard_breakable_column_spacing`（`spacing-basic.cc:40-83`）**。★★★ **そこに*2 分岐*ある（第332 は片方しか読んでいなかった）**: **⒜ 両側 breakable**（`is_breakable(l) && is_breakable(r)`）＝`ideal = min_dist + 1.2 × (mlen / gs) × 0.8`・stretch＝space・compress＝space（既定）／**⒝ 片側でも非 breakable**＝`ideal = min_dist + get_duration_space(dt)`・`Spring(ideal, min_dist)` の既定強度（stretch＝ideal・compress＝ideal − min）。**mlen は `measure-length`＝拍子の小節長**（`paper-column-engraver.cc:185-194` が各小節の先頭 command 列に stamp）。
⚠️⚠️ ★★★★ **⑵ 「非 breakable」の出所＝`Forbid_line_break_engraver`**（`lily/forbid-break-engraver.cc:41-62`・`ly/engraver-init.ly:378` Staff）: **鳴っている（busyGrobs の）rhythmic grob があれば forbidBreak** → `paper-column-engraver.cc:264-271` が command 列の line-break-permission を消す。**RepeatSlash／DoublePercentRepeat は rhythmic-grob-interface で body 長だけ鳴る** ⇒ **`%%` の対の内側（`double-percent-repeat-engraver.cc:63-71` も明示）・3 小節以上の slash 反復の内側の小節線は非 breakable＝⒝**。**単一 `%`・`s1`・`| |` は両側 breakable＝⒜**。**ALLCOL dump（`scratch/p333/fx/allcol-settings.ly`＝x／when／starter／break／line-break-permission／measure-length ＋ PROBEGS＝gs）で 4 冊確認**: `tab-percent-blank-bars` の blank 2 小節は `lbp=()` で 6.39＝0.39 + (2+log2 8)×1.2（gs 1/8）／`pc4r` の `%%` 対の中の線 `lbp=()`・列間 5.69（=0.39+5.30）／9.26（=3.96+5.30・**3.96 は `%%` 印の ink が staff-bar 列の skyline に入った min_dist**）／`pc3r`・`pc5r` は全部 `allow`。
★★★★ **⑶ 移植＝`SpacingRules.EmptyBar.cs`（新）＋4 軒**: **⒜ `StandardBreakableColumnSpacing(minDist, bothBreakable, mlen, dt, gs)`＝⑴ の 2 分岐を字面で／`EmptyBarSprings(columnCount, leftBound, leadingItems, bothBreakable, mlen, dt, gs)`＝内側 N 本は剛体 0（`LILYSHARP-OWN`＝列を消せない Lily# の器の都合・`observed by` 番人）＋末尾に LP の 1 本を Lily# の枠（左小節線の描画幅を引く・`MmrRodDistance` と同じ向きの補正）で**／**⒝ 「grob の無い小節」＝`ScoreSideTables.UsedBars(score)`（memo・全 staff の全 voice の item が spacer だけ（先頭の break-aligned change は除く＝`BarHoldsOnlySkips`）かつ Lyrics／ChordNames／Dynamics／Articulations／CustomTexts／TupletBrackets／FiguredBasses／GraceNotes／beat-slash の PercentRepeats がその小節に無い）・`PrevailingMeters(score)`（memo）**／**⒞ 置き場所＝`MultiStaffLayouter.ApplySharedColumnReservations` の冒頭**（layouter と break gate が読む 1 軒・引数に `baseShortestDuration` を足した＝SystemBreaker も渡す）＋ **item 系 `CreateSpringsForMeasure`**（単一小節なので側表・前小節の線・meter は見えない＝⒝ 導出として remark に 3 点明記）／**⒟ `CommonShortestDuration` の投票から staff の spacer を外した**（`spacing-engraver.cc:176-197 add_starter_duration`＝grob からしか来ない。text row の slot は ChordName が rhythmic なので残す・**lyric row の slot は LP では投票しないが Lily# は投票したまま＝未測**）／**⒠ collector `WriteRepetitionAsSpacers`: 反復の内側で閉じた小節に `SetNoBreak()`**（⑵ の写し）。**新規 LILYPOND-REF 33 本・LILYSHARP-OWN 1 本**（§7.5 の数え）。
★★★★ **⑷ 門（第332 ⑾⒠）は全部乗った**（`scratch/p332/t7/measure.ps1 -Settings ragged-settings.ly`・`scratch/p333/probes-after*.txt`）: **`pc5r` 5.51／`pc6r` 8.07／`pc7r` 5.51／`pc3r` 5.51／`fx-tabpct` 15.75＝全部 LP と桁一致**（末尾の音楽小節まで）。`pc4r` は 5.69/5.69（LP 列間 5.69/9.26・小節線間 7.57/7.38）＝差は `%%` 印の幅だけ。**番人 9 本 `EmptyBarSpacingTests`**（剛体 0 の腕・gs を振って線形則（log ではない）・item 系との一致・staff skip は投票しない（対照は 4 分 3 小節）・`r1` は列を保つ・`%` 被覆は空で beat slash は使用中・和音記号の下の skip は使用中・slash body の内側 Forbid ＋ 6.39・`%%` 対の Forbid ＋ 5.69）。**snapshot 前の絵（HEAD）が陽性対照**（`scratch/p333/before-widths.ps1`＝HEAD の svg と作業ツリーの svg の小節幅を並べる）。
⚠️⚠️ ★★★★ **⑸ 出力＝snapshot 6 枚が動く（`scratch/p333/snapshot-widths2.txt`・before → after・LP）**: `test/section-empty-placeholder` 6.39 → **5.51**（`| |` は 2026-08-28 の決定で `s1` と同じ音楽＝同じばね・双子は `| |` を落とすので LP 側は無し・`s1` の 5.51 で照合）／`test/tab-percent-repeat` 8.29 → **15.75**（LP 15.75）／`test/percent-covered-bows` 6.39 → **5.69**（LP 8.51/8.32＝`%%` 印の幅 2.8 が残る＝⑺⒜）／`test/tab-percent-blank-bars` blank 6.39 → **6.39 exact**（音楽小節 13.07/9.6/… → 15.88/11/7.09＝gs が 3/16 → 1/8 に LP と揃った（LP 15.33/10.65/8.03・残差は別件）・`r1` 7.69 → 8.39＝LP exact）／`test/beam-over-stem` 20.61/15.2 → **15.8/11.6**（LP 20.93/20.63・**gs は両者 1/8 で一致**（ALLCOL）・差は⑺⒝ の「音符の後ろの skip 列」＝旧 gs 1/16 が偶然 LP に近かっただけ）／`showcase/05-special-techniques` 2 段目 9.82/25.13/37.53 → 18.31/22.59/34.06（双子不可＝未判定）。**`LILYSHARP_UPDATE_DOCS=1` で inventory 2 つ再生成済み**（行番号）。
★★★ **⑹ コーパス（`scratch/p332/relayout332.ps1 -Csv scratch/p333/structure-after333.csv`・7 分 47 秒）**: **T7 指標 KindMatched 209 / SigMatch 143 / 段違い 47 / 小節数違い 19＝第332 と同じ**。**全対 SigMatch 422 → 432（+16／−4）・SystemsMatch 466 不変・全対一致の本 198 → 202**。得た 16＝December 1963 ×2・Dream Come True・Holiday ×2・Le Freak ×3・Marry You ×2・Stand By Me ×2・Sweet Memories・Whatever ×2・未来予想図Ⅱ。**失った 4＝Addicted To Love both/tab（4,8 → 12＝`%%` 対が LP より狭い＝⑺⒜）・Express Yourself staff/tab（8,4,4,4 → 4,4,2,2＝`repeat percent 7 { r1 }` の被覆 6 小節が gs 1/16 で 15.75 になり、LP は `\break` までの 8 小節を f≈−0.3 で*押し込む*（ALLCOL: 被覆小節 11.17＝圧縮後）が Lily# は 4,4 に割る＝圧縮行の採点の差・未追跡）**。
✅ ★★★★ **⑼ 第 3 便＝ユーザー「次便がこの便で有利なら着手」→ ⒝ skip 列の削除を移植した**。⚠️ **最初は「列の同一性に触るので次便」と判断したが、§5.0 の型で*先に数えた*（捨て計器 `ZzSkipColumnCensusTests`＝全 fixture＋実コーパスを collect して「notation staff の spacer だけの onset」を数える → `scratch/p333/ZzSkipColumnCensusTests.cs.txt`）: **fixture 2 冊（beam-over-stem 5 onset・beam-skip 1）・実コーパス 1 冊 3 小節**＝射程が小さく、probe `scratch/p333/ps`（ps1 `c4 s2.`／`c8 s8 c8 s8 c4 s4`・ps2 2 声・ps3 `s4 c4`／`s2 c4`）の ALLCOL で LP の式も割れたので着手。**LP**: `c4 s2.` は列が bar・c4・bar だけ＝**c4→bar は `note_spacing` の `fraction = delta_t(1) / shortest_playing(1/4) = 4` × 4 分の spring（12.75 = 1.23 + 11.52・stem 補正 −0.18）**／`fills_measure` は skip の unused 列が `next` になるので偽（bar→c4 1.23＝fmes 無し）／skip で開く小節は bar→note が dt≠0 の対＝`standard_breakable_column_spacing` の dt 分岐（`s4 c4` 3.288 = 0.39 + 2.898・`s2 c4` 4.488）。**移植**: `PruneSpacerOnlyOnsets`＝「鳴っている音が跨ぐ」条件を捨て、grob の無い onset（notation staff の spacer だけ）を落とす（text row の slot／timing 付き和音記号／beat slash は anchor・**text row の空 cell の slot は保つ＝`LILYSHARP-OWN`**）／`SkipOpenedBarFirstSpring`（EmptyBar.cs）／`CreateBarlineToFirstSpring(droppedOnsetFollows)`／item 系は kept 列＋`CreateSpring(shortestPlaying:)` の fraction。⚠️ **1 度目は spring 0 の枠で小節線幅を 2 度引いた**（`RightSkylineFromBarLine` の原点は小節線の左端＝列原点なので距離はそのまま LP の min_dist）→ ps3 が 0.19 広く出て気付いた。**結果: ps1 12.75／15.16・ps3 12.15／10.35＝LP と桁一致・ps2 11.79 対 11.81（0.017 残・2 声の末尾 leg・未診断）・`beam-over-stem` 15.8/11.6 → 21.26/20.66（LP 20.93/20.63・bar 1 の +0.33 未診断）**。**番人 5 本 `SkipColumnSpacingTests`**（列が消える・fraction の leg・skip で開く小節の分岐と既定強度・両系統の一致・fmes が skip で消える）。**snapshot は `beam-over-stem` 1 枚（承認・再ベース）・`beam-skip` 不動**（loose の prune と同じ答え）。
★★★ **⑺ 残差＝次の島（§2 T7 ▶）**: ✅ **⒜ `DoublePercentRepeat` の ink＝第 2 便で閉じた（⑻）**／✅ **⒝＝第 3 便で閉じた（⑼）**。**残る小差**: ps2 の 0.017・beam-over-stem bar 1 の +0.33・text row の空 cell の slot（LP は落とす）・lyric row の slot の投票・Express Yourself の圧縮行の採点。**旧 ⒝ の起票**: 音符の後ろの skip 列（`c4 s2.`）＝LP は unused 列を落として音符→小節線を `fraction × shortest_playing`（`spacing-basic.cc:107-162`）の 1 本にする（beam-over-stem の LP 20.93 対 15.8）。⚠️ **行頭の空小節は未測**（LP の min_dist は prefix 列の reach・Lily# は spring 0（剛体 0）を prefix ばねに差し替えて LP のばねを後ろに置く）。
✅ ★★★★ **⑻ 第 2 便＝ユーザー「リベースは承認する」→ 6 枚を再ベースして 1 commit（`c8a2b988`＝Core 6 ファイル＋新 `SpacingRules.EmptyBar.cs`＋新 `EmptyBarSpacingTests.cs`＋CHANGELOG＋inventory 2＋snapshot 6・full 7115 / 0 / 4 / 7119）**。そのまま**ユーザー「次便がこの便で有利なら着手」→ ⑺⒜＝`%%` 印の幅を移植した**（§2 T7 ▶ ⒜ ✅・`dp-settings.ly`＝DoublePercentRepeat と BarLine の X／X-extent を刷る計器・**印の X＝小節線の X（左端）・幅 3.757645（staff）／5.636468（tab ss 1.5）＝`PercentRepeatEngraver.Geometry` の GroupWidth と 6 桁一致**）: renderer の inline 幾何を `PercentRepeatEngraver.Geometry`（1 軒・描く箱＝予約する箱）に畳み、`BoundaryColumn.DoublePercentBox`→`MmrRodMinimumDistance(left/rightDoublePercentHalfWidth)`→`EmptyBarSprings`、`ScoreSideTables.DoublePercentHalfWidths` memo（staff ごとの ss・union は最大）、印の中心を小節線の*左端*へ（`Calculate(…, measures)`・前は右端＝0.19 ずれ）。**pc4r 7.57／7.37／7.57／7.38（LP 7.57／7.38）・fx-bows 8.51／8.31（LP 8.51／8.32）・番人 +1（幅 6 桁）＋ `%%` 対の番人を両小節 7.57／7.38 まで**。**snapshot は `test/percent-covered-bows` 1 枚（5.69／5.69 → 8.51／8.31）＝ユーザー承認のうえ再ベース・1 commit（下の終了時）**。

★ **開始時裏取り**: HEAD `49fc9b73`＝**未 push 40**（第332 終了時と同じ・`origin/master` は `f30cdfeb`＝間で push 無し）・木 clean・未追跡 0・`bin\Debug` は net10.0 のみ・`xunit.runner.json` は並列のまま・**solution build 0 エラー・Core 0 警告（`--no-incremental`・6 分 25 秒＝今日は遅い）・full 7106 / 0 / 4 / 7110（`scratch\p333\run1.trx`・3 分 38 秒）**・`gh run list`＝下の終了時に読んだ（同じ push）。
終了時（第 1 便）: **commit 1 本（`572c470d`＝HANDOFF＋`HANDOFF-ARCHIVE.md`＝第331 の経緯を末尾へ逐語＋`RULES.md` §5.3 の「1 段の probe で伸びは測れない」）・未 push 41・木は製品未 commit で*汚い*（M 9・?? 2）**・**`--no-incremental` build 0 エラー・Core 0 警告（3 分 37 秒）・full 7109 / 6 / 4 / 7119（`run3.trx`・+9＝新番人・赤 6＝⑸ の snapshot）**・焦点 81 / 0（`focus2.trx`）。
**終了時（第 2 便）**: **commit 3 本（`c8a2b988`＝⑻ の承認済み移植＋snapshot 6 枚（full 7115 / 0 / 4 / 7119・`run4.trx`）／`70e67f42`＝⒜ `%%` 印の幅＋`test/percent-covered-bows` 再ベース（full 7116 / 0 / 4 / 7120・`run6.trx`・+1＝幅の番人）／`71ee4e35`＝HANDOFF）・未 push 44・木 clean**。
**終了時（第 3 便）**: **commit 2 本（`a4d5bae2`＝⒝ skip 列の削除＋`SkipColumnSpacingTests`＋CHANGELOG＋inventory 2＋`beam-over-stem` 再ベース（full 7121 / 0 / 4 / 7125・`run8.trx`・+5）／この HANDOFF）・未 push 46・木 clean・未追跡 0**・台帳／`.lys`（追跡）不動・実コーパス不動。⚠️ **relayout332 は第 1 便の木（`c8a2b988` 相当）でしか回していない**（⑹ の数）——⒜⒝ の後の段署名は次便の最初に 1 度（7 分）。

★★★ **⑽ 第 4 便＝⒜⒝ 後の段署名（`relayout332.ps1 -Csv scratch/p333/structure-after333b.csv`・7 分 54 秒・最終木 `a4d5bae2`）**: **T7 指標 KindMatched 209 / SigMatch 143 → 148 / 段違い 47 → 42 / 小節数違い 19**。**全対 SigMatch 432 → 440（+17／−7）・SystemsMatch 466 → 475・全対一致の本 202 → 207**。得た 17＝Dream Come True both・First Love tab・Real Gone ×2・Sweet Memories・The Bitch Is Back ×2・同 OrigKey ×2・Video Killed The Radio Star both・にんじゃりばんばん ×2・ポリリズム ×2・未来予想図Ⅱ ×2・同 ChordedByLLM both／失った 7＝Dream Come True tab・First Love staff・**Le Freak ×3（第 1 便で得た 3 対が戻った）**・Video Killed tab・未来予想図Ⅱ ChordedByLLM tab。⚠️ **同じ本で kind ごとに得失が割れる**（Dream Come True・First Love・Video Killed）＝双子の kind（staff／tab／both）で LP 側の段組みが違う本で、Lily# は 1 つの絵しか持たない＝比較の枠の問題で製品の欠陥ではない（第321 の KindMatched の読み）。**Addicted To Love（第 1 便で失った 4,8 → 12）は戻らず**（⒜ の `%%` 幅では足りない＝次便の材料）。
⇒ ★★★★ **次の一手**: ✅ **⑴ ⑻ は承認・commit 済み**。✅ **⑵ ⒝ skip 列＝第 3 便で閉じた（⑼）**。✅ **⑶ 段署名の掃き＝第 4 便（⑽）**。**⑶′ Le Freak の 3 対と Addicted To Love の 2 対を `beng332` の型で割る**（break 位置一致か・どの段で割れるか・LP の圧縮か Lily# の幅か）。**⑷ lyric row の slot の投票**（⑶⒟・LP は投票しない・`CommonShortestDuration` の text row 分岐を lyric row だけ外す）。**⑸ Express Yourself の圧縮行の採点**（⑹・LP は `\break` までの 8 小節を f≈−0.3 で押し込む）。**⑹ 小差 2 つ**: ps2 の 0.017（2 声の末尾 leg）・beam-over-stem bar 1 の +0.33。**⑸ 以下は第332 の並び**: 第332 ⑵ ピックアップの直し方をユーザーに訊く／⑶ T7 C の冊ごと判断の材料／push 後 `gh run list`／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／§2 A/B/D/E／§2 C⑴／§2 G／`:|.|:` の幅 0.08／smart typing の小穴／第330 ⑸ の overlay 群。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**

## 以下は第332セッションの経緯

最終更新 第332セッション＝**入り方は第298〜第331 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。第331 の次の一手 ⑴⑵ はユーザー側（「手元の `.lys` の手入れは私がする」）なので、**⑶ §2 T6 の残り＝⒞ 記号・⒟ 同時 2 声を*数え直し*から入り、⒞ の計器を建てて verify まで回した（コーパスには当てていない）**。製品側は §2 G の小物 1 つ（`HANDOFF §1 ⒪` 参照 4 本）。**骨は 5 つ**:

⚠️⚠️ ★★★★ **⑴ ⒞ の数え直し＝「`\mark` 21 対 0 の 1 冊」ではなく 158 冊 461 札だった**（`scratch/p332/marks332.ps1` → `marks332.csv`。数え方＝`.ly` はコメント（`%{ %}`・`%` 行末）を剥いでから `\mark` の札を*集合*にする——同じ札が staff／tab／chords の各 context に 1 つずつ書かれるので出現数では 4 倍に出る——`.lys` は `section NAME {`（`~` 付きは匿名）＋ `@mark("X")` の集合）。**内訳: 匿名 section `~Body_1` 化 266 札（79 冊）／`section Body`＋`form { ~Body }` で*先頭の札だけ*落ちた 173 札（68 冊・GRAMMAR SectionDecl「shown = 宣言の既定 == 参照の `~`」で Body の札は伏せられ、`.ly` の最初の `\mark Intro` に当たる物が無い）／その他 11 冊**。**別勘定 2 つ**: `A1'`→`A12`／`A1d` の写し替え 23 札（`'` は識別子に使えない。⚠️ **文法の穴ではない**＝form 参照の `[ String ]`（GRAMMAR StructureItem）で `A12 "A1'"` と表示札を持てる）／segno・coda・D.S.・to Coda のナビ記号 85（Lily# は form の NavMark＝札ではない）。
★★★ **⑵ 計器＝`scratch/p332/t6c/RestoreMarks`**（第331 RestoreMeter の複製＋ `marks`・`dotnet run -c Release -- marks`・`verify.ps1 -Mode marks [-Only 名前…]`・`apply.ps1`）。`.ly` の `\mark` を*次の音*に付ける——**変数参照 `\Intro` は `Intro = { … }` の先頭音**（参照しか持たない変数は再帰・`s1` で始まる blank／chord トラックは飛ばす＝同じ札の複製）——揃った `.lys` 音の正典スロット（`核 \N @…`）へ `@mark("X")`。**書かない規則 6 つ**（全部 detail に「left for a hand」で出る）: 既に `@mark` あり（同札＝already 1173／別札＝42＝`'` 落ち）・section 名が札を刷る（同名 824／別名 55＝rename 候補・`A2` 対 `A`・`C12` 対 `C1'`）・小節頭でない・**全 form が再生しない section**（10・Rose Garden J／K・She Bangs D2＝`form unfold` だけ）・**section 境界から ±4 event**（134・A Thousand Miles＝`.ly` の A1 は `r1 r1` で開き `.lys` はそれを Intro に置くので LCS が滑り、A1 が Intro の最後の休符に乗って以後の札が 1 section 早かった）・**休符上の札で section 頭でない物**（120・同じ `r1` の並びは LCS がどこにでも揃える）。⚠️ **⑶ 罠 3 つ（順に踏んだ）**: ⒜ 1 度目の verify（208 冊・16 分）は**新規警告 3 冊 4 件**「this rehearsal mark is not printed by this score」＝form が再生しない section に書いた → 規則 4／⒝ A Thousand Miles の詳細を読んで滑りに気付いた → 規則 5・6（休符の並びは揃えの証拠にならない）／⒞ `repeat percent N { … }` の body に置いた `@mark` が N 回刷られないか＝probe（job tmp `pm1.lys`: `repeat percent 4 { c4@mark("X") … }`）で **1 回だけ・9 小節**＝健全（body 内 213 札はそのまま）。
★★★ **⑷ 結果＝`@mark` 438 札・171 冊**（`out-marks/`・`report-marks.csv`・`detail-marks.txt`）。**門: `lysc check` 原本 vs patched＝error 0→0・新規診断 0・warning 不増・`lysc layout` の小節数不動**（v1 208 冊 16 分 → 規則 4〜6 で変わった 94 冊を再 verify 8 分・残り 77 冊は v1 の門のまま）。**当てていない**——`apply.ps1 -Mode marks` はユーザーの判断（原本は `backup-marks/` へ写す・patch 後に編集された本は弾く）。手に残る物は detail の 4 群（境界近傍 134・休符 120・札違い 42・section 名違い 55）＝`.lys` の section の切り方が `.ly` の札と違う本で、**機械が決めてよい事ではない**。
★★ **⑸ ⒟ 同時 2 声の射程＝`.ly` 3 冊 5 箇所**（`<<\s*[a-gr]` で数えた）: **`Addicted To Love` L88／L93 `<<d'1~ a1~>> | <<d4 a4>> r4 …`** → `.lys` L28／L32 は `d1~ | a,1~ | d4 a,4 r4 d,@staccato d,@staccato |` と*逐次*に写っている＝**`<a, d>1~ | <a, d>4 r4 d,@staccato d,@staccato |`**（T7 C の LP 99 対 L# 103 の正体）／**`Let's Stay Together` L79／L80 `<<d4. fis'>> <<d,8\2 fis'\1>>`・`<<d8~ fis'~>> <<\tnh d,4 fis'>>`**（和音 2 つ×2）／**`アシンメトリー` L96 `<< b'\bendAfter#-2 e\bendAfter-2 >>`**（LP 85 対 L# 86 の正体）。**LP の `<< >>`（`\\` 無し）は 1 声の同時＝Lily# では和音 `<a, d>`**。ユーザー側の手直し（3 冊）。**T7 C の残り 13 冊のうち ⒟ で 2 冊が閉じる**。
★★★ **⑺ 第 2 便（ユーザー「本便が有利なら着手」）＝T7 C の残り 11 冊の正体を全部付けた**（計器に `gaps` モード＝対ごとに「片側にしか無い run」「form が再生しない section」「section／変数 × 再生回数 × 概算小節（percent × N）」・`detail-gaps.txt`／`report-gaps.csv`・コーパス 0 変更）: **⒜ form が section を落としている 4 冊**＝Rose Garden −20（J 8＋K 12）・She Bangs −8（D2 は `form unfold` だけ・main は `[2. C1_2]`）・All Star −2（Outro）・A Thousand Miles −8（`form main { Intro A1 B1 A2 B2 C1 A3 C2 }` に **B3 が無い**）／**⒝ `.ly` 側の書き忘れ 1 冊**＝Endless Story −17: `.ly` L76 `\time 2/4 r2 |` が 4/4 に戻らず **LP は D／B3／Outro を 2/4 で数えている**（Bars 推定 95＝LP 95 に一致・`.lys` は section 限りの 2/4 で正しい読み）＝**直すのは `.ly`**／**⒞ 小節の端数（ピックアップ）4 冊**＝Butterfly +1（`.lys` の `Intro2` が 0.3 小節の section）・Hello, my friend +1（89.8）・9 to 5 (Xanadu) +1（A2 9.8・A3 9.6・C 17.8）・Automatic −1（87.9）＝第331 ⑷ の「`partial` の綴りを訊く」族／**⒟ 既知 2 冊**＝Arthur's Theme +1（`| break |`）・You're So Vain +1（`.lys` L25 の `e1` 1 小節が `.ly` に無い＋break 1/2）。**音符が揃っていて小節数が違う本は、percent の回数ではなく form の参照と拍子だった**。
★★★★ **⑻ 第 3 便＝ユーザーの 6 決定（1a 当てる／2a 私が直す／3c 冊ごと／4a `.ly` を直す／5a `partial` 方針／6b 触らない）を実行した**: **⒜ ⒞ を当てた**（`apply.ps1 -Mode marks`＝171/171・原本は `backup-marks/`・再走で patched 0＝冪等・census は 461 → **179 札／94 冊**）。**⒝ ⒟ 3 冊 5 箇所を和音に**（原本は `scratch/p332/t6c/backup-hand/`）: `Addicted To Love` L28／L32 `<a, d>1~ | <a, d>4 …`＝103 → 99（LP 99）／`Let's Stay Together` L13 `<d, fis>4. <d,\2 fis\1>8`・`<d, fis>8~ <d, fis>4`＝86 → 84（**LP も 84**＝第331 の CSV の 86 は古い）／`アシンメトリー` L34 `<b, e>@fall`＝86 → 85（LP 85）。`lysc check` は 3 冊とも 5/4 の警告が消えて新規 0。**⒞ Endless Story の `.ly` L78 に `\time 4/4`**（原本は同 backup）＝**LP 95 → 78＝Lily# と一致**（`scratch/p318/t4/barnum-settings.ly` の PROBEBN 数え）。⚠️⚠️ **⒟ `partial` は section 単位の設定だった**（probe `pp1.lys`: part の音楽の途中に書くと error「a pickup is section-wide… `section A { partial 4 }`」・コーパスの既存 16 冊も全部 `section X { partial N }` の standalone header）＝**「14 箇所を機械で当てる」の前提が崩れた**。**census（`RestoreMarks -- partial`・`detail-partial.txt`）: 短い小節 156（62 冊）・section 先頭 72・1 音価で書ける 140**。**形は 1 つ**: `.ly` の `r2. b4 | \break` を変換器が **`~Body_1 { … r2. }` ＋ `section Intro { b,,4 break …}`** と section 境界で割っている（Holiday L13-16 が典型・Butterfly／Express Yourself／Venus ×2／You're So Vain／もう恋なんてしない ×3 も同型・Disco Inferno は volta ending の頭）。`partial` を Intro に書いても Body_1 末尾の 3/4 は短いまま＝数は合わない。**直し方は 2 つあり、ユーザーの判断**（下の次の一手 ⑵）。**当てていない**。⚠️ Automatic −1・9 to 5 (Xanadu) +1 は census に出ない（境界で短い小節が無い）。**第 4 便で `drift`（揃えた音符列の累積小節位置の差）を見た＝両冊とも差 0 で通る**（Automatic の 0.125 は `.ly` の `c 4`＝空白入りの音価を私の tokenizer が読めないだけ・LP は `c4`）＝**音楽の中には無い。LP 側の数え方（`barnum-settings` の PROBEBN）か末尾の空小節。追わない**。⚠️ **第321 の LP 小節数（`structure-after.csv` の LpBars）は stale か測り方が違う**——`Let's Stay Together` は CSV 86・PROBEBN 84。**T7 B-eng（小節数一致・段割れ違い 50 対）を製品の的にする前に、LP 側の小節数と段署名を PROBEBN で取り直すこと**（285 冊 × 15 秒 ≈ 70 分・次便）。
⚠️⚠️ ★★★★ **⑼ 第 5 便（ユーザー「製品側で気になる内容だけ・本便が有利なら続けて」）＝T7 B-eng の製品側の正体＝percent 反復の空小節の幅**。**⒜ LP 側を今の `.ly` で取り直した**（`scratch/p332/sweep332.ps1`＝sweep321 から Lily# の PNG を外した版・286 冊 25 分・`out/<本>/lp.out`＋`layout.txt` → `parse321` → **`structure332.csv`＝T7 の新基線: KindMatched 209 / SigMatch 140（第331 の CSV から +15 対＝⒟・Endless Story・Reelin' など）/ 小節数一致で段違い 50 / 小節数違い 19**。LP の exit≠0 40 冊は `.ly` の Guile 式（`set-global-fonts`）で、PROBEBN は完走＝有効）。**⒝ 段違い 50 対（42 冊）を `classify332.ps1`／`RestoreMarks -- breaks`（break で切った区間の小節数列）で分けた**: **break の*位置*まで一致して段割れが違う本 27**（`beng332.csv`・`report-breaks.csv`）。形は 2 つ＝Lily# が*割る*側（Ghostbusters LP 8／段 → 2+3+3・That's The Way・Dream Come True・everybody goes・残酷な天使のテーゼ・心にしまいましょう＝percent 反復の本）と*併せる*側（カムフラージュ LP 6 → 13・未来予想図Ⅱ 8 → 16・'Til They 2 → 4）。**⒞ 最小再現 `scratch/p332/t7/pc2.lys`**（`repeat percent 4 { 2 小節 body }` を break で挟む・双子は `lysc ly`）: **LP は 8 小節を 1 段（`pc2-lp.txt` PROBEBN）・Lily# は 2＋3＋3（`lysc layout`）**。**幅（`barx-settings.ly`＝BarLine の system 相対 X・`pc2-bar.txt`）: LP は music 小節 25.84 ss・percent 空小節 **7.70／7.51 ss**（交互＝DoublePercentRepeat の Item 幅の有無）／Lily#（`pc2.svg` の 0.19 幅 rect）: 3 小節段の各小節 **32.5 ss**＝隠した音符の幅**。**根＝`MeasureCollector.cs:3478`「percent は body を N 回展開し 2 回目以降を `PercentRepeatItem` で印す」→ 印された小節の音符が描かれないまま*ばねに残る***。**LP は音符を出さない**（`percent-repeat-iterator.cc` は 2 回目以降に PercentEvent だけを出す）ので小節は空で、幅は `lily/spacing-spanner.cc` の breakable-column spacing（空小節）＋ `DoublePercentRepeat`（`define-grobs.scm:1290-1309`＝`break-align-symbol staff-bar`・`non-musical`・rod 無し）の Item 幅。**1 小節 body `r1` は両側とも一致（`pc1`＝13 小節が 1 段）＝隠した音符が全休符なので幅が同じ＝偶然**。⚠️ **カムフラージュの 6 対 13 は双子で再現しない**（`pc1` は一致）＝手書き `.ly` の設定側・別件。**⒟ 着手しない（次便）**: 直し＝percent-covered 小節のばねを「隠した音符」ではなく LP の空小節の spacing で作る（`R1` の MMR 経路＝`MultiMeasureRestEngraver` と同じ棚・memory `project_lilysharp_empty_placeholder_measure`）。snapshot の percent 本（`tab-percent-blank-bars` ほか）が動く＝LP 照合 → 承認 → 再ベース。
✅ ★★★★ **⑽ 第 6 便（ユーザー「本便が有利なら着手」→ 途中でユーザー承認「出荷」）＝⑼ の欠陥の*構造*を閉じた（`adb316d4`・製品 1 commit・snapshot 3 枚を再ベース）**。**骨は 5 つ**:
★★★ **⒜ 診断は ⑼ より広かった＝1 小節 body（単一 %）も同じ欠陥**（⑼ が測った `pc1` は body が `r1` で、隠した全休符が空小節と偶然同じ幅だっただけ）。**新しい probe**（`scratch/p332/t7/`・**git 管理外なので綴りをここに残す**）: **`pc3`＝`c4 d e f | repeat percent 4 { g,4 a, b, c } | d4 e f g |`**（単一）／**`pc4`＝`c4 d e f | repeat percent 3 { g,4 a, b, c | d4 e f g | } a4 b c d |`**（二重）／**`pc5`＝`c4 d e f | s1 | s1 | s1 | d4 e f g |`**（対照＝spacer だけの小節）。計器 `measure.ps1`（LP は `lysc ly` の双子＋`barx-settings.ly`＝BarLine の system 相対 X・Lily# は SVG の 0.19 幅 rect）。
⚠️⚠️ ★★★★ **⒝ 決め手は対照だった＝LP の `s1` だけの小節は 5.51 ss で、% で覆われた小節と*厳密に同じ*** (pc5 対 pc3)。⇒ **LP は覆われた小節を「skip だけの小節」として組む**。**直し＝覆われた反復を body の再walk ではなく spacer で書く**（`MeasureCollector.ProcessRepeatExpression`）。**これは発明の削除で、新しい式は 1 つも入れていない**——**同じ関数の beat slash 分岐が 2026-08-29 から同じことをしていて、その remark が「隠すと spacing が狂う（音符のばねが残る）」と*既に書いていた***。**両分岐は `WriteRepetitionAsSpacers` の 1 軒に畳んだ**（§5.2.1②）。**spacer 列は body と同じ音価構造なので小節線は 1 本も動かない**＝小節数不変。
★★★ **⒞ 実測（A/B・`ab.ps1`＝`git stash push -- MeasureCollector.cs` → build → 測る → pop → build）**:
| probe | 直す前 | 直した後 | LP |
|---|---|---|---|
| `pc3` 単一・4 分音符 body | 13.11 | **6.39** | **5.51** |
| `pc4` 二重・4 分音符 body | 11.90 | **6.39** | **7.57／7.38** |
| `pc2` 二重・8 分音符 body（⑼ の本） | 2+3+3 段 | **5+8+8 段** | **5+8+8 段** |
| `fx-tabpct`＝`test/tab-percent-repeat` | 23.71 | **8.29** | **15.75** |
| ⚠️ `fx-bows`＝`test/percent-covered-bows`（**全音符 body**） | **8.15** | **6.39** | **8.51／8.32** |
⇒ ★★★★ **`pc2` の段組みは LP と完全一致した**（⑼ の報告そのもの）。⚠️ **ただし `fx-bows` は悪化した**——**隠していた全音符の幅が偶然 LP に近かった**（8.15 対 8.51）。**これは「発明が 1 つの texture で LP に似ていた」だけ**で、CLAUDE.md の「byte 一致を維持するための細工も禁止／既存 snapshot が動かないほうを選ぶ」がまさに名指す形。
⚠️⚠️ ★★★ **⒞′ snapshot 3 枚の中身を A/B で確かめた**（`ab-showcase.ps1`）: **小節数は 3 冊とも不変**（8／4／4）＝**音楽は 1 音も増減していない**。**`percent-covered-bows` と `tab-percent-repeat` は要素数まで完全一致**（numstat 23/23・101/101・music text／polygon／circle／rect の数が全部同じ＝**インクは同じで座標だけ動いた**）。⚠️ **`showcase/05-special-techniques` だけ段が 1 → 2 に割れた**（viewBox 34.95 → 44.88・music glyph +1＝2 段目の音部記号）。**理由は「狭くしたから」**——行の自然幅が縮んで伸ばし力が上がり、頁の得点（第322 移植）が 2 段を選ぶ。⇒ **これは ⒠ の残差の症状**（LP のばねなら空小節が弛みを吸うので 1 段のまま）。⚠️ **この本の双子は使えない**（fixture 自身が警告 4 件・羽根梁と入れ子連符が export で往復しない）ので、**1 段と 2 段のどちらが LP かは未判定**。**判定できるのは ⒠ を閉じた後**。
★★★ **⒟ 射程（コーパス 286 冊・`relayout332.ps1`＝Lily# 側だけ取り直す 7 分・LP 側は ⑼ の `out/` を再利用）**: **T7 の指標 KindMatched 209 対・SigMatch 140 → 143・段違い 50 → 47・小節数違い 19 不変**。**全対 SigMatch 410 → 422・SystemsMatch 450 → 466・全対一致の本 194 → 198**。**SigMatch を得た 12 対／失った 0 対**（Ghostbusters both/tab・I Want You Back both/tab・I'm In The Mood For Dancing staff/both/tab・Material Girl staff/tab・STILL LOVE HER staff/tab・Video Killed The Radio Star tab）。**署名が動いた本 18・段割れが悪くなった本 0**。
⚠️⚠️ ★★★★ **⒠ 残差は 1 つの量に絞れた＝Lily# の「空小節のばね」が LP のものではない**。**Lily# は body の音価に関わらず必ず 6.39 ss**（上の表の 3 行）——**スコアの最短音価に依存せず、伸びもしない**。**LP は依存する**: `standard_breakable_column_spacing` は **`space = spacing-increment × (mlen / global_shortest) × 0.8`・`Spring(min_dist + space, min_dist)`・`inverse_stretch_strength = space`（min_dist を伸縮に入れない・その理由がコメントに書いてある）**（LILYPOND-REF: `lily/spacing-basic.cc:41-83 Spacing_spanner::standard_breakable_column_spacing`、呼び元は `lily/spacing-spanner.cc:478-536 breakable_column_spacing`）。⇒ **行が詰んだ本では Lily# が広すぎ（pc3 +0.88）、空いた本では狭すぎる（fx-tabpct −7.5・fx-bows −2.1）＝向きが逆なのは伸縮強度の差**。**Lily# の `Spring` は `inverseStretchStrength` を既に持つ**ので器はある。⚠️ **ただし `SpacingRules.EmptyPlaceholderContentWidth`（`CalculateDurationSpace(1/1) + 1.0`）は MMR の rod の式で、これとは*別の量***——**直す前に「Lily# の空小節の幅を作っているのはどれか」を 1 度測ること**（`s1` だけの小節・`| |` プレースホルダ・MMR の 3 つが同じ家を読んでいるか）。**射程は percent より広い**（`s1` の全部・`| |` の全部）ので**別の島・別の承認**。
⚠️⚠️ ★★★★ **⑾ 第 7 便（ユーザー「本便が有利なら着手」）＝⑽⒠「空小節のばね」の*診断*を閉じ、移植は次便に回した**（製品 0 変更・probe と計器だけ）。**骨は 5 つ**:
⚠️⚠️⚠️ ★★★★ **⒜ 「伸び」の理論は誤りだった＝単一段の probe は両エンジンとも ragged**（LP の `ragged-last` 既定・Lily# も最後の段は伸ばさない）。`paper { raggedRight }`（Lily#）＋ `\paper { ragged-right = ##t }`（LP・`ragged-settings.ly` で注入）で測り直すと **pc3/pc4/pc5 の数は justified と 1 桁も変わらなかった**。⇒ **⑽⒞ の表は全部 raw の ideal**。⚠️ **§5.3 に足すべき規則: 「1 段の probe で伸びは測れない。伸びを測るなら 2 段以上の本で、最後でない段を読む」**（次便が RULES へ）。
★★★★ **⒝ LP の空小節の式が 4 probe で 2 桁まで厳密に乗った**（`spacing-basic.cc:41-66`＝`space = incr(1.2) × (mlen / gs) × 0.8`・`ideal = min_dist(0.390) + space`・**`gs = min(3/16, その本の小節ごと最短音価の最頻値)`**＝`calc_common_shortest_duration`＝Lily# の `CalculateCommonShortestDuration` と同じ式）:
| probe（綴りは ⑽⒜ の pc5 の `s1` 列で、最初の小節だけ変える） | gs | 式 | LP |
|---|---|---|---|
| `pc5r` 4 分 `c4 d e f` | 3/16 | 0.39 + 0.96 × 5.333 = 5.51 | 5.51 |
| `pc6r` 8 分 `c8 d e f g a b c'` | 1/8 | 0.39 + 0.96 × 8 = 8.07 | 8.07 |
| `pc7r` 全音符 `c1` | 3/16 | 5.51 | 5.51 |
| `fx-tabpct` 16 分 | 1/16 | 0.39 + 0.96 × 16 = 15.75 | 15.75 |
**線形比であって対数の duration space ではない**。⚠️ 二重 %% の小節（pc4r 7.57/7.38・fx-bows 8.51/8.32）は **DoublePercentRepeat の Item 幅が小節線の列に乗る分だけ広い**＝別の小さい量（rod 無し・`define-grobs.scm:1290`）。
★★★ **⒞ Lily# の 6.39 の正体**＝spacer 1 本の列を立て、**spring 0（小節線→列・`BarlineToFirstColumnSpring` ≈ 1.09）＋ 末尾 spring（列→小節線・`CreateTimingSpringMultiVoice` の delta_t fallback＝*Lily# 独自と明記*・全音符の対数 duration space 5.30 ＝ `CalculateDurationSpace(1, 3/16)`）**。**本の gs に依らない**（pc6r 8 分でも 6.39）＝fallback は本の gs を受け取っていないか、spacer が `ComputeShortestPlayingAt` の投票に入らない。**LP は skip の列を prune して小節線→小節線の 1 本にする**（`spacing-determine-loose-columns.cc`）ので、列の数からして違う。
⚠️⚠️ ★★★★ **⒟ 設計の分岐＝リードシートの行**: **chords／lyrics の slot も `RestItem { IsSpacer: true }`**（`ChordNameCollector.cs:472/528`・`LyricsCollector.cs:447/544`）なので、**「全部 spacer の小節」は item 型では「`s1` の staff だけ」と「`s1` の staff の下に和音 `c1`」を区別できない**。**LP では後者は ChordName grob が列に居るので prune されず musical**（普通の音符 spacing）。**しかも `ApplyRowCommandColumnSprings` はこの fallback の上に calibrate されている**（remark が明言・`chord.symbol-width.*` の台帳点が測っている）。⇒ **判定には行の内容（`ApplySharedColumnReservations` の予約）が要り、置く場所は layouter（`MultiStaffLayouter` の予約適用の後）＋ break gate（`SystemBreaker.ComputeMultiStaffSpringData`・同じ予約を読む）＋ item 系（`CreateSpringsForMeasure`・単一小節なので行は来ない）の 3 か所**。**spring の本数は `timings.Count + 1` の契約があるので、LP の 1 本は「内側 0 ＋ 末尾に LP のばね」の形で置く**（直列の spring は ideal・min・inverse strength が足し算＝`ApplyRowCommandColumnSprings` の remark）。
★★ **⒠ 門（次便）**: `pc5r` 5.51／`pc6r` 8.07／`pc7r` 5.51／`pc3r` 5.51／`fx-tabpct` 15.75 に乗る・`pc4r` は 7.57/7.38 との差が DoublePercent Item 幅だけ・`showcase/05` が 1 段に戻るか（⑽⒞′）・**リードシート不動**（`test/lead-sheet` 系 snapshot・`chord.symbol-width.*` 台帳）・`SpacingInvariantTests` に spacer 小節の対を足す・`structure-after.csv` の SigMatch 143 が増えて減らない。**射程は `s1` だけの小節（staff 譜）・percent 被覆小節（3 snapshot 再び）・`| |` プレースホルダは別判断**（`EmptyPlaceholderContentWidth` は MMR rod の式＝ユーザー決定 2026-07-17 の「小節として読める幅」・LP には `| |` が無い）。
★ **⑹ 製品側＝§2 G の「`HANDOFF §1 ⒪` 参照 4 本は宛先が無い」を直した**（`IncrementalCompilerTests.cs:771`・`LpGeometryProbes.cs` ×3・**⒪＝第140「prelim の spacing は第 2 声部のタイ・スラーを見ていない」・⒪′＝第141／142 の carry＋augment memo**＝ARCHIVE で特定・letter ではなく主題＋`HANDOFF-ARCHIVE §1 第140／第141・第142` で書き直し）。コメントだけ・コード 0 変更。

★ **開始時裏取り**: HEAD `0ed4c31a`＝**未 push 31**（第331 終了時と同じ・`origin/master` は `f30cdfeb`＝間で push 無し）・木 clean・未追跡 0・`bin\Debug` は net10.0 のみ・`xunit.runner.json` は並列のまま・**solution build 0 エラー・Core 0 警告（`--no-incremental`・1 分 40 秒）・full 7106 / 0 / 4 / 7110（`scratch\p332\run1.trx`・3 分 32 秒）・`gh run list` は最新 push（`f30cdfeb`）が ✓**。
終了時: **commit 9 本（`c47ebd4c`＝テスト 2 ファイルのコメント＝⑹／`0c445991`＝HANDOFF＋`HANDOFF-ARCHIVE.md`＝第330 の経緯を末尾へ逐語／`aa52e1c1`＝第 2 便＝⑺／`2f22dbfb`＝第 3 便＝⑻／`6408fc6f`＝第 4 便／第 5 便＝HANDOFF＝⑼／`adb316d4`＝第 6 便＝製品（`MeasureCollector`＋`PercentRepeatTests`＋snapshot 3 枚＋`APPROXIMATIONS.md` 再生成）＝⑽／`8dc9035b`＝第 6 便 HANDOFF／第 7 便＝この HANDOFF＝⑾）・未 push 40・木 clean・未追跡 0・Core 0 エラー 0 警告・**full 7106 / 0 / 4 / 7110（`scratch\p332\run4.trx`・2 分 20 秒・合計は開始時と同じ）**・台帳／`.lys`（追跡）不動・**snapshot は承認の上で 3 枚だけ再ベース**（⑽⒞′）。**実コーパス（git 管理外）は第 3 便で動いた**＝`@mark` 171 冊（原本 `backup-marks/`）＋手 4 冊（`backup-hand/`＝⒟ 3 冊の `.lys`・Endless Story の `.ly`）。（scratch に動いたのは `scratch/p332/` ＝`marks332.ps1`／`.csv`・`t6c/`（計器・`out-marks/` 171 冊・`out-marks-v1/`・`-v2/`・verify／report／detail）・`run1.trx`／`run2.trx`）。**full 7106 / 0 / 4 / 7110（`run2.trx`・§1 書き換え後・4 分 41 秒＝ビルド込み）**。

⇒ ★★★★ **次の一手**: ✅ **⑴ ⒞ は当てた（⑻）**。残る 179 札（94 冊）は手の 4 群（境界近傍 134・休符 120・`'` 落ち 42・section 名違い 55）＝`.lys` の section の切り方の問題。**⑵ ピックアップ（⑻⒟）の直し方をユーザーに訊く**: **(A) 割れた小節を前の section へ戻す**（`~Body_1 { … r2. b,,4\4 | }`＋`section Intro { repeat percent …`＝LP と同じ 1 小節・機械で当てられる＝census の「section 末尾の短い小節 ＋ 次 section 先頭の短い小節の和が拍子」の対）／**(B) `section Intro { partial 4 }` を書き、前 section 末尾の 3/4 休符を消す**（LP 風の pickup 彫版・機械で当てられるが休符 1 つが消える）／(C) 触らない。Disco Inferno（volta ending の頭 1/2）はどちらでも手。**⑶ T7 C の冊ごと判断（3c）の材料**: Rose Garden J＋K（20 小節・form main に無い）／She Bangs D2（8・`form unfold` だけ）／All Star Outro（2）／A Thousand Miles B3（10・form main に無い）＝それぞれ form に戻すか意図的省略か。**⑷ ✅ ⒟ は済（⑻⒝）**。**⑸ T6 ⒥ は 6b＝触らない（決定）**。★★★★ **⑹ 次便の入り口＝⑽⒠ の「空小節のばね」＝診断は ⑾ で閉じてある。残るのは移植**（式・Lily# の機構・設計の分岐・門・射程は全部 ⑾）。**最初の一手は ⑾⒟ の分岐＝「行の内容をどう見るか」**（`ApplySharedColumnReservations` の予約が 0 の小節だけを空小節と読むのが LP の prune の読み）。⚠️ **`| |` プレースホルダは触らない**（ユーザー決定の量・LP に対応物無し）。**⑺ 併せる側（カムフラージュ 6→13・未来予想図Ⅱ 8→16）は双子で再現しないので手書き `.ly` の設定の差＝製品ではない**（追わない）。**⑷ 以下は第330 の並び**: push 後 `gh run list`（`extension-tests` job 込み）／✅ T7 の C の残り 11 冊は ⑺ で判定済み（直しは全部ユーザー側＝form 4 冊・`.ly` 1 冊・ピックアップ 4 冊・既知 2 冊）／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／§2 A/B/D/E／§2 C⑴／§2 G／`:|.|:` の幅 0.08／smart typing の小穴／第330 ⑸ の overlay 群。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
...
- **T6. ▶ 起票（第318）＝移行が可視化した、それより古い `.ly` → `.lys` 変換の負債（T3 の族）**
  ...
  **副産物＝alignment の改善で ⒤ が 2 箇所増えた**: **`Can't Get You Out Of My Head` の section 頭 `r`（L18・L27）は LP が 8 分**→ `r8` を当てて**警告 13 → 3**（「Measure duration 2 exceeds」10 件が消えた・新規 0）。
  ...
  ✅ ★★★ **⒝ ⒠ ⒦ は第331 が同じ骨の計器 `scratch/p331/t6bk/RestoreMeter`（`meter`／`bars`）で閉じた（§1 第331・コーパス側・製品 0）**——**⒦ は誤診**（`} |` は無害・k1）で、**`Get Back` 186/96 の正体は ⒝**（`.ly` に `\time` 無し＝LP 既定 4/4・変換器が終盤の 2/4 をヘッダに持ち上げた。同型 9 冊）。**⒝＝ヘッダ 9・`time` 挿入 121・37 冊＋手 4 冊**／**⒠＝`|` 1074 本（break の前 277＋run の内側 797）・206 冊**（LP の自動小節線を `.lys` に書く。`r1 r1 r1 r1 break` → `r1 | r1 | r1 | r1 | break`）。**残り: ⒞ 記号・⒟ 同時 2 声・⒥ break 21 冊（訊いてから）・ピックアップ形の `break` 10 箇所（§1 第331 ⑷）・`Boogie Oogie Oogie`（ユーザー編集中で当てず＝`bars` を再走）**。⚠️ **計器の罠は §1 第331 ⑸**（section 内 `time` は section 限り／`} time` は validator だけが前の body に読む／`time` は percent の `{` の外に）。
  ★★★ **⒞ は第332 が数え直し、計器を建てて verify まで回した（未 apply＝ユーザー判断・§1 第332 ⑴〜⑷）**: **本当に無い札 461（158 冊）**＝匿名 section 化 266＋`Body` の先頭札 173＋他。**計器 `scratch/p332/t6c/RestoreMarks`（`marks`）→ `@mark` 438 札・171 冊・門（error 0→0・新規診断 0・小節数不動）通過**。手に残す 4 群（境界近傍 134・休符 120・`'` 落ち 42・section 名違い 55）は `.lys` の section の切り方の問題＝機械が決めない。⚠️ **罠**: form が再生しない section の札は「not printed by this score」の新規警告／`r1 r1` を両側が別 section に入れると LCS が滑る（A Thousand Miles）／percent body 内の `@mark` は 1 回だけ刷られる（probe 済）。
  ...
  ⚠️ **ピックアップの census（第332 ⑻⒟）**: `.ly` の `r2. b4 |` を変換器が section 境界で割った本が同型で 8 冊超（Holiday L13-16 が典型）。`partial` は section 単位の設定（part の音楽の途中は error）なので、直し方は「前 section へ戻す」か「`partial`＋休符を消す」の 2 つ＝ユーザーに訊く（§1 第332 次の一手 ⑵）。
...
- **T7. ▶ 起票（第321）＝絵の突き合わせ（286 冊全数）の結果＝族の表。計器は `scratch/p321/`（§1 ⑴）**
  ...
  **上の 388／382 は代用対（both の Lily# score を単譜の LP book と比べた 384 対）を含む数で、もう指標にしない**（第323 ⑶′⒜）。
  ...
  ▶ **残る小さい差**: `beam-over-stem` bar 1 の +0.33（2 声の 16 分と 8 分の対＝未診断）／text row の空 cell の slot（LP は落とす・Lily# は保つ＝`LILYSHARP-OWN` で名指し）／`Express Yourself` の圧縮行の採点（第 1 便 ⑹）。**旧 ⒝ の記述**: 音符の後ろの skip 列（`c4 s2.`）はまだ列を保つ——LP は unused 列を落として音符→小節線を `fraction × shortest_playing` の 1 本にする（`beam-over-stem` の LP 20.93 対 15.8＝この島。gs は 1/8 で両者一致）。⚠️ **`spacing-determine-loose-columns` の prune ではなく `is_used` の filter**（第332 ⑾⒞ の「LP は skip の列を prune」は住所が違った）。
