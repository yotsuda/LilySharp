...
ユーザーの目的は **0.3.0 を出すこと**で、優先「**文法を完全にしてから出す**」は**この便で満たされた**。
...
終了時 **未追跡 0・作業ツリー項目 0・未 push 12**（**この行を書く commit を含めて 12**。
...
- **⑸ ★★★ 次に触るならここ**——**⓪ タグを打つ。「文法を完全にしてから」は満たされ、
  §1 に文法の未決は残っていない**: push（未 push 12・`master` を先に）→ CI 緑 → `v0.3.0`。
  CHANGELOG は 3 機能とも Highlights 反映済み・最上段=0.3.0 確認済み・署名の行は残す（§3）。
