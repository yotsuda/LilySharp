...
最終更新 第289セッション＝**`@!X`（テキストスパナの終端必須化）を 1 便で着地させた**——**コード `7b0df578`・ハンドオフはこの 1 本。** **第288 が「小さく刻めない」と書いて意図的に戻した島**で、**parser・モデル・語彙・コレクタ・engraver・診断・fixture 移行が 1 つの commit に入っている**。**台帳は 1 点も動いていない**（742／exact 582／ss 非ゼロ 190／総和 25.844537623／count 156 中 2）——**終端が動かしたのは*長さ*であって*高さ*ではない。**

**⑴ 引き継ぎの数は「§1 が第288 のまま」だった**（**§1 は 2 便続けて古い**）。**§1 が名指す HEAD `4db35c23` はもう 4 本古く**、**第288 は自分のハンドオフを書いたあとに §2 と §3 へ 4 本足していた**（`3c648c46`／`92f4bdeb`／`3708ff49`／`672994e9`）。**開始は HEAD `672994e9`・未 push 22・木 0・未追跡 0**で、**§1 の終了時の数（suite 6480/0/4/6484・台帳 742／582／190／25.844537623・追跡 579 冊・snapshot 229 枚）は 1 つも違わなかった**——**動いていたのは §2・§3 だけ**。⇒ ★★ **「§1 が最新とは限らない」は第288 の教訓だが、*その第288 自身が同じことをしている***。**`git log` と §1 の HEAD が合わないときは、差分が誰の便かを `git show --stat` で先に読むこと**（**docs だけの 4 本＝§2 に起票が積まれた形**）。

⚠️⚠️ ★★★ **⑵ 「閉じられなかった span は*短い span* ではない」**。**LP の `Text_spanner_engraver::finalize`（`lily/text-spanner-engraver.cc:117-127`）は warning を出して `suicide()`**——**線だけでなく*語ごと*消える**。**Lily# の 1 小節フォールバックは「短くする」という第 4 の答えを発明していた**もので、**それを読み手に告げる術は無かった**。⇒ **LYS4018 は「描かなかった」を*言う*ための診断で、代わりに何かを描くためのものではない。**

★★★ **⑶ LP のこの engraver は答えを 3 つしか持たず、本便はその 3 つを写した**（`process_music:59-88` と `finalize:117-127`）: **⒜ 開いていない `@!` は「cannot find start」で何も作らない**／**⒝ 開いている最中の START は「already have a text spanner」で*捨てられる***（**開いている方が span を保つ＝入れ子にならない・置き換わらない**）／**⒞ 閉じられなかった START は `suicide()`**。⇒ ★★ **「3 つの状態を数えて enum にする」のではなく、*LP の engraver が何回 warning を出すか*を数えると、状態の分類がそのまま出てくる**（`TextSpanPairingFault` の 3 arm はその 3 つ）。

⚠️⚠️ ★★★ **⑷ 退役した 3 つは全部「対にする相手が無い」ことの帰結だった**——**1 小節フォールバック／「次の rit/accel が閉じる」探索／第288 が足した「自分の次の演奏では閉じない」ガード**。**演奏順に対にすると 3 つとも消える**（**演奏 1 の STOP は演奏 2 の START より先に来るので、ガードは*規則としてではなく順序として*成り立つ**）。⇒ ★★★ **1 便前に足した特別扱いが、次の便で*構造ごと*要らなくなることがある。** **「その規則は何を補っていたのか」を先に訊くこと。**

★★★ **⑸ parser は字句を 1 文字も変えずに通った**（第288 が実証済み・本便で確認）。**`!` は既に `SyntaxKind.DashedBar`**（`Lexer.cs:196`）で、**`@` の直後は名前の位置＝小節線が来られない**ので**2 つの読みが同じ位置を争わない**。**`Name` と `MarkName` が `DashedBar` を飛ぶ**ので、**`@!rit` と `@rit` は*同じ名前*を報告する**——**語彙も「もしかして」の候補リストも 1 本のまま。** ⇒ ★★ **新しい綴りを足すとき、*名前空間を増やさない*形があるなら選ぶこと**（**終端が自分の名前を持つと、語を足すたびに 2 か所へ足すことになる**）。

⚠️ ★★ **⑹ 型は語を持てない**。**`MusicMarkType.Rit`／`Accel` を退役して `TextSpanStart`／`TextSpanStop` にした**——**`ly/articulate.ly` が `"rall"`／`"rit."`／`"accel."`／`"poco rall."` を*文字列比較*しており、同じ場所の TODO が「もっと synonym を」と言っている**ので、**語彙は原理的に閉じない**。**印字文字列は `MusicMarkItem.TextSpanSugarText` の 1 表と `@textSpan("…")` の引数**で、**`GetMarkText(TextSpanStart)` は `""` を返す**（**推測で "rit." を刷ると、誰も書いていない語が紙に出る**）。**両方とも `BuildPlain` という 1 つの扉を通る。**

⚠️ ★★ **⑺ 対応付けは (譜, *声部*) ごと**——**LP の `Text_spanner_engraver` は `Voice` コンテキストに立っている**（`ly/engraver-init.ly:375`）。**そのために `MusicMarkItem.VoiceIndex` を足した**（**コレクタは `StaffIndex` を書くその場で `_cursor.VoiceIndex` を持っていた**）。**別の声部の `@!rit` は何にも届かず、両方が「対にならなかった」として報告される**——**LP と同じ答え**で、**黙って声部を跨ぐ span を引くよりよい。**

★★ **⑻ 診断と描画は 1 つの呼び出しから出る**（`TextSpannerEngraver.PairTextSpanners` が `(Spanners, Unpaired)` を返し、**layout は前半・`TextSpanPairingValidator` は後半を読む**）。**`SlurPairingValidator` の型をそのままなぞった**——**対応付けを 2 か所で決めると、警告とページが食い違う。**

⚠️ ★★ **⑼ 引っかかった番人は 4 つで、どれも「言語が変わったこと」を正しく報告していた**: **`LpReferenceCitationTests`（新しい引用が名前を言っていない・第288 が予告していた）**／**`ApproximationInventoryTests`（`OWN` が 115 → **114**＝**`TextSpannerEngraver` の「LILYSHARP-OWN, ALL OF IT」が返済された***）／**`MagicConstantInventoryTests`（行番号のずれだけ）**／**`CollectEditResumeTests`（`01-expressions` の文字列を探していた）**。⇒ ★★ **`OWN` が 1 つ減ったのがこの便の性格**——**足した宣言は 0、返した宣言は 1。**

⚠️ ★ **⑽ `ly/articulate.ly:565-589` には引用ラチェットが検査できる名前が 1 つも無い**（**下線も 3 節ハイフンも無い**）。**行範囲を*外して*ファイルだけ引き、理由をその場に書いた**——**ラチェットが数えるのは「行範囲を出して名前を言わない引用」なので、これは回避ではなく*正しい精度の申告***。⇒ ★★ **名前を言えない場所は、行範囲を出さない。**

⚠️ **見つけて直していないもの**（前便から引き継いだ分に加えて）:
- **★★ スパナの*左端*が小節の頭に釘付け**（**§2 に ▶ で起票**）。**`StartItemIndex: 0` が定数**で、**本便で*右端*だけが書き手のものになった**ので**いま左右が非対称**。**LP は START の音符から引く**（`stop_translation_timestep:108-115` が LEFT を `currentMusicalColumn` に置く）。**あえて入れなかった**——**台帳にスパナの左端を測る点が 1 つも無く**、**言語の変更と測っていない X の変更を同じ便に混ぜないため。**
- **★★ `@ottava` と ペダルの終端**（§2 の ▶）。**寄せ先はできた**が、**`@!ottava` と既存の `@loco` のどちらを正典にするかは要ユーザー判断。**
（以下、前便からの一覧はそのまま）

⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 U の 6 件は全部閉じており**、**本便で「見えている問題」を新しく足したのは 1 つだけ**——**`Untitled-6.lys` の `rit.` が消えたこと**（**⑾・これは決定 B の代償そのもので、直すのはユーザーが `@!rit` をどこに置くか決めたとき**）。**手元に在るのは §2 の ▶ 3 つ**（**ペダル／オッターヴァの終端＝ユーザー決定で優先度高**・**スパナの左端**・**loose-line の閉じの距離を部屋の外側プロファイルへ**）**と、第288 が残した ⑺（歌詞のバネが最小に座る・台帳に `OPEN:` で待っている）**。

⚠️⚠️ ★★★ **⑾ ユーザーの本が 1 冊、いま何も描かない**。**`scratch/ベースタブLy/Untitled-6.lys` の `@rit` は繰り返される section の中に在って閉じられていない**ので、**LYS4018 が 2 行出て spanner は描かれない**（**「既に開いている」＝2 回目の演奏 ＋ 「閉じられていない」＝1 回目の残り**・**1 つの書かれたマーク**）。**その本には触っていない**——**rit をどこで終えるかはその人の音楽**（起票時の指示）。**diagnostic が位置を指す。**

★ **開始時裏取り**: HEAD `672994e9`・**未 push 22**・木 0・未追跡 0・Core 0 エラー 0 警告（`--no-incremental`）・**Windows Debug 6480 合格 / 0 失敗 / 4 skip / 合計 6484**・台帳 **742 点／exact 582／総和 25.844537623／ss 非ゼロ 190／count 156 中 2 が非ゼロ**・追跡 `.lys` **579 冊**・snapshot **229 枚**——**§1 の終了時の数と 1 つも違わなかった**（⑴）。
終了時: **commit 2 本**（`7b0df578` コード ＋ **この 1 本＝ハンドオフ**）＝**未 push は 開始 22 ＋ 本便 2**・木 0・未追跡 0。**suite 6521 合格 / 0 失敗 / 4 skip / 合計 6525**（開始 6484 から **+41**＝**退役 5〈古い規約を測っていた `DetectTextSpanners` の 5 本〉／新設 46〈対応付け 9 ＋ `TextSpanPairingValidatorTests` 17 ＋ `SpanTerminatorSyntaxTests` 20〉**）。Core 0 エラー 0 警告。**追跡 `.lys` 579 冊・snapshot 229 枚**（**どちらも不動**——**本便は本を足さず、4 冊を*移行*した**）。**台帳全点不動。**
⚠️ **掃きは 1 回**（**母集団はディスク上の全 `.lys` 1582 冊＝ユーザーの実コーパス込み・base と head は*本ごとに続けて*・第286 ⑼**）: **SAME 1554 / MOVED 28・レンダー失敗 0**。**28 はテキストスパナを書く本ちょうど**（**追跡 fixture 4＝移行した本・scratch 23・ユーザー実コーパス 1＝`Untitled-6.lys`**）。**`lysc check` は 28 冊で違い、エラーが増えた本は 0**（**9 冊は*base が*新しい綴りを読めずエラーを出していた側**）。
⚠️ ★★ **掃きは並列化した**（`scratch/sweep289p.ps1`——**`scratch/` は git 管理外なので消えていたら建て直す。40 行**）。**直列版は 1582 冊で 2547 秒**かかっていた（**1 冊につき exe を 4 回起動＝約 6300 回**）が、**並列版は 24 ワーカーで 145 秒＝17.6 倍**。★★ **その並列版は*自己検査してから*信じること**——**base と head に*同じ* exe を渡して回すと、並列化が持ち込みうる唯一の欠陥（ワーカーが互いの一時ファイルを潰し、*他の本のページを読んで SAME と言う*）が **MOVED になって出る**。**第289 はそれを回して 1582/1582 SAME・MOVED 0 を見ている**。**守るのは 2 つだけ**: **base と head は同じ反復の中で叩く**（**2 パスに分けると、その間に編集された本が偽の差を出す**）／**一時 SVG は冊ごとに別名**（**直列版は `b.svg`／`h.svg` を使い回していて、並列にすると互いに潰し合う＝*他の本のページを読んで SAME と言う*掃き**）。
★★ **緑は*コード commit のあと*に取った**（第282 ⑴ の再発防止）。★★ **WSL ubuntu Release も同じ 6521 / 0 / 4 / 6525**（`scratch/linux-repro` の手順・**`7b0df578` の木で**）。⚠️ **CI だけは未実行**（未 push 24 本）。


## 以下は第288セッションの経緯


最終更新 第288セッション＝**ユーザー報告を 2 件閉じ、2 件目の兄弟も閉じた（3 便）**——**U5（タブ譜と歌詞行が重なる）が `7900181c`**、**U6（`@rit` の長さが 1 本目と 2 本目で違う）が `fd46cdda`**、**その兄弟（hairpin が同じ形で壊れていた）が `4db35c23`**。**台帳は 2 点増えた**（740 → **742**・**exact 582 は不動**）——**新しい対が 1 つと、その対照が釣った*別の*欠陥が 1 つ。** ⚠️ **U6 はユーザーが「どちらが正しいか確認して」と訊いたもの**で、**答えは「1 小節のほうが正しい」**。
...
⚠️⚠️ ★★★ **⑼ 2 便目（U6）＝マークが「自分の次の演奏」で終わっていた**。**`musicMarks` は*演奏された*曲のマーク**なので、**form が繰り返す section は同じ `@rit` を 1 演奏につき 1 個積む**——**同じ `SourcePosition`・違う小節**。**「次の rit/accel で閉じる」探索がその 2 個目を拾い**、**1 本目が間の全部（この本では section B 丸ごと）を貫いていた**。⇒ ★★★ **`m != mark` は「同じ書かれたマークの別の演奏」を*見られない***——**2 つの演奏は 2 つのオブジェクト**。**同一性は `SourcePosition`。** ⇒ ★★ **展開後のリストを歩く探索は、「これは同じ源か」を必ず訊くこと**（**繰り返し・volta・D.S. のどれでも同じ形が出る**）。

⚠️⚠️ ★★★ **⑽ その欠陥は第286 が*自分で建てたプローブの中に映っていた***。**`scratch/p286/u2/twice.lys` は「同じ section を 2 回演奏」を測るために建てた本**で、**その中で 1 本目の rit が 1 段まるごと・2 本目が 1 小節**だったのに、**建てた便はそれを見ていない**（**その便が見に行った量は「和音行が rit を避けるか」だった**）。⇒ ★★★ **プローブは「見に行った量」しか見せない。** **同じ本をもう一度開くときは、*前の便が見なかったもの*を先に数えること。**

⚠️ **⑾ remark が書いていて*コードがしていなかった*ことを 2 つ直した**（U6 の島）: **⒜「tempo 変更が span を終わらせる」は嘘**（**探索リストは rit/accel だけ**——**bar 1 の rit と bar 5 の `tempo 160` で実測: 1 小節＝fallback、4 小節ではない**）／**⒝ `LILYPOND-REF` は engraver に付いていたが、この規則自体は LP に無い**（**LP は `\startTextSpan`〜`\stopTextSpan`**）。⇒ ★★ **「LP がこうしている」と読める comment の下に、LP に無い規則が座っていることがある。**

⚠️ ★★ **⑿ 単体テストが `sourcePosition: 0` を 2 つ並べていた**——**parse された本では起こり得ない状態**で、**しかも今回の直しが弾かねばならない形そのもの**。**赤くなって初めて気づいた。** ⇒ ★★ **手で組んだテストデータは「実在し得る状態か」を訊かれていない**——**不変条件を足すと、まずそこが落ちる。**
...
⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 U の 6 件は全部閉じた**ので、**「読み手が見ている問題」の在庫はまた空**。**手元に在るのは ⑺（証拠が揃っていて、台帳の点が 1 つ赤く待っている）と、U6 が残した「終端の綴りが無い」という要ユーザー判断と、第286 が残した ⑹ の広い側（loose-line の閉じの距離を部屋の外側プロファイルへ移す）と、§1 の「要ユーザー判断」の束**。★ **ユーザーは「残債の返済をやめて、見えている問題を直せ」と言っている**ので、**見えるものが在るならそれが先。無ければ ⑺ が一番安くて一番証拠が揃っている一手**（**LP 側は測ってあり、番人になる台帳の点も既に置いてある**）。
...
終了時: **commit 7 本**（`7900181c` U5 のコード ＋ `afba7f09` ハンドオフ ＋ `6a669507` ubuntu 脚 ＋ `fd46cdda` U6 のコード ＋ `25726008` U6 のハンドオフ ＋ `4db35c23` U6 の兄弟（hairpin）＋ **この 1 本＝兄弟を §1 と §2 に入れるもの**）＝**未 push は 開始 12 ＋ 本便 7**・木 0・未追跡 0。**suite 6480 合格 / 0 失敗 / 4 skip / 合計 6484**（開始 6477 から **+7**＝**snapshot の番人 3 ＋ 台帳の点 2 ＋ 単体テスト 2**）。Core 0 エラー 0 警告。**追跡 `.lys` 579 冊**（+3）・**snapshot 229 枚**（+3）。**台帳 742 点／exact 582（不動）／ss 非ゼロ 190／総和 25.844537623／count 156 中 2。**
⚠️ ★ **本便は「ハンドオフを書いたあとにユーザーがもう 1 件報告した」形が 2 度**——**だから §1 は 1 つの便に 3 つの島が入っている**。**`6a669507` までが U5、`25726008` までが U6、`4db35c23` からがその兄弟。**
⚠️ **掃きは 3 回**（**島ごとに 1 回・母集団はディスク上の全 `.lys`＝ユーザーの実コーパス込み・base と head は*本ごとに続けて***・第286 ⑼）:
**U5＝1559 冊 SAME 1525 / MOVED 34**（**実コーパス 20 ＋ scratch 11 ＋ 追跡 3**＝`test/tab-below-range`・新しい `test/tab-lyrics-inside-strings`・`audit/lp-regression/lys/automatic-polyphony-tabstaff`）／
**U6＝1567 冊 SAME 1557 / MOVED 10**（**実コーパスは*報告された 1 冊だけ*・scratch 8 ＋ 追跡 1**＝新しい `test/rit-span-in-a-repeated-section`）／
**兄弟＝1572 冊 SAME 1569 / MOVED 3**（**実コーパス 0 冊・scratch 2 ＋ 追跡 1**＝新しい `test/hairpin-in-a-repeated-section`）。
**どの回も `lysc check` はバイト同一・レンダー失敗 0。**
⚠️ ★★ **U5 の掃きは 1 度やり直した**——**1 回目の途中で head の exe を建て直してしまった**ので、**前半と後半が別の exe だった**。**やり直しの head は最終の木とバイト同一であることを確かめてから回した。**⇒ ★★ **掃きの最中に建てない。建てたらやり直す。**（**U6 と兄弟の掃きはこれを守った。**）
★★ **緑は*コード commit のあと*に取った**（第282 ⑴ の再発防止を踏襲）。★★ **WSL ubuntu Release も同じ 6480 / 0 / 4 / 6484**（`scratch/linux-repro` の手順・**`4db35c23` の木で**——**§0 の「この機械の Windows での緑でしかない」に両脚で答えた**）。⚠️ **CI だけは未実行**（未 push 19 本）。
...
> （2026-08-29・第288 起票・**★★★ ユーザー決定で優先度高**／**第289 で寄せ先ができた**）
>
> ★★★ **`@!X` は入った**（上の項・`7b0df578`）ので、**この項の前提はもう揃っている**——
> **`@!loco` ではなく `@!ottava`**、**`@!sustainOn` ではなく…** は**設計の問題としてまだ開いている**が、
> **綴りも parser も validator も既に一般形**（**`Parser.Music` の `DashedBar` 分岐は名前を選ばない**）。
> **足すのは `MusicMarkItem.ParseSpanEndName` の arm と、その族の対応付けだけ。**
>
> **⚠️ 何が問題か＝1 つの問いに 3 つの答えがあった**（第288 実測・`scratch/p289/pair1`・`pair2`・`r1`）:
> **⒜ `@sustainOn` を閉じないと*何も描かれない***（**"Ped." の文字ごと消える。しかも無言**）／
> **⒝ `@ottava` を閉じないと*描かれ*、12.01→33.64 で止まる**（**五線の右端は 60.37・第 3 の独自長**）／
> **⒞ `@rit` を閉じないと 1 小節**。⇒ **⒞ は第289 で退役した**（**LYS4018 を出して描かない＝LP と同じ**）。
> **残るのは ⒜ と ⒝**——**⒜ は既に LP と同じ「描かない」だが*無言*なのが欠陥**（**診断を足すだけで寄る**）／
> **⒝ は「独自の長さを描く」ので、終端必須へ移すと出力が動く**——**射程は掃きで測ること。**
>
> **⚠️ LP はこの問いに答えを 1 つしか持たない**: **閉じられなかった span は warning を出して
> `suicide()`＝何も描かない**（`lily/text-spanner-engraver.cc:117-127` で実測・`\cresc` 側も同じ）。
> ★ **`@ottava` の終端は既に語彙にある**（`@loco`）。**ユーザーは `@loco` を残す方に傾いている**
> （2026-08-29・第289 に「**レンダする文字は loco になるなら `@loco` がわかりやすい。でも、これは
> `@!ottava` の糖衣としてもいいかもしれない**」）。
>
> ⚠️⚠️ ★★★ **その前提は測ったら外れている——`loco` という文字はどこにも刷られていない**
> （2026-08-29・第289 実測・`scratch/p289/ott.lys`／`ottopen.lys`）。**`@loco` を書いた本の SVG に
> `loco` の text 要素は 1 つも無い**——**変わるのは括弧の右端だけ**（**`@loco` 有り 12.01→20.36 ／
> 無し 12.01→33.64**）。**刷られているのは左端の `8va` だけ。**
> **⒜ Lily# 側**: **`GetMarkText(Loco) => "loco"` は在る**が、**`IsSpannerHandled` が `Loco` を含む**ので
> **`DrawMusicMarks` はこれを描かない**＝**その文字列は誰も刷らない死んだ綴り**（`MusicMarkItem.cs:258` と `:419`）。
> **⒝ LP 側**: **`loco` は 2.26.0 の木に*1 か所も無い***（**`grep` で当たるのは drum の `loconga`＝低音コンガだけ**）。
> **`OttavaBracket` が刷る text は左端の `ottavation`**（`scm/translation-functions.scm:1145` の
> `ottavation-numbers`）**で、右端は `edge-height` のかぎ爪だけ**（`lily/ottava-bracket.cc:140`）。
>
> ⇒ ★★★ **だから決めるべきことは 2 つに割れている。両方ユーザー判断**:
> **⑴ オッターヴァの終わりに*何か刷るか***（**`loco` を刷るなら、それは LP に無い＝宣言する乖離**。
> **刷らないなら現状どおりで、`@loco` は「刷る語」ではなく*動作の語***）。
> **⑵ 綴りは `@loco` か `@!ottava` か**（**⑴ の答えで理由が変わる**——**刷るなら `@loco` は*ink の名前*として
> 正しく、刷らないなら `@!ottava` の糖衣という説明の方が正直**）。
> ⚠️ **両方を無条件に置くと「1 つの問いに 2 つの綴り」に戻る**ので、**糖衣にするなら
> `MusicMarkItem.ParseSpanEndName` の arm 1 本で寄せ、`@loco` と `@!ottava` が*同じ mark* を作ること**
> （**`@!rit` と `@!textSpan` が同じ mark なのと同じ形**）。
>
> ⚠️ **`@cresc` 族はこの島ではない**——**LP でも `\cresc` は Dynamic_engraver が「次の強弱」で
> 自動的に閉じる**ので、**自動終端が LP 由来である唯一の族**。**触らないこと。**

> # ▶ **テキストスパナの左端は「小節の頭」に釘付けされている**
> （2026-08-29・第289 起票・**証拠は揃っている／掃きも LP 側の点もまだ**）
>
> **`TextSpannerEngraver.PairTextSpanners` は `StartItemIndex: 0` を*定数で*渡す**——
> **`@rit` を小節の 3 番目の音符に書いても、線と語は小節の頭から始まる**。
> ⚠️ ★★ **第289 で*右端*だけが書き手のものになった**（`EndItemIndex` は `@!` の音符）ので、
> **いま左右が非対称**：`c4 d e@rit f | g@!rit` は **c から g まで**を引く。
> ★ **LP は START の音符から引く**——`lily/text-spanner-engraver.cc:108-115`
> `Text_spanner_engraver::stop_translation_timestep` が **LEFT 境界をその瞬間の
> `currentMusicalColumn`** に置く。**直しは 1 項**（`Math.Max(start.Mark.AnchorItemIndex, 0)`）。
> ⚠️ **第289 は*あえて*入れなかった**——**言語の変更と、測っていない X の変更を同じ便に混ぜないため**。
> **台帳にスパナの*左端*を測る点は 1 つも無い**（**textspanner.* の 4 点は全部*高さ***）。
> ⇒ **次にやるなら順は「LP のプローブで左端を測って点を置く → 直す → 全数掃き」**。
> **追跡コーパスで小節頭以外に `@rit` を書く本は `showcase/01-expressions` の 1 冊**（`e@p@rit`）。
