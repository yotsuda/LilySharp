...
- **⑹ ★★★ `perf-plain1k` の打鍵 46.1 MB の現在の地図**（第4便の後・単位 MB）:

  | 段 | MB | % | 状態 |
  |---|---|---|---|
  | `L1-break`（DP） | 19.3 | 42% | **閉じた**＝本質的・2 案とも測って却下（§1 第191） |
  | `4-render` | 8.9 | 19% | **床に近い**（構造上の下限 約 7.5・第191 第7便） |
  | **`1-collect` の `CollectMeasures`** | **2.68** | **6%** | ⚠️ **再開機構そのもの**（下の ⒜） |
  | `P4-prelimAnnotations` ／ 最終注釈パス | 2.7 ／ 2.7 | 12% | 済 |
  | 梁の検出（1 軒に畳んだ後） | 2.6 | 6% | ⑵ の後。中身は `CreateBeamGroup` |
  | `2a-contentKeys` | 2.0 | 4% | ⑶ の後。**残りは `BoundaryClefAllowance` 0.93 が首位**（下の ⒝） |
  | `3-layout` の未帰属 | 約 2.0 | 4% | ⚠️ **未分解**（下の ⒞） |
  | `1-collect` の `ResolveBeamStemDirections` | 1.77 | 4% | ⑷ の後。残りは刻印 1 回ぶん |
  | `CreatePages` | 1.0 | 2% | 済（第191 第5便） |

  ⇒ ★★★ **これで打鍵の 46.1 MB は*全部*名前が付いた**（未帰属は 3-layout の約 2 MB だけ）。
  **次の島は 3 つ**:
  ...
  **⒞ `3-layout` の未帰属 約 2 MB**（唯一まだ名前の無い塊）。
...
>
> **⑴ 残っている 3 つの島**（MB は `perf-plain1k` の 1 打鍵 46.1。**内訳は §1 ⑹ の表で、
> 未帰属はもう 約 2 MB しか無い**）:
> **⒜ `CollectMeasures` 2.68 MB＝再開機構そのもの**（歩きではない——生の `ProcessMusicNode` は
