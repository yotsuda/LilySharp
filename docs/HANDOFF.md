...
最終更新 第404セッション（2026-09-18・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `851e5538`・**未 push 0＝ユーザーが push 済み**・**full `sessions/p404/run1.trx` 8667 / 0 / 3 / 8670＝第403 最終と一致**・CI は `851e5538` の run が緑（開始時 in_progress → 完走 success・それより前の 4 run は赤のまま）・build 0 エラー・`Session-Check -Build -Test` 62 秒）、指示「HANDOFF を読んで着手」＋途中で「`LilySharp-Lab` の中身も活用して」（CLAUDE-OPERATIONS・p402/p403 の計器と出力・DEV_BUGFIX_WORKFLOW §10/§14・第330 の経緯を読んだ）→ §1「次の一手」の **⒜ R13⒝＝render 段の SVG 全文コピーを差分応答に、を採った**。道具は pwsh MCP（Bash 0 回）。**閉じたのは 1 本＋副産物 1 本**:

★★★ **⑴ 先に計器で wire の形を取った**（`sessions/p404/Zz404Probe.cs.txt`・結果 `sessions/p404/wire-shape.txt`・Release・既定 JIT・**一声かけずに回した＝比と数だけを根拠に**）: **plain1k の preview SVG は 3.13M chars（16 頁・200 system・data-pos 18,201）＝JSON 3.59 MB／打鍵、fingbeam1k 10.7M chars＝12.2 MB、v2bow1k 3.2M＝3.7 MB**。**Newtonsoft の直列化だけで 8.9／30.9／7.2 ms・割当 17／59／18 MB＝in-process の render 全体（15／26／14 ms）と同じ桁**、node 側はさらに parse（同程度）→ `svgPostKey` 連結 → `postMessage` の clone → `splitPages` → 伏せ regex の 5 回＝起票の「7 回コピー」。**頁で見ると**: 末尾の edit で 16 頁中 15 頁がバイト同一／bar 500 に空白挿入（Δ=+1）で 7 頁同一・**9 頁が「data-pos の shift を除いて同一」**・0 頁変更／bar 2 に挿入で 16 頁全部 shift／bar 500 に音符挿入は後続 9 頁が変更＝**小節が 1 つ増えて後続全頁の小節番号が動く**（正当）／v2bow1k は DP が組み直して 13→14 頁（fragment 55/63）。**頁単位の比較（exact・window 込み）は全文で 0.3〜1.5 ms**。⚠️ **副産物＝1 頁の本は interactive でも `<g class="page">` の包みが無く、webview の `splitPages` が null → 毎打鍵 `innerHTML` 全置換だった**（第330 の頁単位 reconcile は複数頁の本にしか効いていなかった＝`Assemble` の「single page は wrapper 無し」が export と preview の両方に効いていた）。

★★★ **⑵ 直し＝応答を頁にして、閲覧側が持つ絵との差分だけ送る**（`2f6bba07`・Core 5・Lsp 2・拡張 3・網 2・CHANGELOG）。**Core**: `SvgDocumentContext.BuildPieces`＝文書の形の 1 綴り（一文字列の `Assemble` と新しい `ToPages` が同じ piece 列を結ぶ）・`ToPages(previous, window)` が頁ごとに **Same（前回の string instance＝builder 上で `StringBuilder.Equals(span)` で判定・コピー無し）／Shifted（`SvgPageSet.SameModuloWindow`＝lockstep の走査で data-pos/data-alt の数だけが window で写像された値・それ以外は同一）／Changed** に分類＝**比較による証明であって memo からの推論ではない**（memo が同じバイトを描き直した頁も Same・memo 経由でも絵が変われば Changed）。window の写像は `SvgEditWindow.TryMap` の 1 綴りに畳み、fragment memo の `TryMapSlots`／`AnchorsFollowWindow` もそれを読む。`IncrementalCompiler.RenderIncrementalPages` が前回の set を持つ（一文字列の入口は忘れる＝次は全頁 Changed）。**interactive の 1 頁本も包む**（export は不変＝snapshot 249 不動）。**LSP**: `lilysharp/svg` に `PageDiff`／`ShownVersion` → `Pages{Version, BaseVersion, Head, Tail, Window, Items[Change, Markup?]}`＝client が session の前回の絵を持っていれば差分（changed だけ markup）・それ以外は全頁。**version はサーバ全体で 1 本の counter**（別 score の slot の version を自分の前回と取り違えない）。fallback（session 破棄）と `PageDiff` 無しの client は従来の一文字列（`LastPagesVersion` を 0 に）。**拡張**: `shownPagesVersion` を URI ごとに持ち（reload・dispose・re-key・一文字列応答・`requestFull` で忘れる）・差分の base が持っている version と違えば version 無しで取り直す。**webview**: `applyPages`＝same は触らず・shifted は頁の全 `[data-pos],[data-alt]` を window で書き換え（stored markup を `stale` に印＝以後の reconcile は carried group を必ず restamp）・changed は既存の群 reconcile（`reflowPages`）へ・適用できない形（絵が無い・頁数違い・frame 違い・window の外の offset・parse 失敗）は `requestFull` で全頁を取り直す。**網 23 本**: `SvgPageSetTests`（結合＝一文字列＝`Generate`／Same は同一 instance／offset だけの edit は全頁 Shifted で**閲覧側の再スタンプ規則（独立に綴った regex）が各頁を完全に再現**・誤った window は再現しない／音符 edit は自頁 Changed・前は Same・後は Shifted／頁数変化は全部 Changed／1 頁の包み／`SameModuloWindow` の受理・拒否 9 行）＋`SvgPageDiffTests`（初回は全頁＝一文字列と同一／同文は全 same・markup 無し／edit は 1 頁だけ markup・**client 側の再構成（same は保持・shifted は再スタンプ・changed は markup）が編集後の全文描画とバイト一致、undo でも**／stale な version は全頁／PageDiff 無しは文字列＋version リセット／別 score は version を取らない）。**full `run2.trx` 8690 / 0 / 3 / 8693（+23）・`npm test` 100 / 0・check-types 0・esbuild 0**。⚠️ **実機の VS Code では回していない**（ユーザー不在の autonomous 便）＝webview は parse 検査のみ。**次便が最初に見る計器**＝出力チャネルの `Got response … pages=16 (delta vA->vB: same S, shifted H, changed C; K chars)` と `PREVIEW update: pages kept …, shifted …` の行（K が 1 頁ぶんなら効いている・`Webview asks for the whole picture:` が出たら適用できない形の理由がそこに在る）。**配備は `tools/Deploy-Lsp.ps1`（ユーザーの未 commit 版）でユーザーが**。⚠️ 踏んだ罠 2 つ＝Edit ツールの引数に `'\n'` を書くと実改行になる（TS の `svgPostKey` 本文を anchor にできない＝別の行を anchor に）／新規 .cs は LF で出る → CRLF に直した（3 本）。

★★ **⑵′ 第 2 便（同日・指示「このセッションが有利なら着手」→ 載っている計器で*後*の wire を取るのが有利・実機は次便）＝差分応答の後の 1 打鍵の JSON を LSP 本体経由で量った**（`sessions/p404/Zz404Probe2.cs.txt`・結果 `wire-shape-after.txt`・`GetSvg` を in-process で `PageDiff`＋`ShownVersion` を繋いで・Newtonsoft の utf-8 bytes・一声かけずに回した＝bytes だけを根拠に・render の ms 列は single-shot で計時ではない）: **同文の打鍵と offset だけの edit（bar 500／bar 2 に空白）は 1.3〜1.4 KB**（旧 3.6〜12 MB）・**末尾の音高 edit は自頁 1 枚＝91 KB（plain1k）／306 KB（fingbeam1k）／136 KB（v2bow1k）**・初回（full）は旧と同じ 3.6〜12 MB。**残っているのは「小節が増える edit」**＝bar 500 に音符を足すと後続全頁の小節番号 overlay が動くので 16 頁中 9 頁＝2.0 MB（plain1k）／6.6 MB（fingbeam1k）が丸ごと届く（各頁は overlay 群 1 つしか違わない）・v2bow1k は DP が組み直して 13→14 頁＝全頁（正当）。⇒ **次の梃子は頁ではなく群**（`<g class="system">`／`<g class="overlay">` を item にする＝webview は既に changed 頁を群単位で reconcile しているので、server が群を頁と同じ規則で分類すれば済む）。この便では着手しない（実機の 2 行を見てから）。

★ **⑶ 終了時**: commit 2 本（code `2f6bba07`＋docs）＋第 2 便の docs 1 本・**最終 full `sessions/p404/run3.trx` 8690 / 0 / 3 / 8693**（run2 と同数・docs 書き換え後）・§7.5（対 `851e5538`）**Core `+` 399 行／REF 0／OWN 0**（数値定数なし・LP 由来の式なし＝wire と memo の配管＝監査対象は「同じ答えか」＝結合＝一文字列の網・再構成＝全文の網・full 同一）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。§7 3.5 は済（第402 を `-Archive 402`＝9 行 5,168 chars）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も・Lab は sessions/p404 を commit 済み）。⇒ **次の一手**: ⒜ **R13⒝ の実機確認**（Deploy → Reload → 出力チャネルの 2 行・1 頁本で `pages kept`／複数頁の途中 edit で `shifted`・`requestFull` が出ないこと・`PREVIEW update` の ms＝shifted 9 頁 × 1000 要素の DOM 属性書換えの実測）／⒝ **群単位の item**（小節が増える edit で後続全頁が bar-number overlay だけの違いで丸ごと届く＝⑵′・2〜6.6 MB）＝`SvgPageSet` の分類を `<g class="system">`／`<g class="overlay">` に下ろす／⒝′ frame 変更（頁数は同じで高さだけ動く auto-size の本＝fixture の形）は今 `requestFull` で全頁を取り直す＝実機で頻度を見てから `applyFrame`（root svg の width/height/viewBox と背景 rect だけ差し替え）／⒞ 第403 の残り＝edit 打鍵の `prelim` 1.4〜2.5 ms・v2bow1k の `voices`（`CalculateVoiceCollisions` 3.1 ms／7.5 MB 毎打鍵・memo 無し＝R13⒧）・count loop の page DP 5.3 ms（R13⒦・census で hit 率）・診断 pass の残り（`AnnotationName` 14／`MeasureValidator` 10／`LyricSings` 6.5）／⒟ LSP 起動直後の tier-0（ReadyToRun・`TieredPGO` の knob は packaging 側＝**要ユーザー判断**）／LP 双子が要る R7〜R11／R15（要承認）。

## 以下は第403セッションの経緯

最終更新 第403セッション（2026-09-18・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `567355ef`・未 push 32・**full `sessions/p403/run1.trx` 8664 / 0 / 3 / 8667＝第402 最終と一致**・CI は直近 5 run が赤＝未 push のまま・build 0 エラー・`Session-Check -Build -Test` はこの PC で 64 秒）、指示「HANDOFF を読んで着手」→ §1「次の一手」の **⒜ R13⒟＝plain1k の keys+gate 7.2 ms を段で割る、を採った**。道具は pwsh MCP（Bash 0 回）。**コードは 1 行も変えていない（docs のみ）＝測ったら病名が違った**:

★★★ **⑴ 「keys+gate 7.2 ms」は JIT tier-0 の数で、最適化済みの `MeasureContentKey.Compute` は plain1k で 1.0 ms**。**計器**（`sessions/p403/Zz403Probe.cs.txt`＝第402 の段階割りに、`IncrementalCompiler` の keys+gate 段（`Collected`→`BeforeLayout`）へ一時的な `SubStageProbe` 7 点（guard／keys／cache／same／springs／gate／reuse・**commit せず `git checkout` で戻した**）を足し、段ごとに **ms・割当 KB・GC 回数** を刷る・Release・Δ=0 は min-of-5・edit は **6 打鍵全部が edit**）。**結果 `sessions/p403/substages.txt`**: **第 1 run（順序 Δ=0 → edit mid → edit end・既定の tiered compilation）**: keys（＝`Compute` だけ）**7.04／6.20／0.86 ms、割当は 3 つとも 1188 KB＝同じ仕事**。他の段は全部 0.0x ms（guard 0・cache 0・same 0.01・springs Δ=0 で 0／edit で 0.5（memo 997 再利用／3 再計算）・gate 0.03・reuse ≤ 0.13）＝**keys+gate 段は `Compute` そのもの**。**run A（順序を逆＝edit end → mid → Δ=0・既定）**: keys **6.47／6.66／5.62**＝「edit end だけ速い」は消えた＝**速さは scenario ではなく順序**。**run B（`DOTNET_TieredCompilation=0`・順序は第 1 run と同じ）**: keys **1.00／0.99／0.98 ms**・割当同一。fingbeam1k 1.88／1.87／1.83（33000 item）・v2bow1k 0.80／0.77／0.77。⇒ **病名＝`Compute` の hot path（`HashContent`・`Hash64.Add<T>`・compiled getter 呼び出し）が tier-0 のまま走っていた**。call counting は新しい tier-0 JIT が 100 ms 途切れるまで始まらない（`TC_CallCountingDelayMs`）ので、**数秒の in-process 計器では昇格する run としない run がある**（第 1 run の edit end だけ昇格後だった）。実 LSP は長寿命＝打鍵が止まれば昇格する＝**定常は 1 ms・第402 の「keys+gate 3.5〜4.7 ms が打鍵の次に大きい固定費」は撤回**。⚠️ ベンチは一声かけずに回した（数秒の in-process 計器・ユーザー不在の autonomous 便）＝根拠は絶対値ではなく **割当 KB の同一・順序を逆にすると数が付いてくること・TC=0 で 3 scenario が同じ数になること**。

★★★ **⑵ R13⒟ の reference-identity memo は現状 0 hit＝実装しない**。**Identity 計器**（同じファイルの `Identity`・`sessions/p403/identity.txt`）＝full collect を recorder で記録 → 同じ木／edit end／edit mid に `CollectResumePlanner.Plan` → resume → 前の score と `ReferenceEquals` を数えた: **3 冊 × 3 scenario 全部 0/1000 measure・0/8000 item**（plain1k Δ=0 で 1000 採用・splice 0 でも 0）。採用した prefix は `MeasureBuilder.Restore` が*参照で*受けるが、walk 後の pass が Measure を作り直す（`with { Items = … }` は Collector 配下に 12 箇所＝`MeasureCollector.Stems.cs` の stem/beam 解決・`StaffAccidentalColumns`・`OttavaTransposer`・`TabResolver` 等・**どれが plain1k で走るかは数えていない**）。memo を当てるには先にこれらを identity 保存にする仕事が要り、取れるのは 1 ms（plain1k）／1.8 ms（fingbeam1k）まで。**`Compute` の内訳（TC=0・min-of-5）**: intrinsic fold 0.85 ／ 全体 0.99 ms（plain1k）・1.60／1.77（fingbeam1k）・0.64／0.72（v2bow1k）＝side table・context chain・mmr・clef 幅は 0.1〜0.2 ms。**Δ=0 打鍵の全体（run B）**: **plain1k 4.65 ms**（keys 1.0・render 2.3＝**8.4 MB 割当**・残り collect）・**fingbeam1k 13.2**（collect 5.3・keys 1.9・render 7.7＝**33.6 MB**）・**v2bow1k 10.7**（keys 0.8・render 4.2＝9.7 MB）。**edit end（run B）**: plain1k 20.8（layout 16.0・render 1.6）・fingbeam1k 32.8（layout 19.3・render 5.9）・v2bow1k 28.7（springs 3.5・layout 9.7・render 4.3）＝**Δ≠0 の最大の段は layout**。⚠️ 踏んだ罠 4 つ＝**第402 の計器の「edit」列は交互列の 1 打鍵目が Δ=0 だった**（warm が `other` で終わり最初の timed も `other`）＝直した／pwsh MCP の pipeline 内でも `$pat` 等の変数は **`$script:` が要る**（無いと後段が空振りして出力が消える）／`--filter "DisplayName~plain1k"` は xUnit の theory 行に当たらない（`FullyQualifiedName~` だけで絞る）／**Core に一時計器を置いたら `git checkout -- <file>` で戻してから `git status` を見る**。

★★★ **⑶ 第 2 便（同日・指示「このセッションが有利なら着手」→ 計器と compile pipeline の文脈が載っているので ⒝ layout 段の段割りに着手）＝system 0 の梁を memo の外で毎打鍵 quanting していたのを閉じた**（`4f6e5603`）。**計器**（`sessions/p403/Zz403Probe2.cs.txt`＝`LayoutEngine` に一時的な static `ZzProbe` を 20 点（撤去済み・commit せず）・edit 3 種（bar 2／500／998）× 6 打鍵全部 edit・TC=0・段ごとに ms／割当 KB・`SystemLayoutCache.PassCounters` の hit/miss と count loop の「trying」行数）。**結果 `sessions/p403/layout-stages.txt`／`layout-stages2.txt`**（plain1k・edit end＝21.7 ms 打鍵の layout 段 16 ms の内訳）: `breaks`（line DP・row-prefix resume）**0.18 ms（end）／11.0（mid）／16.3（start）＝edit 位置に比例・設計どおり**／`sys0`（loop の前の system 0 の配置）**3.2 ms／3.1 MB＝全部 `EdgeStaffBeams`（梁の quanting）**・`systems`（200 system の loop・memo 全 hit）0.62・`prelim` 1.4／3.7 MB・**`count`（`ChooseSystemCount`）6.9 ms／13.9 MB＝うち「ideal より多い count を試す」loop（`c.up`）6.1／13.0 MB＝78 count を毎打鍵 pricing して常に ideal を選ぶ**（`place2` は 3 冊 × 3 edit で 0 回）・`pages` 0.7・`annot` 0.9／3 MB・`spanners` 0.14・`voices` 0（**v2bow1k は 3.1 ms／7.5 MB**）。fingbeam1k edit end 39.4: sys0 3.7／4.4 MB・count 7.5・prelim 2.5・annot 1.9・render 6.8。v2bow1k edit end 27.4: count 1.2（11 count）・voices 3.1・prelim 1.9・render 4.2・**edit start は breaks 37 ms**（DP の頭から）。**病名（sys0）**＝`PlaceSystems` はページの最初の Y のために system 0 の silhouette を loop の*前*に読むが、そこは `BuildSystemSkylines` の直呼び（＝`EdgeStaffBeams` で edge staff の梁を quanting）で、loop 側の同じ system 0 は `ComputeSystemSkyline` の memo（Skylines 200/0/0 hit）から来ていた＝**前置の 1 回だけが memo の外**。**直し**＝前置の読みを loop と同じ key（range 0..n・first・last-if-only・indent・shortest・height）で `ComputeSystemSkyline` に通す（退化腕＝measure 0 本・body 0 は直呼びのまま＝loop はその key で memo に訊かない）。**後**: `s0` **3.10 ms／3027 KB → 0.00／0**（plain1k）・3.66／4406 → 0（fingbeam1k）・0.28 → 0（v2bow1k）・**Skylines hit は N → N+1（200 → 201・111 → 112）・miss 0 不変**（`layout-stages3-after.txt`）。網 `LayoutMemo_TheFirstSystemsSilhouette_IsReadThroughTheMemo`（6 system pinned 本の最後の system だけを edit → full と同一＋Skylines (6, 0, 1)＝直呼びなら (5, 0, 1)）。⚠️ 網の後半に「同じ内容の再打鍵で (7,0,0)」を書いて赤にした＝Δ=0 は whole-layout reuse が layout ごと飛ばすので memo は 1 度も読まれない (0,0,0)＝別の網の領分。⚠️ ベンチは一声かけずに回した（数秒の in-process 計器）＝根拠は割当 KB の消滅と hit 回数の +1。

★★ **⑷ 第 3 便（同日・同じ指示→ count loop の構造と段割りの計器が載っているので R13⒦ に着手）＝count loop の割当を半分にした。時間は DP そのもので残る**（`c151fcd9`）。**計器**（`sessions/p403/Zz403Probe3.cs.txt`＝up loop の 3 段に一時累算器・TC=0・edit end × 6 の min）: **`EstimatedSystemDetails` 0.5 ms／3.5 MB・`MinPageCount` 0.4／3.5 MB・`BreakIntoPagesScored` 5.3／5.9 MB＝うち `CalcLineHeights` 0.3／3.7 MB・`SolveUnconstrained` 5.3 ms／2.6 MB**（`count-loop.txt`・72 count）。読み＝**DP が 87%**（72 count × 約 240 行 × ページ 1 枚ぶんの後戻り≈29 万 step・18 ns/step＝LP `page-spacing.cc:298-405 calc_subproblem` の仕事そのもの＝意味論を変えずには減らない）。割当のほうは 2 綴り: count ごとに `MinPageCount` と `BreakIntoPagesScored` が**同じ details に `CalcLineHeights` を別々に当てて SystemDetails を 2 度 copy**／DP が**行ごとに `new PageSpacing`**（72 × 240＝1.7 万個）。**直し**＝up loop は `PageBreaker.CalcLineHeights` を 1 回にして `MinPageCountOfLines`／`BreakIntoPagesScoredOfLines`（元の入口は thin wrapper で残す・他の呼び手用）へ／DP は `PageSpacing` 1 個を行ごとに `Clear()`（LP は行ごとに `Page_spacing` を作り直す＝ctor の状態に戻れば同値・band は毎 prepend の前に `Resize` で再設定される）。**後（同じ区間＝Probe2 の `c.up`）**: **plain1k 6.08 ms／13,001 KB → 5.65／7,683・fingbeam1k 6.18／13,001 → 5.68／7,683・v2bow1k 0.18／571 → 0.17／356**（`count-loop-after.txt`）＝割当 −41%・時間 −0.4 ms（読みどおり小さい）。網 2 本（`SystemCountPageScoreTests`）: stacked-lines 入口 2 つ＝元の入口と同じ答え（DP の網の 4 冊＋高い 3 冊 × header 2）／`PageSpacing_ClearedIsAsGoodAsNew`＝overfull で終わった前世を `Clear()` した個体が新品と同じ Force・rod・spring を Resize→Prepend の全 step で出す。⚠️ 判断の但し書き＝この便の第 1〜第 3 便で「打鍵の固定費」を 3 つ測って、**閉じられたのは memo の外の 1 回（3 ms）と割当（5 MB）だけ**・残る大物は render 段の SVG 全文コピー（R13⒝）と edit の layout 段の DP 2 つ（line DP は edit 位置に比例・count loop の page DP は定数 5.3 ms）。

★ **⑸ 終了時**: commit 4 本（docs 2＋code 2）＋docs 1 本・**最終 full `sessions/p403/run8.trx` 8667 / 0 / 3 / 8670**（+3＝⑶ の網 1 本＋⑷ の網 2 本・run7 は ⑷ の code commit 前で同数）・§7.5（対 `567355ef`・スクリプトの出力）**Core `+` 67 行／REF 0／OWN 0**（数値定数なし・LP 由来の式なし＝memo の経路と割当の畳みだけ＝監査対象は「同じ答えか」＝⑶⑷ の網と full 同一）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。§7 3.5 は済（第401 を `-Archive 401`＝11 行 5,271 chars）。RULES §5.3 に「数秒の in-process 計器は tier-0 を含む」を汎化して置いた。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⇒ **次の一手**: ⒜ **R13⒝＝render 段（Δ=0 打鍵の最大の段・plain1k 2.3 ms／8.4 MB・fingbeam1k 7.7 ms／33.6 MB を*打鍵ごとに*割当）**＝SVG 全文の 7 回コピー（`Assemble`→JSON→…）を差分応答に／⒝ edit 打鍵の残り＝`prelim` 1.4〜2.5 ms／3.7〜9 MB・v2bow1k の `voices`（`CalculateVoiceCollisions` 3.1 ms／7.5 MB 毎打鍵・memo 無し＝R13⒧）・edit 位置が前ほど `breaks` が伸びる（v2bow1k bar 2 で 37 ms＝DP の 111 行を頭から）・count loop の page DP 5.3 ms（R13⒦・答えの memo は「ink だけの edit」でしか当たらない＝先に census で hit 率）／⒞ 診断 pass の残り＝`AnnotationName` 14／`MeasureValidator` 10／`LyricSings` 6.5（fingbeam1k・warm）／⒟ LSP 起動直後の数秒は `Compute` 等が tier-0 で 6 倍＝ReadyToRun 発行や `TieredPGO`／`TieredCompilation` の knob は packaging（ユーザーの `tools/Package-And-Install.ps1`）の側＝**要ユーザー判断**／LP 双子が要る R7〜R11／R15（要承認）。
