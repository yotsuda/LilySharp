...
> ⚠️⚠️ **CI は課金で停止中**（2026-08-21 17:47Z〜）: `b009c9ac`・`bea5ee2e`・以降の run が **4 秒 failure**＝「The job was not started because recent account payments have failed or your spending limit needs to be increased」（Billing & plans を確認・**ユーザーにしか直せない**）。**両 commit とも本便のローカル両 OS（Windows/WSL Release）5717/0/4 全緑が証拠**。復旧後: `gh run rerun 32398900747 --failed` と `gh run rerun 32399585728 --failed`（handoff 分は次の push が拾う）。
