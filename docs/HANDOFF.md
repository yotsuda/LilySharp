...
★ **⑸ 開示（*やっていない*こと）**: **⒜ 休符境界の bracket の X 枠**——**Lily# は休符の列にも `StemAttachX(up)` を足して x0 を置く**（実測: LP の bracket X 範囲は梁左端の +1.7524・Lily# は +3.0062＝**1.1738 ≈ 1.2392 − 0.065 右**）。**LP の `get_x_bound_item` は符尾の無い境界で*列*へ落ちる**（`TupletBracketEngraver` の開示 ⑸ そのもの）。**効きは dy × Δx/span＝upr で 0.040574**。**→ ✅ 第 3 便（下 ⑻）で閉じた。****⒝ skyline の梁の帯は x をアンカーに置いたまま**（`AddBeamsToSkyline`・Y は符尾で読む＝バイト同一。**LP の帯は描画範囲**＝上向きで 1.24 右。段間を動かす移植で点が無い）。**⒞ `ElementCoordinator.BeamStemX`（衝突の枠）は `BeamMemberStemX` を通していない**（invisible stem の枝が無い綴り。tremolo の全音符対でだけ違う）。**⒟ tab 枝の `beam.LeftX + StemAttachX`（`:384`）は tab 方向の attach なので残した。**
...
✅ ⚠️⚠️ ★★★★ **⑻ 第 3 便＝⑸⒜ の休符境界の X 枠を閉じた**（ユーザーの判断規則「このセッションが有利なら着手」＝**有利**: LP オラクル・双子・掃きの台・算術が全部この便の手元に在った）。**製品 1 ファイル 1 関数**（`TupletBracketEngraver.BoundEdgeOffset`）:
★★★ **⒜ LP の規則はソースで割れた**（`lily/tuplet-bracket.cc:71-85 get_x_bound_item`・`:180-189 calc_x_positions`）: **bound は「列に*見える*符尾が在り、それが bracket の向き」のときだけ符尾（その縁）**、**それ以外は列そのもののインク端**——**休符の符尾は invisible なので休符は列**。**Lily# は全 bound に「bracket 向きの符尾」を与えていた**（休符にも `StemAttachX(dir, 4, Default)` ＝上向きで +1.1742）。**逆向き符尾と全音符も同じ節**で、既定符頭ではその列の縁が旧式の attach 縁と*同じ数*（上 attach + halfStem = 1.3042 = 符頭右端・下 attach − halfStem = 0 = 左端）なので**動かないはず**——**全音符だけは旧式が黒符頭の attach を読んでいた**（1.3042 vs 全音符の 1.962）ので動く。
★★★★ **⒝ 予測を書いてから測った**（§5.0）: **upr 両端 +0.040574 で LP 六桁一致／TFB・TFR・up3・e1 不動**——**全部そのとおり**（`targeted3.trx` 836 緑・`Rest8th.Left = 0` なので下向きの TFR は動かない）。
★★★ **⒞ 掃き 920 冊 MOVED 15**（追跡 3 ＋ ユーザー 12・`base2/head2`）: **全部 tuplet の bracket と番号だけ**。**追跡 3 冊は全部 LP 双子を持つ**ので突き合わせた（`tuprest-probe.ly`／`tupdir-probe.ly`／`whole-probe.ly`＝双子に `probe-header.ly` の dump を include・`svgx.ps1` で Lily# の線を読む）: **`tuprest` 8 本・`tupdir` 9 本・全音符 1 本の bracket X 範囲が、休符境界・逆向き符尾（`r c, c'''` の右端＝符頭右）・符尾の縁（`f, c'' r` の左端＝relX+1.1742）・全音符（符頭のインク端）の全部で LP の `relX + X-positions` と F2 一致**。⚠️ **Lily# の描線は x_span の 0.2 外**（LP の stencil 範囲は x_span − 0.28 ＝ 線の中心 −0.2・半太さ 0.08＝同じ約束）。
★ **⒟ 番人**: `AnUpStemSlopedBeamOverABoundingRest_SlopesTheBracketWithIt_LpExact`（名前つき残差 → LP 六桁一致に書き換え）。**snapshot `test/tuplet-bracket-whole-notes` 1 枚再 seed**（正当化: `whole-probe.ly`＝LP の x_span が符頭のインク端 (8.4897 . 21.1679)）。
⚠️ **⒠ 残る開示**: **隠し bracket ＋ 括弧つき休符境界**（`r8[ c c]` の tuplet）の番号 x は `beam.LeftStemX`（メンバー符尾）のまま——LP は休符の列。**掃きで動いた本は無い**（その構文は第343 ⑺ が通したばかり）。

✅ ⚠️ ★★★ **⑼ 第 4 便＝⑻⒠ の「隠し bracket ＋ 括弧つき休符境界」を閉じ、⑸⒝ の帯は*数えて止めた***（同じ判断規則。**第 3 便の commit はユーザー承認「コミットは承認する」を得た**）:
★★★ **⒜ 正体は番号の x ではなく*可視性*だった**: `tuplet 3/2 { r8[ c c] }` を LP 双子（`hid-probe.ly`）に通すと **TupletBracket の extent が空＝隠れる**（梁の bound は休符の列なので tuplet と等長）。**Lily# は `HasSameBounds` が `Members` だけを見て「梁が 1 列短い」と判じ、bracket を*描いていた***。**直し**: `HasSameBounds` が `BracketBound` の休符を梁の bound に数える。番号は bound の x_span（第 3 便の `BoundEdgeOffset`）に centre するので**梁のメンバー符尾で上書きしていた 2 か所を消した**（音符境界では ∓halfStem が相殺して不動）。**実測**: 番号 centre＝休符インク左 + 3.0042（LP 3.0042）・中線の 1.5 上（LP `positions (1.5 . 1.5)`）。**掃き 920 冊 MOVED 0**（`base3/head3`）。**番人** `ABeamOpeningOnTheBoundingRest_IsEquallyLong_AndHidesTheBracket_LpExact`。
⚠️⚠️ ★★★ **⒝ 帯の x（`AddBeamsToSkyline` を `DrawnLeftX..DrawnRightX` に）は掃きで MOVED 93**（追跡 16 ＋ ユーザー 77・`head3/head4`）——**いったん戻した**。**動きは頁高や段の 0.01〜0.12 の平行移動**（`tuplets-beamed` 段 −0.11・`greensleeves` −0.01・`timesig-grandstaff` 頁 +0.01）。**台帳 803 点は帯を動かしても 1 点も動かない**（818 本緑）＝**帯の端を見る点が無かった**。

✅ ⚠️⚠️⚠️ ★★★★ **⑽ 第 5 便＝⑼⒝ の点を開いて、帯を入れた**（同じ判断規則: 93 冊の描画と差分の道具がこの便に在り、**「何が帯の端に当たるか」を今なら安く名指せた**）。**製品 1 ファイル 1 か所**（`SkylineBuilder.AddBeamsToSkyline`）:
★★★★ **⒜ probe `audit/lp-geometry/probes/beam-band-extent.ly`＝3 冊 BXS/BXW/BXO**（下段 bass `c8 ×8`＝上向き符尾の梁が面 3.05 で段間へ・上段 treble の第 2 声 `g'4`（Lily# `g`）を下向きに強制＝先端 4.0 が*第 1 列*＝梁の第 1 列の左端 + 0.065 に立つ＝**アンカーと第 1 符尾（+1.174）の間の窓**）。**予測を符号つきで先に書いた**: BXS＝LP 7.05（符尾 4.0 ＋ 五線 2.05 ＋ 1＝符尾は*線*に当たる）・旧 Lily# 8.05（帯に当たる）・新 7.05／BXW＝6.59（記号）／BXO（梁が 1 列遅れて始まる＝符尾はどちらの帯からも外）＝7.05 で旧新とも不動。
★★★★ **⒝ 全部そのとおりだった**（`ledger-bx-old2.trx`／`ledger-bx-new2.trx`）: **BXS 旧 +1.000000 → 新 0.000000・BXO 0 → 0・BXW −0.006512447（TFC/TNC と同じ記号の sliver・帯に依らず不動）**。
⚠️⚠️ ★★★ **⒞ 耳が聞こえない稿が 2 つ＋綴りの罠 1 つ**（probe の header に記録）: **⑴ 中線 `b'4` の下向き符尾は 3.333（中線の短縮）で ト音記号 3.54 に負け、3 冊とも 6.59**（TFB の初稿と同じ）／**⑵ 第 2 声の残りを休符で埋めると `\voiceTwo` が休符を下へ寄せて*両エンジンで*梁の帯に当たる（7.33 ＞ 符尾 7.05）＝spacer `s` で埋める**／**⑶ Lily# 側を `g,`（G3）と綴った**（memory の「Lily# の絶対 `c` は LP の `c'`」）——**BXS と BXO が*同じ* +3.0 を読んだので対照が捕まえた**（帯は 2 冊を同じには動かせない）。
★ **⒟ 数**: **台帳 803 → 806（exact 632 → 634・ss 非ゼロ 206 → 207・総和 25.6781681623963 → 25.6846806093963＝BXW の sliver）**・掃きは ⑼⒝ の 93 冊そのまま（同じ変更）・**snapshot 6 枚再 seed**（`test/beamlet-peaks`・`scripts-stem-support`・`tab-beam-slope`・`timesig-grandstaff`・`tuplets-beamed`・`voice-dynamics-offbeat`。正当化: `staff.staff.beam-band-left-end`）・**full 7210 / 0 / 4 / 7214**（`scratch/p346/full5.trx`＝7202 緑＋snapshot 6・生成物 2 の赤 → 再生成して 1082 本再実行で緑・**+3＝台帳点**）。

⇒ ★★★★ **⑾ 次の一手**: **⒜ push は禁止のまま**（未 push は終了時の行）。**第 4 便・第 5 便の commit は未承認**（第 5 便＝93 冊が 0.01〜0.12 動く）。**⒝ 第343 ⑸ の休符 encompass 点（else 枝）**——LP オラクルを新たに建てる仕事（`@rest` で押し下げた休符）。**⒞ 第342 の並び**（拾い箱の ⑺⒜／`lysc ly` の ChordNames／lyric row の slot の投票／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／A/B/D/E／C⑴／G）。**⒠ 以下は第342 の並び**（拾い箱の ⑺⒜／`lysc ly` の ChordNames／lyric row の slot の投票／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／A/B/D/E／C⑴／G）。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
...
**終了時**: **commit 9 本**（`a7e1f816`＝製品・検査・台帳・snapshot・生成物・CHANGELOG／`14262479`＝この HANDOFF＝第342 の経緯を ARCHIVE の先頭へ逐語／`584c0bfe`＝承認の 1 行／`6ffe3421`＝第 3 便＝休符境界の X 枠＋番人＋snapshot 1 枚＋生成物＋CHANGELOG／`94b72999`＝その HANDOFF／`e8acf417`＝第 4 便＝隠し bracket の等長判定＋番人＋生成物＋CHANGELOG／`42e77625`＝その HANDOFF／第 5 便＝帯の x ＋ 台帳 3 点＋snapshot 6 枚＋生成物＋CHANGELOG／この HANDOFF）・**未 push 22**（第343 までの 13 を含む・**第 1 便と第 3 便は ✅ ユーザー承認済み・第 4 便と第 5 便は未承認・push は指示により保留**）・木 clean・未追跡 0・**full 7210 / 0 / 4 / 7214**（`scratch/p346/full5.trx`・**+3＝第 5 便の台帳点**）・`--no-incremental` build 0 エラー／Core 0 警告・**台帳 806／exact 634／ss 非ゼロ 207（25.6846806093963）／count 178 うち非ゼロ 0／OPEN 0・snapshot 247・追跡 597**・木 clean・未追跡 0・**台帳 803／exact 632／ss 非ゼロ 206（25.6781681623963）／count 178 うち非ゼロ 0／OPEN 0・snapshot 247・追跡 597**。⚠️ **Co-Authored-By は付けていない**（2026-07-06 のオーナー決定・memory）。
...
**⒜ `SvgGenerator` は座標を `F2` で刷る**＝**0.01 のグリッド**。`RecordingDocumentContext` の頭注が**まさにこのために自分は在ると書いている**（「LP-fidelity measurements are not taken from SVG text … quantises to 0.01」）のに、2 本が SVG の字を読んでいた。
- **`TabHalfNoteTremolo_CentresOnTheDoubleStem`**: 斜線の立ち上がりを `Assert.Equal(0.375, y1-y2, 2)`。**F2 の 2 数の差は必ず 0.01 の倍数なので 0.375 には決してならない**（0.37 か 0.38 で、どちらになるかは**譜が頁のどこに座ったかだけ**で決まる）。**素の値は 0.375000000000・幅 1.500000000000 ちょうど**——**毒の前後で 1 ビットも動いていない。**
- **`TheSameBlockOnTwoSystems_SitsTheSameDistanceBelowItsOwnStaff`**: 2 段の差を 6 桁で。**素の値は 5.864959964924 対 5.864959965167＝差 2.428e-10 で、これも毒の前後で*同一***。F2 で読むから 3.87 対 3.86 になっていた。
**⒝ 3 本目は*読み手自身*が捨てた桁だった**: `RenderedGeometry.StaffLineYs` が **`GroupBy(Math.Round(y, 9))` の*キー*をそのまま答えとして返していた**ので、譜線から取る全ての読みに読み手の量子化が乗る。**`ATabSlurTakesItsDirectionFromTheString_AndTheTwoAreMirrorImages` の 9 桁の鏡像主張は、それだけで出来ていた**——**弓は厳密に鏡像**（素の y 10.588414262266500 / 14.704414262266500 の中点 12.646414262266500）で、**`g.Key` が返した 12.646414262 との 2.665e-10 が報告された非対称のすべて**（÷1.5 で 1.777e-10、両側で 3.553e-10＝観測値ちょうど）。
⇒ ★★★★ **直しは 3 つとも検査側**（`RenderedGeometry` は `g.Min(l => l.Y1)` を返す＝**キーは grouping のためであって答えではない**。2 か所とも。`RoundedKey` に理由を書いた）。**製品は 1 行も触っていない。** ⚠️ **どれも*緩めて*緑にしたのではない**——tremolo は 2 桁 → **9 桁**、歌詞は F2 の 6 桁 → **素の 6 桁**で、**前より厳しい**。

★★★ **⑵ commit 2 本**（**✅ ユーザー指示「未コミット分を先にコミットして」**）: `6167721c`＝第342 の印の箱（製品 3・検査 4 ＋上の ⑴ の 3 本・snapshot 234・台帳 8 点再 seed・生成物 2・CHANGELOG）／`88273fa3`＝下 ⑶。⚠️ **生成物は commit ごとに*その状態で*建て直した**（`LILYSHARP_UPDATE_DOCS=1`）——片方の commit にもう片方のコードを説明する行が入らないように。

⚠️⚠️ ★★★★ **⑶ 報告の欠陥＝「梁を追うか否か」の判定が LP と違っていた。1 述語の直し**（`88273fa3`）:
**⒜ 正体**: tuplet の bracket は**梁を追う**か**追わない**かで置き方が丸ごと変わる（追えば符尾先端＋padding で*傾く*／追わなければ **`if (!follow_beam)` で譜の縁が encompass に加わり**〔`lily/tuplet-bracket.cc:633-637`〕**平坦**になる）。**Lily# は「tuplet の全*音符*を覆う梁が在るか」で決めていた（休符は透明）。LP は `scm/output-lib.scm:3945-3968 ly:tuplet-bracket::calc-potential-beam` で*外側 2 列だけ*を見て、両方に符尾が要る**——**休符の列に符尾は無い**。
**⒝ LP の実測**（`scratch/p345/m4-probe.ly`＝報告の本の第 4 小節の双子。grob dump）: **`beam=#f`**・**`positions=(-3.4 . -3.4)`**・`edge-height=(0.7 . 0.7)`・**Rest のインク Y `(-2.05 . 0.82)`**。**譜のインク 2.05 ＋ `staff-padding` 0.25 ＋ `padding` 1.1 ＝ 3.40 ちょうど。**
**⒞ Lily# は −1.382893 〜 −1.913535 に*傾けて*引いていた**＝**インク −2.05 の 16 分休符を貫く**（`scratch/p345/tuplet-head.txt`）。**直して −3.400000 平坦・フックは 0.7 上がって −2.700000**＝**LP と六桁一致**（`tuplet-fix.txt`）。
★★ **⒟ 移植は 1 述語で足りた**: **Lily# では梁の member は符尾**——**休符は「rides the beam as an INVISIBLE stem: no member」で `BeamRestStem` に居る**（`BeamDetector.cs` にそう書いてある）ので、**「外側 2 列がこの梁の member か」だけで LP の符尾テストと梁同一性テストの両方に答えている**。⚠️ **最初 `Stemmed()` の門を別に足したが、毒を当てたら*効かなかった*＝到達不能だった**ので落とした（**その事実自体を注記に書いた**）。**LP の残りの節（`tupletFullNote` の paper-column 門）は Lily# に文法が無いので*書かずに開示*。**
**⒠ 掃き 920 冊**（追跡 597 ＋ ユーザーの `ベースタブLy` 323。base/head の SVG バイト比較・`scratch/p345/sweep345.ps1`）: **MOVED 11・全部ユーザーの実コーパス**（**追跡側は 0＝だから snapshot も台帳も 1 つも動かない**）。**11 冊とも「tuplet の端が休符」の本。9 冊は差分が bracket の線と数字だけ**（`whatmoved.ps1`）・`rest-tuplet.lys` は頁が伸びた分・**`Baby It's Cold Outside.lys` は段が高くなって下の段が動いた**（**頁数・要素数・頁高は不変**＝**消えても増えてもいない**）。**bracket の本数はどの本でも不変**＝**可視性は動いていない**（`brackets.ps1`）。
**⒡ 番人 3 本 `TupletBracketRestBoundTests`**: **LP の数**／**帰結**（bracket のインクが休符のインクより下。**両方とも描いたものから読む**）／**控え**（休符を音符に替えると同じ梁が tuplet と同境界になり **LP は bracket を描かない**＝`scratch/p345/m4ctl.ly` で `extent (+inf.0 . -inf.0)` を裏取り済み）。**毒**: 旧述語に戻すと **2 本が赤**——`-1.382892722` と**「the two overlap, which is the reported defect」**が出る。

★ **⑷ 数**: full **7192 / 0 / 4 / 7196**（`scratch/p345/full3.trx`・2 分 41 秒・**+3＝番人**。引継ぎ 7193 ＋ 3）・`--no-incremental` build 0 エラー／**Core 0 警告**・**台帳 800／exact 632／ss 非ゼロ 203（25.6716136193963）／count 178 うち非ゼロ 0**・**snapshot 247**・**追跡 `.lys` 597**。⚠️ **台帳の総和が引継ぎの 26.7886591883963 から 25.6716136193963 に減っているのは第342 の印の箱**（10 点良化・悪化 0）。

⚠️ ★★ **⑸ *やっていない*こと（開示）**: **LP の `:554-562` は `columns` を*生で*歩き、休符の列のインクも encompass 点に積む**。**Lily# はいまも音符／和音の列しか積まない**（`TupletBracketEngraver` の `pos == null` の門）。**この本では効かない**——休符の底 −2.05 は譜の縁 −2.30 に負けるので、点になっても答えは変わらない——**が、`@rest` で押し下げた休符や譜の外へ出る休符では効きうる**。⇒ **次に tuplet を触る人の候補。台帳点が無いので、入れるなら測ってから。**

⚠️⚠️⚠️ ★★★★ **⑹ 第 2 便＝ユーザーの問い「休符を member にするように修正すべきでは？」で、⑶ で書いた*理由*が誤りだと分かった**（`6ef71e09`）。**答えは合っていたが、根拠が嘘だった**:
**⒜ 「休符の列に符尾は無い」は LP について偽**（`scratch/p345/beamrest.ly`・4 冊）。**LP は*すべての* NoteColumn に符尾を与え**（`stem=YES rest=YES`）、**梁の中の休符はその `stems` の一員**（`STEMLESS`＝頭の無い符尾）。**梁は休符から始まることもできる。**
**⒝ 報告の本で `beam=#f` になる本当の理由は「その休符の符尾が*梁を持っていない*」**（梁は次の音符から始まる）。**跨がれていれば LP は梁を追う**——`c,16[ tuplet 3/4 { r16 c a, ] }` で **`beam=<Beam>`・`positions=(-4.315073 . -3.271576)`＝傾き**（`e1-probe.ly`）。**⑶ の述語はそこで false を返していた**ので、**`Members` だけでなく `RestStems` も見る**形に直した（掃き 920 冊 **MOVED 0**＝当時その構文は書けなかった）。
★★ **⒞ ただし「休符を `Members` にする」は*採らなかった***: **LP 自身が `Stem::is_normal_stem`（`beam-quanting.cc:299`）で休符を*stem 採点から外す***（実測: 境界が休符の梁と、そこを音符にした梁は **positions が同一 `(0.0 . 0.0)`**）。**LP は「1 本のリスト＋使う場所ごとの門」、Lily# は「2 本のリスト」で、後者は門が構造になっている。** `Members` に移すと quanter・描画・向きの投票の全部に門を書き戻す必要がある。

✅ ⚠️⚠️ ★★★★ **⑺ 第 3 便＝ユーザー決定「LP が受け付けるなら Lily# も受け付けるように修正すべきだ」＝`r16[` を通した**（`256cc217`。**文法はユーザーの決定事項**なので指示を受けてから着手）:
**⒜ LP の実測**（`scratch/p345/beambound.ly`・3 冊）: **見えない符尾は*休符インクの中心*に立ち、梁はその半符尾（0.065）だけ外へ伸びる**——`r8[ c c c]` 中心 9.085 → 梁左 **9.020**／`c8[ c c r]` 中心 16.5976 → 梁右 **16.6626**。**傾いた本**（`beamslope.ly` ＋ `lysc ly` 双子 `s1-probe.ly`）: LP `x 9.0200..16.4234`・`positions (-2.81 . -1.19)`・ink `(-3.05 . -0.95)` ⇒ **Lily# は `x 9.02..16.42`・ink `(-3.05 . -0.95)`・中心線 −2.81/−1.19＝描画精度で一致。**
**⒝ 層は 7 つ**: `RestItem` に `HasBeamStart/End`（walk の休符アーム 2 か所が渡す。**slur の 2 つと同じ穴・同じ直し**）／`BeamPairingScanner`・`DetectManualBeamGroups`・`DetectCrossMeasureManualBeams` が休符の括弧を読む（**memo の hash にも足した**）／「休符は内側だけ」の門を外す／`RestStemsStandingIn` が**書き手が括弧を置いた**端の休符を残す（`BracketBound`。**漂着した休符は従来どおり捨てる**）／clip-edges を外側の**列**へ／**quanter の span を梁自身の extent へ**。
⚠️⚠️ ★★★ **⒞ 描画の内挿枠は*動かしてはいけない***: `PrimaryBeamYAt` の枠は外側**メンバー**の符尾で、そこに入れる Y は `BeamScoringProblem.AtOuterStems` が**その符尾で**返した値。**枠だけ広げると梁全体が傾く**——実測で**左端が LP より 0.39 上・右端は 0.01 で一致**という形で出た。**描画の伸びは segment 側**（休符の x を食わせてあるので `CalcBeamSegments` が外挿する）。
**⒟ `MeasureCollector.Stems` が落ちた**: pure な rest-push が `Members[BeforeMember - 1]` を素で引いていた。**LP は明示的に扱っている**（`beam.cc:1461-1464`＝端では `left = right = 隣の 1 本`）。そのとおりに直した。
**⒠ 掃き 920 冊 MOVED 0**（**この構文は今まで書けなかった**）。**番人 4 本 `BeamOnRestBoundTests`**（括弧が通る／開く側・閉じる側の梁が休符に届く〔**両方とも描いたグリフから読む**〕／**控え**＝休符を音符にすると梁は外側の符尾で終わる）。**毒**: 端の休符をまた捨てると**届く 2 本が赤**。**文書も直した**（`SYNTAX_REFERENCE.md`／`GRAMMAR_FOR_LLM.md`）。

★ **⑻ 数（最終）**: full **7201 / 0 / 4 / 7205**（`scratch/p345/full11.trx`・**+4＝⑺・+2＝⑼・+3＝⑽ の番人／点**）・build 0 エラー／Core 0 警告・**snapshot 247 枚**（**⑼ で 4 枚を再 seed**）・**台帳 803／exact 632／ss 非ゼロ 206（25.6925058903963）／count 178 うち非ゼロ 0**。

✅ ⚠️⚠️⚠️ ★★★★ **⑼ 第 4 便＝⑻⒞ に着手した**（`d575a7df`。ユーザーの判断規則「このセッションが有利なら着手」＝**有利**: この差は ⑺ で `r16[` を通して*初めて観測可能になった*もので、LP oracle も掃きの台も全部この便が握っている）。**1 つ直しに行って 3 つ出た。全部 follow-beam 枝の中**:
★★★★ **⒜ LP の 2 本の腕は「フラグ付きの 1 本の腕」ではない**。**follow 枝**（`tuplet-bracket.cc:495-519`）は**外側 2 *列*の符尾先端を取り・その差を dy にし・その 2 点だけを積む**。**譜との合併も・音符の符号ゲートも・damping も・列ごとの点も、全部 else 枝**（`:520-631`）で、`:633-637` の譜の縁も `if (!follow_beam)`。**Lily# は else 枝の検査を follow 枝の答えにも掛けていた**——**梁が上がり音符が下がる本で `sign(graphicalDy) != musDown` が dy を 0 に潰す**。⇒ **`e1.lys` は LP `positions=(-4.315073 . -3.271576)`（rise 1.043497）に対し*平坦* −3.730 だった。直して rise **1.043497＝六桁一致**。**
★★★ **⒝ follow 枝の点は外側*列***: **境界の休符は見えない符尾を持つ**（インク中心に立ち梁の面で終わる＝`beamslope.ly` 実測）。**そして*音符*列の先端も、列の頭の錨ではなく*描いた符尾の x* で読む**——**この継ぎ目はこのファイルが 2026-08-09 から「傾いた partial beam でだけ表面化する。コーパスに測る本が無い」と開示していたもの**で、**`r16[` がその regime を書けるようにした。実測 0.0133。** ⚠️ **面を符尾 x で読み直すのではなく*傾き*で補正した**——`BeamLayout.LeftX/RightX` は列の錨・`LeftY/RightY` は符尾での答えなので**錨の外は clamp される**が、**傾きだけは両枠で厳密に同じ**。
★★★★ **⒞ そして `FindCoveringBeam`／`MemberBeam` の*別の譜への fallback* が出た**: **LP の `par_beam` は tuplet 自身の列の符尾の梁**（`output-lib.scm:3955-3967`）で、**符尾の梁はその譜の事実**。**GrandStaff の右手 tuplet が左手の梁を追っていた**——`multistaff-tuplet-beams.lys`（**上段は梁の無い 4 分 3 つ**）で **LP は平坦 `positions -3.4`・Lily# は −1.500**。**両方を「自分の譜だけ」に絞って −3.400＝LP と一致。** ⚠️ **fallback の口上（staff+tab の対）はその対では*同じ譜の梁が必ず在る*ので、一度も役に立っていなかった**（掃きでも tab 本は 1 冊も動かない）。
**⒟ 掃き 920 冊 MOVED 18**——**全部「梁の上に tuplet bracket が在る本」**。譜の narrowing の寄与は `FindCoveringBeam` 3 冊・`MemberBeam` 2 冊。**追跡は 4 冊で snapshot を再 seed**。**番人 +2**（`ABeamOverTheBoundingRest_SlopesTheBracketWithIt_LpExact`＝rise 1.043497／`ATupletDoesNotFollowAnotherStaffsBeam_LpExact`＝平坦 −3.4）。
⚠️⚠️ ★★★★ **⒠ 検証したのは 18 冊のうち*2 冊*（上の 2 つ。誤差 0.585・1.93 → どちらも一致）。残り 16 冊は個別に LP と突き合わせていない**——**この regime を測る台帳点が 1 つも無いから漂えた**のであって、**正直な次の一手は「傾いた follow-beam bracket の pinned pair を開くこと」**（HANDOFF 5.0「ports の前に points」。**このファイル自身が 2026-08-09 にそう書いている**）。

✅ ⚠️⚠️ ★★★★ **⑽ 第 5 便＝⑼⒠ の pinned pair を開いた**（`dd6a4720`。判断規則＝**有利**: 測る regime も LP の実数も掃きの台も全部この便が握っていて、**`scratch/p345` は git 管理外だから次便には双子が残っていない**）。**製品は 1 行も触っていない・本も 1 冊も動かない**:
★★★ **⒜ probe 3 冊**（`audit/lp-geometry/probes/tuplet-bracket-follow-beam.ly`。tuplet の譜を**上段**に置き、bracket を下へ垂らして譜間を binding にする＝`tuplet-bracket-encompass.ly` の配置）: **TFB** `b'2 e''8[ \tuplet 3/2 { d''8 c'' b' ] } b'8`＝**7.527419740671665**／**TFR**（先頭の列が梁の跨ぐ休符）＝**7.611907881566059**／**TFC**（同じ 4 音・tuplet 無し＝falsifier）＝**6.590000**。**LP は 2 冊とも `beam=SET`・positions は傾き**（`(-3.549603 . -4.149803)`／`(-3.719148 . -4.149235)`）＝**TBSD/TBSA が測る ±0.2 の平坦化がここには無い**——**この本が「別の腕」を測っている証拠**。
⚠️⚠️ ★★★ **⒝ 最初の草案は*耳が聞こえて*おらず、自分で書いた falsifier がそう言った**: `tuplet-bracket-sloped.ly` を鏡にして tuplet を下段・bracket を上に置いたが、**follow-beam の bracket は梁から padding 1 つしか離れない**（梁の無い符尾先端よりずっと譜に近い）ので **2.94 しか届かず、その譜自身のト音記号の 4.13 に負けて 3 冊とも 6.826 を刷った**。⇒ **下向きに垂らして解決**。**経緯は probe の header に残した**。⇒ ★★ **「床を binding にする」設計は、腕が変われば作り直し**——先行 probe の配置をそのまま流用してはいけない。
