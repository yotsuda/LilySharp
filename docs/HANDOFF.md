...
最終更新 第194セッション＝**§2F の「伝聞」を測ったら、双子と MusicXML と MIDI が
それぞれ別の曲を出していた便**（**実装 3 便**。**perf は 1 バイトも触っていない**——
**この便は全部 correctness で、全部 0.3.0 の門の内側**）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **第192・第193 が 2 便続けて ★★★ で
「§2F は 1 つも触っていない＝『0.3.0 の前でなければ不可能』な項目は 0 のまま」と書いていた。**
**perf の優先度を上げたのはユーザー決定だが、0.3.0 を出す優先は「文法を完全にしてから」**なので、
**この便は §2 の頭ではなく §2F に入った。ユーザー決定 0 件。**
★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 516 点・ss 非ゼロ 98・
総和 3.609965521・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 11**・
**snapshot 217 枚**・suite **5263 passed / 0 failed / 4 skipped**
＝**引継ぎの数は全部合っていた**（**24 回連続**）。
★★ **そして未 push が初めて両端で繋がった**——**第193 が終了時の 11 を書いたので、
開始時の 11 と引き算できた**（＝間で push は無い）。**この形を続けること。**
 終了時 **未追跡 0・作業ツリー項目 0・未 push 16**（★ **セッション最後の commit まで含む数**）・
⚠️ ★★ **ここは 14 → 15 → 16 と 2 度直した**。**14 は引継ぎ commit を*作る前*に数えた数**、
**15 はそのあとの訂正 commit を勘定していない数**。
⇒ ★★★ **「この commit を含む」と書く数は、*セッションの最後の* commit のあとに数え直す。**
**訂正 commit も 1 本は 1 本**（§0 ★ の当日版・**第192 の `handoff: ahead count` と同じ形**）。
suite **5286 passed / 0 failed / 4 skipped**（+23＝新しい網ちょうど）・
**台帳 516 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・
**snapshot 217 枚で不動＝再ベース 0 枚**・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 2 本使った**（`Cumin` が途中で閉じ、`Paprika` が継いだ）。
⚠️ **計器は Core に 1 行も置いていない**（`Select-String AllocProbe` は **0 件**）。
**測定は全部リポジトリ外の道具**——`scratch\p194\Sweep-Twins.ps1`（566 冊の双子 A/B）と
`audit/LilySharp.Probe -- sweep/cmp`（566 冊の SVG A/B）と LP 2.26.0 の grob dump。

> ## ★★★ この便の骨＝**「双子が同じ曲か」を誰も測っていなかった。3 冊とも別の曲だった**
> 忠実度作業はすべて `lysc ly` の双子を通して測る。**その双子がページと同じ曲であることは、
> どの網も主張していなかった**——**そして 3 つの綴りで違っていた**:
> **⑴ `repeat unfold 13 { ground }` が `\repeat unfold 13 {  }`**（ページは 53 小節・双子は 1 小節）／
> **⑵ `transpose d` を書いた本の双子が*書かれた調*のまま**（ページは D major・双子は C major）／
> **⑶ `a4@rest` が MusicXML では `<pitch>`・MIDI では鳴る音**（ページは休符）。
> ⇒ ★★★ **どれも「黙って」ではない——*正しく見える出力を出していた*。**
> **⑴ は LP が警告 0 で `Success` と言い、⑵ は調号を 1 つも出さず、⑶ は音符として妥当だった。**
> ⇒ ★★★ **判定法**: **出力どうしを突き合わせる網を、出力ごとの網とは別に持つ。**
> **`PitchedRestOutputsTests` は 1 冊を 4 出力すべてに訊く 1 本の網**で、
> **それが「2 出力は正しかった」ことを*見えなくしていた*理由そのもの**（片側だけ見れば緑）。
> ⚠️ **⑵ と ⑶ は「双子」と「ページ」だけでなく「MusicXML」「MIDI」も食い違う**——
> **4 出力の一致は、この repo でまだ 1 度も体系的に測られていない**（下の ⑺）。

> ## ★★★ 2 つ目の骨＝**射程の引き算は、閉じた後にもう一度やる。「0/300 冊」は 566 冊では 1 冊だった**
> ⑴ の穴は `CarryFrameInto` の注記に **「measured 2026-08-16 — 0 of 300 books write one」**
> と書いてあり、**§2F は「falsifier になる本が無いので §5.4 の順序どおり直さず起票」**と結んでいた。
> ⇒ **木は 566 冊。1 冊居た**——**`samples/canon-in-d.lys`**、しかも
> **その本のヘッダが「the 4-bar ground is written ONCE ... and cycled 13 times」と
> この機能を宣伝している**。**居たのは第186 が射程を広げるまで誰も掃いていなかった `samples`。**
> ⇒ ★★★ **第186 の骨（「前より広い」と「全部」は違う数）が*1 セッション後に自分で*再発した**——
> **あちらは網の射程、こちらは*注記に書いた測定の*射程。同じ引き算で出る。**
> ⇒ ★★ **だから網は「綴り」ではなく*不変条件*で書いた**——
> `TheNotDeclaredWarning_NeverNamesAPhraseTheBookDeclares` は
> **566 冊すべてに「この警告は宣言済みの phrase を名指さない」を当てる**ので、
> **7 番目の入れ子 site が生えた日にその日から効く。**

> ## ★★ 3 つ目の骨＝**「決定が要る」と引き継がれた項が、LP に 1 コマンド訊いたら決定ではなかった**
> §2F の transpose の項は **「綴りの設計判断が要る: `\transpose c <target>` で包むか、
> 鳴る音高を直接書くか」**と書いていた。**訊いた**: 双子を `\transpose c d` で包むと
> **LP 2.26.0 はページが解決する 10 音をそのまま読み**（D5 E5 F#5 G5 A5 B5 C#6 D7 D#8 E9）、
> **KeySignature の `alteration-alist` が `()` → `((0 . 1/2) (3 . 1/2))`＝C major → D major**。
> ⇒ ★★★ **第188 の骨（「規則は決定ではなく LP のものだった」）の 2 例目**で、
> **fixture 自身が「Verified against LilyPond \transpose c d」と書いていた**
> ——**散文は主張なので測ったが、測ったら当たっていた。**
> ⇒ ★ **`\drummode` に巻いても LP は SVG バイト同一**（測定）＝**drum の特例は要らない。**

> ## ★★ 4 つ目の骨＝**起票の述語が「小さい半分」を名指していると、それに合わせた修理が出てくる**
> §2F は ⑶ を **「MusicXML が*高さ*を落とす——音高付き休符は `<rest/>` になる（音価は正しい）」**
> と書いていた。**測ると `<rest/>` ではなく `<note><pitch>A4</pitch>`**——
> **休符が音符になっていた**。**MIDI も鳴らしていた**（noteOn 3 対 対照 1）。
> ⇒ ★★★ **起票を読んで直しに行くと、`<rest>` に `display-step` を足す仕事**になる。
> **その要素は出ていないので、直しは 1 行も効かない。**
> ⇒ ★★ **そして正しさは*コードが自分で*書いていた**——`CreatePitchedRestItem` の注記が
...
| 便 | 何が動いたか | 全数 A/B |
|---|---|---|
| ⑴ 入れ子の phrase 参照 | 双子 **1/566 冊**（`samples/canon-in-d.lys`） | SVG 不変（経路外） |
| ⑵ transpose | 双子 **4/566 冊**（transpose を書く 4 冊ちょうど） | SVG 不変（経路外） |
| ⑶ 音高付き休符 | MusicXML・MIDI（掃きは無い） | **SVG 0/566・双子 0/566**（陽性対照＝毒で **3/566**） |

- **⑴ ★★★ 第1便＝入れ子の中の phrase 参照が空の本体になっていた**
  （`e38770ea`・**Core 1 ファイル＋網 6 本**）。**入れ子の exporter は 6 site**
  （phrase 参照・tuplet・voice span・grace・cue・repeat）で、**`_phrases` を持っていたのは 2 site だけ**
  ——**残り 4 つは空の辞書に解決して何も出さず、「referenced but not declared」と警告していた**
  （**宣言は 3 行上に在る**）。⇒ **規則は `CarryFrameInto` 1 軒**（2 site の private な複製は削除）。
  ⚠️ **`_activePhrases` は共有**（複製すると `phrase A { tuplet 3/2 { A } }` が無限に展開する）。
  ★ **LP が音単位で裏を取った**——**NoteHead dump が 380 → 484＝+104 ＝ 13 × 8**、
  **内訳 D3 +26・A2 +26・G2 +26・B2 +13・F#2 +13 ＝ ground の 8 音 × 13**。
  ⚠️ ★★ **計器が 1 度嘘をついた**: **LP は `Drawing systems...` を改行なしで書く**ので
  **`^NH ` のアンカーは 1 ファイルにつき 1 行落とす**（379/483 と出た）。**差は正しく、総和が違った。**
- **⑵ ★★★ 第2便＝transpose を書いた本の双子が「書かれた調」のままだった**
  （`c24a2334`・**Core 1 ファイル＋網 12 case**）。**part 変数を `\transpose c <target>` で包む**
  ——**`\relative`/`\fixed` の*外側***（LP は書かれた音高の相対を解いてから移す＝collector と同じ順）。
  ★ **3 綴りとも家に訊いた**（`PartTranspose.Read` ／ `ReadProperty` ／ `PitchTransposer.Compose`）
  ＝**この repo は「3 綴りが食い違う」欠陥を第182 で 1 度出している。**
  ⚠️ **「fixture 3 冊」は 4 冊**（`_keySharps` の注記は「1 冊」と書いていた）。
  ⚠️ **per-score 綴りはツリーに 0 冊**なので**網に書き出した**（§5.4）。
- **⑶ ★★★ 第3便＝`a4@rest` が MusicXML では音符・MIDI では鳴る音だった**
  （`733305a8`・**Core 5 ファイル＋新規 1 軒＋網 3 本**）。**両 exporter は collector の item ではなく
  *構文*を歩く**ので**それぞれ綴りの読み手が要る**——**どちらも持っていなかった。**
  ⇒ **`Semantics.PitchedRest` 1 軒**（collector と双子もそこを読む）。
  ★ **MusicXML は `<rest><display-step>/<display-octave></rest>`**、**MIDI は無音のまま時間を取る**
  （**対照との等式で網にした**＝「何も出さない」と「何も出さず拍を飲む」は本数では区別できない）。
  ⚠️ **双子の読み手は*厳しく*なった**（`ArticulationType.None` を要求＝collector と同じ）。
  **566 冊の双子はバイト同一**で測ってある。
- **⑷ ★ §7.5 の監査**＝**3 便合計で Core +286 行（うち*コード 110*）・削除 −32・
  REF +2/−0・OWN +0/−0**（⚠️ **`git --no-pager diff --no-color 58c1c922 HEAD -- LilySharp.Core`
  で数えた**＝**セッションの頭から**）。**彫版の定数は 0 個。**
  **分類は ⑴⑵ が ⒟**（既存の家を指し直した／既存の 3 読み手に訊いた）・**⑶ は ⒝**
  （2 exporter が「実装していなかった LP の規則」を導出した）。
  **§7.7 の匂い**——**3 便とも消しているのは「同じ量の 2 つ目の綴り」**で、
  ⑶ はその 1 段手前＝**読み手が*存在しない*ので grep しても不一致として出ない**（§5.2.1⑤ の型）。
- **⑸ ★★ 毒は 3 便とも当てた**（§5.4）:
  ⑴ **毒＝直す前のビルドで新しい網 5/6 本が赤**（真の警告を主張する 1 本＝対照は緑のまま）。
  ⑵ **毒＝`TransposeTarget()` を null 固定で 12 case 中 11 本が赤**（transpose を書かない対照は緑）
  ——⚠️ ★★★ **既存の網は 1 本も赤くならない＝この便まで双子の transpose には観測者が 0 だった。**
  ⑶ **毒 2 種**: `PitchedRest.Is` を殺すと **6 本赤**（新 3 ＋ ページ側の既存 3）、
  `IsMarker` を殺すと **7 本赤**（双子の網が加わる）＝**4 出力すべてが観測下に在ることの示し方。**
- **⑹ ★★ §2 の頭（perf）は 1 バイトも触っていない。**
  **地図と次の島は第193 の §1 ⑷ のまま**（ARCHIVE ではなく §2 の頭に要約が在る）。
- **⑺ ⚠️⚠️ ★★★ 残した宿題＝「4 出力の一致」を測る計器がまだ無い**（**この便が 3 例出した族**）。
  **今在るのは出力ごとの網**で、**出力どうしを突き合わせるのは `PitchedRestOutputsTests` 1 本だけ**
  （しかも 1 冊）。⇒ **次に触るなら「1 冊を 4 出力に訊いて音符の多重集合を比べる」道具**——
  **ページは `Semantics.ResolvedPitches.ForFile`、MIDI は `MidiExporter`、MusicXML は
  `MusicXmlExporter`、双子は LP に通して NoteHead を dump**（この便の
  `scratch\p194\Compare-Pitches.ps1` がその 2 出力版）。
  ⚠️ **`check --pitches` には既知の射程の穴が在る**（どの score も描かない part は報告に出ない・
  第182）。**道具を作る前にその引き算をすること。**
