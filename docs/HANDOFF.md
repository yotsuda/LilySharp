...
終了時: 本便 8 本（`f83a255c` 番人一式＋workflow／`67d87e1a` 番人の射程訂正／`669b82f0` handoff＋分類器の訂正／`d777c9a1` §7 3.5 の番人／`75093b53` handoff＋床の更新／`446e3810` 断った triage ⑹／`da583e35` README の残債欄⑥／この行）＝**ユーザーが本便の途中で `75093b53` まで push 済み・未 push は 3 本**。⚠️⚠️ ★★★ **タグは origin でまだ旧い**（`git ls-remote --tags origin v0.3.0` が第1パスの tag object を返す・実測）——**`git push --force origin refs/tags/v0.3.0` が未実行**なので、**②の修復は origin では完了していない**。**clone し直すと番人がまた赤になる**（それが正しい振る舞い）。⚠️ **その旧 tag object の SHA はここに書かない**——**本便が prune で消した物**なので、綴った瞬間に dead 引用が 1 つ増える（**国勢調査が実際に赤くしてそれを教えた**＝⑤ と同じ形の 2 例目だが、こちらは**散文のほうが間違っている**側）・未追跡 0/木 0・suite **Windows 5806/0/4・WSL 5806/0/4＝両 OS 完全緑（開始比 +6）**・**引用 live 510／dead 469＝両機械一致**・snapshot **222 枚不動**・台帳 **566 点・ss 非ゼロ 110／総和 3.876038461・count 107／非ゼロ 2＝完全不動**・追跡コーパス **572 冊（1 冊も触っていない）**・Core 0 警告。⚠️ **加えて ref が 2 つ動いている**（`v0.3.0` の張り直し／WSL の `win/backup/*` 削除）——**commit ではないので `git status` にも `rev-list` にも出ない。**
...
| ⑤ ★★★ **§7 手順 3.5 の番人**（続き便＝同じ委任→着手） | 新テスト 3 本（経緯ブロックが 1 本／**継ぎ目**／穴 9 個の天井） | 同上・**文書も engine も 0 接触**・**規則にしか無かった手順が初めて機械の下に入った** |
| ⑥ **README の残債欄をコードと突き合わせた**（ユーザー依頼） | `README.md` の `### Planned` 3 → 2 件・Implemented の但し書き 1 行を訂正 | **文書のみ**・**3 件目の stale 棚を閉じた**（次項に 4 件目を起票） |
...
- **⑸ ★★★ 次に触るなら＝残債**（第235 の一覧を引き継ぎ、閉じたものを落とした）: 言語仕様の宿題は §1.2 リネーム（ユーザーが MSVS で）だけ／**名指し穴**＝⒤ exporter の paper 未輸出（**需要待ち**＝`LilyPondExporter.cs:662` が「no tracked book writes paper{} as of 2026-08-23」と書いており、**第234 の importer が paper を*出す*ようになったので、MusicXML → .lys → 双子の往復を測る日が来たらここが最初に効く**）／▶ perf（歌詞打鍵の章はほぼ完了＝55.1 vs 非歌詞 45.3。残り ~10 MB は hyphen／apply／非歌詞 L5/L9 等の小粒）／⒡ 配管 6 site／⒣ removeEmpty/pedal の score 移行検討（別便＝ユーザー指示・着手はユーザー決定から）／小粒: twin の歌詞行・`lines` twin 未輸出・マークの X・chord-row の上帯スカラー・非ペア ToCoda の reserve≠draw・lead-sheet 音節×縦線の対・lead-sheet の mid-piece `time` 変更の表示・実譜の `%` 記号。Marketplace は PAT 待ちのまま（第220 ①）。第97 の二重走査（`StaffAccidentalColumns.cs:103` と `ElementCoordinator.cs:131`）・§2 E の未移植 LP 計算（volta shorten・hairpin niente・`ledger_positions`）・courtesy 群の第131 起票分は**第235 が実コードで開いていることを確かめてある**。
  - ✅ **本便が新しく名指し、そのまま閉じた 1 件＝§7 の手順 3.5 に計器が付いた**（続き便＝同じ委任「有利なら着手」→**着手**。**判断根拠は文脈の重なり**＝この便は archive の移設・CRLF のバイト手術・引用ラチェットの動き方を全部温めており、**次便には移らない**）。番人 `HandoffArchiveContinuityTests`（3 本）＝⑴ **§1 が持つ経緯ブロックはちょうど 1 本** ⑵ ★★★ **archive の最新は §1 が持つ番号の 1 つ前**（＝**継ぎ目**。忘れた *その commit* で赤くなる＝**ブロックが作業ツリーからまだ回収できる最後の瞬間**）⑶ **穴 9 個の国勢調査（天井）**。
    ⚠️ **穴は 9 個で、うち losses と確定できるのは 3 個**（第230・第231・第233＝**隣のブロックが「HEAD `175b8b12`（第231 の閉幕 handoff）」等と引いているので §1 を書いたことが証明できる**）。残り 6 個（58・59・95・177・207・226）は**未分類**——この repo には**§1 を書かなかった便**（第87 の見出しが「commit message からの復元」と明言）も、**1 見出しで複数便を覆うブロック**も在るので、**個別の考古学は買わない**。
    ⚠️ ★★ **最初の測定は 27 個と言った**（見出しが `第Nセッション第M便の経緯` や括弧つきの形も取るのを知らず、**archive 済みの 18 個を穴と数えた**）。**過大報告は安全側ではない**——**本物の 3 個をノイズに埋める**。第235 の「入れ子の点名を境界付きで測り直す」と同型が **9 例目**。
    ⚠️ **ブロックの*回収*は着手していない**＝**27000 行の文書に 9 便ぶんの逐語を戻すのは内容の判断でユーザー決定**（回収すると引用が増えるので、**dead の天井 469 と live の床を同じ commit で上げ直すこと**。落ちたブロックは*displaced した commit* の `docs/HANDOFF.md` に在る）。

- ⚠️ ★★★ **⑹ 本便の末尾でも同じ委任を受け、⑸ を triage して*断った*。数えた結果を置く**（第234 ⑸' と同じ形＝**次便が §2 を読み直さないため**。断ったこと自体ではなく、数えた結果が引継ぎの価値）。
  **⑴ 温めた島は空**——本便が温めたのは **git の履歴・ref・引用・§1/archive の構造・番人の建て方**で、**その島の項は 2 つとも本便で閉じた**（引用の番人／§7 3.5 の番人）。残る 1 つ＝**ブロック回収 9 便はユーザー決定**と本便自身が名指した。⇒ **温かい島に着手できる項は 0 件。**
  **⑵ 残り 11 項の内訳**（数えた）＝**ゲート付き 4**（§1.2 リネーム＝ユーザーが MSVS で／⒣ removeEmpty・pedal＝「ユーザー決定から」と自分で書いてある／Marketplace＝PAT 待ち／ブロック回収）・**engine 側で文脈が冷えている 7**（⒤ は観測者ゼロで本を足すか probe を設計する所から／▶ perf の小粒／⒡ 配管 6 site／小粒 8 件は §2 に本文が無く**起票から**／第97 の二重走査は「+0.3% でコスト対効果の判断」と自分で書いてある／§2 E の未移植 LP 計算＝**6 桁 exact で出力が広く動く**＝第234 が断ったのと同じ形／courtesy 第131 分は点が無く起票から）。**本便の島との重なりは 1 件も無い。**
  **⑶ ★★★ 入力の鮮度は*機械で*示せる**（第235 骨 2 への返答）——**本便の commit は `LilySharp.Core`／`Cli`／`Lsp` を 1 ファイルも触っていない**（`git diff --stat a97aec3a..HEAD -- LilySharp.Core LilySharp.Cli LilySharp.Lsp` が空・実測。⚠️ **便数を書かないのは、あとで便が増えても腐らないため**＝本便の ⑥ が実際に増えた）。⇒ **第235 が実コードで確かめた「⑵ 開いている」判定は構成上そのまま有効**なので、**次便は §2 の再 triage から始めなくてよい**（§0 の数は毎便どおり取り直す）。
  ⚠️ ★★ **⑷ 委任を断ったあとユーザー依頼で README を見た＝そこにも stale が居た**。`### Planned` の 3 件のうち **⑵「MusicXML export: lyrics and tuplet output」は実装済み**（`MusicXmlTypes.cs:578` が `<lyric>`／`MusicXmlExporter.cs:3026` が `<tuplet>`・観測者は `MusicXmlExportShapeTests.cs:211,367` と `MusicXmlRoundTripTests.cs:243`）。**同じ主張が Implemented 側の但し書きにも 2 綴り目で座っていた**ので両方直した（§2 A の「同じ量が 2 か所」の*散文*版・**片方だけ消すと文書が自己矛盾する**）。⚠️ **但し書きを*消す*のではなく*差し替えた***——消すと「MusicXML export は完全」と読め、それは新しい嘘（RULES §5.1「削除も主張である」）。**残り 2 件（cross-staff beam の production・LilyPond → Lily#）は実コードで開いていることを確かめた。**
  ⚠️⚠️ ★★★ **⑸ そこで 4 件目の stale を見つけた＝*コード側***。**`MusicXmlExporter.cs:421` の remark**（`WalkForm` の上）が「**Nav marks, custom text and volta ENDING brackets are not yet mapped to MusicXML**」と書いているが、**nav marks は 10 種すべて移植済み**（`ApplyNavMark`＝`<segno>`/`<coda>`/`<words>`＋対応する `<sound>`）、**volta ending も移植済み**（`EmitVoltaRepeatBlock`・観測者 `MusicXmlExportShapeTests.cs:255` が `<barline><ending>` を assert）。**生き残っているのは custom text だけ**（`WalkForm` は `MusicMarkSyntax` を 1 度も見ない）。⇒ **本便は直していない**（README の依頼とは別の島）。**1 行の訂正＝次便の最安の一手。**⚠️ **この形は §5.1 が名指す最悪の札**——「未実装」と書いた札は、**次の人に移植済みの島を作り直させる。**
  ⇒ ★★★ **骨＝断るときは「入力がまだ新しい」ことを機械で示す。** 第234 の断り方は健全だったのに**入力が 157 便前から嘘**で、そこだけが弱点だった（第235 骨 2）。**「engine に 0 接触」は 1 コマンドで、triage 全体の鮮度証明になる**——**選別をやり直すより 3 桁安い。**
