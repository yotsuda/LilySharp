...
- ★★★ **⒮⁶ 「建てない」の島の残り＝約 62,314 B／打鍵 1.36%**（第448 実測 92,144 −第449 の 18,788
  −第450 の島ぶん 11,042＝**引き算で出した値・未再測**。次に census を回す便は、この数ではなく脚の出力で
  置き換えること）。⚠️ **上位 4 軒 23,140 は*門前払いと実証済み*＝器が呼び手へ出る**
  （`ItemSkylineFactory.cs:465` 9,118／`MultiStaffLayouter.cs:754` 7,082＝builder ごと返す／
  `HorizontalSkyline.cs:63` 4,171＝field を `Merge` が読む／`BeamSubdivision.cs:253` 2,769）。
  ⇒ **その 4 軒を除くと、次の段は 1 軒 1,000 B／打鍵 級**（第448 の表から）: `LayoutEngine.Annotations.cs:531`
  **2,639**（Dictionary・23.8% 1 件）／`BeamSubdivision.cs:256` **1,804**（SortedSet・63.2% 1 件）／
  `MeasureLayouter.cs:466` **1,070**（**99.6% 1 件**）／`MeasureLayouter.cs:319` **1,012**／
  `SpacingRules.Springs.cs:394` **952**／`MultiStaffLayouter.cs:2513` **934**／`ElementCoordinator.cs:1877`
  **910**（79.3% 空＝⒩⁵ の族）／`LedgerLineSpannerEngraver.cs:160` **909**／`MultiStaffLayouter.cs:2557`
  **891**（**100% 空**）。**第450 が閉じた 3 軒はこの一覧から落とした**。
  現在地は Lab `sessions/p448/site-prices-after.txt`（脚は `instrument.ps1`＋`Zz448.template.cs`＝1 run で全サイト）
- ★★★ **⒱ 毒が見せた網の穴 3 件（第450 起票・*この便の書き換えが作った穴ではない*）**＝**赤と予測して
  緑で返った毒 3 本**。⑴ `PageLayouter.RespaceStaves` の**ばね 2 本以上の腕**（spill で 1 本目を落としても
  0 赤＝実コーパスもスイートも 1 本しか描かない）／⑵ `MeasureContentKey.BucketSpan` の遅延生成
  （spanner が「まだ空の小節」に落ちる形がスイートに無い）／⑶ **`SharedRenderer.DrawChord` の付点 support**
  （空の span を渡して 0 赤。**`DrawNote` の同じ毒は +5 赤**）。⇒ **⑶ が一番大きい**＝和音の付点列は
  `test/dotted-flag-dot-column` の兄弟が要る。規則は RULES §5.4 末尾（→CASES 5.4-057）
...
### 1.1 第450セッション（2026-09-21・YT-DELL2）
