...
（**実装 3 便＋計器 1 便**。**`perf-plain1k` の打鍵 60.2 → 47.7 MB＝−20.8%**。
★ **最後の 1 便は本の形によらず全部の本に届く**——`v2bow1k` −5.5%・`scripts1k` −5.3%・
`fingstack1k` −3.4%）。
...
**566 冊のバイト A/B は 4 回とも 0 冊**（毎回 `git ls-files "*.lys"` の **566 冊**）
——⚠️ **ただしその 0 が*証拠*なのは第1便と第2便についてだけ**（第3便には掃きが届かない＝下の骨 ⑴）。
★ **陽性対照は 3 種**（clip-edge を落とす **148 冊**／memo の hit を空にする **11 冊**／
鍵を盲にする毒で **suite が 22 本赤**）＝**3 便それぞれに 1 つずつ。**
...
> ## ★★ 6 つ目の骨＝**`perf-v2bow1k` は floor-of-3 でも決定的でない。危うく +3.7 MB の退行を報告した**
...
| 1 打鍵 | 便前 | ⑴ 群 | ⑵ 検出 | ⑶ 鍵 |
|---|---|---|---|---|
| `perf-plain1k` | 60.2 MB | 56.7 | 54.1 | **47.7（−20.8%）** |
| `perf-v2bow1k` | 83.7 MB | 83.7 | 83.7 | **79.1（−5.5%）** |
| `perf-scripts1k` | 77.2 MB | 77.2 | 77.1 | **73.0（−5.3%）** |
| `perf-fingstack1k` | 171.8 MB | 171.8 | 171.8 | **165.9（−3.4%）** |
...
- **⑷ ★ 第4便＝計器の所見を計器自身の doc へ**（`c031c5c8`・**Core 非接触**）。
  **`Alloc` の「暖機後は決定的」に「ただし本による」を足した**（骨 ⑹）。
- **⑸ ★★★ `perf-plain1k` の打鍵 47.7 MB の現在の地図**（第3便の後・単位 MB）:

  | 段 | MB | % | 状態 |
  |---|---|---|---|
  | `L1-break`（DP） | 19.3 | 40% | **閉じた**＝本質的・2 案とも測って却下（§1 第191） |
  | `4-render` | 8.9 | 19% | **床に近い**（構造上の下限 約 7.5・第191 第7便） |
  | `1-collect` | 6.1 | 13% | ⚠️ **まだ一度も割っていない** |
  | `3-layout` の残り（梁・注釈・ページ以外） | 約 5 | 10% | ⚠️ **未分解** |
  | 梁の検出（1 軒に畳んだ後） | 2.6 | 5% | ⑵ の後。中身は `CreateBeamGroup` |
  | `2a-contentKeys` | 2.0 | 4% | ⑶ の後。残りは `BoundaryClefAllowance` 0.93 が首位 |
  | `P4-prelimAnnotations` ／ 最終注釈パス | 2.7 ／ 2.7 | 11% | 済 |
  | `CreatePages` | 1.0 | 2% | 済（第191 第5便） |

  ⇒ ★★★ **次の島は 3 つ、どれも「割ってみる」から始まる**:
  **⒜ `1-collect` 6.1 MB**（一度も割っていない・第191 が梁の bake だけ返した残り）／
  **⒝ `3-layout` の未帰属 約 5 MB**／
  **⒞ `BoundaryClefAllowance` 0.93 MB**＝**1 個の nullable を読むために
  `BoundaryColumn` を丸ごと建てている**（1000 小節で 0.93 KB/小節）。
  ⚠️ **⒞ には `CreateSpringsForMeasure` という 2 人目の呼び手が居る**ので、一緒に見ること。
...
- **⑺ ★ §7.5 の監査**＝**Core +364 行（うち*コード 200*）・削除 −127（コード 107）・
  REF +2/−2（同じ 2 本が clip-edge の注記と一緒に動いただけ）・OWN 0**
  （⚠️ **`git --no-pager diff --no-color` で数えた**）。**彫版の定数は 0 個**。
  **分類は 3 便とも ⒟**（何も足していない＝複製の削除／既存の家の指し直し／署名を狭めただけ。
  **LP の意味論に 1 つも触っていない**）。**§7.7 の匂い一覧はどれも当たらない**——
...
> # ▶ **打鍵のアロケーション＝「置いて捨てる」「同じ物を 2 度作る」の返済。残るのは*一度も割っていない塊***
> （2026-08-17・第190 起票／**第191 が 6 段・第192 が 3 段返済**＝
> **`perf-fingstack1k` 3304.5 → 165.9 MB（−95.0%）・`perf-plain1k` 95.9 → 47.7 MB（−50.3%）**。
> **次の一手と現在の地図は §1 ⑸ の表**）
>
...
>
> **⑴ 残っている 3 つの島**（**どれも「割ってみる」から始まる**。MB は `perf-plain1k` の 1 打鍵）:
> **⒜ `1-collect` 6.1 MB（13%）＝一度も割っていない**（第191 第8便が梁の bake だけ返した残り）／
> **⒝ `3-layout` の未帰属 約 5 MB（10%）＝同じく未分解**／
