...
（**実装 4 便**＋文書の便）。ユーザーの目的は **0.3.0 を出すこと**で、優先は
...
終了時 **実装の最後は `b9e8cbe8`**（以降は文書のみ）・**未 push は
`git rev-list --count origin/master..master` が言う**・**未追跡 0**・
suite **4941 passed / 0 failed / 4 skipped**（+2＝新しい網ちょうど）・
...
- **⑸ ⚠️⚠️ ★★ 残る穴＝`GRAMMAR.md` は*この網に足せない***。**plain な fence が 0 本**（実測）で、
  例は **EBNF の `(* … *)` の中**に住んでいる＝**別の抽出器が要る**。
  ⚠️ **空理論ではない**——**この便が直した `@segno` の 1 行はまさに `GRAMMAR.md` の例**で、
  **網は 1 本も鳴らなかった**。§2F に起票した。
...
- **⑻ ⚠️ 環境＝pwsh の MCP コンソールが 2 本、110 分以上走りっぱなしだった**（開始時から）。
  **ユーザー判断で放置**。⚠️ **▶ の打鍵順位を net10 で取り直す便は、先にこれを片付けること**
  ——**静かな窓にならない**（§5.3「帯を先に測る」）。
- **⑼ ★ 次の一手はこのセッションでは選んでいない**。**▶ の打鍵順位は net9 実測の ms のまま**で
  net10 の床は動いている（§5.5）ので、**順位を取り直してから選ぶ**——第174 の判断のまま。
  **§2F に残っているのは ⑴ `GRAMMAR.md` の抽出器 ⑵ parse しない fixture 2 本（直すか消すか）
  ⑶ 繰り返し縦線の ⑹（section 音楽中の片側 `:|`）**。**どれもリリースブロッカーではない**
  （⑵⑶ は `エラー → 警告 → 動く` の向き＝後から広げても誰も壊さない）。
...
- ~~★★ **仕様書の網は `GRAMMAR_FOR_LLM.md` しか読んでいない**~~ — **第175 で閉じた**
  ...
  **正典 58/0・TUTORIAL 13/0・LLM 版 22/0**。**毒は両方の新ファイルに入れて赤を見た**。
  ⚠️ ★★★ **残っているのは `GRAMMAR.md` で、これはこの網に足せない**——**plain な fence が
  0 本**（実測）で、例は**EBNF の `(* … *)` の中**に住んでいる。**別の抽出器が要る**。
  ⚠️ **この穴は空理論ではない**——**第175 が直した `@segno` の 1 行はまさに `GRAMMAR.md` の
  例で、網は 1 本も鳴らなかった**。着手するなら**抽出器を書いてから「線を引く前に測る」**。
