...
★★★ **⑶ 和音記号の上付き＋三角も出荷した（ユーザー「⑴ 入れて。⑵ 三角も入れて」＝承認済み）**: **`\super` は font-size −3 かつ magstep(元の font-size) だけ上げる**（根音 2.6165 対 上付き 1.8500・持ち上げ 1.1892）。**ベースラインに残るのは根音・`m`・`/低音`・`+`・`°`（`°` は `whiteCircleMarkup` の `\fontsize #2` で*大きい* 3.2965）だけ**で、数字・`ø`・`sus`/`add`・三角は上付き。**三角は多角形**（底辺 1.0703／高さ 0.9204／線幅 0.1・丸キャップ）。**全部 LP 2.26.0 の実測と一致**（計器は `scratch/p372/lpnames.ly`／`lp-words.svg`／`lpacc.ly`／`lptri.ly`）。**`Dmaj7` のインクが `(0.000000 . 2.497137)`＝LP の dump `(0.0 . 2.5008)` と下端完全一致**（`j` の下ひげが消えた）。**家**: `Music/ChordSymbolText(Text, SuperFrom)`＝namer が上付きの開始位置を*構造として*運ぶ／`ChordNameGlyphRun` の piece は **`ChordPieceKind`（Text/Accidental/Triangle）と `FontSize`** を持つ＝**LP の markup は font-size を運び stencil の種類ごとに自分の規則でサイズへ変える**（因子 1 つで 3 種を賄うのは LP に無い数・RULES §5.2 の worked example）。

★★★ **⑷ 数**: **開始時**（`Session-Check -Build -Test -Scratch p372`）: HEAD **`6ba76059`**・**未 push 14**（`origin/master`＝`b3bf9364`）・木 clean・未追跡 0・`gh run list` **緑**（34543531416）・build 0 エラー／Core 0 警告・**full `scratch/p372/run1.trx` 7764 / 0 / 4 / 7768**（第370 末 run9 と一致）・台帳 821・snapshot 249・追跡 `.lys` 599・ディスク 27085。**終了時: full `scratch/p372/run22.trx` 7782 / 0 / 4 / 7787**（+19＝新しい番人）・**台帳 821／OPEN 0／snapshot 249／追跡 `.lys` 599**（snapshot は第 4 便で 9 枚だけ再ベース・台帳は 7 点だけ再記録）。⚠️ scratch は **`p372`**。⚠️ §7 3.5: 第369 を `-Archive 369` で ARCHIVE へ（済）／**第371 の便ごとの逐語も §1 の 20,000 字の門に当たったので ARCHIVE の先頭へ早出しした**（`scratch/p372/fold-current-block.ps1`・49 行 19,312 字）。⚠️ 記憶: 新規 1（`feedback_ripple_mcp_banned_use_pwsh_mcp`）。

★★★ **⑸ 台帳 7 点を再記録した（ユーザー承認済み・LP 照合済み）**: `lyric-chord-run.staff-to-staff` **−0.593669 → −0.003782**（★ 台帳の記録原因が「この移植で閉じる」と*予言していた*点＝機構を閉じて確認された）。**`staff-to-chord` は −0.252403 → −0.851723（CHL1・CHL2 同値）**＝⚠️ **旧値は 1 つの量ではなく、実在する −0.85 と `j` の下ひげの +0.60 が符号違いで相殺した和**だった（§5.2 の相殺の形）。部屋はほぼ不動（+0.0195）。

⇒ ★★★ **⑹ 次の一手＝`staff→chord` の −0.851723138**。第 5 便で解剖済み（**測るだけ・製品 0**）: **ばねの床ではなく binding X における*五線の下向き skyline の中身***（LP は `nonstaff-relatedstaff-spacing` に padding 0.5 だけを宣言し `VerticalAxisGroup` にも基本距離の既定は無い＝床は無い／和音行のインク上端 ＝ 五線の下向き skyline ＋ 0.5 は Lily# でも成立＝機構は正しい）。**上付き移植で記号が 1.71 ss 細くなったので縛る X が動いただけ**で、私の移植が作った欠陥ではない。⚠️ **殺した仮説 2 つと 3 回の測定の経緯は ARCHIVE の第371 逐語 ⑿**（捨て計器 `scratch/p372/ZzP372WalkProbe.cs.txt` のヘッダにも）。**次便の最初の一手**: **LP の staff1 の下向き skyline を X ごとに dump し、和音行の箱が当たる区間で突き合わせる**（`-ddebug-skylines` か page-layout の dump＝**新しい計器が要るので新しい便で始める方が有利**と第 5・6・9・10 便で繰り返し判断した）。**⒝ 第369 ⑸⒝ の並びはそのまま**（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。**push はユーザー（未 push 27＋この便）**。

★★★ **⑺ この便が残した恒久ルール 2 件（蒸し返さない側へ出した）**: **⒜ RULES §5.2 に worked example を足した**＝**最も捕まりにくい字面違反は「LP の式と*同値*な簡約」**（実測に合わせた定数はいつか実測とずれて捕まるが、同値な簡約は**出力が 1 バイトも変わらないので反証しうる観測が原理的に無い**）。**判定は出力ではなく*語*で行う**——LP の式に現れている property 名・offset の数値・関数名がコードにそのまま現れているか。**「同値だから」は綴りを変える理由にならない**（`-0.5` から `(- font-size 3)` へは辿れない）。**⒝ §3 に決定 2 件**（`layout {}` の 7 キー＝候補は全部出荷／字面移植の原則）。⚠️ **そして語の監査は「同値な簡約を探す作業」に見えて*まだ描いていない絵*を見つける**——`symbols` ＋ `minorChords lower` の `°` が第 1 便から上付きで縮んでいた（snapshot に観測者 0）。**直したら番人を立てる**（`TheDrawnPieces_AreSetTheSizeLilyPondSetsThem`＝LP の 5 つの数を piece ごとに訊く・毒で赤を確認）。
