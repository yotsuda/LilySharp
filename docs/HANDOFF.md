...
- ★★ **`lysc ly`（双子 exporter）の穴**。**塞ぐたびに LP と突き合わせられる本が増える**ので、
  ...
  **残っているのは 1 つ**:
  ...
  ⇒ ★ **塞いだら双子 199 本の before/after を全数比較する**（第62セッション ② の手順。
     1 回目で本物の退行を捕まえている）
- ★★ **仕様書の網は `GRAMMAR_FOR_LLM.md` しか読んでいない**（2026-08-15・第174 実測）。
  第173第9便が `DocExamplesParseTests` のバーを **"parse" → "compiles"** に上げたが、
  **読んでいるファイルは 1 つだけ**（`RepoFile("docs/GRAMMAR_FOR_LLM.md")`）＝
  **`SYNTAX_REFERENCE.md`（正典）と `GRAMMAR.md` の例には観測者がゼロ**。
  ⇒ ★ **第174 で足した例は手で `check` に通した**（`No errors found`）——**網ではない**。
  ⚠️ **着手するなら「線を引く前に測る」**（第173第9便の型そのもの）——**除外なしで何本落ちるかを
  先に数える**。あの便は 22 本中 8 本落ち、うち 6 本は正当な例示だった。
