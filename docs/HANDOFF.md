...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第243セッション＝**ユーザーが残債の返済を保留させ、見えている不具合を*6 件*出した。⑴ 歌詞が次の段を突き抜ける ⑵ 並びを変えるとセクションマークが高すぎる ⑶ テンポとコード名が近すぎる ⑷ グリッドの拍子が中心に無い ⑸ セクションマークが五線から離れすぎ ⑹ 1 小節 4 和音で和音名が横に重なる。⑴〜⑸ を閉じ、⑹ は 9 通り試して再現できず実物待ち。** ⚠️⚠️ **本便は同じ領域で 3 回自己訂正した——3 回とも「6 桁で閉じた算術を機構と読んだ」**（骨 8）。**うち 1 つは*移植の承認*にまで届いており、着手直前に LP の実装を読んで止めた**（⑭⒝）。（10 commit＝`2ca29d46` 歌詞の帯／`5e10e399` §7.5 監査／`f1f21087` handoff／`287dc5ca` マークの錨／`f517d792` padding と機構の訂正／`92cb4a9d` handoff／`3211e0e9` 拍子の中心／`12679a4a` probe／`f0b4c3ab` 台帳の grob 訂正／`91d043d1` 幻の音部記号／この行）⚠️ **台帳 578→580**（新しい対 2）／**ss 非ゼロ 116→118**／**総和 6.557938014→7.643880014**。⚠️ **その総和は途中で 10.177938014 まで上がってから下りた**——**新点 2 つを残差 1.810000 で開き、同じ日に「別の grob と比べていた」と分かって 0.542971 に訂正した**（⑭⒜）。**count 113・非ゼロ 2 不動**・**追跡コーパス 572 不動**・**snapshot 222 枚は不動だが*12 枚*を再ベースした**（§5.2.1③ の台帳キーは新しい対）。**6074→6084**（＋6 新網 −1 退役した空の対照 ＋2 新台帳点 ＋3 拍子の中心）。
...
**⑧ ★★★ 2 便目の報告＝`chords / staff / lyrics` の並びでセクションマークが高すぎる**（`287dc5ca`）。**7.060 対 2.660**（他の並びの値）。原因は **1 つの番兵が 2 通りに解決されていた**こと: `MusicMarkLayout` は「最上段の staff」を `StaffIndex = -1` と綴り、**`OutsideStaffStacker` の tracker は最上段の*置かれた*要素**に解決し、**`StaffOffsetInSystemUp` は解決すらしていなかった**（`staffIndex >= 0` の門を落ちて **0＝system の頂点**を返す）。**2 つが一致するのは system の先頭要素が staff のときだけ。** ⇒ **値段を付けた行と描いた行が別**。両方 `StaffAffinity.TopSpaceableStaff` を通すようにした。

⚠️⚠️ ★★★ **同じ欠陥は 2026-08-20 に*bar number では閉じていた*。** `barnumber.chord-row.staff-to-ink-bottom` の `why` は**一般形をすでに書いていた**——「the two differ exactly when a chords/lyrics row leads the system」——うえに、自分の綴りを **「one spelling shared with the stacker's tracker choice」** と書いていた。**共有されていなかった。** mark は自分の綴りと自分の欠陥を 4 便持ち続けた。⇒ `BarNumberEngraver.AnchorStaff` は共有の歩きへの**別名**にした。

**⑨ ★★★ LP に訊いて新しい対を作った**（`audit/lp-geometry/probes/mark-chord-row.ly`・**台帳 2 点**）。**MKC と MKP は変数 1 つ違い**（chords 行が先頭に居るか）で、**LP はどちらも 2.850000**——**RehearsalMark の Y 親は staff の axis group で、ChordNames context はそれではない**。**Lily# は 2 冊のあいだで 4.400000 食い違っていた。** ⇒ **対が主張なので、点が見るのは*恒等*であって絶対値ではない**（絶対の残差 1.810000 は**別の・古い量**＝Lily# は箱を描き LP は素のボールドを描く／`AboveStaffOffset` は LP に対して 1 度も測られていない）。**毒**: 2 つの継ぎ目を戻すと**対が割れる（10.605 対 4.660）**。

**⑩ ★★ chords 行をこの判定に届かせたら、*届かなかっただけ*の欠陥が 2 つ落ちてきた。**
⒜ **天井がスコア全体で 1 つのスカラーだった。** inline chord しか来ないうちは出ようがない（inline chord は自分の音符の上に居るので、重なる mark はその隣の mark だけ）。**行が入ると全小節に記号が在り、本当に重なっている 1 つの mark が本の中の*全部*を持ち上げた**（実測: ラベル `B` は自分の小節の和音と本当に重なる。4 小節前の `A` が道連れで 7.805）。⇒ **grob ごとにした**（`axis-group-interface.cc:865-984` がそうしている）。
⒝ **X の窓が `cn.X` を*中心*として読んでいた。** 和音記号は `TextAnchor.Start` で描かれるので**`cn.X` は左端**。`cn.X ± (幅/2 + 0.3)` は**左へ半記号ぶん伸び、右へ半記号ぶん足りない**——全部の和音が自分の音符の上に居るうちは効かないズレで、**行の第 1 記号が x≈0 から始まって行頭の mark の隣に立った瞬間に効く。**
⇒ **LP の規則は両腕ともこの loop に在り、両方測ってある**: X が離れていれば **2.850000**（LP 本 MKC の第1段）、重なれば **5.845000＝和音の 5.045 ＋ RehearsalMark padding 0.8**（第2段）。

⚠️⚠️ ★★★ **⑧ だけを入れた時点で `samples/greensleeves.lys` のテンポ記号が第1和音を貫いた**——**途中状態で実証した**。**配置と予約は 1 つの claim**（§5.3）で、**片方だけ入れると「正しい場所に部屋の無い grob」になる。**

**⑪ ★★★ 3 便目〜6 便目＝ユーザーが「見た目」を 4 回続けて出し、そのたびに*測る場所*が動いた。** ⑧⑨⑩ のあと、⑴ テンポとコード名が近すぎる ⑵ グリッドの拍子が中心に無い ⑶ セクションマークが五線から離れすぎ、と続いた。**⑴⑵ は素直に閉じた。⑶ が本便で一番学びの多い一連になった**（`f517d792` padding／`3211e0e9` 拍子の中心／`12679a4a` probe／`f0b4c3ab` 台帳訂正／`91d043d1` 幻の音部記号）。

**⑫ ★★★ マークとコードの隙間は*マーク一族自身の* padding だった。** 天井は `axis-group-interface.cc:45` の `default_outside_staff_padding_ = 0.46`——**padding を宣言しない grob 用のフォールバック**——を足していた。マーク一族は宣言している（`define-grobs.scm` RehearsalMark `padding = 0.8`）。**しかもその REF は同じファイルの 2 行上に既に在った。** ⇒ `ChordClearancePadding = 0.8`。greensleeves でコード↔テンポ 2.400→2.740。

**⑬ ★★★ グリッドの拍子は帯の中心に無かった——繰り返し点と*同じ畳み込み*。** `DrawTimeSignature` は渡された上端から**名目の半五線 2.0** を下げる。それが中心なのは**帯が 4 スペースのときだけ**。2 番付きグリッド行は 7.2 で中心は 3.6 下 ⇒ **メーターは自分が開ける小節線の中心より 1.600000 上**（実測 帯 19.42‥26.62・中心 23.02・メーター 21.42）。**帯の中心を中線とする合成上端**を渡す＝`DrawTabStaff` が中線を持たないタブ譜に既にやっている反転。**4 スペースの帯では厳密に旧式と一致**するので 1 番だけのグリッドは不動。

**⑭ ★★★ セクションマークの高さは*X の症状*だった——そして「LP に準拠か」の私の答えは 2 回とも間違っていた。**
⒜ **朝の答え「1.81 高い・LP 非準拠」は誤り。** 台帳 2 点が **Lily# の SectionLabel を LP の RehearsalMark と比べていた**。LP ではこの 2 つは**別の grob で錨が違う**: `SectionLabel (left-edge staff-bar) / self-alignment-X LEFT / priority 1450` 対 `RehearsalMark (staff-bar key-signature clef) / priority 1500`。**実測: LP の SectionLabel は X=0.000＝左端**（book MKB。Lily# は 0.300）、**RehearsalMark は X=3.365、調号を足すと 6.385**（book MKK・1 変数）。⇒ **五線のある本では Lily# はほぼ LP どおり**（LP 自身の 2 段 4.117/4.687 が Lily# の 4.660 を挟む）。台帳を 2.850000→**4.117029**・残差 1.810000→**0.542971** に訂正（`f0b4c3ab`）。
⒝ ⚠️⚠️ **その誤った数の上に A′「break-alignment の移植」の承認が出ていた。** 移植前に LP の実装を読んで気づき、**承認を使わずに返上した**——実行していれば **222 枚中 213 枚の snapshot のラベルを「LP に合わせる」という名目で LP の錨から引き剥がしていた。**
⒞ **rows-only は別のセルで、そこでは本当に離れすぎていた**（book MKT＝rows-only ＋ `\sectionLabel` を新設して実測）: **LP はコード行のインク上端 ＋0.460000、Lily# は ＋1.960000。**
⒟ **原因は幻の音部記号**（`91d043d1`）。`SeedClefInk` は `staff.Clef` を読むが、**`StaffLayout.Clef` は全 staff が持ち既定は Treble**——コード行も歌詞行も。switch が `_ =>` に落ち、**描かれないト音記号**（高さ 1.800000＝`ClefG.Top 4.800` − アンカー線 3.0、x 0.300..2.865）を support に混ぜていた。**ラベルの X は 0.300..2.373** ⇒ ぴったり重なる。**support はラベルの X で厳密に 1.800000、他は 0.000000、五線のある本では同じ X でも 0.000000。** ⇒ **1.960 → 0.900。**

⇒ ★★★ **次の一手**: **⑭ の残り 0.440000。ユーザーが「本便で有利なら着手」と委任し、*不利*と判断して着手しなかった**（理由は次項）。**それ以外の 5 件は閉じている。**

⚠️⚠️ ★★★ **その残り 0.44 は「明らかな詰め方」が*行き過ぎる*ので、定数では閉じない。**
`MusicMarkEngraver` の base は `2.0 - AboveStaffOffset`＝**「上端*線*の 2.5 上」＝五線 5 本をよける形**。行には線が無くインクは帯上端の 0.300 上で止まるので、**base がインクより上に居て outside-staff パスが一度も働かない**。**base を帯上端まで下げてパスに任せると 0.160＝0.300 近すぎになる**——**パス自身の「行のインクの位置」が同じだけ低い**からで、`OutsideStaffStacker.AboveTrackers`（`p.Up.Raise(topUp - StaffBottom / 2.0)`）が profile を**名目の半五線**で持ち上げているのに対し、**テキスト行の profile は自分のテキストベースライン基準**（`MultiStaffLayouter` が `RowSkylines` を merge する箇所にそう書いてある）。
⇒ **要るのは「その継ぎ目に行自身の refpoint を渡すこと」で、`StaffLayout` はそれを持っていない**（`IsTextRow` も baseline も無い）。**定数ではなくモデルの変更。**
⚠️⚠️ **そして*今日のユーザー決定*と正面から当たる**: `barnumber.rows-only.row-to-ink-bottom` の `why` は小節番号の基準を帯上端に据える決定（2026-08-24）を記録し、その理由を**「テキスト行は outside-staff パスが働く相手のインクプロファイルを持たない（実測で空）」**と書いている。**profile のフレームを直すとその前提が変わる。** ⇒ **コードの問題ではなく決定の問題。着手する便は台帳のその `why` から読むこと。**
⚠️ **射程はマークだけではない**——同じ support にテキスト行の上の**小節番号・volta・カスタム文字・トリル・強弱**が全部載る。**本便で動いた 6 冊よりずっと広い。**
⚠️ **急がない理由**: **0.44 は「遠すぎ」＝絶対に重ならない側**。**0.30 近すぎは重なりうる。** 半端に閉じるほうが悪い。
...
- **★★★ 本便の宿題＝セクションマークが rows-only でまだ 0.440000 遠い。****定数では閉じない**（⑭ の下の警告を読むこと）——**`StaffLayout` に行の refpoint を持たせるモデル変更**が要り、**`barnumber.rows-only.row-to-ink-bottom` の `why` に記録された 2026-08-24 のユーザー決定と正面から当たる。****射程はマークだけでなくテキスト行の上に載る全部**（小節番号・volta・カスタム文字・トリル・強弱）。⚠️ **ユーザーは「本便で有利なら着手」と委任し、*不利*と判断して着手しなかった**（誤り率が高い状態・モデル変更・決定との衝突・0.44 は重ならない側）。
- **★★ 本便で再現できなかった報告＝1 小節に和音を 4 つ書くと和音名が横に重なる（ユーザー実測）。****9 通り試して全部隙間は正**（4/4・3/4・6/8・5/4・1 小節 5 個・`as roman`・roman グリッド・グリッドのみ・tab・a5/a6 用紙・全音符 1 個に 4 和音・長い名前）。**最も狭くて 0.15（roman）。**⚠️ **`ChordNameEngraver.ClearOfPrevious` に明示的な抜け道が 1 つある**——`if (!cur.UseTiming || !prev.UseTiming) return curX;`＝**`@chord(…)` の注釈が絡むと押し出しが効かない**。**実物を待つこと。当てずっぽうで直さない。**
...
終了時: **本便は 10 commit**（headline に列挙）・**未 push 25**・未追跡 0/木 0・**Windows 6084/0/4**・**WSL Ubuntu Release も 6084/0/4**・Core 0 警告（`--no-incremental`）・**台帳 580／ss 非ゼロ 118／総和 7.643880014／count 113・非ゼロ 2／snapshot 222（12 枚再ベース・追加 0）／追跡コーパス 572**。⚠️ **掃きは 6 回走らせた**（before→final2→mark2→pad→ts→clef）。**動いた本は延べ 20 冊で全部 PNG で目視した**——**掃きは「動いた」としか言わない**（骨 6）。⚠️ **§7.5 の数え上げ**: 第1便 `+` 98／REF 0／OWN 1／新定数 0、第4便 `+` 229／**REF 2**／OWN 0／新定数 1（*改名*＝`chordInkPadding = 0.3` は置き換えた窓に既にインラインで在った）、以降の新しい数値定数は 1 つ（`ChordClearancePadding = 0.8`＝`define-grobs.scm` RehearsalMark padding の REF つき）。**分類**: 帯の予約は **§7.6 ⒞**、`is_spaceable`／`skyline_spacing`／幻の音部記号のガードは **⒜**、マークとコードの clearance は **⒞**（**LP はその配置を一度も作らないので数を持たない**——2×2 の probe で全セル X が交わらないことを確かめた）。⚠️ **再現物**は `scratch\ベースタブLy\Untitled-6.lys`（ユーザーの本・`.gitignore` 配下）。⚠️⚠️ **本便のあいだにユーザーが 3 回編集している**——並びを 2 回変え、`section B` の和音を殺し、最後に `staff melody` を殺して **rows-only にした**。**§1 の数を読むときは「そのとき本がどの形だったか」を確かめること。** **追跡されている等価物**: `LilySharp.Tests/TrailingLyricsRowBandTests.cs`・`LilySharp.Tests/GridRowMeterCentringTests.cs`・`audit/lpreg/lyhygrace.lys`・`audit/lp-geometry/probes/mark-chord-row.ly`（**本便の新 probe・MKC/MKP/MKL/MKQ/MKS/MKK/MKB/MKT の 8 冊**）。
...
> ## ★★★ 骨 4＝**閉じた点の `why` が「一般形」を書いていたら、その一般形を*grep して他の綴りを数える***
> `barnumber.chord-row.staff-to-ink-bottom` は 2026-08-20 に閉じ、その `why` に
> **「the two differ exactly when a chords/lyrics row leads the system」**と
> **一般形そのもの**を書き、さらに自分の綴りを**「one spelling shared with the
> stacker's tracker choice」**と書いていた。**その主張は偽で、誰も確かめなかった。**
> **mark は自分の綴りを持ち、同じ欠陥を 4 便持ち越した。**
> ⇒ ★★★ **判定法**: **台帳の `why` に「〜のときに限って食い違う」と書いたら、
> その条件を満たす*他の grob*を数える**。「共有している」と書いたら、**その場で
> 呼び出し元を grep する**——**書いた本人が確かめていない主張は、次の人には
> 「確かめ済み」に見える。**（第242 の骨 4「片方だけ閉じた島」の*台帳*版で、
> あちらは doc、こちらは**閉じた点の説明文**。）

> ## ★★★ 骨 5＝**「効かないだけで壊れている」コードは、射程が広がった瞬間に 2 つまとめて出る**
> 天井のスカラーも X の窓の中心／左端の取り違えも、**inline chord しか来ないうちは
> 出ようがなかった**——**どちらも「その和音は自分の音符の上に居る」に依存**していた。
> **chords 行を届かせた 1 行が、その前提を同時に外した。**
> ⇒ ★★ **判定法**: **除外条件（`if (… ) continue;`）を外すときは、
> その条件が*他に何を保証していたか*を数える。** 除外は
> 「この場合を扱わない」だけでなく、**残りの場合についての暗黙の前提を養っている。**
> ⚠️ **そして片方は「1 つのスカラーを全員に配る」形**——
> **LP は grob ごとに置くので、スコア全体で 1 つの量は、たいてい移植の跡。**

> ## ★★ 骨 6＝**配置だけ直して出荷しかけた（§5.3 が名指している形を、実際に踏んで途中で捕まえた）**
> 番兵を直した時点で mark は正しい行に降りた——**そして
> `samples/greensleeves.lys` のテンポ記号が第1和音を貫いた。**
> **降りた先の部屋を誰も空けていなかった。**
> ⇒ ★★★ **外側 grob を「どこへ置くか」直す便は、同じ commit で
> 「誰がその場所を空けるか」も直す**（§5.3 の figured bass 条項がそのまま当たった）。
> ⚠️ **捕まえたのは掃きではなく*目視*だった**——**掃きは「動いた」としか言わない。**
> **動いた 7 冊を PNG で開いたから、2 冊が悪化しているのが分かった。**
> ⇒ ★★ **掃きで動いた本は数えるだけでなく*描いて見る*。**

> ## ★★★ 骨 7＝**「LP に準拠しているか」を答える前に、*両側でどの grob か*を名指す**
> 台帳 2 点が **Lily# の SectionLabel を LP の RehearsalMark と比べていた**。
> **同じ見た目の箱を描く 2 つの grob が、別の錨・別の priority を持つ**
> （`(left-edge staff-bar)` 対 `(staff-bar key-signature clef)` / 1450 対 1500）。
> **算術はどちらでも閉じる**——残差 1.810000 はもっともらしく、
> **その数の上に「移植する」という承認が出た。**
> ⇒ ★★★ **判定法**: 台帳の `probe` と `score` は*本*を名指すが、**grob は名指さない**。
> **「マーク」「記号」と書いたら、LP 側の grob 名を `why` に書く。**
> ⚠️ **裏取りは 1 変数**——本便は**調号を 1 つ足して**（book MKK）
> 「X が 3.365→6.385 と動く／Y は不動」を見て、**錨が行頭でないことを確定**した。
> **宣言（`define-grobs.scm`）を読むだけでは足りない**——**どの記号に解決するかは実装が決める。**

> ## ★★★ 骨 8＝**本便は同じ領域で 3 回自己訂正した。3 回とも「6 桁で閉じた算術」だった**
> ⑴ 「LP はマークを和音の上に持ち上げる。5.845＝和音 5.045 ＋ padding 0.8」
> ——**5.845 はその和音のインク上端 6.9837 より 1.1387 *下***。合っていたのは*ベースライン*で、
> **ベースラインに一致することはインクをよけることではない。**
> ⑵ 台帳の grob 取り違え（骨 7）。
> ⑶ 「A の梃子は `AboveStaffOffset`」——**1.41 動かしてもマークは 1 ミリも動かない。**
> ⇒ ★★★ **判定法**: **6 桁一致を見たら、*別の説明*を 1 つ挙げてから採用する。**
> 挙げられないなら、**その量を作っている関数を最後まで読む**（§5.2 の「足りないのは項」）。
> ⚠️ **そして 1 変数の対を作る**——⑴ は「音の高さ」、⑶ は「solution ビルドか」で潰れた。
> ⚠️⚠️ ★★ **3 回のうち 2 回は*出荷後*に自分で見つけた。** commit message に書いた機構が
> 誤りだったので、**同じ日に訂正 commit を出した**（`f517d792`・`f0b4c3ab`）。
> **台帳とコメントは「次の人が信じる」ので、誤った機構は欠陥より高くつく。**

> ## ★★ 骨 9＝**同じ畳み込みが本便だけで 3 か所に出た。畳み込みは grob の性質ではなく*帯モデル*の性質**
> **名目の半五線 2.0 を「帯の中心」として使う**形が:
> ⒜ **繰り返し点**（第226 で解いた・ユーザー報告）
> ⒝ **グリッドの拍子**（本便で解いた・同じユーザー報告の形）
> ⒞ **outside-staff の support**（`AboveTrackers` の `topUp - StaffBottom / 2.0`・**未**）
> **4 スペースの帯では 3 つとも正しく見える。** 見分けるのは**帯が 4 でない本**だけ。
> ⇒ ★★★ **判定法**: **1 つの grob を名目の半五線から解いたら、*同じ帯に描かれる他の grob を grep する***。
> ⚠️ **そして ⒞ は定数では解けない**——**行の refpoint を継ぎ目に渡すモデル変更**が要る（§1 の次の一手）。
