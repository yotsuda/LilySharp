...
⚠️ **⑥ perf。** **ms は主張しない**（§5.3・**StdDev が平均の 25〜33%**）。**確保は横ばい**:
**multi reuse 1329.1 KB（前 1329.1）・multi full 4486.54 KB（前 4485.96）・
single reuse 733.08 KB（前 733.36）。** ★ **reuse 行が full 行の 1/3 で在ること自体が
「whole-layout reuse は今も発火する」の対照。**
...
**未 push 51**（⚠️ **足し算しない**。`git rev-list --count origin/master..master` で数え直す。
...
- ★★★ **LP の `inside_staff_skylines` が 4 通りに綴られている**（2026-08-04・第90セッション。
  **休符の項だけ 1 本にした・残りは測って名指しただけ**・§1 と ▶ に全文）。
  ...
  **消費者ごとに建て直す限り部分集合は必ずずれる。**
  ⚠️ **測った穴**: `:2948` の seed に **slur（+0.641810）・tie（+0.421394）・tuplet（+1.180000）**
  が入っていない（**部屋を陽性対照にした**）。**要承認**（snapshot が動く）。
  ⚠️ **もう 1 つ**: 同じ script を **支持 ±0.6 の名目箱／mover 実アウトライン**で 2 回読んでいる
  （**0.598269 対 1.065278**）。**コード内の `LILYSHARP-OWN` が「観測者が来るまで待つ」と
  書いているが、観測者は部屋の数として今できた。**
