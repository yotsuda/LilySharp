...
▶ ★★★ **⒳ タブの和音のタイ＝1 枚の絵に欠陥が 2 つ**（第179 起票・**数は全部測ってある。
着手前に測り直す必要はない**）。**まず ⑵ の決定が要る**——決めずに ⑴ だけ直すと
**x は正しい数字に付いたが y は 3 弦ぶん高いまま**の中間状態になり、snapshot を 1 回焼き直して
見た目はほとんど変わらない。**⑴ が効く本は ⑵ が壊れている本と同じ 1 枚**（ジグザグは和音にしか
無いので、単音では ⑴ は起きない）。観測は全部 `test/tab-chord-tie`（`<c' e' g'>2~ <c' e' g'>4`・
guitar・双子は `lysc ly`）:
- **⑴ タイの端点がジグザグを見ていない**。**3 本のタイが全部 x 10.95 … 13.45 の同一区間**に出る。
  数字は和音1 が **8.42 / 8.42 / 10.33**、和音2 が **12.70 / 12.70 / 14.60** と段が違う。
  出所は `ElementCoordinator.GetChordHeadXOffset`＝**記譜の和音のずらし**
  （`ChordHeadPositioning.CalculateOffsets`＝2 度の衝突回避）で、**タブのジグザグではない**。
  ⇒ 触るべき数字のインク（中心 12.70・右端 13.5）を **1.55 通り過ぎて上を横切る**。
  ★ **第179 の stem x と同じ欠陥が音符の反対側にある形**（x は和音から・y は数字から）。
  ⚠️ **ジグザグの offset は `SharedRenderer.AssignTabChordOffsets`（Rendering）にあり
  `ElementCoordinator` は Layout**——**層をまたぐので offset を Layout へ移すのが先**
  （呼び手は renderer 2 か所＋テスト 5 か所）。
- **⑵ 和音のタイが自分の弦に留まらない**＝**要決定**。弦の行は **20.64 / 22.14 / 23.64** なのに
  タイは **20.05 / 18.78 / 17.28**（1.5 間隔で**上へ積み上がる**）。タブ分岐が置いた値どおりなのは
  1 本目だけ（`digitY − clearance = 20.10`）で、残り 2 本は後段が動かしている
  ⇒ **`TieFormattingProblem` の交差タイ項**（`EdgeYUp` 周辺）。**その remark 自身が
  `observed by: NOTHING on the tab side` と書いている箇所で、第179 でその観測が付いた。**
  ★★ **決定の中身**: **LP は和音のタイを広げる**（実測 **dir = −1, +1, +1**＝一番下は下・上 2 本は上）。
  **Lily# は「タイは stem と反対側に固定」という独自規則**（§3・`ElementCoordinator` の
  `forcedCurveUp: !stemUp`）。**同じ本の五線側では Lily# も LP どおり −1, +1, +1 を出している。**
  ⇒ **「和音でも独自規則を通すか、LP の広げ方に譲るか」がユーザー決定。**
- ⚠️ **背景（測ってある）**: ジグザグが**タイの通り道を潰している**——和音と和音の間の空き回廊は
  **LP 3.14 に対し Lily# 0.77**（LP は和音の数字が 1 つの x に揃う＝実測 `Lx=8.82 / Rx=12.951`・
  インク幅 0.099…1.089）。**一次の要因はジグザグで、数字の大きさは delta（= 最大幅/2 + 0.1）を
  通してしか効かない**＝ユーザーの見立てどおり。**ジグザグと数字の大きさは死守**（§3・2026-08-16）。
- ★ **重なりを消す手は「x を離す」以外にもある**（着手は ⑴⑵ の後）: **LP の
  `TabNoteHead (whiteout . #t)`**（`scm/define-grobs.scm:3743`）＝数字が背後を白で抜く。
  **Lily# は既に「弦の線を数字の幅だけ切る」`digitGaps` を持っている**のに、
  **その考えが弦の線にしか当たっていない**（実測：`tab-chord-tie` の SVG に白抜き 0 個）。
