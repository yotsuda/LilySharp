...
★★★ **⒀ 数**: **開始時**: HEAD `1e80a9b2`・未 push 9・木 clean・未追跡 0（**`Session-Check -Build -Test` は走らせていない**）。**終了時**: HEAD `ff40fdf8`・未 push 23・**台帳 843 点／ss 非ゼロ 210／総和 23.216135909／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249／追跡 `.lys` 599＝全部不変**（**絵は 1 枚も動かない**＝触ったのは LSP と test だけ・**Core は 3 便とも 1 行も触っていない**＝§7.5 の数えは 0 行）。**full `scratch/p375/run13.trx` 7943 / 0 / 4 / 7947**（第 1 便末 `run1.trx` 7816・第 2 便末 `run4.trx` 7827・第 3 便末 `run5.trx` 7829・第 4 便末 `run6.trx` 7839・第 5 便末 `run8.trx` 7842・第 6 便末 `run10.trx` 7854・第 7 便末 `run12.trx` 7869。**8 便とも合計を読んでいる**＝新 test は 6 + 10 + 2 + 10 + 3 + 12 + 15 + 78 = 136 本（第 8 便の 78 ＝ 26 綴り × 3 主張）。⚠️ 第 5・6 便は既存 test を**書き換えている**ので増分はそれより小さい）。★ **第 7 便は 7 つの context の配線を丸ごと替えたのに既存 7854 本が全部緑**＝**行の読みが 1 つになっても、読めていた綴りの答えは変わっていない**。⚠️ scratch は **`p375`**。⚠️ §7 3.5: 第372 を `-Archive 372` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝**第372 の天井 commit `9f6df2a3` が未 push のまま**。**push はユーザー（未 push 12）**。★ **LSP は 6 度配布した**＝`tools/Deploy-Lsp.ps1` が `0.6.0+b3742fcf` を `yotsuda.lilysharp-0.5.0-win32-x64\server` へ（95 ファイル byte 一致）⇒ **ユーザーは Reload Window が 1 回要る**（package.json は動かしていないので 2 回は不要・記憶 `feedback_lilysharp_lsp_deploy_target`）。
