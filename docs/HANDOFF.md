...
- **⒱ ✅ 閉じた（第453 が残り 2 件）**＝第450 が「赤と予測して緑で返った」3 件は、第452 が 1 件（⑶）と
  第451 の 4 件、第453 が ⑴⑵ を閉じて**全部に観測者が付いた**。⚠️ **閉じ方は*3* 通りあった**＝
  ⒜ **網を建てた**（第452 の 4 本／第453 の 2 本）／⒝ **「観測者が原理的に居ない」と実証**＝⒳⁵／
  ⒞ **網を*別の層*に建て直した**＝第453 の ⒱⑵（絵では 5 冊とも緑・**鍵**なら赤）。規則は RULES §5.4 末尾
...
- **⒩⁴ の脇 ✅ 閉じた（第453・第440 の毒 ⑶）**＝**あの毒は母集団の問題ですらなく、構造上 vacuous**。
  `GraceNoteEngraver.BeamedPrefix` は **`return n >= 2 ? n : 0;`（`:478`）＝1 を返せない**ので、
  `SharedRenderer.cs:389` の `beamed < 2` は **`== 0` と同値**——外しても通るのは 0 の場合だけで、
  直後の `for (i = 0; i < beamed; …)` は 0 回しか回らない。**histogram で裏取り**（スイート 1 回・
  187 回: **0=105・1=0**・2=63・3=11・4=5・5=3）。⇒ **`< 2` は事後条件の 2 つ目の綴り**（RULES §7.7）＝
  **`n >= 2` を緩める日が来たら、ここが読み手**
