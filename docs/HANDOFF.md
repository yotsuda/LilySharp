...
⇒ ★★★★ **次の一手**: **⑴ Boogie の 1 頁目の*段の高さ*を名指しする**——⑸ が題を無罪にしたので残るのは `Line_details` の高さで、LP はそれを吐かない。**`grob-top.ly` の型で grob 族を 1 つずつ落とす A/B**（雛形は `scratch/p338/nobracket.ly`＝`\override <grob>.stencil = ##f` を 1 行足すだけ・`scratch/p338/lp-ab.ps1` が 2 腕を回して頁ごとの段数を並べる・1 腕 30 秒）で「**どのインクを落とすと LP が 8,8,8 になるか**」を名指しする。候補は `RehearsalMark`・`VoltaBracket`・`Accidental`・`Script`（⑵ の計器が段ごとの上位 grob を既に刷っている＝`scratch/p338/boogie/lp.out`）。**名指しできたら `SkylineBuilder` にその族が在るかを突き合わせ、欠けていれば台帳点 → 移植（出力を変える＝承認事項）。** ⚠️ **Lily# 側の段の高さを LP と並べるときは必ず ⑴ の `ToFirst` を足す。** ⑵ 以下は第337 の並び（`lysc ly` の ChordNames＝ユーザーと綴りを決めてから／lyric row の slot の投票／ps2 の 0.017・beam-over-stem bar 1 の +0.33／push 後 `gh run list`／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／A/B/D/E／C⑴／G）。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
