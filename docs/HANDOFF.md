...
計器を通してしか測られていなかったので、半分が嘘だった便**（**実装 2 便**）。
...
終了時 **実装の最後は `e4218079`**・**未 push は
`git rev-list --count origin/master..master` が言う**・**未追跡 0**・
suite **5113 passed / 0 failed / 4 skipped**（+40＝新しい網ちょうど）・
**台帳 516 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・
**コーパス 0/300 を 3 チャンネルで**（**絵・双子・報告**を同じ run で・
...
★★ **環境＝pwsh の MCP コンソールは 1 本のまま**（`Thyme`）。**長い sweep の最中は
Read/Grep ツールだけで待った**——第183 に続いて 2 便連続で 1 本。次の人も 1 本で通せる。
...
- **⑷ ★ この便が*直さずに済ませた*もの 1 件（新規ではない）**——**双子は `octave absolute` の
  本で `part X { octave N }` を読まない**（`\fixed c'` 固定）。`test/octave-base.lys` で実測：
  **ページ・報告・MusicXML は C3、双子だけ C4**＝1 オクターブ違う音楽。
  ⚠️ ★★ **これは新発見ではない**——**`LilyPondExporter.AbsoluteBaseOctave` の remark が
  既にそう書いてある**（"an existing gate, not a new one"）。**自分の掃きが告発した先を開いたら、
  そこに答えが書いてあった**（RULES §5.0）。⇒ **第2便の入れ子 `\fixed` は*その同じ定数*から
  引くようにした**ので、**門は 1 つのまま**（2 つ目の綴りを作りかけて消した——§7.7 の
  「同じ量の 2 つ目の綴り」）。**塞ぐなら exporter の島（双子 199 本の全数比較）で、§2F の
  `lysc ly` の transpose 落ちと同じ手順。**
- **⑸ ★ §7.5 の監査**＝**第1便 Core +19 行・第2便 +60 行・REF 0 / OWN 0・数値定数 0**
  （数に見える 2 つは `groupOctaves = 0` と `octaveOffset = 0` ＝どちらも「動かさない」既定値）。
  **どちらも分類 ⒟**——**第1便は既存の家を指し直しただけ**（`ShiftOctave` の削除）、
  **第2便は既存の家を、それが飛ばしていたモードへ広げただけ**。
  **LP に対応物が無い**: phrase 参照は Lily# 独自で、群記号の全体伝播も
  「A deliberate Lily# divergence from LilyPond's per-member relative chain」と正典が書いている。
  `\fixed` は**双子が既に使っている綴り**（`EmitPartVariable` が同じ定数から引く）。
- **⑹ ★ perf（RULES §7 ⑼）＝計算を*減らした*便**。第1便は **`ShiftOctave` のレコード複製
  ...
  **collector は `PublishDiagnostics` 経由で打鍵ごとに走る**ので、これは打鍵経路が安くなった側。
  ...
  3 つの値を save/restore している**。**新しい走査も確保も memo も無し。**
- **⑺ ★★★ §2F に残っているのは 6 つ**——**⑴ 一覧に構文を与えるか（決定であって作業ではない。
  ...
  ⑹ 「Guards X」と名乗る fixture の残り 13 冊**。
