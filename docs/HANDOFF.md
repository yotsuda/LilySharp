...
★★★ **⑵ 穴 1＝condensed／combined の譜は双子に書かれていなかった**。`LilyPondExporter.RenderRows` が最上位の `condensedStaff`／`combinedStaff` を yield せず**譜ごと黙って消えた**（第380 の bench 双子＝和音行だけで譜 0）。group の中では `Skip`（"not exported"）で group がその譜を失った。⇒ **condensed＝`\new Staff { \clef … << \a \\ \b >> }`・combined＝`\new Staff { \clef … \partCombine \a \b }`**（手書き probe `audit/lpreg/pcombine-ctl.ly`／`pcombine-lp.ly` の綴り・clef は最初の part＝`RenderSpecParser.ParseCombinedStaff` と同じ）＝`EmitSharedStaff`。**LP 2.26.0 実測**（`scratch/p382/shared/clef-pc.ly`）: `{ \clef bass \partCombine … }` も `<< \\ >>` も譜 1 本・警告 0・combined は a2／Solo／Solo II を刷る。inline 和音行と歌詞行（`\lyricsto` を使わない duration 形）は各 part について従来どおり置く。**instrument name は書かない**（ページは SingleStaffSpec にだけ名を付ける）。

★★★ **⑶ 穴 2（掃きの途中で見つけた・より広い）＝数字だけ違う part 名が同じ変数になっていた**。`SanitizeVar` が数字を捨てた＝`fl1`／`fl2`→両方 `\fl`＝**LP は最後の定義を採り、全譜が最後の part を演奏**（警告 0）。**plain な `staff violin1 staff violin2` でも起きる**（`test/instrument-names`・`key-per-staff`・`part-header-key-per-part`・`treble8`＝`t`／`t8`・part-combine probe 群）。⇒ `VarName`＝**数字を語で綴る**（`fl1`→`flOne`）＋使用済み名の集合で残りの衝突に接尾辞。**網**: `LilyPondExporterTests` +4（`TopLevelCombinedStaff_…`・`TopLevelCondensedStaff_…`・`CombinedStaffInsideAGroup_…`・`PartsWhoseNamesDifferOnlyInADigit_…`）。

★★ **⑷ 射程＝追跡 599 冊の双子を base（`9ab0738a` の worktree・Release）と head で `lysc ly --batch -j 0`（`scratch/p382/twins/`）→ 35 冊が動き、全部この 2 族**（condensed／combined 28 冊＋数字衝突）。**動いた 35 冊の新しい双子は LP 2.26.0 で全部 exit 0・error 0**（`twins/lp-head/`）。警告 2 冊: `part-header-key-per-part` の key-change 衝突 4 は **base も 4**（前からある・変数が分かれて出る行が変わっただけ）／⚠️ **`pcsm-probe` の "too many colliding rests" は新規**＝原因は今回の綴りではなく**既存の voice span の書き方**: Lily# の `voice { R1 } { s1 } { s4 }` を双子は `<< { R1 } \\ { s1 } \\ { s4 } >>`（別 voice）で書くが、原典の LP は `<< R1 s1 s4 >>`（1 voice の同時音楽・probe の header が明記）＝`\partCombine` の中で休符が衝突する（**未着手**）。両 batch とも同じ 1 冊が失敗（`repeat volta` は Lily# の構文でない・前から）。**ユーザー実コーパス** `scratch\ベースタブLy` で condensed／combined を書く本は 2 冊（`bench.lys`・`beam-slur.lys`）。worktree は消した（`exe-base`／`exe-head` は `scratch/p382` に残る）。
