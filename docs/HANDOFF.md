...
最終更新 第286セッション＝**ユーザーが「残債の返済をやめて、見えている問題を直せ」と委任し、見えるものを 4 つ閉じた（4 便）**——commit は `b6327486`（強弱の X 列）・`d3ec055b`（起票）・`40928ec5`（percent の絵と LYS2014 の退役）・`8aba7105`（和音行が rit. を避ける）・`2b0b888c`（行頭の音符の住所）。**台帳は 1 点も動いていない**（740／exact 582／ss 非ゼロ 188／総和 24.714341137）。

**⑴ 引き継ぎの数は全部当たった**。**HEAD `5b00f5c9`・未 push 4・木 0・未追跡 0・Core 0 エラー 0 警告・suite 6484/0/4/6488・台帳 740／exact 582／ss 非ゼロ 188／総和 24.714341137／count 156 中 2・追跡 573 冊・snapshot 223 枚**が 1 つも違わない。

⚠️ ★★★ **⑵ この便の 4 つは「読み手が紙とプレビューで見たもの」で、追跡コーパスは 4 つとも*ほぼ盲*だった**。**動いた追跡コーパスの本は 4 便あわせて 0 冊**（**snapshot が動いたのは U3 の住所付け直しだけで、それは幾何ではない**）。⇒ ★★ **「suite が緑」は「読み手に見えるものが正しい」を*一度も*言っていない。** **4 件のうち 3 件で番人を新しく置いた**（`voice-dynamics-offbeat` / `chordrow-rit-second-system` / `LineLeadingSourcePositionTests`）——**どれも毒でその 1 枚だけが赤になることを確かめてある。**

**⑶ 強弱の X 列が自分の声部を読まない**（§2 U の前の便の起票）。**`GetItemXOffset` は ItemIndex を*タイミング*に畳んで列を引く**ので、**添字は「それが記録されたストリームの中でしか意味を持たない」**。`DynamicEngraver` は*譜の* measures を渡していたので、**下声の添字が上声の同じ序数の音符を名指していた**（`voice { c''8 d'' … } { c2 e2@f }` で `f` が x 11.78＝上声の 2 番目、自分の音符は 18.60）。★ **兄弟の `ArticulationEngraver` は staff→voice の二段を踏んでいて正しい**——**同じ音符に書いた `@f` と `@staccato` が別の列に出る**という形で並んでいた。**直下のコメントは正しい振る舞いを書いていた**（"the dynamic's own voice's note column"）。

⚠️⚠️ ★★★ **⑷ percent repeat の第 3 分岐は「3 つ」ではなく「1 つ」だった＝第282〜285 が LP を読み違えていた**。**ユーザーが「LYS2014 が出るのは不正だ」と報告し、それが当たっていた。** **`percent-repeat-iterator.cc:86-99` の分岐は `== mlen` と `== mlen*2` の 2 つだけで、`else` は 1 本**——**body の全長を持つ `RepeatSlashEvent` を 1 個出す**ので、**反復は「先頭にスラッシュ 1 本＋残りは空小節」。** ⇒ ★★★ **第282〜285 が「LP 自身の分岐」と信じた根拠は grob の *description*（"repeating patterns shorter than a single measure"）で、iterator にその条件は無い。** **description は用法の要約であって分岐ではない。** ⇒ ★★ **「LP はこうしている」を書くときは、*description* ではなく*分岐の在るファイル*を読むこと。** **`WholeMeasureRun` と `Ragged` は Lily# が発明した細分**で、**LYS2014 はその発明の自白だった**（定数ごと退役・番号は再利用しない）。

**⑸ ⑷ の実装で 1 つ躓いた＝`MeasureBuilder.AddItem` は 1 アイテムにつき 1 小節しか自動で閉じない**。**3 全音符を持つ spacer を 1 個渡すと 1 小節を閉じて残り 2 を飲み込む**ので、**「スラッシュ 1 本＋1 小節」**になった（LP は 3 小節）。**LP にはそういうアイテムが無い**——**イベントは長さを持ち、小節機構がその下で回り続ける**。⇒ **spacer を小節サイズに割って書く**。**1 小節未満の body は開いている小節の残りより短いので 1 個のまま＝ページはバイト同一**（実測）。

**⑹ 和音行が `rit.` を避けるのは 1 段目だけだった＝2026-08-28 の修理の*残り半分***。**その便は「`@rit` は外側インク」と決めて `MultiStaffLayouter.BuildAllStaffSkylines` で `sky.Up` に合流させた**が、**行が 2 段目以降に立つときはそこを通らない**——**後続段の行は loose-line の鎖（`LayoutEngine.Rows.LeadingLinesOfSystem`）が置き、その閉じの距離は五線の*内側*シルエットで測っていた。** ⚠️ ★★★ **切り分けの決め手は計器 1 本**——**`LeadingLinesOfSystem` が sys=1 でしか走らないことを stderr に出して初めて「1 段目と 2 段目は*別の経路*」と言えた**。**それまでの仮説（節の再生・volta・歌詞）は全部外れている**（再生では再現せず、volta 入りの 3 段でも再現しない）。⇒ ★★ **「同じ見た目の 2 つが違う答えを出す」ときは、まず*同じ経路を通っているか*を計器で確かめること。**

⚠️⚠️ ★★★ **⑺ 行頭の音符に住所が無かった＝`GreenNode.LeadingTrivia` は `virtual => null` で、override しているのはトークンだけ**。**合成ノード（音符・和音・repeat）は、先頭トークンが改行とインデントを抱えていても「前方トリビア 0」と答える**ので、**`SyntaxNode.Span` がそれを素直に足していた**。**行内の空白は*前の*トークンの後続トリビアになる**ので、**食い違うのは改行の後だけ**——**だから 551 個中 15 個、全部「行の最初の音符」だけが外れていた。** ⚠️ **直しは 2 段**（collector が `.SourceStart` を読む／`Span` が終端まで降りる）で、**⒜ だけでは 1 mm も動かない**（**実測した**——合成ノードでは `Span.Start == Position` だったから）。★★ **⒜ の置換はコンパイラに検証させた**——**`.Position` → `.SourceStart` を collector 全体に当てて建て、`SourceStart` を持たない受け手 6 件をコンパイラが弾いた**（**うち 3 件は増分 resume の番地で `Position` が正しい側**）。

**⑻ 「動いたのは住所だけ」を*機械で*証明した**（⑺ は 597 冊動くので、目視では言えない）。**動いた 597 冊の SVG から `data-pos` の値を伏せて突き合わせ、597/597 一致・それ以外で違う本 0 冊**。**`lysc check` の 466 行も (行,桁) だけの違いで、文言・件数・終了コードの変化 0**。⇒ ★★ **「大量に動いたが本質は動いていない」は、*伏せて突き合わせる*と 1 行で言える。**

⚠️ ★★ **⑼ 掃きの母集団にユーザーの生きたコーパスを入れると、2 パスの間に編集されうる**。**`Walk.lys` が 11:45 に編集され、時刻の離れた base/head の対で「LYS2002 が 4 行消えて 5 行増えた」という*偽の*差が出た**（同じ exe で続けて叩いたら同一）。⇒ **base と head は*続けて*取ること**（§0 の「A/B の before はその場で写す」の掃き版）。

⚠️ **見つけて直していないもの**（前便から引き継いだ分に加えて）:
- **★★★ loose-line の鎖の閉じの距離は今も*内側*シルエット**（⑹ の広い側）。**LP は loose line を VerticalAxisGroup の*スカイライン*＝置かれた外側 grob 込みに対して配る**（`axis-group-interface.cc:860-985`）。**今日入れたのは text spanner 1 つだけ**で、**dynamics・script・beam はこの鎖から見えていない**——**remark 自身が「NOT MEASURED」と書いていた棚**。**部屋の外側プロファイル（`placed.StaffSkylines`）はこの呼び出し地点から届く**ので、**移すなら独立した便と全数掃き**（§2 A の島）。
- **★ percent のスラッシュの小節内 X が LP と少し違う**（3 小節 body で LP は小節線から 2.09、Lily# は 1.09）。**構造ではなく spacing**で、**第282 が移植した拍スラッシュと同じ配置規則を共有**している。**触るなら拍スラッシュ側の番人と一緒に。**
- **★★ 2 冊のテスト本が壊れていて、error-recovery 出力を測っていた**（`part g { }`——`g` は音名なので LYS0002 で落ち、「パースできた部分から」描かれる）。**tab の percent テストは直した**が、**`scratch/p278/tabpct.lys` は同じ綴りのまま**。⇒ ★★ **LP に pin した数を主張するテストは、その本が*パースできているか*を先に言うこと。**
- **★★ `FlowsThroughBarAccounting` の厳しい gate は第283 から動いていない**（**114/472 が `break` かタイで落ちる**）。**LYS2014 の退役でこの gate の唯一の緩い呼び出しは消えた**が、**問い自体は残っている**——数は `MeasureValidator` の remark に移した。
- **★★ `lysc check` の重複 15 行 10 冊も第283 から動いていない。**
（以下、前便からの一覧はそのまま）

⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 U の 3 件は全部閉じた**ので、**この便の「見えている問題」の在庫は空**。**残っているのは台帳の残債と、⑹ の広い側（loose-line の閉じの距離を部屋の外側プロファイルへ移す）と、§1 の「要ユーザー判断」の束**。★ **ユーザーは「残債の返済をいったんやめて、見えている問題を直せ」と言った**ので、**次も見えるものが在るならそれが先**——**無ければ ⑹ の広い側が、証拠の揃っている一番大きい一手**（射程は掃きで測るところから）。

★ **開始時裏取り**: HEAD `5b00f5c9`・**未 push 4**・木 0・未追跡 0・Core 0 エラー 0 警告（`--no-incremental`）・**Windows Debug 6484 合格 / 0 失敗 / 4 skip / 合計 6488**・台帳 **740 点／exact 582／総和 24.714341137／ss 非ゼロ 188／count 156 中 2 が非ゼロ**・追跡コーパス **573 冊**・snapshot **223 枚**——**全部当たった**（⑴）。
終了時: **commit 5 本**（`b6327486`・`d3ec055b`・`40928ec5`・`8aba7105`・`2b0b888c`）＋**ハンドオフ 1 本＝この段落を書くもの**＝**未 push は 開始 4 ＋ 本便 6**・木 0・未追跡 0。**suite 6472 合格 / 0 失敗 / 4 skip / 合計 6476**（開始 6488 から **−12**＝**LYS2014 の検証テスト 10 本を消し**、**新しい番人を 3 本足した**〈snapshot 2 ＋ `LineLeadingSourcePositionTests` 5〉——**引き算は「テストが減った」ではなく「退役した規則の番人が消え、見える欠陥の番人が増えた」**）。Core 0 エラー 0 警告。**追跡コーパス 575 冊**（+2＝新しい番人の本）・**snapshot 225 枚**（+2）。**台帳全点不動。**
⚠️ **掃きは母集団 1519 冊（ディスク上の全 `.lys`＝ユーザーの実コーパス込み）で、便ごとに base と head を*続けて* 2 パス**（⑼）。**SVG は 4 対・`check` は 4 対**。**動いた本**: 強弱 **6**（全部 scratch のプローブ）／percent **35**（**ユーザー実コーパス 32 ＋ scratch 3・追跡 0**）／和音行 **1**（**報告された本だけ**）／住所 **597**（**全部 `data-pos` だけ**・⑻）。**`check` の変化**: percent で **LYS2014 96 行が消えただけ**／和音行は**バイト同一**／住所は **466 行が (行,桁) だけ**。**終了コードの変化はどの対でも 0。**
★★ **緑は*最後のコード commit のあと*に取った**（第282 ⑴ の再発防止を踏襲）。★★ **WSL ubuntu Release も同じ 6472 / 0 / 4 / 6476**（`scratch/linux-repro` の手順・**§0 の「この機械の Windows での緑でしかない」に両脚で答えた**）。⚠️ **CI だけは未実行**（未 push 10 本）。
