...
- **U8. ▶ 半分閉じた（第298）＝`grace { }` の本体は*歩かれない*。起票の「注釈を運ばない」より広かった**
  （**第293 起票・第298 が engine に訊き直して起票を訂正し、可聴化と 2 つの carry を入れた**）。
  ...
  **engine に訊けば族の定義ごと直る**（**LYS4020 は書いた初日に読み手の本を 1 冊名指した**）。
  ...
  ▶ **残っているのは「列を要る」側＝⒜⒝⒞⒟ の全部**（**`@mark` と `\N` を除く**）。
  ...
  **その行を消すのが閉じる作業の一部**）。**毒 8 本で赤くなる集合は全部別**（下記）。
