...
- **R4 `voice {} {}` 越しの音価記憶が 2 綴り**（voice 0 は inline で継ぐ・extra voice は `_defaultDuration = Quarter` で再構築 `MeasureCollector.cs:2116`）＝`c8 voice { d e } { f g }` で voice 2 が 4 分。`_parallelSpans` に (Duration, Dots) を載せる。MIDI（`:652-673`）と twin（`EmitParallel` の nested buffer）も同じ穴。
...
- **R14 exporter の残り**＝MIDI の歌詞 meta が 1 tick に全部（`ProcessLyrics :2567`）／twin が `CarryFrameInto/Back` で `_lastWrittenValue/Dots/_forceNextDuration` を運ばず tuplet/repeat/cue の後で音価がずれる／`\time 3+2/8` の綴り（LP は `#'((3 2) . 8)`・要実測）／`_pendingGraceSteal` が lane 跨ぎ／`c4 r4 ~ c4` で前の音が休符を跨いで延びる／`tempo 2 = 60` の beat unit（XML は quarter 固定・MIDI は 4 分 BPM＝`Bpm` の契約を確認）。
