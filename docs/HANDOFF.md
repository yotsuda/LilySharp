...
⇒ ★★★ **次の一手**: **§2 F ⒭ の残り**（**`transpose` のスコープ漏れ**＝出力が動く欠陥。**未決定・未着手**）／**§2 U8 の残り**（`grace { }` 本体の和音・休符／スラー・梁・タイ／注釈——**本便は 1 行も触っていない**）／**§2 の X/Y 側**。⚠️ **承認待ちは無い**（snapshot 3 枚は承認を取って再ベース済み・⑽）。
...
終了時: **commit 10 本**（**HANDOFF の stray CR 正規化 `72c8ba59` ／ form の 3 つ目の括弧 `be1c9047` ／ form の `:|*N` が MIDI に届く `26038c65` ／ importer が単純な繰り返しを form へ分解 `17165b87` ／ **⒫ 本体 `4e61cd8a`** ／ §1・§2・§3・RULES とアーカイブ `ffbf485a` ／ snapshot 3 枚の再ベース `82ae6037` ／ 数の閉じ直し `cf5d0acc` ／ **⒭ のヘッダ位置＝`LYS1035` `0eaa5d1b`** ／ ⒭ と §3 の書き換え `e5bb47cf`**）・**未 push 26**・作業ツリー 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6770 / 0 / 4 / 6774**（**開始 6754 から +20＝新しい番人 2 本**＝`RepeatStructureScopeValidatorTests` 13 ＋ `PartSettingInSectionHeaderValidatorTests` 7）・**台帳 748／588／190／24.714570188／158 中 2／`OPEN:` 0・追跡 `.lys` 581・snapshot 231 は開始時と同じ**（**枚数は不動・中身が 3 枚動いた**）。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけ**（定数は 1 つも足していない・`audit/magic_constants.csv` も不動）。⚠️ **間の push は無し**（`origin/master` は `be16a770` のまま）。★ **第304 が残した `C:\MyProj\lys-base304` の worktree は本便で消した。**
★★ **WSL ubuntu Release も同じ 6770 / 0 / 4 / 6774**（最終木 `e5bb47cf`・build 7 秒 / test 28 秒）——**§0 の両脚を読んだ。**★ **途中の `4e61cd8a` でも両脚を読んでいる**（そのときは snapshot 再ベース前なので両脚とも同じ 3 本が赤だった＝**赤の集合まで一致**）。
...
最終更新 第304セッション＝**§1 が名指した「次の一手」＝§2 F ⒬（section 参照に音高の引き継ぎ記法）をそのまま取って実装した**。**入り方は第298〜第303 と同じ**——**ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った**ので、**§1 の「次の一手」を取りに行き、その §1 自身が命じている「着手する便は、まず自分の取る 1 つを測ること」を先にやった**。⇒ ★★ **測ったら起票の見積りが 1 つ足りず、それが設計を変えた**（⑴）。**そして入れたあとに 2 つ目の穴が出た**（⑸）。**掃きは 2 回・両側 Release・4 出力・1920 冊＝動いた本は 12 冊で 12 冊とも scratch のプローブ（11 冊は本便が書いた本、1 冊は第303 の `~B''` プローブ）。追跡本は 1 冊も動かず、snapshot も台帳も不動。**

⚠️⚠️⚠️ ★★★★ **⑴ 着手前の測定が起票の見積りを 1 段広げた＝*ユーザーの本の 87% が `octave absolute` だった***（**283 / 326**。**section を 2 つ以上持つ 75 冊でも 52 冊が absolute**）。**起票は「phrase 参照の `~P'` / `~P,` を section 参照に通すのが最小の追加（新しい概念はゼロ）」と書いていて、*概念の数については正しかった***——**phrase 参照は既に両モードを持っている**（**relative は `EnterDefaultFrame` が `CurrentOctave` を動かし、absolute は `OctaveBase +=` と twin の入れ子 `\fixed`**）。⇒ ★★★ **だが「最小の追加」は*読み手ごとに腕が 2 本*という意味で、relative だけ実装していたら「この記法が存在する理由の本」の大半で黙って落ちていた。** ★ **判定法**: **「同じ印を通すだけ」と書いてある起票は、*その印が既に何通りに読まれているか*を数えてから着手する**（ここでは 2 モード × 4 読み手＝8 か所）。

⚠️⚠️ ★★★ **⑵ 実装は 4 読み手ぜんぶに別々に教えた**（第303 ⑹ の拍子と同じ形）: **ページ `OctaveContext.ResetForSection(offset)`／`.ly` は `SectionPlayMarker` の 3 つ目の積荷（label・header-key・header-time に続く）／`midi` は `PlayInPart` が anchor と absolute base の両方を動かす／`xml` は `EmitPartMusic` が同じ 2 つを動かす**。**検算は毎回*恒等の対***——**`~B'` と「B をオクターブ上に書いて素直に引用した対照」が、ページ（`data-pos` 伏せでバイト同一）・`.ly`（文字列同一）・MIDI（音高列同一）・MusicXML（`<pitch>` 列同一）で一致し、かつ 3 つとも*印なしの play とは違う***。**片側だけなら、印を落とす読み手でも全部緑になる。**

⚠️⚠️ ★★★ **⑶ volta ending にも通した＝*測ってから決めた*。** **`[1. B']` は起票に無い**。**数えた**: **ユーザーの 326 冊のうち form 本体に `[N. …]` を書く本は 6 冊で、77 冊は music にインラインで書く**（**インラインの ending は section を切らないので枠のリセットも起きない＝この記法が要らない側**）。⇒ **6 冊は少ない。それでも取ったのは 2 つの理由**: **⑴ §2 F ⒫（`|:` を form 限定にする）が入れば form 側の ending が*唯一の綴り*になる**／**⑵ 取らないと、`'` を蹴る診断文が「a trailing `'` or `,` to shift the octave」と*その場で宣伝している*ことになる**——**嘘をつく計器を出荷しないこと。**

⚠️⚠️⚠️ ★★★★ **⑷ その ending が、共有ヘルパの規則を 1 つ壊した＝*範囲区切りの `,` も Comma トークン*だった。** **`[1,3. B]` の `,` は section 名より*前*に立つ直接の子**で、**「印はそのノードの唯一の `'`/`,` の子である」という前提が 6 つの参照形では真、7 つ目だけ偽**。**素直に全 slot を数えると、木じゅうの範囲つき ending が*その日から 1 オクターブ下がる*。** ⇒ **ending だけ「section 名の次の slot から数える」**（`SyntaxFacts.NetOctaveMarksFrom`）**で、例外はヘルパの doc に書いた**——**8 人目が同じ発見をしないため。** ⚠️ **同じ族の 2 つ目**: **`FormAlternativeSyntax.HasSeparator` は `SlotCount == 9` で区切りの有無を判定していた**ので、**`[1. A'']` が区切り無しで 9 slot に届いた瞬間に `.` を区切りと読む**。**slot 2 を*種類*で見るように直した**（`SectionName` と `DisplayLabel` と `IsClosed` も同じ理由で index から kind へ）。

⚠️⚠️⚠️ ★★★★ **⑸ 出荷したあとに 2 つ目の穴が出た＝`section B { P }` を `~B'` で鳴らすと *absolute だけ*が動いた**（**実測: absolute は G4 → G5、relative は G3 のまま**）。**phrase の本体は「どの呼び出し元でも同じ音」のために*新しい枠*を開く**（`OctaveContext.ResetToInitial`）——**その枠の anchor が part のものだったので、relative では play の shift が落ちていた**。**absolute が動いていたのは偶然ではなく、そちらは参照が `OctaveBase` を*既に動いた base の上に*押すから。** ⇒ ★★★★ **1 冊の本に 2 つの答えがあり、決めていたのは `octave absolute` だった。** **直しは 1 本のフィールド**（`OctaveContext.SectionOctaveOffset`）**を `ResetToInitial` が読むだけ**（＋ ly と xml の phrase 腕に同じ 1 語。**midi は `_partOctaveAnchor` が既に shift 済みで最初から正しかった**）。⇒ ★★★ **判定法として残す**: **「モードごとに腕を 2 本書いた」ら、*その量を読み直す他の場所*を数える**——**穴は「片方のモードだけが既に正しい」という顔で出る**（RULES §5.4 の「規則を破ったら出力が変わるか」の*モード*版）。

⚠️⚠️ ★★★ **⑹ 7 人目を足すときに、既に在った 5 つの写しを 1 軒に畳んだ**。**「末尾の `'`/`,` を数える」は `PitchSyntax`・`ScaleDegreeSyntax`・`ChordSyntax`・`ArpeggioSyntax`・`VariableReferenceSyntax` が*同じループを 5 回*書いていた**（**違いは開始 index が 0 か 1 かだけ＝差の無い違い**）。**`SyntaxFacts.NetOctaveMarks` 1 本にした。** ⚠️ **そして畳んだその日に ⑷ が例外を出した**——**doc に「6 人については真、7 人目は違う」と書いてある。** ★ **§7.7 の「同じ量の 2 つ目の綴り」は、*6 つ目を足す人*が畳む機会でもある。**

⚠️⚠️ ★★★ **⑺ 本便の計器は 2 度嘘をつき、2 度とも*自分が仕込んだ*ものだった。** **⒜ 毒の runner が 17 本すべてで「suite 合計＝空」を刷った**——**`.ps1` を bash の heredoc で書いたので日本語の正規表現 `合計:` が壊れていた**（**RULES §5.5 の「合計を読むまで緑は意味を持たない」を、合計を読めない計器で回していた**）。**`DOTNET_CLI_UI_LANGUAGE=en` にして `Total:` で読むよう直した。** **⒝ その直しが次の嘘を作った**——**MSBuild の英語サマリは `    0 Error(s)` で終わるので、`Select-String 'error'` が*成功したビルドを BUILD FAILED と報告*し、17 本すべてが「ビルド失敗」で埋まった**（`: error` に直した）。⇒ ★★★ **計器を国際化すると、その計器を読む正規表現も国際化される。** ⚠️ **memory `bash-heredoc-strips-one-backslash` を 3 例目で踏んだ**——**`\relative` と書いた python が `\r` を書き出し、`poison.py` の 92 行目が CR で割れた。**

⚠️⚠️ ★★★ **⑻ 委任が出た**（「このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）。**答えは「着手しない」だが、*委任の時間で ⒮ の起票そのものを測ったら、起票の診断が間違っていた***。**⒮ は「form の repeat と music の volta を*混ぜた*から余分な `\bar ":|."` が出る」と書いていた**が、**`form main { ~B A }`（repeat 無し）でも出る**——**本当の項は「`.ly` 双子が music の inline volta を*1 度も変換しない*」**で、**`InlineVolta not exported` という*別の名前で既に警告が出ている***（ページ側も測った＝**3 冊とも小節数はページと一致・食い違っているのは `.ly` だけ**）。**射程はユーザーの 77 冊。** ⇒ ★★★ **段 ⑴ は GO**（`CreateEnding` / `EmitInlineRepeat` / `EmitBarline` は本便が丸一日居た場所）**だが段 ⑵ が NO で、理由は大きさではなく*所有者***: **「双子に inline volta を教える」と「⒫ でその綴りを禁止する」は同じ 1 つの決定の裏表**で、**⒫ は「方向には合意・決定ではない」のまま**——**今作ると ⒫ が消すものを作りかねない。** ⇒ ★★ **委任の時間は「次便の 1 時間目を消す仕事」に使った**（第302 の型）: **⒮ の測り直しと §2 の書き換え、＋本便が今日書いた「〜である」2 件の裏取り**（**`SyntaxFacts.NetOctaveMarks` の「6 つのノードでは印が唯一の直接 `'`/`,` トークン子」は*5 つの green コンストラクタ*を読んで確かめ、`InitialOctaveBase` の「`OctaveBase` の変更は全部つり合っている」は*書き手を列挙*して確かめた**——**どちらも書いた時点では推論だった**）。

⚠️⚠️⚠️ ★★★★ **⑼ セッションの最後にユーザーが 2 つ決めた**（§3 に行を足した）。**⑴ `|:` `:|` `[N. …]` を music に書けなくするのは*決定*になった**（第303 は「方向には合意・決定ではない」だった）。**⑵ 構造 section のラベル既定は*宣言側*で `section ~A { … }` と書いて*ひっくり返す***（ユーザー案・私は賛成）。★★ **⑵ が純粋な追加であることは測った**: **`section ~A` を書いている本はディスク 1925 冊中 0 冊**で、**今は硬いエラー**（`Expected a name, found 'Tilde'`）——**既存の綴りを 1 つも読み替えず、参照側の `~` 260 個は全部これまで通り「隠す」**（既定を裏返す section が 0 冊だから）。⚠️ **名指した帰結**: **`section ~A` を宣言した本では `form { ~A }` が「*表示*」を意味する行になる**（`~` を「隠す」と読む習慣とぶつかる——既定という概念の当然の帰結として受け入れる）。⚠️⚠️ ★★★ **そして ⒫ の着手前に*もう 1 つ*決めることが残っている＝移行**: **禁止すると*ユーザーの 326 冊のうち 115 冊（35%）が通らなくなる***（第304 実測。追跡側も 581 冊中 20 冊）。✅ **答えは ⒜ 即エラー**（猶予も自動書き換えも無し）。**私は ⒞ を勧めたが、115 冊という数を見たうえでユーザーが ⒜ を選んだ**——**ユーザーの原則「実装コストや移行コストは選択肢の並べ方に混ぜない」の適用**で、蒸し返さない。

⚠️⚠️⚠️ ★★★★ **⑽ 決定が出て委任の理由が消えたので、前提条件 ⑵ を*同じ便で実装した*（`207956f3`）。****`section ~A { … }` がその section のラベル既定をひっくり返す**——**`~` は両側で 1 つの意味のまま**（「既定でないほう」）。⚠️⚠️ ★★★ **だが先にやったのは規則の*集約*だった＝ラベルを決める文が **17 か所**に分かれていた**（**page 13＝`MeasureCollector.cs` 4 ＋ `MeasureCollector.Form.cs` 9（うち 6 は小節数え `AdvanceSection` の walk＝switch 2 ブロック × 3 腕）、twin 4**。各自が `silent ? null : DisplayLabel ?? name` を綴っていた）。⚠️⚠️ ★★★ **この数を最初「11」と書き、3 つのファイルと commit message `207956f3` に運んでしまった**——**部分的な grep で小節数え walk を 2 腕と読んだのが出所**（実際は 6）。**委任の時間の自己監査で捕まえて直した**（コードのコメント 3 か所＋ここ。**commit message は出荷済みなので直せない**）。⇒ ★★★ **数え方を書く**（§0）: **4 つの引数整形関数と `SectionLabelRule.LabelFor` を `LilySharp.Core` で grep し、整形関数の*定義* 4 本を引く**。**2026-08-25 に、そのうち 1 腕が `IsSilent` を教わっておらず、twin のコメントが「その腕を写した」と書いていた**——**引用が欠陥を出力境界の向こうへ 2 度運んだ**。⇒ ★★★ **項が増える規則は、増やす前に 1 軒にする**（`Semantics.SectionLabelRule`）。**11 軒のままでは*項を足せない*。** ★ **集約は掃きで恒等**（1931 冊・追跡本 0 冊）。⚠️⚠️ ★★★ **そして既定をひっくり返した瞬間に、*古い意味が隠していた欠陥が 2 つ*出た**: **⒜ `~` 綴りは parked label を捨てていた**（`FormWalk` が `null` を渡す。`~` が常に「隠す」なら無害＝**刷らないラベルは運ばなくてよい**——**`~A` が刷り得た途端に嘘になる**。実測: `section ~A` ＋ `form { ~A "shown" }` が section 名を刷った）／**⒝ LYS0012 は*パーサ*の警告で「`~` で隠れている」と言っていた**——**パーサは宣言を見られない**ので、**ページが刷るラベルを「隠れている」と報告する計器になるところだった**。**`FormDeclarationValidator` へ移し、表面（`~` があるか）ではなく規則（この play は刷るか）を訊くようにした。** ⇒ ★★★★ **その移動が、*診断が 1 つも無かった黙った取りこぼし*を捕まえた＝`[2. ~O "alt"]` は何も刷らない**（tilde は section 名に付く＝2026-08-25 の実測）。**旧警告は `~Name` しか見ていなかったので「警告しない」が*すべての ending について*真で、`FormVoltaTests` はそれを「正しい」として固定していた**——**その根拠（「ending の `~` は括弧だけを消す」）は 2026-08-25 に反証済みだった。** ★★ **教訓**: **「この綴りは警告しない」を固定するテストは、*その綴りに診断が存在するか*を先に確かめる。存在しなければ、そのテストは何も守っていない。**

⚠️⚠️⚠️ ★★★★ **⑾ 委任の 2 度目と 3 度目。答えは 2 度とも「⒫ 本体には着手しない」。⚠️ ただし*2 度目に出した数は間違っていた***——**「壊れる追跡本 20 冊・うち snapshot 10・LP 忠実度 7」は、コメントの中の `[1-4]` を数えていた**（例: `break.lys` は music に `|:` を持たない）。**数え直しは 3 度間違えてから収束した＝20 → 14 → 12 → 11 冊**（snapshot 3・LP 回帰コーパスの本 1・`audit/lpreg/` 5・showcase 2）。**外し方が 3 つとも違う**: ⒜ `//` コメントを読んだ／⒝ 行頭アンカーの regex が*1 行の* `form { … }` を剥がせなかった／⒞ **lyrics の `[1. …]` を数えた**（あれは `LyricVoltaGreen`＝繰り返しではなく歌詞の番で、**⒫ が禁止してはいけないもの**）。⇒ ★★★ **3 度とも「同じ量を別の壊れ方で数えた」**——**計器を `scratch/p304/count.py` に残した**（コメント除去 → 波括弧対応で form 除去 → lyrics volta を除く）。§2 F ⒫ に表。★ **対照的に、ユーザーの 115 冊は 3 通りの数え方すべてで同じ数**＝頑健。⚠️⚠️⚠️ ★★★★ **3 度目に着手しない理由は、数ではなく*⒫ の中に私が答えられない問いが 1 つ入っていた*こと**——**1 冊で実際に切り直して見つけた**: **`|:` は書いた場所で意味が違う。** **最小の実測**（2 パート・上のパートにだけ繰り返しを書く）: **music に書くと上 8 音・下 4 音／form に書くと上 8 音・下 8 音**＝**music の `|:` はそれを書いたパートだけを展開し、form の繰り返しはスコア全体を展開する。** **`grandstaff-repeat.lys` を実際に切り直したら MIDI が 18 → 20 音になった。** ⚠️ **ページは前から score 級に扱っている**（その fixture のコメント自身が「rh にだけ書いた繰り返しも両方の五線に出る」と書いている）＝**ページと MIDI は前から食い違っていた。** ⇒ ★★★ **だから ⒫ は「構文の移動」ではない**——**多パートの 2 冊は切り直すと*音楽が変わる*。** ⚠️⚠️ ★★★★ **ここで私は「それは意図か欠陥か」をユーザーに訊きかけ、*決定済みの話を別の服で訊き直していた*と指摘された。** **`|:` を music に書けなくすると決めた時点で、この綴りが書けなくなることは決定に含まれている。** ⇒ ★★★ **教訓（§5 へ）**: **新しい実測が出たら、それが*決定を覆す事実*か*決定の帰結*かを先に仕分ける。** **帰結を「新しい問い」として差し出すのは、蒸し返しと見分けがつかない。** **残るのは決定ではなく網の張り直し**（その 2 冊の主張が観測できなくなる）。 ★ **ユーザーのコーパスには当たらない**（`|:` を music に書く 115 冊は全部 1 パート）。 ⇒ ★★ **段 ⑴ も 1 時間前より弱い**: **music 側の `|:` は本便が 1 行も触っていない場所に居る**（触ったのは form 側と twin）。★ **住所は実測した**（本便の最後・次便が探さなくて済むように）: **music の `|:` / `:|` は `Parser.Music.cs:101`**（`ParseMusicItem` の `if (SyntaxFacts.IsBarlineKind(kind)) return ParseBarline();`）、**music の `[N. …]` は `Parser.Music.cs:128` → `ParseBeamOrInlineVolta` → `ParseInlineVolta`（`InlineVoltaGreen`）**。

⚠️⚠️ ★★★ **⑿ 委任の時間の自己監査で、*この便で出荷した数が 1 つ間違っていた*＝「ラベル規則は 11 軒」は 17 か所だった**（⑽ に数え方つきで訂正）。**部分的な grep で小節数え walk を 2 腕と読み**、**その 11 をコードのコメント 3 か所と commit message `207956f3` に運んだ**。**コメントは直したが commit message は出荷済み。** ⇒ ★★★ **本便 2 度目の「30 分前の自分を測る」で、2 度とも当たり**（1 度目は ⒮ の起票、2 度目はこれ）——**「N 軒ある」と書いたら、*その N を数えた式*を同じ行に書く**（§0 の「数え方も書く」の、*自分が今日作った数*版）。

⇒ ★★★ **次の一手＝§2 F ⒫ 本体**（`|:` `:|` `[N. …]` を music で禁止）。**方向・前提条件 ⑴⑵・移行 ⒜・LP 本の扱い ⒝ はすべて決まっている。** ⚠️⚠️ ★★★ **未回答は無い。⒫ は仕事だけで、次便は「⒫ に着手して」だけで始められる。**

★★★ **診断の置き場所も決めて渡す**（本便が足場を確かめた）: **パーサではなく*意味検証*に置く**——**`ISemanticValidator` を 1 本足し、`BarlineSyntax` の token が repeat 種（`RepeatStartBar` / `RepeatEndBar` / `RepeatBothBar`）のもの**と**`InlineVoltaSyntax`** のうち、**`IsInside<FormDeclarationSyntax>()` が false のものを報告する**。⇒ **述語 1 つで、phrase の中・`chords` 行・part-major の section・section-major の part block を*全部*同時に捕まえる**（11 冊はその 4 形に散らばっている）——**パーサの腕を N か所いじると、その N を数え違える**（本便が同じ量を 3 度数え違えた直後なので、なおさら）。★ **足場は在る**: `SyntaxNode.IsInside<T>()`（`SyntaxNode.cs:456`・`FormDeclarationValidator` と `CueRegionValidator` が既に使う）、`SyntaxFacts.IsMeasureBarlineKind`。⚠️ **禁止しないもの**: **lyrics の `[N. …]`**（`LyricVoltaGreen`＝別ノード）・**`repeat percent` / `repeat unfold` / `tremolo`**（順序を変えない）。 **第304 は最後に「1 パートだけの繰り返しは意図か欠陥か」を訊きかけたが、それは決定の帰結であって決定ではない**——**ユーザーに「music に `|:` を書けなくするのではなかったか」と指摘されて取り下げた**（RULES §5.1 に汎化）。

⚠️⚠️⚠️ ★★★★ **⒀ それでも第304 は着手しなかった。理由は外の障害ではなく*この便の後半の誤り率***: **同じ 1 つの数を 3 度続けて間違えた**（20 → 14 → 12 → 11。⒫ の射程）**うえ、その 1 時間前には「ラベル規則は 11 軒」も間違えていた**（17 か所）。**どれも再測定でしか見つからなかった。** ⇒ ★★★ **⒫ の 11 冊の切り直しは*間違えても静かに通る*仕事**（切り直した本はコンパイルもレンダもする——ただ違う音楽になる。しかも snapshot を再ベースすればそれが祝福される）。**その形の仕事を、同じ量を 3 度間違えた直後に始めるのは分が悪い。** ⇒ ★★ **これは「大きさ」ではなく「今の私の誤り率」を根拠にした NO で、外の障害を作らずにそう言うほうが正直**——**次便は数を測り直す必要がない**（`scratch/p304/count.py` が在る）。 ★ **独立した小粒**: **⒭（section ヘッダの `clef` が効かない）は手つかず。⒮ は測り直して書き換えたが着手していない**（⑻）——**⒮ も ⒫ の決定待ちになった。** ⚠️ **§2 U8 の残り**（`grace { }` 本体の和音・休符／スラー・梁・タイ／注釈）は**本便では 1 行も触っていない**。

★ **開始時裏取り**: HEAD `be16a770`・**未 push 0**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6701 / 0 / 4 / 6705**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2・`OPEN:` 0**・追跡 `.lys` **581 冊**・snapshot **231 枚**——**§1 の終了時の数と 1 つも違わなかった**（**10 個の数え方レシピは全部そのまま再現した。素朴に数えると `exact` 556／`OPEN:` 12／snapshot 0 が出る**）。⚠️⚠️ **未 push は 0 で、第303 の §1 は「未 push 45・`origin/master` は `11f48202`」と引き継いでいた**——**セッションの*あいだ*にユーザーが push している**（§0 の「両端だけでは足りない」の 2 例目。`origin/master` は `be16a770`＝第303 の最後の commit）。
終了時: **commit 16 本**（**⒬ 本体 `b06f6968` ／ 文法書 `96a965bf` ／ §1・§2・§3 とアーカイブ `b8954d85` ／ ubuntu 脚 `47ef56c3` ／ RULES への汎化 `515039e5` ／ 数の閉じ直し `dda6c577` ／ 委任への答えと ⒮ の測り直し `82246a9a` ／ ユーザー決定 2 件 `c979a4c5` ／ 移行の決定 `aaf3f47b` ／ **ラベル既定の反転と 11→1 の集約 `207956f3`** ／ 文法書 `d3df1288` ／ 数の閉じ直し `47e3c1fa` ／ 委任 2 度目への答えと「11 軒」の訂正 `ea815570` ／ 委任 3 度目・LP 本の決定・「1 パートだけの繰り返し」の発見 `f194e2f5` ／ 蒸し返しの取り下げ・数の収束・RULES §5.1 `73733b62` ／ ⒫ の住所と診断の置き場所（この commit）**）・**未 push 16**・木 0・未追跡 0。**Windows Debug 6750 / 0 / 4 / 6754**（開始 6705 から **+49**＝**⒬ の網 29 ＋ ラベル既定の網 20**）。Core 0 エラー 0 警告。**台帳・追跡コーパス・snapshot は開始時と同じ**（748／588／190／24.714570188／158 中 2／`OPEN:` 0／581 冊／231 枚）。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけ**——**定数は 1 つも足していない**。⚠️ **`82246a9a` は Core を*コメントしか*変えていない**ので掃きを取り直していない（§7.9）。**Windows Debug 6750 / 0 / 4 / 6754**（開始 6705 から **+49**＝**⒬ の網 29 ＋ ラベル既定の網 20**）。Core 0 エラー 0 警告。**台帳・追跡コーパス・snapshot は開始時と同じ**（748／588／190／24.714570188／158 中 2／`OPEN:` 0／581 冊／231 枚）。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけ**——**定数は 1 つも足していない**（`audit/magic_constants.csv` も不動）。
★★ **WSL ubuntu Release は 2 回とも Windows と同じ**（**⒬ のあと `b8954d85` の木で 6730 / 0 / 4 / 6734**、**ラベル既定のあと `d3df1288` の木で 6750 / 0 / 4 / 6754**。build 7 秒 / test 26〜27 秒）——**§0 の「この機械の Windows の緑は Windows の緑でしかない」の両脚を、本便は 2 度とも両方読んだ。** ⚠️ **CI は `be16a770` までしか走っておらず、未 push 12 本は未実行。**
★ **§5 へ出した恒久知識は 3 つ**: **§7.6 に「1 つの文を N 人が読む」の*3 つ目の数え方＝モード*（＋着手前の版「『同じ印を通すだけ』の起票は、その印が既に何通りに読まれているかを数えてから始める」）**、**§5.5 に「出力を正規表現で読む計器は、言語を固定し、その言語で偽陽性を出さないアンカーで読む。直したら 1 本だけ回して読む」**。
⚠️ **計器は `scratch/p304/` に全部残っている**: **毒は 2 組**——`poison.py`（**⒬ の 17 本**）＋`runpoison.ps1`／`poison2.py`（**ラベル既定の 8 本**）＋`runpoison2.ps1`、結果は `poison-results.txt` と `poison2-results.txt`。`sweep304.ps1`＋`sweep.json`／`sweep2.json`／`sweep3.json`／`sweep4.json`（**掃きは 4 回。引くのは 3 回目＝⒬ の出荷木と 4 回目＝ラベル既定の出荷木**）・A/B のプローブ（`u*` 相対と絶対の対・`w*` ending・`y*` phrase 本体・`x*` slash）。⚠️ **`scratch/` は git 管理外。** ⚠️ **`C:\MyProj\lys-base304` の worktree は次便が消してよい**（`git worktree remove`）。
