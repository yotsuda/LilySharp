...
### 1.1 第436セッション（2026-09-20・YT-DELL2）
...
`/clear` 直後の新セッション。指示は「**HANDOFF を読んで着手**」で、**着手先は §1.0 ⒜ の ⒴⁵**（ユーザーが
選んだ・⒜ の大きい 3 つと LP 双子を並べて訊いた）。★ **`-Start p435` の 1 コマンドで §0 が全部済んだ**
（HEAD `6477c6cc`・**未 push 55**・**full `sessions/p435/run1.trx` 8768 / 0 / 3 / 8771＝第434 最終と一致**・
台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・§7 3.5 の `-Archive 433` も自動で回って
69 行 4,090 chars）。★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` の
ままだから＝第412〜第434 が予告したもの）。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第434 と
同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）。
⇒ **起票は 1/8 の値段で閉じ、同じ計器が隣の腕を名指した。**

⚠️⚠️ **⑴ 起票の天井は*測った側*の天井だった。** ⒴⁵ は第417 が「`StackBelowStaffMemoized` の
Filter/remap/Rebuild＝天井 0.174%」の*内側*として起票したもので、**名指された無駄（artic と trill の
remap 辞書）は実測 1,056 B／打鍵＝0.013%＝その天井の 8 分の 1**（ほかに短い経路で捨てる空辞書 4 つ＝
582 B／打鍵 0.007%）。⚠️ **そして天井のほうは*below 側だけ*に掛かっていた**——**同じ 2 つの形
（`Filter`／`Rebuild`）は above 側にも在り、家族が 4 ではなく 11 で、`a.filter` 10,485 ＋ `a.rebuild`
17,337＝27,822 B／打鍵**（below は 6,525 ＋ 8,781＝15,306）。⇒ ★★★ **起票の「天井」は*その起票が
計器を入れた腕*の天井で、同じ形をした隣の腕には掛かっていない**（RULES §5.0 へ 1 本）。

★★★ **⑵ 直しは 1 つの形を両腕に**（`6c84eec1`・Core `+` 99 行・**数値定数 0・LP 由来の式 0・REF 0／OWN 0**
＝§7.6 ⒟）: ⑴ **`Filter` が live 部分集合を*寸法ちょうど*で建てる**——**生存数は歩かなくても分かる**
（`Collect` の述語と `Filter` の skip は*同じ述語*なので、落ちるのは hit した system の索引リストそのもの
＝`Collect` が既に建てている）。**hit を 1 パス舐めて全家族を数え**、builder は copy ではなく `MoveToImmutable`、
`map` は `List<int>`＋`ToArray` ではなく素の `int[]`／⑵ **remap 辞書は line group が読む 2 家族だけ**
（短い経路は空辞書でなく `null`）／⑶ **`Rebuild` は `b.ToImmutable()` → `b.MoveToImmutable()`**
（`ToBuilder()` は `Count == Capacity == Length` を残すので、`ToImmutable` の複写は家族 1 本ぶんの配列がまるごと捨て）。

★★★ **⑶ A/B ＝ −0.358%**（**7,971,391 → 7,942,820 B／打鍵**・before はこの便で取り直した）。継ぎ目は
**`b.filter` 6,525 → 335**・**`a.filter` 10,485 → 1,056**・**`b.rebuild` 8,781 → 4,413**・
**`a.rebuild` 17,337 → 8,771**・**`b.remap.dropped` 1,056 → 0**・**`b.remap.empty` 582 → 0**で、
**`a.part`／`a.prog`／`b.part`／`b.prog` は 1 バイトも動かない**（＝触っていない家の対照）。
**冊ごとは 231 冊中 230 冊が改善・中央値 −0.197%・最良 −3.406%**で、**上がった 1 冊は +0.005%＝
同じ木を 2 回回した対照帯（−0.035%〜+0.026%）の内側**。

⚠️ **⑷ 予測は安全側に外れ、外れ方が所見だった。** 実装前に書いた予測（Lab `sessions/p435/prediction.txt`）は
**−0.29%〜−0.31%** で、**`Filter` は「hit が多数派だから live はその小さい割合」で 30〜40% に落ちると見た**。
**実際は below 5%・above 10%＝live 部分集合は「小さい割合」ではなく*ほぼ空***（`Rebuild` の −45% は当たり）。
⇒ **`Filter` は「家族まるごとの大きさの配列を 2 枚建てて、ほぼ空の答えを返して」いた。**
⇒ **残り 14,575 B／打鍵 0.183% は*実仕事*＝この 2 つには戻らない**（§1.0 ⒞）。

★★ **⑸ 出力同一は 3 つとも**: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline `17fb650e`）／
⑵ **実コーパス全ページ SHA-256＝5,824 行・0 行差**（第434 の `hashes-after.txt` と突き合わせ）／
⑶ suite 全緑・snapshot 249 枚不動・台帳 851 点／総和 22.584727806 不変。

★★ **⑹ 網 3 本（`OutsideStaffStackLiveSubsetTests`）・毒 3 つ・被害者の見分けつき。**
⚠️ **below の front は*どちらの半分にも*単体の観測者が居なかった**——既存の memo 網は
**2 つの system が両方 hit か両方 miss** なので、**部分 hit の索引の算術が一度も走らず**、
**`AlignedLineGroup` はテストに 1 件も無かった**（実コーパスも作らない＝`b.groups` は 1,848 打鍵で 0 B）。
毒: ⑴ **生存数の引き算を落とす** → `MoveToImmutable` が投げて **below 4 本赤**（新 2・既存 2）、
同じ毒を above の腕に当てると **7 本赤**／⑵ **dyn/hp の remap を建てない** → **line group の網 1 本だけ**／
⑶ **またぐ group の forced-live の門を外す** → **その門の網 1 本だけ**。**単独 `--filter` で確かめた。**

⚠️⚠️ **⑺ 冊ごとの A/B で「193 冊悪化・最悪 +45%」という*嘘*を 1 度出した。** 比べた baseline が
**Lab `sessions/p434/ab-after-1-tc0.txt`** で、**そのファイルの見出しは `books 235 / keystrokes 1880 / threw 0`
＝別母集団の run に上書きされていた**（第434 の *before* は 231／1848／threw 4、第434 §1 が書いた after
7,971,322 はこのファイルのどこにも無い）。**自分の木で before を取り直したら 230/231 改善に反転した。**
⇒ ★★★ **A/B の baseline ファイルは、比べる前に*ファイル自身の見出し行*（冊数・打鍵数・threw）を読む。
前便の §1 が書いた数と一致しない baseline が Lab に在りうる**（RULES §5.3 へ 1 本）。⚠️ **同じ木を 2 回
回した対照が ±0.035% しか振れないことも同時に測った**＝**冊ごとの割当は*同じ便の中では*安定**で、
第418 の「冊ごとは正しくない」は*便をまたいだ*ときの話。

⚠️ **⑻ §7 9 の「計算を足したか」で初稿を書き直した。** 最初は `Filter` が家族ごとに `hits` を舐めており、
**above 11 家族 × hit 22.6 × 2.17 回＝打鍵あたり約 540 回の辞書引き**——**歩く item 自体が 120.6／打鍵**
なので**歩きの 4.5 倍**だった。**hit を 1 パス舐めて全家族を同時に数える形に畳んで約 49 回／打鍵**
（第422 の「鍵を持つ側で 1 度やる」と同じ形）。**割当は中立**（畳む前後の帯 −0.025%〜+0.029% が
対照帯の内側）。

★ **⑼ 終了時**: commit 2 本（code `6c84eec1`・docs 1 本＝この §1 と RULES §5.0 ×2・§5.3 ×1。**この文を
含むので SHA は書かない**＝§5.4）・**未 push 57**（開始時 55）・**最終 full `sessions/p435/run5.trx`
8771 / 0 / 3 / 8774**（**第434 最終の合計 8771 から +3＝今便の網ちょうど 3 本**。`Zz435`／`ZZPOISON` の
grep は**コードに 0 件**）・§7.5（対 `6477c6cc`）**Core `+` 98 行／REF 0／OWN 0＝算術を 1 つも足していない**
（§7.6 ⒟「呼び手が既に持っていた数を渡しただけ」）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚
不動・追跡 `.lys` 609・**棚卸しは `APPROXIMATIONS.md` 3 行・`magic_constants.csv` 16 行とも*行番号だけ*
＝定数の増減 0**（`LILYSHARP_UPDATE_DOCS=1` で再生成）。⚠️ **§7.7 の「同じ量の 2 つ目の綴り」を 1 つ
作った**——生存数は `Collect` の述語の 2 つ目の綴りで、**畳めない**（2 つは違う時刻に生きている）ので
**毎回の呼び出しで突き合わせる**形にした（食い違えば `MoveToImmutable` が投げる＝1,848 打鍵と suite 全部で
一度も鳴らなかった）。計器と全文は Lab `sessions/p435/`（**`prediction.txt`＝実装前の予測**・
**`stacker-counts-before.txt`／`stacker-counts.txt`＝継ぎ目の前後**・`Zz435.cs.txt`／`Zz435Leg1.cs.txt`＝計器・
**`Zz435Ab.cs.txt`＝再利用可の A/B**・`Zz435Hash.cs.txt`＝証明 ⑵ の harness・`ab-before-1-tc0.txt`／
`ab-after-1-tc0.txt`／`ab-after-2-tc0.txt`＝対照／`ab-after-3-folded-tc0.txt`＝出荷版・
`OutsideStaffStacker.cs.after`）。⇒ ⚠️⚠️ **HANDOFF は第434 の記録 442,177 B → `-Archive 433` 後 434,660 B →
今便を書いて 445,370 B＝天井 450,000 まで残り 4,630 B**（`Fold-ClosedHandoffItems` の弁は第426 の時点で 0 件・
§3 の圧力弁は第409・第410 で使い切り。**天井は上げない**）。⚠️⚠️⚠️ **次便は §1 に 1 字も足せない**
＝**`-Archive 434` を回すまで書き始めないこと**（回せば今便の下のブロック約 8 KB が空く。**途中で回すと
`HandoffArchiveContinuityTests` が 3 本赤くなる**＝第410 ⑾）。⇒ **次便は `-Start p436` の 1 コマンドから入る**
（裏取り → `-Archive 434` 自動 → §1.0 の一覧）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push される
まで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第435 の欠陥ではない。**作業ツリーは空**。
**push はユーザー**（Lab も）。
