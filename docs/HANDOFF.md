...
- ★★★★ **⒮⁵ `Parser/GreenRun.cs:52` の `more`＝18,821 B／打鍵 0.41%**（第448 実測・**214.68 回／打鍵で
  99.6% がちょうど 1 件**＝run は*ちょうど 2 本*）。**第448 の直しが作った新しい頭**で、**直し方は同じ技を
  もう一段**＝`Take` に「2 本目も手で持ち、3 本目で list を建てる」を足す（`first`／`second`／`more`）。
  ⚠️ **呼び手 4 か所の仕上げ式（`[first!, .. more]` 等）も同時に直す＝単独では入らない**
- ★★★ **⒮⁶ 「建てない」の島の残り＝92,144 B／打鍵 2.01%**（第448 実測・381 軒。122,756 → 92,144）。
  ⚠️ **上位 4 軒 23,140 は*門前払いと実証済み*＝器が呼び手へ出る**（`ItemSkylineFactory.cs:465` 9,118／
  `MultiStaffLayouter.cs:754` 7,082＝builder ごと返す／`HorizontalSkyline.cs:63` 4,171＝field を `Merge` が読む／
  `BeamSubdivision.cs:253` 2,769）。**⒮⁵ の次に手が動くのは `PageLayouter.cs:882` 3,832（100% 1 件）・
  `MeasureContentKey.cs:531` 3,166（76.4% が 0 件＝`List<long>?[]` にする）・`DotColumn.cs:279` 2,709
  （92.7% 1 件＝`OffsetX` が `ReadOnlySpan<Support>` を取れば local に置ける＝第448 が意図的に外した）**。
  現在地は Lab `sessions/p448/site-prices-after.txt`（脚は `instrument.ps1`＋`Zz448.template.cs`＝1 run で全サイト）
...
- ⚠️⚠️ **`docs/RULES.md` は 246,027 / 250,000 B・1,854 / 2,000 行＝天井の 98%**（第448 実測）。**第449 は
  ルールを 1 行足すのも難しい**＝先に §5 の実例を `RULES-CASES.md` へ出すこと（第433 がやった形）。

### 1.1 第448セッション（2026-09-21・YT-DELL2）

`/clear` 直後の新セッション。指示は「**HANDOFF を読んで着手**」で、**着手先は §1.0 ⒜ の ⒮′**（ユーザーが
選んだ・⒮′ と ⒮‴＋⒮″ と ⒭′ と LP 双子を並べて訊いた）。★ **`-Start p448` の 1 コマンドで §0 が全部済んだ**
（HEAD `db270011`・未 push 4・full `sessions/p448/run1.trx` 8774 / 0 / 3 / 8777・台帳 851 点／
総和 22.584727806・snapshot 249・追跡 `.lys` 609・`-Archive 446` も自動）。**裏取りは 1 つも赤を出さなかった**。
道具は pwsh MCP（Bash 0 回）。CI は緑のまま。

★★★★ **⑴ 起票の処方箋が外れていて、外したのは*平均*だった。** ⒮′ の「**寸法を言っても消えない＝建てない
だけが効く**」は正しい（`obj` は文字どおり `回数 × 器の object`＝461.70 × 32 = 14,774 が 1 B も違わない）。
外れたのは頭の直し方で、**「92% は 1 件以上入る＝遅延生成は効かない」は*平均 1.37 件／回*から出た読み**。
⇒ **計器の `Add` に 0／1 を数える 2 行を足したら、`octaveMarks` は 48.3% がちょうど 1 件・7.6% が 0 件**＝
**「最初の 1 本を手で持ち 2 本目で建てる」が 55.9% の建てを消して 20,731 B／打鍵**＝処方された
「lookahead で寸法を言う」（8,606）の **2.4 倍**。★ **その直し方は第447 が書いた `GreenRun.Take` で、
同じファイルの 35 行下で呼ばれていた**。⚠️ **⒮′ の「2.32%」は分母が render だけ**＝計器は parse も
括っているので**正しくは 2.151%**（14例目の再演）。

★★★★ **⑵ 島は起票より大きい。** `Core` 全体で **建ての 22.2%（989,132 回）が 0 件・27.4%（1,221,919 回）が
ちょうど 1 件＝建ての半分**、**合わせて 122,756 B／打鍵＝2.65%**（object の島 99,628 より大きい＝0／1 の
建ては器だけでなく*敷いた配列*も払うから）。**300 軒が非ゼロ・top10 で 57.7%・top20 で 70.9%**。

★★★ **⑶ 直したのは 14 軒**（器が呼び手へ出ないものだけ）: `ParsePitch` の octaveMarks（`GreenRun.Take`）／
`AccidentalPlacement` の ape 列（99.9% 1 件）／`MultiStaffLayouter` の StaffSpring（76.6% 1・23.4% 0）／
**⒮‴ の 9 軒**（100% 空。⚠️ **起票の「約 6,600」は実測 6,692＝1.4% 以内で当たっていた**）／
`SvgSystemFragmentCache` の `outerIds`（100% 空）／`MusicMarkEngraver` の chord band memo（99.8% 空）。
**A/B は parse −5.57%（372,202 → 351,471）・render −0.63%（4,260,330 → 4,233,657）・合計 −1.02%**。
⇒ ★★★ **parse の −20,731 は計器の予測と 1 B も違わない**。render は −26,673 に対し会計 27,645＝**96.5%＝
*下*に外れた**（第445・第446 は上に外れていた＝「隣に座っていた」形は今回は無かった）。

★★★ **⑷ 出力同一は 2 つとも**: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**／⑵ **実コーパス全ページ
SHA-256＝5,824 行・0 行差**（baseline `p439/hashes-after.txt`）。

★★★ **⑸ 毒 11 本——10 本は予測どおり、1 本が予測を割った。** 赤: 2 本目以降を落とす **+198**／1 件だけの
run を落とす **+636**／最初の 1 本を二重に数える **+190**／ape 列を再利用しない **+2**／全部同じ列にする
**+26**／唯一の spring を捨てる **+50**／2 本目で最初を忘れる **+5**／TieVariant の builder を捨てる **+16**／
Pedal の bracket を捨てる **+16**。緑（予測どおり）: chord band の memo が一度も store しない **+0**
＝**memo は memo であって判断ではない**。
⚠️ ★★★ **予測を割った 1 本＝「octave mark の run を逆順にする」が +3 赤**（緑と予測した）。**trivia は
token に張り付いている**ので、run の*最後*のマークが後ろの空白を持つ＝`c,, |` が `c, ,|` になる。
⇒ **同じ綴りのマークでも可換ではない**。見ていたのは round trip の 2 本と `PartSectionLayoutConverterTests`
の 1 本（`c,,1~@mark("A") c,, |`）。

★★★ **⑹ 直した後の島**: 建て数 **2,410.5 → 2,017.3／打鍵**・objects **99,628 → 81,693**・
「建てない」の島 **122,756 → 92,144（2.01%）**。⚠️ **建て数の落ち 393.2 は、サイトごとの 0／1 の取り分の
和 392 と 0.3% 以内で一致**（起票時に「回数まるごと」を引いた予測 660 は外れ＝*言い方*が外れて機構は当たった）。
⇒ ★★★★ **新しい頭は自分が作った**——`Parser/GreenRun.cs:52` が **214.68 回／打鍵・99.6% がちょうど 1 件**
（run は*ちょうど 2 本*）＝**18,821 B／打鍵**。**同じ技をもう一段**で消える＝§1.0 ⒮⁵。

★ **⑺ 終了時**: commit 2 本（code `ddd88ef5`・docs 1 本＝**この文を含むので SHA は書かない**＝§5.4）。
**最終 full 8774 / 0 / 3 / 8777＝開始時と同値**・§7.5（対 `db270011`）**Core +153 −46 行／12 ファイル／
REF 0／OWN 0**（足した式も定数も 0＝§7.6 ⒟。⚠️ **`0.0` の初期化を 1 つ足して `magic_constants.csv` に
載ったので `double?` に畳んで消した**）・**未 push 6**・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚
不動・追跡 `.lys` 609・**棚卸しも `magic_constants.csv` も net 新規 0／net 消滅 0＝行番号だけ**。
全文は Lab `sessions/p448/`（罠 3 つは `notes/CLAUDE-OPERATIONS.md` §1 へ）。**push はユーザー**（Lab も）。
⚠️⚠️ **天井の残りは 881 B＝次便は `-Archive 447` を回すまで §1 に 1 字も足せない**。**`-Start p449` の
1 コマンドから入る**。
