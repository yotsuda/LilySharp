...
終了時: **commit 3 本**（**本体 `36988c3a` ／ §1・§2・RULES `1f467e80` ／ 数を閉じる（この commit）**）＝**未 push 28**（**この commit を含む。commit 2 本の時点で 27 だったのを確かめてから書いた**）・木 0・未追跡 0。**suite 6670 合格 / 0 失敗 / 4 skip / 合計 6674**（開始 6658 から **+16**＝**本便の網 16 ケース**）。Core 0 エラー 0 警告。**台帳・追跡コーパス・snapshot は開始時と同じ**（748／588／190／24.714570188／158 中 2／`OPEN:` 0／581 冊／231 枚）。
★★ **WSL ubuntu Release も同じ 6670 / 0 / 4 / 6674**（`1f467e80` の木＝**この最後の commit は `docs/HANDOFF.md` の 1 行しか変えていない**）。⚠️ **CI は `11f48202` までしか走っておらず、未 push 28 本は未実行。**
⚠️ **`C:\MyProj\lys-base301` の worktree はこの便の最後に消した**（`git worktree remove`）。**次便が同じ名前で建て直してよい。**
⚠️ **計器は全部ディスクに残っている**: `scratch/p301/` の `ab/`（A/B の本と出力）・`lp/`（LP 2.26.0 のプローブ）・`poison.py`＋`runpoison*.ps1`＋`poison-results*.txt`・`sweep301.ps1`＋`sweep.json`・`measurements.md`。⚠️ **`scratch/` は git 管理外。**
...
- **U8. ▶ 4 分の 3 まで閉じた（第298 ＋ 第299 ＋ 第300 ＋ 第301）＝`grace { }` の本体は*歩かれない*。起票の「注釈を運ばない」より広かった**
  ...
  **`ProcessGraceNotes` が主旋律の `_defaultDuration` を共有していた**ため。
  ...
  ⚠️ **MIDI の*主旋律*の phrase 展開には予算が無い**——**別の穴で、第301 は触っていない。**
