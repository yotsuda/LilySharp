...
最終更新 第365セッション（2026-09-10）＝**入り方は第298〜第364 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・口挟み 0）。第364 ⑺⒝ の**拾い箱 ①〜⑤ のうち ②③④⑤ を実装・① は見送り**（chord quality 35 種＝「7 度×4 形のまま・行数はユーザー判断」と第364 が書いたとおり、ユーザー不在で決めない）。**製品＋番人＋CHANGELOG＋GRAMMAR＋この HANDOFF を 1 commit（snapshot 0＝承認ゲート無し）・push はユーザー**。**骨は 5**:

★★★ **⑴ 裏取り＝拾い箱の 4 項は全部本物・第364 の「`@8va` は項目にしない」は半分違った**。実コード: **`8va`／`8vb`／`15ma`／`15mb` は lexer が 1 つの Identifier に読む**（`Lexer.GluedOttavaSuffixLength`＝`transposition 8vb` のため）→ `ParseArticulations` の `IsArticulationName` を通り `MusicMarkItem.ParseMarkName` が `ottava` と同じ mark に読む＝**`c4@8va … @!8va` は書ける**。第364 は「`ExtraSearchTerms` の "8va" で `ottava` に届く」と言ったが、**検索語は行を見つけるだけで挿入するのは `ottava`**＝綴りとしては未露出だった。／`tab NAME |`＝`ParseTabRender` が `ConsumeAsSelector`（`as numbers|full`）を読む・staff／chords の行には `as` 継続の文脈が在り tab には無かった。／**GRAMMAR §1 は「'using' is reserved for multi-file support」と言うだけで `using` の production が無かった**（`ParseUsingDirective`＋`UsingExpander` は実装済み）。／**GRAMMAR SectionItem に `override` が無かった**（`ParseSectionItem` は取り・stray message も「(key, time, tempo, partial, override)」と名指す）。／`size` は `PaperPlanReader` の中で `"size"` リテラル 3 か所（`AllKeySpellings`・`Canonical` の fallback・`ApplySize` の分岐）。

★★★ **⑵ 形＝Core は語彙の公開 3 本・LSP は文脈 2 つ**。Core: **`SyntaxFacts.ScoreItemKeywordVocabulary`**（15 語＝`ParseRenderItem` の keyword 枝・素の part 名は除く・LSP の並び順）／**`SyntaxFacts.SectionSettingVocabulary`**（partial・key・time・tempo・override＝**`ParseSectionItem` の stray message がそれを `string.Join` で読む**＝parser と popup が 1 表）／**`PaperPlanReader.SizeKey`**（3 か所のリテラルを 1 定数に）／`LanguageVocabulary.ScoreItemKeywords`・`SectionSettings`・`PaperSizeKey`。LSP: **`AfterTabTuningRef`**（`tab TUNING |`＝part＋`as` selector・`bass` は tuning でもあり part 名でもある＝`AfterStaffClefRef` と同型・**旧 `tab TUNING |` → AfterTabRef を差し替え**）・**`AfterTabAttachName`**（`tab NAME |`・`tab TUNING NAME |`＝`as numbers`／`as full`＋score 継続＝`AfterStaffAttachName` と同型）。リスト: `GetScoreBlockCompletions` は `ScoreItemKeywords` を読み `ScoreItemDetails`（insert・prose・retrigger）は辞書＝知らない語も `kw $0` で出る／`SectionHeaderDirectiveItems` は `SectionSettings` を読み `SectionSettingDetails` は辞書／`size` 行は `PaperSizeKey`／`GetKeyTonicCompletions` の 15 語は選抜のまま（remark に理由）・**detail の「N ♯／♭」は `KeySpelling.SharpsFor(tonic,"major")` から**／`GetArticulationCompletions` に `8va`・`8vb`・`15ma`・`15mb` の 4 行（sort は `4ottava.8va` 等＝長い綴りの隣）／`TabStyleSelectorItems` は `TabStyles` から（`TabStyleDetails` を `GetTabDisplayModeCompletions` と共有）。docs: GRAMMAR に `UsingDecl = 'using' , String ;`（TopLevelItem にも）・SectionItem に `OverrideDecl`・§1 の文を「implemented, top level only: LYS0029 elsewhere」に。⚠️ C# の罠: `cond ? namedTuple : (a, b, c)` は**名前を落とす**（tuple literal の自然型が勝つ）＝受け側を `(string Insert, …) d = …` と明示。

★★★ **⑶ 番人＝+14**: `CompletionAuditTests` +13（contexts 4 caret＝`tab bass5 `→AfterTabTuningRef・`tab bass `・`tab melody `／`tab bass5 melody `／`staff m  tab melody `→AfterTabAttachName・**既存 1 行を差し替え**／`TabRow_…`＝selector 行が `TabStyles` と一致し `tab m as X`・`tab bass5 m as X`・`tab bass`（part 名 bass・`instrument bass`・`r1`）が compile／`ThePrintedOttavaSpellings_…` ×4＝`c'4@8va … f'@!8va |` が compile／`ScoreBody_…`＝label 列＝`ScoreItemKeywords`・part を取る 5 語は retrigger・brace 5 語は無し／`SectionHeader_…`＝part-major header の popup＝`SectionSettings`・**stray message（`section A { "oops" … }` の LYS0030）が 5 語を名指す**・5 語が part-major header と section-major の cell の隣で compile（bar 長の警告だけ除外）／`PaperSizeRow_…`／`KeyTonics_…`＝c 0・cis 7♯・fis 6♯・ces 7♭・f 1♭＋15 語 compile）＋**`DocKeywordListTests.TheScoreItemVocabulary_IsWhatTheParserAccepts`**（語彙 sorted == `KeywordScoreItems()`＝SYNTAX_REFERENCE の予約語を 1 語ずつ score body に入れて枝の有無を測る既存の機械＝語彙は parser に両方向で縛られる）。`TabDisplayCompletionTests` の架空 tuning `drop-d` → `bass5`。`AtCompletionParsesTests.EveryAtCompletionItem_ParsesAndIsRecognized` は 4 行をそのまま通した（`c4@8va` が parse され unknown にならない＝⑴ の裏取りの陽性対照）。

★★★ **⑷ 数と射程**: **開始時**（`Session-Check -Build -Test -Scratch p366`）: HEAD **`562a0a09`**・未 push 16・`origin/master`＝`ba70753f`・木 clean・未追跡 0・`gh run list` **赤**（34416221197＝第361 ⑷⒝ の赤・以後 push 無し）・build 0 エラー／Core 0 警告・**full `scratch/p366/run1.trx` 7655 / 0 / 4 / 7659（第364 末 run3 と一致・RunInfos は SKIP 4 だけ）**・台帳 816・snapshot 249・追跡 `.lys` 599・ディスク 21827。**終了時**: 補完系 645 / 0（+14）・inventory／Provenance／DocExamples 170 / 0（APPROXIMATIONS は触ったファイルを載せていない＝stale にならず）→ HANDOFF／CHANGELOG の後に **full `run2.trx`＝下 ⑻**。§7.5（作業ツリー・`git diff -- LilySharp.Core`・commit 前なので `-DiffBase` は 0 と出る）**Core `+` 69／REF 0／OWN 0**（語彙の公開と定数化だけ・式は 1 本も無い＝LP から導いた量は無い）。台帳 816／snapshot 249／追跡 `.lys` 599 不動（出力を変える変更は無い＝掃きは回していない・補完と doc だけ）。CHANGELOG 1 節（Editor）。⚠️ `git worktree`: p362／p363／p364 の wt-base が残ったまま（この便は足していない・`git worktree remove` で外す）。⚠️ scratch は **`p366`**（p365 は第364）。⚠️ §7 3.5: 第363 のブロックを `-Archive 363` で ARCHIVE の先頭へ（済）。⚠️ 記憶: 新規無し（`reference_lilysharp_session_number_vs_scratch` が今回も当たった）。

⇒ ★★★ **⑺ 次の一手**: **⒜ push はユーザー**（未 push 17）。**⒝ 拾い箱の残り 1**: ①chord quality 35 種（`ChordStructure`・補完は 7 度×4 形＝**popup の行数 7×35＝245 をどうするかはユーザー判断**・道具は要らない）。**第 2 便は着手しない**（同じ指示が来たら）: ① はユーザー決定、⒞ は LP 計器（双子と cairo png）が要り、この便の道具（scratch/p366 には trx しか無い）と無関係。**⒞ 第363 ⑺⒝③（title 残差 +0.21）・④（`raggedLastBottom`＝OFF にする値付き flag の形が要る）、第362 ⑹⒝②〜④、第361 ⑺ の並びはそのまま**（第363 の本文は ARCHIVE の先頭）。

**⑻ 終了時**: **full `scratch/p366/run2.trx` 7669 / 0 / 4 / 7673（+14＝番人・HANDOFF／CHANGELOG 込み・RunInfos は SKIP 4 だけ・6 m 26 s）**→ commit（Core 4＋Lsp 3＋Tests 3＋GRAMMAR＋CHANGELOG＋この HANDOFF＋ARCHIVE）→ `Deploy-Lsp.ps1`（server だけ＝Reload 1 回）。push はユーザー（未 push 17）。
