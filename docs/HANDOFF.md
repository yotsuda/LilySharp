...
- **⑼ ⚠️ 次に触るならここ**（**分類は済んでいる。残っているのは決定と、決定の要らない 1 件**）:
  **⒜ 移調 clef の実音オクターブを MusicXML が持たない**（**44 冊・要ユーザー決定**・§2F ⒜）／
  **⒝ `repeat unfold` の相対の意味**（**要ユーザー決定**・§2F ⒝）／
  **⒞ 2 つ目以降の score が MIDI・MusicXML・双子に出ない**（**要ユーザー決定**・§2F ⒞）／
  **⒟ 小節途中の `clef` がページの相対枠だけを付け替える**（**3 冊・要ユーザー決定**・§2F ⒟）／
  **⒠ ★ 決定不要で残っている唯一の項＝2 冊の fixture が*意図しない音楽*を彫っている**（§2F ⒡）。
  ⇒ ★★★ **⒜⒝⒞⒟ は 4 便続けて「決定待ち」で据え置かれている。**
  **次便が最初にすべきなのは着手ではなく*ユーザーに 4 つ訊くこと*で、
  それをしない限りこの 4 項は永久にこの表に残る。**
