...
最終更新 2026-07-31（第52セッション＝**島②の残り半分「覆われたステムの供給」を移植して閉じ、
そのあと ▶ の次点を測ったら「欠陥ではなかった」**。
HEAD **`74460573`**（＋この handoff commit）・**未 push 23**・
テスト **3593 passed / 0 failed / 3 skipped**・
台帳 **253 点**（**ss 非ゼロ 72・総和 0.108590402** ＋ **count 点 2**）。**新しい 6 点は全部 exact**）。
⚠️ **この行は書いた直後に stale になる**（HEAD を書く commit が HEAD を動かす）。§0 のとおり
**開始時に必ず実測すること**——第52セッションは終了直前の点検で**自分の書いた HEAD が
2 commit 遅れている**のを見つけた（同じ形の嘘を、同じセッションで引継ぎから 2 件見つけた直後に）。

⚠️★★ **引継ぎが渡してきた数が 2 つとも嘘だった**（§0 の「数え方も書く」の 2・3 例目）:
・**「非ゼロ 74・総和 4.108590402」は `unit: count` の 2 点（各 −2）を ss の総和に足していた**
  （`lyrics.hara-kiri.*.staves-on-first-page`）。台帳自身が「count を ss の総和に入れるのは
  *悪い数*ではなく*無意味な数*」と書いている（`LpGeometryLedgerTests` の `Unit` の doc）。
  **正しい数え方は §0 に追記した。**
・**「`StemCollisionFactor` に読み手がゼロ」も stale**——**プロパティ自体が `9cbb407c`
  「Dead-code sweep」で削除済み**だった。書き直した（今度は読み手がある）。

★★★ **移植した（`613216cc`・snapshot 0 枚・全緑）＝`beam-quanting.cc:394-418`**。
覆われた grob は**2 回**予約される: 自分の箱と、**自分の `stem`**——`Stem::chord_start_y` から
**符尾方向へ無限に伸びる区間**（重み `STEM_COLLISION_FACTOR` 0.1・**自分のビームを持たない
符尾なら 1.0**）。**無限なのは意図**で、隣のビームを量子化している最中に覆う側の符尾の長さは
まだ決まっていないから、LP は半平面を予約して 1/10 に割り引く。

| 点 | LP | 前 | 後 |
|---|---|---|---|
| `beam.quant.over-stem.beamed` | 5.81 | −2.810000 | **0 exact** |
| `beam.quant.over-stem.unbeamed` | 5.81 | −2.810000 | **0 exact** |
| `beam.quant.over-stem.free`（対照） | 3.00 | 0 | **0・不動** |

⚠️ **予測を 4 枝書いて 4 枝とも当たった**（2 点が 0 へ／対照は不動／
`beam.quant.over-other-voice.covered` も**不動**＝全音符は `Stem` grob を持つが**normal でない**
（`is_normal_stem` は duration-log ≥ 1）ので**あの本はこの regime に最初から入っていない**）。

★★★ **「どの regime で効くか」はコードを読んでも出ない。LP 側に恒等対を作らせて測った**
（§5.3 に手順を追加）。`\override Beam.details.stem-collision-factor = #0` と既定 0.1 を
**同じファイル・同じ音楽・同じ run** で並べると、差は**この供給そのもの**になる。
**LP 自身の `input/regression/beam-collision-opposite-stem.ly`（16 冊）のうち 6 冊が動く**
（最大 **5.0 ss**）。⚠️ **読みだけで出した「箱に支配されるから効かない」は半分正しく半分誤り**
——覆う符頭の**反対側**では確かに箱に支配され、**符尾側**では全候補が同額を払う定数になるが、
**候補が `chord_start_y` をまたぐ本**では効く。それが meshing regime。
⚠️ **効かない場所も測って書いた**（新プローブのヘッダ）: 全音符の上（`beam-over-other-voice`）と、
**覆う符頭が x span の端**にある本（符尾の x が描画 segment の外＝`beam_y_` が空で返る）。

★★ **`columnX + (up ? StemUpAttachX : StemDownAttachX)` の 7 綴りを 1 軒に**
（`LayoutUtilities.StemAttachX(up, headScale)` / `StemX(columnX, up)`）＝第51セッションが
「次に beam か stem を触るときに」と書いた宿題。**観測者も同時に入れた**
（`BeamStemFrameTests`・**摂動して落ちることを確認済み**）。⚠️ **その点が守るのは
「ビームの描画端＝外側ステムの ±半太さ」**（`beam.cc:631`）で、**量子器と renderer が同じ
綴りを読むこと自体は 1 軒しか無いことで構造的に保証した**。

⚠️ **perf は訊かれる前に測った**（供給を 1 件増やしたので）＝**「計測不能」**（退行なしではない）:
2 声 meshing 200 小節の合成本で **base 6454.5 / 6494.7 / 6727.3 ms 対 HEAD 6536 / 6816.6 / 6410.8**
（min-of-5 × 3 セット・Release CLI）＝**HEAD が最小値を持ち、帯が完全に重なる**（§5.3 の帯）。

⚠️★★ **コーパスは 1 byte も動かなかった**（snapshot 0 枚）＝**fixture に meshing する本が 1 冊も無い**。
**byte 不変は移植の証拠にならない**——証拠は上の 3 点だけ。
⇒ **候補作業**: `test/` に meshing の本を 1 冊入れる（出力が増えるので GO ゲート）。

★★★ **そして「▶ の次点」は測ったら欠陥ではなかった**（`dd6f1b73`・**コード変更ゼロ・3 点 exact**）。
引継ぎは「`dense-chromatic` 第2小節の和音は符尾方向が LP と逆（LP 上・Lily# 下）」と書いていたが、
**LP は 4 本とも下**（heads `(1 3 5) (2 4 6) (3 5 7) (3 5 7)`）で、**両engine とも
`(-1.81 . -1.19)`**。**この本に乖離は無い。**
⚠️★★★ **誤診の機構のほうが所見**（§5.0 罠 17 に一般化して追加した）——この fixture の
**第1小節は 4/4 に 16分が 12 個**で、**Lily# の `|` は小節線**だが**LP の素の `|` は小節*チェック*で
小節位置をリセットしない**。`|` を残したまま写すと LP は**4 つの和音を 3/4 地点から始まる 2 群**に
beam する＝**別の音楽**で、その beam を Lily# と比べても何も言っていない。
⇒ ★★ **「両側が同じ音楽か」の確認項目に「小節線がリセットするか」を足した**（従来の
①音高 ②小節線の*字面* ③紙面 ④spacing のどれでもない 5 つ目）。
★ **falsify したあとも対は残した**——`beam.quant.chord.{left,right,single-note-control}`。
**コーパス初の「和音の上のビーム」の点**（符尾は一方の端の符頭に付き、長さは反対端から測る）で、
**LP が対照と完全に同値**＝**恒等の対**なので、以後 Lily# がこの 2 読みをずらしたら定義により欠陥。

★ **ついでに第50セッションが残したリリース債務を 1 件閉じた**（`361ca8ae`）——
**`LYS0014` を 2 つの診断が持っていた**（`KeyModeAssumedMajor` と `UnexpectedCharacter`）。
`UnexpectedCharacter` を **`LYS0018`** に（**`LYS0013` は退役のまま・再利用しない**）。
⚠️ **原因は配置**——`LYS0014`〜`0017` が**ファイル末尾の `LYS7xxx` の見出しの下**に足されていて、
**自分の帯の次の空き番号が視界に無い**。帯に戻した。**網も入れた**
（`DiagnosticCodeTests`・**重複を戻すと落ちることを確認済み**）。⚠️ **この欠陥は何も壊さない**
（両方 fire するし、呼び手は全部シンボルを見る）——**症状はユーザーが 2 つの無関係な
メッセージを区別できないことだけ**。だから網が要る。

▶ **次の一手＝内側メンバーの beamlet が `min` で消える**（第50セッション⑤の残り）。
**LP 側は第52セッションで実測済み**（`audit/lp-geometry/probes/beam-beamlet.ly`・`Stem.beaming` を
6 パターンで dump）。**次のセッションは C++ の読みからではなく、この表から始めてよい**:

| 本 | count | LP の内側 stem (left . right) | Lily# の `min` | |
|---|---|---|---|---|
| A | 1 **2** 1 | **(1 . 2)** | (1 . 1) | ⚠️ **乖離＝報告された欠陥** |
| B | 2 **1** 2 | (1 . 1) | (1 . 1) | 一致 |
| C | 2 **2** 1 | (2 . 1) | (2 . 1) | 一致 |
| D | 1 **2** 2 | (1 . 2) | (1 . 2) | 一致 |
| E | 2 **1 1** 2 | (1 . 1) 両方 | 同じ | 一致 |
| F | 1 **3** 1 | **(1 . 3)** | (1 . 1) | ⚠️ 乖離・**chip は 1 本ではない** |

⇒ ★★ **乖離するのは「自分の count が*両隣とも*上回る」ときだけ**。そこで LP は
**片側に満額を残し**、反対側だけ `max(count(i) − count(隣), 1)` 引く（`:169-183`）。
`min` は**両側を落とす**ので stub が消える。**B〜E が一致するのは偶然ではない**——
片隣だけ上回る場合、LP の結果は `min` と同値になる。
★ **規則そのもの**（`:121-183`・1 ≤ i < n−1、tuplet 境界でなく `count(i) > min(左, 右)` のとき）:
```
point_right = (右 count > 左 count)                          … 左右が違えば
            = (start_moment == cur_beat)                     … 「拍頭で始まる」と
                                                                「次の拍で終わる」の
                                                                ちょうど一方だけ真なら
            = rhythmic_importance(i) < rhythmic_importance(i+1)  … それ以外
```
そのあと補正 pass（`:161-167`・LEFT と RIGHT に挟まれた CENTER を寄せる）→ chip。
⚠️★★★ **見積りの要点**: **A（`8-16-8`）は 3 つ目の枝に落ちる**（左右とも 1 で等しく、
16分は拍頭でも拍末でもない）＝**最も単純な報告例ですら `set_rhythmic_importance` が要る**。
その関数は `:292-380` で **tuplet の span context を持つ**（`Span_position`・約 90 行）。
⇒ **1 行 clamp の除去では終わらない。島として起票すること。**
★ **点が先。そして今回の点は新種**——`Beam.positions` ではなく**描かれた beamlet の本数**を
読む必要があり、**`RenderedGeometry` にその reader が無い**（`BeamPositionAboveStaffMiddle` は
群ごとに primary 1 本しか返さない）。**reader を作る段が最初の仕事**で、
第52セッションの経験では**そこが本体になる**。
★ **その次**: **量子器の stem x は knee では今もコラム由来**（第51セッション②はビーム外の
供給側だけ直した）／**`test/` に meshing の本を 1 冊**（上・GO ゲート）／
