...
⇒ ★★★ **次の一手＝残りの U8＝*grob を要る 3 つ*＝⒜ の和音・休符・tuplet ／ ⒞ 本体の中のスラーとタイ ／ ⒟ 注釈の全族**（§2 U8 の ▶）。⚠️⚠️ **和音・休符・tuplet を `GraceNoteInfo` の中に彫らないこと**——**それは和音レイアウト／休符レイアウトの 2 綴り目**（§5.2.1②）。**本便が phrase 参照だけを取ったのは、それが*どの綴りも増やさない*唯一の 1 つだったから。** ⚠️ **⒞ の梁は住所を要らない**（⑼）ので、**⒞ を「スラー・タイ」と「梁」に割ってよい**。★ **一番安い次の 1 手は ⒞ の梁**（`GraceNoteEngraver` の中で閉じる・`audit/lpreg` の 2 冊が既に `grace { c16~ }` を書いているのでタイのほうは実コーパスの射程が 2 冊ある）。
...
終了時: **commit 4 本**（**本体 `76e60e9e` ／ 7.7 の差分の網と LP 実測 `bf205ec3` ／ §1・§2・RULES `ac68f2db` ／ この数を閉じる 1 本**）＝**未 push 24**・木 0・未追跡 0。**suite 6654 合格 / 0 失敗 / 4 skip / 合計 6658**（開始 6647 から **+11**＝**本便の網 11 ケース**）。Core 0 エラー 0 警告。**追跡 `.lys` 581 冊・snapshot 231 枚**（**書き換え 0・追加 0・削除 0**）・**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2／`OPEN:` 0＝すべて不動。** ⚠️ **`docs/APPROXIMATIONS.md` も `audit/magic_constants.csv` も 1 行も動いていない**——**本便は定数を 1 つも足していない**（**移植ではなく容れ物の修理だから**。**`LILYPOND-REF` を 1 本足したのは実測の記録として**）。
★★ **WSL ubuntu Release も同じ 6654 / 0 / 4 / 6658**（**`ac68f2db` の木＝docs を commit した*あと*に回した**。第298 ⑽ の棚）。⚠️ **CI は `11f48202` までしか走っておらず、未 push 24 本は未実行。**
...
- **U8. ▶ 4 分の 3 まで閉じた（第298 ＋ 第299 ＋ 第300）＝`grace { }` の本体は*歩かれない*。起票の「注釈を運ばない」より広かった**
  ...
  **`ProcessCueRegion` が cue 領域にしているのと同じ**（`MeasureCollector.MusicWalk.cs` の
  ...
  ⇒ ★★★ **教訓は §2 U8 が既に 3 回教えたものと同じ**: **この項の「射程」も「難所」も、
    *書いた便が測らずに書いて*、*次の便が測って直した*。⇒ 起票の数値は、着手前に 1 回測る。**
