...
最終更新 第410セッション（2026-09-18・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `edb05f03`・未 push 40・**full `sessions/p410/run1.trx` 8715 / 0 / 3 / 8718＝第409 最終と一致**・CI は `851e5538` の run が緑（それより前の 4 run は赤のまま・以降は未 push）・build 0 エラー・`Session-Check -Build -Test` 122 秒）、指示「HANDOFF を読んで着手」→ §1「次の一手」が名指していた **★★★「collect は pass の約 3 倍＝次に測るならそちら」**を採った。★ **ベンチは着手前に AskUserQuestion で静かな窓をもらった**（「今から回してよい」）。根拠は回数と割当バイトを主に（CLAUDE-OPERATIONS §2）。道具は pwsh MCP（Bash 0 回）:

★★★ **⑴ 第 1 便＝コード変更 0。*その「collect は pass の 3 倍」が、打鍵の上では逆だった***（**全文は Lab `sessions/p410/summary.txt`**・計器 `Zz410Probe1`／`2`／`3`・**製品コードは触らず、既存の `StageProbe` の継ぎ目で測った**）。⚠️ **第409 の ~89 ms は *fresh tree の full collect***＝**打鍵はそれを走らせない**（preview が `CollectWithResume` で刻み、`CollectFor` で診断 pass に貸す）。**打鍵 1 回の実測**（fingbeam1k・Release・TC=0・実 1 音編集 11 回の min・GC 柵）: **collect 4,816,832 B／prelim 5,281,424／layout 28,485,792／render 36,775,032＝preview 合計 75.4 MB ~42 ms**、**debounced な診断 pass 33,509,192 B ~21 ms**。⇒ ★★★ **collect の枠は full collect の 6.1%**（resume が 1000 小節を adopt）＝**pass は collect の 7 倍**（plain1k 2.2 倍・v2bow1k 0.6 倍＝この本は splice 側）。**2 便かけて畳んだのは*大きいほう*だった。**

★★★ **⑵ そして resume は red 実体化を*消して*いない——診断 pass へ*移した***。**full collect 済みの木の pass 14,320,424 B 対 打鍵の木の pass 33,509,192 B＝差 19,188,768 B**＝第409 が独立に測った red 実体化 19,318,536 B と **0.13 MB 以内で一致**。⇒ **adopt した小節の red を collect が触らなくなったので、pass が最初の触り手になって払う。**

★★★ **⑶ pass の中で払っているのは validator ではなく*索引*だった**（同じ打鍵の木で、索引を先に建てて単独で値付け）: **`DescendantIndex` の建て 26,198,520 B（112 B/節点＝配列と*それが強制する red 全部*・第409 の 30＋83＝113 と一致）／その上に乗る pass 7,310,712 B** ⇒ **索引が pass の割当の 78%**（plain1k 94%・v2bow1k 85%）。**3 本目の検算＝26,198,520 − 19,188,768 = 7,009,752 B** は第409 が測った索引 7,009,640 B と **112 バイト以内**。⇒ **2 便の折り畳みの後、validator 自身は 33.5 MB のうち 7.3 MB。残り 78% は土台。**

★★★ **⑷ `DescendantIndex` の註が前提を書いており、その前提は期限切れだった**——「*the index exists for the passes that materialize them anyway*」は第401 では真（28 本が各々 red 全木 walk）。**第401・408・409 が全部 kind bucket へ移した**ので、**pass が今なお要る節点は**: plain1k **9,010／43,043＝20.9%**・fingbeam1k **33,008／234,030＝14.1%**・v2bow1k 21.9%・★ **ユーザーの実コーパス 330 冊で 180,946／919,472＝19.7%**（冊ごと min 12.5／中央 19.8／max 32.1%。**実冊は中央 2,816 節点・最大 7,643＝fingbeam1k の 1/30〜1/80** ＝形は移るが絶対 ms は移らない）。**型の SCAN 退避は全冊 0 本**。

⇒ ★★★ **⑸ だが「全木の平坦 walk」が 3 本*残っている*——そしてそれが red を在らしめている**。計器が名指した（perf 3 冊と実コーパス**330 冊すべてで同じ 3 本**）: **`MeasureValidator.CollectPhraseBodies`**／**`SectionBarCounts.SemanticVoices`**／**`SectionBarCounts.LyricsCells`**（後 2 者は `CrossPartMeasureValidator.ValidatePartMajorSections` 経由）＝**fingbeam1k で 3 × 234,030＝702,090 節点訪問／打鍵**。⚠️⚠️ **第409 §1 の「尾部に畳める validator は 1 本も残っていない」は *ms の一覧*から読んだ結論**——平坦配列の walk は 2.2 ns/節点（第408 ⑶）なので **3 本とも床 0.05 ms の下に隠れる**。**回数で見ると pass の最大の単品。** ⇒ **702,090 を隠す床は、この問いには合わない計器**（§5.3 へ）。

★★★ **⑹ 第 2 便＝その 3 本を畳んだ**（`e60e915a`・**R13 の文字は ⒭ を飛ばして ⒮ から**＝退役した ▶ ⒭ と §2 F ⒭ に 3 つ目を足さないため）。★★★ **2 本は綴りを 1 つも増やさない**＝`CollectPhraseBodies` と `SemanticVoices` 自身の gather は**第409 が既に畳んだ `PhraseCycleValidator` と*同じ問い***なので**その表を使う（1 つの表・3 つの switch）**／`LyricsCells` は **1 型なので表は要らない**（第409 の規則）。**新しい表は `SectionBarCounts.SemanticVoiceKinds` 1 本だけ**。**A/B（計器）＝平坦 walk 3 → 0**（perf 3 冊・実コーパス 330 冊すべて）・**SCAN 退避は 0 のまま**・**訊く distinct 節点は不変**（33,008／234,030）。⚠️ **割当は +288 B（enumerator 2 個）＝勝ちは*回数*であって割当ではない**——平坦配列の walk は何も割り当てていなかった。**ms も測ったが、同じ run の中で索引の建てが 16.5 → 29.4 ms と振れた＝この機械の雑音の帯の中なので ms の主張はしない**（CLAUDE-OPERATIONS §2）。健全性＝**census 760 冊 435 行 73,495 バイトがバイト同一**・**毒 4／4 本**（`SemanticVoiceKinds` から 1 kind ずつ落として 2 回・`LyricsCells` を別 kind に向けて 1 回・共有表から `VariableDeclaration` を落として 1 回）。⚠️ **最後の 1 本は*振る舞いの*テストを 1 本も赤くしなかった**（赤くなったのは kind の網だけ）＝**共有表には他に見張りが無い**ので、その事実を表の隣に書いた。網 `FlatWalkKindsTests` 3 本。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: ⒜ **R13⒝ の実機確認**（第404 ⑵＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／**⒮ ✅ 今便の第 2 便で閉じた**／★★★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械＝緑 walk・位置は積算・red は消費された site だけ）＝上限は **red を 100% でなく 14〜20% だけ実体化＝fingbeam1k で 26.2 MB → 約 10 MB**。⚠️ **未着手・未見積もり**＝平坦 `Nodes` 配列と `OfKinds` が併合する pre-order ordinal が**どちらも red walk を前提にしている**＝validator ではなく**土台の変更**。**前提条件だった ⒮ は今便で片付いた＝いま着手できる**／⚠️⚠️ ★★★ **そして一番大きい島は pass ですらない＝layout ＋ render が打鍵のバイトの 83〜84%**——**第 3 便が割った（上の ⑺・§2 R13⒰）**⇒ ⚠️ **⒱ `L3.chooseSystemCount` は第 4 便が census を取って落とした**（上の ⑻＝実冊では打鍵の 2.4%・memo は当たりようがない＝**R13⒦ ⒤ は閉じ**）。⚠️⚠️ **⒲ `R1.draw`（合成本 36%）と ⒳ `L1.lineBreaks`（合成本 58%）も第 5 便が実冊で落とした**（12.5% で筆頭 4 冊／**0.2%**）。⇒ ★★★ **実冊での次の一手は ⒴ `PlaceSystems`＝L2 ＋ L4 で打鍵の 32.2%・単独最大**（⑼⑽）。⚠️ **ただし第 6 便が `L4` を値付け済み**＝**per-system memo は既に 99.9% 当たっている**ので **「2 度目の系を memo する」は無し**。残る 2 案は **⑴ 2 度目を要らなくする（候補 count を*配置せずに*値付け）**か **⑵ per-system より*下*を memo する**（どちらも未着手・**まず `PlaceSystems` の*中*を段別に割ること**＝`Zz410Probe7` に 1 段足すだけ）。次点は **⒵ collect 18.0% と prelim 17.0%**（第399〜407 が何度も削った島だが、実冊では今なお合計 35%）／⒥ は第409 が上限 4.7 ms と測った（採るならそれを知って採る）・count loop の page DP 5.3 ms（R13⒦・census で hit 率）／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅ 第407 が測って「やらない」と決めた**／**⒟ ✅ 第407 で閉じた（R2R）**／LP 双子が要る R7〜R11／**R15 ✅ 削除は第407 で完了**（残り＝「2 綴り」一族のみ＝統合・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★★★ **⑺ 第 3 便＝コード変更 0（ユーザー指示「有利なら着手」の判定＝`sessions/p410` に残った `Zz410Probe1` が §1 の「未測」項そのものの計器だったので着手）。layout ＋ render を割り、*その過程で第 1 便の数を訂正した***（**全文は Lab `sessions/p410/summary.txt`**・計器 `Zz410Probe4`＋一時 `ZzStage`・**バイトだけを読む＝機械が静かでなくても汚れない通貨**）。⚠️⚠️ **第 1 便が測ったのは*エディタが通らない*経路だった**＝`RenderIncremental`（全文 1 本の文字列）を測ったが、**LSP は第404 の ⒝ 以降 `RenderIncrementalPages` を呼ぶ**（`ToSvg` 対 `ToPages`）。**その最後の 1 段が小さくない**＝fingbeam1k で **14,319,488 B 対 946,200 B**。⇒ ★ **打鍵あたりの合計を訂正**: fingbeam1k **75.4 → 62.0 MB**・plain1k 31.7 → 28.2・v2bow1k 66.5 → 62.3。**「layout ＋ render が 87%」も同様に 83〜84%**（⚠️ 境界＝サーバが変更頁を*直列化*する分はこの測定の外）。★★★ **内訳**（エディタ経路・打鍵あたり・`L4` は全冊で発火せず＝count loop が選ぶ数は常に ideal と同じで 2 度目の配置は走らない）: **fingbeam1k 62.0 MB**＝`R1.draw` 22.47（36%）／`L2.placeSystems` 9.27／`L3.chooseSystemCount` 8.66／`L6.finishLayout` 7.29／prelim 5.28／collect 4.82／`L1.lineBreaks` 2.03／`L5.createPages` 1.23／`R2.toPages` 0.95。**plain1k 28.2 MB**＝`L3` 8.66（31%）が筆頭。**v2bow1k 62.3 MB**＝**`L1.lineBreaks` 36.23（58%）**が筆頭。⇒ ★★★ **⑴ `L3.chooseSystemCount` は plain1k と fingbeam1k で *8,660,816 B ちょうど同じ***＝**節点数が 5.4 倍違う 2 冊で同じバイト**＝費用は音楽ではなく **count の格子**（第403 の R13⒦＝毎打鍵 72 count）。**plain1k では打鍵の割当の 31%**で、**既に書かれた手がある唯一の項**（R13⒦ ⒤＝答えの memo・「先に census で hit 率を数えること」）。⇒ ★★★ **⑵ 冊によって高い所が違い、順位は節点数ではない**——v2bow1k は打鍵の 58% が**線分割 DP**（fingbeam1k の 18 倍を、1/6 の木で）・fingbeam1k は 36% が**描画**。**1 冊の profile でこの仕事の順番は決められない＝次に採る物は、それが実際に出る冊で採ること。** ⚠️ **計器の最初の版は「未計上の残り」が*負*だった**＝`using var` と明示 `Dispose()` で struct の Dispose が 2 回走り L5 を二重計上していた（RULES §5.5「新しい計器の最初の食い違いは、まず計器を疑う」がそのまま当たった）。直した後の残りは 2,808〜3,000 B＝scope 自身。⚠️ **実コーパスでは未測定**（中央 2,816 節点＝この絶対値は読み手が払う額ではない。移るのは*形*）。

★★★ **⑻ 第 4 便＝コード変更 0（判定＝§1 が「要 census」と明示していた R13⒦ ⒤ に、`sessions/p410` の打鍵 harness と実コーパス掃きがそのまま噛み合う）。第403 から待っていた census を取り、⒤ に「やらない」と答えが出た**（**全文は Lab `sessions/p410/summary.txt`**・計器 `Zz410Probe5`・実冊 **292／330**・8 打鍵ずつ・**エディタの入口 `RenderIncrementalPages`**）。★★★ **⑴ ink が動く打鍵**（音名 1 文字）＝count loop は **2,320 layout の 99.7% で走り**・**15.9% で実際に段数を変え**（**perf 本では 1 度も変えなかった＝第 3 便の L4 未発火**⇒ **止めてはいけない実仕事**）・**1 回 171,376 B**・**打鍵 1 回は 6,924,431 B** ⇒ ★★★ **count loop は実冊の打鍵の 2.4%**（perf-plain1k の 30.7% ではない）。線分割の門が保たれたのは **13.4%**。★★★ **⑵ ink が動かない打鍵**（音楽中に空白 1 個）＝**layout が 0 回**（2,336 打鍵で 1 度も走らない）＝**全体再利用が count loop ごと既に飛ばしている**・打鍵 1 回は 2,699,177 B。⇒ ★★★ **だから memo は当たりようがない**——**当たり得る唯一の打鍵（ink 不動）は、そもそも layout ごと飛ばされている**。ink が動く側は estimate が同じ鍵の一部なので 13.4% より下。**賞金は打鍵の 0.3% 未満、鍵は線分割 DP の表まるごと** ⇒ **R13⒦ ⒤ は「測って、やらない」で閉じる。** ★ **併せて誰も持っていなかった数＝実冊の打鍵 6.92 MB（ink 変更）／2.70 MB（ink 不動）**＝**合成本は実冊の 4〜9 倍**（plain1k 28.2・fingbeam1k 62.0）⇒ **第 3 便の順位は*合成本の*順位**。⚠️ **標本の偏り**＝冊あたり編集 1 箇所・測れなかった 33 冊はこの脚本が置換できない綴りの本。

★★★ **⑼ 第 5 便＝コード変更 0（判定＝§1 が第 4 便の末尾に自分で「次に採るなら、まず実冊で `Zz410Probe4` の段割りを回して順位を確かめること」と書き、道具が Lab に在った）。実コーパスで段別に割ったら、順位は合成本とほぼ無関係だった**（**全文は Lab `sessions/p410/summary.txt`**・計器 `Zz410Probe6`・実冊 **292 冊 × 8 打鍵**・`RenderIncrementalPages`）。**実冊の打鍵 6,891,053 B**（collect 18.0%／prelim 17.0%／layout 50.1%／render 14.9%）。★★★ **実冊の順位**（括弧内は「その段が筆頭になった冊数／292」）: **`L2.placeSystems#1` 24.0%（110）／`S0.collect` 18.0%（94）／`S1.prelim` 17.0%（72）／`R1.draw` 12.5%（4）／`L4.placeSystems#2` 8.2%（7）／`L5.createPages` 8.1%（5）／`L6.finishLayout` 7.0%／`L3.chooseSystemCount` 2.5%／`R2.toPages` 2.4%／`L1.lineBreaks` 0.2%**（未計上 0.05%）。⇒ ★★★ **合成本が言っていたことは全部外れていた**: `L1.lineBreaks` は **v2bow1k で 58% → 実冊 0.2%（約 290 分の 1）**・`R1.draw` は **fingbeam1k で 36% → 12.5% で筆頭はわずか 4 冊**・`L3` は **plain1k で 31% → 2.5%**（第 4 便の 2.4% と独立に一致）。⇒ ★★★ ⚠️ **そして*合成本が 1 度も走らせなかった*段が実仕事の 8.2%**＝**`L4.placeSystems#2`**（count loop が ideal 以外を選んだときの再配置）は **perf 3 冊で発火 0**（第 3 便が記録）なのに **実 layout の 15.9% で発火**（第 4 便）し、**7 冊で筆頭**。⇒ ★★★ **`PlaceSystems` は実は L2 ＋ L4 ＝ 実冊の打鍵の 32.2%＝単独最大の島で、その 3 分の 1 は*同じコードの 2 度目*。** ⇒ **count loop の本当の値段はそれ自身の 2.5% ではなく、引き起こす再配置 8.2%**（第 4 便の「答えを memo しない」判断は有効なまま・**「2 度目の配置を 1 度目より安くする」は誰も見積もっていない**）。⚠️ **この順位は「ink が動く打鍵」のもの**＝ink 不動の打鍵では layout が 1 度も走らない（第 4 便）ので上の layout 段は全部 0%。⚠️ 冊あたり編集 1 箇所・1 種類。

★★★ **⑽ 第 6 便＝コード変更 0（判定＝§1 が「未見積もり」と書いた `L4` に同じ harness がそのまま使える）。2 度目の配置を値付けしたら、*明らかな手はもう打ってあった***（**全文は §2 R13⒰ 末尾と Lab `summary.txt`**・計器 `Zz410Probe7`）。**P2 は layout の 15.9%・しかし 292 冊のうち*46 冊だけ*で発火**。⚠️ **最初の比較は交絡していた**（P1 の平均に、P2 が発火しない 246 冊＝小さい冊が入って P2 が 1 系あたり 2.1 倍に見えた）⇒ **同じ 368 layout に限ると P1 1,370,496,560 B 対 P2 1,312,726,808 B＝0.96 倍**（1 系あたり 156,592 対 161,506）＝**2 度目は 1 度目そのままの値段**。★★★ **そして per-system memo は P2 の照会の 99.9% を既に返している**（hit 50,109＋shifted 87 対 miss 28・P1 は 99.4%）＝**配置の費用は memo が覆う per-system 層ではなく*その周り*で、それが丸ごと 2 度払われている**。⇒ ★★ **「2 度目の系を memo する」は提案しないこと（測定済み・既に当たっている）。勝ち筋は ⑴ 2 度目を要らなくする（候補の count を*配置せずに*値付けする）か ⑵ per-system より*下*を memo する。**

★ **⑿ 第 7 便は着手しない**（判定＝道具はある・**天井が律速**）: `PlaceSystems` の中割りは `Zz410Probe7` に 1 段足すだけだが、**§1 の残りは 1,503 バイトで見積 1.5〜3 KB が入らず**、圧力弁は**両方 0**（`-Before 2026-12-31` でも §2 0 項・§3 0 セル）。残る `-Archive 409` は**頭の手順**（⑾ の警告）。**測れても渡せない便はやり直しになる**ので、次便が archive してから採る。

★ **⑾ 終了時**: commit 8 本（第 1 便 docs `79da4fff`＝コード変更 0・第 2 便 code `e60e915a`＋docs `67bc343d`・第 3 便 docs `55d3392e`・第 4 便 docs `10210119`＋`931af1ae`・第 5 便 docs `16143066`・第 6 便 docs 1 本。**コードを触ったのは第 2 便だけ**）＋Lab 5 本・**最終 full `sessions/p410/run3.trx` 8718 / 0 / 3 / 8721**（第 3 便は計器を全部外して**作業ツリーが `67bc343d` と同一**に戻ったので、この run がそのまま最終）（第409 最終 8715 / 0 / 3 / 8718 に対し**網 +3**＝`FlatWalkKindsTests`）・§7.5（対 `edb05f03`＝スクリプトの出力）**Core `+` 63 行／REF 0／OWN 0**（**数値定数なし・LP 由来の式なし**＝同じ述語をより少ない節点に向けただけで計算は 1 つも動いていない・大半は註＝監査対象は「同じ答えか」＝**census 760 冊 435 行 73,495 バイトがバイト同一**＋毒 4／4＋snapshot 249 不動）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。棚卸し `docs/APPROXIMATIONS.md` は**不変**（`TheInventoryIsNotStale` は緑のまま）。§7 4（恒久知識）＝**RULES §5.3 に 3 項**（「床のある ms の一覧は*回数*の問いに答えない」「**測る前に*呼び手*を確かめる＝ベンチが通る経路が製品の経路とは限らない**」「**合成本の順位を実コーパスの順位だと思わない＝島を選ぶ前に実冊で分母と*段別の順位*を取る。合成本で 0 回走る枝は安全な枝ではなく測っていない枝**」）。計器と全文は Lab `sessions/p410/summary.txt`（`Zz410Probe1`〜`6`＝**製品コードを触らない `StageProbe` の継ぎ目と、commit しない一時 counter／`ZzStage`**・`Zz410Census`）。§7 3.5 は済（第408 を `-Archive 408`＝17 行 4,738 chars）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️⚠️ **第 3 便を書いたら 451,064 バイトで天井を*超え*、ラチェットが赤くなった**（`HandoffArchiveContinuityTests.TheHandoffStaysReadable`＝**この門は働いている**）。**§3「決定済み」の根拠セル 11 個を ARCHIVE へ畳んで直した**（`Fold-ClosedHandoffItems.ps1 -Session 410 -Before 2026-09-16`＝12,853 バイト・**決定・日付・★ は §3 に残る**）⇒ 439,575 まで戻し、第 5 便で 447,317 になったので**第 6 便の前に §3 をもう 2 セル畳んだ**（`-Before 2026-09-18`＝1,696 バイト）。⚠️⚠️ ★★★ **第 6 便の途中で `-Archive 409` を試したのは誤り**——**§7 3.5 は*セッションの頭*の手順**で、途中で回すと §1 から直前便のブロックが消えて **`HandoffArchiveContinuityTests` が 3 本赤くなる**（`TheHandoffKeepsExactlyOnePredecessorBlock` ほか。**§1 は「現在便＋直前 1 便」ちょうど**という不変条件）。**戻して §3 の圧力弁を使った。** ⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 409` を回すこと**（**そのときは正しい手順**＝旧文の上に「## 以下は第410セッションの経緯」を立ててから）。**天井は上げない。** ⚠️ §3 の圧力弁は 3 回使った（第409 が 22 個・今便 11＋2 個）＝**もう残っていない。**

## 以下は第409セッションの経緯

最終更新 第409セッション（2026-09-18・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `b6026a2d`・未 push 32・**full `sessions/p409/run1.trx` 8704 / 0 / 3 / 8707＝第408 最終と一致**・CI は `851e5538` の run が緑（それより前の 4 run は赤のまま・`16f5157a` 以降は未 push）・build 0 エラー・`Session-Check -Build -Test` 116 秒）、指示「HANDOFF を読んで着手」→ §1「次の一手」のうち ⒜⒝⒝′ は実機の 2 行が要る、⒟ と `RestCollisionsOf` は閉じている／ユーザー決定済みなので、**⒞ の ⒤＝「kind 表を 1 本ずつ足して尾部を bucket に移す」を採り、2 便で*尾部を全部*畳んだ**。根拠は回数を主に（ユーザー指示・CLAUDE-OPERATIONS §2）。★ **ベンチは着手前に AskUserQuestion で静かな窓をもらった**（「今から回してよい」）。道具は pwsh MCP（Bash 0 回）。**閉じたのは 1 本＝R13⒤ の全部（⒬＋⒬′）**。**第 3 便はコード変更 0 で、⒥ の起票の数を測って訂正した**:

★★★ **⑴ 第 1 便＝`LyricSings` の全木 3 周を index に訊いた**（`310f5ea9`・**全文は §2 R13⒬**・計器は `sessions/p409/`）。★★★ **数が着手順を決めた**＝打鍵あたり **3 × 234,030＝702,090 節点訪問して会うのは *2 節点*** （この冊の part 宣言 2 本だけ。net 冊 259 では 140,805 → 約 812）・**2.14 ms に対し割当 464 B＝時間はほぼ全部「歩き」**。⇒ **probe の床 0.05 ms を下回って一覧から消えた**・pass 41.2 → 36.9 ms fresh／14.7 → 12.5 warm。⚠️ **`LyricBindings` の 3 本も同じ病気だった**（`Conflicts` は毎回全木・`BuildMap`／`BuildVoiceMap` は CWT で覚えるが**木は打鍵ごとに新しい**＝毎打鍵 1 回は払う）。健全性＝census 760 冊 435 行**バイト同一**・毒 3 腕・網 8 本。

★★★ **⑵ 第 2 便＝尾部 9 本を同じ型で畳み、⒤ は*全部*終わった**（`b0298b08`・**全文は §2 R13⒬′**）。★★★ **まず数えた**（`sessions/p409/tail-counts.txt`）＝234,030 節点に対し switch が会えるのは **`Duration` 8,000（3.42%）・`RepeatStructureScope` 1,000・`ScoreSettingInPartHeader` 1・残り 5 本は *0***。★★★ **形は 2 つあり、安いほうは代償ゼロ**＝**⑴ 1 型なら kind 表は要らない**（`DescendantNodes<T>()`＝**2 綴り目が存在しない**・`DescendantIndexTests` が既に網）／**⑵ switch なら kind 表を隣に置く**——**述語に切り出せるものは切り出して*その述語に訊く*網**（5 本）、**型束縛とメッセージを抱えた case は切り出せない**ので（2 本）**テスト側が型を綴って平坦 walk と突き合わせる**。⇒ **pass 36.9 → 30.9 ms fresh／12.5 → 7.5 warm**（**便頭からは 41.2 → 30.9／14.7 → 7.5**）。**対照が内蔵**＝`AnnotationName` 3.57 → 3.48・`MeasureValidator` 2.77 → 2.78 は**動かない**。健全性＝census バイト同一・**毒 7／7 本**（7 つの表から 1 kind ずつ落として 7 回）・網 3 本・snapshot 249 不動。

★★★ **⑶ 第 3 便＝コード変更 0。*自分が ⑵ の段落に書いた「約 23 ms」を測って、間違いだと分かった***（**全文は Lab `sessions/p409/freshtree-summary.txt`**・計器 `Zz409Probe3`／`Zz409Probe4`）。あの数は **fresh 30.9 − warm 7.5 の*引き算*で、一度も測っていなかった**。⚠️⚠️ **最初の計器は*あり得ない*数を出した**（index を先に建てた pass のほうが遅い）＝JIT と GC。暖機＋GC 柵を入れ、**根拠を割当（バイト）に移した**（2 本の独立な probe が 112 バイト以内で一致）。★★★ **`Zz409Probe4` が名指した＝28 本の validator*全部*が、1 回目と 2 回目の差にきっかり同じ `7,009,752 B` を払う**（fingbeam1k・新しい instance を毎回作るので per-tree キャッシュしか効かない）＝**それは `DescendantIndex` の建て**（独立測定 7,009,640 B）で、**木ごとの構造は他に 1 つも無い**（在れば、その 1 本だけ差が大きく出る）。⇒ **内訳**（fingbeam1k 234,030 節点）: parse 38.5 MB／red 実体化 **19,318,536 B＝83 B/節点**／index **7,009,640 B＝30 B/節点**。⇒ ★★★ **red 実体化は pass の中に無い**——在れば validator の 1 回目は 26.3 MB 払うはずで、実測は 7.0 MB＝**parse と collect が先に払っている**。

⇒ ★★★ **だから ⒥ は「島の名前」を間違えていた**（§1 のこの段落を訂正）: ⑴ **pass の中で誰かが*建てる*木ごとの費用は 7.0 MB／約 4.7 ms だけ**＝「index を建て直さない」で取れる上限はそこまでで、**旧 §1 が約束していた 21 ms の 1/4**。⑵ **残りは建てではない**（何も割り当てていない）＝parse と collect が書いたメモリの初回タッチ＋validator ごとの初回効果＝**RULES §5.3 が「当てにするな」と言っている種類の数**で、**この便の最初の probe が現にそれを踏んだ**。⑶ **red 実体化 19.3 MB は実在するが、それは parse ＋ collect の島**で R13⒥ とは別。★★★ ⚠️ **そして探していなかったものが出た＝同じ条件で parse ~15.2 ms・`TryCollect` ~89.2 ms・診断 pass ~27.3 ms＝collect は pass の約 3 倍**。**2 便かけて畳んだのは小さいほう**だった。⚠️ **留保**: この ~89 ms は fingbeam1k（1000 小節・注釈 24,000 の合成本）の数で、**読み手が実際に編集する本では未測定**。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: ⒜ **R13⒝ の実機確認**（第404 ⑵ のとおり＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item（第404 ⑵′・実機の 2 行を見てから）／⒝′ frame 変更の `applyFrame`（実機で頻度を見てから）／**⒞⒤ ✅ 終わった（今便）**＝この機械の現在値は `sessions/p409/validators-head2.txt`（fingbeam1k pass **30.9 ms fresh／7.5 warm**・床 0.05 を超えるのは **4 本だけ**＝`AnnotationName` 3.48・`MeasureValidator` 2.78・`RehearsalMarkEngraved` 0.66・`Duration` 0.54）。⚠️⚠️ **`RehearsalMarkEngravedValidator` 0.66 は畳む余地が無い＝*すでに*索引に訊いている**（`DescendantNodes<MusicMarkSyntax>()`・:88）。**この 0.66 は walk ではなく、会った mark 1 つずつに対する `AnnotationValues.Rehearsal` の実仕事**（fingbeam1k は注釈が 24,000 ある本＝第408 ⑴）。⇒ **尾部に「まだ畳める validator」は 1 本も残っていない。**（⚠️ この段落は当初「`DescendantNodes()` を使っておらず別の形＝次に見るならここ」と書いていた＝**誤り**。便の終わりに実物を読んで訂正した——§5.0 の「起票の住所は仮説」がそのまま自分の書いた §1 に当たった。）⇒ ★★★ **⒥ は第 3 便が測り直した（上の ⑶）＝「約 23 ms の red 実体化＋index 建て」は誤り**。**pass の中で建てているのは `DescendantIndex` の 7.0 MB／~4.7 ms だけ**（28 本全部が同じバイト数を払う＝他に per-tree 構造は無い）・**red 実体化 19.3 MB は parse ＋ collect の島**・残りは建てではなく初回タッチ。⇒ **⒥ を採るなら上限は 4.7 ms と知って採る。** ★★★ **そして同じ条件で collect は pass の約 3 倍（~89 対 ~27 ms）＝次に測るならそちら**（バイトで測ること）。⚠️ **この 89 は「collect の持ち分」であって「collect の仕事量」ではない**——fresh tree での呼び出し丸ごとなので、**collect が最初に触った red の実体化（最大 19.3 MB）を含む**。**割るには collector の*中*の phase ごとの回数が要る**（第408 `Zz408Counts` を `MeasureCollector` に向けた形）＝**今便の 2 本の probe の変種では足りず、1 便ぶんの仕事**。⚠️ **実コーパスでは未測定**（fingbeam1k は合成本）。**validator 側を削る島は尽きた**＝残るのは count loop の page DP 5.3 ms（R13⒦・census で hit 率）／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝fingbeam1k の `fs.walk` 0.41 ms／1.5 MB と `fs.assemble` 0.29 ms／4.0 MB（24,000 digit の実仕事＝「何を返すか」を変える別の島）／**⒞″ ✅ 第407 第 2 便が測って「やらない」と決めた**／**⒟ ✅ 第407 で閉じた（R2R）**／LP 双子が要る R7〜R11／**R15 ✅ 削除は第407 で完了**（残り＝「2 綴り」一族のみ＝統合・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑷ 終了時**: commit 7 本（第 1 便 code `310f5ea9`・第 2 便 code `b0298b08`・第 3 便はコード変更 0・docs `065054be`・RULES `1cb72a0c`・§1 の手直し 2 本・第 3 便 docs 1 本）＋Lab 3 本（`b166649`〜）・**最終 full `sessions/p409/run6.trx` 8715 / 0 / 3 / 8718**（第408 最終 8704 / 0 / 3 / 8707 に対し**網 +11**＝`LyricSingsWalkTests` 8 本＋`TailValidatorKindsTests` 3 本）・§7.5（対 `b6026a2d`＝スクリプトの出力）**Core `+` 216 行／REF 0／OWN 0**（**数値定数なし・LP 由来の式なし**＝同じ述語をより少ない節点に向けただけで計算は 1 つも動いていない＝監査対象は「同じ答えか」＝**census 760 冊 435 行 73,495 バイトが 2 便ともバイト同一**＋毒 10 腕＋snapshot 249 不動）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。棚卸し `docs/APPROXIMATIONS.md` は**不変**（`TheInventoryIsNotStale` は緑のまま）。§7 4（恒久知識）＝**RULES §5.3 に 2 項**（「2 綴り目の代償は filter の形で決まる＝1 型なら kind 表は要らない」「木ごとの CWT は打鍵ごとに払っている」）。計器と全文は Lab `sessions/p409/summary.txt`（第 3 便は `freshtree-summary.txt`）。§7 3.5 は済（第407 を `-Archive 407`＝21 行 5,998 chars）。⚠️ **それでも §1 を書いたら 449,895＝天井まで 105 バイトだったので、§3「決定済み」の根拠セルを 22 個 ARCHIVE へ畳んだ**（`Fold-ClosedHandoffItems.ps1 -Session 409 -Before 2026-09-10`・18,444 バイト・**決定・日付・★ は §3 に残る**＝表の仕事は「蒸し返さない」ことで、それは主張が残れば足りる。§2 は畳む物が 0 だった）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️⚠️ **天井 450,000 バイトに対し今便の終わりで約 439.2 KB＝残り約 10.8 KB**（§3 を畳んで空けた）＝**次便も §1 を書く*前*に §7 3.5（`-Archive 408`）を回す**（天井は上げない）。
...
- **R13 プレビュー遅延の構造**（第395 は ⒜ の cancel と validator の走査だけ入れ、**第399 が ⒜ の collect 側を閉じた**）＝⒜ ✅ collect の 2 重は閉じた（第399・`8a3b7f68`）＝preview の `IncrementalCompiler.CollectFor(tree)` が最後の compile の collector（resumed でも）を「同じ tree ＋ 先頭 render block」のときだけ貸し、LSP の pass は自前の `TryCollect` を要らなくなった。R13′ は panel の契約側で固定（名前付き render の session は貸さない）。**第 2 便が validator 側を計器で割った**: 「~50 本の全木 walk」は 25 ms で、46 ms は `LyricPlainVerseShadowedValidator` の自前 collect（3 本目）だった→ shared collect に載せ替え（`51ee54a8`）＝pass 110 → 59 ms。**残り**＝`MeasureValidator` 15（R12⒠ の第 2 模型）／25 本 × 1 ms の全木 walk。**第 3 便が `NavigationPlacementValidator` を census で除外**（759 冊中 20 冊・最大 3.7 ms・undrawn part の mark 0・旧と shared は全冊一致＝触らない）、`SymbolReference`＋`LyricSings` の walk 畳みは 2 綴り目を作るので見送り。**第400 が `MeasureValidator` 15 ms を閉じた**（`e4fb4bc5`＝cross-part pass が単一 part の本で捨てる Split を 2 回・phrase 表の 3 回目の walk・census 759 冊で診断 120 行一致）＝**validator 側の残りは 24 本 × 全木 walk 1 本（plain1k 25 ms・fingbeam1k 108）と `SymbolReference`／`LyricSings`／`AnnotationName`（fingbeam1k 32／26／17）だけ**。**同便が collect 側も 1 段閉じた**（`ed3dceb0`＝採用済み resumed collect が edit と無関係な全木 walk を 5 本払っていた: `GetPartDefaults` の transpose／pitch 既定 2 本 × part 数と `CanonicalByNameSyntactic` の 3 本→ `CollectDefinitions` の 1 walk に畳む・plain1k collect 25 → 18 ms・fingbeam1k 72 → 33）＝**collect 側の残り**＝section の GreenSite gather（採用しても払う・plain1k 3 ms・fingbeam1k 12）・Debug だけの beam memo 照合 12 ms（Release は 0）・v2bow の extra voice live walk 6 ms（設計）。**第402 がその gather を閉じた**（`0e19c0d1`＝checkpoint が site の slot path を持ち（`WalkCheckpoint.GatherPath`）、resume の prefix 側は `SyntaxNode.TryGreenSiteAt` で container の green walk に*その site から*入る・ProcessNodes の flat list は `MusicSiteList`（記録側 eager／resume 側 lazy＝予算の課金順が観測されるのは full collect だけ）・無変更打鍵の collect plain1k 16.8 → 13.7／fingbeam1k 34.5 → 22.2 ms＝gather の全額・最後の checkpoint からの resume が gather する site 9000 → 1／33000 → 1・網 `GatherSeekTests` 5 本）＝**collect 側の残り**＝Debug だけの beam memo 照合（Release 0）・defs walk ~2 ms・v2bow の extra voice live walk（設計）。**第401 が validator 側の残り（28 本 × 全木 walk）を*書き直さずに*閉じた**（`0dbd862e`＝root の全 descendant を 1 回歩いて保持する `DescendantIndex`・`DescendantNodes<T>()` は root なら kind bucket で O(matches)・validator の root `DescendantNodes().OfType<T>()` 20 箇所を `DescendantNodes<T>()` に・pass（fresh tree）plain1k 51 → 11／fingbeam1k 292 → 107／v2bow1k 42 → 9 ms・census 760 冊 435 行が順序込みで一致）＝**診断 pass の残り**＝`SymbolReference`（fingbeam1k 15 ms＝234k 要素の型 switch 2 pass＋`PartReferenceFinder` の scan）／`AnnotationName` 14（実仕事）／`MeasureValidator` 10／`LyricSings` 6.5＝どれも walk ではなく scan。**第401 第 2 便が `SymbolReference` を kind bucket に載せ替えた**（`29a59634`＝`DescendantIndex.OfKinds`＝bucket（pre-order の ordinal）の k-way merge・`SyntaxNode.DescendantNodesOfKinds`・述語の隣に kind 表（`SectionSymbols.DeclaringKinds`／`ReferencingKinds`・`PartReferenceFinder.DeclaringKinds`／`ReferenceKinds`／`TrackKinds`）＝2 綴り目を承知で置き `SymbolKindsTests` の差分の網＋全綴りの手書き 1 冊で押さえる・fingbeam1k pass 107 → 91 ms・census 760 冊一致）＝**診断 pass の残り**＝`AnnotationName` 14（実仕事）／`MeasureValidator` 10／`LyricSings` 6.5／fresh tree の red 実体化 ~19 ms（fingbeam1k・validator が red を読む限り要る）。経緯は §1 第399 第 1〜第 3 便・第400・第401／⒝ ✅ 閉じた（第404・`2f6bba07`・**実機未確認**）＝SVG 全文が打鍵ごとに 2 プロセス跨ぎで ~7 回コピー（`Assemble`→JSON→parse→`svgPostKey` 連結→保持→`postMessage`→`splitPages`＝plain1k で JSON 3.6 MB／打鍵・直列化だけで render と同じ桁）→ **応答を頁にし、閲覧側が持つ絵（`ShownVersion`）との差分だけ送る**（`SvgPageSet`＝頁ごとに Same／Shifted（data-pos を window で写像）／Changed を*比較で*分類・`lilysharp/svg` の `Pages`・webview の `applyPages`）。副産物＝1 頁の本は interactive でも包みが無く webview が毎打鍵 `innerHTML` 全置換だった → 包む。残り＝frame（頁の高さ）が動く本は `requestFull` で全頁・実機の計測は次便。経緯は §1 第404／⒞ debounce 既定 100 ms（`package.json` minimum 100・コードの `DEBOUNCE_DELAY_DEFAULT = 60` は死んでいる）／⒟ ✅ 測って閉じた（第403・コード変更なし）＝`MeasureContentKey.Compute` の「keys+gate 7.2 ms」は JIT tier-0 の数で、最適化済みは plain1k 1.0／fingbeam1k 1.8／v2bow1k 0.8 ms（`DOTNET_TieredCompilation=0`・割当同一）。reference-identity memo は resume 後の Measure／Item が 3 冊 × 3 scenario で 0/1000・0/8000 同一 instance（walk 後の pass が `with { Items = … }` で作り直す）＝当たらないので置かない。経緯は §1 第403／⒠ overlay drawer 28 本が page ごとに全配列を走る（fingering だけ bucket 済み）／⒡ `LayoutStaffGroups`・`StaffSprings` が per-system memo 無し（**第403 実測: system 0 の `LayoutStaffGroups` は 0.01 ms・200 system の loop 全体で 0.6 ms＝小さい**）／⒢ `PlaceSystems` が count 変更打鍵で 2 回（prelim annotation 込み）（**第403 実測: perf 3 冊 × 3 edit で 2 回目は 0 回＝loop は常に ideal を選ぶ**）／⒥ ✅ 一部閉じた（第403 第 2 便・`4f6e5603`）＝`PlaceSystems` が system 0 の silhouette を loop の前に直呼びで建て、edge staff の梁を毎打鍵 quanting していた（plain1k 3.1 ms／3.0 MB・fingbeam1k 3.7／4.4 MB）→ loop と同じ memo に通した／**⒦ ▶ 割当は閉じた・時間は DP そのもの（第403 第 3 便・`c151fcd9`）＝`ChooseSystemCount` の「ideal より多い count」loop（LP `optimal-page-breaking.cc:192-248` の transcription）が plain1k・fingbeam1k で 72 count を毎打鍵 pricing**: 段割り（`sessions/p403/count-loop.txt`）＝`EstimatedSystemDetails` 0.5 ms／3.5 MB・`MinPageCount` 0.4／3.5 MB・`BreakIntoPagesScored` 5.3／5.9 MB（うち `CalcLineHeights` 0.3／3.7 MB・**`SolveUnconstrained` 5.3 ms／2.6 MB＝72 count × 約 240 行 × ページ 1 枚ぶんの後戻り≈29 万 step・18 ns/step＝LP の `Page_spacing::calc_subproblem` の仕事そのもの**）。閉じたのは割当＝count ごとの `CalcLineHeights` 2 回目（`MinPageCountOfLines`／`BreakIntoPagesScoredOfLines`）と行ごとの `new PageSpacing`（1 個を `Clear()`）＝**6.08 ms／13.0 MB → 5.65／7.7 MB**。残る 5.3 ms を減らす手は ⒤ 答えの memo か ⒥ LP と同じ count を試す以上は不可＝意味論の側。**⒤ ✅ 第410 第 4 便が census を取って「やらない」で閉じた**（実コーパス 292／330 冊・8 打鍵ずつ・`Zz410Probe5`）＝⑴ **ink が動く打鍵では count loop は layout の 99.7% で走り、15.9% で実際に段数を変える**（⇒ **止めてはいけない実仕事**。perf 本では 1 度も変えない）が、**1 回 171,376 B ＝実冊の打鍵 6,924,431 B の 2.4%**（perf-plain1k の 30.7% ではない）／⑵ ★★★ **ink が動かない打鍵では layout が 0 回**（2,336 打鍵で 1 度も走らない）＝**全体再利用が count loop ごと既に飛ばしている** ⇒ **memo が当たり得る唯一の打鍵は、そもそも layout ごと飛ばされている**。ink が動く側は estimate が同じ鍵の一部なので門の 13.4% より下＝**賞金は打鍵の 0.3% 未満、鍵は線分割 DP の表まるごと。この島には戻らないこと。**／⒧ ✅ 閉じた（第405・`078e503b`）＝`CalculateVoiceCollisions` が v2bow1k で毎打鍵 3.1 ms／7.5 MB（memo 無し）——測ったら spacing 側の `VoiceCollisionShiftsOf`（`Voice[]` 鍵の静的 CWT＝毎 edit 新品）も同じ全小節走査を毎打鍵していて 2 × 1000 小節／6,000 列／15 MB・filed 0 entry → 答えを小節単位の `VoiceCollisionTable`（訊かれた小節だけ解く）にし、finishing 側は `SystemLayoutCache` の `VoiceCollisions` store（staff × system の slice・shifted hit で振り直し）から file＝解く小節 3／列 8。同じ顔で未計測＝`RestCollisionsOf`（`Staff` 鍵）・`RestDotOffsetsOf`。経緯は §1 第405／⒣ `IncrementalCompiler` の cancel が 3 点だけ（layout 全体が atomic）／⒤ `UsingExpander.FindUsings` が main text を再 parse／**⒨ ✅ 閉じた（第406・`7bc22207`）＝prelim 段（第403 の残り）を engraver ごとに割ったら、注釈 engraver 群は ~0 で、65% は layout 側の梁検出＝`MultiStaffLayouter.StaffBeamGroupsOf`（`Staff` 鍵）／`BeamGroupsOf`（`Voice` 鍵＝毎 edit 新品＝R13⒧ と同じ scratch の顔）が毎打鍵 1000 小節を歩いて 2,000 群（plain1k 2.8 ms／2.8 MB・fingbeam1k 3.4／4.1 MB・edit 位置に無関係）——collect 側の probe は同じ 1000 小節を `BeamDetectionMemo` で 1000/0 replay していた → `SystemLayoutCache.BeamDetection`＝同じ機構の 2 つ目の instance（打鍵ごとに generation・replay は `WithLiveItems` で生きた item に結び直す＝layout の読み手は `Member.Item` を読む）＝歩く小節 1000 → 0（実 session は 1）・検出 0.3 ms／0.8 MB・prelim 段 4.4 → 1.7／5.9 → 2.7 ms。二声（fan）は従来どおり live。経緯は §1 第406／**⒨′ ✅ 同便第 2 便で `p.scripts` も閉じた（`3ff919d0`）＝`ComputeFingeringsAndScripts` を割ったら walk ではなく「誰のためでもない仕事」が 2 つ＝⑴ script も指番号も無い本が pass ごとに表 3 つと unit 200 件の probe を回して空を返す（plain1k 0.44 ms／1,445 KB × 2 pass）→ 入口の門（`AnyFingering`・下流 guard の前倒し・memo が 0/0 になることで発火を数える）／⑵ memo が 199/200 hit なのに miss 1 本のために全曲 8,000 件の beam tip 表を pass ごとに畳む → miss は自分の梁だけ畳む（鍵が一意＝上書き不能）。打鍵あたり plain1k 0.87 ms／2.9 MB・fingbeam1k 0.50 ms／1.7 MB。網 2 本（既存の 4 本は全部梁なしで tip 表に届いていなかった）。経緯は §1 第406 ⑷**。残り＝`p.tail` 0.3 ms（grace／hyphen／tie-variant／MMR／ledger／stanza の全木 walk）・v2bow の ties／slurs 検出 0.5 ms・`p.augment` 3 MB（bow の paging program）・fingbeam1k の `fs.walk` 0.41／`fs.assemble` 0.29（24,000 digit の実仕事＝返す物を変える別の島）。／**⒨″ ✅ 第407 が「1 つの量が N 綴り」を pass 全体で数えて畳んだ（`c18ca01a`）＝第406 第 3 便が尾部の 3 家を畳んだ後も、同じ小節→system／→(system, layout) の表は pass のあちこちで建っていた**（pass 自身の staffY resolver・pedal 参照・`StackBelowStaff`・`StackAboveStaff`（`StackAboveStaffMemoized` が建てた直後に `StackAboveStaffCore` が建て直す＝註が「the same map the core builds (**cheap**)」と自白していた）・`ComputeFingeringsAndScripts` の `layoutOf`／`systemOf`・`ArticulationEngraver` の `layoutAt`）。**計器で数えた打鍵あたりの fill＝plain1k 14／fingbeam1k 18／v2bow1k 16・全部 1000 小節の走査・edit 位置に無関係**（`sessions/p407/mapfills-before-raw.txt`）。⇒ **pass の頭で 2 本建てて配る**（既定 null で各家の自前建設は残す・`layoutOf` は鍵集合と 1 参照しか使われておらず丸ごと消えた）＝**14 → 8／18 → 8／16 → 10・pass 自身は 5〜9 本 → 2 本**（`mapfills-after-raw.txt`）。⚠️ **時間はこの機械の床以下＝時間の主張なし**（割当は打鍵あたり 0.5〜1 MB 減だが桁が小さく、根拠は回数）。健全性＝全綴りが重複 MeasureIndex で「最後の system」を採る（unit plan がその性質から導かれている）・snapshot 249 不動・**毒: 共有した system 表で 205 本、layout 表で 148 本が赤**＝どちらの腕も無観測ではない。網 3 本 `MeasureToSystemHandoffTests`（自前建設の腕＝pass が通らない呼び手を押さえる・memo あり／なしの両経路・**わざと壊した表で赤くなるのを確かめてから採った**＝最初の below 側は壊れた表でも緑だった）。**残りは第 2 便で数えて「畳まない」と決めた（コード変更 0・`sessions/p407/leg2-before-raw.txt`）**＝打鍵あたり 8 本の内訳は **注釈 pass 4（2 種 × 2 pass＝床。pass ごとに systems が別の配列）**＋`PagingSkylines` 2 ＋（梁のある本なら）`Prelim.LayoutBeamsMemoized` 1 と `CalculateRestShifts` 1、（二声なら）`Prelim.MeasureToSystemOf` 2。⇒ **畳める余地は 1 冊 2 本**で代償は `RunPreliminaryAnnotationPass` → 3 つの per-staff ヘルパ＋public `CalculateRestShifts` の署名＋final ループからの hoist＝**配管のほうが大きい**。★★★ **`PagingSkylines` の 3 箇所は*畳めない*＝別の量**（`n = Math.Min(perSystemExtents.Count, systems.Length)`／`s < systemSkylines.Count`／`s < systemCount`＝**狭い表**で、full を渡すと後ろの system の小節が解決するようになる）——**「同じ表か」は字面ではなく*境界*で確かめること**。`MusicMarkEngraver` の 2 本も別の量（staff group の bottom）。⚠️ 留保: 前 2 者は per-staff ループの中なので構造上は staff 数で増えるが、**梁のある多 staff の本が木に無く測れない**（`perf-hairpingrand1k` が 0 本なのは 4 分音符だけで梁が無いから＝両方とも早期 return）。**この島には戻らないこと。** 経緯は §1 第407 ⑸。／**⒩ ✅ 閉じた（第408）＝診断 pass の最後の大物 `AnnotationNameValidator`（第401 実測 fingbeam1k 14〜19 ms）は「walk ではなく scan」ではなく*3 つとも*だった**。**計器で割った打鍵あたり**（全文 `sessions/p408/annotationname-summary.txt`・probe 3 本 `Zz408Probe`／`Zz408Counts`／`Zz408Census` は commit せず Lab へ・Release・TC=0・min-of-5）: ⑴ 全木 walk＝**234,030 節点を歩いて 24,000 の注釈に会う**（0.57 ms）／⑵ **dotted `MarkName` を 24,000 本組み立てる**（2.0 ms・3.07 MB＝**報告する枝しか読まない＝誰のためでもない仕事**）／⑶ **`Arguments` を 24,000 回建て直す**（4.53 ms・15.74 MB＝**1 個 656 B**）。`IsKnownCompoundName` の費用は `Named` 1 回（0 B）＋`Arguments` 1 回で*全部*＝**述語の鎖ではなく「建て」が費用**。⇒ 直し 3 本: ⑴ kind bucket（`AnnotationKinds`＝第401 の `SymbolReferenceValidator` と同じ綴り）／⑵ `MarkName` を*読む枝の中*へ（`name == "chord" && Parent is X` を `Parent is X && name == "chord"` に＝**両辺が純粋なので意味は動かず建つ物だけが変わる**）／⑶ `MarkArgument.FromTokens`・`ArgumentTokens` が**その引数が実際に持つ形だけ**建てる（1 token の run はその token の `Text`・1 引数は list 不要・exact-size＋`DrainToImmutable`）＝**run の切れ目は不変**。**後**: 節点 234,030 → 24,000・`MarkName` 24,000 → 0・**18.82 MB → 6.92 MB／6.81 → 3.13 ms**。★★★ **per-node cache は counter で「当たらない」と決めた**＝cold pass 72,000 回でも**実 keystroke の render は 48 回**（incremental collect が memo 済み）＝**validator の 24,000 は新しい木の初回 touch**。⚠️ **validator に安い述語を書き直すのは禁じ手**（VALUE_SITE_AUDIT §9.3）⇒ 唯一の綴りを軽くしたので **collector の 48,000 回も同じだけ軽い**。健全性＝census 435 行が順序込みで一致＋**毒 4 腕**（33／175／20／9）＋網 `AnnotationKindsTests` 2 本。汎化は RULES §5.3。**残り＝1 個 288 B**＝**`AnnotationValues` が*何を返すか*を変える別の島**（第406 の `fs.walk` と同じ形）。経緯は §1 第408。／**⒪ ✅ 閉じた（第408 第 2 便・`a743726e`）＝尾部 11 本の「全木 scan」を*綴りを 1 つも増やさずに*軽くした**（全文 `sessions/p408/walk-shape-summary.txt`・probe は `Zz408Probe2`／`3`／`4`）。★★★ **まず一覧を取り直した**——§1 が名指していた順位は**第401 が*前の機械*で測った数**で、この機械では違う（fingbeam1k の pass は 44.4 ms fresh／21.0 warm）。⇒ **古い機械の一覧で validator を割らない。** 尾部＝**11 本が `foreach (var n in root.DescendantNodes())` で 234,030 節点を舐めて一握りに会う＝打鍵あたり約 2.6M 節点訪問**。bucket 化は 11 本の 2 綴りと網が要るので**やらず**、「その scan の何割が*列挙*か」を測った＝**IEnumerable 0.56 ms 対 配列 0.40 ms**。**root では `DescendantNodes()` は既に index の*配列*を返していて `IEnumerable` に入れていた**＝要素ごとに interface 越しの `MoveNext`。⇒ **戻り値を構造体 `DescendantNodeList` にした**（`GetEnumerator` が構造体＝`foreach` はそれに束縛・`IEnumerable` も実装するので **呼び出し側を 1 箇所も変えていない**）。**A/B（stash 往復）**: **pass 44.4 → 39.9 ms・warm 合計 21.0 → 17.3 ms**（`MeasureValidator` 5.17→4.48・`LyricSings` 2.69→2.40・`Duration` 1.79→1.34・他 7 本も 0.8 台 → 0.5 台）・**scan あたり割当 72 → 40 B**。★★★ **対照が内蔵＝`AnnotationName` は第 1 便で bucket に移したので*動かない*（3.68 → 3.66）。** 健全性＝census 435 行一致・**毒 309 本**・⚠️ **`DescendantIndexTests` が唯一の実害を捕まえた**＝「built once」が**戻り値**に `Assert.Same` していたが戻り値は値になった＝**性質は包んでいる側に在る**ので網をそちらへ。汎化は RULES §5.3。**測って「やらない」と決めた分の数は summary に**（`MeasureValidator` 3.94 の内訳＝`CollectPhraseBodies` 0.64（この冊では*空の辞書*を建てる全木 walk）・`CrossPart` 1.08・`ValidateNode` 再帰 2.21）。⚠️ **この便が「再帰は flat scan に畳めない」と書いたのは*不正確*で、第 3 便が畳んだ（下の ⒫）＝正しくは「*刈り込みを平坦リストが表現できない*」で、**刈り込みは*祖先*に訊けば済む**。／**⒫ ✅ 閉じた（第408 第 3 便・`c168b220`）＝その「畳めない」はずの再帰を畳んだ**。ユーザーの問い「再帰を繰り返しに書き直したら良い形になるか」から出発して**traversal だけを抜き出して測った**（**全文 `sessions/p408/traversal-summary.txt`**）: ⑴ **明示スタックの繰り返しは*遅い*（1.44 → 1.68 ms）**＝**再帰という「形」は費用ではない**（第401 が反復化で効いたのは*イテレータが階層ごとに連鎖*していた別の病気）。⑵ ★★★ **刈る walk は 22 ns/節点・平坦配列は 2.2 ns/節点**＝**「全部見る 234,030」のほうが「刈って 65,009 だけ見る」より速い**（0.52 対 1.44 ms）＝費用は*節点*でなく**slot あたり**（節点ごとに全 slot へ `GetChild`）。⑶ そして **65,009 節点を歩いて着くのは*2 節点*だった**。⇒ **switch が仕事をする 4 kind（`MusicBlock`／`SectionDeclaration`／`TimeSignature`／`PartialDeclaration`）を index に訊き、刈り込みは*候補の祖先*に訊く**（`IsPrunedContainer` に 1 本化・**「なぜ刈るか」＝報告された不具合の経緯はそのまま移した**）。**A/B（stash 往復）: whole 3.39 → 2.33 ms・walk 2.12 → 1.06**（残りは検査の実仕事）。網 `MeasureValidatorWalkTests` 9 本＝**旧再帰を*テスト側に*reference として残し**（**kind 表でなく switch と同じ*型*を名指す**＝ただで一致しないように）全 net 冊で**同じ節点・同じ順序**＋刈る容器ごとの手書き 7 冊。毒 8／8 本・census 一致。／**⒬ ✅ 閉じた（第409 第 1 便・`310f5ea9`）＝`LyricSings` の全木 3 周を index に訊いた**。計器は `sessions/p409/`（counts と validator 別 min-of-5・Release・`Zz409*.cs.txt`）。**打鍵あたり 3 × 234,030＝702,090 節点訪問して会うのは *2 節点***（fingbeam1k の part 宣言 2 本だけ。net 冊 259 では 140,805 → 約 812）・**2.14 ms／割当 464 B＝時間はほぼ全部「歩き」**。⇒ ⑴ `DeclaredName` は既存の `DeclaringKinds` ⑵ `CollectVoiceNames` に**新 `VoiceIntroducingKinds`**（`ParallelExpression`＝`voice sop { } alt { }` と復旧形の `<< … >>` は同じ節点）⑶ `SingsOf` に**新 `SingsKinds`**（`LyricsBlock`／`LyricsRowRender`＝`PartReferenceFinder.SingsTargetToken` が答える 2 site と同じ）。⚠️ **`LyricBindings` の 3 本も同じ病気だった**——`Conflicts` は毎回全木、`BuildMap`／`BuildVoiceMap` は CWT で覚えるが**木は打鍵ごとに新しい**＝毎打鍵 1 回は払う。前 2 者は `DescendantNodes<LyricsBlockSyntax>()`、後者は `DeclaringKinds`（**part の*中*の walk は subtree なので lazy のまま**＝index は root だけ）。⇒ **probe の床 0.05 ms を下回って一覧から消えた**・pass 41.2 → 36.9 ms fresh／14.7 → 12.5 warm。網 `LyricSingsWalkTests` 8 本＝**旧全木 walk をテスト側の reference にし**（kind 表でなく*型*を綴る＝ただで一致しないように）全 net 冊で同じ節点・同じ順序＋**あらゆる歌詞の綴りを書いた 1 冊**＋束縛（`Conflicts`／`TargetOf`／`VoicesOfPart`）を平坦 walk の答えと突き合わせ。毒 3 腕＝`SingsKinds` から row を落とす（赤 2）／`VoiceIntroducingKinds` を空（赤 3・うち 1 本は**実在の本で validator が黙る**のを捕まえた）／`DeclaringKinds` から `PartBlock` を落とす（赤 3・`SymbolKindsTests` 込み）。census 760 冊 435 行 73,495 バイト**同一**。／**⒬′ ✅ 閉じた（第409 第 2 便・`b0298b08`）＝尾部 9 本を同じ型で畳み、⒤ は*全部*終わった**。★★★ **まず数えた**（`sessions/p409/tail-counts.txt`）＝234,030 節点に対し switch が会えるのは **`Duration` 8,000（3.42%）・`RepeatStructureScope` 1,000・`ScoreSettingInPartHeader` 1・残り 5 本（`ChordRepetition`／`BareDuration`／`OverrideVocabulary`／`PhraseCycle`／`DuplicateGlobalSetting`）は *0***。★★★ **形は 2 つあり、安いほうは代償ゼロ**: ⑴ **1 型なら kind 表は要らない**——`ChordRepetitionValidator`／`BareDurationValidator` は `DescendantNodes<T>()` に替えるだけで**2 綴り目が存在しない**（`DescendantIndexTests` が既に型の答えを平坦 walk に対して pin している）。⑵ **switch なら kind 表を隣に置く**（`PartReferenceFinder.ReferenceKinds` の形）——**述語に切り出せるものは切り出して*その述語に訊く*網**にした（`DurationValidator.DurationOf`・`DuplicateGlobalSettingValidator.SettingKindOf`・`OverrideVocabularyValidator.CommandOf`・`RevertContextValidator.DirectiveKindOf`・`ScoreSettingInPartHeaderValidator.SettingKindOf`＝**式を動かしただけで書き換えていない**）。**型束縛と長いメッセージを抱えた case は切り出せない**ので（`RepeatStructureScopeValidator`・`PhraseCycleValidator`）**テスト側が型を綴って平坦 walk と index を突き合わせる**（RULES §7.7 の差分の網）。⇒ **pass 36.9 → 30.9 ms fresh／12.5 → 7.5 warm**・9 本すべてが床 0.05 を下回った（**唯一の例外 `Duration` 1.17 → 0.54**＝8,000 節点に本当に会うから）。**対照が内蔵**＝`AnnotationName` 3.57 → 3.48・`MeasureValidator` 2.77 → 2.78 は**動かない**。網 `TailValidatorKindsTests` 3 本・**毒 7／7 本**（7 つの表から 1 kind ずつ落として 7 回・毎回赤）・census バイト同一・snapshot 249 不動。⇒ ★★★ **残った教訓＝「2 綴りの代償」は*switch の形*で決まる**——**1 型の filter は index に訊いても綴りが増えない**ので、尾部を見るときは**まず「この validator は何型を見ているか」を数える**（9 本のうち 2 本がこれだった）。
