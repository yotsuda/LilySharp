...
最終更新 第379セッション（2026-09-13）＝**入り方は第298〜第377 の型**（ユーザーは `docs/HANDOFF.md` を読ませて「作業に着手して」だけ）。**§0 の裏取りを走らせた**（`Session-Check -Build -Test -Scratch p380`＝規約どおり第379＝`p380`）。道具は記憶どおり pwsh MCP。**着手したのは第377 ⑶ の要ユーザー判断 2 件**＝最初にユーザーに訊いた。**骨は 6**:

★★ **⑴ 開始時の数は第378 末の記録と全部一致**: HEAD `1bedfc1d`・未 push 82・木 clean・未追跡 0・**台帳 843 点／ss 非ゼロ 210／総和 22.584587806／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249・追跡 `.lys` 599**・lysc は net10.0 だけ・build 0 エラー・**full `scratch/p380/run1.trx` 8519 / 0 / 4 / 8523**（RunInfos は SKIP 4 だけ）。

★★★ **⑵ ユーザー決定（2026-09-13・第379）**: **`spacingIncrement`＝配線する**／**`topSystemPadding`＝退役**（§2 E ⒠ の 2 件・どちらもディスクの `.lys` に 0 冊）。

★★ **⑶ 退役（commit `1d762e0b`）**: `ScalarKeys`・`LayoutOptions.TopSystemPadding`・LSP の help・tmLanguage・`GRAMMAR.md`・`GRAMMAR_FOR_LLM.md` から除いた。**書いた本は `UnknownPaperKey` で、鍵の一覧ではなく `topSystemSpacing { padding 1 }` を名指す**（`PaperBlockTests.TheRetiredTopSystemPadding_NamesTheSpecThatSpellsIt`）。full `run2` 8519 / 0 / 4 / 8523。⚠️ **途中の赤 1 本（`EditorColouringTests`）は古い DLL**＝build が落ちていた: **solution は `LilySharp.slnx`（`.sln` は無い）**＝`dotnet build LilySharp.sln` は MSB1009 で何も建てず、`--no-build` の test が前の DLL を走らせた。

★★★ **⑷ 配線（commit `f25e830f`）＝`SpacingOptions(GlobalShortest, Increment)`＝LP の `Spacing_options`**（`spacing-options.cc:30-53`）。**2 つを 1 つの値で運ぶ**: `get_duration_space`・`note_spacing` の min（`spacing-basic.cc:152`）・`standard_breakable_column_spacing`（`:53-55`）・左頭の refinement の `- increment`（`note-spacing.cc:77`）・knee の頭幅の種（`:120`）。**作るのは `score.Paper` から**（`SpacingOptions.For`）＝`MultiStaffLayouter.LayoutMeasures`・`SystemBreaker`（gate の static と greedy）。**`LayoutEngine`・`IncrementalCompiler`・`SystemLayoutCache` は double のまま**（paper の編集は cache を捨てる）。grace の 0.8（GraceSpacing 自身の increment）は触っていない。**LP 2.26.0 実測**（`scratch/p380/incr/incr.ps1`＝`lysc ly --pin-fonts` 双子に `Score.SpacingSpanner.spacing-increment`・ragged・四分／八分／十六分／skip だけの小節／全音符）: **1.2 / 1.5 / 1.8 で両エンジンとも 1 段の譜が 74.95 / 86.89 / 98.83**（配線前の Lily# は 3 つとも 74.95）⇒ `SpacingIncrementTests`。**2.4 は LP が改行する**＝この読みの外。**摂動網**: `PaperKeysWithNoReader` は空になって消え、`spacingIncrement` は **`PaperKeysThatNeedARaggedLine`**＝**justified な `PaperBook` は各段が等しい四分 1 小節なので 5mm と 25mm が同じ頁**（ragged は動く・`scratch/p380/incr/vp`）＝**鍵ではなくフィクスチャ**。test の呼び手 60 か所は `scratch/p380/incr/fix-tests.ps1`（件数を主張する置換表）。生成物 2 本は再生成（行番号だけ・LF で出るので CRLF に戻した）。

⚠️ **⑸ 計器の罠 3・未確認の疑い 1**: ① **`incr.ps1` の初版は「最右の譜線の x2」を読んだ**＝LP の 2.4 は 2 段（75.60＋50.48）で 75.60 と出た⇒**全段の譜線長の和と段数**に直し、**段数が揃う行だけ比べる**。② **PowerShell の変数名は大小を区別しない**＝結果を `$lp` に入れたら **`$Lp`（LP の exe パス）を上書き**し、2 つ目の値で cmd に object を渡した。③ **pwsh MCP で here-string を `git commit -F -` に流すと stdin に届かず pathspec 扱い**⇒ `var1` を `$var1 | git commit -F -`。✅ **疑い（第 2 便で測って閉じた＝⑻・床は発明だった）**: `SpacingRules.ApplyLeftHeadWidth` は refinement 後の ideal を **`Math.Max(increment, …)` で床**にするが、LP の `note-spacing.cc:77` にその床は無く、`:113` の `max (0.0, ideal)` は stem correction の後＝**Lily# の床は発明の可能性**。**読みが動くのは「duration の ideal − increment ＋ 左頭幅 < increment」の対だけ**（短い音価と大きい increment）＝**`p380/incr` の双子で問える**。

★★ **⑹ 数**: **開始時**は上 ⑴。**終了時**: HEAD＝この §1 を書く commit（親 `f25e830f`）・未 push 85・**full `scratch/p380/run4.trx` 8523 / 0 / 4 / 8527**（**+4＝新 test 5 − 摂動網の case 1**（`topSystemPadding` が鍵の一覧から消えた）・RunInfos は SKIP 4 だけ）・**台帳 843 点／ss 非ゼロ 210／総和 22.584587806・snapshot 249・追跡 `.lys` 599 は全部不変**（追跡コーパスに `spacingIncrement` を書く本は 0）。§7.5: **退役 `1d762e0b` は Core `+` 10 行／REF 0／OWN 0**（⒟ 削除＝観測者は `PaperBlockTests`・摂動網）・**配線 `f25e830f` は Core `+` 174 行／REF 6／OWN 0**。§7 3.5: 第377 を `-Archive 377` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**＝push はユーザー。★ **LSP の配布はユーザーが行う**（Core・Lsp が動いた）。

⇒ ★★★ **⑺ 次の一手**: **§2 E「section label と和音行の上下」＝要ユーザー判断のまま**（第378 ⑸）。残りは第377 ⑸ の並び＝**⒝ 摂動網の残り**（chord quality 35・`@` の引数つき形）／**⒠ drummap の `${1:hh}`＝UX 案件**／**⒢ 第369 ⑸⒝・第368 ⑺⒝ の並び**／§2 E の副産物（内容で決まる頁の高さ）／**新: ⑻ の残り 2 件**（floor2 の 1.2 で −0.45・`CreateTimingSpring` の床）。★ **判定の記録**（常設指示「次便は、このセッションでやる方が有利なら着手して…」）: 第 1 便末は「着手しない」と書いたが、**ユーザーが同じ指示を出し直した時点で判定し直し、第 2 便は着手した**＝⑸ の疑いは**未測**で、**`p380/incr` の双子＋override 挿入＋譜線長の読みがそのまま噛み合った**（記憶 `feedback_lilysharp_next_leg_only_if_session_tooling_pays` の (c)）。**第 3 便は着手しない**＝⑻ の −0.45 は**床と無関係な別の量**で、まず列ごとの X を読む計器（LP の column dump）を新しく作るところから＝`incr.ps1` の譜線長の読みでは局所化できない＝次のセッションでやっても損しない／`CreateTimingSpring` の床は**観測 0 の縁**（shortest playing が無い列の fallback だけ）／section label はユーザー待ち。

★★★ **⑻ 第 2 便＝⑸ の床の疑いを測って閉じた（commit `7e539a90`）**。**計器**: `scratch/p380/incr/incr.ps1` に `-Base` を足し、**`floor2.lys`＝八分の小節 3 つ＋32 分を含む小節 1 つ**（**32 分が 1 小節だけ**なので common shortest は八分のまま＝32 分が `get_duration_space` の線形枝に入り、refinement 後の ideal が increment を割る）。⚠️ **最初の `floor.lys`（全小節に 32 分）は床が効かない本だった**＝common shortest が 1/32 になり r=1＝**3 値とも LP と一致して何も言えない**（陽性対照が要る・記憶の「陰性には陽性対照」）。**LP 2.26.0 と Lily#（ragged・譜線長の和／段数）**: increment 1.2＝**LP 96.84／Lily# 96.39**・2.0＝**126.33／126.64**・3.0＝**157.85／161.64**。**床を外すと 3.0 は 157.85（−0.00）・2.0 は 125.85（−0.48）・1.2 は 96.39（不変）**、`base.lys` の 3 値も不変＝**床は LP に無い発明**（`note-spacing.cc:77` に床は無く、`:113 set_ideal_distance (std::max (0.0, ideal))` は stem correction の後＝呼び手の `MergeVoiceStemWishes*` が同じ 0 clamp を持つ）⇒ `Math.Max (0.0, …)`。★★ **ユーザー指摘で字面に直した（commit `e584ddf2`）**: `7e539a90` は 0 clamp を refinement 自身＝**`:77` の位置**に置いていた（LP の `:77` に clamp は無い）⇒ **`ApplyLeftHeadWidth` は `:77` の値を clamp せず返し（負もありうる）、`:113` の clamp は各呼び手の「stem correction の後」に置いた**＝`MergeVoiceStemWishes*` は**全 wish を無条件に**（従来は補正 0 のとき素通り）・`MeasureLayouter` の列間／小節線 spring は merge の後（**左が休符の wish は `NoteColumnAt` が拾わない**ので漏れていた）・`CreateSpringsForMeasure` の音符間 leg は refinement の後（`CreateSpring` が補正を先に足している）・`MidMeasureChangeGaps` の左 gap（`:105-107` の結果）。**出力は測った所すべて不変**（`base`／`floor2` の LP 一致・追跡 599 冊を `7e539a90` と svg 比較＝動いた本 0）・full `literal2.trx` 8524 / 0 / 4 / 8528・Core `+` 32 行／REF 6／OWN 0・生成物 2 本は再生成（新しい Green 行 2＝`:113` を引く 0.0）。**網**: `SpacingIncrementTests.AShortNoteUnderALargeIncrement_IsNotFlooredAtTheIncrement`（157.85・2 段）。**射程＝追跡 `.lys` 599 冊を `32981b70` の worktree build と今の木（Release・`--batch`・`scratch/p380/incr/sweep-floor.ps1`）で描いて svg バイト比較＝動いた本 0**（両側とも同じ 1 冊が失敗・陽性対照＝floor2 の 3.0 を両 exe に通すと異なる）＝**1.2 では床が効く対が既存コーパスに無い**（黒符頭の右端 1.18 が線形枝の不足を埋める）。worktree は消した。⚠️⚠️ **残り 2 件（未着手）**: ① **floor2 の 1.2 で Lily# が LP より 0.45 短い**（2.0 で −0.48・3.0 で 0.00）＝**床とは別の量**・32 分の小節が要る（`floor.lys`＝全小節 32 分では 0.00）＝**線形枝か 32 分の梁／符尾補正の側**が候補・未読。② `SpacingRules.CreateTimingSpring` にも `Math.Max (idealDistance, defaultMin)` の床（LP の `note_spacing` に無い）が残る＝呼ばれるのは `CreateTimingSpringMultiVoice` の shortest playing ≦ 0 の fallback だけ。**数**（第 2 便末）: HEAD＝この追記の commit（親 `7e539a90`）・未 push 87・**full `scratch/p380/floor2.trx` 8524 / 0 / 4 / 8528**（+1＝網 1 本）・台帳 843 点／総和 22.584587806・snapshot 249・追跡 `.lys` 599 は不変・§7.5（`7e539a90`）: **Core `+` 11 行／REF 1／OWN 0**。★ **LSP の配布はユーザー**（Core が動いた）。

★★★ **⑼ 第 3 便＝⑻ の残り ①（floor2 の 1.2 で −0.45）の原因を特定した＝`Ledger_line_spanner::set_spacing_rods` の未移植（製品 0・commit は docs だけ）**。**着手の根拠**: 未測で、`p380/incr` の双子・LP 実行がそのまま使え、読みを 1 段足せば届いた（記憶の (c)・第358 実例 2 の型）。**計器**（全部 `scratch/p380/incr`）: ⒜ **`cols.ps1`＝符頭→符頭の間隔を両エンジンで 1 つずつ**（LP は `nhdump.ily`＝`NoteHead.after-line-breaking` の system 相対 x、Lily# は svg の `<text class="music" data-pos>` の data-pos ごとの最左）。⚠️ **罠**: LP の自前出力が record に割り込む＝最初の NH は `Drawing systems...` と同じ行、**`warning: ignoring unsupported formats (pdf)` は record の中（`NH x=` の直後）に入った**＝**行単位で読むと 33／34 本**⇒ **log 全体から前後の改行ごと warning を除いて record 形で探す**。⒝ `skydump2.ily`＝NoteColumn と PaperColumn の `horizontal-skylines`（`ly:skyline->points (car|cdr sp) Y`・⚠️ **2.26.0 に `ly:skyline-pair::skyline` は無い**）と Stem の extent。⒞ `posdump.ily`＝符頭の staff-position／octave。**結果**: ⑴ **差は 32 分の 3 間隔だけ**（a''→b''→c'''→d''' が **LP 1.9563／Lily# 1.80**・g''→a'' は両方 1.80・他は ±0.01）。⑵ **increment 1.0／1.2／1.5 で 1.96 のまま不変**＝ideal ではなく rod／min 側。⑶ **両エンジンの skyline は同じ**（頭 1.4042／符尾 0.23／左 −0.1＝距離 1.5042＋0.3＝1.80）＝**skyline では 1.96 が出ない**。⑷ **双子は octave も一致**（`\fixed c'` の `c'8`＝C5・Lily# の `c'` も C5＝記憶 `reference_lilysharp_twin_fixed_c_octave_reading`）⇒ **g''（pos 5）は加線 0、a''〜d'''（pos 6〜9）は加線 1〜2**＝**差の出る 3 対は「両側とも加線を持つ」対だけ**。⑸ **LP の `lily/ledger-line-spanner.cc:39-140`**: 同じ側（UP／DOWN）に加線付きの頭を持つ隣接列の間に **rod `2 × (head width × minimum-length-fraction) − 右列 head extent[LEFT] + 左列 head extent[RIGHT]`**（`minimum-length-fraction` 0.25・`define-grobs.scm` LedgerLineSpanner）＝**2 × 1.3042 × 0.25 + 1.3042 = 1.9563**＝実測と桁まで一致。**Lily# の `LedgerLineSpannerEngraver`（:48 がこの関数を REF で名指す）は annotation pass で span を作るだけで、spacing に rod を 1 本も出さない**。★ **判定＝第 3 便は移植に着手しない**: ⒜ **出力が動く**（加線上の短い音価が並ぶ所＝32 分や、common shortest が八分のときの 16 分）＝**承認が要る側**（記憶の ⑴）／⒝ 設計が要る＝**LP の rod は Staff 単位の spanner の中の「隣接する加線付き列」**＝他の譜の列を挟むと複数 spring をまたぐ（`looseRods`／`SpringSolver.ApplyRods` の形）・**小節線をまたぐ対**も rod を持つ・item 側の estimate（`CreateSpringsForMeasure`）と gate にも同じ rod が要る（2 系統の一致＝`SpacingInvariantTests`）／⒞ 計器（`cols.ps1`・`skydump2.ily`・`posdump.ily`・`floor2.lys`）はディスクに残る＝**次のセッションでも同じ所から始められる**。⇒ **要ユーザー判断＝§2 E に起票**（移植するか・射程は patch を作って追跡 599 冊を `sweep-floor.ps1` で数える）。**数**（第 3 便末）: HEAD＝この追記の commit・**製品コード無変更**（full は第 2 便末の `literal2.trx` 8524 / 0 / 4 / 8528 のまま）。

★★★ **⑽ 第 4 便＝ユーザー承認「移植を承認する」で加線の spacing rod を移植した（commit `c6f716e8`）**。**実装**: ⒜ **`SpacingRules.LedgerRods.cs`**＝`LedgerColumnsOf`（Staff の全 voice・和音の displaced 頭は `ChordHeadPositioning.CalculateOffsets`・衝突 voice は `VoiceCollisionShiftsOf` の shift・加線の判定は renderer と同じ |pos| ≥ 6・grace は除外）／`LedgerRodDistance`（`2 × 左列 head width × 0.25 − 右列 extent[LEFT] + 左列 extent[RIGHT]`・UP／DOWN の大きい方）／`ApplyLedgerLineRods`（小節内の対を `SpringSolver.ApplyRods`）。⒝ **小節内は `MultiStaffLayouter.ApplySharedColumnReservations`**＝レイアウトと改行 gate の両方が読む（bare timing springs は触らない＝`SpacingInvariantTests` の 2 系統一致はそのまま）。⒞ **小節線をまたぐ対は `LayoutMeasures` の system rods**（間の小節線インクを引く＝cross-bar lyric rod と同じ規約）・⚠️ **gate は cross-bar の対を値付けしない**（名指して残した）。**台帳 +2（両方 EXACT）**: **`ledger.rod.thirty-second`**（新 probe `audit/lp-geometry/probes/ledger-rod.ly` score LR＝a'' の 32 分・LP 1.956300＝移植前の Lily# は 1.804200）と対照 **`ledger.rod.thirty-second.no-ledger`**（score LN＝g''・加線なし・1.804200）。⚠️ **`NoteheadAnchorsOnSystem` は頭の Y で system を分ける＝1 本の本は 1 pitch に限る**（floor2 は使えなかった）。**網**: `LedgerLineRodTests`（floor2 の 3 間隔・svg の F2 量子化で ±0.011）。**snapshot 再ベース 5**（`showcase/02-ornaments`・`test/beam-over-stem`・`test/beam-rest-extreme`・`test/multivoice-tuplet-beams`・`test/notes`＝正当化は `ledger.rod.thirty-second`）。**射程＝追跡 599 冊のうち 9 冊が動いた**（`267764b7` worktree と svg 比較）。**動いた本は LP に近づいたかを測った**（`scratch/p380/incr/verify-moved.ps1`＝ragged 双子の譜線長・base → head の残差）: beam-over-stem −0.05 → 0.00・beam-rest-extreme（fixture と audit の 2 冊）−0.31 → 0.00・notes −0.31 → +0.01・**samples/canon-in-d −17.66 → −1.72**・02-ornaments／perf-sd5 は ragged で不変（動いたのは justified）・feature-tour は段数が違って比較不能（19 対 20）。⚠️⚠️ **multivoice-tuplet-beams だけ +0.29 → +0.64 と総和が悪化したが、間隔ごとには正しい**: LP は加線付きの c''' → a'' の列を 1.9563 に rod し（Lily# は 1.60 → 1.95）、**残る +0.65 は別の古い差＝3 連符の 2 つ目の列が LP では八分の 0.80 後、Lily# では 1.45 後**＝**rod の欠落がその差を半分隠していた**（未起票の新しい差・§2 に書く価値あり・未着手）。⚠️ **計器の罠 2**: ① **`git show HEAD:<file>` は正規化済みの blob を返す＝改行は常に LF**＝それを見て「元は LF」と戻したら作業コピーが LF になった（autocrlf=true の checkout は CRLF・commit は正規化されるので実害なし・作業コピーは CRLF に戻した）。② svg の X は F2＝LP の 4 桁の値と `Assert.Equal(…, 2)` で比べると 1.95 と 1.96 の丸めで落ちる。**数**（第 4 便末）: **full `scratch/p380/run5.trx` 8527 / 0 / 4 / 8531**（+3＝網 1 本＋台帳 2 点）・**台帳 845 点／ss 非ゼロ 210／総和 22.584587806（不変）／exact 670（+2）／OPEN 0**・snapshot 249（5 枚は内容更新）・追跡 `.lys` 599・§7.5（`c6f716e8`）: **Core `+` 264 行／REF 6／OWN 0**・未 push 91（この追記込み 92）。★ **LSP の配布はユーザー**（Core が動いた）。

★★★ **⑾ 第 5 便＝⑽ の「3 連符の列」の差の原因を特定した＝rod を読む paper column の skyline の縦 padding が 0.15（NoteColumn の値）で、LP は 0.08（PaperColumn の値）（製品 0・commit は docs だけ）**。**着手の根拠**: 未測で、第 4 便の計器（`scratch/p380/incr/verify/` の双子・`voicedump.ily`・`skydump2.ily`）がこの本にそのまま噛み合った。**計器**: 捨て計器 `Zz379ProbeTests`（**退避済み＝`scratch/p380/incr/Zz379ProbeTests.cs.txt`**・出力 `zz379-springs.txt`）＝小節の spring 鎖を bare／`ApplyCrossVoiceColumnSpacing` だけ／`ApplyLedgerLineRods` だけ／全 reservation で並べ、対の 2 項目の skyline の building を刷り、**private の `ItemSkylineFactory.Boxes` を reflection で呼んで同じ箱を別の padding で組み直す**。**結果**: ⑴ **LP の列**（`voicedump`）: 1/8（voice 2 の f''）→ 1/6（voice 1 の 3 連符 c'''）＝**0.80**・1/6 → 1/4＝1.604。Lily# は 1.45／1.60。⑵ **Lily# の spring 2（1/8 → 1/6）は bare で ideal 0.8000・min 0**（wish の無い対・shortest playing 1/8・fraction 1/3・len 2.4＝LP と同じ）→ **`ApplyCrossVoiceColumnSpacing` の rod が min を 1.4492 に上げる**（加線 rod ではない＝`ledgerOnly` は 0.4630 で効かず、移植前の base も 1.45）。⑶ **skyline の形**: LP（`skydump2`・PaperColumn）＝f'' の頭の上端と c''' の箱の下端（最初の加線の位置まで伸びる＝`include-ledger-line-height`・Lily# も同じ形）が**縦に 0.135 離れる**＝rod 0。**Lily# は両方を 0.15 で太らせて 0.14 重なる**＝rod 1.4492。⑷ **陽性対照つきの決め手**: 同じ箱を padding **0.15 で組み直すと 1.4492（Lily# を再現）、0.08 で組み直すと rod 0**（＝LP の 0.80 に戻る）。⑸ **LP のソース**: `scm/define-grobs.scm:2577` **NoteColumn `skyline-vertical-padding` 0.15**／`:2747` **PaperColumn 0.08**・`lily/separation-item.cc:94-110 calc_skylines`＝各 grob の値で intrinsic に padded。**rod（`Separation_item::set_distance`）は PaperColumn の skyline**（0.08）、**wish の min（`note-spacing.cc:78-83`）は NoteColumn の skyline**（0.15）＋右列の 0.08 を horizon padding。**Lily# の `ItemSkylineFactory.Build` は view を問わず `NoteColumnSkylineVerticalPadding`（0.15）で padded**（:213）＝**rod の view（`ColumnElements.Elements`／`All`）だけ 0.08 であるべき**。★ **判定＝第 5 便は修正に着手しない**: rod は全ての列対（音符間の `SeparationRodDistance`・声部をまたぐ rod・小節線への rod…）が読む＝**出力が広く動く**＝承認が要る側（記憶の ⑴）／計器（`Zz379ProbeTests.cs.txt`・`verify/`・`skydump2.ily`）はディスクに残る。⇒ **要ユーザー判断＝§2 E に起票**。**数**（第 5 便末）: HEAD＝この追記の commit・**製品コード無変更**（full は第 4 便末の `run5.trx` 8527 / 0 / 4 / 8531 のまま）。

★★★ **⑿ 第 6 便＝ユーザー承認「承認する。」で ⑾ を直した＝rod の skyline だけ縦 padding 0.08（commit `6ccbd3e8`）**。**実装**: `ItemSkylineFactory.Build` が padding を受け取る＝**rod の view**（`CreateRightSkyline`／`CreateLeftSkyline` と AtColumn 形・`ColumnElements.Elements`／`All`）は **`MusicalColumnSkylineVerticalPadding` 0.08**、**wish の view**（`CreateWish*SkylineAtColumn`）は **`NoteColumnSkylineVerticalPadding` 0.15 のまま**（REF `separation-item.cc:92-110`・`define-grobs.scm:2577`／`:2747`）。**台帳 +1（EXACT）**: **`cross-voice.rod.paper-column-padding`**（新 probe `audit/lp-geometry/probes/cross-voice-rod.ly` score CVR＝voice 1 の 3 連符 c''' と voice 2 の八分 f''・1 voice 1 pitch・LP 0.800000）。**snapshot 再ベース 1**（`test/multivoice-tuplet-beams`＝正当化は同じ key）。**射程＝追跡 599 冊のうち 3 冊が動いた**（`0c85069c` worktree と svg 比較・`sweep-floor.ps1`）・**動いた本を LP と突き合わせ**（`verify-moved.ps1`・ragged の譜線長の残差 base → head）: **multivoice-tuplet-beams +0.64 → 0.00**・`audit/lpreg/perf-restpoly1k` −0.80 → −0.80（動いたのは justified 側）・feature-tour は段数違いで比較不能（20 対 19・従来どおり）＝**悪化した本 0**。生成物 `APPROXIMATIONS.md` は再生成（行番号だけ・`magic_constants.csv` は不変）。**数**（第 6 便末）: **full `scratch/p380/run6.trx` 8528 / 0 / 4 / 8532**（+1＝台帳 1 点）・**台帳 846 点／ss 非ゼロ 210／総和 22.584587806（不変）／exact 671（+1）／OPEN 0**・snapshot 249（1 枚は内容更新）・追跡 `.lys` 599・§7.5（`6ccbd3e8`）: **Core `+` 26 行／REF 3／OWN 0**・未 push 94（この追記込み 95）。★ **LSP の配布はユーザー**（Core が動いた）。⚠️⚠️ **`origin/master` の CI の赤と push はユーザー**。★ **判定＝第 7 便は着手しない**: 残る候補は ⑺ の並び（section label＝ユーザー待ち・⒝／⒠／⒢＝この便の計器と噛み合わない）と ⑻ ②（`CreateTimingSpring` の床＝観測 0 の縁＝射程を数える計器から作る）と ⑽ の範囲外（gate が cross-bar の加線対を値付けしない）＝**どれも `p380/incr` の双子・列の読みの延長ではなく、次のセッションで始めても損しない**。
...
- ⚠️⚠️ **section label と和音行の上下が LP と逆になる本がある＝要ユーザー判断・未着手**（第378 起票・a2 の移植 `f913ba60` の後に残った差）。**LP 2.26.0 の実測**（`scratch/p378/a2`・font pin・値は**譜の上端から上へ**の baseline・ss）:
