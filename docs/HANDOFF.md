...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第238セッション＝**残債の返済を中断し、ユーザーが目で見ているリリースブロッカーを直した。歌詞の行が五線の音符を貫いており、5807 本・snapshot 222 枚・台帳 566 点は 1 つもそれを見ていなかった**（7 commit＝`65d5e048` 修正＋観測者 2 本／`be5273fb` handoff／`cc4c6094` 閉幕訂正／`f31a5af5` **LP 双子＋台帳 7 点**／`401b9055` handoff／`cc89de72` Linux 脚／この行＝**2 件目の LP 双子**）。⚠️ **本便は engine を動かしている**（`LayoutEngine`／`LyricEngraver`）——にもかかわらず**台帳・snapshot・追跡コーパスは 1 バイトも動かなかった**。**それが症状ではなく、この便の最初の発見**（②）。⚠️⚠️ ★★★ **そして末尾でユーザーが「LP 双子で確認しろ」と言い、その台帳の穴は埋まった＝7 点**（⑧）。**修正は正しかったが、*正しい理由*は本便が予測したものと違った**——**6 つの予測のうち 3 つが外れ、書いておいた falsifier は正しい修正を*差し戻させる*ところだった**（骨 5）。
...
終了時: 本便 7 本（`65d5e048` 修正／`be5273fb`・`cc4c6094`・`401b9055`・`cc89de72` handoff 群／`f31a5af5` LP 双子＋台帳 7 点／この行＝**2 件目の LP 双子**）＝**未 push 5**。⚠️⚠️ ★★ **ユーザーが本便の途中で push した**＝`git log -1 origin/master` が `be5273fb`＝**本便の commit を指した**（§0 の第201 の形）——**開始 0 ／ 第1便で 1 ／ 第2便で 2 ／ そこで push ／ 終了 1**。**両端（0 → 1）だけ見ると便数とも commit 数とも合わない**ので、この行が唯一の記録。⚠️ **`be5273fb` は push 済みなので amend せず、訂正を 3 本目として足した**（履歴の書き換えは第235〜237 が 3 か所で払った）。・未追跡 0/木 0・suite **Windows 5816/0/4**（開始 5807 ＋観測者 2 ＋台帳 7）・**WSL/Linux も `401b9055` で 5816/0/4＝両 OS 完全緑**。⚠️ ★ **台帳 7 点は font 依存の残差を持つ**（+0.040901／−0.000096）ので、**Linux で回すのは形式ではない**——**両 OS で同じ残差が出ることまで含めて緑**・**台帳 566 → 573 点／ss 非ゼロ 110 → 115／総和 3.876038461 → 3.957938014／count 107 → 109・非ゼロ 2 のまま**（⑧ が足した。差 0.081899553 は 7 点の残差の絶対値の和と一致）・**snapshot 222・追跡コーパス 572 は不動**・Core 0 警告。**ref は 1 本も動かしていない**（§1 骨 4・第237）。⚠️ **修正 commit は push 前に 2 度 amend しており、置き換えられた 2 本は孤児になっている**——**その SHA はここにも書かない**（第236 の警告どおり。**孤児は名前を書いた瞬間に dead 引用になる**＝骨 4）。
