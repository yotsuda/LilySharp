...
最終更新 第371セッション（2026-09-11）＝**入り方は第298〜第367 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った。⚠️ ただし**開始直後に「ripple は使用禁止にして。pwsh MCP を使って作業して」**＝道具の指示が 1 つ。記憶に `feedback_ripple_mcp_banned_use_pwsh_mcp` として残した）。第370 ⑹⒝ に 1 つだけ残っていた **⑤ 和音記号の綴り**（第 5 便「すべて進めて」で承認済み・第 6 便が「新しい便で始める方が有利」と送った件）を出荷した。**骨は 6**:

★★★ **⑴ まずコーパスに訊いた（射程）**: `scratch/p372/chord-census.ps1`（ディスク 27,085 冊の `chords NAME { }` の本体を brace 対応で切り、記号を数える）。**`chords` 行を持つ本 659・異なり記号 90**。多い順に `C` 834/445 冊・`G` 643・`Am` 465/250 冊・`F` 326・`G7` 172・**`Fmaj7/{E,F,G}` 各 120（90 冊）・`Gmaj7`／`Dmaj7` 各 90・`Cmaj7` 48/45 冊**——⇒ **maj7 族は約 270 site・135 冊**、**短調は 465 site 超**、**♭ を綴る根音は `Bb7` 4・`Bb7/D` 2・`Db/F` 1 だけ**。⇒ 「♭ 対 b」は観測者がほぼ居らず（しかも Lily# は既に ♭ を描く）、**値打ちが在るのは「Δ 対 maj7」の側の語彙**だと数が言った。

★★★ **⑵ 出荷した形（commit 前・承認待ちではない＝出力不変。下 ⑷ が根拠）**: **⑤ を 2 キーに割った**——**`layout { chordQualities words|symbols  minorChords upper|lower }`**。LP でも別々の property だから（例外表と boolean）、かつ譜面によって片方だけ欲しいから。**`chordQualities`**＝品質の語彙。`words`（既定・今日の絵）は `Cdim`／`Caug`／`Cm7♭5`／`Cdim7`、`symbols` は **LP の `ignatzekExceptions` が名指す 4 つだけ**を `C°`／`C+`／`Cø`／`C°7` に。**他の品質は両語彙で同じ**（LP も数字で綴るから＝表が短いのは省略ではなく構成）。**`minorChords`**＝LP の `chordNameLowercaseMinor`。`lower` は**短 3 度を持つ和音**の根音を小文字にし、**接頭の `m` を落とす**（LP は `minorChordModifier` を `empty-markup` に差し替える＝2 つは必ず一緒に動く）。**⚠️ 判定は*3 度*** なので `Cdim` も小文字化し、`sus` は決して小文字化せず、**スラッシュ低音は大文字のまま**（LP の `chordNoteNamer` は `lowercase?` に常に `#f`）。**家**: `Semantics/ChordSpelling`（`ChordQualityStyle`＋`bool LowercaseMinor` の record struct・`default` が今日の綴り）・`ChordQualityRegistry.SymbolSuffix`／`HasMinorThird`（音の集合に訊く＝LP と同じ問い）・`ChordStructure.DisplayName(spelling)`。**配管**: `MeasureCollector._chordSpelling` ←`_meta.LayoutPlan.Chords`（`_accidentalStyle` と同じ 1 行の隣）→ `ChordNameCollector.Spelling`（和音行・付随トラック）と `CollectChordAnnotations`（inline `@chord`）。

★★★ **⑶ 引数を*必須*にしたのが効いた**（§5.2.1②・第 6 便の箱と同じ手）: `DisplayName` を**プロパティからメソッドへ**、`GetSuffix(quality)` を `GetSuffix(quality, style)` へ、`AnnotationValues.Chord(mark, out _)` を `Chord(mark, spelling, out _)` へ。⇒ **Core と Lsp は 0 エラーで通り、落ちたのはテスト側の呼び手だけ**＝製品側に「スイッチを読み落とした namer」は 1 つも残らない。**`ChordSpelling.Default` を渡す側も宣言になった**: **MusicXML は絶対に追随してはいけない**（`<harmony>` は*データ*で、`BuildHarmony` が Lily# の正規の綴りを読み戻す＝`C°` は何の品質にも parse しない）・注釈名の validator は「和音かどうか」しか訊かない・エディタの補完は挿入する音を説明するので正規の綴り。**Roman 度数は語彙に追随しない**——`RomanSuffix` が同じ 4 品質を先に `°`／`ø7`／`+`／`°7` で綴っており、残りは両語彙で同じ＝**恒等式**（番人 `TheRomanDegreeCannotFollowTheChordVocabulary` が「語彙が動かす品質は必ず roman 表が上書きしている」を*導出して*訊く）。

★★★ **⑷ LP と 1 文字ずつ突き合わせた（測った・議論していない）**: 絵を 2 枚（`scratch/p372/probe-words.lys` 対 `probe-symbols.lys`＝同じ音楽・同じ 5 和音 `Cdim|Caug|Cm7-5|Cdim7|Am7/C`）。**Lily# 既定**: `Cdim | Caug | Cm7`+♭glyph+`5 | Cdim7 | Am7/C`（`<text>` 29）。**両スイッチ**: `c° | C+ | cø | c°7 | a7/C`（`<text>` 27＝`m7♭5` の 3 片が `ø` の 1 片になった差）。**双子を LP 2.26.0 で走らせた**（`lp-symbols.svg`）: **`c | ° | 7 | a | 7 | / | C | C | + | c | ° | c | ø`**＝**同じ 5 和音を LP も `c°` `C+` `cø` `c°7` `a7/C` と綴る**。⚠️ **既定の本の双子（`lp-words.svg`）は `C° C+ Cø C°7 Am7/C`**＝**LP は例外表が常時 on なので、Lily# の既定とは元から食い違っている**（登録済みの Phase-1 乖離）。双子は `\new ChordNames \with { chordNameLowercaseMinor = ##t } \progChords` を書く（`chordQualities` は双子に届かない＝2026-09-08 の決定で双子は LP 自身の名前を運ぶ）。

★★★ **⑸ 数・射程・起票**: **開始時**（`Session-Check -Build -Test -Scratch p372`）: HEAD **`6ba76059`**・**未 push 14**（`origin/master`＝`b3bf9364`）・木 clean・未追跡 0・`gh run list` **緑**（34543531416）・build 0 エラー／Core 0 警告・**full `scratch/p372/run1.trx` 7764 / 0 / 4 / 7768**（第370 末 run9 と一致）・台帳 821・snapshot 249・追跡 `.lys` 599・ディスク 27085。**終了時 full `scratch/p372/run3.trx` 7771 / 0 / 4 / 7775（+7＝新しい番人）**。**snapshot 0 枚・台帳 0 点が動いた＝出力不変**（新キーを書く本は 1 冊も無い）。⚠️ **途中 run2 で赤 2**＝第370 が註記した 2 つの型そのまま: `ApproximationInventoryTests`（行番号ずれ→`LILYSHARP_UPDATE_DOCS=1` で再生成）と **`CitationsThatNameNothing_DoNotGrow` が +8**——**`LpReferenceCitationTests.SymbolPattern` は `_` か `-` で*繋いだ*名前しか記号と見なさない**ので、**camelCase の LP property（`chordNameLowercaseMinor`・`ignatzekExceptionMusic`・`chordNoteNamer`）も `prefix-modifier->markup`（`->` で切れて 2 節になる）も「何も名指していない」**。⇒ **行範囲を落として名前を残し、行番号は散文で書く**綴りに直した（既存の前例＝`ChordNameGlyphRun.ShortGlyph`・`ly/engraver-init.ly printPartCombineTexts`）。**⚠️ 起票＝`maj7` は切り替えられない**: LP の `majorSevenSymbol` は `\super` の中の `\triangle`（`ly:round-polygon`・`ex = magstep(font-size)*1.8`・頂点 (0,0)(ex,0)(0.5ex,0.86ex)）で、**Lily# の和音記号は 1 本のベースライン text run**（`ChordNameGlyphRun`＝上付きも多角形も家が無い）。**上付き自体が和音記号ぜんぶに掛かる Phase-1 の簡略化**なので、これは*別の便*（`\super` の移植＋多角形の piece＋箱と skyline の測り直し）。**この 2 キーはその簡略化の内側を切り替えるもので、閉じてはいない**。

⇒ ★★★ **⑹ 次の一手**（第 1 便時点の記録。第 2 便は下 ⑻＝自分の穴を塞いだ／**⒝ は第 4 便で出荷した＝下 ⑾**）: **⒜ `layout {}` の候補 5 つは全部出荷した**（第370 ⑴ の ①〜⑤）＝この道は閉じた。**次に `layout {}` へ足す語が要るなら、まず「表示専用・score 全体・単位なしの閉じた語彙・grob 単位でない」の 4 条件に当ててから**（§3 の行）。**⒝ 和音記号の上付き＋三角**（✅ 第 4 便＝下 ⑾）。⇒ **その代わりに名指せた新しい島＝`staff→chord` が −0.851723138 足りない**（LP は下ひげが無くても 5.659653422 を保つのに Lily# は 4.807930284 に落ちる・**CHL1 と CHL2 で同値＝行ごとの不変量は生きている**）。**第 5 便で解剖した＝下 ⑿**: ばねの床ではなく**binding X における五線の下向き skyline の中身**。**次便の最初の一手は LP の skyline を X ごとに dump すること**。**⒞ 第369 ⑸⒝ の並びはそのまま**（tab の wish の merge＝設計級・`lily/spacing-spanner.cc generate_springs` の wish の本数を読むところから）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。**push はユーザー**。

**⑺ 第 1 便の終了時**: **full `scratch/p372/run4.trx` 7771 / 0 / 4 / 7775（run3 と合計一致・RunInfos は SKIP 4 だけ・4 m 41 s・HANDOFF／CHANGELOG／APPROXIMATIONS 込み）**・commit **`65ed6f72`**（30 ファイル・+730/−92・`scratch/p372/commit1.txt`）＋HANDOFF `de500297`・§7.5 Core `+` **341 行**／**REF 9**（全部 LP の既存 property・例外表・namer の住所）／**OWN 0**・台帳 821／OPEN 0／snapshot 249／追跡 `.lys` 599 は不動・ディスク 27087（+2＝probe 2 本）・`Deploy-Lsp.ps1` 済（95 files 検証・0.6.0+65ed6f72）。

★★ **⑻ 第 2 便＝今便で自分が空けた穴を塞いだ（製品 0・番人 2 本）**: **和音記号を綴る namer はページに 3 つ在る**（`chords` 行／staff に付随したトラック／音符の inline `@chord`）のに、第 1 便の番人は**行しか見ていなかった**。**`TheInlineChordAndTheAttachedTrack_SpellItTheSameWayTheRowDoes`**＝**同じ 1 和音を 3 つの道で出し、どの綴りでも 3 つが一致する**ことを*出来上がったページ*に訊く（型ではなく）。**毒で赤を見た**（`CollectChordAnnotations` に `ChordSpelling.Default` を渡す＝スイッチを読み落とした namer の形）: `Assert.All() Failure: 1 out of 3 items … Expected: "c°" Actual: "Cdim"` と**はみ出した 1 つを名指して赤**。毒は抜いてある（経緯はテストの remark）。**`ASpellingEdit_RendersIdenticalToFullRecompile_BothWays`**＝増分の門（`accidentals` の同名番人と同じ形）。⚠️ **これが効いている仕組み**＝綴りは `LayoutPlan` の*フィールド*で、`LayoutPlan` は record ＝ resume の門が解決済みの plan を値で比べる（`MetaMatchesShifted`）。**collector 側にだけ持たせたら黙って腐る**——それがこの番人の訊いている形。

★★★ **⑽ 第 3 便＝キー名を `chordNames` → `chordQualities` に改名（ユーザーの問い）**: **入り方が違う**——ユーザーが「**`chordNames` は `names|roman` とした方が一貫性が出るのでは。それとも既存の `names|roman` を `words|symbols` にすべき？ それとも全然違う機能？**」と問うた。**答えは「全然違う」だが、指摘の実体は私のキー名の側に在った**:
**⒜ 2 軸は直交**（`chords NAME as names|roman`＝**何を出すか**＝絶対和音か調の度数か／`layout { … }`＝出すものが決まった後の**綴り**）。**⒝ `as names|roman` を `layout {}` に移すのは不可能**（好みではなく構造）——GRAMMAR「SHOWING ONE TRACK TWO WAYS」の正本 `score main "sheet" { chords prog as roman  chords prog as names }` が**1 つの score で両方の値を同時に要求する**（`as both` 退役＝2026-08-23 の決定がこれに依存）。`layout {}` の入居条件は「**score 全体**」なので、1 score が 2 値を要る設定は載らない。**⒞ 逆向き（`as words|symbols`）も違う**＝量の軸に綴りの軸の語を付けることになる。
⇒ **⒟ 直すべきは私のキー名**: `chordNames` は 2 行先の `chords prog as names` と**同じ語が別の意味**で、これは第370 ⑴⒝（`fonts { marks }` と top-level `marks` の衝突）＝`layout {}` を作った動機そのものの再発だった。**`chordQualities words|symbols` に改名**（`names` の語を落とす／モデルの語 `ChordQuality`・`ChordQualityRegistry` と 1 語 1 意味／兄弟キー `barNumbers`・`sectionLabels`・`minorChords` と同じ「対象＋複数形」で `minorChords` と対になる）。**型も揃えた**: `ChordNameStyle(s)` → `ChordQualityStyle(s)`・`ChordSpelling.Names` → `.Qualities`・`AfterLayoutChordNames` → `AfterLayoutChordQualities`。⚠️ **`chordNames` の小文字は*別物*が 3 ファイルに在る**（`ChordNameItem` の集合を受ける引数・局所変数＝`ChordNameTests`／`ScoreAssemblerTests`／`RehearsalMarkTests`）＝**一括置換の対象から外す**（memory の手順②「旧名で再 grep して 0 件」はこの 3 つを除いて 0）。**番人 1 本追加**＝`TheDegreesRowIsUnmovedByTheVocabulary_WhileTheNamesRowBesideItChanges`（`as roman` と `as names` の 2 行を**同じ score に**置き、語彙を切り替えると**名前の行だけが動き度数の行は不動**＝直交をページで言う。今までは registry 級の導出だけだった）。

★★★ **⑾ 第 4 便＝和音記号の上付き＋三角を移植して出荷した（ユーザー承認済み＝下 ⒠）**。**入り方**: 私が「次便は着手しない（設計級＋ユーザー決定が要る）」と答えたのに対しユーザーが「**上付きを入れるか、とはどういう意味？ 私が判断できるように説明して**」→ LP の実測で説明（下 ⒜）→ ユーザー「**⑴ 入れて。⑵ 三角も入れて。これは chordQualities でスイッチできるよね？**」。**⑵ の答えは「載る」**——LP でも `majorSevenSymbol` と `chordNameExceptions` は別 property だが**既定は両方とも LP 自身の絵**なので、`words`／`symbols` は「語の家風」対「LP 自身の記号の家風」の 2 組として一貫する（キーを 3 つ目に割るより第370 ⑴ の入居条件に合う）。

★★★ **⒜ LP の数値（全部実測・`scratch/p372/lpnames.ly`＝登録 29 品質を LP に刷らせた台／`lp-words.svg`／`lpacc.ly`／`lptri.ly`）**: **`\super` は font-size −3 かつ magstep(元の font-size) だけ持ち上げる**（`scm/define-markup-commands.scm` super-markup）＝根音 2.6165 に対し上付き **1.8500**（比 0.70705 ＝ 2^(−1/2)）・持ち上げ **1.1892**（＝magstep(1.5)）。**ベースラインに残るのは根音・`m`・`/低音`・`+`・`°`（`°` は ×1.2599 で*大きい*）だけで、数字・`ø`・`sus`/`add`・三角は全部上付き**（29 品質で確認）。**上付きの中の臨時記号も同じ 3 段ぶん縮む**（LP scale 0.0030 対 根音の 0.0042）。**三角は多角形**（`triangle-markup`＝`ex = magstep(font-size)×1.8`・頂点 (0,0)(ex,0)(0.5ex,0.86ex)・`polygon-markup` の `filled #f` は `ly:round-polygon`）＝LP の SVG は `points="0.5351 -0.9204 1.0703 0 0 0" stroke-width="0.1000" stroke-linecap="round"`。**式の値は 1.070287／0.920447 で実測と一致**（step は 1.5−3−3 ＝ −4.5）。

★★★ **⒝ 出来た形と一致の証拠**: **`Music/ChordSymbolText(Text, SuperFrom)`**＝印字文字列と**上付きの開始位置**を namer が*構造として*運ぶ（印字済み文字列から推定し直さない＝`Cmaj7` は `m` から上がり `Cm7` は `7` から上がるので、字面の規則では区別できない）。⚠️ **index は 1 つだけ**——上付きは必ず `/` か末尾で終わる（LP は品質と低音の間にベースラインの物を置かない）ので 2 つ目は同じ事実の 2 綴り目になる。**`ChordQualityRegistry.BaselineSuffixLength`** が「suffix の先頭何文字がベースラインに残るか」を答える（`m` 前置／`°` `+`／それ以外 0）。**`ChordNameGlyphRun` は piece に `ChordPieceKind`（Text/Accidental/**Triangle**）と `EmFactor` を足した**＝`\super` は中身の text も `\musicglyph` も一律 3 段落とすので**因子は 1 つ**。三角は**文字ではなく描く**＝`TriangleCarrier`（U+25B3）は**運び手**で、面には一度も渡らない（`ChordCharactersTheFaceCannotDraw` に加えた）。描画は**丸キャップの直線 3 本**＝`ly:round-polygon` の丸角がキャップで再現される。**Lily# の実測**: 上付き 1.85・持ち上げ 1.19・上付き♭の縮尺 3.00/4.24・三角 底辺 1.07／高さ 0.92／線幅 0.100 ＝**全部 LP と同値**。**⚠️ そして決定的なのはインク**: `Dmaj7` が **(0.000000 . 2.497137)** ＝ **LP が dump した `(0.0 . 2.5008)`**（台帳の記録原因が引いている値）と下端は*完全一致*・上端 0.0037 差。移植前は `(−0.570344 . 1.907250)`。**記号幅は 7.5798 → 5.8726**。

★★★ **⒞ ⚠️⚠️ 台帳 7 点が LP から*遠ざかった*——そして台帳自身がこの日を予言していた**。`lyrics.chord-lyric-run.chord-to-lyric` の記録原因は「残差 +0.570188682 は **`Dmaj7` の `j` の下ひげ**＝0.218 em × 2.616256 で、**Lily# がベースラインに置く `maj7` を LP は上付きにする**（LP の dump は下ひげ無しの `(0.0 . 2.5008)`）＝**和音*語彙*の島・ユーザー決定待ち**」。⇒ **下ひげは実際に消えた**（上 ⒝）。**が、残差は消えずに*隣の段へ移った***: `staff-to-chord` 5.407 → **4.808**（LP 5.660・残差 −0.252 → **−0.852**）、`chord-to-lyric` 2.890 → **3.509**（LP 2.320・残差 +0.570 → **+1.189**）、**部屋（`staff-to-staff`）は 0.3178 → 0.3373 とほぼ不動（+0.0195）**。⇒ **鎖は床で解くので、下ひげが空けた 0.6 を第 1 段が飲み、第 2 段へ押し出した**。**新しい島が名指せる**: **LP は下ひげが無くても staff→chord を 5.660 に保つのに、Lily# は 4.808 まで落ちる**＝この ばね は**下ひげに隠されていた −0.85 の不足**を持っていた。台帳の記録原因は**半分だけ当たっていた**（島は語彙だが、閉じた先は隣の段だった）。⇒ **台帳の再ベースは「LP 照合 → 承認 → 実行」の対象**で、承認と同時に**原因欄を書き換える**のが正しい（「下ひげの遮蔽は消えた／残るのは staff→chord ばねの不足」）。

**⒟ 第 4 便の赤 17 と作業ツリー**: **snapshot 9**（`chordnames`／`chords-attached`／`lead-sheet`×3／`figbass-chordname-lower-staff`／`rit-across-systems`／`chordrow-rit-second-system`／`showcase/07-lead-sheet`＝和音記号を持つ本ちょうど）＋**台帳 7**（上 ⒞）＋`ApproximationInventoryTests` 1（再生成済み）。**他は全部緑（7757）**。⚠️ **テスト側の探り方を 5 本直した**——記号が 1 つの `<text>` だという前提が崩れたため（`ChordPartTests` 3 本は `">G7</text>"` → `">G<"`＋`">7<"`、`LeadSheetHeaderClearance` と `TabStaffStencil` は**piece ごとに*その piece 自身のサイズ*でインクを測る**形へ）。**`TheRomanDegreeCannotFollowTheChordVocabulary` は前提が変わったので書き直した**: 語彙が動かす品質は 8 つになり、**4 つは roman 表が上書き・4 つは三角**で、**三角は 1 本のベースラインに載らない**（`\super` の中で寸法が決まる多角形だから）＝`Imaj7` は両語彙で `Imaj7`。**patch は `scratch/p372/superscript-triangle.patch`**・捨て計器は `scratch/p372/ZzP372InkProbe.cs.txt` へ退避済み。

★★★ **⒠ ユーザー「承認する」→ 出荷した**: `Approve-Snapshots.ps1`（引数無し）で **snapshot 9 枚**（和音記号を持つ本ちょうど＝run8 の赤の内訳が証拠）→ **台帳 7 点を再記録**（`scratch/p372/record-ledger.ps1`＝JSON 全体を `ConvertTo-Json` で書き戻すと整形が 439 バイトぶん変わるので、**該当エントリの `residual` と `why` だけを狙って**書き換える。差分は 14 行）→ full 緑。**新しい残差**: `chord-lyric-run.staff-to-chord` と `chord-run.staff-to-chord` **−0.252403051 → −0.851723138**（両書で同値＝行ごとの不変量は生きている）／`chord-lyric-run.chord-to-lyric` **+0.570188682 → +1.189052065**／`chord-lyric-run.staff-to-staff` **+0.317785632 → +0.337328927**（部屋はほぼ不動＝0.6 は段の*間*を動いただけ）／`chord-run.staff-to-staff` **−0.182059319 → +0.337483977**（⚠️ **2 冊の部屋残差が 0.000155 まで一致した**＝残る項は共有の第 1 段のものだけ）／`lyric-chord-run.lyric-to-chord` **−0.593576126 → −1.192896213**／★ **`lyric-chord-run.staff-to-staff` −0.593669291 → −0.003782263**。**原因欄には「下ひげの遮蔽が消えたこと」「残るのは staff→chord ばねの −0.85＝次の島」「★ 第260 便が書いた予言はこの移植で確かめられた（機構を名指す原因は反証可能で、これは機構を閉じて確認された）」を書いた**。

⚠️ **教訓（§5.2 の相殺の形）**: **−0.252403051 は 1 つの量ではなく、実在する −0.85 と `j` の下ひげの +0.60 が符号違いで相殺した和だった**。下ひげを取り除くまで見えなかった。**台帳の点が「近い」ことは、その点に誤りが 1 つも無いことを意味しない。**

★★ **⑿ 第 5 便（ユーザー「有利なら着手」→ **有利**＝数分前に自分で見つけた島で、数値も台帳の履歴も LP のファイル位置もコード経路の名前も全部頭に在る）＝`staff→chord` の −0.851723138 を解剖した（**測るだけ・製品 0**）**。**⚠️ 仮説を 2 つ立てて 2 つとも殺した**のがこの便の中身:
**⒜ 仮説 1「LP には床がある」＝死んだ**。`ly/engraver-init.ly` の ChordNames context は `nonstaff-relatedstaff-spacing.padding = 0.5` と `nonstaff-nonstaff-spacing.padding = 0.5` **だけ**を override し、`VerticalAxisGroup` 側にも `nonstaff-relatedstaff-spacing` の既定は**無い**（`scm/define-grobs.scm:4232-4240` が持つのは `default-staff-staff-spacing` と `nonstaff-unrelatedstaff-spacing . ((padding . 0.5))` だけ）。⇒ **LP の 5.659653422 は基本距離ではなくインク**。⚠️ **そして段の向きを読み違えていた**——probe の註（`audit/lp-geometry/probes/chord-lyric-run.ly`）が言うとおり、この段は `nonstaff-unrelatedstaff-spacing` + LARGE_STRETCH の枝で、**和音行は staff1 の*下*に吊られる**。
**⒝ 仮説 2「Lily# は定数 5.5 に留めている」＝3 回目の測定で死んだ**（捨て計器 `scratch/p372/ZzP372WalkProbe.cs.txt`・経緯はそのヘッダに逐語）: ⑴ `Dmaj7|Em7` で `YUp −7.997137` ＋ ink top 2.497137 ＝ 上端**ちょうど −5.500000**／⑵ 記号を `C|Am` に替えて背を変えても `YUp −7.454343` ＋ 1.954343 ＝ 上端**また ちょうど −5.500000**（⚠️ **5.5 は `ChordNameEngraver.StaffPadding` の註が「Lyrics/Dynamics の basic-distance を ChordNames に誤って当てた値」として退役させた数そのもの**なので、もっともらしかった）／**⑶ 五線の音を低くしたら上端は −10.497907 に動いた**＝**定数ではない**。丸い数が 2 回続いたのは五線側のインクが 5.0 に落ちた偶然。
⇒ ★★★ **⒞ 機構は正しく、島は skyline の中身**: **和音行のインク上端 ＝ 五線の下向き skyline ＋ padding 0.5**（記号の背はベースラインだけを動かし上端を動かさない＝LP と同じ形）。残る −0.851723138 は **binding X における五線の下向き skyline の中身の差**。**上付き移植で記号が 1.71 ss 細くなったので縛る X が動いた**＝台帳が旧残差を「the staff's down ink at the binding X differs from LilyPond's by 0.252」と書いていたのと**同じ族で、X が動いたぶん大きくなっただけ**。⇒ **私の移植が作った欠陥ではない**。**次に要る計器**: LP 側の staff1 の下向き skyline を X ごとに dump し、和音行の箱が当たる区間で突き合わせる（`-ddebug-skylines` か page-layout の dump）＝**次便の最初の一手**。
⚠️ **⒟ 同じ便で自分の註を 1 つ直した**（第 4 便で書いたもの）: `Pieces` の三角の `Advance` に「MEASURED against LilyPond's own advance for `C△9`」と書いていたが、**測ったのは*stencil の extent* であって LP の*次の piece までの送り*ではない**。LP の `C△9` は三角の原点 10.9296 → `9` の原点 12.5499 ＝ **1.6203** で、**extent 1.1703 ＋ 0.5 に*ちょうど*分解する**——0.5 は `chordNameSeparator`（`ly/engraver-init.ly` 行 949・`make-hspace-markup 0.5`）で、**LP は上付きの piece を全部これで繋ぐ**（`markup-join`）。**Lily# の品質は 1 つの語なので繋ぎ目が無く、この 0.5 は移植していない**＝`7♭9` も `m7♭5` も LP より繋ぎ目 1 つあたり 0.5 狭い。⚠️ **射程も「3 site」と書きかけて census に訊いたら 40 site / 40 冊だった**（うち 30 は `Gm7♭5`）＝**数は数えてから書く**。移植するには namer が「piece の切れ目」を渡す形に変える必要があり、それは別の変更なので**近似せず名指した**。

★★ **⒀ 第 7 便＝第 4 便の註の「観測者が居ない主張」に番人を立てた**（製品 0・番人 4 本）。`SuperRaise` の註は「a score that writes `fonts { chordName step … }` moves it with everything else」と書いていたが、**em と lift は別の場所で計算されており、一緒に動くことを誰も見ていなかった**。**まず測った**（捨て計器 `scratch/p372/ZzP372StepProbe.cs.txt`）: 既定 em 2.616256／lift 1.189207・`step +2` で 3.296276／1.498307・`step -2` で 2.076523／0.943874 ＝**どちらも magstep(±2) 倍ちょうど**＝主張は真。⇒ **`FontAttributeTests.ASteppedChordName_StepsItsSuperscriptWithIt`**（`step` の家に置いた・Theory 4 形＝既定／+2／−2／+5）。⚠️ **比で訊く**形にした——最初は既定値 2.616256 をハードコードして magstep を掛けたが、**6 桁の写しでは実値 2.6162561… との積が最下位で合わない**（`step -2` の腕で赤になって気付いた）＝**主張は「一緒に動く」なので比がその主張そのもの**。**毒で赤を見た**（`SuperRaise` から score の step を落とす＝既定から lift を計算する形）: `Expected: 1.25992105 Actual: 1` と**失われた比を名指して赤**。毒は抜いてある。

★★★ **⒁ 第 7 便の途中でユーザーが割り込んだ＝「いっしょに動くことに基づいて発明するのではなく、LP のコードを字面通りに移植して」**。**当たっていた**——私は 2 箇所で*代数簡約*を書いていた: **⒜ `SuperEmFactor = Math.Pow(2.0, -0.5)`** は `magstep(-3)` の簡約で、LP に在る語は `(- font-size 3)` の **3**／**⒝ `SuperRaise = Magstep(FontSizeStep + StepOf(...))`** は「font-size そのもの」の*再構成*で、LP に在る語は `(* 1.0 (magstep font-size))` の **font-size**。⇒ **字面に直した**: **`FontSize(fonts)`＝symbol の中で効いている markup property `font-size` の唯一の家**、**offset を 3 本の定数に**（`SuperFontSizeOffset = -3`＝`super-markup`／`SmallerFontSizeOffset = -1`＝`smaller-markup`／`TriangleFontSizeOffset = -3`＝`whiteTriangleMarkup` の `\fontsize #-3`）、**`EmAt(fontSize)`＝その font-size の text em**、`SuperRaise = 1.0 * Magstep(FontSize(fonts))`。★ **`Piece.EmFactor`（因子）を `Piece.FontSize`（font-size）に替えた**のが本体——**LP の markup は font-size を運び、stencil の種類ごとに自分の規則でサイズへ変える**（text は `EmAt`・`\musicglyph` は `AccidentalGlyphEm`・三角は `magstep × 1.8`）。**1 つの縮小因子で 3 種を賄うのは LP に無い数**だった。番人の主張も比から**LP の算術そのもの**へ（raised piece の `FontSize` ＝ symbol の `FontSize` ＋ `SuperFontSizeOffset`）。**出力はバイト単位で不変**（`probe-words.svg`／`probe-tri.svg` を再生成して一致＝数式は同値で綴りだけが変わった）。⚠️ **教訓**: 「同値だから」は字面を変える理由にならない。**LP に在る語がコードに現れているか**で見る。

★★★★ **⒂ 第 8 便＝ユーザー「`LILYPOND-REF` を付けて。出力に基づいて発明してはいけない、可能な限り字面移植すべきであるのは Lily# 開発の原則だ」**。⇒ **⒜ 原則は RULES §5.2 に既に在った**（「実測値に基づいてコードを直すことは禁止／可能な限り字面通りに移植」「LP 由来なら字面でなくても REF を付け、なぜ字面にできなかったかを 1 行添える」）＝**私は常設ルールを破っていた**。**§5.2 に新しい worked example として追記**（上 ⒁ の 2 例＋「最も捕まりにくい破り方は*同値な簡約*で、反証しうる観測が原理的に存在しない」＋判定法「出力ではなく語で見る」＋直し方の型）。**§3 にも決定として立てた**。⇒ **⒝ REF を機械で洗って足した**: `Piece`（`ignatzek-format-chord-name:103-209`＝名前は markup の*線*で、piece はその 1 つ）・`ChordPieceKind`（`name-step`＝多角形か臨時記号＋数字かの分岐／`accidental->text-markup`＝臨時記号が `\musicglyph` になる場所）・`AccidentalStep`（`smaller-markup`）・`Pieces`（`:179-209`＝4 つの並び順）・`TriangleBaseOf`（`polygon-markup`＝extent は outline＋blot 半分）・`ChordSymbolText`（`:179-209`＝上付きが 1 つの連続した span である根拠）・`DropMinorModifier`（`prefix-modifier->markup`＝`empty-markup` への差し替え）・**`BaselineSuffixLength`**（3 本＋⚠️ **字面でない 1 点を §7.6 ⒝ として明記**＝LP は 2 つのリストを*作って*決めるが Lily# の品質は語なので「語の何文字が第 1 リストか」を答える形になる）・`SpellPitch` の `lowercase`（`conditional-string-capitalize:28-32`）。**REF は `ChordNameGlyphRun` 21 本／`ChordStructure` 16 本／`LayoutPlan` 7 本・OWN 0**。⚠️ **引用の網が +2 で赤**＝`name-step`／`prefix-modifier->markup`／`accidental->text-markup` はどれもハイフン 2 節で記号と見なされないので**行範囲を散文へ**（既存の前例と同じ）。**出力不変・full 7778 / 0 / 4 / 7782**。

**⑼ 終了時（第 3 便まで）**: **full `scratch/p372/run6.trx` 7774 / 0 / 4 / 7778（便ごと: run4 7775 → run5 7777 → run6 7778）**・commit 5 本（`65ed6f72`＝2 キー／`de500297`＝HANDOFF／`ed532bfd`＝第 2 便の番人・`commit2.txt`／`96f8e87b`＝改名・`commit3.txt`／第 3 便の HANDOFF）・`Deploy-Lsp.ps1` 済（95 files 検証・0.6.0+96f8e87b）・**push はユーザー**。⚠️ `ed532bfd` は amend 済み（HANDOFF が古いハッシュを名指していた）＝**HANDOFF に commit ハッシュを書くなら amend の前に書かない**・台帳 821／OPEN 0／snapshot 249／追跡 `.lys` 599 は 2 便とも不動。⚠️ scratch は **`p372`**。⚠️ §7 3.5: 第369 のブロックを `-Archive 369` で ARCHIVE の先頭へ。⚠️ 記憶: 新規 1（`feedback_ripple_mcp_banned_use_pwsh_mcp`）。

## 以下は第370セッションの経緯

最終更新 第370セッション（2026-09-11）＝**入り方が違う**（ユーザーは HANDOFF を読ませたうえで「**`marks {beside|stacked}` をトップレベルに書く今の規則は妥当か。`layout {}` のようなブロックを足してその中に書いてはどうか。ほかに `layout {}` に入れるべき語はあるか**」と問い、私の答え（下 ⑴）に「**良い。着手して**」）。**言語の変更 1 件を出荷した＝`layout { }` ブロックの新設＋裸の `marks` directive の撤去＋`barNumbers` の新設**。**骨は 6**:

★★★ **⑴ 問いへの答え（私の分析・ユーザーが承認）**: **今の裸 `marks` は 3 つの根拠（override にしない／paper にしない／裸 2 語は `pitch` と同形）が全部生きているが、違和感の出所は 2 つ実在する**——**⒜ `marks` だけが「音楽の設定」の列に混じった*表示専用の裸グローバル***（`tempo`／`time`／`key` は音楽、`pitch`／`octave` は入力の約束）／**⒝ 語の衝突**（`fonts { marks "Georgia" }` の `marks` は font グループ＝*顔*、top-level の `marks` は*配置*。同じ語が 2 つの意味を持ち、裸形にはどちらか言う場所が無い）。**`layout {}` が効く本当の理由は「名前付きブロックの再利用機構」**（`fonts NAME`／`paper NAME` と同じ 2 段）で、**キーが 1 つだけのブロックは裸語より悪い**＝**2 つ目のキーと同時に作るのが筋**。⇒ **`layout {}` に入る条件**＝「表示専用・score 全体・単位を持たない閉じた語彙・grob 単位でない」、**paper との線引きは「単位のある量（mm・ss・justify のフラグ）は paper、描き方の切り替えは layout」**（`indent`／`raggedRight`／`spacingIncrement` は LP 自身が `\paper` に置く量なので **paper に残す**）。**候補として挙げた 5 つ**: ① `barNumbers none|lines|every N`（**今便で実装**）／② `accidentals modern|cautionary|forget`（LP の `\accidentalStyle` は `\layout` の context 設定・今は `@courtesy` の 1 音単位しか無い）／③ `sectionLabels boxed|plain|none`（`section ~A` は宣言側の 1 section 単位で、パート譜だけ消す用途は score 側）／④ `partCombineText on|off`（`a2`／`Solo`・今は常に刷る）／⑤ 和音記号の綴り（♭ 対 b・Δ 対 maj7＝2026-07-25「LP に合わせる」決定が在るので既定 LP のまま切替だけ）。**②〜⑤ は未着手＝ユーザー決定待ち（下 ⑹）**。

★★★ **⑵ 出荷した形（commit 前・承認待ちではない＝出力不変。下 ⑷ が根拠）**: **`layout [NAME] { marks stacked|beside  barNumbers lines|none|every N }`**。**家は `Semantics/LayoutPlan.cs`（`LayoutPlan` record＝`MarksBeside`＋`BarNumberPolicy`・`BarNumberMode` enum）・`LayoutPlanReader.cs`（読みの唯一の家＝collector と validator と双子が同じ表を読む・`Resolve(root, render)` が「score の参照 → file の無名ブロック → 既定」の順を答える）・`LayoutValidator.cs`（`PaperValidator` の写し＝未知キー error・二度書き warning・名前層 4 種）**。**診断は LYS9101〜9110**（paper の LYS90xx に対する 91xx 帯）。**配管**: `LayoutDeclarationGreen`／`LayoutDeclarationSyntax`（`Entries` は「キーはキー語彙のときだけキー」＝`marks sideways` の `sideways` は値のまま・`mark beside` は `mark` がキーとして拒否される）→ `RenderSpec.LayoutRef` → `MeasureCollector.LayoutOverride` → `MetadataState.LayoutPlan` → `ScoreContent.LayoutPlan` → `Score`／`MultiStaffScore.LayoutPlan`（`MarksBeside` は計算プロパティとして残した＝読み手 20 箇所を触らずに済む）→ `BarNumberEngraver.Calculate(policy:)`。**`marks` は予約語から外した**（`part marks { }` が通る）・**`layout` を予約語に足した**（掃き 27,075 冊で `layout` を名前に使う本 **0**・実測）。**後方互換は取らない**（未リリース＝memory の家訓）。

★★★ **⑶ `barNumbers` は LP の語彙の字面移植**: **`lines`（既定）＝ `first-bar-number-invisible-and-no-parenthesized-bar-numbers`（`translation-functions.scm:1005-1007`・`engraver-init.ly:858`）＋ grob の `begin-of-line-visible`（`define-grobs.scm:324`）／`none`＝`\remove Bar_number_engraver`（`engraver-init.ly:774`）／`every N`＝`every-nth-bar-number-visible`（`:987-988`＝`(= 0 (modulo barnum n))`）＋ `break-visibility` を `end-of-line-invisible` に開く**。⇒ **`every 2` では行頭の bar 3 に番号が*付かない***（可視性関数が先に答える）——これが LP の絵で、読み手が最初に「直したくなる」形なので番人 `EveryN_NumbersTheMultiples_WhereverTheyStand_AndNothingElse` が押さえる。**⚠️ 弱起（bar 0）は `every 2` で番号が付く**（0 % 2 == 0・LP の関数に最初の小節の門は無い＝門は*既定の関数*の側に在る）。**双子も同じ語を `\layout { \context { \Score … } }` に書く**（`BarNumberContextLines`）。

★★★ **⑷ 数と射程**: **開始時**（`Session-Check -Build -Test -Scratch p371`）: HEAD **`94e2d859`**・**未 push 7**（`origin/master`＝`b3bf9364`）・木 clean・未追跡 0・`gh run list` **緑**（34543531416）・build 0 エラー／Core 0 警告・**full `scratch/p371/run1.trx` 7699 / 0 / 4 / 7703**（第370 の開始＝第369 末と一致）・台帳 821・snapshot 249・追跡 `.lys` 599。**変更後 full `scratch/p371/run2.trx` 7738 / 2 / 4 / 7744（+41 件＝新しい番人）・赤 2 は想定どおり**（`ApproximationInventoryTests`＝REF を足したので行番号が動いた・`LILYSHARP_UPDATE_DOCS=1` で再生成／`CitationsThatNameNothing_DoNotGrow`＝新 REF 1 本が**行範囲の*同じ行*に記号を書いていなかった**）。**両方直して緑**（下 ⑸⒞）。**snapshot 0 枚・台帳 0 点が動いた＝出力不変**（`layout` を書かない本は 1 冊も変わらない）。§7.5 Core `+` 約 **+310 行**（新ファイル 3 本＋配管・式 0）／**REF 4**（全部 LP の既存関数の住所）／**OWN 0**。⚠️ scratch は **`p371`**。⚠️ §7 3.5: 第368 のブロックを `-Archive 368` で ARCHIVE の先頭へ（済）。

★★★ **⑸ 踏んだ罠 3 つ（次便が同じ形で踏む）**: **⒜ `Layout` という名前はこの木では使えない**——`Svg.Model`／`Svg.Collector` は `Layout.LayoutOptions`（＝*名前空間* `Svg.Layout`）を書いており、**同名のメンバを足すと名前空間が隠れて `CS0236`**（field initializer が非静的メンバを参照）。⇒ **メンバ名は `LayoutPlan`**（3 ファイルに理由をコメント）。**⒝ 双子の `PageModel` は score ごとの override を*持たない***（`SvgGenerator` は `collector.FontsOverride/PaperOverride/LayoutOverride` を立てるが、`LilyPondExporter.PageModel` は素の `MeasureCollector` を使う）＝**`score main { layout chart }` が双子から見えない**。⇒ **`ResolveLayoutPlan` はページではなく*ソース*から読む**（`LayoutPlanReader.Resolve`＝collector と同じ読み・一つの家）。**⚠️ fonts 側は同じ穴を今も持っている**（per-score `fonts NAME` が双子に届かない）——今便では触っていない＝**起票（下 ⑹⒜）**。**⒞ `LILYPOND-REF` は*行範囲と同じ行*に記号名を書く**（`CitationsThatNameNothing_DoNotGrow`）。`scm/…:1005-1007` と書いて名前を次行に置いたら赤。**ハイフン名は 3 節以上**（`break-visibility` は 2 節＝記号と見なされない・`begin-of-line-visible` は可）。

⇒ ★★★ **⑹ 次の一手**（第 1 便時点の記録。⒜ は第 2 便で閉じ、⒝ は ⑤ を除いて第 3・第 5・第 6 便で全部出荷した）: **⒜ 双子の per-score `fonts NAME` が `PageModel` に届かない**（✅ 第 2 便＝下 ⑻）。**⒝ `layout {}` の 2 つ目以降のキー（上 ⑴ の ②〜⑤）はユーザー決定**——私の推し順は ② `accidentals`（✅ 第 3 便＝下 ⑼）→ ④ `partCombineText`（✅ 第 5 便＝下 ⑾）→ ③ `sectionLabels`（✅ 第 5 便＝`boxed|none`・第 6 便＝`plain`＝下 ⑿）→ **⑤ 和音記号の綴り＝これだけ残っている**。⚠️ **⑤ は次の便で新しく始める方が有利**（第 6 便でそう判断して着手しなかった）＝LP の property 調査（`majorSevenSymbol`／`chordNameLowercaseMinor`／german・italian の chord 族）とコーパスの実測が先に要り、`layout {}` の道の記憶では安くならない。**⒞ 第369 ⑸⒝ の並びはそのまま**（tab の wish の merge＝設計級・`lily/spacing-spanner.cc generate_springs` の wish の本数を読むところから）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。**push はユーザー**。

**⑺ 第 1 便の終了時**: **full `scratch/p371/run3.trx` 7740 / 0 / 4 / 7744（run2 の赤 2 を直した後・RunInfos は SKIP 4 だけ・5 m 28 s・HANDOFF／CHANGELOG／APPROXIMATIONS 込み）**・commit **`b18a266b`**（50 ファイル・+2034/−434）・`Deploy-Lsp.ps1` 済（95 files 検証・0.6.0+b18a266b）。

★★ **⑻ 第 2 便（ユーザー「あなたの提案通りに進めて」＝⑹⒜ と ⑹⒝ を私の推し順で）＝双子の per-score 参照の穴を塞いだ（commit `6e1238b3`）**: **射程は 0 冊**（追跡 599 冊に名前付き `fonts`/`paper` ブロックを書く本は無い＝`.ly` は 1 冊も動かない）。`LilyPondExporter.PageModel` は素の `MeasureCollector` を建てており、`SvgGenerator.CollectFor` が立てる 3 つの override をどれも立てていなかった＝**`score main { fonts house }` はページに届いて双子に届かず、双子はファイル既定の font-size を書いていた**。番人 `NamedBlockReferenceTests.TheTwinReadsTheScoresOwnFontsReference_NotTheFilesDefault`（＋layout 版）。⚠️ 期待文字列は `= #3`（`\override RehearsalMark.font-size = #3`）＝`LyNumber` は `#` を付ける。

★★★ **⑼ 第 3 便＝`layout { accidentals … }`（LP の `\accidentalStyle` 表の移植）**: **語は 5 つ＝`default`（既定・18 世紀＝Lily# が元から描いていた唯一の style）／`modern`（Kurt Stone・他オクターブと次の小節でも打ち消す・restore natural 無し）／`modernCautionary`（modern が*足す*ぶんを括弧で）／`forget`（何も憶えない）／`noReset`（小節線で忘れない）**。**家は `Semantics/AccidentalStyle.cs`**（`AccidentalRule(Octaveness, Laziness)`・`AccidentalStyleSpec`・表は `scm/music-functions.scm:1901-2072` の逐語）、**エンジンは `MeasureCollector.CheckPitchAgainstSignature`**（`check-pitch-against-signature` の字面移植＝same-octave／any-octave／key-sig の cond と need-accidental・need-restore）と **`GetDisplayAccidental`**（規則を OR で畳み、cautionary の score が勝ったら括弧＝`accidental-engraver.cc:219-231`）。**記憶は `(step, octave) → (Alter, Bar, Order)`**＝LP の `localAlterations` で、**Order は `ly_assoc_prepend_x` の「先頭が最新」を再現**（any-octave は最新のオクターブを読む）。**戻り値を `(Name, Cautionary)` の対にしたので呼び手 7 箇所を触った**。⚠️ **`ForgetsAtBar`（laziness が全部 0 以下）の style は今までどおり小節線で `Clear()` する**——観測的に同値で、かつ **resume の門 `WalkCarriesNothing` が「臨時記号の記憶が空」を見る**ので、全書に記憶を持ち越すと既定の本まで incremental resume を失う（長記憶の 3 style では resume が起きない＝安全側・番人 `AStyleEdit_RendersIdenticalToFullRecompile_BothWays`）。**双子は各パートの音楽の先頭に `\accidentalStyle modern`**（`\accidentalStyle` は context mod ではなく MUSIC・既定 context が Staff）。**既定の本は出力不変**（`WritingTheDefault_IsTheSamePageAsWritingNothing`・snapshot 0 枚）。**絵を 1 枚描いた**（`scratch/p371/probe-accidentals.lys`＝`modern` は既定より `<text>` が 1 つ多い＝2 小節目の打ち消しナチュラル）。**載せなかったもの**＝LP の voice／piano／choral 族（Voice/GrandStaff/ChoirStaff の**context を名指す**＝score 全体の switch には置き場が無い）と neo-modern／teaching／dodecaphonic 族（`make-accidental-rule` でない規則が要る）。**表に無い語は語の上で拒否**。

★★ **⑽ 第 4 便（ユーザー「このセッションでやる方が有利なら着手して。次のセッションの方が有利なら着手してはいけない」→ *有利*と判断して着手＝**今日 3 回通った `layout {}` の道が頭に在るうちにしか見えない穴で、言語の決定も要らない**）＝`layout {}` に自分で空けた穴を塞いだ**: **公開したキーに読み手の腕が無くても、黙って何も束縛しない**（`ReadEntriesInto` の `_ => plan`）。**`TheVocabularyIsPublished_AndTheReaderReadsIt` はこれを見られない**——`AllKeySpellings()` が語彙そのものを返すので*恒等式*。⇒ **`EveryPublishedKey_IsBoundByTheReader_AndServedByTheEditor`**＝公開キーそれぞれについて **⒜ 値の語が在り ⒝ その語のどれかが plan を既定から動かし ⒞ エディタがそのキー専用の値リストを出す**（`layout { KEY ` の文脈が `LayoutBlock` に落ちない）ことを**列挙して**訊く。**毒で赤を見た**（`poisonKey` を語彙に足し `ValueWords` に語を与え、腕だけ足さない＝半端に足したキーの形）: 「'poisonKey' is published as a layout key and NO value of it changes the plan」と**キーと欠けた腕を名指して赤**。毒は抜いてある（経緯は `TheCheckThatEveryKeyBinds_CanFail` の remark）。**ついでに top-level の `layout` 雛形が 3 つ目のキーを書いていなかった**のも直し、雛形が全キーを名指すことも同じ網が見る。

★★★ **⑾ 第 5 便（ユーザー「すべて進めて」＝残る 3 候補の承認）＝`layout {}` に `sectionLabels` と `partCombineText` を足した**: **`sectionLabels boxed|none`**（既定 boxed＝Lily# が元から描く箱・**箱そのものが OWN**＝LP の `SectionLabel` は素の字／`none` は名前を engrave しない＝パート譜の答え・**双子も `\mark` を書かない**・form は変わらず鳴る＝表示スイッチ）。**家は `BuildAllMarks` の 1 箇所で落とす**——⚠️ **`MusicMarkLayout.SourceIndex` はこのリストの添字**なので、filter する呼び手としない呼び手が混ざると**最初のラベル以降の click 先が全部ずれる**。だから 4 呼び手全部に style を渡す。⚠️⚠️ **`Calculate(score:)` の `score` は multi-staff では null**——`score?.LayoutPlan` から読むと**2 段以上の本で黙って既定になる**（実装中に踏んで番人が赤で捕まえた）。`marksBeside` と同じく**引数で渡す**のが正解。**`partCombineText on|off`**＝LP の `printPartCombineTexts`（off は `LayoutEngine.Finishing` の配置ループごと飛ばす＝text item を作らない LP と同じ形・双子は `\set Staff.printPartCombineTexts = ##f`）。**絵を 2 枚描いた**（`scratch/p371/probe-labels.lys`／`probe-combine.lys`: `none` は `<text>` −2・`<rect>` −2＝名前と箱、`off` は `<text>` −2＝a2 と Solo）。**⚠️ `plain`（箱なしの名前＝LP 自身の絵）は入れなかった**: 箱の寸法は **9 箇所**で値付けされており（`LabelBoxMargin` の remark が列挙）、スイッチは**マークに載せて必須引数で配る**形にしないと 2 箇所が別々に値付けする（§5.2.1② をこの箱は一度教えている）。**押し込まず起票**。**⚠️ 第 4 便のラチェットが追加当日に噛んだ**——雛形が新キー 2 つを名指していないと赤（`EveryPublishedKey_…`）。

★★★ **⑿ 第 6 便（ユーザー「このセッションでやる方が有利なら着手して。次のセッションの方が有利なら着手してはいけない」→ **1 件だけ有利**と判断＝`sectionLabels plain`。理由は**箱を値付けする 9 箇所を数分前に自分で数えてコード註に書いたばかり**で、その表が頭に在るうちにしか安く通せないから。**和音記号の綴りは逆に不利**＝LP の property 調査（`majorSevenSymbol`／`chordNameLowercaseMinor`／german・italian 族）とコーパスの実測が要るので**新しい便で始める方が良い・着手せず**）＝**`layout { sectionLabels plain }` を出荷した**: **箱の無い名前＝LP 自身の `SectionLabel` の絵**（`boxed` が Lily#-own な方）。**スイッチはマークに載せた**＝`MusicMarkLayout.Boxed`。⚠️ **9 箇所の値付け関数は `bool boxed` を*必須*引数で取る**（`LabelBoxMargin`／`LabelBoxHalfWidth`／`LabelBoxHalfHeight`／`LabelBaselineBelowCentre`／`GetMarkHalfExtent`／`BesideTempoX`／`BesideTempoBaselineUp`／`MarkXExtent`／`CalculateXPosition` の行頭腕）——**既定値を付けないのが要点で、落とした site はコンパイルが通らない**（§5.2.1② をこの箱は一度教えている）。**読みは 1 箇所 `IsBoxDrawn(type, style)`**＝`Rehearsal` は `plain` でも箱のまま（キーは section label の名前であり、LP の `RehearsalMark` も箱）。**双子は `\box` を落として `\mark \markup "A"`**。**絵を 1 枚描いた**（`scratch/p371/probe-plain.lys` 対 `probe-boxed.lys`）: **`<text>` は 16 で同数・`<rect>` は 7→5**（枠 2 つだけ消える）・**名前の X は 7.46／29.64 で不動**・**baseline は五線第 1 線の上 1.24 → 0.89**＝**ちょうど枠の margin ぶん下がって、消えた枠の底（線の上 0.85）に字の下端が座る**＝LP の side-position が素の ink に対して measure するのと同じ場所。頁は 21.56 → 20.93 と縮む（reserve が小さくなる）。**⚠️ 唯一そのまま残した site**＝`LabelBoxHalf(MusicMarkType)`（境界 "To Coda" を label の下辺に差し込む古い定数見積り・ink でなく em×0.55）＝**近似台帳が持っている別物なので、ここから padding を引くと嘘が 2 つになる**。remark に明記した。**既定は 1 バイトも動かない**（`SectionLabels_Boxed_IsTheDefault_…` の byte 比較・snapshot 0 枚・台帳 0 点）。

**⒀ 終了時**: **full `scratch/p371/run9.trx` 7764 / 0 / 4 / 7768（run8 の 7766 から +2＝第 6 便の番人・RunInfos は SKIP 4 だけ・4 m 00 s・HANDOFF／CHANGELOG／APPROXIMATIONS 込み）**（便ごと: run3 7744 → run5 7760 → run6 7762 → run7 7766 → run9 7768）・**台帳 821 点／OPEN 0／snapshot 249 枚は 6 便すべてで不動**・commit 6 本（`b18a266b`＝layout ブロック／`6e1238b3`＝双子の穴／`b7f568db`＝accidentals／`f79770a2`＝網／`8dd5204a`＝sectionLabels・partCombineText／`3b4873d2`＝sectionLabels plain）・`Deploy-Lsp.ps1` 済。⚠️ **第 6 便で `ApproximationInventoryTests`／`MagicConstantInventoryTests` が 2 本とも赤**（行番号のずれ＋`LILYSHARP-OWN` の語をコード註の散文で使ったせいで OWN が 136→137 に増えた）＝**散文では `Lily#-own` と書き、タグは宣言にだけ使う**。`LILYSHARP_UPDATE_DOCS=1` で再生成して 248 に戻した。⚠️ **この便の途中で ripple の pwsh が PATH から消えた**（全コンソール closed・`start_console` も失敗）＝**pwsh MCP に切り替えて継続**。§7.5（第 3 便）Core `+` 約 +250 行（新ファイル 1 本＋エンジン）／**REF 11**（全部 LP の既存関数・表の住所）／**OWN 0**。⚠️ **`LILYPOND-REF` の記号名は*行範囲と同じ行*に**——第 3 便は 11 本まとめて踏み、`recent-enough?`／`accidental-styles`／`same-octave` はどれも**ハイフン 2 節＝記号と見なされない**ので「行範囲を落として名前だけ書く」綴りに直した（既存の前例と同じ）。**未追跡の新規テストファイルは `git diff` に出ない**ので、自作の走査で漏れた 2 本はそこに在った。**push はユーザー**。
...
- ✅ **F-mdfence. `.md` の ```` ```lys ```` フェンスを VS Code 標準の Markdown プレビューで楽譜にする**（2026-09-09・第359 起票・**ユーザー指示「起票して着手して」**・✅ **段階 ⑴ は第 2 便・⑵ snippet は第 4 便で実装＝§1 第359**・残り＝⑶ フェンス内の診断・補完（需要待ち）。題名が音楽より広い本の切り抜きは第 5 便で閉じた（`HeaderBand.Width`）） → **本文は `HANDOFF-ARCHIVE.md`「閉じた §2 の本文」の同じ見出し**（2026-09-12 に落とした）
