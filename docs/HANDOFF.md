...
- ⚠️ ★★ **新規＝repeat の無い volta ending が黙って消える**（第187 実測・**未修理**・
  **空 form の島の引き算で出た 1 形**）。**`form main { [1. A] }` は section を名指しているのに
  0 バイトの SVG**（`lysc check` は `No errors found`）で、**`form main { A [1. B] }` は
  `form main { A }` と*インクがバイト同一***（data-pos を伏せて `0076EA2E…`。対照 `[1. B]` が
  実在すれば `E6021E15…`）。**`[1. ]`（名前なし）も同じハッシュ。**
  ⇒ ★ **綴りは 2 つに割れる**: **bracket `[1. A]` は丸ごと消える**が、
  **legacy `1. A` は `1` と `.` が迷子トークン（LYS0030）として報告され、`A` は普通の参照として彫られる**
  ——**報告される側が正しく振る舞っている**という、第185 の「報告する／保つ」の裏返し。
  ⇒ ★★ **規則の形が決定**（「volta ending は repeat を要る」を error にするか）。
  ⚠️ **「repeat の外の ending」は正規の綴り**（`|: A [1. B] :| [2. C]` の `[2. C]` が正にそれ）なので、
  **規則は「その form に repeat block が 1 つも無い」でなければならない。**
  ⚠️ **落ちる本は 0 冊**（1025 冊で 0）＝falsifier が無いので §5.4 の順序どおり起票。
  ★ **射程は数で書いてある**: `DiagnosticCodes.EmptyForm` の remarks（46 形中 16 形が 0 バイト・
  15 形は LYS6007・**残りがこれ 1 形**）と `EmptyFormValidatorTests
  .AVoltaEndingNoRepeatOpens_IsNotThisChecksBusiness_AndStillEngravesNothing`（**穴をピン止め**）。
