...
最終更新 第352セッション（2026-09-08）＝**入り方が違う便**: ユーザーが「HANDOFF の*ユーザー判断*を今まとめて決めたい」と言い、§2 の判断待ち 9 件を私が並べ、ユーザーが 3 往復で全部決めた（第 1〜3 便＝決定・製品 0 変更・commit `881556ed`＝HANDOFF §3 に 2026-09-08 行 7 本＋各項に ✅）。**第 4 便＝ユーザー「このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」**→ 決定と LP の出典と該当行が全部文脈に在り、儀式 1 回で小物を束ねて通せるので**決定のうち小物 4 件に着手した**（下 ⑵）。**第 5 便（同じ指示の 2 度目）＝⑥ `removeEmpty` の score 移行を入れた**（⑵⒠）。**着手しなかった 2 件と理由は ⑹**。**骨は 6**:

★★★★ **⑴ 決定 9 件（§3 の 2026-09-08 行と各項が正・ここは索引）**: ① T6⒞ `@mark` 438 札＝apply（**→ 既に第332 第 3 便で apply 済みだった**・§2 の行が stale＝直した。⚠️ **§2 の「▶」を読むときは ARCHIVE の同じ便の「実行した」行も grep すること**）／② T6⒥ break 21 冊＝触らない／③-i **途中の `partial` は許す・パートごとに書く**（`time` と同じ規則・§2F ⒱）／③-ii **T6 ピックアップ census 8 冊＝弱起を前 section へ戻す**（`.ly` と同じ絵・途中の `partial` は census には効かない＝section は小節線で始まるので小節 +1 のまま）／④ ⒵ suffix splice＝閉じる（「落ちる」はクラッシュではなく splice 不採用・出力は full と同一）／⑤ **`time none` は彫る**（LP は `\cadenzaOn`・双子は既にそう書く・§2F ⒤）／⑥ **`removeEmpty` は score 側 `staff m as lines 1 removeEmpty all`（`as` 1 つに列挙）・`pedal` は part のまま**（§2F ⒣）／⑦ 展開の予算＝絵だけのもの・LYS1033 の文面（§2F ⒩）／⑧ `_ "text"`＝GLUED 維持・診断文面だけ／⑨ **歌詞の `~` は Lily# の規則として残し、双子は `~` の音節に `\once \override LyricText.self-alignment-X = #LEFT`**（§1 第351 ⒠⑴）。

★★★ **⑵ 第 4 便で入れた 4 件（製品・未 commit＝作業ツリー・下 ⑸）**: **⒜ 途中の `partial`**＝`PartialScopeValidator` を「bar の無い場所だけ拒む」（top-level と part ヘッダ）に緩めた。walker（`MusicWalk.cs:1702 → SetPartial`）と充足検査（`MeasureValidator.cs:343`）は元から途中の `partial` を読んでいたので製品は validator＋案内文（`MeasureValidator` LYS2006 の文・LSP 補完に `partial` を戻す）＋GRAMMAR §2／§8.1／GRAMMAR_FOR_LLM。**番人 `PartialScopeValidatorTests` 9 本**（part-major cell と part block は OK に反転・**mid-piece `| partial 2. r2. |` が 3 小節目を 3/4 に閉じ LYS2001 無し**・**片方のパートだけに書くと LYS2004 の不一致**・top-level は error のまま・`MusicCompletionFlatSpellingTests` の網に `partial 4 g4 |` を足した）。**Lily# の規則＝「その bar はこの長さ」**（bar の頭に書く。LP の「残りの長さ」は音符の後に書いたときだけ違う＝GRAMMAR に明記）。**⒝ 双子の melisma LEFT**＝`EmitLyricTracks` で `l.MelismaAlignLeft` の音節の前に `\once \override LyricText.self-alignment-X = #LEFT`（番人 `AttachedLyrics_AMelisma_*` に主張追加）。**LP 対（`scratch/p352/lp/pair.ps1 -Lyr`・LP 2.26.0）**: `lyrics-verses` の "how"/"my"/"saved"/"grace" の LP ext が `(0 . w)`＝LEFT に**なった**・bar 4 は Lily# 8.44 対 LP **8.251**（override 前の LP は 8.15＝alignment の分 +0.10 は閉じ、**残差 0.19 は別の島**）・bar 1-3 は 12.89/19.34 対 12.887/19.306。**`.ly` 掃き（`scratch/p351/ly/sweep351ly.ps1`・921 冊・base＝HEAD `881556ed` の exe）: MOVED 5＝追跡 4（`lyrics-verses`・`lyrics`・`samples/amazing-grace`・`greensleeves`）＋ユーザー 1（`雪やこんこ`）・全部 override 行の追加だけ**。page/svg・snapshot・台帳は不動（exporter だけ）。**⒞ LYS1033 の文面**＝「Only the picture: 'lysc midi', 'lysc xml' and 'lysc ly' write the whole expansion」（`ExpansionBudgetTests` に主張・Midi/MusicXml exporter の注記も追随）。**⒟ LYS1005 の `_` ヒント**＝`_` が未定義のときだけ「glue the quote to the underscore: _"text"」（`SymbolReferenceValidatorTests` 3 本＝空白あり／GLUED は無診断／他の名前にはヒント無し）。CHANGELOG 0.7.0 に Language 1・Diagnostics 2・twin 1。

✅ ★★★ **⒠ 第 5 便（ユーザー「有利なら着手」の 2 度目）＝決定 ⑥ `removeEmpty` を score 側へ**（`time none` と census は着手せず＝⑹）。**綴り `staff [~][clef] part ["名"] [as lines N] [removeEmpty V]`＝`as` 1 つの後に選択子を任意の順で列挙**（`ossia` も同じ・`tab` は取らない＝tab は元から hara-kiri しない）。**製品**: `Parser.Form.ConsumeStaffSelectors`（旧 `ConsumeLinesSelector`・値は `SymbolCaseValidator.RemoveEmptyValueVocabulary` を Ordinal で検査・`as` の後が選択子でなければ LYS8005 系 `UnknownSymbolCase`）／`RenderSpecParser.CutStaffSelectors`（旧 `CutLinesSelector`・`StaffSelectors(Lines, RemoveEmpty, RemoveFirst)` を返す・`as` の直後が選択子語のときだけ切る＝`as` という名の part は無事）／`ParseStaff` は part ヘッダの `removeempty` を読まなくなった（`pedal` はそのまま）／`SymbolCaseValidator.PropertyNames` から `removeEmpty` を落とした（part ヘッダでは unknown property＝`lines` と同じ網・新 code 0）／LSP: part 補完から外し、`staff NAME` の後に `as lines`／`as removeEmpty`、`as` の後に両語、`removeEmpty` の後に値（`StaffAsGoverns`＝`as` が支配しているときだけ）／TextMate: part-body の規則と key list から除去（score の `as` 選択子は `lines` 同様に色を付けない）／GRAMMAR §3・§7（`StaffSelector` 生成規則）・GRAMMAR_FOR_LLM・CHANGELOG Language。**番人**: `HaraKiriTests`（selector→flags の Theory 6 行＝列挙順 2 通り込み・`TRUE`／`banana`／`as hidden` の拒否・part ヘッダの `removeEmpty` は unknown property）・`StaffLinesCompletionTests` +6（文脈 4・項目・提供値が全部コンパイル）・`SymbolCaseValidatorTests`／`PartPropertyCompletionTests`／`CompletionVocabularyTests`／`EditorColouringTests` を綴りの移動に合わせた。**移行**: fixture `test/hara-kiri`＋`audit/lpreg` 3 冊＋`audit/lp-regression` 1 冊（⚠️ **`harakiri-percent.lys`／`hara-kiri-percent-repeat.lys` は `part rs { lines 1 … }` のままで 2026-08-19 から壊れていた**＝lpreg 取り直しが拾うはずだった債・今回 `as lines 1 removeEmpty true` に直した）＋`LpGeometryProbes` の 5 本（LYRHKD/LYRHKG/HKW/HKCD/ROWH＝台帳の点は綴りに依らず不動）。**snapshot `test/hara-kiri` 1 枚を再ベース＝data-pos だけ**（fixture の行が動いた分。`data-pos` を伏せた本文は HEAD と同一＝下 ⑸ の確認行）。ユーザー実コーパス 0 冊（掃き不要）。

✅ ★★★ **⒡ 第 6 便（ユーザー「census 8 冊は当てて良い」）＝T6 のピックアップ census を当てた（コーパス側・製品 0）**: 計器 **`scratch/p353/pickup/apply-pickup.ps1`**（`-Apply`・原本は `backup/`）＋ **`check-books.ps1`**（`lysc check`／`layout` の before／after／compare）。⚠️⚠️ ★★★ **向きは本によって逆で、`.ly` が決める**——第332 の「弱起を前 section へ戻す」（A 形＝`… r2. b4 \break \mark X`）は **38 対のうち 3 対だけ**で、**35 対は B 形**＝`.ly` の `\mark N` が*休符で始まる小節の頭*に立ち（`\mark D1  r8 b b r …|`・`\mark B1  r2 e4 e8 a |`）、**変換器がその先頭の休符を前 section の末尾に置き去りにしていた**（変換器は mark の後の最初の*音符*で section を切っていた）。⇒ **B 形の直し＝P の末尾の休符を N の頭へ*進める***。P が変換器の隠し section（`~Body_1 { r8 }`・`~C1_2 { r1@mark("D1") r1 r1 r2 r8. }`）で休符しか無ければ**section ごと消し、form の参照も外し、N と同名の `@mark` は落とす**（`section N` の札と二重になる）。移した `r1` には `|` を打つ（`r1 r2.. fis,8 |` を 1 小節と読む LYS2002 を出さないため）。**1 度目は全部 A 形で当てて `Private Eyes` の B1 札が 1 小節遅れた**（svg 比較で気付き backup から戻した）。**結果**: **35 か所・22 冊**（Danger Zone／Eine kleine ×3／Eternal Frame／Express Yourself／Gotta Pull／Hello／Holiday／Hotel Pacific ×3／Locked out of Heaven／Neutron Dance／Nothing's Gonna Stop Us Now／Private Eyes ×3／Round and Round／Venus ×2／たかがアイラブユー／にんじゃりばんばん ×4／もう恋なんてしない ×3／未来予想図Ⅱ ×2／残酷な天使のテーゼ／渚・モデラート ×2／遠く遠く）。**門: `lysc check` error 0→0・警告は減るだけ（例 Hotel Pacific 6→0・にんじゃりばんばん 7→0）・新規の診断種 0／`lysc layout` の小節数は全冊 LP（`structure332.csv` LpBars）と一致のまま**（ページは block の終端で閉じないので、割れた小節は元から 1 小節に組まれていた＝動くのは札の位置と警告だけ）／**svg（data-pos 伏せ）: 22 冊中 14 冊バイト同一・8 冊は section 札が休符の小節頭へ移った**（`.ly` の `\mark` の位置）／**census 156 → 86**（`RestoreMarks -- partial`・−70＝35×2）。**手に残った 3 対は第 7 便で手で当てた**（`.ly` を読んで）: `Butterfly`＝Body_1 を `\break` で割り、7 小節の休符と `r2. ais,4\2` を Intro2 へ（`.ly` L73-76）／`Time After Time`＝`repeat percent 2 { r1 | r1 } r1 | r2 r8` を Body_2 から C2 の頭へ（`.ly` L79-85）／`You're So Vain`＝弱起 `a,,4@staccato e,\3@fall break` を Body_1 の末尾へ戻し、弱起だけの `section Intro` を消して form から外した（`.ly` L45-60・A 形）。3 冊とも error 0・警告 2→0・**census 86 → 80**。⚠️ `Butterfly` 108 対 LP 107・`You're So Vain` 37 対 LP 36 の +1 は**この前から**（T7 C＝`You're So Vain` は `.lys` L25 の `e1` が `.ly` に無い・ARCHIVE 第332 ⑺⒟）。⚠️ **Edit ツールは CRLF の本を LF に書き換える**——2 冊を CRLF に戻した（`You're So Vain` は元から CRLF のまま）。**残る census 80 は別の族**（曲頭の弱起＝`section Body { partial N }` の候補 30・`My Girl` のように宣言済みで census に残る物・隣接しない短小節）。

⚠️⚠️ ★★★ **⑶ 開始時の full が赤 1 本＝`HandoffLedgerCitationTests.TheCitationsAreNotQuietlyDeleted`（36 → 28）——第351 第 4 便の Fold が原因**（§2 の閉じた 64 項目の本文を ARCHIVE へ落とし、HANDOFF §2 の `<!-- ledger: -->` 札が 42 → 19 に・ARCHIVE は 29 → 58＝消えていない。**その便は製品 0 変更なので full を回さず、赤は 1 便遅れて今日出た**）。**直し＝`CitationsWhenWritten` を 28 に下げ、理由を remark に書いた**（ARCHIVE は `Covered` の外＝逐語の歴史・意図どおり）。⚠️ **Fold を走らせた便は製品 0 でも full を回すこと**（門は docs も見る）。

★★ **⑷ 拾い箱（着手していない）**: **⒜ `audit/lpreg/lymelmel.lys` は第216 の文法で stale**——`lyrics w`（`sings` 無し・名前も part と違う）は今日は*独立行*で、`RowSyllables` が `__ ~ ~` を extender に畳んで "looong" を**中央寄せ**（svg `x=19.68`＝c16 頭中心 19.03+0.65）。LP 手書き双子（`\addlyrics`）は左寄せ＝**対は壊れている**。`LyricMelismaAlignmentTests` は `lyrics w sings v` で緑。直しは fixture に `sings v` を足す（lpreg 取り直しのとき）。**⒝ `lyrics-verses` bar 4 の残差 0.19**（Lily# 8.44 対 LP 8.251・alignment の後）は未分類。**⒞ MIDI は途中の `partial` を読まない**（`MidiExporter` に `PartialDeclarationSyntax` の腕が無い＝小節番号を持たないので実害は無いはず・未測）。

★ **⑸ 数**: **開始時裏取り**（`tools\Session-Check.ps1 -Build -Test -Scratch p353`）: HEAD **`881556ed`**・**未 push 35**・`origin/master`＝`21efb993`・木 clean・未追跡 0・`--no-incremental` build 0 エラー／Core 0 警告・**full 7266 / 1 / 4 / 7271**（`scratch/p353/run1.trx`・+1＝第351 第 4 便の門 `TheHandoffStaysReadable`・**赤 1＝上 ⑶**）・`gh run list` 5 本緑（最新 34121145135）。**終了時（第 4 便）**: **commit 2 本（製品＝Core 6 ファイル＋Lsp 1＋番人 6 ファイル＋CHANGELOG＋GRAMMAR／GRAMMAR_FOR_LLM／APPROXIMATIONS（`LILYSHARP_UPDATE_DOCS=1`・行番号 1 行）／この HANDOFF＋ARCHIVE）・未 push 37・木 clean・未追跡 0・full 7272 / 0 / 4 / 7276（`scratch/p353/run3.trx`・+5＝番人 5・`run2.trx` は赤 2＝引用の記号名（`\partial` は記号に数えられない→ `context-spec-music` を同じ行に）と APPROXIMATIONS の行番号）**・台帳 810／snapshot 247／追跡 `.lys` 597 不動。§7.5: Core `+` 83・LILYPOND-REF 1（`music-functions-init.ly partial = context-spec-music 'Timing`）・LILYSHARP-OWN 0。**perf: 計算は足していない**（validator の分岐と exporter の 1 行）。**Fold**: ⒵ と ⒱ の本文 36 行を ARCHIVE へ（HANDOFF 392 KB）。**終了時（第 5 便）**: **commit 2 本（製品＝Core 6＋Lsp 2＋TextMate 1＋番人 8 ファイル＋fixture 5＋probe 1＋snapshot 1（data-pos のみ・`git diff -U0` で data-pos 以外の変更行 0）＋CHANGELOG＋GRAMMAR／GRAMMAR_FOR_LLM／この HANDOFF）・未 push 39・木 clean・full 7284 / 0 / 4 / 7288（`scratch/p353/run4.trx`・+12＝番人）**・台帳 810／snapshot 247（枚数不動・1 枚再ベース）／追跡 `.lys` 597 不動。§7.5（第 5 便）: Core `+` 148・LILYPOND-REF 2（`context-mods-init.ly RemoveEmptyStaves`×2・範囲なし＝記号名の規則）・LILYSHARP-OWN 0。**perf: 計算は足していない**（parser の分岐と選択子の切り出し）。**終了時（第 6 便）**: commit 1 本（この HANDOFF）・未 push 40・木 clean（コーパス `scratch/ベースタブLy` は未追跡＝22 冊を書き換え・原本 `scratch/p353/pickup/backup/`）・製品 0＝full は run4 のまま。**終了時（第 7 便）**: commit 1 本（この HANDOFF）・未 push 41・木 clean・コーパス 25 冊（+3・原本は同じ backup）・製品 0。⚠️ **scratch は `p353`**（`p352` は第351）。⚠️ **§7 3.5**: 第350 のブロックを `Session-Check.ps1 -Archive 350` で ARCHIVE の先頭へ・第351 の経緯は下の見出しの下に残した。

⇒ ★★★★ **⑹ 次の一手**: **⒜ push は禁止のまま**。**⒝ 残る決定 1 件**: ~~③-ii census~~＝**第 6 便で当て、第 7 便で手の 3 対も閉じた（⑵⒡）**／**⑤ `time none` を彫る**＝`MeasureBuilder` の境界を作らない腕・fixture＋LP 双子の対から（観測者 0・§2F ⒤ に移植点 4 つ）＝**設計級で LP 対の測定が要り、文脈の利点が小さい**。（⑥ `removeEmpty` は第 5 便で入れた＝⑵⒠。） **⒞ 第351 ⑹⒞ の並びの残り**（⒡⑵ 行の拍グリッド＝実需ゼロ／ps2 の 0.017／`audit/lpreg` 取り直し（上 ⑷⒜ も一緒に）／§2 U8b・U8／A/B/D/E／C⑴／G）。**⒟ 段端の floor の regime**（第351 ⑹⒟）は今も未測。
