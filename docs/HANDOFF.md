...
### 1.1 第432セッション（2026-09-20・YT-DELL2）
...
`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `7f4cf2ce`・**未 push 47**・**full `sessions/p431/run1.trx`
8760 / 0 / 3 / 8763＝第430 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・129 秒）、
★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第430 が
予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 429`＝22 行 6,434 chars）。指示は「**HANDOFF を
読んで着手**」＝⒳⁗ に入りかけたところで、★★★ **ユーザーから 2 つの指示**——「**セッション開始のプロトコルは
少し重すぎる。もっと安く合理的な手順に直せるか**」「**セッションを閉じるときの手順と重複があれば整理したい**」。
⇒ **着手を protocol に切り替えた**（**⒳⁗ はコードに 1 行も触っていない**・道具は pwsh MCP・Bash 0 回）:

★★★ **⑴ まず測った——protocol は*書かれたとおりには実行されていない***。`RULES.md` は **5,410 行**で §0 は
「通しで読む」と言うが、**§5.0 1,547 行＋§5.3 1,501 行＝全体の 56% は規則ではなく*事例集***。**今便が実際に
読んだのは 860 行／5,560＝15%** で、**§0 が 4 回名指す §5.5 は 1 行も開いていない**。⇒ ★★★ **同じ失敗が 1 つ前の
寸法で記録されている**——RULES 自身のヘッダが「1.7 MB のころ §5 は grep で当たったものしか存在しなかった」と
書いており、**分割で住所は直ったが §5.0 が 1,547 行に育って同じ状態に戻った**。⚠️ **値段が高いのは*裏取り*では
ない**（あれは既に 1 コマンド）。**その周りの散文と、同じ事実が 3〜4 か所に居ること**だった。

★★★ **⑵ 重複は 3 つとも「1 つの事実が 3〜4 か所」の形**。⑴ **§7 3.5 は自分の本文で「これは*セッションの頭*の
手順」と言いながら*終了時チェックリスト*に居り**、そのうえ**毎便の §1 末尾が手書きで「次便は `-Archive N` を
回すこと」と再掲していた**＝3 か所。⑵ **引き継ぐ数は §6.1 が定義・§0 が警告・§7 1 が要求・スクリプトが実装**
＝4 つの言明に実装 1 つ。⑶ ★★ **§1 の「次の一手」は*毎便 2 度*書かれていた**（§1 が持つ 2 つの語りブロックの
両方に全文）＝**最初に読む物が 2 つの複写を持ち、1 便 4 KB 級が 450 KB の天井に載っていた**。

★★★ **⑶ 直し**（`71209287`・**Core 0 行**・網 1 本・**測定は 1 つも足していない**）: **`-Start pNNN` と
`-End pNNN`**。⚠️ **`-Start` は*裏取りが先・アーカイブが後***＝**順序が load-bearing**（アーカイブを先に回すと
§1 が predecessor を失い、**`HandoffArchiveContinuityTests` の 2 本が*裏取りの run で*赤くなる**）。落とす
ブロック番号は**HANDOFF 自身から読む**（網と同じ正規表現＝手で「N−2」を数えない）。そのあと **§1.0 を逐語で
刷り**、天井の残りと**スクリプトが言える罠 4 つ**を出す。**`-End` は §7 の機械的な半分を*チェックリストの番号付き
門の表*に**（天井・継ぎ目・毒と計器の残骸・棚卸し・作業ツリー・未 push）。⇒ ★★ **散文に残ったのは 3 つだけ＝
§7 7.5・7.6・7.7**（どれも判断で、機械が言えない）。**「次の一手」は §1.0 の一覧へ**（保守は 1 か所）。

★★ **⑷ 網 1 本と毒 1 つ＝今度は被害者が出なかった**。`TheHandoffCarriesOneNextMoveList`（§1.0 の見出しが
ちょうど 1 つ・§1 の中に在る）。**毒＝見出しを改名**すると**その 1 本だけが赤く、既存 4 本は緑**（第430 ⑸ の
「被害者」の形にはならなかった＝**汚染ではなく読み取りの網だから**）。⚠️ **見出しだけを見て中身は見ない**——
一覧の*形*は次便の仕事で、そこを固定する網は「守っている当の仕事のために書き換えねばならない網」になる。

⚠️⚠️ **⑸ 一覧を移すついでに ⒳⁗ の値段が*3.3 倍*だったことが出た**（§5.0 14例目と同じ型・**16 例目**）。
第430 の起票は「`SortAndResolve` の `resolved` ＝ 0.128%（18,912,792 B／sweep・10,234／打鍵）」と書いていたが、
**引用元 `sessions/p430/pad-split-leg1.txt` のその数は `pad.sr` ＝ region 全体**で、第430 自身の内訳は
`walk.add` 3,082 ＋ `ov.range` 6,437 ＋ `sr.own` 715（／打鍵）。**名指された直し（`RentResolveOutput` を借りる）が
当たるのは第 1 項だけ＝約 0.039%**。⇒ **§1.0 に「値段は分母つきで」「賞金と『直すなら X』は別々に反証する」を
書き、§7 2 にも 1 本**（RULES §5.0 12例目・14例目の運用版）。

★ **⑹ 終了時**: commit 2 本（protocol `71209287`・docs 1 本＝この §1。**この文を含むので SHA は書かない**＝§5.4）・
**未 push 49**（開始時 47）・**最終 full `sessions/p431/run2.trx` 8761 / 0 / 3 / 8764**（**第430 最終 8763 から
+1＝今便の網ちょうど 1 本**。`Zz431`／`ZZPOISON` の grep は**コードに 0 件**）・§7.5（対 `7f4cf2ce`）**Core `+` 0 行／
REF 0／OWN 0＝Core を 1 行も触っていない**（§7.6 ⒟）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・
追跡 `.lys` 609・**棚卸しは `APPROXIMATIONS.md` も `magic_constants.csv` も差分 0**。⇒ ⚠️ **HANDOFF は第430 の記録 445 KB 級 →
アーカイブ後 435,467 B → 今便を書いて 442,072 B＝天井 450,000 まで 7,928 B**（**一覧の複写をやめた分 4 KB が
返ってきている**・`Fold-ClosedHandoffItems` の弁は第426 の時点で 0 件・§3 の圧力弁は第409・第410 で使い切り。
**天井は上げない**）。⇒ **次便は `-Start p432` の
1 コマンドから入る**（裏取り → `-Archive 430` 自動 → §1.0 の一覧）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が
push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第431 の欠陥ではない。**作業ツリーは空**。
**push はユーザー**（Lab も）。
