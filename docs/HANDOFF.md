...
最終更新 第400セッション（2026-09-18・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `26fd4cf5`・未 push 20・**full `sessions/p400/run1.trx` 8647 / 0 / 3 / 8650＝第399 第 2 便の数と一致**・CI は直近 3 run が赤＝未 push のまま・build 0 エラー）、指示「HANDOFF を読んで着手」→ §1「次の一手」の R13 の続き。道具は pwsh MCP（Bash 0 回）。**まず打鍵 1 回を段階で割った**（`sessions/p400/Zz400Probe.cs.txt`＝`IncrementalCompiler.StageProbe` で collect／keys+gate／layout／render・min-of-5・Debug）: plain1k の無変更打鍵 33 ms＝**collect 25**（resumed・1000 小節全部採用でも）＋keys 3.5＋layout 0＋render 3.5、fingbeam1k 99 ms（collect 72）、v2bow1k 29（collect 21）。**診断 pass はそれより大きい**（`SemanticValidation.Run(lent)` plain1k 59・fingbeam1k 333・v2bow1k 53 ms）。**閉じたのは 2 本**:

★★★ **⑴ `MeasureValidator` 15 ms の正体は `CrossPartMeasureValidator` の *捨てる Split***（`e4fb4bc5`）。validator の中を割った計器（`Zz400Probe2.cs.txt`）: plain1k 14.8 ms のうち cross-part が 12.2・fingbeam1k 69 のうち 58。単一 part の本で ⒜ `SectionBarCounts.SemanticVoices(root)` が section-major の voice まで `MeasureModel.Split` してから part-major pass が捨て、⒝ `ValidateSectionCrossPart` が「voice が 2 未満なら return」の門の**前**に全 part block を Split、⒞ phrase 表を 3 回目の全木 walk で作り直していた。直し＝`SemanticVoices(root, phraseBodies, partMajorOnly)`（section-major は Split しない）・門の後で Split・`MeasureValidator._phraseBodies` を ctor で渡す。**後**: 14.8 → 4.9／69 → 22.5／15.8 → 3.6 ms（pass 全体 59 → 52／333 → 282／53 → 44）。**射程**＝census（`Zz400Probe3.cs.txt`・759 冊の MeasureValidator 診断を `sessions/p400/census-{base,head}.txt` に dump）**120 行が完全一致**。網 `PartMajorOnly_IsTheUnfilteredCallsPartMajorSubset_OnEveryNetBook`（表あり／なし両方）。**残り**: fingbeam1k で `SymbolReference` 32／`LyricSings` 26／`AnnotationName` 17（3000 個の `@finger` に 4 µs ずつ）／24 本 × 4.5 ms の全木 walk（＝第399 の判断どおり見送り。⚠️ `KindSites` は red が cached のとき `DescendantNodes` より*遅い*（1.8 対 1.0 ms・plain1k）＝validator の green walk 化は効かない）。

★★★ **⑵ 採用済み resumed collect の 25 ms の正体は「edit と無関係な事実のための全木 walk 5 本」**（`ed3dceb0`）。一時計器（`ZzPhase.Mark` を Core に置いて測り repo からは消した・`Zz400Probe4.cs.txt`）で plain1k の 27 ms を割ると **beams 12.2／walk 9.0／partdefs 3.7／defs 2.0**。⒜ partdefs＝`GetPartDefaults` → `PartTranspose.Read(root, part)`＝`ReadScoreDefault`＋`FileIsConcert` の green walk 2 本 **× part 数**。⒝ walk 9.0 のうち **5.6 は `GetCanonicalSectionBars` → `CanonicalByNameSyntactic(root)`**＝`KindSites(SectionDeclaration)`＋`PhraseGreens` の walk 3 本、3.0 は section の GreenSite list の gather（全部採用しても要る・未着手）、restore 自体は 0.2。⒞ beams 12 は **Debug の memo hit が live 再検出で照合する `VerifyReplayAgainstLiveDetection`**＝Release は払わない（`BeamDetector.cs:199-204`）。⒟ v2bow1k の 6 ms は extra voice の live walk（設計どおり）。**直し＝`CollectDefinitions` の 1 walk（`DefinitionSites`・同じ pre-order の green walk）で拾う**: `PropertyAssignment` を definition kind に足し、`PartTranspose.TryReadScoreDefault`／`ConcertPitch.TryReadFileMode`（全木 reader もこの述語に委譲＝1 綴り）の**最初の site**を `_fileTransposeDefault`／`_fileIsConcert` に、section／phrase／variable 宣言を順に list へ → `CanonicalByNameSyntactic(sections, phrases)`／`PhraseGreens(phrases, variables)` の overload（root 版は委譲）。**後（無変更打鍵・collect 段）**: plain1k 25.4 → 17.8／fingbeam1k 72 → 33／v2bow1k 21 → 13 ms（打鍵全体 33 → 25／99 → 51／29 → 18・fresh `TryCollect` 35 → 29／177 → 126／27 → 18）。網 `CollectDefinitionsFoldTests` 3 本（net＋audit 全冊で 2 つの既定と canonical 表が全木 reader と等しい＋手書き 2 形）。⚠️ 踏んだ罠 2 つ＝`MeasureCollectorResetTests` の Reset 網（新しい list は `Reset()` でも clear する）・XML doc の `<param>` を `//` 註の後ろに置くと CS1587（Core は 0 警告）。

★ **⑶ 終了時**: commit 2 本＋docs・**最終 full `sessions/p400/run5.trx` 8651 / 0 / 3 / 8654**（+4＝⑴ 1 本・⑵ 3 本・run4 は docs 前の同数）・§7.5（対 `26fd4cf5`・スクリプトの出力）**Core `+` 236 行／REF 0／OWN 0**（数値定数なし・LP 由来の式なし＝監査対象は「同じ答えか」だけ＝⑴ の census と ⑵ の網）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 608・APPROXIMATIONS.md 再生成（行番号のみ）・CHANGELOG 0.8.0 Editor に 2 項。§7 3.5 は済（第398 を `-Archive 398`＝21 行 9,800 chars）。**途中でユーザーが origin を merge した**（`fc20afb9`＝origin/master `4ee6bdf0` docs 3 ファイル・以後 HEAD はその上・未 push 24）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⇒ **次の一手**: ⒜ 打鍵の残り＝section の GreenSite gather（plain1k 3 ms・fingbeam1k 12）を採用時に lazy に／⒝ 診断 pass＝24 本 × 全木 walk 1 本（fingbeam1k 108 ms・plain1k 25）の 1 walk 化は依然「大手術」・`SymbolReference`／`LyricSings` は第399 の見送りのまま／⒞ R13⒝〜⒤／LP 双子が要る R7〜R11／R15（要承認）。
