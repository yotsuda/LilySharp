...
⇒ ★★★★ **⑺ 次の一手**: **⒜ push はユーザー**。**~~⒝ 承認ゲート~~＝第 2 便で適用・commit 済み**。**~~⒝′ 第 3 便の patch~~＝第 4 便で適用・commit 済み**。**第 5 便は着手しない**: p361 の計器で測れる残りは ⒞②〜⑥（DotGap は EngravingDefaults の記録どおり台帳点を先に・drhs の臨時記号側・bar 1 の −0.05・slash・ps2 の 0.017）で、どれも観測者 1 冊か別 regime＝この便の道具の利が無い。**⒞ 拾い箱（この便が測って名指しただけ・未起票）**: ~~①衝突 shift の量~~＝第 3 便で閉じた（承認待ち）／②`DotGap` 0.3 対 LP 0.45（`EngravingDefaults` に記録済み・台帳点が要る・dcv／dfd の −0.15）／③drhs の r16.→cis'♯ +0.19（base から在る・臨時記号側）／④bos bar 1 の −0.05（16 分 4 つの梁の対）／⑤`slashprobe` の +1.1（percent の別島）／⑥ps2 の 0.017＝c2→e4 の wish 無し対が LP 2.066 対 素の 2.049（**LP 側が素の ideal より 0.017 広い理由は未特定**）。**⒟ 第358 ⑺ の並び（旗 reach 島の分離・`time` 再印字＋全音符の +1.0・⒞⒟⒠）はそのまま**。
