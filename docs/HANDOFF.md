...
**未 push 67**（**この引継ぎ commit まで数えた値**）・
テスト **3812 passed / 0 failed / 3 skipped**（**+25**）・
台帳 **388 点**（**ss 非ゼロ 86・総和 0.438008611**／**count 点 99・うち非ゼロ 2**）
＝**総和が 1.088457611 から 0.650449 縮んだ**（⑪）。
...
## ⑪ ★★ **grace の approach を LP の機構に替えた**（`af968e4a`・**0.850449 → 0.200000**）
...
**＝ Lily# の 1 本のばねが LP の 3 本に当たるという構造の差。** **▶ に別項として起票。**
★ **control `grace.column.approach.main-control` は EXACT のまま**＝**島の外に手が届いていない証拠**。
...
★★ **`grace.column.approach` は 0.850449 → 0.200000**（`af968e4a`・**ユーザー承認・snapshot 4 枚**）。
**残った 0.2 は別の項＝ステム補正の相手**（⑪）。
...
★★ **起票＝grace の approach のステム補正が LP と別の相手を見ている**（**⑪ の残り 0.2・実測済み**）。
**Lily# は 1 本のばねを主音に対して価格付ける／LP は 3 本に割れていて最初の grace に対して付ける**。
**直すなら「先頭 grace を右列として補正を計算する」**——**`GraceNoteInfo` は StaffPosition と
BaseDuration を持つので stand-in は作れる**が、⚠️ **grace の符尾は LP では常に上向き強制**
（`ly/grace-init.ly`）なので、**音高から向きを導く経路をそのまま使うと高い grace で割れる**。
**要承認＝描画が動く。**
