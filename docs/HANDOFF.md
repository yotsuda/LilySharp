...
> ## ⚠️ ★★ **打鍵の外れ値＝articulation 1 個あたり約 0.15 MB・約 30 µs**（2026-08-17・第190 実測・**未修理**・**射程は実ファイル 0 冊**）
>
> **`perf-fingstack1k` が 1 打鍵で 3.3 GB 確保して 430 ms** かかるのを二分した結果。
> **アロケーションは決定的**（同じ本でレンダごとに 0.1 MB まで同値）なので、時間ではなくこれで割った。
> ⚠️ **時間では割れない**——同じ run で**同一コードの対照が 16% 揺れた**。
>
...
>
> **⑵ 何が高いかを書き分けて測った**（同じ骨格の本を 4 形 × 125〜1000 小節）:
...
>
> **⑶ 住所**: `ArticulationEngraver.ScriptSkylines`（**置いた pair を返す綴り**）が
...
>
> ⚠️⚠️ ★★★ **射程を測ったので、いま着手する理由は無い**（ディスク上の **1026 冊**を collect して数えた）:
...
> `audit/lp-regression` 81 冊＝18。⇒ **この機械の実ファイルで影響を受ける本は 1 冊も無い。**
> ⚠️ **ただしこれはこのコーパスについての主張**。**弓記号やアーティキュレーションを敷き詰めた
> 弦・管のパートは普通に届く**——目安は **1000 script で約 40 ms・4000 で約 130 ms**。
> ⇒ ★ **着手条件**: **script 500 個以上の実ファイルが 1 冊でも現れたら**。
