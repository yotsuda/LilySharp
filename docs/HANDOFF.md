...
最終更新 第299セッション＝**§2 U8 の ⒝ 付点を閉じた（`e735fa88`）＝装飾音符の付点は*ホストの列を要らなかった***。**入り方は第298 と同じ**——**ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った**ので、**§1 の「次の一手＝残りの U8」をそのまま取った**（**第298 が委任を断った理由は「次便のスロットは空いている」で、その次便がこれ**）。**commit 2 本**（本体 1 ＋ docs 1）。**掃きは 1 回・1724 冊・両側 Release・`svg --combined`＝MOVED 6／`check` の差 6＝*同じ 6 冊*／レンダー失敗 2＝両 exe で同じ＝本便のものではない。** **台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188／追跡 `.lys` 581／snapshot 231 はすべて不動**（**snapshot は書き換えも追加も削除も 0 枚**）。

⚠️⚠️ ★★★ **⑴ 起票の仕分けが 1 段ずれていた、そして*ずれ方*が第298 自身の規則の中に在った**。**第298 は「carry と drop の境目は族ではなく*列を要るか*」と書いて ⒝ 付点を drop 側に置いた**が、**付点が要るのは*ホスト音符の列*ではなく*装飾音符自身の列***——**符頭の右に、装飾音符自身のフォントでぶら下がるだけ**。**「装飾音符は measure の item ではない」は 1 つも邪魔をしていない。** ⇒ ★★★ **正しい規則を書いたのに、その規則の中の名詞（「列」）を 1 つしか数えなかった**——**族で仕分けるのをやめた次の失敗は、*正しい軸で仕分けたつもりで値域を狭く取る*こと。** ⇒ ★★ **判定法**: **「X を要る/要らない」で仕分けたら、**X **が何通りあるかを 1 行書く。書けないなら仕分けはまだ二分されていない。**

⚠️⚠️⚠️ ★★★★ **⑵ 付点の X は定数ではなく*移植*で、それを示す対は音高 1 段で切り替わる**。**LP の規則は `Dot_column::calc_positioning_done`＝support の箱（符頭は名目 `(-1.1 . 1.1)`・茎は 7 position・旗は*実インク*）の右スカイラインを、符頭のインク右で床を張り、*各付点の段で*読み、付点幅 1 つを足す**（`lily/dot-column.cc:229-232` → `dot-configuration.cc:129-137` → `dot-formatting-problem.cc:23-28`、padding は `scm/output-lib.scm:692-704`）。**LP 2.26.0 実測**: **`grace { e'8. }` は空きに乗るので付点はその段に留まり 1.226600**（＝0.917939 ＋ 0.308661）／**`grace { d'8. }` は線に乗るので付点が 1 段持ち上がり、そこに*旗*が居て 1.747300**（＝1.438627 ＋ 同じ 0.308661）。**差 0.520688 ＝ 旗右 − 符頭右で 6 桁一致。** ★ **全サイズの旗つき音符は線でも空きでも 1.754200 で不動**（茎が長く旗の最下端がどの付点の段にも届かない）。⇒ ★★★ **平らな「符頭＋付点」はこの 2 つの片方を必ず外す**——**Y の gate は飾りではなく、2 つの場合を分ける唯一のもの。** ⇒ ★★ **そしてこれは*予測してから*測った**（「線に乗った本だけ押されるはず」）——**当たったので、規則の読みが正しいことまで一緒に確かめられた。**

⚠️⚠️ ★★★ **⑶ 予測が外れた 1 件＝「付点は列を広げる」。外れが規則の*形*を教えた**。**`Dots` grob は separation の箱を宣言している**（`scm/define-grobs.scm:1272-1279`・`extra-spacing-width (0.0 . 0.2)`）**ので最初に reach へ足した**——**LP は `grace { d'8. }` と `grace { d'8 }` を*同じ版面幅*で彫る**（付点 1 個が増えるだけ・他の座標は 1 つも動かない）。⇒ ★★★ **理由は「LP の separation は*スカイライン*で、この reach は*平らなスカラー*」**——**付点の箱は 1 空きぶんの高さで、主音符の頭が居ない段に在る**ので facing skyline はそこで出会わない。**平らな項を足すと、稀な場合を買うために普通の場合を壊す。** ⇒ ★★ **だから「LP がその grob を separation に入れている」は「幅が要る」の証拠ではない**——**入れているのはスカイラインで、当たるかは*段*が決める。** ⚠️ **`departs from` で残した**（`SpacingRules.Grace.cs`。**戻すには reach をスカイラインにする＝別の島**）。

★★ **⑷ spring は付点を読む、そしてそれを言う本が在る**。**LP は `grace { d'8. e'16 }` の 2 列間を 2.915900、`grace { d'8 e'16 }` を 2.448000 に置く**（**差 0.467900 ＝ `spacing-increment × log2(3/2)` ＝ `0.8 × 0.584963` ＝ 0.467970＝LP が印字する桁で一致**）。⚠️ **Lily# はこの gap を*両方の本で* LP より 0.246 短く描く**＝**混在 run の古い乖離**（台帳 `grace.column.*` の島）。⇒ ★★ **網は*絶対値ではなく差*を主張する**——**絶対値を pin すると、その古い残差をこの網の中に埋め込んで、直したときに赤くしてしまう。**

⚠️⚠️ ★★★ **⑸ 毒 8 本のうち 1 本が 0 赤＝網の穴を毒が見つけた**。**⒣「spring が音価を読む（モーメントではなく）」は 0 赤**——**私は LP の式どおりに直したのに、それを見る網が 1 本も無かった**。⇒ **⑷ の本を測って `GraceSpacingReadsTheDottedMoment` を足したら 1 赤になった。** ⇒ ★★★ **毒は「実装が効いているか」だけでなく「*網が在るか*」を測る**——**0 赤は合格ではなく、その変更が無検証で出荷されようとしている報せ。** ★ **同じ形をもう 1 つ塞いだ**: **⒜（収集が付点を読まない）と ⒞（renderer が描かない）が*同じ 2 本*を赤くしていた**ので、**モデルを見る `TheCollectorCarriesTheDot` を足して分けた。**

★★ **⑹ 分けられなかった 1 対は、分けられない*理由*を書いて残した**。**⒟（スカイラインを丸ごと殺す）と ⒠（旗だけ support から外す）は同じ assert を赤くする**——**装飾音符の茎は上向き固定で、その右端は必ず符頭のインク右より左**なので、**binding しうる support は旗ただ 1 つ**。⇒ **`GraceNoteEngraver.Dots` に「茎は忠実だが不活性」と書いた**（**LP が入れているので入れる／観測できるようになるのは grace の符頭が音価ごとに描かれる日**）。⇒ ★★ **「同じ assert が落ちる 2 毒」は §5.4 の「同じ網でも assert が違えば別」の*裏返し*で、こちらは*本当に同じ*。区別できないなら、できない理由をコードに書く。**

★★ **⑺ 番人が予告どおり働いた 1 例目**。**`GraceBodyValidatorTests.EverythingReported_IsAbsentFromThePage` の付点の行が赤くなり、その行を消すのが閉じる作業の一部だった**（第298 が「穴を塞いだ人はその行が赤くなって迎えられる」と書いたとおり）。⇒ ★ **「閉じたら赤くなる網」は書いておくと本当に効く**——**設計の意図が、次の人の手元で*機械から*届く。**

★ **⑻ 掃きは仮定しなかった**。**MOVED 6 と `check` 差 6 は*同じ 6 冊***（第298 ⑿ の「母集団を推測するな」の直後なので、**group してから書いた**）。**6 冊とも付点つき装飾音符を書くプローブ**（`p298/ab` 3 冊 ＋ 本便の `p299/ab` 3 冊）で、**追跡 fixture 0・snapshot 0・読み手の 326 冊 0**。**`check` の差はどれも「付点の警告が消えた」だけ。** **レンダー失敗 2 は両 exe で同じ**（`scratch/p216/pins/chords-*.lys`＝**§2 ⒨ の既知**）。
...
終了時: **commit 4 本**（**本体 `e735fa88` ／ §1・§2 `f95c3b92` ／ ubuntu 脚 `39d8f94d` ／ この数の閉じ直し 1 本**）＝**未 push 17**・木 0・未追跡 0。**suite 6643 合格 / 0 失敗 / 4 skip / 合計 6647**（開始 6641 から **+6**＝**本便の網 6 ケース**）。Core 0 エラー 0 警告。**追跡 `.lys` 581 冊・snapshot 231 枚**（**書き換え 0・追加 0・削除 0**）・**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2／`OPEN:` 0＝すべて不動。**
...
★★ **WSL ubuntu Release も同じ 6643 / 0 / 4 / 6647**（`f95c3b92` の木＝**docs をcommit した*あと*に回した**。§1 と §2 を書き換えたら全数をもう一度回す——第298 ⑽ が ubuntu 脚 1 本でそれを教えた棚）。⚠️ **CI は `11f48202` までしか走っておらず、未 push 17 本は未実行。** ⚠️ ★ **この 3 行は 1 度嘘をついた**——**commit 2 本・未 push 15 と書いたのは、docs を commit する*前*の見込みを書いたから**。**第210 と第298 が踏んだのと同じ場所で、同じ直し方（commit の*あと*に数え直す）**。

## 以下は第298セッションの経緯

最終更新 第298セッション＝**§2 U8 を半分閉じた（`49886b42` ＋ `c3451bda`）＝`grace { }` の本体は歩かれていなかった**。**入り方は第295〜第297 と違い、ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った**——**第297 の §1 が「次の一手＝ユーザーに訊く」で終わっていたが、その第297 自身が委任を断る理由として「U8 は次便が自力で開ける唯一の項＝スロットは空いている」と書いていた**ので、**訊き直さず U8 を取った**（⑴）。**commit 6 本**（本体 2 ＋ docs 4）。**掃きは 2 回・両側 Release**——**1 回目は既定（1705 冊・MOVED 5）で、⑺ の理由でこの便の欠陥を*見なかった*／2 回目が `svg --combined`（1713 冊・MOVED 10＝9 冊はプローブ、1 冊は読み手の本／`check` の差 45／レンダー失敗 2＝両 exe で同じ＝本便のものではない）。** **引くべきは 2 回目**（1 回目は「既定の掃きは盲」の証拠としてだけ残す）。 **台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188／追跡 `.lys` 581／snapshot 231 はすべて不動**（**snapshot は書き換えも追加も削除も 0 枚**）。

⚠️⚠️ ★★★ **⑵ 起票より病気が広かった、そして測ってから着手したから判った**（§5.0 の「引き継がれた処方箋は着手前に診断を 1 回測る」）。**起票の病名は「装飾音符は注釈を運ばない」**だが、**engine に訊いたら本体が*歩かれていない***: **`CollectGraceNotes` が読むのは裸の `NoteSyntax` だけ、その中の音高と音価の*値*だけ**。**本体は `ParseMusicBlock` で普通に構文解析される**ので、**和音・休符・tuplet・phrase 参照は列を 1 本も作らず（本体がそれだけなら装飾音符ごと消える）、付点も、本体の中のスラー・梁・タイも落ちる**。**測り方は「その綴りと対照を描いて `data-pos` を伏せてバイト比較」**（`scratch/p298/ab`）。⇒ ★★ **「注釈の族」で病名を書くと、族の外に在る半分が見えない。**

⚠️⚠️⚠️ ★★★★ **⑶ 起票の射程「実コーパス 0/326」は外れており、外れると起票自身が予告していた**。**起票は「これは engine ではなく*文字列の走査*なので、閉じる便は engine に訊き直すこと」と書いていた**——**訊き直したら読み手の `scratch\ベースタブLy\Real Gone.lys` が `grace { a,16\2 }` を 2 か所書いており、両方とも弦番号を捨てて resolver の選んだ弦に描かれていた。** ⇒ ★★★ **走査が見落とした理由は「`@` を探していたから」で、`\2` は `@` を持たない**——**射程を「その族の綴り方」で数えると、族の定義が間違っているときだけ 0 が出る**（**そして族の定義が間違っていることこそ ⑵ の中身**）。⇒ ★★ **第297 はこの 0 を「読んだ数」として §1 に書き、それを根拠に委任を断った**。**数は読まれていたが、読んだ計器が間違っていた。** ⇒ ★ **`LYS4020` は書いた初日に読み手の本を 1 冊名指した。**

★★★ **⑷ carry と drop の境目は「族」ではなく*列を要るか***。**落ちる族は全部ホスト音符の `itemIndex` を要る**が、**装飾音符は measure の item ではないので渡せない**。**要らないものが 2 つ在る**: **⒜ `@mark`＝grob が SCORE のもの**（`ly/engraver-init.ly:729 \name Score`, `:764` が `Mark_engraver` を consist する。**LP 2.26.0 実測 `\grace { d'8^\markup{x} \mark "P" }` は P も x も描く**——`scratch/p298/lpmark.svg`。**復元したのは P、まだ落ちているのが x**）／**⒝ 弦番号 `\N`＝そもそも grob ではない**（**notation staff では `c'4\2` と `c'4` がバイト同一**＝**描かない**。**`Tunings.CalculateFret` の入力**なので `GraceNoteInfo.StringNumber` で運ぶ）。

★★ **⑸ 狭めは 1 か所に書いて 2 人が読む**（`Semantics.GraceBodySupport`。collector は描くものを取り、validator は残りを報告する）。⚠️ **`CarriedStringNumber` はわざわざ `NeedsNoColumn` を通す**——**ノード型を直接読めば短いが、それだと毒 1 つで両側が同時に赤くならない**。**通しておけば「弦を無視する」と「弦を drop と報告する」が 1 回の編集で同時に起きる**＝**それが link の*測定*。**

★★★ **⑹ 毒 8 本・赤くなる集合は全部別**（§5.4）: **⒜ `NeedsNoColumn` を mark に false → 4**／**⒝ collector の腕を呼ばない → 3**／**⒞ 位置 de-dupe を外す → 1**／**⒟ `engravesNothing` を false → 1**／**⒠ carry を skip しない → 6**／**⒡ `CalculateFret` に 0 を渡す → 1**／**⒢ `NeedsNoColumn` を `\N` に false → 2**／**⒣ 付点を報告しない → 1。** ⚠️ ★★ **⒜ と ⒝ は同じ網 3 本が赤くなるが assert が違う**（§1 第297 ⑹ と同じ形）: **⒜ は `Assert.Empty(Warnings)`＝validator の主張／⒝ は `Assert.Contains(">P</text>")`＝インク**。**どの assert が落ちたかまで読んで初めて 2 本は別。**

⚠️⚠️ ★★★ **⑺ 既定の掃きはこの便の欠陥を見なかった＝RULES §5.0 に 3 例目を足した**。**`lysc svg` は既定で*最初の* score ブロックしか描く**——**`Real Gone.lys` は `score main { staff bassline }` で開き、tab は 2 つ目と 3 つ目**なので、**既定の掃き 1705 冊はその本を「SAME」と答えた**。**`--combined` で取り直したら MOVED**（`scratch/p298/sweep298.ps1`）。⇒ ★★ **「届きうる出力を名前で列挙する」は*出力の種類*で止めると足りない。1 冊が複数の版面を持つなら、その全部を描かせる。** ⚠️ **`--combined` は既定より脆い**（構文エラーを持つ本で例外＝**掃きの `no-output 2` の正体**。**base でも同じ**）——**§2 ⒨ に起票した。**

★ **⑻ 動いたインクは*確かめた*、仮定しなかった**: **MOVED 10 のうち 9 冊は `scratch\p293` と `scratch\p298` のプローブ**（**`@mark` を書く本と、この便が建てたタブ本**）、**1 冊が `Real Gone.lys`**。**その 1 冊の動き方も見た**——**tab の弦線 y=76.30 と y=134.38 の「穴」（フレット数字の抜き）が head では消えている**＝**2 つの装飾音符が resolver の選んだ弦から離れた**（**本文の `grace { a,16\2 }` 2 か所とちょうど一致**）。

★ **⑼ perf は測った**（§7.9）。**⒜ 診断経路**（`SemanticValidation.Run` は LSP の打鍵ごとに走る）: **読み手の最大の本で `Run` 14.222 ms・`GraceBodyValidator` 単体 0.0467 ms＝0.33%**（**1 プロセス内・20 回×5 バッチの床**）。⚠️ **プロセス越しの `lysc check` A/B では起動 336 ms に埋もれて分離できなかった**ので、**in-process で測り直した**。**歩きは `KindSites(SyntaxKind.GraceExpression)`＝緑の walk で、grace を書かない本は red を 1 つも作らない**（`c3451bda`）。**⒝ 描画経路**: **`alloc` 3 run の床（base ／ head）＝perf-plain1k 471.2 / 471.2・打鍵 26.3 / 26.3 で厳密に不動**、**perf-grace200 53.2 / 53.0・perf-slurgrace300 169.5 / 171.3 はどちらも自分の run 間のばらつきの中**（**`GraceNoteInfo` が太ったのは nullable int 1 つ＝8 バイト×300 装飾音符＝2.4 KB 対 171 MB**）。

⚠️⚠️ ★★★ **⑿ 「`check` の差 45 は全部 grace の本」は*外れ*で、外れたのは*読まずに要約したから***（**委任の返事のついでに読み直して見つけた**）。**内訳を code で割ると 38 が `LYS4020` だけ・2 が `LYS4020`＋`LYS4019`・**★★ **5 が `LYS4019` だけ**。**その 5 のうち 3 は「base が警告・head は `No errors found`」＝*grace の `@mark` が描かれるようになった本*で、これは変更が効いている証拠**。**残る 2（`scratch\p293\s1.lys`・`s2.lys`）は grace を 1 つも書かない本で、位置も code も同じ、*文言だけ*が違う**——**`LYS4019` から「or a grace note」の節を外したから。** ⇒ ★★★ **診断の*文言*を直すと、その診断を出す本*全部*で `check` が動く**。**その母集団は変更の主題と無関係**（ここでは「未描画の印を持つ本」＝2 冊で済んだが、**族の大きい診断なら数百冊が動きうる**）。⇒ ★★ **RULES §5.0 ⒜「届きうる出力を名前で列挙する」の*一段下*＝「触った診断の文言を読む本」も出力のうち。** ⇒ ★ **そして掃きの要約行は、group してから書く**（**45 を「全部 grace の本」と書いたのは、内訳を出さずに母集団の名前を推測したから**——§1 冒頭の ⑾ と同じ失敗の 2 例目、**同じ便で**）。

⇒ ★★★ **次の一手＝残りの U8＝「列を要る」側**（§2 U8 の ▶）。**直し方は本体を*普通の walker で歩く*で、`ProcessCueRegion` が cue にしているのと同じ**——**LilyPond の grace も Voice context 1 つ**。**難所は住所で、`ArticulationEngraver` が `measure.Items[itemIndex]` から staff position・stem 向き・符頭幅・タイ・梁・スラーを全部引いている**ので、**`(measure, item)` を鍵にしている memo 4 軒まで波及する＝1 便では終わらない。** ⚠️ **並行に第 2 の script engraver を建てないこと**（§5.2.1②）。★ **番人は入っている**: `GraceBodyValidatorTests.EverythingReported_IsAbsentFromThePage` は**「LYS4020 が鳴る」と「対照とページ同一」を同時に述べる theory** なので、**穴を 1 つ塞いだ人はその行が赤くなって迎えられる**（**赤くするのが正しい向き**）。

★ **開始時裏取り**: HEAD `74f8ae3e`・**未 push 6**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6613 / 0 / 4 / 6617**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2・`OPEN:` 0**・追跡 `.lys` **581 冊**・snapshot **231 枚**——**§1 の終了時の数と 1 つも違わなかった**（**10 個の数え方レシピは全部そのまま再現した。素朴に数えると `exact` 556／`OPEN:` 11／snapshot 232 が出る**）。
終了時: **commit 7 本**（**本体 `49886b42` ／ 緑歩き `c3451bda` ／ docs `243d83d8` ／ ⑽ の書庫移し `e85dab19` ／ 閉じ直し `450d6cba` ／ 掃きの回数の直し `f3d1960d` ／ ⑿ の内訳 1 本**）＝**未 push 13**・木 0・未追跡 0。
⚠️⚠️ ★★★ **⒀ ユーザーが委任を出した**（「次便は、このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）。**答えは「着手しない」**——**第297 が U8 に対して出したのと同じ答えで、理由も同じ形が*一段内側で*成り立っている**: **⒜ 温まっているのは collector と validator の側で、残りの U8 の難所は `ArticulationEngraver` と `(measure, item)` を鍵にする memo 4 軒＝本便が 1 行も開いていない場所**／**⒝ 計器は全部ディスクに残り §1・§2・RULES から名指されている**（`scratch/p298/ab`・`poison2.py`・`sweep298.ps1`。**base exe は worktree 10 秒**）＝**非対称ではない**／**⒞ 次便のスロットは空いている**——**§1 の「次の一手」が残りの U8 を筆頭に置き、競合する ▶ は ⒤ と ⒣（本文自身が「着手はユーザー決定から」）と ⒨（壊れたプローブ 2 冊）だけ**。⇒ ★★★ **この規則で取るべきは「*理由ではなくスロット不足で*飛ばされた項」で、残りの U8 は*理由*で飛ばされている——しかもその理由を私は本便の §2 U8 に書いたばかり。** ⇒ ★★ **代わりに取ったのが規則の名指す「派生ではなく*読まれた*数に直す安い測定」**＝**⑿**（**そして読んだら、自分の主張が 1 つ外れていた**）。
⚠️ ★★ **⑾ 閉じ直しは 2 度要った、そして 2 度目は*数え違いではなく書き落とし***——**§1 の冒頭が「掃きは 1 回」と書いていたのに ⑺ が「既定の掃き 1705 冊」と書いており、同じ §1 の中で矛盾していた**（**実際は 2 回**）。⇒ ★★★ **要約行はいちばん最後に書く**。**冒頭の 1 行は本文より先に書かれ、本文が育っても直されない**——**第294 の「同じ節が 1 つの量について 2 つのことを言っていたら通しで読む」の*自分版*。****suite 6637 合格 / 0 失敗 / 4 skip / 合計 6641**（開始 6610 ではなく **6617** から **+24**＝**本便の網 24 ケース**）。Core 0 エラー 0 警告。**追跡 `.lys` 581 冊・snapshot 231 枚**（**書き換え 0・追加 0・削除 0**）・**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2／`OPEN:` 0＝すべて不動。**
⚠️⚠️ ★★★ **⑽ WSL の ubuntu 脚が 1 本赤くした＝`HandoffArchiveContinuityTests.TheArchiveResumesWhereTheHandoffStops`**——**第296 の経緯を §1 から落としたまま `HANDOFF-ARCHIVE.md` へ移していなかった**（終了時チェックリスト 3.5）。**移して緑**（43 行・逐語・書庫の先頭）。⇒ ★★★ **Windows が緑だったのは*嘘ではなく古かった*から**——**最後の全数を回したのは §1 を書き換える*前*で、この網が見るのは §1 そのもの**。⇒ ★★ **§1 と §2 を書き換えたら、全数をもう一度回す**。**docs は「テストの外」ではない**（この repo は §1 の形・`APPROXIMATIONS.md` の行番号・`docs/*.md` の例をテストで縛っている）。★ **ubuntu 脚が先に鳴ったのは偶然**——**Windows で回し直しても同じ 1 本が鳴る**（`docs/HANDOFF-ARCHIVE.md` の直しだけ stash して回して確かめた。**残り 2 本は緑のまま**＝**seam の網だけが落ちる**）。
★★ **WSL ubuntu Release も同じ 6637 / 0 / 4 / 6641**（`e85dab19` の木）。⚠️ **CI は `11f48202` までしか走っておらず、第297 の 3 本と本便の 5 本＝未 push 11 本は未実行。**
⚠️ **`C:\MyProj\lys-base298` の worktree はこの便の最後に消した**（`git worktree remove`）。**次便が同じ名前で建て直してよい。**
⚠️ **`docs/APPROXIMATIONS.md` が 1 行動いた**——**中身ではなく行番号**（`SharedRenderer.GraceNotes.cs` に 4 行入ったため）。**`LILYSHARP_UPDATE_DOCS=1` で再生成した**（編集ではなく再生成が正しい直し方だと、そのテスト自身が言っている）。
⚠️ **base 側 exe は `git worktree add --detach C:\MyProj\lys-base298 HEAD` で建てた**——**`git stash` を使わなかったのは、`scratch\` が ignore なので `-u` の巻き添えが読み手の 326 冊に届きうるから**。**掃きが終わったら worktree は消すこと。**
...
- **U8. ▶ 3 分の 2 まで閉じた（第298 ＋ 第299）＝`grace { }` の本体は*歩かれない*。起票の「注釈を運ばない」より広かった**
  （**第293 起票・第298 が engine に訊き直して起票を訂正し可聴化と 2 つの carry を入れ、第299 が付点を移植した**）。
  ⚠️⚠️ ★★★ **第299 の骨＝「carry と drop の境目は*列を要るか*」の*列*が 2 種類あった**。
  **第298 はそう書いて ⒝ 付点を drop 側に置いた**が、**付点が要るのは*ホスト音符の列*ではなく
  *装飾音符自身の列***——**符頭の右に、装飾音符自身のフォントでぶら下がるだけ**なので、
  **「装飾音符は measure の item ではない」は 1 つも邪魔をしていなかった**。
  ⇒ ★★★ **族で仕分けたのではなく「どの列を要るか」で仕分けたつもりで、*列*を 1 つしか
  数えていなかった**。**⒜⒞⒟ はホストの列を要り、⒝ は自分の列で足りる。**
  ...
  **⒞ 本体の中のスラー・梁・タイは落ちる**／**⒟ `@staccato`・`@text`・`@f`・`@finger`・
  ...
  ⇒ ★★★ **だから「注釈の族」ではなく*本体を歩いていない*が正しい病名**。**起票の
  「`CollectArticulations` を通らない」は当たっていたが、通らないのは articulation だけではない。**
  ...
  **`@mark`**（**LilyPond は `Mark_engraver` を SCORE context に consist する**＝
  `ly/engraver-init.ly:729 \name Score`, `:764`。**grob は最初から音符の Voice のものではない**ので、
  **grace が別 Voice でも間に立てない**。**LP 2.26.0 実測: `\grace { d'8^\markup{x} \mark "P" }` は
  P も x も描く**——`scratch/p298/lpmark.svg`）と、**弦番号 `\N`**（**そもそも grob ではない**——
  **notation staff では `c'4\2` と `c'4` がバイト同一**で、**`Tunings.CalculateFret` の*入力*
  でしかない**）。⇒ ★★★ **carry と drop の境目は「族」ではなく*列を要るか***。

  ⚠️⚠️ ★★★ **⑶ 起票の射程「実コーパス 0/326」は*外れていた*、そして外し方が起票自身の警告どおり**
  ——**起票は「これは engine ではなく文字列の走査なので、閉じる便は engine に訊き直すこと」と
  書いており、訊き直したら当たった**: **ユーザーの `scratch\ベースタブLy\Real Gone.lys` が
  `grace { a,16\2 }` を 2 か所書いており、両方とも弦番号を捨てて resolver の選んだ弦に描かれていた。**
  ⇒ ★★ **文字列の走査が見落とした理由は「`@` を探していたから」**——**`\2` は `@` を持たない。**
  ⇒ ★★★ **射程を「その族の書き方」で数えると、族の定義が間違っているときに 0 が出る。**
  **engine に訊けば族の定義ごと直る**（**LYS4020 は書いた初日に読み手の本を 1 冊名指した**）。

  ★★ **第299 が閉じたのは ⒝ 付点**（`e735fa88`・`Svg/Layout/DotColumn.cs`）。
  **`GraceNoteInfo.Dots` は `BaseDuration` の*隣*に置く**——**音価は符頭・旗・梁の本数を決めるので、
  付点を分数に畳むと `grace { d'8. }` が 16 分になって梁が 2 本になる**（`NoteItem` が
  `Dots` を分けているのと同じ理由）。**分数側は `GraceNoteInfo.Length`＝*モーメント*で、
  spacing がそれを読む**（LP の grace spring は `delta_t.grace_part_`）。
  ⚠️⚠️ ★★★ **付点の X は定数ではなく*移植*で、それを示す対は音高 1 段で切り替わる**
  （`DotColumn` ＝ LP の dot 列規則を 1 か所に書いたもの＝**support の箱の右スカイラインを
  *各付点の段で*読み、符頭のインク右で床を張り、付点幅 1 つを足す**）。**LP 2.26.0 実測**:
  **`grace { e'8. }` は空きに乗るので付点はその段に留まり 1.226600**（＝0.917939 ＋ 0.308661）／
  **`grace { d'8. }` は線に乗るので付点が 1 段持ち上がり、そこに*旗*が居て 1.747300**
  （＝1.438627 ＋ 同じ 0.308661。**差 0.520688 ＝ 旗右 − 符頭右**）。
  ★ **全サイズの旗つき音符は*どちらでも押されない***（実測 1.754200 で不動）——**茎が長く、
  旗の最下端がどの付点の段にも届かない**。⇒ **平らな「符頭＋付点」はこの 2 つの片方を必ず外す。**
  ⚠️⚠️ ★★★ **付点は幅を 1 も予約しない、そしてそれは*測って*判った**——**`Dots` grob は
  separation の箱を宣言している**（`scm/define-grobs.scm:1272-1279`）ので**最初は reach に足した**が、
  **LP は `grace { d'8. }` と `grace { d'8 }` を*同じ版面幅*で彫る**（付点 1 個が増えるだけ・
  他の座標は 1 つも動かない）。⇒ ★★★ **LP の separation は*スカイライン*で、この reach は
  *平らなスカラー***。**付点の箱は 1 空きぶんの高さで、主音符の頭が居ない段に在る**ので当たらない。
  **平らな項を足すと、稀な場合を買うために普通の場合を壊す**（`SpacingRules.Grace.cs` に
  `departs from` で残した）。
  ⇒ ★★ **一方 spring は付点を読む**: LP は `grace { d'8. e'16 }` の 2 列間を **2.915900**、
  `grace { d'8 e'16 }` を **2.448000** に置く（**差 0.467900 ＝ `0.8 × log2(3/2)` ＝ 0.467970**）。
  ⚠️ **Lily# はこの gap を両方の本で LP より 0.246 短く描く**——**混在 run の古い乖離**
  （台帳 `grace.column.*` の島）。**網は*差*を主張しているので、この残差を隠さない。**

  ▶ **残っているのは「*ホストの*列を要る」側＝⒜⒞⒟**（**⒝ 付点は閉じた・`@mark` と `\N` は第298**）。
  ⚠️ **直し方は「装飾音符に住所を足す」ではなく*本体を普通の walker で歩く***——
  **`ProcessCueRegion` が cue 領域にしているのと同じ**（`MeasureCollector.MusicWalk.cs` の
  「A cue is a REGION, walked with the ordinary walker so that everything a voice can hold
  ...
  **穴を 1 つ塞いだ人はその行が赤くなって迎えられる**（**赤くなるのが正しい向き**——
  **その行を消すのが閉じる作業の一部**）。★★ **第299 で実際にそう働いた**：**付点の行が赤くなり、
  その行を消すのが閉じる作業の一部だった**（**番人が予告どおり機能した 1 例目**）。
