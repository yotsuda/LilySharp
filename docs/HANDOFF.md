...
最終更新 第372セッション（2026-09-12）＝**入り方は第298〜第371 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った。道具の指示は無し＝記憶どおり ripple ではなく pwsh MCP で作業した）。**第371 が「次の一手」として残した `staff→chord` の −0.851723138 は、engine ではなく*計器*の欠陥だった**——残差そのものが存在しなかった。**そして第 2 便でユーザーが `chordQualities` の既定を LP の語彙に決め、和音行の島が 9 桁で閉じた**。**骨は 12**:

★★★ **⑴ 残差の正体＝計器が「上付き run の baseline」を読んでいた**。上付き移植で**1 記号は 2 つの `<text>` になった**のに、`RenderedGeometry.ChordSymbols` は **run** を返したままで、`ChordBaselineBelowStaff` は「五線より下で*最小*の Y」＝**上付き run の baseline** を拾う。ずれは **1.189207115 ＝ magstep(1.5) ＝ `\super` の持ち上げそのもの**。⇒ **描かれている baseline は五線中線の 5.997137399 下**で、台帳が記録していた **4.807930284 は上付き run**。**最初の証拠は絵**（`lysc svg` の `scratch/p373/chl2.svg`: 本体 `y=17.69`・上付き `y=16.50`・五線中線 `11.69`＝**17.69 − 11.69 = 6.00 で、4.81 はどこにも無い**）。**engine を 1 行も読まずに、描いた絵と台帳の数が合わないことだけで分かった**。

★★★ **⑵ 計器を直した（commit `a6d31bc6`）**: **`DrawnText` が「描かれたときの source scope」を持つ**（`IDocumentContext.Source`＝engine 自身の記号の同一性・click 先と同じもの。記録側は `NullScope` で捨てていた）。**`ChordSymbols` はその scope で run を束ねて記号ごとに 1 つ返す**（root の anchor と baseline・run の文字列を連結）／生の list は **`ChordSymbolRuns`**。**番人 `AChordSymbolIsOneReading_ThoughItIsDrawnAsSeveralRuns`**＝⒜ run が 4・記号が 2（**陽性対照**: 上付きが在る絵であることを先に言う）／⒝ 各記号の Y は group の最大で、混同された run はちょうど 1 raise 上／⒞ **両側から読んだ和が五線間距離に一致**（`ChordBaselineBelowStaff(0) + ChordBaselineAboveStaff(1) == StaffGap()`＝下から読む側は Max なので毒が効かない）。**毒 2 種で赤を確認**（run を返す／raise 側を選ぶ）。

★★★ **⑶ 台帳 4 点が動いた（全部 LP に近づいた）**: `lyrics.chord-lyric-run.staff-to-chord` と `lyrics.chord-run.staff-to-chord` **−0.851723138 → +0.337483977**／`lyrics.chord-lyric-run.chord-to-lyric` **+1.189052065 → −0.000155050**（＝歌詞の face ビット。**この段は最初から合っていた**）／`lyrics.lyric-chord-run.lyric-to-chord` **−1.192896213 → −0.003689098**。⚠️ **CHL1 の 3 段が部屋にぴったり足し合うようになった**: 0.337483977 − 0.000155050 + 0 = **0.337328927 ＝記録済みの room 残差**（9 桁）。**この和が、直した 4 つの読みが整合している証拠**。

★★★ **⑷ 残る +0.337483977 の正体＝*語彙の既定*（⇒ ユーザー決定 1 件）**: **両エンジンとも和音行のインク上端を五線 refpoint の*ちょうど 3.500000000 下*に置く**（符尾の先 3.0 ＋ padding 0.5）＝**機構は一致**。LP 側は今便の計器 `scratch/p373/skyline-dump.ly` が直接言う（**staff1 の下向き skyline と chords の上向きが x=11.633168293＝`Dmaj7` の箱の右端＝2 つ目の符尾の所で触れ、距離 5.159653422 ＝ 段 − padding 0.5**）。⇒ **差は記号そのもの**: **LP の `Dmaj7` は (0 . 2.159653422)＝三角**、**Lily# は (0 . 2.497137399)＝`maj7` の字**。⚠️ **第371 ⑶ の「LP の dump (0.0 . 2.5008) と一致」は*別の記号*と突き合わせていた**——この本で 2.5008 なのは **`Em7`**（数字 7 が上付きの背）で、`Dmaj7` は 2.1597。**`layout { chordQualities symbols }` と書けば Lily# も三角を描き、五線→和音は 5.66＝LP の 5.659653422**（実測・`scratch/p373/chl2-symbols.lys`）。⇒ **問い: 既定を LP の絵（三角）に寄せるか、`words` を既定のまま残すか。** 台帳はどちらでも良いように、**新しい対（下 ⑹）には maj7 を使っていない**。

★★★ **⑸ engine 側の第 2 の島も閉じた＝*上付きを知らない測定 10 site*（commit `d2e42b39`）**: `SymbolInk`／`SymbolInkWidth` に **`SuperFrom` を渡さない**呼び手が 10 箇所＝**移植前の綴り**（上付き群を原寸でベースラインに置いた箱＝`Dmaj7` で **1.71 広く・上端が 0.59 低い**）。**家を 1 つにした**＝`SymbolInk(fonts, ChordNameItem)` と `(fonts, ChordNameLayout)` の overload を足し、**対 (text, superFrom) を分解できないようにした**（`DisplaySymbol` が「何を刷るか」の唯一の答えのまま）。**触った pass**: 頁の注釈 extent・X を見た system 間距離・mark 族の clearance・outside-staff stack・水平ばね。**snapshot 3 枚が動いた**（`test/lead-sheet`・`test/chordnames`・`test/chordrow-rit-second-system`）＝**system 2 が 0.59 下がり頁が 0.59 伸びた**＝**描いた記号の上端を頁が予約していなかった**ぶん（記号の X は 1 つも動かない）。⚠️ **`page.inline-chord.gap-first` の平箱（縦 1.9/0.3）はそのまま**——幅だけ直した。あの箱の縦を `SymbolInk` に替える取引は remark が書くとおり**ユーザーのもの**。⚠️ §7.5 の数え: **Core `+` 57 行／`LILYPOND-REF` 0／`LILYSHARP-OWN` 0**——**新しい量も定数も式も足していない**ので 0 が正しい（overload は `DisplaySymbol` を通して既存の `SymbolInk`／`SymbolInkWidth` に委譲するだけで、LP の住所は委譲先に既に在る）。**2 つ目の綴りを*消した*差分は REF を増やさない**。

★★★ **⑹ 再ベースを正当化する台帳点を*先に*作った（§5.2.1③）**: **新しい probe `audit/lp-geometry/probes/chord-superscript-row.ly`＋本 CSR/CSN**（1 変数＝**上付き run が在るか**だけ）。⚠️ **shipping の紙では両方 12.000000＝`system-system` の basic-distance で、対は何も言わなかった**（§5.0 trap 7）ので、**ideal と minimum を外した床の紙**（`ClefFloorPaper` の双子）で測り直した——**「正しい対」でも紙を間違えると無言になる**。**LP: CSR 10.775757854 / CSN 10.182224744（差 0.593533110 ＝上付き run のインクそのもの）**・**行の baseline は両本とも五線の 2.550000000 上**（＝動きうるのは上端だけ）。**Lily#: 10.747918349 / 10.182080958**＝**control は face ビットまで一致（−0.000143786）**・CSR の **−0.027839505 は raised digit の face**（Lily# の `Em7` は (0 . 2.473087762)・LP は (0 . 2.500823590)＝差 0.027735828 ＝**残差の全部**）。**台帳 +4 点**（床 2・数 2）。

★★★ **⑺ 数**: **開始時**（`Session-Check -Build -Test -Scratch p373`）: HEAD **`18efc322`**・**未 push 0**（⚠️ **ユーザーが第368〜第371 の 28 本を 7 本に畳んで push 済み**）・木 clean・未追跡 0・build 0 エラー／Core 0 警告・**full `scratch/p373/run1.trx` 7782 / 1 / 4 / 7787**。⚠️ **赤 1 は squash 孤児**（`DeadCitationsDoNotGrow` 651 対天井 635）＝**記憶 `reference_lilysharp_squash_orphans_doc_citations` の形そのもの**。**孤児 28・引用されている 17・うち a-f が 2 文字未満 1 ⇒ 予測 +16 が実測と一致**・**畳む前後の tree が同一オブジェクト**（reflog に残る旧 tip と新 tip。⚠️ **その旧 SHA をここに書いてはいけない**——書いた瞬間 dead が 1 増えて、今上げたばかりの天井を割る）⇒ **天井を 651 へ**（commit `9f6df2a3`・remarks に旧 SHA は書かない）。⚠️⚠️ **`origin/master` の CI は今この赤で落ちている**（run 34663615384）＝**この commit を push すれば緑**。**第 1 便の終了時: full `scratch/p373/run6.trx` 7788 / 0 / 4 / 7792**（+5＝番人 1・台帳 4）・**台帳 825 点／ss 非ゼロ 213／総和 27.896261602 → 24.517662441／exact 647**・commit 4 本（`9f6df2a3`＝天井／`a6d31bc6`＝計器と台帳 4 点／`d2e42b39`＝engine と snapshot 3 枚と新しい対＋HANDOFF）。**第 2 便（既定の変更）の終了時: full `scratch/p373/run10.trx` 7788 / 0 / 4 / 7792**（**本数は不変＝直したのは期待値だけ**・RunInfos は SKIP 4 だけ）・**台帳 825 点／OPEN 0／ss 非ゼロ 210／総和 23.216135909（便全体で −4.68 ss）／exact 650（+5）**・**snapshot 249（この便で計 6 枚再ベース・枚数は不変）／追跡 `.lys` 599**。⚠️ scratch は **`p373`**。⚠️ §7 3.5: 第370 を `-Archive 370` で ARCHIVE へ（済）。

⇒ ★★★ **⑻ 次の一手**: **⒜ ✅ 決定済み・第 2 便で出荷（下 ⑽）＝ユーザー「chordQualities の既定を symbols に変更して」**（上 ⑷。`words` のままなら +0.337483977 は「決めた差」として台帳に残り続けるところだった／`symbols` を既定にして **0.000000000**。**LP の既定は三角**）。**⒝ 残った face の島**＝`Em7` の上付き数字が Nimbus Sans では 0.0277 高い（上 ⑹）——これは「どの sans を同梱するか」の島で、`page.chord-row.staff-to-chord-baseline` と同じ決定。**⒞ 第369 ⑸⒝ の並びはそのまま**（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。**push はユーザー（未 push 5）**。

★★ **⑼ この便の教訓（§5 へ汎化する前の生の形）**: **⒜ 描き方を変えた便の*次*は、計器がまだ同じ量を読んでいるか確かめる**——1 記号が 2 run になった瞬間、「いちばん近い baseline」は別の物を指した。**engine を疑う前に、台帳の数と*絵*を突き合わせる**（今回は絵を 1 枚描いた時点で終わった）。**⒝ 対は紙で無言になる**（⑹）。**⒞ 「LP の dump と一致した」と書くときは、*どの記号の* dump かを x で確かめる**（⑷＝第371 は隣の記号と突き合わせていた）。

★★★ **⑽ 第 2 便（ユーザー指示「chordQualities の既定を symbols に変更して」＝上 ⑻⒜ の決定）＝既定を LP の語彙にした**: **`ChordQualityStyle` の並びを入れ替えて `Symbols` を enum の 0 に**した——**既定を「2 つ目の綴り」にしない唯一の形**で、`default(ChordSpelling)` がそのまま既定の絵になる。⚠️⚠️ **`ChordSpelling.Default` は 2 つの意味を兼ねていた**: 「書かなければこう描く」と「**パーサが読み戻せる綴り**」。**既定が動いた瞬間に別物**になるので後者を **`ChordSpelling.Canonical`** として分けた（読み手は **MusicXML の `<harmony>`** と**エディタ補完の detail**＝`C°` を渡すと綴りとして読めない側。§5.2.1② の「1 つの名前が 2 つの量」で、**今までは*たまたま*同じ値だったので両方正しかった**）。

★★★ **⑾ 台帳 6 点が動き、和音行の島が閉じた**: **`lyrics.chord-lyric-run.staff-to-chord` と `lyrics.chord-run.staff-to-chord` が +0.337483977 → 0.000000000**（LP の 5.659653422 と **9 桁一致**）・**`lyrics.chord-run.staff-to-staff`（部屋）も 0.000000000**（9.704653422）・**`lyrics.chord-lyric-run.staff-to-staff` は −0.000155050**＝歌詞の face だけ・**3 段が部屋にぴったり足し合う**（0 − 0.000155050 + 0）。⚠️ **CHL4 の 2 点は −0.0037 → −0.0277 で「LP から遠ざかった」**が、**悪化ではなく*束縛する記号が替わった***——`Dmaj7` が三角になって背が低くなり、**行のいちばん高いインクが `Em7` の上付き数字に移った**。その数字は Heros 2.473087762 対 Nimbus 2.500823590＝**0.027735828 の face**で、残差 −0.027738736 と 3e-6 まで一致する。**⑹ の CSR と同じ島・同じ決定（どの sans を同梱するか）**。⇒ **和音行に残る機構の項は 0 本**。

★★★ **⑿ 直した test 44 本の内訳（形は 3 つしか無い）**: **⒜ 「データの綴り」を訊く fixture 2 本を `Canonical` へ**（`ChordNamesTests.Plain`／`ChordNameTests.Chord`＝「書いた文字が読み戻るか」を訊く case は既定に従ってはいけない）／**⒝ *描かれた文字*を見る期待値**（`B°`・`C+`・`C△`・`Gø`・`F△/E`＝auto-name・roman 解決・小節割り当て・行の整列の key）。**書いた本文は `Cmaj7` のまま**で、変えたのは**ページが刷る文字列**だけ／**⒞ 既定を持つ番人 2 本を反転**（`ChordNames_Symbols_IsTheDefault_AndWordsSpellsThemOut`・`TheDegreesRowIsUnmovedByTheVocabulary_…`）。⚠️ **`minorChords lower` の番人は*もう一方の軸*を `words` に固定した**——読んでいるのは根音の大小と `m` の行方で、`°` には落とす `m` が無い（両方の軸を同時に動かす主張は `BothSwitchesTogether_…` が持つ）。⚠️ **第371 の番人 `AChordSymbolIsOneReading_…` の本は `Dmaj7` から `Em7 | A7` に替えた**＝**maj7 の上付きは text run ではなく多角形**なので、run を束ねる主張の材料にならない。

★★★ **⒀ snapshot 3 枚＋絵の確認**: `test/chordnames`（**`Gm7♭5` → `Gø`**）・`test/chordrow-rit-second-system`・`test/rit-across-systems`。**三角は描かれている**（丸キャップの線 12 本＝maj7 4 記号 × 3 辺）・**頁は 503.1 → 499.5 と縮む**（三角は `maj7` の字より背が低い＝予約も減る）・**記号の X は動かない**。⚠️ **再ベースを正当化する台帳キーは `lyrics.chord-run.staff-to-chord`（= 0.0）**＝絵が動いたのは「描く記号が LP のものになった」からで、その結果が LP の数そのものになったことを台帳が言う。
