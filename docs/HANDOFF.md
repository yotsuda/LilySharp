...
★★★ **⑴〜⑹ 起票の 2 脚（chords／lyrics）＝逐語は `HANDOFF-ARCHIVE.md`「第374 の chords/lyrics 脚（逐語）」**（20,000 字の門で出した）。**規則だけ**: **先に赤を作る**（記憶 `feedback_lilysharp_reproduce_ticket_before_fixing`）。**病名は「1 つの語彙が 2 階層に出ていた」**＝`IsInsideChordsBlock` がトラック本体とセルを区別せず、しかも `Completion()` が context の switch より**前**にそれを見る。**セルの隣の和音は `ChordNameCollector` が読まない**ので、**「うるさい」ではなく「通らない綴りを教えていた」**。⇒ **修理は LSP 3 箇所・Core は 1 行も触らない**（commit `fc5edc83`）＝`ChordsTrackBody` を立て、`IsInsideSectionContainer` に `chords` を足し、intercept を譲らせた。★ **どちらの語彙かは*形*が決める**——**判定は `TrackNeedsSectionsValidator` と同じ対**なので**validator が黙る所では popup も和音に落ちる**。**⑹ 第 2 便＝lyrics 側は 1 段悪かった**（commit `fb1893de`）: **`lyrics w sings melody {` は枠に (Prefix=`sings`, Name=`melody`) と読まれ `lyrics` は 4 語前＝手が届かない**⇒ **束ねたトラックの本体が音楽の補完に落ちていた**（実測 98 件）。⇒ **`IsLyricsFrame` を唯一の読み手にした**。★ **test は 2 つの綴りを*並べて*主張する**——片方しか知らない読み手はそれで落ちる。
...
★★★★ **⑽⑾ 逐語は `HANDOFF-ARCHIVE.md`「第374 の ⑽⑾（staff 行を語数で読むのをやめた 2 脚）逐語」**（20,000 字の門で出した）。**規則だけ**: **⑽ 第 6 便**（ユーザー起票 2 件・commit `7735687d`）＝**`staff treble ▮` に selector が出て `staff treble melody ▮` に出ない**——**1 つの欠陥を両側から見たもの**で、**読み手が*語数*を数えていた**から clef を書いた瞬間に位置が 1 つずれる。★★★ **⒜ は判定ですらなかった**——**「`staff bass ▮` が clef＋名前待ちか part＋selector 待ちか」は*この本*が答える問い**で、根拠にされていた「clef 語は part 名にもなる」は言語の事実で本に何も訊いていない。**⑾ 第 7 便**（ユーザー指示・commit `b3742fcf`）＝**`staff m as lines 1 ▮`・`staff ~m ▮`・`staff m "Violin I" ▮` が全部 素の一覧に落ちる**＝**省略可能な節が 4 つ**なのに読み手は全員「2 語前が `staff` か」を訊いていた。★★★ **連鎖（`{ StaffSelector }`・上限なし）が決着をつける**——**固定の後方参照では原理的に読めない**⇒ **`ScanStaffRow` が `{` から token を再生して「カーソルが行のどこに居るか」を言う**（5 つの return で行を尽くす）。**語数規則と `StaffAsGoverns` は消えた**＝**行の読みが 1 箇所に 1 つ**。
...
★★★★ **⒂ 第 11 便＝placeholder の全数監査**（逐語は ARCHIVE「第374 の ⒂」・commit `4566a82f`）。**規則だけ**: **監査の問いは「名前かどうか」ではなく「*その物は既に在らねばならないか*」**——**答えが違う 2 項目が同じ「form 名」を書いていた**（`score main { }` は*参照*で `LYS1018`／`form main { }` は*宣言*で `LYS1017`＝**逆向きに間違っていた**）。⇒ **`score` は本の form を書き、`form` は空いている名前を書く**。⚠️ **drummap の `${1:hh}` は実測して欠陥ではない**＝**UX 案件として残る**（下 ㉔⒠）。

★★★★ **⒃ 第 12 便＝*裸の MIDI 行の option を言語から外した***（ユーザー決定「後方互換の問題であれば、除外すべきだ。lily# のユーザーはまだほとんどいない」・commit `de98c1b4`）。⚠️ **出所は後方互換の残骸**（doc コメントが `instrument:25` 時代を書き、`ConsumeRejectedColon` を呼んでいた）。⚠️⚠️ ★★★ **除去の前に射程を数えた**: **この機械の `.lys` 27,095 冊を全部 parse**（追跡＋audit＋ユーザーの scratch）＝**MIDI 行 128 本・option 付き 0 本**⇒ **移行するものが無い**。**語は LYS0030 の stray item 報告に落ちる**。⚠️⚠️ ★ **落ちた test 2 本は*残骸自身の網*だった**（`guitar octave 1 instrument 25` が parse できることだけを主張）＝**両側を主張するよう書き換えた**。⚠️ ★ **その書き換えが最初*間違った入力*に対して緑になった**——**CRLF の raw literal に `\n` の針**で `Replace` が空振りし、変えたつもりの本文が無改変だった（記憶 `reference_edit_tool_rewrites_crlf_to_lf` の隣の罠）。

★★★★ **⒄ 第 13 便＝死語の全数に着手し*見つからなかった*——結論は「安い代理指標は 2 つとも効かない」**（逐語は ARCHIVE「第374 の ⒄」／**正本は `VocabularyPerturbationTests` の class remark**・出荷物の変更ゼロ）。**規則だけ**: **死語はどの本にも書かれていない**ので**コーパスを掃いても見つからない**。⑴ **構造（到達できない子）**は 7,221 万ノード走査で 44 型が該当＝**ほぼ全部が句読点**（消費側は `DescendantNodes<T>()` で*総称的に*歩くので「アクセサが無い」は「読まれない」ではない）／⑵ **型名の被参照数**は候補 2 つとも誤検出。⇒ ★★★ **唯一効く検出は*摂動***。

★★★★ **⒅ 第 14 便＝摂動ハーネスを建てた＝*言語が受ける語は全部ページか演奏を動かす*（commit `7fad4176`・`VocabularyPerturbationTests`）**。**136 摂動**を **SVG ＋ MIDI の署名**で baseline と比べる。⇒ **新しい死語は無い**（が、**⚠️ その緑の一部は第 19 便まで人工物だった**——下 ㉒′）。★★★ ⚠️⚠️ **INERT は「フィクスチャが差を表現できる」ときしか意味を持たない**——**最初の inert 読み 4 つは全部*私の*フィクスチャの誤り**（同じ clef を足した／空の段が無い本で hara-kiri を測った／popup の*ラベル*で摂動したが挿入は**ラベル＋オペランド**／span を開いて閉じ忘れ、しかも**族ごとに閉じ方が違う**）＝**全部コメントとして現地に残した**。★ **「正しく inert」な値は等値で主張する**（`as removeEmpty false`＝既定そのもの）ので、**既定が変わった日は死語ではなく既定変更として赤くなる**。⚠️⚠️ ★ **毒で発火を確認済み**。
...
★★★★ **㉒′ 第 19 便＝*網そのものが空振りだった*（判定は「着手」・commit `a6b84ccc`・出荷物は無変更）**。**着手の理由**: 第 18 便が入れた 22 本の弦配列は、**`Tunings.cs` と `TuningTableMatchesLilyPondTests` が同じ生成器から出ている**＝**あの theory は転記を独立に検証していない**。それを知っているのはこの便だけ。⇒ **陽性対照を 1 本置いた**: **`tuning standard` と `tuning guitar` は同じ調弦なのだから、ページは同一でなければならない**。**違った**。⚠️⚠️ ★★★ **差は `data-pos`＝SVG が持つ*原文オフセット***。**摂動は音楽より*前*に書かれる**（part 属性・`layout { }`・`drummap { }` が全部そう）ので、**綴りの長さが違うだけで後ろの data-pos が全部ずれ、`AssertMoves` はその語が何をしようとしまいと緑になる**。**リポジトリの他の比較は 2026-08-15 からこの属性をマスクしている**（`Diagnostic.cs` の注記 4 箇所）——**この 1 軒だけが素で比べていた**。⇒ **マスクすると 21 件が動かなくなった**（＝第 14・15 便の「136 本すべて緑」の一部は人工物）。★ **21 件に死語は 1 つも無く、全部*フィクスチャが鍵の効果を表現できていなかった***: `Plain` は**1 段・2 小節・臨時記号なし・和音行なし・2 つ目の part なし・section ラベルなし**なので、**layout の鍵はほぼ何も見せられない**。⇒ **鍵ごとに本を割り当てた**（barNumbers に 3 段／accidentals に *重複符 + 異オクターブの取消*／section ラベル 2 つ／combinedStaff／和音行／`pitch concert` に移調 preset／mark には*マークを持つ* drum）。★★★ **既定値は「語彙の先頭」と `LanguageVocabulary` が宣言している**ので、**手書きの一覧ではなくそれを読んで「既定は inert」を*等値で*主張する**（`as removeEmpty false` と同じ扱いが 4 件に増えた）。⚠️⚠️ ★★★ **毒が 1 度目は発火しなかった**——**私の対照分岐が `Tunings.Parse`（被験者）で選ばれていた**ので、`guitardropd`→Guitar と潰すと **test が「inert を主張する側」に落ちて緑**。**対照を語で書き下した**ら、**同じ毒が 3 つの test で同時に赤くなった**。⇒ ★ **恒久の教訓: 仕掛け線の*どちらの側に落ちるか*を、見張っている当の関数に決めさせてはいけない。** ⚠️ 新設の網 2 本: **調弦語 32 を tab 本で全数摂動**（`tuning guitar` 1 件だけだった）／**調弦表の番人**（drum 表と同じ形＝LP が二重に綴る 3 組以外、同じ弦を持つ語があってはならない）。★ **`TuningTableMatchesLilyPondTests` は LP の和音テキストを*自分で parse* するようになった**＝生成器が信頼の鎖から外れた。

★★★★ **㉒″ 第 20 便＝*族を掃いて空振り、だから固定して返した*（判定は「着手」・commit `4b5f2284`・出荷物は無変更）**。**掃いた問い**: **「2 つの本を比べる計器で data-pos を素で読んでいる者が他に居るか」**⇒ **居ない**（`AccidentalStyleTests`・`EmptyChordTests`・`FontAttributeTests`・grace の `Page` 2 軒は既にマスク／`FormVoltaWithoutRepeatTests` は素だが**主張が等値**＝素の方が強いので安全／`FontEditIncrementalTests` は**同じ罠を自分の remark で名指し済み**）。⇒ **`Diagnostic.cs` の「本を比べるときはマスクする」は本当だった**——例外は前便の 1 軒だけ。★★★ **空振りは固定して返す**（第 16 便の規則）ので、**同じ道具を第 18 便のもう半分＝打楽器 126 綴りに向けた**（表の番人しか無く、掃きが 1 本も無かった）。**3 本**: ⒜ **canonical 63 は全部 bassdrum と違うページか音になる** ⒝ **略号 63 は full name と*バイト一致*＝63 本の陽性対照**（**別の行を指す alias はページを動かす——ただし行き先が違う**＝`hhs` の形）／⒞ ★★★ **mark が唯一の差である 11 対は、その mark が*描かれて*いなければならない**。⚠️⚠️ **⒞ が本命**: **`guiro` と `longguiro` は行も符頭も GM 74 も同じで、LP は `tenuto` だけで分ける**。**`MarkArticulation` から `tenuto` を落とす毒を入れると ⒞ だけが赤で、他の drum test 399 本は緑**——**表の照合も衝突番人も `DrummapTests` も registry を読む**ので、**描かれない mark はそのどれにも見えない**。⇒ ★ **「表が正しい」と「ページがそれを描く」は別の主張で、後者を訊いているのはこの 1 本だけ**。⚠️ **11 という数は書いたとき 16 と書き間違えていた**——**test が先に報告した**（bongo 3 対 ×2＋conga 2＋guiro 1＋triangle 1＋hihat 1）。

★★★ **㉓ 数**: **開始時**: HEAD `1e80a9b2`・未 push 9・木 clean・未追跡 0（**`Session-Check -Build -Test` は走らせていない**）。**終了時**: HEAD `4b5f2284`（**出荷物の最後の変更は第 18 便 `851e5469`＝LP の打楽器 33 種と調弦 25 種。第 13・14・16・17・19・20 便は production 無変更**）・未 push 47（うち 3 本は doc）・**台帳 843 点／ss 非ゼロ 210／総和 23.216135909／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249／追跡 `.lys` 599＝全部不変**（⚠️ **第 18 便は Core を 7 ファイル触ったのに、絵は 1 枚も動かない**——**既存の綴りの振る舞いを 1 つも変えていない**からで、**台帳・snapshot・追跡 `.lys` が全部不変なのがその測定**。第 13・14・16・17 便は Core を 1 行も触っていない）。**full `scratch/p375/run30.trx` 8353 / 0 / 4 / 8357**（第 20 便＝+127 本＝drum 126 綴りの掃き＋mark の網 1 本）・第 19 便末 `run29.trx` 8226 / 0 / 4 / 8230（+33＝調弦語 32 の摂動ほか）・第 18 便末 `run27.trx` 8193 / 0 / 4 / 8197（+68＝打楽器 3 theory・調弦 1 theory の case が大半。⚠️ **`run24` は 1 本赤**＝`ApproximationInventoryTests` は**生成物 `docs/APPROXIMATIONS.md` の行番号**を見張るので、exporter が 11 行縮んだだけで赤くなる⇒ `LILYSHARP_UPDATE_DOCS=1` で再生成・差は 4 行）・第 17 便末 `run23.trx` 8125 / 0 / 4 / 8129・第 16 便末 `run22.trx` 8119 / 0 / 4 / 8123（drum 照合 31 本）・第 15 便末 `run21.trx` 8088 / 0 / 4 / 8092（表の番人 +1）・第 14 便末 `run20.trx` 8087 / 0 / 4 / 8091（摂動 136 本ぶん増）・第 12 便末 `run19.trx` **7951 / 0 / 4 / 7955**（⚠️ **第 12 便で 9 本*減った*のは正しい**＝matrix の MIDI 行 3 綴り × 3 主張が言語から消え、代わりに「退役した綴りは断られる」2 本が入った。第 11 便末 `run16.trx` 7964・第 10 便末 `run15.trx` 7956・第 9 便末 `run14.trx` 7953・第 8 便末 `run13.trx` 7947・**第 1〜7 便末は `run1`→`run12` で 7816→7869**。**13 便とも合計を読んでいる**＝新 test は差引 144 本（第 12 便は −9）（第 8 便の 78 ＝ 26 綴り × 3 主張）。⚠️ 第 5・6 便は既存 test を**書き換えている**ので増分はそれより小さい）。★ **第 7 便は 7 つの context の配線を丸ごと替えたのに既存 7854 本が全部緑**＝**行の読みが 1 つになっても、読めていた綴りの答えは変わっていない**。⚠️ scratch は **`p375`**。⚠️ §7 3.5: 第372 を `-Archive 372` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝**第372 の天井 commit `9f6df2a3` が未 push のまま**。**push はユーザー（未 push 47）**。★ **LSP は 12 度配布した**（第 19・20 便は Core も Lsp も無変更なので再配布していない）＝`tools/Deploy-Lsp.ps1` が `0.6.0+851e5469` を `yotsuda.lilysharp-0.5.0-win32-x64\server` へ（95 ファイル byte 一致）⇒ **ユーザーは Reload Window が 1 回要る**（package.json は動かしていないので 2 回は不要・記憶 `feedback_lilysharp_lsp_deploy_target`）。

⇒ ★★★ **㉔ 次の一手＝✅ ⒜（LP の語彙をどこまで入れるか）は第 18 便のユーザー決定「両方とも入れて」で閉じた**（打楽器 30→63・調弦 7→32 語）。★★★ **⒜″ 最優先は*摂動網の残り*で、理由が変わった**——第 19 便まで「網は緑だから残りを足すだけ」だったが、**網自身が 21 件空振りだった**ので、**残り（paper キー・chord quality 35・`@` の引数つき形）は「足す」より先に「今在る主張が何を測っているか」を疑う仕事**になった。⚠️ **やり方は決まっている**: **陽性対照を 1 本置く**（同義の 2 綴りがページを変えないこと）＋**フィクスチャがその鍵を表現できるか先に確かめる**。**残った札は 3 枚**: **⒜′ `InstrumentDefaults.GetTuning` は preset 5 族しか答えない**（`violin`／`cello`／`viola`／`mandolin`／`banjo` の preset を書いた part を tab で描くと**ギターに落ちる**）＝**調弦は在るのに preset が繋がっていない**。**触っていない理由は 1 つ**——**既存の本の tab の絵が変わる**（`instrument violin` ＋ tab の本があれば 6 弦→4 弦）ので、**射程を数えてから**（第 12 便の型）。⒝ **摂動網の残り**（paper キー・chord quality 35・`@` の引数つき形）。⒠ **drummap の `${1:hh}`＝UX 案件**（drum 語彙を出すと親切。`drummap { ▮ }` の本体に語彙の家が無いので context が 1 つ要る）。⒡ **第373 ⑼⒜（probe 24 冊の「量を測ったか／問いに答えたか」の仕分け）**も先頭級のまま。**ヘッダの綴りの島は閉じた**——⑻ の 25 綴りが全数で、**残る未点検はゼロ**（`{` を開く構文は score／staff group 5／part／phrase／form／lyrics／chords／section／fonts・paper・layout とその入れ子／音楽の容れ物で尽きる）。⚠️ **閉じたのは*補完の*読み手だけ**: 同じ「2 語しか読まない枠」を持つ道具が他に居ないかは見ていない（`SemanticTokens`・折り畳み・`BarCountFix` は別の木＝**実構文木**を読むので原理的に無縁、という*予想*まで）。⒢ 第369 ⑸⒝（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。

★★ **㉕ 判定の記録（ユーザーの常設指示「次便は、このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）**: **第 2・5・6・7・8・9・12〜18 便はユーザー起票／指示**（判定の対象外）。★★★ **第 19 便は判定で着手し、着手の根拠が「道具の温度」ではなく*この便しか知らない事実*だった**——**第 18 便の theory と出荷物が同じ生成器から出ている**。**冷えた便には見えない**（どちらも緑で、コメントにも書いていなかった）ので、**書き残すコストと塞ぐコストがほぼ同じ**＝着手が有利。**結果、探していた穴の 10 倍のものが出た**（網が 21 件空振り）。⇒ ★ **「自分が今作ったものの*検証*が独立か」は、作った便にしか問えない。** ★★★ **第 20 便も判定で着手**——**直前の的の見つけ方（「2 つの本を比べる計器に陽性対照を当てる」）を*族に広げる*のは、同じ操作の N を増やすだけ**（第 3 便で 1 度書き間違えた形）。**掃きは空振りだったが、規則どおり固定して返した**＝第 18 便のもう半分に同じ道具を向け、**そこで本命（描かれない mark）が獲れた**。⇒ ★★ **「空振りの掃き」の正しい終わり方は、*その道具を次に向けること*であって、報告して終わることではない。**★★★ **第 18 便はユーザー決定で着手した脚だが、判定の教訓を 1 つ足した**——**「移植は機械的」と第 17 便が書いたのは*値*の話で、決めることは 4 つ在った**（どの style 表から取るか／mark 語彙が足りるか／LP 自身の dangling 名 `tt` を入れるか／綴りの規則）。**⇒ 「機械的」と書いた次の便は、まず*決める点*を数える。**⚠️ **「どこに写しが在るか」も移植の一部だった**——名前→`TuningType` の switch が 3 軒に在り、**先に 1 軒へ寄せなければ 25 種を 3 度書いていた**（寄せたあとは 1 箇所）。**判定で着手したのは 8 脚**（⑺ tilde・⑻ 掃き・⒁ MIDI 行・⒂ 名前の監査・⒅ 摂動網・⒆ 網の拡張・⒇ LP 照合・㉑ tuning 照合）で、**8 つとも「直前の脚が*何で*断定したか」を道具に採った的**。★★★ **⒇ は「掃いて何も出ない」が*成果*だった脚の 2 つ目**（1 つ目は第 13 便）——**ただし今回は網に翻訳できた**（LP の行を checked-in の test に）。**⇒ 空振りの掃きは、次に誰かが同じ疑いを持つ前に*固定*して返す**。★★★ **⒆ で初めて「網を建てた次の便が獲物を獲った」**＝`hhs`。**網は建てた便では何も釣らず、次の便で釣れる**（第 14 便は 136 本すべて緑だった）。★★★ **基準はこの 1 行＝「どの道具が温まっているか」ではなく*直前の的をどうやって見つけたか***（第 3 便で 1 度書き間違え、第 13 便で 1 度守り損ねて 40 分落とした——**2 度とも同じ形**）。⚠️ **仕掛け線を張ったら必ず毒で赤を見る**（第 8・14 便は入れた。入れない網は「在るつもり」になる）。⚠️⚠️ **冷えた便が §1 の見出しだけで着手すると*逆に壊す***——⒁ は「穴」ではなく死語だった（埋めていたら何もしない綴りを補完に足した）／⒂ は `form` が*逆向き*だった（在ってはいけない名前を書く側）。★ **1 便だけ shell 切断で「有利／不利」ではなく*実行可能か*が判定を決めた**。**着手しなかったもの**: ㉒⒜ は**ユーザー決定 2 件**（語彙をどこまで入れるか＝承認要）＝着手できない。㉒⒝ は**同じ機械の続き**で冷えていても進む／㉒⒡⒢ は**別 regime**（LP を回す／設計級）＝次のセッションでやっても損しない。★ **この便は「照合して差がゼロ」を 2 表で出した**——**空振りに見えるが、次に疑う人の 40 分を先に払った**（第 13 便の教訓の実践）。★ **便ごとの逐語は `HANDOFF-ARCHIVE.md`「第374セッションの『判定の記録』逐語」**（20,000 字の門で出した）。
