...
- **⑹ ★★★ `perf-plain1k` の打鍵 46.1 MB の完全な地図**（第4便の後・**全部*読んだ*数**・単位 MB）:
  ...
  | 段 | MB | % | 状態 |
  |---|---|---|---|
  | `L1-break`（DP） | 19.32 | 42% | **閉じた**＝本質的・2 案とも測って却下（§1 第191） |
  | `4-render` | 8.93 | 19% | **床に近い**（構造上の下限 約 7.5・第191 第7便） |
  ...
  | `L6-pages`（`CreatePages`） | 1.00 | 2% | 済（第191 第5便） |
...
> （2026-08-17・第190 起票／**第191 が 6 段・第192 が 4 段返済**＝
> **`perf-fingstack1k` 3304.5 → 165.9 MB（−95.0%）・`perf-plain1k` 95.9 → 46.1 MB（−51.9%）**。
> **次の一手と現在の地図は §1 ⑹ の表**）
>
...
>
> **⑴ 残っている島は 2 つ**（MB は `perf-plain1k` の 1 打鍵 46.1。**内訳は §1 ⑹ の表**——
...
> 専用の便と falsifier から。**／
> **⒝ `BoundaryClefAllowance` 0.93 MB＝`BoundaryColumn` を丸ごと建てて nullable を 1 個読んでいる**
> （1000 小節で 0.93 KB/小節。⚠️ **`CreateSpringsForMeasure` という 2 人目の呼び手が居る**ので一緒に見る）。
> ⚠️ **「`3-layout` の未帰属 約 2 MB」という 3 つ目の島は*存在しない*。** 第192 が引き算で出して
> **同じ便のうちに region で測って消した**——梁の検出を `L2-firstSystem` と二重に数えていた分だった
> （§1 ⑹ の訂正）。**引き算の残りを島として引き継がないこと。**
>
> **⑵ もう見なくてよい所**（測って閉じた・**もう一度開けないこと**）:
> **`L1-break` の DP 19.3 MB（40%）＝本質的**（列を切る案・帯状化の案とも実測で退行・§1 第191）／
...
> 第191 第4便と同じ「順序の決定」**（§1 第191 の骨 ⑹）。**着手するならユーザー決定から。**
