...
最終更新 第306セッション＝**§1 が名指した「次の一手」の 1 つめ＝§2 F ⒯（`.ly` 双子が part の後ろの独立ヘッダで section の音楽を落とす）をそのまま取って実装した**。**入り方は第298〜第305 と同じ**——**ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った**。**第305 は「最小の対と対照が在るので着手しやすい」と書いており、*対も住所の当たりも合っていて再現に 5 分もかからなかった***。⇒ ★★★ **だが起票が「次便の最初の一手」と名指した射程の裏取りは、**当の proxy を否定した**（⑴）。**掃きは 1966 冊・両側 Release・**4 出力**（うち 1 つは本便が足した）。

⚠️⚠️ ★★★ **⑴ 起票が「まず確かめよ」と言った 1 冊は白だった＝proxy の偽陽性。** **起票は「独立ヘッダを持つ本 32 冊のうち*後ろ*に置くのは 4 冊で、1 冊はユーザーの `(They Long To Be) Close To You.lys`」と書き、「双子の小節線 75 に対しページは 83 小節」を状況証拠にしていた。** ★ **開いたら、その本は独立ヘッダを 1 つも持っていない**——**9 つの section は全部 `part bassline` の中**。**双子は完全で、音楽は 1 音も落ちていない。** ⇒ ★★ **8 の差は欠陥ではなく `\repeat percent 4 { r1 | r1 }`**: **ページでは 8 小節に開くのに双子は `|` を 1 本しか書かない（−7）／最後の `s1` に閉じの小節線が無い（−1）＝ちょうど 8。** ⚠️ **起票自身が「小節線の数と小節の数は別の量」と警告していて、その警告のほうが正しかった**——**proxy が名指した本の名前は、確かめるまで射程ではない。**

⚠️⚠️ ★★★ **⑵ 原因は述語の食い違い 1 か所で、直したのは 1 行。** **`LilyPondExporter.OrderedMusic` は section が part に届く 3 つの綴りに 1 本ずつ腕を持つ**。**3 つ目＝「単一 part の略記」（cell を被せず top-level の section に直接書いた音楽）の門番が `LooseSectionMusic(s).Any()` で、*その一覧は指示を音楽に数える***。⇒ **`section A { key g major }` が「鳴る宣言」として登録され、この辞書は*後勝ち*なので、part の後ろに書いたヘッダが part の本物の cell を上書きした**。**前に書けば cell がヘッダを上書きするので同じ本が健全**＝**違いは行の順だけ**。★★ **コレクタ側の同じ腕は `SectionHasInlineMusic`（THE one spelling・指示を除く）を訊いている**ので、**双方が同じ問いを訊くようにした＝ページと双子が偶然ではなく構成上一致する。** ⚠️ **言語自身が既にそう言っていた**——**`SectionMusicNeedsPartValidator` は part-major の本の top-level section に音楽を書くことを拒む**ので、**この腕が在る理由の側が、正典の述語がちょうど通す側**。

⚠️⚠️ ★★★ **⑶ 掃きに 4 つ目の出力を足さないと、この便の掃きは*何も見えないまま全緑*だった。** **第305 の `sweep305.ps1` は svg / midi / xml / check を撮る**——**`.ly` は 1 度も描かない**。**⒯ が動かす唯一の出力がそれ**。⇒ ★★ **`ly` を 4 つ目として足し、動いた本は双子の本文を JSON に残した**（掃き直さずに diff が読める）。**結果 1966 冊で SVG 0／MIDI 0／XML 0／`check` 0**・**LY が動いたのは 7 冊で 7 冊とも scratch のプローブ**（第305 の 2 冊＋本便の 5 冊）。**追跡本・snapshot・ユーザーの本は 1 冊も動かない。** ⚠️ **7 冊の diff は全部意図したもの**——**音符が戻り、指示が 1 度だけになる。**

⚠️ ★★ **⑷ form 無しの腕は二重に当たっていた。** **ヘッダは `inOrder` にも 2 つ目の項として入っていた**ので、**form を書かない本は「空の 2 度目の A」を*幻の play* として鳴らしていた**（`\mark \markup \box "A" \key g \major \key g \major`）。**同じ 1 行で一緒に消えた。**

⚠️⚠️ ★★★ **⑸ 隣に*別の*欠陥が在り、混ぜずにピン留めして起票した＝§2 F ⒰。** **宣言が「ヘッダだけ」の名前**（`section A { key g major }` だけが在り、どの part も A の音楽を持たない）**を form が鳴らすと、ページはその調を次の section の小節に効かせるのに双子は調を 1 つも書かない**。⚠️ **本便の前は 3 つ書いて*最後が違う調*だった**（`\key g \major \key g \major \key c \major`）ので、**食い違いは前から在って本便で*小さく*なった**——**だが食い違いではある**。⇒ ★★ **「0 小節しか鳴らない section が次の小節に調を armする言語か」は*言語の決定*なのでユーザー待ち**。**番人には*穴として*ピン留めしてある**（§5.4・閉じた日にその行が直す場所を名指す）。

★ **GRAMMAR.md 自身の独立ヘッダの参考例が、壊れていたほうの綴り**（`part melody { section A { … } }` の*後ろ*に `section A { key g major }`）。**文法書は直していない**——**綴りは正しく、直ったのは実装のほう。**

⇒ ★★★ **次の一手**: **§2 F ⒰**（本便が起票した穴。**言語の決定が要るのでユーザーに訊くところから**）／**§2 F ⒭ の残り**（part ブロック option 位置＝**`transpose` のスコープ漏れ**は出力が動く。**未決定**・**測定は第305 で済んでいる**）／**§2 U8 の残り**（`grace { }` 本体の和音・休符／スラー・梁・タイ／注釈——**本便も 1 行も触っていない**）／**§2 の X/Y 側**。⚠️ **承認待ちは無い**（snapshot は 1 枚も動いていない）。

★ **開始時裏取り**: HEAD `270673af`・**未 push 29**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6773 / 0 / 4 / 6777**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 点 158（非ゼロ 2）／OPEN: 0**・追跡 `.lys` 581・snapshot 231——**第305 の終了時の数と 1 つも違わなかった**（**素朴に数えると `exact` 556 が出る。§0 のレシピは `-le 1e-6`**）。
終了時: **commit 2 本**（**⒯ 本体 `10321fbd` ／ §1・§2 とアーカイブ（この commit）**）・**未 push 31**・作業ツリー 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6785 / 0 / 4 / 6789**（**開始 6777 から +12＝新しい番人 1 ファイル**＝`LilyPondStandaloneSectionHeaderTests` の 12 行）・**台帳 748／588／190／24.714570188／158 中 2／`OPEN:` 0・追跡 `.lys` 581・snapshot 231 は開始時と同じ**（**snapshot は 1 枚も動いていない**）。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけ**（**定数は 1 つも足していない**・`audit/magic_constants.csv` も不動・**REF/OWN の増減 0**＝**LP からの移植は 1 行も無い**）。⚠️ **間の push は無し**（`origin/master` は `be16a770` のまま）。★ **掃きの base 用に建てた `C:\MyProj\lys-base306` の worktree は本便で消した。**
★★ **WSL ubuntu Release は `10321fbd` の木で Windows と同じ 6785 / 0 / 4 / 6789**（build 7 秒 / test 31 秒）——**§0 の両脚を読んだ。**⚠️ **この docs commit では ubuntu を回していない**（**動いたのは `.md` 3 本だけ**）。
⚠️ **計器は `scratch/p306/` に全部残っている**: **`sweep306.ps1`**（第305 の掃きに **`ly` の 4 つ目**と「動いた双子の本文を残す」を足したもの）**と `sweep306.json`**（1966 行・**動いた 7 冊は双子の全文つき**）**と プローブ 8 冊**（`b1`〜`b8`。**`b8` は陽性対照＝ヘッダを消した本**）。


## 以下は第305セッションの経緯

最終更新 第305セッション＝**§1 が名指した「次の一手」＝§2 F ⒫（`|:` `:|` `[N. …]` を form 限定にする）をそのまま取って実装した**。**入り方は第298〜第304 と同じ**——**ユーザーは `docs/HANDOFF.md` を読んで §2 F ⒫ に着手せよと言った**。**第304 は「⒫ は仕事だけで、次便は『⒫ に着手して』だけで始められる」と書いており、*住所も診断の置き場所も当たっていて探す時間は 0 だった***。⇒ ★★★ **だが「仕事だけ」は正しくなかった**: **決定を実行しようとしたら*form 側が同じことを言えない*ところが 1 つあり、それは帰結ではなく前提の反証だった**（⑷）。**掃きは 1946 冊・両側 Release・4 出力。**

⚠️⚠️ ★★★ **⑴ 起票が設計した通りに入った＝診断 1 本・パーサの腕は 0 個。** **`RepeatStructureScopeValidator`（`LYS1034`）は 1 つの述語で 4 つの形をまとめて取る**——**`BarlineSyntax` の token が repeat 種、または `InlineVoltaSyntax`。それが `IsInside<FormDeclarationSyntax>()` でなければエラー。** **禁じたのは token ではなく*場所*なので、その綴りは今も*読める*＝診断がその場所を名指せる。** ★ **島はシンクで数えた**（§5.0）: **木に立ちうる `BarlineSyntax` の生産者は 4 か所**（music の item・`chords` 行の item・form の裸の `:|`・`ParseFormBarline`）**で `InlineVoltaSyntax` は 1 か所**（`ParseInlineVolta`）。**form の腕が在るから祖先テストが要る**——**form 自身の `|: … :|` ブロックは小節線を*生トークン*で持つのでノードを作らない**が、**裸の `:|` / `:|:` は作る。** ⚠️ **合成ノードは見えない**（`LilyPondExporter.CreateEnding` と `MeasureCollector.Form.CreateBarlineSyntax` は親 null で作る＝**正しい form の繰り返しを描く経路そのもの**。根から歩くのが、この規則を「作者が*書いた*もの」の話に留めている）。

⚠️⚠️ ★★★ **⑵ 第304 の数が*別の計器*で再現した＝追跡 11 冊・ユーザー 115 冊。** **第304 はこの数を 3 度間違えてから収束させた**（テキスト走査）。**第305 はコンパイラで数えた**（パーサのノード型）——**同じ 11 と 115**。**内訳も表のとおり**（snapshot 3・LP 回帰 1・`audit/lpreg/` 5・showcase 2）。★ **lyrics の `[N. …]` と `repeat percent` / `unfold` / `tremolo` は 1 冊も当たっていない**——**除外リストではなく*ノード型が別*だから**（`LyricVoltaSyntax`／`RepeatExpressionSyntax`）。⚠️ **lyrics 行の `|:` だけは見えない**（`LyricMeasureGreen` の生トークンで `BarlineSyntax` にならない）——**書けてしまうが、歌詞の行は何も演奏しないので順序も変えない。実測して記録した**（`scratch/p305/b4_lyric_row_repeat_bar.lys`）。

⚠️ ★★ **⑶ 掃きで「新しい診断はこの 1 本だけ」を機械で示した**（起票の指示どおり）: **1946 冊で `check` が動いたのは 253 冊、*増えた行は全部 LYS1034*・減った行は「No errors found.」**。**例外は 5 冊だけで、全部こちらが意図したもの**——**言い換えた診断文 3 本**（`repeat volta` の案内・LYS4017 の 2 か所）**と、3 つ目の括弧が通るようになって消えた LYS6008 の警告 2 本**。**SVG / MIDI / XML が動いたのは、その 3 つ目の括弧を使う 2 冊と `:|*N` の probe だけ。**

⚠️⚠️⚠️ ★★★★ **⑷ 決定を実行しようとして*前提の反証*が出た＝form は 3 つ目の volta 括弧を綴れなかった。** **music は `|: X | [1. A] :| [2. B] :| [3. C]` と書けたが、`ParseFormRepeatBlock` は `:|` のあとの括弧を 1 つしか取らず、3 本目は LYS6008 で落ちて素の参照になっていた**（実測）。⇒ ★★★ **仕分けが要る場面**（RULES §5.1 の「帰結か反証か」の*裏側*）: **決定の文は「form の中だけで書けるようにする」で、それは form が同じことを言えることを*前提にしている*。前提が偽なら、実装を足すのが決定の実行。** ★ **着手前に数えた**: **ユーザー 326 冊のうち 13 冊・追跡 1 冊**（`voltasky`）**が 3 つ目以降を使う**。**数え方はコンパイラ**——**(p) の診断文が `('[3. … ]')` と綴りを引用するので掃きの出力を grep すればよく、素朴な regex（21 冊）は lyrics の第 3 節を混ぜていた。** ⇒ **`ParseFormRepeatBlock` に「`:| [N. …]` をもう一度」の腕を足した**（`FormRepeatBlockGreen` は平らな子リストで `FormWalk` が slot を歩くので、**下流は 1 行も変えていない**）。**検算は恒等の対**: **ページはマスク後バイト同一・MIDI は 6 音で音列一致。** ⚠️⚠️ ★★★ **そのとき*旧綴りのほうが壊れていた*のが出た**——**music 綴りの 3 パス目は本体を飛ばして括弧だけ鳴らしていた**（X E1 X E2 **E3**）。**form 綴りは X E1 X E2 X E3。**

⚠️⚠️⚠️ ★★★★ **⑸ 禁止が「唯一の家」を作った瞬間、その家に潜んでいた欠陥が*唯一の挙動*になった＝form の `:|*N` は MIDI に届いていなかった。** **`MidiExporter.PlayRepeatBlock` は `Math.Max(2, alternatives.Count)` で、`FormWalk` が持っている play count を読んでいない**——**`form { |: ~X :|*3 }` は 2 回、同じ音楽を music に書くと 3 回**（**実測 16 音 対 24 音**）。**base でも同じ＝第305 が作った欠陥ではない。** ★ **ページと `.ly` 双子は前から正しく、4 読み手のうち MIDI だけがずれていた**——**だから music 側が生きているあいだは誰も踏まなかった。** ⇒ **music 側と同じ 3 段の規則に直した**（明示 `*N` ＞ 括弧の数 ＞ 2）。**射程はディスク 19 冊・ユーザー 8 冊。**

⚠️⚠️ ★★★ **⑹ 自分の道具が自分の禁止を破っていた＝MusicXML importer が music に `|:` を書いていた。** **`LysWriter` は ending 付きだけを「名前つき section ＋ form」に分解し、単純な繰り返しは `BarlineBetween` が music に書く**——**つまり import した本がコンパイルできなくなっていた**（`MusicXmlRoundTripTests.SimpleRepeat_RoundTrips` が赤で教えた）。⇒ **`TryFactorPlainRepeats` を足した**: **repeat 小節線ごとに section を切り、form が順序を言う**（`:|:` も一方通行の `:|` も取る。**閉じられていない `|:` は曲末で閉じて `report.Warn`**——**黙って落とすともう 1 つの「音楽が変わるのに何も言わない」になる**）。**`BarlineBetween` の repeat 3 腕は到達不能になったので、消さずに理由を書いた。**

⚠️ ★★ **⑺ 嘘になった計器を 3 本直した**（§5.2.1「嘘を立てたまま残さない」）: **`RepeatVoltaRemoved` の案内文が「インラインの `[1. …] [2. …]` を使え」と*その場で宣伝していた***／**LYS4017 の 2 か所が「この section か form のどちらでもよい」「逆向き（music の `|:` を form の `:|` で閉じる）は効く」と書いていた**。⚠️ **言語資料 4 冊は `DocExamplesParseTests` が全部赤にして教えてくれた**（GRAMMAR / GRAMMAR_FOR_LLM / SYNTAX_REFERENCE / TUTORIAL）——**生成器と診断文は誰も守らない。**

⚠️⚠️ ★★★ **⑻ 切り直した 11 冊は「移動」ではなく、3 冊で*出力が良くなった*。** **`grandstaff-repeat` はページに 8 要素*増えた*＝volta 括弧 2 本と "1." "2." で、旧綴りはこの本でどこにも描いていなかった**（削除は 0。MIDI は第304 の予測どおり 18 → 20 音）。**`rehearsal-marks-inside-containers` の MusicXML は `<ending>` を*得た***（インライン綴りは 1 度も書いていなかった＝⒮ の兄弟）。**grace × music `|:` の 4 冊は幽霊空小節が消えた**（MusicXML の小節が 2 → 1）。**`lead-sheet-repeat` と `voltagrace-ctl` は 3 出力とも同一**（ページは `data-pos` のみ）。

⚠️⚠️ ★★★ **⑼ 網は張り直した。黙って別のテストに替えていない**（§5.4）。**`grandstaff-repeat` と `grammar-tour` の「1 パートにだけ書いた繰り返し」という主張は*書けない*ので、両方のヘッダに「この本が言えなくなったこと」を名指して残した。** **`rehearsal-marks-inside-containers` も同じ**——**「インライン volta という容れ物の中のリハーサル記号」は観測できなくなり、生き残った容れ物は tuplet だけ**と書いた。**`VoltaBracketSkylineTests` は inline 綴りの 1 行を form 綴りに書き直した**（**量は不動＝括弧 6 本・2 つの鎖が 2.0 差**）。**`audit/lp-regression/status.json` は 1 行だけ書き換えた**——**`repeat-volta-initial-grace` は LP 原本が `\grace f'8 \repeat volta 2 b'1`＝**grace がリピートの外**で、**form は section 参照しか持てないので Lily# はもう同じ構造を綴れない**。**state=open は据え置き（根は動いていない）だが、open を支えていた観測はこの本から取れなくなった**と書いた。

⚠️⚠️⚠️ ★★★★ **⑽ snapshot 3 枚は再ベースした＝ユーザー承認を取ってから**（§5.1＝出力を動かす変更は承認前に出荷しない。**中身を 1 枚ずつ見せて「3 枚とも再ベースする」）。★ **第304 の「インクが同一に切り直せれば再ベースは 0 件」は*原理的に成り立たなかった***——**`data-pos` は本文を書き換えれば必ず動き、snapshot はそれを比べるから**。**保てるのは*インク*のほうで、3 枚のうち 2 枚は保てた。** ★ **numstat がその区別をそのまま言う**: `lead-sheet-repeat` **9/9**・`rehearsal-marks-inside-containers` **46/46**（＝出入りが同数＝`data-pos` だけ）／`grandstaff-repeat` **56/48**（＝**8 要素の増加・削除 0**）。**この 3 枚以外の snapshot は 1 枚も動いていない**（`git status` が 3 ファイル）。

⚠️⚠️ ★★★ **⑾ `docs/HANDOFF.md` に stray CR が 4 つ在り、それが「§1 を書き換える」を*できなくしていた***。**`CR CR LF` が 4 箇所**あって git は blob を `-text` と分類しており、**その 4 バイトを消すだけで `3334 insertions / 3334 deletions`**（1 行だけ直せば `1 1`）。⇒ **RULES §5.1 の「`--numstat` を見て編集行数と合うことを確かめてから commit する」が、この 1 ファイルでは*構造的に守れなかった***。**正規化だけの commit を 1 本立てて分離した**（`72c8ba59`。**CR を全部剥がすと本文は文字単位で同一**であることを確かめてある）。★ **以後この便の HANDOFF 編集は普通の numstat で読める。**

⚠️⚠️ ★★★ **⑿ 委任が出て、⒭ には*着手しなかった*。⚠️ ただし委任の時間で ⒭ を測り直したら、1 つの欠陥だと思われていたものが 3 つだった**（§2 F ⒭ に書き換えた）。**ユーザーが「⒭ はどういう内容か詳しく」と訊いたので、起票文をそのまま説明せず実コードで裏取りした**——**それが広がりを出した。** ⑴ **起票の主張（section ヘッダの `clef` が落ちる）は再現する。** ⑵ **起票が「確かめていない」と書いた問い——言語が約束しているか——には答えが在った**: `GRAMMAR.md` の `SectionSetting = KeyDecl | TempoDecl | TimeDecl | PartialDecl` に **clef は無い**＝**修理は実装ではなく診断**。 ⑶ **根は起票の診断より 1 段下**: ヘッダ設定として読まれていないのではなく、**`IsMusicItemStart()` が `ClefKeyword` を受けるので「どの part にも属さない裸の音楽」になる**（`SectionMusicNeedsPartValidator` は part-major でしか報告しない＝section-major では無言）。 ⑷ ★★★ **そして第 2 の綴りが在った**: **part ブロックの option**（`IsPartOption` ＝ transpose / octave / instrument / clef。**`GRAMMAR.md` の `PartBlock` には option がそもそも書かれていない**）。**4 つ測ったら 3 つが黙って無視され、`transpose` だけが*読まれてスコープを間違える***——**section A に書いたのに part 全体が D5 になった**。⇒ **性質が違う**（出力が動く）**ので別項に切る余地がある。**
⇒ ★★ **着手を保留した理由は大きさではなく*形***: **⒭ は綴りごとに「診断して拒否する／実装する」をユーザーが決める必要があり**——**⒫ を 2 便止めていたのと同じ形の問い**——**しかも `transpose` の側は出力を動かす**。⇒ **その決定は測定を見せた直後に出た**（次項）。

⚠️⚠️⚠️ ★★★★ **⒀ 測定を見せたらユーザーが即決し、⒭ の半分がその場で閉じた**（`LYS1035`・§3 に 2 行）。**理由もユーザーの言葉**: **「あまり意味がないというか、役に立たない指示に思える」**。⇒ ★★ **これは ⒭ の ⑵ が示していた向きそのもの**（`GRAMMAR.md` の `SectionSetting` に clef は無い＝言語は約束していない）——**測ってから見せると、答えられなかった問いが 1 往復で閉じた。**
⇒ ★★★ **実装で分かったのは「規則を足す」のではなく「*対称性*を閉じる」だったこと**: **その位置に書ける part 設定は 4 つで、`instrument` と `transpose` は元からエラー**（誰も要求しないので `ReportStrayItem`）。**`clef` と `octave` だけが `IsMusicItemStart` に載っている**（他所では本物の music item だから）**ので music 腕に取られ、孤児の音楽になって黙っていた**。⇒ **4 つのうち 2 つが喋っていたのを 4 つにした。`instrument` は作業ゼロで、直したのは LYS0030 の文面だけ**（「正しい家」を言っていなかった）。
⚠️⚠️ ★★ **`octave` を巻き込んだ判断は設計論ではなく実測**: **音高は 1 つも動かないのに `.ly` 双子だけ part 全体の包みが `\relative c'` → `\fixed c'` に変わる**＝**「効かない」ではなく*読み手が食い違う*。**
★★ **述語は語ではなく*位置***: **section の本体が音楽そのものなら同じ `clef` は正しく効く**（part-major と、1 パートで裸の音楽を書く section。両方とも実測）——**cell を持つ section だけが置き場所を持たない。** ⇒ **番人はその 3 行を「黙る側」として一緒に留めてある**（`PartSettingInSectionHeaderValidatorTests`）。**毒**: 位置テストを外すと**その黙る 2 行だけが赤くなる**。
★ **射程はゼロ**（ディスク 1954 冊で当たるのは scratch のプローブだけ＝追跡本 0・ユーザーの 326 冊 0）。**`section ~A` と同じ「純粋な追加」。**
⇒ ★★ **そしてユーザーの問い「octave と transpose はサポートしたら便利か」には「形が違う」と答えて承認された**（§3）——**`transpose` が欲しい場面（最後のコーラスだけ半音上げ）は*参照側*の印であって宣言側ではない。⒬ がその判断の先例。** **未起票。**

⚠️⚠️⚠️ ★★★★ **⒁ そして*出荷した 20 分後の自分の commit に欠陥を見つけた*——見つけ方は「本便が RULES に足したばかりの規則を、自分の 2 本目に当てていなかった」**。**⒞「禁じた綴りを*書く側*を grep する」を LYS1034 では importer に当てたのに、LYS1035 では当てていなかった。** ⇒ **書く側は*エディタ*だった**: **`PartSectionLayoutConverter.IsSectionDirective` は `clef` / `octave` を section directive として*動かす***ので、**part-major の「独立ヘッダ」**（`part m { section A { … } }` ＋ `section A { clef bass }`＝GRAMMAR が `key` について認めている形）**を LSP の convert-layout で変換すると、LYS1035 を出す本になる**。**実測: 変換前は無警告・変換後にエラー**＝**ユーザー操作が合法な本を非合法にする。**
⇒ ★★ **述語を直した＝「cell を持つか」ではなく「*その setting が加われる音楽の流れを section が自分で持っているか*」**。**cell の音楽は cell のもの**なので cell だけの section は持たず、**directives だけのヘッダも持たない**——**両方を*源*で捕まえる**ので変換器は届かない。**効く 3 形は全部「流れを持つ」側**で、黙る行としてそのまま留めてある。
⇒ ★★★ **番人は*一般形*で書いた**（`Convert_NeverProducesABookTheCompilerRefuses`＝「きれいな本を変換したらきれいな本になる」）——**個別形では気づけなかったから**: **検証器と書き換え器は、片方ずつ見ると両方正しく見えて食い違う。**
★ **射程は直後も変わらずゼロ**（1957 冊中 6 冊・全部 scratch のプローブ）。

⚠️⚠️ ★★★ **⒂ その追跡中に*別の*欠陥を踏んだので起票だけした＝§2 F ⒯。** **`.ly` 双子は「part の*後ろ*に置いた独立ヘッダ」でその section の音楽を落とす**（1 変数の対で切り分け済＝**違いは行の順だけ**。前に置けば `\key g \major c'4 c c c |`・後ろに置けば `\key g \major \key g \major` で音符ゼロ）。**ページと音高は無傷・`check` は無言**。⚠️ **`clef` の話ではない**——**`key` でも起きる**＝**本便の決定とは無関係の、前から在った欠陥。** ⇒ ★★ **混ぜずに別項にした**（「⒭ を追っていて見つけた」と「⒭ の一部」は違う）。

⇒ ★★★ **次の一手**: **§2 F ⒯**（双子の音楽落ち。**最小の対と対照が在るので着手しやすい**）／**§2 F ⒭ の残り**（part ブロック option 位置＝**`transpose` のスコープ漏れ**は出力が動く。**未決定**）／**§2 U8 の残り**（`grace { }` 本体の和音・休符／スラー・梁・タイ／注釈——**本便は 1 行も触っていない**）／**§2 の X/Y 側**。⚠️ **承認待ちは無い**（snapshot 3 枚は承認を取って再ベース済み・⑽）。

★ **開始時裏取り**: HEAD `66587eda`・**未 push 16**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6750 / 0 / 4 / 6754**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 点 158（非ゼロ 2）／OPEN: 0**・追跡 `.lys` 581・snapshot 231。
終了時: **commit 13 本**（**HANDOFF の stray CR 正規化 `72c8ba59` ／ form の 3 つ目の括弧 `be1c9047` ／ form の `:|*N` が MIDI に届く `26038c65` ／ importer が単純な繰り返しを form へ分解 `17165b87` ／ **⒫ 本体 `4e61cd8a`** ／ §1・§2・§3・RULES とアーカイブ `ffbf485a` ／ snapshot 3 枚の再ベース `82ae6037` ／ 数の閉じ直し `cf5d0acc` ／ **⒭ のヘッダ位置＝`LYS1035` `0eaa5d1b`** ／ ⒭ と §3 の書き換え `e5bb47cf` ／ 数の閉じ直し `0edf3f95` ／ **`LYS1035` の述語を「流れを持つか」に直す `0e1e1eb0`** ／ ⒯ の起票と終了時の数（この commit）**）・**未 push 29**・作業ツリー 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6773 / 0 / 4 / 6777**（**開始 6754 から +23＝新しい番人 2 本 ＋ 変換器の一般形 1 本**＝`RepeatStructureScopeValidatorTests` 13 ＋ `PartSettingInSectionHeaderValidatorTests` 7 ＋ `Convert_NeverProducesABookTheCompilerRefuses` 2 ＋ 既存 theory の行 1）・**台帳 748／588／190／24.714570188／158 中 2／`OPEN:` 0・追跡 `.lys` 581・snapshot 231 は開始時と同じ**（**枚数は不動・中身が 3 枚動いた**）。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけ**（定数は 1 つも足していない・`audit/magic_constants.csv` も不動）。⚠️ **間の push は無し**（`origin/master` は `be16a770` のまま）。★ **第304 が残した `C:\MyProj\lys-base304` の worktree は本便で消した。**
★★ **WSL ubuntu Release は `e5bb47cf` の木で Windows と同じ 6770 / 0 / 4 / 6774**（build 7 秒 / test 28 秒）——**§0 の両脚を読んだ。**★ **途中の `4e61cd8a` でも両脚を読んでいる**（そのときは snapshot 再ベース前なので両脚とも同じ 3 本が赤＝**赤の集合まで一致**）。⚠️ **最後の 2 commit（`0e1e1eb0` と本 commit）では ubuntu を回していない**——**動いたのは検証器 1 本の述語とテスト 2 ファイルとドキュメントだけで、Windows で全緑を確認済み。**
★ **§5 へ出した恒久知識は 3 つ**（全部 RULES §5.0 の末尾＝§5.1 の直前）: **⒜「X を場所 Y だけに移す」決定は *Y が X を綴れるか*を綴りごとに数えてから着手する**（⑷ の汎化）／**⒝ 綴りを 1 つ禁じたら、残る綴りの読み手を全部並べて同じ答えを出すか 1 つずつ測る**（⑸）／**⒞ 禁じた綴りを*書く側*（生成器と診断メッセージ）を grep する**（⑹⑺）。
⚠️ **計器は `scratch/p305/` に全部残っている**: **`sweep305.ps1`**（第304 の掃きに「増えた行が (p) の 1 本だけか」を足したもの）＋`sweep305.json`（⒫ 着手前）／`sweep305-final.json`（⒫ 完成木）／**`sweep-r.json`（⒭＝`LYS1035` の掃き・base は `cf5d0acc`。1954 冊で SVG/MIDI/XML は 0 冊・`check` は 11 冊で全部 scratch のプローブ）**・**`cmp.ps1`**（1 冊を head で描いて掃きの base ハッシュと突き合わせ、SVG が動いたときは `data-pos` を伏せてインクが同一かまで言う）・**`midinotes.ps1`**（SMF の note-on を数える。⚠️ **長い本では desync することがある——`grammar-2026-06-09` で 18 対 10 という嘘を出した**。**小さな probe 専用に使い、大きい本は MusicXML の小節数と音符数で見ること**）・**`base/`**（着手前の Release exe 一式）・probe 群（`a*` 陽性・`b*` 陰性・`c*` 3 つ目の括弧・`d*` `:|*N`）。⚠️ **`scratch/` は git 管理外。**
...
- **U6. ✅ 閉じた（第288・ユーザー報告 2026-08-29）＝`@rit` は「自分の次の演奏」で終わっていた**
  ...
  **しかもこの直しが弾かねばならない形そのもの**。**data を実在の 2 位置に直し、規則と対照の
  テストを 2 本足した。**
  ⇒ ★★★ **残っている問い＝「終端の綴りが無いこと」自体は要ユーザー判断**。**裸の `@rit` は
  「1 小節」以外を意味しようがない**（**`@rit(4)` のような長さも、`@!rit` のような終端も無い**）。
  **「section の終わりまで」を既定にするか、終端の綴りを足すかは、決めるのはユーザー。**
  ⚠️⚠️ ★★★ **その判断のために測った事実＝「閉じられなかった span」に言語は*3 つの違う答え*を
...
- **U8. ▶ 容れ物は全部閉じた（第298 ＋ 第299 ＋ 第300 ＋ 第301 ＋ 第302）。残るのは*本当に grob を要る*ものだけ＝`grace { }` の本体は*歩かれない*。起票の「注釈を運ばない」より広かった**
  ...
  **梁の有無という*質的*な形なので WSL で足りるが、座標を要るときは 2.26.0 を待つこと。**
  ...
  ▶ **残っているのは grob を要るものだけ＝⒜ の*和音と休符* ／ ⒞ 本体の中のスラー・梁・タイ ／ ⒟ 注釈の全族**
  ...
  **MIDI だけ動き XML は動かなかった本**）。
  ...
  - ★★ **その `item` の出どころは `measure.Items` ではなく*スパンの模型*** ——
    **`TieItem.Start/EndItemIndex`・`BeamGroup.Members[i].ItemIndex`・`SlurItem.Start/EndItemIndex`**。
    ⇒ **`ItemIndex` を名乗る模型型は 10 個**（`ArticulationItem`・`BeamGroup`・`ChordNameItem`・
    `DynamicItem`・`FiguredBassItem`・`MusicMarkItem.AnchorItemIndex`・`SlurItem`×2・`TieItem`×2・
    `VoiceColumn`・`GraceNoteItem.MainNoteItemIndex`）。**⒞ が要るのは Tie と Slur と Beam、⒟ が要るのは
    Articulation と Dynamic と MusicMark。**
...
> ## ✅ **⒯ 独立ヘッダは*どこに書いても*ヘッダ＝第306 で入った**（起票第305・実装 2026-08-31・第306・`10321fbd`）
...
>
> ## ▶ **⒰ 宣言が「ヘッダだけ」の名前を form が鳴らすと、ページはその調を armし双子は 1 つも書かない** ← **2026-08-31・第306 起票・最小の対と陽性対照つき・*言語の決定待ち***
> ⚠️⚠️ ★★★ **これは ⒯ を直した*結果*ではなく、⒯ の隣に前から在った欠陥**（⒯ の修正で*小さくなった*）。**混ぜずに別項にした。**
> ★ **形**（`scratch/p306/b5_header_only_audible.lys`）: **`section A { key g major }` だけが A の宣言で、どの part も A の音楽を持たない**。**`form main { ~A ~B }` で A を先に鳴らす。**
>
> | 読み手 | 出るもの |
> |---|---|
> | ページ | **調号を描き、それが B の小節に効く**（**陽性対照 `b8` ＝ヘッダを消した同じ本は調号を描かない**。グリフ 8 対 6） |
> | `.ly` 双子 | **調を 1 つも書かない**＝B は c major |
> | `lysc check` | **「No errors found.」で黙る** |
>
> ⚠️ **第306 の前は双子が 3 つ書いていた**（`\key g \major \key g \major \key c \major`）——**最後が勝つので実質 c major**。**つまり食い違いの向きは前から同じで、雑音だけが減った。**
> ⇒ ★★★ **先に決めることが 1 つ＝「0 小節しか鳴らない section は、次の section の小節に自分の調を armするか」。** **ページは *arm する*と答えていて、双子は *しない*と答えている。どちらが言語かはユーザーの決定**（§3 の section 境界の決定＝**調はリセット／音部記号もリセット／テンポはリセットしない**——**その決定は「小節を持つ section」しか想定していない**）。
> ★ **住所は 2 つとも判っている**: **arm する側に倒すなら `LilyPondExporter.AppendSection` の先頭の `if (!byName.TryGetValue(name, out var entry)) return;`**——**名前に header 登録が在るなら marker と header を出してから返る**ようにする（`_sectionHeaders` は*名前*で引く辞書なので、鳴る宣言が無くても引ける）。**arm しない側に倒すなら `MeasureCollector` の section-header レジストリが「小節を持つ宣言が在る名前」だけを登録するようにする。**
> ⚠️ **番人は `LilyPondStandaloneSectionHeaderTests.AHeaderOnlyNameWritesNoKey_AKnownHole` に*穴として*ピン留めしてある**——**今の（双子が書かない）挙動を主張しているので、決まった日にその 1 メソッドが「直す場所」を名指す**（§5.4）。
>
> ## ▶ **⒭ 残っているのは *part ブロックの option* 位置だけ＝とくに `transpose` のスコープ漏れ** ← **第303 起票／第305 が測り直して 3 つに分け、*ヘッダ位置は同便で閉じた***
> ✅ **閉じた半分**（2026-08-31・第305・ユーザー決定・`LYS1035`）: **section の「ヘッダ位置」に書いた `clef` / `octave` はエラーになった**（`PartSettingInSectionHeaderValidator`）。**`instrument` / `transpose` は元からエラー**で、直したのは LYS0030 の文面だけ。**射程ゼロ**（ディスク 1954 冊で当たるのは scratch のプローブだけ）。**詳細と「なぜ 4 つのうち 2 つだけが黙っていたか」は §3 の 2 行**。
> ⚠️⚠️ ★★★ **開いている半分＝*part ブロックの option* 位置**（`section A { m clef bass { … } }`）。**パーサは専用の腕を持つ**（`Parser.Sections.IsPartOption` ＝ transpose / octave / instrument / clef）**のに `GRAMMAR.md` の `PartBlock = Identifier , MusicBlock ;` には option がそもそも書かれていない**。**4 つ測った**（`scratch/p305/r4_partblock_options.lys`）: **`clef` / `octave` / `instrument` は黙って無視され、`transpose` だけが*読まれてスコープを間違える***——**section A の cell に書いたのに part 全体が D5 になった**（双子は `m = \transpose c d \relative c'` を part 全体に掛ける）。
> ⇒ ★★★ **`transpose` の漏れは性質が違う＝*出力が動く*欠陥**（他の 3 つは no-op）。**先に決めることが 2 つ**: **⑴ option 位置そのものを言語が持つのか**（GRAMMAR に無い＝持たないなら 4 つとも LYS1035 の兄弟で蹴る）／**⑵ 持つなら `transpose` のスコープを cell に閉じるのか。** ★ **測定は済んでいるので次便は数え直さなくてよい。**
> ⚠️ **番人は `PartSettingInSectionHeaderValidatorTests` に*穴として*ピン留めしてある**——option 位置の 1 行が「まだ黙る」を主張しているので、**閉じた日にその行が「直す場所」を名指す**（§5.4）。
>
