...
- ★★ **⒮⁶ 「建てない」の島の残り＝約 36,900 B／打鍵 0.82%**（**第455 が census を回し直した**＝Lab
  ...
  どれも器が呼び手へ出る）。⇒ **手が動く残りは約 20,800 B／打鍵 0.46%**で、うち 6,409 は下の ⒮⁸。
...
- **`docs/RULES.md` は 232,164 / 250,000 B・1,864 / 2,000 行**（第455 が規則を 5 本足した）。
  ⇒ **次に詰まったら、割るのではなく*規則そのもの*を畳む**（印のほうが高くつく）。

### 1.1 第455セッション（2026-09-21・YT-DELL2）

`/clear` 直後の新セッション。指示は「**HANDOFF を読んで着手**」で、**着手先は §1.0 ⒜ の ⒮⁷**（ユーザーが
選んだ・⒮⁷ と ⒮″ と ⒲ と ⒭′ を並べて訊いた）。★ **`-Start p455` の 1 コマンドで §0 が全部済んだ**
（HEAD `b22561d0`・未 push 25・full `sessions/p455/run1.trx` 8780 / 0 / 3 / 8783・台帳 851 点／
総和 22.584727806・snapshot 249・追跡 `.lys` 609・`-Archive 453` も自動＝moved 95 行 5,431 字）。
**裏取りは 1 つも赤を出さなかった。**

★★★★ **⑴ 起票が外したのは値段ではなく*直し方*で、しかも起票が「入れよう」と言った型は 9 便前から
木の中に在った。** ⒮⁷ は「6 軒 5,081 B／打鍵・**直し方は 1 つ**＝`OneOrMany<T>` 相当の struct を
`GetValueRefOrAddDefault` で」と書いていた。**正解は軒ごとに 4 通り**で、**2 軒は触ってはいけなかった**。
そして `Svg/Layout/ItemColumn.cs` は第446 が **「⚠️⚠️ THIS TYPE EXISTS IN ORDER NOT TO BE ALLOCATED」**
と書いた 0／1／多の struct で、**⒮⁷ の 6 軒のうち 2 軒の 1 ファイル隣に座っていた**（spacing rules の
3 本は既にそれを受け取っている）。⇒ 規則 2 本を RULES §5.0 へ（**17例目**・**採用済みの代案を grep する**）。

★★★★ **⑵ 島の最大 8,408 B／打鍵は「器」ではなく「鍵」の問題だった＝*地図ではなく配列*。**
`MeasureLayouter` の `Dictionary<Fraction, List<MusicItem>>`（:302 の器 3,900＋562・:319 の値 2,511＋1,435）は、
**4 か所の読みが全部 `TryGetValue(timings[k])`**（`timings[0]`／`[i-1]`／`[i]`／`[^1]`）。`timings` は
`CollectAllTimingsForMeasure` が `Sort()` して返す**昇順**なので、**カーソル 1 本で列番号が付き hash は 0 回**。
⇒ `ItemColumn[timings.Count]` 1 枚（約 1,200）に置き換えた。⚠️ **おまけに、`PruneSpacerOnlyOnsets` が
既に落とした onset を地図は*なお*バケツに入れていた**——建てて、詰めて、誰も訊かない。規則は RULES §5.3 末尾。

★★★ **⑶ 4 軒・8 ファイル・A/B は −13,286 B／打鍵 0.294%（会計 13,411 の 99.1%）。**
内訳は **⑴ の配列化 8,408**／**`wishLefts` を `ItemColumn` の 2 枠目に 3,327**（census の **max が 2**＝
1 枠では 74.2% を取り逃す。3 本目は list に spill し、その腕は毒 11 が 4 赤で押さえた）／
**`MeasureContentKey` の side table を `Hash64[]` に 2,312**（**器が消える**＝読みは `hc.Add` の fold だけで、
`Hash64` は ulong 1 個＝置き換える参照と同じ 8 バイト。⚠️ **鍵の*値*は動く**が opaque な cache 同一性）／
**`ArticulationEngraver` の slur bucket に `OneOrMany<T>` 538**（one 85.3%＝一覧で唯一の教科書例）。
**ただ乗りで 1 要素配列 3 つ**（`new[]{ownPrev}`・`new[]{firstItem}` × 2）も消えた＝census には映らない分。

★★★★ **⑷ 予測の*方向*は自分で判定不能にした。** 会計は A を「flat な CSR（`MusicItem[]`＋`int[]`）」で
組んだが、実装は `ItemColumn[]` を選んだ。実測 −13,286 は**書いた会計 13,411 に対し 99.1%＝下**、
**実装した設計の会計 12,991 に対し 102.3%＝上**。⇒ **第451 は `prediction.txt` に AMENDED 節を
（実装中・計測前に）足していた。私は足さなかった**＝450 上・451 下・454 上に続く 4 例目を自分で潰した。
規則は RULES §5.0。

★★★★ **⑸ 毒 11 本中 10 本が予測どおり。予測外の 1 本は「4 つ目の顔」ではなく*網の穴*で、
それを割ったのは「もう一方の母集団」だった。** 赤は CSR の 3 本（207／185／50）・`ItemColumn` の
2 枠目（50）・spill（4）・`Hash64` の 3 本（7／1／1）・`OneOrMany` の単一（5）。緑を予測して緑は 1 本
（2 枠の**順序**を入れ替える対照＝第446 の「max と mean は順序に盲目」）。**予測外は毒 10＝
`OneOrMany` の overflow（1 小節に立つ 2 本目以降のスラー）を捨てても 8,780 本全緑**。
⇒ 計器は**生きている**と答えた（コーパス overflow 4,682 回／run・非先頭を選ぶ 22 回／スイートは
multi 36 回のうち **36 回**が非先頭）。⇒ **同じ毒を実コーパスに当てると 16 行動いた**
（`Boogie Oogie Oogie.lys` の 8 ページ × 両側）。⇒ ★★★ **絵は動く。観測者が居ないだけ**＝起票 **⒮⁹**。
⇒ **緑の毒の切り分けは「4 つの顔のどれか」ではなく*順番***（事後条件 → 計器 → **もう一方の母集団** →
下流の不等号）。規則は RULES §5.4 末尾。

★★★ **⑹ その網を 2 本書いて、2 本とも取り下げた。** ⑴「2 冊の差」で書いたら**毒の下でも別の理由で
差が残って緑**（第453 の「相対の網」の再演）。⑵ 絶対の主張（同じ図形 2 つは同じ高さ）に直したら、
**スラーを 1 本も置かない対照でも accent が 2.67 と 8.665 に開いた**＝**割れていたのは計器のほう**
（§5.0「新しい計器の最初の食い違いはまず計器を疑う」）。⇒ **主張しない網を出荷するより、
穴を「起票＋証拠」で渡すほうが安い**（Lab `SlurBucketCoverageTests.cs.txt` に 2 本とも残した）。
規則は RULES §5.4 末尾。

★ **⑺ 出力は同一。** 実コーパス 231 冊 × 8 打鍵の**全ページ SHA-256＝5,824 行・0 差**（p451／p439 と同じ絵）。
full は開始時と同じ **8780 / 0 / 3 / 8783**＝**網は 1 本も増えていない**（⒮⁹ は起票のまま）。
