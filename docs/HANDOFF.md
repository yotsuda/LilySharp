...
**⑴ 普通のスラー採点器が、タブ譜の*自分の枠*で走る**（`ly/engraver-init.ly:1248-1258` が `Stem.details` を全部 0 にする理由を「`slur::calc-control-points` への影響を最小化するため」と*自分で書いている*）／**⑵ そのあと制御点 4 点を `staff-space × direction × 0.35` だけ数字側へ平行移動**（`scm/tablature.scm:144-157`）。
★★★ **「タブ譜の枠」は縮尺ではない**——**LP は `staff_space_` を 6 か所で掛けている**: **base attachment の持ち上げ `dir·0.5·ss`（`:557`）・格子の刻み `ss/2`（`:798-801`）・minimum-length（`:729`）・height-limit（`:714`）・符尾 extent の widen（`:747`）・五線からの逃げ 2 か所**（`:650`/`:655` と `slur-configuration.cc:61`/`:69`）。**それ以外（`slur-details` の重み・音符列の extent・線の太さ）は絶対量で、そのまま通る。**
⚠️⚠️ ★★★ **4 本線のタブの*線*は奇数位置**（3, 1, −1, −3）。**5 線の述語を使い回すと「弦を隙間、隙間を弦」と答える**ので、`EngravingDefaults.OnStaffLine` に線数を取る版を足した（**5 では既存と全位置で一致することをテストにした＝五線側が no-op である証明**）。
★ **`BuildTabSlurLayout` が投げ捨てたのは発明そのもの**: **`0.36 × FretFontSize + 0.1` で頂点を作る対称三次・向きは常に上**。**発明したアーチに LP の 0.35 を掛けても LP の曲線にはならない**（§5.2.1 の「発明を 2 つ目の発明で覆う」）。

⚠️⚠️⚠️ ★★★★ **⑵ 対は「両向き」では足りない。*鏡像*でなければ何も主張しない。** **最初に作った 2 小節は 1 小節目が弦 2,1,1,2・2 小節目が 4,3,3,4** で、**確かに上向きと下向きの対**だが——**1 小節目の*端*は内側の弦、2 小節目の端は外側の弦**なので、**LP は無関係な 2 つの数を返し、対称性については何も言えない**。**自分で書いた「鏡像である」テストがそこで落ちた**（`-(-2.043326) ≠ 1.043326`）。⇒ **2 小節目を 3,4,4,3 に直すと位置は (1,3,3,1) → (−1,−3,−3,−1) の厳密な反転になり、LP は `1.043326 / 2.074551` に対して `−1.043326 / −2.074551` を返す。** ⇒ ★★★ **「両側を足す」（§0 の教訓）の一段深い版＝*足した両側が同じ量の符号違いになっているか*を確かめること。** **なっていなければ、片側に吸い込まれた定数は隠れられる。**

⚠️⚠️⚠️ ★★★★ **⑶ 掃きが見ていた絵が違った——そして*それは 20 便前から `RULES.md` に書いてあった*。本便でいちばん高くついた失敗はこれで、教訓は掃きの作法ではなく*読む順番*のほう。**
★★★★ **`RULES.md:1111-1117`（第298・2026-08-30）が同じ罠を同じ言葉で記録している**（「`lysc svg` は既定で*最初の* score ブロックしか描かない・ユーザーの本は `staff` の score で開くので既定の掃きは tab を見ない・`--combined` で取り直したら MOVED」）。**私は掃きを組む前にそれを引かなかった。** ⇒ **§5.2 の「『見つけた』と思ったら、まず §2 を grep すること」の掃き版を `RULES.md` に足した＝「掃きを書く前に『掃き』を引く」。**
**`lysc svg <book>` は*最初の* score block しか描かない**（`svg --help` が `--score <name> … (default: the first)` と書いている）。**ユーザー実コーパスの本はたいてい `score main { staff … }` を先に、`score main "both"/"tab"` を後ろに置く**ので、**第1稿の掃き（`tab` を書く 759 冊・54 分）はタブの絵を 1 枚も見ていなかった**——**タブが既定になっている本は 314 冊中 `wrongfret.lys` 1 冊だけ。** **`MOVED 10`という*もっともらしい小さな数*が出て、そのまま報告しかけた。**
⇒ **`--combined`（全 score を 1 枚に積む）で取り直した**。**母集団は「`tab` を書き、どこかに `(` がある本」515 冊＝SAME 493 / MOVED 22**。⚠️ **その途中でもう 1 つ踏んだ**: **最初の述語は「音符に続く `(`」の正規表現**だったが、**`g,4\2(` のように弦番号を挟む音符に当たらない**ので、**fixture の `test/tab-slur-pinned` も `test/tab-grace-slur` も母集団から落ちていた**。**上位集合（素の `(`）が正しい母集団**——**多め側に外れる述語を選ぶこと。**
★ **射程の実体**: **ユーザー実コーパス 7 冊**（`Endless Story` / `Sweet Child Of Mine` ×2 / `wrongfret` / `君の恋人になったら` / `奏（かなで）` / `夕暮れ沿い`）・**追跡コーパス 2 冊**（`test/tab-grace-slur` と本便が足した `test/tab-slur-pinned`）・残りは scratch と backup の複製。**対照は非 tab 本 27 冊が全部 SAME**（`SlurScoringProblem` に `staffSpace`/`staffLineCount` を通したので、既定 1.0 / 5 が本当に元の式と同じかを問うのはこちら）。⚠️ **314 冊のうちスラーを書くのは 11 冊しかない**——**「93% がタブを書く」と「スラーを持つ」は別の数。**

⚠️⚠️ ★★★ **⑷ 台帳 5 点、そして残差が全部 1 つの既決に帰着することを*式で*示した。** **`slur.tab.{up,down}.{string-to-attachment,attachment-to-control}` ＋ `slur.tab.span`**（プローブ `audit/lp-geometry/probes/tab-slur.ly`・.lys 側は `TSL`）。**単位はタブ譜自身の spaces**（`beam.quant.tab.*` が決めた約束）。
★★ **対を「生の高さ」と「採点器の立ち上がり」に割ったのが効いた**: **`string-to-attachment` は数字のインクを含むので Lily# の大きい数字（`FretFontSize 3.0`・§3 で既決）を丸ごと拾う＝`0.722000 − 0.393326 = 0.328674` に 9 桁一致**／**`attachment-to-control` はそれが届かない量**で、**残った `0.073124` は LP 自身の高さの式に Lily# の span を入れると `0.072720` が出る**（残り 0.0004 は indent の頭打ち）。**その span も `slur.tab.span` として測ってある（+0.655403）。** ⇒ ★★★ **生の高さだけを載せていたら、「間違った曲線」が「既知のフォント差」の中に隠れられた。**
★ **番人**: **台帳 5 点 ＋ `TabSlurDirectionTests`（鏡像と 4 本線の線位置）＋ snapshot `test/tab-slur-pinned`**。**毒の効き方**: **向きを常に上に戻すと 2 小節目の主張が落ちる／0.35 を外すと台帳 4 点／`staffSpace` を通さないと格子と持ち上げが変わって台帳が全部動く。**

⚠️ ★★ **⑸ 起票の「同じ棚で一緒に片付くもの 2 つ」は、どちらも着手する物が無かった**（**着手前に測った＝§5.0**）: **⒜ `PhrasingSlur.stencil = ##f` は空振り**——**Lily# に phrasing slur という grob は無い**（`(`/`)` 1 種類だけ。`SlurPairingScanner` が「one bar of music quietly loses its phrasing」と書いているのはこの `(` のこと）／**⒝ `\hideSplitTiedTabNotes` は U4 で既に決着**——**LP の既定は改行先のタイ先を*括弧付きで刷る*が、U4（ユーザー決定）で Lily# はタイ先の数字を*どのモードでも刷らない*ので、`\hideSplitTiedTabNotes` を当てた状態そのもの。** ⇒ **残るのは「行頭の括弧付きだけは出すか」という読み手の判断で、欠陥ではない**（§2 T1 に書いた）。

⚠️⚠️ ★★★ **本便が触っていないのに dirty な木がある。** **`README.md`（+123 行）と `samples/README.md`（+1 行）が、§0 の時点では clean だったのにセッション途中で変更された**——**中身は人の手が入った散文**（`samples/README.md` の canon-in-d 行は行の途中で上書きされた形＝編集途中）で、**`manual-beam-demo.lys` という*まだ存在しないファイル*への行も足されている。** ⇒ **ユーザーの手直しと判断し、1 バイトも触らず、commit にも入れていない**（§5 の[手直し尊重]）。⚠️ **本便は A/B の base exe を作るのに `git stash push` / `pop` をしており、その窓にこの 2 ファイルも入って出た**（pop は clean に成功）。**次便が「誰が書いたか分からない差分」を見たらこれ。**

⚠️⚠️⚠️ ★★★★ **⑹ 便の後半＝T4 を閉じた。ユーザー指示は第315 ⑹・第317 ⑸ と同じ形**（「このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）。
★★★ **T4 を選び、他を選ばなかった理由**: **T4 は `scratch/` に対する*読み取りだけ*なので、§1 を数ごと閉じた後でも 1 つも数を動かさない／新しい §0 基準値を要らない／LP の起動手順・コーパスの住所・4 点がこの便の文脈に載っている。** **そして*最大の未回収リスク*だった**——**第317 は 109 冊を書き換えて 1 冊しか確かめておらず、以後のどの便もその上に基準を置く。** ⇒ **T3（error 11 冊）は却下**（**ユーザーの生きたコーパスに*書き込む*作業で、しかも本便中にユーザーは同じ repo のファイルを編集している**——**⑸ 参照。加えて論理的に T4 の後**）。**彫版側（U8c / U8b / U8 / X-Y / C⑴ / G）も却下**（**どれも新しい測定キャンペーンで、インクを動かした直後の清潔な §0 から始めるべき＝第317 が T1 に当てたのと同じ判断**）。
★★★★ **答え: 移行は意味を変えていない。むしろ*既に壊れていた本を見えるようにした*。** **全数（112 冊）で音符を落とした本 0・`volta` を落とした本 0・拍子が動いた本 1（`ABC` の +5＝意図的な修復）／小節数を LP と突き合わせた 20 冊のうち 18 冊が厳密一致、外れた 2 冊も*移行前のほうが LP から遠かった*。** **詳細と計器は §2 T4 に全部書いた**（§5.2.1② ＝ ここに 2 つ目の綴りを置かない）。
⚠️⚠️ ★★★ **⑹ でいちばん一般的な教訓は「三者比較」**: **第1稿は「手書き `.ly`」対「移行後 `.lys`」を数えて拍子欠落 13 冊・記号欠落 60 冊超を出しかけた**——**が、それは*移行のせい*だとは言っていない。** **`移行後 − 移行前` と `移行前 − .ly` は別の問い**で、**混ぜると私自身の古い変換の傷が第317 の移行の傷に見える。** ⇒ **「A は B と違う」を測る前に、B が*いつから*そうだったかを測ること。**
★ **副産物 2 件を起票した**（直していない）: **§2 T5＝曲頭の反復開始記号を Lily# は刷り LP は刷らない**（**LP の規則は最小対で*構成的に*決めた。射程 23 冊・snapshot 6 枚**）／**§2 T6＝移行が可視化した、それより古い `.ly` → `.lys` 変換の負債 3 種**（**LYS2006 が 28/112・拍子欠落・記号欠落**）。⚠️ **T6 ⒞ の数え方はまだ信用できないと*明記して*起票した**（旧 `.lys` の `section body` を記号に数えてしまう）。

★ **開始時裏取り**: HEAD `e36495fd`・**未 push 23**（**第312〜第317 の 23 本。`origin/master` は `925cab98`**）・木 0・未追跡 0・Core 0 エラー 0 警告（solution 全体で 20＝Tests の analyzer）・**Windows Debug 6818 / 0 / 4 / 6822**（**第317 の終了時と一致**）・追跡 `.lys` 585・snapshot 235・台帳 748 点（exact 588 / ss 非ゼロ 190 / 総和 24.714570）・`gh run list` の `origin/master` は緑。
終了時: **commit 3 本**（**この commit を含む**）・**未 push 26**（**開始時 23 ＋ 本便 3**）⚠️ **T4 は `scratch/`（git 管理外）しか触っていないので、テスト・fixture・snapshot・台帳の数は ⑹ で 1 つも動いていない**・作業ツリーは **README.md と samples/README.md の 2 件だけ**（**⑸ のとおりユーザーの手直し。本便の物は 0**）・未追跡 0（**`scratch/p318` は git 管理外**）・Core 0 エラー 0 警告・**Windows Debug 6826 / 0 / 4 / 6830**（**+8＝台帳 5 点 ＋ `TabSlurDirectionTests` 2 本 ＋ snapshot 1 枚**）・**追跡 `.lys` 586**（**+1＝`test/tab-slur-pinned`**）・**snapshot 236**（**+1＝同じ本。既存で動いたのは `test__tab-grace-slur.svg` 1 枚だけ**）・**台帳 753 点**（**exact 588＝増減 0・ss 非ゼロ 195・総和 26.173568＝+1.458998 で、その全部が新しい 5 点**）。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけ・件数は 58/52/115＝225 で不動**・**`audit/magic_constants.csv` は 914 → 923**（**+9＝`staffSpace = 1.0` などの新しい literal**）。⚠️⚠️ **WSL ubuntu Release は取れない**（**この機械に WSL が無い＝第313 ⑶**）——**本便はインクを動かしているので、`push` 後に `gh run list` で読むこと。**
⚠️⚠️ ★★★ **計器は `scratch/p318/`**（**git 管理外**）: **`tabslur-dump.ly` と `dump.txt`（正典 2.26.0 の grob dump＝TabStaff 素／transformer を外した対照／普通の五線の 3 冊）・`sweep318.ps1`（第1稿＝*最初の score しか見ていない*。残してあるのは ⑶ の教訓の証拠として）・`sweep318b.ps1`（第2稿＝`--combined`。`-Only slur|paren|pred|rest`）・`sweep/` と `sweep-b/`（動いた本の base/head 対）・`exe-base`/`exe-head`（A/B。base は `git stash` で作った）・`ts.lys`/`tsl.lys` と `run{1..6}.trx`。**
