...
- **★ `repeat unfold N` の中の `@mark` は 1 度しか刷らない**（⑶ の位置 de-dupe の帰結）。**LP は unfold を*展開*するので N 度刷る**。**ユーザーの実コーパスに「unfold の中の mark」は 0 冊**（`repeat percent` は body を 1 度しか歩かないので無関係）。**要ユーザー判断で §2 U7 に起票した。**
- **★ 容器の中で消えていたあいだ、診断は 1 行も出ていない**（**両 exe の `lysc check` は 1603 冊バイト同一**）。**「描かないなら位置つきで言う」は `@!X` 族には在って、この族には無い**（§2 U6 の ⒜ が「無言で消える」を最悪と名指したのと同じ形）。
（以下、前便からの一覧はそのまま）

⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 の ▶ は 0 のまま、台帳の `OPEN:` も 0。** ★ **手元で証拠が一番揃っているのは §2 U7 の残り**＝**unfold の中の mark を N 度刷るか 1 度にするか**（**LP の答えは N 度・射程は実コーパス 0 冊・決めるのはユーザー**）。★ **次に読み手の目に触れそうなのは §2 U4 の残債＝タブの `as full` に連桁が無い**（**ユーザーは「符尾・連桁も含めて全部」と決めている**）。★ **ユーザーの本で今 LYS4018 が出ているのは `Untitled-6.lys` の 1 冊だけ**（第288 便の `@rit`）——**本便はその本の*絵*を動かしていない**（掃きの MOVED 45 に入っていない）。
...
終了時: **commit 2 本**（`684d5a3c` code ＋ **この 1 本＝§1 の回転と §2 U7**）＝**未 push 17**・木 0・未追跡 0。**suite 6566 合格 / 0 失敗 / 4 skip / 合計 6570**（開始 6563 から **+7**＝**theory 5 ケース ＋ fact 1 ＋ snapshot 1**）。Core 0 エラー 0 警告。**追跡 `.lys` 580 冊**（**+1＝番人の本**）・**snapshot 230 枚**（**+1・書き換え 0 枚**）。**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188＝すべて不動**（**この便はどの点が読む距離も動かしていない**）。
...
- **U7. ✅ ほぼ閉じた（第293・ユーザー報告 2026-08-30）＝容器の中のリハーサル記号が黙って消えていた**
  ...
  *statement アーム*で作られる唯一のマーク族**で、**そのアームは「音符の*兄弟*として walk が
  ...
  ⇒ ★★★ **残っている問い＝要ユーザー判断（2 つ）**:
  **⒜ `repeat unfold N` の中の `@mark` を N 度刷るか 1 度にするか。** **いまは 1 度**
  （**位置 de-dupe の帰結で、「1 つの書かれた印は 1 つの印」という Lily# の既存の規則と揃っている**）。
  **LP は unfold を*展開*するので N 度刷る。** **射程はユーザーの実コーパス 0 冊**
  （**unfold の中に mark を書く本が無い。`repeat percent` は body を 1 度しか歩かないので無関係**）
  ——**だから「どちらが正しいか」ではなく「どちらが読み手にとって正しいか」の問いで、決めるのはユーザー。**
  **⒝ 消えるとき言うか。** **この族は容器の中で 200 便のあいだ無言で消えていた**
  （**両 exe の `check` が 1603 冊バイト同一＝計器は何も言っていない**）。**`@!X` 族は
  LYS4018 で*位置つきで*言う**（§2 U6 ⒜ が「無言で消える」を最悪と名指した）。**同じ扱いを
  この族にも与えるかは、まず「消えうる形がまだ在るか」を数えてから。**
