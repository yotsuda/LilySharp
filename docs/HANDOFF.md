...
### ▶ 次のセッションの最初の一手 ＝ **譜間 merge_springs（行頭 spring の平均化）を移植する**——**点は既に開いている**（`line-start.time-to-first-note.{tab-concert,tab-keyed}`＝両方 +0.400000000）。LP の行頭 spring は `Spacing_spanner::breakable_column_spacing`（`spacing-spanner.cc:478-517`）が**譜ごとに 1 wish を集めて `merge_springs`（`spring.cc:104` ＝ ideal の単純平均・min は max・床 min+0.3）**。**前置の最後の grob が譜ごとに違うときだけ効く**——記譜譜=TimeSignature（semi-shrink 2.0→ideal 8.82）／tab 譜=Clef（minimum-fixed 5.0→6.0 が共有 min_dist 床で 8.02）→平均 8.42＝**LP 3.300000**。⚠️ **通常譜 2 段では 3.700000 のまま**（wish が等しい＝平均は自分自身）＝**だから既存の多段譜の点では見えなかった**。Lily# は `SpacingRules.FirstNoteSpring` の**系全体 1 本**で per-staff wish が存在しない＝+0.4。**着手前に測るもの**: Lily# の TAB clef ink（LP は原点 0.8・ext (1.0 . 3.6)＝G clef 2.565 より**広い**——LP の TIME 5.12 はこの union から。Lily# の `MaxClefWidth` は tab を skip＝**Clef 列の staff 集合も別の非忠実**で、同じ族）と共有 min_dist。**出力は tab/ossia を含む全スコアで動く**（第1音が ~0.4 左へ＝snapshot 再ベース＋ユーザー承認が要る）。次点: ② break-align 描画 walk の純構造化（sharedKeyX/sharedTimeX の手組み max ループ→`SolvePrefixColumns` 消費へ。値は一致済＝出力不変・ただし予約側は score モデル＋measure 走査、描画側は `ResolveKeySignature`＋`GetSystemStartKeyChange` と **key 解決経路が別**——この解決経路の統一が本丸で、片方だけ挿げ替えると多分岐で壊れる。急がず focused session で）。✅ **widest-key の譜集合（旧 ▶）は本セッションで完了**（下記ブロック・TKC/TKT）。残る忠実化キュー（続き×6 の監査＋本セッションで確定・いずれもアーキ不利なし）: **(0) 予約 key の staff 集合に ossia を入れる**（LP では ossia も KeySignature 群の一員。ただし stencil が **scaled**（OKM: ossia key ink 1.5558 = 2.2×magstep(−3)）で、予約も描画も KeySignature を持ち回っていて幅を表現できない＝今は両側とも除外で**一致**。**ossia 自身の key が全記譜譜より 2 本以上広い**対を起票してから）・**(0') `MaxClefWidth` の staff 集合**（LP の TAB clef は Clef 列に入り、しかも G より広い ext (1.0 . 3.6)。Lily# は tab を skip＝同じ族の非忠実。▶ の merge_springs と同時に測る）・ **(a) 同一譜 knee の実 ink seed**（LP の knee stencil 帯を dump してから＝発明回避）・**(b) `BuildSystemSkylines` の全譜 union**（LP は全譜・probe を対で起票してから・focused session 向き）・**(c) cross-staff beam 機能そのもの**（`TargetStaffIndex` を立てる producer が無い＝`IsCrossStaff` 到達不能。skyline 方針は `72905813` でピン済み）・mid-line clef change の origin（`8ef0044e` の教訓参照）。
...
- **修正**（`fc892032`）: `SpacingRules.ContributesToKeyColumnWidth(staff)` **1 述語**を予約（`WidestActiveKey`）と描画（`SharedRenderer` の共有 time 列 max ループ）の**両方が読む**形へ。tab-keyed **+7.450000000 → +0.400000000＝control と同値**＝対が Lily# 側でも恒等になった（＝譜集合が差の全てだった検算）。**既存 corpus は byte 不変・snapshot 0 枚移動**（最広 key を tab/text/ossia が持つ fixture が 1 つも無かった）。fixture `test/tab-part-key` を新設（PNG 目視済・snapshot 1 枚新規）。
