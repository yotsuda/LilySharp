...
最終更新 第405セッション（2026-09-18・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `16f5157a`・未 push 3・**full `sessions/p405/run1.trx` 8690 / 0 / 3 / 8693＝第404 最終と一致**・CI は `851e5538` の run が緑（それより前の 4 run は赤のまま・`16f5157a` 以降は未 push）・build 0 エラー・`Session-Check -Build -Test` 64 秒）、指示「HANDOFF を読んで着手」（途中でユーザーから 2 行＝**「この PC は処理がとても速い。処理時間はあまり参考にせず、処理の回数を重視して高速化を検討して」「同じ PC 上で計測した時間の相対的な量は、これまで通り良く考慮して」**＝Lab の CLAUDE-OPERATIONS §2 に置いた・以下の根拠は回数と割当を主に、同条件の相対 ms を従に書く）→ §1「次の一手」のうち ⒜ 実機確認はユーザー不在で不可・⒝ 群単位は前便が「実機の 2 行を見てから」と留めたので、**⒞ の R13⒧＝v2bow1k の `voices`（`CalculateVoiceCollisions` 毎打鍵 3.1 ms／7.5 MB・memo 無し）を採った**。道具は pwsh MCP（Bash 0 回）。**閉じたのは 1 本**:

★★★ **⑴ 先に計器で回数を取った**（`sessions/p405/Zz405Probe.cs.txt`＝`ElementCoordinator.ComputeVoiceOffsets` に一時カウンタ＝呼び出し回数・呼び手（StackTrace 3 段）・歩いた列数・filed した entry 数・段ごとの ms／割当・**commit せず `git checkout` で戻した**・Release・TC=0・6 打鍵・一声かけずに回した＝根拠は回数と割当）: **v2bow1k の edit 打鍵は `ComputeVoiceOffsets` を毎打鍵 2 回**＝⒜ spacing 側 `SpacingRules.VoiceCollisionShiftsOf`（`Voice[]` を鍵にした静的 CWT memo＝resume 後の `Voice[]` は毎 edit 新品なので、spring memo が組み直す 3 小節の `ApplyCrossVoiceColumnSpacing` に訊かれた瞬間に 1000 小節を丸ごと歩いて埋める）＋ ⒝ finishing 側 `LayoutEngine.CalculateVoiceCollisions`（memo 無し・全小節）。**合計 6,000 列（2 × 3,000）・15,082 KB・6〜10 ms／打鍵、filed した entry は 0**（この本には衝突が 1 つも無い）。plain1k（一声）は入らない・Δ=0 打鍵も入らない（whole-layout reuse）。結果 `sessions/p405/voice-collisions-before.txt`。

★★★ **⑵ 直し＝答えを小節単位にして、両方の読み手が同じ表を読む**（`078e503b`・Core 9 本＋新規 `VoiceCollisionTable.cs`・網 2 本・CHANGELOG・棚卸し 2 本は行番号のみ）。**`VoiceCollisionTable`**＝staff（`Voice[]`）ごとに 1 表・小節は初めて訊かれたときに解く（`VoiceCollector.CollectMeasure`＝1 小節の列 → `NoteCollision` → `VoiceCollisionEntry`（MeasureIndex・VoiceId・ItemIndex・XOffset・wipe・dot）＝小節単位の 1 綴り。`Collect(voices)` はその連結・`ComputeVoiceOffsets(voices)` はその和集合＝綴りは 1 つ）。spacing 側・ledger rods・skyline seed・beam frame は同じ静的 CWT の家から表を受けて `ShiftOf(measure, voice, item)` を読む（訊いた小節だけ解ける）。**finishing 側は `SystemLayoutCache` の新 store `VoiceCollisions`**（staff × system の slice・鍵は content slice と staffIndex だけ＝値が読むのはそれだけ・shifted hit は MeasureIndex を delta で振り直す）を同じ表経由で埋め、renderer の 3 表（offsets／wipes／dots）に file する。健全性の根拠＝**1 小節の答えはその小節の全 voice だけを読む**（LP `note-collision.cc:403-472 calc_positioning_done`＝moment ごとに 1 grob・`voice{}` span の強制向きも同じ小節から読む）。**後**（`voice-collisions-after.txt`）: **解いた小節 3・列 8**（spring memo が組み直す 3 小節ぶん・旧 2 × 1000 小節／6,000 列）・store 111 hit／0 miss（計器は 2 つの text を交互に打つので edited system も既知＝他の store と同じ顔・実 session では edited system の ~9 小節が表経由で解ける）・打鍵の割当 51,066 → 36,184 KB。**網 2 本**（`IncrementalCompilerTests`）: `LayoutMemo_VoiceCollisions_AreServedPerSystem`（二声 6 system の pinned 本＝毎 system に shift と dot adjustment・最終 system の edit＝full と同一＋store (5,0,1)）／`LayoutMemo_VoiceCollisions_AreReStampedWhenABarIsInserted`（第 1 system に両 voice へ 1 小節ずつ挿入＝各打鍵が full と同一（途中の voice 長が揃わない本も）＋store (0,5,1)）。**full `run2.trx` 8692 / 0 / 3 / 8695（+2）**・台帳 851 点／総和不変・snapshot 249 不動。⚠️ 踏んだこと 2 つ＝ユニゾンの 2 分音符は wipe を出さない（LP は merge-differently-* を跨ぐときだけ wipe・`note-collision.cc:254-318`＝両 head を重ねて描く）＝網は shift と dot で押さえた／`dotnet test --no-build` は古い DLL を回す（新しい網が「通った」ように見える）＝build を挟む。

★ **⑶ 汎化**: RULES §5.3 に「edit ごとに作り直される model instance を鍵にした静的 CWT は session memo ではなく打鍵ごとの scratch＝埋めた回数を数える」を置いた。**同じ顔の memo が 2 つ残る**＝`MultiStaffLayouter.RestCollisionsOf`（`Staff` 鍵・`CalculateRestNoteCollisions` の全小節走査）と `ElementCoordinator.RestDotOffsetsOf`→ ⑶′ で数えた。

★★ **⑶′ 第 2 便（同日・指示「このセッションが有利なら着手」→ 計器の型と文脈が載っているので ⑶ の 2 つを数えた）＝両方とも毎打鍵 1 回埋め・1 fill が 1000 小節を歩く（R13⒧ と同じ回数の顔）が、仕事は走査で解きではない**（`sessions/p405/Zz405Probe2.cs.txt`・結果 `rest-memo-fills.txt`・両 factory lambda に一時カウンタ＝`git checkout` で戻した・Release・TC=0・コードは 1 行も変えていない）: v2bow1k の edit 打鍵で **`RestCollisionsOf` 0.23 ms／336 KB**（小節ごと voice ごとに全 item の List を作る）・**`RestDotOffsetsOf` 0.15 ms／0 KB**、一声の本（plain1k・fingbeam1k）は 0.02 ms／0 KB と 0.00（pitched-rest の index walk だけ）、filed entry は 3 冊とも 0（休符なし）・Δ=0 は入らない。**打鍵（22 ms／36 MB）の時間 ~2%・割当 ~1%**。⇒ **測って保留**: 回数は 2 × 1000 小節／打鍵で R13⒧ と同じだが、per-system store に下ろすと shift 関数と store が 2 本ずつ増えて取れるのは 0.4 ms／0.3 MB＝着手は**要ユーザー判断**（「回数重視」の指示に照らして判断を委ねる）。

★ **⑷ 終了時**: commit 2 本（code `078e503b`＋docs `0ac738b5`）＋第 2 便の docs 1 本・**最終 full `sessions/p405/run4.trx` 8692 / 0 / 3 / 8695**（run3 と同数・第 2 便はコード無変更）・§7.5（対 `16f5157a`・スクリプトの出力）**Core `+` 392 行／REF 3（2 本は移動・1 本は新規＝slice が立つ locality の出典 `calc_positioning_done`）／OWN 0**（数値定数なし・LP 由来の式なし＝memo の配管と 1 計算の小節単位への畳み＝監査対象は「同じ答えか」＝和集合＝全体の綴りが 1 つ・incremental==full の網 2 本・full 同一）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。§7 3.5 は済（第403 を `-Archive 403`＝15 行 8,300 chars）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も・Lab は sessions/p405 と notes を commit 済み）。⇒ **次の一手**: ⒜ **R13⒝ の実機確認**（第404 ⑶ のとおり＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item（第404 ⑵′・実機の 2 行を見てから）／⒝′ frame 変更の `applyFrame`（実機で頻度を見てから）／⒞ 第403 の残り＝edit 打鍵の `prelim` 1.4〜2.5 ms・count loop の page DP 5.3 ms（R13⒦・census で hit 率）・診断 pass の残り（`AnnotationName` 14／`MeasureValidator` 10／`LyricSings` 6.5）・`RestCollisionsOf`／`RestDotOffsetsOf` を per-system store に下ろすか（⑶′＝数えた・1 × 1000 小節／打鍵だが 0.4 ms／0.3 MB＝**要ユーザー判断**）／⒟ LSP 起動直後の tier-0（ReadyToRun・`TieredPGO` の knob は packaging 側＝**要ユーザー判断**）／LP 双子が要る R7〜R11／R15（要承認）。
