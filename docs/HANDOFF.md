...
最終更新 第359セッション（2026-09-09）＝**入り方が違う**: ユーザーの問い「`.md` に mermaid のように lys フェンスを書いて VS Code 標準プレビューに楽譜を出せるか」から始まり、**別セッションが Lily# を編集中だったので調査だけ**（可能＝§2F F-export の経緯欄）→「標準プレビューに PDF export は無い→寄付プラグインを尊重する汎用 exporter は先人が撤退した領域なので作らない→Explorer から Lily# 自身の書き出しを」に収束→ユーザーが要件 4 つ（複数選択・形式サブメニュー・フォルダ選択・全 score）を出し、別セッション終了後に「HANDOFF に起票して着手」。**起票＝§2F F-export・同便で実装して閉じた**。製品は commit 済み（下 ⑸）・push はユーザー。**骨は 5**:

★★★★ **⑴ サーバ**: `lilysharp/export` に `Path`／`All`／`OutputDirectory`、応答に `OutputPaths`／`Warnings`（`LspProtocolDtos.cs`）。`Export` は文書か path かで木を作り（path は CLI と同じ `ExpandUsings(text, tree, path, readFile)`・両方あれば文書が勝つ＝未保存の編集を書く）、`All` は `RenderSpecParser.FindAll` を回して `spec.ResolveOutputStem(stem)`＋拡張子（`ExportExtensions`＝CLI の既定）で書く＝CLI `--all` の命名。視覚形式は score 名（basename があればそれ・無ければ form 名）で `Choose`、midi/xml/ly は `Form = spec.Form`（`RunFormOutput` の写し）、**vsqx は Form を持たないので最初の score＋警告**、同じ名に解決する 2 つ目の score は skip＋警告、score 宣言 0 の本は 1 ファイル。書き込みは **`WriteExport` 1 軒**（preview の 1 score 経路も同じ関数・PNG 複数頁の `-pageN` 命名と toast の綴りは据え置き）。番人 **`Lsp/ExportRequestTests` 16 本**（Theory 6 形式・ly の form 差＝同じ staff で form だけ違う 2 score・vsqx 警告・score 0・開いている文書が勝つ・1 score 不変・構文エラーで何も書かない・path 無し／directory 無し／未知形式）。⚠️ 踏んだ穴: 裸の音楽を top-level に書いた fixture は LYS の error（「Music cannot go at the top level」）＝score 0 の本は part＋section＋form で書く。

★★★ **⑵ 拡張**: `src/exportBatchCore.ts`（vscode 非依存＝対象の選別 `lysTargets`・stem 衝突 `duplicateStems`・要約 `summarizeExport`・**node:test 12 本**＝manifest との一致も見る）＋`src/exportBatch.ts`（`registerExportBatch`・Explorer の `(uri, uris[])`／パレットは編集中の `.lys`（untitled は「先に保存」）・衝突は modal 警告「Export anyway」・`showOpenDialog({canSelectFolders})`・`withProgress` 通知＝**冊単位の確定バー＋キャンセル**・開いている文書は `textDocument` で送る・上書きは黙って（CLI と同じ）・完了 toast に Open Folder／Show Log・出力チャネルに 1 ファイル 1 行）。`package.json`: コマンド 7・`submenus` `lilysharp.export`・`explorer/context` に `resourceExtname == .lys`（group `7_modification`）・パレットは `resourceLangId == lilysharp`。`tsconfig.test.json` の include に core を足した。**踏んだ穴 2**: ⒜ `vscode.window.ProgressLocation` ではなく `vscode.ProgressLocation`／⒝ **Explorer からの初回は activate 直後でクライアント未起動**（コマンド起動が拡張を activate する）＝`whenReady`（`clientReadyPromise`）を 20 s 上限で待ってから `isReady` を見る。

★★★ **⑶ 実機（隔離 VS Code・ユーザーの窓と拡張には触らない）**: `scratch/p360/prep.ps1`（インストール済み拡張 `yotsuda.lilysharp-0.5.0-win32-x64` を temp に複製し out/・package.json・syntaxes・framework-dependent の server（`scratch/p360/server`）を重ね、`--user-data-dir`／`--extensions-dir`／`--remote-debugging-port=9222`・**`files.simpleDialog.enable` でフォルダ dialog を DOM に**）＋`drive.js`（CDP: alpha/beta を選択→右クリック→「Lily#: Export」→「Export as PDF」→dialog に path→Enter）→ **toast「Lily#: 4 PDF files written to …\out.」・`alpha.pdf`／`alpha-sub.pdf`／`beta.pdf`／`beta-sub.pdf`（137 KB 級・sub は form が 2 周ぶん大きい）**。⚠️ **1 度目は「Lily#: Export」がメニューに無かった＝Explorer が出てから拡張の登録完了（renderer.log「Completed initializing default profile extensions」）まで 9 s あり、その前に開いたメニューには寄付が無い**→ drive.js はその行を待つ。⚠️ Ctrl+click は CDP では選択を広げなかった（Shift+Down で代替）。`close.js`（`Browser.close`）で隔離側だけ閉じる。

★★ **⑷ 決めた小さな物**（§3 には入れない）: HTML 形式は後日（ユーザー）／ABC フェンスは足さない（abcjs 系が成熟・ユーザー同意）／汎用 Markdown→PDF exporter は作らない（`markdown.api.render` で寄付プラグイン込みの HTML は取れるが、client 側描画の拡張・非公式 API・headless Chromium・remote で崩れる＝vsc-print が撤退した領域）／`.md` の lys フェンス（`contributes.markdown.markdownItPlugins`＋`lilysharp/renderText`＋`markdown.preview.refresh` の再描画・フォントは `markdown.previewStyles`・snippet プリセット＝lilypond-book 型の `raggedRight`＋`indent 0`＋頁なし＋インク切り抜き）は**可能と結論したが未起票**・フェンス名は **`lys`**（別名 `lilysharp`・`lily#` は info string の慣例に反するので不採用）。CLI の `lysc pdf/png` に `--all` が無い非対称は F-export に残した。

★ **⑸ 数**: **開始時裏取り**（`tools\Session-Check.ps1 -Build -Test -Scratch p360`）: HEAD **`14b71903`**・**未 push 37**・`origin/master`＝`cf6edb6f`・木 clean・未追跡 0・build 0 エラー／Core 0 警告・**full 7522 / 0 / 4 / 7526**（`scratch/p360/run1.trx`＝第358 終了時と一致）・`gh run list` 緑（最新 34286219497）・台帳 810・snapshot 249・追跡 `.lys` 599・ディスク 15702。**終了時**: commit 1 本（Lsp 2＋Tests 1（新規）＋拡張 6（新規 3）＋CHANGELOG＋この HANDOFF）・未 push 38・木 clean・**full 7538 / 0 / 4 / 7542（`scratch/p360/run2.trx`・+16＝`ExportRequestTests`）**・**§7.5（`-DiffBase 14b71903`）Core `+` 0／LILYPOND-REF 0／LILYSHARP-OWN 0＝Core は触っていない（Lsp と拡張だけ・LP の式は 1 本も無い）**・台帳 810／snapshot 249／追跡 `.lys` 599 不動・拡張 `npm test` 83 本緑・`tsc --noEmit` 0。⚠️ **scratch は `p360`**（`p359` は第358）。⚠️ §7 3.5: 第357 のブロックを `Session-Check.ps1 -Archive 357` で ARCHIVE の先頭へ・第358 の経緯は下の見出しの下に残した。⚠️ Edit ツールの CRLF→LF 化は今回 0 ファイル（7 ファイル全部 bare LF 0 を確認）。

⇒ ★★★ **⑹ 次の一手**: **⒜ push はユーザー**。**⒝ インストール済み拡張への配布は済み**＝`tools/Deploy-Lsp.ps1`（`yotsuda.lilysharp-0.5.0-win32-x64` に `0.6.0+14b71903` の server 95 ファイル byte 一致・out/・package.json）→ **ユーザーは Developer: Reload Window**（それまで右クリックに「Lily#: Export」は出ない）。**⒞ F-export の残り**（HTML 形式・`lysc pdf/png --all`）と **`.md` の lys フェンス**（⑷）は起票を含めてユーザーの合図待ち→ **第 2 便で lys フェンスは起票・段階 ⑴ 実装済み（下）**。**⒟ 第358 ⑺ の並び（旗 reach 島の分離・`time` 再印字＋全音符の +1.0・⒞⒟⒠）はそのまま**。

✅ ★★★★ **第 2 便（ユーザー「シェル拡張は保留。`.md` の lys フェンス対応を起票して着手して」）＝§2F F-mdfence を起票し、段階 ⑴（拡張側＋`renderText` の静的 SVG）を実装・隔離 VS Code で実機確認・配布まで**。**⑴ 拡張**: `src/markdownFenceCore.ts`（vscode 非依存＝`isLysFence`（`lys`／`lilysharp`・大小無視）・`fenceKey`（sha1）・`fenceHtml`（pending／error／svg・XML 宣言を落として inline）・`FenceCache`（LRU 200）・node:test 9 本＋manifest の門）＋`src/markdownFence.ts`（markdown-it の `renderer.rules.fence` を包む・miss は placeholder＋`lilysharp/renderText {interactive:false}`・答えが揃ったら 120 ms 後に 1 回 `markdown.preview.refresh`）。`activate()` が `extendMarkdownIt` を返す。**LSP クライアントは遅延起動**（`startClient`／`ensureClientReady`＝`.lys` が開いているか・開かれたか・フェンス／export が要るまで .NET を立ち上げない。`onLanguage:markdown` を足したので必要）。`media/markdown-lys.css`（`markdown.previewStyles`＝`@font-face` 2 面を `fonts/` 相対で・`max-width:100%`・`.vscode-dark` で invert）・`syntaxes/lilysharp.markdown.injection.json`（`injectTo: text.html.markdown`・`meta.embedded.block.lilysharp`）。**⑵ サーバ**: `RenderTextParams.Interactive`（既定 true＝AI パネル・false＝hit-rect と system group 無し・`data-pos` は残る）・番人 `Lsp/RenderTextRequestTests` 2 本（viewBox は同じ）。**⑶ 実機**（`scratch/p360/prep3.ps1`＋`drive3.js`・`ws3/notes.md`＝lys 2 本・壊れた 1 本・js 1 本）: プレビューの入れ子 iframe（`#active-frame`）を読み、**最初の描画は pending 3 つ→ 6 s 後に svg 2 つ（viewBox `0 0 119.50 27.45`／`31.38`）＋壊れた 1 つは「Line 2, Col 1: Expected 'CloseBrace'…」と赤字でソース付き・js は素のまま・svg は列幅 277 px に縮小・`nh-hit` 0・`document.fonts.check('12px Emmentaler')` true・`text.music` 16**・出力チャネルの順は「activated → plugin installed → rendering → Starting language client → started」＝遅延起動どおり。`shot3.png` に窓ごと。**⚠️ 踏んだ穴 3**: ⒜ **`contributes.markdown` の入れ子は黙って無視される＝正しくは `"markdown.markdownItPlugins"` のドット付きキー**（mermaid 拡張の綴り・番人が入れ子を拒む）／⒝ プレビューの「Open Preview to the Side」は Lily# と markdown 拡張が activate する前に押しても開かない（exthost.log の `_doActivateExtension yotsuda.lilysharp` を待つ）・DOM の `click()` は効かず座標の mouse gesture で開く／⒞ webview target の `document` は host ページで、本文は `iframe#active-frame.contentDocument`。**⑷ 見た目の宿題＝段階 ⑵**: 紙の横幅（119.5 ss）で描くので 2 小節の例は右が空き、ダークでは白い紙が黒い帯になる→ snippet プリセット（F-mdfence ⑵）。**⑸ 数**: 終了時 commit 1 本（Lsp 2＋Tests 1（新規）＋拡張 8（新規 5＝core・plugin・test・css・injection）＋CHANGELOG＋この HANDOFF）・未 push 40・木 clean・**full 7540 / 0 / 4 / 7544（`scratch/p360/run3.trx`・+2＝`RenderTextRequestTests`）**・§7.5（`-DiffBase 14b71903`）Core `+` 0／REF 0／OWN 0・拡張 `npm test` 93 本緑・`tsc --noEmit` 0・台帳 810／snapshot 249／追跡 `.lys` 599 不動。**配布**: `Deploy-Lsp.ps1` 済み（manifest が変わるので **Reload は 2 回**）。

✅ ★★★ **第 3 便（ユーザー「別名は `lilysharp` ではなく `lily#` に」→「フェンスでは score を書けず form は 1 つ、を描いては」→ 私の懸念（score 禁止だと tab／chords 行／lyrics 行／grandStaff が書けない）→ ユーザー決定「score は任意・ただし 1 つまで」）＝別名の差し替え（commit `4e744f56`・配布済み）と、フェンスの「絵は 1 枚」規則（§3 先頭行）**。**形**: `RenderSpecParser.ImpliedScore(tree)`＝全 `part` を宣言順の `SingleStaffSpec`（clef・表示名／instrument・pedal は `staff NAME` と同じ読み）＋`ApplyEnsembleDefault`（Parse から切り出した 1 軒＝2 譜表以上なら part 名を大文字始まりでラベル）＋唯一の form（0 なら null＝section 宣言順）／form 2 個以上は理由付きで null。`SvgGenerator.GenerateForSpec(tree, spec, options)`。LSP `RenderTextParams.Fence`（既定 false）: score 2 個以上→エラー「declares N」・1 個→その score・0 個→ImpliedScore。拡張は `fence: true` を送る。**番人 `RenderTextRequestTests` +5**（**暗黙の score は `score tune { staff melody staff bass }` を書いた絵と byte 一致**／form 無しでも描く／form 2 個は拒否（名を列挙）／score 2 個は拒否／score 1 個はフェンス外と同じ絵）。実機 `ws3/notes.md` に score 無し 2 part のフェンスを足して 3 枚（viewBox 高さ 27.45／31.38／**34.88＝2 譜表・ラベル "Melody"／"Bass"**・`shot3.png`）。**⑸′ 数**: 終了時 commit 1 本（Core 2＝`RenderSpecParser`／`SvgGenerator`＋Lsp 2＋Tests 1＋拡張 1＋CHANGELOG＋この HANDOFF）・未 push 42・木 clean・**full 7545 / 0 / 4 / 7549（`scratch/p360/run4.trx`・+5）**・**§7.5（`-DiffBase 14b71903`）Core `+` 81／REF 0／OWN 0＝監査対象**: 読み直した結果、式は 1 本も無い（ImpliedScore は `staff NAME` の読みの写し・`ApplyEnsembleDefault` は Parse からの切り出し＝移動）・台帳 810／snapshot 249／追跡 `.lys` 599 不動。配布済み（manifest 不変＝Reload 1 回）。**⑹′ ユーザーの次の問い（回答済み）**: フェンス内の `pageBreak` は `break`、自動改頁は無しで段間は通常幅＝LP の `ly:one-page-breaking`（頁高さ＝中身）に写す→ 第 4 便で実装。

✅ ★★★★ **第 4 便（2026-09-10・ユーザー「着手して」）＝F-mdfence 段階 ⑵ snippet＝§3 先頭行**。**読んだら Core に既に在った**: `LayoutOptions.PageHeight = 0` が「中身で決まる 1 頁」で、段数採点（`LayoutEngine.cs:179`）・強制改頁の経路（`Pages.cs:386`）・溢れ検査（`:554`）が全部 `PageHeight > 0` の裏＝`one-page-breaking.cc:64-183` と同じ形。**足したのは 3 つ**: ⑴ `LayoutOptions.CropWidth`＋`LayoutOptions.Snippet = Default with { PageHeight = 0, CropWidth = true }`（REF: `one-page-breaking.cc` `One_page_breaking::solve`・`lilypond-book-preamble.ly:47 use-paper-size-for-page`・`define-paper-variables.scm:401-405 page-breaking`）／⑵ 幅の切り抜き＝`CreatePages` の単一頁経路で `pageWidth = MarginLeft + max(段の描画右端) + MarginRight`。右端は **`SharedRenderer.StaffRightEdges(score, system)` に切り出した 1 軒**（最終小節線の右端＋行末 courtesy の key／meter の幅＝描画が譜線を終える所と同じ読み）／⑶ 土台の経路＝`MeasureCollector.PaperBase`（`CollectDefinitions` の頭で `_meta.Paper = PaperBase`・`PaperPlanReader.Read(paper, base)`／`ReadReference(…, base)`）・`SvgGenerator.BuildLayout/CollectScore(tree, spec, paperBase)`・`SvgRenderOptions.Snippet`（LSP の fence が立てる）。**ragged と indent は触らない**（段 1 つは LP の規則で元から ragged・複数段は justify＝頁幅は行幅のまま・indent は楽器名の場所）。**番人 `SnippetLayoutTests` 6 本**（1 段は `StaffRightEdges`＋余白ちょうどに切れ高さは Default と同じ／`pageBreak` は Default で 2 頁・Snippet で 1 頁 2 段（B が 2 段目）／80 小節は Default で 2 頁以上・Snippet で 1 頁・段数同じ／複数段は行幅のまま／`paper { raggedRight }` が Snippet の上に重なる／Snippet と Default の差は PageHeight と CropWidth だけ）＋`RenderTextRequestTests` の fence は viewBox 幅 < 119.5。**実機**（`ws3/notes.md` に pageBreak の本を足して 4 枚）: viewBox 幅 **47.76／41.73／54.90**（紙は 119.50）・pageBreak の本は **119.50 × 68.56＝1 枚に 2 段（justify＝行幅のまま）**・プレビューは 1 staff space＝6 px の自然寸（`PIXELS_PER_STAFF_SPACE`・列幅で上限）＝`shot3.png`。**§7.5（`-DiffBase 14b71903`・作業ツリー）: Core `+` 227／LILYPOND-REF 3／LILYSHARP-OWN 0**（REF 3＝`one-page-breaking.cc:64-183 One_page_breaking::solve`・`lilypond-book-preamble.ly:47 use-paper-size-for-page`・`define-paper-variables.scm:401-405 page-breaking`。OWN 0＝式は 1 本も無い: `StaffRightEdges` は描画からの切り出し・幅は余白＋右端の和・`PaperBase` は土台の差し替え）。**数**: 開始時＝第 3 便終了時（`run4.trx` 7545／0／4／7549）→ **full `scratch/p360/run5.trx` 7550 / 1 / 4 / 7555（+6＝`SnippetLayoutTests`・赤 1＝APPROXIMATIONS の目録 stale → `LILYSHARP_UPDATE_DOCS=1` で再生成・再実行で緑）**・台帳 810／snapshot 249／追跡 `.lys` 599 不動（snippet は fence だけの経路＝Default の出力は byte 不変）・拡張 `npm test` 94 本・`tsc` 0。commit 1 本（Core 9＋Lsp 1＋Tests 2（新規 1）＋拡張 3＋APPROXIMATIONS＋CHANGELOG＋この HANDOFF）・未 push 43・配布済み（Reload 1 回）。

✅ ★★ **第 5 便（ユーザー「残債のうちこのセッションでやる方が有利なものがあれば着手」）＝第 4 便が残した「題名が音楽より広い本は題名が切り抜きからはみ出す」を閉じた**（この便の probe と `ws3/notes.md` がそのまま使える唯一の残債）。**形**: `HeaderBand.Width`＝行の advance の最大（`Build` の `Stack` が描画と同じ size／style で `fonts.Advance`）→ `CreatePages` の切り抜きは `max(header.Width, 段の右端)`＋余白。番人 `SnippetLayoutTests` +1（長い題名の本は `MarginLeft + header.Width + MarginRight` ちょうど・題名無しより広い）。実機 `notes.md` に長い題名＋composer の本を足して確認（viewBox 幅 **92.26**＝題名の advance＋余白・同じ 2 小節の音楽だけなら 41.7 級）。**数**: **full `scratch/p360/run6.trx` 7551 / 1 / 4 / 7556（+1・赤 1＝また APPROXIMATIONS の目録 stale → 再生成で緑＝Core の doc を触るたび出る門）**・§7.5（`-DiffBase 14b71903`・作業ツリー）Core `+` 238／REF 3／OWN 0（+11 行＝`HeaderBand.Width`・式無し）・台帳 810／snapshot 249 不動・commit 1 本（Core 2＋Tests 1＋APPROXIMATIONS＋CHANGELOG＋この HANDOFF）・未 push 44・配布済み（Reload 1 回）。**第 6 便は着手しない**: この便の道具で閉じられる残債はこれで尽きた（残りは HTML 形式（ユーザー保留）・`lysc pdf/png --all`（需要無し）・フェンス内の診断（需要待ち）・シェル拡張（ユーザー保留））。

## 以下は第358セッションの経緯

最終更新 第358セッション（2026-09-09）＝**入り方は第298〜第357 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。第357 ⑹ に「決定済み・未実装」は 0・⒟ は承認要だったので、**⒠ の並びから「`time none` の縁 ⑵⑶⑷」（第353 ⑹⒞・観測者 0 だが `scratch/p354/lp` の計器で測れる）を選び、3 つとも LP で測って閉じた**——**測ったら 2 つは起票と違う病名だった**（⑵「前半を梁で結ぶ」は*予測*で、実物は「凍った時計」＝§5.0 の「起票の難所は予測」の 9 例目／⑷「反復記号に接した cadenza」は反復とは無関係で加線域の旗の reach）、**そして双子の退行を 1 つ拾った**（第357 第 2 便の `| |` 規則が form の `|:` の前に `s1` を書く＝ユーザー本 98 冊の双子）。**製品 commit `6b0e8fec`（Core 10＋Tests 2＋CHANGELOG＋GRAMMAR／GRAMMAR_FOR_LLM＋APPROXIMATIONS／magic_constants 再生成）・push はユーザー**。掃き 923 冊 **MOVED 0**（`time none` を小節途中に書く本は 0）・台帳 810 不動・snapshot 249 不動。**骨は 6**:

★★★★ **⑴ ⑵＝小節途中の `time none` の梁＝LP の時計は*止まる*のではなく*凍る***（`scratch/p359/lp/pair.ps1`＝p354 の型＋`settings.ly` に NoteHead の X／staff-position `PROBEHEAD`・`-Cols` で ALLCOL も刷る）: **`c'8 d time none e8 f g a b c d e f4 g |` は LP 2.26.0 で 10 個の 8 分が 1 本の梁（PROBEBEAM 8.585→31.253）・Lily# は旗 10 本**。機構＝`\cadenzaOn` は measurePosition を*進めない*だけで値は残り、auto-beam の end 判定（`auto-beam.scm:82-90`・`consider_end` が読む `measure_position_at_start_of_timestep_`）は毎 stem その凍った値を訊く＝**1/4 で凍れば 4/4 の 8 分の梁はどこでも終われず（f4 まで 1 本）、1/2（第353 の probe）や小節線（fixture）で凍れば毎 stem 終われ＝梁 0**。**凍った値は cadenza の書かれた `\bar "|"` を越えて残る**（`midbar-8th-2bars.ly`＝`c'8 d time none e8 f g4 | a8 b c d e4 |`→ 梁 2 本 8.585–16.228／23.325–30.967・`midbar-4th-2bars.ly`＝1/2 で凍る対照は 0 本）。**形**: `MeasureBuilder._frozenPosition`（`FreezeOrThaw`＝`time none` が来た時の `_currentDuration`・metered `time` で 0・checkpoint に `FrozenPosition`）→ `Measure.UnmeteredPosition`（collector の組み直し 5 か所も運ぶ）→ **`BeamDetector.DetectBeamGroupsInMeasure` は `position`（実時間）と `Clock()`（凍った値）を分け、beat 判定は `Clock()`・pattern と recheck は実時間**（recheck は LP の `end_moment(i) − start_moment(0) + measure_offset_` の写し＝`beamOrigin`/`beamOriginReal`）。**senza の `TimeSignatureChangeItem` は梁を切らない**（property set・grob ではない）。**beat grid は最後の metered meter**（`beamingMeter`・`\cadenzaOn` は timeSignature を触らない・memo key も同じ）＝`BeamingPattern.Options.For(senza)` の OWN 腕は tuplet-span walk だけが通る（文言を直した）。**番人 `SenzaMisuraTests` +2**（10 member 1 本／1/2 で 0・2 小節の carry＋対照）。**ページの幅**: `midbar-8th` の cadenza 小節 Lily# 40.89＝LP 40.893（梁付き同士）。

★★★ **⑵ 双子の復帰＝`\cadenzaOff` も measurePosition を戻さない→小節途中で開いた cadenza の戻りは `\partial <小節長>`**（`midbar-8th.ly`: 無しだと "mid-measure time signature without \partial"＋"bar check failed at: 1/4"＋**c1 の中に自動小節線**（bars 40.893 / 50.795）・`midbar-8th-fix.ly`＝`\cadenzaOff \time 4/4 \partial 1` で警告 0・bars 40.893 / 52.295／3/4 復帰は `\partial 2.`（`-34-fixA/B.ly`・`\time` と `\partial` の順はどちらでも同じ＝mid-piece の partial は「小節長 − dur」＝0）。**形**: `EmitMusicStream` の `timeSinceBoundary` を field `_timeSinceBoundary` にして `EmitTime` が `time none` 到着時に読む（`_cadenzaOpenedMidBar`）→ 復帰の `EmitTime`／section 復帰の `\cadenzaOff \time 4/4` に `CadenzaReturnPartial(小節長)` を足す。小節線で開いた cadenza（fixture）は何も足さない（番人）。

★★ **⑶ senza の中の `partial`＝LYS2015（warning）＋双子は書かない**（`partial-senza.ly`: LP の `\partial 4` は凍った measurePosition を 3/4 に*動かし*、戻りで bar check 3/4 失敗＋a'1 の中に自動小節線＝別の本になる・Lily# は無効・無音だった）。`MeasureValidator` の bar 内 scan を item 順に（`senzaHere`）・`Diagnostic.cs` に `PartialUnderTimeNone`・`ArmPartial` は `_timeSenza` なら warning を積んで空。番人 +1（陽性対照＝metered では `\partial 4` が出る）。

★★★★ **⑷ 双子の退行＝form の `|:` の前に `s1`**（`formrep-plain.lys`＝`A |: B :| C`・LP は 5 小節（空の 5.51）・ページは 4）: 第357 第 2 便の「`|:` が scope を開かないなら空小節」規則が、form 展開の `|:`（`AppendRepeatBlock` が section-play marker の*前*に置く）を A の `|` と対にしていた。**形**: `EmitMusicStream` で `|:` の次が `SectionPlayGreen` なら pair しない（`opensASectionPlay`）。**射程（`scratch/p359/sweep-ly.ps1`・`|:`か`time none` を書く 146 冊＝追跡 23＋ユーザー 123・base=`539f2084` Release）: MOVED 104（追跡 6＋ユーザー 98）・消えた spacer 139・差分は `s1` トークンの除去だけ（全対を Compare-Object で検算・2 冊は同じ行の `] s1`／mark 行の末尾）**。第357 の commit は未 push なので origin には出ていない。番人 `EmptyBarExportTests` +3（Theory: `A |: B :| C`／`:|:`／2 つの反復）。

★★ **⑸ ⒠⑷「反復記号に接した cadenza の −2.29」を分類した＝反復ではなく*加線域の旗の reach***（§2 H の先頭に起票・`flag-{low,high,metered,metered2}.lys`・`ledger-beamed.lys`）: 反復を外した `flag-low`（`g8 a b c d c b a` を加線域で・stem 全部 up）でも −1.39＝LP は 8 対とも 2.567、Lily# は上昇 2.57／**下降 2.11**（最後→小節線 1.27 対 2.567）。1 オクターブ上（譜内・下降側は下向き符尾）は exact 19.313・拍のある本の旗付き 8 分→低い音 4 形は exact（gs 1/8 では 8 分の理想 2.5≈旗 reach なので隠れる・gs 3/16 で理想 2.1 に落ちて露出）・梁付き加線 8 分 exact・加線 rod を切っても LP 不動。**LP 側の機構は未特定**（`Stem::width` は thickness だけ）＝次は `pcdump.ily` の WISH/min_dist を旗付き対で。**もう 1 つ見えた島**: 段の途中の `time 4/4` 再印字＋全音符の小節（`midbar-8th` の c1: LP 11.402 対 Lily# 12.4＝+1.0・`probe-formrepeat` の C: 10.7 対 12.6＝+1.9・段頭の再印字（fixture 2 段目）は exact）＝未分類・観測者 0。

★ **⑹ 数**: **開始時裏取り**（`tools\Session-Check.ps1 -Build -Test -Scratch p359`）: HEAD **`539f2084`**・**未 push 28**・`origin/master`＝`cf6edb6f`・木 clean・未追跡 0・build 0 エラー／Core 0 警告・**full 7511 / 0 / 4 / 7515**（`scratch/p359/run1.trx`＝第357 終了時と一致）・`gh run list` 緑（最新 34286219497）・台帳 810・snapshot 249・追跡 `.lys` 599・ディスク 14764。**途中**: 焦点 353 本緑（`run2.trx` full 7517 / 1 / 4 / 7522＝赤 1 は APPROXIMATIONS stale→`LILYSHARP_UPDATE_DOCS=1` で再生成・単独緑）。**終了時**: commit `6b0e8fec`（製品）＋この HANDOFF・**未 push 30**・木 clean・**full 7518 / 0 / 4 / 7522（`scratch/p359/run3.trx`・+7＝`SenzaMisuraTests` 4＋`EmptyBarExportTests` の Theory 3）**・**§7.5（`-DiffBase 539f2084`）Core `+` 269／LILYPOND-REF 7／LILYSHARP-OWN 0**（読み直し: REF 7 は凍った時計の出典（`timing-translator.cc:478-507`・`auto-beam-engraver.cc:115-118`／`:171-182`／`:421-425`・`auto-beam.scm:82-90`・`beaming-pattern.cc:546-550`・`timing-translator.cc:149-160` の mid-piece `\partial`）・OWN 0＝式は 1 本も足していない＝時計の値を 1 つ持ち回って既存の判定に渡しただけ）・台帳 810／snapshot 249／追跡 `.lys` 599 不動。⚠️ **scratch は `p359`**（`p358` は第357）。⚠️ **§7 3.5**: 第356 のブロックを `Session-Check.ps1 -Archive 356` で ARCHIVE の先頭へ・第357 の経緯は下の見出しの下に残した。⚠️ 掃きの base exe は `git worktree add --detach scratch\p359\wt-base 539f2084` から Release build・終了時に worktree は外した（`git worktree list` には旧 perfbase 群が 26 本残っている＝この便の物ではない）。⚠️ ripple: 2 console で約 30 本、standby への自動ルーティングで落ちたものは 0（memory に追記）。⚠️ 踏んだ穴: `git commit -F - @"…"@` の here-string は pathspec になる（`-F <file>`）。

✅ ★★★★ **第 2 便（ユーザー「このセッションでやる方が有利なら着手」）＝⑸ の旗 reach 島の LP 機構を `scratch/p359/lp` の計器で掘って 2 つ見つけ、両方移植して射程を数えた（製品は作業ツリー＝承認待ち・patch `scratch/p359/note-column-skyline.patch`）**。**このセッションでやった理由**: flag-low 等の双子・`pair.ps1`・`settings-ext.ly`（Flag／Stem の extent・列の shortest-playing・**NoteColumn の horizontal-skylines を `ly:skyline->points` で刷る**＝新しい計器）が全部 p359 に在り、掃きの base 923 枚も残っていた。**⑴ 機構 1＝`NoteHead.extra-spacing-height = ly:note-head::include-ledger-line-height`**（`note-head.cc:124-148`・`define-grobs.scm:2600`）: 譜の外の符頭は spacing box を*最初の加線*（外線 ±1 ss）まで伸ばす。**⑵ 機構 2＝`Separation_item::calc_skylines`（`separation-item.cc:92-110`）は NoteColumn の spacing skyline を `skyline-vertical-padding` 0.15（`define-grobs.scm:2577`）で*内在的に*パディングする**（45° の裾・"we make it intrinsic"）＝距離側の 0.08（PaperColumn）とは別。Lily# は箱をそのまま skyline にしていた（`ItemSkylineFactory.CreateRight/LeftSkyline`）。**証拠**: flag-low の LP skyline dump（`sky-low.txt`）＝加線の 8 分の頭の箱が −5.345〜−3.39 に 1.404・旗が −3.24〜0.175 に 2.167・間は 0.15 の裾＝下降対（d→c・c→b）で旗の裾が次の頭の裾に届く（素の箱では 0.3 外れる）。**形**: `ColumnPart.Head`＋`WithLedgerReach`（`Boxes` で適用・`ColumnYExtent` は触らない＝pure-from-neighbor は pure 高さ）／`SpacingRules.NoteColumnSkylineVerticalPadding = 0.15`＋`CreateRight/LeftSkyline` の `.PaddedCopy`。**対**: flag-low 20.24 → **21.45**（LP 21.629・残 −0.18＝未分類）・flag-high／ledger-beamed／flag-metered／flag-metered2／flag-ledgerbar（加線の旗付き 8 分→小節線・拍あり）は全部 exact のまま・probe-formrepeat 20.99 → 22.2（LP 23.279・残りは `.|:` の ±1 島）。**掃き 923 冊（`sweep359.ps1` base 539f2084 → head2）: MOVED 65＝追跡 14（`beam-rest-extreme`×2・`rest-collision-note-duration`・`lpreg/pcsil-b-probe`・`perf-dots`・`perf-dots1k`・`perf-restpoly1k`・`perf-sd5`・`restdur`・`test/chordnames`・`feature-tour`・`lv-meterchange`・`multivoice-tuplet-beams`・`samples/canon-in-d`）＋ユーザー 51**。**patch を当てた full `scratch/p359/run4.trx`: 7514 / 7 / 4 / 7525＝赤 7＝snapshot 4（`test/multivoice-tuplet-beams`・`test/chordnames`・`test/lv-meterchange`・`test/beam-rest-extreme`＝掃きの追跡 14 のうち snapshot を持つ 4）＋目録 2（再生成済み・patch に入れた）＋番人 1（→ 既知残差を釘にして緑）**・**台帳 810 は全点不動（`LpGeometryLedgerTests` 緑＝この regime を観る点が 1 つも無い）**。**番人 `LedgerHeadSpacingTests` 3 本**（flag-low の cadenza 小節＝41.81 を LP 41.986 −0.22〜+0.05 で釘・譜内 19.313 と梁付き加線 21.303/21.56 は ±0.05・最後の小節線は⑸ の別島なので釘にしない）。**§7.5（patch の中身）: Core `+` 96／REF 5／OWN 0**（`note-head.cc:124-148 include_ledger_line_height`・`define-grobs.scm:2600`／`:2577`・`separation-item.cc:92-110 calc_skylines`・`:168-183 boxes`）。**⚠️ 承認ゲート**: 出力が 65 冊動く＝製品は commit していない。**木は HEAD**（`git apply scratch/p359/note-column-skyline.patch` で Core 2＋番人（新規ファイル）＋CHANGELOG＋APPROXIMATIONS／magic_constants が入る・snapshot 4 枚は再ベース＝`LILYSHARP_UPDATE_SNAPSHOTS=1`）。⚠️ **`LilySharp.Cli\bin` の Debug／Release は patch 入りで建っている**＝次便は `Session-Check -Build` から。**LP の側で*どちらの機構*が何冊を動かすかは分けていない**（Release 再建て 2 回分）。**残 −0.18 の在処**（最後の音→cadenza の `\bar "|"` か d→c か）は未測。

✅ ★★★ **第 3 便（ユーザー「リベースしてコミットして」）＝承認パッケージを適用した＝`git apply scratch/p359/note-column-skyline.patch`（exit 0）→ solution build `--no-incremental` 0 エラー／Core 0 警告 → `scratch/p359/rebase-snapshots.ps1`（`LILYSHARP_UPDATE_SNAPSHOTS=1` で 247 本・env を消して再実行 247 全緑・**動いた snapshot は意図した 4 枚だけ**＝`test__beam-rest-extreme`／`test__chordnames`／`test__lv-meterchange`／`test__multivoice-tuplet-beams`）→ full `scratch/p359/run5.trx` **7521 / 0 / 4 / 7525**（+3＝`LedgerHeadSpacingTests`・snapshot 249 不動＝再ベースは枚数を変えない・台帳 810 不動・追跡 `.lys` 599 不動）→ 製品 commit **`700b9ac2`**（10 ファイル +389/−181＝Core 2＋Tests 1（新規）＋snapshot 4＋CHANGELOG＋APPROXIMATIONS／magic_constants）＋この HANDOFF。push はユーザー。**

★★★★ **第 4 便（同じ指示の 2 度目）＝⒜″ の残 −0.18 を捨て計器で分類して閉じた＝*旗の箱*が LP と違っていた（製品は patch `scratch/p359/flag-ink-band.patch`＝承認待ち）**。**計器**: `scratch/p359/ZzP359ProbeTests.cs.txt`（Lily# の NoteColumn skyline を LP の `sky-low.txt` と同じ枠で刷る＝`sky-ls.txt`・列対の `Distance(0.08)` も）。**突き合わせ**: 符頭の箱（±0.545・加線 reach・0.15 の裾）は LP と一致・**旗の箱だけ 2 つ違った**——⑴ Lily# は旗を*符頭＋理想長 3.5* から吊るしていた（`AddFlag` の `IdealStemLength`）＝加線域で符尾が中線まで伸びる音（g,, なら LP の符尾端 0.0・旗 −3.09〜+0.025）では旗が 1 ss 低い／⑵ 箱の高さが `−Bottom − Top`＝2.985（bbox 3.115 から 2×0.065 を引いた綴り）で足が 0.105 短い。**LP**（`flag.cc:183-196 Flag::internal_calc_y_offset`）: Y-offset＝符尾端 − dir·blot/2（blot-diameter 0.4pt＝0.08 ss・`paper.scm:87`）・Y-extent＝glyph bbox 丸ごと＝実測 −2.09〜+1.025（符尾端 +1.0）。**形**: `EngravingDefaults.BlotDiameter`・`ItemSkylineFactory.FlagInkBand`（1 綴り＝spacing box と `ElementCoordinator` の slur-edge extent が同じ関数を読む）・`AddFlag` は `StemSpacingInfo` の実符尾端から。**対**: flag-low **21.63＝LP 21.629 exact**（列対の距離 8 対とも 2.267）・**flag-metered bar 1 の −0.07 も閉じた**（15.04／15.03／15.23／14.97／15.07 対 15.037／15.037／15.224／14.974／15.071）・flag-high／flag-metered2／flag-ledgerbar 不動 exact。**掃き head2（`700b9ac2`）→ head3: MOVED 39＝追跡 2（`lpreg/pcsil-b-probe`・`test/feature-tour`）＋ユーザー 37**。**番人**: `LedgerHeadSpacingTests` の flag-low を ±0.05 の exact に戻し `FlaggedEighthsInMeteredBars_AreExact` +1。**patch を当てた full `scratch/p359/run6.trx`: 7519 / 2 / 4 / 7525＝赤 2＝目録（再生成して patch に）・snapshot 0 枚（追跡で動く 2 冊は snapshot を持たない）・台帳 810 不動**。**§7.5（patch）: Core `+` 52／REF 4／OWN 0**（`flag.cc:183-196`・`define-grobs.scm:1636`・`paper.scm:87`）。**木は HEAD `cee33472`**（`git apply scratch/p359/flag-ink-band.patch` で Core 3＋番人＋CHANGELOG＋目録 2 が入る・再ベース 0）。⚠️ `LilySharp.Cli\bin` の Debug／Release は patch 入り。⚠️ ripple: standby 初回の pipeline が `Set-Location` だけで落ちた（1 回・打ち直しで正常）／`send_input` で空文字を送ってしまい Ctrl+C にならなかった（`` を書く）。

✅ ★★★ **第 5 便（ユーザー「承認する」）＝第 4 便の patch を適用した＝`git apply scratch/p359/flag-ink-band.patch`（exit 0）→ solution build `--no-incremental` 0 エラー／Core 0 警告 → full `scratch/p359/run7.trx` **7522 / 0 / 4 / 7526**（+1＝`FlaggedEighthsInMeteredBars_AreExact`・snapshot 249／台帳 810／追跡 `.lys` 599 不動）→ 製品 commit **`6a891562`**（7 ファイル +209/−169＝Core 3＋Tests 1＋CHANGELOG＋APPROXIMATIONS／magic_constants・再ベース 0）＋この HANDOFF。push はユーザー。**

⇒ ★★★★ **⑺ 次の一手**: **⒜ push はユーザー**（未 push 37）。**~~⒜′ 第 4 便の patch の承認~~＝第 5 便で適用・commit 済み**。**第 6 便は着手しない**（同じ指示の 3 度目）: p359 の計器で測れる残りは ⑸ の「`\cadenzaOff \time N/M` 再印字→全音符の +1.0」だけで、これは観測者 0（段の途中で cadenza から戻り全音符が続く本）・LP 側の spring dump（`ideal-distances`／`minimum-distances`）が after-line-breaking では空で機構が読めない＝計器を新しく建てる仕事で、この便の道具の利が無い。機構 1／2／3 のどれが何冊かの分離は Release 3 回建て＝値打ち小。残り（⒞ の並び）は別 regime。**~~⒜″ 残 −0.18~~＝第 4 便で閉じた（exact）**；機構 1／2 のどちらが何冊かの分離だけ未（値打ち小）。**~~⒜′ 第 2 便の patch の承認~~＝第 3 便で適用・commit 済み**（patch は `scratch/p359` に残骸として残る）。**⒜″ 第 2 便の残り**: flag-low の −0.18（d→c／c→b の 2 対が 2.48 対 2.567＝旗の裾と頭の裾の接触の 0.09×2・`sky-low.txt` に LP の skyline 点列がある）／機構 1 と 2 のどちらが何冊を動かすかは未分離。**⒜‴ §2 H 先頭の起票は第 2 便で「機構 2 つ・移植済み・承認待ち」に読み替える**（起票文は「未特定」のまま＝次便が直す）。**⒝ §3 に「決定済み・未実装」は 0**。**⒞ 第357 ⑹⒠ の並びの残り**（F-fonts 拾い箱／~~`time none` の縁 ⑵⑶⑷~~＝この便で閉じた／拍グリッド＝実需ゼロ／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8b（列の合体・承認要）・U8／A/B/D/E／C⑴／G）。**~~段端の floor の regime~~＝第357 第 4〜5 便が `LineStartLyricFloor`／`LineEndLyricReservation` を退役させて閉じた（並びから落とす）**。**⒟ この便の拾い箱**: §2 H 先頭の旗 reach 島（LP の机上値を `pcdump.ily` で確定→台帳点→移植・観測者は gs≥3/16 の本の旗付き 8 分下降 site＝未計数）／段の途中の `time` 再印字＋全音符の +1.0〜1.9（未分類）。**⒠ 第356 ⑹⒟ の拾い箱はそのまま**。⚠️ scratch は `p359`。
...
- ✅ **F-export. Explorer の右クリックから複数の `.lys` をまとめて書き出す**（2026-09-09・第359 起票・**ユーザー要件 4 つ**・✅ **同便で実装＝§1 第359**。残り＝HTML 形式（後日・ユーザー指示）と、CLI の `lysc pdf/png` に `--all` が無い非対称（未起票のまま・LSP 側だけが全 score を書ける））
  ★★★ **要件（ユーザーの言葉のまま）**: ⑴ 複数の `.lys` を選択してまとめて pdf に変換できる／⑵ コンテキストメニューはサブメニューで変換形式を選べる（svg/pdf/html など）／⑶ 保存ダイアログはファイル保存ではなくフォルダ選択にして、指定のフォルダにすべて保存／⑷ 各 `.lys` に含まれるすべての `score {}` を処理。**HTML は後日**（ユーザー「今は html は追加しなくて良い」）。**Explorer メニューは `.lys` だけ**（`.md` は対象外＝下の経緯）。
  ★★ **経緯**: 発端は「`.md` の ```` ```lys ```` フェンスを VS Code 標準の Markdown プレビューで楽譜にできるか」（可能＝`contributes.markdown.markdownItPlugins`＋`lilysharp/renderText`＋`markdown.preview.refresh` の再描画・フォントは `markdown.previewStyles`・**同梱で 1 拡張のまま**・snippet プリセット＝lilypond-book の `ragged-right`＋`indent 0`＋頁なし＋インク切り抜き、は §2 の別起票候補・**未起票**）。そこから「標準プレビューに PDF export は無い」→「寄付プラグインを尊重する汎用 exporter は先人が撤退した領域（vsc-print）なので作らない」→「Lily# 自身の書き出しを Explorer から」に収束した。ABC フェンスは既存の abcjs 系が成熟しているので**足さない**（ユーザー同意）。
  ★★★ **今日ある物**: PDF/SVG/PNG/ly/MusicXML/MIDI/VSQX の書き出しは**プレビューの Export ボタンだけ**が入口（`exportPreview` → 保存ダイアログ → `lilysharp/export`）。コマンド登録が無いのでパレット・右クリックのどこからも呼べない。`lilysharp/export` は**開いている文書 1 つ・score 1 つ**しか受けず（`GetDocument` → "Document not found"）、全 score の命名規則（main は元ファイル名・他は `stem-<score>`＝`RenderSpec.ResolveOutputStem`）は CLI の `--all`（svg は `GenerateAll`・midi/xml/ly は `RunFormOutput`）だけが持つ。**pdf/png の CLI には `--all` が無い**。
  ★★★ **形（決定・実装）**: **⒜ サーバ**＝`lilysharp/export` に `Path`（開いていないファイルはディスクから読む・`TextDocument` があればそちらが勝つ）・`All`（全 score を `OutputDirectory` に CLI と同じ名で書く・`OutputPath` は無視）を足し、応答に `OutputPaths`／`Warnings`。視覚形式は score 名で選び（`RenderSpecParser.Choose`）、midi/xml/ly は `Form = spec.Form`（CLI の `RunFormOutput` の写し）、**vsqx は `Form` を持たないので最初の score だけ＋警告**（CLI の `WarnFormsLeftOut` の写し）。score 宣言が 0 の本は 1 ファイル（stem.ext）。**⒝ 拡張**＝`src/exportBatch.ts`（純粋部は `exportBatchCore.ts`・node:test）。`contributes.submenus` の `lilysharp.export` を `explorer/context` に `resourceExtname == .lys` で載せ、7 形式のコマンド（パレットでは開いている `.lys` に効く）。複数選択は第 2 引数の `uris[]`。フォルダは `showOpenDialog({canSelectFolders})`。**同名 stem の衝突は着手前に警告（Continue/Cancel）**・**既存ファイルは黙って上書き**（CLI と同じ）・進捗は取り消し可能な通知・完了通知に「Open Folder」「Show Log」。開いている文書は `TextDocument` で送る（未保存の編集を書き出す＝プレビューと同じ意味）。
  ⚠️ **番人**: `LilySharp.Tests/Lsp/ExportRequestTests.cs`（Path 経由・All の命名・vsqx の警告・構文エラー・score 0）／`editors/vscode/test/exportBatchCore.test.ts`（対象の選別・stem 衝突）。

- ✅ **F-mdfence. `.md` の ```` ```lys ```` フェンスを VS Code 標準の Markdown プレビューで楽譜にする**（2026-09-09・第359 起票・**ユーザー指示「起票して着手して」**・✅ **段階 ⑴ は第 2 便・⑵ snippet は第 4 便で実装＝§1 第359**・残り＝⑶ フェンス内の診断・補完（需要待ち）。題名が音楽より広い本の切り抜きは第 5 便で閉じた（`HeaderBand.Width`））
  ★★★ **要件と決定**: mermaid と同じ体験＝標準プレビュー（`markdown-language-features`）で lys フェンスがその場で楽譜になる。**同梱で 1 拡張のまま**（別パッケージにしない＝サーバの二重化も公開 API も要らない）。**フェンス名は `lys`・別名 `lily#`**（第 3 便・**ユーザー決定「別名は lilysharp ではなく lily# に」**。私は最初 `lilysharp` を別名にし `lily#` を「info string は識別子の慣例」で退けたが、Lily# 自身が読む分には markdown-it も TextMate も `#` を制限しない＝技術的制約は無く、ユーザーの綴りに従った。`lilysharp` は退役）。**ABC フェンスは足さない**（abcjs 系が成熟）。**HTML／PDF exporter は作らない**（別項）。
  ★★★ **仕組み（VS Code 側の制約は第359 で確認済み）**: `contributes.markdown.markdownItPlugins: true`＋`activate()` が `extendMarkdownIt(md)` を返す。**markdown-it は同期・LSP は非同期**なので「フェンス本文の hash でキャッシュ・未描画なら placeholder を返して `lilysharp/renderText` を起動・完了で `markdown.preview.refresh`」。プレビューの CSP は `script-src nonce`／`connect-src` 無し／`font-src 'self' cspSource https: data:`／inline `<svg>`・`<style>` は通る（sanitizer 無し）。**フォントは `markdown.previewStyles` の CSS で `@font-face` 2 面（Emmentaler／Emmentaler-Brace＝現行 webview と同じ）**を寄付し、SVG は `OmitFontFace` のまま（Export モードの data URI 埋め込みは 1 fence 400 KB なので採らない）。ダークは `.vscode-dark` 配下で現行 webview と同じ invert フィルタ。エディタ側のフェンス内ハイライトは TextMate の injection（`injectTo: text.html.markdown`・mermaid と同じ手）。**起動は `onLanguage:markdown` を足すが、LSP クライアントは `.lys` が開くか lys フェンスに出会うまで起動しない**（無関係な `.md` で .NET を立ち上げない）。
  ★★ **段階**: **⑴ 拡張側（この便）**＝上の全部・`renderText` に `Interactive=false` を足す（hit-rect と system group の無い静的 SVG・`data-pos` は export にも在る注釈なので残る）・番人（node:test の純粋部＋LSP 側）・隔離 VS Code で実機。**⑵ snippet プリセット（Core・別便）**＝lilypond-book の型（`raggedRight`＋`indent 0`＋頁を切らない＋インクの外接矩形で切り抜き・白矩形は残す＝「紙と遮蔽箱は同じ塗り」の不変条件）。**頁は LP の `ly:one-page-breaking`（`one-page-breaking.cc`）に写す＝1 頁・頁高さは中身・自動改頁なし・`pageBreak` は `break` と同じ強制改行・段間は system-system-spacing の自然長（頁に合わせた引き伸ばし無し）**（ユーザーの問い 2026-09-09 第 3 便・回答済み）。`PaperPlanReader` の土台を `LayoutOptions.Snippet` に替えてフェンスの `paper { }` が上書きする順序・`renderText` の mode 引数・`lysc svg --snippet`（README／サイト用の断片にも効く）。⑴ の時点では**紙のままの楽譜**が `.md` に出る（列幅に縮小）。**⑶ フェンス内の診断・補完（virtual document）**は需要が出てから。
  ⚠️ **番人**: `editors/vscode/test/markdownFenceCore.test.ts`（言語判定・hash・placeholder／error／svg の HTML・XML 宣言の除去）／`Lsp/RenderTextRequestTests`（Interactive=false）。
...
| ★★ **Markdown の lys フェンスは絵を 1 枚描く＝`score {}` は任意で 1 個まで。無ければ全 `part` を宣言順の譜表にし、唯一の `form` を鳴らす（form 0 なら section 宣言順・form 2 個以上は拒否）。score 2 個以上も拒否**（2026-09-09・第359 第 3 便・**ユーザー提案「score は書けず form は 1 つ」→ 私の懸念「score 禁止だと tab／chords 行／lyrics 行／grandStaff がフェンスで書けない」→ ユーザー決定「score は任意・ただし 1 つまでが一貫する」**・✅ 同便実装＝`RenderSpecParser.ImpliedScore`／`RenderTextParams.Fence`） | フェンスは文書の断片で「どれを描くか」を選ぶ場が無い＝選択が要る綴りは全部拒む、の 1 規則。暗黙の score は `score { staff A staff B … }` を書いたのと同じ絵（clef・ラベル・ensemble 既定は `ApplyEnsembleDefault` の 1 軒を共有・番人が byte 一致で釘）。フェンスの外（`.lys`・AI パネル）の規則は不変。 |
