...
最終更新 第93セッション（＝**引継ぎ ⑴「頭の半分」を閉じた。予測は 4 点とも九桁で当たり、
配置 2 点は exact。そのあとユーザーが snapshot を読んで 2 件指摘し、⑴ は測ったら欠陥でなく、
⑵ は本物で、しかも「家族の 1 人だけが直っている」いつもの形だった**）。

⚠️ **仕事は 2 commit**（**`git show fde4efb1..` で引ける**——`fde4efb1` は第92セッションの最後）。
```
25f485d1 頭の半分＝支持は符頭の実 ink    台帳 4 点改善（うち 2 点 exact）  テスト ±0・snapshot 12
...
**未 push 2**（⚠️ **足し算しない**。`git rev-list --count origin/master..master` で数え直す。
...
⑴ 次の島＝**ラベルの X 中心化**  ★★★ **同セッションで割った（コード変更ゼロ・測定のみ）。**
                        ...
                        **対照 3 冊は不動**。**対の差は 0.004539602 → 0.000492538**（**89% がペン**）。
                        ...
                        ⚠️⚠️ ★★★ **素朴な導出は既に反証済み。同じ道を通らないこと**——
                        **合成 skyline の building スパンで中心化すると全部悪化**
                        （**対照 +0.007261・DSK +0.037497・DSM +0.056579**）。
                        **skyline の horizon はグリフの ink 箱ではない**。**要るのは
                        「letter ごとの ink 箱を同じペン位置で合成したもの」**。
                        ⚠️ **描画と 1 つの claim**——予約だけ動かすと綴りが割れる（§5.0）。
                        **snapshot は全 dynamic が横に約 0.008 動くので要承認**
⑴' perf の借り＝**フレーム変換**  **第92セッションから持ち越し・未着手**（+11%）。
                        **`Raise` をやめて読み手にオフセットを渡す**。手が入るのは
                        `FiguredBassEngraver` / `ChordNameEngraver` / `OutsideStaffStacker`。
                        測り方は在る（stress/control・worktree A/B・min-of-30・確保で見る）。
                        ⚠️ **箱に戻して返すのは禁じ手**
