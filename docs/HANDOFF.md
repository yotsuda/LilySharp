...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第417セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `0abdd19f`・**未 push 12**・**full `sessions/p417/run1.trx` 8727 / 0 / 3 / 8730＝第416 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 69 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第416 が予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 415`＝58 行 7,574 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」の ★★ 2 つを AskUserQuestion に出し、**ユーザーが ⒴′「2 度目の注釈パスを要らなくする」（第411 ⑶ の 7.7%）を選んだ**。★ ベンチは着手前に AskUserQuestion で静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第416 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト）:

★★★ **⑴ 起票の 7.7% は、着手した日にはもう 1.90% だった。** 打鍵 8,728,585 B（素の A/B 8,731,252＝第416 の 8,725,056 と 0.07% 差＝母集団は同じ）に対し **`p2.prelim` 306,163,048 B＝1.90%**。⚠️ **第411 が「2 度目は 1 系あたり 2.8 倍払う」と書いた比は *0.82 倍*＝*1 度目より安い***——**第413（paging augment）と第415（above stack）が部屋を配置ごとに分けた分がそのまま消えていた**。⇒ ★★★ **§5.0「着手する便が N を数え直す」の 9 例目で、外れたのは*賞金の桁*だった**（§5.3 へ）。**2 度目が走るのは打鍵の 17.9%・42/231 冊だけ**（`ChooseSystemCount` が別の数を選んだとき）。

★★★ **⑵ 予測 ⑵ を先に書いて、*外れ、その外れが犯人を名指した***（§5.0②）。「数が変われば最初の break 以降は全部ずれるから、2 度目の系は*新しい小節の連なり*のはず」と書いた。実測は **同じ index に同じ範囲 28.6% / *別の index* に同じ範囲 63.1% / 本当に新しい 8.2%**＝**2 度目が組む系の 9 割は 1 度目が既に組んだ小節の連なりで、*番号だけが違う***。

★★★ **⑶ そして番号で引く店のうち 2 軒が、まだ部屋を 1 つしか持っていなかった。** 配置ごとの hit 率: **above-stack 95.8% / 96.0%**（第415 が直した店＝もう擦り切れない）に対し **below-stack 85.6% / *55.5%*** ・**fing/scripts 83.5% / *38.1%***。**枠を最後に書いた者で miss を割ると**（第415 ⑷ と同じ問い）:

| 店 | 1 度目が 2 度目の枠で外す | 2 度目が 1 度目の枠で外す | 自分の枠で外す（床） |
|---|---|---|---|
| below-stack | **866** | **866** | 199 |
| fing/scripts | **4,208** | **4,208** | 2,340 |

⇒ ★★★ **2 つがぴったり同数＝擦り切れの署名**（第415 は 5,805 対 5,216 で「ほぼ同数」だった）。**1 軒が取った枠は、必ずもう 1 軒の miss 1 件になる。**

★★★ **⑷ 直し＝第415 と同じ形を 2 軒に写しただけ**（`BelowStackMemo`・`FingScriptMemo` に**鍵ごとに 2 部屋・古い部屋で当たったら昇格・miss で古い方を追い出す**）。**2 は count loop 自身の上界**（`Layout` は `PlaceSystems` を最大 2 回呼ぶ）。⚠️ **`Matches` は 1 バイトも触っていない**＝健全性の議論は不変。⚠️ `FingScriptMemo.TryMatch` は**当たった部屋を返す**（昇格が他方の配置の数字を返さないため）。

★★★ **⑸ 効いた**: miss は **below 1,932 → 273（−85.9%）**・**fing 10,756 → 2,886（−73.2%）**、hit 率は **below 97.0% / 97.2%**・**fing 93.4% / 95.6%**、擦り切れは **866/866 → 17/8**・**4,208/4,208 → 102/50**。

★★ **⑹ ところが A/B は −0.22% しかなかった**（**8,731,472 → 8,712,424 B**・継ぎ目を全部外した素の harness）。⇒ ★★★ **miss 7,870 件を消して 18,828 B/打鍵＝*1 miss が約 2,950 B***。**第415 の above-stack の 1 miss は 43,636〜47,945 B だった**——**同じ「擦り切れの署名」でも、賞金は署名ではなく*1 miss の値段*で決まる**（§5.3 へ）。**冊で見ると最良 −2.60%**（`himawari.lys`）・**−1% 以上が 15 冊・0.05% 以上動いたのが 168 冊**。⚠️ **17 冊は*上がった*（最大 +0.65%＝`Billie Jean.lys`）＝機構は未計測**（候補は「hit が増えると below pass の remap 経路（`hits.Count > 0` で初めて走る `Filter`）が走る」＝**これは推論であって測定ではない**・次便の一手に置いた）。

★★★ **⑺ 出力同一は 3 つとも走らせた**（RULES §5.1）: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline はこの便で `0abdd19f` から取り直した）／⑵ **全ページの SHA-256**＝**231 冊 × 8 打鍵＝5,824 ページで差 0 行**（before は製品の 2 ファイルだけ `git stash push` して取った＝ユーザーの未 commit `tools/*.ps1` には触っていない）／⑶ suite 全緑・snapshot 249 枚不動。

★★ **⑻ 網 4 本、毒見で*ちょうど 4 本*赤**（`BelowTwoPlacementsAlternating_BothKeepHitting`／`BelowAThirdPlacement_…`／`FingScriptMemoRoomsTests` の 2 本。3 本目の `TheRoomsAreStillPerStaffAndSystem` は毒で緑のままが正しい）。**既存の網は 1 本も赤くならなかった**＝**絵が正しいまま費用だけが戻る変更は profile の網では捕まらない**（第416 ⑼ と同じ・毒は「2 部屋目の検索を殺す」）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: **⒴′ ✅ 閉じた**（ただし「2 度目を要らなくする」ではなく「2 度目と 1 度目が枠を奪い合うのをやめさせる」で・7.7% ではなく 0.22%）。★★ **⒴‴ 上がった 17 冊の機構を測る**（上の ⑹・`OutsideStaffStacker` の `Filter`／remap が hit 1 件で走り出すのか＝**安い継ぎ目 1 本**）／★★ **⒴″ `b.scriptsFingerings` 4.2%**・未分割（**この便で fing の miss が 73% 減ったので取り直すこと**）／★★ **⒳′ resolve の*複写*＝`EndBatch` の入力複写 0.17% ＋ `MergeInternal` 自身の複写（未計測・推定 0.7%）**＝call をまたいで生きる buffer が要る（instance か static）／★ **⒵‴ `e4.musicMarks` 1.22%**・未分割／★ **⒴ `inkFirst`＋`inkLast` 0.53%**／⚠️ **`g4.core` は repair 後 1.21% で*もう実仕事*＝この島には戻らないこと**／★★ **⒴⁗ 1 度目の配置を*安くする*＝`p1.prelim` は打鍵の 13.96%** で、**数が変わる 17.9% の打鍵ではその全部が捨てられる**（`ChooseSystemCount` が読むのは高さの見積りだけ＝**「見積り専用の安い prelim」が要るかは未測**・土台の変更）／⒜ **R13⒝ の実機確認**（第404 ⑵＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・⚠️ 未着手・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／LP 双子が要る R7〜R11／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑼ 終了時**: commit 2 本（code `ce5db872`・docs 1 本＝**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 1 本・**最終 full `sessions/p417/run3.trx` 8732 / 0 / 3 / 8735**（§1・RULES を書いた*後*の最終ツリーで取った）（⚠️ **第416 最終の 8727 / 8730 から +5＝今便の網ちょうど 5 本**・`Zz417`／`ZZ417`／`ZzStage` の grep は**コードに 0 件**）・§7.5（対 `0abdd19f`＝スクリプトの出力）**Core `+` 92 行／REF 0／OWN 0**（★ **Core に数値定数を 1 つも足していない・LP 由来の式も 0**＝§7.6 ⒟「既存の家を指し直しただけ」＝**第415 が入れた device を 2 軒に写しただけ**で、92 行の**大半は註**（測った数をその店の remarks に置いた）・足したコードは `Slot` の record struct 2 つと古い部屋の枝と昇格）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。⚠️ **棚卸し `docs/APPROXIMATIONS.md` と `audit/magic_constants.csv` は不変**（定数を 1 つも足していないので再生成不要＝差分で確認）。§7 4（恒久知識）＝**RULES §5.3 に 3 項**（「**前の便が直した病名を、*同じ病を持つ兄弟*で grep してから起票を閉じる**」／「**擦り切れの署名は*機構*を証明するが*賞金*は証明しない＝賞金は 1 miss の値段で決まる**」／「**memo の hit を増やすと費用が増える経路がありうる＝A/B は冊ごとに割って、上がった冊を数える**」）。計器と全文は Lab `sessions/p417/`（`summary.txt`・`two-placements.txt`・`ab-{before,after}.txt`・`hashes-{before,after}.txt`・`Zz417Probe{1,2,3}.cs.txt`・`ZzStage.cs.txt`・`zz417-seams-and-change.diff.txt`・`change.diff.txt`）。§7 3.5 は**セッションの頭で**済（`-Archive 415`＝58 行 7,574 chars）⇒ **HANDOFF は 446,965 B → アーカイブ後 439,391 B → 今便を書いて 444,524 B＝天井 450,000 まで 5.3 KB**。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 416` を回すこと**（第416 のブロックは約 9.8 KB＝回せば 15 KB 空く。**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第417 の欠陥ではない。
