...
- ★ **⒩⁴（寸法を言う）は*尾しか残っていない*＝着手先としては最後**。島は 311 軒 150,948 B／打鍵 2.341%
  だが、**top10 の 1.619% のうち 1.144% は「寸法を言えない」と実証済みの 2 軒**
  （`ItemSkylineFactory.cs:465` 0.581%＝第439／`MusicSiteList.cs:83` 0.563%＝第441 が histogram で反証）で、
  **その下も検出の結果ばかり**（`MeasureCollector.Form.cs:1101` 0.135%・`BeamDetector.cs:141` 0.095%・
  `ArticulationEngraver.cs:395` 0.061% …以下 0.03% 台）。現在地は Lab `sessions/p442/site-prices-after.txt`
...
- ⒥ は第409 が上限 4.7 ms と測った／**Ⓑ ⒢′ ⒳‴ ⒳⁗ ⒞″ ⒟ R13⒦ ⒤ ⒴⁵ ⒴⁷ ⒴¹⁰ ⒵⁵ ⒵⁶ ⒩′ ⒩‴ ⒩⁴の脇 ⒩⁵ ⒫ ⒬ ⒬′ ⒭ ⒮ ⒮′ ⒮‴ ⒮⁵ ⒱ ✅ 閉じた**
  （閉じ方と「毒が緑」の**4 つの顔**は RULES §5.4 末尾。経緯は第453・第454 の §1＝ARCHIVE）
- **`docs/RULES.md` は 226,774 / 250,000 B・1,859 / 2,000 行**（第450 が実例 41 件を `RULES-CASES.md` へ
  出して空けた）。⇒ **次に詰まったら、割るのではなく*規則そのもの*を畳む**（印のほうが高くつく）。
...
**未 push 25**（`-End` 時点は 21・そのあと docs 4 本）・台帳 851 点／総和 22.584727806 不変・
snapshot 249 枚不動・追跡 `.lys` 609。
天井は `-End` 時点で **439,897 B / §1 現在便 16,181 字**（そのあと RULES への出しと ⑻⑼ が足された＝
**最終値は ⑼ の末尾**）。
...
★★ **⑼ 「有利なら次便ぶんも着手して」には着手しなかった＝第453 ⑽ と同じ理由で、材料は口座。**
判断時点で **§1 現在便 17,595 / 20,000＝残り 2,405 字**。残る島（⒮⁷／⒲／⒮″／⒳⁷）は**どれも
自分の census か A/B が要り**、直近の語りは 4,925（本便）・15,421・14,509・約 4,000 字＝**測っても書けない**。
**この文脈の利点も 0**——脚は Lab に commit 済み・§1.0 は `-Start` が逐語で刷る。
⚠️ **唯一「今のほうが有利」に見えたのは census の回し直し**（⒮⁶ の 41,734 は行の引き算）**だが、
census は*島に手を入れる便が*回すもの**——次便が ⒮⁷ を閉じれば、その場でもう一度回すことになる。
⇒ 代わりに **§7 3 を回した**＝✅ 2 件と、尾しか残っていない ⒩⁴ の一覧で **§1.0 を約 −850 字**
（12,638 → **11,824**）。⇒ **次便の語りの予算は 8,176 字・HANDOFF の残りは約 8,200 B**
（この文を含む最終値。§1 現在便は 17,2xx / 20,000）。

## 以下は第453セッションの経緯

`/clear` 直後の新セッション。指示は「**HANDOFF を読んで着手**」で、**着手先は §1.0 ⒜ の ⒱**（ユーザーが
選んだ・⒱ と ⒮⁶ と ⒲ と ⒮″ を並べて訊いた）。★ **`-Start p453` の 1 コマンドで §0 が全部済んだ**
（HEAD `7b780011`・未 push 15・full `sessions/p453/run1.trx` 8778 / 0 / 3 / 8781・台帳 851 点／
総和 22.584727806・snapshot 249・追跡 `.lys` 609）。**裏取りは 1 つも赤を出さなかった。**

★★★★ **⑴ `-Archive` が第452 の経緯を道連れにした——§1 の散文が*自分の継ぎ目の名前を引用*していたから。**
`Session-Check.ps1:194` は `$h.IndexOf("## 以下は第451セッションの経緯")` で切っており、**行頭に錨が無い**。
第452 は「次: §1 に『## 以下は第451セッションの経緯』を立ててから full」という*注意書き*を §1 に書いた
ので、`IndexOf` はその**引用**に当たり、そこから §2 の直前までを運んだ＝**126 行 7,644 B**（正しい
**72 行 4,225 B** ＋ **第452 の ⑴〜⑹ 54 行**）。⚠️ **同じ見出しを*数える* `HandoffBlocks`（:57）は
`^##` で錨を打っている**＝**検出と切断が別の読み方をしていた**（`$e2` の §2 側も同じ裸の `IndexOf`）。
⇒ 直した（両方の印を `(?m)^` に）。戻しは `git checkout --` 2 ファイル＋`-Archive 451` 再実行で、
**72 行 4,225 B** になった。⚠️ **気づけたのは `-Start` の刷る「moved: N lines」を*行数で*読んだから**＝
CLAUDE-OPERATIONS の「2 行直したのに 10 行」と同じ合図。⇒ ★★★ **文書が自分の構造の印を
*引用*すると、その印で切る道具が壊れる。切る側は数える側と同じ読み方をすること。**

★★★★ **⑵ 毒が緑のとき次に訊くのは「その腕は走ったか」で、答えるのは毒ではなく*計器*。**
第452 は「巨大にする毒」で「建てていない／建てたが出会わない」を割ったが、**⒱ の 2 件は*箱*ではなく
*腕***なので巨大化する物が無い。⇒ **分岐に計数器を置いた**（Lab `sessions/p453/instrument.ps1`〜
`instrument4.ps1` ＋ `Zz453*.cs.txt`）。1 便ぶんで: **`RespaceStaves` の many-arm は 1,949 回建ち・
19 回読まれ**（allStill を抜けて辞書が読まれた）・**うち 17 回は最初のばねが 0 でない**／
**`BucketSpan` の遅延生成は 42 fold 中 10 回発火**。⇒ ★★★ **どちらも走っている＝第450 の緑は
「到達不能」ではなく「観測者不在」**、と 1 run で確定した。⚠️ **計器の 5 赤のうち 3 本は §1 の継ぎ目
未設置**（第452 の警告どおり）**で、計器のせいではない**——GPL ヘッダを付けた 3 本目は 0 赤。

★★★★ **⑶ ⒱⑴ の穴の正体は*相対の網*だった。** `many live` に届く本は **`VocabularyPerturbationTests` の
`EveryPaperKeyMovesThePage` と `EverySpacingSubKeyMovesThePage` の 2 本だけ**（スタックの*最も外側*の
`LilySharp.Tests` フレームで実測。⚠️ **最も内側を採ると `Signature` というヘルパで止まる**）。
どちらも `AssertMoves`＝**`Signature(a) != Signature(b)`**。⇒ **両側を同じだけ動かす毒は不等号を残す**
＝**相対の網は*系統的な*誤りを原理的に見られない**。⇒ **絶対の主張で書き直した**＝
`PageStaffRespacingTests`（3 譜・ばね 2 本・**⑴ 伸びたこと ⑵ 同じばね 2 本は同じ長さに解けること**）。
**毒 A（第450 の毒 5）は赤 1 本＝この網だけ**。⚠️ **`RaggedLastBottom` の既定 true を切らないと
この網は何も言わない**（1 頁の本は自分の最終頁＝ragged＝allStill で腕の手前で帰る＝
`VocabularyPerturbationTests.FilledPageBook` が paper の鍵について書いている罠と同じもの）。

★★★★ **⑷ ⒱⑵ は*絵では原理的に見えない*——5 冊で実証。** `BucketSpan` の遅延生成を落とす毒は
**歌詞なし 12 小節／疎な歌詞／括弧を生やす編集／24 小節／24 小節＋端だけ歌詞**の 5 通りで全部緑。
24 小節の本では **cache の判断は動く**（clean **hit 1 / miss 12** → poison **hit 5 / miss 8**＝
**4 系が古いまま配られる**）のに **SVG はバイト同一**。理由は fold の形——**跨がれただけの小節の fold は
`(role, 絶対番号を*除いた* content)`＝定数**で、**「まん中を横切るだけの括弧」に依存する cache 値が
1 つも無い**（ink は毎回 live から `PedalEngraver.SolveAndSeed` が解き直す）。**cache に届くのは括弧の
*端*だけで、端は印を持つので器は `BucketSingle` が既に作っている**。⇒ ★★★ **網は「主張が住んでいる層」に
置いた**＝`MeasureContentKeySpanTests`＝**`MeasureContentKey.Compute` の鍵そのもの**を 2 冊
（括弧あり／なし）で比べ、**跨がれた小節は動き・span の外は 1 ビットも動かない**ことを見る。
**毒 B は赤 1 本＝この網だけ**。

★★ **⑸ その途中で割れた 2 つ**: ⒜ **第450 の pedal の網が毒 7 を見られなかった理由**＝あの本は
全小節に `la la la la` があり、**`BucketSingle(score.Lyrics)` が器を先に作ってしまう**。⇒ ★★
**遅延生成の腕は「他に何も無い」本でしか走らない＝網の本の*豊かさ*が腕を殺す**。
⒝ **解放の無い `@sustain` は括弧を 1 つも作らない**（`DetectBracketsForType` は ON を OFF か次の ON と
対にして初めて出す）＝「開始だけ入れる」編集では何も起きない——2 冊ぶん溶かした。

★★ **⑹ 起票 ⒳⁶**（§1.0 ⒝）＝**span の fold は*跨がれた全小節*に入るが、今日それを読む cache 値は
無い＝健全側への過剰無効化**。実測は上の **4 系／24 小節・1 編集**。直すなら**「端だけに fold する」**だが、
**cache の健全性に触るので値段より先に判断が要る**。

★ **⑺ `Core` は 1 行も動いていない。** この便が足したのは試験 2 ファイル（`PageStaffRespacingTests.cs`
`MeasureContentKeySpanTests.cs`＝**+2 本**）と `tools/Session-Check.ps1` の錨 1 か所と docs だけ。
⇒ **出力同一は構成から保証される**（`git diff -- LilySharp.Core` が空）。**台帳点は 1 つも足していない**。

★ **⑻ 終了時**: commit 3 本（tools `c319e243`・test `8550935c`・docs 1 本＝**この文を含むので SHA は
書かない**＝§5.4）。**最終 full 8780 / 0 / 3 / 8783＝開始時 8778 ＋ 新しい網 2 本**・
§7.5（対 `7b780011`）**Core '+' 0 行／REF 0／OWN 0**（⚠️ **`git diff -- LilySharp.Core` そのものが空**
＝§7.6 ⒟ の足した式も定数も 0。`APPROXIMATIONS.md`・`magic_constants.csv` とも差分なし）・
**§7.7 の匂いは 0**（`Core` を触っていないので当たる面が無い）・**未 push 17**・台帳 851 点／
総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。天井は `-End` 時点で
**HANDOFF 435,344 B（残り 14,656）・§1 現在便 15,421 字（残り 4,579）**・**RULES 221,571 B / 1,854 行**。
全文は Lab `sessions/p453/`（`instrument.ps1`〜`instrument4.ps1`＝腕の計数と犯人の特定・
`poisons.ps1`＝陽性対照・`try-b.ps1`＝⒱⑵ の本を毒に選ばせた台・`Zz453*.cs.txt`・
`hits*.txt`／`who*.txt`）。**push はユーザー**（Lab も）。
⚠️ **⒱ は 3 件とも閉じた＝§1.0 ⒜ から消えている**。起票は **⒳⁶**（§1.0 ⒝）。
**`-Start p454` の 1 コマンドから入る**。

★★★ **⑼ 追加の 1 件（ユーザー「このセッションでやる方が有利なら着手して」）＝⒩⁴ の脇を閉じた。**
選んだ理由は **⑴ 技法が今便そのもの**（緑の毒に計器で母集団を訊く）**⑵「先に数えるだけで決まる」と
引継ぎが書いている＝1 run** **⑶ §1 の残り字数に収まる**——**⒮⁶／⒲／⒮″ は別の census が要り、
この文脈の利点が 0 なので次便に置いた**。⇒ **答えは「母集団が無い」より強かった**＝
**`BeamedPrefix` は 1 を返せない**（`return n >= 2 ? n : 0;`）ので、第440 の毒（`beamed < 2` を外す）は
**どんな本でも何も変えられない＝構造上 vacuous**。histogram は **187 回中 1=0**（0=105・2=63・3=11・
4=5・5=3）で裏を取った。⇒ ★★ **「毒が緑」の 3 つ目の顔**＝⑴ 観測者が居ない（⒱）／⑵ 観測者が
原理的に居ない（⒳⁵）／**⑶ 毒そのものが恒等写像**（これ）。脚は Lab `sessions/p453/instrument5.ps1`
＋`Zz453e.cs.txt`・出力は `prefix.txt`。⚠️ **この run の 1 赤は `APPROXIMATIONS.md` の
`SharedRenderer.cs:553` が +1 行でずれただけ**（第449 の罠・毒とは無関係）。**`Core` は今便を通じて
1 行も動いていない**（⑺⑻ の主張は有効）。

★★ **⑽ 2 度目の「有利なら着手して」には着手しなかった**＝**§1 に次便の口座が無い**（判断時点で残り
2,884 字。残る島 ⒮⁶／⒲／⒮″／⒭′ はどれも census か A/B が要り、語りは第451 で約 4,000 字・
第452 で 14,509 字＝**測っても書けない**）。**この文脈の利点ももう無い**（道具は Lab に commit 済み・
