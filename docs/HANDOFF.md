...
終了時 **未追跡 0・作業ツリー項目 0**・suite **5244 passed / 0 failed / 4 skipped**
（+3＝新しい網ちょうど）・**台帳 516 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・
**snapshot 217 枚で不動＝再ベース 0 枚**・**Core は 0 警告**。
**全数 A/B は 4 回**（毎回 `git ls-files "*.lys"` の **566 冊**・**絵は毎回 0 冊動いた**）＋
**陽性対照 4 回**（6 冊／1 冊／37 冊／103 冊）。
★ **ユーザー決定 0 件**——**着手判断は第190 の委任（「有利なら着手」）に従って私が決めた**。
...
- **⑴ ★★★ 第1便＝paging augment が system の silhouette を*ステップごとに*複製していた**
  ...
  ★ **値段**（Release・**アロケーション**。時間では割れない＝同一コードの対照が 16% 揺れる）:
  **打鍵 `perf-fingstack1k` 3305.0 → 2816.1 MB（−14.8%）／`perf-scripts1k` 1118.1 → 932.9（−16.6%）／
  対照 `perf-plain1k` 95.9 → 95.9（±0.00）**。
...
- **⑵ ★★★ 第2便＝skyline の merge が毎回 2 本目の全長リストを作っていた**
  ...
  ★ **値段**（⑴ の上に積んで）: **fingstack1k 2816.1 → 2191.7 MB（−22.2%）／
  scripts1k 932.9 → 698.1（−25.2%）／v2bow1k 158.9 → 153.0（−3.7%）／plain1k 95.9 → 95.1（−0.8%）**。
  **フルレンダは fingstack で 6619 → 4807 MB。**
  ⇒ **2 便あわせて打鍵 3305.0 → 2191.7 MB＝−33.7%**（`perf-scripts1k` は 1118.1 → 698.1＝−37.6%）。
  ...
  **5244 本を通して 1 度も鳴らない＝両呼び手が守っていることの観測。**
- **⑶ ★★ 第3便＝打鍵の*アロケーション*に家を与えた**（`54befaae`・`audit/LilySharp.Probe` の
...
- **⑷ ★ §7.5 の監査**＝**Core +81 行（うち*コード 37*）・削除 −39（コード 37）＝
  正味のコード行 0・数値定数 0・REF 0 増減なし・OWN 0**。**分類は ⒟**
  （何も足していない＝既存の家の指し直し。**LP の意味論に 1 つも触っていない**——
  出力がバイト同一であることがその証明）。**§7.7 の匂い一覧はどれも当たらない。**
- **⑸ ★★ 残っている項＝**打鍵の残り 2191.7 MB のうち、**注釈パスは今も約 87%**。
  **次の 2 つが測って分かっている**:
  **⒜ `AugmentSkylinesWithScripts` に memo が無い**——**隣の `AugmentSkylinesForPaging` は
  `SystemLayoutCache.GetOrComputePagingAugment` を持っている**のに、**同じ
  `PagingAugmentProgram` を組む*もう 1 軒*は毎打鍵ぜんぶ組み直す**＝**§2A「同じ量の 2 軒目」の
  性能側の顔が、引継ぎが予想したのとは*別の住所*に居た**。**打鍵では系のほとんどが
  無変更なので、memo が入れば残りの大半が消える**見込み（**未測定・見込みであって測定ではない**）。
  ⚠️ **`_pagingAugments` は system index だけで引く 1 枠**なので、**同じ memo を共用すると
  2 つの program が枠を奪い合って毎回 miss する**。**別の辞書が要る。**
  **⒝ `MergeInternal` は今も merge ごとに全長の合成リストを 1 本作り、全体を sort する**
  ＝**script 数について二次のまま**。**こちらは全 merge が通る器なので射程が広く、危険も広い。**
...
- **⑹ ★★★ 射程は変わっていない＝実ファイル 0 冊**（第190 の census のまま）。
  ...
  **書けるのは「打鍵の割り当てが 1/3 減った」まで。** ⚠️ **ただし ⑵ は器が全 merge なので、
  *どの本でも*わずかに効く**（plain1k で −0.8%）——**これは実ファイルにも届く唯一の部分**。
...
> （2026-08-17・第190 起票／**第191 が住所を取り直して 2 段まで返済・残り 2 段は未着手**）
>
...
> **`GC.GetAllocatedBytesForCurrentThread` は同じ本でレンダごとに 0.1 MB まで同値**）:
>
> | 1 打鍵 | 第190 時点 | 第191 ⑴ 後 | 第191 ⑵ 後 |
> |---|---|---|---|
> | `perf-fingstack1k`（11,988 script） | 3305.0 MB | 2816.1 | **2191.7**（−33.7%） |
> | `perf-scripts1k` | 1118.1 MB | 932.9 | **698.1**（−37.6%） |
> | `perf-v2bow1k` | 158.9 MB | 158.9 | **153.0** |
> | `perf-plain1k`（script 0・対照） | 95.9 MB | 95.9 | 95.1 |
>
> ⚠️ **`plain1k` は ⑴ の対照であって ⑵ の対照ではない**——**⑵ は全 merge が通る器なので
> 対照になる本が存在しない**。**「対照が動いた」ではなく「これは対照ではない」と読むこと**（§1 の骨 ⑶）。
>
> **⑴ phase 二分**: 打鍵の **約 87% が注釈パス**（`CalculateAnnotationLayouts` を region で囲って実測。
> plain1k では同じ region が 6%）。**その中の 2 大項が下の ⒜⒝**で、**第191 はその複製だけを外した。**
>
...
> ——**測る軸が違うと別の答えが出る例**。
>
> **⑶ 残っている 2 段（着手するならここから・どちらも未測定の見込みつき）**:
> ★★ **⒜ `AugmentSkylinesWithScripts` に memo が無い。**
> **隣の `AugmentSkylinesForPaging` は `SystemLayoutCache.GetOrComputePagingAugment` を持っている**のに、
> **同じ `PagingAugmentProgram` を組む*もう 1 軒*は毎打鍵ぜんぶ組み直す**
> ＝**§2A「同じ量の 2 軒目」の性能側の顔**（**引継ぎが予想したのとは別の住所**）。
> **打鍵では系のほとんどが無変更なので、memo が入れば残りの大半が消えるはず**——
> ⚠️ **これは*見込み*であって測定ではない。着手したら先に測ること。**
> ⚠️⚠️ **`_pagingAugments` は system index だけで引く 1 枠**なので、**同じ memo を共用すると
> 2 つの program が枠を奪い合って毎回 miss する**。**別の辞書が要る。**
> ⚠️ **消費者は 3 つ**（lyrics 行・figured bass・chord names）。**script も lyrics も無い本では
> この augment を*誰も読まない***ので、**「要求されたときだけ組む」形も同じくらい効く**
> （⚠️ ただし**消費者の一覧を持つ guard にすると 4 人目が黙って壊れる**ので、
> **遅延評価にして「読んだ人が組む」形にすること**）。
...
> ⚠️ ★ **第191 の ⑵ だけは例外**——**器が全 merge なので script を 1 個も持たない本にも届く**
> （`perf-plain1k` で −0.8%）。**残り 2 段（⒜⒝）のうち ⒜ は script／lyrics／figured bass／
> chord name のどれかを持つ本にしか届かない。**
