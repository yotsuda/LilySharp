...
> ## ▶ **⒭ section 境界で音部記号とテンポだけが引き継ぐ理由が、どこにも書かれていない** ← **2026-08-31・第303 起票**
> **実測**（§1 ⑸）: **リセットする**のは relative の枠・音価・octave モード・拍子・調・grob override・小節境界。
> **引き継ぐ**のは **音部記号とテンポ**の 2 つだけ。**`_sectionResetClef` は存在する**ので clef は「戻すつもりで書かれている」
> 側に見えるが、**part 既定へ戻すのであって score レベルへではない**——**同じ「自己完結」の語で 2 つの違う操作をしている。**
> ⇒ **どちらが正しいかは測っていない。** テンポには reset 変数すら無い。**⒫ を設計する前に 1 度読むべき島。**
