...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第423セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `f34d033f`・**未 push 27**・**full `sessions/p423/run1.trx` 8743 / 0 / 3 / 8746＝第422 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 108 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第422 が予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 421`＝23 行 7,013 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」の 4 つを AskUserQuestion に出し、**ユーザーが ⒴⁗「1 度目の配置を安くする（天井 3.249%）」を選んだ**。★ ベンチは着手前に AskUserQuestion で静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第422 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、**before 側の打鍵 8,206,704 B は第422 最終の 8,206,656 B と一致**（＝母集団も harness も同じという証拠）:

★★★ **⑴ 起票の天井 3.249% は*入れ子を 2 度数えていた*。** `RunPreliminaryAnnotationPass` は `PlaceSystems` の**最後の文**＝`p*.prelim ⊂ p*.place` は**構成で言える**（測定ではない）。第422 の probe 自身が継ぎ目に「p1.place already contains p1.prelim, so the two are tallied apart」と書き、**p1 側では引いた**（`zz.wasted.p1.place` は prelim を引いた値）のに、**p2 側では足した**。⇒ **⒴⁗ の天井は `p2.place` 単独＝2.03%**（第422 の計器の生値で 2.033%・うち prelim 1.162%）。§5.0「着手する便が N を数え直す」の **11 例目で、外れたのは数でも難所の形でも犯人の住所でもなく*入れ子の算術*だった**。

★★★ **⑵ 予測 ⑵⑶⑷ は 3 つとも外れ、しかも*島が開く方向に*外れた。** 「loop の答えは脆い」と予測して計器を組んだ（`ChooseSystemCount` を 1 打鍵に 9〜13 回、**わざと劣化させた見積り**で走らせ、本番の答えと突き合わせて捨てる。本番の答えを先に出すので絵は動かない）。実測（1,832 打鍵）: **ink ±5% で flip 0・×2 で 32（4 冊）・silhouette を全部隠して flip 0**。⚠️ **計器の検算を先にやった**（§5.3「0 は『測れていない』の顔で出る」）＝silhouette は **43,961 系すべてに在り**、隠すと見積りの ink が **1.585 倍**ずれる＝knob は効いている。さらに **ink を 0 にして 1 冊**・**frame を null にして 1 冊**・**body を既定にして 1 冊**・**配置を 1 つもしない見積りでも 1 冊**（`アゲハ蝶.lys`）。⇒ ★★★ **この loop の答えを担いでいるのは line DP の force² と break penalty で、高さの見積りではない。**

★★ **⑶ そして第335 の当の本が、註の主張を反証した。** `EstimateMeasureHeights` の註は「小節ごとの silhouette 分割」の根拠に `Le Freak`（第335・staff+tab 24 行）を名指している。**その本はこのコーパスに在り、8 打鍵とも loop に訊かれて、`est.flat` でも `est.nothing` でも 1 度も flip しない。** ⇒ ★★ **註が引いている事例は、註が守っている機構をもう要求していない**（要求しているのは別の 1 冊）。⚠️ 無編集 95 冊を「1 回描画」で母集団に足そうとしたが、**同一テキストの再描画は打鍵にならない**＝`asked` は 1,832 のまま動かなかった。冊を広げるなら編集を作ること。

★★★ **⑷ ユーザーの選択で Ⓐ（prelim を数の決定の後ろへ）を実装＝A/B −0.9525%（8,206,704 → 8,128,512 B・両側 TC=0・各側 2 回・床 0.0006%）。⚠️ だが出力同一にならなかった。** 変更は `PlaceSystems` の最後の文を `AddPreliminaryPass` に出し、`SystemPass` に `RowsAboveFirstStaff` を足して、**数が決まった後で 1 回だけ**注釈する形（配置は 2 回のまま・prelim が 1 回）。継ぎ目以外は構成で同じ＝CreatePages も最終 spanner も bow の carry 門も、**今日と同じ placement を 1 文あとで**受け取る。**repo 側は全緑**（⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**／⑵ suite 8742 / 1 / 3 / 8746＝赤は `ApproximationInventoryTests` の**行番号のみ**／**snapshot 249 枚不動・台帳不変**）。★★★ **しかし実コーパスの全ページ SHA が `アゲハ蝶.lys` 1 冊を出した**（8 打鍵とも 2 ページ目）。`lysc layout` で機構を確定＝**系数 24 → 26・ページ配分 13+11 → 13+13**＝数の選択が変わった（配管の欠陥ではなく、⑵ が名指していた当の冊）。

★★ **⑸ 絵で裁定し、ユーザー決定で戻した。** 2 ページ目を前後で PNG にして並べた（Lab `ageha-{before,after}-page2.png`）: **after は 54-57 と 81-84 が 2 小節ずつの行に割れ、4 本の行が隣の 4 小節行の半分の密度で伸びる**。before はページ下の白が 1 割ほど多い代わりに**密度が揃う**。★★ **エンジン自身の物差しも before を選んでいる**——今日の loop は 24 と 26 の**両方を注釈込みの見積りで採点して 24 を採った**のだから、良い情報で測れば 26 の方が高い。⇒ **revert**（**製品コードは 1 行も変えずに閉じた**）。**差分は Lab `sessions/p423/zz423-deferred-prelim.diff.txt` に残した**＝`git apply` で戻せる。

⚠️ **⑹ LP 双子は取れなかった＝この機械の既知の hang**（RULES §5.5）。`lysc ly` は双子を出した（警告 2 本はどちらも原文由来の `LYS2001`／`LYS4007`＝exporter の落としではない）が、**`lilypond.exe` が Guile 初期化前に CPU 0.5〜0.7s で凍結**。kill してやり直しを **5 回**、さらに `{ c'4 d' e' f' }` の **4 音のファイルでも同じ**＝本ではなく機械の状態。⇒ **「LP はこの本を何系に割るか」は未測**＝`アゲハ蝶` の 24 と 26 のどちらが LP 寄りかは**まだ誰も知らない**。双子は Lab `sessions/p423/ageha-twin.ly`＝`lilypond` の通る時間帯なら 1 コマンドで付く。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: ★★ **⒴⁗ ▶ 天井は 2.03%（`p2.place` 単独）に訂正済み。⚠️ 安くすると `アゲハ蝶.lys` 1 冊が 24→26 系になり、絵は今日の方が良い＝これは perf の島ではなく*忠実度の判断*。開けるなら「1 冊が動いてよいか」をユーザーが決めてから**（Ⓐ の実装は Lab に差分で在る＝−0.95%・1 冊）／★ **先に要るのは ⑹ の LP 双子で `アゲハ蝶` を割らせること**／★★ **⒡ bow の carry 門を system ごとに割る 0.595%**（staff 1 以上で 97.7% 拒否・跨ぎの bow の扱いが要る）／★ **⒢ `measureToSystem` の残り 0.453%**（起票のみ）／★★ **⒵⁴ `prefixMarkAnchorX` の解き直し 0.338%**＝**memo は反証済み**（hit 率 0.53%）・土台の変更／★ **⒵⁵ `mm.g.split` 0.163%**（⚠️ `OrderBy` は*安定*ソート）／★ **⒵⁶ 文字の 318,784 回／81 綴り 0.130%**（安い日の仕事）／★ **⒴ `inkFirst`＋`inkLast` 0.53%**・未分割／★ **⒴⁵ artic と trill の remap 辞書は作って捨てている**（天井 0.174%）／★ **⒳″ `Padded` の中に残る複写 0.028%＋`SortAndResolve` 0.15%**（起票のみ）／⚠️ **`g4.core` 1.21% と `p1.s1.beams` 5.538% は*もう実仕事*＝この 2 島には戻らないこと**／⒜ **R13⒝ の実機確認**（第404 ⑵）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・未着手・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／LP 双子が要る R7〜R11／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑺ 終了時**: ★★★ **製品コードは 1 行も変えていない**（commit は docs 2 本＝§1 と RULES。**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 1 本・**終了時の未 push 29**（開始時は 27）・**最終 full `sessions/p423/run3.trx` 8743 / 0 / 3 / 8746**（⚠️ **第422 最終と同じ＝網を 1 本も足していない**・`Zz423`／`ZzStage`／`ZZPOISON` の grep は**コードに 0 件**）・§7.5 は**対象なし**（Core `+` 0 行／REF 0／OWN 0）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・棚卸し 2 本とも不変。計器と全文は Lab `sessions/p423/`（`estimate-tolerance.txt`＝脚 1・`estimate-channels.txt`＝脚 2・`estimate-channels-allbooks.txt`＝脚 3 と Le Freak の行・`ab-{before,after}-tc0-{1,2}.txt`・`hashes-{before,after}.txt`・`zz423-seams.diff.txt`・**`zz423-deferred-prelim.diff.txt`＝戻した実装そのもの**・`ZzStage.cs.txt`・`LayoutEngine.Zz423.cs.txt`・`Zz423Probe.cs.txt`・`ageha-twin.ly`・`ageha-{before,after}-page{1,2}.png`）。§7 3.5 は**セッションの頭で**済（`-Archive 421`＝23 行 7,013 chars）⇒ **HANDOFF は 448,626 B → アーカイブ後 436,299 B → 今便を書いて 447,180 B＝**天井 450,000 まで 2.8 KB しかない****。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 422` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第423 の欠陥ではない。
