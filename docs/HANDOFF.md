...
- **★ `note~@mark("X")` の印は、自分の住所に `~` を含む**（⑻）。**`data-pos` が 1 手前になるので、クリック／ハイライトの的が tie を巻き込む**。**絵は正しい。**第286 U3 と同じ族。
...
最終更新 第292セッション＝**第291 が自分に対して起票した §2 D の先頭項＝「crop が loose-line の伸びのぶん stale」を閉じた（1 件・commit 1 本 `f9ab3cbc`）**。**予約の消費者を 2 つに割った**——**下スカイライン／譜間の床は*最小*のまま、crop だけが*描かれた量*を読む**。**台帳は 1 点も動いていない**（748／exact 588／ss 非ゼロ 190／総和 24.714570188・**この便の量はどの点も読んでいない距離**）。**snapshot 5 枚が書き換わり、どれも 2 行差＝svg のヘッダ高と背景 rect だけ。**

★★★ **⑴ 「近似」だと思って書き始めたものが、途中で*恒等*になった**。**crop 用の読みは鎖を force 0（＝`max(min, ideal)`）で歩くだけ**で、**本当の鎖は紙に対して solve される**ので「force が正なら足りない」はずだった。**ところが crop 経路に留まる条件がその不足を禁じている**——**crop ＝ anchor ＋ depth ＋ margin、鎖の部屋 ＝ paper − margin − anchor** なので、**force 0 の crop が紙に収まるなら room ≥ depth、つまり force ≥ 0** で、**バネは本当に `max(min, ideal)` に座る**。**収まらない本は `OptimalPages` へ抜け、そこでは高さ＝紙で、この profile は 1 度も読まれない。** ⇒ ★★ **「この近似はいつ効かないか」を書こうとしたら、効かない場合が構造的に存在しないことが出た。近似の限界を書く作業は、限界が無いことの証明になりうる。**

⚠️⚠️ ★★★ **⑵ 3 本目の毒が*不発*で、そこが量の在処だった**（予測を外した収穫・§5.0）。**番人 ⒝ は「譜間の床は最小のまま」を主張する**ので、**「at-rest を下 EXTENT に畳む」を毒にすれば赤くなるはず**と書いた。**両方緑のまま 1 も動かなかった。** ⇒ **スカイラインを持つ本では、譜間の対を floor するのは*スカラーの下 extent ではない***——**`LayoutUtilities.InterSystemPairMinimum` は X 認識の `Distance()` を優先し**、**塊はそこへ `PagingAugmentProgram.Builder.AddLyricBand` 経由＝*profile* で届く**（スカラーは「silhouette を持たない system」用の fallback）。**正しい毒は `perSystemLyricBands` に at-rest を渡すこと**で、**それだと ⒝ が 20.152001 → 21.282041 で赤・⒜ は緑**。⇒ ★★★ **毒は量ではなく量の*綴り*を狙う。狙いを外した毒の緑は、番人について何も言わない**——**§5.0 の「弁別できない毒で緑を見て『確かめた』と書く」の、*弁別子はあるのに別の綴りに当てた*版。** ⇒ ★★ **判定法は 1 問**: **その床は誰に訊いているか**（スカラーか profile か）。**答えられないうちは、その毒はまだ狙いが定まっていない。**

⚠️⚠️ ★★★ **⑶ 「上流でスナップショットした値」は、下流がそれを育てる限り必ず腐る**。**最初の実装は `max(staffDown, block)` を `LayoutSystems` で凍らせて crop に渡した**——**その extent は*そのパスのあとで育つ***（**プレリミナリ注釈パスがスラー・タイ・強弱・スクリプトの実インクで下半分を持ち上げる**）。**結果は 40 枚の snapshot ＋ `PreliminaryPassSeedTests` が同時に赤**で、**全部「譜の下に mover が居る本」＝その持ち上げが買った部屋を失った本**だった。**直しは `CropDown` を*生きた* extent との `Math.Max` にすること。** ⇒ ★★ **「あとで育つ値」を上流で読むなら、読むのは差分か max であって、値そのものではない。**

★★ **⑷ 尻尾は今回も*情報*だった、そして今回は先に予測した**（第291 ⑵ の再演）。**番人 ⒜ の残差は 0 ではなく 1.451e-6 で出る**と実装前に書き、**実測 0.000001451**——**LP が同じバネについて公表する 5.500001451282664 の尻尾そのもの**（紙 2.5 nm ぶん）。**番人はこれを丸い tolerance に隠さず `InRange(0, 2e-6)` で*名指して*いる**（欠陥の 1.130041486 は 5 桁外）。

★ **⑸ 射程は第291 の予測どおり「ユーザーの本 0 冊」**。**第291 ⑷ は 3 冊の第 1 バネの床（6.119960／5.864960／6.290221）が理想 5.5 より高いことから 0 冊を*論じて*いた**——**本便は 326 冊を 1 冊ずつ掃いて 0 を*測った***。**タブの対 TBL1 も不動**（床 6.120115）。**動いたのは追跡 6 冊（歌詞 fixture 5 ＋ `samples/amazing-grace.lys`）と scratch のプローブ 25 冊だけ。**

★ **⑹ 2 本目の歩きは*差が出うるときだけ*歩く**。**どのバネの理想も床を超えなかったなら、at-rest の歩きは 0 歩目から最小の歩きと同一で、以降も帰納的に同一**——**だから `slack > 0` のときだけ第 2 パスを取り、それ以外は*同じインスタンス*を返す**（memo の値が参照同一なら paging-augment の baseline 比較も等値のまま）。**普通の歌本はこの速い枝**（⑸ の床がまさにそれ）。

⚠️ **見つけて直していないもの**（前便から引き継いだ分に加えて）:
- **★ 本便で新しく開いたものは無い。** ⑵ で名前が付いた「スカラーの下 extent は silhouette を持たない system 用の fallback」は、**欠陥ではなく地図**（§2 D の残りを読むときに要る）。
（以下、前便からの一覧はそのまま）

⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 の ▶ は 0 のまま、台帳の `OPEN:` も 0、§2 D の先頭項も閉じた。** ★ **手元で証拠が一番揃っているのは §2 U2 の残り 2 つ**——**⒜ `ClosingProfileWithoutTheRoom`（プレリミナリ・パスの閉じプロファイルが薄い＝部屋がまだ無い段では外側の層が取れない・観測者ゼロ）／⒝ ペダルの対応付けに譜／声部フィルタが無い件**。★ **次に読み手の目に触れそうなのは §2 U4 の残債＝タブの `as full` に連桁が無い**（**ユーザーは「符尾・連桁も含めて全部」と決めている**）。★ **ユーザーの本で今 LYS4018 が出ているのは `Untitled-6.lys` の 1 冊だけ**（第288 便の `@rit`）——**本便はその本を 1 バイトも動かしていない。**

★ **開始時裏取り**: HEAD `9a937dd3`・**未 push 12**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6557 / 0 / 4 / 6561**・台帳 **748／exact 588／総和 24.714570188／ss 非ゼロ 190／count 158 中 2・`OPEN:` 0**・追跡 `.lys` **579 冊**・snapshot **229 枚**——**§1 の終了時の数と 1 つも違わなかった**。
終了時: **commit 3 本**（`f9ab3cbc` code ＋ `8c6c21f4` §1 と §2 と第290 のアーカイブ ＋ **この 1 本＝両脚の緑**）＝**未 push 15**・木 0・未追跡 0。**suite 6559 合格 / 0 失敗 / 4 skip / 合計 6563**（開始 6561 から **+2**＝**番人 2 本。台帳の点は 1 つも足していない**——**crop は LP に対応物が無いので点で測れない**）。Core 0 エラー 0 警告。**追跡 `.lys` 579 冊**（不動）・**snapshot 229 枚**（不動＝**5 枚を*書き換えた***）。**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2＝すべて不動。**
⚠️ **掃きは 1 回**（**母集団はディスク上の全 `.lys`＝ユーザーの実コーパス込み・per-PID workspace・両側 Release**）: **1595 冊 rendered 1595 / ink MOVED 31 / midi MOVED 0 / レンダー失敗 0**。**31 の内訳は追跡 6（歌詞 fixture 5 ＋ `samples/amazing-grace.lys`）＋ scratch 25（第216/222/225/257/279/287/288/291/292 の歌詞プローブ）・ユーザー実コーパス 0 / 326。** **`lysc check` も同じ 1595 冊を両 exe で回して差 0**（**うち 538 冊は実際に診断を出す＝計器は盲目でない**）。
★★ **緑は*コード commit のあと*に取った**。★★ **WSL ubuntu Release も同じ 6559 / 0 / 4 / 6563**（**2 度**——**`f9ab3cbc`（コード）と `8c6c21f4`（docs）の両方の木で**・§0 の「この機械の Windows での緑でしかない」に両脚で答えた）。★ **docs の木でもう 1 度回したのは形式ではない**——**`HandoffArchiveContinuityTests` は docs を読む**ので、**§1 の回転（落とす＋入れる）が正しいと言えるのはこの run だけ**（第291 はまさにここで 1 度赤くした）。⚠️ **CI だけは未実行**（未 push 15）。
...
- **U9. ▶ `note~@mark("X")` の印は自分の住所に `~` を含む**（**第293 実測・小物**）。
  **絵は正しい**が、**`data-pos` が書かれた `@` より 1 手前**になるので、
  **エディタのクリック／ハイライトの的が tie を巻き込む**。**第286 U3（行頭の音符が
  自分の住所を持っていなかった）と同じ族**で、**`GreenNode` の span の取り方の問題。**
  ★ **射程はユーザーの本 56 か所**（**`~@mark` の綴りを使う 35 冊級**）——**インクは動かない。**
