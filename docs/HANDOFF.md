...
✅ ★★★ **⑹ 結果＝全木 2007 冊 × 5 出力（svg / midi / xml / ly / check）が*1 バイトも動かない***（`scratch/p310/sweep310.ps1` / `.json`・**両側 Release**・base `2bbaf14a`）。⚠️ **`NO-OUTPUT` が 2 冊在るが*両側とも*で、本便より前からの状態**（`scratch/p216/pins/chords-attach.lys`・`chords-row.lys`）。
...
★ **開始時裏取り**: HEAD `2bbaf14a`・**未 push 58**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6808 / 0 / 4 / 6812**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 点 158（非ゼロ 2）／OPEN: 0**・追跡 `.lys` 581・snapshot 231——**第309 の終了時の数と 1 つも違わなかった**（**素朴に数えると `exact` 556・`OPEN:` 12 が出る。§0 のレシピは `-le 1e-6` と `why -like 'OPEN:*'`**）。
終了時: **commit 4 本**（**⒝1 の実装 `fd1d9a1e`** ／ **§1・§2 U8・RULES と LP 実測 `6f655c70`** ／ **⒝2 の掃き母集団の数え直し＝`grace` は述語として狭かった `e35f7916`** ／ **この行＝この commit**）・**未 push 62**⚠️ **数を閉じる commit が数を動かすので、最後の 1 本は*自分を数えて*「この commit」と書く**（第305〜第309 と同じ形）・作業ツリー 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6808 / 0 / 4 / 6812**（**開始と同じ**）・**台帳 748／588／190／24.714570188／158 中 2／`OPEN:` 0・追跡 `.lys` 581・snapshot 231 は開始時と同じ**。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけ動いた**（**`UNWATCHED` の増減 0・定数 0・`audit/magic_constants.csv` も行番号だけ・REF/OWN の増減 0**）。⚠️ **間の push は無し**（`origin/master` は `be16a770` のまま）。★★ **WSL ubuntu Release は Windows と一致**（`fd1d9a1e` で **6808 / 0 / 4 / 6812**・build 8 秒 / test 26 秒）——**§0 の両脚を読んだ。** ★ **base 用 worktree `lys-base310` は `git worktree` からは外した**（`git worktree list` は `LilySharp` 1 本）——**ただし空のディレクトリ `C:\MyProj\lys-base310` が残っている**: **別の PowerShell コンソールがそこを cwd にしていて削除できなかった**。**中身は 0 ファイル。**
⚠️ **計器は `scratch/p310/` に全部残っている**: **`lp/`**（**LP 22 冊＋`dump.py`＋`measurements.md`**＝**スラー・タイ・script の 3 量と、その*対照*。⚠️ `dump.py` は curve を*4 つの制御点*で出す——slur/tie の決めている量は端点ではなく制御点だから**）／**`sweep310.ps1`/`.json`**（**2007 冊・5 出力・base `2bbaf14a`**）。
...
最終更新 第309セッション＝**§1 が名指した「次の一手」の 1 つめ＝§2 U8 の残り（⒞ スラーとタイ／⒟ 注釈の族／U8b）に着手し、*その設計そのものが要らない*ことを測って、ユーザー決定まで取った**。**入り方は第298〜第308 と同じ**——**ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った**。⇒ ★★★★ **本便の骨は「§1 が『住所を設計せよ』と出した宿題が、*隣の構文で 1 対測ると消えた*」**: **住所は欠けているのではなく届いていないだけで（⑵）／LP は cue と grace を Lily# と*逆*に持っており（⑶）／起票の「10 型」は 21 型だった（⑴）。** ⚠️ **コードは 1 行も変えていない。commit 2 本とも docs。**

⚠️⚠️ ★★★ **⑴ 起票の数を着手前に数え直したら、2 倍以上ずれていた。** **§2 U8 は「`ItemIndex` を名乗る模型型は 10 個」と書き、10 個を名指していた**——**実数は 21**（**落ちていた 11 型＝`ArpeggioItem`・`CrossStaffItem`・`GlissandoItem`・`GrobProperty`・`HairpinItem`・`LyricItem`・`OttavaBracketItem`・`PedalItem`・`PercentRepeatItem`・`TextSpannerItem`・`TrillSpannerItem`**）。⇒ **第300 は*⒞⒟ が要る型*を数えて、それを*存在する型*として書いた。** ★ **ほかの数**: **Core の非コメント `ItemIndex` は 421 箇所／順序・算術が 43／`Items[…]` 直接添字が 16。**（**RULES §5.0「起票の数値は着手する便が数え直す」の 6 例目**）

⚠️⚠️⚠️ ★★★★ **⑵ 「装飾音符が `ItemIndex` を名乗れない」は*機構が欠けている*のではなく*届いていない*。判定は隣の構文 1 対で付いた。** **`cue { }` の中にスラー・タイ・`@staccato` を書いて描かせたら 3 つとも描かれ、drop を 1 行も出さない／`grace { }` の中では 3 つとも対照とバイト同一で LYS4020 が出る**（`scratch/p309/ab`・両側 Release・`data-pos` 伏せ・**各プローブに*その印だけが違う*対照**）。⇒ ★★★ **差は 1 行**——**`ProcessCueRegion` は `ProcessMusicNodeSequence` を*同じ builder* へ通すので cue の音符は `measure.Items` の実項目になり、`CollectGraceNotes` は音高と音価だけを脇の配列へ読む。** ⚠️ **反証は最初から手元にあった**——**LYS4020 の文面自身が「a grace note *is not a measure item*, so there is no column for it to hang off」と書いていた**（**「添字が無い」ではなく「項目でない」。第178・第306・第307・第308 に続く *`observed by: NOTHING` の兄弟***）。⚠️ **タイの対は 1 度壊した**（対照 `{ d16 e16 }` 対プローブ `{ d16~ d16 }`＝**音高の差が「タイが描かれた」と読めた**・第60 の罠）——**`{ d16 d16 }` に直して取り直した。**

⚠️⚠️⚠️ ★★★★ **⑶ 正典を読んだら、Lily# は cue と grace を*逆*に持っていた。** （`C:\MyProj\lilypond-src` @ `v2.26.0`）: **`ly/engraver-init.ly:432` に `\name CueVoice`＝*cue は独立 context*／`grep -c "name Grace"` は `0`＝*grace は context ではない*。`:368` は `\name Voice` の中の `\consists Grace_engraver`（コメント「sets properties」）で、`lily/grace-engraver.cc` の `make_item|make_spanner|Grob *` は 0 件＝*grob を 1 つも作らず、grace time に出入りするとき設定を切り替えるだけ*。** **⒞⒟ が要る engraver は全部その同じ Voice に `\consists` されている**（`Slur_engraver`・`Tie_engraver`・`Script_engraver`・`Script_column_engraver`・`Beam_engraver`・`Stem_engraver`・`Dots_engraver`・`Note_heads_engraver`・`Rhythmic_column_engraver`）。⇒ ★★★★ **だから LP は ⒞ も ⒟ も*特別なコードを 1 行も持たずに*描く。** ⇒ ★★★★ **そして Lily# は、LP が独立 context を与えたほう（cue）を普通に歩き、LP が普通の Voice に置いたほう（grace）を脇の模型へ持ち出している**——**⒞・⒟・U8b が「同じ 1 つ」なのは*住所を共有するから*ではなく、*同じ 1 つの構造から締め出されているから*。** ⚠️ **⇒ `VoiceContextId.Grace` を足すのは誤り**（**`Cue` が在るのは `CueVoice` が実在の context だから。grace は context ではなく*時間の領域***）。

⚠️⚠️ ★★★ **⑷ ⇒ 「住所を設計する」は*やってはいけなかった仕事*だった。** **設計していたら住所の第 2 の綴りができ**（RULES §5.2.1②）、**LP に無い分離を模型に彫っていた。** ★★ **そして §2 の*本文*は最初から正しい修理を書いていた**——「**装飾音符に住所を足すのではなく*本体を普通の walker で歩く***」。**§1 がそれを「住所の問題」と 1 語に畳み、次の便はその要約から始めた。** ⇒ ★★★ **判定法として RULES §5.3 に汎化した**: **§1 の一文と §2 の本文が食い違ったら*本文*を採る／「X ができない」の起票は*隣の構文*で 1 対測ってから読む／相手がその 2 つを同じ族にしているかを `grep -c "name <構文>"` と engraver の `make_item` の 2 行で先に訊く。**

✅ ★★★ **⑸ ユーザー決定＝⒝「grace を普通の walker で歩く」**（**⒜「複合住所 `(itemIndex, graceColumn)` を足す」は採らない**）。**並べ方に実装・移行コストは混ぜていない**（ユーザーの原則）。⚠️ **第309 は決定を取っただけで実装は未着手**——**理由は下の「次の一手」に数で書いた。**

⚠️⚠️ ★★★ **⑹ ⒝ の射程を測った＝§2 が「snapshot が動きうる・未測定」としか書けていなかった数。** **ディスク 2007 冊のうち grace を書く本は 166**（`scratch/` の過去便プローブ 135 ／ `audit/` 20 ／ Tests fixtures 11。**ユーザーの 326 冊は 0**）／**追跡 `.lys` 581 冊のうち 31**／★★★ **再ベースが要る snapshot は 9 枚で、全部名前が分かっている**（`showcase/02-ornaments`・`test/grace-accidental-line-start`・`test/grace-chord-accidental`・`test/grace-lower-staff`・`test/grace-notes`・`test/ossia-beams`・`test/single-staff-arpeggio-grace`・`test/tab-grace-slur`・`test/tab-grace`）。⇒ ★★ **つまり ⒝ は「承認待ちで止まる大手術」ではない。** ⚠️ **この 9 枚も 1 度 0 枚と出した**——**snapshot は `<群>__<本>.svg` なのに拡張子を落としただけの名前で突き合わせたため**（**RULES §5.3「0 は『測れていない』の顔で出る」**）。

⇒ ★★★ **次の一手**: ★★★★ **⑴ §2 U8 ⒝ の実装＝grace body を普通の walker で歩く。1 便まるごとをこれに充てること。** **段階に割れない**——**「項目を足す」と「脇の配列を畳む」を*同じ 1 歩*でやらないと、木が「同じ音楽が 2 か所に在る」二重状態になる**（**MIDI は二度鳴り、XML は二度出る**）。**型は §2 U8 に書いた**（`ProcessGraceRegion`／`_graceDepth`／`AddItemWithoutDuration`／**印を要るのは描画・spacing・MIDI・MusicXML・LilyPond export の 5 つだけで、`.Items` を読む 325 箇所の大半は*何も変えなくてよい*——grace は普通の Voice のイベントなので普通の項目として見えるのが正しい**）。**網は全木 4 出力 sweep（2007 冊・うち grace 166 冊）。** ★★★ **⒝ の中に分岐が 2 つ在るので、入る便はまずそれを選ぶこと**（**§2 U8 に書いた**）: **⒝1＝住所のためだけに項目にし、脇の配列はその項目から*導出*する**（**ページは動かない＝掃きでバイト同一を示せる**）／**⒝2＝脇の配列を畳んで普通の engraver に描かせる**（**1620 行が畳めるがページを計算し直す**）。★★ **符頭と臨時記号は ⒝2 でも測り直しが要らない**——**`GraceColumnHeads` 自身が「A TRANSLATION, NOT A SECOND MODEL」と書いており、フル サイズの和音と*同じ* `ChordHeadPositioning`／`AccidentalPlacement` を grace のフォントで呼んでいるだけ**（第309 が確認）。**残る grace 固有は 4 つだけ＝spring・梁の接頭辞と quant・acciaccatura の斜線・主音符への anchor。** ⚠️⚠️ ★★★★ **入口は*まだ 1 度も測っていない LP の量*＝grace body の中のスラー／タイ／script の座標**（**第309 は LP に*ソースしか*訊いていない**）。**正典 exe は 13 分・17 分ブロックの前科（§2 G）。** ⚠️⚠️ **第309 は「このセッションでやる方が有利か」を訊かれ、*着手しないと答えた*。理由は 3 つ**: **⑴ 次の 1 歩は*長く遅延の読めない LP 実測*から始まり、本便が持っている文脈（cue と grace の逆転という*閉じた議論*）はそこに 1 つも効かない**／**⑵ 移行は二重状態の点で割れず、しかも snapshot 9 枚の承認で 1 度止まる**／**⑶ 本便の文脈で安いのは*読んだばかりのファイル配置*だけで、それは §2 U8 に全部書き出したので grep 数回で建て直せる**（**第307 が U8 を見送ったのと同じ判断・同じ理由**）。★ **逆に「いま読んだから安い」ものだけを置いた＝⒝1/⒝2 の分岐と、LP に訊く 3 つの量。**／**⑵ §2 U8c**（cue の和音の 0.004270。**⒝ と独立**）／**⑶ §2 の X/Y 側**（§2 A/B/D/E）／**⑷ §2 C⑴ の多声 walk の moment 順への再設計**（**最大の構造負債・未着手**）。⚠️ **承認待ちは無い**（snapshot は 1 枚も動いていない）。⚠️ **§2 F は開いている項目が無い。**

★ **開始時裏取り**: HEAD `e050c73a`・**未 push 55**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6808 / 0 / 4 / 6812**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 点 158（非ゼロ 2）／OPEN: 0**・追跡 `.lys` 581・snapshot 231——**第308 の終了時の数と 1 つも違わなかった**（**素朴に数えると `exact` 556・`OPEN:` 12 が出る。§0 のレシピは `-le 1e-6` と `why -like 'OPEN:*'`**）。
終了時: **commit 3 本**（**§2 U8 の訂正と第309 の骨・RULES §5.3 の 2 則 `4c970ccb`** ／ **§1・§2 U8 の射程の数・第307 をアーカイブへ `bfa8f514`** ／ **⒝1/⒝2 の分岐と LP に訊く 3 つの量＝*着手しないと答えた便の産物*（この commit）**）・**未 push 58**⚠️ **数を閉じる commit が数を動かすので、最後の 1 本は*自分を数えて*「この commit」と書く**（第305〜第308 と同じ形）・作業ツリー 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6808 / 0 / 4 / 6812**（**開始と同じ＝コードは 1 行も変えていない**）・**台帳 748／588／190／24.714570188／158 中 2／`OPEN:` 0・追跡 `.lys` 581・snapshot 231 は開始時と同じ**。⚠️ **`docs/APPROXIMATIONS.md` は不動**（**定数も `audit/magic_constants.csv` も REF/OWN も増減 0**）。⚠️ **間の push は無し**（`origin/master` は `be16a770` のまま）。⚠️ **ubuntu は回していない**（**動いたのは `.md` だけ**）。★ **base 用の worktree は建てていない**（**A/B 掃きをしていない＝コード変更が無い**）。
⚠️ **計器は `scratch/p309/` に全部残っている**: **`ab/`**（**プローブ 10 冊＝cue と grace に同じ 3 つの印を書いた対と、その*印だけが違う*対照。⚠️ タイの対は最初 pitch が違っていて壊れており、直した経緯も含む**）／**`measurements.md`**（**上の ⑴〜⑹ の数え方とコマンドを全部書いた**）。
⚠️ **LP は WSL も正典 exe も 1 度も回していない**——**本便が LP に訊いたのは*ソース*だけ**（`ly/engraver-init.ly` と `lily/grace-engraver.cc`）。**座標は 1 つも測っていないので、⒝ の実装便は幾何を LP に訊き直すこと。**
...
- **U8. ▶ 容れ物は全部閉じた（第298 ＋ 第299 ＋ 第300 ＋ 第301 ＋ 第302）。残るのは*本当に grob を要る*ものだけ＝`grace { }` の本体は*歩かれない*。起票の「注釈を運ばない」より広かった**
  ...
  *難所の形*だった**）。
  ...
  ✅✅ ★★★★ **【⒝1 は第310 が閉じた（`fd1d9a1e`）——grace body は普通の walker が歩き、ページは 1 バイトも動かない】**
  ...
  *1 バイトも動かない*** (`scratch/p310/sweep310.ps1` / `.json`・両側 Release・base `2bbaf14a`)。
...
>
> ## ✅ **⒰ ヘッダだけの宣言を form が鳴らすのは誤り＝第306 で拒否になった**（起票第306・ユーザー決定 2026-08-31・実装同便・`LYS1036` `2833cb2b`）
...
>
> ## ✅ **⒭ 閉じた＝ヘッダ位置は第305・cell の option 位置は第306**（起票第303・第305 が測り直して 3 つに分け・`LYS1035` の 2 つの位置 `0eaa5d1b` と `07c8fc87`）
...
- ✅ **⑶ record モデルの同値性（identity の欠如）**（**2026-08-31・第307 に閉じた**。**方針と判断軸は第306**）。
  ...
  *意味*版——**数え方だけでなく「何を数えたか」も書く**）。
  ⚠️ **本便の commit メッセージ `c161b76f` はこの「30」をそのまま引いてしまった**（訂正はここ）。
