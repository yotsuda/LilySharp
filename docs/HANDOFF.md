...
★★★ **⑴ 先に計器で数えた**（一時カウンタ `LilySharp.Core/ZzMapProbe.cs` ＋ 網 `LilySharp.Tests/Zz407Probe.cs` を、3 綴りの builder（`LayoutUtilities.BuildMeasureMap`／`BuildMeasureLayoutMap`／`SpannerBreakSubstitution.BuildMeasureToSystemMap`・呼び手は StackTrace）と手書き inline 11 箇所に置いた・**commit せず全部手で外した**・Release・TC=0・打鍵 1 回・時間は測っていない＝回数と割当だけ）。⚠️ **builder だけ数えたら見えなかった**——打鍵あたり 14 本のうち builder 経由は 3 本で、残り 11 本は手書きの inline だった。**結果（`sessions/p407/mapfills-before-raw.txt`）: 小節→system／→(system, layout) の全曲辞書を打鍵あたり plain1k 14 本／14,000 entry・fingbeam1k 18 本／20,000・v2bow1k 16 本／16,000＝全部 1000 小節の走査で、edit 位置（bar 2 と bar 998）で 1 本も動かない。** 内訳（注釈 pass は打鍵ごとに 2 回走るので「calls 2」が 1 pass 1 本）: pass 自身の `staffYAt` 用・pedal 参照・`OutsideStaffStacker` の Below／Above／Above core（**`StackAboveStaffMemoized` が建てた直後に `StackAboveStaffCore` が同じものを建て直す**・その註が「the same map the core builds (**cheap**: one dictionary fill)」と自白していた）・`ComputeFingeringsAndScripts` の `layoutOf`＋`systemOf`・`ArticulationEngraver` の `layoutAt`・第406 が入れた尾部の `tailMeasureMap`。

★★★ **⑵ 直し＝pass の頭で 2 本建てて配る**（`c18ca01a`・Core 4 本・網 3 本・CHANGELOG・棚卸し 2 本は行番号のみ）。`CalculateAnnotationLayouts` の先頭で `measureToSystem`（system 添字の半分）と `passMeasureMap`（(system, layout) の半分＝第406 の `tailMeasureMap` を頭へ hoist）を 1 本ずつ建て、`staffYAt`・pedal・`StackBelowStaff`・`StackAboveStaff`・`ComputeFingeringsAndScripts`・`ArticulationEngraver.CalculateWithFingerings` に渡す（**各家は `prebuilt…= null` の既定で自前建設を残す**＝CLI と per-system の呼び手は不変）。`StackAboveStaffMemoized`→`Core` と `StackBelowStaffMemoized`→`Core` は自分の表をそのまま下へ渡す。**`layoutOf` は丸ごと消えた**（使われていたのは鍵集合と 1 参照だけで、どちらも共有表から出る）。**後**（`mapfills-after-raw.txt`）: **14 → 8／18 → 8／16 → 10 本・pass 自身は 5〜9 本 → 2 本**。⚠️ **時間はこの機械の床以下なので時間の主張は書いていない**（割当は打鍵あたり 0.5〜1 MB 減・桁が小さい＝根拠は回数）。健全性＝**全綴りが重複 MeasureIndex で「最後の system」を採る**（`ComputeFingeringsAndScripts` の unit plan がその性質から導かれている）。**網 3 本 `MeasureToSystemHandoffTests`**（上下の stacker × memo あり／なし・**snapshot が見ていないのは*自前建設の腕*＝pass を通らない呼び手**なのでそこを押さえる）。★★★ **無観測の罠を踏まずに済ませた**: 網は**わざと 1 ずれた表**を食わせて赤くなるのを見てから採った——**最初の below 側の網は壊れた表でも緑だった**（per-system profile の down 側を定数にしていたので、どの system も同じ Y に置かれ表が答えに出ない）。毒の側も: **共有した system 表を空にすると 205 本、layout 表を空にすると 148 本が赤**＝どちらの腕も無観測ではない。**full `sessions/p407/run4.trx` 8700 / 0 / 3 / 8703（+3＝網のぶん）**・台帳 851 点／総和 22.584727806 不変・snapshot 249 不動。⚠️ 踏んだこと 2 つ＝⑴ `out MeasureLayout` の局所関数で CS8601（`MeasureLayout` は参照型）＝`[MaybeNullWhen(false)]` を付けた（**Core の 0 警告**）／⑵ 一時計器を PowerShell で剥がすとき `ReadAllText` は BOM を文字列から落とすので書き戻しで BOM が消える＝1 ファイルが差分に出た（`git checkout` で戻した・改行は全ファイル CRLF のまま）。

★ **⑶ 汎化**: RULES §5.3 の第406 第 3 便の項に「1 家を畳んだら同じ表を建てている家を pass 全体で数える（尾部だけ見ると尾部で終わる）・builder だけでなく手書き inline も計器に通す・『安い』と書いてあるコメントが 2 本目の在処・自前建設の腕は snapshot が見ていないので差分の網を足し、その網はわざと壊した表で赤くなるのを確かめてから採る」を足した。

⇒ **次の一手**（⒜⒝⒝′⒟ は第406 から変わらず）: ⒜ **R13⒝ の実機確認**（第404 ⑵ のとおり＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item（第404 ⑵′・実機の 2 行を見てから）／⒝′ frame 変更の `applyFrame`（実機で頻度を見てから）／⒞ 第403 の残り＝count loop の page DP 5.3 ms（R13⒦・census で hit 率）・診断 pass の残り（`AnnotationName` 14／`MeasureValidator` 10／`LyricSings` 6.5）・`RestCollisionsOf`／`RestDotOffsetsOf`（第405 ⑶′＝0.4 ms／0.3 MB＝**要ユーザー判断**）／⒞′ **prelim の残り**＝fingbeam1k の `fs.walk` 0.41 ms／1.5 MB と `fs.assemble` 0.29 ms／4.0 MB（24,000 digit の実仕事＝「何を返すか」を変える別の島）・尾部の `t.mmr` 247 KB・`t.ledger`／**⒞″ ✅ 測って「やらない」と決めた（第 2 便・下の ⑸）＝残りは畳む価値が無い**／⒟ LSP 起動直後の tier-0（ReadyToRun・`TieredPGO` の knob は packaging 側＝**要ユーザー判断**）／LP 双子が要る R7〜R11／R15（要承認）。

★★ **⑸ 第 2 便（同日・指示「このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」→ 計器の型と残りの内訳が載っているので ⒞″ を採り、**数えた結果「やらない」と決めた**。コード変更 0 行）。⑵ の後の残りを 4 箇所に計器を戻して数え直し、**2 声・2 staff の `perf-hairpingrand1k.lys`（999 小節・grand staff）を 4 冊目に足した**（結果 `sessions/p407/leg2-before-raw.txt`）。**打鍵あたりの内訳**: plain1k 8 本＝**注釈 pass 4（2 種 × 2 pass＝これ以上減らない）**＋`PagingSkylines` 2 ＋ `Prelim.LayoutBeamsMemoized` 1 ＋ `CalculateRestShifts` 1／fingbeam1k 同じ／v2bow1k 8＝pass 4 ＋ `PagingSkylines` 2 ＋ `Prelim.MeasureToSystemOf`（ties／slurs）2／**hairpingrand1k は pass 4 ＋ `PagingSkylines` だけ**。⇒ ★★★ **畳める余地は 1 冊あたり 2 本**で、その代償は `RunPreliminaryAnnotationPass` → 3 つの per-staff ヘルパと **public の `CalculateRestShifts`** の署名、それに final の staff ループからの hoist＝**配管のほうが大きい**。⒜ **`PagingSkylines` の 2 本は畳めない**（`n = Math.Min(perSystemExtents.Count, systems.Length)`／`s < systemSkylines.Count`／`s < systemCount`＝**全部*狭い*表**で、full の表を渡すと後ろの system の小節が今まで素通りしていたのに解決するようになる＝**別の量**。第406 の「同じ表か」を字面でなく*境界*で確かめること）／⒝ **注釈 pass の 4 本が床**（2 種 × 2 pass で、pass ごとに systems が別の配列）／⒞ `MusicMarkEngraver` の 2 本も**別の量**（staff group から出す bottom）。⚠️ **正直な留保**: `LayoutBeamsMemoized` と `CalculateRestShifts` は**どちらも per-staff ループの中**なので構造上は staff 数で増えるが、**それを測れる本が木に無い**——hairpingrand1k で 0 本なのは staff が 2 つだからではなく**この本が 4 分音符だけで梁が 1 本も無い**から（両方とも梁ゼロで早期 return する）。**梁のある多 staff の本を作ってまで追う価値は無いと判断した**（1 本/staff × 2 箇所）。⇒ **⒞″ は閉じる。次便はこの島に戻らないこと。**

★ **⑷ 終了時**: commit 2 本（code `c18ca01a`＋docs）＋第 2 便の docs 1 本（コード変更 0）・**最終 full `sessions/p407/run4.trx` 8700 / 0 / 3 / 8703**（第406 最終 8697 / 0 / 3 / 8700 に対し網 +3）・§7.5（対 `cee2c758`＝スクリプトの出力）**Core `+` 106 行／REF 0／OWN 0**（数値定数なし・LP 由来の式なし＝既存の辞書を建て直さずに渡す配管と辞書 1 本の削除＝監査対象は「同じ答えか」＝snapshot 249 不動＋毒 205／148 本＋差分の網 3 本）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。§7 3.5 は済（第405 を `-Archive 405`＝15 行 5,290 chars）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。
...
★★★ **⑴ 先に計器で段を割った**（`sessions/p406/Zz406Probe.cs.txt`＝`RunPreliminaryAnnotationPass` と `CalculateAnnotationLayouts`（両 pass）に一時マーク・`GetOrComputePagingAugment` に一時カウンタ・**commit せず `git checkout`／手戻しで消した**・Release・TC=0・6 打鍵・一声かけずに回した）: **注釈 engraver 群は ~0**（この 3 冊に無い族は即帰る・above／below／fingering／verse／paging-augment の memo は 198〜199/200 hit）。**残っていたのは `p.beamdetect`＝`MultiStaffLayouter.StaffBeamGroupsOf`（`Staff` 鍵の CWT）→`BeamGroupsOf`（`Voice` 鍵）→`BeamDetector` の全 staff 走査＝毎打鍵 1000 小節・2,000 群**（plain1k **2.8 ms／2.77 MB**・fingbeam1k **3.4／4.1 MB**・v2bow1k 0.25／0.27 MB＝二声の fan・梁なし）＝**prelim 段の 65%**。両表は edit が作り直す instance を鍵にした scratch（RULES §5.3・R13⒧ と同じ顔）で、**collect 側の probe は同じ 1000 小節を `IncrementalCompiler._beamMemo` で 1000/0 replay していた**。他は `p.beamlayout`（200 memo lookup）0.3・`p.scripts` 0.5／1.6 MB（fingbeam1k 1.2／7.2 MB＝fingering 24,000）・`p.tail` 0.3／0.9 MB・v2bow の ties／slurs 検出 0.2＋0.3・`p.augment`（bow 1000 本の program 組み）0.5／3.5 MB。結果 `sessions/p406/prelim-before.txt`（生 `prelim-before-raw.txt`・`-raw2.txt`）。

★★★ **⑵ 直し＝layout 側の検出にも collect と同じ per-measure memo を持たせ、replay は生きた item に結び直す**（`7bc22207`・Core 8 本・網 3 本・CHANGELOG・棚卸し 2 本は行番号のみ）。**`SystemLayoutCache.BeamDetection`**＝`BeamDetectionMemo` の 2 つ目の instance（`SetContentKeys` で generation・cache と一緒に shed）→ `LayoutEngine.Layout` が `MultiStaffLayouter.BeamDetectionMemo` に渡し、`BeamGroupsOf` → `ElementCoordinator.DetectBeamGroups(score, memo)` → `BeamDetector.DetectBeamGroups(Score, memo)`（一声の path は memo・fan は既存の eligibility gate（voiceIndex≠0／forceStemUpAt）で素通り＝collect probe と同じ gate）。**`ReplayWithLiveItems`**（memo の init flag）→ replay 枝で `BeamGroup.WithLiveItems(measure, measureIndex)`＝各 member を `measure.Items[ItemIndex]` に結び直す（`BeamMember.WithItem`）——collect の bake は index で住所を引くので要らないが、**layout の quanter・skyline seed・tuplet・script engraver は `Member.Item` を読む**（前打鍵の instance を読んでしまう）。健全性＝memo 自身の主張（鍵 `AddDetectionInputs` が読み集合を全部畳む＝生きた item は畳んだ全 field で一致・検出由来の field は live 検出と同値・Debug の drift net `VerifyReplayAgainstLiveDetection` が layout の replay も見る）。**後**（`prelim-after.txt`）: **歩く小節 1000 → 0**（計器は 2 text 交互なので `layoutBeamMemo=(1000, 0)`・実 session は 999/1）・`p.beamdetect` **2.8 → 0.26〜0.30 ms／2,767 → 829 KB**（fingbeam1k 3.4 → 0.33〜0.39／4,079 → 829 KB・残る 829 KB は 2,000 群の結び直しコピー）・**prelim 段 4.38 → 1.72 ms／6.5 → 4.5 MB（plain1k）・5.95 → 2.73／13.3 → 10.0 MB（fingbeam1k）**・final pass と他の段は不変・v2bow1k 不変（fan）。**網 3 本**: `IncrementalCompilerTests.LayoutBeamMemo_ChainedPitchEdits_RedetectOnlyTheEditedBar_AndMatchFullRecompile`（初回 (0,8)・1 打鍵目と*連鎖した* 2 打鍵目がともに (7,1) で full と同一＝第190 の「2 打鍵目で出る」）／`LayoutBeamMemo_InsertedBar_ReplaysTheShiftedBars_AndMatchesFullRecompile`／`BeamDetectionMemoTests.ReplayWithLiveItems_PointsEveryMemberAtTheLiveVoice`（全 member が live voice の item・surface は live 検出と一致・既定 memo は storing 側の item を保つ＝flag が load-bearing）。**full `run2.trx` 8693 / 2 / 3 / 8698（+3）→ 赤 2 本は棚卸し（`TheInventoryIsNotStale`・`TheCensusIsNotStale`＝行番号の移動だけ・`LILYSHARP_UPDATE_DOCS=1` で再生成し、行番号を落として 116 行が 1 対 1 で一致するのを機械で確かめた）**。⚠️ 踏んだこと＝棚卸しの再生成は LF で書く（autocrlf が正規化する・warning が出るだけ）。

★ **⑶ 汎化**: RULES §5.3 の第405 の項に「collect と layout が同じ計算を 2 度走らせ、collect 側に content 鍵の memo が在るなら layout 側にも同じ機構の 2 つ目の instance を置く。ただし読み手が model の参照を読むなら replay を生きた instance に結び直す」を足し、同じ顔の家の一覧を「数えた結果」に書き換えた。

★★★ **⑷ 第 2 便（同日・指示「このセッションが有利なら着手」→ 計器の型と prelim の内訳が載っているので ⒞ の次の項目＝`p.scripts` を採った）＝`ComputeFingeringsAndScripts` を割ったら、*walk そのものではなく*「誰のためでもない仕事」が 2 つ出た**（`sessions/p406/Zz406Probe2.cs.txt`・結果 `fingscripts-before.txt`／`-after.txt`・両 pass を割った＝この段は打鍵ごとに 2 回走る・Release・TC=0・6 打鍵・commit せず手で戻した・一声かけずに回した）。**直し 1 本**（`3ff919d0`・Core 2 本・網 2 本・CHANGELOG・棚卸し 1 本は行番号のみ）:

**⑴ script も指番号も無い本が、apparatus を丸ごと回して空を返していた**。plain1k は articulation も fingering も持たないのに、pass ごとに beam tip 表（2,000 梁の 8,000 member）・小節ごとの beam／slur 表・unit plan を建て、200 unit 全部を memo に問い合わせていた＝**0.44 ms／1,445 KB／pass・打鍵あたり 0.87 ms／2.9 MB**（17.5 ms の打鍵の 5%）。memo は 199/1 と報告していたが、配っていた値は全部空配列。⇒ **入口に門**（`AnyFingering`＝island が歩くのと同じ voice を 1 item 1 read・割当 0・**placed 小節で絞らない＝安全側**）＝下流の guard が既に畳んでいる条件（`CalculateWithFingerings` の「両方空なら空」＋island の `Fingering.HasValue`）の前倒しで、新しい述語は作っていない。**後**: 0.01 ms／0 KB／pass・**memo は 1 度も訊かれない（0/0）**＝門の発火はこの数で数える。

**⑵ beam tip 表を、miss する 1 unit のために毎回全曲ぶん畳んでいた**。`tips` は全曲 8,000 件の 1 表を pass ごとに建てて全 unit の island に渡していたが、memo が 200 中 199 を hit させるので**呼ぶのは 1 unit だけ**で、その unit が読むのは自分の ~5 小節。⇒ **miss は自分の梁だけ畳む**（`BuildBeamsByMeasure` が既にその unit の (staff, 小節) に attribution していて、それは unit の memo 鍵の材料そのもの）。健全性＝**鍵が一意**（1 音符は 1 梁・鍵は staff と voice を含む＝別の梁に上書きされ得ない）。**後**（fingbeam1k）: `fs.tips` 0.23 ms／818 KB → **0／0**／pass・段 1.19 → 0.95 ms・7,004 → 6,155 KB・**打鍵あたり 0.50 ms／1.7 MB**・counts 不変（units=200 hits=199 misses=1 liveBuilt=120 fingerings=24000）。

**網 2 本**: `FingScripts_BookWithNeitherScriptNorDigit_NeverProbesTheMemo_AndMatchesFull`（門の liveness＝memo 0/0＋full と同一）／`FingScripts_BeamedDigits_PerUnitTips_MatchFullAcrossChainedEdits`（**梁付き**の指番号本を別 system で 2 連打鍵・各回 full と同一）——⚠️ **既存の `FingScriptMemo` の網 4 本は全部*梁なしの 4 分音符*で、tip 表に 1 本も届いていなかった**。**full `run4.trx` 8696 / 1 / 3 / 8700**（赤 1 は棚卸しの行番号・再生成して 6 行が数字を伏せて 1 対 1 と機械で確認）・snapshot 249 不動・台帳総和不変。⇒ **残りは仕事そのもの**＝fingbeam1k の `fs.walk` 0.41 ms／1.5 MB（24,000 digit の script-column walk）と `fs.assemble` 0.29 ms／4.0 MB（24,000 `FingeringLayout` を結果配列へ複写・うち live は 120）＝**下げるには「何を再計算するか」ではなく「何を返すか」を変えることになる＝別の島**。汎化は RULES §5.3 に置いた（「hit 率の高い memo を見たら miss が受け取る引数を数える」「入口の門は下流の guard が畳んでいる条件の前倒し・発火は counter 0/0 で数える」）。

★★ **⑸ 第 3 便（同日・指示「このセッションが有利なら着手」→ 計器の型と尾部の位置が載っているので ⒞′ の `p.tail` を採った）＝尾部を engraver ごとに割ったら、門で飛ばせる walk は 1 つも無く、出たのは「1 つの量が 4 綴り」だった**（`sessions/p406/Zz406Probe3.cs.txt`・結果 `tail-before-after.txt`・両 pass・Release・TC=0・6 打鍵・commit せず手で戻した）。**直し 1 本**（`3859e056`・Core 4 本・網 0 本＝既存の 249 snapshot が観測者・CHANGELOG・棚卸し 2 本は行番号のみ）。⚠️ **計器の読み方**: 各 mark は 1 つ前からの差なので、pass 入口の次の mark（before の `t.grace`・after の `t.map`）は**その engraver ではなく pass 本体全部**を測っている。grace 自身は `graceNotes.IsDefaultOrEmpty` で即 return し 3 冊とも grace を持たない＝費用 0。**尾部の 3 家（半タイ・全休符・加線）が各々全曲の小節→(system, layout) 辞書を建て、半タイは 2 本建てていた**（`BuildMeasureLayoutMap` ＋ `BuildMeasureMap`＝鍵集合は構成上同一）＝**pass あたり 4 本・打鍵あたり 8 本**、しかも半タイと全休符は**3 冊とも出力 0** でそれを払っていた。⇒ **pass が 1 度建てて 3 家に渡す**（各家は既定 null で自前建設を残す＝CLI と per-system の呼び手は不変）・半タイの 2 本は 1 本に畳んだ。**後**: tievar 228 → 1 KB・mmr 375 → 247・ledger 298 → 170（plain1k・共有表 ~128 KB を戻して **pass あたり約 355 KB・打鍵あたり約 710 KB**・3 冊とも同じ幅）・**辞書構築 8 → 2 回／打鍵**。⚠️ **時間は約 0.05 ms／pass＝この機械の床以下なので時間の主張は書いていない**。★★★ **無観測の罠を踏まずに済ませた**: 3 冊とも `tievar=0` なので perf の掃きはこの畳み込みについて何も言っていない——`@laissezVibrer`／`@repeatTie` を持つ fixture が 11 冊在ることを確かめ、**engraver を毒して空を返させると 17 本が赤くなる**ことを見てから「snapshot 249 不動」を証拠に引いた（§5.3）。汎化は RULES §5.3 に置いた。

★ **⑹ 終了時**: commit 6 本（code `7bc22207`＋docs `6d062926`＋第 2 便 code `3ff919d0`＋docs `12af9da1`＋第 3 便 code `3859e056`＋docs）・**最終 full `sessions/p406/run7.trx` 8697 / 0 / 3 / 8700**（第405 最終 8692 / 0 / 3 / 8695 に対し網 +5）・§7.5（対 `40a92a2c`＝便の全体・スクリプトの出力）**Core `+` 246 行／REF 0／OWN 0**（数値定数なし・LP 由来の式なし＝memo の配管・live item への 1 コピー・仕事を飛ばす門 1 つ・畳みを読み手へ 2 つ移動＝監査対象は「同じ答えか」＝incremental==full の網 5 本＋drift net＋毒 17 本＋snapshot 249 不動）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。§7 3.5 は済（第404 を `-Archive 404`＝13 行 6,206 chars）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も・Lab は sessions/p406 を commit 済み）。⇒ **次の一手**: ⒜ **R13⒝ の実機確認**（第404 ⑵ のとおり＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item（第404 ⑵′・実機の 2 行を見てから）／⒝′ frame 変更の `applyFrame`（実機で頻度を見てから）／⒞ 第403 の残り＝count loop の page DP 5.3 ms（R13⒦・census で hit 率）・診断 pass の残り（`AnnotationName` 14／`MeasureValidator` 10／`LyricSings` 6.5）・`RestCollisionsOf`／`RestDotOffsetsOf`（第405 ⑶′＝0.4 ms／0.3 MB＝**要ユーザー判断**）／⒞′ **prelim の残り＝fingbeam1k の `fs.walk` 0.41 ms／1.5 MB と `fs.assemble` 0.29 ms／4.0 MB（24,000 digit の実仕事＝「何を返すか」を変える別の島）・尾部の `t.mmr` 247 KB（`FindRuns` と percent 集合＝自前の計算で誰かの表の写しではない）・`t.ledger`（出力の在る冊では実仕事）。⚠️ **同じ「1 つの量が N 綴り」はまだ pass の中に在る**＝`CalculateAnnotationLayouts` は staffYAt 用の `measureToSystem`・`ComputeFingeringsAndScripts` の `layoutOf`／`systemOf`・`ArticulationEngraver` の `layoutAt` と、尾部の共有表とは別に同じ表を 3〜4 綴り建てている（第406 第 3 便は尾部の 3 家だけ畳んだ）**／⒟ LSP 起動直後の tier-0（ReadyToRun・`TieredPGO` の knob は packaging 側＝**要ユーザー判断**）／LP 双子が要る R7〜R11／R15（要承認）。
...
- **R15 dead code（削除は要承認）**＝`SharedRenderer.DrawLedgerLines`・`EngravingDefaults.BeamSpacing`・`IdealStemLength`（`DefaultStemLength` の 2 綴り）・`BeamScoringProblem` の `stemPositions`/`_isTab`・`SpacingRules.AdjustSpringForGraceNotes`・`GraceNoteEngraver.GetGraceGroupWidth`・`BreakAlignSpacing.CalculateDistance`・`LineStartColumn.Prefatory*`・`NoteSpacingParameters.BaseNoteSpace/SpaceToBarline/MinItemGap`・`SpringSolver.MinTotalLength`・`LyricParameters` の 5 定数・`_percentCoveredDepth` 族・`MeasureCollector.IsInside*` 6 本（要 grep）・`DrawTabGraceNotes` の未使用 2 変数／**2 綴り**＝beam 平行四辺形（`DrawBeamSegment` と `DrawGraceBeam` の local）・clef 表 3 つ・`IsInsideMusicContent` ×3・"structured?" 述語（第395 で 1 軒に）・MIDI の post-event switch ×4・XML の quarter-tone `AccidentalName` ×3／tuplet bracket ×2／drum note ×2・twin の nested exporter 生成 ×6。
...
| 決定 | 根拠（要点） |
|---|---|
| ★★★ **`voice { } { }` の span は音価の既定も動かさない＝全 branch は span が開いた時点の (Duration, Dots) から読み、span の*後ろ*の音楽もそこから読む。octave frame（2026-08-01）と同じ 1 つの規則**（2026-09-17・第398 第 3 便・私が octave の決定の延長として置き、**ユーザー確認「あなたの判断を残す方が、一貫して分かりやすいよね」**・✅ `0c25c5d2`） | LilyPond の parser は「最後に*書かれた*音価」を lexical に継ぐので `c8 << { d4 e } \\ { f g } >> a` の `a` は 4 分になるが、Lily# は span を「同時に鳴る音楽で何も動かさない」と読む＝octave と同じ言い方で音価も言える。4 読み手（page・MIDI・XML・twin）が同じ規則。twin は LP が違う読みをする 2 箇所（各 branch の先頭・span 直後の最初の event）に音価を書き出して対を保つ。掃き 938 冊で page／MIDI／XML は 1 冊も動かず（母集団に差の出る綴りが無い）、twin は 4 冊（2 冊は base の twin が LP に別の音価を読ませていた側の修正）。 |
