...
終了時 **未追跡 0・木 0**・commit 5 本（`a7e6c0d0` rung → `0a95ab29` handoff → `5af72975` font 番人 → `1b42fd74` band memo → 閉幕 handoff）・suite **Windows 5704/0/4・Linux（この機械の WSL）5704/0/4＝両 OS 完全緑**（+3＝新網ちょうど）・snapshot **220 枚不動・再承認 0 枚**・台帳 553 点・ss 非ゼロ 110／3.876975326・追跡コーパス 569 冊・Core 0 警告・sweep は 2 回とも 0/569（rung・band とも）。**code 3 commit と第1 handoff は push 済み・CI success を全部同便で読んだ**（`a7e6c0d0`＝32348513824・`0a95ab29`＝32349057629・`1b42fd74`＝32351010812）。閉幕 handoff commit 自身の CI は次便が読む（docs のみの差分・毎便同じ形）。
...
- **残りの大物（次の返済候補・▶ に記載）**: **注釈 2 パスの歌詞 layout（ann.lyrics ~162 MB）が毎打鍵フルに走る**——帯は続き便で memo 済み。系統横断なので per-system 鍵では畳めない＝鍵設計から。
- **⑸ ★★★ 次に触るなら＝残債**: ⒞ CoPlaceToCodaWithLabels 鏡像／▶ perf（歌詞打鍵の残り＝ann.lyrics ~162 MB＝注釈 2 パスの歌詞 layout。**系統横断なので per-system 鍵では畳めない**＝鍵設計から・専用便）／⒡ 配管 6 site／⒣ removeEmpty/pedal の score 移行検討（別便＝ユーザー指示）／小粒: twin の歌詞行・`lines` twin 未輸出・マークの X・chord-row の上帯スカラー・**多声小節の byItem 予約写像**（rod は直したが in-measure 予約は今も ItemIndex で列を外す。未測定・対から）。Marketplace は PAT 待ちのまま（第220 ①）。
...
> ⇒ ★★ **帯は第224 続き便で memo に載った**（`1b42fd74`・打鍵 347.3→211.9・hit を主張する net つき）。
> **残り＝ann.lyrics ~162 MB＝注釈 2 パスの歌詞 layout（LayoutLyrics→DistributeLooseLines）**——
> **per-system 鍵では畳めない**（systems 全体を歩き・looseChainEnd が隣系統を読む＝系統横断）。
> やるなら鍵設計から＝専用便（増分穴は sweep に見えない・第192 の族。計器の置き場は §1 第224 の地図）。
