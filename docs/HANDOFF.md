...
（**実装 4 便＋道具 2 便**。**合成本の打鍵は 3304.5 → 524.9 MB＝−84.1%**）。
...
**掃きは 18 本**（毎回 `git ls-files "*.lys"` の **566 冊**）＝**比較 12 回とも 0 冊動いた**＋
**毒つきの掃き 6 本**。**陽性対照は 6 回**（ScriptUp **6 冊**／ScriptDown **1 冊**／
箱・bow・tuplet **37 冊**／`RebuildKeepingHighest` **103 冊**／遅延 augment **2 冊**／
`extraPad` **11 冊**）。⚠️ **数え方**——**比較 12 回には「同じ状態を取り直した分」と
「案A/案B を別々に測った分」が含まれる**（`audit/probe-out/p191-*.csv` を数えると 18 本。
うち毒が `p191-p*` の 6 本）。**便の数ではないので「N 便とも緑」とは読まないこと。**
...
| 1 打鍵 | 便前 | 第1便後 | 第2便後 | 第3便後 | **第4便後** |
|---|---|---|---|---|---|
| `perf-fingstack1k` | 3304.5 MB | 2815.6 | 2191.1 | 1362.0 | **524.9**（**−84.1%**） |
| `perf-scripts1k` | 1117.8 MB | 932.6 | 697.8 | 403.9 | **336.8**（**−69.9%**） |
| `perf-v2bow1k` | 157.1 MB | 157.1 | 152.9 | 152.9 | 152.9 |
| `perf-plain1k` | 95.9 MB | 95.9 | 95.1 | 95.1 | 95.1 |
...
- **⑹ ★ §7.5 の監査**＝**Core +223 行（うち*コード 100*）・削除 −65（コード 59）・
  ...
  **分類は 4 便とも ⒟**（何も足していない＝既存の家の指し直し／複製の削除／順序の確定。
  **LP の意味論に 1 つも触っていない**——**566 冊・snapshot 217 枚・台帳 516 点が
  どれも不動**であることがその証明）。**§7.7 の匂い一覧はどれも当たらない。**
- **⑺ ★★ 残っている項＝**打鍵の残り **524.9 MB**。**この島（注釈の skyline）は 3 段とも返した**——
  `AugmentSkylinesWithScripts` は **11.5 MB＝1%**、`Distance` の padding は消えた。
  **次に割るなら `ArticulationEngraver.CalculateWithFingerings` の中で、まだ二分していない部分**
  （第4便の前は 974 MB あり、うち 875.8 が padding だったので、**残りは約 100 MB**＝
  もう首位ではない。**次の首位がどこかは未測定**）。
  ...
  **アルゴリズムを触る前にそこを測ること。**
- **⑻ ★★★ 射程は変わっていない＝実ファイル 0 冊**（第190 の census のまま）。
  **`scratch\ベースタブLy` の 313 冊は articulation 0**・**50〜499 個の本は 0 冊**・
  **500 以上は 7 冊で全部合成 perf 本**。
  ⇒ ⚠️⚠️ **だから「実ファイルが速くなった」とは書けない**（§5.0「何を主張できるかを測る前に決める」）。
  **書けるのは「合成本の打鍵の割り当てが 84% 減った」まで。**
  ⚠️ **ただし第2便は器が全 merge なので*どの本でも*わずかに効く**
  （plain1k 95.9 → 95.1・v2bow1k 157.1 → 152.9）——**これが実ファイルにも届く唯一の部分**。
...
>
> **⑶' 残っている段（着手するならここから）**:
> ★★ **⒜ 打鍵の残りは 524.9 MB で、この島に首位はもう無い。** `CalculateWithFingerings` は
> 第4便の前 974 MB のうち 875.8 が padding だったので**残りは約 100 MB**。
> **次の首位がどこかは未測定**——**region スコープを置いて二分するところから**
> （§1 の骨 ⑴ の手順で 30 分。⚠️ **`AugmentSkylinesWithScripts` は 11.5 MB＝1% なのでもう違う**）。
