...
⇒ ★★★ **⑺ 次の一手**: **§2 E「section label と和音行の上下」＝要ユーザー判断のまま**（第378 ⑸）。残りは第377 ⑸ の並び＝**⒝ 摂動網の残り**（chord quality 35・`@` の引数つき形）／**⒠ drummap の `${1:hh}`＝UX 案件**／**⒢ 第369 ⑸⒝・第368 ⑺⒝ の並び**／§2 E の副産物（内容で決まる頁の高さ）／**新: ⑸ の `ApplyLeftHeadWidth` の床の疑い**。★ **判定の記録**（常設指示「次便は、このセッションでやる方が有利なら着手して…」）: **第 2 便は着手しない**＝⑸ の疑いは**床が効く対を作る probe から**で、`p380/incr` の双子と読みはディスクに残る（次のセッションでも同じ所から始められる）／section label はユーザー待ち／⒝⒠⒢ は別 regime。
