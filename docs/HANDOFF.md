...
最終更新 第330セッション＝**入り方は第298〜第329 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟みはベンチ許可の 1 回）。⇒ ★★★★ **本便は第329 の次の一手 ⓪＝「途中挿入の layout 段 70〜95 ms」を閉じた**（Core 6 ファイル＋番人＋計器・製品 1 commit）。**骨は 7 つ**:

⚠️⚠️ ★★★★ **⑴ 診断＝`SystemLayoutCache.TypedCache` の 8 store は全部キーに `first`（絶対小節番号）を持ち、途中挿入で後続 system の全 store が miss していた**。**値の側で絶対番号を焼いている物を監査した**: `MeasureLayout.MeasureIndex`／`TieItem`・`SlurItem` の `Start/EndMeasureIndex`＋`BowLayout.RenderMeasureIndex`／`BeamGroup.MeasureIndex`＋member・rest stem の明示 index／`TupletBracketLayout.MeasureIndex`／`SolvedPedalLine.StartMeasureIndex`。**system skyline 対・lyric band・loose line は無印**（純幾何）。
★★★ **⑵ 直し＝KeyAlignment ではなく content-addressed**（第329 の「入り方」より単純で一般）: **bucket hash から `first` を外し、`first` 以外の全部（count・edge flag・indent・shortest・extra・extra2・content slice）が一致する entry を「shifted hit」として、値を delta＝`first − e.First` で振り直して*新 entry として保存***（次打鍵は exact hit・同じ instance を返す＝`LyricChainMemo`／`VerseSkylineMemo`／paging augment の参照比較が生きる）。**shift 関数は store ごとに 1 つ**（`Unstamped`＝恒等／`ShiftMeasures`／`ShiftStaffSkylines`／`ShiftBeams`／`ShiftTies`／`ShiftSlurs`・model 側に `MeasureLayout.WithMeasureIndex`・`TieItem`/`SlurItem.WithMeasureIndicesShifted`・`BeamLayout`/`BeamGroup`/`BeamMember.WithMeasureIndex(ices)Shifted`）。**振り直さない物は名指しした**（`ShiftStaffSkylines` の remark）: room の spanners の `TupletBracketLayout.SourceIndex`・各 `SourcePosition`・`SolvedPedalRow.SourcePosition`＝**exact hit でも既に不正確**（前の小節に tuplet が増えれば表の index はずれる・content key は offset を除外）で、**room の spanners は衝突幾何としてしか読まれない**（`BuildInsideStaffSkylines` と 2 つの seed）。**beams／ties／slurs の `extra: systemIndex` は据え置き**＝**system 数が変わる挿入ではこの 3 store は miss**（下 ⑸ の x7）。
⚠️⚠️ ★★★★ **⑶ 副産物＝content key の `BeamId` 罠**（**最初の deletion 番人が「全 store miss・springs (0,23)」で赤になって見つけた**）: **`NoteItem`/`ChordItem.BeamId` は bake が score 順に振る識別子**（`MeasureCollector.ResolveBeamStemDirections` の `_nextBeamId++`）で、**content key が reflection でそれを畳んでいたので、前方で beam group が 1 つ増減すると後続*全*小節の key が動いた**——**梁のある小節を 1 つ消すと 23/23 の後続 key が変わり、springs memo も system memo も*全部* miss**（第329 の bench は挿入した小節に梁が無かったので気付かなかった。**梁のある小節を挿入しても同じ**）。**直し＝`ItemExclusions` に `BeamId` を足し、`AddBeamGrouping` が*関係*を畳む**（小節内の running group 番号＝−1/0/1…・**小節線を跨ぐ手書き梁は前後の小節との boolean 2 つ**＝`BoundaryClefAllowance` と同じ「隣を読む」形）。**番人 `MeasureContentKeyTests` +2**（梁の小節を消しても後続 key 不動／跨ぐ梁は両側の key に入り・番号には依らない）。
★★ **⑷ 計器（恒久）**: `SystemLayoutCache.PassCounters(Store)`（hits／shifted／misses・pass ごとにリセット）＋`PassCountersTotal`・`IncrementalCompiler.LastLayoutMemo`・**`EditKeystrokeBench` に途中挿入の行（`perf-plain1k` に 1 小節）と `layoutMemo hit/shifted/miss` 列**。**scratch 計器は削除済み**（job tmp に `ScratchShiftedMemoProbe.after.cs`＝bench.lys の `// here` に k 小節を累積挿入して毎 round 新しい番号付けにする）。
★★★ **⑸ 実測（bench.lys・Preview・round k は k 小節挿入＝毎 round 後続が新番号・**ユーザー許可あり**・ユーザー作業中なので絶対値は粗い）**: **baseline（`bb402b26` の worktree）353〜847 ms・中央値 ≈514** → **本便 116〜338 ms・中央値 ≈152**（x1 206・x2 338＝layout が 2 度走った pass・x3 169・x4 136・x5 118・x6 116・**x7 211＝system が 1 つ増えて beams/ties/slurs が全 miss**・x8 120）。**incremental==full は両方 True**。**frag は (0〜2, 28)＝render 段は依然ほぼ全 system 描き直し**（座標が下へずれるので原理的に replay 不可＝第329 ⑼ の ⒞）。
★★ **⑹ 番人**: `IncrementalCompilerTests` +3（`LayoutMemo_AnInsertedMeasure_ReStampsTheShiftedSystems`＝Measures／StaffSkylines とも **(0, 5, 1)** を pin・`…ADeletedMeasure…`・`…ChainedEditsOverShiftedSystems_AlwaysMatchFull`＝挿入→遠くの音の編集→前への挿入→取り消し、毎打鍵 full と byte 一致＝data-pos 込み）・`SystemLayoutCacheTests` +1（shifted hit の振り直し・その後 exact・削除側・edge flag 違いは miss）・`MeasureContentKeyTests` +2。**本は `break` で system を固定した 6 段**（下 ⑺⒜）。
⚠️ ★★ **⑺ 罠**: **⒜ 一様な本（lpreg の perf-*）では DP が 1 小節ずつ後続の段へ押し出すので後続 slice が*全部*変わり shifted hit は 0**＝**この直しは `break` で行を固定した本（tab 本＝報告の regime）で効く**。`EditKeystrokeBench` の新しい行はそれを承知で置いた（shifted 0 を刷る）。／**⒝ section 頭の小節は `SectionLabel` を畳むので他の小節と一致しない**（key の番人で比較対象を 1 つずらした）。／**⒞ 最初の chained 番人は「取り消しで shifted」を期待したが、取り消し先の番号の entry は前の打鍵が保存済みなので exact hit**（entry は編集を跨いで生きる）。

★★★★ **第 2〜6 便＝ユーザー対話（「少し改善したが期待から遠い。ほかにできることは」→ 出力チャネルの数 → プロファイル）で、遅さの正体は engine の外にあった**。**骨は 5 つ**:
⚠️⚠️ ★★★★ **⑻ 計器を先に置いた（`d8fe8985`・`8a160b67`）**: `lilysharp/svg` の応答に `SvgTiming`（client 送信→dispatch 受信の `to server`・thread pool `queued`・`expand`・render gate の `lock`・`render`・`server total`・戻りの `back`・直近の診断パス・直近の didChange・pool の thread 数）を載せ、拡張は `Got response` 行の横に刷り、**拡張ホストの event-loop lag（10 ms タイマの最大遅れ＝`host lag max`）**も刷る。**同じ機械なので client と server の clock は比較できる**。⇒ ユーザーの log: **render 24〜93 ms・webview 30〜70 ms に対し、to server 254〜3372 ms・back 51〜1100 ms・host lag max 212〜1562 ms**＝**engine でも webview でもなく拡張ホストの停止**。
★★★ **⑼ 診断パスは render に譲る**（`8a160b67`）: `SemanticValidation.Run` が打鍵ごと（debounce 200 ms）に render の真上で走り 400 ms（render は 31→562 ms に伸びていた）→ svg 要求が飛んでいる間と応答後 300 ms（`SvgQuietMs`）は寝る。効果: 診断 400 → 86〜248 ms・render は 100 ms 前後に。
★★★★ **⑽ 正体＝GitHub Copilot の inline suggestion**（`Developer: Show Running Extensions` → Start Extension Host Profile を打鍵中 3.4 秒。集計は job tmp `profile-top.js`）: **89% が `ext/copilot/dist/extension.js` の `getGhostText` → トークナイザ `Pxt`（self 2293 ms）**。`.lys` を打鍵するたびに Copilot がプロンプトを組み、拡張ホスト（全拡張のメッセージが通る）が 1〜2 秒止まっていた。**1 度目のプロファイル（0.9 秒）は Cline の bundle 読み込みしか写っておらず誤認しかけた**（ユーザーは Cline をアンインストール＝無関係だった）。**対処＝拡張の既定に `"[lilysharp]": { "editor.inlineSuggest.enabled": false }`**（`a32994fb`・package.json の `configurationDefaults`＝Marketplace／VSIX で届く・ユーザー環境は手で同じ設定＋`nextEditSuggestions.enabled=false`）。**ユーザー「素晴らしい、とても快適になった」**。
★★★★ **⑾ system 単位の座標相対化**（`d07b0777`・Core＋webview）: interactive SVG だけ（`IDocumentContext.SystemLocalFrames`＝`SvgDocumentContext` の `Interactive`）、`RenderTo` が各 system を `system with { Y = page.Height }` で `DrawSystem` に渡し（emit される device Y は system 上端からの距離）、`BeginGroup(Translate(0, system.Y − page.Height))`（Y-up・YFlip が device に共役）で本来の位置へ。labeled group と同じく **fragment capture の外側**。⇒ 下へずれた system は**同じバイト列＋transform 違い**。**fragment memo の鍵から `FirstMeasureIndex` と各小節の `MeasureIndex` を外した**（layout memo と同じ罠＝stamp であって入力ではない・`KeyMatches`／`HashGeometry` の remark）。**webview** `updateChunks` は frame transform と data-pos を伏せて比較し、動いただけなら `setFrameTransform`、offset だけなら restamp（`systems moved N` を出力チャネルに）。**番人 `LayoutMemo_AnInsertedMeasure_ReplaysTheShiftedSystemsInTheirOwnFrames`＝replay 5 / 描画 1・interactive の full と byte 一致**・export は不変（snapshot 246 緑）。⚠️ **まだユーザー機で「systems moved N>0」の行は見ていない**（貼られた log の打鍵は高さが動かなかった＝`moved 0`）。
★ **⑿ 一緒に見えた小物**: `pool 7 threads / 0 queued`・`didChange` 1〜9 ms＝LSP の dispatch は空いていた／`lock 17〜142 ms`＝古い render を新しい要求が待つ（render は cancel 不可・小）／webview の「replaced 2 → 200＋245 ms」は ⑾ で消えるはず。

★ **開始時裏取り**: HEAD `bb402b26`＝**未 push 9**（第329 §1 の「5」は HANDOFF commit 前の数）・木 clean・未追跡 0・`bin\Debug` は net10.0 のみ。
終了時（第 1 便）: **commit 2 本（`8155570c`＝製品＝Core 7 ファイル（`SystemLayoutCache`・`IncrementalCompiler`・`ScoreLayout`・`BeamGroup`・`MeasureContentKey`・`TieItem`・`SlurItem`）＋番人 4 ファイル＋`EditKeystrokeBench`＋`APPROXIMATIONS.md`／`magic_constants.csv` 再生成＋CHANGELOG（拡張）1 項／`6b01f560`＝HANDOFF＝第328 の経緯は `HANDOFF-ARCHIVE.md` 末尾へ逐語）**・**Windows Debug 7096 / 3 / 4 / 7103（job tmp `full1.trx`・赤 3 は全部順序＝継続性 2 本は §1 を書き終える前に full run を始めた既知の罠・inventory 1 本は再生成→ 3 本再走 5 / 0）**。
**終了時（第 2〜6 便）**: **commit 6 本**（`d8fe8985`＝svg 応答の計時／`8a160b67`＝診断の譲り＋didChange・pool・host lag／`d07b0777`＝座標相対化＋fragment の鍵＋webview＋番人／`a32994fb`＝`inlineSuggest` 既定 off＋CHANGELOG／`StaffLineGeometry` の transform 対応＋inventory 再生成／この HANDOFF）・**未 push 17**・作業ツリー clean・未追跡 0（`scratch/p330` は管理外・baseline の worktree は remove 済み）・Core 0 エラー 0 警告（`--no-incremental`）・**Windows Debug 7098 / 2 / 4 / 7104（`full2.trx`・2 分 40 秒・+1＝⑾ の番人）→ 赤 2＝inventory（行番号）と `StaffLineGeometry`（preview SVG の生 y を読んでいた＝⑾ で system frame の translate を足す形に）→ 再走 20 / 0（`full2b.trx`）**・台帳／`.lys`／snapshot 不動・**LSP＋拡張は `Deploy-Lsp.ps1` で 3 度配備（95 ファイル byte 一致）**・**`npm test` 71 / 0・`check-types` 0**。⚠️ **`Deploy-Lsp.ps1` は extension.ts の変更は届ける**（ユーザーの log に `host lag max` の行が出た＝配備された dist で動いている）**が、package.json の `configurationDefaults` は届かない**（インストール済み拡張のマニフェストは書き換えない）——ユーザーは同じ設定を手で入れた。

⇒ ★★★★ **次の一手**: ✅ **⑴ ⑾ の実機確認＝済（第 7 便・ユーザーの log）**: 往復 **18〜20 ms**（render 14〜16・to server 1〜2・back 2）・host lag max 4〜6 ms・webview 3.6〜17.8＋4〜28 ms・高さが動く打鍵で **`systems moved 9`・`replaced 0`**。system が頁をまたいで移った打鍵だけ `replaced 2`（12〜18 ms）＝頁の構成が変わる場合は丸ごと parse していた → **第 8 便（ユーザー「次にこれを」）で `reflowPages` を入れた**（`applySvg` は群構成の変わった頁を集めて一括で reflow: 壊れた頁の live 群を内容（transform＋data-pos を伏せた形）のプールにし、新しい群はプールから取って `insertBefore` で頁をまたいで移動・transform 設定・restamp・無い物だけ parse。失敗は `whole()` に落ちる＝部分 reflow 後でも安全。`updateChunks` は群ごとに record を DOM に追随させる＝途中失敗しても record が真）。**1 度目は `carried 0, parsed 4`**（`reflowed` の枠比較が群の間の空白ごとだった→ head／tail 比較に。さらに「群の数が変わらない頁」＝1 つ受け取り 1 つ渡した頁が旧 `updateChunks` の index 対応で全 parse され、渡した system がプールに無かった）→ **`updateChunks` を廃止し、描画が変わった頁すべてを 1 つのプールで reconcile（同じ頁の同じ位置を優先）**。一致しなかった system は最も近い候補との最初の差分を出力チャネルに（編集した system 自身は除く）。**✅ 実機（第 8 便末）: `reconciled 3 (groups kept 0, moved 25, carried 2, parsed 4) in 12.4 ms, then 35.8 ms`・往復 32 ms**。⚠️ 踏んだ罠: テンプレート文字列内のコメントにバッククォート（第329 と同じ・`webviewScript.test.ts` が捕まえた）。`npm test` 71 / 0・配備済み。**朝の「途中挿入 353〜847 ms・往復 2〜12 秒」から 2 桁落ちた。**★★ **⑵ 出力チャネルの `Got response` 行の読み方**（次便が最初に見る計器）: `to server`／`back`／`host lag max` が大きければ拡張ホスト（他拡張）、`lock` が大きければ古い render 待ち、`render` が engine。**今日の決着＝Copilot の inline suggestion**（⑽）。✅ **⑶ system 数が変わる挿入（第 9 便・`fe1e681d`）**: `SystemLayoutCache` の `systemIndex` を stamp に降格（`Entry.System`・bucket hash と content 照合の外・`BeamLayout.WithSystemIndexShifted`／ties・slurs は恒等）。**fragment memo は entry を `_previous`／`_current` の 2 表に**（隣 index の entry を取っても次の system が探す entry を上書きしない）、探索は自 index → ±1 → ±2、鍵は index の「先頭かどうか」の 1 bit だけ、**anchor／slot の照合は候補選びの中**（同じ音楽が並ぶ本では同 index の entry が鍵を通って anchor で落ちる）。`LastPassDeclines`（system ごとの不採用理由）を番人が読む。**副産物＝⑾ の local frame は `Y = page.Height`＋頁の flip 基準で描いていたので頁の高さが鍵と座標演算に入っていた**（自動サイズの頁＝全 fixture で system が増えると全 fragment 不採用）→ **system ごとに flip 基準 0 の `YFlipDrawingContext` を作り device 空間で translate**（emit される y は system 上端からの距離そのもの）。番人 `LayoutMemo_AnInsertedSystem_ReStampsTheShiftedSystems_AndReplaysTheirFragments`（3 store とも shifted ≥ 5・fragment replay 5 / 描画 2・interactive full と一致）。520 本の焦点フィルタ緑・配備済み。**full suite（第 9 便末・`full3.trx`）7100 / 1 / 4 / 7105（+1＝新番人）→ 赤 1 は inventory の行番号（`SharedRenderer.cs`）＝再生成して 5 / 0（`full3b.trx`）**。**終了時（第 9 便）**: commit 3 本（`fe1e681d`・HANDOFF・inventory 再生成＋HANDOFF）・未 push 26・木 clean。✅ **⑷ `lock 17〜142 ms`（第 10 便）**: `RenderIncremental(tree, token)`＝collect 後・layout 前・render 前の 3 境界で中断（各境界で session が健全な理由は `Checkpoint` の remark・`_shortest` は springs と一緒に commit する形に直した）・test seam `StageProbe`・番人 3 本（各境界で中断→同じ本と次の編集が full と一致）。LSP は slot に `Running` CTS を持ち、新しい ticket が来た瞬間に dispatch thread で cancel。`RenderSvgIncremental` は `OperationCanceledException` を session 破棄にしない。241 本（Incremental／Lsp／Fuzz／CollectResume／FontEdit／PaperEdit）緑・配備済み。★ **⑸ overlay 群**（タイ・スラー・歌詞・強弱＝頁座標）は Y ずれで今も描き直し＋webview で parse。**第 11 便で見て着手せず**（この便では不利と判断）: ⒜ 効果が小さい＝Y ずれの打鍵で parse されるのは頁ごとの overlay 群 1 つで、webview の更新は全体で 12〜28 ms・engine の overlay 描画も軽い（fragment memo の実測「fingering と bow 以外 ≈ 0」）。⒝ 危険が大きい＝drawer 25 本は `BuildMeasureToSystemTopYUp(page)` の表で頁所属を判定し、system をまたぐ spanner（ヘアピン・オッターヴァ・volta・text spanner）を分割する。system ごとの表で呼ぶと「隣の system が同じ頁に無い」形になり、drawer によって分割の描き方が変わり得る。**その差は snapshot（export＝頁座標のまま）でも incremental==full（両方 interactive）でも見えない**。やるなら先に ①「interactive と export の絵が一致する」番人（例: 両方を PNG に落として比較、または overlay 群の座標を frame で戻して比較）を置き、② overlay 描画と parse の実測で効果を数えてから。fingering の overlay memo は (drawer, page) 鍵なので unit（system）を足す必要がある。**⑹ 以下は第329 の並び**: push 後 `gh run list`（`extension-tests` job 込み）／T6 の残り／T7 の C 37 冊／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／§2 A/B/D/E／§2 C⑴／§2 G／`:|.|:` の幅 0.08／smart typing の小穴。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ**（**`inlineSuggest` 既定と extension.ts の変更は Marketplace／VSIX でしか届かない**＝リリースの動機が 1 つ増えた）。

## 以下は第329セッションの経緯

最終更新 第329セッション＝**入り方はユーザー報告 1 件**（「音符の印の語順を正規化する拡張を実装したら VS Code の操作がもたつく。性能を改善して。難しければ機能を除去」・background job・口挟み 0）。⇒ ★★★★ **機能は残して読み手を直した。製品（拡張）1 commit `c060214d`・Core 0 変更・語順の表 `POST_EVENT_RANK` と各鍵の書く物は不動**。**骨は 4 つ**:

⚠️⚠️ ★★★★ **⑴ 原因は「毎打鍵 block 先頭からの walk」**——smart key（`' , . \ @ ( ) [ ] ~` と数字）は caret の乗る音符を `musicEvents` で探すが、その walk が `blockBounds` の先頭から始まり、**1 文字ごとに regex `.test()`**（`/\s/`・`BETWEEN_EVENTS`・`/[0-9]/`…）、さらに第327 の `postEventEnd` が**通過する全 event に `coreEndOf`→`noteSlots`（slice＋regex）**を掛けていた（＝語順機能で 3 倍）。**測った**: 1000 小節 block の末尾に caret（`audit/lpreg/perf-slurdot1k.lys` 60 KB・`perf-fingbeam1k.lys` 319 KB・`perf-scripts1k.lys`）で **8–25 ms/鍵**、小節内で済む `<` `>` は 0.03 ms（＝陽性対照）。
★★★ **⑵ 直し 3 段**（`editors/vscode/src/smartTypingCore.ts`）: **⒜ 文字比較**（`isSpace`／`isDigit`／`isOctaveMark`／`isWordChar`／`isNameChar`／`isStringDigit`／`isBetweenEvents`。`\s` の Unicode 空白は `c > '~'` で regex に落として厳密に同値。`isWordChar` は `-` を含めない＝音符の直後判定 `[A-Za-z0-9'_,]` を変えない）→ 3 倍速／**⒝ `postEventEnd` は event.end から `postEventsEnd`**（walk は同じ run の item 境界で止まるので coreEnd から読むのと同じ末尾・`noteSlots` 0）／**⒞ `walkStart`＝caret 手前で文字列・コメントの外にある最寄りの `|` の直後から読む**（`|` は音符・和音・注釈引数の中に立てないので block 先頭からの walk はそこで token 境界＝以後同じ event を読む）。`noteAtCaret`／`slurAnchorAt`／`stringNumberPlan`／`chordMemberAtCaret` がこれを読む。**後ろへ手を伸ばす唯一の読み手 `precedingNote`（`)` の規則 11・スラーは小節線を跨ぐ）は 1 小節ずつ遡って再試行**（2 音見つかるか barrier に当たれば確定＝block 先頭からの walk も同じ barrier で reset する）。**結果 0.2–1 ms/鍵（60 KB）・1–3.4 ms（319 KB の和音本＝残りは `blockBounds`／`hasUnresolvedSlurOpen` の block 全長の文字ループ）**。
★★ **⑶ 計器＝`npm run bench`**（`editors/vscode/test/bench.ts`・`tsconfig.test.json` が拾う・既定は上の 3 冊・引数で他の `.lys`）。**core の判断だけを測る**＝VS Code 側（`getText()`・snippet・往復）は含まない。番人 +2（`c4 | d`+`)`→`c4( | d)`／event 無し小節を挟む／前小節の barrier で reset／コメント・文字列内の `|` は起点にしない）＝**70 / 0**・`check-types` 0・esbuild 緑。CHANGELOG（拡張）Fixed に 1 項。
★ **⑷ 触っていない物（次便の候補・未起票）**: **⒜ 素通しの打鍵は walk 2 回**（intercept 経路で plan null → `type` → change-event 経路が同じ plan をもう一度計算＝第327 以前から・今は 0.5 ms なので放置）／**⒝ `blockBounds` と unresolved-slur の走査は block 全長**（文字比較の単純ループ・319 KB で 0.3 ms 級）／**⒞ VS Code 側の体感は未計測**（`Deploy-Lsp.ps1` で配備し Reload Window 後にユーザーが確かめる番）。

★★★ **第 2 便＝ユーザー「少し改善したが、カーソルキーでキャレットを動かすだけでも少しもたつく。ほかに改善できる所は？」**——**キャレット移動で毎回走る物を 3 か所直した**（**⑸**・commit は下）: **⒜ 拡張 `extension.ts` の `onDidChangeTextEditorSelection`**（プレビューが開いていると毎回 `doc.getText()` で全文を組み立てて token 境界を探していた→ caret の行 `lineAt` だけ読む・token は行を跨がないので同値）／**⒝ プレビュー webview**（`highlightPositions` が `document.querySelectorAll('[data-pos="N"], [data-alt~="N"]')` で SVG 全要素を歩き、`clearHighlights` が `.highlight` で*もう一度*全要素を歩いていた。**webview の frame はエディタの window で動く**ので、これが矢印キーの前に立つ）→ `sourcePositions()` の索引に **`posElements`（offset → 要素リスト・nh-hit と占有 rect は索引時に除外）**を足して Map 引き・塗った要素は `highlighted` に覚えて clear・**`scheduleHighlight`＝1 frame に最後の 1 通だけ描く**（キー押しっぱなしの repeat を畳む）／**⒞ LSP の documentHighlight**（VS Code がキャレット移動ごとに投げる。`SymbolOccurrences` が part／section／form／lyrics／chords の名前 token 一覧を*毎回* `DescendantNodes()` で 5 回全走査＝red node を毎回 materialize）→ **`NameTokensCache`＝`ConditionalWeakTable<SyntaxTree, NameTokens>`**（`LineStartsCache` と同じ形・木は編集ごとに差し替わるので厳密）。Rename／References も同じ表を読む。**番人 `RenameSymbolTests`＋`FindReferencesTests` 21 / 0**（`scratch` 無し・trx は job tmp）・`check-types` 0・esbuild 緑・Core 0 変更。⚠️ **踏んだ罠**: webview の JS は `getWebviewContent` の**テンプレート文字列の中**（L1607〜2969）＝コメントにバッククォートを書くと文字列が切れて tsc が別の行で怪しいエラーを出す（`Property 'highlight' does not exist on type 'string'`）。**体感は未計測**（VS Code 側の 3 つとも「全長 → 局所」の型なので、桁は確実に落ちる）。

★★★★ **第 3 便＝ユーザー「改善したが、まだ余地があればやって」**——**今度は打鍵ごとに走るプレビュー更新を測った**（**⑹**）。**計器＝`scratch` 無し・job tmp の `webbench.html`**（`lysc svg` で作った実曲 5 ページ 517 KB／要素 5,688 と `perf-slurdot1k` 5 MB／28 ページを、aperture のブラウザで `innerHTML` 差し替え・layout・次 frame まで計測）。**実曲 5 ページで全体差し替え＝parse 150 ms＋layout 230 ms＋paint 80 ms ≒ 470 ms／打鍵**（webview の frame はエディタの window なので、これがそのまま打鍵の後ろに立つ）。**1 ページだけの差し替え 37 ms・ページを残して `data-pos` を打ち直すだけ 8 ms・索引再構築 14 ms**。⇒ **webview に `applySvg`**（`extension.ts` の `getWebviewContent` 内・`// ---- page-level update`）: SVG 文字列を `<g class="page" data-page-top=` で head／pages／tail に割り、**ページごとに文字列比較**——同一なら触らない／`data-pos`・`data-alt` の値だけ空にした形（名前は残す＝同じ要素に同じ順で載っている保証）が同じなら **DOM を残して値を打ち直す**（`restampPage`＝markup を 1 回の regex で流し、`[data-pos],[data-alt]` の要素に順に載せる。数が合わなければ差し替え）／違えば **そのページだけ** `createElementNS('svg').innerHTML` で parse して `replaceChild`。frame（svg タグ・style・閉じ）やページ数が違えば全体差し替えに落ちる。**同じ計器で「ページ 2 を編集して以後の offset を +1 した SVG」を applySvg → `innerHTML` を全体差し替えの結果と文字列一致で照合＝一致**・**56〜80 ms＋paint 24〜53 ms（全体 470 の約 1/5）**・同一 SVG は 0.5 ms。`check-types` 0・esbuild 緑。⚠️ **罠 2 つ**: ⒜ 最初の `restampPage` はタグごとに regex を 2 回掛けて 116〜138 ms だった→ 1 回の `/\sdata-(pos|alt)="…"/g` 走査で 60 ms 台／⒝ 例によって template 文字列内のコメントのバッククォート（2 度踏んだ）。

★★★ **第 4 便＝ユーザー「良いね。残りを続けて」→ ページ内を system 単位に**（**⑺**・Core 変更あり・プレビュー出力だけ）。**SVG の構造**: ページ直下は title の `<text>` と余白の `<g transform>` 1 つで、その中に全 system が続き、**system の後にページ単位の overlay（DrawTies／DrawSlurs／DrawLyrics／…／DrawPartCombine＝`SharedRenderer.RenderTo` L156〜186）が 1 塊**。⇒ **`IDrawingContext.BeginLabeledGroup(string label)`**（既定 no-op＝PDF／PNG／Recording は不変・`SvgDrawingContext` は **`_interactive` のときだけ** `<g class="…">`・`YFlip`／`UnscaledX` は転送）で **各 system を `<g class="system">`、overlay 塊を `<g class="overlay">` に包む**（fragment memo の capture の*外側*＝replay も live も同じ包み）。**export SVG は byte 不変**（`SvgSnapshot` 含む 506 本緑＝`IncrementalCompiler`／`NoteheadHitTarget`／`BowSourcePosition`／`IncrementalLyricLinePass`／`FontDebug`／`SvgSnapshot`／`Lsp`・`scratch` 無し・trx は job tmp）。**webview**: `applySvg` はページの描画が変わったとき `splitChunks`（`<g class="(system|overlay)">` から対応する `</g>` まで depth で切る・残りは frame）で群ごとに同一／blank 同一→ `restampPage`／差し替え、frame が offset 以外で違えば・群の数や label が合わなければページ丸ごと。**同じ計器（今度は preview モードの SVG 721 KB・system 35・overlay 5）で DOM 一致**・**全体差し替え 165 ms（負荷の少ない時間帯・第 3 便の 470 は取り合い中の数）に対し 23〜45 ms＋paint 20〜32 ms**。残りの大半は**後続ページ 4 枚の `restampPage`（6 ms/枚）と索引再構築 8 ms**。⚠️ **ベンチはユーザー許可を得てから**（本便で叱られた＝memory 更新済み）。preview SVG の採取は一時テスト（`ScratchPreviewDump.cs`・env var で in/out・**削除済み**）で `SvgGenerator.Generate(…, Preview())`。⚠️⚠️ **第 5 便＝ユーザー報告「プレビューが Loading preview.. のまま」**——**webview の JS はテンプレート文字列の中なので regex の `\s`／`\/` がテンプレートのエスケープとして消え（`\s`→`s`・`\/`→`/`＝regex が途中で閉じて構文エラー）、スクリプト全体が死んでいた**（ベンチは .ts の*文字列*を持ち上げていたので気付けなかった）。**直し＝`\\s`／`\\/` に倍化**（3 か所・コメントに理由）。**番人 `test/webviewScript.test.ts`**＝テンプレートを評価して各 `<script>` を `vm.Script` で構文検査（バッククォート・バックスラッシュの両方の穴を捕まえる）。

★★★★ **第 6 便＝ユーザー報告「プレビュー更新が遅い（回帰だろう）・特に楽譜の途中に音符を挿入する操作」＋「lys は小さく、末尾入力は速いから途中入力固有の原因がある」**——**⑻ 結論: 私の変更による回帰ではなく、(a) 途中挿入は engine の regime ⑶（後続 system 全部の再配置＋改行 DP）で末尾入力の 5 倍、(b) それが Reload Window ごとの LSP 再起動＝JIT cold に掛かっていた**。**計器 3 つ（全部 scratch 一時テスト・削除済み・env var で in/out）**: ⒜ `IncrementalCompiler` 直（`bench.lys`＝`scratch\ベースタブLy\bench.lys`・3 ページ・281 KB）: **preview オプション 200〜300 ms／plain 80〜115 ms**（比 2.2〜2.5 は Core を `<g>` 包み前に戻しても同じ＝包みは無関係・interactive の nh-hit 矩形ぶん）／⒝ **LSP in-process（didOpen→didChange 挿入→svg）: cold 4.7〜5.2 s → 2 回目 0.7〜1.0 s → steady 途中挿入 215〜330 ms・末尾挿入 30〜90 ms**／⒞ **published exe を stdio JSON-RPC で起動（Reload 直後の実態）: JIT 版 cold svg 2.1〜5.0 s・初回挿入 0.67〜1.0 s・以降 0.21〜0.32 s／ReadyToRun 版 1.3〜1.5 s・0.43〜0.45 s・0.15〜0.19 s**。**ユーザーの実測（拡張の新ログ）: 往復 2297→1510 ms・webview 82＋94 ms**＝cold の数字と一致。⇒ **直し＝`tools/Deploy-Lsp.ps1` の publish を `-r win-x64 --self-contained false -p:PublishReadyToRun=true` に**（Core.dll 3.2→8.2 MB・client は coreclr 無しなので従来どおり `dotnet` 起動・配備済み）。**拡張に計時ログ**（`Got response … (N ms round trip)`／webview → `webviewPerf` → `PREVIEW update: pages kept/re-stamped/updated by group/replaced in N ms, then pages/fit/highlight N ms`）。⚠️ **未着手**: ⒜ `editors/vscode/publish-marketplace.ps1` の self-contained publish にも ReadyToRun（cross-OS の R2R が macOS 向けに通るか未確認＝ユーザーの Marketplace ビルド時に）／⒝ **途中挿入の 215 ms そのもの**＝regime ⑶ の memo が挿入で効いていない（full 188 ms とほぼ同じ）＝F3 の宿題（§2 の「perf-plain1k Δ=+1」と同じ族）／⒞ webview の `pages/fit/highlight` 94 ms（索引再構築＋fitWidth の layout）。

★★★★ **第 7 便＝ユーザー「その課題（途中挿入 215 ms）に、この便で取り組むのが有利なら着手」→ 着手（⑼）**。**診断**（scratch テスト・削除済み）: 途中挿入で `LastSpringMemo=(3,109)`・`LastRenderFragments=(0,28)`、末尾挿入は `(110,2)`・`(27,1)`＝**memo が全部外れる**。**段別**（Compile に一時 Stopwatch・撤去済み）: 途中 collect 15・keys 3・**springs 70〜90**・**layout 70〜95**・render 35〜57 ms／末尾 springs 0〜1・layout 20〜35・render 3〜4。**原因**: ⒜ spring memo は「index 揃え」で読む設計（`SpringReusable(i, newKeys, prevKeys)`）＝挿入で後続 key が 1 つずれて全滅／⒝ `SystemLayoutCache.TypedCache` のキーに絶対小節番号 `first` が入る／⒞ fragment は system が下へずれると座標が変わる（`HashGeometry`）ので原理的に replay 不可。**直し＝⒜だけ**: `IncrementalCompiler.KeyAlignment`（前後の key ベクトルの共通 prefix／suffix＝挿入・削除で後続を写像・記録 struct）＋ `SpringReusable(i, previous, …)`（写像先で 3-key 近傍＋両端条件を照合＝写像は「どこを見るか」だけで証明は近傍比較のまま）。**結果 `(109,3)`・springs 段 3〜10 ms・途中挿入 steady 約 200 → 160〜190 ms**。**番人 +2**（`SpringMemo_AnInsertedMeasure_ReusesTheShiftedTail`＝(6,3)・`…ADeletedMeasure…`＝(5,2)・ともに incremental==full＋`AssertSpringsMatchFromScratch`）。⚠️ **罠**: 最初のテスト曲が relative octave で、挿入した小節が後続の音の octave を変えて key が本当に変わっていた（診断で current 5..8 がどの previous とも不一致）→ `octave absolute` に。**次の一手（layout 段 70〜95 ms）**: `TypedCache.GetOrCompute` の `first` をキーから外すには、値（`MeasureLayout` 列・skyline・beam／tie／slur・lyric band・loose line の 8 store）が小節番号を持たないことの監査か、hit 時に番号を振り直す機構が要る（`ResolveDataPos`・小節番号描画が読む）。同じ `KeyAlignment` を渡して `first` を写像する形が候補。render 段は system 単位の座標を相対化しない限り不可。

★ **開始時裏取り**: HEAD `f30cdfeb`＝`origin/master`（**未 push 0**＝第328 の 50 本はユーザーが push 済み）・木 clean・未追跡 0・`npm test` 68 / 0。
終了時: **commit 8 本（第 7 便＝`IncrementalCompiler.KeyAlignment`＋番人 2＋CHANGELOG＋HANDOFF・Incremental／Fuzz／CollectResume／FontEdit／PaperEdit 127 本緑／第 6 便＝計時ログ＋Deploy の ReadyToRun＋CHANGELOG＋HANDOFF／第 5 便＝backslash 倍化＋`webviewScript.test.ts`＋HANDOFF／`c060214d`＝製品＋番人＋bench＋package.json＋CHANGELOG／`54cbd903`＝HANDOFF＝第327 の経緯は `HANDOFF-ARCHIVE.md` 末尾へ逐語／`c015a127`＝第 2 便＝`extension.ts`＋`LilySharpLanguageServer.Editing.cs`＋CHANGELOG＋HANDOFF／`f080135f`＝第 3 便＝`extension.ts` の `applySvg`＋CHANGELOG＋HANDOFF／第 4 便＝Core 5 ファイル（`IDrawingContext`・`SvgDrawingContext`・`YFlip`・`UnscaledX`・`SharedRenderer`）＋`extension.ts`＋CHANGELOG＋この HANDOFF）**・未 push 5・**Core は第 4 便で触った（プレビュー出力だけ・export SVG 不変＝snapshot 246 不動）**・台帳／`.lys` 不動・**拡張＋LSP は `Deploy-Lsp.ps1` で 4 度配備（118 ファイル byte 一致・Reload Window はユーザー）**。⚠️ **次に疑う所（未計測）**: 1 ページの parse＋layout 37 ms と paint 24〜53 ms は残る（ページ内の system 単位に割れば更に減るが、SVG の構造に `system` の印が要る）／LSP の `RenderIncremental` は別プロセスで CPU だけ／`semanticTokens/full` は編集ごとに全 token（delta 未対応）。

⇒ ★★★★ **次の一手**: ★★★ **⓪ 途中挿入の layout 段（70〜95 ms・第 7 便の「次の一手」）＝次セッションに回した**（ユーザー「この便が有利なら着手、次便が有利なら着手するな」→ 8 store の値型（`MeasureLayout.MeasureIndex` 確認済・skyline／beam／tie／slur／lyric band／loose line は未読）と `ResolveDataPos`・小節番号描画の読み手の監査が要り、便の文脈が長い今より、読み直しから入れる次便が有利と判断）。**入り方**: `SystemLayoutCache.TypedCache.GetOrCompute` のキー `first` を `KeyAlignment` で写像し、hit 時に値の小節番号を振り直す（振り直しが要らない store から順に）。**計器は第 7 便の scratch（`LastSpringMemo`／`LastRenderFragments`／Compile の段別 Stopwatch）を HANDOFF の記述から再作成**・`scratch\ベースタブLy\bench.lys` の `// here` に 1 小節挿入。⚠️ ★★ **⑴ ユーザーが Reload Window 後に体感を確かめる**（まだ遅ければ VS Code 側＝`getText()`／snippet／往復か LSP を疑う。core は `npm run bench` で 1 ms 未満）。**⑵ 以下は第328 の並び**: push 後 `gh run list`（`extension-tests` job 込み）／T6 の残り／T7 の C 37 冊／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／§2 A/B/D/E／§2 C⑴／§2 G／`:|.|:` の幅 0.08（第328 第 9 便の副産物）／smart typing の小穴（`c4(`+`)`→`c4)(`・JIS の `\` 束縛・上 ⑷）。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
