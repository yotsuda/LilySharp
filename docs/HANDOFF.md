...
最終更新 第348セッション（2026-09-07〜08）＝**入り方はユーザーの質問から**（「v0.6.0 を出したい、ブロッカーは？」→ GO → 出荷）、**続けて `<< … >>` の 5 便**（規則を変えて戻した 2 便を含む・下の順）。**HEAD＝この HANDOFF の commit（`git log -1`）・その前は `03a91356`＝第 5 便・未 push 5（push は指示待ち）・full 7229 / 0 / 4 / 7234・台帳 810 不動・snapshot 247 不動**。

✅ ★★★★ **0.6.0 出荷済み（2026-09-07・第348・ユーザー GO「tag を打って push して」）**: bump commit `8f75f249`（7 か所・RELEASING §2）→ CI 34118258817 5 脚緑 → tag `v0.6.0`（annotated・本文 `Lily# 0.6.0`）→ Release run **34118792209 は 13 脚すべて緑**（test／cli 4／vsix 8 直列・約 16 分）。**§7 照合**: GitHub Release 12/12 アセット・draft/prerelease とも false・`vsce show` 8 ターゲット 0.6.0・win32-x64 の VSIX HEAD 200。**extensionquery は直後 4/8**（linux-armhf／alpine／darwin ×2 が未索引）＝0.5.0 と同じ索引遅延の形（RELEASING §7・10 分で収束する）——**再照合: 約 5 分で 7/8（darwin-arm64 が最後）・約 8 分で 8/8・darwin-arm64 の HEAD 200**＝手で publish したものは 0。ローカル: full 7215 / 0 / 4 / 7219（`scratch/p348/full1.trx`）・Core 0 警告・`lysc --version` 0.6.0。⚠️ **第345 の「未 push 27・第 4 便/第 5 便は未承認」は stale**——ユーザーが第342〜345 を畳んで push 済み（`b0e68490`＝HEAD 直前）。**次の CHANGELOG 項目は `## 0.7.0` へ**（tag 済みの節へ足すのはリリースノートの改竄・RULES）。

★★★★ **第348 第 5 便（2026-09-08）＝`<< … >>` のメンバーに印と取り分を足した（ユーザー「欠陥は直して。安全に足せるものは足して。ドットは音符や数字にくっつけずに書く」）**: **⑴ 欠陥 2 件**: **`<< c\3 e\2 g >>` と `>>4\3` の弦番号が無言で落ちていた**（`ArpeggioSyntax.Articulations` の型フィルタに `StringNumberAnnotationSyntax` が無く、collector も読んでいなかった）／**`>>4( d)` の `(` を頁が落として `)` に LYS4010 を出していた**（XML は出していた）。**⑵ 足したもの**: メンバーの scripts・`@finger`・`@dead`・dynamic・弦番号（`CollectArticulations`/`ArticulationsOf` に `PitchSyntax` 腕）、メンバーのスラー `<< c( e g) >>`（group の直下の子として保持・`ArpeggioSyntax.Sequence` が返す）、group の弦番号＝全メンバー、**`>>` 後の `~ ( )` は最後のメンバーに掛ける**（XML/MIDI の "last emitted" と同じ規則で 4 出力が揃う）。**⑶ 取り分＝離して書くドット**: `<< c . d >>4` ＝ c が 2 取り分（`tuplet 3/2 { c4 d8 }`）・`<< c . . d >>4` ＝ `c8. d16`・`<< c . d . e >>4` ＝ 5:4。**綴りは `ArpeggioSubdivision.SpellShares`**（1 音で書けなければ長い順にタイ・`<< c . . . . d >>4` ＝ 6:4 の `c4~ c16 d16`）。**くっつけたドット（`c.`・`3.`）と先頭のドットは LYS0023**（木には残す＝round trip）。`~ [ ]` は中では構文エラー＋言い換えのヒント。**検討した代替**: ドット＝1.5 倍（7 月に却下した読み＝5 分割になる）／`~` の繰り返し（冗長）／数字（度数と衝突）。**chord row の `.`（「新しい和音は無い＝前が続く」）と同じ読みなので語彙が揃う**。コーパスの根拠: 不揃い tuplet 63 個中 49 個が 2:1（`tuplet 3/4 { c8 d16 }` 41・`3/2 { c8 d16 }` 8）＝`<< c . d >>N` 1 行。**⑷ 双子**: 全メンバーが音価を書く（`r8 c8 cis8`）。**LP 実測** `scratch/p349/dot/`（`dot.lys` の 7 群）＝Lily# と LP の絵が一致。**LYS4008 は group の印にだけ**残す（dynamic・chord・弦番号以外）。番人: `DoubleAngleArpeggioTests.{ASpacedDot_*, ADot_IsWrittenSpaced_*, Shares_ReachTheMidiAndTheMusicXml, MemberMarks_AreApplied, Slurs_OnMembersAndOnTheGroup}`・`LilyPondExporterTests.Arpeggio_SharesAndMemberMarks_AreWrittenOut`。⚠️ **未対応（意図）**: group の scripts（`>>4@accent`＝どのメンバーかが曖昧・LYS4008 のまま）、中の `~`（ドットで書く）、`[ ]`（自動梁）、`:16`。⚠️ **拾い箱（この便で見つけた別件・未着手）**: **MusicXML は弦番号を*どの音にも*書き出していない**（`MusicXmlExporter` に `StringNumberAnnotationSyntax` の読み手が無い＝`<string>` 要素 0）。`<<>>` に限らない gap なので別便。**ユーザーの `q` 案は退けた**（`q`＝再打鍵の語で「延ばす」と逆・`<< c c d >>` で書ける）。**管理外の `scratch/site-showcase/manual-body.html` の Arpeggios 節にもドットとメンバーの印を足した**。

⚠️⚠️⚠️ ★★★★ **第348 第 4 便＝第 2 便の規則を慣用に戻した（ユーザー「戻すべきなら戻して」）**: **§5.0 の新例＝「ユーザーの手書き 1 例を仕様に一般化し、慣用を調べずに書いた」**。第 2 便は `tuplet 3/4 { r16 c cis }` と `<<r c cis>>4` が違うという報告を受けて、**手書きの側（16 分 3:4）を正として規則を「鳴る長さより長く書かない」に変えた**が、**慣用（Gould『Behind Bars』・連符の数字は M 未満で最大の 2 のべきに対して＝元の規則）では 3 連は 8 分 3:2 で、最初の描画のほうが正しかった**。**第 2 便の規則は 5 連を 32 分 5:8・7 連を 32 分 7:8 にして慣用の 2 本梁から外れる**。⚠️ **LP 双子は仕様の裁定者ではない**——LP は書かれた tuplet をそのまま描くだけで、**「その綴りが慣用か」は答えない**（第 2 便の「LP で確認」は「手書きの tuplet が 16 分で描かれる」を確かめただけ）。**ユーザーが「五連符・六連符の梁の本数は決まっていないのか」と訊いて初めて慣用を引いた**＝**規則を変える前に「この綴りは慣用か／既定を変えるのか」を 1 行で確かめる**。**戻し**: `ArpeggioSubdivision.Compute`＝素の総和は M 以下で最大の 2 のべき（3→2・5〜7→4・9〜15→8）、**付点の総和だけ新規＝3·2^k のうち M に最も近い枠（同点は小さい方）＝2:3・4:3・5:6・7:6**（元の規則は付点 8 分 ×3 を 3:2 で書いていた＝誰も書かない綴り。これが 0.7.0 に残る唯一の変更）。番人・docs・両 CHANGELOG・exporter の番人（`\tuplet 3/2 { r8 c cis }`）を全部書き直し。**LP 再照合**: `tup-arp.lys` 7 小節すべて Lily# の音価/比 ＝ LP の probe（bar4 ＝ bar1 と同じ 4 分 3:2・bar5 ＝ 8 分 3:2・bar7 ＝ 16 分 3:2）。

★★★ **第348 第 2 便＝`<< … >>` の音価規則（ユーザー報告 `scratch/ベースタブLy/tup-arp.lys` L7/L8・0.7.0 の最初の項）** ← **第 4 便で戻した（上）。経緯として残す**: `<<r c cis>>4` が **8 分 3:2**（梁 1 本・8 分休符）で、手書きの `tuplet 3/4 { r16 c cis }`（梁 2 本・16 分休符）と違った。**規則を 1 か所 `ArpeggioSubdivision.Compute` で差し替え**: 枠 P ＝ **総和を割り切る最大の素の音価で数えた個数のうち M 以上の最小**（＝「書かれた音価は鳴る長さより長くしない」）。4 分の 3 → 16 分 3:4／2 分の 3 → 8 分 3:4／4 分の 5 → 32 分 5:8／付点 4 分の 2 → 8 分 2:3／付点 4 分の 3 → 素の 8 分（括弧無し）。**collector・MIDI・MusicXML は同じ Compute を読むので 1 変更で 3 出力**。**LP 双子で正しい絵を確認**（ユーザー「tuplet 側が間違っている可能性もある」）: `scratch/p349/probe.ly`（`-dinclude-settings` で Stem.beaming／Rest.duration-log／TupletNumber.text を stdout）→ **LP 2.26.0 は REST dur-log 4・STEM dur-log 4 ×2 で beaming `(-1 0)`（梁 2 本）・番号 3**。**Lily# は tuplet 版と `<<>>` 版の PNG がハッシュ同一・SVG は data-pos 以外同一**（`ls-a.png`/`ls-b.png`・`lp-a.png`）。番人 `ThreeInAQuarter_IsTheSamePictureAsTheHandWrittenTuplet34`（item 列の同値）＋ `DottedTotal_FramesOnThreeTimesAPowerOfTwo`・既存 5 本を新規則へ。**射程**: 追跡 `.lys` に `<<` を使う本は 0（`pcsm-probe.lys` の `<<` はコメント内の LP 綴り）＝snapshot・台帳とも不動。full **7217 / 0 / 4 / 7221**（`scratch/p349/full1.trx`）。~~⚠️ 開示: `lysc ly` は `<< … >>` を書き出さない~~ → **第 3 便で閉じた（下）**。docs: SYNTAX_REFERENCE／GRAMMAR／GRAMMAR_FOR_LLM／両 CHANGELOG `## 0.7.0`。push は指示待ち。

★★★ **第348 第 3 便＝`lysc ly` が `<< … >>` を `\tuplet` に翻訳する（ユーザー指示）**: `LilyPondExporter.EmitArpeggio`＋`EmitArpeggioChord`（`EmitItem` の 1 腕。以前は `Skip()`＝"Arpeggio not exported"）。**規則は collector の写し**: root＝最初の有音メンバー（bare letter を frame で解いた octave ＋ group の記号・root 自身の記号は局所）、他の有音メンバーは root に積む（`anchor + (step >= root ? 0 : 1) + 自分の記号`）、度数は `ChordDegrees.Resolve`、**和音メンバーは第 1 文字で積み、その中身は積んだ octave を base に絶対読み**（collector が `OctaveAbsolute = true` で `CreateChordItem` を呼ぶ形）。**LP の `\relative` は列として前の音高に対して読む（休符は素通り）ので、記号は LP 鎖に対して計算し直し、group の後は Lily# の frame＝root・LP の frame＝最後の音高に割れる**（度数和音と同じ割れ方・`EmitMusicPitch` が次の音で埋める）。**音価**: 先頭メンバーだけ `sub.MemberValue` を書き、残りは継承。**group の後は Lily# の carry（`>>N` は点ごと・無ければ直前）を次のイベントに強制書き**（`_forceNextDuration`）。番人 4 本（`LilyPondExporterTests.Arpeggio_*`＝tuplet 版と `<<>>` 版の出力同一・`<< c g e >>` → `c16 g' e`・`<< c e g >> c` → `c,`・`>>2 c` → `c,2`・付点 4 分の継承 `c4. c8 e g c,4.`・絶対 `c16 e g,`）。**LP 実測**: `b-arp.ly`（`<<>>` 側の双子）を LP 2.26.0 に通して **`lp-a.png` とハッシュ同一・probe 出力同一**。ユーザーの `tup-arp.lys` の双子は L7/L8 とも `\tuplet 3/4 { r16 c cis }`。
⚠️⚠️ ★★★ **副産物＝pre-existing の双子欠陥を閉じた**: **exporter は「Lily# は付点を捨てる」（2026-08-01 実測）を前提に、付点の次の音を `d4` と値だけ書いていたが、collector は 2026-08-07 から点も引き継ぐ**（`ItemFactory.CreateNoteItem`: `dots = note.Duration?.DotCount ?? _defaultDots`）——**`c4. d` は頁 6/8・双子 5/8 の別音楽だった**。`_lastWrittenDots` を足し、強制書きは点ごと（`d4.`）。番人 `AnEventAfterADottedOne_WritesItsValue_WithTheDotsLilySharpGivesIt` は**前提（collector が d を付点 4 分に読む）を頁で主張してから**双子を見る。**射程＝`scratch/p349/sweep349.ps1`（921 冊・base＝`ce401bbb` の exe・`ly` 本文比較）: MOVED 15＝付点 13（`audit/lp-regression/lys` 4＝`dots`・`dot-rest-*` 2・`fermata-dot-position`／`audit/lpreg/probe-sticky-*-dot` 2／ユーザー 7）＋ `<<` 2（`tup-arp`・`etde10-05.xml`）。** ⚠️ **`audit/lpreg/*.ly` は `lysc ly` の出力ではない**（LP 回帰の原本を手で枠合わせした双子）——この便は最初それと比べて「63/63 動いた」と読み、前提を確かめて捨てた（§5.0）。生成物 2 は再生成（行番号のみ）。
