...
⚠️⚠️ ★★★★ **⑴ 第337 ⑽ の「上側 ~2.2 ss の見積り不足」は*枠の取り違え*だった**（§5.2.1② の族）: `placed sys` の **`top` は系の原点＝最初の譜の*上線*から**測り、LP の段のインクは**譜の *refpoint*（tab なら中央線）から**測っている。差は `ToFirst = −MultiStaffLayouter.StaffRefpoint(first) = Height/2 − Y`＝**4 弦 bass tab でちょうど 2.25**。同じ枠に置き直すと Are You Gonna の 26 段は **20 段で ±0.1 ss 以内に一致**（段2 Lily# 3.800＋2.25＝6.050 対 LP 6.057／段3 4.561 対 4.525／段6・8・10・16 は 6.050 対 6.057）。**「~2.2 の見積り不足」は存在しない。** ⇒ ★★★ **`top/body/bottom` と `up/down` は別の枠。並べる前に `ToFirst` を足すこと**（外れた 3 段のうち 2 段は `.lys` 側で mark が別の小節に居るため＝第337 ⒞ が Sugar で見た源と同じ・ユーザー担当）。

★★★ **⑵ 「未特定の反復項」の正体＝`RehearsalMark`**: 新しい計器 `scratch/p338/grob-top.ly`（`page-post-process` から paper system の `'system-grob` を取り、`all-elements` を**最終座標**で歩いて**段の上／下インクの正体を grob 名で刷る**。**インクは 1 滴も足さない＝stdout だけ**＝第337 ⑵ の番号インクの罠を踏まない）。Are You Gonna の一段おきの 6.057 は `\mark \markup { \box A }` の箱で、**bend でも欠けた grob 族でもない**。⇒ **bend を播く移植は根拠を失ったので打っていない。**

⚠️ ★★★ **⑶ 括弧は「描くのに予約していない」——が、頁は動かさない（記録するが直さない）**: LP の staff+tab 段の*ステンシル*の上下端は `SystemStartBracket` で、外側の譜のインクの縁から **1.643 ss**（`brackettips` の箱 1.368 ＋ `add_at_edge` の据え付け 0.225）。Lily# も**描く**（`SharedRenderer.DrawSystemStartBracket` が `BracketTipUp/Down` を `top`/`bottom` の外へ吊る＝譜線の中心から 1.593＝LP との差 0.05 は譜線の半太さ）が、**`SkylineBuilder` は delimiter をどこにも播かない**（`GrandStaffLayout.BraceTop/Bottom` は譜ちょうど）。⚠️⚠️ **陽性対照で無罪**: `\override SystemStartBracket.stencil = ##f` で Boogie を通しても **LP の頁割りは 8,8,8,1 のまま不動**（`scratch/p338/ab`）。**`build_system_skyline` は*譜*しか舐めない**（`lily/page-layout-problem.cc:1080-1124`＝`staves` を歩く）。頁の得点側は `lily/system.cc:893-923 part_of_line_pure_height` が `other_elements` を unite するので System 直属の grob も入り**うる**が、**実測が動かない以上これは頁の島ではない。**

★★★★ **⑷ 島の正体は「題」だった（実書で A/B）**: `Boogie Oogie Oogie` から `\header` を落とすと **LP は 8,8,8＝Lily# の答えそのもの**（`scratch/p338/ab-notitle`。ヘッダ付きなら 7,8,8,1）。⇒ **残る純頁差は tab のインクでも括弧でもなく、題の載る 1 頁目。**

⚠️ ★★★ **⑸ 台帳 4 点（新 book TTLF/TTNF・STB8T）＝*全部 exact*＝題の値付けは無罪**: `page.titled.full.staves-on-first-page` ／ `page.untitled-full-control.staves-on-first-page`（`probes/titled-page.ly`・16 段強制・justified の 1 譜。**LP は題ありでも 13 段＝帯は 1 段も食わない**＝12 本の system-system ばね（圧縮強度 4）が飲む。⚠️ **LP は題を頁の*行*に数える**ので page 1 の lines は 14。台帳に載せるのは*段*の数）と、その**きつい半分** `page.staff-tab.titled.staves-on-first-page` ／ `page.staff-tab.titled.page-count`（`probes/staff-tab-page.ly` の新 book STB8T＝STB8 に題を載せただけ。**STB8 は既に 1 force ぶん圧縮して 8 段を載せている**のに、**LP は題を足しても 8 段 1 頁**）。**Lily# は 4 点とも LP と一致（16 staves / 1 page・13 / 13）。** ⇒ ★★★ **breaker の題の値段は正しい。Boogie が割れるのは*段の高さ*の側**（Boogie の 1 頁目の段は静かな STB8 より高い＝mark・volta bracket・臨時記号・script）。

★ **⑹ 数**: 台帳 786 → **790**／exact 618 → **622**／ss 非ゼロ 203（総和 26.789）＝不動／count 164 → **168** うち非ゼロ 0／OPEN 0。full **7152 / 0 / 4 / 7156**（`scratch/p338/full1.trx`・5 分 02 秒・+4＝新台帳点）。snapshot 247 枚・追跡コーパス 597 冊＝不動。

★ **開始時裏取り**: HEAD **`300c37f4`**——⚠️ **第337 が引き継いだ「未 push 76」はユーザーが畳んで push 済み**（`git log` の頭は `tests: the dead-citation ceiling carries the sessions 329-337 regroup (529 → 560)`・02:14）＝**未 push 0**・木 clean・未追跡 0。`gh run list` の最新 push（8 分前）は緑。`--no-incremental` build 0 エラー・Core 0 警告（91 秒）・full **7148 / 0 / 4 / 7152**（`scratch/p338/run1.trx`・3 分 17 秒）・台帳 786／exact 618／ss 非ゼロ 203（26.789）／count 164／OPEN 0・snapshot 247・追跡コーパス 597＝**全部引き継ぎどおり**。
**終了時（第 1 便）**: **commit 2 本（台帳 4 点＋プローブ 2 本の新 book＋Lily# 側の双子／この HANDOFF＋ARCHIVE＝第336 の経緯を先頭へ逐語）・未 push 2・木 clean・未追跡 0**・製品コード不変（出力不動・snapshot 0 枚）。
✅ ★★★★ **⑺ 第 2 便（ユーザー「次便がこの便で有利なら着手」→ 計器が温かいので Boogie の 1 頁目のインクを名指しした・製品 0・台帳 0）**: **⒜ LP 側 10 腕の A/B（`scratch/p338/lp-drop.ps1`＋`parse-drop.ps1`＝`\override <grob>.stencil = ##f` を 1 族ずつ・3 分で全腕）で*動いたのは `RehearsalMark` だけ***: 対照 book1(staff+tab)=8,8,8,1／book2(bassTab)=13,12 → **no-RehearsalMark で 9,8,8／14,11**（＝1 頁目が 1 段増える）。Accidental・Slur・Script・Tie・LedgerLineSpanner・TextScript・VoltaBracket・StringNumber・MetronomeMark は**全部不動**。⇒ **LP の 1 頁目を決めているインクは mark。** **⒝ Lily# 側の A/B**（`lysc layout --all`・題を落とした `.lys`／`@mark(...)` を落とした `.lys`）: **both は 8,8,8 で*題にも mark にも動かない*／staff 単独は 12,12 → 13,11（mark を落とすと動く）／tab は 12,12 で不動**。**⒞ ところが Lily# は mark を*正しく*予約している**（`scratch/p338/ZzP338bProbeTests.cs.txt` → `probe2.txt`・段ごとの top を mark 有無で対にした）: tab の mark 段は **6.150／6.152／6.152（refpoint 枠）対 LP 6.096／6.096／6.138＝±0.06**、mark を落とすと 4.561 まで下がる＝**予約は在るし LP と同じ大きさ**。⇒ ★★ **mark は Lily# の欠陥ではない。** ⚠️⚠️ **⒟ 代わりに出た本物の差＝*静かな段***: tab book の s6・s8・s12 は **LP 2.880（tab の音部記号）対 Lily# 4.561＝+1.68 ss**——**Lily# は上弦の上 2.25 に tab 符尾の先端を予約しているが、その段の LP には上向き符尾が 1 本も無い**（LP の上インクは clef）。**3/12 段で +1.68**。**⒠ 「both」で Lily# が 1 段多く詰める分の唯一の未予約インクは `SystemStartBracket`**（⑶ の 1.005 ss／段が 8 段で ~8 ss・mark の ~3.7 ss より大きい）。⚠️ **⑶ の「無罪」は言い過ぎだった**——**括弧だけ落とす腕が動かなかったのは*全頁から*落として最適化ごと変わったから**で、**1 頁目の余白の話としては未決**。⇒ **次便は ⒟（tab の静かな段の符尾）を台帳点にしてから ⒠ に戻る。**
**終了時（第 2 便）**: **commit 1 本（この HANDOFF）・未 push 3・木 clean・未追跡 0**・製品コード不変（台帳・snapshot・追跡 `.lys` すべて不動）。
✅ ★★★★ **⑻ 第 3 便（ユーザー「次便がこの便で有利なら着手」→ ⒠ を決着させて番人にした・製品 0）＝括弧は頁を 1 段も動かさない**: ⚠️ **⑺⒠ の「未決」を測って閉じた。第 1 便 ⑶ の「記録するが直さない」が正しく、第 2 便の訂正が行き過ぎだった。** **⒜ 気づき**: STB8／STB8T は `<< \new Staff \new TabStaff >>`＝**SystemStartBar しか出ない**（括弧が出るのは `\new StaffGroup`）——**あの 2 冊は括弧を一度も試していなかった**。**⒝ `STB8B`**（STB8 を `\new StaffGroup` に入れただけ）＝**8 段 1 頁＝STB8 と同一**。**1 force ぶん圧縮して 8 段を載せている頁の上で、3.2 ss/段 × 8 ＝ 25 ss の括弧インクが 1 段も買わない**⇒ **括弧は頁の得点に入っていない**（`build_system_skyline` は譜だけ・`part_of_line_pure_height` の `other_elements` もこの grob に届かない）。**⒞ ところが Lily# には双子が書けない**——**`staffGroup` に `tab` を入れられない（LYS6011）**＝**ユーザーのコーパスの形（括弧付き staff+tab）は Lily# の文法に無い**（＝Lily# はあの本に括弧を*描いてもいない*。文法の穴であって頁の穴ではない）。⇒ **⒟ 番人は 5 線譜 2 枚で立てた**: 新 book **`TWOP`（素の対）／`TWOB`（`\new StaffGroup`）**＝10 段・**両方 8,2**（頁 1 は圧縮＝6＋8×9＋7×12 対 band 157.628）。**台帳 4 点・Lily# は 4 点とも exact**（`page.bracketed-pair.staves-on-first-page`／`.page-count`／`page.bare-pair-control.*`）。**「delimiter を頁の silhouette に播く」移植をしたらこの 4 点が動く＝それだけが気づく**ので、**defect ではなく guard として起票**。
★ **⑼ 第 3 便の数**: 台帳 790 → **794**／exact 622 → **626**／ss 非ゼロ 203（26.789）不動／count 168 → **172** うち非ゼロ 0／OPEN 0。full **7156 / 0 / 4 / 7160**（`scratch/p338/full2.trx`・5 分 30 秒・+4）・`--no-incremental` build 0 エラー・Core 0 警告（173 秒）・snapshot 247・追跡 `.lys` 597＝不動。
⚠️ ★★★ **⑽ ⒟ の手がかりが鋭くなった（次便の入口）**: Lily# の「静かな段」の上 extent は **どの本でも 2.311 という*定数***——**Boogie の tab book（原点＝上弦）でも both book（原点＝bass 5 線譜の上線）でも同じ 2.311**、一方 LP は段ごとに違う（tab の音部記号 2.880／符尾 4.500／譜線 2.05）。**測ったインクではなく床**の形。⚠️ **「F 音部記号を中央線の枠で読んでいる」説は外れ**（`ClefFOutline.Top` 1.052 ＋ 原点 1.0 ＝ 中央上 2.052＝上線上 0.052）。**正体は未特定。**
**終了時（第 3 便）**: **commit 2 本（台帳 4 点＋プローブ 3 book＝STB8B/TWOP/TWOB＋Lily# 側の双子／この HANDOFF）・未 push 5・木 clean・未追跡 0**・製品コード不変（出力不動・snapshot 0 枚）。
✅ ⚠️⚠️ ★★★★ **⑾ 第 4 便（ユーザー「次便がこの便で有利なら着手」→ ⒟ を追って*また計器の側*だった・製品 0・台帳 0）**: **⒜ `upExtent` は床ではない**（`LayoutUtilities.CalculateUpExtent` ＝ `upSkyline.MaxHeight()`）＝**2.311 は実際に播かれているインク**。**⒝ 正体は `LayoutEngine.SystemCount.cs:100` が既に名指していた**——「**the placed continuation lines are 2.311**」＝**行頭の前置き（音部記号・調号・そして*小節番号*）の上インク**。**小節番号は譜の種類に依らない**ので bass 5 線譜でも bassTab でも同じ値になる（それが「定数に見えた」理由）。⚠️⚠️ **⒞ ⇒ LP 側の基線が番号を消していた**: `scratch/p321/probe-settings.ly` の **`BarNumber.stencil = ##f`**（第337 ⑵ が all-visible の番号の摂動を止めるために足した）は、**LilyPond が*既定で*行頭に描く番号まで消す**。**素の LP で測り直すと静かな段の上インクは `BarNumber` で上線の上 2.279、Lily# は 2.311＝差 0.032**（`scratch/p338/lp-bare.ps1` → `bare/lp.out`・`p1 sys6 topRef=4.279` の上位 grob が BarNumber）。**+1.68 は存在しなかった。** ⚠️ **⒟ 頁割りには効かない**（素の LP も抑制した LP も Boogie は book1 8,8,8,1／book2 13,12＝同じ）＝**第332〜337 の掃き（段数・頁数）は取り直さなくてよい。効くのはインクの数値だけ。** ⇒ ★★★ **インクを LP と比べるときは `probe-settings.ly` を混ぜず `lp-bare.ps1`（grob-top.ly だけ）で素の LP を使う**（memory `reference_lilysharp_probe_settings_hides_barnumber_ink`）。**⒠ 番号を入れて Boogie の 1 頁目を取り直すと、段ごとの項が*全部*合う**: topRef（LP 素）9.251 / 7.075 / 7.545 / 7.075 / 5.594 / 4.279 / 5.594 / 4.305 対 Lily#（+ToFirst 2.0）9.101 / 7.160 / 7.545 / 7.160 / 5.900 / 4.293 / 5.900 / 4.311＝**差 ≤0.31**・下側も LP の priced 分（tab の音部記号 2.880）対 Lily# 2.888・body はどちらも最小枠 8.0。**⇒ 段の高さでは説明がつかない。残るのは 1 頁目の*鎖の解*そのもの**（LP の 1 頁目は 7 段で slack 9.28 ss＝8 段目に要る ~23 ss には全然足りない。Lily# は同じ段で 8 段載せている）。
**終了時（第 4 便）**: **commit 1 本（この HANDOFF）・未 push 6・木 clean・未追跡 0**・製品コード不変（台帳・snapshot・追跡 `.lys` すべて不動）。
✅ ★★★★ **⑿ 第 5 便（ユーザー「有利なら着手」→ 1 頁目の鎖を並べた・製品 0・台帳 0）＝差は*段の高さ*ではなく*段間の距離***: `scratch/p338/ZzP338cProbeTests.cs.txt`（頁ごとの系の Y と題の帯を吐く捨て計器）と `scratch/p338/bare/lp.out` の `yoff` を、**同じ 7 段**（Boogie「both」1 頁目）で並べた。⚠️ **`SystemLayout.Y` は頁の*下端*からの Y-up**（頁上端からの距離 ＝ `PageHeight − MarginTop − Y`）。**段間（原点→原点）**: **LP 22.956 / 20.310 / 21.250 / 22.261 / 22.095 / 19.465（平均 21.39・幅 3.49）対 Lily# 18.733 / 19.171 / 18.733 / 17.630 / 17.973 / 17.774（平均 18.34・幅 1.54）**＝**1 段あたり −3.05 ss・6 段で 18.3 ss ≒ 段 1 本**＝8 段目が載る budget そのもの。**題の帯は逆に Lily# の方が高くつく**（無題との差で Lily# 7.33 ss・LP は約 4.9）ので、**唯一の候補は段間**。⇒ ★★ **原点→原点 ＝ 置かれた譜間距離 ＋ system-system ばね**。LP の幅 3.49 は**床が内容ごとに効いている**（X を見る skyline の距離＝上の段の下インク＋下の段の上インク＋padding 1）ことを意味し、**Lily# の幅 1.54 は「ほぼ一定の項が距離を決めている」＝床がほとんど縛っていない**。⚠️ **注意**: 2 つの頁は載っている段数が違う（7 対 8）ので力の regime が違う——**頑健なのは*ばらつき*の比較**で、平均差 3.05 の帰属は次便が床そのものを吐いて確かめる。
**終了時（第 5 便）**: **commit 1 本（この HANDOFF）・未 push 7・木 clean・未追跡 0**・製品コード不変。
✅ ★★★★ **⒀ 第 6 便（ユーザー「有利なら着手」→ 段間を*床*と*ばね*に分解した・製品 0・台帳 0）＝床の値は正しい。違うのは解いた長さ**: **⒜ LP の床を実データから組んだ**（`scratch/p338/lp-floors.ps1`＝`bare/lp.out` の grob 明細から**括弧を除いた**上下インクを拾い、床＝上の段の下インク＋下の段の上インク＋padding 1）。**⒝ Lily# の置かれた譜間距離を実測**（`scratch/p338/ZzP338dProbeTests.cs.txt`＝`StaffRefpoint = Y − h/2` の実配置）: **8.326**（段 5・7 だけ 8.669／8.665）＝**基本距離 9 から圧縮されている**（LP は 8.927）。⇒ ばね ＝ 原点→原点 − refDist。**⒞ 表（Boogie「both」1 頁目・同じ 6 対）**:

| 対 | Lily# ばね | Lily# 床 | 差 | LP ばね | LP 床 | LP の余裕 |
|---|---|---|---|---|---|---|
|1→2|10.407|11.048|**−0.641**|14.030|10.955|+3.075|
|2→3|10.845|11.433|**−0.588**|11.384|11.425|−0.041|
|3→4|10.407|11.048|**−0.641**|12.323|10.955|+1.368|
|4→5| 8.961| 9.788|**−0.827**|13.335| 9.474|+3.861|
|5→6| 9.647| 8.181|+1.466|13.168| 8.159|+5.009|
|6→7| 9.109| 9.788|**−0.679**|10.539| 9.474|+1.065|

⇒ ★★★ **床の*値*は両者一致**（11.048 対 10.955／11.433 対 11.425／8.181 対 8.159＝差 ≤0.31＝⑾⒠ のインク差そのもの）。**違うのは解いた長さで、LP の 1 頁目は床から +1.07〜+5.01 伸びている（＝縛られていない）のに、Lily# は 6 対中 5 対で床を 0.59〜0.83 下回る**。加えて**譜対そのものも 9 → 8.326 に圧縮**（LP 8.927）。**1 段あたりの差 3.05 ＝ 譜対 0.60 ＋ 段間ばね 2.45。** ⚠️⚠️ **ただし「床を割っている」とはまだ言えない**——ここで組んだ床は**両端の最大インクの単純和**で、実際の床は **X を見た skyline 距離**（`Skyline::distance`）だから**もっと小さくなり得る**。実際この本では**上の段の下インクは tab の音部記号（x≈0.6-0.8）・下の段の上インクは RehearsalMark の箱（x≈5.7）で x が重なっていない**＝真の床は単純和より小さい。**⇒ 次便は Lily# の*エンジン自身*が使った床（`VerticalSkyline.Distance` の値）を対ごとに吐いて、この表の「床」列を置き換える。**
**終了時（第 6 便）**: **commit 1 本（この HANDOFF）・未 push 8・木 clean・未追跡 0**・製品コード不変。
⚠️⚠️ ★★★★ **⒁ 第 7 便（ユーザー「有利なら着手」→ ⒀ の矛盾を潰して*機構*に当たった・製品 0・台帳 0）＝段間の rod は「最小の枠」で作る**: **⒜ 論理で矛盾が出た**——`PageLayouter.PositionSystemsOnPage` は各対に `CreateSpring(spec, skylineDistance + padding)` を渡す＝**rod は在る**。そして**同一 spec のばねは同一の力では同じ長さになる**ので、**6 本が全部違う値なら 5 本以上は rod に乗っている**。これは Lily# でも LP でも成り立つはずなのに、⒀ で組んだ LP の床（両端インクの単純和）だと LP のばねは床から +1〜+5 も離れていて「rod に乗っている」と両立しない ⇒ **⒀ の LP 床モデルが間違っている。** **⒝ LP の実装を読んで判明**（`lily/page-layout-problem.cc:579-634 append_system`）: 段間の rod は `up_skyline.distance(bottom_skyline_, skyline-horizontal-padding) + padding` だが、**その skyline は `build_system_skyline (elts, minimum_offsets_with_min_dist, …)`＝譜を*整列の最小*に置いた状態で作られる**（`Align_interface::get_minimum_translations`）。**Lily# は配置済み＝伸びた位置の skyline から床を出している**（`LayoutEngine` が `sysStaffGroups`／`sysHeight` を配置済みで渡し、`InterSystemSkylineDistance` がそれを読む）。⇒ ★★★ **同じ「段間の床」を*違う枠*で測っている**——LP は最小の枠、Lily# は描かれた枠。**Boogie の 1 頁目は譜対が LP で 8.927（最小 8 から伸びている）・Lily# で 8.326** なので、**枠の差はちょうどその伸び分**であり、**LP の rod の方が大きく出る**＝LP の段間が広い、という観測の向きと一致する。⚠️ **これは*機構の一致*であって、まだ数量では確かめていない**（rod を最小の枠で組み直した値を出して ⒀ の表の「床」列を置き換えるのが確認）。⚠️ **第336 ⑻ の `BreakerRefpointFrame`（ブレーカーは pure な最小の枠で値付けする）と同じ族の話で、そこは*ブレーカー*、ここは*頁の配置の鎖*。**
**終了時（第 7 便）**: **commit 1 本（この HANDOFF）・未 push 9・木 clean・未追跡 0**・製品コード不変。
⚠️⚠️⚠️ ★★★★ **⒂ 第 8 便（ユーザー「有利なら着手」→ ⒁ の機構を*この本で*検算したら inert だった・製品 0・台帳 0）**: **⒜ ⒁ は「移植すべき島」ではない。** `append_system` が rod を最小の枠で組むのは**本当**だが、**枠を移しても skyline が動くのは「錨でない譜のインクが極値のとき」だけ**。Boogie の 2 譜系はそうではない——**下側の極値は全部 tab（down の錨）自身**（tab refpoint から Clef −2.880／StaffSymbol −2.300／BarLine −2.250。**上の五線の下端は tab refpoint の +6.877 で下側 skyline に一切寄与しない**）、上側の極値は mark で第1譜（up の錨）の上。⇒ **最小の枠と描かれた枠で rod は同値。3.05 ss を説明しない。** ⚠️ **⒁ を「次の移植」として渡すと inert な出力変更になるところだった**（陽性対照の族・第338 で 4 度目の「島でなかった」）。**機構の記述としては正しいので消さない**が、**着手の根拠にはしない**（効くのは 3 譜以上、または錨でない譜が外へ出る本）。
★★ **⒝ 残った所在**: ⒁⒜ の論法（同一 spec・同一の力なら自由なばねは同じ長さ ⇒ 6 本が違えば 5 本以上は rod 上）を **Lily# に当てると成立**する——**Lily# の rod ≈ 解いた値 10.407 / 10.845 / 10.407 / 8.961 / 9.647 / 9.109**。これは ⒀ の単純和（11.048 …）より 0.6〜0.8 小さく、**X を見た距離は単純和以下**なので**矛盾しない**（mark は x≈5.7・tab の音部記号は x≈0.7 で重ならない）。⇒ **Lily# の rod は「小さい」だけで、誤りとは言えない。** ところが**同じ論法を LP に当てると成立しない**（LP の rod ≈ 10.5〜14.0 になり、両端インクの単純和 8.2〜11.4 を超える＝skyline 距離では不可能）。⇒ ★★★ **LP の頁の解は paper-system の dump だけからは再構成できない**（ばねの ideal か compliance に、こちらが持っていない項が在る）。**外から LP の solver を逆算する路線はここで打ち切り。**
**終了時（第 8 便）**: **commit 1 本（この HANDOFF）・未 push 10・木 clean・未追跡 0**・製品コード不変。

⇒ ★★★★ **次の一手**: ✅ ⑺ mark・✅ ⑻ 括弧・✅ ⑾ 2.311（小節番号）・✅ ⑿ 段の高さは無罪＝**差は段間の距離**（1 段 −3.05 ss）。**残るのは 1 つ: その距離の床**。
**⑴ Lily# 自身の鎖を計器にする（外から LP を逆算する路線は ⒂⒝ で打ち切った）**: **やること** ⒜ `PageLayouter.PositionSystemsOnPage` に `LayoutEngine.DebugPageBreakingScoring` と同じ形の debug hook を足し、**1 頁ぶんのばねを 1 本ずつ**吐く（種類＝top/staff/system/last-bottom・spec の basic/min/stretch・`ensure_min_distance` に渡した rod・解いた長さ・頁の力）。**inert な debug delegate なので出力は不変**だが**製品コードには触る**＝小さいながら commit する変更（承認は不要のはず。迷ったら聞く）。⒝ Boogie「both」1 頁目で吐き、⒀ の表の「床」列を**エンジン自身の rod** に置き換える。**Lily# の rod は ≈ 解いた値**（⒂⒝）と予想されるので、**当たっていれば「Lily# の頁は全ばねが rod に乗って最大限圧縮されている」**が確定し、**LP は 1 頁目にたっぷり余裕を持つ**（観測）——ならば**差は rod ではなく*段数の選択***（Lily# は rod に触れるまで段を足す／LP はもっと早く止める）。⒞ その線なら次は `PageBreaker.Demerits`／`ChooseSystemCount` と LP の `Optimal_page_breaking` の**罰則**を突き合わせる（第322 で移植済みの部分の再点検）。⚠️ **⒁ の「最小の枠」は着手の根拠にしない**（⒂⒜＝この本では inert）。⚠️ **譜対も Lily# 8.326／LP 8.927** で 1 段の差 3.05 のうち 0.60 はこちら＝同じ「頁の力」の裏表として一緒に読む。
⚠️ **もう疑わなくてよい**: 題の値付け・括弧（台帳 8 点 exact）・段ごとの上下インク（⑾⒠ で ≤0.31）。⚠️ **インク比較は `scratch/p338/lp-bare.ps1`**（素の LP）。⚠️ **`SystemLayout.Y` は頁の下端からの Y-up**・**段の高さを LP と並べるときは `ToFirst` を足す**。
⑵ 以下は第337 の並び（`lysc ly` の ChordNames＝ユーザーと綴りを決めてから／lyric row の slot の投票／ps2 の 0.017・beam-over-stem bar 1 の +0.33／push 後 `gh run list`／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／A/B/D/E／C⑴／G）。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**

## 以下は第337セッションの経緯

最終更新 第337セッション＝**入り方は第298〜第336 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。第336 の次の一手 **⑷′「掃きの LP 基線の鮮度」と ⑸「掃きに頁を足す」**に入り、**両方閉じた**。製品は `lysc layout` の頁行 1 本（描画は不変・snapshot 0 枚）。**骨は 6 つ**:

★★ **⑴ 鮮度は無罪**: 全 286 冊の `scratch/p332/out/*/lp.out` は 09-05 01:23（第332 掃き）で、**`.ly` はそれ以後 0 冊未編集**（mtime 全数比較）。「LP 基線が古い」は外れ。
⚠️⚠️ ★★★★ **⑵ 正体＝計器のインク**: 同じ `Freedom.ly` を include だけ変えて 3 通り（`scratch/p337/freedom-ab.ps1` → `ab/`・各 8 秒）: **`probe-settings.ly` そのまま＝staff book 37 段／`BarNumber.stencil = ##f`＝39／include 無し＝39**（段数ループの足跡「N 個のシステムを試します」が b ≡ c で同一・bassTab book は 3 通りとも 37）。**all-visible の BarNumber は*描かれると*段の上のインクになり、LP の頁の得点（tallness）を動かす**——**第321〜336 の LP 基線は「番号を描いた LP」の段署名だった**。直し＝`scratch/p321/probe-settings.ly` に `\override BarNumber.stencil = ##f`（`after-line-breaking` は `System::post_processing` が all-elements を舐めるので鳴る＝368 行）。⇒ ★★ **LP に何かを*刷らせる*計器は、刷る物のインクが LP の答えを変えないことを先に確かめる**（§0 の「A/B の前提」の族・memory `reference_lilysharp_t7_probe_barnumber_ink_perturbs_lp_paging`）。
★★★ **⑶ `lysc layout` に頁行**: `  pages: 3  |  systems per page: 8, 7, 3`（`LayoutReport`・`ScoreLayout.Pages` を読むだけ・**stale な「頁の概念は無い」コメントを消した**）・番人 `LayoutReportTests.Pages_AreReportedWithTheirSystemCounts`／`SinglePage_ReportsOnePageHoldingEverySystem`・`docs/CLI_REFERENCE.md` の例・CHANGELOG（New）。**`parse321.ps1` は layout.txt の頁行を `LysPages`／`LysPageSig` に読む**（sweep332 以降 PNG 数えで全冊 0 だった）＋ PAGES 行（KindMatched の対だけ）。
★★★ **⑷ 全冊取り直し**（`scratch/p337/sweep337.ps1`＝sweep332 の型・23.4 分・`structure337.csv`・LP exit≠0 40＝前と同数・PROBEBN 0 は Tell Her About It 1 冊＝前と同じ）: **LP 側の段署名が動いたのは 8 対 7 冊**（Freedom staff 37→39・Holiday tab 24→27・アゲハ蝶 staff 24→26／tab 25→24・Don't You Worry tab 26→24・everybody goes tab 45→51・That's The Way tab 24→21・Together Forever tab 22→27）。**T7: KindMatched 209 / SigMatch 149 / 段違い 41 / 小節数違い 19＝不動**（アゲハ蝶の staff を失い tab を得た）。全対 SigMatch 447→446・SystemsMatch 485→484・全対一致の本 210→208。**Lily# 側は 0 対不動**（`scratch/p337/compare337.ps1` が数えた）。
⚠️ ★★★★ **⑸ 頁の初数（KindMatched 209 対）**: **PagesMatch 194／段署名と頁の両方一致 144／段署名一致で頁数違い 5／Lily# が多い 11（うち最終頁 1 段 6）／少ない 4**。**純粋な頁の差 5 冊**: **Sugar** tab（LP 2 頁・Lily# `11,12,1`）・**The Heat Is On** both（LP 3・Lily# `7,8,8,1`）・**Boogie Oogie Oogie** both（LP 4・Lily# `8,8,8`）・**Can't Take My Eyes Off You** ×2 both（LP 5・Lily# `8,8,8,8`）。9 to 5 (Xanadu) tab（26 段一致・LP 2 頁・Lily# `12,13,1`）も同族。**5 冊とも題＋作曲者あり・chords/lyrics 無し**。**形は 2 つ**: **Sugar／9 to 5／The Heat Is On は*題付き 1 頁目*だけが後の頁より 1 段少なく、末尾に 1 段の頁が立つ**（LP は題の頁にも後の頁と同数を載せる＝Lily# の 1 頁目は題込みで LP より 1 段ぶん狭い）／**Boogie・Can't Take は逆に Lily# が 8 段詰め、LP は 6〜7**。
★ **⑹ 数**: 台帳 785／exact 617／ss 非ゼロ 203（総和 26.789）／count 164 うち非ゼロ 0／OPEN 0＝**不動**。full **7147 / 0 / 4 / 7151（`scratch/p337/run1.trx`・2 分 04 秒・+2＝番人）**。snapshot 247 枚・追跡コーパス 597 冊。

★ **開始時裏取り**: HEAD `aff056db`・**未 push 69**・木 clean・未追跡 0・`bin\Debug` は net10.0 のみ・`xunit.runner.json` 並列・`--no-incremental` build 0 エラー・Core 0 警告（93 秒）。
**終了時（第 1 便）**: **commit 2 本（`lysc layout` の頁行＝Core 1＋番人＋CLI_REFERENCE＋CHANGELOG／この HANDOFF＋ARCHIVE＝第335 の経緯を先頭へ逐語）・未 push 71・木 clean・未追跡 0**・台帳／snapshot／`.lys`（追跡）不動。
✅ ★★★★ **⑺ 第 2 便（ユーザー「次便がこの便で有利なら着手」→ 計器が温かい Sugar を測った・製品 0）＝Sugar の頁の差は*tab の段の高さ*で、正体は full notation の tab 符尾の幾何**: **⒜ 五線の対は LP と一致**（両者 13,11 頁・素の 4 小節段の pitch **LP 11.132 対 Lily# 11.131**・`scratch/p337/sugar2/`＝`pages-ext.ly` が段ごとの yoff とインク extent を刷る／`scratch/p337/probe1.txt`＝Lily# の placed sys＋頁の Y）。**⒝ tab の対だけ違う**: LP は題の頁にも 12 段（12,12）・Lily# 11,12,1。**素の 4 小節 tab 段のインク高: LP 9.491（refpoint 上 4.001＝上弦の上 1.001／下 5.490＝下弦の下 2.490）対 Lily# 6＋2.635＋3.430＝12.065**。LP の段間 11.615 は floor（9.491＋1）に届かず ideal 側、**Lily# の 13.065 は tallness そのもの＝floor が縛る**。**差 1.45/段 × 11 段 ≈ 16 ss ＝ 段 1 本ぶん**（頁の力・題の帯・foot は無罪＝第336 の TTL/STBN 対が exact なのと整合）。**⒞ LP の tab 符尾の仕組み（`scratch/p337/stem-probe.ly` → `sugar3/`・Stem 488 本）**: **始点＝数字の*縁***（`define-grobs.scm:3741 TabNoteHead stem-attachment = ly:note-head::calc-tab-stem-attachment`・`stem-begin-position` が弦位置 ±1.8 半間＝±1.35 ss＝font-size 2 の数字の半高・`stem.cc:935-963 internal_calc_stem_begin_position` の `y_attach`）で、**符尾は whiteout の数字（`:3743`）の裏を通る**／**向き＝五線と同じ規則を弦位置に当てる**（`stem.cc:793-809 calc_default_direction`＝中央 0 からの距離・**中央弦は neutral-direction DOWN**・梁は `beam.cc:880-940 get_default_dir` の extremes/count）: E 弦（−2）の単独 8 分は UP 44 本・A 弦（0）は DOWN 72・B 弦（−4）UP 8・梁群は極値で決まる／**長さ＝通常の `calc_length`（3.5 tab-ss＝5.25 ss を頭の中心から）**: E 弦 UP の先端 3.75（上弦の上 0.75）・梁 DOWN の先端 −5.467（下弦の下 2.47）・可視長 3.9＝5.25−1.35。**⒟ Lily# 側の模型は別物**（`TabStaffGeometry`）: `StringStemUp`（弦の半分で向き・梁群は平均弦）・`StemClearance`（数字と隣弦の窓の中央）＋`UnbeamedStemLength` 3.0×ss＝弦線から 5.55・梁 2.4×ss・はみ出しクランプ 1.6×ss。**LILYSHARP-OWN 札が「LP は tab に符尾を描かないから移植する量が無い」と書いているが、`\tabFullNotation` の LP は上の 3 点で描く＝札は full notation で倒れている**（`TabStaffGeometry.cs:200`・`:235-249` は自分で「3.0 は導出・StemCalculator を tab の枠で回すのが字面」と書いている）。⚠️ **Lily# の上 2.635／下 3.430 が*どの符尾*かは段の extent でしか測っていない**（A 弦の UP 単独符尾なら 0＋1.17＋4.5＝5.67 → 上弦の上 2.67 が候補・下は E 弦の梁 DOWN 3.15＋梁厚）。**次便は Lily# 側の計器を符尾 1 本ずつに落とす**（renderer の `DrawUnbeamedTabStem`／`TabBeamQuant` の tip を吐く）→ LP の 3 点（始点・向き・長さ）を `StemCalculator` の枠で移植（出力が動く＝全 full-notation tab 本の snapshot・**承認事項**）。**射程**: 実コーパスの tab 本は全部 `\tabFullNotation`（Sugar.ly:168 と同型）＝tab 対 280 の段の高さが全部動く。**製品 0・捨て計器は `scratch/p337/ZzP337ProbeTests.cs.txt`。** ⚠️ ★★ **ユーザー指摘（第 2 便の途中）＝「A セクションマークの位置が違う」**: **`.ly` は `\mark \markup { \box A }` を bar 5（percent 反復の前・Sugar.ly:54）に置き、`.lys` は `section A` が bar 13 から**（bars 5-12 は匿名 `~Body_1` の中＝Sugar.lys:12-16）。**LP の 2 段目（up 6.594＝A の箱）と Lily# の 3 段目（top 3.800）にインクが移るだけで、頁の結論（tab 段の高さ 1.45/段）は動かない**が、**段ごとの対を取るときはこの 2 段を除く**。**源は T6 ⒞（marks・境界近傍＝percent 反復の前の札）で `.lys` 側＝ユーザーの直し**（`~Body_1` を bars 1-4 に縮め、percent を section A に入れる）。
**終了時（第 2 便）**: **commit 1 本（この HANDOFF）・未 push 72・木 clean・未追跡 0**・製品不変。
✅ ★★★★ **⑻ 第 3 便（ユーザー「移植して」）＝tab 符尾の幾何を LP の 3 点で移植して commit した**（`f4ba2b40`・出力を変える承認済み変更）: **⒜ 先端＝通常の stem end を tab の枠で**（`StemCalculator.CalculateStemEndPosition` 新設＝`CalculateStemEndY` から分離・`details.lengths`／不自然方向短縮／中線への引き・staff radius を引数化＝tab は (弦-1)/2＝2.5）／**⒝ 始点＝数字の far edge**（`TabConstants.StemBeginOffset` ＝ 1.35 × 数字半高・`note-head.cc:224-234 calc_tab_stem_attachment`・whiteout の裏＝可視長は短いが先端は動かない・`StemClearance`／`UnbeamedStemLength` は退役）／**⒞ 向き＝弦で読む LP の規則**（`TabStaffGeometry.TabStemUp`＝`stem.cc:793-809 calc_default_direction`・和音は極値で中央弦は neutral DOWN・`\stemUp/Down` が勝つ／`GroupStemUp`＝`beam.cc:876-940 get_default_dir` の極値→票→到達→neutral・旧平均弦を置換／`BeamScoringProblem` は tab 梁の forced 短縮の免除をやめた＝TabVoice の stem は default-direction を持つ）。**読み手 6 ファイル**（SkylineBuilder・ArticulationEngraver 4 腕・ElementCoordinator の tab タイ/スラー・SharedRenderer の unbeamed/beamed）を弦 index の API に。**結果**: Sugar の tab 頁 11,12,1 → **12,12＝LP**・五線 13,11＝LP。**新台帳点 `stem.tab-full.top-string.eighth.tip-above-middle` = −1.0 tab-space（residual 0）**＝初めて tab 符尾を観測する点（probe `audit/lp-geometry/probes/tab-stem.ly`）。**代価: snapshot 18 枚**（full-notation tab 全部・非 tab は 0 枚）を再ベース（この台帳キーが根拠・RULES §5.2.1③）。番人 `TabScriptStemClearanceTests.TheUnbeamedTabStemTip_IsTheGeometrysOwnNumber` を新 API に。目視: `test/tab-articulations`（低弦の UP 符尾が揃い fermata が上を抜ける）・Sugar（tab 12,12＝LP）。**full 7148 / 0 / 4 / 7152（`scratch/p337/full1.trx`・1 分 58 秒・+1＝台帳点）**・`--no-incremental` build 0 エラー・Core 0 警告（47 秒）・台帳 785 → 786・exact 617 → 618・ss 非ゼロ 203 不動・count 164 不動・OPEN 0。⚠️ **概念の異物**: 移植中に renderer 2 ファイル（`SharedRenderer.Tab.cs`/`.Beams.cs`）が私の編集記録に無いのに新 API へ移っており、`tab-stems.ly`（275B・未参照）も現れた＝**別プロセスが同じ repo を触った疑い**。両方とも私の API と整合し全緑・orphan は削除した。次便は開始時に `git log` で HEAD を裏取りすること。
**終了時（第 3 便）**: **commit 2 本（移植＝Core 8＋番人 2＋snapshot 18＋台帳 1 点＋probe＋CHANGELOG＋再生成 docs／この HANDOFF）・未 push 74・木 clean・未追跡 0**。
✅ ★★★ **⑼ 第 4 便（ユーザー「有利なら着手」→ 計器が温かいので掃きを取り直して移植の効果を数えた・製品 0）**: Lily# だけ新 bin で取り直し（`scratch/p337/relayout332.ps1 -Csv scratch/p337/structure-after337port.csv`・415 秒・LP 側は sweep337 の stencil=##f のまま）。**T7 は 209 / 149 / 41 / 19 不動**（段署名は動かない）。**頁**: PagesMatch 194 → **194（据え置き）**——**Sugar は tab 11,12,1 → 12,12・五線 13,11 で両対とも LP と一致（+2 match）**が、同数のまま。理由＝**`Are You Gonna Go My Way` tab が閾値を跨いだ**（移植前 12,13,1＝LP 3 頁で一致 → 移植後 13,13＝2 頁で不一致・−1）。**符尾が短く正しくなり 1 段多く 1 頁目に載った**。⚠️ **これは符尾のバグではない**: LP の Are You Gonna は `\bendAfter`・`\staccato`・`e'`/`g'` の高い音で段が高く（LP tab の up-ink 4.5〜6.1・Sugar の一様 4.0 に対し）、**Lily# はその上側インクを LP より少なく見積もる＝「Lily# が多く詰める」島（Boogie／Heat Is On／Can't Take と同族）が符尾修正で 1 冊に触れただけ**（`scratch/p337/ayg/` で LP の段ごと ink を測った）。**残る純頁差 5 冊**: Are You Gonna（tab・bend）／Boogie Oogie Oogie（both 8,8,8 対 LP 4 頁）／The Heat Is On（both 7,8,8,1 対 3）／Can't Take My Eyes Off You ×2（both 8,8,8,8 対 5）。
**終了時（第 4 便）**: **commit 1 本（この HANDOFF）・未 push 75・木 clean・未追跡 0**・製品不変。
▶ ★★★ **⑽ 第 5 便（ユーザー「承認する・有利なら着手」→ 島を局在まで測ったが*単一の clean な項ではない*ので移植せず診断を残した・製品 0）**: Are You Gonna tab の placed sys（`scratch/p337/ZzP337bProbeTests.cs.txt` → `probe-b.txt`）と LP book2（`scratch/p337/ayg/`）を並べた。**Lily# の top extent は 2.3〜5.3（多くは 3.8／2.3）・LP の up(ref→hi) は 4.5〜6.1**＝**上側 ~2.2 ss の見積り不足**。**局在**: tab の頁 skyline `SkylineBuilder.AddTabStemsAndBeamsToSkylines` は*符尾と梁だけ*を播く。**script は別経路で入っている**（`AddArticulationLayoutsToSkyline` が tab でも呼ばれる）が、**bend（`@fall`／`\bendAfter`）は SkylineBuilder のどこにも無い**（Are You Gonna に `@fall` ×3・skyline に bend/glissando の seed 0）。⚠️ **ただし LP の up-ink 6.057 は*一段おき*に出て 3 本の bend では説明しきれない**＝**寄与が複数**（bend＋未特定の反復項）。⇒ ★★ **単一 anchor の stem とは違う多 grob の skyline 完全性の島**＝grob ごとに台帳点を起こす必要があり、新セッションで clean に。**この便は移植しなかった**（多項・content 依存で検証しきれない出力変更を承認だけで走らせない）。
**終了時（第 5 便）**: **commit 1 本（この HANDOFF）・未 push 76・木 clean・未追跡 0**・製品不変。
⇒ ★★★★ **次の一手**: **⑴ 「Lily# が多く詰める」島＝tab の頁 skyline の above-tab インク完全性**（⑽ で局在済み）。まず **bend を tab skyline に播く**（`SkylineBuilder` に bend/`@fall` の seed が無い＝最小の 1 grob・台帳点 `system.tab-full.bend.up-ink` の型で LP と対）→ 残る反復項（LP の一段おき 6.057・未特定）を `ZzP337bProbeTests.cs.txt` の型で grob 別に測る。**出力を変える＝承認事項**。計器: `scratch/p337/ayg/`（`sugar-lp2.ps1 -Ly … -Out …`）・`probe-b.txt`・`lysc layout` の頁行。⚠️ **Boogie は score "" が staff 単独を拾う＝probe の scoreStem に "both" の出力名を渡す**（第 5 便の Boogie 行は staff で無効）。**⑵ 以下は第336 の並び**（`lysc ly` の ChordNames＝ユーザーと綴りを決めてから／lyric row の slot の投票／ps2 の 0.017・beam-over-stem bar 1 の +0.33／push 後 `gh run list`／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／A/B/D/E／C⑴／G）。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
