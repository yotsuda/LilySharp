...
- ★ **⒳¹³ フレージング・スラーの残り＝⑷⑸ だけ**（⑴ `.up`/`.down`・⑵ combinedStaff・⑶ MusicXML の読み込みは第483 が閉じた）: ⑷ prelim の系ごとの束ね（`LayoutPreliminaryStaffSlurs`）では、別の系に始まる内側スラーが見えない＝間隔の近似だけ（最終段は全部見る）／⑸ タブ譜の経路には内側スラーを渡していない
- ★★ **⒳¹⁴ 下向きのスラー（符尾も下向き・梁なし）が LP より平たい＝制御点の高さが 0.118 足りない**（第483 起票・実測・Lab `sessions/p483/v2.lys`・`dir.lys`）。**2 声目の普通のスラー** `c''4( d'' e'' f'')` で LP 1.358 に対し Lily# 1.240、`@phrasingSlur.down` でも LP 1.509 に対し 1.390＝**同じ量**なので、スラー基盤の差（フレージング固有ではない）。幅と端は一致。上向き（第482 の 3 本）は一致している＝向きが符尾と同じ側の経路を疑う（付け根か stem の encompass）。台帳の `system.voice2-slur-under-notes` と同じ家かを先に確かめる
