...
# ⚠️⚠️ ★★★ その「0 警告」は **Debug の話**——**§2G に第135 から起票済みの棚が在る**
#    （`GenerateDocumentationFile` が Release だけ true。**NuGet pack は Release** なので
#    0.3.0 を出す経路はそこを通る）。**2026-08-17・第197 で再測＝476 → 283 件**
#    （CS1573 179・CS0419 27・CS1574 26・CS1570 18・CS1734 11・CS1591 7・CS1587 6・CS1572 5・CS1571 4）。
#    ⚠️ ★★★ **第197 はこれを「新発見」として §1 に書きかけた**——**§2G を読む前に数えたため。**
#    **「見つけた」と思ったら、まず §2 を grep すること**（この棚は 62 便前から在る）。
...
最終更新 第198セッション＝**残債（▶ perf）を中断し、ユーザーが自分の本で見つけた
リリースブロッカー「タブに拍子記号が描画されない」を直した便**（**実装 1 便＋台帳 2 点**。
**perf は 1 バイトも触っていない**——**0.3.0 の門の内側**）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **ユーザーが `scratch\ベースタブLy\tabtime.lys` を出して
「タブに拍子記号が描画されない。余白だけ予約されている」と述べた**（コメントは本人が本に書いていた）。
**ユーザー決定 1 件＝snapshot 24 枚の再ベース承認**（診断・射程・台帳点を見せた上で取った）。
★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 516 点・ss 非ゼロ 98・
総和 3.609965521・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 49**・
**snapshot 217 枚**・suite **5350 passed / 0 failed / 4 skipped**
＝**引継ぎの数は全部合っていた**（**28 回連続**）。
★★ **未 push は 5 便続けて両端が繋がった**——**第197 の終了時 49 と開始時 49 が一致**（間で push は無い）。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 52**（★ **第197 が手順に落とした「引継ぎ commit を
最後にする」をそのまま実行した**——**この行を書く commit を含めて 52**。
⚠️ **一度 53 と書いて外している**：**commit 前の実測 51 に +1 する計算を
PowerShell で `$ahead + 1` と書いたら `$ahead` が文字列で `511` が返り、
そこで手計算に戻って気づいた。**⇒ ★ **数を +1 するときは `[int]` を通すこと**）・
suite **5361 passed / 0 failed / 4 skipped**（+11＝新しい網ちょうど）・
**台帳 516 → 518 点**（★ **足した 2 点は両方 exact 着地なので
ss 非ゼロ 98・総和 3.609965521・count 106/2 は全部不動**）・
**snapshot 217 枚で不動＝再ベース 24 枚**（**新規 0・削除 0**）・**Core は 0 警告**（⚠️ **Debug の話**）。
★★ **環境＝pwsh の MCP コンソールは 2 本要った**（`Paprika` が LP の deadlock で固まり `Turmeric` を起こした。下の ⑸）。

> ## ★★★ この便の骨＝**「タブは拍子を描かない」は LP の*片方のモード*の話で、Lily# が使っているのはもう片方だった**
> LilyPond の `TabStaff` は `Time_signature_engraver` を**残したまま stencil を潰す**だけ
> （`ly/engraver-init.ly:1219-1220`）。**`\tabFullNotation` はそれを*最初の行で* revert する**
> （`ly/property-init.ly:825-826`）。そして **Lily# の既定 `tab` はその full notation**——
> **符尾・連桁・旗・付点・休符を描き、`LilyPondExporter` は双子に `\tabFullNotation` を書いている。**
> **素の TabStaff なのは `tab … as numbers` だけ。**
> ⇒ ★★★ **Lily# は「拍子を描くか」を*譜の種別*で答えていた。問いは*モード*だった。**
> **3 綴りが 3 つとも同じ間違いをしていた**（下の ⑵）。

> ## ★★★ 2 つ目の骨＝**予約は既に正しく、欠けていたのはグリフだけだった。だから「余白だけ予約」は正確な報告だった**
> **中途の `time 2/4` に Lily# が空けていた穴は 3.46**、**LP 2.26.0 の実測は 3.4547**
> （full と bare の 2 モードでフレット間隔を引き算した数）。**ほぼ一致していて、glyph だけが無い。**
> ⇒ ★★ **ユーザーの一文「余白だけ予約されている」が診断そのものだった**——
> **こちらがやったのは、その観察が*どの規則の*欠落かを LP に訊いたことだけ。**
> ⚠️ **行頭側は逆で、予約すら無かった**（`time 12/16` に替えても最初のフレットが動かない＝射程 0 の観測）。
> **同じ量の 2 site が Lily# 内部で既に食い違っていた**（§7.7 の匂い）。

> ## ★★★ 3 つ目の骨＝**予測を dump の*前に* probe へ書くと、台帳点が「当たった」と言える形になる**
> 新 probe **TKF**（TKC と同じ系にタブ側だけ `\tabFullNotation`）に、**測る前にこう書いた**:
> 「stencil が戻ればタブの extremal な prefatory grob は TimeSignature になり、五線と同じ
> `semi-shrink-space 2.0` を引く。2 つの wish が一致して `merge_springs` は平均するものが
> 無くなるので、**TKC の 3.300000 を離れて 3.700000 になるはず**」。
> **実測 3.700000。TABTIME の ext は TKC の `(+inf . -inf)` に対し `(0.0 . 1.7)`。**
> **そして Lily# 側も 3.700000000 / residual 0——間隔のコードは 1 行も書いていない。**
> ⇒ ★★★ **述語を直しただけで既存の `merge_springs` 移植が LP の値を自分で出した。**
> **これは「移植が正しかった」ことの独立な裏取りで、予測を先に書いたからそう読める。**

> ## ★★ 4 つ目の骨＝**射程を数える計器を 3 回作り直した。0 は 3 回とも出たが、正しい理由で出たのは 3 回目だけ**
> 残した別件（下の ⑹）の射程を数えるのに、**⑴ `section { … }` の中だけ見る述語**は
> **`test/timesig-change` を False と答えた**——**あの本は変更を `phrase` に置いている**。
> **⑵ 位置比較に広げたら 2 冊拾ったが、両方*コメントの中の "section"* に釣られた偽陽性**。
> **⑶ コメントを剥いでから比べて 0 冊で確定**（毒 4 冊で True・対照 3 冊で False・
> **コーパス全体では 18 冊が中途 time を持つ**＝計器が全体には届いていることの確認）。
> ⇒ ★★★ **§0 の 5 例目「正しい数を誤った理由で出す計器」の当日版。**
> **0 を引き継ぐときは、*0 でない母集団*も一緒に書く**——それが無いと次便は区別できない。

★ **この便の値段**（**perf は測っていない＝計算を足していないため。§7.9 の「足していない例」**）:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ タブの拍子（述語の統一＋描画 3 site） | Core 6 ファイル ＋ 網 8 本 ＋ 台帳 2 点 | **snapshot 24 枚・非タブは 0 枚** |

- **⑴ ★★★ 唯一の実装便＝1 つの規則が 3 軒で*譜の種別*を訊いていた**
  （`e345fb1e`・**Core +193 行（うちコード 57）・削除 −40**）。**述語を 1 軒にした**
  ——`SpacingRules.ContributesToTimeColumnWidth` ＝ **`!IsTextRow && !(IsTab && TabNumbersOnly)`**:
  **⒜ `MultiStaffScore.AllStavesTab` の門（3 呼び出し）**＝**鍵の門で拍子を判定していた**
  →**鍵専用に降格**（`\tabFullNotation` は `\remove Key_engraver` を戻さないので**鍵は両モードとも出ない**＝
  **ここが非対称で、それを取り違えたのが元の欠陥**）／
  **⒝ `LineStartColumn.PrefatoryGrobs` が鍵の述語 `ContributesToKeyColumnWidth` を拍子に流用**
  ——**full notation のタブは prefix を TAB クレフで終え `minimum-fixed-space 5.0` を引いていた**
  （LP は拍子の `semi-shrink-space 2.0`）／
  **⒞ `DrawTabStaff` に描画が無い**→**行頭・中途・行末 courtesy の 3 site を実装**。
  ⚠️ **中途は五線と同じ `EnumerateStaffItems` から読んだ**——**行頭に出る複製を飛ばす規則が 1 軒で済み、
  二重描画が構造的に起きない**（網 `MeterChangeAtALineBreak_…_AndNothingElse` が「ちょうど 2 個」を固定）。
- **⑵ ★★ 垂直の規則も LP 実測**（2.26.0・同じ音楽を五線とタブで比較）:
  **数字の対は両方とも 2.000000 離れる**——**タブの staff-space は 1.5 なのに同値＝
  stencil は弦間隔でスケールしない**／**分子のベースラインは譜の中心・4/4 の C グリフも中心**。
  **これは `DrawTimeSignature` 自身のオフセットを「中央線＝タブの中心」と読んだもの**なので、
  **合成 staffY を渡して 1 軒に保った**（**裸の 2.0 は `StaffMiddleLineDrop = StaffHeight / 2` と命名**＝§5.2.1⑤）。
- **⑶ ★★★ 台帳 2 点を新設**（`line-start.time-to-first-note.tab-full` **3.700000**・
  対照 `line-start.clef-to-time.tab-full` **4.320000**、**両方 residual 0**）。**probe TKF を
  `barline-spacing.ly` に足した**。**対照の役目は「欄は動かない・動くのはばねだけ」**を言うこと。
  ⚠️⚠️ ★★★ **TKC/TKT の Lily# 側 fixture を `tab gt as numbers` に直した**——
  **LP 側の twin は素の `\new TabStaff` なのに Lily# 側は既定 `tab`＝ full notation で、
  両側が別の LP 設定を写していた。**一致していたのは **Lily# がどちらのモードでも拍子を消していたから**で、
  **欠陥が 2 つあって打ち消し合っていた形**（§5.2.1 の型）。
- **⑷ ★★ 網 8 本・毒に当てて 6 本が赤**（`git stash` で Core を戻して実測）。
  **緑のまま残る 2 本は「`as numbers` に拍子が*出ない*こと」を主張するもの＝元から正しかった側**で、
  **形として正しい**（毒で全部赤くなる網は、対照を持っていないということ）。
  ★ `TabOnlyKeyPrefixTests.AllTabPrefix_IsIndependentOfTheMeter` は**前提が偽になった**ので
  **対に割った**（full は拍子で広がる／`as numbers` は広がらない）。
- **⑸ ⚠️⚠️ ★★★ LP の呼び方で 10 分溶かした。答えはリポジトリの中に書いてあった。**
  **`C:\bin\lilypond-2.26.0\bin\lilypond.exe` は継承コンソールから起動すると Guile が deadlock する**
  （CPU 0.8 秒のまま 10 分生存）。**`Measure-LilyPondProbe.ps1` の .NOTES がそう書いている**——
  **`cmd /c "… < NUL"` で detach し、stdout と stderr を*別ファイル*に**。
  ⇒ ★★★ **「LP を直接叩く」前に `audit/lp-geometry/*.ps1` の .NOTES を読むこと。**
  ⚠️ **MCP コンソールは固まったまま復帰しないので、`reason` を付けて 2 本目を起こす**（`Turmeric`）。
  ⚠️ ★★ **WSL の `/home/ytsud/lilypond/build/out/bin/lilypond` は 2.27.3（master ビルド）で 2.26.0 ではない。**
  **最初その 2.27.3 で全部測ってしまい、基準（`C:\bin\lilypond-2.26.0`）で取り直した**——
  **値は全部一致したが、一致することは事前には分からない。**（`VERSION_STABLE=2.26.0` は
  メタデータで、ビルドの版ではない。）
- **⑹ ★★★ 見つけたが直していない 1 件＝§2F ⒣ に起票**（**`as numbers` と譜なしの本が
  *中途の*拍子欄を予約する**）。**射程 0 冊**（数え方は §2F ⒣。**この 0 は 3 度目の計器の数**）。
  ⇒ ★ **着手しなかった判断はこちらが下した**（ユーザーが「有利なら着手／不利なら着手するな」と委任）。
  **不利と判断した根拠＝⒜ 修理は `MeasureLayouter` → `BarlineToFirstColumnSpring` /
  `MidMeasureChangeGaps` / `BoundaryChangePrefix` の署名を触る＝台帳 518 点と snapshot 217 枚が
  乗っている関数群／⒝ §5.2.1③ により先に LP を測って台帳点を作る順序で、それだけで 1 便／
  ⒞ このセッションが持っていて次便が失うものは無い**（**TKF も台帳の `why` も commit 済み**）。
  **第197 ⑽ で「有利」と言えたのは診断が `scratch/` にしか無かったからで、今回はそれが成り立たない。**
- **⑺ ★★ §2 の頭（perf）は 1 バイトも触っていない。**
  **地図と次の島は第193 の §1 ⑷ のまま**（ARCHIVE ではなく §2 の頭に要約が在る）。
- **⑻ ★★ 次に触るならここ**——**候補は §2F ⒜（上の ⑹・射程 0 冊）／
  §2G の Release ビルド Core 283 件の XML doc 警告（第135 起票・NuGet pack が通る経路）／
  §2 の頭の ▶ perf**。
  ⇒ ★★ **0.3.0 の門という観点では §2G が一番近い**（**出荷する成果物のビルドが出す警告**）。

---

## 以下は第197セッションの経緯

最終更新 第197セッション＝**第196 が「決定済み・未実装」で残した ⒜⒝⒞ を 3 件とも実装し、
残った ⒡⒢ もユーザーの承認と判断委任のもとで閉じた便**
（**実装 8 便**。**perf は 1 バイトも触っていない**——**8 件とも correctness で、
8 件とも 0.3.0 の門の内側**）。ユーザーの目的は **0.3.0 を出すこと**で、
優先は「**文法を完全にしてから出す**」。
★★★ **§2F に開いている実装項目は 0 になった**（**残るのは `font "NAME"` の 1 件だけで、
射程 0 冊・対を 1 冊起こすところから**）。
...
★★ **未 push は 4 便続けて両端が繋がった**——**第196 の終了時 35 と開始時 35 が一致**（間で push は無い）。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 49**（★★★ **この数は 4 度書き直した。40 → 42 → 44 → 49。**
**第196 の手順「commit を作る*前*に数えて +1 して書く」は毎回当たり、毎回そのあとに便が続いた。**
⇒ ★★★ **抜けているのは「+1」ではなく*順序*だった**——**引継ぎ commit を最後にするか、
さもなくば「あとで便が続いた回数だけずれる」。この行を書く commit を含めて 49。**
⚠️ **次便への申し送り**: **§1 の数を書くのは、その便で最後に打つ commit の直前。**）・
suite **5350 passed / 0 failed / 4 skipped**（+25＝新しい網ちょうど。**⒡ は網を足していない＝本の修理**）・
...
> ## ★★★ 4 つ目の骨＝**天井は欠陥を隠す。127 の列は何とも食い違えない**
> ⒡ の 2 冊を直す（`'` を落とす）まで、`section-meter-resets-to-global` の
> **pitchDiffers 9 件は全部「page D10 対 midi 127」**だった。**まともな音高にした瞬間に、
> MIDI が section B を 2 オクターブ上で開いているという*本物の*欠陥が出た**（＝⒢ の 2 つ目の症状）。
> ⇒ ★★★ **飽和した計器は「一致」とは言わないが、「食い違い」の*中身*も言わない。**
> **clamp・floor・ceiling の在る量を突き合わせるときは、飽和した点を*別に数える*こと。**
> ⇒ ★★ **そして露出させたのは engine の修理ではなく*本*の修理だった**
> ——**fixture を直すのは「掃除」ではなく、観測の解像度を上げる操作。**

> ## ★★★ 5 つ目の骨＝**「snapshot が動く唯一の項」は動かなかった。射程を数えるのは着手の*前*でよい**
...
| ⑸ ⒡ 2 冊の fixture（**承認後**） | 本のみ（Core 0 行） | **5 → 3 冊**（**天井が隠していた欠陥が出た**） |
| ⑹ ⒢ 素の section（MIDI） | MIDI ＋ 計器 | **3 → 3**（**ページ側が揃い、XML が露出**） |
| ⑺ ⒢ 素の section（MusicXML） | MusicXML | **3 → 1 冊**（**残りは MIDI の床**） |
| ⑻ ⒢ 素の section（双子） | 双子 | **双子が動いた本 2 / 20**（LP 2.26.0 で裏取り） |
...
- **⑸ ★ §7.5 の監査**＝**8 便合計で Core +448 行（うち*コード 163*）・削除 −83・REF +0/−0・OWN +0/−0**
  （**`git --no-pager diff --no-color 7df270f8 HEAD -- LilySharp.Core`**＝**セッションの頭から**）。
  **彫版の定数は 0 個**（追加コード行の数値リテラルは**ループ添字と ottava の ±12/±24 だけ**）。
  **分類は 8 便とも ⒟**——**どれも「既存の家を指し直した」**で、**規則の出所は
  MusicXML の規約・LP 2.26.0 の実測・コーパス自身。**
  **§7.7 の匂い**——**この便で 3 度「同じ量の 2 軒目」に当たった**:
  **⒜ 既定 form の読みが 3 軒（`ScoreForms` へ）／⒝ part を arm する枝が 2 軒（`PlayInPart` へ）／
  ⒞ 素の section の帰属が 4 軒（collector・MIDI・MusicXML・双子）。**
  ⚠️ **4 軒目はいつも双子だった**（下の ⑾）。
...
- **⑺ ⚠️⚠️ ★★★ §0 の「Core は 0 警告」が *Debug の* 話だと*見つけた*と思い、§1 に新発見として
  書きかけて外した**——**§2G に第135 から同じ棚が起票されている**（**62 便前**）。
  **やったのは発見ではなく*再測*で、値打ちはそこだけ**: **476 件（2026-08-11）→ 283 件（この便）**
  （CS1573 179・CS0419 27・CS1574 26・CS1570 18・CS1734 11・CS1591 7・CS1587 6・CS1572 5・CS1571 4）。
  **この便では 1 件も直していない**（**私の追加行が出したものは 0 件**——実測で確認済み）。
  ⇒ ★★★ **教訓は数のほうではない**: **「見つけた」と思ったら §2 を grep する。**
  **この文書は 2000 行あり、§1 しか読まないと 62 便前の棚を新発見として書くことになる。**
- **⑻ ★★ §2 の頭（perf）は 1 バイトも触っていない。**
  **地図と次の島は第193 の §1 ⑷ のまま**（ARCHIVE ではなく §2 の頭に要約が在る）。
- **⑼ ★★★ セッションの終わりにユーザーが ⒡ を承認したので直した**（`52c04380`。**動いた snapshot は
  1 枚で、引継ぎが予言した 2 枚ではない**——`fermata-b-obs-probe` は baseline を持たない）。
  ⇒ ★★★ **直したら、MIDI の天井が隠していた本物の欠陥が出た**——**127 の列は何とも食い違えない**ので、
  **9 件の pitchDiffers は全部「page D10 対 midi 127」だった。まともな音高にした瞬間に
  「MIDI が section B を 2 オクターブ上で開く」が見えた**（＝**⒢ の 2 つ目の症状**）。
  ⇒ ★★ **§1 の骨「片側を規則どおりにすると、もう片側が露出する」の*本*版**——
  **露出させたのは engine の修理ではなく fixture の修理だった。**
- **⑽ ★★★ ⒢ もこの便で閉じた**（`03d3d414` ＋ `08283389` ＋ `409cf3bb`。詳細は §2F ⒢）。
  ★★★ **起票時に「要ユーザー決定」と書いた曖昧さは、数えたら射程 0 冊だった**
  ——**素の section を score が複数の part に割り当てる本は 566 冊に 1 冊も無い**
  （**素の section の本は 23 冊、うち part が既定でないものが 19 冊**）。
  ⇒ ★★★ **「決定が要る」と「決定できる材料が無い」は違う。数えたら決める必要が消えた。**
  **決めた形（1 つなら従う／2 つ以上なら今のまま）は*修理ではなく定義*で、網が固定している。**
  ★ **着手の判断はユーザーがこちらに委ねた**（「有利なら着手／不利なら着手するな」）。
  **有利と判断した根拠＝診断・最小対・射程の数え・全数 A/B の baseline が全部
  `scratch/`（git 管理外）にあり、次便は作り直しになる**——**判断は当たり、
  同じ便のうちに 3 出力とも閉じた。**
- **⑾ ★★★ 4 軒目はいつも双子だった。engine を直したら双子にも site があるかを*必ず*訊く**
  （`409cf3bb`）。**collector・MIDI・MusicXML の 3 軒を直したあとで気づいた**——
  **双子は「戻す」のではなく「差を書く」側**なので、**同じ規則でも綴りが違い、grep では出ない**
  （§1 第195 の骨 ⑷ の当日版）。
  ⚠️⚠️ ★★★ **そして双子の穴は 5350 本の網と 217 枚の snapshot の全部から見えない**
  ——**双子を読むのは LP だけ**。**「LP を回す便」以外では永久に緑。**
- **⑿ ★★ 次に触るならここ**——**§2F に*実装待ち*の項は無い**ので、候補は 3 つとも別の種類:
  **⒜ `font "NAME"` の 2 面問題**（§2F・**射程 0 冊＝今なら誰も壊れない**・対を 1 冊起こすところから）／
  **⒝ Release ビルドの Core 283 件の XML doc 警告**（**§2G に第135 から起票済み＝新規ではない**・
  **NuGet pack が通る経路**。**主眼は「0 にする」ではなく*壊れた cref 53 件を読む*こと**
  ——**リネームで黙って失われた参照の地図**）／
  **⒞ ▶ の perf**（§2 の頭・**第193 の地図のまま**）。
  ⇒ ★★ **0.3.0 の門という観点では ⒝ が一番近い**（**出荷する成果物のビルドが出す警告**）。
...
> ⚠️ **第198 が ⒣ を足したが、あれは「決定済み」ではない**——**先に LP を測って台帳点を作る
> ところから**で、**射程 0 冊**。**この計器（pitches）はそもそも拍子を見ない**ので、
...
- ★★ **⒣ 拍子欄を誰も描かない本が、*中途の*拍子変更の欄を今も予約する**
  （2026-08-18・第198 起票。**タブの拍子を直した便が見つけた、向きの逆の残り**）。
  **`tab … as numbers` だけの本と、譜が 1 つも無い本（コード/歌詞のみ）**は
  **どの譜も TimeSignature の stencil を持たない**のに、**中途に `time` が在れば
  その欄が値付けされる**。**LP は素の TabStaff でも Lyrics/ChordNames でも何も取らない。**
  - ★★★ **行頭側は第198 で閉じた**（`SpacingRules.AnyStaffEngravesTime` が門）。
    **残っているのは中途だけ**で、**そこは `MeasureLayouter` が `IReadOnlyList<MusicItem>` しか
    見ない純関数群（`BarlineToFirstColumnSpring` / `MidMeasureChangeGaps` /
    `BoundaryChangePrefix`）に住んでいる＝譜の種別を知らない。**
  - ⚠️⚠️ **これはタブ固有ではない。** **`staffless-system.ly` が*行頭*について閉じた欠陥
    （scores CO/CO3＝4/4 と 3/4 で first chord name が同じ 0.500000）の*中途*版**が
    未着手のまま、というのが正しい姿。**起票の述語をタブに寄せると、それに合わせた修理が出てくる**
    （第194 の骨）。
  - ★★★ **射程 0 冊**。**数え方は 3 度作り直した**（§1 の骨 4）:
    **母集団A＝全部 `as numbers` のタブのみ 5 冊**（**うち `time` の出現が 1 つも無いのが 5 冊**）／
    **母集団B＝譜が 1 つも無い本 28 冊**（**全部が開幕の `time` 1 つだけ**）。
    ...
    ⚠️ ★★ **0 を信じる前に *0 でない母集団* を必ず出すこと**——**コーパス全体では 18 冊が
    中途 time を持つ**。それが出ないなら計器が届いていない。
  - ⇒ ★★ **着手するなら順序は §5.2.1③**: **先に LP を測って台帳点を作る**
    （**素の TabStaff とコード行の両方で、中途 `time` の欄が 0 であること**）。
    **射程 0 冊なので snapshot は動かず承認は要らない**が、**その 0 は「観測している本が
    1 冊も無い」の意味でもある**ので、**fixture を 1 冊起こすところから**（第197 の骨 5 の裏返し）。
  - ⚠️ **署名を触る先が台帳 518 点と snapshot 217 枚の載っている関数群**なので、
    **専用の便と falsifier から**。第198 はこれを理由に着手を見送っている（§1 ⑹）。
...
> ```
> ★ **再測（2026-08-17・第197）＝ 283 件**（`-v q` でも Core の行は出る。**solution ごと建てると
> 2 回ぶん刷られるので `Sort-Object -Unique` を通すこと**）:
> **CS1573 179・CS0419 27・CS1574 26・CS1570 18・CS1734 11・CS1591 7・CS1587 6・CS1572 5・CS1571 4。**
> ⇒ ★★ **476 → 283 は誰も直していない**——**リネームと doc の書き直しで自然に減った分**。
> **壊れた cref（CS1574＋CS0419）は 84 → 53 で、この棚の主眼はいまもそこ。**
