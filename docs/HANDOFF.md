...
最終更新 第349セッション（2026-09-08）＝**入り方は第298〜第345 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。**第348 第 5 便の拾い箱「MusicXML は弦番号を*どの音にも*書き出していない」に着手し、閉じた**——**病名で数え直したら同じ穴に `@finger` も落ちていた**（§5.0「病名で数え直す」の型。`<technical>` に書かれない注釈は 2 つ）。**第 2 便で逆向き（import）も閉じた**（ユーザー指示「このセッションでやる方が有利なら着手」・下 ⑹）。**commit 4 本**（exporter＋番人＋CHANGELOG／HANDOFF／importer＋番人＋CHANGELOG／この HANDOFF）。**骨は 6**:

★★★ **⑴ 正体**: `MusicXmlExporter.ProcessArticulations` に `StringNumberAnnotationSyntax` の腕が無く、`MusicMarkSyntax` は direction 族（pedal・ottava・chord）にしか読まれていなかった＝**`\N` も `@finger(N)` も XML では 0 個**（page は 2026-08-09・twin は `EmitAttachment` から読んでいた）。⚠️ **0.7.0 の CHANGELOG「メンバーの弦番号と fingering は MusicXML に届く」は第348 の時点で嘘だった**（この便で真になった）。**直し**: 腕 2 本（`<technical><string>`／`<fingering>`）＋**和音の外側 `\N` の対付けを `ProcessChord` に**（collector の `CreateChordItem` と同じ規則＝メンバー自身の `\N` が勝ち・外側は書かれた順に配り・尽きたら最後を繰り返す・**自身が持つメンバーは外側を消費しない**〔`<e\3 dis'>\5\4` → 3, 5〕・REF `articulations.cc:38-80 articulation_list`）＋**`<< >>` の group 弦番号は pitch/degree メンバー全部・tied parts の各音に**（`groupString`＝collector の `BuildArpeggioNoteItems`／`EmitArpeggioDegree` と同形・**和音メンバーは和音自身の対付けだけ**＝page と同じ）。
★★ **⑵ 番人** `MusicXml/MusicXmlTechnicalTests.cs` 4 本（単音・和音 4 綴り＋own-wins・arpeggio 5 形〔own/group/degree/tied parts/chord member〕・fingering＋同じ `<technical>` に両方）。**最初の期待 `[3, 4]` は私の読み違い**——collector の対付け index は消費時だけ進む（page が正・XML はそれに揃える）。
★★ **⑶ 射程**（`scratch/p350/scope.ps1`）: **`\N` を書く本＝追跡 14 / 597・ユーザー実コーパス 268 / 324**（`scratch\ベースタブLy`）・`@finger(` は追跡 13・ユーザー 0。**陽性対照**（`scratch/p350/xml/`・Debug の lysc）: `test/tab-string-pinned` は `<string>` 16 / `<pitch>` 16（fixture 自身が「全音符が弦を名指す」と書いている）・ユーザー `(Everything I Do) I Do It For You` は **`\N` トークン 82 ＝ `<string>` 82**（`<pitch>` 235）。page・snapshot・台帳は 1 バイトも触っていない（exporter だけ・打鍵経路ではない＝perf 不変）。
★ **⑷ 開示**: **⒜ 逆向き（import）は空のまま**——`MusicXmlReader` は `<technical>` を 1 つも読まない（hammer-on 等の既存 technical も）ので round trip で `\N` は落ちる。**別便**（拾い箱）。**⒝ `q\5`／bare duration に付けた `\N`**: page の `CreateBareDurationItem` は post-event を写さず `\N` も読まないが、XML は bare の articulations を第 1 メンバーに書く＝**1 綴り目のずれ**。**書く本は 0 冊＝実測**（第 3 便・`scratch/p350/scope-bare.ps1`: `q…\N` は追跡 0／ユーザー 0・裸の音価 `4\3` は追跡 0／ユーザー 0——正規表現が拾った 2 冊 `audit/lpreg/tabnum*.lys` は `<e dis'>4\5\4` の和音外側の形で、目で確かめて除いた）。**⒞ `<c e g>@finger(1)`（和音級の fingering）は第 1 メンバーだけ**（page は item 1 つに 1 fingering＝同じ形）。
★ **⑸ 数**: **開始時裏取り**: HEAD **`2196fde2`**・**未 push 5**・木 clean・未追跡 0・`--no-incremental` build 0 エラー／Core 0 警告・**full 7230 / 0 / 4 / 7234**（`scratch/p350/run1.trx`）——⚠️ **引継ぎの「7229」は写し違い**（4 skip と合計 7234 に合うのは 7230）。**終了時**: **commit 5 本**（`b71d8f9b`＝exporter＋番人 4＋CHANGELOG／`0c1ca2a1`＝HANDOFF＋ARCHIVE／`851dfe01`＝importer＋番人 1＋CHANGELOG／`28a7b11c`＝HANDOFF／第 3 便＝この HANDOFF＝射程 2 つの実測だけ・コード変更 0）・**未 push 10**（開始時 5 ＋ 5・push は指示待ち）・木 clean・未追跡 0・**full 7235 / 0 / 4 / 7239**（`scratch/p350/run4.trx`・**+5＝番人 4 ＋ 1**。第 1 便の `run3.trx` は 7234 / 0 / 4 / 7238・`run2.trx` は 7232 / 2 / 4 / 7238＝赤 2 は継ぎ目の番人が ARCHIVE 移動の途中の docs を読んだもの・下）・**台帳 810／snapshot 247／追跡 `.lys` 597＝不動（触っていない）**。§7.5: 第 1 便＝Core の `+` 行 64・LILYPOND-REF 1（articulations.cc）・LILYSHARP-OWN 0／第 2 便＝REF 0（MusicXML の読み＝LP に対応物無し・定数無し）。⚠️ **Co-Authored-By は付けていない**（2026-07-06 のオーナー決定・memory）。⚠️ **scratch は `p350`**（`p349` は第348 が第 2〜5 便で使った）。⚠️ **§7 3.5 の継ぎ目**: 第345 のブロックを ARCHIVE へ送ったら `HandoffArchiveContinuityTests.TheArchiveResumesWhereTheHandoffStops` が「archive の最新は 347 であるべき」と赤に——**346・347 は §1 を書いたセッションが無い番号**（第345 の計器が `scratch/p347/`・ユーザーが第342〜345 を畳んだ）。**第87 の型で「ブロック無し」の見出しを 2 つ立てて**継ぎ目と census（穴 9 の上限）の両方を満たした（`scratch/p350/archive-345.ps1`＝バイト保存で移す型）。⚠️ **ripple の罠 2 つ（この便）**: **⒜ `"...\$f.xml"` は `$f.xml` を*プロパティ参照*に読む**（`${f}.xml` と書く）／**⒝ 長い 1 行が `>>` 継続に落ちて console が busy のまま**（第331 と同じ・`.ps1` に落として別 console で打ち直し）。

★★★ **⑹ 第 2 便＝逆向き（import）も閉じた（ユーザー「このセッションでやる方が有利なら着手」＝exporter の形・対付け・番人の helper が文脈に在るうちに）**: `MusicXmlReader.ReadNote` が `<notations><technical>` の `<string>`（→ `ImportNote.StringNumber`）と数字の `<fingering>`（→ `Fingering`・p-i-m-a 等の字は warning で落とす）を読み、`LysWriter` が**単音は音価の後ろ `c'4\3@finger(1)`（§3 の語順 `核 \N @…`）・和音メンバーは括弧の中 `<e'\5 dis''\4>`**（relative の `Chord()` と absolute の両腕・`MemberMarks`）に書く。番人 `MusicXmlRoundTripTests.StringNumbersAndFingering_SurviveRoundTrip`（signature 同一・4 綴りの字面・xml1 ≡ xml2 の `<technical>` 列 `3 f1 5 4 f2 4`・字の fingering は warning）。**陽性対照**（`scratch/p350/import-check.ps1`）: 第 1 便で export した `user1.xml`（`<string>` 82）→ `lysc import` → `\N` 82・`tsp.xml` 16 → 16・戻した `.lys` は `lysc check` clean。⚠️ **和音の外側 `\N` は import では*メンバーの中*に戻る**（`<e dis'>\5\4` → `<e\5 dis'\4>`＝同じ音楽の別綴り・page が同じに読むことは第 1 便の番人が言う）。

⇒ ★★★★ **⑺ 次の一手**: **⒜ push は禁止のまま**（未 push は終了時の行）。**⒝ 第344 ⑾⒞ の並び**（拾い箱の ⑺⒜／`lysc ly` の ChordNames＝ユーザーと綴りを決めてから／lyric row の slot の投票／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8b・U8／A/B/D/E／C⑴／G）。**⒞ ~~拾い箱: MusicXML import の `<technical>`~~ → 第 2 便で閉じた（⑹）。** 残る import の穴は hammer-on 等の既存 technical（exporter は書く・reader は読まない＝この便より前からの非対称）——**第 3 便が測って*着手しなかった***（ユーザー「このセッションでやる方が有利なら着手」の 2 度目）: `@hammerOn|pullOff|tap|snapPizz|stopped|thumb|heel|toe` を書く本は**ユーザー 0 / 324・追跡 3 / 597（全部 fixture＝`snappizzicato`・`tab-technique-letters`・lpreg `articulation-snappizzicato`）**（`scratch/p350/scope-tech.ps1`）。**文脈の有利さは実需ゼロの仕事の理由にならない**ので起票だけ残す。やるなら reader の `<technical>` 腕に名前表を足すだけ（hammer-on／pull-off は `type="stop"` の音に付ける＝exporter が start を*前の音*に置く形の逆）。
