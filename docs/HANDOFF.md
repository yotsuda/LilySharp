...
⚠️ **まだ建て直している呼び出しが 4 つ残っている**（`_skylineBuilder.BuildStaffSkylines` を grep して数えた。
**どれも影響は未測定**——**「無い」ではなく「測っていない」**）:
```
LayoutEngine:2362  LeadingLinesOfSystem の閉じ側     ★ 理由あり。PAGE pass から呼ばれ StaffSkylines がまだ無い
                                                    （コード内に理由ごと書いた。他の 3 つは書いていない）
LayoutEngine:2813 / :2940 / :3100                   chord row 系。理由は誰も書いていない
```
★ **:2362 だけは「読めない」ことに理由がある**——**残り 3 つが同じかどうかは見ていない。**
...
⇒ **閉じるなら台帳の点を先に置くこと**（**休符のある本の譜間が全部広がる＝snapshot 承認つきの島**）。
