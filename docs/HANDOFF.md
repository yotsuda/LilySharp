...
最終更新 2026-07-28（第24セッション・**ossia は spaceable な譜だった。ばね・spec・
spaceable 性の 3 つを同時に入れて、OSSK の 4 読みが揃って着地した**）/ HEAD・ahead 数は §0
で確認すること（⚠️ **ここに数字を書かない**——自己参照で、書いた瞬間から commit のたびに嘘になる）。
⚠️ **未 push が溜まっている**（第21セッション末から。push はユーザー・§5.1）。
**3460 passed / 0 failed / 3 skipped**・Core 0 warn 0 err・
**LP 忠実度 137/177 exact・total |residual| 2.639697 ss / 151 distances・counts 24/26**。
⚠️ **総和 11.066495 → 2.639697（−8.426797）は、前セッションが開いた 4 点ちょうど**で、
**他の点は 1 つも動いていない**。exact 数は増えていない（4 点とも非ゼロのまま小さくなった）。
...
### 第24セッション（2026-07-28） ＝ **ossia は spaceable。3 つの量を同時に入れて島が閉じた**
...
- ★ **top ばねの床**（`page.*.first-staff-refpoint` の対照 **+0.024000**／ossia 側 **+0.073104**）。
  Lily# は**どの本でも床 3.8 + padding 1 = 4.8 に座る**が、LP は top ばねを**その下まで圧縮**する
  （対照 4.776000・ossia 側 4.726896＝**インクが小さいほど下がれる**）。⇒ **1 個の量が 2 点に出ている。**
  ⚠️ **ossia の話ではない**（それは第24セッションで閉じた）。**点はもうある。**
...
**非ゼロで残っている台帳点は 36 点**（数は前回と同じ。**動いたのは ossia の 4 点だけで、
どれも非ゼロのまま小さくなった**——総和 −8.426797・他の 32 点は 1 つも動いていない）:

| 点 | 残差 | 正体 |
|---|---|---|
| `page.ossia-pair.compressed.first-staff-refpoint` | **+0.073104** | ★★ **移植済（旧 −6.850208）**。頭はマージンの下に来た。残りは**対照の +0.024000 ＋ 0.049104**＝**top ばねの床**（Lily# は 4.8 に座り LP は 4.726896 まで圧縮）。⚠️ **もう ossia の量ではない** |
...
| `system.ossia-pair.compressed-distance` | **−0.009234** | ★ **移植済（旧 +0.615613）**。inside の**ちょうど 4 倍**＝第2の量ではない（強度 1 と 4） |
| `staff.ossia-pair.compressed.staff-staff-inside` | **−0.002309** | ★★ **移植済（旧 +0.212184）**。ばね・spec・spaceable 性の 3 つ同時。残りは 1 force で**両端の固定項に閉じる**（36 × 0.002309 = 0.083108 == 0.073104 + 0.010000） |
| `page.ossia-control.compressed.first-staff-refpoint` | **+0.024000** | **Lily# は床 3.8 + padding 1 = 4.8 に座り、LP は top ばねを 4.776 まで圧縮する**差。⚠️ **旧 +3.884000 はプローブの section mark だった**（撤回済・§1 のセッション節） |
