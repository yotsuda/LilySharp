...
その分類が 5 族に割れて 5 族とも閉じた便**（**実装 6 便**。**perf は 1 バイトも触っていない**——
**6 件とも correctness で、6 件とも 0.3.0 の門の内側**）。
...
★★ **未 push は 3 便続けて両端が繋がった**——**第195 の終了時 22 と開始時 22 が一致**（間で push は無い）。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 31**（★ **この訂正 commit まで含む数**——
⚠️⚠️ **29 と書いて外した。開始 22 ＋ 実装 6 便 ＋ 注記 1 ＋ 引継ぎ 1 ＝ 30 で、この訂正が 31**。
**第194・第195 と 3 便連続で同じ所を踏んでいる**＝**§0 ★ の注意書きを読んでも防げていない。**
⇒ ★★★ **手順に落とす**: **引継ぎ commit を作ったあと `git rev-list --count origin/master..master` を打ち、
出た数に *1 を足して* 書き、その訂正を commit する**——**数えてから書くのではなく、
書いてから数えるので必ず 1 ずれる。**）・
suite **5319 passed / 0 failed / 4 skipped**（+25＝新しい網ちょうど）・
...
- **⑺ ★ §7.5 の監査**＝**6 便合計で Core +389 行（うち*コード 222*）・削除 −83・REF +0/−0・OWN +0/−0**
  （**`git --no-pager diff --no-color 77d6e718 HEAD -- LilySharp.Core`**＝**セッションの頭から**）。
  **彫版の定数は 0 個**（追加コード行に数値リテラルが 1 つも無い）。**分類は 6 便とも ⒟**
...
- **⑼ ★★★ 4 便据え置かれていたユーザー決定を、この便の終わりに 4 件とも取った**（§3 に起票）。
  ...
  **⒟ 小節途中の `clef` はページが規則**（**MIDI と MusicXML が実装し、双子は補正した綴りを書く**・
  **3 冊**・**snapshot は動かない**）／
  **⒡ 2 冊の fixture が*意図しない音楽*を彫っている**（**決定不要だが snapshot が動くので見せてから**）。
  ⇒ ★★ **着手の順序の提案**: **⒟ → ⒜ → ⒞**（**どれも snapshot を動かさない**）→ **⒝ → ⒡**
  （**この 2 つは snapshot が動くので §6 の手順と承認が要る**）。
  ⚠️ **⒜ と ⒟ は両方 MusicXML のオクターブを触る**ので、**先に片方を入れて全数を測り、
  もう片方の射程を数え直してから 2 つ目に入ること**（**第196 の骨 ⑷＝閉じない残差は分類し直す**）。

---

## 以下は第195セッションの経緯

最終更新 第195セッション＝**第194 が残した宿題（「4 出力の一致を測る計器」）を建てたら、
その日のうちに 3 件出た便**（**実装 3 便**。**perf は 1 バイトも触っていない**——
**3 件とも correctness で、3 件とも 0.3.0 の門の内側**）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **第194 の §1 ⑺ が「残した宿題＝『4 出力の一致』を測る計器がまだ無い」
と ⚠️⚠️ ★★★ で書いており、同じ便が「この族で 3 例出した」と書いていた。**
**族が 3 例出ているのに観測者が 0 という項は、次の便が最初に触る所。ユーザー決定 0 件。**
★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 516 点・ss 非ゼロ 98・
総和 3.609965521・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 16**・
**snapshot 217 枚**・suite **5286 passed / 0 failed / 4 skipped**
＝**引継ぎの数は全部合っていた**（**25 回連続**）。
★★ **未 push は 2 便続けて両端が繋がった**——**第194 の終了時 16 と開始時 16 が一致**（間で push は無い）。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 22**（★ **セッション最後の commit まで含む数**——
⚠️ **20 と書いてから 2 度直した**。**引継ぎ commit を作る前に数え、そのあと訂正 commit が 2 本増えた**＝
**§0 ★ の「この commit を含む数は*最後の* commit のあとに数え直す」を、その注意書きを引用した便が踏んだ**）・
suite **5294 passed / 0 failed / 4 skipped**（+8＝新しい網ちょうど）・
**台帳 516 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・
**snapshot 217 枚で不動＝再ベース 0 枚**・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Paprika`）で通せた。**
⚠️ **計器は Core に 1 行も置いていない**——**計器そのものは repo 内の
`audit/LilySharp.Probe -- pitches`**（下の ⑴）で、**Core は読むだけ。**

> ## ★★★ この便の骨＝**出力どうしを突き合わせる計器を建てた初日に 3 件出た。3 件とも「片側だけ見れば緑」**
> 第194 は 3 件出して「**出力ごとの網とは別に、出力どうしを突き合わせる網を持て**」と結んだ。
> **建てた。同じ日に 3 件出た**——しかも**どれも既存の網では永久に緑**:
> **⑴ `voice { } { }` の後ろの音楽が MusicXML から丸ごと落ちていた**（566 冊で **10,995 音**）／
> **⑵ `repeat tremolo 32 { g''64 a }` が MIDI では上昇音形になって天井 127 に張り付き、
> MusicXML では*オクターブ 67* まで昇っていた**（ページは G5-A5 の 1 対）／
> **⑶ bass part の phrase が MusicXML では 1 オクターブ上**（ページと MIDI は C3、XML は C4）。
> ⇒ ★★★ **3 件に共通する形**: **その 1 出力だけを見ると*整合している***。
> **⑴ は well-formed な MusicXML で、⑵ は importer が普通に開く和音で、⑶ は自分の importer と
> round trip する。**「短い曲」「高い曲」「1 オクターブ上の曲」であることは、
> **もう 1 つの出力と並べたときにしか見えない。**
> ⇒ ★★ **だから計器は「差の数」を出す。閉じたかどうかは*差が 0 になったか*で読む**
> （下の ⑸ の表＝**92 → 64 冊**）。

> ## ★★★ 2 つ目の骨＝**2 つの誤りが同じ向きなら、突き合わせは「一致」と言う。片方だけ直すと差は*増える***
> ⑵ の tremolo は **MIDI も MusicXML も同じ率で昇っていた**ので、
> **両者を突き合わせても MIDI の clamp が効く所しか差にならなかった**（128 中 118）。
> **MIDI だけ直すと差は 118 → 124 に*増え*、MusicXML も直して 0 になった。**
> ⇒ ★★★ **判定法**: **差の数は*検出器*であって*点数*ではない。** 減ったから良い・増えたから悪い、
> ではない。**増えた理由が「片方が正しくなったから」であることを、その場で言えるかどうか。**
> ⚠️ **同じ形が `automatic-polyphony-tabstaff` でも出た**（⑴ の後に 13 → 14）——
> **落ちていた音が戻り、残った差が別の島（移調 clef）だと*見えるようになった*だけ。**

> ## ★★ 3 つ目の骨＝**対照は「毒が届かない所」に置く。「ここは動かないはず」と考えて選ぶと外れる**
> ⑵ の網の対照を **`repeat percent 3 { g''4 a g a | }`**（「始まりと終わりが同じだから動かない」）
> で書いたら、**毒で本体と一緒に赤くなった**。**`''` は*最も近い* g から数えるので、
> 最後が A5 の本体は見た目がどう揃っていても 2 オクターブ上から再入する。**
> ⇒ ★★★ **対照は*構造で*選ぶ**——今の対照は **`octave absolute`**（**運ぶ枠がそもそも無い**）。
> **毒が届く対照は対照ではない**（RULES §5.4 の「動かない本を根拠にする前に、動く本を 1 冊見せる」の裏）。
> ⚠️ **⑴ でも同じことを 1 度やった**——「span 無しの同じ 6 音」を対照にしたら、
> **逐次の音楽は枠を*動かす*ので同じ 6 音ではなかった**（`b,` は `a'` から読む）。
> **対照は「同じ答えになる本」ではなく「その毒が届かない本」。**

> ## ★★ 4 つ目の骨＝**規則は 3 軒で正しく綴られていて、4 軒目だけが自分の答えを持っていた**
> ⑶ の「phrase は part の anchor で開く」は、**collector（`ResetToInitial`）・MIDI
> （`_partOctaveAnchor + varRef.OctaveOffset`）・そして MusicXML exporter 自身の
> `EmitPartMusic`**（**「the relative frame starts at the part's own anchor — the octave it
> PRINTS — not at a fixed middle C」と*散文で*書いてある**）の 3 か所で正しい。
> **同じファイルの 400 行下の phrase の枝だけが `4` と書いていた。**
> ⇒ ★★★ **grep では出ない**——**正しい 3 軒と間違った 1 軒は*同じ量*だが*違う綴り*
> （`_partAnchorOctave` 対 リテラル `4`）。**「2 つ目の綴り」を探す道具は、
> **片方が定数のときに一番効かない。**
...
| 便 | 何が動いたか | 全数 A/B（566 冊・XML 対 MIDI の差） |
|---|---|---|
| ⑴ voice span の後ろ | MusicXML（**+10,995 音**） | **92 → 72 冊**（20 冊が 0 へ） |
| ⑵ engraved-once repeat | MIDI ＋ MusicXML | **72 → 69 冊**（tremolo 3 冊が 0 へ） |
| ⑶ phrase の anchor | MusicXML | **69 → 64 冊**（10 冊が動き 5 冊が 0 へ） |

- **⑴ ★★★ 第1便＝`voice { } { }` の後ろの音楽が MusicXML から黙って落ちていた**
  （`dc0e960f`・**Core 1 ファイル＋網 3 本**）。**`FlushCurrentMeasure` は小節を閉じると同時に
  カーソルを null にし、このファイルの全 emitter は `if (_currentMeasure == null) return;` で始まる。**
  ⇒ **span の後ろは「どこにも行かない」**（`test/multi-voice`＝ページ 3 小節・MIDI 14 音・XML 2 小節 8 音）。
  ★★ **直したら下から 2 つ目の欠陥が出た**——**戻ってきた音が違うオクターブだった**。
  **sub-voice は `_currentOctave = 4` にリセットされ、span の後ろは voice 1 の末尾から続いていた。**
  **規則は collector が enforce する所に書いてある**（`MeasureCollector.MusicWalk` の
  `ParallelExpressionSyntax` の枝＝「the frame at the span's OPENING is what every voice reads
  from, and what the music after the span reads from」）。
  ★ **プローブは 3 つの候補枠を*分離する*ように作った**（`c'1 | voice { g'2 a' } { b,2 c, } d1`
  ＝span の枠なら D5・voice 1 末尾なら D7・C4 リセットなら B2 C2）。
  **平凡な本では 3 つとも D5 になり、欠陥の上で緑になる。**
  ★★ **LP 2.26.0 で裏を取った**（双子の綴りから推論しない）: 双子は
  `<< { g'2 a' } \\ { b,,,2 c, } >> d''1` と*補正した*綴りを書くので、
  **その綴りを LP に通して NoteHead を dump した**——**ページと同じ 6 音。**
- **⑵ ★★★ 第2便＝engraved-once な repeat が両 exporter で昇っていた**
  （`4217bd0b`・**Core 2 ファイル＋網 3 case**）。**percent と tremolo は 1 度しか彫らない**ので
  **各 pass は*その 1 コピー*でなければならない**のに、**前の pass が残した枠で本体へ再入していた。**
  **実測**（`audit/lpreg/chord-tremolo-whole`＝`repeat tremolo 32 { g''64 a }`・ページは G5-A5 の 1 対）:
  **MIDI 79 81 103 105 → 127 が 60 個**／**MusicXML は G5 A5 G7 A7 … オクターブ 67 まで**。
  ⚠️⚠️ ★★★ **`unfold` は*わざと*触っていない**（下の §2F に決定として起票）——
  **ページ自身の unfold のコピーも昇る**（実測：`repeat unfold 4 { g''8 a }` は 4 対が 1 オクターブずつ上へ）
  **ので、ページと MIDI は一致している。食い違うのは LP のほう**（LP は相対鎖を 1 度解いてから複製するので
  4 対は同一）。**双子は `\repeat unfold 4 { g''8 a }` をそのまま書く＝その規則へ渡す。**
  ⇒ **これは「どの出力が壊れているか」ではなく「文法が何を意味するか」の問い。**
- **⑶ ★★★ 第3便＝bass part の phrase が MusicXML で 1 オクターブ上だった**
  （`2c06364e`・**Core 1 ファイル 2 site＋網 2 case**）。**phrase の本体は fresh な枠で開く**が、
  **fresh ＝ part の anchor**（ページと MIDI は C3 E3 C3 G3・XML は C4 E4 C4 G4）。
  ★ **効き方が大きい**——**`test/feature-tour` は差 250 → 6・`showcase/grammar-tour` は 244 → 18。**
- **⑷ ★★ 計器＝`audit/LilySharp.Probe -- pitches`**（**repo 内・6 番目の project の 4 番目の subcommand**）:
  ```
  dotnet run --project audit/LilySharp.Probe -c Release -- pitches [listfile] [only]
  ```
  **⒜ ページ（collector の item）と MIDI を*ソース位置で*突き合わせる**——
  **MIDI が repeat を展開しても位置は同じ**なので、**列や本数で比べると `|:` の在る本が全部差になる罠**を避ける。
  **⒝ MusicXML と MIDI は*鍵の多重集合*で**（XML にソース位置は無い）、
  **「どの位置も 2 度鳴らない」本＝566 中 554 冊**に限って比べる。
  ⚠️⚠️ ★★★ **ページ側は `check --pitches` では*ない*。**`CreatePitchedRestItem` は音符と同じ家で
  音高を解決するので、**trace は音高付き休符を音符として並べる**——
  **報告で作った計器は第194 の ⑶ を「MATCH」と言っていた。**
  ⚠️ **見えない所は数えて印字する**（黙って落とさない）: **grace はページ側にソース位置が無い**
  （566 冊で 1,459 位置・31 冊）・**タイの 2 つ目は MusicXML には書かれ MIDI では併合される**・
  **part-combine の相方**（`pc*` 9 冊＝ユニゾンは 1 つしか彫られない）。
...
- **⑹ ★ §7.5 の監査**＝**3 便合計で Core +85 行（うち*コード 21*）・削除 −4・REF +0/−0・OWN +0/−0**
  ...
  **彫版の定数は 0 個。分類は 3 便とも ⒟**（**枠は collector が既に snapshot している。
  3 つの walk が自分の答えを持つのをやめただけ**）。
  **§7.7 の匂い**——**3 便とも「同じ量の 2 つ目の綴り」**で、
  ⑵ は**その最悪形＝1 つの規則に walk が 3 軒あり、うち 2 軒が*同じ向きに*間違っていた**
...
- ⚠️ ★★★ **⒜ 移調 clef の実音オクターブを MusicXML が持たない**（2026-08-17・第195 実測・
  **未修理**・★★★ **決定済み（2026-08-17・第196・§3）＝`<transpose>` を書く**）。
  - **現状**: Lily# では **clef が実音のオクターブを持つ**（`InstrumentDefaults.GetTransposition` の
    doc が「guitar/tenor は `treble_8` clef を使うので*その分の移調はクレフに乗る*」と宣言）。
  ...
  - ⚠️⚠️ ★★★ **射程は「xmlDiffers ＋ pitchDiffers」*ではない*。数え方ごと残す**——
    **第196 終了時で合併 49 冊**、**そのうち移調 clef／移調 instrument を 1 つでも書く本は 44 冊、
    ...
    （§0 ★ の当日版）。**残り 5 冊の内訳は下の ⒟（3 冊）と ⒠（2 冊）。**
    ⚠️ ★★ **この 44 という数も*綴りの* grep**（本文に `treble_8` 等が在るかを見ているだけ）
    **であって「その差が移調 clef のせいだ」ことの証拠ではない**（§0 ★ の族）。
    **着手するなら、まず 1 冊で毒を当てて「届く」ことを見せること。**
  - ⚠️⚠️ **決定が要る理由は、これが*外部仕様*についての問いだから**——
    ...
    **それが正しいなら、guitar/tenor/tab の part には
    `<transpose><chromatic>0</chromatic><octave-change>-1</octave-change></transpose>` が要る。**
    ⚠️ **この repo の中にその読みを*測る*材料は無い**（第三者の `.musicxml` がツリーに 1 つも無い・実測）。
    **だから「測って決める」ではなく「決めてから測る」項**。
  - ⚠️ **自分の importer は逆の読みを*明記して*いる**——`MusicXmlReader` の注記が
    **「`<transpose>` は clef のオクターブでは*ない*。それは `<clef-octave-change>` で、
    clef 名にもう入っている。足すと guitar が 2 オクターブ落ちる」**と書いている。
    ⇒ **export と import は互いに整合しており、round trip は緑のまま**。
    **だから「他のプログラムが開いたときに何になるか」だけが問い**で、**それは網では出ない。**
  - **終点の形**: **⑴ `<transpose>` を書く**（他プログラムで正しく鳴る。**自分の importer の
    上の注記を同時に直す**——**両方直さないと round trip が 2 オクターブ落ちる**）／
    **⑵ 今のまま**（自己整合を優先）。**⑴ なら 64 冊の XML が動くが、SVG も MIDI も動かない。**
- ⚠️ ★★★ **⒝ `repeat unfold` の相対オクターブの意味が LP と違う**（2026-08-17・第195 実測・
  **未修理**・★★★ **決定済み（2026-08-17・第196・§3）＝「N 回鳴らす」＝各コピーは同じ音**）。
  - **実測**: `repeat unfold 4 { g''8 a }` は **ページが 4 対を 1 オクターブずつ上へ描く**
    （SVG の符頭 y = 28.7/21.7/14.7/7.7）。**MIDI も同じ**（79 81 103 105 127…）＝**両者は一致**。
    **LP は違う**——**`\relative` は音楽を 1 度解いてから複製する**ので
    `\repeat unfold 4 { g''8 a }` は**同じ 4 対**。**双子はその綴りをそのまま書く。**
    ⇒ **つまりページと双子が別の曲になる**（**枠を動かす本体を unfold したときだけ**）。
  - ⇒ ★★ **問いは「play this N times」か「as if you typed it N times」か**。
    **前者なら engraved-once と同じ扱い**（第195 が percent/tremolo に入れた枠の復元を unfold にも）、
    **後者なら双子が補正した綴りを書く**（`\repeat unfold` を展開して書き下す）。
  - ⚠️ **`percent` / `tremolo` は第195 で閉じてある**（`4217bd0b`）——**あちらは決定が要らない**
    （**ページが 1 コピーしか彫らないので、どの意味でも各 pass はその 1 コピー**）。
  - **射程**: **`repeat unfold` を書く本と、その本体が枠を動かすか**を数えてから着手すること
    （**第195 は数えていない**）。
- ⚠️ ★★ **⒞ 2 つ目以降の score は MIDI・MusicXML・双子に出ない**（2026-08-17・第195 実測・
  **未修理**・★★★ **決定済み（2026-08-17・第196・§3）＝警告 ＋ `--score` / `--all` を midi・xml・ly にも**）。
  - **実測**（`test/multi-movement`＝ツリーで唯一の「form が 2 つ以上ある本」・3 楽章）:
    **ページは `--all` / `--combined` / `--score NAME` の 3 つの入口を持つ**（既定は最初の score）が、
    **`lysc midi` は 40 音＝第1楽章だけ**・**`lysc xml` は 16 小節＝第1楽章だけ**・
    **`lysc ly` は `\score` 1 つ**。**どれも「2 楽章落とした」と一言も言わない。**
  - ★★★ **LP に訊いた（1 コマンド）**: **`\score` を 2 つ書いて `\midi { }` を両方に付けると
    LP 2.26.0 は `ts.mid` と `ts-1.mid` の*2 ファイル*を書く**（SVG は 1 枚に両方）。
    ⇒ **「score ごとに 1 ファイル」は LP の答えで、Lily# の SVG の命名規則
    （`RenderSpec.ResolveOutputStem`＝main は stem・他は stem-name）が既にその家。**
  - ⇒ **決定が要るのは*規則*ではなく*CLI の既定***——**出力パスを明示した `lysc midi x.lys out.mid`
    で勝手に 2 つ目を書くのは驚き**なので、**⑴ 引数なしのときだけ全 score を書く／
...
- ⚠️ ★★★ **⒟ 小節の途中の `clef` がページの相対枠だけを付け替える**（2026-08-17・第195 実測・
  **未修理**・★★★ **決定済み（2026-08-17・第196・§3）＝ページが規則**）。
...
| 決定 | 根拠（要点） |
|---|---|
| ★★★ **移調 clef の part には MusicXML の `<transpose>` を書く**（2026-08-17・第196・**ユーザー決定**） | MusicXML の `<pitch>` は「書かれた音高」で written→sounding は `<transpose>` が与える、という規約に従う。⚠️ **`MusicXmlReader` の注記が逆の読みを明記している**ので**同時に直す**——**片方だけだと round trip が 2 オクターブ落ちる**。射程 44 冊。**SVG も MIDI も動かない** |
| ★★★ **`repeat unfold N` は「N 回鳴らす」＝各コピーは同じ音**（2026-08-17・第196・**ユーザー決定**） | percent / tremolo に第195 が入れた「各 pass はその 1 コピー」と同じ規則へ揃える。⚠️ **今のページは 4 対が 1 オクターブずつ昇るので snapshot が動く**——§6 の再ベース手順（LP 照合＋承認）に従うこと |
| ★★★ **2 つ目以降の score は「警告 ＋ `--score` / `--all` を midi・xml・ly にも足す」**（2026-08-17・第196・**ユーザー決定**） | ページが既に `--all` / `--combined` / `--score NAME` の 3 入口を持つので、その家に揃える。**出力パスを明示したときに勝手に 2 つ目を書かない**のが警告つき既定 |
| ★★★ **小節途中の `clef` は相対オクターブの anchor を付け替える（ページが規則）**（2026-08-17・第196・**ユーザー決定**） | **part ヘッダの clef が既に anchor を決めている**ので、「clef という 1 語が 1 つの意味を持つ」ほうを採る。⇒ **MIDI と MusicXML がこれを実装し、双子は補正した綴りを書く**（`transpose` の前例＝第194）。**snapshot は動かない**。射程 3 冊 |
