...
最終更新 第397セッション（2026-09-17）＝**別の PC（YT-DELL2・x64・fresh clone）で §0 から入った**。開始時 HEAD `184b80d0`・未 push 0・**full `sessions/p397/run1.trx` 8608 / 7 / 4 / 8619・CI は直近 2 run が赤**。赤 7 は全部*環境と履歴*で、コードの赤は 0。それを片付けてから **§2 R の R3（tuplet 腕）を閉じた**（`e1e64006`）。道具は pwsh MCP。**骨は 5**:

★★★ **⑴ 新しい PC の罠 2 つ（コードは 1 行も変えない）**: ⒜ **`C:\MyProj\lilypond-src` が `master`（2.27.3-17）だった**→ `git checkout v2.26.0`（RULES §5.2「版」）で `LpReferenceCitationTests` 6 本の赤（`stanza-number-engraver.cc` が無い・`tie-formatting-problem.cc` が 1284 行・scm/ly の範囲ずれ 85+12）が全部消えた＝**REF の行番号は 2.26.0 の行番号**。⒝ **git の identity が無い**（`Author identity unknown`）→ repo ローカルに `user.name yotsuda`／`user.email` を置いた（global は触っていない）。ほかに: Bash 禁止の hook と settings.json の deny はこの PC には無い（Lab notes §1 の規則どおり pwsh MCP で作業したが、**最初の 3 回は Bash を使ってしまった**＝新 PC では最初に入れること）・`C:\bin\lilypond-2.26.0` はユーザーが便の途中で置いた・**lilypond.exe は起動が CPU ほぼ 0 のまま数分止まることがある**（1 回目 143 秒で成功・2 回目 4.5 秒・3 回目＝別フォルダの新しい `.ly` でまた 9 分超＝規則は未特定。出力は終わるときにまとめて出る＝kill せず `wait_for_completion` で待つ）。LP 双子はこの PC で描ける（`sessions/p397/sweep/probe-lp-page1.png`）。疑うものと調べ方は Lab notes §3 の新 PC チェックリスト。

★★★ **⑵ CI の赤＝`HistoryCitationTests.DeadCitationsDoNotGrow` 699 / 天井 687**（RULES §5.5 の型「ユーザーが便の外で畳んで push」）: 第393〜396 の未 push 20 本が 6 本に畳まれて push されていた（`fa4591d1`〜`e954b17a`）。**census を pwsh で複製して差分を取った**（`git grep -o` → `LooksLikeCitation` → `rev-parse` と `rev-list master`）＝ **green 時 693 / HEAD 705 で差 12＝天井の差と一致**・12 本は全部 §1（第395/396）と ARCHIVE 先頭（第393/394）の SHA。§1/ARCHIVE の 4 ブロックが引く SHA は 15 本、うち 3 本は a-f が 1 文字で census が数えない＝規則どおり予測 12 で実測 12。**天井を 699 に上げた**（`c8cb106a`・remarks は既存 10 段落と同じ型・旧 SHA は書かない）。この clone には reflog も旧 object も無いので木の同一性は検算できていない（remarks にそう書いた）。**push するまで origin の CI は赤のまま**（push はユーザー）。

★★★★ **⑶ R3＝tuplet 腕を main 腕に畳んだ**（`e1e64006`）: `EmitScaledItem` は 5 本の main 腕が同じ item に貼るものを何も貼っていなかった＝`cue { tuplet {…} }` が full size（`BeginsCueRegion` の縁も無い）・**tuplet の前の `grace` が tuplet の*後ろ*の音符に付く**・**tuplet の中の `grace` は無言で消える**（`EmitScaledItem` に腕が無く `Fraction.Zero` を返していた）・`@courtesy` は form の 2 周目だけ・`@notehead`／`@feather` 無視・tremolo pair の半分化なし。**家 2 軒**＝`OpenSoundingItem`（pending grace を歩いて item index を読み直す・courtesy の pre-scan・`_cueDepth`・feather）と `DecorateSoundingItem<T>`（cue の縁・notehead・tremolo pair・LeadingGrace）を両腕から呼び、main 腕 5 箇所の逐語の grace ブロックは前者に畳んだ。**網は差分**（`TupletArmTests` 9 本＝同じ音楽を `tuplet 3/2 { }` の内と外に書いて同じ答えを求める・8 本が赤から緑）。LP は `\tuplet` を `TimeScaledMusic` で*同じ event*を包むだけ（`ly/music-functions-init.ly:2470-2495`）＝engraver は区別できない、が根拠。**掃き 938 冊（追跡 608＋Lab corpora 330・base＝`c8cb106a` の Debug lysc・data-pos 伏せ）moved 0**＝母集団にこれらの綴りを tuplet の中に書く本は 1 冊も無い。陽性対照 `sessions/p397/sweep/probe-cue-tuplet.lys`（base/head の png を目視: cue の連符が小さく・grace が連符の前・× 符頭・(♮)）。**LP 双子は 1 枚だけ取れた**（`probe-lp-page1.png`＝固まった lilypond が書き残した tmp の page png）: grace が連符の前・梁が全部を覆う連符に括弧なし＝head と一致。観察 1 つは**同便で LP 5 冊の実測で閉じた**（`sessions/p397/probes/cue-beam/`）: 双子で `\new CueVoice { \tuplet 3/2 { d'8 e' f' } }` が*梁なし*（旗 3 本＋括弧）だったのは、**直後の主声部の `\grace` が CueVoice の自動梁を切る LP 側の癖**（d 冊＝grace あり→梁なし・b 冊＝grace なし→梁あり・e 冊＝素の Voice なら grace があっても梁あり）＝Lily# の欠陥ではない。⇒ **双子に cue と grace が並ぶ本は梁で比べない**。双子の `@notehead`／`@courtesy` は exporter が落とす（既知）。snapshot 0 枚・台帳不動。

★★★ **⑷ 畳んで見つけた 2 つ**（§5.0「対の食い違いが第 2 の欠陥を出す」）: ⒜ **tuplet 腕の戻り値は*書かれた*音価**＝最初の切り方は scale 済みの `Duration` を返し、`writtenDuration` が 2/3 になって `AddDuration` の時計が狂い、**tuplet の直後の `break` が「break stands inside a tuplet」で断られた**（`MidBarBreakTests.ATupletThatEndsAtTheBreak` が拾った・scale を掛ける前に読む）。⒝ **tuplet の前の grace の列が括弧の index 範囲に入る**（`ProcessTuplet` は `startNoteIndex` を先に取り、grace は最初の音符の腕で歩かれる）＝**梁が全部を覆う 3 連の下に括弧が描かれた**（probe の 1 枚目で目視）。LP の `bracket_basic_visibility`（`tuplet-bracket.cc:97-103`）は括弧の bounds が並行 beam と等しいとき隠す＝grace 列が bound になると隠れない。直し＝**外から来た grace のときだけ**範囲を grace 列の先へ開く（本体の中の grace は tuplet の音楽なので残す・テスト `GraceInsideATuplet_IsInsideItsBracket`）。ほかに 1 つ: bare duration の spacer が cue の縁の stamp を型検査の前に消費していた（`&&` の順）＝helper では型で振り分けるので消費しない（観測者なし・commit message に記載）。

★ **⑸ 終了時（第 1 便）**: commit 3 本（`c8cb106a` 天井・`e1e64006` R3・§1 `71367fe5`）・**最終 full `sessions/p397/run4.trx` 8624 / 0 / 4 / 8628**（+9＝`TupletArmTests`）・§7.5（対 `184b80d0`・スクリプトの出力）**Core `+` 203 行／REF 2／OWN 0**（新しい数値定数なし・2 本は `tuplet` と `bracket_basic_visibility`）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 608・§7 3.5 は済（第395 を `-Archive 395`＝24 行 5,302 chars）。**push はユーザー**（2026-09-17 の指示「プッシュは禁止だ。コミットはしてもよい」＝Lab も含む・Lab notes §2 に記載）。

★★ **第 2 便（同日・ユーザー指示「このセッションが有利なら着手」）＝§2 R5 を閉じた**（`4334ce3d`）: `ProcessCueRegion` が cue の本体を素の `GreenSite` で並べていたので、**`cue { ph }` の phrase 参照は誰も読まない kind の site になり無言で消えていた**（`ProcessGraceRegion` は第300 に同じ直しを受けていた）。直し＝`GatherMusicSite` に通す 1 行。**網は差分 2 本**（`CueRegionTests`）: `cue { ph }` は `cue { ph の本体 }` と同じ item（cue サイズ・縁の stamp）／**validator は素の `ph` と同じ答え**（inline と同じ、ではない＝validator は phrase 参照をどこにあっても不透明に読む＝§2 R12⒝・未着手。`lysc check` で `c'4 d' ph |` も `cue { ph }` も「first measure is shorter 1/2」×2 が出る＝R12⒝ の実物・`sessions/p397/probes/cue-phrase/`）。⚠️ 差分の網は `octave absolute` で書く（phrase 本体は自分の relative 枠を開くので、relative では inline と音高が違って当然）・`p` は予約語（dynamic）で phrase 名に使えない。**掃き 938 冊 moved 0**（母集団に cue の中の phrase 参照は無い）・陽性対照 `probes/cue-phrase/ref.lys`＝base≠head かつ **head の `ref` と `inline` は data-pos 伏せでバイト同一**。**full `sessions/p397/run5.trx` 8626 / 0 / 4 / 8630**（+2）・snapshot 不動・台帳不変・REF 0（既存の展開器を呼ぶだけ）。**同便でユーザーの機械対応 2 つ**: Defender の除外に `C:\bin\lilypond-2.26.0`（UAC）・配下 1223 ファイルの Mark of the Web 解除＝**それでも lilypond の起動停止はランダムに残る**（Lab notes §3）。**HEAD＝この §1 の第 2 便 commit・未 push 5**。**残る道具**: `LilySharp-Lab/sessions/p397/`＝`exe-base/`（`c8cb106a` の Debug lysc）・`sweep/sweep.ps1`（p395 の型・母集団を Lab の corpora に向けた）・`sweep/base/` 938 枚（**HEAD の絵と同一なので次便もそのまま base に使える**・moved 0 の証明つき）・`Zz397ProbeTests.cs.txt`・`commit-r3.txt`。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（ARM64 RID と PATH 再読込・触っていない）。⇒ **次の一手**: §2 R の上から＝R1（exporter の `alternative`＝言語判断・ユーザーに訊く）／R2（MusicXML の section header `partial`＝出力が動くのは XML だけ・掃き不要）／R13⒜（診断の collect 共有）。LP 双子が要る R7〜R11 もこの PC で進められる（⑴ の lilypond は解決済み）。
...
- **R13 プレビュー遅延の構造**（第395 は ⒜ の cancel と validator の走査だけ入れた）＝⒜ 診断が preview と別に **full collect＋~50 全木 walk**（`SemanticValidation.TryCollect`＝resume 無し・`WalkProbe` 無し）→ preview の collector を version 一致で共有する（`FindFirst` は先頭 score・preview は選択 score＝R13′ 食い違い）／⒝ SVG 全文が打鍵ごとに **2 プロセス跨ぎで ~7 回コピー**（`Assemble`→JSON→parse→`svgPostKey` 連結→保持→`postMessage`→`splitPages`）＝差分応答（page/group 単位）＋ `svgPostKey` を hash に／⒞ debounce 既定 100 ms（`package.json` minimum 100・コードの `DEBOUNCE_DELAY_DEFAULT = 60` は死んでいる）／⒟ `MeasureContentKey.Compute` が無変更打鍵でも全 fold（reference-identity memo）／⒠ overlay drawer 28 本が page ごとに全配列を走る（fingering だけ bucket 済み）／⒡ `LayoutStaffGroups`・`StaffSprings` が per-system memo 無し／⒢ `PlaceSystems` が count 変更打鍵で 2 回（prelim annotation 込み）／⒣ `IncrementalCompiler` の cancel が 3 点だけ（layout 全体が atomic）／⒤ `UsingExpander.FindUsings` が main text を再 parse。
