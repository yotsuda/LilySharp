...
末から。push はユーザー・§5.1）。
**3444 passed / 0 failed / 3 skipped**・Core 0 warn 0 err・
...
**commit 2 本**: `fb7ca558`（台帳点 2 件・Core 差分ゼロ）→ `663be3cc`（移植・snapshot 3 枚）。
...
1. ★ **`StaffSprings` の ossia スキップ**（`MultiStaffLayouter`）。**塞いでいた理由は消えた**——
   doc の「距離の移植が先」は果たされ、対は spec の上に座ったので、そこから作ったばねは
   **自分の ideal に居て force 0 で対を動かさない**。⚠️ **それでも今日は外していない**: LP では
   ossia は **spaceable** なので `staff-refpoint-extent` が広がる（**LYROS が 18.000000**、
   Lily# は 2 譜スパン）。**その span を測る点が無いまま外すとページを未測定の量で動かす**。
   ⇒ **`staff-refpoint-extent` に ossia が入ることを測る点が先。**（下の ▶2 と同じ 1 件。）
