...
最終更新 2026-07-31（第53セッション＝**引継ぎ ▶ の beamlet 規則を移植して閉じ、
そのまま「効かない場所」を書こうとして拍グリッドの第2の綴りに行き当たり、GO をもらって
それも閉じた。最後に 1/12 の例外を測ったら移植するものが無く、そこで**
**`tupletBoundaries` が発明だと分かった。欠陥は 3 つ落ちた**）。
HEAD **`8ebcce6f`**（＋この handoff commit）・**未 push 31**・
テスト **3648 passed / 0 failed / 3 skipped**・
台帳 **305 点**（**ss 非ゼロ 72・総和 0.108590402**／**count 点 79・うち非ゼロ 4**）。
⚠️ **この行は書いた直後に stale になる**（HEAD を書く commit が HEAD を動かす）。§0 のとおり
**開始時に必ず実測すること**。

⚠️★ **引継ぎの「count 点 2」は数え方の食い違いだった**（§0 の 4 例目）——**count 点の個数ではなく
その中の非ゼロの個数**。§0 に count 点の数え方を足した。

## ① beamlet 規則（`852c063a`・snapshot 0 枚 → 後述の fixture で 1 枚）
...
★★★ **そして control が本命だった**——**16分3連（1/24）は表に entry が無い**ので
LP は**拍構造に落ちて 4 群 × 6**＝**tuplet 境界をまたいで beam する**。Lily# は **8 群 × 3**。
| 点 | LP | Lily# | 残差 |
|---|---|---|---|
...
▶ **次の一手＝`tupletBoundaries` を退場させ、例外表を最短音価で引く**（③・上の 2 点＋
1/12 の 9 点が判定器。**GO ゲート**）。
★ **その次**: **`test/` に meshing の本を 1 冊**（第52セッションの覆われたステム供給を
コーパスが 1 冊も踏んでいない・**GO ゲート**）／**量子器の stem x は knee では今もコラム由来**
（第51セッション②はビーム外の供給側だけ直した）／**VS Code 拡張の再デプロイ**
（第50セッションが tmLanguage と LSP を変えた・ユーザー側作業）／
**1/16・1/32 の `beamExceptions`**（§2 B・同じ機構に乗る）。
...
- ★★★ **`tupletBoundaries` は発明。LP に「tuplet 境界で beam が切れる」規則は無い**
  （2026-07-31・第53セッション・**実測で反証**）。上の control が出した:
  **16分3連（1/24）は表に entry が無いので LP は拍構造に落ちて 4 群 × 6**＝
  **tuplet 境界をまたいで beam する**。Lily# は 8 群 × 3（`beam.grouping.sixteenth-triplets.*`
  ＝ **+4 / −3**・**この族で唯一の正の残差**＝「切ってはいけない所で切った」署名）。
  ⇒ **閉じるには `MergePureEighthNoteGroups` の問いを「全部 8分か」から
  「この run の最短音価は何で、表は何と言うか」に変える**（そのとき 1/12 の 9 点が
  「答えだけ一致」から本物になる）。**beam の切れる場所が動く＝snapshot の GO ゲート。**
- ★ **1/16・1/32 の `beamExceptions` も未移植**（2/2・4/2・6/4・9/4・12/4・3/2）。
  同じ「最短音価で引く」機構に乗る。⚠️ **点はまだ無い**（`beam-grouping.ly` に score を足すだけ）。
