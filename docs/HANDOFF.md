...
最終更新 第289セッション＝**span の終端を言語全体で 1 つの答えに寄せた（2 便）**——**`7b0df578` テキストスパナ（`@!X`）** ＋ **`1115f42e` オッターヴァ（`@!ottava`・`@loco` 退役）とペダルの診断**。**台帳は 1 点も動いていない**（742／exact 582／ss 非ゼロ 190／総和 25.844537623／count 156 中 2）——**終端が動かしたのは*長さ*であって*高さ*ではない。**
...
★★ **⑸ 診断は族ごとに違う言葉で、規則は 1 つ**（`SpanPairingValidator` が `SpanKind` × `SpanPairingFault` を読む・**LYS4018 は 1 本**）。**2 本目のコードは 2 つ目の答えが漂流する場所になる。** ⚠️ **失う物が族ごとに違うので、そこは言い分ける**——**テキストスパナは*語*を線と一緒に失い、オッターヴァは*移調*を括弧と一緒に失う**（後者は紙を見ても分からない）。

⚠️ ★★ **⑹ ペダルは「インクを動かさず、黙っていたのをやめた」だけ**。**閉じられない `@sustainOn` は*前から*何も描いていない**（実測 `scratch/p289/ped.lys` 対 `pedopen.lys`——**閉じれば括弧＋両端のかぎ爪、閉じなければ何も無い**）。**本便が足したのは診断だけで、掃きでもペダルの本は 1 冊も動いていない。** ⚠️ **ペダルの対応付けには譜／声部のフィルタが無い**（**別の譜の `@sustainOff` が閉じてしまう**）——**直すとインクが動くので本便では触っていない**。**実測: ディスク上でペダルを書く本は 15 冊、うち複数譜の score は 1 冊（`showcase/03-piano`）で、そこでも全ペダルが同じ part に在る**＝**その配置に届く本は 0 冊。**

★★ **⑺ 掃きの計器を 2 度直した**。**⒜ 並列版を*自己検査*した**（**base と head に同じ exe を渡すと、並列化が持ち込みうる唯一の欠陥＝ワーカーが互いの一時ファイルを潰して*他の本のページを読む*が MOVED として出る**）——**1582/1582 SAME・MOVED 0**。**⒝ レンダー失敗に 1 回のリトライを入れた**——**24 ワーカーだと書き込みが稀に負け**、**単体では exit 0 で描ける本が NO-OUTPUT と出た**（実際に 1 度出た）。**「head の engine がこの本を描けない」という*偽の*警報は 1 セッションを潰す**。**本物の失敗はリトライでも失敗する。** ⇒ ★★ **速くした計器は、速くしたことで増える失敗の型を先に塞ぐこと。**
...
終了時: **commit 5 本**（`7b0df578` テキストスパナ ＋ `98198e9d` ハンドオフ ＋ `e0736dca` loco の実測 ＋ `1115f42e` オッターヴァとペダル ＋ **この 1 本**）＝**未 push は 開始 22 ＋ 本便 5**・木 0・未追跡 0。**suite 6540 合格 / 0 失敗 / 4 skip / 合計 6544**（開始 6484 から **+60**）。Core 0 エラー 0 警告。**追跡 `.lys` 579 冊・snapshot 229 枚**（**どちらも不動**——**本を足さず、6 冊を*移行*した**）。**台帳全点不動。**
⚠️ **掃きは 2 回**（**便ごとに 1 回・母集団はディスク上の全 `.lys`・base と head は*本ごとに続けて***）: **テキストスパナ＝1582 冊 SAME 1554 / MOVED 28**（**追跡 fixture 4 ＋ scratch 23 ＋ ユーザー実コーパス 1**）／**オッターヴァ＝1586 冊 SAME 1581 / MOVED 5**（**移行した 2 冊 ＋ scratch 3・ユーザー実コーパスは 0 冊**）。**どちらもレンダー失敗 0・エラーが増えた本 0。**
★★ **緑は*コード commit のあと*に取った**。★★ **WSL ubuntu Release も同じ 6540 / 0 / 4 / 6544**（`1115f42e` の木で）。⚠️ **CI だけは未実行**（未 push 27 本）。
...
>
> ⚠️ **残った小物 2 つ**: **⒜ ペダルの対応付けに譜／声部フィルタが無い**（**別の譜の `@sustainOff` が閉じる**——**直すとインクが動く**。**実測でその配置に届く本は 0 冊**なので、**直す便は本を書くところから**）／**⒝ `@sustainOff` 族の綴りは触っていない**（**`@!sustainOn` にするかは未決**——**ペダルの終端は*かぎ爪という ink* を持つので、`@!` に寄せるべきかはオッターヴァと同じ問いではない**）。
