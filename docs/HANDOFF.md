...
⇒ ★★★ **次の一手＝⑴ の判断**（volta の中の小節検査を、96 本の警告ごと入れるか）、**そして form 限定案の前提条件 2 つ**（§2）。
...
終了時: **commit 2 本**（`02613871` ／ `655149a8`）・**未 push 40**・**Windows Debug 6701 / 0 / 4 / 6705**・**台帳・追跡 `.lys`・snapshot はすべて不動**・**作業ツリー 2 項目（未追跡 1）＝ ⑴ の未 commit 分だけ**。
