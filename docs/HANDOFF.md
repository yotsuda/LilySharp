...
⇒ ★★★ **次の一手（自律で回せる側）の候補**（帳簿としての強制は無し）: **custom text／dynamics の paging 一族は本便で完済**（envelope・frame・X の 3 つとも移植済み・番人 30 点）。残る自律筋は **⒜ `EstimateAboveStaffExtents` の定数（3.5 / 3.0 / 2.0）**＝Prelim スカラーで「観測者が要る」側（下の一覧・第255。本便の 5 幕がそのまま型になる: 対を設計→Lily# を先に測る→probe→台帳→毒→port）・**⒝ `@text` の InkOf fallback**（dynamics 腕の縦は glyph だけが実測インクで、@text は保守的な fallback のまま＝DynamicEngraver.InkOf の注記）。**要ユーザー判断で開いているもの**（前便どおり）: silhouette chordNames scalar の一つの家への出荷（第271 第 5 幕・LP 審判なし regime）・和音語彙（CHL 3 点）・Nimbus Sans 同梱・CHANGELOG 版番号・SectionLabel の X（第257）・F【8】3・混在綴りの同名 section（第269）・標の箱そのもの（0.400000 の項・SectionLabel X の台帳点は `mark.section-label.line-start.box-left-from-clef-left` +0.300000）。
