...
最終更新 第296セッション＝**§2 の項目ではなく*ユーザーが画面で見ていた欠陥*に着手＝`scratch\ベースタブLy\Untitled-6.lys` の「五線とコード名と rit が重なる」**（第295 の §1 が「次は訊く」で終わり、訊いたら *これ* が返ってきた）。**ユーザーの指示は直し方まで指定していた＝「ロジックを発明すると、もぐらたたきのような修正が収束しない。LP のコードを模倣して」**。**重なりは 1 つではなく *3 つ* の欠陥で、3 つとも LP に既に在る答えを 1 つ移すだけで閉じた**（**3 つ目はユーザーが 1 便目のレンダを見て「まだ不完全」と*その場で*報告した分**）。**commit 4 本**。**掃きは 2 回**（**変更 2 つに 1 回ずつ・各 1645 冊 4 出力**）で、**どちらも動いたのは svg 1 冊＝報告されたその本だけ**（midi/xml/ly は 0・`check` の差 0）。**台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188 は不動**（**snapshot は 230→231・追跡 `.lys` は 580→581**＝**3 つ目の網の fixture 1 冊。既存の snapshot は 1 枚も書き換わっていない**）。
...
⇒ ★★★ **次の一手＝ユーザーに訊く**。**⑺ の値段（タブだけの score が注釈を失ってよいか＝§2 U12）が*この便が作った*決定で、いちばん近い。** ★ **§2 の ▶ は据え置きで U8（装飾音符が注釈を運ばない・射程はユーザーの本 0 冊）と U11（l.v./repeat tie の半弓の住所・射程は追跡 1 冊）。**
...
終了時: **commit 4 本**（**タブの stencil 表＋網 ／ docs ／ 段間の rit＋網＋fixture ／ この 1 本**）＝**未 push 31**・木 0・未追跡 0。**suite 6605 合格 / 0 失敗 / 4 skip / 合計 6609**（開始 6600 から **+9**＝**本便の網 9 ケース**）。Core 0 エラー 0 警告。**追跡 `.lys` 581 冊**（**+1＝`test/rit-across-systems`**）・**snapshot 231 枚**（**+1＝同じ本。既存の 230 枚は 1 枚も書き換わっていない**）・**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188＝すべて不動**（**`page.inline-chord.gap-first` を含む＝⑿ の広い直しならここが 2.5 動いていた**）。
★★ **WSL ubuntu Release も両方の木で Windows と同じ**——**`fa57384c`（2 commit 目）で 6603 / 0 / 4 / 6607、`5e50d9cb`（3 commit 目）で 6605 / 0 / 4 / 6609**。⚠️ **CI だけは未実行**（未 push 31）。
...
- **U12. ▶ タブ*だけ*の score は注釈を丸ごと失う — 要ユーザー判断**（**第296 が「五線とコード名と
  rit が重なる」を閉じる過程で*自分で作った*決定。起票も第296**）。
  ★ **第296 は LP の `TabStaff` の stencil 表（`ly/engraver-init.ly:1277-1285`）を移し、タブが上の
  五線と同じ注釈を二重に描くのを止めた**（`TabStaffStencils`）。**LP の blank は*文脈プロパティ*
  なので、五線が隣に在るかどうかを見ない。**
  ⚠️ **帰結**: **`score { tab m }` のようにタブ*しか*無い score は、`@text`・dynamics・`rit.` を
  1 度も描かなくなる**（**前は 1 度描いていた**）。**実測 `scratch/ベースタブLy/奏（かなで）.lys`**:
  **`--score both` は `@text("人差し指で")` を 2 回 → 1 回（これが直した欠陥）／`--score tab` は
  1 回 → 0 回（これが値段）**。**タブを読む奏者に宛てた運指の指示が消える。**
  ★ **LP には忠実**（`\new TabStaff { c4\f }` は LP でも何も出さない。**LP の書き手は score ごとに
  `\revert` する**——**Lily# にその面は無い**）。
  ⇒ **要決定は 1 問**: **タブだけの score は、注釈を*保つ*べきか、LP どおり*落とす*べきか。**
  ★ **保つなら工事は 1 条件・1 か所**——**`TabStaffStencils.Blanks` を「その part が同じ score で
  *五線にも*出ているか」で gate する**。⚠️ **それは LP からの明示的な逸脱**なので、**黙って
  入れないこと**。
  ★ **現在の答えは網で固定してある**＝`TabStaffStencilTests.ATabOnlyScorePrintsNoBlankedMarkupEither`。
  **どちらに決めてもこの網を書き換えるのが着手点で、それは意図した設計**（黙って drift しない）。
  ★ **射程（2026-08-30・ディスク 1645 冊）**: **タブを持つ本 341・タブと五線の両方 320・
  blank される族を書く本は 2 冊**（**どちらもユーザーの `ベースタブLy`**）。**そのうちタブだけの
  score を持つのは `奏（かなで）.lys` の 1 冊 1 か所。**
