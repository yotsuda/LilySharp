...
**次の一手**（⒟ ⒩ は取り下げ・⒟′ は第2便で閉じた。**以下は第2便の測定がそのまま名指したもの**）:

⚠️⚠️ **着手前に読むこと: 「打鍵」には 2 つの regime があり、混ぜると測れない**（第135 の第2便で判明）。
**⑴ 内容不変の木**（`PreviewUpdateBench` が測るもの）は**全体再利用が発火して layout が丸ごと消える**ので、
費用は collect＋key＋springs＋render。**⑵ 本物の編集**（1 音変える）は**再利用が降りる**ので **layout が支配的**。
**この 2 つは桁が違う**（`plain1k` で 0.7 秒 対 3.2 秒＝第2便の後）。**どちらを直しているのか毎回書くこと。**

▶ ★★ **⒟⁵ 本物の打鍵の内訳を取り直す**（**第2〜4便で 3 つの項を抜いたので、上位が入れ替わっているはず**）。
  ★ **手順は 3 回なぞってあるので速い**: `IncrementalCompiler.Compile` と `LayoutEngine.Layout` に
  `Console.Error` の段階計時を env 変数ゲートで挿し（**commit しない**）、
  **本物の編集**（1 音打ち直す）を `perf-plain1k` に当てる。**第2便の直後の内訳**は
  **break 1500–1800 ／ prelim annotation 630 ／ spanners 500–1200 ／ pages 317–360 ／
  per-system pass 80 ／ annotation pass 30**（layout 計 3229 ms）で、
  **第3便が break から約 900 ms、第4便が layout の外から 281 ms を抜いた**。
  ⚠️ **打鍵まるごとの時計はこの機械では帯 50% 超**なので、**段の内訳（1 回の分解）で読み、
  A/B が要る項は in-process の床に落とすこと**（第3・4便がその型）。
▶ ★ **⒟″ の残り: 改行 DP の line-count ループ**（第3便で**数えたが直していない**）。
  ...
  **i ごとに到達可能な line count の下限・上限を持てば範囲で回せる**（外は定義上 Infinity なので
  出力同一）。⚠️ **ただし DP はもう plain1k で 183 ms** なので、**下の annotation / spanners より後**。
  ★ 第3便の後の layout の内訳（`plain1k`・要再測）＝**break・prelim annotation 630 ／
  spanners 500–1200 ／ pages 317–360 ／ per-system pass 80 ／ annotation pass 30**。
▶ ★ **⒟‴ 改頁 DP には LP の早期打ち切りがまだ入っていない**（**第2便の残り**・**LP の 1 行**）。
  `CalculatePagePenalty` の remark が `page-spacing.cc:339-349` の
  `if (!breaker_->too_few_lines (line_count) && page_start < line && overfull) break;` を**引用していて、
  `break` だけ移植されていない**。Lily# は i を 0→j-1（大きい頁から）で回すので、
  **入れるには LP と同じ向き（小さい頁から）に回す必要がある**。**溢れは system を足すほど単調**なので
  **落ちる候補は全部 MaxValue** ＝出力同一のはず。⇒ **append は今 O(n³)（200 systems で 1.35M）で、
  これが O(n×頁あたり systems) に落ちる**。⚠️ **DP はいま layout の 10% なので、⒟″ より後**。
