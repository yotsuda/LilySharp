...
⇒ **承認の順**: P5（LP 回帰本が答え）→ P4・P6・P3（ink only の小さい族）→ P1（全旗 0.04・snapshot 再ベース大）→ P2（改行が動く＝双子で照合してから）。適用は `git apply scratch/p395/patches/pN.patch` → build → 該当 snapshot の再ベース（P1 は多数）。
...
★ **終了時**: commit 3 本（`509d98df` バッチ・P7・この §1）・HEAD＝この §1 の commit・未 push 8・**full `scratch/p395/run3.trx` 8608 / 1 / 4 / 8613＝赤 1 は `ApproximationInventoryTests.TheInventoryIsNotStale`（P7 の行ずれ）で棚卸しを再生成して緑＝実質 8609 / 0 / 4 / 8613**（+4＝lexer の網）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 608・§7.5（対 `ea31a447`）: **Core `+` 551 行／REF 10／OWN 0**（新しい数値定数なし・`TopLevelNodes` の根拠は parser の行を名指した remark）・§7 3.5 は済（第393 を `-Archive 393`＝18 行 3,439 chars）。⇒ **次の一手＝⑶ の承認（ユーザー）**。承認された patch は `git apply` → build → 掃き（`scratch/p395/sweep/sweep.ps1 -Side head -HeadDir …` → `compare`）→ snapshot 再ベース → full。**P2 は先に双子**（`04-advanced`・`chords-funky-ignatzek` で LP の改行と間隔を見る）。承認と独立に着手できるのは §2 R の R3（tuplet 腕の helper）・R6（resume の門 3 本＝増分＝全体の網で捕まる）・R13⒜（診断の collect 共有）。
...
★★★★ **⑴ 決定＝和音・アルペジオは*枠を読むが書かない*（ユーザー決定・§3 の最上段）**: 動かすのは `>`/`>>` の後のマークだけで、着地は**「元の枠 ± マーク」＝群のアンカーではない**。旧規則は根音の*素の文字*でアンカーを連鎖させ、`<c e g>|<d f a>|<g b d>|<c e g>` が 4 小節で 1 オクターブ漂流していた（3 回とも「最寄り」が上へ倒れる）。**案⒜（根音のマークもアンカーに入れる＝LP 寄り）は退けた**——素の文字の連鎖が残るので**漂流が直らず**、`<c, e g>`＝C3 E4 G4（根音だけ落とす常用ボイシング）を失う。決め手はユーザーの一貫性論「単音 `g,` が伝播するなら `<>,` も」。

★★ **⑵ 実装は 4 箇所**: `CreateChordItem`（入口で枠を保存・末尾で `frameOctaveIn + chordOctave` に復元）／`MusicWalk` のアルペジオ（同型・`rootSet` の門は撤去＝マークは rest だけの群にも効く）／`LilyPondExporter.EmitChord`・`EmitArpeggio` は **Lily# 側の枠 `_lys*` だけ**を変え、**LP 側 `_ly*` は第 1 メンバ連鎖のまま**（2 枠の差を次の event に綴る既存機構＝度数和音のために在ったものがそのまま効く）。commit `a2bde055`。

★★ **⑶ 追従**: snapshot 6 枚（**祝福の前に 6 冊とも本文を読み、全部「和音が隣り合う本」だと確かめた**）。**LP プローブ 4 本**（CSB/CSA/CFB/CFA）は `a b c'` → **`a b c`**——末尾の `'` は*旧い枠*を補正していたもので、残すと最後の音が C6＝**双子と同じ数を測りながら別の音楽**になる。**台帳 851 点・総和 22.584727806 は不変**。

⚠️⚠️ ★★★ **⑷ 私の推論が 3 回外れ、3 回とも「測れば分かる」側だった**: ⒜ **B2（和音は `>` マークでも枠を動かさない）に反対した根拠「持続シフトが綴れない」は嘘**——逃げ道に挙げた**楽中 `octave N` は存在しない**（`OctaveDecl` は `absolute|relative` のみ・`octave N` は part ヘッダ専用・section とセルは LYS1035）。⒝ **§3 の fold で「naive は先頭で切れているだけだから安全側」**——実測すると 42 行中 6 行で中身が違い、**3 行は断片がセルの先頭ですらなかった**。⒞ **`XCH` は「並列スパンだから届かないはず」**——これは当たったが未実測のまま書きかけた（後で測って E4 G4 / G4 と確認）。⇒ **この便の教訓は「自信のある顔で出た推論ほど先に測る」で、第393 ⑸ の「計器を 4 回壊した」と同じ形**。

★★★ **⑸ HANDOFF の天井は §2 では直せなくなっていた**: `TheHandoffStaysReadable` が 453,196 / 天井 450,000 で赤になり、正規の repair（§2 の閉じた項目を畳む）は **0 件で枯れていた**（閉じた 77 項目は**全部が本文ゼロ**＝既にポインタ化済み・§2 の 324,787 バイトは正真正銘の open work）。⇒ **ユーザー決定で `Fold-ClosedHandoffItems.ps1` を §3 対応に拡張**（表なので単位は行ではなく**2 列目＝根拠セル**・既定の切り口は今月 1 日・決定セルは §3 に残る）＋ **CWD バグ修正**（`Set-Location` の後に `[IO.File]` を相対パスで呼んでいた＝ホームから走らせると例外）。⚠️⚠️ **最初の実装は 6 行を壊して revert した**——**バッククォート内の生の `|`**（`` `|:` `` 等）で行を刻み、根拠の代わりに断片を ARCHIVE へ移して残りを削除していた。⇒ **コードスパンを伏せて分割し、`|C1|C2|Tail` が元の行とバイト一致する行だけ畳む**。⚠️ **`<!-- ledger: … -->` は生きた引用**（`HandoffLedgerCitationTests` が `## 2.` から**EOF まで**走査・**ARCHIVE は意図的に対象外**）なので、tail に持つ 1 行はマークを §3 に残した（§3 の marker 11 個は不変）。**42 セル 43,928 バイトを移し、逐語 42/42 一致**（gate＝復元同一性と「移すセルに ledger タグが無いこと」の 2 本・読めない 3 列行 1 件は素通しして計上）。**HANDOFF 453,196 → 411,582 バイト**（さらに下の §7 3.5 で 401,484）。

★ **終了時**: commit 2 本（`a2bde055` ＋ この §1）・**full 8605 / 0 / 4 / 8609**・**HANDOFF 401,484 バイト**（§1 現在ブロック 2,737 chars）・**台帳 851 点／総和 22.584727806 不変**・snapshot は 6 枚が動いた（249 枚のまま）・**§7 3.5 は済**（第392 を `-Archive 392`＝9,134 chars/15,573 バイトを逐語で移し、バイト保存と逐語一致を突き合わせた）。⇒ **次の一手＝本便は着手しない**（判定を書き残す・第356 ⑹ の型）: §1 に開いている項（**改行の門**＝`SystemBreaker`/`KnuthPlassBreaker` の力の上限・**休符の付点の pure 段は未測**）は**第393 が `scratch/p394` に残した道具立て**（`bisect`・`exe-base`・LP 双子）に属し、**本便が残した道具**（新しい枠の Debug ビルド・祝福済み snapshot・§3 対応の fold）と**噛み合わない**。どちらも次のセッションでやって損をしないので、**次便が §0 の裏取りから入るほうが有利**。
...
- **R6 resume の門ずれ 3 本**（増分セッションだけ）＝`SilentSectionReference` 腕が `!_suffixSpliced` を見ない（`MeasureCollector.cs:3309`）／`BuilderCheckpoint` が `_atScopeStart` を運ばない（`| |:` の空小節が増分で消える）／`SuffixStateMatches` が `SenzaMisura`・`FrozenPosition`・`LogicalCount` を比べない。**R6′ 非既定 accidental style は resume を全滅させる**（`WalkCarriesNothing` が `_measureAccidentals.Count == 0` を要る・`ForgetsAtBar == false` で永遠に非空＝打鍵ごと full collect）。
