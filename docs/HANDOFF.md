...
⚠️⚠️⚠️ ★★★★ **⑼ 便の末尾 3＝ユーザー指示（3 度目の「この便で有利なら着手」）→ ⑵ F11 を閉じた**（commit `df4177e5`・製品コードは `SkylineBuilder` 1 file）:
**根**: `SkylineBuilder` のタブ腕（`AddTabStaffToSkylines`）は**フレット数字だけ**を seed し、**梁は notation 枠の `AddBeamsToSkyline` を通っていた**（向きは notation の `StemUp`・線は notation quanter＝**タブが描かない墨**）、**符尾（unbeamed）はどこにも無かった**。⇒ **タブに立つ outside-staff grob（MetronomeMark が筆頭）は符尾・梁の無い silhouette に対して置かれ、tempo の quiet baseline が冒頭の上向き梁を貫通した**（`fx11-tempo-beam.lys`。同じ本の notation staff は梁を持つ skyline を読んで避けていた）。
**LP**: `\tabFullNotation` は Stem/Beam の stencil を revert する（`ly/property-init.ly:822-839`）＝TabVoice の Stem/Beam grob は描かれ、axis group の inside skyline は member の stencil（`lily/axis-group-interface.cc:914-940`）。**手書き twin `fx11-hand-tab.ly`（2.26.0）で tempo は梁の上に載る。**
**直し**: `AddTabStemsAndBeamsToSkylines`＝**梁は `ArticulationEngraver.TabBeamOuterEdgeY`（renderer が描き script が避けるのと同じ `TabBeamQuant` の線）・符尾は `TabStaffGeometry.UnbeamedStemTipY`（renderer と同じ届き）を、タブの枠（中線＝device 0）で seed**。**`BuildInsideStaffSkylines` と段 silhouette（`AddEdgeStaffInk`）の両方でタブだけそちらへ分岐**、numbers tab は seed 無し（描かないので）。**旗（unbeamed 8 分の Flag）も同便末尾で足した**（4 度目の「この便で有利なら」・`\tabFullNotation` は `Flag.stencil` も revert する＝`property-init.ly:833`・notation seed と同じ `Flag::width` の箱を tab の先端に吊る。**先端より上には出ないので snapshot は 1 枚も動かず**、番人 `FullTab_SeedsTheFlagBesideAnUnbeamedEighthsStem`＝旗の下は符尾と同じ高さ・旗の先で下がる）。
**番人** `TabTempoOverBeamTests` 2 本（tempo の baseline が梁の上端より上／numbers tab は quiet baseline のままで full より低い＝陽性対照）＋ **fixture/snapshot `test/tab-tempo-over-beam`**。**既存 snapshot 6 枚が動いた**: **5 枚は `<svg height>`（最終段の crop）だけ**＝下向き符尾が silhouette に入った分（第320 ⑹ のペダルと同形）、**`tab-part-key` は staff↔tab の隙間が上向き符尾のぶん**（絵は同じ・`artifacts/visual-diff`）。**射程**（`scratch/p321/ab321.ps1`＝変更前の exe は worktree `C:\MyProj\LilySharp-perfbase-eae4`＝HEAD `eae4bdd9`、`svg --combined` 全 317 冊）: **MOVED 128 / SAME 189 / 317 冊・27 分**（`scratch/p321/ab/ab321.csv`。**動いた 128 冊は全部 `tab` 行を持つ本・`tab` を持って動かなかった 166 冊は numbers 既定（staff と対）か符尾が届かない本**＝対照は効いている。差分の形は段の Y と最終段の crop＝`'Til They Take My Heart Away` は 6799 → 6803 行で頁が組み直った）。**REF 2・OWN 0・`+` 125 行の数字は構造（`/2`・`−1`・`thickness: 0`）だけ。**
⚠️ **踏んだ罠 2 つ**: **⒜ `& pwsh tools/Approve-Snapshots.ps1 -Name a,b` は配列が潰れて 1 枚も承認しない**（⑹⒥ の再現・警告は出る）——**同一セッションで `& tools/Approve-Snapshots.ps1 -Name @(...)`**／**⒝ `C:\MyProj\LilySharp-base` は*古い*worktree（`cc19cccc`）**で、A/B の base に使うと別の時代を測る——**`git worktree list` を見て、base は `LilySharp-perfbase-<sha>` の規約で新しく建てる**（20 個以上の perfbase worktree が残っている＝掃除は別便）。
...
終了時: **commit 8 本**（**⑷ の修正 `TabPercentBlankBars`／HANDOFF 第321／⑺ を足した HANDOFF／⑻ の設計を足した HANDOFF／⑼ の修正 `df4177e5`／⑼ の HANDOFF `83e6d27d`／⑼ の旗／この HANDOFF**）・**未 push 21**・作業ツリーは README.md と samples/README.md の 2 件だけ・未追跡 0（`scratch/p321` は git 管理外）・Core 0 エラー 0 警告・**Windows Debug 6896 / 0 / 4 / 6900**（**`scratch/p321/f11b-full.trx`。+10＝第320 比: 番人 8 本 ＋ snapshot 2 枚。目録 2 本は `LILYSHARP_UPDATE_DOCS=1` で再生成**）・**追跡 `.lys` 593**（+2）・**snapshot 243**（+2・既存で動いたのは ⑼ の 6 枚）・**台帳 759 点不動**（本便は台帳を触っていない）・`audit/magic_constants.csv` 不動。⚠️⚠️ **WSL ubuntu Release は取れない**（第313 ⑶）——**本便は Core を触っているので `push` 後に `gh run list` で読むこと。**
...
⚠️⚠️⚠️ ★★★★ **⑵ 決定（ユーザー）＝定義の `sings` は track の*既定*メロディ・行の `sings` は*その行の*束縛で既定を上書きする。** **読み手は 1 軒 `LyricBindings.TargetOfRow(root, track, rowSings)`**（**fold＝`RenderSpecParser.RowBindsToPart`・LYS6012＝`LyricSingsValidator`・bound row＝`MeasureCollector` の 3 人が同じ 1 文を訊く**）。**運ぶのは `LyricsRowSpec.Sings`**（`RenderSpecParser.Parse` が `LyricsRowRenderSyntax.SingsTarget` を写す）。**`LyricBindings.Conflicts` / `BuildMap` は定義ブロックだけ読む**（行は衝突しないし既定にもならない）。★ **LP の対応物は `\lyricsto "alt"`＝placement ごとに voice を名指す**——Lily# の綴りがそこへ収束した形（**定数 0・REF 0 の便**）。⚠️ **意図的に採らなかった読み**（§3 行に明記）: 「定義に `sings` が無いとき最初の行の `sings` を既定にする」——**行の束縛は行のもの。他の score の行にまで効かせると 08-19 の曖昧さが戻る**。

⚠️⚠️ ★★★ **⑶ 射程は corpus に訊いた**: **追跡 `.lys` 587 冊で「行に `sings`」を書くのは 3 冊**（`test/chordrow-rit-second-system`・`test/rit-across-systems`・`test/tab-lyrics-inside-strings`）**で、どれも定義は unbound・行 1 本＝新旧どちらの読みでも同じ**。**snapshot 既存 237 枚は 1 枚も動かない**（full run で確認）。**diff の `+` 106 行に数字を含む行 7 は全部日付と LYS 番号**（§7.5）。

★★★ **⑷ 番人**: **`SingsLyricsTests` +3**（**行の `sings` が定義の既定を上書き＝独立行が vocal の 8 分 onset に載る・fold する陽性対照付き**／**コラール 3 譜＝各譜の音節が*その譜の* onset（alto の付点・bass の 8 分）に載る**／**行の `sings` を外すと LYS6012 が*戻る*＝群の拒否は生きている**）＋ **既存 1 本の主張を反転**（行の別 target は LYS7005 ではない・定義ブロック同士は今も LYS7005）＋ **fixture/snapshot `test/sings-per-row`**（`SvgSnapshotTests.TestSamples` は*静的リスト*＝ファイルを置くだけでは走らない。**第320 はここで 1 度「snapshot が作られない」を踏んだ**）。

★★ **⑸ 副産物＝起票のみ（未実装）**: **§2 F ⒲＝同じ track を*独立行*で 2 つの別メロディに置くと 2 本目が 1 本目を上書きする**（`staffVoices` が track 名で辞書引き）。**コラール（fold される形）は 4 譜で実測済み・実需 0 冊。**

★ **docs**: `GRAMMAR.md` 3 箇所（LyricsBlock 注記・lyrics ROW・StaffGroupBody）・`GRAMMAR_FOR_LLM.md`・`SYNTAX_REFERENCE.md`・`TUTORIAL.md`・`CHANGELOG.md` 0.5.0 Language 先頭・LSP の row `sings` 補完 Detail・**`docs/APPROXIMATIONS.md` は行番号だけ再生成**（`LILYSHARP_UPDATE_DOCS=1`——**MeasureCollector に 2 行足しただけで `ApproximationInventoryTests` が赤くなる**）。**§7.3.5＝第318 の経緯を `HANDOFF-ARCHIVE.md` の先頭へ逐語で移した**（4233 → 4194 行・ARCHIVE 33099 → 33138）。⚠️ **`HandoffArchiveContinuityTests` が §1 の構造を見張っている**（「現在便＋前便ちょうど 1 ブロック」と「ARCHIVE の先頭＝その前の便」）——**第320 は「移した」と「新しい `## 以下は第319…` を書いた」の*あいだ*に full run を走らせて 2 本赤にした**（run5.trx。書き終えてから走らせ直して緑）。**§1 の書き換えは full run の*前*に終えること。**

⚠️⚠️⚠️ ★★★★ **⑹ 便の後半＝ユーザー起票 2 件目**: **`scratch/site-showcase/petite-valse.lys` の L15/L17 の `@sustain` と L21 の `@trill`・L22 の `@fermata` が重なる**。**⇒ 譜内ではなく*段間*の衝突**（段 1 の左手のペダル括弧が、段 2 の右手の上の tr／フェルマータに食い込む）。**骨は 4 つ**:
**⒜ 穴＝括弧は staff の down profile には居た（`PedalEngraver.SolveAndSeed`・台帳 `lyrics.pedal-bracket.*`）が、page が段間を測る silhouette（`AugmentSkylinesForPaging`）のどの腕にも居なかった**——**dynamics（08-28）・rit. spanner（08-30）と同じ穴の 3 例目**。段間は basic-distance の床 12 に座り、括弧はその下に描かれていた。
**⒝ ★★★★ ユーザーが着手前に割って入った**: 「**出力からコードを類推するよりも、コードを字面で移植して。`C:\MyProj\lilypond-src`**」。**私は LP 実測 13.345 から「箱を足す」設計を書きかけていた**。⇒ **移植したのは LP の 2 段**: **`axis-group-interface.cc:969-978 skyline_spacing`（置かれた outside grob の skyline を axis group に merge＝Lily# は SolveAndSeed が既に持つ）／`page-layout-problem.cc:1093-1108 build_system_skyline`（各要素の vertical-skylines を自分の translation だけ raise して merge＝**この腕はそのループ本体を、two-edge silhouette（`SkylineBuilder.OuterStaff` の LILYSHARP-OWN）が page に出さない 1 要素について書いたもの**）。**箱は stencil のもの**（`piano-pedal-bracket.cc:39-40,93-94 Piano_pedal_bracket::print` → `bracket.cc:63-86 Bracket::make_bracket`＝直線＋両端の edge-height の縦線・`define-grobs.scm:2865` vertical-skylines from stencil）で、**`PedalEngraver.BracketStencilBox` の 1 軒を staff profile の merge と page の腕が共に読む**（`SolvedPedalLine` が StartX/EndX/hook を運ぶ）。**REF 6 本・定数 0。**
**⒞ 台帳 4 点を先に起票し、修正前を測った**（`audit/lp-geometry/probes/pedal-page.ly`・PDB/PDBN・`page.pedal-bracket.{,no-pedal.}{gap-first,staves-on-first-page}`）: **LP は PDB 13.345000（括弧の底 5.300 + a''' の頭 7.045 + padding 1）／PDBN 12.000000（床）**。**修正前の Lily# は PDBN 12.000000 厳密・PDB は*計器が読めなかった***（`RenderedGeometry.StaffLineYs` が括弧の線を 6 本目の五線と数えて "found 11 staff lines"）⇒ **計器に「五線には 1 段（1.0／tab 1.5）隣の兄弟が居る・孤立した線は五線ではない」を教えた**。**修正後 PDB 13.345000000 厳密・PDBN 不動・他 770 点不動。**
**⒟ 射程**: **snapshot は pedal を持つ 5 冊（`showcase/03-piano`・`test/pedal-{change,note-anchor,below-lyrics,mixed}`）が動き、差分は全冊 `<svg>` の高さ 2 行だけ＝段は 1 つも動かず、最終段の下端の crop が括弧のぶん深くなった**（scalar down extent にも同じ箱を足した＝dynamics の腕と同形・LP は page の底も skyline で決める）。**新 fixture/snapshot `test/pedal-under-next-system`**（グランドスタッフ 2 段＋tr＋フェルマータ＝ユーザーの形）。**showcase 本の絵で重なりが消えたことを目視。**
⚠️ ★★ **踏んだ罠 2 つ**: **⒤ 散文のコメントに `LILYSHARP-OWN` と*語として*書いたら `ApproximationInventoryTests` がそれを 116 件目の approximation に数えた**（「SkylineBuilder.OuterStaff, LILYSHARP-OWN」と書いた）——**目録は接頭辞ではなく語を数える。散文では「Lily#-own deviation」と書くこと**（§0 の `OPEN:` の逆向きの罠）／**⒥ `Approve-Snapshots.ps1` に配列引数を渡すなら `&` で同一セッションから**（`pwsh tools/…` の入れ子は配列を 1 文字列に潰す＝第318 の `barcount.ps1` と同じ）。

★★★ **⑺ 便の後半＝ユーザー起票 3 件目（エディタ）**: 「**section の music の中で、現在のキーのダイアトニックコードを補完入力できるようにして**（C を選ぶと `<c e g>`）。**一度実装したのに回帰した**ようだ。**ローマ数字（`IIm7` → Dm7）も**」。**実査**: music 文脈の補完には**キー由来のコード行が無く**、在ったのは「2 文字以上の*打鍵済み*記号（`dm7`）を `<d f a c>` に展開する 1 行」だけ（`ChordStructure.TryParseSymbol`）。ダイアトニック一覧は `@chord(…)` と `chords { }` にしか無かった（`GetDiatonicChordCompletions`）。**git 履歴に music 文脈のコード行を足した／消した commit は見つからず**（`-S'<c e g>'`・`-S'Diatonic chord'`）——**回帰の証拠は取れなかったが、要求は明確なので実装した**。⇒ **`GetMusicCompletions` にキーの 7 度 × {triad, 7th, sus4, sus2} の名前行と {triad, 7th} の度数行を足した**（insert は `ChordStructure.ToNoteChord`＝打鍵展開と harmonizer と同じ 1 軒・`flatSpelling` 縮約も適用・並びは音名→名前→度数→休符）。**`CurrentKey(text, offset)` を 1 軒にして**（tonic＋sharps）、pitch 行・`@chord`・`chords{}`・music のコード行が同じ宣言を読む。**度数ラベルは filter 語だけ**（`IIm7` は music の綴りではない・挿入は音名）。**打鍵展開はキー内の記号では立たない**（同じ insert の行が既に在る）。**番人 `MusicChordCompletionTests` 13 本**（全キーで挿入がコンパイルする網・e2e で文書のキーと途中転調）。⚠️ **踏んだ罠**: e2e で `key g major` の*直後*にキャレットを置くと mode 補完の文脈になる（1 小節あとに置く）。**配備は `tools/Deploy-Lsp.ps1`**（下の終了時参照）。**続報（同便・ユーザー指示）＝Ctrl+Space の並び**: **音名はキーの*音階順***（D major なら d e fis g a b cis・sortText＝音階度数・emit 順も同じ）**→ コード名（度ごとに triad/7th/sus4/sus2）→ 度数（同じ 4 形＝`Isus4`/`Isus2` も）**。⚠️ **1 ループで名前と度数を交互に emit していた**（sort 順だけ正しい）——**両順を主張するテストが捕まえた**（`DMajor_ListsThePitchesThenTheNamesThenTheDegrees_EachInScaleOrder`）。**度数の sus は music 文脈だけ**（`chords{}` の parser に `Isus4` は無い）。**続報 2（同便・ユーザー指示）＝section の music で入力できないものが補完に出る**: **`|: :|` と `|: :| [1.][2.]` の 2 スニペットが第305（LYS1034＝`|:` は form 限定）以降も music の一覧に残っていた**——**第240 と同じ「補完が拒否される綴りを教える」型の 3 例目**。2 本を消し、**`MusicCompletion_OffersNothingTheMusicGrammarRefuses` が plain insert の全項目を section の music に書いてコンパイルする網**を足した（スニペットはラベルで `|:`/`[1.` を拒む）。**続報 3（同便・ユーザー指示「form に `|:` を足して。妥当な入力の欠けは足し、不正は除け」）**: **実査すると `GetFormCompletions` は `|:` `:|` `[1. ]` `[2. ]` `[1-2. ]` `_""` を*既に*持っていた**——**ユーザーに見えなかった理由は 2 つ**: ⒜ **sortText 無し＝VS Code が label 順に並べ、`|:` は section 名や nav の下に沈む**／⒝ **`|` を打ってから補完すると `|` は word 文字でないので置換範囲が空＝`|:` を選ぶと `||:` になる**。⇒ **parser（`ParseFormItem`）の語彙を全部並べた**: 足りなかった `:|:`・`:|*N`（snippet）・`[3. ]`・`||`・`|.`・`!`・`break`/`noBreak`。**並びは sections → ~sections → repeat → endings → nav → barlines → breaks → text**。**operator 項目は打ちかけの `| : * . !` の並びを TextEdit で置換**（`GetFormCompletions(text, offset, position)`）。**不正な候補は無かった**（`:|` の Detail の「suffix x3」が文法（`:|*N`）と違っていたので直した）。**素の `|` は inert divider なので出さない**。番人: `FormCompletionTests` +5 本（語彙・全 plain 項目を form でコンパイル・並び・e2e 3 形＝Theory・半打ちの置換）。**続報 4（同便・ユーザー指示「music に `partial` は書けないよね」）**: **正しい。`partial` は section の見出し（`section A { partial 4 … }`）だけ**——part/voice の music は LYS1024（`PartialScopeValidator`）、part-major の cell も LYS1024、**part ヘッダは「not a part property」**（ユーザーは「part ヘッダにしか書けない認識」と言ったが実測は逆＝section 見出し。§3 の「ピックアップは section 全体のもの」の決定どおり）。⇒ music の一覧から `partial` を消し、**網 `MusicCompletion_OffersNothingTheMusicGrammarRefuses` を*意味検証まで*通すよう強めた**（parser だけだったので `partial 4` を通していた＝半分の網。各項目を 1 小節に埋めて `SemanticValidation.Run` の error 0 を要求）。**見つけた欠けはユーザー指示で同便に塞いだ**: **section-major の section 見出し位置（`section A { | m { … } }`）の補完に、part の cell の後ろへ `SectionHeaderDirectiveItems`（partial/key/time/tempo/override）を足した**（sortText 接頭辞で cell の後ろに並ぶ）。**網 `EverySectionMajorHeaderDirective_CompilesWhereItIsOffered`**＝各 directive を section-major の見出しに置いて parser＋意味検証で error 0（`partial` の陽性側＝出す場所で通ることの主張）。

★★★ **⑻ 便の後半＝ユーザー起票 4 件目（言語）**: 「**`break`/`noBreak` と同じように改ページを強制する `pageBreak` を追加して。もっといいキーワードはあるかな**」。⇒ **綴りは `pageBreak` / `noPageBreak`**（`noBreak` が LP の `\noBreak` を小文字 1 語にした規約をそのまま `\pageBreak` / `\noPageBreak` に当てた。§3 に行を足した）。**実査**: **モデルは既に `Measure.PageBreakPermission`（`EffectivePagePermission`＝LP の `min_permission` 鎖・REF 付き）を持ち、`PageBreaker.IsValidBreak` も `SystemDetails.PagePermission` の Force/Forbid を読んでいた**——**無かったのは字句から breaker までの*配線*だけ**。⇒ **足した道**: Lexer 2 語 → `BreakSyntax.Kind`（`BreakKind` 4 値・`IsNoBreak` は残す）→ **`MeasureBuilder.ApplyBreak(kind)` の 1 軒**（music walk・form・repeat block の 3 か所が同じ dispatch を呼ぶ。`SetPageBreak` は LP の `pageBreak` が line と page の両方を force するとおり line も Force・`SetNoPageBreak` は page だけ Forbid・mid-measure の pending 2 flag は checkpoint/resume にも載せた）→ `Measure.PageBreakPermission` → **`LayoutEngine.PagePermissionsAfterSystems`**（各段の最後の小節の `EffectivePagePermission`＝`constrained-breaking.cc:530-535` の last column）→ `CreatePagesWithOptimalBreaking` → `SystemDetails.PagePermission`。⚠️ **1 頁に収まる本は single-page stack が breaker を素通りするので、Force を持つ段が在れば `OptimalPages()` へ回す門を足した**（`PageBreakTests.PageBreak_BreaksAPageThatWouldHaveFitOne`＝1 頁の本が 2 頁になり、頁 1 の末尾と頁 2 の先頭の小節番号で主張）。exporter は `\pageBreak` / `\noPageBreak`。補完 3 一覧・tmLanguage・GRAMMAR（Keyword 列・MidMusicCommand・StructureItem）・GRAMMAR_FOR_LLM・SYNTAX_REFERENCE（`DocKeywordListTests` が 3 文書と lexer を突き合わせる）・CHANGELOG 2 本。番人 `PageBreakTests` 11 本。⚠️ **MusicXML は `break` 自体を書いていない**（`new-system`/`new-page` の綴りが exporter に無い）ので改ページも書かない＝既存の穴の延長・未起票のまま。**続報（同便・ユーザー決定）＝綴りを LP のまま camelCase に**: `nobreak` → `noBreak`、`pagebreak` → `pageBreak`、`nopagebreak` → `noPageBreak`（§3 の行）。**一括置換は `\b` 付き正規表現で 44 ファイル**（Core/Lsp/Tests/docs 3 文書/CHANGELOG 2 本/tmLanguage/追跡 `.lys` 1 冊/scratch `.lys` 16 冊。`HANDOFF-ARCHIVE.md` と `audit/lp-regression/status.json` の `bar-number-nobreak.ly` は除外）。**置換で壊れたのは `Parser.Directives` の LP ヒント（`"noBreak" or "noBreak"` の重複 arm）だけ**＝3 本を消して一般ヒント（`tempo` 族）に語を足した。**旧綴り専用の診断は付けていない。**

★★ **⑼ 便の後半＝ユーザー起票 5 件目（彫版）**: 「`tab as numbers` の譜面に付点がレンダされる」（`scratch/site-showcase/tab-dot.lys`＝`<c e g>4.` の和音）。**実査**: `SharedRenderer.Tab` の単音の腕は `numbersOnly ? 0 : note.Dots` で門を通していたが、**和音の腕 `DrawTabChord` は `chord.Dots` を無条件に描いていた**（門が 1 つの腕にしか無い＝§5.2.1② の「同じ規則が 2 か所」の欠け）。⇒ `DrawTabChord` に `dots` を渡し、呼び手が単音と同じ式で 0 にする。番人 `TabNumbersDottedChordTests`（numbers で dot glyph 0・full で 4＝和音 3 ＋ 単音 1 の陽性対照）＋ fixture/snapshot `test/tab-numbers-dotted-chord`。既存 snapshot は不動。

★★★ **⑽ 便の後半＝ユーザー起票 6 件目（彫版）**: 「`repeat percent` のタブ譜にスラーが表示される」（`scratch/ベースタブLy/tab-percent.lys`＝`repeat percent 2 { c1( | c) | }` を `tab melody as numbers` で）。**実査（起票を先に再現＝§5）**: **tab に限らない**——`staff melody` でも % の下にスラーが出る（`scratch/p320/percent-staff.png`）。さらに **`c1@p@accent( | c@f) |` は `p`・accent・`f` も % の下に 2 度目を刷る**（`percent-dyn.png`）。**原因は engraver ではなく collector**: `ProcessRepeatExpression` の percent 枝は 2 周目以降も `ProcessBodyOnce()` で本文を*そのまま*再 walk する（音符を残して視覚の pass が小節単位で隠す＝決定済みの逸脱）が、**再 walk が tie/slur の marker と post-event（script・dynamic）も一緒に運んでいた**。音符・梁・MMR は `percentCovered` の門を持つ 4 pass が隠すのに、Slur/Tie/Articulation/Dynamic の engraver には門が無い。**LP はそもそも 2 周目を*演奏しない*＝`lily/percent-repeat-iterator.cc:75-111 next_element` が `make_music_by_name (event_type_)` を 1 つ報告して本文は流さない。** ⇒ **門は engraver に 4 つ足すのではなく collector に 1 つ**: `_percentCoveredDepth`（`_graceDepth` と同じ深さ型）を覆われた周の `ProcessBodyOnce()` の周りで上げ、**(a) `ProcessMusicNode` と `EmitScaledItem` の marker を `WithoutBowsUnderPercent` で落とす**（tie/slur のみ。梁は残す＝梁 pass が既に隠し、符尾の向きを spacing が読む）、**(b) `CollectArticulations`／`CollectDynamics` は早期 return**、**(c) `WalkCarriesNothing` に `_percentCoveredDepth == 0` を追加**（覆われた周の中に checkpoint を置かない）。beat slash の周は spacer で walk しないので通らない。⚠️ **手を出していない同族**: 覆われた周の figured bass・chord name・cross-staff・glissando（`CreateNoteItem` の `hasGliss`）は今も運ばれる（測っていない＝§2 に残す）。**本文の*末尾*の `~`／`(`** が書かれた周から覆われた周の先頭音へ掛かる件（LP は "unterminated" 警告で描かない・Lily# は tie を描く）も未着手。番人 `PercentRepeatTests` +3（**`PercentCoveredMeasures_CarryNoSlurTieScriptOrDynamic`＝Theory 2 行（staff／numbers tab。numbers tab は tie・dynamic を*そもそも*置かないので「% の下に無い」だけを主張）・`UnfoldRepeat_StillCarriesEverySlurTieScriptAndDynamic`＝陽性対照（unfold は 4 周すべて運ぶ）・`DoubleSignCoveredMeasures_CarryNoSlur`＝2 小節本文の二重記号の周も同じ深さを通る**）＋ fixture/snapshot `test/percent-covered-bows`（staff ＋ numbers tab）。既存 snapshot は不動。

★★★ **⑾ 便の末尾＝ユーザー指示「タブ譜の LP 忠実度ワークストリームに着手。次便の方が有利なら着手しない」**
⇒ **§1 次の一手 ⑴＝§2 T3（error 本）に着手して閉じた。製品コードは触っていない。** 判断: T3 は「`.lys` 9 冊の綴り直し＋
`lysc check`」で閉じる大きさなので今便で済ませ、**絵の突き合わせ（`.ly` を LP に描かせて Lily# と比べる本体）は
次便に回した**——LP を 285 本描かせる（1 本 15 秒＋Guile 死の罠）のは便の先頭で静かな窓を取ってやる仕事。
**中身は §2 T3 の閉じ記録（⒜〜⒡）**。⚠️ **CSV の 14 冊のうち 4 冊は第318 末で既に 0 だった**（`exe-base` で確認）——
**「直した数」を CSV の差で数えると 5 冊過大になる**。⚠️ **T4 の網の穴を 1 つ**: 移行器は section 境界で*和音を割れる*
（`Green-Tinted` の `<e` ┃ `gis1>`）。T4 は「音符を落とした本 0」と数えたが、その数え方は和音の割れを見ない。
**コーパスは git 管理外なので commit は HANDOFF だけ。**

★ **開始時裏取り**: HEAD `b7865029`・**未 push 0**（**`origin/master` ＝ HEAD＝第319 の 29 本はユーザーが push 済み**）・木 2（**README.md と samples/README.md＝第318 ⑸ のユーザー手直し。本便も 1 バイトも触っていない**）・未追跡 0・Core 0 エラー 0 警告・**Windows Debug の基準は本便の full run と第319 の `scratch/p319/run12.trx`（6840）を*テスト名で*突き合わせた＝差は本便の +3 だけ**（消えたテスト 0）・追跡 `.lys` 587・snapshot 237・台帳 755 点（**本便は台帳を触っていない**）。
終了時: **commit 13 本**（**⑴〜⑸＝歌詞の行束縛・⑹＝ペダルの段間・⑺＝music のコード補完・⑺続報＝その並び・⑺続報 2＝`|: :|` の除去・⑺続報 3＝form の語彙・⑺続報 4＝`partial` の除去と意味検証まで通す網・⑺続報 5＝section-major 見出しの directive・⑻＝`pagebreak`/`nopagebreak`・⑻続報＝`noBreak`/`pageBreak`/`noPageBreak` への改名・⑼＝numbers tab の和音の付点・⑽＝`repeat percent` の覆われた周の bow/script/dynamic・⑾＝T3 の閉じ（HANDOFF のみ）**）・**未 push 13**・作業ツリーは **README.md と samples/README.md の 2 件だけ**・未追跡 0（**`scratch/p320` は git 管理外**）・Core 0 エラー 0 警告・**Windows Debug 6886 / 0 / 4 / 6890**（**run17＝`scratch/p320/run17.trx` は 6885 / 1 で、赤は `ApproximationInventoryTests` の行番号ドリフト（⑽ の `MeasureCollector.cs` 編集）。`LILYSHARP_UPDATE_DOCS=1` で再生成し inventory 2 本を再走＝緑。+50＝第319 比: 番人 3 本 ＋ snapshot 4 枚 ＋ 台帳 4 点 ＋ `MusicChordCompletionTests` 14 本 ＋ 文法網 1 本 ＋ `FormCompletionTests` 7 本（Theory 3 行込み）＋ `PageBreakTests` 10 本 ＋ section 見出し網 1 本 ＋ `TabNumbersDottedChordTests` 2 本 ＋ `PercentRepeatTests` 4 本（Theory 2 行込み）**）・`docs/APPROXIMATIONS.md`（409 行）と `audit/magic_constants.csv`（924）は行番号だけ再生成・⚠️ **flake 1 件を見た**（`FontBlockCompletionTests.AROLEBindingKeepsTheWholeList`＝system font の列挙を 2 度呼んで比べるテストで、片方にだけ `Cambria` が現れた。再実行で緑・run10 も緑。**LSP 配備の直後に走らせた便**なので font cache の再構築の疑い。直していない）・**LSP は `tools/Deploy-Lsp.ps1` でユーザーの拡張に配備済み（Reload Window を依頼）**・**追跡 `.lys` 591**（**+4＝`test/sings-per-row`・`test/pedal-under-next-system`・`test/tab-numbers-dotted-chord`・`test/percent-covered-bows`**）・**snapshot 241**（**+4。既存で動いたのは pedal の 5 枚＝⑹ ⒟**）・**台帳 759 点**（**exact 594＝+4・ss 非ゼロ 195・総和 26.173568＝不動。新しい 4 点はどれも 0**）・`docs/APPROXIMATIONS.md` は行番号だけ再生成・`audit/magic_constants.csv` 不動。⚠️⚠️ **WSL ubuntu Release は取れない**（第313 ⑶）——**本便は Core を触っているので `push` 後に `gh run list` で読むこと。**
