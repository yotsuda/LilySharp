...
★★★ **⑻ 数**: **開始時**: HEAD `382bfd6e`・未 push 5・木 clean・未追跡 0（**`Session-Check` は走らせていない**＝台帳 825／exact 650／full 7792 は第372 の記録を引いた値）。**終了時**（`Session-Check -Scratch p374`）: HEAD `140df989`・未 push 7・**台帳 843 点（+18）／ss 非ゼロ 210（不変）／総和 23.216135909（不変）／count 点 180 うち非ゼロ 0／exact 668（+18）／OPEN 0**・**snapshot 249（不変）／追跡 `.lys` 599（不変）**＝**この便は絵を 1 枚も動かしていない**（engine の commit ゼロ・足したのは probe と台帳と test だけ）。**full `scratch/p374/run-final.trx` 7806 / 0 / 4 / 7810**（＝第372 末の 7792 ＋ 台帳 18＝**新しい test は全部 Theory の case**）。⚠️ scratch は **`p374`**。⚠️ §7 3.5: 第371 を `-Archive 371` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝第372 の天井 commit `9f6df2a3` が未 push のまま。**push はユーザー（未 push 7）**。

⇒ ★★★ **⑼ 次の一手＝*probe は guard ではない*を全数で数える**。この便は同じ形を 2 回踏んだ（⑹ は「規則に観測者ゼロ」・⑺ は「実測を持つ probe に点ゼロ」）ので、**残りを一度に見る**: `audit/lp-geometry/probes/*.ly` のうち **台帳点がゼロの本は 24 冊**（実測した census: accidental-skyline／beam-column-spacing〔←⑺ で解消〕／beam-grace-score／beam-least-squares／beam-over-stem-book／beam-stem-x／brace-ladder／brace-name-clear／coda-line-start／compressed-line-force／cue-grace-spacing／dynamic-text-x／figbass-accidental-x／glyph-skyline／grace-accidental-font-size／instrument-name-x／jn-line-forces／line-start-key-change／notehead-stem-attachment／pedal-three／rest-column-x／rows-only-page／skyline-binding／ties-slurs-breaks-ragged）。⚠️ **「点ゼロ＝欠陥」ではない**——**問いに答えて役目を終えた本**（falsifier・器材の下見）と**量を測ったのに固定しなかった本**は別物で、**⑺ は後者だった**。⇒ **1 冊ずつ「この本は*量*を測ったか、*問い*に答えたか」を仕分ける**のが次の一手で、**仕分けだけなら LP を 1 回も回さずに読める**（各 probe のヘッダが自分でどちらか言っている）。**⒝ 第372 ⑻⒝ の残った face の島**（`Em7` の上付き数字が Nimbus Sans では 0.0277 高い＝「どの sans を同梱するか」の決定）は**ユーザー決定待ちのまま**。**⒞ 第369 ⑸⒝（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。**
