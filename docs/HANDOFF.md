...
★★ **同じ議論で出た起票 2 件**（**どちらも描画は動かない**）:
⑴ **`voice { … }` を含む小節が小節長の検査から漏れている**——`voice { c d e f } e f g a` は
**`c d e f e f g a` と 1 バイト差もなく同じ譜面**（`data-pos` を除く）なのに、
**素で書くと出る `LYS2002 Measure duration 2 exceeds 4/4` が出ない**。
⑵ **1 つしか無い `voice { }` は完全に透明**（符尾も音価も変えない）——**2 声を意図した書き手に
何も言わない**ので、警告を出す価値がある。
