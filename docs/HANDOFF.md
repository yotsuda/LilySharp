...
最終更新 第374セッション（2026-09-12）＝**入り方は第373 と同じ「名指しの起票」**。ユーザーは `docs/HANDOFF.md` を読ませたうえで、**エディタの補完を 1 箇所名指しで起票した**（「`chords prog { section A { Fm } /* here */ }` の here で自動補完が正しい候補を出さない。**here でコード名を出してはいけない**。**section を出すべきだが出てこない**」）。⇒ **§0 の裏取りは走らせていない**（開始時に在るのは git だけ＝HEAD `1e80a9b2`・木 clean・未追跡 0・未 push 9）。道具の指示は無し＝記憶どおり pwsh MCP。**そのあとユーザーが「同じ形が lyrics 側にもあるので直して」と 2 便目を起票し、3・4 便目は同じ族の 3 人目・4 人目を*掃いて*見つけ**（4 便目＝**全数の掃き**で、**見つかったのは日常の綴り `score X transpose Y { }`**）、**5 便目はユーザーが*スニペットの中身*を起票し**（`sings part` の `part` は part 名ではない）、**6 便目はユーザーが staff 行を両側から起票し**（`staff treble ▮` に `as` が出る／`staff treble melody ▮` に出ない）、**7 便目はユーザー指示「同じ穴のもう一つに着手して」で*行を語数で読むのをやめ*、8 便目は「分かれ道はどう実装すべきか」→「実装してみて」で*統一せずに基準と仕掛け線を置いた***。★★★ **この便の通奏低音は 1 つ＝「補完の読み手は*語数*を数えるが、文法には*省略可能な節*が在る」**——**7 便目でその読み方そのものを 1 行分だけ捨て**（staff/ossia 行は token を再生して読む）、**8 便目で「どの行がどちらを要るか」を基準として書き下し**、**9 便目はユーザー起票で `fonts { }` の並び順**（補完の *UX* そのものの起票＝この便で唯一「間違い」ではなく「不便」の報告）。**10 便目は判定で ⒂⒜ に着手して穴だと思っていたものが*死語*だと分かり、11 便目は判定で placeholder の全数監査に着手して*同じ名前を書く 2 項目が逆向きに間違っていた*、12 便目はユーザー決定で*死語を言語から外し*（27,095 冊 0 件を確かめてから）、13 便目は*その全数掃きに着手して「見つからなかった」＋安い代理指標 2 本を実測で潰し*、14 便目は*唯一効く方法（摂動）を 136 本の網にし*、15 便目は*その網を広げて最初の獲物（`hhs`＝splash を名乗る pedal hi-hat）を獲り*、16・17 便目は*drum 表と tuning 表を LP の原典と全数照合して「偽名ゼロ・値の不一致ゼロ」を固定し*、18 便目はユーザー決定「(1) (2) 両方とも入れて」で*その照合が名指した欠落を全部埋めた*（打楽器 30→63・調弦 7→32 語）**。**骨は ⑴〜㉕**:

★★★ **⑴⑵⑶ 起票の再現・病名・「書いても描かれない」の確認**＝**逐語は `HANDOFF-ARCHIVE.md`「第374 の chords/lyrics 脚（逐語）」**（20,000 字の門で出した）。**要点だけ**: **先に赤を作り**（記憶 `feedback_lilysharp_reproduce_ticket_before_fixing`）、**病名は「1 つの語彙が 2 階層に出ていた」**＝`IsInsideChordsBlock` がトラック本体とセルを区別せず、しかも `Completion()` が context の switch より**前**にそれを見る。**セルの隣に書いた和音は `ChordNameCollector` が読まない**（`HasSections` で section しか読まない）ので、**「うるさい」ではなく「通らない綴りを教えていた」**。

★★★ **⑷⑸ 修理は LSP 3 箇所・Core は 1 行も触らない**（commit `fc5edc83`）＝**トラック本体に `ChordsTrackBody` を立て**（`LyricsBlock` の双子）、**`IsInsideSectionContainer` に `chords` を足し**、**intercept をこの 2 つの context に譲らせた**。★ **どちらの語彙かは*形*が決める**（`GetChordsTrackCompletions`）: **section が既に在るか part-major なら cell の足場**、**flat なリードシートは今までどおり和音**——**判定は `TrackNeedsSectionsValidator` と同じ対**（`HasSections` ∥ part-major）なので **validator が黙る所では popup も和音に落ちる**。**逐語は ARCHIVE の同じ節**。

★★★ **⑹ 第 2 便＝*同じ形が lyrics に在る*（ユーザー起票・commit `fb1893de`）——1 段悪かった**。**束ねる綴り `lyrics w sings melody {` は枠に (Prefix=`sings`, Name=`melody`) と読まれ、`lyrics` は 4 語前＝手が届かない**。`IsLyricsFrame` だけがそれを知っていて 2 つの読み手は語で訊いていた⇒ **束ねたトラックの本体が音楽の補完に落ちていた**（実測 98 件）。⇒ **`IsLyricsFrame` を唯一の読み手にした**。★ **test は 2 つの綴りを*並べて*主張する**——片方しか知らない読み手はそれで落ちる。**逐語は ARCHIVE**。

★★★ **⑺⑻⑼ 補完の 3 脚＝逐語は `HANDOFF-ARCHIVE.md`「第374 の補完 3 脚（tilde・全数の掃き・スニペットの名前）逐語」**（20,000 字の門で出した）。**規則だけ**: **⑺ 沈黙 `section ~B {`**（commit `9e96174d`）＝**`~` は語文字ではない**ので枠が名前を 1 つずらし、`FrameKeyword` に "section" を訊く読み手全員が見落とす⇒ **`ReadFrame` が tilde を跨ぐ**（引用表示名を跨ぐのと同じ場所＝読み手全員が一度に覚える）。**⑻ 全数の掃き**（commit `0fb4b9e0`）＝**`{` を開く 25 綴りを 1 回の dump で対読み**し、外れたのは**日常の綴り `score main transpose d { ▮ }`** 1 つだけ⇒ **ヘッダを読む予算を文法の数（名前の後ろ 6 語）にし、ヘッダに成り得ない token で必ず止める**。**⑼ スニペットが書く名前は文法ではなく*この本が宣言した名前***（ユーザー起票・commit `33681a4b`）＝`lyrics … sings ${2:part}` の `part` はキーワードの綴り⇒ **clause を本から決める**（0 なら書かない／1 ならその名前／複数なら空の stop ＋ retrigger）。⚠️⚠️ **網が欠陥を内側に抱えていた**——test がスニペットの出力を自分で修理してからコンパイルさせていた（`Replace("sings part","sings melody")`）＝**その修理こそが起票**。

★★★★ **⑽ 第 6 便＝*行は 2 つの長さで読まれる*（ユーザー起票 2 件・commit `7735687d`）——1 つの欠陥を両側から見たもの**。`StaffRender = 'staff' , [ClefName] , PartRef , [ 'as' , StaffSelector , { StaffSelector } ]` に対し、**読み手が*語数*を数えていた**ので、**clef を書いた瞬間**（＝`staff ▮` で popup 自身が勧める語）**part 名が読み手の見る位置から 1 つずれる**: **⒜ `staff treble ▮` に `as lines`／`as removeEmpty` が出る**（part がまだ無い行に selector を勧める）／**⒝ `staff treble melody ▮` にはどちらも出ない**（素の score 一覧に落ちる）。⇒ **⒝ は tab 行が既に持っていた 3 語目の読み**（`tab TUNING NAME ▮`）で、**staff/ossia が育てていなかっただけ**＝score 本体と staff group の両方に足した（group では**継続は group の狭い 2 つのまま**）。⇒ ★★★ **⒜ は判定ですらなかった**——**「`staff bass ▮` が *clef bass＋名前待ち* か *part bass＋selector 待ち* か」は*この本*が答える問い**で、コメントが根拠にしていた「5 つの clef 語のうち 4 つは合法な part 名」は**言語についての事実で、本に何も訊いていない**。⇒ **その名前の part が宣言されている本でだけ selector を並べる**（`part bass` が在れば `staff bass ▮` は今も両方の読みを出す）。⚠️ **tab の tuning 語も同じ無条件リストだった**（`bass` は tuning でもある）＝**同じ問いを読むようにした**。⚠️ **残った穴 1 つ（未着手・実測済み）**: **`staff m as lines 1 ▮` は score 一覧に落ちる**——文法は `{ StaffSelector }` で**selector が連鎖できる**（`StaffAsGoverns` がその前提で書かれている）のに、**値の後ろで次の selector を出していない**。

★★★★ **⑾ 第 7 便＝*行を語数で数えるのをやめた*（ユーザー指示「同じ穴のもう一つに着手して」・commit `b3742fcf`）——⑽ が残した穴と、探しに行って出てきた 2 つ**。**実測**: `staff m as lines 1 ▮`（連鎖）・**`staff ~m ▮`**（ラベル抑制の `~`）・**`staff m "Violin I" ▮`**（表示名）が**全部 素の score 一覧に落ちる**。⇒ **原因は 1 つ**: 行は `'staff' , ['~'] , [ClefName] , PartRef , [String] , [ 'as' , StaffSelector , { StaffSelector } ]`＝**省略可能な節が 4 つ**なのに、読み手は全部「2 語前が `staff` か」を訊いていた。**誰かが思いついた綴りにだけ規則が足され**（clef は前便で足した）、**思いつかれなかった綴りは間違った一覧を貰う**。★★★ **連鎖が決着をつける**: `{ StaffSelector }` に上限が無いので、**固定の後方参照では原理的に読めない**。⇒ **`ScanStaffRow` が本体の `{` から token を再生し、カーソルが行のどこに居るかを言う**（5 つの return で行を尽くす: 値の枠／`as` 直後／連鎖の途中／名前がまだ／名前が済み）。**語数規則と `StaffAsGoverns` は消えた**＝**行の読みが 1 箇所に 1 つ**。★ **連鎖の位置は「まだ書いていない selector」を*裸で*出す**（`as` は 1 つを共有するので 2 つ目の `as` は構文エラー）＋継続（group の中では group の狭い 2 つ）。★ **文法が繋げられない語で行は終わる**ので、**`staff m  n ▮` は今も裸の MIDI 行**（selector を出さない）。⚠️ **`chords … as roman ▮`／`tab … as numbers ▮` は selector 語がちょうど 1 つ**（`ConsumeAsSelector`）＝**そこで行は終わり**で素の一覧が正しい——**連鎖を持たない行に連鎖を配らない**ための対照として主張した。

★★★ **⑿⒀ 逐語は `HANDOFF-ARCHIVE.md`「第374 の ⑿（分かれ道）と ⒀（`fonts { }` の並び順）逐語」**（20,000 字の門で出した）。**規則だけ**: **⑿ 第 8 便＝統一しない。書き下して、網を張る**（commit `ff40fdf8`）＝**基準は `RowReadingRule`（`ScanStaffRow` の隣）に在る**「**行が語数で読めるのは、省略可能な節が*全部*⑴有界 かつ ⑵裸の語だけ のときに限る**」——⑵ は長さの話ではなく、**`~` 1 つ・`"…"` 1 つで、どんな距離でもそれ以前が全部見えなくなる**（tilde も表示名も無限連鎖も同じ日に落ちた理由）。**仕掛け線は `ScoreRowSpellingMatrixTests`＝26 綴り × 3 主張**で、**毒（`chords` 行に節を 1 つ）を入れて 3 綴りだけが赤くなるのを見てから戻した**。**⒀ 第 9 便＝`fonts { }` の値の並び**（ユーザー起票・commit `0ede7658`）＝**step → size → 字体語 → as → `"…"`**（**face はどうせ打つ／step・size は語彙から選ぶ**・preselect は外した）＋**fallback 連鎖の 2 つ目の face を書く手段が無かったのを足した**。

★★★★ **⒁ 第 10 便＝⒂⒜ に着手し、*欠落ではなく死語だった*（判定は「着手」・commit `9b1298f2`）**。**2 つの穴に見えたものは「誤った提案 1 つ」＋「*正しい*沈黙 1 つ」だった**。⚠️⚠️ **決める前に 3 通り測った**: ⑴ **`MidiPartRenderSyntax` は part 名しか公開していない**（option を読む者がリポジトリに 1 人も居ない）／⑵ **`MidiExporter` は instrument も octave も*part の属性*から取る**（`:1130`）／⑶ **6 綴りを MIDI 書き出しして比較＝`instrument violin` と `instrument tuba`、`octave 1` と `octave 5` が音・timbre・channel まで同一**。★ **GRAMMAR §7 はこの行を `PartRef` だけと綴る**（option は文法に無い）。⇒ ★★★ **だから `instrument`／`octave` は補完に*足さない***——**足せば「何もしない綴り」を教える**＝第 5 便で立てた規則（`sings part`）そのもの。**直したのは*積極的に間違っていた*方**: **`octave` の 2 つの mode 語**（トップレベル指令の語）**が行の値枠に出ていた**——**offer の規則が「part header でなければ」しか訊いていなかった**から。⇒ **読まれる場所では今も出る／行の値枠は無言**。⚠️ **無言は前の挙動より 2 重に良い**: 以前はそこに score の render 語が出ており、**選ぶと `m instrument staff` になり、option の `Advance()` がその `staff` を食べて次の行が keyword を失う**。⚠️⚠️ ★ **残る決定はユーザーのもの**: **文法に無く・誰も読まない option を parser が受け続けるべきか**（⑴ 断る＝LYS0030 の stray item にする／⑵ 実装する／⑶ このまま）。**証拠は上の 3 点で揃っている。**

★★★★ **⒂ 第 11 便＝⒃⒝（placeholder の全数）に着手し、*同じ名前を書く 2 項目が逆向きに間違っていた*（判定は「着手」・commit `4566a82f`）**。**監査の問いは「名前かどうか」ではなく「*その物は既に在らねばならないか*」**。⇒ **答えが違う 2 項目が、同じ「form 名」を書いていた**: **`score main { }` は*参照*（在らねばならない）**——**form が `verse` の本で受け入れると `LYS1018 Unknown form 'main'` を打つ**（実測）／**`form main { }` は*宣言*（空いていなければならない）**——**`main` が在る本で受け入れると `LYS1017 Duplicate form name`**（実測）。⇒ **`score` は本の form を書く**（1 つならその名前・複数なら空の stop 1 ＋ retrigger・**form がまだ無い本は `main` を placeholder のまま**＝**隣の `form` 項目が書く名前と一致するので 2 つ受け入れるとコンパイルできる**）／**`form` は空いている名前**（`main`→取られていれば最初に空く `mainN` を*選択済み placeholder*で）。⚠️ **retrigger には着地点が要った**: **`score ▮` は トップレベルの*キーワード*一覧を出していた**＝**form 名を取る唯一の caret に form 名の家が無かった**（`AfterScoreKeyword` を追加）。★ **監査を正直に保つ対照も主張した**: **`chords ${1:prog}` は*これから宣言する*track の名前なので自由文のまま**。⚠️ **drummap の `${1:hh}` は実測して*欠陥ではない***（`hh` は 83 語の drum 語彙に実在／未知語も静かに通る）＝**「語彙を出すと親切」な UX 案件**として残す（⒃⒝ の残り）。

★★★★ **⒃ 第 12 便＝⒃⒜ をユーザー決定で閉じた＝*裸の MIDI 行の option を言語から外した***（ユーザー「後方互換の問題であれば、除外すべきだ。lily# のユーザーはまだほとんどいない」・commit `de98c1b4`）。⚠️ **前提の裏取り＝出所は後方互換の残骸**: ノードの doc コメントが `partName channel:1 instrument:25` と書き、`ParseMidiPartRender` が **`ConsumeRejectedColon`**（今は断るコロン形式）を呼んでいた＝**`instrument:25` 時代の受け皿**。⚠️⚠️ ★★★ **除去の前に射程を数えた**（§1 に自分で書いた条件）: **この機械の `.lys` 27,095 冊を全部 parse**（追跡コーパス＋audit＋**ユーザーの scratch コーパス**）＝**MIDI 行は 128 本、option 付きは 0 本**。⇒ **移行するものが無い**。★ **語は score 本体の stray item 報告（LYS0030）に落ちる**——**「score 本体が持てるもの」を名指す唯一の一覧**が出る。⇒ **言語から消えたので、第 10 便が足した *option の値枠* の context も消した**（`instrument`／`octave` はただの stray 語＝score の一覧が正しい答え）。⚠️ **`octave` の mode 語を score 本体から締め出す gate は残す**（あれは別の欠陥）。⚠️⚠️ ★ **落ちた test 2 本は*残骸自身の網*だった**——どちらも **`guitar octave 1 instrument 25` が*parse できる*ことだけ**を主張していた（**`instrument 25`＝MIDI プログラム番号**が出自の証拠）。**両側を主張するよう書き換えた**（裸の行は通る／退役した綴りは断られる）。⚠️ ★ **その書き換えが最初*間違った入力*に対して緑になった**——**CRLF の raw literal に `\n` の針**で `Replace` が空振りし、「退役した綴り」のはずが**無改変の本文**だった。**全文を書き出す形に直し、理由を隣に書いた**（記憶 `reference_edit_tool_rewrites_crlf_to_lf` の隣の罠）。

★★★★ **⒄ 第 13 便＝⒄⒜（死語の全数）に着手＝*見つからなかった*。そして「安い代理指標は 2 つとも効かない」が結論**（ユーザー指示・**出荷物の変更ゼロ**）。⚠️ **まず設計上の罠に気づいた**: **死語はどの本にも書かれていない**（MIDI の option は 27,095 冊中 0 件だった）ので、**コーパスを掃いても見つからない**。⇒ 代理指標を 2 つ試して**両方外れた**: **⑴ 構造（赤の公開プロパティから到達できない子）**＝27,095 冊・**7,221 万ノード**を反射で走査 → **44 型が該当し、ほぼ全部が句読点**（`{`・`@`・`(`・`~`）。**非 token の子に絞っても 61 形**——**理由: 消費側は型付きプロパティではなく `DescendantNodes<T>()` で*総称的に*歩く**ので、**「アクセサが無い」は「読まれない」ではない**。**⑵ 型名の被参照数**（赤 72 型を Svg/Midi/MusicXml/LilyPond/Semantics/Editing/Lsp/Cli で grep）＝**候補 2 つだけ**だったが**2 つとも誤検出**: `UsingDirectiveSyntax`（**`Parser/UsingExpander` が読む**＝私が検索対象から `Parser/` を外していた盲点）／`AlternativeClauseSyntax`（**`repeat … alternative { … }`**＝`MidiExporter.cs:2176` と `MeasureCollector.Form.cs:1016` が**プロパティ経由**で読む。**実測で生存確認**: 素の repeat は音 2 つ、`alternative` 付きは 4 つ、中身を変えると音高も変わる 74/76→77/79）。⇒ ★★★ **唯一効く検出は*摂動*——綴りを変えて出力が変わるか**（第 12 便が MIDI option を死語と断定したのはこの方法）。**次便はこれを道具にする**（下 ⒅⒜）。

★★★★ **⒅ 第 14 便＝⒅⒜（摂動ハーネス）を建てた＝*言語が受ける語は全部ページか演奏を動かす*（commit `7fad4176`・`VocabularyPerturbationTests`）**。**136 摂動**（`@` 98・layout 7 キーの全値・clef 5・staff 行の selector・part 属性 5・drummap の 4 フィールド）を**SVG ＋ MIDI の署名**で baseline と比べ、**全部が動く**ことを主張。⇒ **新しい死語は無い**（第 12 便が外した MIDI 行の option が唯一）。★★★ ⚠️⚠️ **INERT は「フィクスチャが差を表現できる」ときしか意味を持たない**——**最初の数回の inert 読みは 4 つとも*私の*フィクスチャの誤り**で、**全部コメントとして現地に残した**: ⑴ **既に同じ clef を持つ part に clef を足した**（`treble` が inert に見える）／⑵ **2 つの section が 1 段に収まっていて「空の段」が存在しなかった**（hara-kiri＝`HaraKiriBook` の `break` が効き所）／⑶ **popup の*ラベル*で摂動した**が実際の挿入は**ラベル＋オペランド**（`barNumbers every 4`）／⑷ **span を開いて閉じ忘れた**——しかも**族ごとに閉じ方が違う**（`@!rit`／`@!sustain`／ottava 族はどの綴りで開いても `@!ottava`）。★ **唯一「正しく inert」な値**＝**`as removeEmpty false`**（意味が「隠さない」＝既定そのもの）。**skip せず*等値*で主張**したので、**既定が変わった日は死語ではなく既定変更として赤くなる**。⚠️⚠️ ★ **毒を入れて発火を確認**（`false` を「動くはず」の theory に足す → **その 1 件だけが赤**・メッセージが綴りを名指す）。

★★★★ **⒆ 第 15 便＝摂動網を広げて*最初の獲物*を獲った——`hhs` は splash を名乗って pedal hi-hat を鳴らしていた**（commit `e2f444a0`）。**掃きの追加**: drum 名 83・instrument preset 44・tab tuning/style・chords display・chord 記号 28。**inert と重複を読むと**: preset 44 は全部動き重複ゼロ／`chords … as names` は inert だが**既定そのもの**（`removeEmpty false` と同類）／drum の `r`・`R` と 18 件の refused は**popup が休符と音楽キーワードも出すから**＝フィクスチャ事情。⇒ ★★★ **残った 1 件が本物**: **`hhp` と `hhs` が署名まで同一**。**LP 2.24.4 `ly/drumpitch-init.ly` を引くと `splashcymbal`／`cyms`（シンバル・GM 55・自分の位置）は在るが *splash hi-hat は無い***。**Lily# の `splashhihat` は `pedalhihat` の行を 1 バイト違わず写していた**（position −5・Cross・**GM 44＝Pedal Hi-Hat**）⇒ **popup が splash と称して pedal hi-hat を描き・鳴らしていた**。**正しい splash は 2 行下に在った**ので**除去**（コーパス 27,095 冊中 0 冊・registry 以外の参照ゼロ）。★ **番人は*表*に置いた**（嘘が住む場所）＝**canonical な drum は行（position／notehead／GM／mark）を共有してはいけない**。**共有する 3 対は全部 LP 自身の二重綴り**で、**1 行ずつ LP の原典で確認**（`acousticsnare=snare`＝LP も canonical 2 つで同じ行 `() #f 1`・同じ GM を NATURAL と DOUBLE-FLAT で綴る／`crashcymbal=crashcymbala`／`ridecymbal=ridecymbala`）。⚠️ **この表に 1 行足すことは「LP がそう綴っている」という主張**＝そう書いてある。

★★★★ **⒇ 第 16 便＝drum 表を LP の原典と*名前集合＋行*で全数照合した（commit `6637dd29`・出荷物は無変更）**。**答えは 3 つ**: **⑴ LP に無い名前＝ゼロ**（`hhs` が唯一で、前便で除去済み＝この問いは閉じた）／**⑵ 行の不一致＝ゼロ**（**移植済み 29 個すべて**が `drums-style` の position・notehead・articulation と `midiDrumPitches` の GM キーに一致。GM は `ly:make-pitch` の三つ組から計算して突き合わせた）／**⑶ LP に在って当方に無い＝ラテン／付属打楽器 約 30 種**（triangle・guiro・claves・maracas・cabasa・agogo・bongo・conga・timbale・woodblock・whistle・cuica・side stick とその略号）＝**欠陥ではなく*射程の決定*（下 ㉑⒜）**。⚠️⚠️ ★ **`drums-style` 以外の表を読んではいけない**——LP には `agostini-drums-style`・`weinberg-drums-style`・`congas-style`・`percussion-style` が在り、**同じ名前が別の位置で載っている**（hihat は drums-style で 3・他 2 つでは 5）。**ファイル全体を正規表現で舐めると最後の表が勝つ**ので、**この監査の 1 回目は存在しない position 不一致を 22 件「発見」した**（コメントに書いた）。★ **行は*検査時に LP を読まず*チェックインした**（LP が無い機械でも同じことを言う test）。⚠️ `tambourine` だけ別立て＝**LP は kit ではなく `percussion-style` に置く**ので、**位置は Lily# のもの・GM だけが LP のもの**。

★★★ **㉑ 第 17 便＝同じ照合を tuning に当てた（commit `1599a56a`・出荷物は無変更）**。`ly/string-tunings-init.ly` の `makeDefaultStringTuning` と突き合わせ: **⑴ LP に無い調弦＝ゼロ**（**7 綴りは 5 調弦**で全部 LP のもの＝`standard`／`guitar` は LP の `guitar-tuning`・`uke` は `ukulele-tuning`）／**⑵ 弦の不一致＝ゼロ**（guitar `<e, a, d g b e'>`＝40/45/50/55/59/64・bass 4/5/6・ukulele の `<g' c' e' a'>`＝67/60/64/69 まで半音単位で一致）／**⑶ LP に在って当方に無い＝約 25**（guitar の drop-D・drop-C・open-G・open-D・DADGAD・lute・asus4・7 弦／bass の drop-D／banjo 7 種／mandolin／tenor・baritone ukulele／violin・viola・cello・double bass）＝**打楽器と同じ射程の決定**（下 ㉒⒜）。⚠️ **ukulele の配列は昇順ではない**（re-entrant で LP もそう書く）＝**「並べ直す」ことが欠陥になる**ので行にコメントを付けた。

★★★★ **㉒ 第 18 便＝ユーザー決定で ㉔⒜ の両方を入れた＝*LP の打楽器 33 種と調弦 25 種*（ユーザー「(1) (2) 両方とも入れて。」・commit `851e5469`）**。**移植は機械的**（第 16・17 便のスクリプトがそのまま吐いた）**が、決めることが 4 つ在った**。★★★ **⑴ どの style 表から行を取るか**——**`drums-style` しか使うな**は kit の話で、**新しい 33 種は kit に 1 行も無い**。⇒ **順序を規則として書いた**（registry の class remark）: **kit（`drums-style`）→ その楽器の対表（`bongos-`／`congas-`/`timbales-style`＝hi/lo を ±1 に置く唯一の場所）→ `percussion-style`（全部 position 0）→ LP がどこにも置かない 8 種は中央線・既定符頭**（＝LP 自身の fallback で、`tambourine` が最初からそうだった＝**「位置は Lily# のもの」という第 16 便の注記は*誤り*で、`percussion-style` に `(tambourine () #f 0)` が在る**）。★★★ **⑵ mark は 2 語では足りなかった**——`guiro`／`longguiro`／`shortguiro` は**同じ行・同じ符頭**で、**`guiro` と `longguiro` は GM まで同じ 74**。**LP は articulation だけで分ける**（`staccato`／`tenuto`）ので、**2 語（stopped／open）のままだと「2 つの名前で 1 つの行」＝`hhs` と同じ嘘**になる。⇒ **`DrumNameRegistry.MarkWords`＋`MarkArticulation` を 1 軒建て**、collector・`DrumOverrides`・`DrummapValidator` の 3 軒がそれを読む（`drummap { x: mark tenuto }` も同時に通るようになった）。★★★ **⑶ `tt` は入れない**——**LP の drumPitchNames は canonical 63・alias 64** で、**余る 1 つが `tt → tamtam`＝style 表にも midiDrumPitches にも、canonical の側にも居ない**。**書けば位置も音も持たない名前**＝**`hhs` を外した基準そのもの**。**test で「無いこと」を固定した**。★★★ **⑷ 調弦の綴りの規則**＝**LP の symbol から `-tuning` を落とし、`<n>-string` を数字にし、ハイフンを詰める**（`bass-five-string-tuning`→`bass5`＝**既存の綴りがそうなっていた**／`guitar-drop-d-tuning`→`guitardropd`）。**30 symbol → 32 語 / 27 `TuningType`**（LP 自身が同じ弦を 2 度綴る 3 組＝`bass`＝`bass4`＝`doublebass`、`violin`＝`mandolin` は 1 メンバーに寄せた）。⚠️⚠️ ★ **名前→`TuningType` の switch が 3 軒に写されていた**（`RenderSpecParser`・`PartHeaderDefaults`・`LilyPondExporter`）＝**25 種を 3 箇所に足す作業**になるところだった。⇒ **`Tunings.Parse`／`Tunings.LilyPondName`／`Tunings.Names` に寄せ**、**`SymbolCaseValidator` の語彙も同じ表から採る**（**綴れるのに弾けない・弾けるのに綴れない**が起こらない）。⚠️ **`TuningType.Custom` は誰も読んでいなかった**＝**言語に無い機能を名乗る死語**なので外した。⚠️ **波及 2 件を実測で拾った**: **`\` の後ろの弦番号が 1..6 → 1..7**（7 弦ギターが入ったので＝`MaxTabStrings` は表から採っている）／**TextMate 文法の tuning 語リスト**（`EditorColouringTests` が両方向で見張っていた＝**それが赤くなって教えてくれた**）。★★★ **⚠️⚠️ 副産物＝*phrase の名前空間の実測*（第 15 便の宿題）**: registry の remark は「**NOT MEASURED**・`$` が唯一の逃げ道」と書いていたが、**測ったら `LYS1030` が宣言の側で断っていた**（`phrase tri { }` ⇒「`tri` is a drum-kit name …」）。**その集合も registry から採られている**ので**33 語ぶん自動で広がった**＝**この便のコストは「phrase に使えない語が 33 増えた」だけで、黙って drum staff になる本は無い**。remark を書き換え、theory で固定した。

★★★ **㉓ 数**: **開始時**: HEAD `1e80a9b2`・未 push 9・木 clean・未追跡 0（**`Session-Check -Build -Test` は走らせていない**）。**終了時**: HEAD `851e5469`（**出荷物の最後の変更は第 18 便＝LP の打楽器 33 種と調弦 25 種が入った。第 13・14・16・17 便は production 無変更**）・未 push 42・**台帳 843 点／ss 非ゼロ 210／総和 23.216135909／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249／追跡 `.lys` 599＝全部不変**（⚠️ **第 18 便は Core を 7 ファイル触ったのに、絵は 1 枚も動かない**——**既存の綴りの振る舞いを 1 つも変えていない**からで、**台帳・snapshot・追跡 `.lys` が全部不変なのがその測定**。第 13・14・16・17 便は Core を 1 行も触っていない）。**full `scratch/p375/run27.trx` 8193 / 0 / 4 / 8197**（第 18 便＝+68 本＝打楽器 3 theory・調弦 1 theory の case が大半。⚠️ **`run24` は 1 本赤**＝`ApproximationInventoryTests` が**生成物 `docs/APPROXIMATIONS.md` の行番号**を見張っており、exporter の switch を 2 軒に寄せて 11 行縮んだので**再生成が要った**。`LILYSHARP_UPDATE_DOCS=1` で作り直して 4 行差＝**行番号だけ**）・第 17 便末 `run23.trx` 8125 / 0 / 4 / 8129・第 16 便末 `run22.trx` 8119 / 0 / 4 / 8123（drum 照合 31 本）・第 15 便末 `run21.trx` 8088 / 0 / 4 / 8092（表の番人 +1）・第 14 便末 `run20.trx` 8087 / 0 / 4 / 8091（摂動 136 本ぶん増）・第 12 便末 `run19.trx` **7951 / 0 / 4 / 7955**（⚠️ **第 12 便で 9 本*減った*のは正しい**＝matrix の MIDI 行 3 綴り × 3 主張が言語から消え、代わりに「退役した綴りは断られる」2 本が入った。第 11 便末 `run16.trx` 7964・第 10 便末 `run15.trx` 7956・第 9 便末 `run14.trx` 7953・第 8 便末 `run13.trx` 7947・第 1 便末 `run1.trx` 7816・第 2 便末 `run4.trx` 7827・第 3 便末 `run5.trx` 7829・第 4 便末 `run6.trx` 7839・第 5 便末 `run8.trx` 7842・第 6 便末 `run10.trx` 7854・第 7 便末 `run12.trx` 7869。**12 便とも合計を読んでいる**＝新 test は差引 144 本（第 12 便は −9）（第 8 便の 78 ＝ 26 綴り × 3 主張）。⚠️ 第 5・6 便は既存 test を**書き換えている**ので増分はそれより小さい）。★ **第 7 便は 7 つの context の配線を丸ごと替えたのに既存 7854 本が全部緑**＝**行の読みが 1 つになっても、読めていた綴りの答えは変わっていない**。⚠️ scratch は **`p375`**。⚠️ §7 3.5: 第372 を `-Archive 372` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝**第372 の天井 commit `9f6df2a3` が未 push のまま**。**push はユーザー（未 push 42）**。★ **LSP は 12 度配布した**＝`tools/Deploy-Lsp.ps1` が `0.6.0+851e5469` を `yotsuda.lilysharp-0.5.0-win32-x64\server` へ（95 ファイル byte 一致）⇒ **ユーザーは Reload Window が 1 回要る**（package.json は動かしていないので 2 回は不要・記憶 `feedback_lilysharp_lsp_deploy_target`）。

⇒ ★★★ **㉔ 次の一手＝✅ ⒜（LP の語彙をどこまで入れるか）は第 18 便のユーザー決定「両方とも入れて」で閉じた**（打楽器 30→63・調弦 7→32 語）。**残った札は 3 枚**: **⒜′ `InstrumentDefaults.GetTuning` は preset 5 族しか答えない**（`violin`／`cello`／`viola`／`mandolin`／`banjo` の preset を書いた part を tab で描くと**ギターに落ちる**）＝**調弦は在るのに preset が繋がっていない**。**触っていない理由は 1 つ**——**既存の本の tab の絵が変わる**（`instrument violin` ＋ tab の本があれば 6 弦→4 弦）ので、**射程を数えてから**（第 12 便の型）。⒝ **摂動網の残り**（paper キー・chord quality 35・`@` の引数つき形）。⒠ **drummap の `${1:hh}`＝UX 案件**（drum 語彙を出すと親切。`drummap { ▮ }` の本体に語彙の家が無いので context が 1 つ要る）。⒡ **第373 ⑼⒜（probe 24 冊の「量を測ったか／問いに答えたか」の仕分け）**も先頭級のまま。**ヘッダの綴りの島は閉じた**——⑻ の 25 綴りが全数で、**残る未点検はゼロ**（`{` を開く構文は score／staff group 5／part／phrase／form／lyrics／chords／section／fonts・paper・layout とその入れ子／音楽の容れ物で尽きる）。⚠️ **閉じたのは*補完の*読み手だけ**: 同じ「2 語しか読まない枠」を持つ道具が他に居ないかは見ていない（`SemanticTokens`・折り畳み・`BarCountFix` は別の木＝**実構文木**を読むので原理的に無縁、という*予想*まで）。⒢ 第369 ⑸⒝（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。

★★ **㉕ 判定の記録（ユーザーの常設指示「次便は、このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）**: **第 2・5・6・7・8・9・12〜18 便はユーザー起票／指示**（判定の対象外）。★★★ **第 18 便はユーザー決定で着手した脚だが、判定の教訓を 1 つ足した**——**「移植は機械的」と第 17 便が書いたのは*値*の話で、決めることは 4 つ在った**（どの style 表から取るか／mark 語彙が足りるか／LP 自身の dangling 名 `tt` を入れるか／綴りの規則）。**⇒ 「機械的」と書いた次の便は、まず*決める点*を数える。**⚠️ **「どこに写しが在るか」も移植の一部だった**——名前→`TuningType` の switch が 3 軒に在り、**先に 1 軒へ寄せなければ 25 種を 3 度書いていた**（寄せたあとは 1 箇所）。**判定で着手したのは 8 脚**（⑺ tilde・⑻ 掃き・⒁ MIDI 行・⒂ 名前の監査・⒅ 摂動網・⒆ 網の拡張・⒇ LP 照合・㉑ tuning 照合）で、**8 つとも「直前の脚が*何で*断定したか」を道具に採った的**。★★★ **⒇ は「掃いて何も出ない」が*成果*だった脚の 2 つ目**（1 つ目は第 13 便）——**ただし今回は網に翻訳できた**（LP の行を checked-in の test に）。**⇒ 空振りの掃きは、次に誰かが同じ疑いを持つ前に*固定*して返す**。★★★ **⒆ で初めて「網を建てた次の便が獲物を獲った」**＝`hhs`。**網は建てた便では何も釣らず、次の便で釣れる**（第 14 便は 136 本すべて緑だった）。★★★ **基準はこの 1 行＝「どの道具が温まっているか」ではなく*直前の的をどうやって見つけたか***（第 3 便で 1 度書き間違え、第 13 便で 1 度守り損ねて 40 分落とした——**2 度とも同じ形**）。⚠️ **仕掛け線を張ったら必ず毒で赤を見る**（第 8・14 便は入れた。入れない網は「在るつもり」になる）。⚠️⚠️ **冷えた便が §1 の見出しだけで着手すると*逆に壊す***——⒁ は「穴」ではなく死語だった（埋めていたら何もしない綴りを補完に足した）／⒂ は `form` が*逆向き*だった（在ってはいけない名前を書く側）。★ **1 便だけ shell 切断で「有利／不利」ではなく*実行可能か*が判定を決めた**。**着手しなかったもの**: ㉒⒜ は**ユーザー決定 2 件**（語彙をどこまで入れるか＝承認要）＝着手できない。㉒⒝ は**同じ機械の続き**で冷えていても進む／㉒⒡⒢ は**別 regime**（LP を回す／設計級）＝次のセッションでやっても損しない。★ **この便は「照合して差がゼロ」を 2 表で出した**——**空振りに見えるが、次に疑う人の 40 分を先に払った**（第 13 便の教訓の実践）。★ **便ごとの逐語は `HANDOFF-ARCHIVE.md`「第374セッションの『判定の記録』逐語」**（20,000 字の門で出した）。

## 以下は第373セッションの経緯

最終更新 第373セッション（2026-09-12）＝**入り方が第298〜第372 と違う**。ユーザーは `docs/HANDOFF.md` を読ませず、**実コーパスの 1 冊を名指しで起票した**（`scratch/ベースタブLy/accidental.lys` の section A 3 小節目＝「`gis,, ais,,` の間には `#` が在るのに、`ais,, gis,,` の間より*狭く*見える。不正なレンダに思える。ly 双子を作って調べて」）。⇒ **§0 の裏取りは走らせていない**（開始時に在るのは git だけ＝HEAD `382bfd6e`・木 clean・未追跡 0・未 push 5）。道具の指示は無し＝記憶どおり pwsh MCP。**骨は 10**:

★★★ **⑴ 起票は欠陥ではなかった——Lily# は段内 22 grob 全部 LP と一致**。`lysc ly` で双子を出し（手書きしない）、`after-line-breaking` の dump を挿して LP 2.26.0 に通した（`scratch/p374/acc.ly`／`acc-probe.ly`／`lp.log`）。**符頭 17 個・休符列 5 個の x が SVG の 2 桁で完全一致**（`68.014707→68.01`／`72.618907→72.62`／`77.639796→77.64` …）。⚠️ **LP の起動は `cmd /c "… < NUL"` でデタッチ**（直起動は Guile 初期化でデッドロック＝記憶 `reference_lilypond_mcp_console_hang`。**この便も最初に 1 度踏んで 7 分溶かした**）。

★★★ **⑵ 起票の前提は「どちらの間隔か」で裏返る**。**符頭→符頭**なら ① `gis→ais` 3.154200 ／ ② `ais→gis` 3.027580 ⇒ **① の方が広い**。**白（ink の隙間）**なら ① 0.400000 ／ ② 1.723380 ⇒ **① の方が狭い**——ユーザーが見たのは後者。**両方言わないと答えにならない**（片方だけ言うと「起票は誤り」か「起票は正しい」かのどちらか一方に倒れる）。

★★★ **⑶ 白が狭い理由＝*臨時記号はばねに寄与せず rod としてしか効かない***。① は rod ちょうど: `1.304200(符頭) + 0.400000 + 1.100000(♯幅) + 0.350000 = 3.154200`。素の 8 分のばね 3.027580 を **0.126620 しか超えていない**のに ♯ 自身が 1.85 食うので白が 0.4 しか残らない。**陽性対照は同じ小節の `♮`**（幅 0.666600 ＝ rod 2.720800 < ばね ⇒ 余り 0.306781 が `♮` 手前の白になる）＝**臨時記号の幅で見た目の詰まりが変わるのは LP の設計**。記憶 `reference_lilypond_accidental_widens_only_by_rod`。

★★★ **⑷ 副産物＝bar 1 の 8 分 3 歩のうち 1 歩だけ 0.250000 短い**（3.027581 / **2.777581** / 3.027581）。ユーザー指示「追って」。**同じ本を 4 通り描き分けて特定**（`scratch/p374/acc-probe2.ly`: BASE ／ `same-direction-correction=0` ／ `stem-spacing-correction=0` ／ 両方 0）＝**`same-direction-correction` を 0 にしたときだけ 3 歩が揃う**（`stem-spacing-correction=0` では凹みが残る）。⇒ **0.25 は `NoteSpacing.same-direction-correction`**。

★★★ **⑸ 規則を分岐ごとに実測した**（`scratch/p374/sdc.lys`＋`sdc-probe.ly`・ragged-right・半音符 11 対。素のばね 4.275445・補正 0 では **22 個すべて 4.275445**）: **符尾が同じ向き**＋**符頭の譜位置が 1 を超えて離れている**ときだけ ±0.25／**上行で広がり下行で縮む**（Δ2 も Δ3 も**同じ平らな 0.25**）／**右列に臨時記号があると掛からない・左列はゲートしない**／左の旗は全分岐を殺す。⇒ **bar 3 が無補正だった理由も確定**（`ais→gis` は Δ=1・`gis→ais` と `gis→a` は右列に臨時記号）。**Lily# は 22 個すべて絶対 x まで一致＝移植済み・コード変更ゼロ**（`SpacingRules.Springs.cs CalculateStemCorrection`）。

★★★ **⑹ 第 1 便＝台帳 11 点**（ユーザー指示「台帳に入れて」・commit `6116436e`）。新 probe `audit/lp-geometry/probes/same-direction-correction.ly`＝**5 冊 SDN/SDU/SDD/SDG/SDL**（scratch の 11 対を**台帳の紙で 1 段に収まる大きさに割り直した**。treble・半音符・全符尾 UP を `@stemUp` ゼロで作る＝旗ゲートも踏まない。1 小節 2 音なので臨時記号が次に届かず、ゲート冊をその対照の隣に置ける）。**`note-spacing.same-direction.*` 11 点とも exact**。★ **SDG/SDL は README「両側を足す」の形**——ゲートは**右列しか読まない**ので、「どちらかの列」と書いた移植は **SDG は通って SDL で落ちる**。⚠️ `audit/lp-regression/status.json` の `spacing-stem-same-direction.ly` の `pending` は**動かしていない**（あれは上流の本を .lys に書き直すキューの札で、この 11 点はその本の移植ではない）。

★★★ **⑺ 第 2 便＝knee 腕 7 点**（ユーザー指示「次便は、このセッションでやる方が有利なら着手して」＝下 ⑼ の判定・commit `140df989`）。**同じ method のもう 1 本の生きた枝を誰も見張っていなかった**: `probes/beam-column-spacing.ly` は 2026-08 に **6 冊で knee 項を実測し（head 1.304200 − 符尾 thickness 0.130000 = 1.174200）、台帳点をひとつも作っていない**。⇒ 双子 **KNA/KNB/KNC/KND** と `note-spacing.knee.*` **7 点・全部 exact**。A は自力で knee し（3.678400 対 自分の同向 2.504200）／**B は全列に加線・C は加線ゼロで LP は 2 冊を gap ごとに同一に読む**（probe が「加線が列を広げる」を殺した対）／D は 1 つの音高に両符号を強制し、**中央の 1.804200 は補正ではなく rod（head 1.3042 + 0.5）＝下→上の項が飽和する床**。⚠️ **probe の末尾の注記が stale だった**（「Lily# はこの枝を移植していない・`KneeSpacingCorrection` は誰も読まない」）——`KneeCorrection` は在り `note-spacing.cc:120`/`:131`・`stem.cc:909-913` を字面で引いている。**消さずに書き換えた**（注記が*当たっていた*半分＝「観測者がゼロ」は生き続ける）。⚠️ E/F/G は **`knee-spacing-correction` そのものを動かす冊**＝Lily# に綴りが無いので**対にできない・意図的に未 pin**。⚠️ **`audit/property_coverage.csv` は今も `knee-spacing-correction` を "Mention" と分類する**＝生成物の stale（再生成する便への札）。

★★★ **⑻ 数**: **開始時**: HEAD `382bfd6e`・未 push 5・木 clean・未追跡 0（**`Session-Check` は走らせていない**＝台帳 825／exact 650／full 7792 は第372 の記録を引いた値）。**終了時**（`Session-Check -Scratch p374`）: HEAD `140df989`・未 push 7・**台帳 843 点（+18）／ss 非ゼロ 210（不変）／総和 23.216135909（不変）／count 点 180 うち非ゼロ 0／exact 668（+18）／OPEN 0**・**snapshot 249（不変）／追跡 `.lys` 599（不変）**＝**この便は絵を 1 枚も動かしていない**（engine の commit ゼロ・足したのは probe と台帳と test だけ）。**full `scratch/p374/run-final.trx` 7806 / 0 / 4 / 7810**（第 3 便のあと `run-final2.trx` で再確認・同数）（＝第372 末の 7792 ＋ 台帳 18＝**新しい test は全部 Theory の case**）。⚠️ scratch は **`p374`**。⚠️ §7 3.5: 第371 を `-Archive 371` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝第372 の天井 commit `9f6df2a3` が未 push のまま。**push はユーザー（未 push 7）**。

⇒ ★★★ **⑼ 次の一手＝*probe は guard ではない*を全数で数える**。この便は同じ形を 2 回踏んだ（⑹ は「規則に観測者ゼロ」・⑺ は「実測を持つ probe に点ゼロ」）ので、**残りを一度に見る**: `audit/lp-geometry/probes/*.ly` のうち **台帳点がゼロの本は 24 冊**（実測した census: accidental-skyline／beam-column-spacing〔←⑺ で解消〕／beam-grace-score／beam-least-squares／beam-over-stem-book／beam-stem-x／brace-ladder／brace-name-clear／coda-line-start／compressed-line-force／cue-grace-spacing／dynamic-text-x／figbass-accidental-x／glyph-skyline／grace-accidental-font-size／instrument-name-x／jn-line-forces／line-start-key-change／notehead-stem-attachment／pedal-three／rest-column-x／rows-only-page／skyline-binding／ties-slurs-breaks-ragged）。⚠️ **「点ゼロ＝欠陥」ではない**——**問いに答えて役目を終えた本**（falsifier・器材の下見）と**量を測ったのに固定しなかった本**は別物で、**⑺ は後者だった**。⇒ **1 冊ずつ「この本は*量*を測ったか、*問い*に答えたか」を仕分ける**のが次の一手で、**仕分けだけなら LP を 1 回も回さずに読める**（各 probe のヘッダが自分でどちらか言っている）。✅ **第372 ⑻⒝ の face の島は閉じた**（下 ⑾）。**⒝ 第369 ⑸⒝（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。**

★★ **⑽ 判定の記録（ユーザーの常設指示「次便は、このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）**: **第 2 便は着手した**——⑺ は**この便がディスクに残した道具でしか安くない**（LP の note-spacing 計器・`Measure-LilyPondProbe` の流れ・1 段に収まる ragged の枠・`LpGeometryProbes` の書き方が全部温まっていた）うえ、**この便自身が作った未計数**（「同じ method の兄弟の枝に観測者が居るか」は ⑹ を書くまで誰も問うていない）。**着手しなかったもの**: ⑼⒝ は**ユーザー決定**（承認要）・⑼⒞ は**設計級かつ別 regime**（p374 の道具が 1 つも効かない）＝**次のセッションでやっても損しない**。★ **⑼⒜ 自体も*この便では*着手していない**——24 冊の仕分けは**読む量が大きく、LP を回さない＝道具の温度が要らない**ので、**次の便で丸ごとやる方が有利**（この判定を書き残すのが記憶 `feedback_lilysharp_next_leg_only_if_session_tooling_pays` の `How to apply` (c)）。

★★★ **⑾ 第 3 便＝⑼⒝（sans の face の島）をユーザー決定で閉じた**（ドキュメントのみ・出荷物は 1 バイトも動かない）。⚠️ **まず前提を上流で裏取りした**（ユーザー指示「上流 urw-base35 の条項を確認して」）: `ArtifexSoftware/urw-base35-fonts` の `LICENSE` は **AGPLv3 ＋ 例外は「これらのフォントプログラムを Postscript または PDF ファイルに*含める*こと」だけ**＝**第221 が §3 に書いた前提は字句どおり正しかった**（アプリへの同梱には例外が届かない）。⚠️ **フォントファイル自身は何も名乗らない**——name table に nameID 13/14 が無く copyright `(URW)++ … 2014` のみで、**LP の src `v2.26.0` にも記述が無い**（対照: TeX Gyre は copyright 欄に GUST を明記）＝**出典は上流 `LICENSE` 1 本しかない**。★ **私が立てた反論「Lily# も GPLv3 なのだから例外に頼らず AGPL 本体に乗れるのでは」には答えが在る**＝**乗れる（GPLv3 §13 が結合を明示的に許す）が、同じ §13 が AGPL のネットワーク条項を*結合物全体*と再配布・ホストする全員に載せる**。⇒ **ユーザー決定: Heros のまま**。**§3 にセリフ体の行の隣へ 1 行足し**（条項の逐語・波及は `ChordName` だけ・逃げ道が無い理由＝Heros は Nimbus の metric 互換の子孫・約 1e-5 はフォントでは説明できない旨まで）、**台帳 4 点の `why` に `DECIDED 2026-09-12 (user, session 373): KEEP TeX Gyre Heros … PERMANENT … do not chase it` を追記**（`page.chord-row.staff-to-chord-baseline` −0.013091398／`lyrics.chord-verse-row.staff-to-chord` +0.015643317／`lyrics.lyric-chord-run.lyric-to-chord` −0.027738736／`lyrics.chord-row.leading.superscript.system-floor` −0.027839505＝**合計 0.084 ss が恒久**）。⇒ **台帳の「決定待ち」は 0 本**。
