...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第422セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `b32b31c0`・**未 push 24**・**full `sessions/p422/run1.trx` 8739 / 0 / 3 / 8742＝第421 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 76 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第421 が予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 420`＝23 行 6,697 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」の 4 つを AskUserQuestion に出し、**ユーザーが ⒴⁗「1 度目の配置 `p1.prelim` 13.96% を測る」を選んだ**。★ ベンチは着手前に 2 度とも AskUserQuestion で静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第421 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、**before 側の打鍵 8,231,319 B は第421 最終の 8,231,325 B と一致**（＝母集団も harness も同じという証拠）:

★★★ **⑴ ⒴⁗ の起票は「捨てられる」と「余分」を取り違えていた。** 起票は「`p1.prelim` は 13.96%、数が変わる 17.9% の打鍵でその全部が捨てられる」だが、実測では ⒜ **`p1.prelim` はもう 11.36%**（第417 の 13.91% から第418〜421 が分母を削った＝§5.0「着手する便が N を数え直す」の 10 例目）、⒝ **捨てられるのはそのうち 2.333%**、⒞ 配置ごと合わせても **7.908%**（`zz.wasted.p1.place` 5.574 ＋ prelim 2.333・**loop が「別の数を選んだ」と言った瞬間に計上**＝`ZzStage.LastSpent` が要った理由）。⇒ ★★★ **そして 7.908% ですら賞金ではない——1 度は必ず置くのだから、消せるのは*2 度目*だけ**＝`p2.place` 2.059% ＋ `p2.prelim` 1.190% ＝ **⒴⁗ の天井は 3.249%**。⚠️ **しかも 2 度目が安いのは 1 度目が店を温めたから**（2 度目は 1 度目の 2.4 分の 1・第417 が hit 率 95.6〜97.2% と測った）＝**1 度目を「見積り専用の安いもの」に替えると 2 度目が冷える**。⇒ 起票の数は **13.96% → 3.249%（4.3 分の 1）** に書き換える。

★★★ **⑵ `ChooseSystemCount` が読むものは*構成で*言えた**（測定ではなく）。扉は 2 つだけ＝`ideal.Systems.Count` と `EstimateMeasureHeights(ideal,…)`、後者が読むのは `Systems`／`Extents`／`Heights`／`Prelim.PagingSkylines`／`BreakerFrame`。**beam も bow も注釈レイアウトも 1 つも読まない**＝「読むのは高さの見積りだけ」は真。⚠️ **だが見積りは注釈から*建つ***（s3 → s4 が extent を上げ、s5 が paging skyline を作る）＝**読まれる段は 0.958%、その入力が 10.389%**。⇒ 「安い prelim」は「s1/s3 を飛ばす」ではなく **別の見積り**（LilyPond は pure extent で O(列)・`EstimateMeasureHeights` の註が自分でそう書いている）＝土台の変更のまま。

★★ **⑶ 予測は 3 つ中 2 つ当たり、外れが島を 1 つ潰した**（§5.0②・probe の頭に先に書いた）。⑴「捨てられる配置は 8〜10%」＝7.908% で当たり。⑵「carry 門は staff 0 で 0 回・全体 10〜40% が拒否」＝**41.5% 拒否・staff 0 は 0 回**で当たり。★★★ **⑶「beams が bow より大きい」は当たったが、その先で外した**——`beam.arm.cross`（cross-system 群で staff 丸ごと memo を捨てる腕）は **全コーパスで 0 回**・memo hit **96.0%**＝**beams 5.538% は*もう実仕事*、この島には戻らないこと**。

★★★ **⑷ 門が「観測者が 1 人も居ない」わけではないが、*ほぼ死んだ腕*が出た**（第420 ⑺・第421 ⑺ の 3 例目の顔）。bow の carry 門 `StaffOffsetsUnmoved` は **staff 1 以上では 1,368 回中 1,336 回＝97.7% が拒否**（staff 0 は 0 回）。prelim が置いた bow はその staff では毎回捨てられ、**最終パスが memo 無しで引き直して 0.595%**（slur 0.577 ＋ tie 0.018・5,552 本）。**prelim 側は per-system memo を通るのに、門が落ちた先は staff 丸ごとの素の再計算**。⚠️ 直すなら「門を staff 丸ごとから system ごとに割る」だが、跨ぎの bow の扱いが要る＝**起票（新規 ⒡）**。

★★★ **⑸ 直し＝別の島を取った**（`26cda72a`・Core 16 ファイル・網 4 本・**数値定数 0・LP 由来の式 0・REF 0／OWN 0**）。計器の第 3 脚が出した島: **`measureToSystem` の表は 1 打鍵に 15.75 回・29,108 回建って 0.755%**（`BuildMeasureToSystemMap` 10,221＝ElementCoordinator 5,035／Annotations 4,010／VoltaBracket 1,144 ほか、双子の `BuildMeasureMap` 15,155、＋ `LayoutPreliminaryStaffBeams` の**手写しが staff ごとに** 3,732）。⇒ **2 つの表を「system 配列そのもの」で鍵付けして 1 枚に**（`ConditionalWeakTable`＝**配列と一緒に死ぬので配置を打鍵より長く生かさない**。RULES §5.3 の CWT の警告は「効能書きの*読み*」の話で、ここの鍵は*1 パスの配列*＝重複が居る範囲そのもの）＋ 手写しを共有の表へ**指し直し**。0.755% → **0.453%**（残りは per-system memo が `ImmutableArray.Create(sys)` を毎回渡す分＝同一性の鍵では原理的に当たらない）。

★★★ **⑹ 健全性は*コンパイラ*が証明した。** 表を共有してよいのは「誰も書き換えない」からで、それを註で主張する代わりに**戻り値を `IReadOnlyDictionary` に変えた**——すると**コンパイラが表の流れ先 27 箇所を全部名指し**、そのどれにも代入が無いことが分かった（indexer への書き込みは全部「各家が*自分の*表を建てている」側）。⇒ ★★ **「書き換えないはずだ」を確かめる最短の道は、型を締めて誰が文句を言うかを見ること。** 入力側も構成で言える＝`SystemLayout`・`MeasureLayout` は get-only の `sealed record`＝同じ配列が 2 つの表を表すことはない。

★★★ **⑺ A/B ＝ −0.2997%**（**8,231,329 → 8,206,656 B**・両側 TC=0・各側 2 回・床 0.0002%）。冊ごと（床 0.0183%）: **−1% 以上 1 冊**・最良 `決戦は金曜日.lys` **−1.10%**・**床を超えて上がった冊は 0**（最も正の冊で +0.0103%＝床 0.0183% の下）。

★★ **⑻ 出力同一は 3 つとも走らせた**（RULES §5.1）: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline はこの便で `b32b31c0` の木から取り直した＝製品 16 ファイル＋網 1 本を `git stash push`）／⑵ **全ページの SHA-256**＝**231 冊 × 8 打鍵＝5,824 ページで差 0 行**（ファイル全体の SHA も一致）／⑶ suite 全緑・snapshot 249 枚不動・台帳不変。**毒 2 本がちょうど狙った網を赤にする**（⒜ 共有をやめて毎回建てる → **網⒜ だけ**赤・`MeasureToSystemHandoffTests` 含む他 6 本は緑／⒞ 表の中身を 1 つずらす → **96 本**赤＝**中身は厚く見られている。観測者が居ないのは*共有*だけ**＝第416 の scratch・第421 ⒞ と同じ「費用しか観測者が居ない」型）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: **⒴⁗ ▶ 測って起票を書き換えた（13.96% → 天井 3.249%）＝未着手。要るのは「行頭 prefix と pure height を配置から持ち回る」土台の変更で、`p1` を安くすると `p2` が冷えることまで込みで設計すること**／★★ **⒡ 新規＝bow の carry 門を system ごとに割る 0.595%**（staff 1 以上で 97.7% 拒否・跨ぎの bow の扱いが要る）／★ **⒢ 新規＝`measureToSystem` の残り 0.453%**（per-system memo が渡す 1 要素配列。**賞金が小さいので起票のみ**）／★★ **⒵⁴ `prefixMarkAnchorX` の解き直し 0.338%**＝**memo は反証済み**（hit 率 0.53%）・**要るのは「行頭 prefix を配置から持ち回る」土台の変更**／★ **⒵⁵ `mm.g.split` 0.163%**（⚠️ `OrderBy` は*安定*ソート）／★ **⒵⁶ 文字の 318,784 回／81 綴り 0.130%**（安い日の仕事）／★ **⒴ `inkFirst`＋`inkLast` 0.53%**・未分割／★ **⒴⁵ artic と trill の remap 辞書は作って捨てている**（天井 0.174%）／★ **⒳″ `Padded` の中に残る複写 0.028%＋`SortAndResolve` 0.15%**（起票のみ）／⚠️ **`g4.core` 1.21% と `p1.s1.beams` 5.538% は*もう実仕事*＝この 2 島には戻らないこと**（beams は cross-system fallback 0 回・memo hit 96.0% をこの便で確かめた）／⒜ **R13⒝ の実機確認**（第404 ⑵）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・未着手・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／LP 双子が要る R7〜R11／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑼ 終了時**: commit 3 本（code `26cda72a`・docs 2 本＝§1 と RULES。**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 1 本・**終了時の未 push 27**（開始時は 24）・**最終 full `sessions/p422/run4.trx` 8743 / 0 / 3 / 8746**（⚠️ **第421 最終の 8739 / 8742 から +4＝この便の網ちょうど 4 本**・`Zz422`／`ZzStage`／`ZZPOISON` の grep は**コードに 0 件**）・§7.5（対 `b32b31c0`＝スクリプトの出力）**Core `+` 107 行／REF 0／OWN 0**（**うち註 37 行・実コード 70 行**＝足したのは表 2 枚の CWT と建て手 2 つ、空の表 2 つ、残りは 27 箇所の**型**が `Dictionary` から `IReadOnlyDictionary` へ変わった 1 行ずつ。**算術は 1 バイトも触っていない**・§7.6 ⒟）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。⚠️ **棚卸しは `docs/APPROXIMATIONS.md` が*行番号のみ*動いた**（2 行・**数字を伏せた差 0**＝RULES §5.4 の既知の型。`audit/magic_constants.csv` は不変）。計器と全文は Lab `sessions/p422/`（`first-placement.txt`＝直し後・`first-placement-before-notes.txt`＝**before 表の控えと、同じ名前に 4 回上書きした失敗の記録**・`ab-{before,after}-tc0-{1,2}.txt`・`hashes-{before,after}.txt`・`Zz422Probe.cs.txt`・`ZzStage.cs.txt`・`zz422-seams-and-change.diff.txt`）。§7 3.5 は**セッションの頭で**済（`-Archive 420`＝23 行 6,697 chars）⇒ **HANDOFF は 447,662 B → アーカイブ後 436,044 B → 今便を書いて 448,626 B＝天井 450,000 まで 1.4 KB しかない**（今便のブロックが 12.6 KB）。⚠️⚠️⚠️ **次便は §1 に 1 字も足せない。`-Archive 421` を回すまで書き始めないこと**（回せば今便のブロック約 11.4 KB が空く）。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 421` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第422 の欠陥ではない。

## 以下は第421セッションの経緯

最終更新 第421セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `3b0e327e`・**未 push 21**・**full `sessions/p421/run1.trx` 8736 / 0 / 3 / 8739＝第420 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 64 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第420 が予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 419`＝25 行 7,590 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」の 4 つを AskUserQuestion に出し、**ユーザーが ⒳′「resolve の*複写*」を選んだ**。★ ベンチは着手前に AskUserQuestion で静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第420 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、**before 側の打鍵 8,410,467 B は第420 最終の 8,410,475 B と一致**（＝母集団も harness も同じという証拠）:

★★★ **⑴ 起票の見積りは 2.4 倍低かった。** 複写は合わせて **打鍵の 2.062%**（`EndBatch` 0.188% ＋ `MergeInternal` 1.874%）で、§1 が持っていた「`EndBatch` 0.17% ＋ `MergeInternal` 推定 0.7%」は **EndBatch が当たり・MergeInternal が 2.7 倍低かった**。⚠️ **外した理由は「1 呼び出しに list は 1 本」と思っていたこと**——`MergeInternal` は **1 呼び出し 3,978 B** を払うのに、入る building は **46.5 本 × 32 B ＝ 1,488 B** しかない。差は**再成長**だ: `new List(_buildings)` が*ちょうどの*大きさで建ち、次の行の `AddRange(other)` がそれを溢れさせて**倍の配列に建て直す**＝**1 呼び出しに配列 2 本**。★ **同じファイルの `ReserveForBatch` が 197 便前に「点は最後の数 % ではなく doubling の段だ」と書いていた**のに、その段は 2 軒隣でもう一度踏まれていた。

★★★ **⑵ 「instance の field か、thread の buffer か」を*測って*決めた。** `EndBatch` は **10,284 呼び出し／distinct な skyline 10,284＝比 1.00**——**`BeginBatch`/`EndBatch` は*構築*の作法**（skyline は 1 度 batch され、あとは読まれるだけ）なので、**instance に buffer を吊るしても、それを使えたはずの唯一の呼び出しが自分で allocate するだけで何も償却しない**。`MergeInternal` は 4.51 呼び出し／instance なので field でも半分は取れるが、**thread に 1 本置けば両方が取れる**。⇒ `[ThreadStatic]`。**retain するのは thread あたり 1 本、その thread が見た最大の walk まで＝コーパス全体で 596 building・19 KB**（実測）。⚠️ 第416 の per-walk の局所と違い、**この buffer は call をまたいで生きるので `Clear` が*正しさ*に要る**（前の walk の building をこの skyline へ resolve してしまう＝誰も見ていない grob の ink が湧く）。

★★ **⑶ 予測は 4 つ中 3 つ当たり、外したのは大きさだけ**（§5.0②・probe の頭に先に書いた）。⑵「比 1.00」・⑶「`AddBuilding` の merge 腕は 0 回」・⑷「`placed` は小さい（0.006%）」は当たった。★ **`AddBuilding` の merge 腕は*構成でも*死んでいる**——private で、唯一の呼び手 `FromBox` は 2 行前に空の skyline を作っている。**測ってから消した**（1,848 打鍵で 0 回）。

★★★ **⑷ 直し 3 本**（`b7f87dab`・Core 5 ファイル・網 3 本・**数値定数 0・LP 由来の式 0・REF 0／OWN 0＝移植は 1 行も無い**。Core `+` 134 行のうち **註 85 行**）: ⒜ **walk の入力を thread から借りる**（`RentMergeInput`／`ResolveFrom`＝`EndBatch`・`MergeInternal`・置き直し merge の 3 か所）／⒝ **`FromPlacedProfile`＝4 つの綴りを 1 本に**（`TextOutlineSkylines`・`TrillWaveOutline`・`DynamicOutline` の private `PlaceResolved` が**逐語で同一**、`SkylineBuilder` に 4 つ目が直書き。4 つとも**置いた building の*配列*を建て、`FromResolvedBuildings` がそれをもう一度 List へ複写**＝1 回の置き直しに 2 本）／⒞ 死んだ腕の削除。⚠️ **健全性は測定ではなく*構成*で言える**: ⒜ は walk に渡る列が**同じ順の同じ列**（置き直し腕は `MergeInternal` が受け取っていたはずの列をその場で作る）／⒝ は置き直しが shift ＋ raise ＝ resolve と可換という**既存の註が言っている不変条件そのもの**／⒞ は到達しない。

★★★ **⑸ A/B ＝ −2.1301%**（**8,410,478 → 8,231,325 B**・両側 TC=0・各側 2 回・床 0.0003%）。★★ **継ぎ目からの予測 −2.13% と実測 −2.13% が一致**＝継ぎ目が正しいものに値段を付けていた。冊ごと（床 0.0137%）: **−1% 以上 103 冊・−2% 以上 63 冊・−5% 以上 20 冊**・最良 `Endless Story.lys` **−19.89%**・★★★ **上がった冊は 0**（**最も正の冊で −0.0024%＝231 冊が 1 冊残らず下がった**）。

★★ **⑹ 出力同一は 3 つとも走らせた**（RULES §5.1）: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline はこの便で `3b0e327e` の木から取り直した＝製品の 5 ファイルだけ `git stash push`）／⑵ **全ページの SHA-256**＝**231 冊 × 8 打鍵＝5,824 ページで差 0 行**（ファイル全体の SHA も一致）／⑶ suite 全緑・snapshot 249 枚不動・台帳不変。

★★★ **⑺ 毒が「観測者が 1 人も居ない腕」をまた 1 つ見つけた**（第420 ⑺ の 2 例目）。`Merge(resolved, dx, dy)` には腕が 2 つある（**batch**＝足して後回し／**非 batch**＝既に resolve 済みの skyline へ）。**非 batch の腕から*自分の* buildings を落としても、8,739 本が 1 本も赤くならなかった**——製品はそこを**打鍵掃きで 2,380 回**通るのに。batch の腕は歌詞と system の builder が何度も通るので厚く見られていて、**pedal と注釈が通るこちらは誰も見ていなかった**。⇒ **網 3 本**（`SkylineMergeTests`）: ⒜ 借りた buffer が前の walk の buildings を持ち越さない／⒝ 置き直し merge が**両方の silhouette** を残す／⒞ 費用の門＝大きい skyline への 1 merge が **3,000 B** 未満（第416 の scratch と同じ「費用しか観測者が居ない」型）。**毒 3 本がちょうど自分の網を赤にする**（⒜ `Clear` を抜く → 網⒜ ＋ 既存 16 本〈★ **suite が 50 秒から 10 分超に膨らむ**＝汚れた buffer が skyline を肥らせる〉／⒝ → **網⒝ だけ**・他 18 本は緑／⒟ 毎回複写に戻す → **網⒞ だけ**を 29,488 B で）。★ 置き直しの raise を落とす毒は **345 本**赤＝そこはよく見られている。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: **⒳′ ✅ 閉じた**（複写 2.06% → 0.00%・置き直しの配列も消えた）。⚠️ **分母が 2.1% 動いた**＝以下の比は第420 までの分母で書かれている。**残り**: ★★ **⒵⁴ `prefixMarkAnchorX` の解き直し 0.338%**＝**memo は反証済み**（hit 率 0.53%）・**要るのは「行頭 prefix を配置から持ち回る」土台の変更**／★ **⒵⁵ `mm.g.split` 0.163%**＝group ごとの `Where→OrderBy→ToList` 2 本（**マークは group あたり 1.09 本**・⚠️ `OrderBy` は*安定*ソートなので畳むなら安定性を保つこと）／★ **⒵⁶ 文字の 318,784 回／81 綴り 0.130%**（安い日の仕事）／★ **⒴ `inkFirst`＋`inkLast` 0.53%**・未分割／★ **⒴⁵ artic と trill の remap 辞書は作って捨てている**（天井 0.174%）／★ **⒳″ `Padded` の中に残る複写 0.028%＋`SortAndResolve` 0.15%**（この便で計器は当てた＝`sessions/p421/resolve-copies-after.txt` に段がある。**賞金が小さいので起票のみ**）／⚠️ **`g4.core` は repair 後 1.21% で*もう実仕事*＝この島には戻らないこと**／★★ **⒴⁗ 1 度目の配置を*安くする*＝`p1.prelim` は打鍵の 13.96%** で、**数が変わる 17.9% の打鍵ではその全部が捨てられる**（未測・土台の変更）／⒜ **R13⒝ の実機確認**（第404 ⑵）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・未着手・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／LP 双子が要る R7〜R11／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑻ 終了時**: commit 3 本（code `b7f87dab`・docs 2 本＝§1 と **RULES §5.3 に 2 項・§5.4 に 1 項**。**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 2 本・**終了時の未 push 24**（開始時は 21。⚠️ **終了時 HEAD の SHA は書かない**＝それはこの文を含む commit 自身で、amend や畳み直しで即座に dead citation になる。第421 が一度書いて `DeadCitationsDoNotGrow` に捕まった）・**最終 full `sessions/p421/run5.trx` 8739 / 0 / 3 / 8742**（⚠️ **第420 最終の 8736 / 8739 から +3＝この便の網ちょうど 3 本**・`Zz421`／`ZzStage`／`ZZPOISON` の grep は**コードに 0 件**）・§7.5（対 `3b0e327e`＝スクリプトの出力）**Core `+` 134 行／REF 0／OWN 0**（**うち註 85 行・実コード 45 行**＝足したのは thread の buffer 1 本と借り／返しの 2 メソッド、置き直しの 1 メソッド、呼びの付け替え。**REF 0 は監査対象ではなく「移植が 1 行も無い」**＝算術は 1 バイトも触っていない・§7.6 ⒟）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。⚠️ **棚卸しは 2 本とも再生成した**（`audit/magic_constants.csv` 92 行・`docs/APPROXIMATIONS.md` 4 行＝**どちらも行数不変・数字を伏せた差 0**＝RULES §5.4 の既知の型）。計器と全文は Lab `sessions/p421/`（`resolve-copies.txt`／`-after.txt`・`ab-{before,after}-tc0-{1,2}.txt`・`hashes-{before,after}.txt`・`Zz421{Probe,Ab,Hash}.cs.txt`・`ZzStage.cs.txt`・`zz421-seams.diff.txt`・`zz421-change.diff.txt`）。§7 3.5 は**セッションの頭で**済（`-Archive 419`＝25 行 7,590 chars）⇒ **HANDOFF は 442,925 B → アーカイブ後 435,335 B → 今便を書いて 447,662 B＝天井 450,000 まで 2.3 KB しかない**（今便のブロックが 11,955 B）。⚠️⚠️⚠️ **次便は §1 に 1 字も足せない。`-Archive 420` を回すまで書き始めないこと**（回せば今便のブロック約 11 KB が空く）。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 420` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第421 の欠陥ではない。
