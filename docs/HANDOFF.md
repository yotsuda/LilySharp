...
★★★ **⑶ で「測定器の宿題」が消え、代わりに起票が 1 件出た**（④⑤）——
**sweep の frame は最初から合っていた**。**間違っていたのは双子の表示モード**（22/23 本）。
**未 push 34**・テスト **3768 passed / 0 failed / 3 skipped**（**+15**）・
...
## ⑤ ★ **起票＝`test/tab-percent-repeat` は描画が壊れている**（**未修正・snapshot 再ベース＝要承認**）

★★★ **16 音すべてが `LYS5002`**（tab の最低弦より下＝オクターブ低すぎ）。
**relative モードのファイルに absolute のつもりで書かれている**（`a,,4\4 … g,,16 a,,\4 …` が
**1 音ごとに 2 オクターブ落ち続ける**）。⇒ **音符が加線 100 本ぶんページ下へ雪崩れる**（png で確認）。
★ **その結果 sweep は加線の行を「譜」と読む**——**reach でまとまった 4 本以上の行が 10 組以上**でき、
**ページが十数個の幻の譜になる**。`-31`・`-80` はその staff 0 に対する `%` 記号の位置。
⇒ **直し方は `octave absolute` を足すか `,,` を減らすか**。**どちらも譜面が動く＝snapshot 再ベース。**
⚠️ **`bar check` にも `不一致 8` にも数えられている本なので、直すとコーパスの集計が動く。**
...
★★ **⑤ の `tab-percent-repeat` を直す**（**要承認＝snapshot が動く**）。**⑥ のゲート ⑹ をどうするかは
その次**（`instrument` を exporter に読ませるかは設計判断）。
