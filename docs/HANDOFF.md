...
- **⑵ ★★★ 第2便＝タブ和音のタイが自分の数字から出て、LP どおりに広がるようになった**
  ...
  **LP は数字の中心から中心へ引いて `whiteout` で抜いている**（実測：LP は頭間 2.787 に対し
  タイ 2.467＝88%、Lily# は軸間 2.995 に対し 0.89＝30%）。
...
- **⑻ ★★★ ▶ ⒳ に残っているのは 1 つ＝数字の白抜き**（下の ▶ に測った理由つきで書いた）。
  **ユーザーは「続けて着手する」と答えたが、私の判断で*このセッションでは着手しなかった***
  （ユーザーが「このセッションでやる方が有利なら着手して／次のセッションでやった方が有利なら
  着手してはいけない」と明示したため）。**判断の根拠は実測**（描画順・既存決定の再開・
  温まっている文脈が効かないこと）。**次の人の島。**
...
▶ ★★★ **⒳ タブの数字が背後を白く抜く**（第180 起票・**⒳⑴⑵ は第180 で閉じた**）。
残っている絵の欠陥は **1 つだけ**——`test/tab-chord-tie` で**中段の弦のタイが、その上の弦の
数字を横切る**。⚠️ **これは配置の誤りではない**：**弦間 1.5 に対し数字のインクは 2.17 高い**
（`TabConstants.FretDigitHeight` ＝ 0.722 × font 3.0）ので、**自分の数字に寄り添う弓は
どの x を取っても隣の段のインク帯の中に入る**。**LP の答えは `TabNoteHead (whiteout . #t)`**
（`scm/define-grobs.scm:3743`）＝ **`stencil-whiteout-box`・thickness 0・blot 0**
（`scm/stencil.scm:831-848`）＝**stencil の extent ちょうどの白い矩形を、その stencil の下に足す**。
- ⚠️⚠️ ★★★ **「白い矩形を足す」だけでは効かない。描画順が壁**（2026-08-16 実測）。
  **SVG のペイント順は フレット数字 12〜17 番目・タイ 19〜24 番目**＝**数字が先、タイが後**なので、
  **数字の下に白を敷いてもタイがその上を塗る**。⇒ **移植には「タブ数字を tie の後段へ回す」
  レンダラの描画順の作り替えが要る**（`DrawTabMeasure` の中で描かれ、`DrawTies` は別 pass）。
- ⚠️⚠️ **既存の決定を再開する**——`TabConstants.FretFontSize` の記録が
  **「不透明な背景は意図的に落とした（弦の線を数字幅で切る `digitGaps` に替えた）」**と書いている。
  **白い箱を戻すかは、どのインクに対して戻すかも含めてユーザー判断**。
- ★ **`digitGaps` は既にある**（弦の線を数字の幅だけ切る）**が、その考えが弦の線にしか
  当たっていない**（実測：`tab-chord-tie` の SVG に白抜き 0 個）。**器は半分できている。**
- ⚠️ **第180 は着手しなかった**——ユーザーの「このセッションでやる方が有利なら着手して／
  次のセッションでやった方が有利なら着手してはいけない」に対し、**上の 3 点（描画順・既存決定の
  再開・温まっている文脈が効かない）を根拠に「次のセッション」と判断した**。
- ★ **参考の数**（第180 実測）: LP は**頭間 2.787 に対しタイ 2.467＝88%**（数字の中心から
  中心へ引いて whiteout で抜いている）。Lily# は**軸間 2.995 に対し 0.89＝30%**
  （数字の縁で止めている）。**見える長さは whiteout を入れても変わらない**——
  変わるのは**隣の数字を横切らなくなること**。
...
| ★ **タブのタイは自分の数字の縁から出る**（第180・LP に対応物なし） | Lily# の数字はジグザグで 2 列に分かれるので、**タイは自分の列の数字の縁から縁へ**引く（`軸 + dx ± 数字幅/2`）。⚠️ **LP には問えない**——**LP のタブ数字は 3 つとも同じ x**（実測 8.82 / 12.951）なので、選ぶべき第 2 の x が存在しない。⚠️ **帰結として単音のタイが短くなる**（`test/tab-tie` で 1.29 → 0.89）。LP は**中心から中心へ引いて `whiteout` で抜く**（頭間 2.787 に対しタイ 2.467＝88%）ので、**同じ絵にするには ▶ ⒳ の白抜きが要る**。**「短いから戻す」で数字の縁を捨てないこと** |
