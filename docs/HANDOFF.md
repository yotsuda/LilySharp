...
最終更新 第200セッション＝**ユーザーの指示で `font` の文法を「1 文書 1 face」から
「役どころごとに face」へ広げ、そのために描画面が family ではなく*役どころ*を話すようにした便**
（**実装 4 便**。**perf は測った＝計算を足したため。alloc は base と完全一致**）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **第199 の §1 ⑹ が候補を 3 つ挙げ、⒜（`font "NAME"` の
2 面問題）を「0.3.0 の門に一番近い／出荷後に error 化は破壊的だから決めるのは今」としていた。**
**その ⒜ に着手して 2 件のユーザー決定を取った直後、ユーザーが
「先に、font 指定の文法を拡張したい。今の文法は、別の場所のテキストのフォントをそれぞれ
指定できない」と述べたので、予約/描画の件を保留してそちらへ回した。**
★★ **ユーザー決定 6 件**（**全部この便で取った**）:
**⑴ 未知の face 名は warning**（error でなく）／**⑵ 予約と描画の一致は「埋め込むときだけ実測」**
（**未実装・下の ⑻**）／**⑶ 綴りは `font { … }` ブロック**／**⑷ 役どころは粗い群＋細かい名で
細かい方が勝つ**／**⑸ フォールバック連鎖を入れる**／**⑹ 記譜テキストは既定の射程から外す。**
★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 518 点・ss 非ゼロ 98・
総和 3.609965521・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 55**・
**snapshot 217 枚**・suite **5361 passed / 0 failed / 4 skipped**
＝**引継ぎの数は全部合っていた**（**30 回連続**）。
★★ **未 push は 7 便続けて両端が繋がった**——**第199 の終了時 55 と開始時 55 が一致**（間で push は無い）。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 61**（**この行を書く commit を含めて 61**。
⚠️ **この行を*書いた時点*では作業ツリー項目 2**＝`docs/` の 2 本が未 commit で、**commit 後の実体は 0**。
★ **`$ahead + 1` は `[int]` を通した**——第198 が文字列連結で `511` を出した罠。
⚠️ ★★ **一度 60 と書いて外している**——**§1 を確定させた*あと*に、下の ⑼（着手見送りの記録）を
1 便足したため**。**§0 の「終了時に数えると差が出る」の実例で、`--amend` で畳んで 61 に揃えた。
⇒ ★ **§1 の数は「最後の commit の直前」ではなく「最後の commit*で*」確定する**——
**§1 を書いたあとに何かを足すなら、その commit に §1 の訂正を同梱すること**）・
suite **5361 → 5387 passed / 0 failed / 4 skipped**（**+26**＝**font の網 23 本＋
PDF resolver の 3 本**）・**台帳 518 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・
**snapshot 217 枚で不動＝再ベース 0 枚**（**挙動を 4 つ変えたのに 1 枚も動いていない。
理由は下の ⑸＝射程 0 冊**）・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Turmeric`）で通せた。**

> ## ★★★ この便の骨＝**ページは「どの face か」を言えて「その文字列が何か」を言えなかった。だから役どころごとに束ねる相手が居なかった**
> **`SharedRenderer` の 37 個の `DrawText` のうち 36 個が literal `"serif"` を渡していた**
> （残る 1 個は `"Emmentaler-Brace"`）。**つまり draw 呼び出しが表現できるのは
> 「同梱 2 面のどちらか」だけ**で、**歌詞に 1 つ・和音記号に別の face を与える指示は
> 束ねる相手を持たない。**
> ⇒ ★★★ **`DrawText` の引数を family から `TextRole` に替えた**——
> **face を知るのは `TextFontPlan.Resolve` 1 軒だけになり、`TextFontDrawingContext`
> （描画の出口で family を書き換えるデコレータ）は役目を失って消えた。**
> ⇒ ★★ **そのデコレータは「タイトル」と「タブのフレット番号」を区別できず、両方を書き換えていた。**
> **区別できなかったのは情報が無かったからで、設計の意地悪ではない。**

> ## ★★★ 2 つ目の骨＝**「黙って受理する」は*片方の綴りだけ*だった。棚は 62 便ぶん半分だけ正しかった**
> §2F は第182 から「**存在しない face 名も黙る**（`font "NoSuchFontFace"` は
> `No errors found.`）」と書いていた。**実測したら `font "NoSuchFontFace" embedded` は
> ちゃんと warning を出していた**——**`FontEmbedWarningValidator` が
> `DiagnosticCodes.FontNotFound` を前から持っていて、`embedded` の門の内側にだけ在った。**
> ⇒ ★★★ **同じ事実を、同じコードが見つけて、片方の綴りでだけ報告していた。**
> **足すべきものは無く、外すべき門が 1 つ在っただけ。**
> ⇒ ★★ **棚の「黙る」は嘘ではなく*不完全*だった**——**§0 が「見つけたと思ったら §2 を grep」と
> 言うのの裏で、*§2 を読んだうえで実コードに当たる*とこうなる。**

> ## ★★★ 3 つ目の骨＝**記憶で書いた LP の引用が間違っていて、直したら設計の裏付けが強くなった**
> `TextRole` の注記に **「LP も同じ 2 層で束ねる（`make-pango-font-tree`）」**と書いた。
> **§7.5 が「+ 1542 行に REF 1 本」と言ったので監査したら、`make-pango-font-tree` は
> 2.26 では翻訳マニュアルの `@funindex` にしか無い**——**古い LP の記憶だった。**
> **2.26 の綴りは `\paper { property-defaults.fonts.serif = "DejaVu Serif" … }`**
> （`input/regression/font-family-override.ly:20-24`）**と grob ごとの `font-name`**
> （`font-name.ly:30-33`）。⇒ ★★★ **つまり LP は*まさにこの 2 層*で、
> この便が足した「群」だけが独自**（しかも葉に解決する糖衣で、第 3 の層ではない）。
> ⇒ ★★★ **記譜の切り出しも LP 側から裏が取れた**——
> **`\override TabNoteHead.font-name`**（`dead-notes.ly:55`）**が LP における
> フレット番号の変え方で、LP の広い設定はそこに届かない。**
> ⚠️ **これは規則を決めた*あと*に読んだ**ので**裏付けであって出所ではない**（そう注記に書いた）。

★ **この便の値段**（**perf は測った**＝§7.9 の「足した例」——**draw ごとの文字列比較が
`Resolve` になった**。**base commit の worktree と A/B して `perf-plain1k` は
full 693.8 MB・打鍵 45.2 MB が両向きで一致**。⚠️ **`perf-v2bow1k` の 643.3 → 642.5 は
この本の 4% の揺れの内側で、どちら向きの証拠でもない**）:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ 描画面を役どころ化＋`font { }` | Core 36 ファイル ＋ 網 6 ファイル | **snapshot 0 枚** |
| ⑵ 網 20 本＋文書 3 本 | Tests ＋ `docs/` 3 本 | — |
| ⑶ 未知 face の warning を門から出す | `FontEmbedWarningValidator` ＋ 網 3 本 | **射程 0 冊** |
| ⑷ LP 引用の訂正・7.7 の畳みに観測者 | `TextRole` / resolver ＋ 網 1 本 | — |

- **⑴ ★★★ 第1便＝`DrawText` が `TextRole` を取る**（`b68eaefc`）。**葉 24・群 6。**
  **解決順は「葉→群→総称 family→同梱」で、狭い綴りが*順序によらず*勝つ。**
  ⚠️ **`font "NAME" [embedded]` の意味は変えていない**（**総称 2 面を束ねる糖衣**）——
  **ただ 1 点、記譜に届かなくなった。**
  ★★ **同時に直った 2 件目**: **PDF の resolver は「設定 face を 1 つ」しか持てず、
  非埋め込みの stand-in が*常に serif* だった**——**和音記号を absent な face に束ねると
  Heros で予約して Schola で描く**。**集合にし、各 face が自分の family で stand-in する。**
- **⑵ ★★ 網は「約束」に当てた**（`b7ac32a2`）。**毒を 3 つ当てて、それぞれ対応する網*だけ*が赤くなった**
  （`IsNotation`→false で記譜の 2 本／群を葉より先に解決で順序の 1 本／連鎖を先頭 1 つに畳んで連鎖の 1 本）。
  ⚠️ ★★ **4 つ目の毒は当たらなかった**（`Update-MatchInFile` が `0 replacement(s)`）——
  **その回の緑は何も言っていない**。**§0 の 5 例目「正しい数を誤った理由で出す計器」の当日版**で、
  **印字を読んだから気づいた。**
- **⑶ ★★ 既存の網 1 本が旧挙動を固定していた**（`02fb0e8a`）——
  `NonEmbeddedFont_ProducesNoFontDiagnostic`。**消さずに書き換え、
  「この網が固定していた規則こそが欠陥だった」を本文に残した。**
- **⑷ ⚠️ ★★ §7.7 で畳みを 1 つ見つけた**（`2ce45043`）——
  **PDF resolver は face *名*ごとに stand-in を 1 つ持つので、1 つの名前を serif と sans の
  両方の役どころに束ねると先に宣言したほうが勝つ。** **注記が最初「観測者は無い」で終わっていたので、
  網を書いた**（**「無い」は記録する文ではなく直す文**）。
- **⑸ ★★★ 射程は 0 冊**——**566 冊に `font` を書く本は 1 冊も無い**（`^\s*font\s+"`＝宣言の綴り。
  **数え方の由来は §2F**）。⇒ **だから挙動を 4 つ変えて snapshot が 1 枚も動かない。**
  ⚠️ **逆に言えば、この文法を観測している本はまだ 1 冊も無い**——**網 23 本が唯一の観測者。**
- **⑹ ★★ 双子は `font` を書かない。決定であって穴ではない**（`LilyPondExporter.EmitHeader` の注記）。
  **LP に対応物はある**が、**書くと LP 自身の幅が動いて双子が対照でなくなる**——
  **Lily# 側は版面が動かないのだから、比較の中にだけ差が生まれる。**
- **⑺ ★★ §2 の頭（perf）は 1 バイトも触っていない。地図と次の島は第193 の §1 ⑷ のまま。**
- **⑻ ★★★ 次に触るならここ**——**§2F の ⒜ は*半分*しか閉じていない**:
  **⒜ 残る本体＝「名指し face は*描かれる*が*測られない*」。ユーザー決定は取ってある
  （⒟＝埋め込むときだけ実測）が未実装**。**着手するなら `TextFontMetrics` に
  「同梱以外の face」の経路を開けるところから**で、**`Advance`/`ShapeRun`/`Ink`/`OutlinePath` が
  全部 `(Sans, Style)` 鍵なので、台帳 518 点が乗っている関数群に触る＝1 便では収まらない見込み**／
  **⒝ §2F ⒣**（**`as numbers` と譜なしの本が*中途の*拍子欄を予約する**・射程 0 冊）／
  **⒞ §2 の頭の ▶ perf**。
  ⇒ ★★ **0.3.0 の門という観点では ⒜ が一番近い**——**`font { }` を出荷した以上、
  「名指し face は測られない」は*出荷済みの挙動*になった。あとで測るようにするのは
  版面が動く変更**（**ただし射程 0 冊なので、今なら誰も壊れない側のまま**）。
  ⇒ ★ **もう 1 つ、この便が見つけて動かさなかったもの**: **SVG の和音記号は
  `font-family="sans-serif"` を出すので*読み手の* sans が描き、予約は同梱 Heros。**
  **同梱名を書けば閉じるが、和音記号を持つ snapshot が全部動く**ので
  **出力についての別の決定**（`SvgDrawingContext.FamilyAttributeFor` の注記に書いた）。
  ⇒ ★★★ **⒜ を採るなら、最初に取るユーザー決定は「実測した face が LP-fidelity の
  経路へ漏れないことをどう保証するか」**——**`TextFontMetrics` は
  「THERE IS NO SYSTEM-FONT FALLBACK, deliberately」と明記していて、
  `b69c73e6` はまさにそれを fidelity プローブから取り除いた便**（**当時 4 つの台帳値が
  fontconfig の選んだ face で汚れていた**）。**つまり ⒟ は「鍵に face を足す」打鍵ではなく、
  *閉じてある穴を、限定して開ける*設計**。**そこが決まらないと、13 site
  （キャッシュ 5＋関数 7＋`FileName`）のどれをどう分岐させるかが書けない。**
- **⑼ ★★★ 第200 は「この便で ⒟ に着手すべきか」を訊かれ、見送った**
  （ユーザーの「有利なら着手／不利なら着手するな」の委任）。**不利と判断した根拠**:
  **⒤ この便が持っていて次便が失うものが無い**——**`scratch\p200\facegap` の実体は
  csproj 398 B ＋ Program.cs 1535 B**（残りはビルド出力）で、**それが出した数は上の §2F に
  逐語で在る**。**取り直し 3 分・しかも取り直す必要が無い**（＝**第197 が「有利」と言えた
  条件の裏**）／**⒥ ⒟ の 1 手目は上のユーザー決定で、セッション終わりに新しい決定を
  取りに行くのは一番悪い時機**（**着手すると半開きの項が残る**）／
  **⒦ この §1 は §7 どおり最後の commit の直前に確定させてあるので、着手すると
  suite 5387・台帳 518・snapshot 217・未 push 60 が 4 つとも崩れ、完了しない仕事のために
  §1 を書き直すことになる。**
  ⚠️ ★★ **急ぐ理由がある側も書く**: **`font { }` を出荷した以上「名指し face は測られない」は
  出荷済みの挙動**で、あとで測るのは版面が動く変更。**ただし射程 0 冊なので
  「今なら誰も壊れない」は当分続き、しかも ⒜ を急がせていた圧の一番きつい所
  （未知 face の黙認）はこの便の ⑶ で抜けている。**

---

## 以下は第199セッションの経緯

最終更新 第199セッション＝**§2G の「Release だけで出る XML doc 警告」を読んで直し、
その報告が出るビルドを既定にした便**（**実装 2 便**。**Core はコメント以外 0 行**
——**perf は 1 バイトも触っていない**）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **第198 の §1 ⑻ が「0.3.0 の門という観点では §2G が一番近い
（出荷する成果物のビルドが出す警告）」と書いており、第197 の ⑿ ⒝ も同じ所を指していた。**
**ユーザー決定 0 件**（**この便も 1 件も要らなかった**）。
★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 518 点・ss 非ゼロ 98・
総和 3.609965521・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 52**・
**snapshot 217 枚**・suite **5361 passed / 0 failed / 4 skipped**
＝**引継ぎの数は全部合っていた**（**29 回連続**）。
★★ **未 push は 6 便続けて両端が繋がった**——**第198 の終了時 52 と開始時 52 が一致**（間で push は無い）。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 55**（**この行を書く commit を含めて 55**。
⚠️ **この行を*書いた時点*では作業ツリー項目 3**＝`docs/` の 3 本が未 commit で、
**commit 後の実体は 0**。**§0 が「終了時に数えると差が出る」と警告しているのはこの紛れ**——
**「いつ数えたか」を書かない終了時の数は、次便が引き算に使えない。**
★ **第197 の「§1 の数を書くのは最後の commit の直前」をそのまま実行した。**
⚠️ **第198 が踏んだ `$ahead + 1` の罠は `[int]` を通して回避した**）・
suite **5361 passed / 0 failed / 4 skipped**（**+0＝網を 1 本も足していない**——
**この便が動かしたのはコメントと csproj だけで、主張を足していない**）・
**台帳 518 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・
**snapshot 217 枚で不動＝再ベース 0 枚**・**Core は 0 警告**
（★★★ **もう「Debug の話」ではない**——**下の ⑵ で Release だけの条件を外した。
62 便ぶん付いていた注釈が消えた**）。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Turmeric`）で通せた。**

> ## ★★★ この便の骨＝**棚は「消えた参照の地図」を約束していた。読んだら、消えていたのは 53 件中 3 記号だけだった**
> §2G は **「少なくとも 84 件（CS1574＋CS0419）は『`<see cref>` の相手がもう居ない』
> ＝リネームで grob 不可視の消費者を取りこぼした地図」** と書いて 62 便寝ていた。
> **53 件を 1 件ずつ読んだら、本当に名前ごと消えていたのは 3 つ**——
> **`OssiaShrink.Y`**（Y-up 統一で dead code として削除・`1bf18bdb`）／
> **`BreakAlignSpacing.FirstNoteSpring`**（`1c962f39`＝行頭を「系ひとつの wish」から
> 「譜ごとの wish の merge」へ変えた便で削除）／
> **`EngravingDefaults.PaperColumnXAlignmentExtent`**（`…Width` が正）。
> **残り 50 は「記号は生きているのに cref が届かない」**だった。
> ⇒ ★★★ **棚の見立ては*種類*を外していた。** そして **3 件はどれも「リネーム」ではなく
> *構造の統一***——**削除の副作用は grep では出ない側にだけ残る。**

> ## ★★★ 2 つ目の骨＝**届かない 50 件の理由が、5 つとも grep から見えない形だった**
> **⒜ 後から足した namespace が doc の中で外側を*遮蔽*する**——
> **`LilySharp.Core.Rendering.Svg`**（fragment cache のために足した）があるので、
> **`Rendering` 配下の doc に書いた `Svg.Collector.X` は `Rendering.Svg.Collector` を探して落ちる**（3 site）。
> **⒝ 省略可能引数を 1 つ足すと、署名を綴った cref が全部黙って死ぬ**
> （`FingeringEngraver.Calculate`・`MultiStaffLayouter.StaffSprings`）。
> **⒞ `#if DEBUG` の中の記号を cref すると、エディタでは解決し、doc を吐く唯一のビルドでだけ落ちる**
> （`VerifyReplayAgainstLiveDetection`）。
> **⒟ 型を間違えている**——**`BeginsCueRegion` は `MusicItem` にあり `NoteItem` ではない**
> （**和音も同じ stamp を持つので、これは意味の誤りでもある**。4 site が `NoteItem` と書いていた）／
> **`OuterEdgeStaffSpaceAtX` は `BeamGroup` ではなく `BeamLayout`**。
> **⒠ ★★ doc と関数のあいだに、あとから別の宣言が挿し込まれる**——**doc は黙って
> *その新しい宣言の*説明になる。4 例**: `CalculateStemEndY` の param 一式が
> `CalculateStemLength` に乗っていた（重複 param 警告の正体）／`LayoutBeams` の doc が
> `ApplyVoiceCollisionShifts` に／`RunPreliminaryAnnotationPass` の doc が `PreliminaryPass` record に／
> `VoicedRestPosition` の doc が `RestStaffPosition` に。
> ⇒ ★★★ **⒠ は「壊れた」のではなく「*嘘になった*」形**——**警告が無ければ誰も気づかない。**

> ## ★★★ 3 つ目の骨＝**62 便寝ていた理由は棚の中身ではなく*構成*だった。誰も建てない構成にしか出ない報告は、無い報告と同じ**
> `GenerateDocumentationFile` が **Release の `PropertyGroup` にだけ**在り、
> **§0 の開始時ビルドは Debug** なので、**コンパイラは 283 件を検出し続けながら
> 誰も見ないファイルへ報告していた**。**第135 が起票し、第197 が再測し、この便が読んだ**
> ——**3 便かかったのは、どれも「棚を読む便」を必要としたから。**
> ⇒ ★★★ **条件を外した**（下の ⑵）。**これで新しい壊れた cref は
> 「Core 0 警告」を印字する*その*ビルドで落ちる。**
> ⇒ ⚠️ ★★★ **そして毒を当てて赤くなることを見せた**（`ThisSymbolDoesNotExist` を
> `SpacingRules.ClefGroupInkLeft` に注入 → **Debug が CS1574 で赤**→戻した）。
> **これをやらなければ §0 の 5 例目「正しい数を誤った理由で出す計器」そのものだった。**

> ## ★★ 4 つ目の骨＝**あいまいな cref（CS0419）は「どちらでもいい」ではない。6 件は間違ったほうを指していた**
> **規則を決めて当てた**: **散文が*特定の呼び*を指すならオーバーロードを名指す／
> *家*を指すなら（「ONE HOME」「every Merge」「the same X model」）`<c>` に落とす**
> ——**片方を黙って選ぶ cref は*嘘の絞り込み*。**
> **そして 6 件は「コンパイラが仮定したのと違うほう」だった**＝**doc が別の関数を指していた**:
> `SKTypeface.OpenStream(out int)` 2 件（**face index はその out からしか出ない**）／
> `GetAccidentalBBox(DesignMetrics, string?)`（**本文が「`font` の design から読む」と書いている**）／
> **`GetTimeSigWidth(string, string)`**（**印字された綴り＝ `MeterGlyphRun` の存在理由そのもの**）／
> `MultiStaffLayouter` が実際に呼ぶ item 版の `LayoutSlurs` / `LayoutTies`。

★ **この便の値段**（**perf は測っていない＝計算を足していない。§7.9 の「足していない例」**——
**コメントと csproj の 2 行だけで、走査は 1 本も増えていない**）:

| 便 | 何が動いたか | 警告 |
|---|---|---|
| ⑴ 壊れた doc を 99 件読んで直す | Core 42 ファイル（**コメント行のみ・コード 0 行**） | **283 → 184** |
| ⑵ doc 検査を既定の構成へ | `LilySharp.Core.csproj` | **Debug でも 0・Release 0** |

- **⑴ ★★★ 第1便＝欠陥カテゴリを全部 0 にした**（`729c5ec9`・**変更 390 行／うちコード 0 行**——
  `git diff` で機械的に確認済み）。**内訳**: **CS1574 26 → 0**（壊れた cref）／
  **CS0419 27 → 0**（あいまい）／**CS1570 18 → 0**（**XML が閉じておらず doc がまるごと落ちていた 8 ブロック**）／
  **CS1734 11 → 0**（居ない引数への paramref）／**CS1587 6 → 0**（doc が何にも付いていない）／
  **CS1572 5 → 0**・**CS1571 4 → 0**。
  ⚠️ **残る 184 は CS1573 178 ＋ CS1591 6 で、これは*不足*であって*破損*ではない**
  （**§2G 自身が「純粋に doc の不足で急がない」と書いていた側**）。
  ★★ **`SystemPlacements` の 4 ブロックは「落ちていた」の実例**——
  **record の*位置引数*に `///` を書いても C# は 1 文字も出さない**（CS1587）。
  **`LyricBands` / `StaffSkylines` / `StaffSpanners` / `StaffInside` の説明が捨てられていた**。
  **`<param>` に直し、8 引数すべてを書いた。**
- **⑵ ★★★ 第2便＝`GenerateDocumentationFile` を Release 条件から外した**（`bb0811ab`）。
  **`CS1573;CS1591` だけ `NoWarn`**（**178 ＋ 6 と数を csproj のコメントに書いた**——
  **消すときは測り直すこと、欠陥を黙らせるために足さないこと、も書いた**）。
  ⇒ ★★★ **これで §0 の「Core は 0 警告」が doc の健全性まで含む主張になった。**
  ⚠️ **`dotnet pack -c Release` が警告 0 で `LilySharp.Core.0.3.0.nupkg` を作ることも確認済み。**
- **⑶ ⚠️ ★★ §2G の数え方に穴があった**（**この便が最初に踏んだ**）。
  **棚は「solution ごと建てると 2 回刷られるので `Sort-Object -Unique` を通せ」と書いていたが、
  それでは足りない**——**MSBuild は片方にだけ `1>` のノード接頭辞を付けるので `-Unique` が効かず、
  素直に数えると *ちょうど 2 倍の 566* になる。**
  **`^\d+>` と末尾の `[...]` を剥いでから unique**（**棚を閉じたので数え方ごと消したが、
  同じ形は他の警告を数えるときにも出る**）。
- **⑷ ⚠️⚠️ ★★★ 第198 は第196 の経緯を ARCHIVE に落とさず*消していた***（§7.3.5 違反）。
  **`19314572^` から回収して 197 と一緒に積んだ**（ARCHIVE は 197・196・195… の順に戻った）。
  ⇒ ★★ **落とす手順を飛ばすと、記録は git の中にしか残らない**——
  **§7.3.5 が「1 便ずつなら 5 分」と書いているのは、まさにこれが起きるから。**
- **⑸ ★★ §2 の頭（perf）は 1 バイトも触っていない。**
  **地図と次の島は第193 の §1 ⑷ のまま**（ARCHIVE ではなく §2 の頭に要約が在る）。
- **⑹ ★★ 次に触るならここ**——**§2G の XML doc 棚は閉じたので、候補は 2 つ**:
  **⒜ §2F の `font "NAME"` の 2 面問題**（**射程 0 冊＝今なら誰も壊れない**・対を 1 冊起こすところから）／
  **⒝ §2F ⒣**（**`as numbers` と譜なしの本が*中途の*拍子欄を予約する**・**射程 0 冊**・
  第198 が「不利」と判断して残した項。**判断の根拠は第198 §1 ⑹ に書いてある**）／
  **⒞ §2 の頭の ▶ perf**（**第193 の地図のまま**）。
  ⇒ ★★ **0.3.0 の門という観点では、correctness の残りは §2F の 2 件しか無い**
  （**どちらも射程 0 冊なので、着手すると「まず対を 1 冊起こす」から始まる**）。
  ⇒ ★★★ **裏返して言うと、門の内側に「本が壊れている」項はもう 1 つも挙がっていない。**
  **次に何を直すかは、ユーザーが自分の本で見つけるものになる可能性が高い**（**第198 がその形だった**）。
  ⇒ ★★★ **⒜⒝ とも第199 が「この便で着手すべきか」を訊かれ、両方読んだうえで見送った**
  （ユーザーの「有利なら着手／不利なら着手するな」の委任）。**不利と判断した根拠**:
  **⒤ この便が持っていて次便が失うものが 1 つも無い**（**診断は全部 commit 済み・`scratch/` に
  取り直しの要る baseline も LP 実測も無い**＝**第197 ⑽ が「有利」と言えた条件の裏**）／
  **⒥ 両方とも「対を 1 冊起こす」から始まり、⒝ はさらに §5.2.1③ で先に LP を測る順序が要る**
  ＝**温まった文脈が 1 つも引き継げず、次便の冷スタートと費用が同じ**／
  **⒦ この §1 は §7 どおり最後の commit の直前に確定させてあるので、着手すると
  suite 5361・台帳 518・snapshot 217・未 push 55 が全部ずれて、正しく踏んだ手順を踏み直すことになる**
  （**特に fixture を 1 冊足すと snapshot が 218 になる**）。
  ⚠️ ★★ **急ぐ理由がある側も書いておく**: **⒜ は「`font "NoSuchFontFace"` を黙って受理する。
  出荷後に error にするのは破壊的変更だから、射程が小さいうちに*決めるのは今*」**と §2F が書いている。
  **ただしそれは「0.3.0 までに」という*便スケール*の圧で、しかも未知の face 名の扱いは
  文法の挙動＝ユーザー決定（§2 D）**なので、**着手しても途中で止まって半開きの項が残る。**
  ⇒ ★ **次便へ**: **⒜ を採るなら、最初に取るユーザー決定は「未知の `font` 名を error にするか
  warning にするか」**——**そこが決まらないと fixture の期待値が書けない。**
...
- 🔶 **`font "NAME"` を指定すると、予約と描画が別の face になる**（2026-07-27 起票／
  **2026-08-18・第200 で*半分*閉じた**）。**残っているのは「測る側」だけ。**
  ...
  - 🔶 ★★★ **残っている本体＝「名指し face は*描かれる*が*測られない*」。**
    **`TextFontMetrics` は同梱 face 以外を測らない**（**システム fallback を拒む注記が
    その理由を書いている＝同じ `.lys` が機械ごとに別の版面になる**）。
    ⇒ ★★ **ユーザー決定は取ってある（2026-08-18）＝⒟「埋め込むときだけ実測する」**
    ——**`font "X" embedded` で PDF に face が同梱されるときだけ、その face で測って描く**
    （**そこだけは決定的**）。**それ以外は提示ヒントのままで、予約は同梱 face。**
    **未実装。** **着手するなら `TextFontMetrics` に「同梱以外の face」の経路を開けるところから**で、
    **1 便で収まらない見込み**（**Advance / ShapeRun / Ink / OutlinePath が全部
    `(Sans, Style)` 鍵で、face を鍵に足すと台帳 518 点が乗っている関数群に触る**）。
  - ★★★ **ずれの大きさは測ってある**（2026-08-18・`scratch\p200\facegap`。
    ...
    **face の差が桁で勝っている。**
  - ⚠️⚠️ ★★★ **SVG では ⒜「常に名指し face で測る」は*原理的に*達成できない**——
    **SVG は face 名を属性で書くだけで、実際にどの face が出るかは*読み手の*機械が決める。**
    **PDF の `embedded` だけが face ごと運ばれる。** ⒟ がその形になっているのはこの理由。
  ...
  - ⚠️⚠️ ★★★ **射程は今も 0 冊**（`^\s*font\s+"` ＝**宣言の綴り**で数えた）。
    ...
    **数え方ごと残す**。⇒ ★★ **だから第200 の 4 つの挙動変更は snapshot を 1 枚も動かしていない。**
