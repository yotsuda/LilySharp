...
最終更新 第211セッション＝**エディタの補完を言語の語彙に縛った便**（**実装 3 便**）。
⚠️ **彫版のコードは 1 行も動いていない**（**snapshot 218 枚・台帳 529 点とも不動**）。
**動いたのは LSP・パーサ・doc・網だけ。**
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **§1 ⑺ ⓪ が「まず 0.3.0 の判断」だったので裏取りのあと訊いた**
（**選択肢 4 つ＝⓪ 0.3.0 へ舵／⒢ LSP 補完／⒜⒝⒞⒡ 小さい残債／⒟ ▶ perf**。
**推薦は ⓪ と添えた**——correctness の門は空・§2F に「決定済み・未実装」0 件・木は clean）。
**ユーザー決定は「⒢ LSP 補完の綴り」。**
⇒ ★★★ **0.3.0 を出すかは*まだ決まっていない*＝7 便続けて未決。次便も同じ設問から始まる**（下の ⑹）。

★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 529 点・ss 非ゼロ 100・
総和 3.794108828・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 60**・
**snapshot 218 枚**・suite **5576 passed / 0 failed / 4 skipped**・**追跡コーパス 567 冊**・
**Core 0 警告**＝**引継ぎの数は全部合っていた**（**40 回連続**）。
⚠️ **`origin/master` は `9830873f`＝開始時と同じなので、便の途中の push は無かった**。
⚠️ ★ **ただし「追跡コーパス 567 冊」だけ*数え方が §0 に書かれていなかった***
——**`git ls-files '*.lys'` が 567**（`audit` 配下だけだと 341・ディスク上の全 `.lys` はさらに別）。
**§0 に足した**（§0 ★ の族＝**数を引き継ぐときは数え方も書く**）。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 66**（**この行を書く commit を含めて 66**。
**数えたのは commit の*あと***＝第210 が 1 度間違えた形。
⚠️ ★★ **1 度 64 と書いて古くなった**——**便を閉じたあとに ⑺ の 2 commit が乗った**ので、
**「閉じたあとに走るなら、閉じの数も取り直す」**（memory の警告そのままの形））・
suite **5576 → 5584 passed / 0 failed / 4 skipped**（**+8**）・
**台帳 529 点で不動**（ss 非ゼロ 100・総和 3.794108828・count 106/2 も不動）・
**snapshot 218 枚で不動**・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Turmeric`）で通せた。**

> ## ★★★ この便の骨＝**「族を数え直せ」を*提案する側*に当てたら、3 件目だけが壊れていた**
> **起票（第210）は 2 件で、どちらも「今は一致しているので現物の欠陥は無い／実害が小さい」。**
> **着手時に「Core の語彙を自前で綴っている補完」を数え直したら 3 件だった**
> ——**3 件目 `GetPartPropertyCompletions` は第210 が数えていなかった**（値の補完だけ数え、
> **名前の補完を数えていなかった**）**。そしてそれだけが*今まさに*壊れていた**:
> **9 property のうち 6 つしか出さず**（`transposition`・`lines`・`pedal` が欠落）、
> **`octave` の Detail が `absolute | relative` と書いていた＝第210 が*その前日にエラーにした*ちょうど 2 語。**
> ⇒ ★★★ **第209（色づけ）→ 第210（検証）→ 第211（補完）で、同じ族の別の顔が 3 便続けて出た。**
> **§5.0 罠22 は「族は*検査する側*を数える」だったが、今回効いたのは*提案する側*を数えること。**
> ⇒ **汎化して §5.0 罠23 に置いた**（「起票の件数は族の大きさではない。**同じ量を綴る側を全部数える**」）。

> ## ★★★ 2 つ目の骨＝**起票の「閉じ方は 1 つ」が*実行できなかった*——読みだけで書かれていたから**
> **起票は「`GetRemoveEmptyCompletions` を `RemoveEmptyValueVocabulary` から作れ」だったが、
> `LilySharp.Lsp` は別アセンブリで、その property は `internal`**
> （`InternalsVisibleTo` は **Tests / Benchmarks / Probe の 3 つだけ**）。
> ⇒ ★★★ **だからこそ*補完だけ*がずれた**——**色づけは Tests から縛れる**（`EditorColouringTests`）が、
> **補完は縛れる場所がどこにも無かった。** **欠陥の分布は偶然ではなく、可視性の形そのものだった。**
> ⇒ **`LanguageVocabulary`（public・Core）を置いて全部そこから作らせた**（`InstrumentDefaults` が
> public なので `instrument` の補完だけ既に正しかった——**その形が正解の見本**と起票も書いていた）。

> ## ★★ 3 つ目の骨＝**5 語の綴りが 4 つあり、doc の網は「5 語であること」しか見ていなかった**
> **音楽中の clef 5 語は ⑴ `SyntaxFacts.IsClefKeyword`（kind の述語）／⑵ パーサのエラー文言の散文／
> ⑶ `GRAMMAR.md` の `ClefName`／⑷ LSP の `Clefs` の 4 箇所に在った。**
> **`DocKeywordListTests` は ⑶ を「5 語で、11 語の部分集合」としか縛っていなかった**
> ——**`tenor` の代わりに `soprano` と書いてあっても緑。**
> ⇒ **`SyntaxFacts.ClefNameVocabulary` が 11 語を `IsClefKeyword` で*濾して* 5 語を出す**ので、
> **⑵⑶⑷ はそれを読む＝綴りが 1 つに。** **`treble^8` は `^` でトークンが割れるので自然に落ちる。**

★ **この便の値段**（**彫版のコードは 1 行も動いていない**。**台帳 529・snapshot 218 とも不動。**）:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ 補完 3 件を Core の語彙から作る ＋ `voice-soprano` | Core 6 ＋ LSP 1 ＋ doc 1 ＋ 網 6 | **エディタ**（出力不変）＋ **1 綴りが書けるように** |
| ⑵ `IdentCont` の `-` は嘘だった | doc 1 ＋ 網 1 | **doc のみ**（挙動不変） |
| ⑶ §7.7 ⑷ の観測者 | 網 1 | **観測のみ** |

- **⑴ `792c5f57`**。**LSP 側で直したのは 3 件**（上の骨 1）。
  - ★ **`clef` は位置で 2 つに割った**——**部品ヘッダ 11 語／音楽中 5 語**（**実測で両方向とも裏取り**:
    11 語は全部ヘッダで通り、5 語の外の 6 語は音楽中で `Expected clef name` で拒まれる）。
    **判定は `IsInsidePartBlock`** で、**part の中の `section` は音楽なので 5 語**（網が主張）。
  - ⚠️ ★★★ **⑵ の「実害が小さい」は*向き*の話で、*大きさ*ではなかった**——
    **エディタは「拒む語を出す」のではなく「11 語のうち 6 語を*どの部品ヘッダでも*隠していた」。**
  - ★★★ **網が本物を 1 件釣った＝`instrument voice-soprano` が書けない**（コンパイラ側の欠陥）。
    **ハイフン継続の門が `IsPartNameKind`＝*借り物の述語***（「この語は part 名になれるか」）で、
    **それが許すのは Identifier ＋ クレフ 4 語 `bass/treble/alto/tenor`**
    ——**`voice-alto`/`voice-tenor`/`voice-bass` が通るのは*たまたま*尾がクレフ語だから。**
    **`soprano` はクレフ語でないので落ちていた**（`KnownInstruments` に在り `GetInstrument` も読むのに）。
    ⇒ **`SyntaxFacts.IsBareWord`（テキストを見る述語）に置き換えた＝ユーザー決定**。
    **実測の射程: 追跡 567 冊で `voice-soprano` は 0 冊・part ヘッダ内のハイフンは全コーパスで 1 site
    （`instrument electric-bass`）＝*今コンパイルできる本は 1 冊も変わらない*。38 プリセット全部が書けるようになった。**
  - ★ **Detail の規律を決めた**: **括弧とバッククォートの中は「書いてよい語」だけ・散文は自由**。
    **網がその語を全部コンパイルする**（`octave` は散文で `absolute`/`relative` に触れられる）。
    **語彙を名指す Detail は Core から join**（**数も語彙の写しなので `clef` は 2 つの Count から作る**）。
  - **毒は 5 つ当てた**——**位置を無視（2 本赤）／`removeEmpty` を自前リストに／property 6 語に固定／
    `absolute|relative` を括弧に戻す／借り物の述語に戻す**。**それぞれ*別の*テストが赤になる。**
  - ⚠️ **網の計器自身が 1 度壊れていた**——**テンプレートの `score "s"` が form を名指しておらず、
    全パターンが「拒まれた」と出た**（既存 `SymbolCaseValidatorTests` は
    `UnknownSymbolCase` だけ見るので、同じ嘘の文書を何便も使っていて気づけない）。
- **⑵ `be5cdcb6`**。**`GRAMMAR.md` の `IdentCont` は `-` を含むと書いていて、一度も本当でなかった**
  （レクサーは英数字と `_` で止まる）。**実測: `part foo-bar { }` は書かれた 4 箇所すべてでエラー・
  同じ文書の `foo2bar` は通る**（対照）。**`InstrumentPreset` の production を書いたら見えた。**
- **⑶ `89f69a4c`**。**§7.7 の ⑷「今それを観測している機械があるか」が*無い*だった**
  ——**散文表が語彙を取りこぼすと、語は出るが Detail が空のまま黙って出荷される。**
  **落とすより出すほうが正しい**（落とすのが clef 6 語を隠した形）**が、誰も見ていなかった。**
- **⑷ ★ perf の判定**（§7.9・**回数で測った**。⚠️ **1 度書き直した——下の「⑺」を読むこと**）:
  **`ScanOpenBlocks` にカウンタを置いて実測した**（計器は測ったあと外した）。**1 回の
  `GetCompletionContext` あたりの走査回数**は: **`AfterClef` は 0**（部品ヘッダ・音楽中とも）／
  **`PartBlock` 2・`ScoreBlock` 5・`MusicBlock` 6・`TopLevel` 6**。
  ⇒ **位置を訊く分が*その枝の最初の* prefix 走査＝0 → 1**。**`AfterClef` は今まで
  *ただ 1 つ走査ゼロの枝*だったので、これは「多い中の +1」ではなく「0 が 1 になる」**
  ——ただし**同じ caret を 1 文字戻した `part vln { ` は既に 2 回走査している**ので、
  **エディタにとって新しい桁の費用ではない。**
  **1 回は caret 手前を 1 度なめるだけ**で、**上限は追跡コーパス最大の本＝318 KB**
  （`perf-fingbeam1k.lys`。**実測: その本の末尾で `part vln { ` は 2 走査 636,842 文字**）。
  ⚠️ **これは打鍵ごとの `PublishDiagnostics` ではなく*明示的な補完要求*の経路**（第182 の族とは別）。
  **語彙は static で 1 度だけ・並べ替えは最大 11 要素。**
- **⑸ ★ この便が*起票した*もの＝無し。** **§2F に「決定済み・未実装」の項は 1 つも無い。**
- **⑹ ★★★ 次に触るならここ**——**⓪ まず 0.3.0 の判断**（**7 便続けて未決**。
  ...
  **⒥ 未 push 66 便ぶん**（**タグだけ先に push すると CI が古い木を建てる**ので **`master` を先に**）。
  ...
  **⒡ ★ 配管の残り 6 site**（`TabStaffGeometry` 3・`FetaTextRun` 3）／
  ✅ **旧 ⒢ LSP 補完の綴りはこの便で閉じた**（`792c5f57`・**letter は空いた**）。
- **⑺ ★★★ 便を閉じたあと、ユーザーの「有利なら着手／不利なら着手するな」に*着手しないと答えた***
  ——**⑹ の項はどれも彫版か、ユーザー決定待ちか、「専用の便から」と引継ぎ自身が書いている項**で、
  **この便が持っていて次便が持たない温かさは無い**（**今回作った計器
  `CompletionVocabularyTests`・`LanguageVocabulary` は commit 済みで次便がそのまま受け取る**）。
  ⚠️ **ただし 1 つだけ*自分が書いた数の修理*をした**——**上の ⑷ は初稿で
  「他の枝は既に 1〜11 回走査している」と書いていたが、その 11 は `grep -c "IsInside"` で
  数えた*ソース上の呼び出し箇所*で、*1 回の呼び出しで走る回数ではなかった***
  （**箇所は 12・`&&` の短絡があるので実際に走るのは 2〜6**）。
  **`ScanOpenBlocks` にカウンタを置いて測り直し、⑷ を書き換えた。**
  ⇒ ★★★ **測って初めて出た本当の形は「0 → 1」のほう**——
  **`AfterClef` は `GetCompletionContext` の中で*ただ 1 つ走査ゼロの枝*だった。**
  **初稿の「多い中の +1」という言い方は、間違った数のせいで*費用を小さく見せていた*。**
  ⇒ ★★ **§5.0 の族**: **測った数と読んで数えた数を、同じ段落に同じ顔で並べないこと。**
  **「回数で測る」（§7.9）は*走る回数*であって*書いてある箇所の数*ではない。**
