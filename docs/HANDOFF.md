...
⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: **⒴″ ✅ fingering 側は閉じた**。★★★ **⒴⁶ 残った script の半分＝`fs.walk` は打鍵の 2.82% で、memo が 1 つも無い**（fingering 側には (staff, system) の memo が在るのに、script 側は毎 pass 全曲を組み直す＝**同じ顔の家が片方にしか無い**・RULES §5.3 の「同じ顔の家」の項。**block の 61% で、この便が測った唯一の残り**）／★ **⒴⁵ artic と trill の remap 辞書は作って捨てている**（第418 ⑷ の内側・**天井 0.174%**＝安い日の仕事）／★★ **⒳′ resolve の*複写*＝`EndBatch` の入力複写 0.17% ＋ `MergeInternal` 自身の複写（未計測・推定 0.7%）**＝call をまたいで生きる buffer が要る／★ **⒵‴ `e4.musicMarks` 1.22%**・未分割／★ **⒴ `inkFirst`＋`inkLast` 0.53%**／⚠️ **`g4.core` は repair 後 1.21% で*もう実仕事*＝この島には戻らないこと**／★★ **⒴⁗ 1 度目の配置を*安くする*＝`p1.prelim` は打鍵の 13.96%** で、**数が変わる 17.9% の打鍵ではその全部が捨てられる**（**「見積り専用の安い prelim」が要るかは未測**・土台の変更）／⒜ **R13⒝ の実機確認**（第404 ⑵＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・⚠️ 未着手・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／LP 双子が要る R7〜R11／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑺ 終了時**: commit 2 本（code `5c3ea285`・docs 1 本＝**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 1 本・**最終 full `sessions/p419/run3.trx` 8734 / 0 / 3 / 8737**（⚠️ **第418 最終の 8732 / 8735 から +2＝今便の網ちょうど 2 本**・`Zz419`／`ZzStage`／`ZZPOISON` の grep は**コードに 0 件**）・§7.5（対 `f4e9f4aa`＝スクリプトの出力）**Core `+` 23 行／REF 0／OWN 0**（**大半は註**＝足したコードは `anyFingering` の 1 行と三項 2 つ・§7.6 ⒟「既存の門を 1 つ増やしただけ」）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。⚠️ **棚卸し `docs/APPROXIMATIONS.md` と `audit/magic_constants.csv` は不変**（定数を 1 つも足していない＝差分で確認・`TheInventoryIsNotStale` も緑）。計器と全文は Lab `sessions/p419/`（`summary.txt`・`fingscripts-{before,after}.txt`・`ab-{before,after}-tc0-{1,2}.txt`・`hashes-{before,after}.txt`・`Zz419{Probe,Ab,Hash}.cs.txt`・`zz419-seams-and-change.diff.txt`）。§7 3.5 は**セッションの頭で**済（`-Archive 417`＝32 行 6,192 chars）⇒ **HANDOFF は 439,550 B → アーカイブ後 433,358 B → 今便を書いて約 443,000 B＝天井 450,000 まで約 7 KB**。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 418` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第419 の欠陥ではない。
