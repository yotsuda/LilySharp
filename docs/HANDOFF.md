...
（HEAD `18d2f28a`・**未 push 73**・**full `sessions/p444/run1.trx` 8774 / 0 / 3 / 8777**・台帳 851 点／
総和 22.584727806・snapshot 249・追跡 `.lys` 609・`-Archive 442` も自動で回って 72 行 4,371 chars）。
**裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のまま＝第412〜第443 が予告）。
道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第443 と同じ。

★★★ **⑴ 計器は「静的解析を C# の overload 解決にやらせる」形にした**（Lab `instrument.ps1`＋
`Zz444.template.cs`）。**全 1,418 の `foreach` の*源*を `Zz444.S(id, EXPR)` で包む**だけ——
`S` に interface の overload 群と**catch-all `S<TAny>(int, TAny)`** を並べると、**具体型は恒等変換で
catch-all に落ちて数えられず、*静的型が interface のものだけ*が interface の overload を取る**
（恒等変換どうしなら構築型が型引数に勝つ）。⇒ **箱を作る site が 1 run でちょうど全部出る。**
**値段も模型ではなく実測**（初回の非空 hit で `GetEnumerator()` だけを測った scope に通す・warm 済み）。

★★★ **⑵ 島は 207 軒 167,098 B／打鍵＝打鍵の 3.26%**（`IReadOnlyList` 族 **70 軒 104,199**／
`IEnumerable` 族 **137 軒 62,896**）。★ **箱の寸法は 1 つではない**——32（`T[]`）・40（`List<参照型>`）・
48・56・64（第443 の腕）。⚠️ **頭の 1 軒 38,378 B／打鍵は `IEnumerable`**（⒬′）。
★★★ **島の顔は「空か 1 件の列を歩くために箱を払っている」**——上位の `items/call` は
1.13・1.00・0.94・0.18・0.10。

★★★ **⑶ 直しは 13 ファイル 36 軒**（Core **+123 −50 行**・**REF 0／OWN 0・新しい幾何の数 0**
＝棚卸しは再生成して **Decision と NearbyRef は 927 行すべて不変**、動いたのは行番号 184 行と式の文字列 2 つ）。
**技は 2 つで、選べるとは限らない**: ⒜ **呼び手の具体型に型を狭める**（第443 ⒜ と同じ）。
⒝ ★★★ **狭められないときは添字の `for` に書き換える**——`allMeasures ?? new[]{measure}` は
**片腕が List で片腕が配列**、`SpringSolver._springs` は `ImmutableArray` を箱で持っている＝**型は 1 つに
決まらない。それでも `IReadOnlyList` は `Count` と `this[i]` を約束するので、箱は歩き方で消える。**

★★★ **⑷ A/B ＝ −1.692%**（**5,121,326 → 5,034,678 B／打鍵**・before はこの便で取り直した＝
第443 最終 5,121,323 と 3 B）。⚠️ **会計 102,858 は A/B 86,649 を 16,209 B（15.8%）追い越した。**
★★★ **正体は escape analysis ではない**——`DOTNET_JitObjectStackAllocation=0` で**両側とも**測ると
5,147,206 → 5,060,587 で **A/B は 86,619＝同じ**（JIT は両側で同じ 25,900 B を stack に載せていた）。
★★★★ **正体は「空の列は箱を払わない」**——**BCL は空の `List<T>`／`T[]`／`ImmutableArray<T>` を
interface 越しに歩くとき*キャッシュされた空 enumerator* を返す**。**空の hit を除いて数え直すと会計 87,569 対
A/B 86,649＝920 B・1.05%** で閉じた（島全体の空の割引は 15,315 B／打鍵）。
⇒ ★★★ **`hits × box` は*上限*。分母は hits ではなく「非空の hits」。**
...
★★★ **⑹ 毒 9 つ——8 つは予測どおり、1 つが*予測を割った*。**（素の木が赤 3 本＝§1 の継ぎ目だけの
状態で回したので**差分で読む**。全文 Lab `poisons*.txt`）: ⑴ 最初の声部を飛ばす **+3 赤**／
⑶ `Fold` の最初の規則 **+98 赤**／⑷ `PositionMaxCommon` の最初の rank **+138 赤**／⑸ `FitFactor` の
最初の avoid 点 **+2 赤**／⑹ `TotalLength` の最初のばね **+126 赤**／⑻ `SolveColumns` の頭 **+240 赤**／
⑼ `RestsEverywhere` の最初の staff **+405 赤**／⑺ ★ **`MaxBlockingForce` を逆順に＝+0 緑**
（控え＝max は順序に依らない＝**出力同一の根拠そのもの**）。
⑵ ⚠️ ★★★ **`PrevailingMeters` の声部を*逆順*に歩く → +0 緑。予測は赤だった。**
⇒ **同じ 1 文の 2 つの主張のうち、観測者が居るのは片方だけ**——註の「変更はどの声部にも居られるので
全部を走査する」は ⑴ が観測者を示したが、**「同じ小節で 2 声部が*違う*拍子を言ったら後が勝つ」には
観測者が居ない**。⇒ ★★ **書き換えの安全は網ではなく*順序同一*が保証している**（⒝ の `for` は順序を変えない）。

★ **⑺ 終了時**: commit 2 本（code `01598a2a`・docs 1 本＝**この文を含むので SHA は書かない**＝§5.4）・**最終 full
8774 / 0 / 3 / 8777＝第443 最終と同じ**・§7.5（対 `18d2f28a`）**Core +123 −50 行／REF 0／OWN 0**・
台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・**棚卸しは再生成**（code に同梱）。
全文は Lab `sessions/p444/`（`prediction.txt`・`instrument.ps1`／`Zz444.template.cs`／`Zz444Leg.cs.txt`・
`site-prices-before.txt`＝207 軒の値段表・`Zz444Ab.cs.txt`／`ab-*-tc0.txt`・`Zz444Hash.cs.txt`・`poisons*`）。
⚠️ **毒の複数行パターンを LF で書くと CRLF の原文に当たらない**＝**`PATTERN NOT FOUND` を「緑」と読まない**。
⇒ ⚠️⚠️⚠️ **天井が本当に無い**: この便を書いた時点で **HANDOFF は 449,4xx / 450,000**。
**`-Archive` が空ける量（約 4.4 KB）は 1 便が §1 に使う量に届かない**＝**次便は §1.1 を 3 KB 以内で書くか、
