...
終了時: **commit 3 本（`c060214d`＝製品＋番人＋bench＋package.json＋CHANGELOG／`54cbd903`＝HANDOFF＝第327 の経緯は `HANDOFF-ARCHIVE.md` 末尾へ逐語／第 2 便＝`extension.ts`＋`LilySharpLanguageServer.Editing.cs`＋CHANGELOG＋この HANDOFF）**・未 push 3・Core／snapshot／台帳／`.lys` 不動・**拡張＋LSP は `Deploy-Lsp.ps1` で 2 度配備（118 ファイル byte 一致・Reload Window はユーザー）**。
