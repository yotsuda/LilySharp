...
最終更新 2026-07-27（第18セッション・**▶0 を閉じた——chords 行が loose 鎖の要素になった**）/
HEAD・ahead 数は §0 で確認すること（⚠️ **ここに数字を書かない**——自己参照で、書いた瞬間から
commit のたびに嘘になる）。**全部未 push。**
**3426 passed / 0 failed / 3 skipped**・Core 0 warn 0 err・
**LP 忠実度 122/154 exact・total |residual| 2.506328 ss / 133 distances・counts 19/21**。

前セッション 3.395467 からの −0.889139 は**まるごと ▶0**（+0.891186 → +0.002047）。
母数（154/133/21）は動いていないので、この 1 件だけの比較は成立する。
...
0. **独立 lyrics 行にインクが無い**。P1 は chords 行にだけ実インクを入れた（lyrics 行の
   スカイラインは**空**）。だから lyrics 行は配置も鎖も band のまま——`SelectInterGroupSpec`
   と `LeadingLinesOfSystem` の両方が**同じ理由で**降りている（1 述語ずつ）。
   ⚠️ **§3 の「意図的乖離だから触らない」ではない。インクが無いから測れない**のであって、
   `LyricEngraver.SyllableUpBox`/`DownBox` はもうあるので、要るのは**行用の probe と点**。
...
**非ゼロで残っている台帳点は 28 点**（数は前セッションと同じ。`chord-row.between-systems` が
+0.891186 → **+0.002047** になっただけで、点が消えたわけではない）:
