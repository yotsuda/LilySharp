...
最終更新: 2026-07-24 / master ＝ **`fc892032`＝tab-key 対（TKC/TKT）を起票（`c0ae2338`）→ staff 集合を 1 述語に統一して +7.45→+0.40・未 push（27 本）**。⚠️ hash は自己参照——**HEAD は §0 で裏取り**。**このセッション（続き×7）＝§1 の ▶ どおり widest-key の対象譜集合の不一致を閉じた**：対で起票（**LP 側が恒等**＝tab 譜は `\remove Key_engraver` で KeySignature grob を持たない・13 grob 全て 15 桁一致）→ Lily# の対の差 **7.050000000 exact＝予測（0.82+6.6+1.15−1.52）が 15 桁で的中** → `SpacingRules.ContributesToKeyColumnWidth` 1 述語を予約と描画の両 walk が読む形へ。**既存 corpus は byte 不変（snapshot 0 枚移動）**・fixture `test/tab-part-key` 新設（+snapshot 1 枚）・**3253 緑**・Core 0 warn/err。**LP 忠実度 54/72 exact・total 0.806267 ss**——増えた 0.8 は**新しく開いた 2 点の正直な残差**で、正体は下記 ▶ の**譜間 merge_springs 平均化**（回帰ではない）。
...
- ⚠️ **本題の外で見つけた実バグ（未修正・ユーザー判断待ち）**: `tab X as numbers` / `chords X as roman` を含むスコアが **CLI で LYS1007「Undefined part: 'numbers'」で落ちる**。`PartReferenceFinder.LastTargetToken` が tab/ossia の**最後のトークン**を part 名とするので、`as` セレクタの語（`numbers`/`roman`…）を part 参照だと誤認する（`RenderSpecParser` は正しく読む＝**検証器だけが食い違う**）。**コミット済み fixture `test/tab-as-numbers.lys` すら `dotnet run --project LilySharp.Cli -- svg` で落ちる**。テストが緑なのは snapshot 経路が `SymbolReferenceValidator` を通さないから。LSP/エディタでも赤波線になるはず。
