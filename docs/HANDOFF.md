...
```
★★ **`LILYPOND-REF` の住所を全件開き直す**（第84セッションの自己監査から。**単独 commit 向き**）。
**1 つの関数を開いただけで 3 本が約 38 行ずれていた**（`CalculateStemCorrectionToBarline`）。
⚠️ **`LpReferenceCitationTests` は原理的に捕まえられない**——**「住所と同じ行に記号名があるか」しか
見ない**ので、**正しい関数名を持った stale な範囲は素通りする**。
⇒ **やるなら「住所の行を実際に読んで記号名と突き合わせる」検査**（半自動でよい）。
**件数が多いので、見つけた分を直すのと同じ commit にしないこと。**
...
```
⇒ ★★★ **次の一手はこの隙間ではなく `SpacingRules.StemSpacingInfo`**——**`IsCue` を見ないので
cue にもフルサイズの符尾範囲を渡している**。**予言は台帳に書いた**（この点は −0.000002340 へ、
他の cue 点は 1 桁も動かない）。
✅ **その移植の前提は測り終えた**（§E・第84セッション）。**渡すべきは「範囲 × magstep」ではない**:
```
⑴ 頭の中心から (7 − shorten) × magstep(−4)      中央線の音で double 一致（4.199736832982911）
⑵ そのあと「譜外の符尾は中央線まで」の床        フルサイズでは不活性・縮めた瞬間に効く
                                                忘れると *cue の常用音域だけ* 0.59 半空間短くなる
⑶ stem-begin-position は magstep で縮まない     0.3724 → 0.18958811988894286（design-13 の値）
⚠ 旗つき 8 分は未分離                           4.039985 を法則として書かないこと
```
⚠️⚠️ **移植は 1 行ではない**: **同じ範囲を skyline も読む**うえ、**描画も cue の符尾を縮めていない**
（`StemCalculator.CalculateStemEndY` は cue スケールを取らない）＝**2 つの綴りが揃ってフルサイズ**。
⇒ **`StemCalculator` に length-fraction を通す**のが本体で、**`StemSpacingInfo` はその結果を読む**形。
**cue の snapshot が動く＝要承認。**
