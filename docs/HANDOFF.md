...
⇒ ★★★★ **次の一手**（**第321 の並びの ⑴ を本便が消費した。差し替えと追加**）: ★★★ **⑴ §2 T7 F12＝note spacing の bar ごとの ideal/min が LP と違う**（`Alone Again`・`Livin' It Up`。**再現 `scratch/p322/fx/alone-intro8.lys`＝LP 4|4|4 / Lily# 8|4**。**便の末尾で数えた**: bar 1-8 の Lily# ideal 133.3／min 69.9 対 LP natural ≈ 87（prefix 除く）／min ≈ 74＝**ideal が 1.35 倍・min が 0.9 倍**。gate（SUM）も loop も正しく自分の spacing を読んでいる。**次の一手は台帳**: `alone-intro8` の bar 1〜8 を bar 種別（2 分＋8 分＋休符／付点 8 分＋16 分／4 分＋8 分）ごとに LP と対にし、どの spring（音価・休符・臨時記号 6♯・小節線）が広いかを 1 本ずつ名指す。LP 側の計器は `scratch/p322/fx/probe-barx.ly`（BarLine と NoteColumn の system 相対 X を刷る）。⚠️ **F2 の直しの前に**: 段署名は spacing に敏感で、F12 を先に閉じないと F2 の snapshot 再ベースをもう一度やることになる）／★★ **⑵ perf＝段数ループを静かな窓で測り直す**（⑹′ で候補 1 個 5 ms → 1.3 ms に落ちた。残るのは `EstimatedSystemDetails` の 200 record 生成と `CalcLineHeights` の再走。`PreviewUpdateBench`/`EditKeystrokeBench` を base worktree `LilySharp-perfbase-7c43` と並べて、**1 サンプル目は捨てる**）／★★★ **⑶ F2＝section 名の箱を LP の RehearsalMark の位置へ**（**ユーザー決定済み・§3 第322**: 拍子記号の列＝`BuildPrefixMarkAnchorX` の Rehearsal 腕と同じ anchor・テンポはその下に縦積み。**台帳の SectionLabel 対照点（意図的 OPEN）を閉じる**・snapshot は `\mark` を持つ本が大量に動くので LP 双子で正当化・`fx4-mark-tempo` が陽性対照）／★★★ **⑷ T6＝スラー復元 138 冊**／★★ **⑸ T7 の C 37 冊を 1 冊ずつ**／**⑹ 以下は第321 の並び**: §2 T6 の残り／`audit/lpreg` 取り直し（**本便で段数が動く本は動くはず＝取り直し前に `REGENERATE.md`**）／§2 U8c の島／§2 U8b／§2 U8 の残り／§2 A/B/D/E／§2 C⑴／§2 G／★ **低優先: 表示オプション `marks stacked` / `marks beside`**（§3 第322・ユーザー決定済み・**⑶ を閉じてから**）。⚠️ **承認待ちは §2 T2 の 1 件**（`Lambada Complicada.lys`）＋**§2 T5 の ⏸ 1 件**（F2 は決定済みで待ちから外れた）。⚠️ **§2 F の開いている項目は第320 の ⒲ 1 件。**
...
- **T7. ▶ 起票（第321）＝絵の突き合わせ（286 冊全数）の結果＝族の表。計器は `scratch/p321/`（§1 ⑴）**
  ...
  | **F12** | 段数ループが**LP より安く**段を併せる／割る本（`Alone Again` bar 1-8 を 1 段に併せる・`Livin' It Up` 末尾の 4,4 を 2,2,2,2 に割る・`First Love` tab／`にんじゃりばんばん`／`未来予想図Ⅱ` の 8,8 → 16） | **得点を並べた**（第322 ⑸）: `未来予想図Ⅱ` は LP 27 段 53.70 / 26 段 54.06（差 0.36）に対し Lily# 56.94 / 56.44（逆向き 0.50）＝knife-edge。`Alone Again`（再現 `scratch/p322/fx/alone-intro8.lys`・LP 4\|4\|4・Lily# 8\|4）は **⚠️ 「LP は 8 小節を置けない」が誤りだった**（第322 末尾で測り直し）: LP は `\paper { system-count = 2 }` でも `\noBreak` でも**その 8 小節を warning 無しで 1 段に置き、しかも natural 幅ぴったり（第 8 小節線 102.24 対 line-width 102.05＝力 ≈ 0）**。LP の自由 run が 2 段を inf と刷るのは LP 内部の癖（未解明・`alone-intro8-{sc2,nb45,def}-lp.*`）で、**LP が 4\|4\|4 を選ぶ本当の理由は得点＝3 段 1.27 < 2 段 1.65**（8\|4 は末尾の 4 小節段が伸びる）。**Lily# 側の根は note spacing の幅そのもの**（`alone-intro8-gate.txt`）: **bar 1-8 の ideal 和 133.3・min 和 69.9・幅 88.7**＝Lily# はこの 8 小節を「67% に圧縮・力 −0.62・罰 −0.91・f² 0.82」と安く見るが、**LP の natural は prefix 込みで 102.2＝この段は LP では*自然幅*で、min はむしろ Lily# より大きい（LP 74.4 対 Lily# 69.9）**。⇒ **Lily# の ideal 幅がこの内容（bass の 2 分＋8 分＋16 分対＋休符・6♯）で LP の約 1.35 倍・min が約 0.9 倍**＝bar ごとの spring の ideal/min の乖離（§2 H の族）。`Livin' It Up`（Lily# 22 段 Σf² 6.97 対 LP 1.89）も同じ向き | 7 冊 9 対（−9 の全部） | ▶ **ループでも gate の SUM でもなく、`SpacingRules` の bar ごとの ideal/min**。閉じるなら台帳で bar 種別ごとの ideal/min を LP と対にする（`alone-intro8` の bar 1〜8 を probe に。LP 側は `probe-barx.ly`＝BarLine/NoteColumn の X を刷る） |
