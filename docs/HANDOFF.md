...
**未 push 57**（**この引継ぎ commit まで数えた値**）・
テスト **3795 passed / 0 failed / 3 skipped**（**+8**）・
...
## ⑨ ★ **起票＝弦の選び方が LP と違う**（**未修正・未調査**）

**`tab-beam-script`・`tab-beam-slope`・`tab-below-range` の tab 譜が残っている理由**。
**LP は開放弦を優先して A を 3 弦で取り、Lily# は 4 弦の 5 フレットで取る**（**LP の dump で確認**）。
⇒ **梁ではなく運指の話**なので、**判定器は `\N` で弦を固定しない本では作れない**。
**先に「LP の弦選択規則」を読んで対で起票すること**（§2 B の作法）。

## ▶ 次の一手

★★ **⑨ の起票＝弦の選び方**（**測定のみ・承認不要**）。**LP の弦選択規則を読んで対で起票する**。
**これが片付くと tab の残り 3 冊が初めて比較の土俵に乗る**（**梁はもう合っている見込み**）。
