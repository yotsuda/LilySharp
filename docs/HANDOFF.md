...
最終更新 第364セッション（2026-09-10）＝**入り方は第298〜第361 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。第363 ⑺⒝ から **②「補完の未露出」＝ユーザー起票 ⑶（「ほかに補完に出していない機能が無いか監査」）の拾い箱**を選び、**監査の項目（`?` 付き）を全部パーサで裏取りしてから**実装した。**製品＋番人＋CHANGELOG＋この HANDOFF を 1 commit（snapshot 0＝承認ゲート無し）・push はユーザー**。**骨は 5**:

★★★ **⑴ 裏取り＝監査 16 項は全部本物・2 項は違った**。実コード: `time none`＝`ParseTimeSignature`／`shuffle`＝`TempoValue.IsFeelWord`／`cue [CLEF] { }`＝`ParseCueExpression`／nav mark は music の `ParseMusicItem` にも在る（`GetArticulationCompletions` の remark どおり）／`q`＝`ParseChordRepetition`／phrase 参照＝`ParseBareVariableReference`／section 本文の `lyrics NAME [sings P] { }`・`chords NAME { }`＝`ParseSectionItem`／`using "…"`＝`ParseUsingDirective`＋`UsingExpander`（**実装済み。GRAMMAR §1 の「'using' is reserved for multi-file support」は古い言い方**）／`drummap { hh: position 6 notehead x }`＝`ParseDrummapDeclaration`／top-level `transpose`＝`ParseTopLevelTranspose`／score header の `transpose`・`pitch`＝`ParseRenderDeclaration`（brace の前）／`staff CLEF PART`・`ossia CLEF PART`＝`ParseStaffRender`／`ParseOssiaRender`・`tab TUNING PART`＝`ParseTabRender`（tuning は `LanguageVocabulary.TuningNames` の 7 語＝standard/guitar/bass/bass5/bass6/ukulele/uke・**`drop-d` は語彙に無い**＝TabDisplayCompletionTests の綴りは架空）／素の part 名＝`ParseMidiPartRender`／lyrics の `[N. ]`＝`ParseLyricVolta`（`~` で番号を隠す・`,`／`-`）／`@feather(accel|rit)`＝`AnnotationValues.Feather`／`@arpeggio(bracket)`＝`IsArpeggioBracket`／`@bend(N)`＝`AnnotationValues.Bend` 1..12。**違った 2 項**: `voice { } { }`（section 本文）は欠落ではない（2 本目は素の `{`＝補完する語が無い）／`@8va` 族は `MusicMarkItem` の別名で、`ExtraSearchTerms` の "8va" で `ottava` に届く＝項目にしない。**意図して見送り 1 項**: chord quality 35 種（7 度×4 形のまま・7×35＝245 行はユーザー判断）。**実装中に見つけた第 2 の欠陥**: **lyrics の本文（section-major の `lyrics w { |`・part-major の `lyrics w { section A { |`）は `MusicBlock` の fallthrough に落ちて音高と `@` を全 syllable で提案していた**（`IsInsideTopLevelLyricsBlock` は stack 1 段だけ）。

★★★ **⑵ 形＝Core は語彙の公開 4 本・LSP は文脈 6 つ**。Core: `SyntaxFacts.KeyModeVocabulary`（9 語＝1 表）→ `KeyModeKinds` を `Lexer.GetKeywordKind` で導出 → **`IsKeyModeKeyword` をパーサ（`ParseKeySignature` の 9 本の `Check`）と「Unknown mode」メッセージが読む**（ClefNameVocabulary と同じ型）／`SyntaxFacts.NavigationMarkVocabulary`（10 綴り＝form と music の 2 popup が 1 表を読む）／`TempoValue.FeelWords`（`IsFeelWord` がそれを読む）／`LanguageVocabulary.KeyModes`・`NavigationMarks`・`TempoFeelWords`・`GrobOverrideSpellings`（＝`SupportedGrobOverrides.Spellings` の転送）。LSP: `CompletionContext` に **`AfterScoreHeader`**（`IsScoreHeaderPosition`＝行スコープ・`score NAME ["…"] [transpose P] [pitch M]` で `{` 未到達・末尾が `pitch`/`transpose` なら各自の文脈へ）・**`AfterTransposePitch`**（空リスト＝`transpose |` は音高を打つ・top/part/score の 3 軒共通）・**`AfterTabRef`**（part＋tuning・`tab TUNING |` も）・**`AfterStaffClefRef`**（`staff CLEF |`・group 内も＝part＋`as` selector）・**`BarePartNameList`**（condensedStaff／combinedStaff＝clef を出さない・旧 AfterStaffRef から分離）・**`LyricsBody`**（`IsInsideLyricsBody`＝最内が lyrics frame（`sings` prefix 込み）か、最内 section の外が lyrics frame・1 段は従来の `LyricsBlock`）。リスト: `GetScoreHeaderCompletions`（`{ }`・`""`・transpose・pitch）／`GetStaffRefCompletions`（part＋5 clef＝`ClefNames`・retrigger）／`GetStaffClefRefCompletions`／`GetTabRefCompletions`／`WithMidiOnlyParts`（ScoreBlock の末尾に素の part 名・sort `9z`）／`GetLyricVoltaCompletions`（`[1. ]` `[2. ]` `[1-2. ]` `[~1. ]`）／music に `cue { }`・`q`・nav 10・phrase 参照（`GetMusicCompletions` の 6 番目の引数 `phraseNames`＝`DeclaredNamesOf(text,"phrase")`）／drum list にも nav／section-major の `GetSectionBlockCompletions` に `lyrics ${1:words} sings ${2:最初の part}`・`chords ${1:prog}`（sort `8a`/`8b`＝cell の後・directive の前）／top-level に `transpose`（singleton）・`using "$0"`・`drummap { … }`／`time` に `none`／`tempo` の feel 行は `TempoFeelWords` から生成／`@feather` に accel・rit／`@bend` に 3・4／`arpeggio(bracket)`。**手書き表 2 つを Core から**: `KeyModes` 表 → `KeyModeDetails`（prose だけ）／`RenderedGrobProperties` 4 行 → `GrobOverrideSpellings` から生成（`GrobPropertyDetails` は property ごとの kind と prose・順は color→transparent で従来どおり）。⚠️ 名前の罠: `DeclaredNames` は `SemanticTokens.cs` に型が在る＝`DeclaredNamesOf`。

★★★ **⑶ 番人＝`CompletionAuditTests` 新規 +40**（文脈 26 caret＝`TheNewPositions_GetTheirOwnContext`・**行ごとに置いた位置で compile**＝top-level 3・time 全 label（top と music・`time none` は小節を閉じない）・tempo 全形・music の cue／`cue bass`／`q`／phrase／nav 10（form でも）・section の 2 cell・score header 3・staff の 5 clef（staff／ossia）・tab の 7 tuning・MIDI-only・lyrics volta 4（section-major／part-major）・KeyModes＝語彙＋`IsKeyModeKeyword` の両方向（`Enum.GetValues<SyntaxKind>().Count(IsKeyModeKeyword) == 9`）＋`key c Major` の拒否文が 9 語を名指す・override 全 pair（override＋revert）・feather／bend／arpeggio(bracket)）。**既存 4 本を直した**: `ValueContextCompletionTests`（`tab ` → AfterTabRef）・`ChordDisplayCompletionTests`（bare group → BarePartNameList）・`SectionNameCompletionTests`（cell の並びに lyrics/chords）・`MusicCompletionFlatSpellingTests`（bar 表に cue／q／nav＝`coda4` を音高と読んだ）。`AtCompletionParsesTests` は `arpeggio(bracket)` と feather／bend の新 arg をそのまま通した。

★★★ **⑷ 数と射程**: **開始時**（`Session-Check -Build -Test -Scratch p365`）: HEAD **`405a83b1`**・未 push 15・`origin/master`＝`ba70753f`・木 clean・未追跡 0・`gh run list` **赤**（34416221197＝第361 ⑷⒝ の赤・以後 push 無し）・build 0 エラー／Core 0 警告・**full `scratch/p365/run1.trx` 7615 / 0 / 4 / 7619（第363 末 run6 と一致・RunInfos は SKIP 4 だけ）**・台帳 816・snapshot 249・追跡 `.lys` 599・ディスク 21827。**終了時**: **full `run2.trx` 7655 / 0 / 4 / 7659（+40＝番人・APPROXIMATIONS は stale にならず）**→ HANDOFF／CHANGELOG の後に **full `run3.trx`＝下 ⑻**。§7.5（作業ツリー・`git diff --numstat -- LilySharp.Core`・`-DiffBase` は commit 前なので 0 と出る）**Core `+` 67／REF 0／OWN 0**（語彙の公開だけ・式は 1 本も無い＝LP から導いた量は無い）。台帳 816／snapshot 249／追跡 `.lys` 599 不動（出力を変える変更は無い＝掃きは回していない・補完だけ）。CHANGELOG 1 節（Editor）。⚠️ `git worktree`: p362／p363／p364 の wt-base が残ったまま（この便は足していない・`git worktree remove` で外す）。⚠️ scratch は **`p365`**（p364 は第363）。⚠️ §7 3.5: 第362 のブロックを `-Archive 362` で ARCHIVE の先頭へ（済）。⚠️ 記憶: 新規無し（`reference_lilysharp_session_number_vs_scratch` が今回も当たった）。

⇒ ★★★ **⑺ 次の一手**: **⒜ push はユーザー**（未 push 16）。**⒝ 拾い箱（補完の残り＝この便が名指しただけ・全部小さい・道具は要らない）**: ①chord quality 35 種（`ChordStructure`・補完は 7 度×4 形＝行数の判断はユーザー）／②`@8va` 族を項目に（今は検索語で `ottava` に届く）／③手書き表の残り＝`size` 項目・`GetKeyTonicCompletions` 15 語（文法は任意の PitchBase・循環 5 度の選抜は意図）・`GetScoreBlockCompletions`（`ParseRenderItem` の写し・`ChordDisplayCompletionTests` が 12 語を pin）・`SectionHeaderDirectiveItems`・`GetArticulationCompletions`（同期テスト有り）／④`tab NAME |` の `as` 継続（staff／chords にはあり tab には無い）／⑤GRAMMAR §1 の「'using' is reserved for multi-file support」＝実装済みの語を reserved と言う古い文（doc 1 行）。**第 2 便は着手しない**（同じ指示が来たら）: ①〜⑤はこのセッションの道具（scratch/p365 には trx と commit-msg しか無い）と無関係で、次のセッションでやっても損が無い。**⒞ 第363 ⑺⒝③（title 残差 +0.21）・④（`raggedLastBottom`）、第362 ⑹⒝②〜④、第361 ⑺ の並びはそのまま**。

**⑻ 終了時**: **full `scratch/p365/run3.trx` 7655 / 0 / 4 / 7659（HANDOFF／CHANGELOG 込み・RunInfos は SKIP 4 だけ）**→ commit（Core 4＋Lsp 3＋Tests 5＝新規 1＋CHANGELOG＋この HANDOFF＋ARCHIVE）→ `Deploy-Lsp.ps1`（server だけ＝Reload 1 回）。push はユーザー（未 push 16）。
