...
- **U2. ✅ 閉じた（第286）＝和音行が `rit.` を避けるのは 1 段目だけだった**（**ユーザー報告**・
  ...
  **毒（合流を止める）で 223 枚中この 1 枚だけが赤。**
  ⚠️⚠️ ★★★ **残っている大きい問い＝閉じの距離は*内側*でよいのか**。**今日入れたのは
  「text spanner だけ外側から足す」という狭い直し**で、**LP は loose line を
  VerticalAxisGroup の*スカイライン*（＝置かれた外側 grob 込み）に対して配る**
  （`axis-group-interface.cc:860-985`）。**dynamics・script・beam は今もこの鎖から見えていない**
  ——**remark 自身が「NOT MEASURED」と書いていた棚で、今日それを 1 つだけ埋めた。**
  **部屋の外側プロファイル（`placed.StaffSkylines`）はこの呼び出し地点から届く**ので、
  **移すなら独立した便と全数掃き**（§2 A の「予約と描画の統一」と同じ島）。
