...
### 1.1 第437セッション（2026-09-20・YT-DELL2）
...
`/clear` 直後の新セッション。指示は「**HANDOFF を読んで着手**」で、**着手先は §1.0 ⒜ の ⒵⁵**（ユーザーが
選んだ・⒜ の大きい 2 つと ⒨・LP 双子を並べて訊いた）。★ **`-Start p436` の 1 コマンドで §0 が全部済んだ**
（HEAD `47f8ad89`・**未 push 57**・**full `sessions/p436/run1.trx` 8771 / 0 / 3 / 8774＝第435 最終と一致**・
台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・§7 3.5 の `-Archive 434` も自動で回って
69 行 4,515 chars）。★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` の
ままだから＝第412〜第435 が予告したもの）。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第435 と
同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、
**before 7,942,774 B／打鍵は第435 最終の 7,942,820 と 46 B 違い**（＝同じ木・同じ母集団の証拠）。
⇒ **起票は値段どおりに閉じ、予測は*良い側*に外れ、外れ方が「計器のスコープ」の所見だった。**

★★★ **⑴ 起票は正しい値段で売られていた**（Lab `marks-counts-before.txt`）。**`g.split` 25,250,064 B＝
13,663 B／打鍵＝0.172%**（起票は 0.163%）で、**group は 42,328＝22.90／打鍵**・**マークは 46,138＝1.09／group**。
⇒ **2 本の `Where→OrderBy→ToList` は、9 割方「1 本、上」と答えるために group あたり 596 B を払っていた。**
★ 同じ計器が形も言った: **above が居る group 42,312／below はコーパス全体で 16**（第420 の観測と一致）・
**2 本以上ある group は 3,722**で、**うち 3,594 は優先度順が原順と食い違う**＝**並べ替えは実仕事**。

★★★ **⑵ 直しは「器を便に 1 組だけ持つ」**（`1725682a`・Core `+` 71 行〈**うち註 38 行**〉・**数値定数 0・
LP 由来の式 0・REF 0／OWN 0**＝§7.6 ⒟）: ⑴ `aboveMarks`／`belowMarks`／`placedAbove` を**ループの外**に出して
group ごとに `Clear()`／⑵ **1 パスで仕分け**（`Vertical` が Above でも Below でもないものは落とす＝2 本の
`Where` と同じ述語）／⑶ **安定な挿入ソート**（`SortByOutsideStaffPriority`・**厳密に大きい隣だけをずらす**）。

★★★ **⑶ A/B ＝ −0.199%**（**7,942,774 → 7,926,940 B／打鍵**・before はこの便で取り直した・after は 2 回
回して差 5 B）。継ぎ目は **`g.split` 13,663 → 994 B／打鍵（0.172% → 0.013%）**・**`g.placed` 733 → 0**・
**`mm.loop` 29,135 → 13,063**・**`mm.calc` 71,862 → 55,998（0.905% → 0.706%）**で、**歩いた数は 1 つも動かない**
（46,138 マーク・3,722・3,594 とも同じ）。**冊ごとは 231 冊中 230 冊が改善・中央値 −0.207%・最良 −0.881%**
（`All Star.lys`）で、**上がった 1 冊は +0.005%＝同じ木を 2 回回した対照帯（−0.042%〜+0.024%）の内側**。

⚠️ **⑷ 予測は*良い側*に外れ、外れ方が所見だった。** 実装前の予測（Lab `sessions/p436/prediction.txt`）は
**P1 `g.split` 900〜1,100 B／打鍵＝的中（994）**・**P2 `g.placed` ≈0＝的中**・**P6 歩いた数は不変＝的中**だが、
**P3 の net −0.155〜−0.175% は外れた（実測 −0.199%）**。原因は**計器のスコープ**——`g.placed` は
**`new List<>()` の行しか囲んでいない**ので、**その List を後で `Add` が伸ばす backing 配列（別の scope・
2,670 B／打鍵）を 1 バイトも見ていなかった**。⇒ ★★★ **「その形の値段」は、`new` の行だけでなく*器が生涯で
触る場所全部*に継ぎ目を掛けないと出ない。外側 1 段（ループ全体）を添えておけば、内訳の和との差として
*測る前に*見えた**（RULES §5.3 へ 1 本）。

★★ **⑸ 出力同一は 3 つとも**: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline `17fb650e`）／
⑵ **実コーパス全ページ SHA-256＝5,824 行・0 行差**（第435 の `hashes-after.txt` と突き合わせ）／
⑶ suite 全緑・snapshot 249 枚不動・台帳 851 点／総和 22.584727806 不変。

★★★ **⑹ 毒 3 つのうち 2 つは*よく見られていた*——見られていなかったのは「安定性」だけ。**
⑴ **並べ替えを落とす → 70 本赤**／⑶ **buffer を `Clear` しない → 66 本赤**／⚠️⚠️ ⑵ **比較を `>` から `>=` に
する 1 文字（＝不安定なソート）→ 8,774 本のうち 1 本も赤くならない**。**しかし実コーパスは 32 ページ動いた**
（`The Hustle`・`Top of the World`・`おもかげ`・`キミ、メグル、ボク` の 4 冊 × 8 打鍵。`The Hustle` は
1 小節目に `mark()` を 2 つ書く）。★ **同点は稀な角ではなく普通**——優先度表が名指すのは 5 型で残り全部が 1500。
⇒ **網 2 本（`MusicMarkAnchorOrderTests`・差分型）**: ⒜ **同点は原順で積む**（渡す順を入れ替えたら絵も入れ替わる）
／⒝ **優先度差は原順に勝つ**。**毒 ⑵ は ⒜ だけ・毒 ⑴ は ⒝ だけを赤にする**（見分けつき）。
⇒ ★★★ **LINQ を畳むときは、LINQ が*黙って保証していた性質*を列挙して 1 つずつ毒を当てる**（RULES §5.0 へ 1 本）。

★ **⑺ 終了時**: commit 2 本（code `1725682a`・docs 1 本＝この §1 と RULES §5.0 ×1・§5.3 ×1。**この文を
含むので SHA は書かない**＝§5.4）・**未 push 59**（開始時 57）・**最終 full `sessions/p436/run2.trx`
8773 / 0 / 3 / 8776**（**第435 最終の合計 8774 から +2＝今便の網ちょうど 2 本**。`Zz436`／`ZZPOISON` の
grep は**コードに 0 件**）・§7.5（対 `47f8ad89`）**Core `+` 71 行／REF 0／OWN 0＝算術を 1 つも足していない**
・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・**棚卸しは `APPROXIMATIONS.md`
24 行とも*行番号だけ*（数字を伏せた差 0）・`magic_constants.csv` は差 0**（`LILYSHARP_UPDATE_DOCS=1` で再生成）。
計器と全文は Lab `sessions/p436/`（**`prediction.txt`＝実装前の予測**・**`marks-counts-before.txt`／
`marks-counts-after.txt`＝継ぎ目の前後**・`Zz436.cs.txt`／`Zz436Leg1.cs.txt`＝計器・**`Zz436Ab.cs.txt`＝再利用可の A/B**・
`Zz436Hash.cs.txt`＝証明 ⑵ の harness・`ab-before-1-tc0.txt`／`ab-after-1-tc0.txt`／`ab-after-2-tc0.txt`・
`MusicMarkEngraver.cs.after`）。⇒ ⚠️⚠️ **HANDOFF は 437,680 B → 今便を書いて約 446,000 B＝天井 450,000 まで
残り 4,000 B 級**（`Fold-ClosedHandoffItems` の弁は第426 の時点で 0 件・§3 の圧力弁は第409・第410 で使い切り。
**天井は上げない**）。⚠️⚠️⚠️ **次便は §1 に 1 字も足せない＝`-Archive 435` を回すまで書き始めないこと**
（回せば今便の下のブロック約 8 KB が空く。**途中で回すと `HandoffArchiveContinuityTests` が 3 本赤くなる**
＝第410 ⑾）。⇒ **次便は `-Start p437` の 1 コマンドから入る**。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が
push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第436 の欠陥ではない。**作業ツリーは空**。
**push はユーザー**（Lab も）。
