...
★★ **⑸ 第 2 便（常設指示「このセッションが有利なら着手」）＝⑷ ① の住所と設計の制約まで読んで止めた（製品 0）**。**住所**: 描画 `SharedRenderer.KeyChangeGeometry`＝ナチュラルの kern（`NaturalKernPadding` 0.3／0.15）＋ナチュラル幅＋**KeyCancellation → KeySignature の 0.5**（`define-grobs.scm:1944`）＋key のグリフ／予約 `SpacingRules.GetKeySignatureChangeWidth`＝グリフ幅の和だけ（kern も 0.5 も無い・`KeySignatureInkWidth` も和）＝読み手は `BoundaryColumn.Build`（:135・key grob 1 つに畳む）と `SpacingRules.MidMeasureChanges.cs:52 ChangeItemColumnWidth`。**LP は KeyCancellation（staff-bar から 1.0・esw 0..1）と KeySignature（esw 0..1）を別の break-aligned grob として並べる**（`BreakAlignSpacing.GetKeyCancellationSpacing` は表を既に持つ）。**判定＝着手しない**: ⒜ 字面の移植＝column に KeyCancellation grob を足し、幅を描画の歩きから取る＝**kern は staff position＝clef に依存し、`Measure` は clef を持たない**（`SharedRenderer.ResolveClefAt(staff, …)`）＝`BoundaryColumn.Build` の呼び手 5 軒（`BoundaryClefX`・`BoundaryClefAllowance`・`MmrRodMinimumDistance`・`EmptyBar`・`MultiMeasureRestEngraver.BarWidth`）と `ChangeItemColumnWidth` に clef を通す設計が要る／⒝ cancellation 付き key 変更の spacing が広く動く＝承認要／⒞ p390 の計器（`mmr/keysigspace-*`・`sweep/`）は設計が決まってからしか効かない＝次のセッションで始めても損しない。⇒ **要ユーザー判断**（clef の通し方と、KeyCancellation を grob に割るか）。
