...
最終更新 第182セッション＝**直したハーネスを初めて「差分」ではなく「絶対値」で読み、
そこから沈黙と、それを測る計器の欠陥が芋づるで出た便**（**実装 7 便**）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
開始時の裏取りは**全項目を走らせた**——**台帳 516 点・ss 非ゼロ 98・総和 3.609965521・
count 点 106/うち非ゼロ 2**・HEAD `8812b4a5`・**未 push 27・未追跡 0**・
suite **5034 passed / 0 failed / 4 skipped**＝**引継ぎの数は全部合っていた**（**13 回連続**）。
終了時 **実装の最後は `d921add1`**・**未 push は
`git rev-list --count origin/master..master` が言う**・**未追跡 0**・
suite **5062 passed / 0 failed / 4 skipped**（+28＝新しい網ちょうど）・
**台帳 516 点で不動**（ss 非ゼロ 98・総和 3.609965521・count 106/2）・
**コーパス 0/81 と、最後は 0/300**（コーパス 81 ＋ fixture 219 を
**worktree の base ビルドと HEAD ビルドで描き比べた真の A/B**・**3 回とも陽性対照つき**
——最後のは**変えた構文を書く本 2 冊が実際に動く**ことを確かめてある）・
**snapshot 0 枚**（**絵を 1 ピクセルも動かしていない便**）。
★ **ユーザー決定 2 件**（`using` の規則は *warning*・停止 fixture 2 本は *直す*）。
★ **環境＝pwsh の MCP コンソールは結局 2 本になった**（`Sage` ＋ `Thyme`）。
**前半は 1 本で通せた**——**4 分級の sweep 中に読み取りを打たず、Read/Grep ツールで待った**ので
2 本目が開かなかった。**後半、300 冊の A/B sweep の最中に読み取りを打って開いた**
（第180・181 と同じ形で、**規律は効くが、効かせ続けるのは別**）。**次の人は畳んでよい。**

> ## ★★★ この便の骨＝**沈黙の一族は render 項の外にもう 1 つ居て、それが一番大きく落としていた**
> 第181 は「**名前を取る render 項で誰も見ていないものは 0 になった**」を parser を読んで
> 数え切った。**その主張は正しい。だが *render 項* という枠が効いていた。**
> ⇒ **直したハーネスを初めて HEAD で*絶対値*として読んだ**（第181 は前後差分にしか使っていない）
> ——**912 冊中 292 冊が診断を持ち、error を持つのは 19 冊**。**19 冊を 1 冊ずつ潰す**と、
> 14 冊は意図的な陰性プローブ、2 冊は §2F の既知 fixture、1 冊は `repeat volta` の拒否を
> 主張するプローブ、**残る 2 冊が「壊れたまま追跡されていた perf 本」**だった。
> ⇒ ★★★ **その掃除の途中で `using` に当たった**: **`using "typo.lys"` は
> `No errors found.` と言い、出力は using 行を消した本と（data-pos を伏せて）文字単位で一致する。**
> **落ちるのは 1 ファイルまるごと**——part も section も title も tempo も。
> ⇒ ★★ **しかも半分は「うるさく」失敗していた**: 消えた名前をスコアが参照していれば
> `Undefined section` / `Undefined part` が出る——**ただし正しい 2 行を指して、
> 間違っている 1 行目には何も言わない**。**うるさい側も、間違った場所を指していた。**
> ⇒ ★★★ **枠の教訓**: **「N の一覧を数え切った」は、その N の定義の外を数えていない。**
> 第181 の一覧は *render 項* の一覧で、**`using` は宣言でも参照でもなく*取り込み*だった**。
> ⇒ ★★ **そして同じ枠の話が第5便でもう一度出た**——**RULES §5.3 判定法⑶ が
> 「合成した本は着手前にこれを 1 回通せ」と全セッションに命じている `check --pitches` が、
> 多 part の本で*描かれる音楽と違う音高*を言っていた**（⑸）。
> **沈黙を数える計器と、音高を読む計器が、同じ便で 2 つとも直った。**

- **⑴ ★ 第1便＝壊れたまま追跡されていた perf 本 2 冊を直した**（`e4672c43`）。
  `audit/lpreg/perf136-plain250.lys` と `-plain500.lys` が **`open=5 close=4`**
  ——**`section Main` を閉じる `}` が無い**まま `c5f1b8d2` で `scratch/` から移されていた
  （**移した当時 `scratch/` は一度も sweep されていない**＝ハーネスが盲だった時期）。
  **消さずに直した**（RULES §5.1「未使用に見える ≠ 消してよい」）。裏取り＝**両方 `5/5`・
  両方 `No errors found.`・小節数 250 / 500 対 `perf-plain1k` の 1000＝名前どおりの断面**。
  ⚠️ **生きている主張は載っていない**（第136 の ×3.79/×3.95 は第141 が既に retire 済み・
  ベンチはどれもこの 2 冊を読まない＝`perf136` の grep は 0 件）。
- **⑵ ★★★ 第2便＝`using` が読めないファイルを名指したら言うようになった**（`b675b23b`・
  **LYS0028・ユーザー決定＝warning**）。上の骨。**規則の形が決定だった理由**は、
  **沈黙が*設計*として宣言されていたから**——`UsingExpander` の doc が
  「a missing using never aborts the render」と書き、**`MissingFile_IsSkipped` がそれを pin
  していた**。**warning はその契約を 1 文字も動かさずに沈黙だけ消す**（その網は緑のまま）。
  ★ **`using ""` も同じコード**（**どんなファイルシステムの状態でも正しくなり得ない**＝
  同じ沈黙の別の原因。**名前は 1 つでよい**）。
  ★ **家は expander**（validator ではない）——**解決の可否には base path と file reader が要り、
  `SyntaxTree` はどちらも持っていない**。⇒ 診断は `Expand` の overload で返る。
  ⚠️ **黙ったままにする側も測ってある**: **既に取り込み済みのファイル（diamond・cycle）は
  *解決している*** ので警告しない。**read を `Walk` の外へ出したのはこのため**——
  **「読めない」と「もう取り込んだ」はどちらも `Walk` を即 return させるが、誤りは片方だけ。**
- **⑶ ★★★ 第3便＝Problems パネルが、プレビューと同じ曲を読むようになった**（`49aadc2c`）。
  **`PublishDiagnostics` は `doc.Tree`＝*展開前*の木で validator を回していた**のに、
  **`GetSvg`・export・playback は全部 `ExpandUsings` してから**回していた。
  ⇒ **`using` で分割した曲は、それを正しく描いている当の server から
  「その part は無い」と言われていた**（**実測して網にした**——正しい 2 ファイルの曲に対し
  `Undefined section` ＋ `Undefined part`）。
  ⇒ ★★ **⑵ が単独では*編集画面に届かない*理由でもある**（パネルの経路は `using` を
  1 つも解決しないので LYS0028 が出ようが無い）＝**依存があるので同時投入**（RULES §5.1）。
  ⚠️ **展開後の木で検証する代償＝この文書の外に落ちる診断を捨てる**
  （`d.Span.Start < text.Length`）。**取り込んだファイルの本文は*後ろ*に足されるので、
  その診断はこの文書の末尾より先を指す**＝**別のファイルの診断であって、失われた仕事ではない。**
  ★ **perf を先に測った**（§7 ⑼）: **この経路は打鍵ごとに走る**のに
  `HasUsings(doc.Text)` が**毎回フルパースしていた**——**tree を取る overload を足して
  打鍵あたり 1 パース削った**（プレビュー側は元から払っていた分も込みで）。
  **text の事前検査は賢さではなく必要条件**（キーワード token が `using` という語そのもの）。
- **⑷ ★ 第4便＝停止していた fixture 2 本を今の文法へ**（`5d492d5f`・**ユーザー決定＝直す**）。
  `test/multi-movement`（旧 `name = …` ＋ `staff <変数>`）は**3 section / 3 form / 3 score**
  へ書き直した⇒ ★★ **ツリーで唯一の「form が 2 つ以上ある本」になった**（実測：それまで 0 冊）
  ＝`FormDeclarationValidator` が存在理由にしている配置に、初めて観測者がついた。
  `showcase/grammar-2026-06-09` は**期限切れの主張 3 つを*その場で訂正***（消すだけだと
  G1 の見出しが本文と逆のことを言う＝RULES §5.1「削除も主張である」）。
- **⑸ ★★★ 第5便＝`check --pitches` が、ファイルの順序ではなく*ページ*の読み方で音高を答える**
  （`ad358143`）。**RULES §5.3 判定法⑶ が全セッションに「合成した本は着手前にこれを 1 回通せ」と
  名指している計器**が、**RenderSpec 無しの素の collect** を回していた——
  **section 内の 2 つめの part block が 1 つめの相対鎖を引き継ぐ。**
  ⇒ ★★★ **決着は双子でも推論でもなく*順序の摂動***: **part block の順を入れ替えると
  報告は C6 → C4 と動くのに、描かれた SVG は data-pos を伏せて文字単位で同一**。
  **絵が順序に依存しないのに報告が依存するなら、間違っているのは報告。**
  **値も絵で決めた**——和音は y 24.62/23.62/22.62・下の五線 22.12(A3)…26.12(G2)＝
  **五線の内側・加線ゼロ＝C3-E3-G3**。
  ★ **全数で測ってから 2 つ決めた**（300 冊）: **⑴ 最初の score だけでは痩せる**
  （`cue-notes` は全 score で 26・最初だけで 8）**⑵ *書かれた位置*で畳む**
  （畳まないと tab 本が倍・phrase を 3 section から参照する本が 3 倍）。
  ⇒ **97/300 冊で報告が変わり、うち 32 冊は旧コードでは 1 音も出ず error だけだった。**
  ★ **家は `Semantics.ResolvedPitches.ForFile`**（CLI ではなく Core）＝**網が出荷物を見る**ため。
  ⚠️ **出荷しなかったもの**: **2 つの score が同じ音を違う高さに読むときの警告**。
  **実測 0/300 冊**で、**唯一それを作れる構文（score 単位 transpose）自体が壊れている**
  （下の ⑹）ので、**正しい文言をまだ書けない**。
- **⑹ ★★★ 第6便＝score の `transpose` が「ファイルの既定」として数えられていたのをやめた**
  （`de4acbd0`）。**⑸ の起票を追ったら 1 行だった**——`PartTranspose.ReadScoreDefault` の
  フィルタが「part の中でない」だけなので、**score 単位の transpose がそれに一致していた**。
  ⇒ **宣言した score は 2 回掛かって長3度**・**隣の score は頼んでいないのに 1 回掛かる**。
  **同じ 1 行なのでガード 1 つで両方消えた**。**3 綴りが `d`＝長2度で一致。**
  ★ **絵で確かめ、ハッシュが噛み合った**: **`only-up` の新しいハッシュは、base で
  「無変換のはずの plain」が描いていた絵と同一**（＝漏れて 1 回掛かっていた側）。
  **`part-up`・`only-plain`・`transpose-score` はバイト不動。**
  ⚠️⚠️ **毒（ガードを外す）で赤になるのは新しい 4 本だけ——既存 5061 本は 1 本も落ちない。**
  **それがこの欠陥が生き延びていた理由。**
- **⑺ ★★★ 第7便＝自分の perf 自己監査で、打鍵経路に赤の全木走査を戻しかけていたのを止めた**
  （`d921add1`・**ユーザーの「プレビュー速度は重要だ」の問いが端緒**）。第3便が
  **Problems パネル＝打鍵ごとに走る経路**に `ExpandUsings` を足し、その `HasUsings` が
  **`DescendantNodes().OfType<…>()`**（＝赤の全木走査）だった。**`text.Contains("using")` の
  前置検査は実の本を救わない**——**コメントに `using` を含む fixture が 4 冊**ある。
  ⇒ ★★★ **回数で測った**（§5.3）: **plain1k 43,043 対 7（6,149×）・fingbeam1k
  234,030 対 5（46,806×）**。⚠️ **234,030 は RULES §1 が第153 で消したと書いている
  「fingbeam 234k」そのもの**——**あの便の成果を打鍵経路に戻すところだった。**
  ★ **安い綴りが正しい根拠は構造**（統計ではない）: **`ParseUsingDirective` は
  `ParseTopLevelItem` からしか到達できない**ので `using` は必ず root の直下
  ——`PartTranspose.Read` が part 宣言について既に使っている論法。
  **実測で裏取り**: **section の中／score の中に書いた `using` は
  `UsingDirectiveSyntax` を 1 つも作らない**（descendant 0）＝**子だけ見ても見落としは無い。**
  ⇒ ★★ **むしろプレビューは速くなった**——**第3便より前の `GetSvg` は毎回
  `HasUsings(doc.Text)`＝*フルパース*＋赤走査を払っていた**。今は手元の木に 5 ノード訊くだけ。
- **⑻ ⚠️ ★★★ この便が*直さずに起票*した 3 件**（どれも §2F・実測つき）:
  **⑴ `test/grandstaff-high-bass` はもう自分の名乗る現象を守っていない**——
  「C4-E4-G4・加線を何本も」と書いてあるが実際は**五線の内側・加線ゼロ**。
  **観測者は snapshot 1 枚だけ**（論理の網 0 本）なので、**直すと絵が動く＝要 LP 照合＋承認**。
  **⑵ `lysc ly` が `transpose` を 3 綴りとも黙って落とす**（⑹ の主張を検算していて出た）——
  **双子に `\transpose` が無く、調も書かれたほう、警告もゼロ**。
  ⇒ **移調する本の LP 突き合わせは、ずっと別の音楽を比べていた。**
  **exporter の島（双子 199 本の全数比較が要る）なので着手していない。**
  **⑶ `using` を section／score の中に書くと、診断ゼロで黙って消える**（⑺ の裏取りで出た）——
  **`UsingDirectiveSyntax` が 1 つも作られず `check` は「No errors found」**。
  ⚠️ **part ヘッダの位置は報告する**（「'using' is not a part property」）ので、
  **黙るのは section と score の本体だけ**。**LYS0028 と同じ一族・別の機構**
  （expander ではなく parser の recover）。
- **⑼ ★ 毒は 10 方向**（§5.4）。**⑵＝警告だけ落とす→3 本赤**（diamond と cycle は緑のまま
  ＝**警告が他の枝へ漏れていない**）／**null 判定を反転→10 本赤・うち 4 本は既存**
  （＝その枝は展開そのものを支えている）／**span を keyword に変える→1 本赤**（位置は pin されている）。
  **⑶＝範囲フィルタを外す→1 本赤**／**展開前の木に戻す→`AResolvedInclude_LeavesThePanelClean`
  が赤**＝**欠陥そのものに観測者がついた**。
  ⚠️⚠️ ★★★ **その範囲フィルタの毒は、最初 *緑* で返ってきた**——**網が空集合について
  「全部が範囲内」と言っていた**（fixture が範囲外の診断を 1 つも作っていなかった）。
  **同じテストの中に「範囲外が少なくとも 1 つ在る」の陽性対照を置いて初めて赤**。
  ⇒ **RULES §5.4 に汎化した**（§5.0 罠21 の*テスト*版）。
  **⑸＝最初の score だけにする→2 本赤**／**位置の畳み込みを殺す→1 本赤**／
  **素の collect に戻す→ここで 4 本＋*他所で 31 本*赤**
  （＝validator が依存している描画経路の collect には、私の網以外に 31 の観測者がある）。
  **⑹＝ガードを外す→新しい 4 本だけ赤・既存 5061 本は 1 本も落ちない**
  （＝**観測者ゼロがその欠陥の寿命の説明**）。**⑺＝`HasUsings` に `.Skip(1)`→4 本赤。**
- **⑽ ★ §7.5 の監査**＝第2便 Core +95 行・第3便 +16 行・第5便 +119 行・第6便 +25 行・
  第7便 +19 行、**REF 0 / OWN 0・幾何の数値定数 0**
  （第5便の 1 件は `int collected = 0` ＝ループ計数器）。
  ⚠️ ★★ **§7 ⑼（perf）はユーザーの問いで初めて全部やり直した**——**第3便で打鍵経路に
  計算を足していたのに、その便では測らずに出していた**（⑺）。**「計算を足したか」を
  便ごとに問うこと。足した便は第3便だったのに、気づいたのは 4 便あと。**
  **全部正しい**——**この便は量を 1 つも計算していない**（診断と報告の組み立てだけ）。
  **`using` は移植ではない**: LP の `\include` は**字句段のテキスト取り込みで、
  ファイルが無ければ止まる**。Lily# の `using` は**part と section を partial-class 式に併合**し、
  **宣言された設計として止まらない**。⇒ 対応物が無いので REF も OWN も出番が無い。
- **⑾ ★★★ §2F に残っているのは 3 つ＋3**——**⑴ 一覧に構文を与えるか（決定であって作業ではない）
  ⑵ 繰り返し縦線の ⑹ ⑶ `lyrics` の voice 束縛（要決定）**、＋**⑷ `grandstaff-high-bass` が
  主張を守っていない件（要承認）⑸ `lysc ly` が transpose を落とす件（exporter の島）
  ⑹ section／score の中の `using` が黙って消える件**。
  **「parse しない fixture 2 本」「`--pitches`」「score 単位 transpose」はこの便で閉じた。**
  ⇒ ★★★ **「0.3.0 の前でなければ不可能」な項目は 0 のまま。**
  ⚠️ **名前を取る構文の棚卸しは全部やり直して数え切った**（推論ではなく `check` に訊いた）:
  **clef 名・key の mode は error／`font "NAME"` と `instrument "NAME"` は黙る**——
  **`instrument` は表示テキストなので黙って正しい**が、**`font` は §2F の既知項目**。
...
盲だったと分かった便**（**実装 3 便**）。
開始時 **台帳 516 点・ss 非ゼロ 98・総和 3.609965521・count 点 106/うち非ゼロ 2**・
HEAD `57d8970b`・**未 push 23・未追跡 0**・suite **5003 passed / 0 failed / 4 skipped**。
終了時 **実装の最後は `91257b41`**・suite **5034 passed / 0 failed / 4 skipped**・
**台帳 516 点で不動**・**コーパス 0/81**・**snapshot 0 枚**。
...
- ⚠️ ★★★ **新規＝`test/grandstaff-high-bass` は、もう自分の名乗る現象を守っていない**
  （第182 実測・**未修理・要ユーザー承認**）。本の冒頭は「**高い下段の和音（C4-E4-G4、
  加線を何本も引いて上の譜の音域に届く）が譜間を広げねばならない**」と書き、
  `SkylineBuilder.SeedStaffSymbol` を守ると名乗っている。**実際の絵は C3-E3-G3＝
  下の五線の内側・加線ゼロ**（y 24.62/23.62/22.62 対 五線 22.12…26.12）。
  ⇒ **相対オクターブの鎖が下がった結果**と思われる（`lh { c4 d e f | <c e g>1 | }` は
  bass アンカーから c3 d3 e3 f3、和音の `c` は f3 の最寄り＝c3）。
  ⚠️⚠️ **観測者は snapshot 1 枚だけ**（実測：論理の網は 0 本・`SeedStaffSymbol` を名指す
  テストも 0 本）＝**音楽を直すと絵が動く**ので、**LP 照合 → ユーザー承認 → 再ベース**の順。
  ⇒ ★ **同型を疑う**: 「加線」「上の譜に届く」と書いてある本が他にもないか、
  **コメントの主張と実際の staff position を突き合わせる**（この 1 冊は偶然見つかった）。
