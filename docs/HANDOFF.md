...
**⑵ `tab` の梁は「高さがずれている」のではなく、そもそも量子器を通っていなかった**（⑦・**起票**）。
...
## ⑦ ★★★ **起票＝tab の梁は量子化されていない**（**未修正・要承認＝全 tab 本が動く**・**原因まで割れた**）
...
⚠️ **要承認＝コーパスの tab 本の梁が全部動く**（`test/tab-*` ＋ showcase）。

## ▶ 次の一手

★★★ **⑦ の移植＝tab の梁を量子器に戻す**（**上のとおり原因まで割れている**・
**要承認＝全 tab 本の snapshot 再ベース**）。
**入口は `stemPositions` の seam**、**渡すのは `staffSpace 1.5` と `beamThickness 0.32`
（＝0.48/staffSpace）と弦の位置**。**判定器は `test/tab-string-pinned` の 4 群**
（**記譜譜は既に三桁一致なので、動いてはいけない側の control が同じ本の中にある**）。
...
- ★★★ **tab の梁が量子器を通っていない**（2026-08-01・第67セッション・**原因まで割れた・要承認**）。
  **LP は tab も同じ量子器に通し、違うのは staff の定数だけ**（**実測: `staff-space 1.5`・
  `beam-thickness 0.32`＝0.48/1.5**）。**Lily# の `TabBeamQuant` は `0.6·tanh` の傾きと
  `2.4/1.4/1.6 × 弦間` の 3 定数で、格子に載せる工程が無い**。
  ⚠️ **`BeamScoringProblem.stemPositions` は `88f98480` 以降どこからも渡されていない**
  （＝`_isTab` は常に false・**seam は戻り道として残してある**）。**詳細と実測表は §1 ⑥。**
