...
最終更新 2026-07-28（第27セッション・**保留中だった clef シルエット移植を、それを測る台帳点を
作ったうえで landing させた**）/ HEAD・ahead 数は §0 で確認すること
...
~~0. **点を開く値段を下げる**~~ ／ ~~1. **保留中の移植を landing させる**~~ — **両方とも済**
   （第27セッション・`f0582bbd`＋`ceb73c30`）。⇒ **恒久的な教訓は §5.0 と上のセッション節へ。
   ここに残す 1 文だけ書く: 専用プローブの実測は 13.5 秒で、`-Probe` は既存の
   `Measure-LilyPondPageGeometry.ps1` が最初から取る。もう「点を作る値段」を理由に先送りしない。**

~~1. **残る +0.043376 を割る**~~ — **済**（同セッション・上の節）。**正体は小節番号の書体**で、
   `barnumber.*.staff-to-baseline` と**同じ 1 つの face の 2 つの読み**（baseline −0.024440 ／
   インク上端 +0.044567）。⇒ **下の 1 番に合流した。**

~~1. **小節番号の baseline 位置も face から読む**~~ — **済**（同セッション・上の節）。
   **小節番号の族は閉じた**（残るのは face の差 0.007048／0.000006）。

1. ★ **他の「Own tuning」テキスト量も同じ道**（小節番号でやったことの続き）。
   `OutsideStaffStacker` の **`CapHeightEm = 0.71` / `TextAscentEm = 0.75` /
   `TextDescentEm = 0.25`** は**コメントが自ら「Own tuning approximating a serif face;
   no single LP grob source」と言っている**。小節番号の経路からは外れたが、
   **tuplet 番号・強弱・テキスト系がまだ読んでいる**。
   ⚠️ **点が無い**ので §5.0 どおり**先に対を作る**——ただし**専用プローブは 13 秒**（▶0）。
2. ★★ **歌詞の up-extent を face から読む**——**残る最大の族**（+0.271310 が **9 エントリ**・
   総和 2.801498 の 2.4 以上）。⚠️ **着手前にユーザー判断が要る**: TeX Gyre Schola に CJK が
   無く、一括で移すとかな音節の up-extent が 0 になる（実測で差し戻し済）。
   **CJK の受け皿を決めるのが先**（CJK 書体を持つか／非 Latin は表に落とす分岐か）。
   **コードより先に決定。**
3. **clef シルエット移植が残した繰延 2 件**（上のセッション節に詳細）＝
...
ほとんど書体量である**（**|residual| 総和 2.801498**・ss 154 点中 **127 点が exact**・
count は別勘定＝§3 の決定。⚠️ **符号付きの和は別の数**——引用するときはどちらを足したか言うこと。
⚠️ **第27セッションは 2.606965 → 2.968224 → 2.801498 と動いた**: 上がったのは clef シルエット
移植が**打ち消しを解いた +0.105961 ×3** と**新設点の持参金 +0.043376**、下がったのは
**小節番号のインクを face から読んだ分**。**全部 §1 の上に名前つきで書いてある。**
⚠️ **総和は 0.17 しか下がっていないが、それは読み方を間違えている**——
**小節番号の族が「発明」から「face の差」に変わった**のが今回の中身で、それは総和に出ない。
⚠️ **ここに数を書き足すなら実測してから**——
`Corpus_ReportsTotalDivergence` か `lp-geometry.json` を集計する）:

| 残る族 | 量 | 種類 |
|---|---|---|
| **歌詞書体（up-extent 1.459200 対 1.187880）** | **+0.271310** ×**8**（旧 5 点＋合流した 3 点）＝ **2.17**（＋ `two-verse.staff-staff-inside` の中にも） | **フォント**。⚠️ **0 にしない**（CJK の受け皿が先） |
| `lyrics.*.system-gap` | **+0.167914** ×2 ＝ **0.34**（旧 +0.207200） | **フォント**。⚠️ **小節番号の分は払い済**（残る 0.007048 は face の cap 差）。残りは歌詞面のインク＋五線の線の半太さ 0.050000 |
| `barnumber.*.staff-to-baseline` | **+0.000006** ×2 | **閉じた**（旧 −0.024440）。⚠️ **「Lily# には下はみ出しの実測が無い」という*床の宣言*が stale だっただけ**＝§5.0「stale になるのは理由もである」 |
...
| `between-staves.two-verse.staff-staff-inside` の内 **0.012826** | 0.013 | **機構**（歌詞 descent の書体差 0.010956 込み） |
...
**歌詞書体の +0.271310 は 9 エントリに現れる**（8 点は素で、1 点は +0.284136 の中に）。
⇒ **総和 2.801498 のうち 2.4 以上がこの 1 つの量。**
⇒ ★★ **コーパスが測っている範囲では、レイアウト機構の乖離はほぼ閉じている**——
**機構として名前が付いたまま残っているのは 0.012826 の 1 件だけ**（新設点は exact に着地した）。
