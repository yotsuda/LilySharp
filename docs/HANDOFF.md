...
⚠️⚠️⚠️ ★★★★ **⑽ 決定が出て委任の理由が消えたので、前提条件 ⑵ を*同じ便で実装した*（`207956f3`）。****`section ~A { … }` がその section のラベル既定をひっくり返す**——**`~` は両側で 1 つの意味のまま**（「既定でないほう」）。⚠️⚠️ ★★★ **だが先にやったのは規則の*集約*だった＝ラベルを決める文が 11 軒に分かれていた**（**collector 7 腕＋小節数え walk 2、twin 4**。各自が `silent ? null : DisplayLabel ?? name` を綴っていた）。**2026-08-25 に、そのうち 1 腕が `IsSilent` を教わっておらず、twin のコメントが「その腕を写した」と書いていた**——**引用が欠陥を出力境界の向こうへ 2 度運んだ**。⇒ ★★★ **項が増える規則は、増やす前に 1 軒にする**（`Semantics.SectionLabelRule`）。**11 軒のままでは*項を足せない*。** ★ **集約は掃きで恒等**（1931 冊・追跡本 0 冊）。⚠️⚠️ ★★★ **そして既定をひっくり返した瞬間に、*古い意味が隠していた欠陥が 2 つ*出た**: **⒜ `~` 綴りは parked label を捨てていた**（`FormWalk` が `null` を渡す。`~` が常に「隠す」なら無害＝**刷らないラベルは運ばなくてよい**——**`~A` が刷り得た途端に嘘になる**。実測: `section ~A` ＋ `form { ~A "shown" }` が section 名を刷った）／**⒝ LYS0012 は*パーサ*の警告で「`~` で隠れている」と言っていた**——**パーサは宣言を見られない**ので、**ページが刷るラベルを「隠れている」と報告する計器になるところだった**。**`FormDeclarationValidator` へ移し、表面（`~` があるか）ではなく規則（この play は刷るか）を訊くようにした。** ⇒ ★★★★ **その移動が、*診断が 1 つも無かった黙った取りこぼし*を捕まえた＝`[2. ~O "alt"]` は何も刷らない**（tilde は section 名に付く＝2026-08-25 の実測）。**旧警告は `~Name` しか見ていなかったので「警告しない」が*すべての ending について*真で、`FormVoltaTests` はそれを「正しい」として固定していた**——**その根拠（「ending の `~` は括弧だけを消す」）は 2026-08-25 に反証済みだった。** ★★ **教訓**: **「この綴りは警告しない」を固定するテストは、*その綴りに診断が存在するか*を先に確かめる。存在しなければ、そのテストは何も守っていない。**

⇒ ★★★ **次の一手＝§2 F ⒫ 本体**（`|:` `:|` `[N. …]` を music で禁止）。**前提条件 ⑴⑵ は両方入り、移行も ⒜ 即エラーと決まった**ので、**着手を止めるものは無い**。⚠️ **中身に「追跡側 20 冊の fixture を切り直す」が含まれ、ユーザーの 115 冊は通らなくなる**（それが決定）。 ★ **独立した小粒**: **⒭（section ヘッダの `clef` が効かない）は手つかず。⒮ は測り直して書き換えたが着手していない**（⑻）——**⒮ も ⒫ の決定待ちになった。** ⚠️ **§2 U8 の残り**（`grace { }` 本体の和音・休符／スラー・梁・タイ／注釈）は**本便では 1 行も触っていない**。
...
終了時: **commit 12 本**（**⒬ 本体 `b06f6968` ／ 文法書 `96a965bf` ／ §1・§2・§3 とアーカイブ `b8954d85` ／ ubuntu 脚 `47ef56c3` ／ RULES への汎化 `515039e5` ／ 数の閉じ直し `dda6c577` ／ 委任への答えと ⒮ の測り直し `82246a9a` ／ ユーザー決定 2 件 `c979a4c5` ／ 移行の決定 `aaf3f47b` ／ **ラベル既定の反転と 11→1 の集約 `207956f3`** ／ 文法書 `d3df1288` ／ 数の閉じ直し（この commit）**）・**未 push 12**・木 0・未追跡 0。**Windows Debug 6750 / 0 / 4 / 6754**（開始 6705 から **+49**＝**⒬ の網 29 ＋ ラベル既定の網 20**）。Core 0 エラー 0 警告。**台帳・追跡コーパス・snapshot は開始時と同じ**（748／588／190／24.714570188／158 中 2／`OPEN:` 0／581 冊／231 枚）。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけ**——**定数は 1 つも足していない**。⚠️ **`82246a9a` は Core を*コメントしか*変えていない**ので掃きを取り直していない（§7.9）。**Windows Debug 6730 / 0 / 4 / 6734**（開始 6705 から **+29**＝**本便の網 29 ケース**）。Core 0 エラー 0 警告。**台帳・追跡コーパス・snapshot は開始時と同じ**（748／588／190／24.714570188／158 中 2／`OPEN:` 0／581 冊／231 枚）。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけが 6 行動いた**——**定数は 1 つも足していない**（`audit/magic_constants.csv` も不動）。
...
> ✅ **前提条件 ⑵ も第304 で入った**: **宣言側に `section ~A { … }` と書くと、その section のラベル既定がひっくり返る**（§3）。**純粋な追加**であることは実測済み——**`section ~A` を書いている本はディスク 1925 冊中 0 冊で、直前まで硬いエラー**（`Expected a name, found 'Tilde'`）。★ **前提条件は 2 つとも済んだ**——**⒫ の着手を止めるものはもう無い**（移行も ⒜ 即エラーと決まった）。
