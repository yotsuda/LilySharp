...
⇒ ★★★ **次の一手＝残りの U8＝⒜ の*和音と休符*（tuplet は本便で閉じた）／⒞ 本体の中のスラー・梁・タイ ／⒟ 注釈の全族**（§2 U8 の ▶）。⚠️⚠️ ★★★ **箱の中身が変わった**: **⒜ は 4 つ → 2 つになり、残る 2 つは*本当に grob を要る*側だけ**（和音は符頭が 1 つ増え、休符は休符 glyph が 1 つ増える＝第301 LP 実測）。**「症状で仕分けた箱」はこれで空になった。** ⚠️ **実コーパスの射程はいまもタイだけ**（追跡 2 冊・第301 実測）——**本便の掃きがそれを追認した（追跡本は 1 冊も動かなかった）。** ⚠️⚠️ ★★★ **難所は 2 つに割れた**（⑿ 実測）: **⒜ は住所ではなく `GraceNoteItem` の*模型*（平らな列が「同時」も「休符」も言えない）／⒞ と ⒟ だけが `ItemIndex` に依存する。** ⚠️⚠️ ★★★ **そして*どれを取るにせよ、読み手は 4 人いる***: **和音と休符は MIDI では 2026-07-10 から鳴っていて、ページと XML は落とす**（**本便が 4 出力で裏取りした: 和音だけの本も休符を足した本も、svg と xml は対照とバイト同一で `.mid` だけ動く**）——**その 2 つを閉じる便は、4 人が食い違ったままかどうかを毎回数えること。** ⚠️ **和音・休符を `GraceNoteInfo` の中に彫らないこと**（§5.2.1②）。
...
終了時: **commit 6 本**（**本体 `d6bf1e1b` ／ docs `b296ed7e` ／ 数を閉じる `4093ffe5` ／ 射程の取り直し `ef6f430b` ／ 委任への答えと自己監査 `e8476165` ／ 2 度目の委任と難所の測定（この commit）**）＝**未 push 35**（**この commit を含む。5 本目までの時点で 34 だったのを確かめてから書いた**）・木 0・未追跡 0。**suite 6685 合格 / 0 失敗 / 4 skip / 合計 6689**（開始 6674 から **+15**＝**本便の網 16 ケース − 番人から消えた 1 行**）。⚠️ **⑾ の commit は Core を*コメントしか*変えていない**（`git diff` の Core 側の `+` 行は 1 行残らずコメント）ので、**掃きも perf も取り直していない**（§7.9 の「足していない例」）。Core 0 エラー 0 警告。**台帳・追跡コーパス・snapshot は開始時と同じ**（748／588／190／24.714570188／158 中 2／`OPEN:` 0／581 冊／231 枚）。⚠️ **`docs/APPROXIMATIONS.md` も `audit/magic_constants.csv` も 1 行も動いていない**——**本便は定数を 1 つも足していない**（**足した `LILYPOND-REF` は*決定*の出所であって量ではない**）。
