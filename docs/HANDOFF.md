...
★★★ **⑾ 数**: **開始時**: HEAD `1e80a9b2`・未 push 9・木 clean・未追跡 0（**`Session-Check -Build -Test` は走らせていない**）。**終了時**: HEAD `7735687d`・未 push 19・**台帳 843 点／ss 非ゼロ 210／総和 23.216135909／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249／追跡 `.lys` 599＝全部不変**（**絵は 1 枚も動かない**＝触ったのは LSP と test だけ・**Core は 3 便とも 1 行も触っていない**＝§7.5 の数えは 0 行）。**full `scratch/p375/run10.trx` 7850 / 0 / 4 / 7854**（第 1 便末 `run1.trx` 7816・第 2 便末 `run4.trx` 7827・第 3 便末 `run5.trx` 7829・第 4 便末 `run6.trx` 7839・第 5 便末 `run8.trx` 7842。**6 便とも合計を読んでいる**＝新 test は 6 + 10 + 2 + 10 + 3 + 12 = 43 本。⚠️ 第 5・6 便は既存 test を**書き換えている**ので増分はそれより小さい）。⚠️ scratch は **`p375`**。⚠️ §7 3.5: 第372 を `-Archive 372` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝**第372 の天井 commit `9f6df2a3` が未 push のまま**。**push はユーザー（未 push 12）**。★ **LSP は 5 度配布した**＝`tools/Deploy-Lsp.ps1` が `0.6.0+7735687d` を `yotsuda.lilysharp-0.5.0-win32-x64\server` へ（95 ファイル byte 一致）⇒ **ユーザーは Reload Window が 1 回要る**（package.json は動かしていないので 2 回は不要・記憶 `feedback_lilysharp_lsp_deploy_target`）。

⇒ ★★★ **⑿ 次の一手＝⒜ *省略可能な節を持つ行を、語数ではなく文法で読み直す*（第 6 便が作った未計数・⒜ の中で最優先）**。**この便の 6 件のうち 4 件が同じ原因**（`sings` 節・沈黙 `~`・score の option・staff の clef）＝**読み手が語数を数え、文法には `[ … ]` が在る**。⇒ **GRAMMAR の `[ … ]` と `{ … }` を持つ*行*（ブロックのヘッダは ⑻ で閉じた）を全部並べ、各読み手が「省略時」と「記述時」の両方を読むかを表にする**。★ **実測済みの穴が 1 つ在るので、そこから始められる**: **`staff m as lines 1 ▮` が score 一覧に落ちる**（`{ StaffSelector }` の連鎖＝`as lines 1 removeEmpty all` が書けるのに、値の後ろで次の selector を出していない。**`chords`／`tab` の `as` にも同じ問いが掛かる**）。⒝ **スニペットが書く「名前」の全数**（第 5 便が作った未計数）＝**同じ問い「この placeholder は*文法の語*か、*この本の名前*か」は補完の全項目に掛かる**（`staff ${1:part}`／`chords ${1:prog}`／drummap の `${1:hh}`／template 群）。**①本から取る ②選ばせる ③自由に書かせる のどれかを 1 行ずつ決める**。⚠️ **判定は「実在する物を指すか」**——`${1:prog}` は*これから作る*名前なので自由でよく、`sings ${2:part}` は*既に在る*物を指すので駄目だった。⒞ **第373 ⑼⒜（probe 24 冊の「量を測ったか／問いに答えたか」の仕分け）**も先頭級のまま。**ヘッダの綴りの島は閉じた**——⑻ の 25 綴りが全数で、**残る未点検はゼロ**（`{` を開く構文は score／staff group 5／part／phrase／form／lyrics／chords／section／fonts・paper・layout とその入れ子／音楽の容れ物で尽きる）。⚠️ **閉じたのは*補完の*読み手だけ**: 同じ「2 語しか読まない枠」を持つ道具が他に居ないかは見ていない（`SemanticTokens`・折り畳み・`BarCountFix` は別の木＝**実構文木**を読むので原理的に無縁、という*予想*まで）。⒟ 第369 ⑸⒝（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。

★★ **⒀ 判定の記録（ユーザーの常設指示「次便は、このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）**: **第 2・5・6 便はユーザー起票**（判定の対象外）。**第 3 便（沈黙 `~`）と第 4 便（全数の掃き）は着手した**——**どちらもこの便の道具でしか安くない**（end-to-end ハーネス＋`GetCompletionContext` の dump 計器で**1 回の実行が 25 綴りを対で読む**。**高いのは「どの読み手が何を訊いているか」で、それは⑷⑹を書いた便の頭の中にしか無い**）。★★★ ⚠️ **私はこの判定を 1 度間違えて書いた**——第 3 便の時点の §1 に「全数表は**読む量が大きく道具の温度が要らない**ので次便が有利」と書いたが、**⑺ の tilde は*その計器で*見つけた**のだから、掃きは同じ操作の N を増やすだけだった。**⇒ 判定を書くときは「どの道具が温まっているか」だけでなく「*直前の発見をどうやって*見つけたか」を見る**——**同じ手続きの拡大なら、それは温まっている**。**着手しなかったもの**: ⑿⒜⒝ は**第 5・6 便の終わりに生まれた未計数**で、**読むだけ**——⚠️ ただし**判定の材料は「どの道具が温まっているか」ではなく「直前の的をどうやって見つけたか」**（上の訂正）で、**⑼ も ⑽ も*ユーザーが*見つけた＝掃きの計器で出したものではない**。⇒ **次便は自分で 1 度 dump を取ってから決めること**（`GetCompletionContext` を綴りの対で叩く計器は**毎回 2 分で書き直せる**＝第 4・6 便で 2 度作って 2 度捨てた。**捨てるのが正しい**＝`reference_lilysharp_throwaway_probe_leaves_tests_project`）。⑿⒞⒟ は**別 regime**（LP を回す／設計級）で、p375 の道具が 1 つも効かない＝**次のセッションでやっても損しない**。
