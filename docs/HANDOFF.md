...
- **⑷ ★★★ 締める向きの変更なので全数で測った**——**ツリーの全 `.lys` 897 冊**を
  `lysc check` に通し、**本ごとの診断を記録して前後で差分**：**診断が変わった本 0 / 897**。
...
- ★ **残る同型の穴＝`staff NAME with chords C` / `with lyrics L` の名前**（`StaffPartToken` が
  尾を切るので今も誰も検証していない）。**⑶ と同じ締める向き**だが、**`with` 節が名指し損ねても
  「和音行の無い譜」に落ちるだけ**（スコアが空になるほうとは違う）ので**0.3.0 に間に合わせる
  必要は無い**。⚠️ **同じ島の別の顔が上にある**——**exporter が chords/lyrics 行を双子に出せない**
  （穴の値段は**譜を持たない本 5 冊**・第176 実測）。**塞ぐなら双子 199 本の before/after 全数比較**が
  要る（§2F 上段）＝**この 2 つは 1 便ではなく 1 セッションの形。**
