...
- **⑼′ ★★★ そして本が来たので、その窓で床を取り直した**（Release・RUN 3 本・生データ
  ...
  **次に段を取る人の手順は ▶ 冒頭に書いた**（⑴ regime ⑴ で取る ⑵ 回数で測る ⑶ n 増しは最後）。
...
> ⇒ ★★ **次に段を取り直す人へ**: **⑴ 段の内訳は regime ⑴ で取る**（±2 ms なら 4〜10 ms 帯が見える）
> **⑵ regime ⑶ の段は時計ではなく回数で測る**（§7 ⑼ の「5〜15ms 級を時間で判定できない」は
> **この機械では 15 ms 級まで悪化している**）**⑶ n=14 を増やすのは最後の手段**
