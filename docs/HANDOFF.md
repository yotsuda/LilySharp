...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第426セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `343df97b`・**未 push 34**・**full `sessions/p426/run1.trx` 8751 / 0 / 3 / 8754＝第425 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・74 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第425 が予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 424`＝21 行 6,299 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」の 4 つを AskUserQuestion に出し、**ユーザーが ⒴ の残り「`ink.walk` の幾何」を選んだ**。★ ベンチは着手前に静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第425 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、**before の打鍵 8,111,747／8,111,781 B は第425 最終の 8,111,788 B と一致**。⚠️ **この便の §1 は短い**——HANDOFF が天井 450,000 B に 500 B まで迫り、`Fold-ClosedHandoffItems` は畳めるものが 0 だったので、**経緯は Lab `sessions/p426/` に置いて数と次の一手だけ残した**:

★★★ **⑴ §1 が引き継いだ数が 2 つとも壊れていた**（§5.0「N を数え直す」の **14 例目**）。「包みを除いた後で first 0.460%」の 0.46 は**除く前**の 0.468 で、引用元の Lab `p425/edge-staff-ink.txt` 自身は **0.284** と書いている（**1.6 倍高く売られていた**）。「`fret.ToString` が 31,029 回／打鍵」の 31,029 は **1,848 打鍵の合計**＝1 打鍵 **16.8 回**。再カウント: **島 0.353%（first 0.282 ＋ last 0.071）・`ink.walk` 0.235%**（予測 0.33〜0.38 / 0.21〜0.26 が当たった）。

★★★ **⑵ ⒴ の残りは*幾何ではなかった*。walk の 83% は `List<SkylineBuilding>` が空から倍々に伸びる音だった。** 計器は **scope ではなく構成で数える counter**（append の*前*に `Count == Capacity` なら次の `Add` が `24 + 32×2cap` を割り当てる）。**`grow.ink` は島の 71.4%**・**`grow.wn`＋`grow.wt` は walk の 82.7%**・**walk から growth を引くと 0.041%**（予測「島の 35〜65%」は*低すぎた*方向に外れた・falsifier は 15%）。★ **`n.head`／`n.rest`／`t.box` は直したあと 0 B**＝**「1 音符 225 B」は幾何ではなく 7 棟ぶんの配列**だった。

★★★ **⑶ 0.556% は島の外にも在り、対照は repo に在った。** `grow.all` は打鍵の **0.556%**（**島 0.353% より大きい**）で、全額が **4 つの batch**＝`SkylineBuilder` の `BuildSystemSkylines`（:76/:77）と `BuildStaffSkylines`／`BuildInsideStaffSkylines`（:1192/:1193）。**5 つ目の site は同じ run で 0 B——それが対照**: `LyricEngraver` は append を数えて `VerticalSkyline.ReserveForBatch` を呼ぶ（第224）。**その註が「Only a caller that can COUNT the batch up front can retire them」と書いてから 202 便、兄弟 4 軒は誰も数えなかった**（§5.3）。**1 棟 32 B に配列 92〜94 B＝棟あたり 2.90 slot**・resolved/appended **0.16〜0.59**＝**倍々は捨てる側に払っている**。

★★★ **⑷ 直し＝batch は thread の pool から借りた buffer に積み、`EndBatch` はそれを*そのまま*resolve の入力にする**（`a5fafdf0`・**Core 1 ファイル**・網 1 ファイル・**数値定数 0・LP 由来の式 0・REF 0／OWN 0**）。`_deferResolve` は消え **`_batch is not null` が同じことを言う**（1 状態 1 綴り）。⚠️ **非空の skyline に batch を開いたら既存の棟を buffer の先頭へ移す**——**`List.Sort` は*不安定*なので入力の順序は答えの一部**。`EndBatch` は結果用に **`_buildings.EnsureCapacity(batch.Count)` を 1 度だけ**取る。★ **EndBatch の入力複写も消えた**（rental **83,538 → 73,254**）。⇒ growth **83,353,208 B / 57,381 回 → 16,408 B / 1 回（−99.98%）**。**A/B ＝ −0.3409%**（**8,111,764 → 8,084,109 B**・両側 TC=0・各側 2 回・床 34／40 B）。⚠️ **0.556% 全部は取れない**——**入口の倍々は消えるが resolve が空の `_buildings` へ書き戻すので*出口*に戻る**（差 0.13〜0.20%＝⒴⁹）。

★★★ **⑸ 出力同一を 3 つとも**: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline は `343df97b` の木から `git stash` して取り直した）／⑵ **実コーパス全ページ SHA-256＝5,824 行・0 行差**／⑶ suite 全緑・snapshot 249 枚不動・台帳不変。★★ **構成上の証拠がもう 1 本＝4 site の appended と resolved は 1 棟も動いていない**（235,712／241,889／208,790／211,127 と 98,111／143,830／47,346／34,770）。★ **網 4 本**（非空 skyline の batch／同時 2 batch が buffer を共有しない／1 棟の batch／**pool の cost gate**）に**毒 7 つ**——⑴→1 本赤・⑵→5 本赤・⑷→1 本赤・**⑸ `RentBatch` の `Clear` を落とす→10 本赤**・⑹⑺→cost gate 赤。⚠️⚠️ **「`ReturnBatch` の `Clear` を落とす」は 1 本も赤くならず、それは穴ではなく*2 綴り*だった**（`RentBatch` が既に clear）⇒ **網を足さず重複を消した**。★★★ **そして第421 の cost gate `AMergeIntoALargeSkyline…`（上限 3,000 B）が 19,928 B で赤くなった——主張は今も真で、赤いのは*batch の過剰割り当てが残していた slack* が消えたから**（規則は RULES §5.4）。予約で production の slack は戻り、この fixture は箱が disjoint なので warm を 3 回にした（**上限は動かしていない**）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: ★★★ **⒴⁹ 新規＝出口側の倍々 0.13〜0.20%**＝**resolve が空の `_buildings` に書き戻す**。R を先に知る形（pooled 出力へ解いて exact-size で写す）＝**`RebuildKeepingHighest` の in-place 契約（第191 の註）を読み直すこと**・要設計／★★★ **Ⓑ 新規＝`DotConfiguration.Resolve` が打鍵の 0.320%**（**実測 48,030,424 B / 45,320 回＝1 回 1,060 B・1 打鍵 24.5 回**）。**1 点でも `SortedDictionary` ＋ `Enumerable.Range().OrderBy().ToArray()` ＋ 結果配列**で、**線上（点の半分）は `Shifted` を 2 棟建てる**（島側 2,654 ≒ 2 × 1,323）。⚠️ **速い綴りを 2 本目として足さない**（§5.2.1②）＝**唯一の家を軽くする**／★ **⒴¹⁰ 新規＝tab fork の残りは `TabSoundingNotes` の iterator ちょうど 64 B × 34,717 items＝0.015%**（⚠️ **`fret.ToString`・`FretGlyphWidth`・`CalculateFret` は 3 つとも 0 B**＝.NET は 1 桁の文字列を cache し、このコーパスの fret は全部 1 桁＝**起票が名指した犯人は 1 バイトも使っていなかった**）／★ **⒴⁷ `BeamedItemsToSuppress` は beams が空でも `HashSet` 0.028%**＝**直した後の島の 27% で筆頭**／★ **⒴⁶ 残った包み＝`SeedClef` の `PlaceGlyphOutlineCached`**（天井 `ink.clef` **0.071% → 0.021%**）／⒴⁸ **batch でない `Merge` 147,417 回／打鍵は未値付け**／★★★ **⒡′ bow を*staff 自身の枠*で採点して offset は描画時に足す**（第424 の払い戻し 0.074% ＋ 1 ULP の尾が消える・土台の変更・要設計）／★★ **⒢′ 拒否された staff の前処理＝検出器 0.023%＝`StaffSpannerItemsOf` の memo を最終パスにも通すだけ**（一番安い・出力同一が構成で言える）。⚠️ 第424 の直しで拒否経路はほぼ通らない＝**まず回数を測ること**／★★ **⒵⁴ `prefixMarkAnchorX` の解き直し 0.338%**＝**memo は反証済み**（hit 率 0.53%）・土台の変更／★ **⒵⁵ `mm.g.split` 0.163%**（⚠️ `OrderBy` は*安定*）／★ **⒵⁶ 文字の 318,784 回／81 綴り 0.130%**／★ **⒴⁵ artic と trill の remap 辞書は作って捨てている**（天井 0.174%）／★ **⒳″ `Padded` の複写 0.028%＋`SortAndResolve` 0.15%**（起票のみ）／★★ **⒴⁗ 天井 2.03%＝安くすると `アゲハ蝶.lys` 1 冊が 24→26 系になる＝perf ではなく*忠実度の判断*。開けるならユーザー決定が先**（実装は Lab `sessions/p423/zz423-deferred-prelim.diff.txt`）・**先に LP 双子**／⚠️ **`g4.core` 1.21% と `p1.s1.beams` 5.538% は*もう実仕事*＝この 2 島には戻らないこと**／⒜ **R13⒝ の実機確認**（第404 ⑵）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／LP 双子が要る R7〜R11（✅ **`lilypond.exe` の hang は 2026-09-20 に解決＝MCP コンソールのプロンプトの入力読み取り待ちで、機械のせいではない。`cmd /d /s /c "… < NUL > log 2>&1"` で通る**＝RULES §5.5）／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑹ 終了時**: commit 2 本（code `a5fafdf0`＝Core 1 ＋ 網 1 ＋ 棚卸し 2 本の再生成・docs 1 本＝§1 と RULES。**この文を含むので SHA は書かない**＝§5.4）＋Lab 1 本・**未 push 36**（開始時 34）・**最終 full `sessions/p426/run3.trx` 8755 / 0 / 3 / 8758**（**第425 最終 8754 から +4＝今便の網ちょうど 4 本**。`Zz426`／`ZzStage`／`ZZPOISON` の grep は**コードに 0 件**）・§7.5（対 `343df97b`）**Core `+` 145 行／REF 0／OWN 0**（**算術を 1 つも足していない**＝pool と capacity だけ・§7.6 ⒟）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・棚卸しは **`APPROXIMATIONS.md` も `magic_constants.csv` も行の増減 0**（行番号だけ・OWN 143）。計器と全文は Lab `sessions/p426/`（`edge-staff-geometry-leg1.txt`＝脚 1 と予測・**`batch-sites-leg2-before.txt`＝脚 2 の before**・`edge-staff-geometry-leg3-after.txt`＝直し後・`zz426-seams-and-change.diff.txt`・`zz426-change.diff.txt`・`ZzStage.cs.txt`・`Zz426Probe.cs.txt`・**`Zz426Hash.cs.txt` と `Zz426Ab.cs.txt`＝再利用可の証明 ⑵ と A/B の harness**・`hashes-after.txt`・`ab-{before,after}-{1,2}-tc0.txt`）。⚠️ **脚 2 の計器出力は脚 3 が上書きしてしまった**（§5.3 が名指す罠の 2 例目）＝**転記して残した。次便は脚ごとにファイル名を変えること。** ⇒ ⚠️⚠️⚠️ **HANDOFF は第425 の記録 446,819 B → アーカイブ後 435,835 B → 今便を書いて 448,200 B＝天井 450,000 まで 1,800 B しか無い**（§1 を一度 11.5 KB で書いてから 8.1 KB に削り直した。**`Fold-ClosedHandoffItems` は畳めるものが 0 件＝§2 の弁も空**・**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない**）⇒ **次便は §1 を 1 行も書く前に `-Archive 425` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**そして §1 はこの便の長さ（8.1 KB）以下に保つ**——これ以上は §2 を削るしかなくなる。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第426 の欠陥ではない。


## 以下は第425セッションの経緯

最終更新 第425セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `23aa4d68`・**未 push 32**・**full `sessions/p425/run1.trx` 8743 / 0 / 3 / 8746＝第424 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 62 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第424 が予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 423`＝21 行 6,123 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」の 4 つを AskUserQuestion に出し、**ユーザーが ⒴「`inkFirst`＋`inkLast` 0.53%・未分割」を選んだ**。★ ベンチは着手前に AskUserQuestion で静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第424 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、**before 側の打鍵 8,163,993 B は第424 最終の 8,164,012 B と一致**:

★★★ **⑴ 島を割ったら、島の 78.5% は*幾何ではなく包み*だった**（脚 1）。`inkFirst`／`inkLast` は `SkylineBuilder.AddEdgeStaffInk` の 2 回で、再カウントは **0.632%**（first 0.468 ＋ last 0.165・**第414 の 0.53% は同じ絶対量＝分母が 1.77 MB 縮んだぶん増えた**＝予測 0.60〜0.70% が当たった）。中身は **`ink.walk` 78.5%（0.496%）／`ink.clef` 11.9%（0.075%）／`ink.beams` 4.8%／`ink.suppress` 4.3%／残り 0.5%**。★★★ **そして箱は 200,817 個・1 個 475 B**——**`VerticalSkyline.FromBox` は 32 B の struct を 1 つ届けるために、skyline とその `List` と List の配列を建て、±inf の padder 2 棟で挟んで 3 棟渡し、受け取る `Merge` が batch なので 2 棟を捨てている**（kept/offered 42.5%）。

★★★ **⑵ 予測 ⑶⑷ は 2 つとも割れて、そこが本当の発見だった。** 「first:last の 3:1 は*回数*だ」→ **外れ**（回数は 1.34× で、*大きさ*が 2.12×）。「このコーパスは bass tab だから walk は tab fork が過半」→ **外れ**（tab 1,383 / notation 1,873）。⇒ ★★★ **`ink.first` の呼び出し 1,871 と notation walk の 1,873、`ink.last` の 1,393 と tab walk の 1,383 が一致**＝**この母集団では上の縁が必ず五線・下の縁が必ずタブ**で、**⒴ は「未分割の 1 島」ではなく *fork で割れた 2 島*を 1 つの数で読んでいた**。

★★★ **⑶ 射程を grep で数えたら 1/3 しか見えていなかった**（RULES §5.0 に 13例目として足した）。`Merge(VerticalSkyline.FromBox(` の grep は **51 か所**。当てて測ると打鍵あたりの包み **454,829 個のうち動いたのは 154,317 個**——**残り 300,512 個は `var b = FromBox(…); sky.Merge(b);` という grep に映らない綴り**（**符頭・符幹・旗・休符・加線が全部この形**）。⇒ **`Merge` に `[CallerFilePath]`／`[CallerLineNumber]` を足してコンパイラに呼び手を名乗らせた**（30 秒）。⚠️ **この継ぎ目自身が割当するので、値段はこの run から読まない**（島が 0.532% → 0.607% に膨らんで見える）。

★★★ **⑷ 直し＝箱を包まずに batch へ直接積む**（`220823db`・Core 13 ファイル・網 1 ファイル・**数値定数 0・LP 由来の式 0・REF 0／OWN 0**）。`VerticalSkyline.MergeBox` と双子の `MergeSlope` が、`FromBox`／`FromSlope` の*その 2 行*（向きが選ぶ辺と符号）と `Merge` の ±inf filter を読み直して 1 棟を `_buildings` に積む。**batch でなければ節約は無い**（padder が resolve に参加する）**ので、その呼び手は今までどおり `FromBox` を通る**——1 行の fallback。⇒ 包みは **454,829 → 7,990（−98.2%）**。⚠️ **残ったのは音部記号のグリフ輪郭 2 か所（3,746 回ずつ・20〜22 棟）と歌詞の 249 回だけ**。

★★★ **⑸ 包みの値段は構成と実測がバイトで一致した。** 構成上 `VerticalSkyline` 32 ＋ `List` 32 ＋ 4 要素の配列 152 ＝ **216 B**。実測: 第 1 波で島が 15,258,672 B 減って変換した箱 70,642 個＝**216.0 B/箱**、打鍵全体で 33,345,216 B 減って 154,317 個＝**216.1 B/箱**。⇒ ★★ **推論と測定を混ぜない（§5.3）を守った結果、構成が*予測*として反証可能になり、当たった。**

★★★ **⑹ A/B ＝ −0.6394%**（**8,163,993 → 8,111,788 B**・両側 TC=0・各側 2 回・**床は before 12 B／after 24 B＝0.0003%**）。⚠️ **予測 ⑹ は島の側（0.33〜0.45% へ）が当たり、打鍵の側（0.25〜0.40%）は*小さすぎた***——grep で見えた 51 か所だけを想定していたからで、外れの理由がそのまま ⑶ の発見。

★★★ **⑺ 出力同一を 3 つとも走らせた**（RULES §5.1）: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline はこの便で `23aa4d68` の木から `git stash` して取り直した）／⑵ **実コーパス全ページの SHA-256＝5,824 行・0 行差**（第424 の `hashes-after.txt` と突き合わせ）／⑶ suite 全緑・snapshot 249 枚不動・台帳不変。網は **`SkylineMergeTests` に 2 本（Theory ×4 ＝ 8 本）**＝`MergeBox`／`MergeSlope` が `FromBox`／`FromSlope` の対と**棟ごとに同じ列**を残すこと（batch と非 batch・上下）。★★ **毒 ⑴「向きが選ぶ辺を入れ替える」は batch の 2 本だけ赤くなった**（非 batch 側は `FromBox` 自身を通るので赤くなりようがない＝正しい）／⚠️ **毒 ⑵「±inf の filter を消す」は 1 本も赤くならず、それは穴ではない**——`Merge` の註自身が「resolve がどのみち落とす」と書いており、**filter は絵に届かない**（RULES §5.0 に規則を足した）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: ★★ **⒴ の残りは*幾何*＝`ink.walk` の中身**（包みを除いた後で first 0.460%・last 0.072%）＝**notation fork と tab fork は別の島**（⑵）。**tab 側は `fret.ToString` が 31,029 回／打鍵の文字列**・**notation 側は `DotConfiguration.Resolve` が点ごとに配列 2 本**＝どちらも未計測／★★ **⒴⁶ 新規＝残った包み 7,490 個は `SeedClef` の `PlaceGlyphOutlineCached`**（20〜22 棟ずつ＝包み 216 B ＋ 棟の複写）。**寄せ先は同じファイルに在る `Merge(resolved, dx, dy)`**（その註が「every one of those buildings was copied twice」と書いている＝第421 が 0.33% を取った道）・天井は `ink.clef` **0.071%**／★ **⒴⁷ 新規＝`BeamedItemsToSuppress` は beams が空でも `HashSet` を建てる 0.027%**（安い日の仕事）／★★ **⒴⁸ 新規＝batch でない `Merge` が 147,417 回／打鍵**＝この直しが届かない側・**未値付け**（padder が resolve に参加するので、同じ形の修理にはならない）／★★★ **⒡′ bow を*staff 自身の枠*で採点して offset は描画時に足す**（第424 の払い戻し 0.074% ＋ 1 ULP の尾が消える・土台の変更・要設計）／★★ **⒢′ 拒否された staff の前処理**＝**検出器 0.023%＝`StaffSpannerItemsOf` の memo を最終パスにも通すだけ（一番安い・出力同一が構成で言える）**。⚠️ 第424 の直しで拒否経路はほぼ通らない＝**まず回数を測ること**／★★ **⒵⁴ `prefixMarkAnchorX` の解き直し 0.338%**＝**memo は反証済み**（hit 率 0.53%）・土台の変更／★ **⒵⁵ `mm.g.split` 0.163%**（⚠️ `OrderBy` は*安定*ソート）／★ **⒵⁶ 文字の 318,784 回／81 綴り 0.130%**／★ **⒴⁵ artic と trill の remap 辞書は作って捨てている**（天井 0.174%）／★ **⒳″ `Padded` の中に残る複写 0.028%＋`SortAndResolve` 0.15%**（起票のみ）／★★ **⒴⁗ 天井 2.03%＝安くすると `アゲハ蝶.lys` 1 冊が 24→26 系になる＝perf ではなく*忠実度の判断*。開けるならユーザー決定が先**（実装は Lab `sessions/p423/zz423-deferred-prelim.diff.txt`）・**先に LP 双子**／⚠️ **`g4.core` 1.21% と `p1.s1.beams` 5.538% は*もう実仕事*＝この 2 島には戻らないこと**／⒜ **R13⒝ の実機確認**（第404 ⑵）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・未着手・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／LP 双子が要る R7〜R11（⚠️ **この機械の `lilypond.exe` の hang は第423 も 5 回踏んだ。次の手は Lab `notes/CLAUDE-OPERATIONS.md` §3＝タスクマネージャーの「待機チェーンの分析」**）／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑻ 終了時**: commit 2 本（code `220823db`＝Core 13 ファイル＋網 1 ファイル＋棚卸し 2 本の再生成・docs 1 本＝§1 と RULES。**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 1 本・**終了時の未 push 34**（開始時は 32）・**最終 full `sessions/p425/run5.trx` 8751 / 0 / 3 / 8754**（⚠️ **第424 最終 8746 から +8＝今便の網ちょうど 8 本**＝`SkylineMergeTests` の Theory 2 本 × 4。`Zz425`／`ZzStage`／`ZZPOISON` の grep は**コードに 0 件**）・§7.5（対 `23aa4d68`）**Core `+` 174 行／REF 0／OWN 0**（**算術を 1 つも足していない**＝`MergeBox`／`MergeSlope` は `FromBox`／`FromSlope` の式を読み直すだけ・§7.6 ⒟ の「指し直し」）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・棚卸しは **`APPROXIMATIONS.md` も `magic_constants.csv` も行の増減 0**（行番号だけ・OWN 143 のまま）。⚠️ **dead citation の網は 1 度正しく鳴った**——§1 が amend 前の SHA を引いていた（§5.4 が働いた実例）。計器と全文は Lab `sessions/p425/`（`edge-staff-ink.txt`＝脚 1 と予測・`zz425-seams-and-change.diff.txt`・`ZzStage.cs.txt`・`Zz425Probe.cs.txt`・**`Zz425Hash.cs.txt` と `Zz425Ab.cs.txt`＝再利用可の証明 ⑵ と A/B の harness**・`hashes-after.txt`・`ab-{before,after}-{1,2}-tc0.txt`）。§7 3.5 は**セッションの頭で**済（`-Archive 423`＝21 行 6,123 chars）⇒ **HANDOFF は第424 の記録 445,598 B → アーカイブ後 434,655 B → 今便を書いて 446,819 B（この文ぶんを含む）＝天井 450,000 まで 3,181 B**。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 424` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第425 の欠陥ではない。
