...
最終更新 第169セッション＝**▶ ⒯⒟ の本体＝引数ノードを入れ、7 家族をそこへ移した便**（3 便）。
...
終了時 **HEAD ＝ この handoff commit**（その親は第3便の `8b7c7fd6`）・**未 push 7**
（⚠️ **handoff commit は自分自身を数えられない**ので次の人が見るときも **7**）・
suite **4730 passed / 0 failed / 4 skipped**（+87＝新しい網ちょうど）・
...
- **⑶ ★★ 第3便＝`@feather` と `@arpeggio(bracket)` も移した**（`8b7c7fd6`）。
  **`MarkName` の code read は 18 → 13・7 ファイル → 5**（数え方は監査 §9.5.2）。
  **残るのは frame / fig / chord の 3 部分言語＋点つき名前の家族**（`ParseMarkName`）**だけ**。
  ...
  ~~⑴ `override` / part property の文字列パイプラインを型付きの値にする~~ —
     ...
     **だからこの便では着手しなかった**）。**5 つに割れる。まとめて 1 便に入れないこと**:
     ...
     ⒟ **式の文法そのもの（1 件）**: `@name(引数)` run（`Music:875`）は token を無差別に
        ...
        **7 家族が `Semantics.AnnotationValues` の 1 コピーへ移り、`MarkName` の
        code read は 18 → 13・7 ファイル → 5**（監査 §9.5.2）。
        ⚠️ ★★ **設計の制約が 1 つ測れている**: **値に落とすと壊れる引数がある**
        （`@frame(032010)`→`Int(32010)` で先頭の 0 が消える・`@chord(c:m7)` は部分言語）
        ⇒ **引数ノードは「値」と「書かれた字」の両方を持つこと**（実装済み）。
        ⚠️⚠️ ★★★ **消す順の「値として読める家族」から `fig` を外すこと**——
        **第169 の実測で `@fig` は部分言語だった**（`@fig(#6)`・`@fig(6s)` は
        **run 1 つ／点 2 部**＝切れ目が一致しない・監査 §9.5.1）。
        **残るのは frame / fig / chord の 3 部分言語＋`@mark("A")` のラベル
        ＋点つき名前の家族**（`ParseMarkName`）。**着手順は監査 §9.5.3。**
