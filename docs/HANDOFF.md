...
⚠️ **残る未特定は 1 項だけ**: 手計算で最小を足すと
`3.737890 + 2.800000 + 0.0 + (padding 1 − min_offsets[0])` で、`min_offsets[0]` に
**次 system の staff rel −4.303666 を代入すると 11.841556**。実測は 12.000000 なので
**その代入が 0.158444 ぶん誤り**——**配置後の rel は最小 translation ではない**。
⇒ **`min_offsets[0]` そのものを押さえれば移植できる。** 次の probe は
**verse 1 のインクを変えた本**（床が既知量だけ動く）で、R* が 1:1 で追随すれば
**未特定項は定数**と分かり、そのまま読み取れる。
