...
最終更新 第307セッション＝**§1 が名指した「次の一手」の 1 つめ＝§2 C⑶（record モデルの同値性）を取り、閉じた**。**入り方は第298〜第306 と同じ**——**ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った**。⇒ ★★★ **本便の骨は「起票が*済んだ*と書いた計測が 2 つとも間違っていて、どちらも*毒を当てれば 30 秒*だった」**: **『値同値性に依存する箇所はゼロ』は 3 型ある（⑵）／『全 50 型の実測』は 51 型で落ちた 1 つが一番効く型（⑶）。** **掃きは 1977 冊・両側 Release・4 出力すべてバイト同一。**

⚠️⚠️ ★★★ **⑴ 起票の「record か class か」は*偽の二択*で、第3 の道が両方に勝った＝争点そのものが消えた。** **`record` のまま `Equals(T?)` と `GetHashCode()` を自分で書くと、コンパイラは合成をやめてそちらを使う**——**`with` も分解も `ToString` も残り、`==`・`Equals(object)`・`IndexOf`・`Contains`・`Dictionary`・`HashSet` が全部そこを通る**（`scratch/p307/eqprobe` で実測）。⇒ ★★★ **起票が「争点は `Staff` / `Measure` / `Voice` の 3 型だけ」と書いていたのは、*その 3 型が「実体だが `with` の集中先でもある」から*だった**——**`with` を失わない道が在れば、対立は最初から無い。** ★★ **`MusicItem` 族は*抽象基底 1 か所*で足りる**（**派生 record の合成 `Equals` は `base.Equals(...)` から始まるので identity が伝播する**・実測）。⇒ ★★ **教訓＝「A か B か」と書かれた起票は、*A と B が排他である理由*を先に訊く。** **ここでは「record の同値性は変えられない」という暗黙の前提だけが二択を作っていた。**（**ユーザー承認: 機構も射程も本便で確認**）

⚠️⚠️ ★★★ **⑵ 第306 の ✅「値同値性を*意図して*使っている箇所＝ゼロ」は外れで、3 型ある。** **第306 は `Distinct()` 20 箇所と `IndexOf`/`Contains` を*静的に数えて*そう書いた**（**その 20 箇所が全部*射影されたスカラー*だという調べ自体は正しかった**）。⇒ ★★★ **外れていたのは*探した綴り*のほう**——**依存は `SequenceEqual` と `==` で書かれていた＝「コレクションの中を探す」ではなく「まるごと 1 個を値で比べる」形**で、**コレクション検索を探す網には最初から掛からない。** ⇒ **毒が名指した**（39 型全部に identity → **5 本赤・1 本は行番号の artefact・残り 4 本が 3 型**）: **`GrobOverride` / `GrobRevert`**（`IncrementalCompiler` の `overridesUnchanged`）と **`BeamRestStem`**（`BeamDetector` の memo 健全性）。⚠️⚠️ ★★★ **反証は最初から手元にあった**——**`IncrementalCompiler` のコメント自身が「the override/revert collections themselves are compared by value below」と書いている**（第178・第306 に続く *`observed by: NOTHING` の兄弟*）。★★ **副産物として毒は*分類*も出した**: **3 型とも仕事は 1 つ＝「いま計算し直したものは、しまってあるものと同じ値か」＝キャッシュの妥当性判定。** ⇒ **この木で値同値性が担っている仕事はその 1 種類しかない**（→ RULES §5.3）。

⚠️⚠️ ★★★ **⑶ 「全 50 型の実測・数え直さなくてよい」と明記された計器が、1 つ落としていた。** **`scratch/p306/modelscan.py` の正規表現が `public` の直後に `readonly?`/`sealed?` しか許しておらず、`public abstract record MusicItem` を落としていた**——**実数は 51 型**で、**落ちた 1 つが*項目階層の抽象基底*＝この節で一番効く型**（**identity を 1 か所で効かせられる唯一の場所**）。⚠️ **落ちても表は 50 行きれいに出るので、読んだ人には欠落が見えない。** ⇒ ★★ **判定法は 1 行＝素朴な `grep -c` と計器の行数を突き合わせる**（→ RULES §5.3）。

⚠️⚠️ ★★★ **⑷ ⒜（内容が同じ 2 つが同時に存在しうるか）は「ありうる」で止めずに*実測できた*。** **574 冊 × collect 相の反射走査 ＋ tie/slur/beam/gliss の検出器**（`scratch/p307/twinscan.txt`）: **`MusicItem` 族 396 対 / 4 冊**（**`repeat tremolo` が書かれた 1 群を同一内容の `NoteItem` 多数に展開する**）・**`VoltaBracketItem` 4 / 2**・**`TieItem` 2 / 1**（★ **`chord-X-align-on-main-noteheads`＝第18号と同じ形が今も corpus に居る**）・**`Measure` 1 / 1**・**ほか 20 型は 0**。⇒ ★★ **判断軸に「ありうるか」と書いてあっても、*出るか*は測れることがある**——**測ると「理屈で実体」と「現に双子が出る」の区別が付き、起票の優先順が変わる。**

⚠️ ★★ **⑸ 実装は宣言 25 か所**（`MusicItem` 基底 1 ＋ 単発の印 18 ＋ 容れもの 6 ＝ **実体 31 型**）**・値 8 型はそのまま**。**規則の家は `LilySharp.Core/Svg/Model/ModelIdentity.cs`**（**実体側の `GetHashCode` は全部 `ModelIdentity.HashOf(this)` を通る＝規則に grep で当たる住所が 1 つできる**）。**札は `ModelEqualityKindTests` が両向きに保持**（**新型は札を貼るまで赤／実体が値に戻っても・値が実体になっても赤**）。⚠️ **札は*振る舞い*で読む**——**`MusicItem` 族は基底に override が在るので、宣言の形で読むと派生 6 型を「値」と誤判定する。** ★ **3 本とも毒で赤を見てから置いた。**

⚠️ ★★ **⑹ 第306 の「`ReferenceEquals` 30 箇所」も*数も読みも*外れだった**（§2 C⑶ に訂正とレシピ）。**数え方が書かれておらず再現できない**（レシピは今日 66 を返し、本便が足した 25 を引いて 41・Svg なら 28）。**読みのほうがもっと外れている**——**28 行の大半は双子への防御ではなく memo の「同じ*物*なら不変」という安い早道**で、**双子への防御は少数**。⇒ ★★ **同じ綴りが 2 つの別々の仕事に使われているので、数を 1 つにまとめた瞬間に読みが壊れる。** ⚠️ **本便の commit `c161b76f` のメッセージはこの「30」をそのまま引いてしまった**（訂正は §2 C⑶）。

⇒ ★★★ **次の一手**: **⑴ §2 U8 の残り**（`grace { }` 本体。**第298〜第302 が 1 つずつ移植した続き**。★★ **射程は第306 が着手前に測って §2 に書いた＝ディスク 1973 冊で 47 冊・うち 45 冊は過去便のプローブ・ユーザーの 326 冊は 0**⇒**snapshot の再ベースはほぼ起きない。数え直さなくてよい**）。⚠️⚠️ ★★★ **ただし「U8 の残り」は 1 項目ではなく*難所の違う 3 つ*で、§2 自身が「同じ箱に入れてはいけない」と書いている**（第306 の §1 はこれを 1 行に畳んでいた。**第307 が §2 の本文を読んで気づいた＝着手はしていない**）: **⒜ 和音と休符＝難所は住所ではなく*模型***（`GraceNoteItem.Notes` が平らな `ImmutableArray<GraceNoteInfo>` で「この 2 つは同時に鳴る」「この 1 つは休符」を言えない。★ **§2 が着手する便への宿題を明記している**——**`StaffAccidentalColumns` と `NoteCollision` が grace を知らないのは「要らないから」か「取りこぼしているから」かを 1 冊で測ってから決める**。**今日 grace の和音は 1 つも彫られないので、その 2 軒の沈黙は空虚に真**）／**⒞ 本体のスラー・梁・タイ＝難所は `BeamLeftY`/`BeamRightY` が*単数*で 3 軒・`IsBeamedRun` が all-or-nothing**（**LP の規則は第302 が 16 冊で出した＝「先頭の 2 要素が*どちらも符頭*なら梁、さもなくば全部旗」。⚠️ 測ったのは WSL の v2.27.3＝*質的*確認で、座標を要るなら正典 2.26.0 で取り直す**——**その exe は第302 でこの機械を 13 分ブロックしている**）／**⒟ 注釈の族**。⇒ ★★ **どれも「仕事だけ」ではない**——**C⑶ が「決定済み・仕事だけ」と書かれていて 3 つ訂正が要ったのが第307**（⑴⑵⑶）。**入る便は 3 つのうち 1 つを選んでから始めること。**／**⑵ §2 の X/Y 側**（§2 A/B/D/E）／**⑶ §2 C⑴ の多声 walk の moment 順への再設計**（**最大の構造負債・未着手**。⚠️ **大手術なので踏む本が溜まってから**と §2 自身が書いている）。⚠️ **承認待ちは無い**（snapshot は 1 枚も動いていない）。⚠️ **§2 F は ⒫〜⒰ が全部 ✅・§2 C⑶ も本便で ✅**。

★ **開始時裏取り**: HEAD `fa40c089`・**未 push 47**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6802 / 0 / 4 / 6806**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 点 158（非ゼロ 2）／OPEN: 0**・追跡 `.lys` 581・snapshot 231——**第306 の終了時の数と 1 つも違わなかった**（**素朴に数えると `exact` 556・`OPEN:` 12 が出る。§0 のレシピは `-le 1e-6` と `why -like 'OPEN:*'`**）。
終了時: **commit 3 本**（**C⑶ 本体 `c161b76f`**＝**モデル 24 ファイル ＋ `ModelIdentity.cs` ＋ `ModelEqualityKindTests.cs` ＋ `APPROXIMATIONS.md` の行番号 2 つ** ／ **§1・§2 C⑶・RULES §5.3 の 2 則・第305 の経緯をアーカイブへ `57016af5`** ／ **§1 の「次の一手」を U8 の本文で精緻化（この commit）**＝**ユーザーが「次便はこのセッションでやる方が有利なら着手して」と訊いたので U8 の本文を読み、*着手しない*と答えた便の産物**。**理由は 3 つ**: **⑴ U8 の残りは難所の違う 3 つで、どれも投機ではなく LP 実測から始まる長い調査**／**⑵ ⒞ は正典 2.26.0 を要る場面があり、その exe はこの機械をブロックした前科がある**／**⑶ 本便の文脈は C⑶（record の同値性）で埋まっていて grace の彫版に 1 つも効かない——*閉じた話を抱えたまま次を始める*のは、この §1 が何便も記録してきた誤誘導の形。★ **逆に「いま読んだから安い」ものだけを置いた＝入口の精度**）・**未 push 50**⚠️ **数を閉じる commit が数を動かすので、最後の 1 本は*自分を数えて*「この commit」と書く**（第305・第306 と同じ形）・作業ツリー 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6805 / 0 / 4 / 6809**（**開始 6806 から +3＝`ModelEqualityKindTests` の 3 本だけ**）・**台帳 748／588／190／24.714570188／158 中 2／`OPEN:` 0・追跡 `.lys` 581・snapshot 231 は開始時と同じ**（**snapshot は 1 枚も動いていない**）。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけ**（**定数は 1 つも足していない**・`audit/magic_constants.csv` も不動・**REF/OWN の増減 0＝LP からの移植は 1 行も無い**）。⚠️ **間の push は無し**（`origin/master` は `be16a770` のまま）。★ **base 用の worktree `lys-base307` は消した。**
★★ **WSL ubuntu Release は Windows と一致を 1 度読んだ**: **`c161b76f` で 6805 / 0 / 4 / 6809**（build 7 秒 / test 26 秒）——**§0 の両脚を読んだ。**⚠️ **docs commit では ubuntu を回していない**（動いたのは `.md` だけ）。
⚠️ **計器は `scratch/p307/` に全部残っている**: **`identityscan.py`**（⒝＝値同値性に効く綴りを全部探す。**受信側をグローバルなメンバ表で解決する**・**毒で 3 site を名指させてから使った**）／**`poison_identity.py`**（**39 型全部に identity を入れる毒**。**混在 EOL を壊さないようバイトで書く**）／**`twinscan.txt` と `ZzTwinScanProbe.cs.txt`**（⒜＝574 冊の双子の実測。**テストは撤去済み**）／**`eqprobe/`**（**record ＋ 自前 `Equals` の振る舞いと、抽象基底からの伝播**）／**`sweep307.ps1` と `sweep307.json`**（**1977 冊・4 出力・全部バイト同一**）。
...
⇒ ★★★ **次の一手は 2 つあり、どちらも*決定は済んでいて仕事だけ***: **⑴ §2 C⑶ の型ごとの仕分け**（**ユーザー方針・判断軸 ⒜⒝⒞・全 50 型の実測・計器**が §2 に揃っている。**値同値性に依存する箇所はゼロと確認済みなので、失うものを探す作業は要らない**。**争点は `Staff` / `Measure` / `Voice` の 3 型だけ**）／**⑵ §2 U8 の残り**（`grace { }` 本体の和音・休符／スラー・梁・タイ／注釈。**第298〜第302 が 1 つずつ移植した続きで、残りは*本当に grob を要る*もの**＝レンダリング側の仕事。★★ **射程は本便が着手前に測って §2 に書いた＝ディスク 1973 冊で 47 冊・うち 45 冊は過去便の自分のプローブ・ユーザーの 326 冊は 0**⇒**snapshot の再ベースはほぼ起きない。数え直さなくてよい**）／**§2 の X/Y 側**。⚠️ **承認待ちは無い**（snapshot は 1 枚も動いていない）。⚠️ **§2 F は ⒫〜⒰ が全部 ✅ になった**——**開いている F 項目は無い。**
