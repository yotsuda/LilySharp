...
最終更新 第288セッション＝**ユーザー報告を 2 件閉じた（2 便）**——**U5（タブ譜と歌詞行が重なる）が `7900181c`**、**U6（`@rit` の長さが 1 本目と 2 本目で違う）が `fd46cdda`**。**台帳は 2 点増えた**（740 → **742**・**exact 582 は不動**）——**新しい対が 1 つと、その対照が釣った*別の*欠陥が 1 つ。** ⚠️ **U6 はユーザーが「どちらが正しいか確認して」と訊いたもの**で、**答えは「1 小節のほうが正しい」**。
...
終了時: **commit 5 本**（`7900181c` U5 のコード ＋ `afba7f09` ハンドオフ ＋ `6a669507` ubuntu 脚 ＋ `fd46cdda` U6 のコード ＋ **この 1 本＝U6 を §1 と §2 に入れるもの**）＝**未 push は 開始 12 ＋ 本便 5**・木 0・未追跡 0。**suite 6479 合格 / 0 失敗 / 4 skip / 合計 6483**（開始 6477 から **+6**＝**snapshot の番人 2 ＋ 台帳の点 2 ＋ 単体テスト 2**）。Core 0 エラー 0 警告。**追跡 `.lys` 578 冊**（+2）・**snapshot 228 枚**（+2）。**台帳 742 点／exact 582（不動）／ss 非ゼロ 190／総和 25.844537623／count 156 中 2。**
⚠️ ★ **本便は「ハンドオフを書いたあとにユーザーがもう 1 件報告した」形**——**だから §1 は 1 つの便に 2 つの島が入っている**。**`6a669507` までが U5 の締め、`fd46cdda` からが U6。**
⚠️ **掃きは 2 回**（**島ごとに 1 回・母集団はディスク上の全 `.lys`＝ユーザーの実コーパス込み・base と head は*本ごとに続けて***・第286 ⑼）:
...
**どちらも `lysc check` はバイト同一・レンダー失敗 0。**
⚠️ ★★ **U5 の掃きは 1 度やり直した**——**1 回目の途中で head の exe を建て直してしまった**ので、**前半と後半が別の exe だった**。**やり直しの head は最終の木とバイト同一であることを確かめてから回した。**⇒ ★★ **掃きの最中に建てない。建てたらやり直す。**（**U6 の掃きはこれを守った。**）
★★ **緑は*コード commit のあと*に取った**（第282 ⑴ の再発防止を踏襲）。★★ **WSL ubuntu Release も同じ 6479 / 0 / 4 / 6483**（`scratch/linux-repro` の手順・**`fd46cdda` の木で**——**§0 の「この機械の Windows での緑でしかない」に両脚で答えた**）。⚠️ **CI だけは未実行**（未 push 17 本）。
...
- **U6. ✅ 閉じた（第288・ユーザー報告 2026-08-29）＝`@rit` は「自分の次の演奏」で終わっていた**
  ...
  **「section の終わりまで」を既定にするか、終端の綴りを足すかは、決めるのはユーザー。**
  ⚠️⚠️ ★★★ **兄弟が同じ形をしている＝`HairpinEngraver` の `nextMark`／`nextDynamic` も
  `m != mark` で書かれている**（`HairpinEngraver.cs:603-618`）。**同じ「演奏が繰り返す」本では
  同じことが起きるはず**だが、**本便は*実証していない***——**`@cresc` を 3 通り書いたプローブが
  どれも wedge を出さず**（`scratch/p289/c1〜c4`）、**私の書き方が悪いのか engraver が黙るのかを
  切り分けていない**。**追跡 fixture `test/multi-staff-hairpins` は現に描けている**ので、
  **「`@cresc` が壊れている」ではなく「私のプローブが外れた」ほうが確からしい。**
  ⇒ ★ **hairpin に触る便は、まず `test/multi-staff-hairpins` の綴りで 2 回演奏の本を建て、
  1 本目と 2 本目の wedge の長さを比べること。** **そこが違えば同じ 1 行で閉じる。**
