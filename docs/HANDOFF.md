...
- **R3 collector の tuplet 腕が note/chord 腕より薄い**（`EmitScaledItem` `MusicWalk.cs:1940-2090`）＝`_cueDepth` を読まない（`cue { tuplet {…} }` が full size）・`@courtesy` の pre-scan 無し（form の再演で*だけ*出る）・`ExtractNoteheadStyle`・`GetFeatherDirection`・`_pendingGrace`・`_tremoloPairShape` が無い。直し方＝`DecorateSoundingItem` 1 軒を両腕から呼ぶ（5 箇所の逐語コピー `MusicWalk.cs:936/1173/1257/1342/1399` も畳む）。
