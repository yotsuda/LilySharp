...
最終更新 第288セッション＝**ユーザー報告 U5（タブ譜と歌詞行が重なる）を閉じた（1 便）**——commit は `7900181c`。**第287 が着手して外し、測ったことだけを §2 U5 に残していた本**。**台帳は 2 点増えた**（740 → **742**・**exact 582 は不動**）——**新しい対が 1 つと、その対照が釣った*別の*欠陥が 1 つ。**

**⑴ 引き継ぎの数は「§1 が第286 のまま」だったので、まず現在地を取り直した**。**§1 が名指す HEAD `5b00f5c9` はもう 2 本古い**——**第287 が U4（タブのタイと既定スタイル）と U5 の起票を積んで §1 を書き換えなかった**ので、**開始は HEAD `5cea12d5`・未 push 12・木 0・未追跡 0**。**その他は §1 の数から第287 の分だけ動いていた**: **suite 6473/0/4/6477**（§1 の 6476 から +1）・**追跡 576 冊**（+1）・**snapshot 226 枚**（+1）・**台帳 740／exact 582／ss 非ゼロ 188／総和 24.714341137／count 156 中 2 は 1 つも違わない**。⇒ ★★ **「§1 が最新とは限らない」**——**§1 の HEAD と `git log` が合わないときは、その差分が誰の便かを `git show --stat` で先に読むこと**（第287 の 2 本は `2201818f`／`5cea12d5`）。

⚠️⚠️ ★★★ **⑵ 原因は 1 つの量が 3 か所で綴られていたこと**（§5.2.1②）——**「この譜は何ユニット高いか」**。**⒜ `SkylineBuilder.SeedStaffSymbol` は*どの譜も*スコアの公称の半分で五線を撒いていた**（**その remark 自身が "this builder has ONE staff height, not one per staff" と書いていた**）ので、**6 弦タブは自分の一番下の弦を 1.750000 だけ自分の profile の*内側*に持っていた**。**⒝ `LyricEngraver.ResolveAnchor` はアンカー譜の*上端*から refpoint へ降りる段を同じ公称 2.000000 で踏んでいた**。**⒞ 3 つ目の綴りは*既に正しかった***——**system の silhouette は置かれた `StaffLayout.Height` を読んでいる**。⇒ **⒜⒝ を `MultiStaffLayouter.StaffHeightOf` と `StaffLayout.RefpointBelowTop`＝*既に知っていた家*へ寄せた。**

★★★ **⑶ 「どこを直すか」は台帳が名指ししていた**——**`page.tab-only.first-staff-refpoint` の `why` が「DO NOT CLOSE IT BY SPECIAL-CASING TAB: 同じ公称 4.000000 を `SkylineBuilder` も仮定している」と書いてある**。**第283〜287 はこの棚の前を何度も通っている**。⇒ ★★ **タブ／ossia の幾何に触る前に、台帳の tab 一族の `why` を読むこと。閉じた entry が次の島の地図を持っている。**

⚠️⚠️ ★★★ **⑷ 「真ん中の段だけ壊れる」の正体＝インクが間違った床を隠していた**。**段 1 と段 3 は範囲外の音でフレット番号が五線の*下*にぶら下がり**、**その番号が profile の底を弦より下へ押し下げる**ので、**間違った床は一度も binding にならなかった**。**段 2 はフレットが全部弦の内側**なので**床がそのまま出た**。⇒ ★★ **「一部の段／一部の本でだけ壊れる」は、しばしば「そこだけ*既定値が露出している*」の顔**——**壊れている側ではなく*壊れていない側*が何で隠しているかを見ること。**（**報告本 `Untitled-6.lys` は base では歌詞 4 行のうち 3 行が五線の中に居た**——**うち 1 行は下端の弦の上にちょうど乗っていた**。**head では 4 行とも五線の外。**）

⚠️⚠️ ★★★ **⑸ LP の SVG から測って 1 度間違えた**。**`audit/lp-geometry/README.md` は「SVG から測ってはいけない」と書いてある**のに、**私は LP の SVG から 6.0147 を読んで報告した**。**正しい 6.120115 は grob の dump**（`Measure-LilyPondProbe.ps1`）——**LP の*描いた baseline* と `VerticalAxisGroup` の refpoint は 0.105 違う**。⇒ ★★ **LP の量は grob に訊く。絵は「動いたか」を見るためのもの。**

**⑹ 新しい台帳の対**（`audit/lp-geometry/probes/tab-lyric-row.ly`・`Measure-LilyPondProbe.ps1` の安い側）: **TBL1 6.120115 ＝ 五線のインク 3.800000 ＋ 音節の ascender 1.820098 ＋ padding 0.500000**（**Lily# 4.370000 → 6.120000・残差 −0.000155**）／**TBL2（対照＝通常譜）5.500001 ＝ basic-distance そのもの**。⚠️ **予測 (a) は向きだけ当たり、(b) は外れた**——**対の差は 1.750000 ではなく 0.620114**。**理由は 1 つ**: **`Skyline::distance` は点ごと**なので、**通常譜の一番深いインク（符尾 3.550000）は音節の ascender の真下に居らず binding しない**が、**タブの弦は system の幅いっぱいに走るので*どの x でも*binding する**。⇒ **これがこの欠陥が長く見えなかった理由そのもの。**

⚠️⚠️ ★★★ **⑺ 対照が*別の*欠陥を釣った＝Lily# は歌詞のバネを*最小*に置いている**。**TBL2 は LP 5.500001 に対し Lily# 4.369960**（**残差 −1.130041486**）。**base の exe と head の exe でその本はバイト同一**なので**本便の代償ではない**。**LP の spec は floor ではなく*バネ***（`(basic-distance . 5.5) (padding . 0.5) (stretchability . 1)`・`minimum-distance` は無い・`ly/engraver-init.ly:649-652`）。⚠️ **`...last-system` の 2 点は両engine 5.500001 で exact** なので**「最後の段」一般ではない**——**違う変数は「1 段しか無いページ」**。**台帳に `OPEN:` で置いた**（次の一手はその `why` に書いてある）。⇒ ★★ **対照は「動かないことを確かめる網」であると同時に「主役が隠していた欠陥を出す装置」。**

**⑻ 測る側にも公称 5 線の仮定が入っていた**——**`RenderedGeometry.StaffRefpoints` は「5 本・1 ユニット間隔」を assert して 6 線タブで落ちる**。**`LyricBaselineBelowStaffOfLineCount` を足した**（既にある `StaffRefpointsOfLineCount` 一族に合わせ、**線の数は推測せず渡す**）。⇒ ★ **計器が欠陥と同じ仮定を持っていることがある。**

⚠️ **見つけて直していないもの**（前便から引き継いだ分に加えて）:
- **★★★ 歌詞のバネが最小に座る**（⑺・台帳 `lyrics.tab.control.staff-to-lyric` に `OPEN:` で起票済み・残差 **−1.130041486**）。**`5.5` を `minimum-distance` として入れて閉じてはいけない**（**LP に無く、`lyrics.band-floor` が「5.5 は縮む」を実測で持っている**）。**次の一手は `chainEnd.Room`（`NextStaffMinDistance` が NaN の枝）を LP の room と突き合わせること。**
- **★★ `SkylineBuilder` の `staffTopUp` も同じ量に寄せた**が、**その経路（tuplet bracket・slur・tie の seed）を*測った本は 1 冊も無い***——**掃きで動いた 34 冊はどれも歌詞かタブ対**。**タブの上に slur/tie/tuplet が載る本で LP と突き合わせる点は無い。**
（以下、前便からの一覧はそのまま）

⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 U の 5 件は全部閉じた**ので、**「読み手が見ている問題」の在庫はまた空**。**手元に在るのは ⑺（証拠が揃っていて、台帳の点が 1 つ赤く待っている）と、第286 が残した ⑹ の広い側（loose-line の閉じの距離を部屋の外側プロファイルへ移す）と、§1 の「要ユーザー判断」の束**。★ **ユーザーは「残債の返済をやめて、見えている問題を直せ」と言っている**ので、**見えるものが在るならそれが先。無ければ ⑺ が一番安くて一番証拠が揃っている一手**（**LP 側は測ってあり、番人になる台帳の点も既に置いてある**）。

★ **開始時裏取り**: HEAD `5cea12d5`・**未 push 12**・木 0・未追跡 0・Core 0 エラー 0 警告（`--no-incremental`）・**Windows Debug 6473 合格 / 0 失敗 / 4 skip / 合計 6477**・台帳 **740 点／exact 582／総和 24.714341137／ss 非ゼロ 188／count 156 中 2 が非ゼロ**・追跡 `.lys` **576 冊**・snapshot **226 枚**——**§1 の数からは第287 のぶんだけずれていた**（⑴）。
終了時: **commit 3 本**（`7900181c` コード ＋ `afba7f09` ハンドオフ ＋ **この 1 本＝ubuntu 脚の答えを §1 に入れるもの**）＝**未 push は 開始 12 ＋ 本便 3**・木 0・未追跡 0。**suite 6476 合格 / 0 失敗 / 4 skip / 合計 6480**（開始 6477 から **+3**＝**snapshot の番人 1 ＋ 台帳の点 2**）。Core 0 エラー 0 警告。**追跡 `.lys` 577 冊**（+1）・**snapshot 227 枚**（+1）。**台帳 742 点／exact 582（不動）／ss 非ゼロ 190／総和 25.844537623／count 156 中 2。**
⚠️ ★ **3 本目が要ったのは順序を 1 つ間違えたから**——**ubuntu 脚は「ハンドオフを含む木」で回すべきなので `afba7f09` のあとに回した**が、**その結果を §1 に書けるのは*さらにあと*になる**。**第285 の「『終了時』を書く commit を最後の 1 本にする」を守るなら、両脚の緑は §1 を書く*前*に取っておくこと**（**コードの緑は最後のコード commit のあと・ubuntu はその同じ木で**）。
⚠️ **掃きは母集団 1559 冊（ディスク上の全 `.lys`＝ユーザーの実コーパス込み）で、base と head を*本ごとに続けて*取った**（第286 ⑼）。**SAME 1525 / MOVED 34 / レンダー失敗 0**、**`lysc check` は 1559 冊すべてバイト同一**。**動いた 34 冊**: **ユーザー実コーパス 20 ＋ scratch 11 ＋ 追跡 3**（`test/tab-below-range`・新しい `test/tab-lyrics-inside-strings`・`audit/lp-regression/lys/automatic-polyphony-tabstaff`）。⚠️ ★★ **掃きは 1 度やり直した**——**1 回目の途中で head の exe を建て直してしまった**（⒝ を足したとき）ので、**前半と後半が別の exe だった**。**やり直しの head は最終の木とバイト同一であることを確かめてから回した。**⇒ ★★ **掃きの最中に建てない。建てたらやり直す。**
★★ **緑は*コード commit のあと*に取った**（第282 ⑴ の再発防止を踏襲）。★★ **WSL ubuntu Release も同じ 6476 / 0 / 4 / 6480**（`scratch/linux-repro` の手順・**`afba7f09` の木で**——**§0 の「この機械の Windows での緑でしかない」に両脚で答えた**）。⚠️ **CI だけは未実行**（未 push 15 本）。
