...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第240セッション＝**第239 が測って断った 7 件目を機構から着手して閉じ、そこからユーザーの報告 2 件を続けて閉じた。五線を持たない score まわりの「form が絵に届かない」3 島**（4 commit＝`bfe79e4c` 演奏順／`0991f108` handoff＋RULES 2 規則／`c2a34cee` 繰り返し小節線と volta／`a74f4eb4` 繰り返し点の縦位置／この行）。⚠️⚠️ ★★★ **3 島とも追跡 572 冊が 0 冊動いた**——**しかしそれは「届いていない」ではない**: 島ごとに毒を当てて**新テストが赤になることを先に見せてある**（順に 9 中 8／9 中 3／2 中 1）。**届く形をコーパスが 1 冊も綴らない**（第238② ・第239① と同じ形の **3 例目**）。⚠️ **台帳 573／ss 非ゼロ 115／総和 3.957938014／count 109・非ゼロ 2／snapshot 222／追跡コーパス 572 は 1 つも動いていない**——**出力を動かす変更を 3 つ入れて、再ベースは 1 枚も要らなかった。**
...
終了時: **本便は 5 commit＝`bfe79e4c`／`0991f108`／`c2a34cee`／`a74f4eb4`／この行**・**未 push 7**・未追跡 0/木 0・**5886/0/4**（＋12・全部新規）・**台帳 573／ss 非ゼロ 115／総和 3.957938014／count 109・非ゼロ 2／snapshot 222／追跡コーパス 572 —— 6 つとも不動**・Core 0 警告（`--no-incremental`）。⚠️ **本便は出力を動かす変更を 3 つ入れている**（五線なし × section を 2 度名指す／form の `|: :|`／4.0 でない帯の繰り返し点）が、**追跡 572 冊・snapshot 222 枚のどれもその形を綴らないので再ベースは 1 枚も要らなかった**。⚠️ **§7.5 の数え上げ**＝①②③ は Core の `+` 行 157・`LILYPOND-REF` 0・`LILYSHARP-OWN` 0——**これは正しい 0**: `form { A B A }` は Lily# 自身の構文で LP に対応物が無く、**この島が要る唯一の LP の事実**（囲いの無い alternative は 1 度だけ鳴る＝`alternative-sequence-iterator.cc:83-84`）は**既に `ProcessForm` が住所を持っている**。新アームは**その家を指しただけ**（§7.6 ⒟）。**⑤ は逆で、REF が本体**（`scm/bar-line.scm:360-368`）。数値定数は ⑤ の `0.5` が 1 個で、**それは前から在った 2 個を 1 個に畳んだもの**。
