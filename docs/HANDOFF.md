...
- ★★ **⒩ 打鍵の経路の*寸法を言っていない器*＝未値付け**（第438 の副産物）。1 軒を寸法ちょうどに
  したら **B／回 1,411.78 → 609.25**＝**差は全部*建て直し***。同じ形は `Svg/` の `new Dictionary<>()`／
  `new HashSet<>()` に **80 件以上**あり、大半が「長さの分かっている元から詰める」。**値段が先**
  ——1 軒ずつ継ぎ目を掛けるのではなく、**`Add` の回数と distinct を刷る継ぎ目を 1 本**（第438 ⑴ の型）
...
### 1.1 第438セッション（2026-09-20・YT-DELL2）
...
`/clear` 直後の新セッション。指示は「**HANDOFF を読んで着手**」で、**着手先は §1.0 ⒜ の ⒵⁶**（ユーザーが
選んだ・⒵⁶ と ⒵⁴ と ⒨ と LP 双子を並べて訊いた）。★ **`-Start p437` の 1 コマンドで §0 が全部済んだ**
（HEAD `558908aa`・**未 push 59**・**full `sessions/p437/run1.trx` 8773 / 0 / 3 / 8776＝第436 最終と一致**・
台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・§7 3.5 の `-Archive 435` も自動で回って
90 行 5,697 chars）。★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` の
ままだから＝第412〜第436 が予告したもの）。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第436 と
同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、
**before 7,926,916 B／打鍵は第436 最終の 7,926,940 と 24 B 違い**（＝同じ木・同じ母集団の証拠）。
⇒ **起票は 2 倍安く売られており、同じ形の隣の腕は 9 倍大きく、予測は*良い側*に外れて、外れ方が
「継ぎ目をどこに置くか」の所見だった。**

★★★ **⑴ 64 B は memo の答えの値段ではなく、*質問の作り方*の値段だった**（Lab `text-counts-before.txt`）。
⒵⁶ は第420 が「文字のシェイピング 318,784 回・distinct な綴りは 81・1 回 64 B＝合計 8.6%」と値付けした
起票で、**memo についての読みは正しかった**——**`tfm.run` 479,988 回で 696 B・`tfm.meas` 415,825 回で 0 B
＝答えは本当にタダ**。払っていたのは **`ScoreTextMetrics.Face` の `GetOrAdd` が毎回建てる delegate**で、
**615,475 回＝333.05／打鍵・ちょうど 64.00 B／回＝21,315 B／打鍵 0.269%**（起票の 0.130% の 2 倍）。
⚠️⚠️ **そして `tm.cold`＝0**＝**factory は 1 度も走っていない**（書名に `fonts { }` が無いので `Bundled` の
singleton 1 個・鍵は (role, style)）。★★★ **同じ計器で隣の腕を数えたら、そちらが 4.4 倍大きかった**
（第435 の規則の 2 例目）: **`MeasureContentKey.Getters` は 1,802,668 回＝975.47／打鍵・96.00 B／回
＝93,645 B／打鍵 1.181%**＝**打鍵の中で「温まった memo を読む」最頻の経路**。⚠️ **Core 全体を grep した
`GetOrAdd` の非 static factory はこの 2 つと `PdfDrawingContext`（PDF 専用・打鍵に居ない）だけ**。

★★★ **⑵ 直しは両方とも「訊き方を `static` にする」1 つの形**（`66365e69`・Core `+` 31 行〈**うち註 26 行**〉・
**数値定数 0・LP 由来の式 0・REF 0／OWN 0**＝§7.6 ⒟）: `GetOrAdd(key, key => …捕まえる…)` を
**`GetOrAdd(key, static (key, arg) => …, arg)`**（`TArg` 過載）にした。**コンパイラは `static` ラムダを
field に置く**ので、hit は 1 バイトも割り当てない。**アルゴリズムは 1 文字も変えていない**
（factory の本体は同じ・`_plan` が `self._plan` に、捕まえた `excluded` が引数の `excluded` になっただけ）。

★★★ **⑶ A/B ＝ −1.449%**（**7,926,916 → 7,812,021 B／打鍵**・before はこの便で取り直した・
**before は 2 回で 2 B 差・after は 2 回で 11 B 差**）。**冊ごとは 231 冊中 231 冊が改善**（この harness で
初めて全冊）・**中央値 −1.493%・最良 −4.759%**（`Universe.lys`）・**最悪でも −0.346%**（`アゲハ蝶.lys`）で、
**同じ木を 2 回回した対照帯は −0.015%〜+0.024%**。継ぎ目は **`tm.face` 21,315 → 0**・**`mck` 93,645 → 0**・
`tm.adv`／`tm.ink`／`tm.inkh` も 0 で、**歩いた数は 1 つも動かない**（615,475・1,802,668・479,988・
415,825・7,492 が前後で同一）。

⚠️⚠️ **⑷ 予測は*良い側*に外れ、外れ方が所見だった——第436 の教訓の*翌便での再発*。** 実装前の予測
（Lab `sessions/p437/prediction.txt`）は **P1 `tm.face`→0・P2 `tm.adv`/`ink`/`inkh`→0・P3 `mck`→0・
P5 歩いた数は不変・P6 出力同一＝全部的中**だが、**P4 の net −1.00〜−1.10% は外れた（実測 −1.449%）**。
原因は**継ぎ目の置き場所**——`mck` の継ぎ目を**メソッドの中**に置いたので **64.00 B／回**と読めたが、
**呼び手（`HashContent`）にもう 1 本置いたら 96.00 B／回**。⇒ ★★★ **差の 32 B は閉包の display class で、
`excluded` を捕まえた時点＝*そのメソッドの最初の文より前*に建っていた**＝**メソッドの中に書いた継ぎ目は
原理的に見られない**。⚠️ **同じ便に当たった例もある**: `tm.face` は外側（`Advance`/`Ink`/`InkHeight`）に
継ぎ目が在ったので 64.00 が正しく、**内訳 39,120,384 ＋ 外から呼ぶ 4,219 回 × 64 ＝ 39,390,400 ちょうど**で
和が閉じていた。⇒ ★★★ **「外側 1 段」は*そのメソッドの先頭*ではなく*呼び手*に置く**（RULES §5.3 へ 1 本）。

★★ **⑸ 出力同一は 3 つとも**: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline `17fb650e`）／
⑵ **実コーパス全ページ SHA-256＝5,824 行・0 行差**（第436 の `hashes-after.txt` と突き合わせ）／
⑶ suite 全緑・snapshot 249 枚不動・台帳 851 点／総和 22.584727806 不変。

★★★ **⑹ 毒 3 つ・被害者の見分けつき——そして *片方の腕にだけ観測者が居なかった*。**
引数になった瞬間に「呼び手が正しいものを渡す」が毒を当てられる性質になる:
⑴ **side の畳みに*内在*用の除外集合を渡す** → **8,776 本のうち 1 本も赤くならない**（side 項目の
絶対 `MeasureIndex` が鍵に戻る＝下流の memo が reuse を全部落とすが、**過敏は速度だけの害なので
どの網も見ていない**）／⑵ **内在の畳みに*side* 用の除外集合を渡す** → **7 本赤**（`BeamId` が鍵に戻る＝
第330 が閉じた病・`MeasureContentKeyTests` 1・`IncrementalCompilerTests` 5・`SystemLayoutCacheTests` 1）／
⑶ **`Face` に `this` でなく `Bundled` を渡す** → **4 本赤**（`FontDirectiveTests` 2・
`FontBlockCompletionTests` 1・`CliBatchTests` 1）。⇒ **網 1 本
（`MeasureContentKeyTests.SideTableAnnotations_AtDifferentPositions_ShareKey`）**＝同じ小節を 2 つ並べて
両方に `@f` を書き、**鍵が一致すること**と**`@f` が鍵に入っていること**を同時に張った。**毒 ⑴ はこの網
だけを赤にし、毒 ⑵ ではこの網は緑**。⇒ ★★★ **捕まえた変数は「黙って正しい」を保証していた。引数に
すると*渡し間違え*が可能になる——畳む便は、その新しい間違え方に網を張る**（第436 の「LINQ が黙って
保証していた性質」の、*閉包*版）。

★ **⑺ 終了時**: commit 2 本（code `66365e69`・docs 1 本＝この §1 と RULES §5.0 ×1・§5.3 ×1。**この文を
含むので SHA は書かない**＝§5.4）・**未 push 61**（開始時 59）・**最終 full `sessions/p437/run5.trx`
8774 / 0 / 3 / 8777**（**第436 最終の合計 8776 から +1＝今便の網ちょうど 1 本**。`Zz437`／`ZZPOISON` の
grep は**コードに 0 件**）・§7.5（対 `558908aa`）**Core `+` 31 行／REF 0／OWN 0＝算術を 1 つも足していない**
・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・**棚卸しは
`LILYSHARP_UPDATE_DOCS=1` で再生成して差 0**（`APPROXIMATIONS.md`・`magic_constants.csv` とも）。
★ **`-End p437 -DiffBase 558908aa` の門は 6 行とも OK**（天井・継ぎ目・毒と計器・棚卸し・作業ツリー・
未 push）。計器と全文は Lab
`sessions/p437/`（**`prediction.txt`＝実装前の予測**・**`text-counts-before.txt`／`text-counts-after.txt`
／`text-counts-before-outer.txt`＝*呼び手*に置いた継ぎ目**・`Zz437.cs.txt`／`Zz437Leg1.cs.txt`＝計器・
**`Zz437Ab.cs.txt`＝再利用可の A/B**・`Zz437Hash.cs.txt`＝証明 ⑵ の harness・`ab-before-1-tc0.txt`／
`ab-before-2-tc0.txt`／`ab-after-1-tc0.txt`／`ab-after-2-tc0.txt`・`zz437-seams.diff.txt`）。
⚠️ **`git diff > file` を pwsh MCP でやると ANSI 色が混ざって `git apply` が「No valid patches」と言う**
（CLAUDE-OPERATIONS §5 の `git grep` の罠と同じ・`-c color.diff=never` か色を剥がす）。
⇒ ⚠️⚠️ **HANDOFF は `-Archive 435` 後 435,757 B → 今便を書いて約 446,000 B＝天井 450,000 まで
残り 4,000 B 級**（`Fold-ClosedHandoffItems` の弁は第426 の時点で 0 件・§3 の圧力弁は第409・第410 で
使い切り。**天井は上げない**）。⚠️⚠️⚠️ **次便は §1 に 1 字も足せない＝`-Archive 436` を回すまで
書き始めないこと**（回せば今便の下のブロック約 9 KB が空く。**途中で回すと
`HandoffArchiveContinuityTests` が 3 本赤くなる**＝第410 ⑾）。
⇒ **次便は `-Start p438` の 1 コマンドから入る**（裏取り → `-Archive 436` 自動 → §1.0 の一覧）。
⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、
それは第412〜第437 の欠陥ではない。**作業ツリーは空**。**push はユーザー**（Lab も）。
