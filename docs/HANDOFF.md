...
⇒ ★★★★ **次の一手**: ✅ ⑺ が「どのインクか」を名指しした（mark・ただし Lily# は正しく予約している）ので、残りは**⒟ と ⒠ の 2 つ**。
**⑴ ⒟ を台帳点にする＝*静かな tab 段の上インク***: **LP は上向き符尾を 1 本も持たない段では上インクが tab の音部記号（refpoint 上 2.880）なのに、Lily# は 4.561＝上弦の上 2.25 に符尾の先端を予約している（+1.68 ss・Boogie の tab book の 12 段中 3 段）**。第337 で移植した `TabStaffGeometry.TabStemUp`／`GroupStemUp` の**向き**か、**符尾を持たない項（全休符・percent 反復の小節）にまで先端を播いている**かのどちらか——**`scratch/p338/ZzP338bProbeTests.cs.txt` の型で段を 1 本に絞り、`probes/tab-stem.ly` に「上向き符尾の無い段の上インク」の book を足して LP と対にする**（`page.tab-full.quiet-system.up-ink` の型）。台帳点が立ってから直す＝**出力を変える＝承認事項**。
**⑵ ⒠ 括弧を頁の silhouette に播くか決める**: `SkylineBuilder` は delimiter を 1 つも播かない（⑶）。**「both」で Lily# が 1 段多く詰める分の未予約インクはこれしか残っていない**（1.005 ss／段・8 段で ~8 ss ＞ mark の ~3.7 ss）。⚠️ **⑶ の「無罪」は*全頁から*括弧を落とした腕の結果で、1 頁目の余白の話としては未決**——**LP 側で「1 頁目だけ」を切り出した本（Boogie の先頭 8 段＋題）を作って A/B し直すこと**（`scratch/p338/lp-drop.ps1` に `-Grobs SystemStartBracket` を渡せば 30 秒）。
⚠️ **Lily# 側の段の高さを LP と並べるときは必ず ⑴（枠）の `ToFirst` を足す。** ⑶ 以下は第337 の並び（`lysc ly` の ChordNames＝ユーザーと綴りを決めてから／lyric row の slot の投票／ps2 の 0.017・beam-over-stem bar 1 の +0.33／push 後 `gh run list`／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／A/B/D/E／C⑴／G）。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
