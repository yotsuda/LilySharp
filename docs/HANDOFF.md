...
最終更新 第357セッション（2026-09-09）＝**入り方は第298〜第354 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。第356 ⑹ に「決定済み・未実装」は 0・⒟ の拾い箱は承認要だったので、**⒠ の並びから測れるものを 4 つ測って、2 つ直し、1 つは patch（承認待ち→第 5 便で適用）、1 つは開いたまま数を書き直した**: **⑴ MIDI の途中 `partial`（第352 ⑷⒞「未測」）＝欠陥だった→直した（commit `52b93e77`）**／**⑵ `lymelmel.lys` stale（第352 ⑷⒜）＝fixture を直して対を復元（commit `8f7ebf2c`）**／**⑶ `lyrics-verses` bar 4 の 0.19（第352 ⑷⒝「未分類」）＝分類できた＝keep-inside-line rod の両端の枠（LP 移植・出力が動く＝patch `scratch/p358/keep-inside-line-frame.patch`・承認待ち）**／**⑷ §2 U8b（二声の同時 grace）＝再測定して「住所は出来たが列の合体が無い」に書き直した（§2 U8b 末尾・製品 0）**。掃き 923 冊は ⑶ の patch について **MOVED 11**（下 ⑶）・台帳 810 不動・snapshot 249 不動（patch を当てると 10 枚動く）。**骨は 5**:
...
⇒ ★★★★ **⑹ 次の一手**: **⒜ push はユーザー**（第355 の 8 本＋第356 の 7 本＋この便 13 本）。**⒝ 閉じた＝第 5 便で合成 patch を適用・snapshot 11 枚再ベース済み**（patch 3 本は `scratch/p358` に残骸として残る・木は commit 済み）。**⒞ §3 に「決定済み・未実装」は 0**。**⒟ 第356 ⑹⒟ の拾い箱はそのまま**（跨ぐ音符を LP のように割る／梁を跨いで折る／双子は LYS1037 の本で LP と違う絵／段末の head の末尾 spring）。**⒠ 第355 ⑹⒞⒠⒡ の並びの残り**（F-fonts 拾い箱・`time none` の縁 ⑵⑶⑷・MIDI の途中 `partial` は閉じた・`lymelmel` は閉じた・`lyrics-verses` bar 4 と行端の OWN 2 量は第 5 便で閉じた・拍グリッド／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8b（列の合体・承認要）・U8／A/B/D/E／C⑴／G・段端の floor の regime）。⚠️ scratch は `p358`。**第 5 便は着手しない**: 残りは承認要（⒝・U8b）か、この便の道具立て（`p358` の exe・LP 計器）と無関係な regime（⒠）で、次のセッションでやる方が文脈の利が無い分だけ損もしない。
...
✅ ★★★★ **第 5 便（ユーザー「リベースを承認する」）＝承認パッケージを適用した＝合成 patch（⑶ の枠 ＋ 第 4 便の退役）を当て、snapshot 11 枚を再ベース、製品 commit `938a3e33`（21 ファイル +1130/−815）**。「⑶ だけか合成か」は指示に無く、**合成と読んだ**（第 3 便が「退役は ⑶ と一緒に承認ゲート」と置き、第 4 便の報告も 11 枚の再ベースで書いていた＝承認された数がそれ）。**手順**: `git apply scratch/p358/keep-inside-line-and-retirement.patch`（exit 0）→ solution build `--no-incremental` 0 エラー／Core 0 警告 → `scratch/p358/rebase-snapshots.ps1`（`LILYSHARP_UPDATE_SNAPSHOTS=1` で 247 本・env を消して再実行 247 全緑・**動いた snapshot は意図した 11 枚だけ**＝08-chorale／chordnames／chordrow-rit-second-system／lyric-break-pricing／lyrics-below-marcato／lyrics-verses／lyrics-volta／lyrics／rit-across-systems／sings-per-row／tab-lyrics-inside-strings）。**LP 照合（再ベース後の snapshot を直接読んだ）**: `test__lyrics-verses.svg` の bar 4＝**8.26**（LP 8.251）・最初の音符＝**8.59**（LP 8.585）＝⑶ の 2 点とも届いた。退役側は第 4 便の実物照合（hy2 の次段 5.80＝LP 5.8・段末 18.69＝LP 18.686）と greensleeves の 5.80 がそのまま。**full `scratch/p358/run9.trx`: 7510 / 0 / 4 / 7514（+11＝⑶ の番人 4 ＋退役の番人 7・snapshot 249 不動＝再ベースは枚数を変えない・台帳 810 不動・追跡 `.lys` 599 不動）**。commit は 2 本（製品＝Core 4＋Tests 3（新規 2 ファイル）＋snapshot 11＋CHANGELOG＋`audit/magic_constants.csv`＋APPROXIMATIONS／この HANDOFF）。**push はユーザー**。

✅ ★★ **第 6 便（同じ指示の 5 度目）＝この便の計器で残っていた唯一の未測＝段を跨ぐハイフン（`bright- | ly`）を LP がどちらの段に描くか（製品コード変更 0・番人 +1・commit `c7993d84`）**。**LP**（`scratch/p358/lv/hy-lp.svg` を直接読んだ・LyricHyphen には計器を掛けられない）: 頁に LyricHyphen の rect は **1 つだけ**＝system x **44.363**（頁 52.899 − 8.536）＝"bright" のインク右端（37.329＋7.034）＝行末小節線の右端（44.173＋0.19）から **0.66**（`length`）＝**段の右端を 0.66 越えて描き、次段には描かない**（line-start piece は `kill-zero-spanned-time`）。**Lily#**（`hy-head4.svg`）: 44.21→44.87＝小節線右端（44.02＋0.19）から 0.66・次段に無し＝**規則一致**（`LyricHyphenEngraver.CalculateHyphenLayout` の line-END piece＝右 bound が broken なら縮めも消しもしない・`AppendDashes` の `rightBroken`）。番人 `LineEdgeLyricReservationTests` +1（段末小節線の右端との差 ±0.02・長さ 0.66±0.01・次段の clef の下に dash 無し）。**ついでに見えた既知の島**: 同じ本の第 1 小節で列間が**毎対 −0.05**（LP 4.846／4.820／4.470 対 Lily# 4.79／4.77／4.42・ハイフン対も語間対も同じ量）＝`HyphenSpaceMinimum` 0.1／`WordSpaceMinimum` 0.45 は LP と同じなので音節の**インク幅**の側＝§2 F-fonts の face（LP は "LilyPond Serif" を C059 に解決・Lily# は Schola＝advance 同一でカーンが違う・"Twin"／"twin" の T·w 対）。数だけ置く・着手せず。**終了時: commit `c7993d84`（Tests 1 ファイル）・未 push 27・木 clean・full `scratch/p358/run10.trx` **7511 / 0 / 4 / 7515**（+1＝番人）・台帳 810／snapshot 249／追跡 `.lys` 599 不動**。**第 7 便は着手しない**: `p358` の計器で測れる未測はこれで尽きた。残りは承認要（U8b）か別 regime（⒠）。
