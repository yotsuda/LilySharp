...
- **⑼ ★★★ 残っている項＝打鍵の残りと、その内訳**（**第6便の*直前*に実測**・単位 MB）:
  ...
  **n=1000 なら 8.0 + 4.0 + 8.0 = 20.0 MB**で、**実測 19.3 とほぼ一致＝これで全部**。
  ⇒ ⚠️ **`dp[j,k]` は `k ≤ j` でしか意味を持たない**（小節数より多い行には割れない）ので、
  **構造的に半分は一度も触られない**。**jagged にすれば確保も `Array.Fill` も半減する。**
  ⚠️⚠️ ★★★ **それでも第191 は手を出していない。理由は「測れないもの」と引き換えになるから**——
  **jagged 化はホットな DP の内側に間接参照を 1 段増やす**ので、**アロケーションは半減するが
  *時間*は悪くなりうる**。**この機械では時間が測れない**（同一コードの対照が 16% 揺れる・§5.3）。
  ⇒ ★★ **着手の前に決めること**: **⒜ 時間の計器を先に作る**か、
  **⒝「アロケーションを取る」とユーザーが決める**。
  ⚠️ **配列を session で使い回す案は数字だけが下がる**——**`Array.Fill` の仕事は残る**ので、
  **`alloc` の改善幅をそのまま効果として書いてはいけない**（GC 圧は本当に下がる。両方書くこと）。
