...
⚠️ ★★ **掃きは並列化した**（`scratch/sweep289p.ps1`——**`scratch/` は git 管理外なので消えていたら建て直す。40 行**）。**直列版は 1582 冊で 2547 秒**かかっていた（**1 冊につき exe を 4 回起動＝約 6300 回**）。**守るのは 2 つだけ**: **base と head は同じ反復の中で叩く**（**2 パスに分けると、その間に編集された本が偽の差を出す**）／**一時 SVG は冊ごとに別名**（**直列版は `b.svg`／`h.svg` を使い回していて、並列にすると互いに潰し合う＝*他の本のページを読んで SAME と言う*掃き**）。
...
> ★ **`@ottava` の終端は既に語彙にある**（`@loco`）。**`@!ottava` と `@loco` のどちらを正典にするかは
> 決めていない**——**`@loco` は「原音に戻る」という*記譜の語*で、`@!ottava` は*構造の語***。
> **両方置くと「1 つの問いに 2 つの綴り」に戻る**ので、**決めるのはユーザー。**
>
