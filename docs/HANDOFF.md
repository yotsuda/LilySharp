...
最終更新 第424セッション（2026-09-19・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `823f1388`・**未 push 30**・**full `sessions/p424/run1.trx` 8743 / 0 / 3 / 8746＝第423 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・`Session-Check -Build -Test` 69 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第423 が予告したもの）。**§7 3.5 は*セッションの頭*で回した**（`-Archive 422`＝25 行 7,050 chars）。指示「HANDOFF を読んで着手」→ §1 の「次の一手」の 4 つを AskUserQuestion に出し、**ユーザーが ⒡「bow の carry 門を system ごとに割る 0.595%」を選んだ**。★ ベンチは着手前に AskUserQuestion で静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第423 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、**before 側の打鍵 8,206,672 B は第422 最終の 8,206,656 B と一致**:

★★★ **⑴ 起票 ⒡ の*賞金*は当たっていて、*直し方*が 119 分の 1 だった**（脚 1）。島は測り直して **0.594%**（tie 0.018 ＋ slur 0.575・第422 は 0.595%）。だが「門を system ごとに割る」で取れるのは **0.005%**——**拒否された staff の 32,707 系のうち止まっていたのは 1,223 系＝3.7%**（予測は 15〜40%＝**自分の falsifier「10% 未満なら死」を割った**）、**1,336 回の拒否のうち 1,090 回は*全系が動いていた*＝どう割っても救えない**。segment ごとに値付けすると slur は moved 5,472 本 0.347% / unmoved 80 本 0.005%。⇒ **§5.0 の「N を数え直す」の 12 例目で、外れたのは*直し方***（RULES §5.0 に規則を足した）。★ 副産物: **`RunPreliminaryAnnotationPass` の staffY 表の註「paging は system 自身の Y しか動かさない」は偽**（96.3% の系で staff の中の offset が動く。真なら門は 1 度も拒否しないはず）。

★★ **⑵ 脚 2 の予測も falsifier を割った。** 「余りは検出器の歩きだろう」（`LayoutTies` の註自身が「per system の呼び手はこの歩きを毎回払う」と書いている）＝**9.7%**。0.594% の内訳は: **segment の solve 0.352% / `beamByMember` 0.104% / measure の表 2 枚 0.070% / inside script 0.038% / 検出器 0.023% / 未計上 0.006%**。⚠️ **`setup.tuplet` は 231 冊で 1 度も発火しない**（slur の span が tuplet に触る本がこのコーパスに無い）。★ **予測の鋭い半分は当たった＝tie 側 2,750,640 B のうち 2,697,064 B（98%）は `DetectTies` の歩きで、solve した tie segment は*ゼロ*。1,336 staff/打鍵が「無い tie」を探して全曲を歩いている。**

★★★ **⑶ 島を開けたのは「却下された代案」のほうだった**（脚 3）。**第141 は門を入れたとき*選ばなかった案*を註に残していた**——「carried bows を delta で re-anchor すると 26,140 本中 87 本が 1 ULP ずれるので、代わりに UNMOVED で門にした」。**283 便のあいだ誰も値付けしなかった。** 今日の HEAD で測ると（製品の絵は動かさない比較のみ）: **5,552 本中 5,463 本がビット同一・89 本が 1 ULP・それ以上は 0・X と向きは全数一致**。⇒ **門の費用 0.594% に対し、誤差は 89 本の 1e-15。採った。**

★★★ **⑷ 直し＝門を退役させ、delta を払い戻す**（`850e4a7d`・Core 4 ファイル・**数値定数 0・LP 由来の式 0・REF 0／OWN 1**）。`BowLayout.ShiftedUp` が 4 つの Y-up 座標を動かし（算術は `ShiftedParts` の 1 軒＝§7.7 の「2 つ目の綴り」を作らない）、`BowShiftsOf` が系ごとの delta を出し、`ReanchorBows` が `RenderMeasureIndex` → 系で引いて当てる。**動かなかった staff は shift 配列すら作らず、同じインスタンスが返る**（＝staff 0 は常にここ）。**置けない bow が 1 本でもあれば staff 丸ごと再計算**＝門の姿勢はそのまま残した（推測しない）。

★★★ **⑸ 出力同一を 3 つとも走らせた**（RULES §5.1）: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline はこの便で `823f1388` の木から `git stash` して取り直した）／⑵ **実コーパス全ページの SHA-256＝5,824 行・0 行差**（第422〜第423 が描いていた絵そのもの。**89 本の 1 ULP を含んだまま 0 行差**＝RULES §5.0 に「判定するのは SHA であって ULP ではない」を足した）／⑶ suite 全緑 8743 / 0 / 3 / 8746・snapshot 249 枚不動・台帳不変。⚠️ **`ApproximationInventoryTests` が 1 本赤くなったのは正しい振る舞い**＝OWN が 142 → 143 に増えたので `LILYSHARP_UPDATE_DOCS=1` で再生成して commit に含めた。

★★ **⑹ A/B ＝ −0.5199%**（**15,165,937,700 → 15,087,093,380 B**・両側 TC=0・各側 2 回・床 0.0002%）。冊ごと（最悪の冊の床 0.0413%）: **−1% 以上 37 冊**・最良 `Yesterday Once More.lys` **−4.92%**（`China Grove` −4.85%・同 (Xanadu) −4.70%）・**床を超えて上がった冊は 0**（最も正の冊で +0.02%＝その冊自身の床と同じ）。⚠️ **0.594% との差 0.074% は re-anchor 自身の割当**（5,552 本の record ＋ builder）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: ★★★ **⒡′ 新規＝bow を*staff 自身の枠*で採点して offset は描画時に足す**（`DrawSlurs` が `SystemLayout.Y` に対して既にやっている形）＝**今日の払い戻しが要らなくなる 0.074% ＋ 1 ULP の尾が消える**。土台の変更・要設計／★★ **⒢′ 新規＝拒否された staff の*前処理* 0.235%**＝`beamByMember` 0.104%（prelim は per-system memo 経由で建てるので**同じ表ではない**＝CWT では当たらない・要設計）／measure の表 2 枚 0.070%（第422 の CWT は効いている。これは 1 打鍵 1 回の正味）／inside script 0.038%／**検出器 0.023%＝`StaffSpannerItemsOf` の memo を最終パスにも通すだけ（一番安い・出力同一が構成で言える）**。⚠️ **今日の直しで拒否経路を通る staff は激減した＝この 0.235% は*もう掛からない*。残っているのは「置けない bow」の fallback だけ＝まず回数を測ること**／★★ **⒵⁴ `prefixMarkAnchorX` の解き直し 0.338%**＝**memo は反証済み**（hit 率 0.53%）・土台の変更／★ **⒵⁵ `mm.g.split` 0.163%**（⚠️ `OrderBy` は*安定*ソート）／★ **⒵⁶ 文字の 318,784 回／81 綴り 0.130%**（安い日の仕事）／★ **⒴ `inkFirst`＋`inkLast` 0.53%**・未分割／★ **⒴⁵ artic と trill の remap 辞書は作って捨てている**（天井 0.174%）／★ **⒳″ `Padded` の中に残る複写 0.028%＋`SortAndResolve` 0.15%**（起票のみ）／★★ **⒴⁗ 天井 2.03%＝安くすると `アゲハ蝶.lys` 1 冊が 24→26 系になる＝perf ではなく*忠実度の判断*。開けるならユーザー決定が先**（実装は Lab `sessions/p423/zz423-deferred-prelim.diff.txt`）・**先に ⑹ の LP 双子**／⚠️ **`g4.core` 1.21% と `p1.s1.beams` 5.538% は*もう実仕事*＝この 2 島には戻らないこと**／⒜ **R13⒝ の実機確認**（第404 ⑵）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・未着手・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／LP 双子が要る R7〜R11（⚠️ **この機械の `lilypond.exe` の hang は第423 も 5 回踏んだ。次の手は Lab `notes/CLAUDE-OPERATIONS.md` §3＝タスクマネージャーの「待機チェーンの分析」**）／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑺ 終了時**: commit 2 本（code `850e4a7d`＝Core 4 ファイル＋`docs/APPROXIMATIONS.md` の再生成・docs 1 本＝§1 と RULES。**この文を含むので SHA は書かない**＝§5.4 の dead citation を自分で作らないため）＋Lab 1 本・**終了時の未 push 32**（開始時は 30）・**最終 full `sessions/p424/run5.trx` 8743 / 0 / 3 / 8746**（⚠️ **第423 最終と同じ＝網を 1 本も足していない。⒡ は*門を消した*変更で、消えた分岐に新しい観測者を足せない**——出力同一の観測者は ⑸ の 3 本）・§7.5（対 `823f1388`）**Core `+` 166 行／REF 0／OWN 1**（**うち実コードは約 76 行、残りは註**。算術は offset の加減のみ・§7.6 ⒞）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・棚卸しは `APPROXIMATIONS.md` が OWN 142 → 143（`audit/magic_constants.csv` は不変）。計器と全文は Lab `sessions/p424/`（`bow-carry-gate{,-2,-3}.txt`＝脚 1〜3 と各脚の予測・`ZzStage.cs.txt`・`Zz424Probe.cs.txt`・`zz424-seams.diff.txt`・**`Zz424Hash.cs.txt` と `Zz424Ab.cs.txt`＝再利用可の証明 ⑵ と A/B の harness**・`hashes-after.txt`・`ab-{before,after}-{1,2}-tc0.txt`・`zz424-reanchor.diff.txt`）。§7 3.5 は**セッションの頭で**済（`-Archive 422`＝25 行 7,050 chars）⇒ **HANDOFF は 448,671 B → アーカイブ後 434,660 B → 今便を書いて 445,598 B（この文ぶんを含む）＝天井 450,000 まで 4,402 B**。⚠️⚠️⚠️ **次便は §1 を 1 行も書く前に `-Archive 423` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない。** **作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第424 の欠陥ではない。
