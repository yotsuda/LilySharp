...
最終更新 第303セッション＝**ユーザーが「残債の返済をやめて、私に見えている問題を直して」と言い、見えていた問題は 2 つとも*同じ形の欠陥*だった＝1 つの事実を N 人が別々に決めていて、N が数えられていなかった**。**入り方は第293 と同じ言葉**（§2 U7 がその言葉を記録している）。**commit 6 本**（**書き出しの拍子復帰 `02613871` ／ form の未閉 `|:` の診断 `655149a8` ／ 決定と起票 `12a414b2` ／ 音部記号の主張の訂正 `b033df91` ／ volta の中の小節 `db030c14` ／ §5 への汎化と閉じ `a6c448d1` ／ ubuntu 脚と最終の数（この commit）**）。

⚠️⚠️⚠️ ★★★★ **⑴ 見えていた問題その 1 ＝ Venus.lys の LYS2002 3 本のうち 2 本が嘘だった。`MeasureValidator` だけが inline volta を「長さ 0 の不透明な 1 項目」として持っていた**。**collector は `[1. … ]` の音楽をその場に展開し**（`MeasureCollector.ProcessMusicNode` の `InlineVoltaSyntax` 腕）、**`MeasureModel.Flatten` は「inline-volta interiors are ordinary written measures and flow through as themselves」と*書いてある***。**このパスだけが違うことを言っていた。** 代償は 2 つ: **⒜ 走行中の音価が `]` を跨がない**ので `[2. a,8 … ] c c c c c c c c |` が「6 拍」と読まれる（ページは 8 分で描く）／**⒝ ending の中の小節は誰も数えない**——`[1. c'1 c'1 c'1 | ]` が 4/4 で無言。**直しは 18 行**（`SplitIntoMeasures` で ending を透過させる）。**射程**: **Venus 3 → 1**（残る 1 本は 13 行目の本物の `|` 抜け）／**43 冊で +96 本**（**全部、今まで ending の中に隠れていた本物**——抜き取りで 60/96 は `break` 前の `|` 抜け、残りも `Air on G String` の 7/8 小節・`ミュージック・アワー` の 2 番だけ短い小節）／**インクは動かない**（変化した 46 冊 **0/46**・LP 回帰 **0/81**）／**番人 4 本・毒で 2 本が赤**（⒜⒝ ちょうど）。⚠️ **96 本を出してよいかはユーザー判断だった——数を見せたうえで「入れる」と決まり、`db030c14` で入った。**

⚠️⚠️⚠️ ★★★★ **⑵ 見えていた問題その 2 ＝「三行目の調号と `|:` の隙間が広すぎる」は spacing ではなく*実在する空小節*だった**。`lysc layout` が数える（`system 3: bars 11-15 (5 bars)` に対し見えている音楽は 4 小節）。**第275 の決定（`… | |: …` は空小節を開く）そのもの**で、その決定自身が救済策を「冗長な `|` を落とすこと」と書いている。**Venus は 14 行目の `| break` から `|` を 1 文字落として直り**（93 → 92 小節・`|:` が調号の右に付く・LP 2.27.3 の双子と同じ配置）、**ユーザーはその後この本を form 側の repeat へ書き換えた。**

⚠️⚠️⚠️ ★★★★ **⑶ そこで第275 の射程を測り直したら、8 倍だった**。**第275 が書いた「ユーザーの手元の 9 冊」は*隣接した `| |:` の数とぴったり一致*する**（実測 9 箇所 / 9 冊）——**`| break |:` の形は数えられていない**（**78 箇所 / 66 冊**）。**合計 87 箇所 / 72 冊＝蔵書の 22%**、そして **72/72 で幽霊小節が実在する**（各冊を「冗長な `|` を落とした複製」と `lysc layout` で突き合わせ、**幽霊の総数 87**）。**隣接 9 箇所を 1 つずつ読んだが、空小節が欲しい所は 1 つも無い。** ⇒ ★★ **決定は据え置きだが、その射程の数は §3 で訂正した**（下）。**「レシピを置くだけでは足りない。置いたレシピが引き継がれている数を*再現するか*を打って確かめる」（§0）の、決定側の顔。**

⚠️⚠️ ★★★ **⑷ rule 2（`|:` は span を*閉じない*）を計測ゲートで試したら、エンジンは既に rule 2 の側に立っていた**。**`ArmBoundaryForStructuralBarline` と `_atScopeStart` は、その規則の例外を作るためだけに在る**——**実測: rule 2 で Arm を無効化しても赤は同じ 4 本のまま、`form { ~B |: ~A :| }` も `:|:` も不動**。suite は **4 本だけ赤（全部 `EmptyMeasureValidatorTests`＝第275 の番人そのもの）・snapshot 0 枚**。全木 218 冊のうち **89 冊が変化・幽霊 115 個が消える**（追跡側は 3 冊だけ＝第275 が「本便では直していない」と名指した本）。**`| | |:` は rule 2 でも空小節ちょうど 1 個**（現行は 2 個）で表現力は失われない。⇒ ★★★ **ユーザー判断: rule 2 は実装しない**（form 限定案が上位互換）。**ゲートは木から落とした。**

⚠️⚠️ ★★★ **⑸ section 境界を読み切った——1 箇所（`ProcessSectionPrologue`）で決まっている**。**リセット**: relative の枠／音価・付点／octave モード／**拍子**／**調**（＋ambient tonic）／小節境界／grob override。**引き継ぐ**: **テンポだけ**。⚠️⚠️ ★★★ **ここは 1 度間違えて書いた——「音部記号も引き継ぐ」は誤り**で、**その根拠に使ったのが `lysc xml` の出力**だった。**同じ便の ⑹ で「xml は section 境界の復帰を書かない」と証明した直後に、その壊れた計器で clef を読んでいた。** ★★ **測り直し（ページ＝正しい証人）**: `section A { c'4 … | clef bass c'4 … | } section B { c'4 … | }` は **3 小節目で treble を描き直す**＝**音部記号はリセットされる**。**テンポは 3 小節目に何も刷らず、MIDI も 120 / 120 / 60 のまま戻らない**＝**引き継ぐ**。⇒ **壊れていると証明した計器は、その場で使うのをやめること。**★ **2 オクターブ下がるのはバグではない**: リセット先は `InitialOctave`（**明示 `part { octave N }` ＞ 楽器既定 ＞ 音部記号の既定**）＋ `LastPitchName='c'` なので、treble では裸の `g` は c4 に最も近い **G3**。section A が C5 に居たのは作者が `c'` と*書いた*から。⇒ **ユーザー決定: 調と拍子のリセットは良い。relative と音価のリセットも受け入れる——ただし引き継ぐ記法が要る**（§2 に起票。**phrase 参照は `~P'` / `~P,` で既にできる**＝実測 G3 → G4 → G2。**section 参照だけが受け付けない**）。

⚠️⚠️⚠️ ★★★★ **⑹ その測定の途中で 3 人目の欠陥が出た＝section 境界の拍子・調の復帰を、読み手 5 人のうち 3 人が別々に間違えていた**（`02613871`）。**ページと小節検査は正しい**。**`lysc ly` は調だけ戻す**（→ LilyPond に 4 分音符 4 つ入りの 3/4 小節を渡していた）／**`lysc xml` は両方戻さない**／**`lysc midi` は戻さない**。**原因は 3 つとも別**: ⒜ **ly は運び屋が在るのに拍子が載っていなかった**——`SectionPlayMarker` の doc は「**Carries what the collector decides at the same spot**」と書きながら `MarkLabel` と `HasHeaderKey` しか持っていない（**調のために作られた**）／⒝ **xml は `_keyFifths` を戻すだけで誰も dirty を立てず**、`StartNewMeasure` の `else if (_attributesDirty)` が一度も走らないので**復帰が無音**。拍子は復元自体が無い／⒞ **midi は `PlaySection` が ambient tonic と調号は戻すのに拍子は戻さない**。★ 共有の問いは **`ScoreHomeMeter`** が持つ（`ScoreHomeKey` の双子）。**射程**: 219 冊中 **16 冊が変化・すべて書き出しだけ**（ly 6・xml 15・midi 6。ユーザーの本 6 冊）・**インク 0/81**。**実例 `Endless Story.lys` は MusicXML が 54 小節目で A major に戻るようになった。**

⚠️⚠️ ★★★ **⑺ 番人が当たらなかった理由が記録に値する＝規則の名を冠した fixture が*観測できない形*だった**。`test/section-meter-resets-to-global` は **section A が score と同じ 4/4 で終わる**ので、戻しても何も変わらない。**「score と違う拍子で終わる section」の fixture が 1 つも無かった。** ⇒ ★★★ **規則に名前を付けた fixture を書いたら、「その規則を*破ったときに出力が変わるか*」を 1 度確かめること。** 新しい番人 `SectionBoundaryMeterRevertTests` は**同じ 1 冊を 5 通りに読む 1 ファイル**（読み手ごとに割ると次に足す読み手が次にずれる）。

⚠️⚠️ ★★★ **⑻ 4 つ目＝form の未閉 `|:` がファイル末尾まで食べて誤報を 5 本出していた**（`655149a8`）。`form main { ~Body |: A }` が `}`・`score`・`{`・`staff`・`}` を「form が持てないもの」として報告し、**本当のエラーは 6 本目**。item ループを form の `}` で止め、**`|:` の位置に LYS4017 を 1 本**。★★ **そして測って分かった片方向性**: **music の `|:` は form の `:|` で閉じられる**（通る・LYS4017 が score 展開まで判定を遅らせているのはこのため）が、**form の `|:` は music の `:|` では閉じられない**（form の repeat は括弧構文）。⇒ ★★★ **構造を form へ移そうとする人は、必ず*通らない側*に先に当たる——移すのは `|:` のほうだから。** これが `|:` を music から禁止する案の、いちばん強い証拠になった。

⚠️⚠️ ★★★ **⑼ 私の掃きが 1 度嘘をついた**。before/after の書き出し比較が「**219 冊全部・3 形式全部が変化**」と出た——**`$tmp` が 2 回目の `execute_command` で未定義**（pwsh MCP はローカル変数を跨がない＝memory `pwsh-mcp-does-not-see-bash-env` と同じ形の罠）で、**before 側が全部 `NONE`** だった。取り直して **16 冊**。⇒ ★★ **「全部が変わった」は、まず計器を疑う数字。**

⇒ ★★★ **次の一手＝§2 F ⒬（section 参照に `~B'` / `~B,`）と ⒫ の前提条件 ⑵（構造 section のラベル既定）**——**⒫（`|:` を form 限定に）はその 2 つが済むまで着手できない**。**⒭（section ヘッダの `clef` が効かない）と ⒮（余分な `\bar ":|."`）は独立した小粒。** ⚠️ **§2 U8 の残り**（`grace { }` 本体の和音・休符／スラー・梁・タイ／注釈の全族）は**本便では 1 行も触っていない**——ユーザーが「残債の返済をやめて」と言ったのがこの便の入り口だった。

★ **開始時裏取り**: HEAD `ec291b16`・**未 push 38**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6685 / 0 / 4 / 6689**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 点 158（非ゼロ 2）／OPEN: 0**・追跡 `.lys` 581・snapshot 231。
終了時: **commit 7 本**（`02613871` ／ `655149a8` ／ `12a414b2` ／ `b033df91` ／ `db030c14` ／ `a6c448d1` ／ この commit）・**未 push 45**・**Windows Debug 6701 / 0 / 4 / 6705**・**台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188 は不動**・**追跡 `.lys` 581・snapshot 231 も不動**・**作業ツリー 0・未追跡 0**。⚠️ **間の push は無し**（`origin/master` は `11f48202` のまま＝第302 以前）。
★★ **WSL ubuntu Release も同じ 6701 / 0 / 4 / 6705**（`a6c448d1` の木で・build 7 秒 / test 27 秒）——**§0 が「この機械の Windows の緑は Windows の緑でしかない」と言う両脚を、本便は両方読んだ**。
★ **§5 へ出した恒久知識は 2 つ**: **§5.3「壊れていると*証明した*計器は、その場で使うのをやめる」**（⑸ の訂正の汎化）と、**§5.4「規則の名を冠した fixture は、その規則を*破ったとき*に出力が変わるかを1 度確かめる」＋「1 つの規則を N 人が読むなら番人は 1 ファイルで N 通り読む」**（⑺ の汎化）。
⚠️ **計器は `scratch/p303/` に全部残っている**: `before.txt`／`after.txt`／`sweep.diff`（volta の 1768 冊掃き）・`bars2-*.txt`（rule 2 の小節数掃き）・`exp2-*.txt`（書き出しハッシュ掃き）・`ink-*.json`・`lpreg-after.json`・`probe/`・`sect/`・`readers/`。

## 以下は第302セッションの経緯

最終更新 第302セッション＝**§2 U8 の ⒜ から tuplet を閉じた＝あれは grob ではなく*容れ物*で、しかも落としていたのは*読み手 3 人全部*だった**。**入り方は第298〜第301 と同じ**——**ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った**ので、**§1 の「次の一手＝残りの U8」を取りに行き、その §1 自身が命じている「着手する便は、まず自分の取る 1 つを測ること」を先にやった**。⇒ ★★ **前便が「まだ測っていない」と*名指しで*書き残した 1 つ**（**⑻⒝＝「MIDI で tuplet の比を `_tupletStack` に積むべきか*は 1 度も測っていない*」**）**を測ったら、答えは LP に在り、しかも*ページと音の食い違いまで 1 つの機構から出ていた***。 **commit 9 本**（**本体 `d6bf1e1b` ／ docs `b296ed7e` ／ 数を閉じる `4093ffe5` ／ 射程の取り直し `ef6f430b` ／ 委任への答えと自己監査 `e8476165` ／ 2 度目の委任と難所の測定 `959658bd` ／ 3 度目の委任と `IsBeamedRun` の畳み込み `3d890da7` ／ 4 度目の委任と LP への設計質問 `81e4cd96` ／ 5 度目の委任とその規則の訂正（この commit）**）。 **掃きは 2 回・1761 冊・両側 Release・`svg --combined` ＋ `midi` ＋ `xml` ＋ `check`＝動いた本は 4 冊で、4 冊とも「grace 本体に tuplet を書く scratch のプローブ」**（第298・第300・第301・本便）。**追跡本は 1 冊も動かず、snapshot も 0 枚**。**台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188／追跡 `.lys` 581／snapshot 231 はすべて不動。**

⚠️⚠️⚠️ ★★★★ **⑴ 着手前の測定が起票の見積りを 1 段広げた＝tuplet を落としていたのは*ページだけ*ではなく*narrowing を持つ読み手 3 人全部*だった**（`scratch/p302/ab`・両側 Release・`data-pos` 伏せ）。**`grace { tuplet 3/2 { d'16 e' f' } } c'4 c'2.` の svg・`.mid`・MusicXML は 3 つとも「grace を 1 つも書かない本」と*バイト同一***（svg `9F55AC7B11E3`／mid `B62E55DB4319` 91B／xml `324F2BC7AC76` 1398B——**3 つとも対照 `c'4 c'2.` と同じハッシュ**）。**正しかったのは narrowing を持たない `.ly` 双子だけ**（`\grace { \tuplet 3/2 { d'16 e' f' } }`）。⇒ ★★ **第301 の phrase 参照は「4 人のうち 2 人が取り残された」形だったが、tuplet は*最初から誰も歩いていない*** ——**同じ 1 つの展開でページ・音・XML が同時に閉じた。**

⚠️⚠️⚠️ ★★★★ **⑵ そして LP では「鳴る長さは縮むのに描かれる音符は変わらない」が*2 つの決定ではなく 1 つの機構*だった**。**実測**（`scratch/p302/lp`・LP の `\midi`・division 384・**WSL の LP は v2.27.3 なので質的確認**）: **`\grace { d'16 e' f' } c'4 c'2.` は装飾音符を tick 0 / 21 / 43 に置き主音符を 64 で渡す／`\grace { \tuplet 3/2 { … } } c'4 c'2.` は 0 / 14 / 29 に置き 43 で渡す＝`round(64 × 2/3)`**。⇒ **LP は grace 本体の tuplet の比を*音*に掛ける。** ★★★ **その出どころ**: `LILYPOND-REF: lily/duration-scheme.cc:190-200 ly_duration_compress` ——**`\tuplet` は `TimeScaledMusic` を作り、その element は `normal/actual` で*compress された音楽*で、duration の compress は `factor_` を掛けるだけで `durlog_` と `dots_` を触らない**。**log と dots が符頭・旗・梁の本数を決め、factor が moment を決める** ⇒ **1 つの compress から 2 つの答えが出る**。**だからページの腕は*文書化された no-op*で、2 つの exporter の腕はそうではない。** ⇒ ★★★ **第301 が「ページは byte 同一」と測り、本便が「音は 2/3」と測り、*その 2 つを説明する 1 行が LP の中に在った***——**測定を 2 つ持ってから読みに行くと、規則ではなく*機構*が出る。**

⚠️⚠️ ★★★ **⑶ 容れ物を展開する修理でも、drop は*消えるとは限らない。半分になる***。**phrase 参照は grob を 0 個持つので第300 は drop 行ごと消せた**が、**tuplet は*括弧と数字*という grob を 2 つ持つ**（LP 実測・第301）。**だから LYS4020 は新しい kind（`GraceDropKind.Bracket`）で残り、文面が「どちらの半分を失ったか」を言う**: 「**the bracket and number of a tuplet inside 'grace { }' are not engraved … although the notes it holds ARE drawn**」。⚠️ **本体が結局何も彫らないとき**（`grace { tuplet 3/2 { <c' e'> <d' f'> } }`）**は約束のほうを引っ込める**——**嘘を立てたまま残さない**（毒 `d_bracketpromise` がその 1 行を赤くする）。⇒ ★★★ **判定法（第300 ⑴ の 1 段先）**: **「grob か容れ物か」で仕分けたあと、*容れ物が自分の grob を持つかを 1 行書く***。**0 個なら drop は消える／N 個なら drop は残って*文面が変わる*。**

⚠️⚠️ ★★★ **⑷ 番人は予告どおり自分で赤くなった、そして*同じ theory には戻せなかった***。**`GraceBodyValidatorTests.EverythingReported_IsAbsentFromThePage` の tuplet 行**（第298 が置いた）**が本便の最初の全数 run で単独で赤くなり、削除すべき行を自分で名乗った**——**第300 ⑸ が「phrase 参照の行は最初から無かった」と書いた、その*裏側*が実際に働いた 2 例目**（1 例目は第299 の付点）。⚠️⚠️ ★★★ **だが「閉じたら赤くなる網」は、閉じたあと*同じ theory に別の行として戻れるとは限らない***: **あの theory の形は「報告される ⇒ ページは対照と*同一***」で、**bracket の drop は*ページが動いた本*について報告される**。⇒ **別の網（`ATupletInAGraceBody_EngravesWhatItHolds`＝「対照と同一」かつ「grace 無しとは*違う*」かつ「警告は出る」）に移した。** ⇒ ★★ **番人の行を消すときは、その族が*新しい形の網*を要るかを問う。「行を消した」は「網を移した」ではない。**

⚠️⚠️ ★★★ **⑸ MusicXML はこの便で 1 つだけ narrowing を*決めた*＝`<time-modification>` は書き、`<notations><tuplet>` は書かない**。**理由は「grob を置く読み手は 2 人しかいない」**（`GraceBodySupport` 自身がそう書いている）——**括弧と数字はまさにページが持てない 2 つの grob**で、**XML だけがそれを描けと言うと、同じ本の LYS4020 が*その片方について嘘*になる**。**時間は MIDI も書き出すので書く。** ⇒ **`ATupletInAGraceBody_ClaimsNoBracketThePageDoesNotDraw` が「ページが括弧を覚えた日」に赤くなる行**（主旋律の腕に貼る綴りが既に在る）。

★ **⑹ 直し方は第300・第301 の写しで、新しい規則は 1 つも要らなかった**。**展開は 1 綴り**（`GraceBodySupport.Expand`）、**マーカー対は比だけを運ぶ**（`GraceTupletStartMarker` / `GraceTupletEndMarker`）、**読み手ごとに読む量だけを借りて返す**——**ページは何も読まない／MIDI と XML は `_tupletStack` に積む**（**`FractionToTicks`・`CurrentTupletRatio` が既に読んでいる家＝量を 1 つも足していない＝7.6 ⒟**）。⚠️ **phrase 参照と違い*枠は開かない***: **主旋律でも `tuplet 3/2 { d'16 e' f' } c'` の c は 16 分**なので、**tuplet の境界は音価記憶も相対オクターブもリセットしない**（毒 `e_pageframe` がその決定を裏返して赤くする）。⚠️ **pop に `Count > 0` の門は付けなかった**——**`_tupletStack` は主旋律と共有なので、門は不揃いな close を*安全にしない*（外側の tuplet の分を pop して成功を報告する）**＝7.7 の「バグを緑にする装置」。

⚠️⚠️ ★★★ **⑺ 毒 11 本すべて赤・赤集合すべて相異**（`scratch/p302/poison-results*.txt`）。★★ **今回の runner は赤の数の隣に*suite の合計*も刷る**——**本便で 1 度、`dotnet test` が合計 5307 で「成功!」と報告した**（**正しい合計は 6688**。**次の run では正しい数が出て、失敗は 0 のまま**）。⇒ ★★★ **「0 赤」も「緑」も、*合計を読むまで意味を持たない***（RULES §5.5 に汎化）。

★ **⑻ perf は測った**（§7.9）。**⒜ 診断経路**（`SemanticValidation.Run`＝LSP の打鍵ごと・3 run の床）: **grace 本体に tuplet を書く 200 冊分の本 6.312 / 6.399 ms・perf-grace200 6.924 / 6.898・perf-slurgrace300 9.718 / 9.695・perf-tuplet300 6.028 / 6.088**——**そして本便のコードを 1 行も通らない対照 `perf-plain1k` 自身が 71.63 / 72.18 で、base 側だけで 4.1% 揺れている**⇒ **上の差は全部その中**。**⒝ alloc**: **動いたのは grace-tuplet の本だけ（28.1 → 38.5 MB・+37%）で、それは*描かれるようになった 600 個の装飾音符*であって overhead ではない**（**perf-grace200 は +0.3 MB＝対照 perf-plain1k 自身の +0.7 より小さい**）。

⚠️⚠️ ★★★ **⑼ §2 の射程の表を閉じたあとに取り直したら、*自分の計器がコーパスの顔をしかけた***（`scratch/p302/reach.ps1`）。**本便の perf 用の本 `perf-gracetuplet200.lys` は grace tuplet を 200 行書くので、素直に数えると LYS4020 の 263 行中 204 行がそれ**——**「tuplet は実コーパスで一番多い族」という*完全に偽の*順位が出た**。**除いた実数は 63 行 / 47 冊**（**族別: annotation 20／beam 8／chord 8／rest 6／slur 6／tie 5／dynamic 4／tuplet の括弧 4／phrase 参照 2**）で、**追跡本は依然として 2 冊・2 冊ともタイ**＝**第301 の見立ては閉じたあとも成立**。⇒ ★★★ **判定法**: **コーパスの数を取り直すときは、*その便が足した本*を先に名指して除く**——**計測のために書いた本は、次に数える人にとっては「実コーパス」に見える。** ⚠️ **そして族は*消えず*、`tuplet 2` が `bracket 4` になった**——**⑶ の「drop は半分になる」が、表の上でもそう出た。**

⚠️⚠️⚠️ ★★★★ **⑽ §1 を閉じたあとに 2 本目が在る＝ユーザーが委任を出した**（「次便は、このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）。**答えは「残りの U8 には着手しない」**——**第297〜第301 と同じ答えだが、*今回は理由が証明できる***。**あちらは「温まった場所と次の項の場所がずれている」を*見立て*で書いていた**が、**本便は残り 3 項が要るものを名指しで数えられる**: **⒜ の和音と休符は grace の列が持てない grob を要り（LP 実測・第301）、⒞ のタイとスラーは `ItemIndex` を鍵にする局所辞書 3 軒を要り（第300 実測）、⒟ の注釈も同じ住所を要る**——**つまり残り全部が「装飾音符が名乗れない住所」に依存していて、それは本便が 1 行も開いていない唯一の部分**。**本便が閉じられたのは、*住所も grob も要らない最後の 1 つ*だったから。** ⇒ ★★ **委任の判定は「温まっているか」ではなく「次の項が要る*もの*を、今日触った場所が持っているか」**——**本便は持っていない。**

⚠️⚠️⚠️ ★★★★ **⑾ 代わりに取ったのは第300・第301 と同じ「30 分前の自分を測る」で、*今回は 2 件とも当たりだった*＝出荷済みの誤りが 2 つ出た。**
⒜ ★★★ **`MusicXmlExporter` のコメントが*存在しない記号*を名指していた**——**「ページが括弧を覚えた日、この腕は `StampTupletBracket` を主旋律と同じように貼る」と書いたが、`StampTupletBracket` は repo のどこにも無い**（主旋律の腕は `ProcessNode` の中に直書きで、helper は無い）。⇒ ★★★ **前向きの指示ほど危ない**——**読むのは「まだ本物と区別できない人」で、しかも `LpReferenceCitationTests` が守ってくれるのは *LilyPond* の記号名だけ。C# の記号を散文で名指しても、どのラチェットも見ていない。**
⒝ ★★★★ **網の doc が「On LilyPond 2.26.0's own `\midi`」と書いていたが、その run は WSL の v2.27.3 だった**——**正典は 2.26.0（RULES §5.2）で、「WSL の LP で台帳の点を取り直さない」は memory にも RULES にも在る**。**私は tick を台帳に入れてはいないが、*コメントが版を偽った*のは同じ族の嘘。** ⇒ **4 か所すべてに版を明記した**（`RelativeResetMarker` / `MidiExporter` / `MusicXmlExporter` / 網の doc）＋**§2 U8 にも**。⚠️ **正典で取り直そうとして失敗した**: **`C:\bin\lilypond-2.26.0\bin\lilypond.exe` は同じ 3 冊で*13 分*ブロックし 1 バイトも書かなかった**（**WS 28 MB＝起動しきっていない、この機械の持病の署名そのもの。従来の記録は 295〜460 秒なので範囲が伸びた**）。⇒ ★★ **だから重みを担いでいるのは tick ではなく*機構*のほう**（`ly_duration_compress` は v2.26.0 の *ソース*で読んだ）——**版が怪しい測定を引用するときは、正典の側に立つ文が別に在るかを確かめる。**
⒞ ★★ **そして「網の無い広がり」が 1 つ出た**——**本便の展開は MIDI の*和音と休符*の腕を tuplet の中へ広げたのに、誰も比が届いたか訊いていなかった**（**和音と休符は 2026-07-10 から鳴っている＝§2 U8 の開いている半分**）。**実測**（`scratch/p302/ab/e_chordrest_plain.lys` 対 `f_chordrest_tuplet.lys`・division 480）: **和音の 2 音とも 27 → 18、休符のあとの裸音符が 54 → 36、群は 81 → 54、主音符は 480 でちゃんと終わる**＝**全部正しかったが、正しさを言う網が 1 本も無かった**。⇒ **`AChordAndARestInAGraceTuplet_ScaleWithIt` を足した**（**毒 `f_midinopush`・`a_noexpand` の両方で赤くなるのを確かめた**）。⚠️ ★★★ **休符がこの族で一番静かに腐る**——**音を 1 つも出さないので、比が届かなくても*あとの装飾音符が遅れる*としか出ない**（**聞けば分かるが、音高の assert には映らない**）。
⇒ ★★★ **本便の教訓の形**: **第300・第301 の「30 分前の未測定の主張を測る」は*見積り*を直したが、本便のそれは*出荷済みのコメント 2 件*を直した**——**⇒ 委任の時間は「まだ測っていない数」だけでなく、*自分がこの便で書いた「〜と書いてある」の類*にも当てる。記号名と版番号は、測定と同じ意味で検算できる。**

⚠️⚠️⚠️ ★★★★ **⑿ ユーザーが委任を*同じ便で 2 度目*に出した。答えはやはり「実装には着手しない」だが、取った測定が*前回の私の答えを反証した***。**⑽ で「残り 3 項は全部『装飾音符が名乗れない住所』に依存する」と書き、しかもそれを*証明できる*と書いた**——**測ったら ⒜ には当たらなかった。** ⇒ ★★★★ **§2 U8 で 5 例目の「測らずに書いた見積り」で、著者は*1 手前の自分*。**（射程＝第293／難所＝第298／難所の場所＝第299／安さ＝第300／**難所の*範囲*＝第302 ⑽**。**しかも本便は「起票の数値は着手前に 1 回測る」を守って tuplet を閉じた、その同じ便で違反した。**）
**実測**（読むだけ・ビルド無し）: ⒜ **`ChordNoteInfo` は `ItemIndex` を 1 つも持たない**（住所は容れ物の `ChordItem` の側）／`RestItem` も同じ／⒝ **`ChordHeadPositioning.CalculateOffsets(notes, stemUp, noteValue, headScale)` は住所を取らず、しかも `SharedRenderer.Noteheads.cs:427` が `chord.IsCue ? EngravingDefaults.CueScale : 1.0` を渡している**＝**cue の和音が既に同じ家を小さい縮尺で通っている**／⒞ **住所を鍵にする 2 軒（`StaffAccidentalColumns` の `(measureIndex, voiceId, itemIndex, noteIndex)`・`NoteCollision`）は `grace` という語を 1 度も書いていない**（`grep -i grace` が両方 0 行）＝**装飾音符はその 2 軒を通らない**／⒟ **描画側の縮尺も既に在る**（`GraceNoteLayout.Scale`）。
⇒ ★★★★ **⒜ の難所は住所ではなく*模型***: **`GraceNoteItem.Notes` は平らな列で、「この 2 つは同時」とも「この 1 つは休符」とも言えない。** **⒞ と ⒟ だけが住所に依存する**（第300 が場所まで数えた局所辞書 3 軒）。⇒ ★★ **症状で作った箱を空にした同じ便が、今度は*難所*で 1 つに括っていた**——**「難所は住所」は ⒞・⒟ について正しく、⒜ に広げたのは私。**
⚠️ **測っていないこと**: **その 2 軒が grace を知らないのが「要らないから」か「取りこぼしているから」かは、この測定は答えない**（**今日 grace の和音は 1 つも彫られないので空虚に真**）。**⒜ に着手する便はそこを 1 冊で測ってから決める。**
⇒ ★★★ **それでも実装に着手しないのは、理由が*変わった*から**: **⒜ は住所待ちではないと分かった以上「次便の場所ではない」とはもう言えない**——**言えるのは「⒜ は `GraceNoteItem` の*模型変更*で、4 人の読み手・snapshot 231 枚・掃きが全部その下に在る」**＝**セッション終端で開ける大きさではない**（**本便は既に本体 1 本・docs 4 本を閉じ、両門緑・掃き済み**）。⇒ ★★ **委任の判定は 2 段になった**: **⑴ 次の項が要るものを今日の場所が持っているか**（⑽ の基準・**⒜ については今や「持っている」**）／**⑵ その項が*1 セッションの残りに収まる大きさか***（**⒜ は収まらない**）。**⑴ だけで断るのは、今回のように*測っていない* ⑴ を根拠にしうる。**

⚠️⚠️⚠️ ★★★★ **⒀ 委任の 3 度目。段 ⑴ は今度こそ GO だったので、*段 ⑵ を測った*——そして測った結果は「まだ大きい」、ただし理由が ⑿ で書いたものと違った。** **⑿ で「⒜ は住所待ちではない」と測った以上、今日の場所（`CollectGraceNotes`＝`GraceNoteInfo` を建てる所）は ⒜ が要るものを持っている**＝**段 ⑴ は GO**。**そこで段 ⑵ を*見積らずに数えた***: **grace の列を読む site は 11 ファイル・約 51 か所／`GraceNoteEngraver` だけで per-note の幾何（stem・beam・ledger・臨時記号・staff position・`IsBeamedRun`）が 8 つのループに 21 参照**。⚠️⚠️ ★★★★ **そして決め手は数ではなく*模型*だった**: **`IsBeamedRun` は run 全体の all-or-nothing で、`QuantGraceBeam` は `columnOffsets` を音符と 1:1 に要求する**（`columnOffsets.Length != notes.Length` なら諦める）——**つまり「頭でない構成員」を run に入れた瞬間に両方が綴れなくなる**。⇒ ★★★★ **⒜（和音・休符）と ⒞（梁）は*同じ模型の問題*だった**——**第300 が梁について測った「grace 群が*梁つき部分列と旗つき単独音*を持てるようにする模型変更」と、⒜ が要る「同時に鳴る構成員／休符の構成員」は、同じ 1 語（run の構成員は頭とは限らない）**。**起票は 2 項に分けて別の難所を書いているが、模型変更は 1 回で両方を払う。** ⇒ ★★ **だから ⒜ を単独で取るのは*高くつく*——⒞ と一緒に設計する便のものにする**（**⒜ だけ閉じても `IsBeamedRun` を 2 度書き換えることになる**）。

⚠️⚠️⚠️ ★★★★ **⒁ その測定が*セッションに収まる 1 つ*を掘り当てたので、それは取った＝`QuantGraceBeam` が `IsBeamedRun` を*綴り直していた***。**`IsBeamedRun` の doc は「FOUR READERS, ONE SENTENCE」と書き、しかも「一方が beamed で他方が flag なら埋めない箱を予約する——§7.7 の*同じ量の 2 つ目の綴り*だ」と*警告までしている***。**その 3 行下で `QuantGraceBeam` が `notes.Length < 2` ＋ 全 head の `BeamCountForDuration(...) < 1` を自分のループに inline していた**（**しかもそこのコメントが「the same gate the renderer draws by」と書いている**）。⇒ ★★★ **第301 の「N 人を grep で数える」規則の*穴***: **関数名で grep すると 2 か所しか出ない**（renderer と spacing）——**`Dots` は答えを引数で受け取り、`QuantGraceBeam` は素の量 `BeamCountForDuration` から再導出していた**。**数え方は 2 本要る: ⑴ 関数名／⑵ その関数が組み立てられている*素の量*の名前**（RULES §7.6 に汎化）。**畳んだ**（`if (!IsBeamedRun(notes)) return (null, null);`）。

⚠️⚠️⚠️ ★★★★ **⒂ そして「畳んだ側が load-bearing だ」を毒で示そうとして*反証された*＝本便 3 度目の「予測を実測の顔で書いた」。** **コメントに「`IsBeamedRun` を毒すと、畳む前は quanter のテストが緑で、畳んだ後は赤くなる」と書いてから測った**——**畳む前も後も*まったく同じ 29 本*が赤くなる**（`scratch/p302/foldproof.py`）。**当然で、beamed が 1 つも無ければ renderer と予約だけでページが動き、quanter の答えは誰も読まない。** ⇒ ★★★ **区別できないことは*畳んでよい証拠*であって、畳む必要が無い証拠ではない**——**価値は将来（その述語を変える便が教える家の数）に在り、実測はそれを*安全*と言っただけ**。**コメントを書き直した。** ⚠️ **畳んだあとは番人が付かない**（**再 inline しても赤くならない**）——**それでよい: 1 軒しか無ければ防いでいるドリフトは起こりようがない。** ⇒ ★★★ **本便はこの形を 3 度踏んだ**（⑾⒜ 存在しない記号／⑾⒝ 版番号／⒂ 毒の予測）——**3 度とも「書いた 30 分後に測って直した」ので出荷はされていないが、*同じ便で 3 度*は多い。⇒ 「〜と測った」と書く前に、それを*今から測るのか既に測ったのか*を 1 語で言う。**

⚠️⚠️⚠️ ★★★★ **⒃ 委任の 4 度目。段 ⑵ は依然 NO なので着手しないが、*次便が設計を始める前に要る 1 つの入力*を測った＝「LP は頭でない構成員をどう描くか」**（`scratch/p302/lp/member`・**WSL の v2.27.3＝質的確認**・11 冊）。**⒀ で「⒜ と ⒞ は同じ模型の問題」とコードから結論したので、その模型が*何を綴れなければならないか*は LP に訊くべき数**（§5.2＝発明せず移植する）。**答えは 2 つとも決定的**: ⑴ **和音は梁に何の影響も与えない**（先頭・中間・末尾のどこでも polygon 2・符頭 +4）＝**模型に要るのは「構成員は符頭を N 個持てる」だけ**／⑵ **休符は run を*隣り合う符頭の極大部分列*に割る**——**`{ d'16 e'16 r16 f'16 }` は polygon 2（d'–e' の梁）と*旗 1*（`f'`）を同時に持つ**＝**第300 が音価の混在で測ったのと同じ形**。⇒ ★★★★ **⒀ の結論（⒜ と ⒞ は 1 つの模型）が、コードとは*独立に* LP の出力からも出た**——**2 つの側から同じ答えが出たので、これは見立てではなく所見。** ⚠️⚠️ **外れ値を 1 つ残した**: **`{ r16 d'16 e'16 }` は隣り合う符頭 2 つを持つのに両方旗**（`{ d'16 e'16 r16 }` は梁）。**先頭の休符だけが違う理由は測っていないので、書かない**——**この 1 例に設計を合わせる前に、LP の auto-beam を読むかもう 1 冊測ること。** ⇒ ★★ **本便の委任 4 回の使い方**: **⑴ 自分の主張の監査／⑵ 難所の測定／⑶ 難所が掘り当てた畳み込み／⑷ 次の設計への LP 入力**——**どれも「開けたら閉じる必要がある仕事」ではなく、*次便の 1 時間目を消す仕事*だった。**

⚠️⚠️⚠️ ★★★★ **⒄ 委任の 5 度目。着手はしない**（**段 ⑴ は GO だが段 ⑵ が NO で、理由は*3 つ目***: **LP 実測で「和音は梁に影響 0」＝和音の側は安いと分かった**が、**和音だけ取ると `GraceNoteInfo` の模型を*2 度*決めることになる**——**休符と梁が同じ模型を要る以上、先に和音だけ決めた形はほぼ確実に間違う**。**「⒜ と ⒞ は 1 つの設計」を自分に適用した**）。⇒ **代わりに、⒃ で*自分が「外れ値」と括って出荷した 1 冊*を潰した——そしてそれは外れ値ではなく反証だった。** **⒃ は「休符は run を*極大*部分列に割る」と書いて commit した**が、**`{ d'16 r16 e'16 f'16 }`（e'–f' が隣り合うのに旗）がその規則を殺す**。**5 冊足して 2 分で 16 冊すべてに合う規則が出た**: **「先頭の 2 要素がどちらも符頭なら梁、さもなくば全部旗」**（**和音は符頭に数える**）。⇒ ★★★★ **教訓は本便で一番強い形**: **「規則＋ただし 1 例だけ外れる」を書いたら、それは規則ではない**——**外れ値と名づけた時点で、*反証を外れ値に改名しただけ*かもしれない。** **括り出す前に、その 1 例の周りを 5 冊測る**（RULES §5.0 に汎化）。⚠️ **本便 4 度目の自己訂正で、*出荷されたのはこれが初めて***（前の 3 つは 30 分以内に自分で捕まえた）——**commit `81e4cd96` の §2 U8 が 20 分間まちがった規則を持っていた。**

⇒ ★★★ **次の一手＝残りの U8＝⒜ の*和音と休符*（tuplet は本便で閉じた）／⒞ 本体の中のスラー・梁・タイ ／⒟ 注釈の全族**（§2 U8 の ▶）。⚠️⚠️ ★★★ **箱の中身が変わった**: **⒜ は 4 つ → 2 つになり、残る 2 つは*本当に grob を要る*側だけ**（和音は符頭が 1 つ増え、休符は休符 glyph が 1 つ増える＝第301 LP 実測）。**「症状で仕分けた箱」はこれで空になった。** ⚠️ **実コーパスの射程はいまもタイだけ**（追跡 2 冊・第301 実測）——**本便の掃きがそれを追認した（追跡本は 1 冊も動かなかった）。** ⚠️⚠️ ★★★ **難所は割れ、そして*一部は合流した***（⑿⒀ 実測）: **⒜ は住所ではなく `GraceNoteItem` の*模型*／⒟ と ⒞ の*スラー・タイ*だけが `ItemIndex` に依存する／そして ⒜ と ⒞ の*梁*は*同じ模型*の問題**（**`IsBeamedRun` の all-or-nothing と `columnOffsets` の 1:1 が「頭でない構成員」を綴れない**）。**⇒ ⒜ と ⒞ の梁は 1 つの設計で取る。** ⚠️⚠️ ★★★ **そして*どれを取るにせよ、読み手は 4 人いる***: **和音と休符は MIDI では 2026-07-10 から鳴っていて、ページと XML は落とす**（**本便が 4 出力で裏取りした: 和音だけの本も休符を足した本も、svg と xml は対照とバイト同一で `.mid` だけ動く**）——**その 2 つを閉じる便は、4 人が食い違ったままかどうかを毎回数えること。** ⚠️ **和音・休符を `GraceNoteInfo` の中に彫らないこと**（§5.2.1②）。

★ **開始時裏取り**: HEAD `2ff658d1`・**未 push 29**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6670 / 0 / 4 / 6674**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2・`OPEN:` 0**・追跡 `.lys` **581 冊**・snapshot **231 枚**——**§1 の終了時の数と 1 つも違わなかった。** ★★ **WSL ubuntu Release も同じ 6670 / 0 / 4 / 6674**（開始時に 1 回）。⚠️ **`origin/master` は `11f48202` のまま**＝**間で push は起きていない。**
終了時: **commit 9 本**（**本体 `d6bf1e1b` ／ docs `b296ed7e` ／ 数を閉じる `4093ffe5` ／ 射程の取り直し `ef6f430b` ／ 委任への答えと自己監査 `e8476165` ／ 2 度目の委任と難所の測定 `959658bd` ／ 3 度目の委任と `IsBeamedRun` の畳み込み `3d890da7` ／ 4 度目の委任と LP への設計質問 `81e4cd96` ／ 5 度目の委任とその規則の訂正（この commit）**）＝**未 push 38**（**この commit を含む。8 本目までの時点で 37 だったのを確かめてから書いた**）。⚠️ **8 本目と 9 本目はコードを 1 行も変えていない**（LP 実測と docs だけ）ので、**掃きも perf も取り直していない**。⚠️ **8 本目はコードを 1 行も変えていない**（LP 実測と docs だけ）ので、**掃きも perf も取り直していない**。★★ **2 回目の掃き**（畳み込みの出力不変を示すため・base `959658bd`）＝**1768 冊・4 出力・両側 Release・MOVED 0／`check` の差 0**（svg no-output 2 は両 exe で同じ）。⚠️ **`audit/magic_constants.csv` が 1 行動いた**（`GraceNoteEngraver.cs` の `0.5` が **395 → 413**＝**コメントで行がずれただけ**。**定数の追加も出所の変更も 0**。生成器で再生成した）・木 0・未追跡 0。**suite 6685 合格 / 0 失敗 / 4 skip / 合計 6689**（開始 6674 から **+15**＝**本便の網 16 ケース − 番人から消えた 1 行**）。⚠️ **⑾ の commit は Core を*コメントしか*変えていない**（`git diff` の Core 側の `+` 行は 1 行残らずコメント）ので、**掃きも perf も取り直していない**（§7.9 の「足していない例」）。Core 0 エラー 0 警告。**台帳・追跡コーパス・snapshot は開始時と同じ**（748／588／190／24.714570188／158 中 2／`OPEN:` 0／581 冊／231 枚）。⚠️ **`docs/APPROXIMATIONS.md` も `audit/magic_constants.csv` も 1 行も動いていない**——**本便は定数を 1 つも足していない**（**足した `LILYPOND-REF` は*決定*の出所であって量ではない**）。
★★ **WSL ubuntu Release も同じ 6684 / 0 / 4 / 6688**（**本体 `d6bf1e1b` の木と docs `b296ed7e` の木で 2 回＝どちらも同じ数**。**§1・§2 を書き換えたら全数をもう一度回す**——第298 ⑽ の棚）。⚠️ **CI は `11f48202` までしか走っておらず、未 push 30 本は未実行。**
⚠️ **`C:\MyProj\lys-base302` の worktree はこの便の最後に消した**（`git worktree remove`）。**次便が同じ名前で建て直してよい。**
⚠️ **計器は全部ディスクに残っている**: `scratch/p302/` の `ab/`（A/B の本と出力）・`lp/`（LP の `\midi` プローブ）・`dumpmidi.py`（MIDI の tick を刷る 30 行）・`ed.py`（BOM と CRLF を保つ編集器）・`poison.py`＋`runpoison*.ps1`＋`poison-results*.txt`・`sweep302.ps1`＋`sweep.json`・`diagperf/`＋`perf-gracetuplet200.lys`・`measurements.md`。⚠️ **`scratch/` は git 管理外。**
...
> ## ▶ **⒫ `|:` `:|` を form 限定にする（方向は決まった・前提条件が 2 つ）** ← **2026-08-31・第303 起票**
> **ユーザーの案**: **音楽ブロックには `|:` `:|` を書けなくし、`form {}` の中だけで書けるようにする。**
> **狙いは `.lys` の可読性**——構造が 1 か所に集まる。**方向には合意した。決定ではない**（前提条件が未了）。
...
> ⚠️ **前提条件 ⑴ ＝ 引き継ぎの記法**（下の ⒬）。**これ無しに禁止すると、繰り返し記号がオクターブを動かす言語になる。**
> ⚠️ **前提条件 ⑵ ＝ 構造だけを担う section にラベルを刷らない既定**。今は `~name` と書かないとページにラベルが増えるので、
> **繰り返しの縁を運ぶためだけに作った section が全部ラベルを持つ。**
> ⚠️ **小節途中で開く繰り返しは書けなくなる**——**実害無しと判断した**（記録として残す）。
> ★ **射程（判断基準ではなく事実）**: **ユーザー 326 冊中 127 冊が繰り返しを持ち、112 冊は music にしか書いていない。
> うち 98 冊は section が 1 個以下**＝書き換えではなく**切り直し**。**`[N. …]` を music に持つ本が 83 冊**。追跡側は 16 冊。
> ⚠️ **importer は既に半分 form 側**——`LysWriter.WriteVoltaSections` は first/second ending を「名前つき section ＋ form」に
> 分解する（「repeat + volta brackets live in the structure」）が、**単純な繰り返しは今も `BarlineBetween` が music に `|:` を書く。**

> ## ▶ **⒬ section 参照に音高・音価の引き継ぎ記法を入れる** ← **2026-08-31・第303 起票・⒫ の前提条件**
> **section 境界は relative の枠と音価をリセットする**（§1 ⑸。**ユーザーはそれを受け入れると決めた**・§3）。
> **足りないのは「引き継ぐ／ずらす」ための簡潔な綴り**で、**言語は既に半分持っている**:
> **phrase 参照は `~P'` / `~P,` で枠を動かせる**（実測 `~P` → G3 A3 B3 C4 ／ `~P'` → G4 ／ `~P,` → G2。
> `RelativeResetMarker.OctaveOffset` が運ぶ）。**ところが section 参照は受け付けない**——`form main { ~A ~B' }` は
> 「`'` is not something a form can hold」。⇒ ★★ **同じ印を section 参照に通すのが最小の追加**（新しい概念はゼロ）。
> ★ **音価は新しい記法が要らない**——section の最初の音符に数字を 1 つ書けば済む（LilyPond の綴りでもある）。
> ⚠️ **「前の section から引き継ぐ」を入れるなら参照側の opt-in にすること**（`~B~` のような印）。**宣言側に置いてはいけない**——
...
> ## ▶ **⒭ section ヘッダの `clef` が効かない（section-major で実測）** ← **2026-08-31・第303 起票・当初の起票内容は誤りだったので書き直した**
> ⚠️ **この項は最初「音部記号とテンポだけが引き継ぐ理由が書かれていない」として起票したが、前提が誤っていた**（§1 ⑸ の訂正）。**測り直した現状は、ユーザーの判断とちょうど一致している**——**調はリセット（正しい）／音部記号もリセット（しても音の意味は変わらないので悪くない）／テンポはリセットしない**（§3）。
> ★ **残る本当の穴は 1 つ**: **section-major の section ヘッダに書いた `clef` が効かない**。**実測**: `section A { clef bass m { c'4 c c c | } } section B { m { … } }` は **全部 treble のまま**で、`clef bass` が 1 度も描かれない（`scratch/p303/cleftempo/c1_header.lys`）。
> ⇒ **傍証**: **section ヘッダのレジストリは time / tempo / key / partial しか読まない**（`LilyPondExporter.BuildSectionHeaderRegistry`・`MeasureCollector` の `_sectionHeaderTimes` / `_sectionHeaderKeys`）——**`clef` の腕がどこにも無い**。**`_sectionResetClef` は在る**が、それは *part 既定へ戻す*ための変数で、**ヘッダの clef を*適用する*係ではない**。
> ⚠️ **「section ヘッダに `clef` を書ける」と言語が約束しているかは確かめていない**——**書けないのが仕様なら、黙って落とすのではなく診断を出すべき**、というのがこの項の問い。**part の `clef` と section 中の `clef` は両方とも効く**（同じ probe で確認済み）。

> ## ▶ **⒮ `form { ~B |: A :| }` ＋ music の volta で、`.ly` が余分な `\bar ":|."` を出す** ← **2026-08-31・第303 起票**
> **form の repeat と、section の music に残した `[1. … ] :| [2. … ]` を混ぜた綴り**は**通る**（`lysc check` 無警告）が、
> **`.ly` は `\repeat volta 2 { … } \alternative { … }` のあとに `\bar ":|."` を 1 本余計に出す**。
> **`scratch/p303/probe/z2_both_in_form.lys` が最小再現。** ⚠️ **ページ側は未確認**（`.ly` だけ見て起票した）。
> ★ **⒫ が入ればこの綴り自体が書けなくなる**ので、**⒫ の前に直すかどうかはその判断次第。**
...
| ★★★ **volta 用に切った section でも relative の音高と音価はリセットされる。引き継ぎは*記法*で与える**（2026-08-31・第303・**ユーザー決定**・記法は未実装） | ユーザー: 「volta を書くために導入したセクションにおいても、relative の音高と音価をリセットするのは受け入れても良い。ただし、わかりやすく簡潔な記法を導入して、音高と音価を引き継げるようにする文法があってもいい」。⇒ **リセットは据え置き**（§1 ⑸ の通り、2 オクターブ下がるのはバグではなく `InitialOctave` ＋ `LastPitchName='c'` の帰結）。**記法は §2 F ⒬ に起票**——**phrase 参照の `~P'` / `~P,` が既に在るので、それを section 参照に通すのが最小**。 |
