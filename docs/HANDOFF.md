...
最終更新 第275セッション＝**ユーザーが絵で見つけた欠陥を対の起票 → LP 実測 → 移植 → 全数検証まで完済し、その対照が開けた第 2 の欠陥までユーザーの指示で同じ便のうちに閉じ、そのあとユーザーが絵と編集器で見つけた欠陥を続けて 4 つ閉じた（6 幕＋拡張 1 本）**——第 1 幕はリードシートの `@chord(Fm)` の上を **volta ブラケットが線ごと突っ切る**（`scratch/ベースタブLy/君の恋人になったら（リードシート）.lys` L24）。commit は points `d86637f6`＋code `791e786e`（第 1 幕）／points `578ccd92`＋code `b5f04336`（第 2 幕・**snapshot 6 枚は承認のうえ再ベース**）／`4859f554`（第 3 幕）／`eaedb8ef`（第 4 幕）／`19057610`（第 5 幕）／`abde66c8`（第 6 幕）＋拡張 `fef8e4e1`＋ docs。
...
終了時: **本便の commit は 15 本**——points `d86637f6`（probe `volta-over-chord.ly`＋台帳 2 点・コード変更ゼロ）＋ code `791e786e`（和音記号を outside-staff の support に種として入れる・Core 3 ファイル）＋ docs ＋ **第 2 幕**の points `578ccd92`（点の中線への付け替え＋3 冊目 VOCF）＋ code `b5f04336`（volta の配置と番号を LP の宣言へ・Core 3 ファイル・**snapshot 6 枚をユーザー承認のうえ再ベース**）＋ **第 3 幕**の code `4859f554`（空小節 `| |` を診断せず spacer 1 小節で埋める・Core 6 ファイル・snapshot 1 枚）＋ **第 4 幕**の code `eaedb8ef`（`|:` を直前の小節線と対にする・Core 4 ファイル・sweep 4/572）＋ **第 5 幕**の code `19057610`（chords 補完を names 群 → roman 群に・LSP 1 ファイル）＋ **第 6 幕**の code `abde66c8`（前置きの音部記号・調号を全システムでクリック可能に・Core 2 ファイル・snapshot 5 枚）＋ 拡張の `fef8e4e1`（Export の "Reveal in Explorer" が空白入りパスでドキュメントを開く件）＝**points 2 本・code 7 本（うち 1 本は VS Code 拡張・1 本は LSP）・docs**（本 §1／§7-3.5 で第273 の経緯を ARCHIVE 冒頭へ）・**origin 不動（未 push 16・⚠️ ユーザー指示「プッシュはしないで」）**・木 0。**suite 6376 合格 / 0 失敗 / 4 skip / 合計 6380**（台帳テスト +3・第 3 幕で網 +8 −4 theory・第 4 幕で +10・第 5 幕 +1・第 6 幕 +4）。**Windows Release と WSL ubuntu 24.04 Release の両方で同値**（前半は 6363/0/4 合計 6367 で同じく両 OS）。⚠️ **ただし後半は 1 時間ぶん Windows が塞がれていた**——`LilySharp.Tests.dll`／`LilySharp.Core.dll` が **Debug も Release も** SAC に落ちた（§5.5・memory に第 5 の出方として追記）。**解けた手は `LilySharp.Tests\obj\Release` と `bin\Release` を消してからの素の再ビルド**（第273 の手が 2 例目で通った・50 秒）。**塞がれているあいだも `lysc.exe` と `LilySharp.Probe` は動いた**ので、rerender と全木 sweep は Windows のまま取れている。Core 0 エラー / 0 警告（`--no-incremental`）。**rerender 0/81**（第 1・第 2 幕）／**第 6 幕は 10/81 で、これも `data-pos` を落とすと 81 冊すべてバイト一致**・**全木 sweep は第 1 幕 0/572・第 2 幕 9/572（9 冊とも volta を持つ本）・第 3 幕 1/572（空小節の fixture 1 冊）・第 4 幕 4/572（連続 repeat の綴り 4 冊）・第 6 幕 48/572（⚠️ *`data-pos` 属性の追加だけ*——strip して比較するとバイト一致・絵は不動）**（`lysc.exe` と `LilySharp.Probe` は塞がれなかったので**この 2 つは Windows のまま**取れた）。**snapshot 12 枚（第 2 幕 6・第 3 幕 1・第 6 幕 5）**・一時変更 0（毒は全部 `git checkout` で復旧・測定と予測は `scratch/p275` 残置＝git 管理外）。忠実度計器の消費: **台帳 全点 733（+3）・exact 579/733（+2）・|残差|総和 23.639168・距離 571・count 162 中 160（不動）**——⚠️ **総和は +0.017625 しか増えていない**: 新しい 3 点が持ち込んだのは **−1.537050371 と −0.115 と +0.290365** で、**同じ便のうちに前 2 つが 0 になり 3 つ目が +0.017625 まで落ちた**から。**perf（§7-9）**: 打鍵経路に足したのは**種 1 つのループ**（和音記号を持つ本だけ・記号数に線形）と **memo の program の配列 1 本**、第 2 幕は**定数の置き換えだけ**——`Core` の他の計算は不変。
