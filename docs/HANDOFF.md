...
⇒ ★★★ **次の一手＝ユーザーが選ぶ。⒫ が閉じ、⒮ も同時に閉じたので、§2 F に残る持ち駒は 1 つ＝⒭（section ヘッダの `clef` が効かない・第303 起票・手つかず）**。**ほかは §2 U8 の残り**（`grace { }` 本体の和音・休符／スラー・梁・タイ／注釈——**本便は 1 行も触っていない**）**と §2 の X/Y 側**。⚠️ **承認待ちは無い**（snapshot 3 枚は承認を取って再ベース済み・⑽）。
...
終了時: **commit 8 本**（**HANDOFF の stray CR 正規化 `72c8ba59` ／ form の 3 つ目の括弧 `be1c9047` ／ form の `:|*N` が MIDI に届く `26038c65` ／ importer が単純な繰り返しを form へ分解 `17165b87` ／ **⒫ 本体 `4e61cd8a`** ／ §1・§2・§3・RULES とアーカイブ `ffbf485a` ／ snapshot 3 枚の再ベース `82ae6037` ／ 終了時の数の閉じ直し（この commit）**）・**未 push 24**・作業ツリー 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6763 / 0 / 4 / 6767**（**開始 6754 から +13＝新しい番人 `RepeatStructureScopeValidatorTests`**）・**台帳 748／588／190／24.714570188／158 中 2／`OPEN:` 0・追跡 `.lys` 581・snapshot 231 は開始時と同じ**（**枚数は不動・中身が 3 枚動いた**）。⚠️ **`docs/APPROXIMATIONS.md` は行番号だけ**（定数は 1 つも足していない・`audit/magic_constants.csv` も不動）。⚠️ **間の push は無し**（`origin/master` は `be16a770` のまま）。★ **第304 が残した `C:\MyProj\lys-base304` の worktree は本便で消した。**
★★ **WSL ubuntu Release も同じ数**（`4e61cd8a` の木で **6760 / 3 / 4 / 6767**・赤は再ベース前の snapshot 3 本ちょうど。build 7 秒 / test 35 秒）——**§0 の両脚を読んだ。**⚠️ **再ベース後の木（`82ae6037`）では ubuntu を回していない**——**動いたのは snapshot ファイル 3 枚だけで、Windows 側がその 3 本を緑にしたのは確認済み。**
...
⚠️ **計器は `scratch/p305/` に全部残っている**: **`sweep305.ps1`**（第304 の掃きに「増えた行が (p) の 1 本だけか」を足したもの）＋`sweep305.json`（着手前）／`sweep305-final.json`（完成木）・**`cmp.ps1`**（1 冊を head で描いて掃きの base ハッシュと突き合わせ、SVG が動いたときは `data-pos` を伏せてインクが同一かまで言う）・**`midinotes.ps1`**（SMF の note-on を数える。⚠️ **長い本では desync することがある——`grammar-2026-06-09` で 18 対 10 という嘘を出した**。**小さな probe 専用に使い、大きい本は MusicXML の小節数と音符数で見ること**）・**`base/`**（着手前の Release exe 一式）・probe 群（`a*` 陽性・`b*` 陰性・`c*` 3 つ目の括弧・`d*` `:|*N`）。⚠️ **`scratch/` は git 管理外。**
