...
最終更新 2026-07-28（第20セッション・**▶0 を閉じた——独立 lyrics 行が loose 鎖の要素になった**）/
HEAD・ahead 数は §0 で確認すること（⚠️ **ここに数字を書かない**——自己参照で、書いた瞬間から
commit のたびに嘘になる）。**全部未 push。**
**3434 passed / 0 failed / 3 skipped**・Core 0 warn 0 err・
**LP 忠実度 127/159 exact・total |residual| 2.506328 ss / 137 distances・counts 20/22**。
⚠️ **母数が 157→159 / 135→137 に動いている**（tab 対を 2 点開いた）ので**前セッションとの
総和比較はしない**。今セッション内では **4.256328 → 2.506328**（tab 点が exact になった分）。

前セッション 2.906328 からの **−0.400000 は `lyrics.row.two-verse.verse-step` 1 点が exact に
なった分**。母数は動いていないので**総和どうしを比較してよい回**で、**既存 156 点は 1 つも
動いていない**（**snapshot もゼロ枚**）。
...
### 第20セッション（2026-07-28） ＝ **▶0 を閉じた——行が「置かれる」だけでなく「解かれる」ようになった**

**判定条件は満たした**: LYRRV が **LYRV と六桁で同値**になった（page 1 の全 system で
gap **12.207200** / staff→verse1 **4.009200** / verse step **2.800000**、ページ端 system は
**5.500001**）。LP は 2 冊を 59 行完全一致で読むので、**Lily# の 2 綴りが一致すること自体が
LP の恒等の再現**。台帳 `lyrics.row.two-verse.verse-step` は **+0.400000 → exact**。
**snapshot は 1 枚も動いていない**（＝結果であって構成ではない。この regime を踏む fixture が無い）。

**入れたもの 3 つ**（どれも「行を Lyrics context として扱う」の別の面）:
- ① **鎖の要素キーを `verse` → `(alignment line, verse)` に一般化**し、**system の最後の
  spaceable 譜の下に立つ行の verse を、音符束縛の verse と同じ run に並べた**
  （`page-layout-problem.cc:919-925` は spaceable 2 本の間の非 spaceable を**1 本の run** に
  集める。2 本の鎖を同じ room に解くと重なる、が前セッションの申し送りだった）。
- ② **予約が同じ run を歩く**（`LyricReservationBelowSystem`）＝ **alignment 最小**を予約する。
  「描画到達点を予約する」枝は消えた（`:593-599`）。
- ③ **`RowSkylinesAboutBaseline` の verse step を `AlignmentWalk` で計算**（`max(2.8, ink+0.2)`）。
  平坦な `VerseSpacing` は行の経路から消えた。

**▶0 が名指していた 4 発明**: ⑴⑵⑶ は**コードから消えた**
（`RowReachBelowBandTop`・`RowReservationBelowSystem`・`row.Y >= anchorStaff.Y` を削除。
行の帰属は**位置比較でなく alignment 順**＝`LayoutEngine.ClassifySystem`）。
⑷ `IsSpecSpacedRowBoundary` も**同セッション中に消えた**（`13de8545`）——書いた時点では
「残した／別島」だったが、その別島（tab/ossia の refpoint 枠）を**同じ日に開けたので一緒に落ちた**。
⇒ **▶0 の 4 発明は 4 つとも消えている。**
⚠️ ★ **この 3 行は「残した」と書いたまま数時間 stale だった**——§0 の「『完了』表記は実コードで
裏を取る」が、**同一セッション内の記述にも効く**という実例。

- ★★ **副産物 1（今回の一番の所見）: 同じ walk の「種」が 2 綴りだった**（§5.2.1②）。
  **鎖は system スカイライン**から、**予約はアンカー譜自身のスカイライン**から歩いていた。
  アンカーが system の**最下端である限り同じ数**なので、何セッションも見えない。
  行を下に置いた瞬間に割れる: `BuildSystemSkylines` の DOWN は**最後の要素の下端**を取るので、
  **譜自身の下線が profile から落ちる**（実測: 音節の下の profile が 4.050000 でなく
  **3.000000**＝下向き符尾）→ 第1ばねの床が **1.050000 不足**。
  ⇒ ★ **同セッション中に本丸を直して merge は消した**（`327e3bb9`）: 各 edge 譜が
  **自分の 2 本の線を両方 seed** する。**両端が譜である限り足す線は内側なので inert**
  （実測: snapshot も台帳も不動）で、効くのは片端が text row のときだけ。
  ⚠️ **網が判別することを確かめた**——seed を戻すと `LyricRowIsSolvedLikeTheLyricsContextsItIs`
  が落ちる（LYRRV が 3.433015 対双子の 4.009200）。**2 つの変更が 1 つの量だという確認**。
- ⚠️ **hara-kiri で一度落ちた**: 種を per-system にしたのに base（`LastSpaceableStaffY`）が
  score 全体のままで、**system gap ちょうど（12.207200）**ずれた。
  ⇒ ★ **アンカーは「どの譜か」と「どの高さか」を必ず一緒に運ぶ**
  （`AnnotationLayoutContext.LastSpaceableStaff`）。片方だけ per-system にすると frame が割れる。
- ⚠️ **`VerticalSkyline.Raise` の符号はコメントの主張と逆だった**（DOWN スカイラインに `+a` で
  **上がる**）。**実測で確定**（merge が効かないので dump した）。§5.2.1① の「REF の隣の主張を疑う」
  と同じ形で、**符号は測って決める**。
- ★★ ⚠️ **同セッションで回帰を 1 件出して直した**（`fe679617` → `58f8b210`）。
  **`staff X with lyrics a` ＋ `lyrics b`（音符束縛と行が同じ譜にぶら下がる）で 2 行が重なった。**
  ⇒ 原因は**キーではなく族の分裂**: `BuildStaffAnchorTables` は「配列の最後の群でない」staff を
  upper 扱いにするが、**行は Lily# では独立した群**なので、行を足した瞬間に譜の歌詞が
  inter-group 鎖へ移り、行は below-system 鎖で解かれる＝**1 つの room に 2 本の鎖**。
  **「最後」を spaceable な群で数える**ように直した（数行下の `lastSpaceableStaffY` と同じ述語＝
  1 つの関数に「spaceable」の読みが 2 つあったのが欠陥）。
  ⚠️ ★ **`fe679617` の message はこの配置を名指していた。名指すことは確かめることではない。**
  4 行の本を `png --crop` で 1 回描けば commit 前に落ちていた。
  網は `RowUnderANoteBoundLine_IsOneRunWithIt_NotASecondChain`（修正前に落ちることを実証済）。
  ⚠️ **この対はまだ完全な恒等ではない**——step は双子と一致するが**最初の距離は 5.500000 対
  4.009200**で、**行の band が system を高くして room を増やす**から。テスト側に不等号＋理由で
  書いてあり、**band が消えた日に `Assert.Equal` にする**と明記した（▶3 の実害の 1 つ目）。
- ★ **§7.5 の監査は「他人の差分」でなく「自分の今日の差分」に効いた**（セッション末に実施）。
  Core +613 / −335、`LILYPOND-REF` +7、`LILYSHARP-OWN` **+1 / −5**（発明は差し引き 4 つ減）。
  **byte 一致のための分岐・除外はゼロ**（実際 21 枚動かしている）。出たのは 3 件で、
  **2 件はその場で直し、1 件は次セッションへ**:
  - ~~同じ選択が 2 か所~~ — **直した**（`f41d8f83`・`TextRowRefpointBelowTop` に 1 つの家）。
    ⚠️ **今日私が増やした §5.2.1②** で、監査以外では見つからなかった。
  - **`BuildSystemSkylines` のフレーム混在** — ▶0 の ⑷ へ（値段に影響された線引き）。
  - **`UnmodelledRow` の「降りる」は LP に無い**（LP は常に解く）。既存 bail-out の**拡張**で
    新種ではないが、`LILYSHARP-OWN:` のトークンは付けていない。`StaffSpanAt` の
    `placed` 未指定時 fallback も同型（製品経路は常に渡すので、いま効くのはテストだけ）。
- **網は `LyricRowIsSolvedLikeTheLyricsContextsItIs`**（LYRRV ≡ LYRV を system ごとに）。
  ⚠️ ★ **台帳点は「2.8」と書けば通る。この対は通らない**——そして**両側とも Lily# の書体**なので
  フォント量が相殺し、**書体を変えても生き続ける**。§5.4 の「値でなく規則」の、
  **恒等の対バージョン**。
...
★★ **tab／ossia の refpoint 枠の島は 3 つの扉を持つ 1 つの島**（2026-07-28 に確定）。
どれも**同じ変換**を要求する——**譜を「名目 4.0」でなく自分の置かれた高さで扱う**。

- ~~⑶ system silhouette の**外側の譜線**が名目高さ~~ — **閉じた**（`8e4857c8`）。
  6 弦タブ譜の extent が 4.100000 → **7.600000**＝LP の実測値ちょうど。**snapshot は 1 枚だけ**
  （`test/tab-with-chords` のページ高 +0.050000＝半線厚）。正当化キーは
  `tab.staff.line-span.six-string`（**新しい実測は不要だった**——LP の 7.6 はその `why` に既にある）。
- ~~⑴⑵ 群間ギャップの `GapSpan` が名目 `staffHeight`／`IsSpecSpacedRowBoundary` の門~~ —
  **閉じた**（`13de8545`）。門は削除、`GapSpan` は常に `RefpointSpanToGap`。
  `staff.tab-pair.staff-staff-inside` **10.750000 → exact（9.000000）**、control は不動。
  **snapshot 20 枚（tab 17 ＋ ossia 3）を名指しで再ベース**（キー＝上の台帳点）。
  **昔から言われている「~20 枚」はこの扉のことだった。**
  ⚠️ **ossia 対はこれで直っていない**——span は正しくなったが `gap * OssiaScale` が残る。
  book **OSSD** で実測: **LP は ossia 対も 9.000000**＝**譜は縮めても距離は縮めない**。
  導出では 6.949747 → 7.363961（LP 9.000000 へ向かう）、残り **1.636039** が scale の分。
  ⚠️ **OSSD はまだ台帳の対ではない**——小さい譜を**下**に置いており、Lily# の `ossia` は**上**。
  **上に置く本**で改めて起票すること（§5.0 の罠 5/6）。**これが次の扉。**
- ⑷ ★★ **`BuildSystemSkylines` が今フレーム混在。次セッションの最優先。**
  **譜線は placed span**（タブ 6 弦なら 0..−7.550000）だが、**音符・ビーム・clef は今も derived**
  （中央線 `-_staffHeight/2` = −2.000000）。タブ譜の実際の中央線は −3.750000 なので、
  **同じ関数の中で線と音符が 1.750000 ずれた枠にいる**。
  ⚠️ ★ **これは私が作った混在**（`8e4857c8`）。**変更前は両方 nominal で一貫**していた
  （全体を 4.0 の譜として扱う）。出力は改善（DOWN の極値＝譜の下端が正しくなった。音符は
  内側なので極値にならない）だが、**モデルは後退**。§7.5 の監査で自分で見つけた。
  ⚠️ ⇒ **「原理で切った」のではなく「安いところで切った」**：中央線/clef まで placement から
  採ると **1 枚 → 22 枚**。**この線引きは値段に影響された**と記録しておく。
  - **着手条件（これが無いと 22 枚を動かせない）**: **タブ譜の音符と clef が system silhouette に
    どの高さで入るかを測る台帳点**。⑶ が `tab.staff.line-span.six-string` で通せたのは
    **譜線**だったから。silhouette は LP から直接 dump できないので、**タブ譜だけの本で
    system 間距離を測る**対になる（`page-vertical.ly` に足して
    `Measure-LilyPondPageGeometry.ps1` を切り離し実行。**約 7 分半**、20 分ではない）。
  - **同時に閉じること**: 選択（text row を飛ばす）は placed offset を要求するので**分割不可**。
  ⚠️ 元の記述（選択が要素基準）はそのまま生きている——両端とも text row の system は譜が
  1 枚も入らない。
  ★ **⑴⑵ の後に測り直した（2026-07-28）: 中央線/clef の offset を placement から採るだけで
  snapshot 22 枚**（⑴⑵ 前は 19 枚。譜位置が動いたので増えた）。
  ⚠️ ★ **⑷ は分割できない**: 選択だけ直しても、間の譜は**derived な offset**（先頭譜の上端＝
  system 原点・高さ 4.0）で置かれるので座標が破綻する。**選択は placed offset を要求し、
  placed offset が 22 枚**。⇒ **⑷ ＝ 22 枚の 1 個の作業。**
  ⚠️ **そして今のところキーが無い**——tab 譜の**音符と clef**が system silhouette に
  どの高さで入るかを測る台帳点が存在しない。⑶ が `tab.staff.line-span.six-string` で
  通せたのは**譜線**だったから。**次に触る人は、まずその点を作ること。**

⚠️★★ **「この島は snapshot が ~20 枚動く」は粗すぎた**（何セッションも引き継がれ、私も
繰り返した）。**扉ごとに値段が違う**: ⑶ は **1 枚**、⑴⑵ が **20 枚**、⑷ が **22 枚**。
⇒ **島を「何枚動く」で見積もらない。どの量を変えるかまで割ってから数える。**
⚠️ **そして値段は他の扉を閉じると動く**——⑷ は ⑴⑵ の前は 19 枚、後は 22 枚。
**着手する直前に測り直すこと。**
...
- ★ **ossia 距離そのもの（`gap * OssiaScale`）＝ Stage 1 が裸にした発明**。
  ★ **2026-07-28 に LP 側を実測した**（book **OSSD**）: **LP は ossia 対も 9.000000**——
  tab 対とも notation 対とも同じで、**譜は magstep で縮めても距離は縮めない**。
  falsifier（`9 × magstep(−3)` = 6.363961）は**鳴らなかった**。
  ⇒ **`gap * OssiaScale` は LP に対応物のない発明**であることが測定で確定した。
  ⚠️ **span は `13de8545` で直った**ので、いま残るズレは**scale 1 個**（導出: refpoint 間
  7.363961 対 LP 9.000000）。
  ⚠️ **まだ台帳点にできない**——OSSD は小さい譜を**下**に置くが Lily# の `ossia` は**上**に付く。
  **上に置く本**で対を作り直すこと（§5.0 の罠 5/6）。
...
**非ゼロで残っている台帳点は 28 点**（前セッション 29 −
`lyrics.row.two-verse.verse-step`（**exact になった**）。
**残る 28 点は 1 つも動いていない**——行を鎖へ入れる移植が既存 regime に触れていないことの確認）:
