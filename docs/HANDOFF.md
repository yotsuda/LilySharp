...
（**実装 7 便＋道具 3 便**。**合成本の打鍵は 3304.5 → 171.7 MB＝−94.8%**。
★ **最後の 1 便だけがページ経路で、1 ページを超える*実ファイル*にも届く**——
`perf-plain1k` 95.9 → 62.7 MB＝−34.6%）。
...
| 1 打鍵 | 便前 | ⑴ | ⑵ | ⑶ | ⑷ | ⑸ | ⑹ | **⑺** |
|---|---|---|---|---|---|---|---|---|
| `perf-fingstack1k` | 3304.5 MB | 2815.6 | 2191.1 | 1362.0 | 524.9 | 244.2 | 177.1 | **171.7**（**−94.8%**） |
| `perf-scripts1k` | 1117.8 MB | 932.6 | 697.8 | 403.9 | 336.8 | 87.5 | 80.4 | **77.5**（**−93.1%**） |
| `perf-v2bow1k` | 157.1 MB | 157.1 | 152.9 | 152.9 | 152.9 | 87.9 | 87.9 | **83.7**（−46.7%） |
| `perf-plain1k` | 95.9 MB | 95.9 | 95.1 | 95.1 | 95.1 | 66.3 | 66.3 | **62.7**（**−34.6%**） |
