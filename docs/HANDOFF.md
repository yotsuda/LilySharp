...
⇒ ★★★★ **⑹ 次の一手**: **⒜ push は禁止のまま**。**⒝ F-fonts 段階 ⑵＝従わない 12 role**（上 ⑵）——1 role ずつ全読み手を `fonts.Size/Style` に付け替え、`PlanReachOf` に足す（Theory が両方向で赤→緑を見せる）。**instrument（`DrawInstrumentNames` の裸のリテラル）と partCombine（`CombineTextFontSize` 1 読み手）が一番安い**。fingering／figuredBass／fretFrame／tabFret は Emmentaler 数字＝「文字の em」ではなく glyph の magstep なので `StepOf` で連れる形（chordName の臨時記号と同じ）。**⒞ F-fonts の拾い箱（観測者 0）**: sustain の "Ped." グリフ列は `pedal step` に従わない（doc に明記・LP は `SustainPedal.font-size` で縮む＝乖離）／双子の role→grob 表は近似（`@text` の expressive は Lily# では dynamics role・双子は TextScript）／`size` を双子に書く手は無い。**⒟ 第353 ⑹ の並びはそのまま**（`time none` の縁 3 つ・`lymelmel.lys` stale／`lyrics-verses` bar 4 の 0.19／MIDI の途中 `partial`／拍グリッド／ps2 の 0.017／`audit/lpreg` 取り直し／§2 U8b・U8／A/B/D/E／C⑴／G）。**⒠ 段端の floor の regime**（第351 ⑹⒟）は今も未測。
...
- ✅ **F-fonts. `fonts { }` に*サイズとスタイル*を持たせる（2026-09-07・第342 起票。✅ 文法はユーザー承認済み——「その形で起票して。VS code editor の自動補完も、それに合わせて修正して」・✅ **第354 で実装＝段階 ⑴＋⑶（§1 第354）**）**
  ★★★ **入ったもの（第354）**: 文法どおり（`as` 転送・`step ±n`・`size n`・`bold`/`italic`/`regular`・順不同・次のキーで終わる）。**読み手＝`FontPlanReader.ReadAttributes`／`TextFontPlan.Binding(Step,Size,Style)`／`SizeOf`・`StepOf`・`StyleOf`（leaf→group→既定・属性ごと独立）／`ScoreTextMetrics.Size(role, default)`・`Style(role, default)`・`StepOf`**。**従う role の表＝`TextRoles.PlanReachOf`**（title・composer・lyricText・chordName・tempo・mark・pedal・navigation・text・dynamics・barNumber・tuplet・volta）——**validator（LYS8018 警告）とページの Theory `FontAttributeTests.TheReachTable_IsWhatThePageDoes`（全 role を `step +6` で描いて表と両方向に突き合わせ）が同じ表を読む**。診断 LYS8015（家族への属性・キー前の属性）／8016（数値なし・範囲外 step ±12・size 0.5..20）／8017（step と size 両方）／8018。**双子**＝`\layout { \context { \Score \override Grob.font-size = #n / font-series / font-shape } }`・title/composer は `\markup \fontsize`・navigation は markup 内（RehearsalMark を汚さない）・`size` は warning。**LSP**＝キー後に属性・`as` 後に家族・`step`/`size` 後は空・開いた entry では属性＋キー。**TextMate**＝`as FAMILY`／属性語／数値。**残り＝段階 ⑵（描画点に裸のリテラルが残る role）**: instrument・stanza・fretFrame・figuredBass・fingering（Emmentaler 数字＝文字 em の意味が無い）・partCombine・ottava・bend・tabTechnique・clefOctave・meter・tabFret——表に足すときは全読み手を `fonts.Size` に付け替えてから（Theory が赤で迎える）。⚠️ **sustain の "Ped." はグリフ列で `pedal step` に従わない**（doc に明記）。
