...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第238セッション＝**残債の返済を中断し、ユーザーが目で見ているリリースブロッカーを直した。歌詞の行が五線の音符を貫いており、5807 本・snapshot 222 枚・台帳 566 点は 1 つもそれを見ていなかった**（15 commit＝`65d5e048` 修正＋観測者 2 本／`be5273fb` handoff／`cc4c6094` 閉幕訂正／`f31a5af5` **LP 双子＋台帳 7 点**／`401b9055` handoff／`cc89de72` Linux 脚／`aecdfe0e` 2 件目の LP 双子／**ここからリリース**＝`8fd4f1cc` 0.4.0 版数／`6c07a516` publish 前 verify-pat／`4d265349` release.yml／`9fa2e769` GUID の dead 引用／`9b2f97f9` max-parallel／`e5ab0d11` 梱包／`6157fba8` RELEASING.md／この行）。⚠️⚠️ ★★★ **末尾でユーザーが「拡張として出したい」と言い、`repo を public 化 → 0.4.0 を Marketplace 公開`まで到達した**（⑩）。**本便は「残債を止めて見える欠陥を直す」で始まり、「製品を出す」で終わっている。**⚠️ **本便は engine を動かしている**（`LayoutEngine`／`LyricEngraver`）——にもかかわらず**台帳・snapshot・追跡コーパスは 1 バイトも動かなかった**。**それが症状ではなく、この便の最初の発見**（②）。⚠️⚠️ ★★★ **そして末尾でユーザーが「LP 双子で確認しろ」と言い、その台帳の穴は埋まった＝7 点**（⑧）。**修正は正しかったが、*正しい理由*は本便が予測したものと違った**——**6 つの予測のうち 3 つが外れ、書いておいた falsifier は正しい修正を*差し戻させる*ところだった**（骨 5）。
...
**⑩ ★★★ 0.4.0 を VS Code Marketplace に公開した**（`yotsuda.lilysharp`・**8 ターゲット全て validated**・GitHub Release は 12 資産）。**タグ 1 本で CLI と拡張の両方が出る経路が、初めて通しで動いた。**
⚠️ **publisher の所有は*測って*確定した**——`yotsuda` と `ytsuda` は**両方実在し両方あなたの表示名**（存在しない publisher は 404 を返すことを対照で確認）。第220 の「PAT 待ち」はここで解けた。**`60a68a7f` が「両方 author のもの」と*書いていた*が所有は実測していなかった**ので、`/manage/publishers/yotsuda` が出ることが唯一の確定材料だった。
⚠️ ★★ **Entra ID 認証（`vsce --azure-credential`）はこの publisher では使えない**。`az login` は通るが、**個人 MSA は自動生成の "Default Directory" テナントの Entra ユーザーとしてサインインし、それは publisher を所有する MSA とは別プリンシパル**。⇒ **切り分けの決め手は「同じ ID が*両方の* publisher で拒否された」こと**——片方だけなら「publisher 違い」、両方なら「アイデンティティの種類違い」。`DEPLOY.md` に全文で記録した（⚠️ **そのとき引用した object id が dead 引用になり CI が赤くなった**＝骨 7）。
⚠️ **課金停止は public 化で解けた**——**Actions は public repo なら標準ランナーが無料**で、停止は private の従量枠に効いていた。**3 秒で failure だった run が、公開後は分単位で回って緑**。⇒ **第235 から 4 便「門は課金停止」と引き継いできたが、それは*private である限り*の話だった。**
⚠️ **公開前監査（ユーザー指示）は全部クリーン**: `scratch/`（J-POP 群）は**全ブランチの全履歴で追加 0 件**・CJK 名パスも 0／追跡 187 タイトルは全て PD かテスト記述／資格情報ファイル 0・トークン様リテラル 0（唯一のヒットは base64 の WOFF2）／**ユーザーのメールは追跡ファイルに無い**（在るのは LilyPond 上流作者＝GPL 帰属で必須）／profanity 0（`lazy` 98 は `System.Lazy<T>`）／出荷コードに HACK/XXX 0。
★★ **そして唯一の実害は CI バッジだった**——README 6 行目、タイトル直下。**public 化した瞬間にトップが赤い failing になる**ところだった（実際は 5816/0/4 で緑）。⇒ **公開順序は「public → CI 緑を確認 → 告知」**。`RELEASING.md` に節として書いた。

**⑪ ★★★ そして公開は 8 中 5 しか通らず、しかも*赤い run の下で 5 つは生きていた*。**
⚠️⚠️ **初回公開は 8 個の独立操作ではない**——各脚が「拡張は在るか」を問い、**8 脚同時に「無い」と答えを得て 8 脚とも *create* を試み、1 勝 7 敗**（`The extension already exists.`＝重複バージョンの誤りに*見える*が違う）。
⚠️⚠️ ★★★ **その 7 敗のうち 4 つは実際には公開が通っていた。** 生き残ったのは alpine-x64／darwin-x64／linux-armhf／linux-x64／win32-arm64 の 5 つで、**本当に欠けていたのは 3 つ＝win32-x64（利用者の大半）／linux-arm64／darwin-arm64**。⇒ **run の色からは、どのプラットフォームが入れられるか分からない。**
⚠️ **失敗ジョブの再実行では直らず、手元から*逐次* publish したら 3 つとも一発で通った**——これが「ターゲット固有ではなく並行性」を確定させた。**版は切り直せない**（Marketplace は同じ番号を二度受け付けない）ので、手で仕上げるしかなかった。
⇒ **根治は `9b2f97f9` の `max-parallel: 1`＋`fail-fast: false`**（後者が無いと、公開済みの脚と未起動の脚が両方 "cancelled" になり、残骸が読めない）。⚠️⚠️ **ただしこれは*まだ一度も本番のタグを通っていない*。** 0.4.0 は並列版で走った。**次便が初走行**。
⚠️ **索引の遅れが「欠落」と見分けがつかない**: 公開後 `win32-x64` が現れ、他ターゲットの publish で再インデックスが走って**クエリから消え**、また戻った。その間 VS Code は「Windows 64bit で利用できません」と言い、**`vsce show` は 8 つ全部を返し続けていた**。⇒ **両者が食い違ったら待つ。ここで republish すると版を 1 つ無駄に焼く。**

**⑨ ★★★ 2 件目の LP 双子＝ユーザーの目視報告「`r16` の x はもう少し右でよさそう」に対し、*欠陥ではない*と答えた**（`audit/lp-geometry/probes/rest-column-x.ly`）。**対象**＝`scratch\ベースタブLy\r16.lys`＝1 小節 `e8 e b'16 e,8 e16 r16 e e8 b' gis`、休符は 6 番目（onset 8/16＝小節のちょうど中央）。
⚠️ **双子は手写しではなく `lysc ly` の出力**——**相対オクターブに `b'` と `e,` が混じる行を手で写すのは、双子が双子でなくなる典型**（RULES §5.5 が octave の罠として既に持っている）。
⚠️ **目で切り分けられない 2 つを分ける probe にした**＝⑴ **正しい column の中で glyph がずれている**のか ⑵ **column 自体がずれている**のか。**紙の上では同じに見え、コードでは無関係**（Rest の X-offset と spacing engine）。⇒ **NoteHead/Rest の refX と自分の X-extent、そして各自がぶら下がる PaperColumn を両方出す。**
★★ **結果＝10 列すべて一致・残差 3.6e-15**（＝浮動小数の雑音であって残差ではない）。**休符の列も 24.706000 で一致。**⇒ **直すものは無い。**
⚠️ ★★ **対（`r16` を同じ長さの音符に替えただけ）が P1 を半分否定した**——**休符自身の列は動かない**（両冊 24.706000）が、**その後ろの 4 列は全部ちょうど 0.104200 動く**＝**符頭幅 1.304200 − 休符幅 1.200000**。⇒ **spacing engine は「音符か休符か」は見ないが「幅」は見る**。**「列は長さで決まる」は粗すぎた。**
⚠️ **なぜそれでも左に見えるか**（報告への答えが「変更なし」なので書いておく）: **両側の白は 1.200000 と 1.200000 で完全に等しい**（前の符頭の墨は 23.506000 で終わり、休符の墨は 24.706000〜25.906000、次の符頭は 27.106000 から）。**墨で見て厳密に中央**——**非対称なのは glyph 自身**で、**16 分休符はこのフォントで最も非対称な字**。
⚠️ **台帳には入れていない**（意図的）: **量が exact** で、読むには `RenderedGeometry` に無い column-X ヘルパが要る——**0 を pin するために機械を建てるのは、欠陥の無い所に機械を置くこと**。**probes/ の 22 本は同じ立場**（entry を持たない probe は異常ではない）。**動いた日に entry を足す。**
⚠️ ★ **なお ⑧ の probe と違い、こちらは予測の falsifier が*発火しなかった***——**P3 は当たった**。**外れたのは P1 の後半だけ。**
...
終了時: 本便 15 本（`65d5e048` 修正／handoff 群／`f31a5af5`・`aecdfe0e` LP 双子 2 件＋台帳 7 点／**リリース 8 本**＝`8fd4f1cc` 版数・`6c07a516` verify-pat・`4d265349` release.yml・`9fa2e769` GUID・`9b2f97f9` max-parallel・`e5ab0d11` 梱包・`6157fba8` RELEASING.md・この行）＝**未 push 1**。⚠️⚠️ ★★ **ユーザーが本便の途中で 2 度 push し、後半は私が push した**（§0 第201 の形）——**両端だけ見ても便数と合わない**ので、この行が唯一の記録。・未追跡 0/木 0・suite **Windows 5816/0/4・WSL/Linux 5816/0/4＝両 OS 完全緑**・**台帳 566 → 573 点／ss 非ゼロ 110 → 115／総和 3.876038461 → 3.957938014／count 107 → 109・非ゼロ 2 のまま**・**snapshot 222・追跡コーパス 572 は不動**・Core 0 警告。⚠️⚠️ ★★★ **本便で*木の外*が大きく動いた。`git status` にも `rev-list` にも出ないので、この行が唯一の記録**（第237 骨 4 と同じ形）: **⑴ repo が public になった**（ユーザー実行）⇒ **Actions が無料枠で復活し CI が緑**——**「門は課金停止」は第235 から 4 便引き継いだが、*private である限り*の話だった。次便はこの前提を引き継がないこと。** **⑵ Marketplace に `yotsuda.lilysharp` 0.4.0 が 8 ターゲットで存在する**（うち 3 つは手で publish した）**⑶ タグ `v0.4.0` を打った**（annotated・`0b92a882` → `9fa2e769`）⇒ GitHub Release 12 資産・draft でも prerelease でもない **⑷ repo の `homepageUrl` に Marketplace を設定** **⑸ `VSCE_PAT` secret を登録**（ユーザー実行）**⑹ この機械に Azure CLI 2.89.1 が入り、`az login` の資格情報が `~/.azure` に残っている**（**Entra 経路はこの publisher では使えないので不要だが、将来 Entra 所有に移せば使える**）。⚠️ **`0.3.0` の孤児拡張ディレクトリ（188.9 MB）を削除した**——`extensions.json` に未登録で、**どのスクリプトも作らずどのスクリプトも消さない**孤児だった。
...
| ⑨ 2 件目の LP 双子（`aecdfe0e`・ユーザーの目視報告） | probe 1 本・台帳は増やさず | **「欠陥ではない」と 3.6e-15 で答えた**・**spacer は「種類」でなく「幅」を見ると判った** |
| ⑩ ★★★ **0.4.0 を Marketplace 公開**（ユーザー指示・8 commit） | 版数 7 か所・release.yml・`RELEASING.md`・梱包 | **第220 ① が閉じた**・**public 化で課金停止も解けた**・**公開前監査は全部クリーン** |
| ⑪ ★★ 公開レースの根治（`9b2f97f9`） | `max-parallel: 1`＋`fail-fast: false` | **8 中 3 を手で補完した経験から**・**未走行なので次便が初走行** |

- **⑺ ★★★ 残債は 1 件も返していない**（ユーザーの指示で中断）。**第237 ⑸ の一覧をそのまま引き継ぐ**: 言語仕様の宿題は §1.2 リネーム（ユーザーが MSVS で）だけ／**名指し穴**＝⒤ exporter の paper 未輸出（**需要待ち**＝`LilyPondExporter.cs:662`）／▶ perf（歌詞打鍵の章はほぼ完了＝55.1 vs 非歌詞 45.3。残り ~10 MB は hyphen／apply／非歌詞 L5/L9 等の小粒）／⒡ 配管 6 site／⒣ removeEmpty・pedal の score 移行検討（ユーザー決定から）／**ブロック回収 9 便**（ユーザー決定）／小粒: twin の歌詞行・`lines` twin 未輸出・マークの X・chord-row の上帯スカラー・非ペア ToCoda の reserve≠draw・lead-sheet 音節×縦線の対・lead-sheet の mid-piece `time` 変更の表示・実譜の `%` 記号。⚠️ ✅ **Marketplace（第220 ①）はこの便で閉じた**——**PAT 待ちではなくなり、0.4.0 が 8 ターゲットで公開済み**（⑩⑪）。**次のリリースは `v0.5.0` タグ 1 本**で、手順は `docs/RELEASING.md`。⚠️ **ただし `max-parallel: 1` は未走行**なので、次便は**出荷の確認を run の色でなく `extensionquery` で行うこと**（骨 6）。**§2 の項は全部そのまま開いている**（本便が閉じたのは §1 が本便で起票した穴）。
...
> ## ★★★ 骨 6＝**「run が赤い」と「出荷されていない」は別の集合。出荷は出荷先に訊く**
> 0.4.0 の公開で、**8 脚中 7 脚が failure を報告し、そのうち 4 脚は実際には公開が通っていた。**
> ⇒ **1 つの run の色は「全部成功したか」しか答えない**——**N 個の独立した出荷の、どれが
> 届いたか**は答えられない。しかも publish はジョブの*最終*ステップなので、
> **アップロード成功→ジョブ失敗**が構造的に起きる。
> ⚠️ **これは CI の不具合ではなく、CI に訊いた質問が間違っている。**
> 「ビルドは通ったか」なら run の色でよい。**「出荷されたか」は出荷先の状態**で、
> Marketplace の `extensionquery` にターゲット別に訊くのが答え（`RELEASING.md` 手順 7）。
> ⇒ ★★ **副作用がある操作を CI に置いたら、成否の判定は CI の外に置く。**
> **本便は 8/8 をそれで確定させ、赤い run のまま「出荷は完全」と言えた。**

> ## ★★★ 骨 7＝**教訓を*記録した*ことと*適用する*ことは別。効いたのは 1 秒で赤くなる方だった**
> `HistoryCitationTests` に本便は **3 度**捕まった: ⑴ §1 に孤児 SHA（骨 4 を書いた）
> ⑵ `DEPLOY.md` に Entra の object id——**GUID の先頭 8 桁は 7〜40 桁の hex ラン＝引用形状**
> ⑶ ……そして ⑵ は **骨 4 を書いた 5 commit 後**である。
> ⚠️ **骨 4 は「番人は建てた側を捕まえる」と書いてあった。読んで、そして踏んだ。**
> ⇒ ★★★ **散文の警告は、書いた本人にも効かない。**
> ⚠️ **そして踏めた理由は 2 つ目の失敗が隠していた**——**docs だけの commit で suite を回さなかった。**
> 「ビルドを壊さない」と「suite を落とさない」は**別の主張**で、
> **引用ガードは `.md`／`.yml`／`.json`／`.ps1`／`.csv` を読む**＝**docs だけの commit こそが動かす**。
> ⇒ **§7 の終了時チェックは「どのファイルが重要か」の判断を求めていない。suite を回せと言っている。**
> ⚠️ **安く済んだ理由も書いておく**——**ラチェットが CI に在った**ので、
> 誤った推論の代金は赤い run 1 本と 5 分で、タグまで生き延びなかった。
> **著者の居ない所で走る番人の値打ちは、まさにそこ。**
