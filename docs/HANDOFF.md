...
★★★★ **⒁ 第 10 便＝穴だと思って着手したら*死語*だった**（判定は「着手」・commit `9b1298f2`）。**決める前に 3 通り測った**（読む者がゼロ／`MidiExporter` は part の属性から取る／**6 綴りの MIDI が音・timbre・channel まで同一**）。⇒ ★★★ **だから補完に*足さない***——**足せば「何もしない綴り」を教える**（第 5 便の `sings part` と同じ規則）。**直したのは*積極的に間違っていた*方**＝`octave` の mode 語が行の値枠に出ていた（offer が「part header でなければ」しか訊いていなかった）。★ **穴を埋める前に、それが穴か死語かを測る**——これが第 12 便のユーザー決定（除去）に繋がった。
...
★★★★ **⒆ 第 15 便＝摂動網を広げて*最初の獲物*を獲った——`hhs` は splash を名乗って pedal hi-hat を鳴らしていた**（commit `e2f444a0`）。⇒ **`hhp` と `hhs` が署名まで同一**で、**LP には splash hi-hat が無い**（`splashcymbal`／`cyms` はシンバル・GM 55）＝**`splashhihat` は `pedalhihat` の行を 1 バイト違わず写していた**（GM 44＝Pedal Hi-Hat）⇒ **popup が splash と称して pedal hi-hat を描き・鳴らしていた**。**除去**（27,095 冊中 0 冊）。★★★ **番人は*表*に置いた**（嘘が住む場所）＝**canonical な drum は行（position／notehead／GM／mark）を共有してはいけない**。**共有する対は全部 LP 自身の二重綴り**で、**1 行ずつ LP の原典で確認した**。⚠️ **この表に 1 行足すことは「LP がそう綴っている」という主張**＝そう書いてある。

★★★★ **⒇ 第 16 便＝drum 表を LP の原典と全数照合した**（commit `6637dd29`・出荷物は無変更）。**LP に無い名前ゼロ／行の不一致ゼロ／LP に在って当方に無い＝約 30**（下 ㉒ で全部入った）。⚠️⚠️ ★ **kit の行は `drums-style` 以外の表から採ってはいけない**——LP には `agostini-`・`weinberg-drums-style`・`congas-style`・`percussion-style` が在り、**同じ名前が別の位置で載る**（hihat は 3 と 5）。**ファイル全体を正規表現で舐めると最後の表が勝つ**ので、**この監査の 1 回目は存在しない position 不一致を 22 件「発見」した**。★ **行は*検査時に LP を読まず*チェックインした**（LP が無い機械でも同じことを言う test）。

★★★ **㉑ 第 17 便＝同じ照合を tuning に当てた**（commit `1599a56a`・出荷物は無変更）。**答えは drum と同じ形**: LP に無い調弦ゼロ／弦の不一致ゼロ／**LP に在って当方に無い＝約 25**（下 ㉒ で全部入った）。⚠️ **ukulele の配列は昇順ではない**（re-entrant で LP もそう書く）＝**「並べ直す」ことが欠陥になる**ので行にコメントを付けた。
...
★★★★ **㉒′ 第 19〜21 便＝*摂動網そのものを直し、3 つの表に当てた*（3 便とも判定で着手・出荷物は無変更・commit `a6b84ccc`／`4b5f2284`／`4aa1ddc2`）**。**着手の根拠は毎回「この便しか知らない事実」**——第 18 便の theory と出荷物が**同じ生成器**から出ていた。⇒ **陽性対照 1 本**（`tuning standard` と `guitar` は同じ調弦だからページは同一のはず）が**網の欠陥を出した**: ⚠️⚠️ ★★★ **差は `data-pos`＝SVG の原文オフセット**。**摂動は音楽より*前*に書かれる**（part 属性・`layout { }`・`drummap { }`）ので、**綴りの長さが違うだけで後ろが全部ずれ、`AssertMoves` はその語が何をしようと緑**。**他の比較器は 2026-08-15 からマスクしている**（`Diagnostic.cs` 注記 4 箇所／第 20 便で族を全数確認＝**例外はこの 1 軒だけ**）。⇒ **マスクすると 21 件が動かなくなった**（第 14・15 便の「136 本すべて緑」の一部は人工物）。★ **21 件に死語は無く、全部*フィクスチャが鍵を表現できていなかった***（`Plain` は 1 段・2 小節・臨時記号も和音行も 2 つ目の part も section ラベルも無い）⇒ **鍵ごとに本を割り当て**、**既定値は「語彙の先頭」という `LanguageVocabulary` の宣言を読んで「既定は inert」を*等値で*主張**（`as removeEmpty false` と同じ扱いが 4 件に）。⇒ **同じ道具を 3 つの表に当てた**: **調弦 32 語**（tab 本で全数摂動＋表の番人）・**打楽器 126 綴り**（canonical は bassdrum と違う／**略号 63 は full name とバイト一致＝63 本の陽性対照**／**mark が唯一の差である 11 対はその mark が*描かれて*いること**）・**和音品質 40 綴り**（**補完は根音ごと 4 形しか出さない**ので残りはページ側の網ゼロだった）。⚠️⚠️ ★★★ **毒が 3 度外れ、3 度とも*私の計器*の欠陥だった**——**⑴ 対照分岐を `Tunings.Parse`（被験者）で選んでいた**ので潰すと「inert を主張する側」に落ちて緑／**⑵ `Signature` は MIDI を含む**ので「正しい和音の上の間違ったラベル」を隠す⇒ **衝突番人は描かれた `ChordText` だけを読む**／**⑶ 和音の表は 2 つ在った**（既定 `symbols` は `SymbolSuffix`・毒を置いた `Suffix` は `words` 側＝**本が一度も通らない枝**）⇒ **両語彙を掃く**。**直した後はどの毒も*その 1 本だけ*を赤にし、他は 399／1,046 本が緑**。⇒ ★★★ **恒久の教訓 3 つ**: **仕掛け線の*どちらの側に落ちるか*を、見張っている当の関数に決めさせない**／**「署名」は万能ではない——*何を訊いているか*で計器を選ぶ**／**毒が外れたら、まず毒ではなく*計器と経路*を疑う**。⚠️ **数を 2 度書き間違え、2 度とも test が先に報告した**（mark 対 16→11・§1 の char 数）。★ **`TuningTableMatchesLilyPondTests` は LP の和音テキストを*自分で parse* する**＝生成器が信頼の鎖から外れた。★★★★ **第 22 便＝同じ網を*注釈のオペランド*に向けて、本物を獲った**（commit `31273028`）: **ラベルの掃きは `@name` を「無い本」と比べる**ので、**引数が読まれずに捨てられていても注釈自身のインクで動いて緑**＝**生きた語の中の死んだオペランド**。⇒ **`@fig`/`@chord`/`@finger`/`@mark`/`@text` と、popup が一覧で出す 4 語彙（`@feather`/`@bend`/`@pluck`/`@fig` の図）を掃いた**。⚠️⚠️⚠️ **`@feather` は何も描かない**——⑴ **`right`／`left`／無しがマスク後にバイト一致**（137,849 バイト）／⑵ **`BeamGroup.GrowDirection` はリポジトリ中どこでも*コンストラクタ引数*でしかない**（検出器が立て、`With` が写し、`ElementCoordinator` が 2 回・`TabStaffGeometry` が 1 回素通しし、メモ鍵に入り、**engraver も renderer も読まない**）／⑶ **それを言うはずの test 3 本は*同語反復*だった**（`growDir > 0 ? 0.0 : 1.0` を test 自身が計算して assert・production を 1 行も呼ばない＝**誰も気づかなかった理由**）。⇒ **3 本は消し、意図した幾何は仕様としてその場に残した**。⚠️ **射程を数えた: 27,143 冊中 33 冊が `@feather` を書く**（**出荷 showcase `05-special-techniques.lys` を含む**）＝**MIDI 行（0 冊）とは違う**。**今日の答えを等値で固定した**ので、描かれた日に赤くなる。

★★★★ **㉒″ 第 23 便＝ユーザー決定「幾何を実装して」＝*羽根梁を描いた*（commit `3740810d`・出荷物が動いた便）**。⚠️ **規則は LP の原典から読んだ**——**この機械に LilyPond 2.26.0 のソースが在る**（`C:\MyProj\lilypond-src`）ので、`beam.cc` 自身のコメントを引いた: **`feather dir = 1 , relx 0->1 : factor 0 -> 1`／`dir = 0` は 1→1／`dir = -1` は 1→0**。**`calc_stem_y` がそれを `stem_y += feather_factor * beam_translation * beam_multiplicity[stem_dir]` として当て、`print` は同じ扇を*ランクごとの追加傾き*として描く**（全長 segment ではこの係数を両端で評価したものに一致）。⇒ **各ランクの主梁からの距離を*両端で*係数倍する**＝扇いだランクは平行移動ではなく*自分の傾きを持つ線*になる。⚠️⚠️ ★★★ **2 つは推測せず測った**: ⑴ **通常の梁の符尾は動かない、そしてそれが正しい**——**符尾は自分の側の最外ランクで終わり、通常の梁ではそれが主梁（rank 0）**だから（**knee では 0 ではない**ので係数は当てる）。**この test は最初*逆向き*に書いて落ちた**（「右成長の梁の外側 2 本は同じ長さになり得ない」）＝**ページが私の推測を訂正した**。⑵ **showcase `05-special-techniques` の snapshot が動いたのは梁 2 本だけ**（1 本は左で収束、1 本は右で収束＝両方向が 1 冊に在る）で**他は 1 バイトも動かない**＝コーパスで見える変化の全部。★ **同語反復 3 本は、描かれた polygon を読む test に置き換えた**。⚠️ **名指した穴 1 つ**: LP の `normalized-endpoints`（**系をまたいで折れた羽根梁はここでは 2 度扇ぐ**）。★★★ **もう 1 つは第 24 便で閉じた**（commit `bf39967d`）＝**双子**: **ページを描いた瞬間、page／twin／MusicXML の「等しく黙った一致」が壊れて*私が*食い違いを作った**ので、同じ便で塞いだ。⇒ **`\once \override Beam.grow-direction = #RIGHT` を梁を開く音符に前置**（`\once` が正しい scope＝**Beam grob は梁の最初の瞬間に作られる**）。⚠️⚠️ ★ **綴りは記憶から選ばず、双子に LilyPond 2.24.4 を*実際に通して*確かめた**——**override 付きで梁の遠い端が −1.01 → −1.82（ちょうど 1 beam translation）動き、近い端は 0.2 のまま**＝ページと同じ扇・同じ量。⚠️ **`\featherDurations` は書かない**（印刷上の音価を伸縮させる＝`@feather` はしない。書けば双子がページと違う音楽を鳴らす）。⚠️ **MusicXML は黙ったままで、理由は 1 段深い**——**あの exporter は `<beam>` を 1 つも書かない**（連桁は consumer 任せ）ので、**MusicXML 自身の `fan` 属性を掛ける先が無い**。梁の書き出しは別仕事＝幾何の隣に名指して置いた。⚠️ **`dotnet build LilySharp.Core` だけを建てて古い `lysc` で測り、「効いていない」と 1 度誤診した**（記憶どおりの罠＝**solution を建てる**）。

★★★ **㉓ 数**: **開始時**: HEAD `1e80a9b2`・未 push 9・木 clean・未追跡 0（**`Session-Check -Build -Test` は走らせていない**）。**終了時**: HEAD `bf39967d`（**出荷物の最後の変更は第 24 便＝双子が羽根梁を書く。第 13・14・16・17・19〜22 便は production 無変更**）・未 push 55（うち 3 本は doc）・**台帳 843 点／ss 非ゼロ 210／総和 23.216135909／count 点 180 うち非ゼロ 0／exact 668／OPEN 0・snapshot 249／追跡 `.lys` 599＝全部不変**（⚠️ **第 18 便は Core を 7 ファイル触ったのに、絵は 1 枚も動かない**——**既存の綴りの振る舞いを 1 つも変えていない**からで、**台帳・snapshot・追跡 `.lys` が全部不変なのがその測定**。第 13・14・16・17 便は Core を 1 行も触っていない）。**full `scratch/p375/run37.trx` 8424 / 0 / 4 / 8428**（第 24 便＝+5＝双子の主張）・第 23 便末 `run35.trx` 8419 / 0 / 4 / 8423（+5＝羽根梁の幾何。⚠️ **snapshot 249 のうち 1 枚が動いた**＝showcase `05-special-techniques`・**台帳 843 点は不変**）・第 22 便末 `run33.trx` 8414 / 0 / 4 / 8418（+21＝注釈オペランド。⚠️ `run32` は 1 本赤＝**私が足した LP 引用の綴りが `lily/` 前置きで「名前を挙げていない引用」に数えられた**＝`CitationsThatNameNothing_DoNotGrow`。既に通っている同じ引用の綴りに揃えた）・第 21 便末 `run31.trx` 8393 / 0 / 4 / 8397（+40＝和音品質 40 綴り）・第 20 便末 `run30.trx` 8353 / 0 / 4 / 8357（+127＝drum 126 綴りの掃き＋mark の網 1 本）・第 19 便末 `run29.trx` 8226 / 0 / 4 / 8230（+33＝調弦語 32 の摂動ほか）・第 18 便末 `run27.trx` 8193 / 0 / 4 / 8197（+68＝打楽器 3 theory・調弦 1 theory の case が大半。⚠️ **`run24` は 1 本赤**＝`ApproximationInventoryTests` は**生成物 `docs/APPROXIMATIONS.md` の行番号**を見張るので、exporter が 11 行縮んだだけで赤くなる⇒ `LILYSHARP_UPDATE_DOCS=1` で再生成・差は 4 行）・第 17 便末 `run23.trx` 8125 / 0 / 4 / 8129・第 16 便末 `run22.trx` 8119 / 0 / 4 / 8123（drum 照合 31 本）・第 15 便末 `run21.trx` 8088 / 0 / 4 / 8092（表の番人 +1）・第 14 便末 `run20.trx` 8087 / 0 / 4 / 8091（摂動 136 本ぶん増）・第 12 便末 `run19.trx` **7951 / 0 / 4 / 7955**（⚠️ **第 12 便で 9 本*減った*のは正しい**＝matrix の MIDI 行 3 綴り × 3 主張が言語から消え、代わりに「退役した綴りは断られる」2 本が入った。第 11 便末 `run16.trx` 7964・第 10 便末 `run15.trx` 7956・第 9 便末 `run14.trx` 7953・第 8 便末 `run13.trx` 7947・**第 1〜7 便末は `run1`→`run12` で 7816→7869**。**13 便とも合計を読んでいる**＝新 test は差引 144 本（第 12 便は −9）（第 8 便の 78 ＝ 26 綴り × 3 主張）。⚠️ 第 5・6 便は既存 test を**書き換えている**ので増分はそれより小さい）。★ **第 7 便は 7 つの context の配線を丸ごと替えたのに既存 7854 本が全部緑**＝**行の読みが 1 つになっても、読めていた綴りの答えは変わっていない**。⚠️ scratch は **`p375`**。⚠️ §7 3.5: 第372 を `-Archive 372` で ARCHIVE へ（済）。⚠️⚠️ **`origin/master`（`18efc322`）の CI は今も赤**（run 34663615384）＝**第372 の天井 commit `9f6df2a3` が未 push のまま**。**push はユーザー（未 push 55）**。★ **LSP は 13 度配布した**＝`0.6.0+3740810d`（第 19〜22 便は Core も Lsp も無変更なので配布していない）＝`tools/Deploy-Lsp.ps1` が `0.6.0+851e5469` を `yotsuda.lilysharp-0.5.0-win32-x64\server` へ（95 ファイル byte 一致）⇒ **ユーザーは Reload Window が 1 回要る**（package.json は動かしていないので 2 回は不要・記憶 `feedback_lilysharp_lsp_deploy_target`）。

⇒ ★★★ **㉔ 次の一手＝✅ ⒜（LP の語彙をどこまで入れるか）は第 18 便のユーザー決定「両方とも入れて」で閉じた**（打楽器 30→63・調弦 7→32 語）。✅ **⒜‴ `@feather` はユーザー決定「幾何を実装して」で閉じた**（第 23 便・下 ㉒″）。★★★ **⒜″ 摂動網の残りは *paper キー* だけになった**（和音品質は第 21 便・`@` の引数は第 22 便で閉じた）。⚠️ **やり方は 3 便で確立した**: **⑴ 陽性対照を 1 本置く**（同義の 2 綴りがページを変えないこと）／**⑵ フィクスチャがその鍵を表現できるか先に確かめる**／**⑶ 毒を入れ、外れたら毒ではなく*計器と経路*を疑う**（表が 2 つあるかもしれない）。**残った札は 3 枚**: **⒜′ `InstrumentDefaults.GetTuning` は preset 5 族しか答えない**（`violin`／`cello`／`viola`／`mandolin`／`banjo` の preset を書いた part を tab で描くと**ギターに落ちる**）＝**調弦は在るのに preset が繋がっていない**。**触っていない理由は 1 つ**——**既存の本の tab の絵が変わる**（`instrument violin` ＋ tab の本があれば 6 弦→4 弦）ので、**射程を数えてから**（第 12 便の型）。⒝ **摂動網の残り**（paper キー・chord quality 35・`@` の引数つき形）。⒠ **drummap の `${1:hh}`＝UX 案件**（drum 語彙を出すと親切。`drummap { ▮ }` の本体に語彙の家が無いので context が 1 つ要る）。⒡ **第373 ⑼⒜（probe 24 冊の「量を測ったか／問いに答えたか」の仕分け）**も先頭級のまま。**ヘッダの綴りの島は閉じた**——⑻ の 25 綴りが全数で、**残る未点検はゼロ**（`{` を開く構文は score／staff group 5／part／phrase／form／lyrics／chords／section／fonts・paper・layout とその入れ子／音楽の容れ物で尽きる）。⚠️ **閉じたのは*補完の*読み手だけ**: 同じ「2 語しか読まない枠」を持つ道具が他に居ないかは見ていない（`SemanticTokens`・折り畳み・`BarCountFix` は別の木＝**実構文木**を読むので原理的に無縁、という*予想*まで）。⒢ 第369 ⑸⒝（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。

★★ **㉕ 判定の記録（ユーザーの常設指示「次便は、このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）**: **第 2・5・6・7・8・9・12〜18 便はユーザー起票／指示**（判定の対象外）。★★★ **第 19 便は判定で着手し、着手の根拠が「道具の温度」ではなく*この便しか知らない事実*だった**——**第 18 便の theory と出荷物が同じ生成器から出ている**。**冷えた便には見えない**（どちらも緑で、コメントにも書いていなかった）ので、**書き残すコストと塞ぐコストがほぼ同じ**＝着手が有利。**結果、探していた穴の 10 倍のものが出た**（網が 21 件空振り）。⇒ ★ **「自分が今作ったものの*検証*が独立か」は、作った便にしか問えない。** ★★★ **第 20 便も判定で着手**——**直前の的の見つけ方（「2 つの本を比べる計器に陽性対照を当てる」）を*族に広げる*のは、同じ操作の N を増やすだけ**（第 3 便で 1 度書き間違えた形）。**掃きは空振りだったが、規則どおり固定して返した**＝第 18 便のもう半分に同じ道具を向け、**そこで本命（描かれない mark）が獲れた**。⇒ ★★ **「空振りの掃き」の正しい終わり方は、*その道具を次に向けること*であって、報告して終わることではない。** ★★★ **第 21 便も同じ判定で着手**——**的の選び方が 3 便とも同じ 1 行**になった: **「表が主張していることを、ページに一度も訊いていない場所はどこか」**。⇒ **これが次便の探索語**（drum・調弦・和音品質が済んだので、次は paper と `@` の引数）。★★★ **第 18 便はユーザー決定で着手した脚だが、判定の教訓を 1 つ足した**——**「移植は機械的」と第 17 便が書いたのは*値*の話で、決めることは 4 つ在った**（どの style 表から取るか／mark 語彙が足りるか／LP 自身の dangling 名 `tt` を入れるか／綴りの規則）。**⇒ 「機械的」と書いた次の便は、まず*決める点*を数える。**⚠️ **「どこに写しが在るか」も移植の一部だった**——名前→`TuningType` の switch が 3 軒に在り、**先に 1 軒へ寄せなければ 25 種を 3 度書いていた**（寄せたあとは 1 箇所）。**判定で着手したのは 8 脚**（⑺ tilde・⑻ 掃き・⒁ MIDI 行・⒂ 名前の監査・⒅ 摂動網・⒆ 網の拡張・⒇ LP 照合・㉑ tuning 照合）で、**8 つとも「直前の脚が*何で*断定したか」を道具に採った的**。★★★ **⒇ は「掃いて何も出ない」が*成果*だった脚の 2 つ目**（1 つ目は第 13 便）——**ただし今回は網に翻訳できた**（LP の行を checked-in の test に）。**⇒ 空振りの掃きは、次に誰かが同じ疑いを持つ前に*固定*して返す**。★★★ **⒆ で初めて「網を建てた次の便が獲物を獲った」**＝`hhs`。**網は建てた便では何も釣らず、次の便で釣れる**（第 14 便は 136 本すべて緑だった）。★★★ **基準はこの 1 行＝「どの道具が温まっているか」ではなく*直前の的をどうやって見つけたか***（第 3 便で 1 度書き間違え、第 13 便で 1 度守り損ねて 40 分落とした——**2 度とも同じ形**）。⚠️ **仕掛け線を張ったら必ず毒で赤を見る**（第 8・14 便は入れた。入れない網は「在るつもり」になる）。⚠️⚠️ **冷えた便が §1 の見出しだけで着手すると*逆に壊す***——⒁ は「穴」ではなく死語だった（埋めていたら何もしない綴りを補完に足した）／⒂ は `form` が*逆向き*だった（在ってはいけない名前を書く側）。★ **1 便だけ shell 切断で「有利／不利」ではなく*実行可能か*が判定を決めた**。**着手しなかったもの**: ㉒⒜ は**ユーザー決定 2 件**（語彙をどこまで入れるか＝承認要）＝着手できない。㉒⒝ は**同じ機械の続き**で冷えていても進む／㉒⒡⒢ は**別 regime**（LP を回す／設計級）＝次のセッションでやっても損しない。★ **この便は「照合して差がゼロ」を 2 表で出した**——**空振りに見えるが、次に疑う人の 40 分を先に払った**（第 13 便の教訓の実践）。★ **便ごとの逐語は `HANDOFF-ARCHIVE.md`「第374セッションの『判定の記録』逐語」**（20,000 字の門で出した）。
