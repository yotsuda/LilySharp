...
最終更新 第369セッション（2026-09-11）＝**入り方は第298〜第367 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・口挟み 0）。第368 ⑺⒝ の並びは全部ユーザー決定か設計級なので、**§2 T7 の「小節数一致・段割れ違い 41 対」を解剖した（測るだけ・製品 0・commit は HANDOFF だけ）**。**骨は 5**:

★★★ **⑴ 41 対の族は 4 つで、製品側は 1 つ（詳細は §2 T7 の第369 行・T6 ⒧⒨）**: **⒜ `.lys` のオクターブ誤り**（`Get Back`＝手書き `.ly` bar 41 の `a\2a,\4`（空白落ち）を変換器が読み違え、音 1 つ欠落＋以後 relative の錨が +12 → tab の fret 17／19・8 分 8 個の小節が +10 ss → 4 小節/段が 2,2。**修正コピー `scratch/p370/oct/gb-fixed.lys`（`fix-getback.py`）は LP と同じ 24 段・3 頁・LYS2001 も消える**。射程は MIDI 音高 census＝**±12 の連続区間を持つ本 4 冊＋単発**）／**⒝ `\noBreak` が `.lys` に無い**（`.ly` 15 冊 58 箇所・41 のうち 8 冊）／**⒞ `\break` が無い**（Amanda＝T6 ⒥・決定済み「触らない」）／**⒟ 製品**＝下 ⑵。

★★★ **⑵ 製品側の島は 2 つ（`scratch/p370/mb/`・紙幅 210〜230mm の閾値＋捨て計器 `scratch/p370/ZzP370ProbeTests.cs.txt`＝行 DP の候補 k 段の Σf² と段数ループのログ）**: **⒜ 圧縮行の力の値付け**——`Never Stop`（LP 27 段・Lily# 28）は **27 段が置ける（Σf² 14.59）のに 28 段（10.96）を得点で選ぶ**。**LP の `-ddebug-page-breaking-scoring`（`mb/Never_Stop/dpbs.log`・bass book）: 27 段 6.659／28 段 7.077／29 段 8.725**＝LP は 27 が最安、Lily# は 27 だけが 2.2 倍（28・29 は 1.3〜1.5 倍）→ **bar 29-32 の 4 小節を 73% に圧縮した 1 行の f² が Lily# で 4 前後、LP でごく小**。`瞳をとじて` も同型（23 段 17.6 対 24 段 12.6・LP は 23）。両側の自然幅は一致（bar 29-32: LP 32.6/33.9/34.1 対 Lily# 33.2/34.0/33.2）なので、差は圧縮側。**第 2 便が列で突き合わせた（`scratch/p370/ns/`＝bar 29-32 の抜粋 `ns4.lys`・双子 `ns4.ly`・LP は `dump-settings.ly` の列 dump を「広幅 ragged＝自然」と「`system-count = 1`＝圧縮」で 2 走・`gaps.ps1` が列間隔と差を表に・Lily# は捨て計器 `ZzP370bProbeTests.cs.txt` がばね列を刷る）**: **LP は 45 本のばねを全部比例で縮める**（16 分 2.2153 → 1.6911・8 分 3.54 → 2.49＝力 0.437・rod 0.81〜1.60 はどれも縛らない）。**Lily# は 16 分→16 分のばねが ideal 2.3072 ＝ min 2.3072・compress 0**（縮まない）。**その 2.3072 は tab の数字予約**（`ApplyTabChordSpacing`＝右半幅 0.8536＋左半幅 0.8536＋`FretColumnGap` 0.6）＝**staff 単独の抜粋なら Σmin 61.5・力 0.53（LP 0.44）で 4 小節が入り、tab 単独は staff＋tab と同じ Σmin 93.5**。⇒ **力の値付けの島ではなく、Lily# 固有の大きい fret 数字（§3 F9・OWN）と列間 0.6（`TabConstants.FretColumnGap`・0.2 → 0.6 に上げた経緯が remark に在る）の設計の帰結**。LP の `TabNoteHead` は既定 font-size −2・extra-spacing-width 無し（ユーザーの `.ly` は font-size 2）で、数字は互いに 0.3 程度まで寄る。**16 分が密な bass tab の行は Lily# では 2.31/16 分より縮まず、LP は 1.69 まで縮む**＝T7 の「Lily# が割る」側（Never Stop・瞳をとじて・全力少年 bar 58-61 の置けない行も同族の見込み）は**この設計を変えない限り閉じない**（ユーザー決定＝下 ⑸⒝③）。⚠️ 列 dump の罠: 抜粋を 1 行に置くには `system-count = 1`（ragged では LP が 2,2 に割る）。⚠️ **計器の罠**: LP に `ragged-right` を注入すると改行が変わる（自然幅比較にだけ使う・§2 T7 第369 行）／`[IO.File]::WriteAllText` の相対パスは process CWD（memory）。

★★ **⑶ 計器（全部 `scratch/p370/`・製品 0）**: `oct/octave-census.py`＋`make-lys-midi.ps1`（LP `bassRef` MIDI 対 `lysc midi`・247 冊比較・`octave-census.csv`／`census.txt`）／`gb/widths.ps1`（LP `PROBEBAR` ログと Lily# svg（多頁・五線の小節線 rect だけ）の段ごとの小節幅）／`mb/measure-book.ps1 -Name <本>`（1 冊を両側で・LP は ragged＝自然幅・Lily# は A4 と 620mm）／`oct/fix-getback.py`（区間の音を 1 オクターブ動かす型）。LP の起動は `cmd /d /s /c … < NUL`（memory）。

★★★ **⑷ 数**: **開始時**（`Session-Check -Build -Test -Scratch p370`）: HEAD **`4ce85a16`**・**未 push 3**（`origin/master`＝`b3bf9364`）・木 clean・未追跡 0・`gh run list` **緑**（34543531416）・build 0 エラー／Core 0 警告・**full `scratch/p370/run1.trx` 7699 / 0 / 4 / 7703（第368 末 run4 と一致・RunInfos は SKIP 4 だけ・4 m 59 s）**・台帳 821・snapshot 249・追跡 `.lys` 599・ディスク 25491。**終了時＝下 ⑹**。§7.5 Core `+` 0／REF 0／OWN 0（製品を触っていない）。⚠️ `git worktree`: p362／p363×2／p364／p368×2／p369 のまま（`git worktree remove` で外す・この便は足していない）。⚠️ scratch は **`p370`**。⚠️ §7 3.5: 第367 のブロックを `-Archive 367` で ARCHIVE の先頭へ（済）。⚠️ 記憶: 新規 1（`reference_lilysharp_t7_octave_census_and_ragged_probe_trap`）。

⇒ ★★★ **⑸ 次の一手**: **⒜ 製品側で残るのは ⑵⒜ の設計決定だけ（第 2 便で閉じた＝力の島ではなかった）**。**⒝ ユーザー決定 3 つ**: ① T6 ⒧ オクターブ誤りを当てるか（Get Back は `gb-fixed.lys` が出来ている・他 3 冊は同じ型で書く）／② T6 ⒨ `\noBreak` を `.lys` に写すか（p331 `RestoreMeter` に mode を足す）／③ **tab の数字の列間 `FretColumnGap` 0.6（と数字の大きさ）を LP 寄りに縮めるか**＝縮めれば 16 分の密な bass tab の段割れが LP に寄る（Never Stop は 16 分ばね約 20 本×0.4 で min 93.5 → 85 ＜ 93 で 4 小節が入る見積り・未計測）が、読みやすさのために 0.2 → 0.6 に上げた値（remark の経緯）を戻すことになる＝**§3 F9 の再決定**。**第 3 便は着手しない**: 残りは全部ユーザー決定（①②③）か第368 ⑺⒝ の設計級で、p370 の道具で測れる未計数の宿題は無い。**⒞ 第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）はそのまま**。**push はユーザー（未 push 4）**。

★★★ **⑺ 第 3 便＝ユーザー決定 3 つ（2026-09-11）「① 当てる ② 写す ③ 列間は縮める・tabFret の既定の数字は大きいまま・双子で比べるときは `.lys` に `fonts { tabFret size 2 }` を書いてから双子を作る」を実行**:
**③ 製品（承認待ち）**: `TabConstants.FretColumnGap` 0.6（OWN の読みやすさ）→ **LP の rod の空き `2 × DefaultExtraSpacingWidth + SeparationRodPadding = 0.3`**（REF `separation-item.cc:166-167 extra-spacing-width`／`spacing-spanner.cc:315-316 set_column_rods`・OWN 137 → 136・APPROXIMATIONS 再生成・CHANGELOG Engraving 1 項）。**裏取り（ユーザーの手順どおり `fonts { tabFret size 2 }` の tab 単独抜粋 `scratch/p370/ns/ns4-tab-s2.lys` → 双子 → LP 列 dump `tsw.log`／`tsj.log`・BarLine に hook（tab 単独に NoteHead は無い））**: LP の tab 単独は 16 分の間隔 1.93 → 1.55（rod 0.81〜1.28・force 0.316）／Lily# は Σmin 93.5 → 68.6・force 0.489（残りは 16 分の ideal 側 2.31 対 1.93＝別島）。**実本: Never Stop 28 → 27 段・瞳をとじて 24 → 23 段＝どちらも LP と同じ署名**（全力少年 58-61 は残る）。**掃き 925 冊（`sweep370.ps1`・base `scratch\p370\wt-base`＝9f8d18eb Release・`compare-svg.txt`）: MOVED 284＝追跡 20＋ユーザー 264**（tab の本は隣り合う数字の予約が全部縮む）。**full `scratch/p370/run4.trx` 7685 / 14 / 4 / 7703・赤 14＝snapshot 13（`test/dead-note`・`system-count-line-start-ink`・`tab-as-numbers`・`tab-beam-script`・`tab-beam-slope`・`tab-below-range`・`tab-chord-spacing`・`tab-grace-slur`・`tab-numbers-dotted-chord`・`tab-percent-repeat`・`tab-string-pinned`・`tab-technique-letters`・`tab-tempo-over-beam`）＋inventory 1（再生成済み）**。patch `scratch/p370/tab-gap.patch`（Core 1）。⚠️ 承認前に commit・Deploy はしない（RULES §5.1）。承認後: `tools\Approve-Snapshots.ps1`（引数無し・動くのは run4 の 13 枚だけ）→ full → commit（Core 1＋APPROXIMATIONS＋magic_constants＋CHANGELOG＋snapshot 13＋この HANDOFF）→ `Deploy-Lsp.ps1`。
**① コーパス（当てた・原本は `scratch/p370/oct/backup/`）**: Get Back（`gb-fixed.lys`＝bar 41 の欠落音＋bar 42-96 −1 oct・LYS2001 消滅）／夏の終わりのハーモニー（`fix-octave.py` Body #56 後半〜#77 +1 oct・86 音・警告 35 → 2・残り bar 61 の C 対 D 1 音＝音違いで手）／'Til They Take My Heart Away（**`//octave absolute`＝relative の本**・1 音ずつ当てたら後続 46 音に伝播したので backup から戻し、`.ly` と同じ相対綴り `gis16\2 cis,`・`f'8 f,16` で当て直し）／9 to 5（bar 31 `fis,,8` → `fis,8`）／Don't Answer Me（bar 1 `c` → `c,`）。**census（`mid2/`・`census2.csv`）は 5 冊とも octave の少数派 0**。**当てなかった 3 冊（報告）**: クリスマスメドレー (UiPath)BK＝**LP 側（手書き `.ly`）の relative 事故**（終盤 4 小節が 1 オクターブ上に飛ぶ・`.lys` の低い側が正しい）／銀河鉄道999＝octave ではなく **8 分ずれ**（`cis, break fis,,8` の小節線無し break＝T6 ⒠ の族）／Air on G String bar 36＝音違い。計器: `lys-bars.py`（form 展開で MIDI 小節 → section #）・`dump-bar.py`（両側の生音列）・`fix-notes.py`（バイト保存の名指し置換・Edit ツールは CRLF を壊す）。
**② コーパス（当てた）**: `RestoreMeter` に `nobreak` mode（`\noBreak` の次の `.ly` event を LCS で `.lys` に写し、小節頭なら `noBreak ` を前置）→ 66 site・65 挿入・unmatched 1（さよならエレジー コード譜の `s1`）・verify 門（error 0→0・warning 10→10・新規診断 0・小節数不動）→ `apply.ps1 -Mode nobreak` 14 冊（backup `scratch/p331/t6bk/backup-nobreak/`・corpus の `noBreak` 51 → 116）。
**T7 取り直し（③＋①＋②・`relayout369.ps1 -Bin Release` → `scratch/p370/structure-after369.csv`・`compare337.ps1` → `compare369.txt`・15 分）: KindMatched 214 / SigMatch 152 → 160 / 小節数一致・段割れ違い 41 → 33 / 小節数違い 21・PagesMatch 199 → 204・SigMatch で頁数違い 5 → 7**。**得 11 対**（Get Back・Never Stop・瞳をとじて・himawari ×2・Hold the Line・Need You Now・Don't Stop Believin'・That's The Way (I Like It)・残酷な天使のテーゼ・悲しみにさよなら）**／失 3 対**（I Want You Back・Video Killed The Radio Star・いとしのエリー＝列間が縮んで Lily# が LP より詰める側へ倒れた本＝残る 33 の「Lily# が併せる」側の族）。頁数違いの新顔 2（himawari 5 → 4・That's The Way 3 → 4）。**第 4 便は着手しない**: 残る 33 の内訳は「Lily# が併せる」側（16 分の ideal 2.31 対 LP 1.93＝tab の数字の ideal 側・別島）と「全力少年 58-61 の置けない行」（2 桁 fret の予約）＝どちらも測って起票する regime で、承認待ちの出荷が先。

**⑻ 第 4 便（ユーザー「承認する」）**: `Approve-Snapshots.ps1`（引数無し）で再ベース＝動いたのは run4 の赤と同じ 13 枚だけ → **full `scratch/p370/run5.trx` 7699 / 0 / 4 / 7703（run1〜3 と合計一致・RunInfos は SKIP 4 だけ・6 m 58 s・HANDOFF／CHANGELOG／APPROXIMATIONS 込み）** → commit（Core 1＋APPROXIMATIONS＋magic_constants＋CHANGELOG＋snapshot 13＋この HANDOFF・`scratch/p370/commit3.txt`）→ `Deploy-Lsp.ps1`（Core が動いたので server）。push はユーザー。

★★ **⑼ 第 5 便（ユーザー「有利なら着手」→ 抜粋→双子→列 dump→ばね列の型がそのまま効くので着手・測るだけ・製品 0）**: **⒜ 全力少年 bar 58-61（置けない行）＝F9 の帰結・設計どおり**: 抜粋 `scratch/p370/ns/zr4.lys`（双子 `zr4.ly`・`zrw.log`／`zrj.log`）。Lily# のばね列は 2 桁 fret（`d\2`＝12・`g,\3`＝10 …）の予約 **3.7143 ＝ 1.7072×2 ＋ 0.3** が 16 分／8 分ばねの ideal に届いて compress 0 が 14 本・Σmin 95.9 ＞ avail 92.96 で「1 lines: (none)」。LP の同じ列の rod は 2.075（既定の小さな数字）。**数字を大きいままにする決定（③）のもとでは閉じない**＝`fonts { tabFret size 2 }` を書けば入る本。**⒝ 副産物＝次の島が名指せた「staff＋tab の wish の merge」**: 8 分→8 分ばねの LP ideal（staff＋tab・`nsw.log`）3.5403 ＝ **(Lily# ＝ staff の wish 3.7042 ＋ LP tab 単独の wish 3.3765) / 2 ＝ 3.54035（4 桁一致）**＝LP は譜ごとの wish を `merge_springs` で平均する（`lily/spring.cc` merge_springs・第361 ⑸⒝ は同じ譜の 2 声で見た同じ機構）のに、Lily# は notation の wish だけを採る（tab は `ApplyTabChordSpacing` で min を上げるだけ・wish を出さない）→ staff＋tab の本は Lily# の ideal が 1 ばね +0.1〜+0.3 広い＝T7 の残る「Lily# が割る」側の候補。**式は読んだ（`lily/spring.cc:104-129 merge_springs`）: ideal ＝ 各譜の wish の素の平均・min ＝ max・ideal ≥ min＋0.3・inverse_stretch は平均・inverse_compress は調和平均**。16 分→16 分で素の平均が合わない（(2.3072＋1.9265)/2 ＝ 2.117 ≠ 2.2153）のは **staff の wish を Lily# の 2.3072 と置いたから**＝LP の staff wish を 2.5042（2.4＋頭 1.3042−1.2・符尾補正無し）とすれば (2.5042＋1.9265)/2 ＝ 2.21535 で 4 桁一致・付点 8 分→16 分は 2×4.3673−4.3285 ＝ 4.4061 が staff wish（Lily# 4.6562 と +0.25 差＝Lily# の staff wish 自体も 2 か所で LP と違う疑い）。**3 者目も取った（`ssw.log`＝`ns4-staff.ly`・広幅 ragged）: LP の staff 単独の wish は Lily# の staff 単独の wish と 10/10 一致**（4.6562／2.7542／2.2542／2.2542／3.7042／2.4／2.7542／2.2542／2.2542／5.0828＝16 分→16 分も 2.2542。Lily# の 2.3072 は staff＋tab で tab の予約が ideal を持ち上げた値で、staff wish の残差ではない）。**LP の staff＋tab の ideal は全ばねで [tab 単独, staff 単独] の内側**（16 分 2.2153 ∈ [1.9265, 2.2542]・8 分 3.5403 ∈ [3.3765, 3.7042]・付点 4.3673 ∈ [4.3285, 4.6562]・4 分→bar 4.849 ∈ [4.7521, 5.0828]）**が、素の 2 平均が 4 桁で合うのは 8 分だけ**（16 分 2.090・付点 4.492・4 分 4.917 は外れ）＝**merge の入力は staff と tab の 2 本ではない**。**次の一手**: `lily/spacing-spanner.cc generate_springs`／`musical_column_spacing` が 1 列に集める `Note_spacing` の wish の数と中身（voice ごと・`\transpose c c,` の tab の TabVoice・rest の列）を読み、3 本目の wish を名指してから Lily# に「譜ごとの wish を出して merge する家」を設計する（`ApplyTabChordSpacing` は min だけ）。**第 6 便は着手しない**: 式を読んでから移植する regime（設計級＝tab の wish を出す家が要る）。

**⑹ 終了時（第 5 便末）**: **full `scratch/p370/run6.trx` 7699 / 0 / 4 / 7703（run1〜5 と合計一致・RunInfos は SKIP 4 だけ・HANDOFF 込み）**・§7.5 Core `+` 0（第 5 便）／出荷済みの ③ は `952ca4ae`（Core +24/−8・REF 2・OWN 0）・台帳 821・snapshot 249（13 枚は再ベース済み）・追跡 `.lys` 599 不動・**捨て計器 2 本は `scratch/p370/Zz*.cs.txt` へ退避済み（Tests に残っていない）**。commit＝HANDOFF 4 本（`47317e59`・`9f8d18eb`・第 5 便の `commit4.txt`）＋製品 1 本（`952ca4ae`・Deploy 済み）。**push はユーザー（未 push 7）**。⚠️ `git worktree`: **`scratch\p370\wt-base`** を足した（`git worktree remove` で外す）。第 1 便の記録: full run2 7699 / 0 / 4 / 7703・commit `47317e59`（`commit1.txt`）。**第 2 便（ユーザー「有利なら着手」→ p370 の道具がそのまま効くので着手）＝⑵⒜ を列で突き合わせて閉じた（上）**: **full `scratch/p370/run3.trx` 7699 / 0 / 4 / 7703（run1・run2 と合計一致・RunInfos は SKIP 4 だけ・4 m 9 s）**→ commit（この HANDOFF だけ・`commit2.txt`）。捨て計器 2 本は `scratch/p370/Zz*.cs.txt` に退避済み。push はユーザー（未 push 5）。
