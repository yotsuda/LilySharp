...
- ★★★ **T7 の製品側の頭＝staff＋tab の wish の merge**（第369 ⑸⒝／第370 ⑼⒝ が式まで読んで*起票しなかった*島）: 第575 の双子基準の掃き（Lab `sessions/p575/twin/twin575.ps1`＝`.lys` の各 score を `lysc ly --all` の双子で LP に描かせ、Lily# と段署名を比べる・333 冊 478 score が 1.6 分）で **Lily# ≢ 双子 30 score のうち 15 が staff＋tab で Lily# が段を増やす側**（Amanda・Butterfly・全力少年・星になれたら…）。LP は譜ごとの wish を `merge_springs` で平均する（16 分で LP 2.2153 対 Lily# 2.3072）のに Lily# は notation の wish だけ＝ideal が広い向きと一致。**次は抜粋→双子→列 dump で 1 冊の ideal を列ごとに突き合わせてから**「譜ごとの wish を出して merge する家」を設計する（設計級・`ApplyTabChordSpacing` は min だけ）
...
- **T7. ▶ 起票（第321）＝絵の突き合わせ（286 冊全数）の結果＝族の表。計器は `scratch/p321/`（§1 ⑴）**
  ...
  **上の 388／382 は代用対（both の Lily# score を単譜の LP book と比べた 384 対）を含む数で、もう指標にしない**（第323 ⑶′⒜）。
  ...
  ▶ **残る小さい差**: ✅ **`beam-over-stem` bar 2 の +0.33＝第360 が診断・移植（✅ `17f7b7b3`＝第360 第 2 便で承認・§1 第360）**＝wish は Voice ごと（cross-voice 対は rod だけ）・skip は端点でない・返された梁の pure tip・Dots esw 0.2・shift 込みの頭幅 refinement＝bar 2／3 は LP と 4 桁一致・bar 1 は −0.05 残／text row の空 cell の slot（LP は落とす・Lily# は保つ＝`LILYSHARP-OWN` で名指し）。
...
- **R7 横 spacing の残り**＝**全項 ✅**（⒜⒝⒞⒟⒠・第569〜第573。⒞ は `998a01fd`＝`ApplyRods` を LP の `add_rod` の 1 回通し＋`range_solve` に・掃き 0 冊） → **閉じた項の本文は HANDOFF-ARCHIVE.md「閉じた §2 の本文」の「R7〜R12 の行（第573 が落とした）」**
- **R8 頁 DP の重みの 2 綴り**（⚠️ 第573 の値付け＝**射程はほぼ 0**: `SystemDetails` の `PagePenalty`/`BreakPenalty`/`TurnPenalty` は Core のどこも設定しない＝常に 0。残る差は f² の ×10 と clamp の位置だけ＝f が ~300 を超える頁でしか効かない）（`PageBreaker.DemeritsOf :1389-1412` は f²×weight を clamp 前に掛け penalty は無重み・LP は `min(f²,BAD)` を page ごと足し `page_weighting` は `finalize_spacing_result` で 1 度）＋ `BreakPenalty` は line DP の項／⒝ ✅ 第574（walk の早期脱出を LP の `rod > paper_height`（ragged は ＋spring）に・掃き 0 冊＝BAD で頭打ちの overfull 頁は Lily# の重みでは分割に必ず負ける＝網は規則の単体 `TheWalkEndsPastThePaper_…`）／⒞ `SystemDetails` の scalar extent と shape の max の食い違い（`Height`/`InverseHooke`/`MinWhitespace*` が scalar を読む）／⒟ 単一頁 stack が `PositionSystemsOnPage` の 2 綴り（`LayoutEngine.Pages.cs:393-589`）／⒠ ✅ 第570 が閉じた（`TightSpacing`＝代入 0 の発明・LP の `tight_spacing_` とは別物＝撤去）。
- **R9 梁・弓の残り**＝**全項 ✅**（⒢ は第574＝台帳 `beam.tremolo.*` 17 点 exact）／⒜⒝⒞⒟⒠⒡ ✅（第570・第572・第573。⒟ は起票が古かった＝slur 対 slur の項は第470/481 が「LP に無い」と撤去済み・phrasing slur の中の slur は `AddEnclosedSlurPoints` が LP の 3 点で既に積む） → **閉じた項の本文は HANDOFF-ARCHIVE.md「閉じた §2 の本文」の「R7〜R12 の行（第573 が落とした）」**
- **R10 記号・歌詞の残り**＝⒜ `ArticulationEngraver` の aligned_side が 3 綴り（note 支持は箱の算術、chain/tie/slur は pointwise、`DynamicEngraver.SpannerOffsetY`）＋`on_line` の ledger parity＝**第574 が LP 双子で測った＝忠実度の欠陥は無い**（`probes/script-positions.ly` の 77 個が全部 exact・台帳 `script.staccato.*`／`script.tenuto.*` 8 点で regime ごとに固定）＝残るのは*保守性*（1 綴りに畳むこと）だけ・畳むときはこの 8 点が網／⒝〜⒣ ✅（第565〜第570） → **閉じた項の本文は HANDOFF-ARCHIVE.md「閉じた §2 の本文」の「R7〜R12 の行（第573 が落とした）」**
- **R11 renderer の残り**＝**全項 ✅**（⒜〜⒢・第561〜第566） → **閉じた項の本文は HANDOFF-ARCHIVE.md「閉じた §2 の本文」の「R7〜R12 の行（第573 が落とした）」**
- **R12 言語層の残り**＝⒠ `MeasureModel` と collector の予算の単位違い（予算 50,000 を超えた本だけ・LYS1033 が既に「頁は切り詰め」と言う＝優先度は低い）／⒜⒝⒞⒟⒡ ✅（第570・第571） → **閉じた項の本文は HANDOFF-ARCHIVE.md「閉じた §2 の本文」の「R7〜R12 の行（第573 が落とした）」**
