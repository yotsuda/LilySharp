...
149 どちらにも梁が無い
```
★ **13 の内訳**: **タブ 5 冊＋`instrument` 1 冊**（**既知の非欠陥**・⑸⑹）／
**⑵`grandStaff` 入れ子・⑶`ossia` で LP 側が 0 本になる 3 冊**／
★★★ **exact なのに違って見える 2 冊**——**`TwinBeamSweep` は梁を*最寄りの*譜の refpoint に
割り当てる**ので、**自分の譜から遠くに浮いた梁は別の中央線で測られる**。
`test/beam-under-staves` は **−10.1 の 1 回のシフトで完全一致**、
`test/grace-lower-staff` は **片方 0・片方が譜間隔ちょうど**（**同じ register の台帳点は独立に exact**）。
⇒ ⚠️ **測定器が exact な本を発散に見せる**＝**今日 2 回踏んだ型の 3 回目**。
★ **未説明は `showcase/grammar-tour` だけ**（10 対 10・5 本一致・**LP が負で Lily# が正**）。

## ▶ 次の一手＝**まず測定器を直す。それから grammar-tour**

⚠️ ★★★ **`TwinBeamSweep` の譜割り当てを直すのが先**——**直すまで sweep の出力は読めない**
（`grace-lower-staff` は**当の台帳点が exact なのに**発散として出てくる）。
**梁は自分の譜の refpoint で測る**（x で重なる譜ではなく、**その梁が届いているステムの譜**）。
★ **そのあと `showcase/grammar-tour`**——**上を直すと消えるかもしれない**ので**順番が大事**。
★ **その次は exporter の残り**: **⑵ `grandStaff` の入れ子**（`EmitScore` が
`EnumerateChildren(render)` の直下しか見ない・3 冊）・**⑶ `ossia`／`part` 宣言なし**。
**この 2 つを塞ぐと LP 側が 0 本だった 3 冊が測れるようになる。**
...
- ★★★ ⚠️⚠️ **sweep が反転したら、まず両側が同じ音楽かを疑う**（2026-08-01・**第60セッションが
  ...
  **壊れた対から出た強い言明は、ただの強い間違い**で、**2 セッション分の探索を誤誘導した**。
  ...
  ★★★ ⚠️ **そして「対」には*測定器*も含まれる**（同じ日の 3 回目）。コーパス双子 sweep で
  `test/beam-under-staves` と `test/grace-lower-staff` が発散に見えたが、**両方 exact**だった——
  **`TwinBeamSweep` が梁を*最寄りの*譜の refpoint で測る**ので、**自分の譜から遠くに浮いた梁は
  別の中央線で読まれる**（前者は **−10.1 の 1 回のシフトで完全一致**）。
  ⇒ ★★ **判定法**: **食い違いが*一定量のシフト*なら、まず frame を疑う**——
  **量子の欠陥は格子の段差で跳ぶのであって、本ごとに違う定数だけずれたりしない。**
  ⚠️ **`grace-lower-staff` は当の台帳点が独立に exact だった**＝**手元に反証があるのに
  sweep の出力を信じかけた**。**強い道具ほど、その道具自身の対を先に作る。**
