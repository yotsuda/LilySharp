...
⇒ ★★★ **次の一手**——**死物件（CalculatePureSystemHeight・KnuthPlass 単譜オーバーロード＝テスト網の付け替えが要る）は削除にユーザー承認が要る**（RULES §5.1）＝**GO 待ち**。自律で回せる次があるなら: ②で残した乖離 ⑴（HalfFirst 対 ToFirst）の観測者（行先導 system の複数ページ本の LP 対）／レビュー打鍵残債の推奨順（3-3 → 2-2〈Completion 分割と同便〉→ 1 群 → 3-2 → 4-2・レビュー §5）。
...
⚠️ **見つけて直していないもの**:
- **★★★【ユーザー決定 2026-08-26・優先度高】リファクタ在庫の残り**——**⑨⑪⑬⑭ は第263・④②＋ファイル分割（SpacingRules／MeasureCollector／LayoutEngine）は第264 で完済**（⑬ は stage 1＝FormWalk 読者化まで。stage 2〈ページ walk の消費者化＋pass 展開サービス〉はやるなら別便・engraving が観測者）／**LSP Completion.cs の分割は 2-2（CompletionContextScanner 抽出）と同便で**（第264 で判定・位置切りは散らす）／**CollectionCursor 束ね**（付録 E【9】・リスク中）は移送でなくコード変更＝別便／死物件の残り（CalculatePureSystemHeight・KnuthPlass 単譜オーバーロード＝テスト網の付け替えが要る・**削除はユーザー承認**）。
...
終了時: **本便のコード commit は 5**（`3326bc3f` ④ StaffMiddleUpInSystem＝18 サイトの 1 軒化／`8f7e41dd` ② InterSystemPairMinimum＝2 ページ経路の 1 軒化・統一はゼロ実測で出荷／`65c905b8` SpacingRules 分割／`47a22326` MeasureCollector 分割＋Form.cs 近接／`c8d37306` LayoutEngine 分割）＋ handoff 系（総数は `git rev-list --count origin/master..master` − 開始時 59 で引くこと）・origin `7aa1406f` 不動＝間で push なし・未追跡 0/木 0・**Windows Release 6240 合格 / 0 失敗 / 4 skip / 合計 6244**（±0＝網の追加なし・両便の pin はコンパイラと証明 3 点＋sweep）・**WSL ubuntu Release も 6240 / 0 / 4 実測（④ 後と ② 後の 2 回）**・Core 0 エラー / 0 警告（`--no-incremental`）・台帳 **675／exact 531／総和 24.740990165／非ゼロ 172／count 132・非ゼロ 2／snapshot 222（再ベース 0）／追跡コーパス 572＝全部第263 終了時と同値**（§0 の式で数えた・忠実度コード不触＝suite 全緑＋sweep が計器）。**絵が動いた本 0**——rerender-ls 0/81（stash A/B ×2）＋ **p188 全木 A/B 7 本全部 0/1386**〈④ 1 本＋② 3 本＋分割 3 本・各 1386/1386 render・両側 fail 0・ink/data-pos/midi 0〉。**WSL ubuntu Release は ④ 後・② 後・分割後の 3 回とも 6240/0/4。**毒 3 つ（④ ヘルパ −0.01 → 5/81 赤→復旧緑／② 全 return +0.01 → rerender 0/81=盲目・台帳 23 点赤→復旧 688/688 緑／並列 sweep 検証 −0.01 → 862/1386 赤→復旧 0）・一時変更 0（B1 測定版は commit 前に最終形へ置換済み・`git diff` 清）・base worktree（`$env:TEMP\p264base`・`p264base2`・`p264base3`）掃除済み。
