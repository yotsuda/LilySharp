...
（HEAD `49d19524`・**未 push 77**・**full `sessions/p446/run1.trx` 8774 / 0 / 3 / 8777**・台帳 851 点／
総和 22.584727806・snapshot 249・追跡 `.lys` 609・`-Archive 444` も自動で 66 行 3,973 chars）。
**裏取りは 1 つも赤を出さなかった**。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第445 と同じ。
★ **途中でユーザーが push した**＝`origin/master` は `49d19524` で、**第412 から赤かった CI が緑に戻っている**。

★★★ **⑴ 着手の 1 手目は起票の測り直し＝計器を `Core` 全体に広げた**（脚の 1 行）。**島は 34,722 →
38,527 B／打鍵・114 軒 / 1,841＝+11% で、予測の「2〜4 倍」は外れた。** ⇒ ★★★ **`foreach` の島は
`Svg`＋`Rendering` でほぼ全部**（外から来たのは `KeySpelling` 2,855・`LyricBindings` 683 ほか 3 軒だけ）。
**第445 の A/B 超過 56,139 は「別 namespace の `foreach`」ではなかった**＝見えていないのは
**LINQ の呼び手**と**軒の隣**の 2 つ。

★★★★ **⑵ その「隣」が本体だった。A/B −3.396%**（**4,444,904 → 4,293,941 B／打鍵**・**231 冊中 231 冊改善**・
中央値 −3.194%・最良 −9.913%・最悪 −0.412%）**は会計 32,803 の 4.6 倍**で、**切り分け run 4 本**
（その群だけ stash して測る）が内訳を名指した:

  | 群 | A/B の取り分 | 会計 |
  |---|---|---|
  | **`BestFirstScorer.Solve` 1 軒** | **120,775＝打鍵の 2.72%** | 860 |
  | SpacingRules ＋ `ItemColumn` | 11,000 | 10,987 |
  | Syntax（`ChildNodesOfKind`・`Properties`） | 3,479（`Properties` 844） | 約 2,650 |
  | 残り（beam・key・tab・tie・break-align・skyline） | 15,709 | 18,553（差＝第444「空の歩きは只」） |

⇒ ★★★★ **起票が名指した箱は 860 B で、*同じ 1 行の隣*に 119,915 B が座っていた**＝`new PriorityQueue<,>()`
が寸法を言わず、候補 218 件（max 1,024）を階段で伸ばしていた。**直したのは 1 行**（`(candidates.Count)`）。
⇒ ★★★ **見落とした理由は母集団の定義**＝⒩ の計器は `new Dictionary`／`new HashSet`／`new List` の
3 綴りしか包まない（⇒ 新起票 ⒮・RULES §5.3 に規則化）。

★★★ **⑶ 島は 5,724 B／打鍵（85.1%・81 軒）。*高くなった軒は 0***（第445 ⑷ の「struct walk を歩く
外側の iterator は太る」は出なかった＝今便の直しは*型と添字*の側で、器の形を変えていないから）。
新しい綴りは **`ItemColumn`**（列は「無い・1 つ・リスト」＝`One()` の `yield` を消す＝**計器に映らない側**）・
**`TypedChildNodeList<T>`＋`SyntaxNode.ChildNodesOfKind<T>()`**（`ChildNodes().OfType<T>()` の named form）・
`AllCount`／`AllAt` 等の Count/At 対。
...
★★★ **⑸ 毒 12 本——9 本は予測どおり、3 本が予測を割り、割れ方が所見だった。** 赤: `AllCount` から
右側を落とす **+8**／`SolveColumns` の幅 0 の skip を外す **+8**／`ContextMask` から非 spacer の休符を
外す **+5**／`ItemColumn` の単元が空を名乗る **+4**／`TypedChildNodeList` を 1 slot 遅らせる **+83**／
`SignatureSteps` を逆順 **+32**。緑（予測どおり）: `AllAt` の左右入れ替え・`ApplyLeftHeadWidth` を逆順
（max と平均は順序盲＝**書き換えの安全の根拠そのもの**）。
⚠️ ★★★ **予測を割った 3 本**: **和音の member を「全部 note 0 で読む」毒は tab の fret 桁も tie の
音高照合も +0 緑**。そこで**一段粗い毒＝「和音は 1 つも鳴らさない」**を当てると **tie は +4 赤・
tab は +0 緑**。⇒ **tie は「和音に member が在る」までが観測されていて「*どの* member か」は
観測されていない／tab の和音は*丸ごと*無観測**（第443〜第445 の 4 例目・今度は*粒度*で割れた）。
★ **`SolveColumns` の註も反証された**——「呼び手は幅 0 の item を渡さない」と書いてあったが毒は +8 赤
＝**渡す**（行 score の clef 幅 0）。**直したのは註**（3 か所とも書き直した。コードではない）。

★ **⑹ 終了時**: commit 2 本（code `64c60d31`・docs 1 本＝**この文を含むので SHA は書かない**＝§5.4）・
**未 push 2**（開始時 77 → ユーザーの push で 0 に戻った）・**最終 full 8774 / 0 / 3 / 8777＝第445 最終と同じ**・
§7.5（対 `49d19524`）**Core +417 −121 行／19 ファイル／REF 0／OWN 0**（足した式も定数も 0＝§7.6 ⒟）・
台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・**棚卸しと
`magic_constants.csv` は再生成して net 新規 0／net 消滅 0＝行番号だけ**。全文は Lab `sessions/p446/`
