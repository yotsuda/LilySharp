...
最終更新 第335セッション＝**入り方は第298〜第333 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。第334 の次の一手 ⑶′＝**「Le Freak 3 対・Addicted To Love 2 対を `beng332` の型で割る」**に入り、**Le Freak の根＝段数ループの頁見積り（候補行の begin bucket）を移植して閉じた。出力が動く（追跡 snapshot は 0 枚不動・**実コーパス 9 冊**の段組みが動く）ので RULES §5.1 に従い*製品は commit していない＝作業ツリーに置いたまま*（下 ⑹）。骨は 6 つ**:

★★ **⑴ 計器 3 本（`scratch/p335/`）**: `barsig.ps1`＋`when-settings.ly`＝BarLine ごとに system・X・**when（moment）**を刷り、LP（`lysc ly` の双子）と Lily#（svg の 0.19×4.00 rect）を*同じ小節線*で並べる（第332 `measure.ps1` は X の集合で行頭の broken 片が混ざり、同幅の段を dedup していた）／`handsig.ps1`＋`barnum-sys.ly`＝手書き `.ly` の book ごとの段署名（PROBEBN＋system id）／`ZzP335LeFreakProbeTests.cs.txt`＝break 門のばね（bar ごとの ideal/min/強度/permission）・候補行の force（solver 経路を reflection で再現）・段数ループの得点・頁の実配置を吐く捨てテスト（**`DebugPageBreakingScoring` は static なので並列の他テストが割り込む＝1 度 "Collection was modified" で落ちた**）。
⚠️ ★★ **⑵ Addicted To Love（4,8 → 12）は枠の差で製品の欠陥ではない**: Lily# ≡ LP 双子＝12 小節 1 段（幅 16.73/16.58/16.59/4.76/3.71… 対 LP 16.93/16.79/16.79/4.1/3.69…・段数ループも 20 段 6.21 対 21 段 6.90 で同じ向き）。手書き `.ly` だけが 4|8 で、**`TabNoteHead.font-size = 2` を 0 にした変種も 4|8**（`atl-font0.ly`）＝フォントは犯人ではない。残る差（`\omit StringNumber`・numbers の tab・indent）は双子で組めない（`lysc ly` は "both" score を出せない＝form 単位）。**追わない。**
⚠️⚠️ ★★★★ **⑶ Le Freak の正体＝段数ループの*頁見積り***: 線の和は LP と 3 桁一致（23 段 12.684 対 LP `-ddebug-page-breaking-scoring` 12.705・24 段 13.199 対 13.932）なのに Lily# は 24 段＝**ループが 24 で止まっていた**（`optimal-page-breaking.cc:183-185` の「頁が減って伸びた」出口）。**見積りが 25 段を 5 頁（6,6,6,6,1）・24 段を 4 頁と数え、LP は 25 段で「3 か 4 頁」**。debug hook を広げて並べたら、**`EstimateMeasureHeights` は全候補行に*最大の* begin bucket（1 段目の tempo＋"Intro" 箱＝7.301）を配り、実配置の継続行は 2.311**＝tallness 22.84 対 17.85〜19.44（1 頁 6 段対 8 段）。**LP の `begin_line_heights` は break rank ごと**（`axis-group-interface.cc:417-458`・`system.cc:906-928` は行頭 1 点を読む）。
★★★★ **⑷ 移植（Core 2 ファイル・未 commit）**: `MeasureHeightEstimate.BeginUpAt/BeginDownAt`（小節ごと）＝その小節で段を開いた実配置の begin bucket、無ければ**継続行の最小**（素の prefix＝clef＋小節番号。mark/tempo が乗る行頭はその行だけ）。`EstimatedSystemDetails` が行頭の bucket を読む。**debug hook の出力を広げた**（`est line`／`placed sys` の top/body/bottom/shape/tallness＝LP の `-ddebug-page-breaking-scoring` の隣に置ける）。番人 `SystemCountPageScoreTests.LineStartInk_IsPricedPerCandidateLine`（fixture `test/system-count-line-start-ink`＝Le Freak 1-57 小節・staff＋tab・**2 頁の snapshot 新規 1 枚**＋ tempo/"Intro" を外した対＝同じ 11 段）。`SystemCountPageScore`＋`PageBreaker` 36 本不動。
⚠️ ★★ **⑸ 双子と手書きは同点を逆に割る**: fixture の LP 双子は 11 段 `5,4,6,10,…`、手書き Le Freak（全曲）と Lily# は `5,4,4,12,…`。**2 つの 11 段割りは Lily# の値付けで 12.82 対 13.28（Δf² 込み）だが DP は (25,4) の状態で部分和 2.52 対 2.55 の A を採る**（LP も同じ 1 状態 1 前任者の DP）。⚠️ **第 1 便は「双子の Staff が `\N` の丸数字を刷るから」と書いたが、第 3 便で測って外れた（⑻）**。番人は段数（11）を守り、割りは手書き＝コーパス掃きが測る。
★★★ **⑹ 結果**: **Le Freak 3 対が LP と一致（`5,4,4,12,4,6,2,8,…`）**。コーパス（`scratch/p332/relayout332.ps1 -Csv scratch/p335/structure-after335.csv`・9 分 46 秒）: **T7 KindMatched 209 / SigMatch 148 → 149 / 段違い 42 → 41 / 小節数違い 19**。**全対 SigMatch 440 → 443・SystemsMatch 475 → 481・全対一致の本 207 → 208**。**得た 6＝Carnival ×2・Le Freak ×3・Sweet Memories staff／失った 2＝Nothing's Gonna Stop Us Now staff/tab（代用対＝kind 不一致・`4,4,4,4,…` → `4,4,2,2,4,…`・LP `4,4,4,4`）**。**動いた本 9**（Carnival・everybody goes・himawari・Holiday - OctabeDown・Le Freak・Nothing's Gonna・That's The Way・Universe・カムフラージュ・遠く遠く＝全部 mtime が掃きより前＝この変更で動いた）。**full 7125 / 2 / 4 / 7131（`scratch/p335/run3.trx`・+2＝番人＋snapshot・赤 2＝`HandoffArchiveContinuityTests`＝この §1 を書く前の木）**。`docs/APPROXIMATIONS.md` 再生成（行番号 6 行）。

★ **開始時裏取り**: HEAD `17323393`・**未 push 53**（第334 の 52＋第 4 便の HANDOFF commit）・木 clean・未追跡 0・`bin\Debug` は net10.0 のみ・`xunit.runner.json` 並列・solution build 0 エラー・Core 0 警告（`--no-incremental`・2 分 19 秒）・**full 7125 / 0 / 4 / 7129（`scratch/p335/run1.trx`・2 分 39 秒）**。
**終了時（第 1 便）**: **製品 commit 0（作業ツリー）・docs commit 1（`930068a7`＝HANDOFF＋ARCHIVE＝第333 の経緯を ARCHIVE の先頭へ逐語）・未 push 54**・台帳／`.lys`（追跡）不動・`--no-incremental` build 0 エラー・Core 0 警告（2 分 05 秒）。
✅ ★★★ **⑺ 第 2 便（ユーザー「次便がこの便で有利なら着手」→ 計器が温かいので Nothing's Gonna Stop Us Now を測った → ユーザー「コミットは承認する」）**: **同じ枠（staff＋tab）の LP 双子は 21 段 `4,4,2,2,4,3,3,3,4,6,4,4,2,4,4,4,4,4,4,4,4`＝今の Lily# と完全一致**（`scratch/p335/ngs-pages-when.txt`・LP の頁 8/6/7・Lily# 7/7/7）。失った 2 対は LP の staff だけ／tab だけの本との代用対＝枠違いで、退行ではない。見積り（`est line`）と実配置（`placed sys`）は 21 段全部で同じ数字（`probe6.txt`）。段数ループは LP と同じ出口で 21 に止まる（20 段の線の和 44.85 は試さない＝LP も 21）。**製品を 1 commit（`0ab5ce35`＝Core 2＋番人＋fixture＋snapshot 1＋CHANGELOG＋APPROXIMATIONS）・未 push 55・木 clean。**
★★★ **⑻ 第 3 便（ユーザー「次便がこの便で有利なら着手」→ 双子の忠実度 2 点・予測は 2 度外れた）**: **⒜ 双子の Staff に `\omit StringNumber`**（part が `\N` を持つときだけ・`_stringNumberParts`・Lily# は五線に丸数字を描かない＝collector は tab の弦選択にだけ読む）／**⒝ 対の tab の既定＝numbers を双子にも**（`RenderSpecParser.TabIsNumbersOnly(tab, render)`＝ページの `ParseTab` の答えをそのまま・exporter は `as` の語だけ読んでいたので **2026-08-29 の決定以来、対の tab の双子は全部 `\tabFullNotation`＝ページと別の絵だった**）。番人 `StaffTwin_OmitsStringNumbers_WhenThePartCarriesThem`・`TabTwin_TakesThePagesStyle_FullForALoneTab_NumbersBesideItsStaff`（旧 `TabTwin_AsksForFullNotation_…` は U4 前の既定を固定していた）。⚠️⚠️ **どちらも fixture の LP 双子を 1 桁も動かさなかった**（A1 は 6|10 のまま・bar 11 は 29.93 → 29.8）＝**⑸ の「丸数字が傾ける」は外れ**。**残る差＝Lily# の bar 10-11 自身が LP より 1.3 ss 広い**（bar 11 ideal 32.5 対 LP ragged 31.2）＝T7 の spacing 残差の族・追わない。**LP のソースも読んだ**: `page-breaking.cc:780-872 find_chunks_and_breaks` の chunk は*強制 page break* と score 境界だけ（`is_break` は Optimal_page_breaking では null）→ `constrained-breaking.cc:101-108 calc_subproblem` の `prev_f = 0` は chunk の先頭行だけ＝**強制 `break` をまたぐ Δf² は LP も持ち越す**（Lily# の 1 本 DP と同じ構造・仮説は撤回）。`audit/lpreg` の対 tab は全部 `as full` 明示（U4）＝双子不動。**終了時（第 3 便）: commit 2 本（exporter＋番人＋CHANGELOG＋fixture/番人の注記訂正＋snapshot 1 枚の再承認＝data-pos だけ／この HANDOFF）・full 7126 / 2 / 4 / 7132（`run4.trx`・赤 2＝inventory の行番号＋その data-pos・再生成で焦点 367 / 367）・未 push 58・木 clean。**
★★★ **⑼ 第 4 便（ユーザー「次便がこの便で有利なら着手」→ 計器が温かい `Express Yourself` を測った・製品 0）**: **圧縮行の採点ではなかった**。同じ枠の LP 双子も 11 段（手書きと同じ）。線 DP の理想は両者 17 段・11 段の線の和は 15.52 対 LP 14.92 で近い。**違いは段数ループの下り方**: Lily# は 14 段（2 頁 7,7・力 +0.011）で LP の「頁が減って伸びた」出口が鳴り、LP は 11 まで下る（LP の 1 頁目は題＋8 段）。⇒ **頁 DP の側の問い**（§2 T7 ▶ に計器と次の一手を書いた）。probe は `scratch/p335/ZzP335cProbeTests.cs.txt`。
⇒ ★★★★ **次の一手**: ✅ **⑴ 承認・commit 済み（`0ab5ce35`）**。✅ **⑵ exporter の忠実度 2 点＝⑻**。✅ **⑶ Nothing's Gonna＝枠違い（⑺）**。✅ **⑶′ Express Yourself＝第 5 便が測った（§2 T7 ▶）**: LP の 14 段は同じ行割りで題＋8／6 の 2 頁＝頁の力の符号が違うだけ（LP 1 頁目は最小の積みで満杯・Lily# は 5 ss 余る）。**次は縦の台帳で対を起票**（題付き 1 頁目の最初の段の yoff・staff＋tab の段の tallness）＝新しい便で。⑷ 以下は第334 の並び（⑵′ `lysc ly` の ChordNames＝ユーザーと綴りを決めてから／lyric row の slot の投票／Express Yourself の圧縮行／ps2 の 0.017・beam-over-stem bar 1 の +0.33／push 後 `gh run list`／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／A/B/D/E／C⑴／G）。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
