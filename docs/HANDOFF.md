...
⇒ ★★★★ **次の一手**: ⚠️⚠️ **⑴ この便の製品は*承認待ち*（RULES §5.1）**——snapshot も台帳も不動で射程は ⑺ のとおりだが、**「出力を変える変更はユーザー承認前に出荷しない」**。⇒ **承認をもらって commit する**のが次便の 1 手目（作業ツリーに置いてある）。
...
**⑶ 第339 の路線（頁の島）はそのまま生きている**——**残るのは 1 つ: breaker が 1 行を値付けする 3 つの数（`full_height` / `tallness` / `inverse_hooke`）**。⒜ Lily# 側は取ってある（第339 ⒜ の表）・⒝ **LP 側は `ragged-bottom = ##t` の `yoff` 差分から引く**のが一番安い（第339 ⒝(iii)）。⚠️ **`⒁` の「最小の枠」は着手の根拠にしない**（第338 ⒂⒜＝この本では inert）。
