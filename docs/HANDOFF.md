...
「数字なしの点」を先に直した便**（6 便。第1便＝LYS0023、第2便＝`@chord`、
第3便＝その診断の文言、第4便＝文書、第5便＝裸 `@chord` の判定、
**第6便＝一次資料を `scratch/` から repo へ移設**＝下 ⑹）。
開始時の裏取りは**全項目を走らせた**——build 通過・suite **4740 passed / 0 failed / 4 skipped**・
**台帳 514 点・ss 非ゼロ 97・総和 3.609963181・count 点 106/うち非ゼロ 2**・
HEAD `8b0684ec`・未 push 8・未追跡 1＝**引継ぎの数は git も含めて全部合っていた**（2 回連続）。
終了時 **HEAD ＝ 第6便**（この §1 の更新を同じ commit に入れた）・**未 push 1**
（⚠️ **第1〜5便の 13 個はユーザーが push 済み**＝この数は第6便のぶんだけ）・
suite **4776 passed / 0 failed / 4 skipped**（+36＝新しい網ちょうど。第6便は資料の移設のみ）・
**台帳 514 点で不動**（ss 非ゼロ 97・総和 3.609963181）・**snapshot 0 動**。
⚠️ ★★ **開発機が別の PC に移る**（ユーザー・第170 中に決定）。**▶ の床は旧 PC の数**
——`audit/perf/README.md` に「機械をまたいだ比較は成立しない」と書いた。**新しい環境の
順位は取り直してから書くこと。** ⚠️ **LP のインストール（`C:\bin\lilypond-2.26.0` /
`2.24.4`）は clone に来ない**——`audit\lp-geometry\Measure-*.ps1` 3 本・
`audit\scripts\Run-LilyPond.ps1`・`LilySharp.Tests\Svg\TextOutlineSkylineTests.cs` が
そのパスを直に叩く。★ **そのテストは無ければ黙って skip する**ので、
**移行後の裏取りでは skipped が 4 のままかを必ず見ること**（増えていたら LP が無い）。
- **⑴ ★★★ 第1便＝数字なしの点がエラーになった**（`e947ef65`・**ユーザー起票**＝
  「`c4 g.` は `4.` と読まずエラーにすべきでは？」）。**答は「そうすべき」で、理由は測った**:
  ⚠️ **LP は syntax error**（2.26.0 実測: `\new Staff { c'4 g'. a'4 }` →
  `syntax error, unexpected '.'`・**ファイルごと拒否**）。文法は `steno_duration: UNSIGNED dots`。
  ⚠️ ★★ **Lily# はどちらでもなく、黙って点を捨てていた**——点はどの規則にも届かず、
  music-item ループの skip 回復が**木から落として**いた。⇒ **`c4 g. a4` は `c4 ga4` と
  書き戻る**（**隣とくっつく**ので、第168第1便の点落ちより悪い）。**診断ゼロ・exit 0。**
  ⇒ **LYS0023 を足し、点は木に残した**（round-trip 網＋**陽性対照**
  `AStrayTokenWithNoRule_IsStillDropped`＝規則の無い `?` は今も落ちる、という言明）。
  ★ **「継承＋点を足す」と読む案は棄却**——**継承は点ごと運ぶ**（`c4. g` は付点4分が 2 つ・
  実測＋短い小節の陽性対照 LYS2006）ので、`c4. g.` が `4.` か `4..` か決まらない。
- **⑵ ★★★ 第3便＝その診断の文言を、原因を数えてから書き直した**（`1e30ae56`）。
  ⚠️ **原因は 1 つではなく 2 つだった**——**ディスク上の .lys 1,443 冊で LYS0023 を数えると
  23 冊**で、**その全部が「注釈の旧・点つき綴り」**（`@finger.3`／`@chord.C`／`@mark.A`）、
  **音価の書き損じは 1 冊も無い**（**コーパス 0・フィクスチャ 0**＝全部 `scratch/` と
  `output/` の残骸。うち 12 冊は元から別のエラーで落ちていた）。
  ⇒ ★★ **診断の文言は、原因を数えてから書く**。最初の文言は音価しか名指しておらず、
  **読者の大半を間違った方向へ送るところだった**（名前も `BareDurationDot` → `UnclaimedDot`）。
  ⚠️ **点つき綴りはこの便より前から受理されていない**（点が落ちて `3` が LYS0016 に
  なっていた）＝**変えたのは「黙って」の部分だけ。**
- **⑶ ★★★ 第2便＝`@chord` が引数から読まれるようになった**（`8f4b6a81`・監査 §9.5.3 ⑴）。
  **`ChordNameItem.ParseChordName` は 2 つの `MarkName` 読みごと消え**、
  `AnnotationValues.Chord` が **run の Text をそのまま `TryParseChordEntry` へ**渡す
  ＝**§9.3 の 3 往復が 1 往復**。**`MarkName` の code read は 10 → 9・残る「引数」は `@fig` だけ。**
  ★ **測ってから書いた**（引継ぎの指示どおり）: **コーパス＋フィクスチャの `@chord(…)` は
  14 site・8 綴りで全部 1 run＝旧＝新**。受理される綴りの battery（`g:7/b`・`f:/a`・
  引用形 `"G7#9"`／`"N.C."`・`@chord()`／裸・`c, m7`）も**全部同一**。
  ⚠️ ★★ **監査の「`Arguments[0].Text` で足りる」は偽だった**——**`@chord(c :m7)` は
  引数 2 つで今日 Cm7 として通る**ので、**全引数の Text を連結**しないと受理が狭まる。
  ⚠️⚠️ ★★★ **食い違いは 1 系統だけ実在＝括弧の中に書かれた `.`**。`MarkName` は Dot を
  落とすので **`@chord(b.es:7)` は B♭7・`@chord(.c:m7)` は Cm7 と鳴っていた**。
  ⇒ **ユーザー決定＝落とす**（宣言・網つき・**書いている本は 0 冊**＝299 冊を書き戻して測定）。
  ★ **gate は case-SENSITIVE**（`@Chord(c)` は昔も今も chord ではない）＝
  `AnnotationValues.Named`（OrdinalIgnoreCase）は使えない。
  ⚠️ **他の 8 家族はそれで正しい**——**旧 gate は `MarkName.ToLowerInvariant()` してから
  `StartsWith` だった**（実測）＝**第169 は受理を広げていない**。疑って確かめた項。
- **⑷ 監査の数の訂正**: **§9.1 の「chord 16」はコメント内の 2 件を数えていた**
  （`07-lead-sheet.lys:1`／`chords-funky-ignatzek.lys:4`＝**後者は `)` すら無い**）。**実ソースは 14。**
- **⑸ ★ 第5便＝裸 `@chord` の判定も `MarkName` から外した**（監査 §9.5.3 ⑶）。
  **collector は `AnnotationValues.Chord` に 1 回だけ訊く**（null＝chord でない／
  `""`＝自分では名乗らない＝裸／それ以外＝記号）＝**同じ問いを 2 つの物に訊くのをやめた**。
  ⇒ **`MarkName` の code read は 8 site。**
  ★ **等価は自明ではないので測って網にした**——**`@chord.c` も `MarkName=="chord"` に
  落ちる**（点は mark に食われず、第1便から LYS0023）が**そこでも引数 0 個で一致**し、
  **唯一ずれ得た `@chord.up` は mark ノードすら作らない**。
  ⚠️ **この便を今やったのは測定が手元にあったから**——次便に送ると §0 の家訓どおり
  測り直すので、**同じ測定を 2 度払う**ことになる。
- **⑹ ★★ 第6便＝引用の指す先を repo に入れた**（開発機の移行に伴う・ユーザー指示）。
  **`scratch/` は `.gitignore` に入っているので clone に来ない**——なのに
  **文書がそこを 102 箇所から名指していた**。移設先は 2 つ:
  **`audit\lpreg\`（673 ファイル・3.2 MB）** と **`audit\perf\`（stagebench 7 本）**。
  ★ **同時に 3 か所を書き換えた**: 文書の引用 102 箇所／**lpreg の `.ps1` 56 ファイル・
  290 箇所の自己参照**（スクリプトが `scratch\lpreg` を自分の場所として書いていた）。
  **バイト単位の置換なので他は 1 バイトも動いていない。**
  ★ **検算**: 文書の「完全なファイル名の引用」**51 件が全部実在する**（移設前は 0 件が
  clone に存在しなかった）。
  ⚠️ ★★ **`.svg` 522 枚・2.1 GB は持ってきていない**——**ここのスクリプト自身の出力**で
  再生成できるうえ、`.gitignore` の `*.svg` がそもそも弾く。**旧 PC 側は消していない**
  （そちらは stale）。⇒ **`audit\lpreg\*.svg` を名指す引用は、再生成してから読むこと。**
...
生データ `audit\perf\stagebench-159.txt`）**——**床 plain 114.2 / fingbeam 227.4 / v2bow 166.2。
