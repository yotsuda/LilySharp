...
★ **⑷ 終了時**: commit 2 本（docs 1＋code 1）＋docs 1 本・**最終 full `sessions/p403/run6.trx` 8665 / 0 / 3 / 8668**（+1＝⑶ の網・run4 は網の後半とAPPROXIMATIONS 前で 2 本赤・run5 は code commit 前で同数）・§7.5（対 `567355ef`・スクリプトの出力）**Core `+` 24 行／REF 0／OWN 0**（数値定数なし・LP 由来の式なし＝memo の経路だけ＝監査対象は「同じ答えか」＝⑶ の網と full 同一）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。§7 3.5 は済（第401 を `-Archive 401`＝11 行 5,271 chars）。RULES §5.3 に「数秒の in-process 計器は tier-0 を含む」を汎化して置いた。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⇒ **次の一手**: ⒜ **R13⒥＝`ChooseSystemCount` の「ideal より多い count」loop（plain1k・fingbeam1k で 6 ms／13 MB 毎打鍵・78 count・常に ideal）**＝LP の transcription なので意味論は動かせない＝候補は ⒤ 答えの memo（入力＝line DP の表（count ごとの breaks・force 和）＋estimate 配列＋permission＝全部 value・hash して前打鍵と同じなら同じ答え・§5.4 の memo==recompute の網が要る）か ⒥ count ごとの `EstimatedSystemDetails`＋page DP の割当を減らす／⒝ **R13⒝＝render 段（Δ=0 打鍵の最大の段・plain1k 2.3 ms／8.4 MB・fingbeam1k 7.7 ms／33.6 MB を*打鍵ごとに*割当）**＝SVG 全文の 7 回コピー（`Assemble`→JSON→…）を差分応答に／⒞ edit 打鍵の残り＝`prelim` 1.4〜2.5 ms／3.7〜9 MB・v2bow1k の `voices`（`CalculateVoiceCollisions` 3.1 ms／7.5 MB 毎打鍵・memo 無し）・edit 位置が前ほど `breaks` が伸びる（v2bow1k bar 2 で 37 ms＝DP の 111 行を頭から）／⒟ 診断 pass の残り＝`AnnotationName` 14／`MeasureValidator` 10／`LyricSings` 6.5（fingbeam1k・warm）／⒟ LSP 起動直後の数秒は `Compute` 等が tier-0 で 6 倍＝ReadyToRun 発行や `TieredPGO`／`TieredCompilation` の knob は packaging（ユーザーの `tools/Package-And-Install.ps1`）の側＝**要ユーザー判断**／LP 双子が要る R7〜R11／R15（要承認）。
