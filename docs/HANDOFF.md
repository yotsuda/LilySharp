...
>
> ★★★ **ユーザーが「pedal と ottava は別便で統合する」と決めた**（2026-08-29）。**同じ便で
> `@rit` 族を B（終端必須）に寄せることも決まっている**ので、**この項はその続きであって
> 独立した island ではない**——**寄せる先は `@rit` 族が先に作る。**
>
> **⚠️ 何が問題か＝1 つの問いに 3 つの答えがある**（第288 実測・`scratch/p289/pair1`・`pair2`・`r1`。
> **3 つとも `lysc check` は `No errors found`**）:
> **⒜ `@sustainOn` を閉じないと*何も描かれない***（**"Ped." の文字ごと消える。しかも無言**）／
> **⒝ `@ottava` を閉じないと*描かれ*、12.01→33.64 で止まる**（**五線の右端は 60.37 なので
> 「最後まで」でもない・第 3 の独自長**）／**⒞ `@rit` を閉じないと 1 小節**（**B で退役予定**）。
> ⇒ **どれも LP のものではなく、どれも読み手に告げられていない。§5.2.1② が言語のレベルに出た形。**
>
> **⚠️ LP はこの問いに答えを 1 つしか持たない**: **閉じられなかった span は warning を出して
> `suicide()`＝何も描かない**（`lily/text-spanner-engraver.cc:121-130` で実測・
> `\cresc` 側も `unterminated crescendo` で同じ）。**LP に「既定の長さ」という概念は無い。**
>
> ⇒ **⒜ は既に LP と同じ「描かない」だが*無言*なのが欠陥**——**診断を足すだけで寄る**。
> **⒝ は「独自の長さを描く」ので、終端（`@loco`）必須へ移すと出力が動く**——**射程は掃きで測ること。**
> ★ **`@ottava` の終端は既に語彙にある**（`@loco`）ので、**新しい綴りは要らない可能性が高い。
> 確かめてから決めること**（§5.2.1 の「無いと言う前に語彙・パーサ・fixture を grep」）。
>
