...
**未 push 121**（この引継ぎ commit まで）・テスト **3893 passed / 0 failed / 4 skipped**（**+8**）・
台帳 **416 点**（**ss 非ゼロ 87・総和 4.547345519**／**count 点 99・うち非ゼロ 2**）。
⚠️ **総和が 4.055931346 → 4.547345519 に増えたのは悪化ではない**——**今まで一度も測っていなかった量**
（テキスト幅）を 8 点足した分＋上の ottava の +0.0462（**算術が LP と一致した結果**）（README
「比較は同じ点集合の中でのみ意味を持つ」）。
