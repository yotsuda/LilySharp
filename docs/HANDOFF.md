...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第245セッション＝**PC の再起動で途中に終わった第244 を閉じ、そのあとユーザーが「五線の1段目と2段目が近すぎる。これは明らかに不具合だ」と報告した。段間の床が*段と段*のあいだの量として書かれていた——LP はそれを*五線と五線*のあいだで言う。**（2 commit＝`49f7af51` 第244 の §1 復元／`83709aed` 段間の床の枠／この行）⚠️ **台帳 580→583**（新点 3）／**総和 7.643880014→7.843880014**（新しい非ゼロ 1 つ＝`system.indent-floor.one-staff` の 0.200000）／**exact 488→490**／**count 113・非ゼロ 2 不動**／**追跡コーパス 572 不動**／**snapshot 222 は不動だが 3 枚を再ベース**（追加 0）。**6088→6095**（＋4 新網 ＋3 新台帳点）。

**① ★★★ まず第244 の残骸を閉じた。** **第244 は 3 commit を積んだところで PC が落ち、§7 チェックリストの step 3.5（第242 をアーカイブへ）だけ済ませて §1 を書いていなかった。** ⇒ **`HANDOFF.md` のセッションブロックが 0 になり、`HandoffArchiveContinuityTests` の 2 本が赤**（コードは 1 本も赤くない・6088/0/4）。**§1 を 3 つの commit message から復元し、数は木で測り直した**（`49f7af51`）。⇒ **骨 3 に一般化した**（下）。

**② ★★★ 報告された本＝`scratch/ベースタブLy/Untitled-6.lys`、そのとき `staff melody` を 2 回。** **第1段の下の五線と第2段の上の五線が 2.870**、**同じ A→B の対が同じ本の後ろでは 8.200**。**第2段のセクションラベルの箱が、第1段の楽器名「Melody」を貫いて印字されていた。**

**③ ★★★ 機構の半分は欠陥ではなく、それを判らせるのが仕事の大半だった。** **下の段のラベルと小節番号は*インデントの列*に立つ**——**上の段はインデントされている側なので、そこに五線が無く、輪郭も無い。** ⇒ **per-X の clearance は「よけるものが無い」と正しく答え、段が上がってくる。** **LP もそうする**: 高いマークを置いて 2.26.0 に訊くと、**第1対は床に落ち、同じ本の後ろの同じ対は refpoint 間 21.434270** と読む。**対照 SIF2N（インデントだけ外した本）では同じ対が 17.705730。**

**④ ★★★ LP がしないのは*床を突き抜けること*で、そこが欠陥。** **`system-system-spacing`（basic 12・minimum 8・padding 1）は「上の段の*最下*五線 ↔ 下の段の*最上*五線」の量として書かれている**ので、**段の本体はその量の外**。**`CreatePages` は origin↔origin の距離をその数で床張りしていた**——**その距離は `SysHeight(i)` で始まる**ので、**2 五線で本体 13.000000 になった瞬間に `Math.Max(12, …)` は何も守らなくなる。**

⚠️⚠️ ★★★ **1 五線はそれを*厳密に*隠す。** 本体 4.000000 で **12.000000 − 4.000000 ＝ LP が引く 8.000000**——**2 つの枠が算術の偶然で一致する。** ⇒ **追跡 572 冊が一度も動かなかった理由。**

**⑤ ★★★ そして 2 つ目の綴りで、出荷されているほうが壊れていた。** **`PageLayouter` の chain（複数ページ／最適経路）は同じ変換を*既に持っており*、コメントで理由まで書き、REF も付けている**:
`dist += anchor(sysIdx + 1).ToFirst - originToLast[local];`
**`CreatePages` の速い経路には変換が 1 行も無かった**——**そして `UseOptimalPageBreaking` の既定は false** なので、**`lysc svg` / `png` とエディタのプレビューが通るのはそちら。** ⇒ **同じ `PageAnchorOffsets` から床の前後で枠を往復させ、`originToLast` の hara-kiri 規則（staff spring を持たない段は chain が最下五線に届いていないので first refpoint から出る・番人は LYRHKG）もそのまま写した。** **LILYPOND-REF: `lily/page-layout-problem.cc:1120-1126 build_system_skyline`（`first_spaceable_dy` / `last_spaceable_dy`）。新しい定数 0・Core の `+` 45 行・REF 1 本。**

**⑥ ★★★ LP 実測は 3 冊 1 変数**（`audit/lp-geometry/probes/system-indent-floor.ly`・新設）: **縮んだ対は五線 1・2・3 本のどれでも refpoint 間 12.000000**——**本体は入らない。** **予測と反証を走らせる前に probe に書いた**（§5.0-2）: **staff 数で*育つ*なら LP も origin の枠で床張りしていることになり、Lily# の旧枠が正しく報告は別件**。**育たなかった。** ⚠️ **高いマークの本 SIF1/SIF2/SIF3 が「12.000000 は*床*であって偶然ではない」を作っている**——**自由な対が別の数を読む唯一の本だから。** ⚠️ **それらに Lily# の双子は無い**（`markup box column` に Lily# の綴りが無い）ので、**台帳の本は普通のマークの SIFO1/2/3。**

**⑦ ★★★ 台帳 3 点で、三つ組であることが要点**:
| id | 修正前 | 修正後 | |
|---|---|---|---|
| `system.indent-floor.two-staff` | 10.540000 | **12.000000** | EXACT |
| `system.indent-floor.three-staff` | 10.545000 | **12.000000** | EXACT |
| `system.indent-floor.one-staff` | 12.200000 | 12.200000（**不動**） | 残差 **0.200000**・OPEN |

⚠️ **3 本目が「もっともらしい間違った直し方」を落とす**——**「basic-distance ＋ 本体」と書けば 3 五線で 21.000000 になる。** ⚠️ **2 五線と 3 五線が *違う*数（10.540 対 10.545）に壊れていたことも徴候**——**本ごとに違う値になる欠陥は「定数が違う」ではなく「床が無い」の顔。** ⚠️⚠️ **1 五線の 0.200000 は*床ではない*。** **その本では LP が床に載り Lily# は床の 0.200000 上に居る**ので、**その残差が測っているのは「継続段のマークと小節番号の上に Lily# が余分に取る分」＝`barnumber.*` の一族**で、**この便の枠ではない。** **exact に丸めず 0.200000 と記録し、どの量かを `why` に書いた。**

**⑧ ★★ 網は 4 本・毒 3 種で、赤くなる集合が互いに素**（`LilySharp.Tests/SystemGapStaffFrameTests.cs`）: **変換を戻す／変換を 0.5 ずらす → 縮んだ対の 2 本が赤、他 2 本は緑**。**全対を basic-distance に固定する → 1 五線の腕と「後ろの対は今も輪郭が決める」が赤、縮んだ対の 2 本は緑。** ⚠️ **変換の毒は 1 五線の腕に*届かない*——そう書いた**: **1 五線では `ToLast` と `ToFirst` が同じオフセットなので変換が恒等 0**。**それは修正についての*事実*で、腕は「逆側の間違い（床を張りすぎて輪郭が決めなくなる）」を見張ることで席を得ている。**

**⑨ ★★★ 掃き 572 冊で 10 冊動き、10 冊とも開いた**（骨 6）。**動きは全部*増加*で、全部 8.000 に着地**——**つまり 10 冊とも LP の床より下に居た。** `grammar-tour` 7.650/4.590/6.150/7.860/5.510・`hara-kiri` 3.360×2・`ossia` 4.320・`hara-kiri-percent-repeat` 4.320・`hairpin-spanbar` 4.320・`harakiri-percent(-ctrl)` 4.320・`amazing-grace` 7.950・**`fur-elise` 3.550/4.460/3.380**・**`greensleeves` 7.590** → すべて **8.000**。**部屋が減った箇所はどこにも無い。** ⚠️ `grammar-tour` は**段内**も ±0.010 動いた（ページが伸びて五線のばねが解き直された・どちらも ideal 5.000 より広い側）。

⇒ ★★★ **次の一手**: **下の「見つけて直していないもの」から選ぶ。本便の島は閉じている**（報告・LP 実測 3 冊・台帳 3 点・移植・網 4 本／毒 3 種・掃き・目視・両 OS）。

⚠️ **本便で新しく判ったこと（起票のみ）**:
- **★★★ `CreatePages` の速い経路と `PageLayouter` の chain は今も 2 つの綴り。** **本便は枠を揃えただけで、畳んでいない。** ⚠️ **「畳めないなら差分の網を張る」（§7.7）が未着手**——**同じ本を `UseOptimalPageBreaking` あり／なしで組んで段間が一致することを見る網**は書けるし、**期待値を知らなくても書ける。** **この欠陥はその網が在れば 1 便で出ていた。**
- **★★ `UseOptimalPageBreaking` の既定 false は「利用者が通る経路＝速いほう」を意味する。** **台帳の既定 paper もそちらを通る**（本便で確認）。**「ページの計算を測った」と書くときは、どちらの経路かを言うこと。**

⚠️ **見つけて直していないもの**（第244 から持ち越し・**本便が消したのは無し**）:
- **★★ 第244 の乖離の在処＝`MusicMarkEngraver` の rows-only の腕**（⒜ を落とせば `base + Padding` に戻り予約も落ちる）。**LP は 0.460000 上に置く**（実測・`f1200c21`）。**ユーザー決定なので閉じない。**
- **★★★ 骨 9 ⒞ の残り＝テキスト行に*インクの profile* を与える島**（`SkylineBuilder.AddStaffToSkylines` はテキスト行で早期 return）。**第241 から名指され、`barnumber.rows-only.row-to-ink-bottom` の 2026-08-24 のユーザー決定と当たる。**
- **★★ 再現できていない報告＝1 小節に和音 4 つで和音名が横に重なる。** **第243 が 9 通り試して全部隙間は正**（最も狭くて 0.15）。⚠️ **`ChordNameEngraver.ClearOfPrevious` の抜け道**＝`if (!cur.UseTiming || !prev.UseTiming) return curX;`（`@chord(…)` が絡むと押し出しが効かない）。⚠️ **第244 がその隣に第 2 の押し出しを足した**ので**実物が来たら両方見ること。当てずっぽうで直さない。**
- **★★★ `UnmodelledRow` は*スコア全体*を降ろす**（第243 起票・未着手）。`BuildLooseChainEnds` は全 system を回る loop の中で `if (alignment.UnmodelledRow) return null;`——**`return` はメソッドを抜ける**（`LayoutEngine.cs:2990` 付近）。**旗を退役させるのが本来の島。**
- **★★ `staff / chords / lyrics` に Lily# は無言。LP は警告する。** **診断を出すかは言語の決定＝要ユーザー判断。** ⚠️ **「score は帯の縦並び」はユーザー決定（2026-08-19・§3）なので、拒むのは筋が悪い。出すとしても注意まで。**
- **★★ `SkylineBuilder.OuterStaff` の staff 半分は今も残る**（番人は LYRHKG の 4 点）。
- **★★ `make-colon-bar-line` の `dist` 探索が未移植**（第240 ⑤）。**器を作る島**で、**Lily# のリードシート行は既に意図的乖離**（§3 第226）なので「LP に合わせる」の意味を先に決めること。
- **★ `RepeatDotRadius = 0.2` は LP の 0.225 に対して小さい**（`LILYSHARP-OWN` 起票済み・LP は `dots.dot` グリフ）。⚠️ **閉じると `RepeatDotsOffset` 経由で横の予約も動く。**
- **★★ 旋法の語彙は 4 綴りあり `LanguageVocabulary` に載っていない**／**★ `RenderSpecParser` と `PartReferenceFinder` は今も `as` を自分で切ってパートを探す**。
  > ⚠️⚠️ ★★★ **上の 2 件は「独立した便」にしないこと**（§5.1）。**その語に触る便に括り付ける。餌ではなく在庫。**
- **★ 診断パスは 1000 小節の本で 148〜218 ms**（44 validator・上位 10 は全部 `DescendantNodes()` の全木走査）。⚠️ **打鍵ごとではない**（debounce ＋ cancel）。
- **音楽を持つが行のセルを持たない section は rows-only グリッドで今も 0 小節**／**`audit/magic_constants.csv` の `RepeatDotPosition1/2` の行番号が約 700 行ずれたまま**（生成物・再生成は独立の diff）／**`@chord(…)` と chords ブロックが違う和音語彙を読む**。
- **★★ CHANGELOG は「独立した残債」ではなくリリース着手**（本便で確認）。`docs/RELEASING.md` が**バージョン番号の決定と changelog を「自動化しない＝人間の判断」**と明記し、**最上段セクションがそのまま GitHub Release の本文**になる。⇒ **0.4.0 はタグ済み・第242 が言語を 3 つ狭めている**（未知の主音・未知のタブ様式・未知の調弦）ので**次は breaking**。**第243 の `staff / chords / lyrics`、第244 の rows-only セクションマーク、本便の段間**も Fixed / Changed に要る。**着手はユーザーの版番号決定から。**

★ **開始時裏取り**: HEAD `f6f83d3a`・未 push 28・未追跡 0・**木に第244 の未コミットのアーカイブ移動が残っていた**・Windows **6086 合格 / 2 失敗**（`HandoffArchiveContinuityTests` の 2 本だけ）・台帳 580／総和 7.643880014／snapshot 222／追跡コーパス 572・Core 0 警告・CI は直近 5 本すべて success（**最後の push は `7930f50b`**）。
終了時: **本便は 2 commit**（`49f7af51` handoff 復元／`83709aed` 段間の床）・**未 push 30**・未追跡 0/木 0・**Windows 6095/0/4**・**WSL Ubuntu Release も 6095/0/4**・Core 0 警告（`--no-incremental`）・**台帳 583／exact 490／総和 7.843880014／count 113・非ゼロ 2／snapshot 222（3 枚再ベース・追加 0）／追跡コーパス 572**。⚠️ **§7.5 の数え上げ**: `LilySharp.Core` の `+` **45 行**／**`LILYPOND-REF` 1**（`page-layout-problem.cc:1120-1126 build_system_skyline`）／**`LILYSHARP-OWN` 0**／**新しい数値定数 0**。**分類は §7.6 ⒝**（LP の座標変換から導出。**LP は skyline 自体を持ち上げて枠を移す**が、**Lily# はばねの側で引く**——`SkylineBuilder` を他の読み手のために 1 つの枠に保つため。**字面にするには builder の枠を全読み手ごと移す必要**があり、それは `PageLayouter` のコメントが既に「HANDOFF 2D」と書いている島）。⚠️ **再現物**は `scratch/ベースタブLy/Untitled-6.lys`（ユーザーの本・`.gitignore` 配下）。**追跡されている等価物**: `LilySharp.Tests/SystemGapStaffFrameTests.cs`・`audit/lp-geometry/probes/system-indent-floor.ly`（SIF1/SIF2/SIF3/SIF2N/SIFO1/SIFO2/SIFO3 の 7 冊）・台帳 `system.indent-floor.*` の 3 点。

> ## ★★★ 骨 1＝**「ユーザーの報告どおりに壊れている」と「報告された動きが全部欠陥だ」は別**
> 第1段と第2段が近すぎたのは本当で、直すべきだった。**しかし*近づいた理由*の半分は LP も同じことをする**——
> **インデントの列に下の段のマークが入り込み、上の段はそこに輪郭を持たない。**
> **高いマークを置いて LP に訊くまで、それは判らなかった**（普通のマークだと LP も床に居るので、
> **床と輪郭の区別がつかない**）。
> ⇒ ★★★ **判定法**: **報告が「A が B に近すぎる」の形なら、*どこまで近づいてよいか*を LP に訊く前に、
> *なぜ近づいたか*を LP に訊く。** **前者だけ訊くと、LP と同じ機構を欠陥として消しにいく。**
> ⚠️ **そして「LP も床に居る」本では床は測れない**——**測るには LP を床から離す変数が要る**（ここでは高いマーク）。

> ## ★★★ 骨 2＝**定数の枠は、その定数を超える大きさの入力が来るまで*無言で*効かなくなる**
> `Math.Max(12, …)` は五線 1 本（本体 4）では正しく 8 を残し、五線 2 本（本体 13）では**何もしない**。
> **エラーも警告も出ない。max が恒真になるだけ。**
> ⇒ ★★★ **判定法**: **床や上限を書いたら、*その数と同じ単位の量*が式の中に何個入っているかを数える。**
> **床と同じ側に「本体の高さ」のような伸びる項が入っていたら、それは床ではなくオフセット。**
> ⚠️ **そして 1 五線が「たまたま正しい」のが最悪の形**——**12 − 4 = 8 は LP の値そのもの**なので、
> **コーパスの大多数が正しい絵を出し続け、572 冊の掃きが 0 冊を返す。**
> ⇒ ★★ **「この量は N=1 のとき退化するか」を先に訊く。退化するなら N=2 と N=3 の本を*同時に*作る**
> （本便の台帳が三つ組なのはそのため。**2 点は直線で通せる**）。

> ## ★★★ 骨 3＝**§7 のチェックリストは順序を持つ。§1 を書いてから、いちばん古いブロックを落とす**
> 第244 は **step 3.5 を先に済ませ、§1 を書く前に落ちた**。残ったのは **「ブロック 0」**——
> **`TheHandoffKeepsExactlyOnePredecessorBlock` は「多すぎ」と「少なすぎ」を 1 本で見張っている**ので
> 赤にはなったが、**赤が指すのは「アーカイブし忘れ」で、本当の欠落は「§1 を書き忘れ」だった。**
> ⇒ ★★ **判定法**: **§1 を書き換えてから落とす。** その順なら途中で落ちても残るのは「ブロック 2」＝
> **何が起きたか読める状態。** ⚠️ **網は正しく働いた**（次のセッションが 30 秒で見つけた）。
> **直せなかったのは網ではなく手順の向き。**

> ## ★★ 骨 4＝**「印字するだけの計器」を測定として読むと、毒が緑で返ってくる**
> 本便は `Corpus_ReportsTotalDivergence` の出力を見て「台帳は不動」と 2 度書いた。
> **あれは JSON に*記録された*残差を印字するだけで、Lily# を測っていない**——
> **RULES §6 にその警告が書いてあり、本便はそれをセッション頭に読んでいた。**
> **毒を当てても 491/583 exact のまま返ってきて、初めて気づいた。**
> 測っているのは **`Geometry_MatchesLilyPondWithinTheRecordedResidual`**（probe ごとに 1 本）。
> ⇒ ★★★ **判定法**: **「この数はどこから来たか」を、緑を根拠にする*前*に 1 行で言えるか。**
> **言えないなら、それは計器ではなく掲示板。**
> ⚠️ **そして毒はこの取り違えを*必ず*捕まえる**——**緑のまま返る計器は、その毒について何も見ていない。**
> **新しい点を足したら、必ず一度赤にしてから記録すること**（本便はそれで
> `one-staff` の残差が 0 ではなく 0.200000 だと判った。**0 と書きかけていた**）。

---

## 以下は第244セッションの経緯
最終更新 第244セッション＝**第243 が「不利」と判断して着手しなかった 0.44 の島を、ユーザーが*別の閉じ方*で開いた。** 報告は「**セクションマークとコード名は同じ y 位置に表示したい。セクションマークとコードの x が重ならないように注意して**」——⇒ **LP に寄せるのではなく、印刷譜（リードシート）の慣習に寄せる*決められた乖離*として閉じた。**（4 commit＝`2f94800a` 継ぎ目／`f1200c21` probe／`f6f83d3a` 配置と予約／この行）⚠️ **台帳 580・ss 総和 7.643880014・count 113・非ゼロ 2・追跡コーパス 572 は全部不動**（**本便は台帳点を 1 つも開いていない——測ったうえで*開かない*と決めた**・⑤）。**snapshot 222 は不動だが 4 枚を再ベース**（追加 0）。**6084→6088**（＋4＝新網 `RowsOnlySectionLabelTests`）。

⚠️⚠️ ★★★ **本便は 3 commit を積んだところで PC が意図せず再起動し、§1 の書き換えだけが未了で残った。** ⚠️ **§7 のチェックリストは 3.5（第242 をアーカイブへ）を*先に*済ませていた**ので、**`HANDOFF.md` のセッションブロックが 0 になり `HandoffArchiveContinuityTests` の 2 本が赤**——**`TheHandoffKeepsExactlyOnePredecessorBlock`（0 blocks）と `TheArchiveResumesWhereTheHandoffStops`（Assert.NotEmpty）**。**コードは 1 本も赤くない**（6088/0/4）。⇒ ★★ **この §1 は次のセッションが 3 つの commit message から復元したもので、作業そのものを見てはいない**（**根拠は全部 commit message に在り、数は本便で測り直した**）。⚠️ **step 3.5 を §1 の書き換え*より先*にやると、途中で落ちたときに「ブロック 0」という*どちらとも取れない*状態が残る**——**次からは §1 を書いてから落とすこと**（骨 3）。

**① ★★★ 報告された本＝`scratch/ベースタブLy/Untitled-6.lys` の rows-only 形**（第243 のあいだにユーザーが `staff melody` を殺して rows-only にした、あの本）。**`A`・`B`・`A2` が `Dmaj7` の帯まるごと 1 つ上に浮いていた。** ⚠️ **同じ本を `chords / staff / lyrics` と書けば昔から正しい**——**ユーザーはその非対称を同じ息で言っている**ので、**要求は「rows-only のリードシート」という 1 つの形についてだけ。**

**② ★★★ 原因は第243 が §1 に書いた通りだった＝*マークの base は staff の形*。** `MusicMarkEngraver` の base は `2.0 - AboveStaffOffset`＝「**上端線の 2.5 上**」で、**それは五線 5 本をよける形**。**コード行に線は無い。** ⇒ 第243 が「定数では閉じない・`StaffLayout` が行の refpoint を持っていない」と書いた診断は正しく、**本便はまさにその器を作った**（③）。

**③ ★★★ 器＝`OutsideStaffStacker` が握っていた「名目の半五線」を、置かれた layout 自身の refpoint に替えた**（`2f94800a`）。**`AboveTrackers`（上）と `StackBelowStaff`（下）が両方とも `StaffBottom / 2.0` と綴っていた**——**4 スペースの五線でだけ正しく、タブ譜・ossia・そして何より*テキスト行*で誤る**（**テキスト行の profile は自分のテキストベースライン基準**で、`MultiStaffLayouter` の merge のコメントが元からそう書いている）。⚠️ **量には既に家が在った**＝`MultiStaffLayouter.RefpointBelowTop`（2026-07-28 に「**LP のフレームと Lily# の帯モデルの唯一の継ぎ目**」と書いて開いたもの）。**この 2 人がそこを通っていなかったのは、stacker が持つのが `StaffLayout` で、量が `Staff` に住んでいたから。** ⇒ **`StaffAffinity` と同じ理由で、量が*置かれた layout*に引っ越した。** **LILYPOND-REF: `lily/align-interface.cc:201-285 internal_get_minimum_translations`**（**alignment は VerticalAxisGroup の*参照点*どうしで働き、group の refpoint は一般に extent の中央ではない**——Lyrics / ChordNames の refpoint は**テキストベースライン**）。

⚠️⚠️ ★★★ **この commit は絵を 1 mm も動かさない。そしてそれは*結果*であって細工ではない**（§5.2）——**この変更だけを木に入れて測って、222 枚と全網が緑（6084/0/4）**。**テキスト行の profile は*空***（`SkylineBuilder.AddStaffToSkylines` がテキスト行で早期 return する）**なので直った歩幅が動かすインクが無く**、**コーパスにはこの 2 つの継ぎ目を通る tab / ossia の本が 1 冊も無い。** **観測しているのは次の commit。**

**④ ★★★ 配置と予約は 1 つの commit**（§5.3・**第243 が「片方だけ入れて実証した」あの罠**）。**Y だけ入れると、ユーザー自身の本で `A2` が `Dmaj7` を*貫いて*印字された**——**第243 が greensleeves で見たのと同じ失敗。** ⇒
⒜ **Y＝`MusicMarkEngraver.StafflessAnchorRefpointBelowTop`** が**錨 layout 自身の refpoint**（③ で持たせた場）を読み、その上に箱の中心を `LabelBaselineBelowCentre` だけ置く。⚠️ **この経路では和音の天井をわざと適用しない**——**天井はマークを記号から*剥がす*装置**で、**ここは 2 つが同じ線を共有するのが要求。**
⒝ **X＝`MusicMarkEngraver.BoxedLabelXWindows`** が `ChordNameEngraver` にラベルの枠を渡し、**列が塞がっている記号が右へ動く**（**`ClearOfPrevious` が隣から逃がすのと厳密に同じ `SymbolGap`**）。**隣の cascade の*前*に走る**ので、**動いた記号から先は cascade が全部取り直す。**
⚠️ **X のパスは全体レイアウトが呼ぶのと*同じ 2 関数*（`CalculateXPosition` ＋ `MarkXExtent`）の 2 度目の呼び出しで、2 つ目のモデルではない**——**記号が空ける区間と、ラベルが描かれる枠が、食い違いようがない。** **別パスなのは和音 layout が `MusicMarkEngraver.Calculate` の*入力*だから**で、**マークの X は break-align 列だけで決まるので早く訊いても定義できている。**

**⑤ ★★★ LP に訊き、そして*台帳点を開かなかった*——これが本便で一番きわどい判断**（`f1200c21`）。**probe に MKV を足した＝MKT の変数を 1 つだけ変えて和音記号を*高く*した**（`cis` が持ち上がった臨時記号を印字する）。**予測を走らせる前に書いた**（§5.0-2）: **両方とも同じ隙間 0.460000 を読む。反証**: MKV で隙間が**育つ**なら、マークは記号のインク以外の何かに置かれている。**実測（2.26.0・各本の第1段）**:

| 本 | grob | ラベル下端 − 最寄り和音のインク上端 |
|---|---|---|
| MKT | SectionLabel | **0.460000** |
| MKV | SectionLabel（記号が高い） | **0.460000** |
| MKS | RehearsalMark | **0.460000** |

⇒ **3 冊・2 grob・2 通りの記号高で 1 つの数**なので、**0.460000 は `outside-staff-padding` であって 1 つのグリフ輪郭の偶然ではない。**
⚠️⚠️ ★★★ **そのうえで台帳点は開いていない。** **台帳点は「Lily# が詰めようとしている距離」の宣言**で、**この距離は*詰めない*と決めた**（ユーザー決定 2026-08-24）。**開けば「未移植の島」として次の便を誘い、閉じれば嘘になる。** ⇒ **測定は probe と commit message に置いた**——**次の便が「LP は何をするか」と「乖離は*選ばれた*ものだ」を両方読めるように。**

**⑥ ★★ 門は 1 行で、両方の主張を同時に言う**＝**これらの grob がぶら下がる行が CHORD-ROW の記号を持っている**こと。**「五線が無い」は無料で付いてくる**——`LayoutUtilities.TopScoreGrobStaff` は system に spaceable な staff が在れば必ずそれを返し、**独立した chords 行は決して spaceable ではない。** ⚠️ ★★★ **`TopSpaceableStaff is not null` の別 guard がここに在って、消えた**——**どんな毒を当てても単独で赤にならなかった**（**骨 2＝赤にできない腕は対照ではない**）。**2 つを 1 行に畳んだことで、両方の対照がその 1 行に載った。**

**⑦ ★★ `MusicMarkEngraver.LabelBoxPadding` は*引っ越し*であって新しい数ではない**（§7.6 ⒟）。**枠の 0.2 は 3 か所に綴られていた**——renderer の `pad`・`MarkXExtent`・`GetMarkHalfExtent` のコメント。**家は 1 つになり、`LabelBaselineBelowCentre` は half extent から*導出*されるので、フォントサイズも 2 度書かれない。**

**⑧ ★★★ 掃き 572 冊で動いたのは 5 冊、全部 PNG で目視した**（骨 6＝**掃きは「動いた」としか言わない**）。**4 冊は lead-sheet の fixture で本便で再ベース**（`test__lead-sheet.svg`・`test__lead-sheet-chords.svg`・`test__lead-sheet-repeat.svg`・`test__rows-song-sheet.svg`）。**5 冊目 `samples/drunken-sailor.lys` は snapshot を持たないので目視だけ**——**`Verse` がコード線に乗り、`Dm` がそれを避け、*小節番号は動いていない***（**`barnumber.rows-only.row-to-ink-bottom` の `why` に記録された 2026-08-24 のユーザー決定は無傷**＝テキスト行は今もインクの profile を持たないので段階 2 はそこに何も足さない）。⚠️ **`test/lead-sheet-lyrics` は動かない——それが対照**（**歌詞だけの五線無し本は帯を保つ**）。

⇒ ★★★ **次の一手**: **下の「見つけて直していないもの」から選ぶ。本便の島は閉じている**（ユーザー要求・LP 実測・器・配置と予約・毒 4 本・掃き・目視）。⚠️ **ただし §7 の宿題が 1 つ残っている**——**⑨ の CHANGELOG。**

**⑨ ★★ 本便は利用者から見える絵を変えたのに CHANGELOG に触れていない**（**第239 から続く未更新の項に、本便ぶんが 1 行増えた**）。**書くべきは Changed**: **五線を持たないリードシート（`score` が chords 行と歌詞行だけ）のセクションマークが、コード名と同じ線に乗るようになった**——**これは LP と違う、意図した乖離**。⚠️ **第243 ぶんの「`staff / chords / lyrics` が初めて正しく出るようになった」（Fixed）も未記入のまま。**

⚠️ **本便で新しく判ったこと（起票のみ）**:
- **★★★ 骨 9 ⒞ は*継ぎ目側だけ*閉じた。** `AboveTrackers` / `StackBelowStaff` は行の refpoint を受け取るようになったが、**テキスト行に*インクの profile* を与える島は開いたまま**（`SkylineBuilder.AddStaffToSkylines` はテキスト行で早期 return する）。⚠️ **この 2 つは別の仕事**で、**後者は第241 から名指されている**うえ **`barnumber.rows-only.row-to-ink-bottom` の 2026-08-24 のユーザー決定と当たる**（第243 §1 の警告がそのまま生きている）。
- **★★ 乖離の在処は 1 か所**: **⒜ の腕を落とせばマークは `base + Padding` に戻り、予約もいっしょに落ちる。** **元に戻すのが 1 手で済むように書いてある**（§7.6 ⒞ の「いつ消えるか」）。

⚠️ **見つけて直していないもの**（第243 から持ち越し・**本便が消したのは 0.44 の項 1 つだけ**）:
- **★★ 本便で再現できなかった報告＝1 小節に和音を 4 つ書くと和音名が横に重なる（ユーザー実測）。**第243 が **9 通り試して全部隙間は正**（最も狭くて 0.15）。⚠️ **`ChordNameEngraver.ClearOfPrevious` に明示的な抜け道が 1 つある**——`if (!cur.UseTiming || !prev.UseTiming) return curX;`＝**`@chord(…)` の注釈が絡むと押し出しが効かない**。**実物を待つこと。当てずっぽうで直さない。** ⚠️ **本便は `ClearOfPrevious` の隣に第 2 の押し出し（⒝）を足した**ので、**実物が来たら両方を見ること。**
- **★★★ `UnmodelledRow` は*スコア全体*を降ろす**（第243 起票・未着手）。`BuildLooseChainEnds` は全 system を回る loop の中で `if (alignment.UnmodelledRow) return null;` と書いており、**`return` はメソッドを抜ける**——**1 つの system が旗を立てただけで全 system の loose chain が降りる**（`LayoutEngine.cs:2990` 付近）。**旗を退役させるのが本来の島。**
- **★★ `staff / chords / lyrics` に Lily# は無言。LP は警告する**（`staff-affinities は減少するだけです`）。**診断を出すかは言語の決定＝要ユーザー判断。** ⚠️ **「score は帯の縦並び」はユーザー決定（2026-08-19・§3）なので、LP に倣って*拒む*のは筋が悪い。出すとしても注意まで。**
- **★★ `SkylineBuilder.OuterStaff` の staff 半分は今も残る**（「outer element が行の multi-staff system は、自分の staff を silhouette に入れない」・**番人は LYRHKG の 4 点**）。**第243 の帯はその肩代わりを*一部*戻しただけ。**
- **★★ `make-colon-bar-line` の `dist` 探索が未移植**（第240 ⑤）。**staff symbol を持たない帯は LP では ±0.45 ss で、Lily# は ±0.5**。⚠️ **renderer が `line-positions` を持つ必要がある**。⚠️ **Lily# のリードシート行は既に意図的乖離**（§3 第226）なので**「LP に合わせる」の意味を先に決めること**。**器を作る島。**
- **★ `RepeatDotRadius = 0.2` は LP の 0.225 に対して小さい**（`LILYSHARP-OWN` で起票済み）。**LP は円ではなく `dots.dot` グリフを描く**。⚠️ **閉じると `RepeatDotsOffset` 経由で横の予約も動く。**
- **★★ 旋法の語彙は 4 綴りあり、`LanguageVocabulary` に載っていない**（第242 で数えた）: parser の 9 つの `Check(SyntaxKind.*Keyword)`／`KeySpelling.SharpsFor` の switch／**LSP の `KeyModes` 配列（手書きの写し）**／`GRAMMAR.md` ほかの一覧。
- **★ `RenderSpecParser` と `PartReferenceFinder` は今も `as` を自分で切って*パート*を探す**（*様式*の読みは第242 で 1 軒にした）。**畳むには `TabPartToken` が doc に書いている「語の無い裸の `as`」の扱いを再現する `PartToken` が要る。**
  > ⚠️⚠️ ★★★ **上の 2 件は「独立した便」にしないこと**（§5.1）。**旋法の語彙に触る便／タブのパートを読む便が来たとき、その便に括り付ける。** **餌ではなく在庫。**
- **★ 診断パスは 1000 小節の本で 148〜218 ms**（`SemanticValidation.Run` の床・44 validator）。⚠️ **打鍵ごとではない**——`ScheduleDiagnostics` は debounce ＋ cancel。**上位 10 は全部 `DescendantNodes()` の全木走査**（`LyricPlainVerseShadowedValidator` 39 ms を筆頭に 30 ms 級が 9 本）。
- **音楽を持つが行のセルを持たない section は、rows-only グリッドで今も 0 小節**（`RowGridSectionBars` に音楽の fallback が無い）。
- **`audit/magic_constants.csv` が `RepeatDotPosition1/2` を行 119/121 として載せたまま**。⚠️ **生成物**（`audit/scripts/Find-MagicConstants.ps1`）で、**行番号は第240 の前から約 700 行ずれていた**——**再生成は独立の diff**。
- **`@chord(…)` と chords ブロックが違う和音語彙を読む**（注釈は絶対名のみ・第240 ⑩）。
- **第239 が残した 5 件**: **file レベルの stray-item メッセージが 5 つの合法な宣言を名指していない**（`lyrics`/`chords`/`fonts`/`paper`/`drummap`）／**`MidiPartRenderSyntax` の doc が stale**（`partName channel:1 instrument:25`——`:` オプションは 0.4.0 で消えた）／**`form main` / `score main` 自身の名前が無色**／**`extension.ts` の `NEW_SCORE_TEMPLATE` と LSP のテンプレートが同じ曲の 2 つ目の綴り**／**CHANGELOG 未更新**（⑨ で 2 行ぶん増えた）。⚠️ **0.4.0 は*タグ済み*で第242 は言語を 3 つ狭めている**（未知の主音・未知のタブ様式・未知の調弦）——**次のリリースの Breaking changes に要る**。

★ **開始時裏取り**: HEAD `3a65fe2a`・未 push 25・未追跡 0/木 0・Windows **6084/0/4**・台帳 580／総和 7.643880014／count 113・非ゼロ 2・追跡コーパス 572・snapshot 222・Core 0 警告・CI は直近 5 本すべて success（**最後の push は `7930f50b`**）。
終了時: **本便は 4 commit**（`2f94800a` 継ぎ目／`f1200c21` probe／`f6f83d3a` 配置と予約／この行）・**未 push 29**・未追跡 0/木 0・**Windows 6088/0/4**・Core 0 警告（`--no-incremental`）・**台帳 580／総和 7.643880014／count 113・非ゼロ 2／snapshot 222（4 枚再ベース・追加 0）／追跡コーパス 572**。⚠️ **§7.5 の数え上げ**: `LilySharp.Core` の `+` 行 **337**／**`LILYPOND-REF` 2**（どちらも `align-interface.cc:201-285 internal_get_minimum_translations`＝③ の継ぎ目）／**`LILYSHARP-OWN` 2**（⒜ の Y と ⒝ の X）／**新しい数値定数 0**（`LabelBoxPadding` は 3 綴りの*統合*・⑦）。**分類は §7.6**: ③ の継ぎ目は **⒝**（LP の refpoint 概念から導出・Lily# の帯モデルに直したので字面ではない。**字面にするには VerticalAxisGroup 相当の器が要る**）、⑷⒜⒝ は **⒞**（**LP はこの配置を作らない。0.460000 を測ったうえで*従わない*と決めた**——**「いつ消えるか」＝⒜ の腕を落とせば `base + Padding` に戻る**）、⑦ は **⒟**（引っ越し）。⚠️ **§7 項目 6 の注記**: **再ベースした 4 枚を正当化しているのは*台帳キーではなくユーザー決定***（2026-08-24）で、**その LP 実測は先行 commit `f1200c21` に置いてある**——**「台帳キーを名指せなかった」のではなく「名指すべき点を*作らないと決めた*」**（⑤）。⚠️ **再現物**は `scratch/ベースタブLy/Untitled-6.lys`（ユーザーの本・`.gitignore` 配下）。**追跡されている等価物**: `LilySharp.Tests/RowsOnlySectionLabelTests.cs`・`audit/lp-geometry/probes/mark-chord-row.ly`（**本便で MKV を追加**）・`test/lead-sheet*` の 4 fixture。

> ## ★★★ 骨 1＝**残差を見て「移植の島」と読む前に、ユーザーがその残差を*望んでいるか*を訊く**
> 第243 は 0.44 を正しく診断し（**定数では閉じない・モデル変更が要る・今日のユーザー決定と当たる**）、
> **正しく「不利」と判断して着手しなかった。** **その判断は本便でも正しかった。**
> **間違っていたのは前提のほうで、それは Lily# 側の前提ではなかった**——
> **ユーザーが欲しかったのは LP の 0.460000 ではなく、印刷譜の慣習（同じ線）だった。**
> ⇒ ★★★ **判定法**: **台帳の残差は「LP との差」であって「欠陥」ではない。**
> **絵に出る残差を島にする前に、*どちらの絵が欲しいか*を訊く。**
> ⚠️ **そして訊いた結果が「LP でないほう」なら、台帳点を開かない**（⑤）——
> **開いた点は次の便を誘い、閉じられない点は嘘になる。**
> ⚠️ **測るのはやめないこと。** **本便は従わない数を*3 冊 2 grob で*測ってから捨てた**：
> **「LP が何をするか」を知らずに乖離すると、それは決定ではなく無知になる。**

> ## ★★ 骨 2＝**器を作る commit は「何も動かない」ことが*結果*だと言えなければならない**
> `2f94800a` は 222 枚と 6084 本を緑のまま通す。**それは細工ではなく主張**で、
> **主張の中身は「テキスト行の profile は空・コーパスに tab/ossia のこの経路の本が無い」**——
> **つまり *なぜ* 動かないかを構造で言えている。**
> ⇒ ★★ **判定法**: **「無変更」を報告する commit は、*動かない理由*を書く。**
> **書けないなら、それは「測っていない」の顔をした「動くかもしれない」。**
> ⚠️ **そして観測者を*次の commit で*付ける**（§5.2.1）——
> **観測者の無い器は、次の便に「使われていない」と読まれて消される。**

> ## ★★★ 骨 3＝**§7 のチェックリストは順序を持っている。3.5 を §1 より先にやってはいけない**
> 本便は **step 3.5（第242 をアーカイブへ移す）を先に済ませ、§1 を書く前に落ちた。**
> 結果は **「ブロック 0」**——**`TheHandoffKeepsExactlyOnePredecessorBlock` は
> 「多すぎ」と「少なすぎ」を同じ 1 本で見張っている**ので赤にはなったが、
> **赤が指すのは「アーカイブし忘れ」ではなく「§1 を書き忘れ」だった。**
> ⇒ ★★ **判定法**: **§1 を書き換えてから、いちばん古いブロックを落とす。**
> **その順なら、途中で落ちても残るのは「ブロック 2」＝*何が起きたか読める*状態。**
> ⚠️ **網は正しく働いた**——**壊れた木を次のセッションが 30 秒で見つけた。**
> **直せなかったのは網ではなく手順の向き。**
