...
最終更新 第401セッション（2026-09-18・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `131ac87b`＝ユーザーの merge の上・未 push 25・**full `sessions/p401/run1.trx` 8652 / 0 / 3 / 8655＝第400 最終 8651 に origin から来た 1 本を足した数**・CI は直近 5 run が赤＝未 push のまま・build 0 エラー・`Session-Check -Build -Test` はこの PC で 38 秒）、指示「HANDOFF を読んで着手」→ §1「次の一手」を読み、⒜（section gather 3 ms）ではなく **⒝ 診断 pass を採った**（数が違う: fingbeam1k で打鍵 51 ms に対し pass 292 ms・plain1k 25 対 51＝RULES §5.6 の側）。道具は pwsh MCP（Bash 0 回）。**閉じたのは 1 本**:

★★★ **⑴ 「24 本 × 全木 walk＝大手術」は validator を 1 本も書き直さずに閉じた**（`0dbd862e`）。**先に計器**（`sessions/p401/Zz401Probe.cs.txt`＝validator ごとの min-of-5・warm tree・pass 全体は fresh tree）: base＝plain1k pass 50.9 ms（**28 本が 1 ms ずつ＝`tree.GetRoot().DescendantNodes()` の red walk 1 本ずつ**・2 ms の 4 本は 2 walk）・fingbeam1k 292（4.5〜5 ms × 28）・v2bow1k 42。**直し＝root の全 descendant を 1 回歩いて保持する `DescendantIndex`**（`Syntax/DescendantIndex.cs`・`CompilationUnitSyntax.Descendants` に Volatile／CompareExchange で 1 度だけ・pre-order の flat list ＋ kind ごとの bucket ＋ kind → red 型）: `DescendantNodes()` は root なら index の list、`DescendantNodes<T>()` は root なら **T が sealed で kind を 1 つだけ持つとき bucket（O(matches)）・それ以外は list の scan**（`GenericSyntaxNode`／`SyntaxTokenNode`／abstract 基底は scan・型が 2 kind を持てば scan＝常に正しい側へ倒す）。root 以外は従来の iterator（`WalkDescendants`）。**`KindSites`／`GreenSites` は index を通さない**（render 経路の打鍵 walk は red-free のまま＝index は red を全部作る）。validator 側は root の `DescendantNodes().OfType<T>()` 20 箇所を `DescendantNodes<T>()` に（`Update-MatchInFile -Pattern` の regex 1 発・`NavigationPlacement`／`GraceBody` の `KindSites` も）＋ついでに 2 本＝`DuplicateGlobalSetting` は型 switch を親鎖 walk の前に（6.4 → 2.2 ms）・`DuplicateCell` は section の part block を直接の子から（parser の事実＝`ProcessSectionBody` と同じ根拠・5.4 → 0）。**後（fresh tree の pass・min-of-5）**: **plain1k 50.9 → 11.3／fingbeam1k 292 → 107／v2bow1k 42 → 9.4 ms**。**index の費用**（`Zz401Probe3.cs.txt`）＝warm walk 1.0／4.0 に対し warm build 1.7／7.6 ms（plain1k／fingbeam1k）＝walk 1 本＋bucket・tree ごとに 1 回。fresh tree の最初の `DescendantNodes()` は 3.3／22.6 ms＝**red 実体化が大半で、これは以前も最初の validator の walk が払っていた**。**射程＝census**（`Zz401Probe2.cs.txt`＝`SemanticValidation.Run` の全診断を 760 冊（net 258＋Lab corpora）で dump・`sessions/p401/census-{base,head,head2}.txt`）**435 行が順序込みで完全一致**。網 `DescendantIndexTests` 4 本（root の list＝walk・同一 instance・1 度だけ建つ／**全 red 型（reflection で列挙・abstract 基底と 2 つの wrapper 含む）で `DescendantNodes<T>()`＝walk の `OfType<T>`**／「1 kind 1 型・sealed 型は 1 kind」が net 全冊で成立／root 以下は walk のまま）。**残り（fingbeam1k・warm）**: `SymbolReference` 15（234k 要素の型 switch 2 pass＋`PartReferenceFinder` の scan）／`AnnotationName` 14（3000 個の `@finger` の実仕事）／`MeasureValidator` 10／`LyricSings` 6.5／`Duration`・`OverrideVocabulary` 3＝**全部「全木 scan に型 switch」の形で、walk ではない**。⚠️ 罠 1 つ＝Claude Code の Edit ツールは*このセッションで読んでいない*ファイルを拒む＝20 validator の同型置換は `Update-MatchInFile -Pattern` の regex 1 発が速い（CRLF も保つ）。

★★★ **⑵ 第 2 便（同日・指示「このセッションが有利なら着手」→ 診断 pass と index の文脈が載っているので着手）＝`SymbolReference` を kind bucket に載せ替えた**（`29a59634`）。第399 第 3 便が「`PartReferenceFinder` の 2 綴り目を作る」で見送った件。index が入って残りは walk ではなく scan（234k 要素 × 型 switch × 4 pass＝fingbeam1k 15 ms）になったので、**bucket を pre-order に merge して返す `DescendantIndex.OfKinds`**（bucket を node ではなく pre-order の ordinal で持ち k-way merge・同じ kind の重複は 1 回・`SyntaxNode.DescendantNodesOfKinds(params SyntaxKind[])`＝root は index・root 以下は walk＋kind 判定）を足し、validator の 2 pass と `ReferenceTokens`／`Tracks` がそれを読む（診断の順序＝pre-order は保たれる）。**2 綴り目は承知で置いた**: 述語（`SectionSymbols.DeclaredName`／`ReferencedName`・`PartReferenceFinder.DeclaredName`／`CollectReferenceTokens`／`DeclaredTrackName`／`ReferencedTrackName`）は LSP の semantic tokens が node を手に訊く形のまま残し、その隣に kind 表（`DeclaringKinds`／`ReferencingKinds`／`ReferenceKinds`／`TrackKinds`）を置く。網＝`SymbolKindsTests` 2 本（RULES §7.7「畳めないときは差分の網」）: net 全冊＋全綴りの手書き 1 冊の全 node で「述語が non-null ⇒ kind が表にある」／手書きの 1 冊が表の全 kind を実際に書き・parse も validator も clean・4 族を毒すと 4 件を名指す。⚠️ 手書きの本で踏んだ罠 3 つ＝`p` は予約語（dynamic）・`score` は form 名が必須・**`tab low` の次の行の裸 `low` は「tuning low の part low」と読まれる**（MIDI 行は `tab` の直後に置かない）。⚠️ もう 1 つ＝**新規ファイルを CRLF に直すと raw string literal の中の改行も CRLF になり `"\n"` の Replace が空振りする**（改行を跨ぐ置換は regex `\s*` で）。**後（fresh tree の pass・min-of-5）**: fingbeam1k 107 → 91／v2bow1k 9.4 → 7.1・`SymbolReference` は 3 冊とも上位から消えた（fingbeam1k 15 → 1 未満）。plain1k は 11.3 → 16.9 と*逆に*出たが warm の合計は 8.5 で不動＝この機械の 5〜15 ms 級の揺れ（RULES §7 9）で主張しない。census 760 冊 435 行一致（`census-head3.txt`）。`DescendantIndexTests` に OfKinds の網 1 本。

★ **⑶ 終了時**: commit 2 本＋docs 2 本・**最終 full `sessions/p401/run6.trx` 8659 / 0 / 3 / 8662**（+7＝⑴ の網 4 本・⑵ の網 3 本・run3 は第 1 便の docs 後 8656・run5 は第 2 便の docs 前の同数）・§7.5（対 `131ac87b`・スクリプトの出力）**Core `+` 376 行／REF 0／OWN 0**（数値定数なし・LP 由来の式なし＝監査対象は「同じ答えか」だけ＝⑴ の census と網）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・APPROXIMATIONS.md 再生成（行番号のみ）・CHANGELOG 0.8.0 Editor に 1 項。§7 3.5 は済（第399 を `-Archive 399`＝17 行 6,028 chars）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⇒ **次の一手**: ⒜ 打鍵の残り＝section の GreenSite gather（plain1k 3 ms・fingbeam1k 12）を採用時に lazy に（第400 ⑶ ⒜）／⒝ 診断 pass の残りは小さい＝`AnnotationName` 14（実仕事）／`MeasureValidator` 10／`LyricSings` 6.5（fingbeam1k・warm）と fresh tree の red 実体化 ~19 ms（validator が red を読む限り要る）／⒞ R13⒝〜⒤／LP 双子が要る R7〜R11／R15（要承認）。
