...
★★★ ⚠️ **⑺ の診断を同じセッションで自分で倒した**（⑧）——**tab の食い違いは符尾の向きではなく、
双子が別のフレットを弾いていること**。**要承認だった作業の前提が崩れたので保留にした。**
**未 push 39**・テスト **3770 passed / 0 failed / 3 skipped**（**+17**）・
...
⇒ **次にやること: exporter が tab の表示→実音オクターブを双子へ渡す**
（`\transpose` か、`stringTunings` を 1 オクターブ上げる）。**描画は動かない＝承認不要。**
⚠️ **それを直すまで tab の符尾・梁の向きは比較できない**（⑦ の起票は保留）。
...
★★★ **⑧ を先に直す＝exporter が tab の表示→実音オクターブを双子へ渡す**（**承認不要・描画は動かない**）。
**これを直すまで tab の梁も符尾も比較できない**——双子が別のフレットを弾いている。
★★ **そのあと ⑦ の tab 符尾方向**（**要承認＝描画が動く**・**ユーザー承認済みだが前提が崩れたので保留**）。
⚠️ **`SharedRenderer.Tab.cs` の「LilyPond と同じく上を向く」というコメントは、
単音では LP と一致する**（実測: 単独の 4 分音符は両者 UP）。**群の集計だけが未検証**で、
**それを測るには ⑧ が先。**
...
★ **`tab-below-range` の群の数が LP 7 / Lily# 6**（④'''）——**未調査**。⚠️ **⑧ の影響下にある数字**。
