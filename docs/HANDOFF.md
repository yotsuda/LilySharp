...
⑵ **測り直したら測定器のほうが壊れていた**ので直した（`69bedd9c`）。
★★★ **コーパス一致は 11 → 31 冊**（梁のある 44 冊中）で、**未説明の発散は 1 冊だけ**になった。
**どちらも Core の描画は 1 行も動かない**）。
**作業は `1793d9ea`（exporter）と `69bedd9c`（測定器）**（＋この handoff commit 1 本が HEAD）・
**未 push 9**・テスト **3737 passed / 0 failed / 3 skipped**・
...
**`drum-groove`**（`DrumNote not exported`）。⇒ ★★★ **未説明は `showcase/grammar-tour` ただ 1 冊**
（10 対 10・LP が負で Lily# が正）。
...
## ▶ 次の一手＝**`showcase/grammar-tour` 1 冊**

★★★ **コーパスで未説明の発散はこれだけになった**（10 対 10・**LP が負で Lily# が正**）。
**符号が逆＝向きの話**なので、**まず梁の向き（符尾方向）が両側で同じかを見る**
——`grammar-tour` は文法の見本市なので**多声・グレース・タブが混ざる**。
⚠️ **測定器はもう一度疑わなくてよい**（4 冊の既知解で校正済）が、
**「直したら出力が動いたか」は必ず確かめること**（§5.0）。
