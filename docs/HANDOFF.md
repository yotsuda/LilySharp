...
最終更新 第368セッション（2026-09-11）＝**入り方が違う**（ユーザーは HANDOFF を読ませたうえで「残債はあとどのくらい」「タブ譜 LP 忠実度は完了したか」を訊き＝総覧を返した・そのあと**第367 ⑺⒝① を決定「section label の em は LP と同じにして」**＋**⑺⒝③（`lysc ly` 双子の font pin）は「詳細を説明して」＝決定はまだ・下 ⑸**）。**製品は作業ツリー＝承認待ち**（**snapshot 234 枚が動く**・patch `scratch/p369/section-label-em.patch`＝Core 1＋台帳＋APPROXIMATIONS＋CHANGELOG）。**骨は 5**:

★★★ **⑴ 決定と形（Core `+` 20／REF 0／OWN 1＝宣言だけ・式は足していない）**: `MusicMarkEngraver.LabelFontSizeStep` は **Rehearsal も SectionLabel も 2**（em 2.2×magstep(2)＝2.771822）。「LP と同じ」の読みは**双子が描く grob**＝`LilyPondExporter` は `form` の section 名を `\mark \markup \box "A"`（RehearsalMark・font-size 2）で書くので、LP がその本で描く箱は 2.744098。LP 自身の `SectionLabel` grob（font-size 1.5・2.616256）は **LILYSHARP-OWN 宣言**（X が第324 F2 で Rehearsal の錨を取ったのと同じ理由・remark に明記）。`LabelBoxMargin` も同じ step を読むので箱の padding も 0.2×magstep(2)+0.1＝0.351984＝双子どおり。**第367 ⑴ の「最初の譜の上線 −0.14」はこれで閉じた**（下 ⑵ の台帳点）。⚠️ 踏んだ罠: `Debug.Assert` は `using System.Diagnostics` が無い＝完全修飾で書く（build で CS0103・run1 を 1 回無駄にした）。

★★★ **⑵ 台帳 7 点が動いた（821 不動・exact 645 不動・ss 非ゼロ総和 25.684765 → 25.894357）**: **`page.section-label.first-staff-refpoint` −0.140235 → +0.002706**（RWMN＝双子の RehearsalMark に対する top spring・残りは箱の上端 対 stencil 上端＝bold 対 regular の face 差）／**`lyrics.chord-row.marked.gap-second`・`every-system.gap-second` +0.062689 → +0.207964＝`mark.rehearsal.lift.gap-second` と*同じ数*（9 桁）**——form 綴りと `@mark` 綴りの箱が一致したので、第324 が「UNDECOMPOSED」と書いた 2 箱の差 0.327835 は構築で 0。残るのは rehearsal lift 自身の draw-side 残差（既 OPEN）／**`mark.chord-row`・`mark.plain`・`mark.over-chord`・`mark.over-chord.tall` の `staff-to-baseline` 0.3378 → 0.351984（−0.00004 の face 項込み）＝`LabelBoxMargin(2)`＝「Lily# は箱・LP は素の字」の意図した差・OPEN のまま**。why は 7 本とも先頭に UPDATED 2026-09-11 (session 368) を書いた。`LpGeometryLedgerTests` 836 / 0。

★★★ **⑶ 射程（掃き 925 冊・base b3bf9364 Release＝`scratch\p369\wt-base`・`sweep369.ps1`・svg バイト比較・`compare-svg.txt`）: MOVED 460＝追跡 280＋ユーザー 180**（section 名を持つ本は全部＝箱の成長）。**snapshot 234／249**。**`lysc layout --all` の比較（`layout369.ps1`・`layout-compare.txt`・460 冊）: 段数・頁数が変わった本 0・頁割れが変わった本 4**（`Amanda` 13,5 → 12,6／`Beat It` 12,13,3 → 12,12,4／`Can't Fight This Feeling` 13,6 → 12,7／`Love Will Keep Us Together` 13,13,2 → 12,13,3＝いずれもユーザーの本・1 頁目が満杯で箱の +0.14 ぶん 1 段が次頁へ）。

★★★ **⑷ 数**: **開始時**（`Session-Check -Build -Scratch p369`・build／full は第367 末 run6 7697 / 0 / 4 / 7701 を引き継ぐ）: HEAD **`b3bf9364`**・**未 push 0**（ユーザーが第367 の 26 を push 済み・`origin/master`＝`b3bf9364`）・木 clean・未追跡 0・`gh run list` **緑**（34543531416）・build 0 エラー／Core 0 警告・台帳 821・snapshot 249・追跡 `.lys` 599・ディスク 23967。**変更後 full `scratch/p369/run1.trx` 7455 / 242 / 4 / 7701（RunInfos は SKIP 4 だけ・6 m）・赤 242＝snapshot 234＋台帳 7＋`ApproximationInventoryTests` 1**（行番号 1 行＋OWN 136 → 137＝`LILYSHARP_UPDATE_DOCS=1` で再生成）→ 台帳 7 本を書き直し → **終了時＝下 ⑻**。§7.5 Core `+` 20／REF 0／OWN 1（式 0＝定数の選び直しと宣言だけ）。台帳 821・snapshot 249（**234 枚赤＝承認待ち**）・追跡 `.lys` 599 不動。CHANGELOG 1 項（Engraving）。⚠️ `git worktree`: p362／p363／p364／p368×2 に加えて **`scratch\p369\wt-base`**（`git worktree remove` で外す）。⚠️ scratch は **`p369`**。⚠️ §7 3.5: 第366 のブロックを `-Archive 366` で ARCHIVE の先頭へ（済）。⚠️ 記憶: 更新 1（`reference_lilysharp_mark_box_height_is_nominal_not_glyph`＝em の決定を書いた・新規無し）。

★★ **⑸ ⑺⒝③「`lysc ly` 双子に `\paper { property-defaults.fonts.serif/sans }` を書くか」の説明（ユーザーに返した要点・決定は未）**: LP 2.26 は **backend が svg のときだけ** `fonts.serif/sans/typewriter` を generic 名（"serif"/"sans"）に落とす（`ly/paper-defaults-init.ly:169-181`）→ fontconfig の優先表でマシン依存の face（Noto／DejaVu／Verdana…）に解決され、文字の ext と spacing が正典（同梱 C059／Nimbus Sans）とずれる（実測: chord "Am" 4.336 対 3.926・第367 の title +0.21 も第350 の歌詞 +0.47 もこの偽残差）。pdf／png と `-dbackend=null` の `Measure-*` 計器は無縁。**書く場合の代償**: 双子 `.ly` に Lily# 由来でない `\paper` 行が入る＝追跡 599 冊の ly 出力が全部動く（svg snapshot は不動）・ユーザーが双子を pdf にする経路には無益・LP 側の face 名（"LilyPond Serif"）を Lily# が知る形になる。**書かない場合**: svg で測る便が毎回踏む（memory に「最初の 2 行は pin」と書いてある＝第350・第367 の 2 回）。**選択肢 3**: ⒜ exporter が常に書く／⒝ `lysc ly --pin-fonts`（既定は書かない・測るときだけ）／⒞ 書かず手で挿す（今）。**私の推奨は ⒝**（双子の意味を変えず、測る側の手順を 1 flag にする）。

⇒ ★★★ **⑺ 次の一手**: **⒜ 承認待ち＝ユーザー「承認する」→ `tools\Approve-Snapshots.ps1`（引数無し＝`*`・動いたのは意図した 234 枚だけ＝run1 の赤の内訳が証拠）→ full → commit（Core 1＋台帳＋APPROXIMATIONS＋CHANGELOG＋snapshot 234＋この HANDOFF＋ARCHIVE）→ `Deploy-Lsp.ps1`（server だけ）**。**push はユーザー**。**⒝ 拾い箱**: ③ font pin＝ユーザー決定（上 ⑸）／第367 §7.6⒝ の slash 箱 2 軒（設計級）／第365 ⑺⒝（chord quality 35 種）・第363 ⑺⒝④（`raggedLastBottom`）・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥ の並びはそのまま。⚠️ 承認前に commit・Deploy はしない（RULES §5.1）。

**⑻ 終了時**: **full `scratch/p369/run2.trx` 7463 / 234 / 4 / 7701（第367 末 run6 と合計一致・HANDOFF／CHANGELOG／APPROXIMATIONS／台帳込み・RunInfos は SKIP 4＋その赤だけ・3 m 52 s）・赤 234＝`SvgSnapshotTests` だけ（showcase 8＋test 226＝⑶ の 234 枚）＝承認待ち**。**commit 無し・Deploy 無し**（出力を変える変更は承認前に出荷しない＝RULES §5.1）。§7.5 の `-DiffBase` は commit 前なので 0 と出る（読み直しは手で: `+` 20 行＝remark 17＋Assert 3・REF 0・OWN 1）。patch は `scratch/p369/section-label-em.patch`（`git diff`・6 ファイル）。**第 2 便（ユーザー「⒝ で。承認する」）**: `Approve-Snapshots.ps1`（引数無し）で 234 枚を再ベース（動いたのは run2 の赤と同じ 234 枚だけ）→ **full `scratch/p369/run3.trx` 7697 / 0 / 4 / 7701（RunInfos は SKIP 4 だけ・2 m 58 s）**→ commit（Core 1＋台帳＋APPROXIMATIONS＋CHANGELOG＋snapshot 234＋この HANDOFF＋ARCHIVE）→ `Deploy-Lsp.ps1`。push はユーザー。**⑸ の font pin はユーザー決定 ⒝＝`lysc ly --pin-fonts`（既定は書かない）→ 同便の第 2 commit で実装**（下 ⑼）。commit **`c2a1f914`**・`Deploy-Lsp.ps1` 済（95 files 検証）。

★★ **⑼ 第 2 commit＝`lysc ly --pin-fonts`（ユーザー決定 ⒝・2026-09-11）**: `LilyPondExporter.PinFonts`（init・既定 false）が `\version` の直後・`\header` の前に `\paper { property-defaults.fonts.serif = "LilyPond Serif"  property-defaults.fonts.sans = "LilyPond Sans Serif" }` を書く（probe の pin と同じ 2 行・コメント 2 行付き）。CLI は `RunLy` が `--pin-fonts` を `RunFormOutput` の厳格 parser の前に剥ぐ（`--verbose`／`--batch` と同じ理由＝3 exporter 共有の形に ly だけの flag を足さない）→ `WriteLy(…, pinFonts)`。help・`docs/CLI_REFERENCE.md`（`CliReferenceSyncTests` 緑）・CHANGELOG（twin 節）。番人 `LilyPondExporterTests.Default_WritesNoPaperBlock`／`PinFonts_WritesTheProbesPaperBlockAfterVersion`。⚠️ 踏んだ罠: REF に行範囲を付けて `property-defaults.fonts.serif` を名指すと `CitationsThatNameNothing_DoNotGrow` が落ちる（ハイフン 2 節は LP 記号と見なされない＝3 節か `_` が要る）→ 既存の `:3964` と同じ「行範囲なし」の綴りに。**出力を変える変更 0**（flag 無しの双子はバイト不変＝番人 `Default_WritesNoPaperBlock`・snapshot は svg なので無縁）。Core `+` 30（remark 20・emit 8）／REF 1／OWN 0。**full `scratch/p369/run4.trx` 7699 / 0 / 4 / 7703（+2＝番人・RunInfos は SKIP 4 だけ・4 m 40 s・HANDOFF／CHANGELOG／CLI_REFERENCE／APPROXIMATIONS 込み）**→ commit（Core 1＋Cli 1＋Tests 1＋APPROXIMATIONS＋CLI_REFERENCE＋CHANGELOG＋この HANDOFF）→ `Deploy-Lsp.ps1`（Core が動いたので）。**push はユーザー（未 push 2）**。**第 3 便（ユーザー「有利なら着手」）＝T7 の指標を取り直した（測るだけ・製品 0）**: 第337 以降 30 便のあいだ T7 の表（KindMatched／SigMatch／PagesMatch）は取り直されておらず、この便は section 名を持つ全冊の絵を動かしたので、base exe（`b3bf9364`）と head exe が手元に在る今が最安。`scratch/p369/relayout369.ps1`（`relayout332.ps1` の写し・bin を引数に・LP 側 `scratch/p332/out/*/lp.out` は不変）→ `structure-before368.csv`／`structure-after368.csv` → `compare337.ps1`（`compare368.txt`）。**結果: em の変更で T7 は 1 対も動かない**（Lily# sig moved 0・SigMatch 得失 0・PagesMatch 不動）。**現在の T7（第337 の 209 / 149 / 41 / 19・PagesMatch 194 から、第338〜367 の移植と `.lys` 編集で動いた分）: KindMatched 214 / SigMatch 152 / 小節数一致・段割れ違い 41 / 小節数違い 21・PagesMatch 199 / SigMatch で頁数違い 5**（Boogie Oogie Oogie 4→3・Can't Take My Eyes Off You ×2 5→4・The Heat Is On 3→4＝第337 の 4 冊のまま・**＋`Sharon`（tab 単独・LP 3 頁 / Lily# 2 頁）が新顔**）／Lily# が多い 10・少ない 5。⚠️ 取り直しは 1 回 13〜16 分（`lysc layout --all` 286 冊）。**第 4 便は着手しない**: 残りは ⑺⒝ の並び＝ユーザー決定か設計級で、p369 の道具が効く未計数の宿題は無い。
