...
★★★ **⑵ `\`（tab 弦番号）の smart key**——音符の上で `\` → 核の直後に `\` を入れ**カーソルは `\` の直後へ**（ユーザー指定・数字を続けて打つ意図。他の印は stayPut）／既に `\N` があれば**N を選択状態にして挿入しない**（ユーザー指定・`\`＋数字で弦を変える）／`\` の直後に数字が無い音符で数字を打つとその数字は `\` の後ろ（rule 25・カーソル不動）。**休符と和音は素通し**（和音は `<c\3 e\2>` とメンバー側・`<c e>4\5\4` の和音レベルも parser は読む＝`TabStringNumberEntryTests`）。`package.json` に `"key": "\\"` を足した（**JIS の 2 つの `\` キーのどちらに束縛が解決するかは未確認**・予備経路は change-event）。
