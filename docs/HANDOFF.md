...
最終更新 第300セッション＝**§2 U8 の ⒜ から phrase 参照を閉じた＝`grace { G }` は容れ物であって grob ではなかった**。**入り方は第298・第299 と同じ**——**ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った**ので、**§1 の「次の一手＝残りの U8」をそのまま取った**。**掃きは 1 回・1742 冊・両側 Release・`svg --combined`＝MOVED 5（プローブ 4 ＋ `scratch/p194/four-containers.lys`）／`check` の差 7／レンダー失敗 2＝両 exe で同じ＝本便のものではない。** **台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188／追跡 `.lys` 581／snapshot 231 はすべて不動**（**snapshot は書き換えも追加も削除も 0 枚**）。

⚠️⚠️ ★★★ **⑴ ⒜ の 4 つは*症状*で 1 箱に入っていた**。**第298 は「本体がそれだけなら装飾音符ごと消える」で和音・休符・tuplet・phrase 参照をまとめ、第299 はその箱ごと「*ホストの*列を要る側」と書いた**——**phrase 参照は列を 1 つも要らない**。**grob を 1 つも名指していないから**: **和音・休符・tuplet は装飾音符の列がまだ持てない grob を要るが、phrase 参照は「よそに書いた音楽」の別名＝*容れ物***。⇒ ★★★ **修理を決める軸は「どの列を要るか」ではなく「grob か容れ物か」**——**症状で仕分けると、修理の違うものが同じ箱に入る**（第299 は「軸は正しいが値域を 1 つしか数えなかった」と書いた。**今度は軸そのものが症状だった**＝**同じ族の 1 段外側**）。⇒ ★★ **判定法**: **その項を閉じる編集が*どの綴りを増やすか*を 1 行書く。「増やさない」なら容れ物、「1 つ増える」なら grob で、後者は §5.2.1② の 2 綴り目になりうる。**

⚠️⚠️ ★★★ **⑵ 同じ文法の容れ物は他に 3 つあって、3 つとも phrase 参照を展開していた**（`tuplet { A }`・`cue { A }`・`repeat unfold 2 { A }`）——**そして 4 つを*並べて*確かめる本が repo に在った**: **`scratch/p194/four-containers.lys`**（第194 が書いた。4 行が 4 つの容れ物に同じ `phrase A` を入れる）。**grace だけが 106 便のあいだ落としていて、掃きで動いたプローブ以外の唯一の本がこれ。** ⇒ ★★★ **「N 個並べて確かめる本」は、1 つだけ赤いまま何年でも通る**——**`scratch/` は git 管理外なので誰の網でもなく、読まれるのは掃きのときだけ**。⇒ ★★ **そういう本を書いたら、同じ主張の網を 1 本 `LilySharp.Tests` に置く**（本便は `APhraseReferenceInAGraceBody_EngravesWhatThePhraseHolds` がそれ）。

⚠️⚠️ ★★★ **⑶ 展開は 1 綴りで、collector と validator が*両方それを読む***（`GraceBodySupport.BodyElements`・phrase 表は呼び手が渡す）。**collector だけが展開すると、validator は参照で止まったまま「1 段下の `<c e>` について黙り」、しかも「装飾音符 2 つを彫る本」を*装飾音符ごと消えた*と言い続ける**——**毒 `b_novalidate` がその 2 つを同時に赤くした。** ★ **報告の文面も変わった**: **span が phrase 宣言の中を指すので、その行に `grace` は 1 文字も無い**。⇒ **`a chord inside 'grace { C }'`＝*読者が書いた綴り*で本体を綴る**（毒 `g_novia` の観測者）。

⚠️⚠️ ★★★ **⑷ `EnterDefaultFrame` は呼ばない、そしてそれは「grace は何も変えない」ではない**。**あの helper は*声部の*音価記憶も消すが、grace 本体はそれを 1 度も読まない**（読むのは grace 自身の既定 8 分）。**消すと `grace { A }` が*装飾音符の後ろの音符*の音価を変える**＝**同じ音楽を inline で書いた `grace { d'16 }` には出せない副作用。** ⇒ **grace 自身の音価記憶のほうは境界で戻す**（`grace { c'16 G }` の G の無音価は 16 分ではなく 8 分＝main stream の `c'16 A` と同じ規則）。⇒ ★★ **規則は「境界が*その読み手が読むもの*を戻す」**で、「境界が全部戻す」でも「何も戻さない」でもない。

⚠️⚠️ ★★★ **⑸ 番人は 1 行も赤くならなかった＝phrase 参照の行が theory に最初から無かった**。**`EverythingReported_IsAbsentFromThePage` は「閉じた人を赤で迎える」ために在り、第299 の付点では予告どおり働いた**——**本便では緑のまま**。**理由はその theory の `Book()` が phrase を宣言できない形だから**で、**drop 一覧が `"a phrase reference"` と綴っている族が、番人には 1 行も無かった。** ⇒ ★★★ **「閉じたら赤くなる網」は*族ごとに*在るか数えること**（§5.4 の「毒 0 赤は網が無い報せ」の、**毒を打つ*前*版**）。⇒ ★ **数え方は 1 行**: **drop を綴る `Describe` の分岐名を、番人の InlineData と突き合わせる。**

⚠️⚠️ ★★★ **⑹ 毒 9 本のうち 2 本が 0 赤で、理由が*別*だった**。**⒡（声部の音価記憶も消す）は網が在るのに 0 赤**——**その網の*本*が `c'2 grace { … } c'4 c'4` で、後ろの音符が音価を書いていたので誰も声部の記憶を読まなかった**（**しかも消し先が Quarter なので、四分音符だらけの本では*当たっても*見えない**）。**本を `c'2 grace { … } c'` に直したら 3/4 で赤くなった。** **⒣（枠の釣り合い）は毒の狙い違い**——**`if (charge())` を `if (true)` にしても 2 つのマーカーは*一緒に*出るので不変条件は壊れない**。**壊すのは「2 つの add の*あいだ*で止まる」綴りで、そこに当てたら赤くなった。** ⇒ ★★★ **0 赤には 2 種類ある**: **⑴ 網が無い（⑸）／⑵ 網は在るが*その本*が量を読めない（⒡）／⑶ 毒が量の綴りを外した（⒣・§5.0 の第292 と同じ形の 2 例目）。⇒ 0 赤を見たら、まず網・本・毒のどれを直すのかを 1 語で言う。**

★★ **⑺ 同じテスト名を赤くする毒 2 本は、落ちた assert まで読んで別と確かめた**（§5.4）: **⒠（grace の音価記憶を戻さない）は `[16, 8]` が `[16, 16]`（L479）／⒡ は小節合計 1 が 3/4（L490）。** **1 本の網に独立な 2 主張が同居しているのは意図で、doc にそう書いてある。**

⚠️⚠️ ★★★ **⑻ 射程を数える計器が最初 0 を返した＝*偽の 0***。**ディスクの 1733 冊に `lysc check` を回して `LYS4020` を grep したら 0 冊**——**`check` は診断コードを印字しない**（出るのは `warning: a chord inside 'grace { }' …`）。**文言で数え直したら 22 冊 32 行**（**うち 20 冊はプローブ、`audit/lpreg` の 2 冊が `grace { c16~ }`＝⒞ のタイ**）。⇒ ★★ **§5.0 の「毒が生きていることを同じ run で示す」は*コーパス走査*にも当たる**——**0 を書く前に、当たると分かっている 1 冊で計器が鳴ることを見せる**（本便は自分のプローブで鳴らして気づいた）。

⚠️⚠️ ★★★ **⑼ U8 の「難所」を 3 度目に数え直したら、今度は*場所*が出た**（第298＝予測／第299＝「memo 4 軒ではない」／第300＝「ではどこか」）。**タイ・梁・スラーは `ArticulationEngraver:648` の `measure.Items[…]` からは 1 つも入らない**——**すでにレイアウト済みの `tieLayouts`・`beamLayouts`・`slurLayouts` として渡される**。**住所を要求しているのは*そのファイルの中の局所辞書 4 軒*で、鍵は 3 軒が `(staff, voice, measure, item)`**（`tiesAtBound :385`・`BuildBeamGroupMap :1947`・`BuildBeamedStemTips :2004`）／**`slursAtMeasure :408` だけは `(staff, voice, measure)`＝item を要らない**。**その `item` の出どころは `measure.Items` ではなく*スパンの模型***（`TieItem.Start/EndItemIndex`・`BeamGroup.Members[i].ItemIndex`・`SlurItem`）。⇒ ★★★ **⒞ の*梁*だけは住所を 1 つも要らない**——**grace run の梁は `BeamLayout` を通らず、`GraceNoteEngraver.IsBeamedRun` が音価だけで決める**ので、**本体の中の `[ ]` は「住所」ではなく `GraceNoteEngraver` の*部分グループ*の問題。**

★ **⑽ perf は測った**（§7.9）。**⒜ 診断経路**（`SemanticValidation.Run`＝LSP の打鍵ごと・in-process・20 回×5 バッチの床を 3 run）: **perf-grace200 7.07〜7.75 / 7.47〜7.78 ms・perf-slurgrace300 8.13〜8.36 / 7.77〜8.37・`Real Gone.lys` 6.08 / 6.10**——**すべて自分の run 間のばらつきの中**。**判定できるのは control が動いたから**: **`perf-plain1k`（grace を 1 つも書かない＝本便のコードが 1 度も走らない本）自身が 71.10〜73.84 ms＝2.2% 動いている。** ⚠️ **phrase 表は「grace を書く本」でだけ、しかも 1 度だけ建てる**——**`DescendantNodes()` ではなく緑 walk**（`GreenSites`）。**⒝ 描画経路**: **`alloc` 3 run の床（base ／ head）＝perf-plain1k 471.4 / 471.3・打鍵 26.3 / 26.3・perf-grace200 53.1 / 53.2・perf-slurgrace300 170.0 / 170.2・perf-dots1k 393.7 / 393.9**。**grace の本だけ +0.1〜0.2 MB＝body 1 つにつき `List<GraceBodyElement>` 1 個**（53 MB に対して 0.2%）。

⇒ ★★★ **次の一手＝残りの U8＝*grob を要る 3 つ*＝⒜ の和音・休符・tuplet ／ ⒞ 本体の中のスラーとタイ ／ ⒟ 注釈の全族**（§2 U8 の ▶）。⚠️⚠️ **和音・休符・tuplet を `GraceNoteInfo` の中に彫らないこと**——**それは和音レイアウト／休符レイアウトの 2 綴り目**（§5.2.1②）。**本便が phrase 参照だけを取ったのは、それが*どの綴りも増やさない*唯一の 1 つだったから。** ⚠️ **⒞ の梁は住所を要らない**（⑼）ので、**⒞ を「スラー・タイ」と「梁」に割ってよい**。⚠️⚠️ ★★★ **「一番安い次の 1 手は ⒞ の梁」と書きかけたが、測ったら外れだった**（⒀）——**梁は住所を要らないが、*模型*を要る**（**LP は 1 つの grace 群に「2 音を覆う梁」と「3 音目の旗」を同時に置く**のに、**Lily# の `BeamLeftY`/`BeamRightY` は単数で 3 軒に在り `IsBeamedRun` は all-or-nothing**）。★ **実コーパスの射程が在るのはタイのほう**（`audit/lpreg` の 2 冊が既に `grace { c16~ }` を書いている）。⇒ ★★ **残りの U8 は「どれが安いか」が*まだ 1 つも測られていない***——**⒜ の 3 つも ⒟ も、同じ形で見積りだけが書いてある。着手する便は、まず自分の取る 1 つを測ること。**

⚠️⚠️ ★★★ **⑾ チェックリスト 7.7 の自己監査が 1 件出した＝*私が phrase 展開の 2 つ目の綴りを書いた***。**`MeasureCollector.ExpandVariable`（main stream）と `GraceBodySupport.BodyElements`（grace 本体）**——**「同じ量の 2 つ目の綴り」は 7.7 が「この repo で最も繰り返し出た欠陥」と呼んでいるもの。** ⚠️ **今回は畳めない**: **前者は `_variables` と `ChargeExpansion` を読む instance method で validator からは触れず、しかも `MusicSitesLazy` で*容れ物の中まで平らにする***——**main stream には正しく、grace 本体には誤り**（**tuplet は 1 要素のまま残らないと narrowing が名指せない**）。⇒ **7.7 自身の答え（畳めないなら*差分の網*）を取った**: **`APhraseInAGraceBody_OffersTheSameBodyTheMainStreamDoes`＝同じ phrase を両方の展開器に通して音高を突き合わせる**（アンカーの側は ⒟ の毒が既に見ていた）。**期待値を 1 つも書き下ろさないので、音高の定義が変わっても生き残る。** ⚠️ **長さも assert する**（**空配列どうしは一致する**＝§5.4 が 2 便ぶん失った形）。**毒 `j_shortbody`（phrase 本体を先頭 1 要素に切る）で 3 本赤になるのを確かめた。**

★★ **⑿ LP 実測を後から足した＝本便が暗黙に寄りかかっていた文**。**LilyPond 2.26.0 で `G = { d'16 e' }` を書き、`\grace { \G }` と `\grace { d'16 e' }` は*バイト同一***（**8379 バイト・SHA `1EC0BE9A4B9E`**）**で、装飾音符を書かない本とは違う**（`scratch/p300/lp`）。⇒ ⚠️ **fresh frame のほうは LP 由来ではない**——**LP に phrase は無く、変数は自分の相対枠を持たない**ので、**あれはこの文法自身の規則を*他の呼び出し口と揃えるために*当てたもの**。**`LILYPOND-REF` と `LILYSHARP-OWN` の線がちょうどここに入る**（7.6 ⒝ 対 ⒞）。

⚠️⚠️⚠️ ★★★★ **⒀ §1 を閉じたあとに 5 本目が在る＝ユーザーが委任を出した**（「次便は、このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）。**答えは「残りの U8 には着手しない」**——**第297・第298・第299 と同じ答えで、理由も同じ**: **本便で温まったのは `GraceBodySupport` と phrase 展開の側で、残りの U8 は grob の側＝本便が 1 行も開いていない場所**／**計器は全部ディスクに残り §1・§2 から名指されている**（`scratch/p300/ab`・`lp/`・`poison.py`・`sweep300.ps1`・`perf.ps1`・`diagperf/`）＝**非対称ではない**／**次便のスロットは空いている**（**競合する ▶ は ⒤ と ⒣ で、どちらも本文自身が「着手はユーザー決定から」と書いている**）。⇒ ★★★ **代わりに取ったのが規則の名指す「派生ではなく*読まれた*数に直す安い測定」で、直す対象は*本便が 30 分前に書いた見積り*だった**——**§1 の「一番安い次の 1 手は ⒞ の梁」。**
⇒ ⚠️⚠️ ★★★★ **測ったら外れていた、そしてこれは §2 U8 で 4 例目の「測らずに書いた見積り」**（射程＝第293／難所＝第298／難所の場所＝第299／**安さ＝第300＝私**）。**しかも規則「起票の数値は着手前に 1 回測る」を §5 へ汎化したのと*同じ便*で書いた。** **LP 2.26.0 実測**: **`\grace { d'16[ e'] f'16 }` は梁 polygon 2 枚が x 0.0400→`1.5079`／`\grace { d'16 e' f'16 }` は同じ 2 枚が 0.0400→`2.9259`、`<path>` は 9 対 8＝*3 音目の旗が 1 つ増える***。⇒ **LP は 1 つの grace 群に「2 音を覆う梁」と「旗つき単独音」を同時に置く**。⚠️ **Lily# の模型はそれを綴れない**——**`BeamLeftY`/`BeamRightY` は*単数*で 3 軒**（`GraceNoteEngraver.cs:65-66`・`ElementCoordinator.cs:2756`・`SharedRenderer.GraceNotes.cs:185`）、**`IsBeamedRun`（`:303`）は run 全体の all-or-nothing**。★ **既定の側は既に LP と同じ形**（Lily# の対照も 2 枚・8.17→11.14＝run 全体を 1 本で覆う）で、**`[ ]` を書いた本と書かない本は `data-pos` 以外バイト同一＝印が完全に無視されている。** ⇒ ★★★ **「住所を要らない」は「安い」ではない**——**層は住所の仕事と同じだけ在る**（layout・coordinator・renderer・quanter・`IsBeamedRun`）。⇒ ★★ **残りの U8 は「どれが安いか」が*まだ 1 つも測られていない*。着手する便は、自分の取る 1 つを先に測ること。**

★ **開始時裏取り**: HEAD `e40902b2`・**未 push 20**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6643 / 0 / 4 / 6647**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2・`OPEN:` 0**・追跡 `.lys` **581 冊**・snapshot **231 枚**——**§1 の終了時の数と 1 つも違わなかった**（**10 個の数え方レシピは全部そのまま再現した**）。⚠️ **`origin/master` は `11f48202` のまま**＝**間で push は起きていない。**
終了時: **commit 5 本**（**本体 `76e60e9e` ／ 7.7 の差分の網と LP 実測 `bf205ec3` ／ §1・§2・RULES `ac68f2db` ／ 数を閉じる `28e249c7` ／ 委任への答えと ⒀ の測定 1 本**）＝**未 push 25**・木 0・未追跡 0。**suite 6654 合格 / 0 失敗 / 4 skip / 合計 6658**（開始 6647 から **+11**＝**本便の網 11 ケース**）。Core 0 エラー 0 警告。**追跡 `.lys` 581 冊・snapshot 231 枚**（**書き換え 0・追加 0・削除 0**）・**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2／`OPEN:` 0＝すべて不動。** ⚠️ **`docs/APPROXIMATIONS.md` も `audit/magic_constants.csv` も 1 行も動いていない**——**本便は定数を 1 つも足していない**（**移植ではなく容れ物の修理だから**。**`LILYPOND-REF` を 1 本足したのは実測の記録として**）。⚠️ **⒀ はコードを 1 行も変えていない**ので、**掃きも perf も取り直していない**（**測定と docs だけ**）。
★★ **WSL ubuntu Release も同じ 6654 / 0 / 4 / 6658**（**2 度回した＝`ac68f2db` の木と、⒀ を書き足した最後の木**。**§1・§2 を書き換えたら全数をもう一度回す**——第298 ⑽ の棚）。⚠️ **CI は `11f48202` までしか走っておらず、未 push 25 本は未実行。**
⚠️ **`C:\MyProj\lys-base300` の worktree はこの便の最後に消した**（`git worktree remove`）。**次便が同じ名前で建て直してよい。**
⚠️ **この 3 行は第299 ⑿ の順序（7.5 → 7.7 → §5 汎化 → §1 の数）どおり最後に書いた**——**7.7 が実際に 1 件（⑾）出して commit 1 本になった**ので、**数を docs の前に閉じていたら 2 度目の直しになっていた。** ★ **`未 push 24` はこの commit を含む**（**commit 3 本の時点で 23 だったのを確かめてから書いた**）。
...
- **U8. ▶ 4 分の 3 まで閉じた（第298 ＋ 第299 ＋ 第300 ＋ 第301）＝`grace { }` の本体は*歩かれない*。起票の「注釈を運ばない」より広かった**
  （**第293 起票・第298 が engine に訊き直して起票を訂正し可聴化と 2 つの carry を入れ、
  第299 が付点を、第300 が phrase 参照を移植した**）。
  ...
  *装飾音符は 1 つも描かれない***／**⒝ 付点（`d'8.`・`d'8..`）は無視される**／
  ...
  **`ProcessGraceNotes` が主旋律の `_defaultDuration` を共有していた**ため。
  ...
  ▶ **残っているのは grob を要る 3 つ＝⒜ の和音・休符・tuplet ／ ⒞ 本体の中のスラー・梁・タイ ／ ⒟ 注釈の全族。**
  ...
  **増えるのは括弧と数字を描くときだけで、それは drop に残せる。**
  ...
  ⚠️⚠️ ★★★ **第301 実測＝実コーパスの射程はタイだけ**。**ディスク 1744 冊のうち `grace` 語を
  書くのは 133 冊、LYS4020 は 59 行 / 43 冊**（**族別: annotation 20／beam 8／chord 7／slur 6／
  rest 5／tie 5／dynamic 4／phrase 参照 2／tuplet 2**）——**そのうち*追跡されている*本は 2 冊だけで、
  2 冊ともタイ**（`audit/lpreg/grace-tie-probe.lys`・`grace-slash-probe.lys`）。
  **残り 41 冊は第293・第298・第300 が書いた scratch のプローブ。**
  ...
  **MIDI だけ動き XML は動かなかった本**）。
  ...
  ⚠️ ★★ **同じ理由で和音・休符・tuplet を `GraceNoteInfo` の中に彫らないこと**——
  **それは和音レイアウト／休符レイアウトの 2 綴り目になる。第300 が phrase 参照だけを取ったのは、
  それが*どの綴りも増やさない*唯一の 1 つだったから。**
