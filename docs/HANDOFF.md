...
最終更新 第295セッション＝**§1 が名指していた §2 U10 に着手し、*ユーザーの決定を得て閉じた*（`ca0cbf2e`）**＝**タイとスラーが `data-pos` を 1 つも持たず、プレビューで指せなかった件**。**snapshot の弓 56 本のうち住所を持つのは 6 本（全部 装飾音符のスラー）だったのが 55 本になった**。**commit 2 本**。**インクは 1 点も動かない**（**1645 冊の全数掃きで 387 冊が MOVED、全部 `data-pos` だけ**）。**台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188・追跡 `.lys` 580 は全部不動**（**snapshot は 230 枚のまま・18 枚が書き換え**）。

⚠️⚠️ ★★★ **⑴ 起票が名指した「決定できない理由」のほうが古かった**（§5.0 の「引き継がれた処方箋は着手前に*診断*を 1 回測る」の**ブロッカー版**）。**U10 は「弓は 2 か所に書かれるが、エディタは 1 要素 1 `data-pos` を仮定しているので決められない」と書いていた**。**実測: その仮定は barline の便から存在しない**——**`IDrawingContext.Source(int, aliases)` が `data-alt` を吐き、`extension.ts:2184` は `[data-pos="p"], [data-alt~="p"]` の*両方*で引く**。⇒ **決定は「できるか」ではなく「どちらにするか」だけになり、ユーザーが選んだ**（**`(` が primary＝クリックの跳び先、`)` が alias／記号の上では*弓だけ*が光る**）。⇒ ★★ **起票が「これが決まらないと進めない」と書いた条件も、処方箋と同じく*着手前に 1 回測る*。1 便ぶんの往復がこれで消えた。**

★★ **⑵ 落ちていたのは*住所*で、機構ではない**。**`TieSyntax`／`SlurSyntax` は木に正しい span で立っている**（**第294 がそれを正しくした**）**のに、`MarkerFlags` が 5 つの bool に畳んで「どこに立っていたか」を捨てていた**。**直しは 3 つ**: **⒜ `MarkerFlags` が `FoldMarker` で marker ノードの `SourceStart` を持つ／⒝ `MusicItem` が 3 つの offset を持ち、`WithBowSources` の 1 軒だけが打つ（flat walk と tuplet body の全アーム）／⒞ `DrawTies`/`DrawSlurs` が `DrawBow` を `Source` スコープで包む**。⚠️ **何も書かれていない弓は `-1` を持つスコープではなく*スコープを開かない***。

⚠️⚠️ ★★★ **⑶ キャッシュされた弓は「それを*計算した*編集の住所」を持つ**——**そしてその罠は `SystemLayoutCache` の注記が既に名指していた**（第190 の骨）。**同キャッシュは `TieLayout`/`SlurLayout` を measure の*内容*キーで memo する**ので、**内容が同じで文字位置だけずれた編集は古い layout を配る**。**答えも同じ注記に書いてある＝`SharedRenderer.ResolveDataPos` が生きた score から引き直す**。**弓もそこへ入れた**（**glissando と同じ note locator**）。⇒ **入れないと 5 本の網が赤**（**`ChainedEditsOnABowedTwoVoiceScore_AlwaysMatchFull`＝第190 がこのために残した網そのもの ＋ fuzz 3 ＋ `SystemLayoutCacheTests`**）。⇒ ★★★ **新しい grob 族に `data-pos` を足す人は、`ResolveDataPos` に足すまで終わっていない。**

★★★ **⑷ 機械が先に鳴った、2 度とも**。**⒜ `CollectTailShifterTests.ShifterInventory_CoversEveryPositionField` は「`Position` で終わる int で shifter が re-home しないもの」を反射で名指す**ので、**本を 1 冊も描く前に 3 つとも赤くなった**（**注記の「extend BOTH together」がそのまま効いた**）。**⒝ `MeasureContentKey` の除外表にも 3 つを足した**——**offset を内容キーに入れると、上の行を 1 文字足しただけで memo が全部外れる。**

★★ **⑸ タブは `TieItem` を建て直しており、住所を落としていた**（`ElementCoordinator`）。**タブ*だけ*の本＝実コーパスの大半の形では、それは「本の全部のタイ」**。**毒でその 1 行を戻すと `test/tab-tie` と `test/tab-chord-tie` の 2 枚ちょうどが赤。**

⚠️ ★★★ **⑹ 「何本中何本が直ったか」を数えたら 3 番目の族が出た**。**弓型の `<path>` は 56 本で、55 本が住所を持つようになった。残る 1 本は `test/lv-meterchange` の laissez-vibrer**——**記号ではなく*注釈*（`@laissezVibrer`）が書く第 3 の族**。⚠️ **`TieVariantLayout` は既に `SourcePosition` を持っていて `DrawTieVariants` が読んでいないだけ**だが、**その値は*音符*の住所**なので、**包むと音符も一緒に光る＝今回ユーザーが選んだのと逆**。**だから包まず §2 U11 に起票した。** ⇒ ★★ **直した数だけ書くと「全部直した」と読める。分母を数えると、残りがそのまま次の項目になる。**

★★ **⑺ 毒 5 本・全部違う集合**（§5.4「どの case が赤くなるかで読む」）: **⒜ タイのスコープを外す → 16 赤（タイ 3 ケース ＋ タイのある snapshot 13・スラーと「弓の無い本」の対照は緑）／⒝ `data-alt` を外す → ちょうど 2（alias のケースだけ。alias は interactive 専用なので snapshot は 1 枚も見えない）／⒞ 記号ではなく*音符*の住所を打つ → 24（新ケース全部 ＋ snapshot 18）＝**この 1 本が「住所は記号のもの」を*測定*にしている**／⒟ `ResolveDataPos` の配線を外す → ちょうど 5（incremental の網だけ）／⒠ タブの建て直しの住所を外す → ちょうど 2。**

★★ **⑻ 掃きは 4 出力**（**この変更は collector を通るので SVG だけでは盲目**・`sweep4.ps1`・本ごとに base と head を続けて・両側 Release）: **1645 冊・svg SAME 1258 / MOVED 387・midi 0・xml 0・ly 0・`lysc check` の差 0・レンダー失敗 0**。★★★ **387 は*確かめた*、仮定しなかった**: **`data-pos` を伏せると 387 冊すべて base とバイト同一**（**追加された属性は 21,581 個**）。**内訳＝ユーザー実コーパス 244・audit 71・scratch 52・追跡 fixture 20。**
⚠️ ★★ **§5.2.1③（snapshot を再ベースする commit は差分を正当化する台帳キーを名指す）への答え**: **名指せる鍵は無く、無いことが正しい**——**書き換えた 18 枚の差は距離ではなく属性で、台帳の点はどれもこれを観測できない**。**代わりに置いたのは*マスクして突き合わせる*機械的な証明**（**18 枚も掃きの 387 冊も、`data-pos` を伏せると base とバイト同一**）で、**これは点より強い**——**点は 1 つの距離を見るが、マスクは*絵の全部*を見る。** ⇒ ★ **③ の趣旨は「再ベースは承認であって観測ではない」**なので、**観測を別の形で置いたなら、それを message と §1 に書くこと。**

★★ **⑼ perf は測った**（§7.9＝計算を足した。**`WithBowSources` は item ごとに record を複製しうる**）。**`audit/LilySharp.Probe alloc`・3 run の床・full render MB**（**base ／ 緑の事前判定なし ／ あり**）: **plain1k 471.0 / 472.8 / 471.3**（打鍵 26.1 / 26.3 / 26.2）・**fingbeam1k 961.5 / 962.8 / 961.7**・**slurscript1k 1253.3 / 1254.4 / 1254.0**（打鍵 51.5 / 52.2 / 52.2）。**真ん中の列が「毎 item 複製する」費用で、3 つの bool を先に読む事前判定がその大半を返す。** ★★ **その事前判定は*出力が同じ*ことを議論ではなく測定で示した**——**4 出力の掃きをやり直して 1645 冊の head ハッシュが全部一致**。⚠️ **この変更に対照本は存在しない**（全 music item を通る）ので「対照は動かなかった」は使えない主張（§5.3）。

⚠️ **本便で新しく開いたもの**:
- **★ laissez-vibrer／repeat tie の半弓が住所を持たない**（⑹・**§2 U11**）。**射程は測った＝ディスク 1645 冊のうち 12 冊が `@laissezVibrer`／`@repeatTie` を書き、ユーザーの実コーパスは 0 冊 0 箇所**・**インク不動**。**閉じる前に「注釈が書いた弓は*どの文字*を名乗るか」の決定が要る。**

⇒ ★★ **その射程を測ったので、§2 の ▶ は U8・U11 とも「読み手の目には 0 冊」に揃った**——**U10 を閉じた時点で、ユーザーが画面で困る既知の欠陥は §2 に 1 つも残っていない。** ⇒ **だから次の一手は「訊く」で正しい**（第293 と同じ形）。

⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 の ▶ は U8（装飾音符が注釈を 1 つも運ばない・射程はユーザーの本 0 冊・閉じる便は設計から）と U11（上・射程は追跡 1 冊）**。★ **どちらも射程は狭い**——**U10 を閉じた今、読み手の目に触れる既知の欠陥は §2 に残っていない**ので、**次は「ユーザーが画面で見ているもの」を訊くのが素直**（第293 がそうしたように）。

★ **開始時裏取り**: HEAD `58111f4d`・**未 push 25**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6589 / 0 / 4 / 6593**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2・`OPEN:` 0**・追跡 `.lys` **580 冊**・snapshot **230 枚**・**CI は最後の push（`91650d7d`）で success**——**§1 の終了時の数と 1 つも違わなかった**（**10 個の数え方レシピは全部そのまま再現した**）。
終了時: **commit 2 本**（`ca0cbf2e` 本体 ＋ **この 1 本**）＝**未 push 27**・木 0・未追跡 0。**suite 6596 合格 / 0 失敗 / 4 skip / 合計 6600**（開始 6593 から **+7**＝**本便の網 7 ケース**）。Core 0 エラー 0 警告。**追跡 `.lys` 580 冊・snapshot 230 枚**（**18 枚が書き換え・追加も削除も 0 枚**）・**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188＝すべて不動。**
★★ **WSL ubuntu Release も同じ 6596 / 0 / 4 / 6600**（`ca0cbf2e` の木）。⚠️ **CI だけは未実行**（未 push 27）。
...
- **U8. ▶ 装飾音符は注釈を 1 つも運ばない**（**第293 が LYS4019 を作る過程で実測・起票**）。
  ...
  **`CollectArticulations` を通らないので、これは記号 1 族の話ではなく*注釈の全族*。**
  ★ **射程はユーザーの実コーパス 0 冊**（**装飾音符に注釈を書く本が無い**）。
...
- **U11. ▶ laissez-vibrer と repeat tie の半弓だけが住所を持たない**（**第295 が U10 を閉じる過程で数えて起票**）。
  ★ **弓型の `<path>` は snapshot 56 本で、U10 のあと 55 本が住所を持つ**。**残る 1 本は
  `test/lv-meterchange` の l.v.**——**記号（`~`・`(`・`)`）ではなく*注釈*（`@laissezVibrer`／`@repeatTie`）が
  書く第 3 の族**で、**`TieVariantEngraver` の別経路で描かれる。**
  ⚠️ **`TieVariantLayout` は既に `SourcePosition` を持っていて、`DrawTieVariants` が読んでいないだけ**
  （**隣の drawer は全部 `using (gc.Source(x.SourcePosition))` している**）。**だから工事は小さい。**
  ⚠️⚠️ **小さいのに勝手に閉じてはいけない理由が 1 つ**: **その値は*音符*の住所**なので、
  **そのまま包むと「注釈の上にカーソル → 音符も光る」**＝**U10 でユーザーが選んだのと逆**になる。
  ⇒ **要決定は 1 問**: **注釈が書いた弓は、`@laissezVibrer` の位置を名乗るか、それが付いた音符を名乗るか。**
  ★ **射程は測った**（2026-08-30・ディスク上 1645 冊）: **`@laissezVibrer`／`@repeatTie` を書く本は 12 冊**
  （**audit 7・追跡 fixture 3＝`grammar-tour`・`feature-tour`・`lv-meterchange`・scratch のプローブ 2**）で、
  **ユーザーの実コーパスは 0 冊 0 箇所**。**インクは動かない。**
  ⇒ ★★ **つまり U8 と同じ「読み手の目には 0 冊」の項**で、**U10 のような緊急性は無い**——
  **閉じる価値は一貫性（弓は全部指せる、の「全部」）であって、読み手の不便ではない。**
