...
**0.006268 →（伸長の 2 点を開けて）0.875472 →（移植して）0.038509 →（圧縮の 2 点を開けて
移植の欠陥を直して）0.058509**。**開けた分の増加は後退ではない**（§5.2.1④）。非ゼロは
**20 点**＝従来の 16 点（clef sliver・Pango 量子化。⚠️ **この回は 1 つも動いていない**）＋
この回に開けた 4 点（伸長 2・圧縮 2）。
...
| **`{page,system}.stretched.*two-staff` 2 点** | −0.002480 / −0.029762 | 譜間ばね移植後の残り。**同じ 1 つの量**（force 差 Δf=0.000496）⇒ ▶ の符頭インク族 |
