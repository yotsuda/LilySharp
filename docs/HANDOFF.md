...
**未 push 127**（この引継ぎ commit まで）・テスト **3895 passed / 0 failed / 4 skipped**（**+10**）・
...
★★★ ⚠️ **【ユーザー判断待ち】どのセリフ体を同梱するか**（§1・**技術的な未解明はもう無い**）。
`text.width.{aa,va}` の残差 **−0.136573228 / −0.034143307 は 100% フォントファイルの差**。選択肢:
```
⑴ C059 を同梱      残差 0 になる。**AGPLv3**（URW の例外は PS/PDF 埋め込みのみ＝同梱は覆わない）
⑵ Schola のまま    今の状態。残差は「face のカーン差」として台帳に名前つきで残る（実害は
                   大文字ペアを含む文字列の幅が最大 0.13ss ずれる＝歌詞・マーク・和音記号）
⑶ C059 のカーン値だけ持つ  ⚠️ AGPL フォントからのデータ抽出＋当てはめ。**両方の欠点**を持つ
```
⚠️ **⑵ を選ぶなら台帳の 2 点はそのまま**（原因は完全に書いてある）。**⑴ を選ぶ場合だけ**
`TextFontMetrics.SerifFamily` と `Fonts/` の入れ替え＋ライセンス表記が要る。
