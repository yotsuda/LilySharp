...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第249セッション＝**第248 が入口に置いた「第5報」の台帳点を開いた。開けたら、引き継いだ診断そのものが間違っていた。** ⚠️ ★★★ **報告の犯人は和音行ではなく*段落標*で、和音行はそれ単独では 1 ミクロンも動かさない。** ⚠️ ★★★ **そして LP も同じ段間を広げていた**——**形は Lily# が正しく、量だけが違う**（LP +0.563793／Lily# +4.188166）。（1 commit＝`8f301912` 14 点＋この行）⚠️ **台帳 606→620／exact 502→513／総和 18.421820311→25.911639304／非ゼロ 131→134／count 117→122・非ゼロ 2 不動／snapshot 222 不動（再ベース 0）／追跡コーパス 572 不動。6139→6153。Core 変更ゼロ・絵は 1 冊も動いていない。**

**① ★★★ 名指された第一手（台帳点を開く）を実行し、対は*予測を反証した*。** `ROWA`＝**U6 の配置**（五線／歌詞／五線・和音行は 1・3 段目のみ）、`ROWAC`＝**同じ本の 2 段目にも和音行**。**LP は予測より強い恒等**——**段間 12.000000・内側 9.000000・鎖 4.027851・冒頭インク 6.691667・末尾インク 66.240551 が両書で 6 桁一致**。⇒ ★★ **ところが Lily# も 4 読みとも 12.000000。** **`ROWA` の `why` に書いた反証子「12.000000 なら本が報告を再現していない」がそのまま鳴った。**

**② ★★★ 外れの方向が真因を指した（§5.0）。1 変数ずつユーザーの本へ戻す掃きで項が出た。**
```
標なし・和音行 1,3 段    12.000000  12.000000
標なし・和音行 全段      12.000000  12.000000
標あり・和音行 1,3 段    12.000000  16.188166
標あり・和音行 全段      12.000000  16.188166   ← 同一
標あり・和音行なし       12.000000  12.241073
標あり・歌詞なし・行1,3  12.000000  16.188166
```
⇒ ★★★ **どちらか片方では起きない**——**標だけで 0.241073・行だけで 0・両方で 4.188166。** **歌詞は無関係。** ⇒ ★★★ **そして「無い段→有る段」という交互配置は変数ですらない**（全段に行がある本が同じ 16.188166 を出す）。**引き継いだ診断の前提が丸ごと偽だった。**

**③ ★★★ LP を同じ配置で測ったら、LP も広げていた。** `ROWM`／`ROWMN`／`ROWMA`（同じ 3 段に標を戻した本）を 2.26.0 で実測: **ROWM と ROWMA が 12.000000 / 12.563793・ROWMN が 12.000000 / 12.000000。** ⇒ ★★ **LP は標のある対で和音行に 0.563793 を払う。Lily# は 4.188166 払う。** ⇒ ★★★ **これは「LP が動かさない段間を Lily# が広げている」ではない。*量*の欠陥で、`ROWM` の `why` に書いた反証子がそのとおり鳴った側だった。**

**④ ★★ 両 engine とも gap 1 は 5 冊すべてで exact 12.000000。それが残りを説明する。** **段間は 2 つのシルエットの距離で、標も和音行も*下の段の左端*に立つ。1 段目は indent されていてその x に五線のインクが無いので押す相手がいない。** **LP も同じことをする。** ⇒ **だから 1 変数の掃きは「標」に届くまで何も動かなかった。**

**⑤ ★★ Lily# 側は 6 桁で分解できている**（計器 `GAPTRACE`・本便で入れて外した）:
```
s2s = (sysH − o2last) + down + upNext + o2firstNext
    =      2.000000   + 1.540000 + 5.701073 + 5.947093 = 15.188166  → gap 16.188166
```
**`upNext`＝標の上インク／`o2firstNext`＝和音行を載せた段の原点から最初の五線まで。** ⚠️ **安い否定を 1 つ閉じた**: **Lily# は*空の*和音行にも `TextRowHeight = 2.5` を予約している**（ROWM の 2 段目は和音が 1 つも無いのに `sysH` 15.500000）——**が、その 2.5 は `sysH` と `o2last` の両方に乗るので `s2s` で相殺し、値段は付いていない。** **ここを直しても段間は 1 ミクロンも動かない。**

**⑤ʼ ★★★ そして LP 側と項ごとに突き合わせたら*閉じなかった*。移植を始める前にここを和解させること。** `ROWM` の 3 段目について LP の dump（`PROBEV SYS 1 2`）を読むと、**原点から最初の五線まで 5.187144**（Lily# の `o2firstNext` は **5.947093**＝**+0.759949**）、**原点より上のインク 3.060184**（Lily# の `upNext` は **5.701073**＝**+2.640889**）。**和は +3.400838。ところが実測残差は +3.624373 で、0.223535 足りない。** ⇒ ★★★ **両 engine の「system の原点」が同じものを指していない**——LP の原点は `staff-refpoint-extent` の外に在り（和音行のある段で最初の五線の 5.187144 上、無い段では 5.555286 上）、**Lily# の原点とは別の量**。 ⚠️ **だから上の 2 つの差を「移植すべき 2 項」として読んではいけない**——**閉じない分解の上に移植を建てることになる**（§5.0「強い言明ほど先に対を検算する」）。 ⇒ ★★ **次便の第一手は移植ではなく*測定***: `PROBEV GROB` を `ROWM` に流して**標の箱と和音行の grob が LP のどの refpoint から何処に置かれるか**を出し、**原点の定義を 1 行で書けるようにする**。**書けたら 3 項が閉じ、そこで初めて移植の設計が決まる。**

**⑥ ★★★ 行き先は既に名前が付いている。** **Lily# は行に*公称帯*（`MultiStaffLayouter.TextRowHeight`＝`LILYSHARP-OWN` の 2.5）を課金し、LP は*インク*を課金する。** ⇒ **これは `TextRowVerseSpacing` の注記が既に書いている債務**（「The port is to take the band's height from the same walk」）＝**HANDOFF 3 の残り**で、**§1 の ⇒次の一手がずっと名指してきたものと同じ島**。**いま LP の数と、ユーザーに見えている症状が両方付いた。** ⚠️ ★★ **移植の着地点は 12.563793 であって 12.000000 ではない**——**行に何も課金しない実装は逆向きに行き過ぎる。その数が反証子。**

**⑦ ★★ 再現できなかったものを正直に書く。** **第5報の「12.000 / 16.970」は commit されずに消えた本で測られており、木の中のどの本も 16.970000 を出さない。** **標付きの本は和音名 1 文字で 16.188166・`Dmaj7` で 17.211417。** **16.970 はそのあいだに在るので同じ欠陥の第三の和音幅である*可能性が高い*が、それは推論**で、**信じてよいのは木に入っている数のほう。**

**⑧ ★★ `ROWA`/`ROWAC` は「何も測らない」まま残した。** **交互配置が変数でないことを*両側で*言う点だから**（LP も Lily# も平ら）。**§5.2.1 ⑷「exact は『その regime では動かない』の意味でありうる」——ここではそれ自体が結果。** **消すと散文しか残らず、次便が同じ誤った筋を再導出する。**

**⑨ ★★ 環境: Smart App Control が Release *も Debug も*塞いだ。** ⚠️ **`-p:Deterministic=false` ＋ `--no-incremental` でも振り切れなかった**（HANDOFF ⑩ の「Release は通る」はもう当てにできない）。**本便の測定は全部 WSL Ubuntu Release**（`git diff` → `git apply`）。⚠️ **塞がれた形は「合否行を 1 行も出さずに終了コード 0」**——**`Skipping: … 0x800711C7` を毎回 grep すること。緑に見える。**

⇒ ★★★ **次の一手は移植ではなく*測定*＝⑤ʼ**: **`PROBEV GROB` を `ROWM` に流して両 engine の「system の原点」を 1 行で書けるようにする。** **書けたら 3 項（+0.759949／+2.640889／欠けている 0.223535）が閉じ、そこで初めて移植の設計が決まる。** ⚠️ **本便はここで止めた**——**閉じない分解の上に移植を建てないため**（§5.0）。⇒ **そのあとの一手**: **`TextRowHeight` の公称帯を walk の答えに置き換える**（＝⑥）。**観測者はもう在る**: `lyrics.chord-row.marked.gap-second`（**12.563793 が着地点**）・`lyrics.chord-row.marked.every-system.gap-second`・`lyrics.chord-row.marked.no-row.gap-second`（**+0.241073 は別の・小さい第2の欠陥。行の分を閉じても残る**）。⚠️ ★★ **この移植は絵を動かす**——**和音行の上に段落標が乗る本＝リードシートの最も普通の形**なので、**572 冊と 222 snapshot がどれだけ動くかを先に数え、ユーザー承認を取ること**（§5.1）。**`ROWA`/`ROWAC` が平らなままであることも同時に確かめる**（標の無い regime を壊さない番人）。⚠️ **ユーザー判断が要る項は 5 つ**（テキスト行のインク profile／`staff / chords / lyrics` の診断／`make-colon-bar-line` の「LP に合わせる」の意味／**CHANGELOG＝版番号**／**Smart App Control を切るか**）。

⚠️ **見つけて直していないもの**:
- **★★★ 和音行の*部屋*を walk から取る＝上の ⇒。** **LP の数は `lyrics.chord-row.marked.*` に入った**（着地点 12.563793）。⚠️ **第248 が「和音行が段間を +4.970 広げる／LP は広げない」と書いた項は*本便で反証済み***——**LP も広げる／犯人は標との組**（①〜③）。**その旧い読みで着手しないこと。**
- **★★ 標*単独*の +0.241073**（`lyrics.chord-row.marked.no-row.gap-second`）。**行の分とは別の修理**で、**行を閉じても残る**。⚠️ **まとめて 1 つの欠陥として扱わないこと。**
...
- **★★ 第244 の乖離の在処＝`MusicMarkEngraver` の rows-only の腕**（⒜ を落とせば `base + Padding` に戻り予約も落ちる）。**LP は 0.460000 上に置く**（実測・`8d6b7978`）。**ユーザー決定なので閉じない。** ⚠️ **本便の ④ で標が段間を動かす経路が測れたので、この項と隣り合っている可能性がある。**
- **★★★ 骨 9 ⒞ の残り＝テキスト行に*インクの profile* を与える島**（`SkylineBuilder.AddStaffToSkylines` はテキスト行で早期 return）。**第241 から名指され、`barnumber.rows-only.row-to-ink-bottom` の 2026-08-24 のユーザー決定と当たる。** ⚠️ **上の ⇒ と同じ島の別の面**——**公称帯をやめるならインクの profile が要る。**
...
★ **開始時裏取り**: HEAD `66502a79`・**未 push 17**・未追跡 0/木 0・**Windows Release 6139 合格 / 0 失敗 / 4 skip**・台帳 606／exact 502／総和 18.421820311／非ゼロ 131／count 117・非ゼロ 2／snapshot 222／追跡コーパス 572・Core 0 警告。
終了時: **本便は 2 commit**（`8f301912` 14 点＋この行）・**未 push 19**・未追跡 0/木 0・**WSL Ubuntu Release 6153 合格 / 0 失敗 / 4 skip**（⚠️ **Windows では 1 度も走っていない＝ SAC**・⑨）・Core 0 警告（`--no-incremental`）・**台帳 620／exact 513／総和 25.911639304／非ゼロ 134／count 122・非ゼロ 2／snapshot 222（再ベース 0）／追跡コーパス 572**。⚠️ **§7.5 の数え上げ**: `LilySharp.Core` の `+` **0 行**——**本便はコードを 1 行も変えていない**（§5.0 ⑴ の「点を対で起票する・出力不変」の commit）。**`LILYPOND-REF` 0／`LILYSHARP-OWN` 0／新しい数値定数 0。** **分類は §7.6 ⒟**（何も足していない）。

> ## ★★★ 骨 1＝**引き継いだ診断は、引き継いだ*数*とは別に検算する。数が正しくても原因の名前が間違っていることがある**
> 第5報は「12.000 対 16.970」と**正しく測られ**、「和音行が段間を広げる／LP は広げない」と
> **診断されていた**。**数は本物で、診断は偽**——**LP も広げるし、和音行は単独では何もしない。**
> ⇒ ★★★ **判定法**: 引き継いだ診断が「LP は X をしない」と言っているなら、
> **その X を*その配置で* LP に測らせるまでは仮説**（§5.2.1 の `departs from` と同じ規則）。
> ⚠️ **本便が引き継いだ文は LP の測定を*引用していた***——ただし**別の配置の**（和音行が全段に在る `LYRMC`）。
> **「LP で測った」は「この配置で測った」ではない。**

> ## ★★★ 骨 2＝**対が予測を反証したとき、対を捨てずに*変数を掃く*。外れの方向が変数を教える**
> `ROWA`/`ROWAC` は 4 読みとも exact で「何も測らなかった」。
> **捨てて本を作り直す代わりに、ユーザーの本へ向けて 1 変数ずつ戻した**——
> 標／和音名の幅／小節数／第2詩／五線の本数。**3 手目で標が跳ねた。**
> ⇒ ★★ **原則**: **反証された対は「間違った本」ではなく「変数が 1 つ足りない本」**。
> **その本を*端点*にして掃くと、変数は数手で出る**（本便は 6 変種・15 分）。
> ⇒ ★ **そして掃きの表そのものが成果物**——②の 6 行が、
> 「どちらか片方では起きない」という**残差より強い言明**を持っている。

> ## ★★ 骨 3＝**「Lily# が広げすぎている」と言う前に、LP に同じ本で*同じ量*を訊く。形と量は別の欠陥**
> Lily# の 16.188166 は、LP が 12.000000 を出すなら「広げてはいけない段間を広げた」で、
> LP が 12.563793 を出すなら「広げ方が 3.6 大きい」。**修理も反証子もまるで違う。**
> ⇒ ★★★ **`ROWM` の `why` は着手前にその分岐を両方書き、どちらでも読めるようにした**——
> **「LilyPond が広げるなら Lily# の 16.188166 は*形*が正しく、残るのは*量*」。**
> ⇒ ★ **副産物**: **移植の着地点が 12.000000 ではなく 12.563793 だと*先に*分かった。**
> **0 を目指す実装は逆向きに行き過ぎる。**

> ## ★★ 骨 4＝**分解して見つけた「明らかにおかしい項」が、値段を持っているとは限らない**
> Lily# は**和音が 1 つも無い段にも `TextRowHeight = 2.5` を予約している**。
> 目を引くし、実際おかしい。**が、その 2.5 は `sysH` と `o2last` の両方に乗るので
> `s2s` で相殺し、段間は 1 ミクロンも動かない。**
> ⇒ ★★ **判定法**: 分解で見つけた項は、**それを消したときに*測っている量*がどう動くかを
> 式の上で先に計算する**。**動かないなら、それは別の便の別の項目**——
> **「おかしいから直す」で着手すると、緑のまま何も改善しない commit ができる。**

> ## ★ 骨 5＝**SAC は Release でも Debug でも塞ぐことがある。「-c Release なら通る」を手順に固定しない**
> HANDOFF ⑩ は「**`-c Release` は通る／WSL も通る**」と書いていた。**本便は両方塞がれ、
> `-p:Deterministic=false` ＋ `--no-incremental` でも新しい識別子が通らなかった。**
> ⇒ ★ **手順は「WSL に回す」を既定にし、Windows は*運が良ければ*にする。**
> ⚠️ **そして塞がれた形を毎回 grep する**——**`Skipping: … 0x800711C7` は
> 合否行を 1 行も出さずに終了コード 0 で返る**（§5.5）。

---

## 以下は第248セッションの経緯
最終更新 第248セッション＝**ユーザーが残債の返済を中断させ、画面に見えている欠陥を*4 つ*直させた。4 つとも別の欠陥で、上を直すたびに次が見えた。** ⚠️ ★★★ **4 つ目は 572 冊・222 snapshot・6136 本の網が全部緑のまま*エディタでだけ*壊れていた**（`SystemLayoutCache` を渡す経路＝プレビューだけ）。（8 commit＝`5fd5c770` staff ばね／`e216ec08` §1／`af693184` 行を鎖の要素に／`d2f8ae4d` anchor を system ごとに／`518fcea6` §1／`07c8af04`・`5471ea67` deploy／`1cb68ea4` verse メモの分割）⚠️ **台帳 606 不動／exact 498→502／総和 37.645494543→18.421820311／非ゼロ 135→131／count 117・非ゼロ 2 不動／snapshot 222 不動（再ベース 0）／追跡コーパス 572 不動。6128→6139。Windows も WSL Ubuntu Release も一致。**

**① ★★★ 第1便＝`MultiStaffLayouter.StaffSprings` が LP の `append_system` を*隣接*として書いていた。** **LP は `is_spaceable` な要素*どうし*にばねを張り、あいだのものは `loose_lines` へ積む**（`page-layout-problem.cc:660-672`）。**裸の `lyrics` 行は alignment の要素**なので `staff / lyrics / staff` は spring を 1 本も持たず、`OriginToChainEnd` が `ToFirst` に落ち、**system 間の床が 2 つの system の*最初の*五線のあいだに書かれた**。**1 段目どうしがぴったり 12.000000＝床は*効いていた*、相手を間違えて。** 描かれた 2 枚目と次 system の 1 枚目は **0.470000＝貫通**。

**② ★★★ 第2便＝`ClassifySystem` が「行の下に spaceable 五線が来たら `UnmodelledRow`」で run ごと捨てていた。** それは `distribute_loose_lines` の**もう一方の呼び出し**（`:936-939`）で、run は run。**`BuildBetweenRowStaves` で run を保持し、`BuildChainPrefix` が末尾の行と同じように歩く。** ⚠️ **行の所属は (system, 行) の事実**（hara-kiri が 1 ページの途中で run を移す。行だけで持ったら mid-system の verse step が 2.8→4.6 に戻り、実測して直した）。**6 点が閉じ、閉じ方が「束縛歌詞の双子と 7 桁一致」**（`staff-to-lyric` +1.472149→−0.000100321 に対し双子 −0.000100246／**verse step ×2 は exact**）。

**③ ★★★ 第3便＝`ResolveAnchor` が上側の族の anchor を `systemsArray[0]` から取っていた。** **`BuildStaffAnchorTables` の注記が射程まで書いていた**——「hara-kiri が…かつ非最終グループから…／fixture も台帳点も到達しない」。⚠️ **和音行は hara-kiri 無しで到達する**（1 つの system に在り別に無い行は、下の五線を帯ごと動かす）。**鎖は 0 / 5.175 / 7.975 / 12.033515 と解いていたのに、音節は*その全部より 1.895000 下*に描かれた**（＝和音行の帯）。最後の詩は次の五線の 0.160000 上→**2.060000**。

**④ ★★★ 第4便＝ここからが本便で一番高い教訓。「直った」と報告した直後にユーザーが「まだ直っていない」と言い、私の CLI 計測は全部正しかった。** **プレビューは `IncrementalCompiler` 経由で描かれ、`SystemLayoutCache` を渡すのはこの経路だけ**。渡すか渡さないかで歌詞の Y が変わっていた（`plain −12.265` 対 `cache −8.051`＝**4.214000 高い＝五線を貫通**）。⇒ **原因は verse skyline のメモを 2 つの annotation パスで*共有*していたこと。** `DistributeLooseLines` は profile を **alignment line**（`LineKeyOf`）で仕分けるが、それは `noteBoundAnchorY` で決まり、**予備パスはまだ持っていない**。**予備が `-1` で保存し、最終が staff index で引いて空を得て、音節のインクが 1 つも無い鎖を解いていた。** ⇒ **store をパスごとに分割**（Core は +2/−2 行）。⚠️ **隣の `AboveStackMemo`・`FingScriptMemo`・`LyricChainMemo` は全部パスごとに 1 つ**で、verse だけが例外だった。

**⑤ ★★★ ④ を切り分けた手順を残す（次に「エディタでだけ違う」が出たとき、これをそのまま使える）。** ⑴ **同じ本を `SvgGenerator.Generate` と `IncrementalCompiler.RenderIncremental` で描いて比べる**（初回レンダーで既に違った＝再利用ではなく*キャッシュを渡すこと自体*）。⑵ **`Layout(score, lineSizes, cache, springs)` の引数を 1 つずつ落として二分する**（springs は無罪・cache が犯人）。⑶ **`systemCache?.XxxMemo` を 1 つずつ `null` に毒して**どのメモかを決める。⑷ **本便*前*の commit で同じ測定をして「自分が壊したのか」を切る**（`git worktree add` で 2 分）。**⑷ は先にやること**——私は ④ の途中まで自分の回帰を疑っていた。

**⑥ ★★ 網がバグを仕様として固定していた。** `LyricBandMemoTests` は「最終パスは予備パスの entry を serve すべき」と assert しており、**④ の修正で赤くなる網**だった。⇒ ★★ **共有を pin する網は、共有が間違っていることを検出できない。** 検出したのは同じファイルの隣の行の `incremental == full` 比較。**書き直して、両 store の合計で「打鍵をまたぐ再利用」を assert する形にした**（win は共有ではなく再利用だったので性能の主張は変わらない）。

**⑦ ★★ incremental==full の網に*この形の本が 1 冊も無かった*。** **リードシートの歌詞は最後の五線にぶら下がる**ので両パスとも `-1` で一致し共有は無害。**非最後の五線にぶら下がる `sings` だけ**が食い違う。`IncrementalLyricLinePassTests` にその形と、無害な形を対照として追加（毒で `BetweenStaves` の 2 本だけ赤・対照は緑）。

**⑧ ★★ 毒の赤は 4 便とも交わらない／掃きは ⑴⑵ が 1 of 572（`lyhygrace`）・⑶ ⑷ が 0 of 572。** **⑶ の 0 は本物**（**網を書き・毒で赤を見せ・そのあと掃いた**＝§5.0 ⑸ の順）。⚠️ **⑦ の 0 は「コーパスが盲」の意味で、④ がそれを証明している**——**エディタ経路は 572 冊のどれも通らない。**

**⑨ ★★ 正直に書く損失が 1 つ。** **`lyhygrace` の*あいだ*の行は、帯の 5.500000（LP 5.368569 と偶然 0.131 差）から鎖の 5.150000（0.219 差）へ動いた＝LP から遠くなった。** 残るのは名前の付いた 2 量（部屋 9.490000 対 9.368569／閉じる最小 4.340000 対 4.000000）。**§5.2.1 ⑷「解かれない鎖が理想に座り、それがたまたま近い」の実例。**

**⑩ ★★ 環境: この機械の Smart App Control が Lily# のバイナリをブロックする。** ⚠️ **`dotnet test -v q` が合否行を 1 行も出さず終了コード 0 で返る**形で出る（`Skipping: … 0x800711C7`）。**`-c Release` は通る／WSL も通る**（`git diff > wip.patch` → `git apply`）。⚠️ ★★ **ビルドは決定論的なので、一度ブロックされた版は再ビルドでは永久に回避できない**（実測: 再ビルドで SHA256 一致）。`-p:Deterministic=false` で新しい識別子になる。**`tools/Deploy-Lsp.ps1` が自動でやる**（配置後に server を実際に起動し、ブロックなら 1 度だけ再 publish）。**切るかどうかはユーザー判断**（Windows 再インストールでしか戻せない）。詳細は `memory/smart-app-control-blocks-lilysharp-dlls.md`。

**⑪ ★★ `tools/Deploy-Lsp.ps1` を「確実に」した**（2 commit）。**旧版は publish した版を印字するだけ＝*試みた*コピーの報告**で、ロック・半端なツリー・**エディタがもう読まないフォルダへの配置**が同じ緑で終わっていた。⇒ **全ファイルのバイト比較／エディタの `extensions.json` に訊いて配置先を決める／staging＋ディレクトリ swap（失敗しても拡張を空にしない）／kill してから 800 ms 連続で rename（クラッシュ通知 5 回→0 回）／配置した server を実際に起動する／最後に古い server を落とす**。**毒で赤を確認済み**（swap を飛ばして 1 バイト変えると DIFFERS を名指して exit 1）。

⇒ ★★★ **次の一手**: **残債（HANDOFF 3）の残りは*部屋*だけ**。**行の*位置*は第248 が閉じたが、行に与えられる*部屋*はまだ `MultiStaffLayouter.GetStaffHeight` の公称帯で、walk の答えではない**——**2 詩を裸の行 2 本で綴ると 14.970000、`sings` で綴ると 13.170000**（**音節のベースラインは両者完全一致**）。⚠️ **`TextRowVerseSpacing = 3.2` の注記が入口を書いている**：「The port is to take the band's height from the same walk, which wants the pair first」——**その pair はもう在る**（`StaffSprings` が consecutive spaceable で張る）。⚠️ ★★ **射程は狭い＝ fixture は『五線／裸の行／五線』を 1 冊も綴らない（実測 0 冊）**ので **snapshot は動かず承認も要らない**。観測者は台帳の ROWB/ROWH/ROWV/ROWVH と `lyhygrace`。⚠️ **`…hara-kiri.system-gap` の +1.220090／+5.820090 は 1 ミクロンも動いていない**（実測）＝**system 間の部屋は別の島**。⚠️ **ユーザー判断が要る項は 5 つ**（テキスト行のインク profile／`staff / chords / lyrics` の診断／`make-colon-bar-line` の「LP に合わせる」の意味／**CHANGELOG＝版番号**／**Smart App Control を切るか**）。

⚠️ **見つけて直していないもの**:
- **★★★ 和音行が段間を +4.970 広げる（ユーザー報告 2026-08-25・第248 末尾で測った）。** **LP は和音行で段間を広げず、run に押し込む**——**実測（U6 と同じ配置の本を `PROBEV` で）: 和音行の*無い*段→無い段 も 無い段→*有る*段 も どちらも 12.000000**（LP は和音行を system の refpoint の **+0.999 上**に置く＝部屋の中）。**Lily# は 12.000 / 16.970。** ⚠️ **`BandUp` は無関係**（両 pair とも 0.0000）——驅動しているのは `CreatePages` の **X 依存 `Distance()`**で、**pair 1 で 26.1075**。`staffToStaff = dist + toStaffFrame = 26.1075 - 10.1335 = 15.974`、`max(12, 15.974 + 1) = 16.974`。⚠️ **1 番目の pair では同じ帯が*相殺*する**（実測: B にも和音を足すと `dist` 6.0949⇒19.0631 だが `toStaffFrame` も動いて gap は 12.000 のまま）。⇒ **問いは「帯が 2 度数えられる pair と相殺する pair の違いは何か」**。⚠️ **台帳点 `lyrics.chord-row.between-systems.system-gap` は exact（LP 12.000000）**——**その本 LYRMC は*全 system に*和音行がある**ので、**「無い段→有る段」は台帳に無い配置**。**最初にやるのはその点を開くこと**（LP の数は上にある）。
- **★★★ 行に与えられる*部屋*を walk から取る**（＝HANDOFF 3 の残り・上の ⇒）。**入口は `TextRowVerseSpacing` とその注記。**
- **★★ `StaffSprings` と `ComputeBetweenStavesEnd` は「あいだの行が全部 lyrics 行」でないと今も降りる**（第248・`LILYSHARP-OWN`）。**鎖は 3 つの歩幅を affinity UP の線から読み、和音行は逆向き**（`:1280-1332` の別の枝）。**閉じるには `staff / chords 行 / staff` の affinity DOWN の歩幅を LP で測る点が要る。その配置は追跡コーパス 572 冊に 1 冊も無い。**
- **★★ `lyhygrace` の 2 つの残差**（⑨）＝**部屋 +0.121431 と閉じる最小 +0.340000**。**LP の実測は取ってある**（`page-vertical.ly` の `PROBEV` を `-dinclude-settings` で流す）。**台帳点にするなら probe を `audit/lp-geometry/probes/` に足すところから。**
- **★★ 第244 の乖離の在処＝`MusicMarkEngraver` の rows-only の腕**（⒜ を落とせば `base + Padding` に戻り予約も落ちる）。**LP は 0.460000 上に置く**（実測・`8d6b7978`）。**ユーザー決定なので閉じない。**
...
★ **開始時裏取り**: HEAD `e6bf0c45`・**未 push 7**・未追跡 0/木 0・**Windows 6128 合格 / 0 失敗 / 4 skip**・台帳 606／exact 498／総和 37.645494543／非ゼロ 135／count 117・非ゼロ 2／snapshot 222／追跡コーパス 572・Core 0 警告。
終了時: **本便は 9 commit**（上の 8 ＋この行）・**未 push 16**・未追跡 0/木 0・**Windows 6139/0/4**（**Release で。Debug は SAC に塞がれている**＝⑩）・**WSL Ubuntu Release も一致**・Core 0 警告（`--no-incremental`）・**台帳 606／exact 502／総和 18.421820311／非ゼロ 131／count 117・非ゼロ 2／snapshot 222（再ベース 0）／追跡コーパス 572**。⚠️ **§7.5 の数え上げ**（4 便合計）: `LilySharp.Core` の `+` **345 行 / − 54 行**／**`LILYPOND-REF` 5**（`:660-672 append_system`・`:589 filter_dead_elements`・`:1173-1177 is_spaceable`・`:936-939 distribute_loose_lines`・`:896-914 find_system_offsets`）／**`LILYSHARP-OWN` 2**／**新しい数値定数 0**——**式は 1 つも足していない。足したのは「既にある walk にどの線を渡すか」と「どの store をどのパスが使うか」だけ。** **分類は §7.6 ⒜**（LP のループそのもの）**＋ ⒝ 1 件**（写す相手の無い decline）。④ は LP と無関係の**この port 自身のキャッシュ設計**なので分類の対象外。

> ## ★★★ 骨 1＝**コメントが正しくてコードが違うことがある。REF ラチェットは「出所が書いてあるか」しか見ない**
> `StaffSprings` の注記は **60 便のあいだずっと**
> 「A spring is made between consecutive SPACEABLE elements (:660-672 append_system)」
> と書き、**コードは隣接で対を作っていた**。**住所も記号名も正しく、
> `LpReferenceCitationTests` も `LpProvenanceTests` も緑。**
> ⇒ ★★★ **判定法**: **REF の文が「LP はこう*ループする*」と言っているなら、
> そのループを 1 行ずつコードと突き合わせる。**「consecutive」「between」「for each」は
> **制御フローの主張**で、**式の一致では確かめられない。**
> ⚠️ **本便は同じ形を 3 回踏んだ**——②（`ClassifySystem` の decline）・
> ③（anchor の射程）・④（「X-only だから共有してよい」）。**3 つとも
> *注記が自分の前提を述べていて、その前提が偽*だった。**

> ## ★★★ 骨 2＝**「no fixture reaches it」は fixture についての言明であって、幾何についての言明ではない**
> anchor の注記は射程を丁寧に書いていた——「hara-kiri が…かつ非最終グループから…だけ」。
> **和音行が hara-kiri 無しで到達した。**
> ⇒ ★★ **除外の射程を「今のコーパスが綴らない」で書いたら、それは*コーパスの*性質。**
> ⇒ ★ **書き方**: 「no fixture reaches it」ではなく
> **「到達するのは X という配置で、コーパスにその冊は無い」**と*配置*を名指す。
> **名指せないなら射程は分かっていない。**

> ## ★★★ 骨 3＝**緑の網が 6136 本あっても、ユーザーの経路をどれも通っていないことがある**
> ④ は **572 冊・222 snapshot・6136 本が全部緑のまま**、
> **`SystemLayoutCache` を渡す経路＝エディタのプレビューでだけ**壊れていた。
> **その経路を通る網は `IncrementalCompilerTests` だけで、そこにこの配置の本が無かった。**
> ⇒ ★★★ **判定法**: **「ユーザーは何を通って見ているか」を経路として書き出し、
> その経路を通る網が*その配置で*在るかを数える。** 出力が同じはずの 2 経路があるなら、
> **同じ本で `A == B` を assert する網を、配置ごとに置く。**
> ⚠️ **そして「直りました」と言う前に*ユーザーの経路で*見る**——
> 本便は 3 回「直った」と報告し、3 回ともユーザーの画面では直っていなかった。

> ## ★★ 骨 4＝**共有を pin する網は、共有が間違っていることを検出できない**
> `LyricBandMemoTests` は「最終パスは予備パスの entry を serve すべき」と assert し、
> **④ の修正で赤くなる網**だった。**検出したのは同じファイルの隣の
> `incremental == full` 比較のほう。**
> ⇒ ★★ **原則**: **最適化の*内部構造*（どの store が誰に serve するか）を assert する網は、
> その構造ごと間違っているときに味方をする。** **観測すべきは*出力の同一性*で、
> 内部の計数は「serve が起きたか」の補助にとどめる。**

> ## ★ 骨 5＝**「自分が壊したのか」は 2 分で切れる。先に切る**
> ④ の途中まで、自分の 3 便のどれかの回帰を疑っていた。
> **`git worktree add <dir> <本便前の commit>` に probe を 1 本置いて同じ量を測ると、
> 19.980 と 71.340 が 1 ミクロンも同じ**＝**本便前から壊れていた**と即断できた。
> ⇒ ★ **ユーザー報告に着手したら、原因を探す前にこれをやる。**
> **「自分の回帰かどうか」は探索の方向を丸ごと変える。**
