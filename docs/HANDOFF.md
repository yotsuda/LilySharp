...
★★★ **⑴ 計器で 3 つに割ったら、`AnnotationName` の費用は「述語の鎖」ではなく*建て*だった**（`3b12cbd6`・R13⒩ に全文・probe は `sessions/p408/` の 3 本で commit せず）。打鍵あたり fingbeam1k: **全木 234,030 節点を歩いて 24,000 の注釈に会い**（0.57 ms）・**dotted `MarkName` を 24,000 本建てて*報告する枝しか読まない***（2.0 ms／3.07 MB）・**`Arguments` を 24,000 回建て直す**（4.53 ms／15.74 MB＝1 個 656 B）。⇒ 3 本とも直して **18.82 MB → 6.92 MB／6.81 → 3.13 ms・節点 234,030 → 24,000・`MarkName` 24,000 → 0**。★★★ **per-node cache は counter で「当たらない」と決めた**（cold pass 72,000 回でも**実 keystroke の render は 48 回**＝validator の 24,000 は新しい木の初回 touch）＝**覚えるのではなく建てを軽くする**。⚠️ **validator に安い述語を書くのは禁じ手**（VALUE_SITE_AUDIT §9.3）＝唯一の綴りを軽くしたので**collector の 48,000 回も同じだけ軽い**。健全性＝**census 760 冊 435 行が順序込みで完全一致**＋**毒 4 腕**（33／175／20／9）＋差分の網 `AnnotationKindsTests` 2 本。汎化は RULES §5.3。

★★★ **⑵ 第 2 便（指示「このセッションが有利なら着手」→ 計器の型が手元に在るので有利と判断）＝尾部 11 本の「全木 scan」を*綴りを 1 つも増やさずに*軽くした**（`a743726e`・R13⒪ に全文）。★★★ **まず一覧を取り直したのが効いた**——§1 が名指していた「`MeasureValidator` 10／`LyricSings` 6.5」は**第401 が*前の機械*で測った数**で、この機械では順位が違った。⇒ **古い機械の一覧で validator を割らない。** 尾部の正体＝**11 本が `root.DescendantNodes()` で 234,030 節点を舐めて一握りに会う＝打鍵あたり約 2.6M 節点訪問**。kind bucket に畳むと **11 本の 2 綴り＋11 本の網**が要るので**やらず**、「その scan の何割が*列挙*か」を測った＝**IEnumerable 0.56 ms 対 同じ配列を直接 0.40 ms**。**root では既に index の*配列*を返していて `IEnumerable` に入れていた**＝要素ごとに interface 越しの `MoveNext`。⇒ **戻り値を構造体にした**＝`foreach` は構造体 enumerator に束縛、`IEnumerable` も実装するので**呼び出し側は 1 箇所も変えていない**。A/B（同一セッションで stash 往復）: **pass 44.4 → 39.9 ms・warm 合計 21.0 → 17.3 ms・scan あたり割当 72 → 40 B**。★★★ **対照が内蔵**＝第 1 便で kind bucket に移した `AnnotationName` だけ**動かない**（3.68 → 3.66）。健全性＝census 435 行一致・**毒 309 本**・⚠️ `DescendantIndexTests` の「built once」が**戻り値に `Assert.Same`** していて唯一の実害を捕まえた（性質は包んでいる index 側に在るので網をそちらへ）。⚠️ **測って「やらない」と決めた**＝`MeasureValidator` の最大項 `ValidateNode` 再帰 2.21 ms は **tuplet／grace／cue／repeat／parallel の本体を*意図的に刈る*ので flat scan に畳めない**（数は `sessions/p408/walk-shape-summary.txt`）。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: ⒜ **R13⒝ の実機確認**（第404 ⑵ のとおり＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item（第404 ⑵′・実機の 2 行を見てから）／⒝′ frame 変更の `applyFrame`（実機で頻度を見てから）／⒞ **診断 pass の残り**＝⚠️ **第401 の一覧（`MeasureValidator` 10／`LyricSings` 6.5）は前の機械の数で使えない**。この機械の現在値は `sessions/p408/validators-after.txt`（fingbeam1k pass 39.9 ms fresh／17.3 warm・`MeasureValidator` 4.48・`AnnotationName` 3.66・`LyricSings` 2.40・尾部 9 本が 0.5〜0.8）＝**残るのは ⒤ kind 表を 1 本ずつ足して尾部を bucket に移す**（1 本あたり ~0.5 ms・代償は 2 綴り＋網。**一番安いのは `LyricSings` の第 1 walk**＝`PartReferenceFinder` に第401 の*網付き*kind 表が既に在る）か **⒥ fresh tree の red 実体化＋index 建て（39.9 のうち ~22 ms）**＝第401 が「validator が red を読む限り要る」と書いた所・count loop の page DP 5.3 ms（R13⒦・census で hit 率）／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝fingbeam1k の `fs.walk` 0.41 ms／1.5 MB と `fs.assemble` 0.29 ms／4.0 MB（24,000 digit の実仕事＝「何を返すか」を変える別の島・**今便の残り 288 B もこの島**）／**⒞″ ✅ 第407 第 2 便が測って「やらない」と決めた＝残りは畳む価値が無い**／**⒟ ✅ 第407 で閉じた（R2R）**／LP 双子が要る R7〜R11／**R15 ✅ 削除は第407 で完了**（残り＝「2 綴り」一族のみ＝統合・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑶ 終了時**: commit 4 本（第 1 便 code `3b12cbd6`＋docs `64487311`・第 2 便 code `a743726e`＋docs）・**最終 full `sessions/p408/run5.trx` 8695 / 0 / 3 / 8698**（第407 最終 8693 / 0 / 3 / 8696 に対し**網 +2**＝`AnnotationKindsTests` 2 本。第 2 便は網 0 本＝既存の `DescendantIndexTests` を 1 行向け直しただけ）・§7.5（対 `e34b0a33`＝スクリプトの出力）**Core `+` 254 行／REF 0／OWN 0**（第 1 便 137＋第 2 便 117）（**数値定数なし・LP 由来の式なし**＝建てる物を減らしただけで計算は 1 つも動いていない＝監査対象は「同じ答えか」＝census 760 冊 435 行の順序込み完全一致＋毒 4 腕＋snapshot 249 不動）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。棚卸し `docs/APPROXIMATIONS.md` は**行番号 1 行だけ**（`:444` → `:474`・`LILYSHARP_UPDATE_DOCS=1` で再生成し、行番号を伏せて機械で一致を確認）。§7 3.5 は済（第406 を `-Archive 406`）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。
