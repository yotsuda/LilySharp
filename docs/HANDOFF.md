...
最終更新 第183セッション＝**「自分は X を測る」と名乗っている計器と fixture が、
X を 1 ピクセルも測っていなかったのを 3 件見つけた便**（**実装 4 便**）。
...
- **⑸ ⚠️ ★★★ この便が*直さずに起票*した 1 件**——**`ParseList` の裸 `Advance()`
...
- **⑺ ★ §7.5 の監査**＝**第1便 Core +99 行・REF 0 / OWN 0・数値定数 0**（**正しい**——
  ...
  ＝対応物が無い。第182 ⑽ と同じ理由）。**第2便は annotation のみ・第3便と第4便は Core を 1 行も触っていない。**
- **⑻ ★ perf（RULES §7 ⑼）＝計算を足していない**。第1便は**トークン種別の switch の腕を 5 本**
  （ツリーのどの本も書かない綴り）で、**新しい走査も確保も memo も無く、打鍵経路の `HasUsings` は不動**。
  第2〜4便は annotation と fixture のみ。⚠️ **第182 ⑽ が「便ごとに『計算を足したか』を問え」と
  書いたので、この便は毎回問うた。**
- **⑼ ★★★ §2F に残っているのは 4 つ＋1 新規**——**⑴ 一覧に構文を与えるか（決定であって作業ではない）
  ⑵ 繰り返し縦線の ⑹ ⑶ `lyrics` の voice 束縛（要決定）⑷ `lysc ly` が transpose を落とす件
  （exporter の島・それだけで 1 セッションの形）**、＋**⑸ 新規＝`ParseList`／`ParseMusicBlock` の
  裸 `Advance()`（上の ⑸）**。
  **「section／score の中の `using`」「`grandstaff-high-bass`」はこの便で閉じた。**
