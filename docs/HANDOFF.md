...
★ **⑷ 終了時**: commit 2 本（code `7bc22207`＋docs）・**最終 full `sessions/p406/run3.trx` 8695 / 0 / 3 / 8698**・§7.5（対 `40a92a2c`・スクリプトの出力）**Core `+` 151 行／REF 0／OWN 0**（数値定数なし・LP 由来の式なし＝memo の配管と live item への 1 コピー＝監査対象は「同じ答えか」＝incremental==full の網 3 本＋drift net）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609。§7 3.5 は済（第404 を `-Archive 404`＝13 行 6,206 chars）。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も・Lab は sessions/p406 を commit 済み）。⇒ **次の一手**: ⒜ **R13⒝ の実機確認**（第404 ⑵ のとおり＝Deploy → Reload → 出力チャネルの 2 行・`requestFull` が出ないこと）／⒝ 群単位の item（第404 ⑵′・実機の 2 行を見てから）／⒝′ frame 変更の `applyFrame`（実機で頻度を見てから）／⒞ 第403 の残り＝**prelim の `p.scripts`（fingbeam1k 1.2 ms／7.2 MB＝`ComputeFingeringsAndScripts` が 24,000 fingering layout を毎打鍵組む・FingScriptMemo は per-system で hit しているので組み立て（連結）の側＝回数を数える）**・count loop の page DP 5.3 ms（R13⒦・census で hit 率）・診断 pass の残り（`AnnotationName` 14／`MeasureValidator` 10／`LyricSings` 6.5）・`RestCollisionsOf`／`RestDotOffsetsOf`（第405 ⑶′＝0.4 ms／0.3 MB＝**要ユーザー判断**）／⒟ LSP 起動直後の tier-0（ReadyToRun・`TieredPGO` の knob は packaging 側＝**要ユーザー判断**）／LP 双子が要る R7〜R11／R15（要承認）。
