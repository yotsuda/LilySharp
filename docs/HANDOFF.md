...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第427セッション（2026-09-20・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `513da970`・**未 push 38**・**full `sessions/p427/run1.trx` 8755 / 0 / 3 / 8758＝第426 最終と一致**・台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・68 秒）、★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` のままだからで、第412〜第426 が予告したもの）。⚠️ **HEAD は第426 の `a5fafdf0` から +2＝ユーザーが `tools/Deploy-Lsp.ps1` 群（`e4975ef3`）と lilypond の hang の説明（`513da970`）を commit していた**。**§7 3.5 は*セッションの頭*で回した**（`-Archive 425`＝24 行 7,053 chars・448,328 → 436,164 B）。指示は「**優先度の高いものから順に着手して**」＝§1 の「次の一手」の ★★★ を上から。★ ベンチは着手前に静かな窓をもらった。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第426 と同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、**before の打鍵 8,084,142／8,084,141 B は第426 最終の 8,084,109 B と一致**。⇒ **島を 2 つ閉じた**:

★★★ **⑴ ⒴⁹ の起票は島の半分しか名指していなかった**（脚 1・計器は**構成で数える counter**＝割当時計を使わない）。出口の 3 経路を分けて数えると: **b＝`EndBatch` の batch 腕**（5.6 walk／打鍵・in 486.5・out 176.8・**out/in 0.363**）が**払い 0.206%・exact-size なら 0.072%＝賞金 0.134%**／**n＝非 batch 腕は 1 度も通らない（0 walk）**／★★★ **m＝`MergeInternal` が払い 0.200%（全額が倍々）・賞金 0.104%**＝**§1 が名指していなかった同額の島**。⚠️ **予測 ⑶「`b.grow` は ~0」は外れ**（0.012%・**377 walk が `batch.Count` を超えた**）＝**resolve は分割で入力数を超えうる**＝`batch.Count` は上界ですらない場面がある。

★★★ **⑵ 直し＝結果の配列を「渡された数」ではなく「出た数」で確保する**（`7d23bb43`・**Core 1 ファイル**・網 1 ファイル・**数値定数 0・LP 由来の式 0・REF 0／OWN 0**）。walk は thread が貸す buffer に書き、**R が判った後で `_buildings.Capacity = R`**（⚠️ **`EnsureCapacity` では駄目＝倍々で返すので rung が戻る**）。⇒ **A/B ＝ −0.1347%**（8,084,141 → 8,073,241 B・床 52 B）。★★★ **⚠️ そして「exact-size を全部に」は*間違い*で、第421 の cost gate がそれを捕まえた**——**1 棟ずつ育つ skyline では毎回確保し直す**＝倍々の償却を捨てる。**方針を決める測定はこのファイルに既に在った**＝第421 の「`EndBatch` は 10,284 の*別々の* skyline に 1.00 回＝構築の慣用句／`MergeInternal` は 1 instance に 4.51 回」⇒ **構築は exact・累積は倍々のまま**（`sizeResultExactly`）。

★★★ **⑶ Ⓑ は §1 の数と*完全一致*で、§1 に無いことが 2 つ出た**（脚 1）。`DotConfiguration.Resolve` **45,320 回／打鍵 24.5 回・0.315%・1 回 1,036 B**（§1 の 45,320 回／1,060 B と一致）。⇒ ★★★ **入力の 99.84% は点が 1 つ**（45,248 / 45,320・n=2 が 2 回・n=3〜4 が 70 回・**n≥5 は 0**）／**内訳は `b.sort` 0.109%（1 要素を並べ替えるのに 360 B）・`b.insert` 0.174%・`b.result` 0.032%**（`SortedDictionary` の enumerator が木を歩く `Stack` を建てる）・**`Shifted` は 1 回の Resolve につき 1.04 棟**（線上の点 23,537＝51.9%）。

★★★ **⑷ 直し＝容れ物を span にした。家は 1 つのまま**（`bfc77dc0`・**Core 1 ファイル**・網 1 ファイル・**REF 0／OWN 0**・LP の REF 註は 1 字も動かしていない）。`SortedDictionary` → **位置で整列した `Slot` の span**・LINQ → **挿入ソート（⚠️ *安定*でなければならない＝2 点が同じ位置を共有でき、map は*後に来たほう*を残した）**・scratch 3 本は stack から。⚠️ **速い綴りを 2 本目として足していない**（§5.2.1②）＝**size で分岐するのは scratch の*出どころ*だけ**（8 slot）。⇒ **A/B ＝ −0.312%**（8,073,267 → 8,048,053 B）＝**島から「返す配列」を引いた全額**。

★★★ **⑸ 網は*差分の網*で、毒が「観測者が居ない規則」を 1 つ見つけた**。**旧綴りを verbatim でテスト側の oracle として残し**（RULES §7.7・⚠️ 実装と何も共有させない＝ただで一致しないように）、**箱の中の全 chord で突き合わせた＝23,718 chord・71,870 dot**（1〜4 点・全 direction・**同位置の同点**）。毒 ⑴「並べ替えを不安定に」→ **この網＋既存の振る舞い 2 本が赤**／★★★ 毒 ⑵「**cascade を止める行（`if (!Contains(dst, m, p)) offset = 0;`）を落とす**」→ **この網 1 本だけが赤で、8,760 本の他は 1 本も鳴らない**＝**LP の `shifted()` のこの規則には、このファイルができるまで観測者が 1 つも居なかった**（第421 の「この腕には観測者が無かった」と同じ形）。⒴⁹ 側の毒も 2 つ＝「渡された数で確保」→新網だけ赤／「exact-size をやめる」→第426 の網だけ赤＝**2 本の gate は構成上お互いを覆わない**。

★★★ **⑹ 出力同一は 3 つとも**: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline は `513da970` の木から取り直した＝**今便の 2 つの変更を通しての比較**）／⑵ **実コーパス全ページ SHA-256＝5,824 行・0 行差**（第426 の `hashes-after.txt` と突き合わせ）／⑶ suite 全緑・snapshot 249 枚不動・台帳 851 点／総和 22.584727806 不変。

⇒ **次の一手**（⒜⒝⒝′ は第406 から変わらず）: ★★★ **⒴¹¹ 新規＝`MergeInternal` の出口の倍々 0.200%・賞金 0.104%**＝**⒴⁹ の exact-size は*ここには効かない*（1 instance 4.51 回＝償却する側）**。⚠️ **測ったうえで「別の手が要る」と判った島**＝入力の上界（`_buildings.Count + other.Count`）で 1 発確保する案は*出口を 8.7% 過剰にするだけ*で、次の merge がまた外す⇒**先に「1 skyline あたり何回 merge が来て、capacity が何回足りなくなるか」を数えること**／★ **⒴¹⁰ tab fork の残りは `TabSoundingNotes` の iterator ちょうど 64 B × 34,717 items＝0.015%**（⚠️ `fret.ToString`・`FretGlyphWidth`・`CalculateFret` は 3 つとも 0 B）／★ **⒴⁷ `BeamedItemsToSuppress` は beams が空でも `HashSet` 0.028%**／★ **⒴⁶ 残った包み＝`SeedClef` の `PlaceGlyphOutlineCached`**（天井 `ink.clef` 0.021%）／⒴⁸ **batch でない `Merge` 147,417 回／打鍵は未値付け**／★★★ **⒡′ bow を*staff 自身の枠*で採点して offset は描画時に足す**（0.074% ＋ 1 ULP の尾・土台の変更・要設計）／★★ **⒢′ 拒否された staff の前処理＝検出器 0.023%＝`StaffSpannerItemsOf` の memo を最終パスにも通すだけ**（一番安い・出力同一が構成で言える）。⚠️ **まず回数を測ること**／★★ **⒵⁴ `prefixMarkAnchorX` の解き直し 0.338%**＝**memo は反証済み**（hit 率 0.53%）・土台の変更／★ **⒵⁵ `mm.g.split` 0.163%**（⚠️ `OrderBy` は*安定*）／★ **⒵⁶ 文字の 318,784 回／81 綴り 0.130%**／★ **⒴⁵ artic と trill の remap 辞書は作って捨てている**（天井 0.174%）／★ **⒳″ `Padded` の複写 0.028%＋`SortAndResolve` 0.15%**（起票のみ）／★★ **⒴⁗ 天井 2.03%＝安くすると `アゲハ蝶.lys` 1 冊が 24→26 系になる＝perf ではなく*忠実度の判断*。開けるならユーザー決定が先**（実装は Lab `sessions/p423/zz423-deferred-prelim.diff.txt`）・**先に LP 双子**／⚠️ **`g4.core` 1.21% と `p1.s1.beams` 5.538% は*もう実仕事*＝この 2 島には戻らないこと**／⒜ **R13⒝ の実機確認**（第404 ⑵）／⒝ 群単位の item／⒝′ frame 変更の `applyFrame`（どちらも実機の 2 行を見てから）／★ **⒯ 索引を*緑*にする**（`SyntaxNode.GreenSitesLazy` が既にその機械・未見積もり＝土台の変更）／⒵ **collect 17.8% と `S1.prelim` 17.0%**／⒥ は第409 が上限 4.7 ms と測った／**R13⒦ ⒤ ✅ 第410 が閉じた**／⚠️ `RestCollisionsOf`／`RestDotOffsetsOf` は **ユーザーが「今はやらない。着手はずっと後だ」と決めた（第407 ⑺⑴）＝提案しない**／⒞′ prelim の残り＝`fs.walk` 0.41 ms・`fs.assemble` 0.29 ms／**⒞″ ✅**・**⒟ ✅**（第407）／★★★ **LP 双子が要る R7〜R11 は*今日から着手できる*＝`lilypond.exe` の hang は 2026-09-20 に解決**（MCP コンソールのプロンプトの入力読み取り待ち・`cmd /d /s /c "… < NUL > log 2>&1"` で通る＝RULES §5.5）／**R15 ✅**（残り＝「2 綴り」一族のみ・要承認）。⚠️ **⒜ と push は「後回し」＝催促しない（第407 ⑺⑸）。**

★ **⑺ 終了時**: commit 3 本（code `7d23bb43`＝⒴⁹・code `bfc77dc0`＝Ⓑ・docs 1 本＝§1。**この文を含むので SHA は書かない**＝§5.4）・**未 push 41**（開始時 38）・**最終 full `sessions/p427/run4.trx` 8757 / 0 / 3 / 8760**（**第426 最終 8758 から +2＝今便の網ちょうど 2 本**。`Zz427`／`ZzStage`／`ZZPOISON` の grep は**コードに 0 件**）・§7.5（対 `513da970`）**Core `+` 281 行／REF 0／OWN 0**・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・棚卸しは **`APPROXIMATIONS.md` も `magic_constants.csv` も行の増減 0**（⒴⁹ で行番号だけ・Ⓑ では 1 行も動かず）。計器と全文は Lab `sessions/p427/`（`exit-side-leg1.txt`＝⒴⁹ 脚 1・`dot-resolve-leg1.txt`＝Ⓑ 脚 1・`zz427-change.diff.txt`・`Zz427Ab.cs.txt` と `Zz427Hash.cs.txt`＝再利用可の A/B と証明 ⑵ の harness・`hashes-after.txt`・`ab-{before,after}-{1,2}-tc0.txt`＝⒴⁹・`ab-b-after-{1,2}-tc0.txt`＝Ⓑ）。⚠️ **⒴⁹ の脚 1 の計器は*直した後*は嘘をつく**（rung 模型で exact 1 発を数える）＝**after の数は A/B の通貨で読むこと**。⇒ ⚠️⚠️⚠️ **HANDOFF は第426 の記録 448,328 B → アーカイブ後 436,164 B → 今便を書いて 447,742 B＝天井 450,000 まで 2,258 B しか無い**（今便の §1 は 8.6 KB。**`Fold-ClosedHandoffItems` の弁は第426 の時点で既に 0 件**）。⚠️ **次便は §1 を 1 行も書く前に `-Archive 426` を回すこと**（**途中で回すと網が 3 本赤くなる**・第410 ⑾）。**§3 の圧力弁は第409・第410 で使い切っている。天井は上げない。** **作業ツリーは空**（ユーザーの未 commit 分は第426 のあとユーザー自身が commit した）。**push はユーザー**（Lab も）。⚠️ **`origin/master` の CI は `9f1d74a9` 以降が push されるまで赤のまま**＝次便が「CI が赤」を見ても、それは第412〜第427 の欠陥ではない。
