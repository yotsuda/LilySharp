...
- ★★★ **⒮¹⁰ 「器ごと憶える」の島の残り＝336 軒 254,879 B／打鍵 6.37%・*全部が liveness を通っている***
  ...
  **生きている 356 軒の actual＋obj は 481,208＝打鍵の 12.03%**で、**escape はたった 16 軒 102,619**
  ...
  そこから**第457 が閉じた 4 軒 123,703** を引いた。
  ⇒ **次の軒は値段順に並んでいる**: `MusicSiteList.cs:83` **55,484**（⚠️ 器は `MusicSiteList._sites`＝
  *返る*器なので give 点を設計する＝第441 の「前回の数を憶える」とは別の直し方）／
  `MeasureCollector.Form.cs:1101` **13,310**（`MusicSiteList.Eager` に渡り `processNodes` の後で死ぬ）／
  `BeamDetector.cs:141` **9,345**／`LedgerLineSpannerEngraver.cs:79` **7,822**（Builder）／
  `ArticulationEngraver.cs:395` **6,564**／`LayoutEngine.Prelim.cs:374` **6,192**（入れ子の bucket）／
  `CollectResumePlanner.cs:286/287` **5,297×2**（＝**⒮¹¹ はこの島に畳んだ**）。
