...
終了時: **commit 3 本**（`b6c33de3` 点 2 つ ＋ `b8fde5b4` 直しと掃き ＋ **この 1 本**）＝**未 push 3**・木 0・未追跡 0。**suite 6551 合格 / 0 失敗 / 4 skip / 合計 6555**（開始 6551 から **+4**＝**台帳の点 2 ＋ 対応付けの単体テスト 2**）。Core 0 エラー 0 警告。**追跡 `.lys` 579 冊**（不動＝**本を 1 冊も足していない**）・**snapshot 229 枚**（不動＝**4 枚を*書き換えた***）。**台帳 744 点／exact 584／ss 非ゼロ 190／総和 25.844537623（不動）／count 156 中 2。**
...
★★ **緑は*コード commit のあと*に取った**。⚠️ **WSL ubuntu Release と CI は未実行**（未 push 3）。
