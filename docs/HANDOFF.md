...
最終更新 第334セッション＝**入り方が第298〜第333 と違う**（ユーザーが `docs/HANDOFF.md` と `scratch/ベースタブLy/bench.lys` を読ませて「L14 のふたつの @chord が重なる問題を直して。x 方向の余白が狭すぎる」と*的を名指し*・background job・口挟み 0）。**骨は 5 つ**:

★★★ **⑴ 再現**: `bench.lys` 8 行目 `<b cis' fis>,@chord | <e' gis b dis>,@chord`（全音符の和音 2 小節）を `lysc svg` すると **`F♯sus4`（x 16.34・描画幅 8.30）と `Emaj7/D♯`（x 25.42）の間が 0.78 ss**＝一語に読める（`scratch/p334/bench-before.png`・LP の絵 `bench-cn-lp.png` は sus4 を上付きにするので狭く見えない）。
⚠️⚠️ ★★★★ **⑵ 正体＝インライン `@chord`（`UseTiming=false`）は幅を 1 つも予約していなかった**。`SpacingRules.ApplyChordRowSpacing`／`ChordInkRightReachPerColumn` の filter `(!includeAttached || !cn.UseTiming)` が inline を落とし（`Timing` が既定 0 のままなので列も引けない）、`ChordNameEngraver.ClearOfPrevious` も inline は動かさない。**chords track の記号（UseTiming）は同じ経路で `w+0.6` を小節右端に予約している**（`ChordAttachedSpacingTests`）ので、inline だけが枠の外だった。
★★★★ **⑶ 直し＝inline を既存の attached 経路に乗せた（製品 5 ファイル・⚠️ 未 commit＝出力が動くので RULES §5.1）**: `MeasureCollector.CollectChordNames(…, anchorTiming)`（13 call site・`builder.CurrentDuration`＝articulation と同じ anchor・arpeggio は `startTiming` を先に取る）→ `ChordNameCollector.AddInline(timing)` → `ChordNameItem.Timing` に音符の onset（`UseTiming` は false のまま＝描画は `ItemIndex`・`SymbolX` 不変）。spacing の filter は `!cn.IsChordRow && !includeAttached` に。**番人 3 本**（`ChordAttachedSpacingTests.InlineChord_OnAWholeNoteBar_HoldsTheBarOpenOnTheRight`／`InlineChord_PricesTheColumnAtItsOnset`・`ChordNameTests.Collector_ChordName_CarriesTheNoteOnset`）。CHANGELOG Engraving に 1 項・`docs/APPROXIMATIONS.md` 再生成（行番号 720 → 740）。
★★★ **⑷ 結果**: bench は `F♯sus4` 15.03／`Emaj7/D♯` 27.47＝**間 4.1 ss**（`bench-after.png`）。**full 7121 / 3 / 4 / 7128（`scratch/p334/run1.trx`・+3）**。赤 3＝inventory（再生成で緑）＋ **snapshot 2 枚が動く（✅ ユーザー承認「承認する。snapshot を再ベースして commit して」→ 再ベース → full 7124 / 0 / 4 / 7128（`run2.trx`）→ `8d587290` に同梱）**: `test/degree-chord-name`（小節幅 17.58 → 17.71＝`Dm` の w+0.6 だけ・絵は同じ）／`showcase/04-advanced`（**`C DmEm F G AmG C` の団子が解けて等間隔**・1 段目 6 小節 → 5 小節・`B` が 2 段目の途中へ）。`artifacts/visual-diff/` に before／after の png。
★★★ **⑸ LP 側の枠（`scratch/p334/bench-cn.ly`＝双子に `\new ChordNames \chordmode { s1*5 fis1:sus4 e1:maj7/dis s1*4 }` を足した・`cn-settings.ly`＝ChordName の X／X-extent・BarLine・NoteColumn の system 相対 X）**: **LP の rod は ChordName 同士（w+0.5+0.5+0.1）と行末だけ**（Separation_item は水平 skyline＝高さの重なる物どうし）。**小節線も次の音符列も名前の下に潜る**（`F♯sus4` ext 6.90＠16.34・次の小節線 21.88・`Emaj7/D♯` 25.42・その次の音符列 31.94 ＜ 名前右端 32.60）。Lily# は最後の記号の幅を小節右端に予約する＝LP より強い（既存の attached 経路の設計・`ApplyChordRowSpacing` の remark に NAMED DIVERGENCE として書いた）。⚠️ **exporter の欠陥 1 つ（未修理・別件）**: `<b cis' fis>,` を `<b, cis', fis,>` と出す＝**`cis',`（`'` と `,` の混在）は LP の構文エラー**。正味オクターブ（`cis`）で出すべき。probe は手で直した。⚠️ `lysc ly` は inline `@chord` を「out of scope」で落とすので双子に ChordNames は無い（手で足した）。

★ **開始時裏取り**: HEAD `1e362e59`・**未 push 47**（第333 の §1 は 46 と書いたが、その後の第 4 便の HANDOFF commit が乗っている）・木 clean・未追跡 0・`bin\Debug` は net10.0 のみ・solution build 0 エラー・Core 0 警告（4 分 01 秒・通常 build）・焦点 207 / 0（`scratch/p334/focus1.trx`）。
**終了時（第 1 便）**: 製品 commit 0（作業ツリーに置いたまま）・docs commit 1（`859733c8`＝HANDOFF＋ARCHIVE＝第332 の経緯を ARCHIVE の先頭へ逐語）・未 push 48。
**終了時（第 2 便＝承認後）**: **commit 2 本（`8d587290`＝製品 5＋番人 2＋CHANGELOG＋inventory＋snapshot 2 枚・full 7124 / 0 / 4 / 7128／この HANDOFF）・未 push 50・木 clean・未追跡 0**・台帳／`.lys`（追跡）不動。
✅ ★★★ **⑹ 第 3 便（ユーザー「次便がこの便で有利なら着手」→ 双子と LP の検証環境がこの便に揃っているので着手）＝exporter の `cis',` を閉じた**: `LilyPondExporter.EmitChord` の absolute 分岐がメンバーの印と和音の印を*別の文字列*で続けて書いていた（`OctaveMarks(p.OctaveOffset + _sectionOctaveOffset)` ＋ `marks`）→ `off` を 1 つの `OctaveMarks` に畳んだ（relative 分岐は chain に対して印を再計算するので元から無関係）。**`lysc ly bench.lys` → `<b, cis fis,>`／`<e gis, b, dis,>`・LP 2.26.0 exit 0・警告 0（`scratch/p334/bench2.ly`）**。番人 `LilyPondExporterTests.Absolute_ChordLevelShift_FoldsIntoEachMembersMarks`・CHANGELOG（0.6.0 に twin 節を新設）・inventory 再生成。**full 7124 / 1 / 4 / 7129（`run3.trx`・赤 1＝inventory の行番号・再生成で緑）**。絵は 1 バイトも動かない（exporter だけ）。
**終了時（第 3 便）**: commit 2 本（exporter／この HANDOFF）・未 push 52・木 clean。
⇒ ★★★★ **次の一手**: ✅ **⑴ 承認・再ベース・commit 済み**。✅ **⑵ exporter の `cis',`＝第 3 便で閉じた（⑹）**。**⑵′ `lysc ly` に `ChordNames` を出す件＝着手せず（第 4 便で射程だけ確かめた）**: **`chords` row も出ない**（`LilyPondExporter.cs:2902` が「chord row '…' is not exported — the twin has no chord row」と警告して落とす）・inline `@chord` は `EmitMark` の out of scope。⇒ 双子は chord 記号を 1 つも持たず、chord 記号の LP 照合（今便の `bench-cn.ly`）は毎回手で `\new ChordNames` を足すことになる。**これは機能 1 つ分で、綴りの決定が要る**（`ChordStructure` から `\chordmode` を出すと LP が*自分の*名前（`F♯sus4` は LP でも `fis:sus4` だが、Lily# の表示文字列と LP の Ignatzek 名は一般には一致しない）を刷る／表示文字列そのものを `\markup` で出すなら `chordNameFunction` を差し替える）。**ユーザーと決めてから、新しい便で**。⑶ 以下は第333 の並び（Le Freak 3 対／Addicted To Love 2 対を `beng332` の型で割る・lyric row の slot の投票・Express Yourself の圧縮行・ps2 の 0.017／beam-over-stem bar 1 の +0.33・push 後 `gh run list`・`audit/lpreg` 取り直し・§2 U8c・U8b・U8／A/B/D/E／C⑴／G）。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
