...
> **保守は 1 か所**（第431 まで毎便 2 つの語りに全文複写されていた）。**閉じたら消す**（§7 3）。
...
### 1.1 第568セッション（2026-09-24・YT-DELL2）

同じ会話の続き（**ユーザー報告 2 件**＝showcase `scratch/site-showcase/petite-valse.lys` を見ての指摘: L28 の `@arpeggio` が小節線と重なる／B の section mark が L22 の grace と重なる）。★ `-Start p568`（HEAD `755702af`・full 8962 / 0 / 2 / 8964・`-Archive 566`）。作業ツリーの `samples/*` はユーザーの手（触らない）。**どちらも「Lily# の側に*その ink が無い*」形**で、片方は水平の列の reach、片方は垂直の staff profile。

★★ **⑴ U14 arpeggio の wiggle は*列の左端の ink* だった**（起票＝ユーザー）。LP: 小節線→列の最小距離は `Paper_column::minimum_distance`（paper-column.cc:145-164）で、**右列の CONDITIONAL skyline を merge する**＝arpeggio は `paper-column-engraver.cc:246-261` がそこへ回す；さらに `Accidental_engraver::make_standard_accidental`（accidental-engraver.cc:298-307）が**作った臨時記号を全部その arpeggio の support に足す**（"so it is put left of the accidentals"）。Lily# は列の reach を符頭と臨時記号で止め、wiggle は符頭だけを避けていた。**新しい台帳 2 点を先に開けた（コード変更 0 の commit `664b654c`）**: `arpeggio.x.barline-to-wiggle`（probe ABL・LP **0.500000**＝小節線 0.1 ＋ wiggle 0.1 ＋ headroom 0.3）と `arpeggio.x.right-edge-to-accidental`（AAC・LP **0.500000**＝sharp の ink 左から）。**予測を書いてから読み、2 つとも当たった**（−0.400000000／−0.950000000）。直し（`8174f7e8`）＝**⒜ `SpacingRules.ChordSupportLeftReach`＝和音自身の左 ink（逆向き符頭 or 最左の臨時記号）を 1 か所に**し、`ArpeggioEngraver`・`ItemSkylineFactory.AddArpeggio`・`CalculateLeftExtent` の 3 軒が読む／**⒝ `CalculateLeftExtent` がその support から `ArpeggioEngraver.ReachPastSupport`（padding 0.5 ＋ glyph 0.8、bracket は自身の幅）だけ伸びる**＝LP の conditional skyline の scalar 版／**⒞ `LeftmostGrobExtraSpacingWidth`＝最左の箱は arpeggio の既定 0.1**（臨時記号の 0.2 ではない）。**2 点とも exact（0.500000000）**。網 `ArpeggioSpacingTests` 6 本（摂動＝同じ和音に arpeggio を付けたり外したり）。snapshot 2 枚（`test/arpeggio-second` の行頭 wiggle の列 +0.5＝LP が AQ と ABL の間に見せる同じ +0.5／`showcase/02-ornaments` は 1 列の最小が育ったぶんの justify 反映・ページ高 39.40 → 39.39）。

★★ **⑵ U15 grace の ink は*譜表の内側の ink* だった**（起票＝ユーザー）。LP に Grace context は無い（`\name Grace` は 0 件・`engraver-init.ly:368` の `\consists Grace_engraver` だけ）ので、**grace の符頭・符尾・旗・臨時記号は普通の Voice の grob＝`inside_staff_skylines`（axis-group-interface.cc:914-935）に入り、outside-staff の mover がそれを避ける**。Lily# の `SkylineBuilder` は grace 時間の item を**全部 skip**していた＝**どの vertical skyline にも grace が無い**。台帳 2 点を先に開け（`5e7a2b4f`）、**`mark.over-grace.staff-to-baseline`（probe MGF・LP 5.911984＝grace の符尾頭 5.1 ＋ 0.46 ＋ 箱 0.352）と control（MGN・4.356984）**。⚠️ **予測は外した**——control と同じ約 −1.555 と書いたが実測 **−1.999999790**：grace の列が clef と最初の符頭の間に立つので**その符頭も mark の x 窓の外**＝旧 profile は control より*少ない*ものしか差し出していなかった。直し（`729a6c64`）＝**grace 時間の item を普通の seed に通す**。サイズは grob に訊く（`GrobFontSize`）＝符頭は 14 の design、符尾は `StemCalculator` ＋ `GraceStemDetails`（length-fraction 0.8・no-stem-extend）、**旗は 14 の *輪郭***、臨時記号は 13 の design で自分の `position_apes`。梁のある run は quant 済みの梁とそこで終わる符尾を seed（`AddGraceBeamsToSkyline`）。**X は `GraceNoteEngraver.RunPlacement` を切り出して 1 か所に**（描画・梁の quanter・profile が同じ式を読む）、鍵は公開済みの item 住所（`GraceSeedsFor`）。⚠️ **旗は箱ではなく輪郭**——箱で seed すると **+0.018409**（16 分旗の LILC 箱は原点より 0.058 上だが、その ink は符尾側に無い）。**2 点とも exact**。
  ★ **同じ profile を*横から*読む点も足した**: `staff.staff.grace-over-notes`（GCL・LP **9.384000**）と control（GCN・**9.000000**）＝段間。**両方 exact**。⚠️ **最初に書いた GCL/GCN は何も測っていなかった**——第 2 小節の grace を 1 オクターブ下に書いたので梁が bass 譜の中に収まり、LP が**両冊 9.000** を返した（RULES §5.0「対の両側が同じ音楽か」）。fixture の音高（D4/E4）に直して差が出た。
  **毒**（`poison.ps1`・3 本）: ⑴ 列の reach を support で止める ＋ wiggle を符頭だけで置く／⑵ grace の列 seed を skip／⑶ grace の梁 seed を skip。**9 赤**。⚠️ **⑵ と ⑶ は*別の観測者***——⑵ だけだと **`staff.staff.grace-over-notes` は緑のまま**（段間を開けるのは列ではなく*梁*）。網が観測されているつもりで観測されていない形なので、腕ごとに毒を 1 本ずつ。⚠️ **毒の罠**: `[IO.File]` は `Push-Location` を見ない＝相対パスはプロセスの CWD に解決される（第567 の script をそのまま持ってきて落ちた・絶対パスに直した）。
  snapshot 3 枚（`grace-notes`・`grace-explicit-slur` は ink 同一でページ高 +0.35＝下の spring が grace を読む／`grace-lower-staff` は段間 9.00 → 9.38＝**その本の双子で 9.384**、grace を外した対照で 9.000 を確認）。**hash A/B（baseline＝p567 の after 5,800 行・Lab `Zz568Hash.cs.txt`）: 0 行**——**コーパス 231 冊に `@arpeggio` は 0 冊、grace は 3 冊**（タブ譜の grace は数字で描く別腕＝seed しない）。full **8973 / 0 / 2 / 8978**（+9）。表: `APPROXIMATIONS.md`（277 → 278・新 APPROX 1＝grace 梁の符頭定数）・`magic_constants.csv` 再生成。⚠️ **引用の ratchet で 4 件赤**——私の LILYPOND-REF が行範囲だけで記号名を持っていなかった（`CitationsThatNameNothing_DoNotGrow` 681 → 685）。**記号は*同じ行*に要り、ハイフン語は 3 節以上**（`side-axis` は不可・`side-position-interface` は可）。全部 LP の実行を読んで実名を付けた＝681 に戻した。
  ⇒ 判定: ユーザー報告 2 件は閉じた。**seed していない残り 2 つを seed に名指し**: grace の**付点**（X は `DotColumn.ReservedForGrace`＝旗から測る別式）と**cue の縮小列**（既存の島）。どちらも観測者 0。次の一手は §1.0 の方針どおり LP 忠実度＝R10 の残り（⒠⒣ か `chords` track の 1.0）。この便の文脈に依らない＝(c)。

## 以下は第567セッションの経緯

### 1.1 第567セッション（2026-09-24・YT-DELL2）

同じ会話の続き（§2 R10 の残り ⒟⒡⒢）。★ `-Start p567`（HEAD `88509ba8`・full 8957 / 0 / 2 / 8959・`-Archive 565`）。作業ツリーの `samples/*` はユーザーの手（触らない）。

★★ **⑴ R10⒢ 和音記号の行の高さ＝`StaffPadding 0.6`（fit）を LP の規則に**: 行の baseline＝**記号ごとに（譜表の skyline の高さ（記号の x 窓）＋ padding 0.5 − 記号の ink 下端）を取り、行で max**（`ChordNameEngraver.RelatedStaffPadding`＝engraver-init.ly:722 の nonstaff-relatedstaff-spacing padding・align-interface.cc:228-238 の最小 translation）。裸の譜表では 0.5 ＋ 線の半厚 0.05 − ink 下端。**双子で実測**（Lab `sessions/p567/chord-bare.ly`＋`dump-chord.ily`）: 何も上に出ない譜表で baseline＝譜表 ink 上端 ＋ **0.537**（= 0.5 ＋ G7 の ink 下端 0.037）／a'' の下では**その符頭の上端 ＋ 0.537**（c を C6 に揃えた第 2 冊 2.587）。**after の Lily#**（`chord-bare-ls-after.svg`）: 裸 0.60・a'' 2.65＝LP の 0.587・2.637 と **0.013 差＝面項**（Heros の G の overshoot 0.05 対 Nimbus 0.037・§3 の決定）。⚠️ **この腕が置くのは note-attached `@chord` と最上段の attached 行だけ**——`chords` track の行は loose line の walk（既に LP）で置かれ、fixture `chordnames.lys` の 2.90 対 LP 3.90 の差は walk の skyline 側（未着手・下の判定）。網 `ChordNameStaffPaddingTests` 3 本は `@chord` で書いた（最初 `chords` track で書いて毒が緑＝計っていたのは walk の腕だった）。既存 2 本（`ChordRowInlineAlignmentTests`・`ChordNameTests` の tab）は期待値を規則へ。**帯（`MultiStaffLayouter.ReserveChordRowBand`）も同じ 0.5 を読む**＝下段に和音行を持つ本の段間が 0.10 縮む（snapshot 3 枚＝`showcase 04/07`・`figbass-chordname-lower-staff`・ページ高 −0.10・記号と番号以外の差分は全部この 0.10 の平行移動）。
  ★ **PagingSkylines の和音 silhouette 腕（平坦 `[cnY+1.9, cnY−0.3]`）を実 ink（`SymbolInk`）に**——第273 の註は「面項のせいで scalar の方が近い＝ユーザーの取引・黙って移植するな」と留めていたが、行の baseline を LP の規則に置いた時点でその偶然が消え（scalar は LP の ink 上端の **0.052 下**）、実 ink にした（−0.0445）。台帳 `page.inline-chord.gap-first` を **+0.001116 → −0.044541** に再ベース（why に経緯・残りは面項だけ＝両エンジンとも ink 下端が refpoint + 2.55 に立つ）。⚠️ **第273 が「ユーザーの取引」と書いた腕を私が取った**＝報告する（戻すなら `AddMarkBox` の 2 引数と台帳の 1 行）。
★★ **⑵ R10⒡ stanza number の X**: LP は x-aligned-side（LEFT・padding 1.0）で、supports は `Stanza_number_align_engraver` が全 verse の先頭音節に広げる＝**全番号の右端＝最左の先頭音節 ink 左 − 1.0**。手書き双子（`stanza-hand.ly`＝lyrics-verses に `\set stanza`・`dump-stanza.ily`）: "1."・"2." とも右端 **5.659**＝"Twas" 6.659 − 1.0（verse 1 の "A" は 8.349 から）。Lily#: 右端＝min(音節中心 − advance/2) − 1.0・X＝右端 − advance（`Calculate` に fonts を渡す・音節の ink 幅は advance＝R10⒝ の残差と同じ）。lead sheet は OWN のまま（Indent − 4.0・2026-08-20 のユーザー報告）。LP は `\set stanza` の 1 回だけ・Lily# は毎 system＝OWN 註。網 `StanzaNumberAlignmentTests` 2 本＋既存 `StanzaNumberTests` 1 本を規則へ。snapshot 3 枚（`lyrics-verses`・`lyrics-volta`・`named-voice-lyrics`＝番号だけ）。
★ **⑶ R10⒟ `_"text"` の Y**: 起票「固定・priority 450 未使用」は古い＝stacker の `PlaceCustomTexts` が staff-padding 0.5 の床と outside-staff pass（450 の順）を既に持ち、engraver の seed（「譜表の 5.5 下 − 0.5」・REF は TimeSignature の行 :3925）は床に必ず負ける dead。seed を aligned_side の床（2.05 ＋ 0.5＝`AlignedSideBaselineYUp`）に＝stacker と 1 か所・出力不変。⚠️ **`_"text"` は LP の `^\markup`（譜表の*上*）として置かれている**（ledger `textscript.no-descender.staff-to-baseline` はそう測った・snapshot も上）＝fixture と engraver の「below」は古い。`_` の綴りで上に出るのはユーザー判断の余地（下の判定に 1 行）。
  **毒**（`poison.ps1`）: 旧規則（0.6 ＋ peak・measures[0].X − 4）で chord 2／stanza 2 が赤（向きだけを言う 2 本は緑のまま＝想定）。⚠️⚠️ **毒の罠を 2 つ踏んだ**: ⑴ 復元に `git checkout` を使い*未コミットの編集ごと*消した（再適用した）／⑵ 同じファイルの 2 つ目の編集で backup を上書き＝毒が木に残った（`double floor = 0.6 + …` のまま full が緑だった——網が `chords` track を測っていたため）。どちらも script に註（復元は byte copy ＋ touch・backup はファイルごと 1 回）。**hash A/B（baseline＝p566 の after 5,800 行・Lab `Zz567Hash.cs.txt`）: 0 行**（コーパスに `@chord`・複数 verse・attached 行が無い）。full **8959 / 0 / 2 / 8964**（+5）。表: `APPROXIMATIONS.md`・`magic_constants.csv` 再生成・台帳 1 点再ベース。文書: CHANGELOG 0.8.0 Engraving 2 項・§2 R10 ⒟⒡⒢ ✅。
  ⇒ 判定: R10 の残り＝⒜ aligned_side の 3 綴り（大）・⒞ `ResolveOverlaps`・⒠ figured bass の 0.8・⒣ `TextOffsetInSpanner 0.6`・残差 3 つ（音節の ink 幅・melisma 割当・**`chords` track の行が LP より 1.0 低い＝chordnames.lys の walk 側の skyline**）。ユーザー判断が 1 つ: **`_"text"` を上に出す今の意味論を残すか**（LP の `_` は下）。次は ⒠⒣（小）か chordnames の 1.0。この便の文脈に依らない＝(c)。
...
- **R10 記号・歌詞の残り**＝⒜ `ArticulationEngraver` の aligned_side が 3 綴り（note 支持は箱の算術、chain/tie/slur は pointwise、`DynamicEngraver.SpannerOffsetY`）＋`on_line` の ledger parity／⒝ ✅ 第565 が閉じた（extender＝`h = lt×0.8`・padding は h・`minimum-length 1.5` を系の右端で切る・最後の符頭の右端まで・次の音節の左端 − h で切る・baseline の上に箱・`w < 1.5h` で消す＝手書き twin で実測一致・網 `LyricExtenderGeometryTests` 5 本。⚠️ 残差 2 つは別項: 音節の ink 幅（Lily# は LP より 0.1〜0.4 広い＝`lo` 2.00 対 1.91・`la` 2.16 対 1.78・start が 0.1 ずれる）と melisma 割当（`c'4( d e) f` に `la __ la`＝Lily# は 2 つ目を e に、LP は f に置く））／⒞ `LyricEngraver.ResolveOverlaps :1818-1870` は LP に無いシフト（LP は `LyricSpace` の rod）／⒟ ✅ 第567 が閉じた（起票は古かった: stacker が staff-padding 0.5 の床と 450 の順を持つ・engraver の dead な seed を aligned_side の床 `AlignedSideBaselineYUp` に＝出力不変。⚠️ `_"text"` は `^\markup`＝譜表の*上*に置かれている＝ユーザー判断の余地）／⒠ `FiguredBassEngraver.MinFigureBoxWidth 0.8`（LP は stencil の advance・左寄せ）／⒡ ✅ 第567 が閉じた（全番号の右端＝最左の先頭音節 ink 左 − 1.0＝x-aligned-side ＋ `Stanza_number_align_engraver`・手書き双子で一致・網 `StanzaNumberAlignmentTests` 2 本。lead sheet は OWN のまま）／⒢ ✅ 第567 が閉じた（attached 行の baseline＝記号ごとの skyline ＋ 0.5 − ink 下端の max＝`RelatedStaffPadding`・双子で 0.013＝面項まで一致・網 `ChordNameStaffPaddingTests` 3 本・帯も 0.5・PagingSkylines の silhouette 腕も実 ink に＝台帳 `page.inline-chord.gap-first` −0.044541 に再ベース <!-- ledger: page.inline-chord.gap-first = -0.044541027 -->。⚠️ 残差: `chords` track の行（walk の腕）は chordnames.lys で LP より 1.0 低い＝walk 側の skyline）／⒣ `DynamicEngraver.TextOffsetInSpanner 0.6` は `scale-by-font-size` を 1 に畳む。
