...
最終更新 第301セッション＝**§2 U8 の「1 つの文」には*読み手が 4 つ*あって、第300 が閉じた phrase 参照はそのうち 2 つにしか届いていなかった＝ページに描かれている装飾音符が、鳴らず、export されていなかった**。**入り方は第298・第299・第300 と同じ**——**ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った**ので、**§1 の「次の一手＝残りの U8」を取りに行き、その §1 自身が命じている「着手する便は、まず自分の取る 1 つを測ること」を先にやった**。⇒ ★★★ **測ったら、取ろうとしていた 3 つではなく*前便が閉じたはずの項*が開いていた**——**規則は「見積りを測れ」だったが、測って出てきたのは*終わったことになっている項*のほうだった。** **commit 4 本**（**本体 `36988c3a` ／ §1・§2・RULES ／ 数を閉じる ／ 委任への答えと ⑻ の測定**）。 **掃きは 1 回・1754 冊・両側 Release・`svg --combined` ＋ `midi` ＋ `xml` ＋ `check`＝SVG 0 冊／MIDI 8 冊／XML 9 冊／`check` の差 0／svg レンダー失敗 2＝両 exe で同じ＝本便のものではない。** **動いた追跡本は 1 冊だけ**（`LilySharp.Tests/Fixtures/test/ossia-beams.lys`＝**XML のみ**）。**台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188／追跡 `.lys` 581／snapshot 231 はすべて不動**（**snapshot は書き換えも追加も削除も 0 枚**）。

⚠️⚠️⚠️ ★★★★ **⑴ 骨＝`GraceBodySupport` は「written once and read TWICE」と*書いてある*が、`grace { }` の本体を歩く walk は 4 本ある**: **ページ `MeasureCollector.CollectGraceNotes`／報告 `Semantics.GraceBodyValidator`／音 `Midi.MidiExporter.ProcessGrace`／MusicXML `MusicXml.MusicXmlExporter.ProcessGraceNotes`**。**第300 は前の 2 つに「phrase 参照は容れ物だ」を教え、後ろの 2 つは `grace.Body.Items` を自分で歩いたまま**だった。**実測**（`scratch/p301/ab`・`octave absolute` の対・`phrase G { d'16 e' }`）: **`grace { G } c'4 c'2.` の svg は inline 綴りと*バイト同一*（`data-pos` 伏せ・`2E4317760555`）、`.ly` 双子も同一（`\grace { d'16 e' }`）なのに、`.mid` は*装飾音符を 1 つも書かない本と*バイト同一（`F551A25F1260`・91B 対 inline 107B）、MusicXML は `<grace/>` が 0 個（inline は 2 個）。** ⇒ ★★★ **判定法**: **「1 つの文を N 人が読む」と書いたら、*その N を grep で数える***——**`GraceExpressionSyntax` を受け取る walk を数えれば 30 秒で 4 と出る**（**書いた便は 2 と書いた**）。★ **5 本目の `.ly` 双子 `EmitGrace` は narrowing を持たず原文を出し直すだけなので最初から正しかった**＝**「読み手」に数えるのは*絞る* walk だけ**（RULES §7.6 に汎化）。

⚠️⚠️ ★★★ **⑵ そして MusicXML は「装飾音符の既定音価」の*4 つ目の答え*を持っていた**——**`grace { c' } d'4` を `<type>quarter</type>` で書き出す**（**ページ・MIDI・`.ly` 双子は 8 分**）。**`ProcessGraceNotes` が主旋律の `_defaultDuration` を共有していた**ためで、**2026-08-01 が「3 つの答えを 1 つにした」と §1 に記録した*その質問*の 4 人目**（**当時の 3 つはページ 1/8・MIDI 1/32・双子 4 分**）。⚠️⚠️ ★★★ **共有は外へも漏れていた**: **追跡 fixture `test/ossia-beams.lys` は `d4@glissando grace { d8 } c` を 4/4 に書いており、export された小節は 3.5 拍だった**（**`c` が `d4` の 4 分ではなく grace の 8 分を継いでいた**）——**掃きが動かした唯一の追跡本がこれで、ページと MIDI は最初から正しかった。** ⇒ ★★ **「N 箇所を 1 つにした」と書いてある*記録*を読んだら、その N も数え直す。**

⚠️⚠️ ★★★ **⑶ 掃きの計器を作り直した＝第300 の `sweep300.ps1` をそのまま使っていたら「0 MOVED＝全部無事」と報告していた**。**あれは `svg --combined` と `check` しか比べない**が、**本便が触ったのは MIDI と MusicXML だけ**で、**ページが 1 冊も動かないのが*正解*だから。** ⇒ ★★★ **掃きが比べる出力は、その便の変更が*届きうる*出力でなければならない**——**「動かなかった」と「見ていない」は同じ顔で出る**。`sweep301.ps1` は 4 つ（svg・midi・xml・check）を取る（RULES §5.4 に汎化）。

⚠️⚠️ ★★★ **⑷ 同じ罠の*文字列*版を同じ便でもう 1 度踏んだ＝`lysc check` は `LYS4020` という綴りを刷らない**（**刷るのは `warning:` と本文だけで、コードが出るのは `lysc svg` の診断行**）。**最初の全数掃きは `-match 'LYS4020'` で 1744 冊 0 行を返した**——**7 分かけて。** ⇒ ★★★ **全数掃きは陽性対照 1 冊で先に鳴らしてから回す**（`audit/lpreg/grace-tie-probe.lys` は必ず 1 行出す＝**20 秒**）。**数え直した実測**: **`grace` 語を書く本 133 冊・LYS4020 は 59 行 / 43 冊**、**族別は annotation 20／beam 8／chord 7／slur 6／rest 5／tie 5／dynamic 4／phrase 参照 2／tuplet 2**。⇒ ★★★ **追跡されている本は 2 冊だけで、2 冊ともタイ**——**残り 41 冊は第293・第298・第300 が書いた scratch のプローブ**。**「残りの U8 で実コーパスに射程があるのはタイだけ」が見積りから実測になった。**

⚠️⚠️ ★★★ **⑸ 毒 9 本のうち 3 本が 0 赤で、3 本とも「網が無い」のほうだった**（`e_midinorestore`・`h_xmlnoframe`・`i_xmlnoanchor`）。**理由は 3 本とも同じ**: **本便の網が `octave absolute` の本ばかりで書かれていて、*借りた量が 1 度も非ゼロにならなかった***（**相対フレーム／参照に付けた `'` 印／可動 phrase の移調**）。★★ **MIDI 側の同じ 2 文（`b_midinoframe`・`c_midinoanchor`）は初回から赤**だったので、**「規則ではなく網に穴がある」と*その場で*言えた**＝**同じ規則を 2 人の読み手に書いたら、毒の表を*読み手どうしで横に*読む**（RULES §5.4 に汎化）。⇒ ★ **XML の 1 本は 2 本に割った**（fresh frame と anchor で赤集合が同一だったため）。**最終: 9 本すべて赤・赤集合はすべて相異**（`scratch/p301/poison-results*.txt`）。

⚠️⚠️ ★★★ **⑹ LP 実測＝`grace { }` の中の tuplet は*容れ物*で、和音・休符と同じ箱に入れたのはまた症状での仕分けだった**（`scratch/p301/lp`・`data-pos` 伏せ diff）。**`\grace { \tuplet 3/2 { d'16 e' f' } }` は `\grace { d'16 e' f' }` と*音符 3 つが座標までバイト同一*で、増えるのは斜体 serif の `3` だけ**（**梁が括弧の代わりをする**）。**音価を 4 分にして梁を外すと、増えるのは*括弧（`<line>` 4 本）と `3`*で、音符 3 つはやはり同一。** ⇒ ★★★ **tuplet を grace 本体で展開しても綴りは 1 つも増えない**（**phrase 参照と同じ**）——**増えるのは括弧と数字を描くときだけで、それは drop に残せる**。**和音（`<path>` +1＝符頭）と休符（`<path>` +1＝休符 glyph）だけが本当に grob を要る。**

★ **⑺ 直し方は「読み手ごとにフレームを持つ」**。**展開そのものは 1 綴り**（`GraceBodySupport.BodyElements`）で、**2 つのマーカー（`RelativeResetMarker` / `PhraseEndMarker`）が来たときに*その読み手が読む量だけ*を借りて返す**——**MIDI は鳴らす移調・絶対オクターブの基準・grace 群の音価記憶／MusicXML は相対フレーム・移調・絶対アンカー・grace 群の音価記憶**。**これは第300 ⑷ の規則「境界は*その読み手が読むもの*を戻す」がそのまま 2 人ぶん増えただけ**で、**新しい規則は 1 つも要らなかった。** ⚠️ **`Count > 0` の門はページの `ExitPhraseTranspose` と同じ形**（7.7 の「fallback で握りつぶす」ではない＝**発明ではなく 1 軒目の写し**）。⚠️ **7.5 の数え方は「`+` 200 行に REF 0 / OWN 0」**——**本便は量を 1 つも足しておらず、既存の家（`MeasureCollector.cs:3736` の `LILYSHARP-OWN:` の 8 分）を*指し直しただけ*＝7.6 の ⒟**（**住所を 2 つにしない**）。⚠️ **7.7 の自己監査が出したのは「phrase フレーム算術の 2 つ目の綴り」**（各 exporter の `ProcessNode` の参照腕 対 マーカー腕）——**畳めないので差分の網を張った**（`APhraseInAGraceBody_Hands*ChainBackAtItsAnchor` が両方の綴りに同じ問いを出す）。⚠️⚠️ **そして「MIDI の*主旋律*の phrase 展開には予算が無い」と書いたが、*測ったら MusicXML も同じで、しかも桁がひとつ大きかった*＝⑻。**

⚠️⚠️⚠️ ★★★★ **⑻ §1 を閉じたあとに 2 本目が在る＝ユーザーが委任を出した**（「次便は、このセッションでやる方が有利なら着手して。次のセッションでやった方が有利なら着手してはいけない」）。**答えは「残りの U8 には着手しない」**——**第297〜第300 と同じ答えだが、*理由が違う***。**あちらは「温まった場所と次の項の場所がずれている」**だったが、**本便はずれていない**（**次に安いと自分で測った tuplet の修理は `GraceBodySupport.Expand`＝一日中居た関数**）。**それでも着手しない理由は 3 つ**: ⒜ **閉じる費用はセッション単位で作業単位ではない**——**§1 は既に書き換えて commit 済み・両門緑・掃き済みなので、2 本目を開けると「半端に終わる」か「閉じる手順を 2 度払う」かのどちらか**（**第300 の「数を docs の前に閉じていたら2 度目の直しになっていた」と同じ形**）／⒝ **tuplet は着手前に測るべき数をまだ持っていない**——**LP の*絵*は測ったが、*MIDI で tuplet の比を `_tupletStack` に積むべきか*は 1 度も測っていない**（**本便が汎化したばかりの規則がそこで止まれと言っている**）／⒞ **引き継ぎは非対称ではない**（計器は全部ディスク・§1 と §2 から名指し済み・base worktree は 10 秒・競合する ▶ は ⒤ と ⒣ でどちらも「着手はユーザー決定から」）。
⇒ ★★ **代わりに取ったのは第300 と同じ「自分が 30 分前に書いた未測定の主張を測る」**——**⑺ の末尾に書いた「MIDI の主旋律の phrase 展開には予算が無い」。** ⇒ ⚠️⚠️ ★★★ **外れてはいなかったが、*狭かった***: **MusicXML も同じで、しかも桁がひとつ大きい。****実測**（`scratch/p301/budget`・倍々の phrase DAG `P(n) = P(n-1) P(n-1)`・**原文 26 行**）:
| 本 | 展開後の音符 | `svg` の `data-pos` | `xml` の `<note>` | `.mid` の note-on |
|---|---|---|---|---|
| dag14 | 16,384 | 20,992 | 16,384 | 16,384 |
| dag17 | 131,072 | **21,350**（打ち切り） | **131,072** | **131,072** |
| dag20 | 1,048,576 | **21,349**（打ち切り） | **1,048,576** | **1,048,577** |
（**`.mid` の数は生バイトの `0x90` 数え＝データバイトが 1 個混じるので ±1。`svg` と `xml` は要素の数え**）。**時間と大きさ**: **`svg` は 2.0→2.4 秒で*平ら***（**予算で打ち切るので 64 倍にしても増えない**）／**`midi` は 0.64→1.9 秒・147 KB→9.4 MB**／**`xml` は 0.66→4.2 秒・3.0 MB→**`192 MB`。
⇒ ★★★ **ページは `LYS1033` で「*絵*はここから打ち切られた」と*正しく*言う**——**そしてその文は絵についてしか言っていない**。**読み手はその警告を読んだあと、打ち切られていない 100 万音の MIDI を再生し、192 MB の MusicXML を書き出す。**⇒ ★★ **本便が grace 本体で直したのと同じ族が 1 段外側に在る**＝**「1 つの本にどれだけ音楽が在るか」で 4 つの出力が食い違っている**（§2 F ⒩ に起票した）。⚠️ **重さは低い**（**実コーパスに倍々 DAG は 1 冊も無い**）——**が、*「MIDI だけ」と書いた 30 分後に測ったら 2 つだった*のが本便 2 度目の「測らずに書いた」。**

⇒ ★★★ **次の一手＝残りの U8＝⒜ の和音・休符・tuplet ／ ⒞ 本体の中のスラーとタイ ／ ⒟ 注釈の全族**（§2 U8 の ▶）。**本便で「どれが安いか」が 2 つだけ測られた**: **tuplet は容れ物（展開だけで音符が出る・残る drop は括弧と数字）／タイは実コーパスの射程を持つ唯一の族（追跡 2 冊）**。**和音・休符・スラー・注釈は依然として見積りしか無い。** ⚠️⚠️ ★★★ **そして*どれを取るにせよ、読み手は 4 人いる***——**ページに grob を足す変更は、MIDI と MusicXML の narrowing がページと食い違ったままかどうかを毎回数えること**（**今も食い違っている**: **和音と休符は MIDI では 2026-07-10 から鳴っていて、ページと XML は落とす**）。⚠️ **和音・休符を `GraceNoteInfo` の中に彫らないこと**（§5.2.1②）。

★ **開始時裏取り**: HEAD `1e2e830d`・**未 push 25**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6654 / 0 / 4 / 6658**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2・`OPEN:` 0**・追跡 `.lys` **581 冊**・snapshot **231 枚**——**§1 の終了時の数と 1 つも違わなかった。** ★★ **WSL ubuntu Release も同じ 6654 / 0 / 4 / 6658**（**開始時に 1 回。CI は `11f48202` までしか走っておらず未 push 25 本は未実行なので、ubuntu の証拠はこれだけ**）。
終了時: **commit 4 本**（**本体 `36988c3a` ／ §1・§2・RULES `1f467e80` ／ 数を閉じる `8430b50e` ／ 委任への答えと ⑻ の測定（この commit）**）＝**未 push 29**（**この commit を含む。commit 3 本の時点で 28 だったのを確かめてから書いた**）・木 0・未追跡 0。**suite 6670 合格 / 0 失敗 / 4 skip / 合計 6674**（開始 6658 から **+16**＝**本便の網 16 ケース**）。Core 0 エラー 0 警告。**台帳・追跡コーパス・snapshot は開始時と同じ**（748／588／190／24.714570188／158 中 2／`OPEN:` 0／581 冊／231 枚）。
★★ **WSL ubuntu Release も同じ 6670 / 0 / 4 / 6674**（`1f467e80` の木で 1 回、⑻ の commit の木でもう 1 回＝**2 回とも同じ数**）。⚠️ **CI は `11f48202` までしか走っておらず、未 push 28 本は未実行。**
⚠️ **`C:\MyProj\lys-base301` の worktree はこの便の最後に消した**（`git worktree remove`）。**次便が同じ名前で建て直してよい。**
⚠️ **計器は全部ディスクに残っている**: `scratch/p301/` の `ab/`（A/B の本と出力）・`lp/`（LP 2.26.0 のプローブ）・`budget/`（⑻ の倍々 DAG）・`poison.py`＋`runpoison*.ps1`＋`poison-results*.txt`・`sweep301.ps1`＋`sweep.json`・`measurements.md`。⚠️ **`scratch/` は git 管理外。**
