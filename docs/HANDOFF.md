...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第236セッション＝**第235 が「次便が書くところから」と名指した `is-ancestor` 検査を建てたら、初回が*無毒で*赤になり、trailer 除去が 2 か所で未完だったことが出た**（6 commit＝番人 `HistoryCitationTests` 3 本＋workflow 2 本／番人自身の射程訂正／handoff＋分類器の訂正⑤／番人 `HandoffArchiveContinuityTests` 3 本／handoff＋床の更新／この行＝委任を断った triage ⑹。**engine 0 接触**）。⚠️ 3 本目は**単独では入れられない**——この §1 が引く tree の SHA を番人が dead に数えるので、散文と分類器は同じ commit に入る（RULES §5.1「依存があるなら同時投入」）。
**① 何を建てたか**：`HistoryCitationTests`——追跡下の `.md/.cs/.json/.ps1/.csv/.yml` から 7〜40 桁の hex を全部拾い、**1 回の `cat-file --batch-check`** で commit かどうかを訊き、**`rev-list <tip>` の集合**と突き合わせる（`merge-base --is-ancestor` の集合版・git 呼び出しは 3 回）。⑴ **他の ref に生かされている引用は赤**（本体）⑵ **live 引用のラチェット 508**（＝**547 → 503 の落下を「調査の入口」にしたはずの計器**）⑶ **dead の天井 469**（＝修理不能な旧引用の国勢調査。「今日書く引用は今日在る commit を指す」ので、**この数が増えるのは書き換えが貼り替えを取りこぼしたときだけ**）。⚠️ **git が使えないときは skip ではなく赤**——§0 の「無変更の増分ビルドは警告 0 を*自明に*報告する」と同型の偽の緑になるから。**そのため `ci.yml`／`release.yml` の suite を回す脚に `fetch-depth: 0` を足した**（`actions/checkout` の既定は shallow＝1 commit で、全引用が一斉に死んで見える）。
...
終了時: 本便 6 本（`f83a255c` 番人一式＋workflow／`67d87e1a` 番人の射程訂正／`669b82f0` handoff＋分類器の訂正／`d777c9a1` §7 3.5 の番人／`75093b53` handoff＋床の更新／この行＝断った triage ⑹）＝**ユーザーが本便の途中で `75093b53` まで push 済み・未 push はこの 1 本のみ**。⚠️⚠️ ★★★ **タグは origin でまだ旧い**（`git ls-remote --tags origin v0.3.0` が第1パスの tag object を返す・実測）——**`git push --force origin refs/tags/v0.3.0` が未実行**なので、**②の修復は origin では完了していない**。**clone し直すと番人がまた赤になる**（それが正しい振る舞い）。⚠️ **その旧 tag object の SHA はここに書かない**——**本便が prune で消した物**なので、綴った瞬間に dead 引用が 1 つ増える（**国勢調査が実際に赤くしてそれを教えた**＝⑤ と同じ形の 2 例目だが、こちらは**散文のほうが間違っている**側）・未追跡 0/木 0・suite **Windows 5806/0/4・WSL 5806/0/4＝両 OS 完全緑（開始比 +6）**・**引用 live 508／dead 469＝両機械一致**・snapshot **222 枚不動**・台帳 **566 点・ss 非ゼロ 110／総和 3.876038461・count 107／非ゼロ 2＝完全不動**・追跡コーパス **572 冊（1 冊も触っていない）**・Core 0 警告。⚠️ **加えて ref が 2 つ動いている**（`v0.3.0` の張り直し／WSL の `win/backup/*` 削除）——**commit ではないので `git status` にも `rev-list` にも出ない。**
...
- ⚠️ ★★★ **⑹ 本便の末尾でも同じ委任を受け、⑸ を triage して*断った*。数えた結果を置く**（第234 ⑸' と同じ形＝**次便が §2 を読み直さないため**。断ったこと自体ではなく、数えた結果が引継ぎの価値）。
  ...
  **⑶ ★★★ 入力の鮮度は*機械で*示せる**（第235 骨 2 への返答）——**本便の 5 commit は `LilySharp.Core`／`Cli`／`Lsp` を 1 ファイルも触っていない**（`git diff --stat a97aec3a..HEAD -- LilySharp.Core LilySharp.Cli LilySharp.Lsp` が空・実測）。⇒ **第235 が実コードで確かめた「⑵ 開いている」判定は構成上そのまま有効**なので、**次便は §2 の再 triage から始めなくてよい**（§0 の数は毎便どおり取り直す）。
