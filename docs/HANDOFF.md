...
**射程を 300 → 563 冊に広げた**）：**絵 0/563・data-pos 3/563**（**予測どおりちょうど**）。
...
- **⑵ ★★★ 第2便＝「every book」と名乗る網が 566 冊中 300 冊を読んでいた**（`ca7060b8`・
  **Core は 1 行も触っていない**）。`audit\lpreg`（257）と `samples`（6）を射程に入れて **563 冊**へ。
  **床の assert も 250 → 500**（コーパスのパスが動いたときに*黙って縮む*のを止める・§5.4 の空集合の罠）。
...
- **⑺ ★★★ §2F に残っているのは 9 つ**——**⑴ 一覧に構文を与えるか（決定であって作業ではない）
  ...
  ⚠️ ★★ **round trip は parser の不変条件に「近い」まま**——**563 冊で破れるのは 11 冊**、
...
- ✅✅ ★★★ **`lyrics` のハイフンが隣の空白を落とす件は第186 で閉じた**（`afb6b5be`）。
  ...
  **`EveryBook_SpellsItselfBackOutOfItsTree` の射程外**（`audit\lpreg` 257 ＋ `samples` 6）に
  居たから。**射程は `ca7060b8` で 300 → 563 冊に広げた**（床の assert も 250 → 500）。
