...
最終更新 第208セッション＝**エディタの色づけを言語に合わせた便**（**実装 2 便**。
**perf は測っていない＝彫版を 1 バイトも触っていないため**。§7.9 の「足していない例」）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **§1 ⑻ ⓪ と §2F のチケットが両方「着手前にユーザーに訊け」と
書いていたので訊いた。ユーザー決定は「⒜ エディタの色づけ」＋設問の答え「⒤」**（下の骨 1）。
**便の途中でユーザーがビルド＋デプロイして欠陥を 1 件報告し、それが第2便になった**（骨 3）。
**最後に「他の残債を見て判断して」と委任された**——§2 を通して読み、**着手せず起票した**（下の ⑹）。
⇒ ★★★ **0.3.0 を出すかは*まだ決まっていない*。次便も同じ設問から始まる。**

⚠️⚠️ ★★★ **第207 は §1 を 1 行も書かずに終わっている**（**12 commit・suite +50**）。
**§1 は第206 のままだった**ので、**開始時の裏取りで「未 push 32 のはずが 44」**という形で出た。
**第207 の経緯はこの §1 にも ARCHIVE にも無い**——**残っているのは 12 本の commit message だけ**
（`15eeea51`〜`6a0ca9d0`＝**`fonts` 改名・font 補完・そして §2F の監査チケット**）。
⇒ ★★ **数が合わない便が 1 つでも挟まると、次便は「間で push された」と「数え違い」を
区別できない**（§0 ★ の族）。**便を閉じるときは §1 を書くこと。**

★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 529 点・ss 非ゼロ 100・
総和 3.794108828・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 44**・
**snapshot 218 枚**・suite **5530 passed / 0 failed / 4 skipped**・**追跡コーパス 567 冊**
（ディスク 1176）・**Core 0 警告**＝**第207 の 12 commit を足せば引継ぎの数は全部合っていた**
（**37 回連続**。⚠️ **ただし「合っていた」と言えるのは commit を数え直したからで、§1 からではない**）。
⚠️ **`origin/master` は `9830873f`＝開始時と同じなので、便の途中の push は無かった**。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 47**（**この行を書く commit を含めて 47**）・
suite **5530 → 5536 passed / 0 failed / 4 skipped**（**+6＝新しい網 1 ファイル**）・
**台帳 529 点で不動**（ss 非ゼロ 100・総和 3.794108828・count 106/2 も不動）・
**snapshot 218 枚で不動＝中身も 1 枚も動いていない**・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Turmeric`）で通せた。**

> ## ★★★ この便の骨＝**「これは誤りだ」は文脈を持つ側の仕事**
> **ユーザー決定 ⒤**: TextMate の `invalid.illegal.removed-keyword` をやめ、誤りは LSP の診断へ一本化。
> **理由は 1 語で測れる**——**`fonts { volta "TeX Gyre Schola" }` は受理・`repeat volta 2` は LYS0006**。
> **1 行の正規表現に位置は無い。** ⇒ **文法は「その語が何か」だけを言う。**
> ⚠️ **これは begin/end の禁止ではない**（第2便がまさに begin/end を足した）——
> **禁じたのは「誤りの主張」で、文脈そのものは TextMate が正直にできること。**

> ## ★★★ 2 つ目の骨＝**網が両方向で欠陥を出した。そして「数え方」が数を作っていた**
> **⑴ 予約語 → 文法: 23 語が無色**（全部書ける・probe が exit 0）。
> **⑵ 文法 → 予約語: 6 語がキーワード色なのに予約語でない**（`part relative { }` が通る）。
> **うち `channel` は production・lexer・`.cs` のどこにも無い**——**言語が一度も持たなかった綴り。**
> ⇒ ★★★ **チケットの「18 語」は数え方の産物だった**。**第207 は 23→18 に「訂正」したが向きが逆**
> ——**ファイル全文を検索すると `"comment"` の散文中の語を数える**（`coda fine segno to with`）。
> ⇒ ★★★ **そして「一致する」は「色が付く」ではない**——**歌詞コネクタ `--|__|~|_` が
> `bass_8` の `_` に一致する**ので、部分一致で数えると 8va 音部記号 2 つを「色あり」と誤報する。
> **網は先頭から末尾まで覆うことを要求する。**

> ## ★★★ 3 つ目の骨＝**ユーザーがビルドして、網が*見られない*欠陥を報告した**
> **`fonts { }` の中で `serif`/`sans`/`barNumber` が無色なのに `tempo` だけ色が付く。**
> ⇒ ★★★ **網は「予約語」しか訊いていなかった。fonts の鍵は予約語ではない**
> （`part barNumber { }` が通る）——**`tempo` が色付きだったのは「別の理由で予約語だから」。**
> ⇒ **正典は `TextRoles.AllKeySpellings`。`#fonts-block` の begin/end 文脈で閉じた。**
> ⚠️ ★★ **そして毒が網の穴を 1 つ暴いた**——**`Patterns()` は JSON 全体を読んでいたので、
> *include されていない* repository 項目も「色あり」と数えていた。** **規則を書いて include を
> 忘れると全部緑。** **根から include をたどる形に直した**——**その毒（include だけ外す）が
> ユーザーの報告状態を 26 語ちょうどで再現した。**

★ **この便の値段**（**perf は測っていない**＝**彫版のコードを 1 行も触っていない**。
**動いたのはエディタの文法・LSP のトークン一覧・テスト・doc だけ。**）:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ `invalid` 廃止 ＋ 予約語 24 語 ＋ 逆方向 6 語 | 文法 1 ＋ LSP 1 ＋ 網 1 ＋ doc 2 | **追跡 567 冊で誤った赤は 0 冊**（下の ⒜） |
| ⑵ `fonts { }` の鍵 26 語 | 文法 1 ＋ 網 1 | **`fonts` を書く本すべて**（＝これから書かれる本） |

- **⑴ `cd9f55c4`**。**塗らないものも測って決めた**——**水準記号 7 語**（**素の `p` は refuse**＝`@p` と書く）
  と **`alternative`**（削除済み構文の中でしか届かない）は**理由付きの明示リスト**。
  **⒟ の 6 語のうち 5 語は `#directive-value` の 2 語規則へ**（`octave relative` のように
  *属する語と一緒に*塗る＝`#instrument-value` と同じ device）、**`channel` は削除。**
  - ★ **射程を数えた**（チケットが「優先度を決める前に数えろ」と書いていた未測定の数）:
    **追跡 567 冊のうち、コメントと文字列の外に素の `volta`/`alternative` を持つ本は 1 冊**
    （`audit/lpreg/voltagrace-probe.lys`）で、**それは死んだ綴り＝赤が正しかった。**
    **剥ぐ前は volta 9 冊 / alternative 2 冊**＝**チケットの数はコメント中の語。**
  - **`GRAMMAR.md` の「`volta`/`alternative` は削除済み構文を拒むためだけに予約」は
    `fonts` が入った時点で `volta` について嘘になっていた**ので直した。
  - **semantic tokens に `VoltaKeyword` を追加**——**対の片方しか居らず、semantic は TextMate に
    *重なる*ので、同じ規則から `alternative` はキーワード色・`volta` は赤で出ていた。**
- **⑵ `8f7620c6`**。**2 規則・順序に理由**（**鍵が総称 family を指す対を先に読む**）。
  ⚠️ **`sans-serif` は `sans` より前**——**TextMate は最初に一致した候補を採り、`\b` はハイフンの
  手前で成立する**ので、短い綴りが `sans` だけ塗って `-serif` を残す（`#instrument-value` の注記と同じ罠）。
  ⚠️ **`WordsPaintedAsKeywords()` は `begin` を持つ規則で止まるようにした**——
  **「予約語でない語をキーワード色にするな」は*文脈を持たない*規則についての主張**で、
  **文脈こそが「そこでだけ鍵」を塗る正当な手段**（直さなければ第2便で網自身が赤くなっていた）。
- **⑶ ★★ 毒は 5 つ当てた**（全部赤→全部戻した）: **`volta` を外す→1 語**／**`channel` を戻す→2 本**／
  **fonts の鍵を 1 つ外す→2 本**／**パーサが知らない鍵を足す→1 本**／
  **`#fonts-block` の include だけ外す→26 語**（＝ユーザーの報告状態）。
- **⑷ ★★★ この便が測って*着手しなかった*もの＝§2F の頭の新しいチケット。**
  **同じ形の文脈語彙があと 5 組・無色 17 語**（`SymbolCaseValidator` が正典）。
  ⚠️⚠️ **部分的に直すと「なぜこれは色が付いてこれは付かないのか」がそのまま残る**＝
  **ユーザーの報告そのものを再現する**ので、**1 つの語彙は全部いっぺんに。**
  ⚠️ **`swing`/`shuffle` は正規表現で持てない**（3 通り試して全部捨てた・理由はチケット）——
  **正直な置き場は LSP。**
- **⑸ ★ 道中で見つけた doc の欠陥**（チケットに書いた）: **`GRAMMAR.md` の `ClefName` は 5 語**だが
  **部品本体は 11 語を取る**。**一方で*音楽中*の `clef` は本当に 5 語**（**実測**）。
  **1 つの production が 2 つの位置を名乗っている。**
- **⑹ ★★★ 次に触るならここ**——**⓪ まず 0.3.0 の判断**（**4 便続けて未決**。
  **`Directory.Build.props` は `0.3.0`・タグは 1 つも無い・`v0.3.0` を push すると
  `.github/workflows/release.yml` が test → CLI 4 RID → VSIX を回す**）。
  **correctness の門は空のまま**（§2F の新チケットは**エディタのみ・門ではない**）。
  ⚠️ **出す前に片付ける候補は変わらず 2 つ**: **⒤ `CHANGELOG` が無い**
  （**リリースノート無しで出る**）／**⒥ 未 push 47 便ぶん**（**タグだけ先に push すると
  CI が古い木を建てる**ので **`master` を先に**）。
  ／**⒜ ★ 台帳点 `courtesy.key.key-to-line-end` の残差 0.15**（第206 が開いた。
  **閉じ方は `SharedRenderer` 自身が名指している**＝予約幅を描画幅にして `SolveColumns` に渡す）／
  **⒝ ★ ペダルの段間隔**（LP は 1.961 → 2.443・Lily# は一律 2.46。**台帳点は無い**）／
  **⒞ ★ `CoPlaceToCodaWithLabels` の鏡像ケース**／**⒟ §2 の頭の ▶ perf**（門の外）／
  ...
  **⒢ ★ §2G の `DrawingTransform.Identity` が単位変換でない**（**`new()` は record struct を
  全ゼロにするので `ScaleX/ScaleY = 0`・`Identity.IsIdentity` 自身が false**）。
  **第208 が読んで着手しなかった**——**製品コードは 1 箇所も読んでおらず**、
  **参照は「これを*使わない*理由」を書いた 2 つのテストのコメントだけ**（＝§2G の「作者を 2 人捕まえた」）。
  **1 行・出力不変・falsifier は `Assert.True(DrawingTransform.Identity.IsIdentity)`。**
...
> ## ▶ **部品本体の 2 つの値が検証されていない**（2026-08-19・第209 が測って起票・**要ユーザー判断**）
> **`part m { removeEmpty banana }` と `part m { lines banana }` は今日どちらも通る**（実測）。
> **前者は `RenderSpecParser` が `"true" or "all"` としか比べないので静かに `false`**、
> **後者は `1..5` の範囲外・非整数を静かに既定の 5 に読む。**
> **`SymbolCaseValidator` の switch に `removeEmpty` の枝も `lines` の枝も無い。**
> ⇒ ★★ **これは「片付け」ではなく決定**——**今日通る本を拒むようになる。**
> ⚠️ **ただし射程は測ってある: 追跡 567 冊に違反は 0 冊**（`removeEmpty` は `all` 2・`true` 14 のみ。
> **`false` は 1 冊も書いていない**）。**`lines` の実サイトは `lines 1` が 5 件**（他は全部コメント中の語）。
> ⇒ **閉じるなら 1 行**: 語彙は既に `SymbolCaseValidator.RemoveEmptyValueVocabulary` に在り、
> **`HaraKiriTests.TheRemoveEmptyVocabulary_IsWhatRenderSpecParserReads` が読み手と縛ってある**
> （第209 が §7.7 の「同じ量の 2 つ目の綴り」で自己監査して足した）。
> ⚠️ **`lines` の値は整数なので集合ではない**——**別の形の検証**（範囲つき `int.TryParse`）。

> ## ▶ **大小文字を無視するのは `removeEmpty` ただ 1 つ**（2026-08-19・第209 が測って起票・**要ユーザー判断**）
> ★★★ **これは第209 が 1 度書き間違えて、§7 のチェックリストで自分の記述を実測して直した項**——
> **最初「`transposition` だけ大小文字を無視する」と書いて commit した。逆だった。**
> **実測（部品本体・大文字始まりの値）**: `clef Treble` ✗／`tuning Guitar` ✗／`pedal Bracket` ✗／
> `instrument Violin` ✗／`transposition 8VB` ✗／**`removeEmpty TRUE` ○**。
> ⇒ ★★ **`RenderSpecParser` は `removeEmpty`・`pedal` を `.ToLowerInvariant()` で読むが、
> `pedal` のほうは `SymbolCaseValidator` が先に Ordinal で拒むので*その小文字化は発火しない***。
> **`removeEmpty` だけが素通りするのは「誰も検証していない」から**＝**上の項と同じ根**。
> ⇒ ★★★ **`transposition 8VB` が拒まれる理由は大小文字ではない**——
> **レクサーが `8` と `VB` に割り、`VB` を*次の property 名*として読む**
> （`Unknown part property 'VB'`＝**値について言うには紛らわしい文言**）。
> **対照は `8vb` で、こちらは 1 トークンとして通る。**
> ⇒ **`InstrumentDefaults.ParseTranspositionSemitones` の `ToLowerInvariant()` は到達不能**
> （**大文字綴りがそこへ届かない**）。**remarks にこの経緯を書いてある。**
> ⚠️ **閉じ方は 2 つあって向きが逆**: **⒜ `removeEmpty` を Ordinal にする**（今日通る本を拒む）／
> **⒝ 誤解を招くエラー文言だけ直す**（`transposition` の値位置で property 名の話をしない）。
> **⒝ は今日通る本を 1 冊も拒まないので、⒜ と切り離して先に閉じられる。**
