...
- ★★ **⒩⁵ 「一度も埋まらない器」＝会計に*出ない*族**（第442 起票）。`ElementCoordinator` の
  tie outline は 1 回につき **4 本の List を建てて 0.00・0.00・0.21・0.04 件しか入れない**
  （`:1785 others`・`:1843 dots`・`:1864 flag`・`:1880 accidentals`）。★ **第442 の脚が
  `others` は 26,524 回すべて空だと*証明した***（tied == positions.Count）。**14.35 回／打鍵 ×
  空の List object 32 B ＝ 約 1,837 B／打鍵 0.028%** で、**計器は容量しか見ないので A/B にしか出ない**
...
### 1.1 第442セッション（2026-09-20・YT-DELL2）

`/clear` 直後の新セッション。指示は「**HANDOFF を読んで着手**」で、**着手先は §1.0 ⒜ の ⒩‴**（ユーザーが
選んだ・⒩‴ と ⒫ と ⒨ と LP 双子を並べて訊いた）。★ **`-Start p442` の 1 コマンドで §0 が全部済んだ**
（HEAD `f8fd0869`・**未 push 69**・**full `sessions/p442/run1.trx` 8774 / 0 / 3 / 8777＝第441 最終と一致**・
台帳 851 点／総和 22.584727806・snapshot 249・追跡 `.lys` 609・§7 3.5 の `-Archive 440` も自動で回って
88 行 5,282 chars）。★ **裏取りは 1 つも赤を出さなかった**（CI の赤は `origin/master` が `9f5cd52a` の
ままだから＝第412〜第441 が予告したもの）。道具は pwsh MCP（Bash 0 回）。母集団と harness は第414〜第441 と
同じ（実コーパス 231 冊 × 8 **forward** 打鍵・`RenderIncrementalPages`・**Release**・割当バイト・**両側 TC=0**）で、
**before 6,476,385 / 6,476,400 は第441 最終の 6,476,370 と 15〜30 B 違い**。
⇒ **「頭の無い島」を起票どおり 0.1% 級の束で刈った。25 サイト・4 つの算術・1 便。**

★★★ **⑴ 「直す前に寸法を訊く」脚を先に立てた**（第440 ⑸ の規則。Lab `Zz442S.cs.txt`／`Zz442Leg.cs.txt`）。
**15 サイト中 13 が `asked == Count`・UNDER 0・over 0**。★★★ **上限のつもりで入れた 5 つ**
（cursors ×2・`tied`・`beamByMember`・`placed`）**が 5 つとも Count と一致した**——`placed` は「どの呼び手も
幅 0 の item を渡さない」、`beamByMember` は「(小節,item) が衝突しない」、`tied` は「tie column の head は
全部 tied」。⚠️ **残る 2 つのうち `toStore` は*自分で自分を反証*した**＝asked 23.69 に対し count **1.05**
（maxslack 47）で、**既定の階段のほうが安い**＝直さない。`hits` だけ slack 1.05／回で採用。
⇒ ★★ **`hits` と `toStore` は*補集合*だから、両方に `parts.Count` を渡すと partition を二重に予約する。**

★★★ **⑵ 直しは 4 つの算術で 25 サイト**（Core **+116 −31 行**・**LP 由来の式 0・REF 0／OWN 0**。
**新しい幾何の数は 0**）:
⒜ **ループの周回数**＝★★★ **`LayoutEngine.LayoutSystems` の 12 本**（1 周につき 12 本すべてに 1 件ずつ
書き、早期脱出が無い＝寸法は `systemMeasures.Count`。**これ 1 つで 6,919 B／打鍵**）・`ElementCoordinator:327`
の X 表（枝ごとに自分の歩きの周回数）・`OutsideStaffStacker:1355` の `staves`（群の staff 数の**和**）・
`MultiStaffLayouter:2813` の `flat`（**min の和**）・`ElementCoordinator:1759/1760`。
⒝ **既に建っている `Count`**＝prelim の beams／ties の `perSystem` と `cursors`、`columnSystem`。
⒞ **上限**＝`beamByMember`・`placed`・`hits`。
⒟ **観測**＝`ArticulationEngraver:397` と `Prelim:494` の**容量 1**（計器が読んだ 1.01／1.00 件・max 2）。

★★★ **⑶ A/B ＝ −0.441%**（**6,476,392 → 6,447,804 B／打鍵**・before はこの便で取り直した）。
**231 冊中 231 冊が改善**（中央値 −0.410%・最良 −1.191%）。⚠️ **対照帯 −0.037%〜+0.026% の外に出たのは
230 冊**＝`Universe.lys` 1 冊だけが帯の中（**−0.003%**）＝**予測 P4「231 冊すべてが帯の外」は外れ**。

★★★ **⑷ 会計は 180,021 → 150,948 B／打鍵（−29,073）**＝**実装前の予測 28,700 ± 600 の内側**。
**A/B の −28,588 との差 485 B は会計が*上限側*に立つから**（Dictionary/HashSet の素数への切り上げと、
観測した容量の 1% の尾）＝**第440 ⑹ と同じ向き**（第441 は器を消したので逆だった）。★ **回数は動かない**
——触っていない **210 サイトで回／打鍵も件／回も 1 つも動かず・GONE 0**。

★★★ **⑸ 出力同一は 2 つとも**: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（baseline `17fb650e`）／
⑵ **実コーパス全ページ SHA-256＝5,824 行・0 行差**（baseline は第441 が健全と示した `p439/hashes-after.txt`）。

★★★ **⑹ 毒 7 つ——色は 6 つ当たり、1 つは*毒のほうが壊れていた*。**（素の木が赤 3 本＝継ぎ目だけの
状態で回したので**差分で読む**。全文 Lab `poisons.txt`、規則は RULES §5.4 の 3 本）:
⑴ ★★★ **`a` の寸法 −1 → +2 赤。だが毒が測ったのは容量ではない**——system を 1 つも組まない本で
容量が −1 になり `ArgumentOutOfRangeException`。**床を 0 に張ると +0 緑**＝「容量は正しさではない」6 例目／
⑵ **⒟ の容量 1 → 4 → +0 緑**（7 例目）／⑶ ★★★ **同じ 1 行の*どちらを壊すか*で色が変わった**——
**`b` の*容量*を逆の枝から → +0 緑**（短い容量はただ伸びる）**／`b` の*歩き*を逆の枝に → +16 赤**
（X 表はすぐ下で添え字として読まれる）。⚠️ **予測 P7⑶ は前者を「赤」と書いていて*書いた時点で誤り***
＝容量と歩きを言い分けていなかった／⑷ **`c` の数える歩きが何も足さない → +0 緑**（第441 ⑹⑷ の裏返し）／
⑸ **`k` の tied と untied を入れ替える → +33 赤**＝**網は tie outline そのものを見ている**＝
他が緑なのは*容量*の性質で、網が鈍いからではない。

★ **⑺ 終了時**: commit 2 本（code `1fd3c20e`・docs 1 本＝この §1 と RULES §5.3 ×2・§5.4 ×3。
**この文を含むので SHA は書かない**＝§5.4）・**未 push 71**（開始時 69）・**最終 full
`sessions/p442/run3.trx` 8774 / 0 / 3 / 8777＝第441 最終と同じ**（網は 1 本も足していない。
`Zz442` の grep は**コードに 0 件**）・§7.5（対 `f8fd0869`）**Core `+` 116 −31 行／REF 0／OWN 0**・
台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・**棚卸しは再生成して
両方とも*行番号だけ***（`magic_constants.csv` は Line 列を伏せると 928 行が完全一致・code の commit に同梱）。
計器と全文は Lab `sessions/p442/`（`prediction.txt`＝実装前の予測・`Zz442S/Leg.cs.txt`＝⑴ の脚・
`sized-sites.txt`・`instrument.ps1` 一式＝⑷ の値付け・`counts-compare.ps1`・`ab-*`・`hashes-after.txt`・
`poisons.ps1`／`poison1b.ps1`・`zz442-fix.diff.txt`）。
⚠️ **毒スクリプトの書き戻しは BOM を落とす**＝**毒のあとは `git status` を読む**（RULES §5.4）。
⚠️ **§2・§3 はもう畳めない**（`Fold-ClosedHandoffItems.ps1` が 0 件）＝**天井を空けるのは §1 だけ**。
第442 は §1.0 ⒞ の 9 便ぶんの宣言を 1 表に畳んで 1.1 KB 空けたが、**1 便が §1 に使う量（約 9 KB）と
`-Archive` が空ける量（約 8 KB）はもう釣り合っていない**＝**次便以降は §1.1 を短く書く**。
⇒ ⚠️⚠️ **次便は `-Archive 441` を回すまで §1 に 1 字も足せない**。
⇒ **次便は `-Start p443` の 1 コマンドから入る**。⚠️ **`origin/master` の CI は赤のまま**＝第412〜第442 の
