...
### ▶ 次の一手 ＝ **verse 間を「定数」から「規則」にする（⚠️ 構造変更・出力が変わる）**

`lyrics.verse-step` が **+0.400000 で開いている**。⚠️ **定数の差し替えでは閉じない**——
LP の 2.8 は **ideal 0 の下の MINIMUM** なので、**平坦な 2.8 は平坦な 3.2 と同じくらい間違い**に
...
- ⚠️ 実装後は **3 箇所（`LyricEngraver`／`MultiStaffLayouter`／`SharedRenderer`）と
  `LayoutEngine` の見積り**が全部追随する必要がある。見積りは `c5c90192` で
  **配置側を読むようになった**ので、規則化するときも**そこ 1 箇所を直せば伝わる**。
...
| 点 | 残差 | 正体 |
|---|---|---|
| `lyrics.verse-step` | **+0.400000** | ★ **この回開いた。犯人も直し方も確定済＝上の ▶。** 定数でなく**規則**の移植が要る |
