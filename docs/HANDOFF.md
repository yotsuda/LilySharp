...
最終更新 第399セッション（2026-09-18・YT-DELL2）＝`/clear` 直後の新セッション。§0 の裏取りから入り（HEAD `0dca0eae`・未 push 15・**full `sessions/p399/run1.trx` 8640 / 0 / 3 / 8643＝第398 第 4 便の数と一致**・CI は直近 3 run が赤＝未 push のまま・build 0 エラー・`Session-Check -Build -Test` はこの PC で 40 秒）、ユーザー指示「このセッションが有利なら着手」→ 文脈が空なので次のセッションと同条件＝着手し、**§2 R13⒜ の *collect の側* を閉じた**（`8a3b7f68`）。道具は pwsh MCP（Bash は 0 回）。**骨は 4**:

★★★ **⑴ 実体＝診断 pass が preview の直後に同じ本をもう 1 回 full collect していた**（`PublishDiagnostics` → `DocumentDiagnostics` → `SemanticValidation.Run` → `TryCollect`＝resume 無し・`WalkProbe` 無し）。**直し＝貸す**: ⒜ `SemanticValidation.Run(tree, token, previewCollect)` の overload（shared Lazy が先に供給者へ訊き、null なら従来の `TryCollect`）／⒝ `IncrementalCompiler.CollectFor(tree)`＝最後の compile の collector（**full でも resumed でも**・`CollectWithResume` が `out` で返す・`LentCollect` record を Volatile で丸ごと差し替え）を「**同じ tree（instance か text）かつ*先頭* render block**」のときだけ貸す／⒞ LSP `DocumentDiagnostics(Document, token, out lentByPreview)` が document の**全 slot** の session に訊く（**gate は取らない**＝session 参照は丸ごと差し替え・貸した collector は以後その session に触られない＝次の compile は新しい collector に集めて旧を resume の *source* として読むだけ）。**preview の pass は `SvgQuietMs` 待ちで診断より先に走る設計なので、落ち着いた打鍵では貸せる。**

★★★ **⑵ R13′（`FindFirst`＝先頭 score／preview＝選択 score）は *panel の契約側* で固定**: 名前付き render の session は貸さない（LYS4019 の remarks「asked of ONE score — the first」）。preview が非既定 score を出している間は従来どおり自前 collect＝出力は変わらない。**`Choose` が名前不一致で先頭へ fallback したときは先頭 block なので貸す**（`FirstBlock = ReferenceEquals(spec, specs[0])`）。

★★★ **⑶ resumed collect を貸せる根拠（先に読んでから書いた）**: validator が読む表は全部 (a) `MeasureCollector.CumulativeSideTables` の一員で resume が source から**コピー**で採用する（tie／slur／beam／cue boundary／`_musicMarks`＝LYS4019 と span pairing／chordName items／navPlacement／pitchTrace）か、(b) post-walk scanner が live で作り直す（`_unpairedRepeatWarnings` は意図的に表の外・lyrics・tab・midBarBreak・chordRowGrid）。**source は読まれるだけで変異しない**（`MeasureCollector.Resume.cs:235-244`・`:441-459`）。**網 `PreviewCollectSharingTests` 5 本**: 差分ネット（`NetBooks` 258 冊 × `SyntheticEdits`＝2344 edits・planned 1259・**resumed 491**・bail 99・警告を持つ本 16／edits 35＝LYS4001×31・4010×7・4007×2・床は resumed≥100／books≥8・`SyntheticEdits` を internal に）／session 単体（警告を*先頭 2 小節*に置き末尾を編集→ `AdoptedMeasures≥30` で**採用側にある警告**が fresh と等しい＝コーパスが薄い場所を 1 本でピン）／LSP 2 本（貸す・tree が動いたら断る・名前付き render は断る・既定 session が出来たら貸す）／比較の毒（stale collect は位置が 1 ずれて不一致＝比較は位置を見ている）。

★★ **⑷ 数**: perf-plain1k の panel pass min-of-5 **fresh 139 ms → lent 98 ms・`TryCollect` 単体 36 ms**（時間は参考・**回数は 2 collect → 1**）。**残る 98 ms は validator ~50 本の全木 walk＝R13⒜ の後半（未着手）**。計器は `sessions/p399/Zz399Probe.cs.txt`（差分ネットの計数＋timing・repo からは消した）。**最終 full `sessions/p399/run3.trx` 8645 / 0 / 3 / 8648**（+5・run2 は docs 前の同数）・§7.5（対 `0dca0eae`・スクリプトの出力）**Core `+` 90 行／REF 0／OWN 0**（数値定数なし・LP 由来の式なし＝監査対象は「貸したものが同じ collect か」だけ＝⑶ の網）・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 608・CHANGELOG 0.8.0 に `### Editor` を新設して 1 項。§7 3.5 は済（第397 を `-Archive 397`＝17 行 6,129 chars）。**触っていない**: `BarCountFix` と `CheckCandidate` の `SemanticValidation.Run(tree)`（自前 collect のまま・打鍵経路ではない）・CLI `check`。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**push はユーザー**（Lab も）。⇒ **次の一手**: R13⒜ の後半（validator の全木 walk ~50 本を 1 walk に畳む）／R13⒝〜⒤／LP 双子が要る R7〜R11／R15（dead code・要承認）。

★★★ **第 2 便（同日・同じ指示「有利なら着手」→ 経路を読み込んだ直後なので着手）＝R13⒜ の後半を*計器で割ってから* 1 本閉じた**（`51ee54a8`）。**「~50 本の全木 walk」は起票の予測で、実測は違った**（§5.0「起票の難所は予測」）: validator ごとの min-of-5（perf-plain1k・`sessions/p399/Zz399Probe2.cs.txt`）＝**合計 110 ms のうち `LyricPlainVerseShadowedValidator` 1 本が 46 ms**＝歌詞の無い本でも**自前で 3 本目の full collect**（primary voice のみ・「shared collect は no-voice で数え損なう」という 2026-08-16 より前の註が根拠）／`MeasureValidator` 15／`SymbolReference` 5／`LyricSings` 5（DescendantNodes ×3＋Conflicts）／残り ~25 本は 1 ms ずつ（＝全木 walk 1 本 ≈ 1 ms・畳んでも 25 ms）。**直し＝`ISharedCollectValidator` にして shared collect の `LyricShadowedPlainWarnings` を読む**（`Reset` は pass 頭に 1 回で staff ごとではないので表は全 staff 分たまる・source position で dedup）。**射程は先に数えた**（`Zz399Probe3.cs.txt`＝旧 validator 対 shared・fixtures 258＋Lab corpora 501＝759 冊・歌詞あり 92）: **759 冊で一致・警告は両側 0**＝出力は動かない。**意図した広がり 1 つ**＝旧は*先頭 part* しか見なかったが shared は score が描く全 staff＝2 段目の shadowed verse を新たに報告（網 `AShadowedVerseUnderTheSecondStaff_IsFlagged`・⚠️ part 名 `alto` は clef の予約語で parse が落ちる＝`harmony` に）。毒＝`ItReadsTheLentCollect_NotOneOfItsOwn`（shadowed な本の collect を歌詞の無い木に貸すと警告がついてくる＝自前 collect をしていない証明）。**後**: 110 → **59 ms**（fingbeam1k 505 → 294・v2bow1k 82 → 56）。**最終 full `sessions/p399/run4.trx` 8647 / 0 / 3 / 8650**（+2）・§7.5（対 `8a3b7f68`）**Core `+` 30 行／REF 0／OWN 0**・台帳・snapshot 不動・CHANGELOG の Editor 項に 1 文追記。**触っていない＝次便の候補**: ⒜ `NavigationPlacementValidator` は同じ形（**宣言 part ごとに** 1 本ずつ自前 collect）だが「nav mark 無し→即 return」の門の裏＝mark を持つ本では part 数 × collect。**ただし旧の答えは score が描かない part の mark も含む**＝shared に載せ替えると undrawn part の警告が消える＝**判断が要る**（LYS4019 の「描かない part」の扱いと揃えるか）。⒝ `MeasureValidator` 15 ms＝R12⒠ の第 2 模型（`MeasureModel`）そのもの。⒞ `SymbolReference`＝全 node を List に写して 2 pass・`LyricSings`＝3 walk → 各 1 walk に畳める（合わせて ~5 ms）。⒟ 残り 25 本 × 1 ms は「1 walk に畳む」大手術の値打ちが 25 ms＝後回し。

★★ **第 3 便（同日・同じ指示）＝コードは 1 行も変えず、次便候補 ⒜ を*数で*閉じた**（`sessions/p399/Zz399Probe5.cs.txt`＝759 冊の census・min-of-3）: **legacy `NavigationMark`（`segno`／`coda`／`ds` …の裸綴り）を持つ本は 759 冊中 20 冊**・part 数は最大 2・**score が描かない part の mark は 0**・**旧（宣言 part ごとの自前 collect）と shared collect の答えは 20 冊全部一致**（警告を持つ本 3／3）・旧の費用は最大 3.7 ms（`残酷な天使のテーゼ`）で 20 冊合計 14 ms。⇒ **`NavigationPlacementValidator` は触らない**: 門（mark 無し→即 return）の裏の費用は実在せず、載せ替えは「描かない part の警告を消す」判断だけを要る＝値打ちが無い。`@segno` 等（`MusicMark`）は別経路（form が置く）で、この validator の射程ではない（`MusicWalk.cs:1363-1387` が `NavigationMarkSyntax` だけを記録）。**walk 畳み（`LyricSings` 4 walk・`SymbolReference` の List 化＋2 pass＋`ReferenceTokens`／`Tracks`）も見送り**: 合わせて ~5 ms（plain1k）で、`LyricBindings.Conflicts`／`PartReferenceFinder` の *2 綴り目*を validator に作ることになる（§7.7「同じ量の 2 つ目の綴り」）。⇒ **R13⒜ の残りは `MeasureValidator` 15 ms＝R12⒠（第 2 模型）と、25 本 × 1 ms の全木 walk だけ**＝どちらも「この便で」の有利は無い。full は回していない（docs のみ・継ぎ目のテスト 3 本は緑）。

## 以下は第398セッションの経緯

最終更新 第398セッション（2026-09-17・YT-DELL2）＝§0 の裏取りから入り（HEAD `f03bc076`・未 push 5・**full `sessions/p398/run1.trx` 8626 / 0 / 4 / 8630＝第397 の数と一致**・CI は直近 3 run が赤＝天井 `c8cb106a` が未 push のまま・build 0 エラー）、**§2 R の R2（MusicXML が section header の `partial` を落とす・header だけの section で空の `<part>`）を閉じた**（`764aa9b6`）。道具は pwsh MCP。**途中でユーザーが origin を merge した**（`9c96b4bc`＝README 1 行・以後 HEAD はその上）。**骨は 5**:

★★★ **⑴ 出だしの罠 2 つ**: ⒜ **Bash を 4 回使ってしまった**（HANDOFF §0 を読む前。Lab notes §1 の規則を読んでから pwsh MCP に切り替えた）＝**この PC の `~/.claude/settings.json` の deny は `PowerShell` だけで `Bash` が無い**。足そうとしたら harness の分類器が「Self-Modification」で拒否した＝**ユーザーが手で `"Bash"` を deny に足す**（Lab notes §3 チェックリスト ⑴）。⒝ `dotnet build LilySharp.sln` は無い＝**`LilySharp.slnx`**（RULES §6 のとおり）。

★★★★ **⑵ R2 の実体は「header を*宣言単位*で読んでいた」**: `EmitSection` は手元の宣言の直接の子から `key`/`time` だけ拾い、`partial` は誰も読まず、**part-major の本では header（`section A { partial 8 }`）と cell（`part m { section A {…} }`）が*別の宣言*なので header の key/time も cell に届いていなかった**（プローブ `sessions/p398/probes/xml-partial/header-key.lys`＝`key d major time 3/4` が fifths 0・4/4 で出た）。しかも header 宣言そのものを music として emit し、score が 2 譜なら `_bareSectionOwner` が無いので **"Part 1" に空の `<part/>`**（`two-staves.lys`＝3 parts）。**直し＝ページ・MIDI・双子と同じ形**: 名前鍵の registry（`BuildSectionHeaderRegistry`＝inline music の無い宣言の最初の `key`/`time`/`tempo`/`partial`・先勝ち）→ `EmitSection` が名前で読む → `EmitPartMusic` が collector の順（time・tempo・key・partial）で全 part の play に当てる → **top-level の directive だけの宣言（`IsHeaderOnly`＝双子の「a header is exactly what it turns away」の線）は何も emit しない**。`ArmPickup` が music 中の `partial` と header の `partial` の 1 軒。

★★★ **⑶ 網は差分＋読み手横断**（`MusicXmlSectionHeaderTests` 6 本＋`MusicXmlTests` 1 本・**最初の 4 本は base で 4/4 赤**＝Core を stash して実測）: header 綴り＝inline 綴り（measure の XML が列で等しい）／2 譜の本に "Part 1" が無く両 part の 1 小節目が implicit 0／key・time が名前で届く／tempo（曲頭なら opening tempo・途中なら section 頭の direction）／空の cell は header ではない／途中の pickup は番号を保つ。

★★★ **⑷ 掃きが 2 つの誤りを出した（`sweep-xml.ps1`＝`lysc xml` 938 冊・base `exe-base`＝`f03bc076`）**: 1 周目 31 冊 moved のうち **`test.lys`（`section A { tempo 110 }` が 120 に）と `さよならエレジー コード譜.lys`（`part bl { section Body { } }` の空 cell が header 扱いで part ごと消え `<part-list />`）**＝**tempo を registry の 4 本目に**（ページの `_sectionHeaderTempos` と同じ）・**`IsHeaderOnly` は top-level のみ**。⇒ 2 周目 **moved 29 / 938**＝28 冊は header pickup が effective になった（bar 0 implicit・うち 3 冊は pickup が独立 measure に割れてページの小節数と一致: `lysc layout` 3/68/113＝xml 3/68/113）・1 冊は `がくふ.lys` の空 "Part 1" が消えた。**§5.0「対の食い違いが第 2 の欠陥を出す」が 2 つ**: ⒜ **pickup 上のスラー開始が消える**（`星になれたら` の `dis,4( | d2.)`）＝`MaybeClosePickup` が音価が満ちた瞬間に measure を閉じ、その後に歩く `(` が空の measure を見る。tie の腕は `_lastEmittedNotes` を読んで落とさない＝**slur の腕も同じ記録を読む**（inline `partial 4 g'4( | …)` で base 0/1 → head 1/1）。⒝ **途中の `partial` が番号を 0 から振り直していた**（inline の腕は常に Number 0・`_measureNumber = 1`）＝ページは*先頭*の pickup だけ bar 0（`Measure.IsPickup`・`LayoutEngine.Annotations` の offset）なので **途中は implicit のまま番号を保つ**。

★ **⑸ 終了時**: commit 1 本＋この §1・**最終 full `sessions/p398/run3.trx` 8633 / 0 / 4 / 8637**（+7）・§7.5（対 `f03bc076`・スクリプトの出力）**Core `+` 153 行／REF 1（`partial` の既存 REF が `ArmPickup` へ移動・新しい数値定数なし）／OWN 0**・台帳 851 点／総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 608・§7 3.5 は済（第396 を `-Archive 396`＝15 行 4,391 chars）。**push はユーザー**（2026-09-17 の指示のまま・Lab も）。**残る道具**: `LilySharp-Lab/sessions/p398/`＝`exe-base/`（`f03bc076` の Debug lysc・**xml 以外は HEAD と同一**）・`sweep/sweep-xml.ps1`（`-Side base|head|compare`・`*-xml/` は Lab の gitignore に足した）・`probes/xml-partial/`（陽性対照 5 冊＝chord-flag／two-staves／header-key／pickup-slur、base/head の xml）・`commit-r2.txt`。**作業ツリーに残したもの＝ユーザーの未 commit `tools/Deploy-Lsp.ps1`・`tools/Package-And-Install.ps1`**（触っていない）。**触っていない**: MIDI は header だけの bare section を宣言ごと鳴らす読み（registry に tempo なし）・空 cell の part は measure 0 本のまま（`さよならエレジー`・従来どおり）。⇒ **次の一手**: §2 R の上から＝R4（`voice {} {}` 越しの音価記憶）／R13⒜（診断の collect 共有）。LP 双子が要る R7〜R11 もこの PC で進められる。

★★★ **第 2 便（同日・R1 の説明をユーザーに返したら 3 つの決定が返った）＝§2 R1 を*言語の側で*閉じた**（`54fe39fb`・§3 の最上段）。⑴ **LYS0006 の文面と名前**: 「削除した」体（`RepeatVoltaRemoved`）は不正＝`repeat volta` は Lily# に在ったことがない LilyPond の綴り。`LilyPondRepeatVolta` に改名し文面は「'repeat volta' is LilyPond's spelling. In Lily# a repeat is written in the form: …」（番号は LYS0006 のまま＝GRAMMAR_AUDIT §1.1 の「LP の綴りを書いた人への案内」）。⑵ **`volta` を予約語から外した**（ユーザー「外してもよいと思うけどどうかな」→ 私も賛成）: 生きた綴り `fonts { volta "…" }` は fonts key を text で読むので予約は要らず、`repeat` の kind も text で読む（`unfold`/`percent`/`tremolo` は元から識別子）＝`VoltaKeyword` 撤去。⑶ **`alternative` 節をパーサから消した**（ユーザー「後方互換な警告も完全に削除して普通の文法エラーで十分」）＝`AlternativeKeyword`／`AlternativeClause`／`AlternativeClauseGreen`・`Syntax`／`ParseAlternativeClause`・MIDI と collector の腕を撤去、`RepeatExpression` は 4 slot。**R1 の「4 読み手の食い違い」「MIDI が最後の ending」は読むものが無くなって問いごと消えた。** 実測（`sessions/p398/probes/alternative/`）: `repeat unfold 2 {…} alternative { {…} {…} }` は base「No errors found」→ head「'{' is not something a music block can hold」×3＋閉じ括弧の拒否／`part volta`・`part alternative` は base「reserved word」→ head「No errors found」。射程: この機械の 27,000 冊にこの 2 語を識別子で使う本は 0・`alternative {` を書く `.lys` も 0（コメント外の当たり 7 行は全部文字列・score 名・`voltagrace-probe.lys` の `repeat volta`＝LYS0006 のまま）＝**出力は 1 バイトも動かない**（掃きなし）。触った文書: GRAMMAR（Keyword production と註）・SYNTAX_REFERENCE（予約語表・fonts key の註）・GRAMMAR_FOR_LLM・tmLanguage（keyword.other から `volta` を落とす・`keywords` の註は「refused」）・semantic tokens・CHANGELOG 0.8.0 Breaking changes・APPROXIMATIONS.md 再生成（MidiExporter の行番号 1 つ）。網: `ParserTests` 3 本を言い直し（with-alternative の case は 2 語が識別子に lex され `RepeatExpression` が 4 slot であることを主張）・`EditorColouringTests`（`alternative` を `ReservedOnlyToRefuse` から外し、⑹ の実測語は `partial` に替え **`volta` が予約語表にも塗る語にも*無い*ことを主張**）・`ReservedWordsTests` の表・`CompletionInsertsValidSyntaxTests` のコード集合。**赤の証拠**: 直す前の full `run5.trx` 8630 / 3 / 4 / 8637（`ReservedWordsTests` の volta／alternative の 2 case＋inventory の行ずれ）。**最終 full `sessions/p398/run6.trx` 8631 / 0 / 4 / 8635**（−2＝theory の 2 case）・§7.5（対 `764aa9b6`）**Core `+` 42 行／REF 0／OWN 0**（対 `f03bc076` では 195／1／0）・台帳・snapshot 不動。⚠️ **`LanguageReference` の `repeat` の help（「volta is the symbolic |: :| form」）は触っていない**（文面は今も真）。

★★★ **第 3 便（同日・ユーザー指示「このセッションが有利なら着手」→ exe-base と掃きの道具が揃っているので着手）＝§2 R4 を閉じた**（`0c25c5d2`）。**規則＝octave frame と同じ「span は何も動かさない」を音価の既定にも当てる**（2026-08-01 のユーザー決定の延長として置き、同日ユーザーが「あなたの判断を残す方が一貫して分かりやすい」と確認＝§3 の最上段）: 全 branch は span が開いた時点の (Duration, Dots) から読み、**span の後ろの音楽もそこから読む**。直す前は 4 読み手が 4 通り＝page は extra voice を fresh Quarter で開き voice 0 の末尾を後ろへ漏らす・MIDI は branch を span の値で開き*最後の* branch の末尾を漏らす・XML は page と同じ Quarter・twin は fresh buffer。**twin で R14 の twin 項を踏んだ**: branch の先頭に音価を強制して書かせたら、nested buffer（tuplet／cue／repeat）が `_lastWrittenValue/Dots/_forceNextDuration` を運ばないせいで `test/multivoice-voice2-tuplet` が `\tuplet 3/2 { e'8 f' g' } a'4` になった（page は 8 分）＝**`CarryFrameInto/Back` で往復させ**、grace は stream の値を戻す（Lily# は grace の値を持ち出さない）・**phrase 参照の body は従来どおり fresh "4"＝byte 不変・正しいかは未判定（R14 に残した）**。**掃き 938 冊×4 出力**（`sweep-all.ps1`・base `exe-base2`＝`4fbf65c1`）: **svg 0／xml 0／midi 0**（母集団に extra branch の先頭を bare で書く本は「4 分と一致する本」しか無い）・**ly 4**＝`collision.lys`・`voiceissue.lys` は branch 先頭の明示化、**`Locked out of Heaven`・`STILL LOVE HER` は twin の*修正***（`\repeat percent 2 { f,,8 … } c,4 c` → `c, c`＝page は 8 分・base の twin は古い "4" を強制していた）。**網 `VoiceSpanOutputsTests` 2 本**（読みを分ける本＝span は 8 分で開き・voice 0 は 2 分で終わり・最後の branch は 4 分で終わる→ 4 読み手で読む／bare 綴り＝明示綴りの差分）・**base Core で 2/2 赤**（stash）。**最終 full `sessions/p398/run10.trx` 8633 / 0 / 4 / 8637**（+2）・§7.5（対 `4fbf65c1`）**Core `+` 92 行／REF 0／OWN 0**・台帳・snapshot 不動・APPROXIMATIONS.md 再生成（行番号のみ）。⇒ 第 3 便の終わりの次の一手は R13⒜／R14 の残り／R7〜R11 だった（第 4 便が R14 を閉じた）。

★★★ **第 4 便（2026-09-18・同じ指示「有利なら着手」→ 3 exporter と 4 出力の掃きが手元にあるので着手）＝§2 R14 の残り 6 項を閉じた**（`76cfb9eb`）。**全部プローブで先に実測**（`sessions/p398/probes/r14/`・MIDI は捨て計器 `Zz398ProbeTests.cs.txt` で dump）: ⑴ **MIDI の歌詞 meta は 1 つも出ていなかった**（parser が syllable node を作るようになった後も `SyntaxTokenNode` だけを読んでいた）→ block を queue に積み、section の音が揃ってから onset に乗せる（`AttachLyrics`＝XML と同じ規則: 和音は 1 onset・grace／休符／tie 継続は歌わない・歌詞の `|` は次の小節の最初の onset へ・melisma は onset を消費）。`MidiNote.IsGrace` を足した。⑵ **`tempo 2 = 60` を MIDI は 60 qbpm・XML は `beat-unit quarter`／`sound tempo="60"` で出していた**→ `TempoValue.QuarterBpm` の 1 軒（MIDI は µs/quarter＝**切り捨てのまま**＝最初 `Math.Round` にしたら 157 冊の tempo meta が 1 µs 動いた・XML は `<beat-unit>`＋`<beat-unit-dot/>`＋`sound tempo` は 4 分換算）。⑶ **`c4 r4 ~ c4`**: 休符の後の `~` が休符の前の音を延ばして後ろの音を飲み込んでいた（960 tick 1 音）→ `ForgetOnset`。⑷ **grace の steal が lane を跨いだ**（次 part の最初の音が 453 tick・section が 27 短い）→ lane（part block・voice branch・part-major の copy）の入口で 0。末尾 grace の steal は捨てる。⑸ **`time 3+2/8`**: LP 2.26.0 は `\time 3+2/8` を syntax error・`\compoundMeter` は「unknown command」・**`\time #'((3 2) . 8)` が通る**（`cm1`〜`cm3.ly`）→ twin は pair で書く（REF `music-functions-init.ly:2375-2410 time, sane-time-signature?`）。⑹ **phrase body の音価**: ページは fresh frame（4 分・`EnterDefaultFrame`）で開くのに twin は "4"・force なしで LP に lexical に読ませていた（`c'8 G` → G が 8 分）→ 先頭を強制・末尾の値は持ち出す（R14 の twin 項はこれで全部閉じた）。**網 `ExportTimingAndMemoryTests` 7 本**（各 assert の横にプローブの旧答を書いた）・`MidiTests.ExportWithLyrics`（Phase 8 の skip）は退役。**掃き（base `exe-base3`＝`a9ae4540`・`midi-diff.ps1`＝.mid のイベント差分読み）**: svg は触っていない・**xml 1**（`tempo-beat-units`）・**ly 0**・**midi 16**＝15 冊が歌詞 meta を得た＋`tempo-beat-units`（1=40→160・2=60→120・4.=80→120・8=240→120 qbpm）。tie-over-rest と lane 跨ぎは母集団に 0＝網だけが観測者。**最終 full `sessions/p398/run15.trx` 8640 / 0 / 3 / 8643**（+7・skip −1）・§7.5（対 `a9ae4540`）**Core `+` 205 行／REF 1／OWN 0**・台帳・snapshot 不動・APPROXIMATIONS.md 再生成（行番号のみ）・**CHANGELOG 0.8.0 に本便 4 脚の MIDI／XML／twin の項を足した**。**残したもの**: 歌詞の小節同期は section 末の拍子と header pickup で数える（section 途中の拍子変更でずれる）・歌詞 meta は最初の part track に載る・tick 0 に既定 120 と本の tempo の 2 event。⇒ **次の一手**: §2 R の残り＝R13⒜（診断の collect 共有）／LP 双子が要る R7〜R11／R15（dead code・要承認）。HEAD＝この §1 の commit・未 push 15（LilySharp）・Lab 8。
