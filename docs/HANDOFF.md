...
最終更新 2026-07-28（第26セッション・**何セッションも BLOCKED だった `Skyline::distance` の
当たり点を LP に訊いて割り、移植を書いた（ただし未 commit）**）/ HEAD・ahead 数は §0 で確認すること
（⚠️ **ここに数字を書かない**——自己参照で、書いた瞬間から commit のたびに嘘になる）。
⚠️ **未 push が溜まっている**（第21セッション末から。push はユーザー・§5.1）。
**commit 済 HEAD は 3463 passed / 0 failed / 3 skipped**・Core 0 warn 0 err。

⛔⛔ **ワーキングツリーに未 commit の移植がある**（第26セッション末・**ユーザー判断で保留中**）。
`git status` の M 印（生成器・生成物・`VerticalSkyline`・`SkylineBuilder`・
`StaffSkylineFrameTests`）が **clef のアウトライン移植**で、**意図した 5 件だけが赤**
（台帳 3 点＋snapshot 2 枚）。**捨てないこと**——`git stash` も避ける。
何をどう測ったかは下の第26セッション節。⚠️ **ユーザーの指示は「先に台帳点を作ってから landing」**。
（未追跡の `HANDOFF-*.md` 14 本はセッション前からのもの＝§8。**この移植とは無関係。**）
...
### 第26セッション（2026-07-28）＝ **BLOCKED を割った：clef の箱とシルエットは別物**

**commit 1 本**（`7fdace7f`・**測定のみ・出力不変**）＋**未 commit の移植 1 本**。

★★★ **`lyrics.*.staff-to-lyric` の閉じ方 −0.105961 は「LP を instrument するまで割れない」と
何セッションも書かれていたが、LP は Scheme から自分で答える。**
`ly:skyline-distance` / `ly:skyline-touching-point` / `ly:skyline->points` / `ly:skyline-merge`
が全部 `lily/skyline-scheme.cc:26-216` に生えている。新しいプローブ
`audit/lp-geometry/probes/skyline-binding.ly` が `align-interface.cc:228` の当の対を直接叩く:

| | |
|---|---|
| LP の答え | `dist=7.210038725633767` `touch=2.188` `hdn=-3.446039` `hup=3.764000` |
| 素朴な max どうしの和 | `3.540000 + 3.776000 = 7.316000` ⇒ **deficit −0.105961** |
| 予測（実行前に書いた） | `4.972149 + 3.737890 − 1.500000 = 7.210039` ⇒ **15 桁で当たり** |
| 対照 PLAIN（歌詞を外す） | **15 桁で同値** ⇒ **歌詞は 1 ミリも入っていない** |

★★ **正体は「グリフの skyline はアウトラインの箱ではなくアウトラインの多角形」**。
`stencil-integral.cc:562` → `freetype.cc:174-202` は cubic を `max(2,|end-start|/0.2)` で
折り、輪郭の向きで UP/DOWN に振り分ける。**G clef は最深が x=1.84・最高が x=2.228 で別の x**
なので、2 枚が向き合うと和の最大はその間（x=2.188）で取られる。
⇒ ★ **`6c6be1af` はアウトラインの「箱」を移植した**。箱は `max_height` を正しく再現するので
**1 枚のプロファイルに当たる読みは全部合い、2 枚の pointwise 比較だけが合わない**（§5.3 へ汎化）。

**未 commit の移植の中身**（`audit/scripts/Extract-EmmentalerSkylines.py` が臨時記号の**横**
スカイラインで**同じ平坦化を既に実装していた**ので、横軸を X に一般化して clef G/F/C を足しただけ）:
`VerticalSkyline.FromGlyphOutline` ＋ `SkylineBuilder.SeedClef`（箱→シルエット）。
**生成器の出力は LP の dump と全頂点が六桁一致**（定数オフセット x0=0.800000 のみ）。

**測った値段**（§5.0「島を『何枚動く』で見積もらない」の実践）:

- 台帳 **3 点**（`lyrics.{between-staves,chord-row,ossia}.staff-to-lyric`）が
  **同一値 4.299161339 へ同一量 0.105961339 だけ動き**、residual は
  **+0.165349 → +0.271310** ＝ **予測どおり歌詞書体の項だけが残った**（§5.3 の「打ち消しの解除」）。
  ⚠️ **これは後退ではない。** 3 冊が同値のままなのは LP 側の恒等の再現。
- snapshot **2 枚だけ**（`test/hara-kiri` −0.890000・`test/ossia` −0.530000。どちらもページが縮む）。
  ★ **持ち主を摂動で割った**: hara-kiri は **UP 側**・当たり x は **clef の右の尾**（グリフ枠 x>2.06）
  ＝**箱が 3.776 を予約していたのにインクが 1 つも無い領域**。ossia は **DOWN 側**。
  消費者は **`LayoutEngine.cs:965` の system 間 padded スカイライン距離**
  （`SystemSkylineHorizontalPadding` = 1.0）＝**単一ページ経路**（§2D の「自前で積む」やつ）。
  ⚠️ **ページ高は split に対して連続に動く**（0.80→736.2 / 0.90→738.8 / 0.92→739.0 で飽和）
  ＝ padding の 45° 斜面と当たっている証拠で、**段差ではない**。

### ★ 保留中の移植を §5.2 片手に読み直した結果（§7.5・**やった。1 件出て直した**）

**Core の実コード ~161 行に `LILYPOND-REF` 8 本 ／ `LILYSHARP-OWN` 1 本**（残り ~492 行は生成データ）。
追加コードの数値リテラルは **clef の譜位置だけ**（出典 `scm/parser-clef.scm`）で、実測の貼り付けはゼロ。
byte 一致のための分岐・除外もゼロ。**字面から外れているのは次の 3 つで、全部宣言済み**:

1. ~~**`bbox / real_bbox` の比を計算していない**~~ — **直した**。`stencil-integral.cc:551-557` は
   **毎回この比で割っている**のに生成器は掛けておらず、**「ここでは必ず 1 になる」を値に畳んだ**形だった
   （§5.2 が名指す型そのもの）。fontTools で両方の箱が出せる——**`bbox` は FreeType の
   グリフメトリクス**（`freetype.cc:51-65` が `FT_LOAD_NO_SCALE` で読む＝CFF ではコントロール
   ボックス＝`ControlBoundsPen`）／**`real_bbox` は輪郭の真の bbox**（`freetype.cc:67-88` の
   `FT_Outline_Get_BBox`＝`BoundsPen`）——ので**割り算に置き換えた**。
   **焼き込む 8 グリフ全部で厳密に 1.0**・**臨時記号 9 配列は HEAD と完全一致**＝**出力不変**。
   1.0 を外れたら **stderr に NOTE** が出る。⚠️ 残る推測は **「CFF では FreeType のメトリクスが
   コントロールボックス由来」だけ**で、これは LP の字面ではなく FreeType の実装からの推測。
2. ⏳ **量子化数がフレーム依存なのに焼き込みは全サイズ 1 種類**（`freetype.cc:128-150` は
   `max(2,|end-start|/0.2)` を **transform 後**の長さで数えるので、ossia では本数が変わる）。
   ⇒ **本当の直し方は「折った線分」ではなく**輪郭パス（font units の制御点）**を焼き、
   平坦化・向き分類をレイアウト時に C# で行う**こと。そうすれば乖離は回避でなく**消える**。
   値段は C# に `Path_interpreter` 相当 100〜150 行＋全 clef の再照合、データはむしろ小さくなる。
   ⚠️ **landing してから**（1 島 1 関心。今やると承認ゲートの対象が倍になる）。実害は ossia の
   clef の量子化誤差だけ。
3. ⏳ **clef の X アンカーが近似のまま**（`systemLeft + ClefGlyphXOffset`）。
   ...
   ⇒ ★ **▶0 で作る台帳点と同じ島**（行頭・clef・system 間）。**ついでに直さない。点で測る。**

（**第25セッションの節は落とした**。commit `2feb6021`/`c2955ba3`/`6c6be1af`/`94705160` と
台帳の `why` に全部ある。**原則は §5 へ汎化済み。**）

### ▶ 次の一手

★★★ **推奨順（第26セッション末に棚卸し）**

0. ★★★ **点を開く値段を下げる**（**ユーザー判断・2026-07-28＝次セッションでこれに取り組む**）。
   ⚠️ **単独の道具作りとしてやらない。下の 1 番（landing を止めている点）を最初の客にする。**
   道具だけ作ると使われないまま腐り、点だけ作ると次も同じ値段を払う。**同じセッションで両方。**

   **なぜこれが律速か**（第26セッションの結論・§1 の残差内訳）: **機構の乖離はコーパスが
   測っている範囲ではほぼ閉じており、残りはテキストメトリクスと「点が 1 つも無い regime」。**
   ▶ の項目が軒並み「台帳点が無いので先に点を作る」で止まっているのはそのため。
   **今回も、移植は書けているのに点が無いという理由だけで landing できていない。**

   **実測した値段**（2026-07-28）:
   - `page-vertical.ly` は **book 62 冊**（1 冊が 120 小節 ×480 音節）で **111 点**を支える。
     `Measure-LilyPondPageGeometry.ps1` は**全冊走らせて 20 分以上**・出力は最後にまとめて出る・
     **MCP のツールタイムアウト 170 秒では絶対に完走しない**（§5.5）。
     ⇒ **点を 1 つ足すたびに、以後の再測定が全部 20 分になる。**
   - 対して **専用プローブ 1 ファイル**（`skyline-binding.ly`・4 冊）を
     `cmd /c "lilypond -dbackend=svg -o <work> <probe.ly> > out 2> err < NUL"` で直に走らせると
     **約 80 秒**＝**1 ツールコールに収まる**。第26セッションはこれで 3 回まわして BLOCKED を割った。

   ★★★ **決定的な事実（見落とされていた）: 台帳は `probe` を*エントリ単位*で持っており、
   既に 6 ファイルに分散している**（`page-vertical.ly` 111／`barline-spacing.ly` 51／
   `staffless-system.ly` 8／`line-start-mindist.ly` 5／他 2）。
   ⇒ **新しい点は `page-vertical.ly` に入れなくてよい。** `\book` は互いに独立なので、
   **専用プローブに置いた点は 80 秒で再測定でき、既存 111 点の 20 分を一切引きずらない。**

   **やること 3 つ**:
   ⑴ **単一プローブ runner**（`audit/lp-geometry/Measure-LilyPondProbe.ps1`・~30 行）＝
     第26セッションが手で打っていたものを一級にする。デタッチ（`< NUL`）と
     `PROBE*` 行の抽出まで。⚠️ 既存 2 本の Measure-* を壊さない（別スクリプト）。
   ⑵ ~~規律を §5.0 に足す~~ — **済（第26セッション）**。「新しい対は専用プローブで固める・
     `page-vertical.ly` には原則追加しない」を §5.0 に入れた。⑴ ができたら**そこから参照するだけ**。
   ⑶ ~~§5.5 の「book が 40 冊」は stale~~ — **済（62 冊に訂正）。**
   ⇒ **残っているのは実質 ⑴ だけ**（~30 行）。**道具作りに時間を使わず、下の 1 番へ進むこと。**

   ⇒ ★ **最初の客は下の 1 番**。新しい pair を専用プローブに書き、80 秒で測り、
   点を起票し、保留中の移植を landing させる——**そこまでを 1 セッションで通す。**

1. ⛔ **未 commit の移植を landing させる**（上の道具の最初の客）。
   ⚠️ **ユーザー判断（2026-07-28）＝「先に台帳点を作ってから landing」**（§5.2.1③ どおり）。
   **名指せる台帳キーは 3 つある**（`lyrics.{between-staves,chord-row,ossia}.staff-to-lyric`・
   residual を +0.165349 → **+0.271310** に更新する）が、**snapshot 2 枚が踏む regime の点が無い**。
   ⇒ **足りないのは 1 点だけ**: **system 間のばねが「padded スカイライン最小」に座り、
   当たりが行頭の clef である配置**（`LayoutEngine.cs:965`・horizon padding 1.0）。
   ★ **設計に要る材料は全部そろっている**——持ち主も当たり x も上のセッション節で名指し済み。
   ⚠️ **対の組み方**: 恒等の対が最強（§5.0）。**同じ音楽で、行頭 clef の「尾」の x に
   相手のインクが在る本／無い本**を組めば、LP 側の差がそのまま量になる。
   ★★ **LP 側の字面は確認済（第26セッション末）＝ Lily# の形は忠実で、あとは regime だけ**:
   **LILYPOND-REF: `lily/page-layout-problem.cc:625-632` `ensure_min_distance`** ——
   `up_skyline.distance(bottom_skyline_, skyline-horizontal-padding) + padding` を
   **system-system ばねの床**にする。⇒ `LayoutEngine.cs:965` と同じ量で、
   **どちらも system 全体のスカイライン**（`build_system_skyline`）どうしの距離。
   ⚠️ **だから点の条件は 1 つだけ**: **その床が binding していること。**
   ⚠️★ **既存の点は 3 つとも使えない。理由まで書いておく（再走査しない）**:
   - `system.clef-bounded-distance`（book S・単一譜）＝ **clef どうしは 3.540+3.776=7.316 しか
     稼げず、`+padding 1` でも ideal の 12 に届かない**。⇒ **床は原理的に binding しない。**
     ⇒ ★ **単一譜では作れない**（clef だけで 11 を超えられない）。**多段譜が要る**——
     system 全体のスカイライン距離は system の高さを含むので、2 譜あれば床は自然に binding する。
   - `system.stretched-distance.two-staff`（JSS）＝ 伸長 regime でばねが最小から離れている（§5.0 罠 7）。
   - `system.compressed-distance.two-staff`（JSK）＝ **移植で動かなかった**ので、
     この本では**当たりが clef の領域に無い**（音符が決めている）。
   ⇒ ★ **要るのは「2 譜・床が binding・かつ当たりが行頭の clef」の本**。
   ⚠️ **設計したら「自分がその regime に居る」ことを点で assert すること**（§5.0 罠 7・§5.4）——
   床に座っているかは **`(ideal 12) < 実測距離`** で判定できる。
   ⇒ 点が出来たら: snapshot 2 枚を再ベース＋台帳 3 点を更新＋**1 commit**。**承認は取得済み**だが
   **再ベースの commit message に上の台帳キーを名指すこと**（§5.2.1③・§7-6）。
   ⚠️ 移植そのものは**もう書いてある**（`git status` の M 印）。**書き直さない。**
   ⚠️ **§7.5 の読み直しも済んでいる**（上の節）。**landing のときに繰延 ⑵⑶ を混ぜないこと。**
2. ★ **歌詞の up-extent を書体から読む**——**残る最大の族**（+0.271310 が 5 点以上。
   **上の移植が入ると 3 点がここへ合流して更に大きくなる**）。
   ⚠️ **着手前にユーザー判断が要る**: TeX Gyre Schola に CJK が無く、一括で移すと
   かな音節の up-extent が 0 になる（実測で差し戻し済）。**CJK の受け皿を決めるのが先**
   （CJK 書体を持つか／非 Latin は表に落とす分岐か）。**コードより先に決定。**
3. **保留中の移植が残した繰延 2 件**（第26セッションの §7.5 節）＝
   **⑵ 平坦化をレイアウト時へ**（ossia の量子化・landing 後）／
   **⑶ clef の X アンカー**（1 番で作る点と同じ島なので、その点で測る）。
4. **annotation pass の magnification**（第25セッションが名指しで残した「1 フレーム外の同じ問い」）。
   ⚠️ **`TupletBracketLayout` は `MeasureIndex` しか持たず譜の帰属が無い**ので、
   **annotation layout に譜を持たせるモデル変更が本体**。出力は動かない見込み＝**テストが網**。
5. **棚卸し ⑶（walk がペアごと・本丸）**／**▶ の silhouette**／**§2D 未移植 ⑵⑶** — **全部「台帳点が先」**。
   発火する regime（譜と譜の間に loose line）を作る本を起票するのが最初の作業。
   ⇒ ★ **0 番の道具が効いてくるのはここから先**——この 3 件はどれも「点が無い」で止まっている。
...
ほとんど書体量である**（**|residual| 総和 2.606965**・count は別勘定＝§3 の決定。
⚠️ **符号付きで足すと 2.509046 になり、これは別の数**——負の残差 −0.048960 の 2 倍だけ小さい。
**総和を引用するときはどちらを足したか言うこと**）:

| 残る族 | 量 | 種類 |
|---|---|---|
| **歌詞書体（up-extent 1.459200 対 1.187880）** | **+0.271310** ×5 ＝ **1.36**（＋ `staff-to-lyric` 3 点と `two-verse.staff-staff-inside` の中にも） | **フォント**。⚠️ **0 にしない**（CJK の受け皿が先） |
| `lyrics.*.system-gap` | +0.207200 ×2 ＝ **0.41** | **フォント**＋小節番号 cap-height |
| `lyrics.*.staff-to-lyric` ×3 | +0.165349 ×3 ＝ **0.50** | **保留中の移植が入ると +0.271310 ×3 になり、丸ごと上の「歌詞書体」へ合流する** |
| `barnumber.*.staff-to-baseline` | −0.024440 ×2 | **フォント**（数字のインク下端 対 baseline） |
| `between-staves.two-verse.staff-staff-inside` の内 **0.012826** | 0.013 | **機構**（歌詞 descent の書体差 0.010956 込み）＝ 実質ほぼ唯一の残り |
...
**歌詞書体の +0.271310 は 9 エントリに現れる**（5 点は素で、3 点は −0.105961 と相殺された
+0.165349 として、1 点は +0.284136 の中に）。⇒ **総和 2.606965 のうち 2.1 以上がこの 1 つの量。**
⇒ ★★ **コーパスが測っている範囲では、レイアウト機構の乖離はほぼ閉じている**——
**機構として名前が付いたまま残っているのは 0.012826 の 1 件だけ**（＋保留中の移植が消す
0.105961 ×3。⚠️ **それは今は総和を*下げて*いるので、移植が入ると総和は上がる**・§5.3）。
残っているのは**テキストメトリクスと、点が 1 つも無い regime**。
⚠️ **だから次の投資先は「もっと移植する」ではなく「点を増やす」**——▶ の項目が
軒並み「台帳点が無いので先に点を作る」で止まっているのは、それが**律速だから**。
