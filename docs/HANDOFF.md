...
最終更新 2026-07-28（第25セッション・**代理表現 A を閉じ、ossia のインクが縦スカイラインで
scale されていない欠陥を摘出して移植した**）/ HEAD・ahead 数は §0 で確認すること
（⚠️ **ここに数字を書かない**——自己参照で、書いた瞬間から commit のたびに嘘になる）。
⚠️ **未 push が溜まっている**（第21セッション末から。push はユーザー・§5.1）。
**3462 passed / 0 failed / 3 skipped**・Core 0 warn 0 err・
**LP 忠実度 140/177 exact（＋3）・total |residual| 2.606964 ss / 151 distances・counts 24/26**。
⚠️★★ **総和は上がったが後退ではない**——12 点が 1e-7 まで閉じ（うち 3 点 exact）、
**上がったのは lyrics 3 点だけ**で、そこは**打ち消しが解けた**結果（§1 のセッション節）。
⇒ **総和ではなく「残ったものが 1 方向の名前付き量か」で読む**という §1 の原則が、
**初めて総和と逆を向いた回。**
...
### 第25セッション（2026-07-28） ＝ **代理表現 A を閉じた。値でなく規則を測る網とセットで**

**commit 1 本**: `2feb6021`（Core **+89 / −75**・テスト **+46**・`LILYPOND-REF` **+4**・
**snapshot も台帳も 1 つも動かない**）。§7.5 は同じ commit で走らせた。

- ★★ **予測どおり byte 不変だったが、それは「結果」であって「構成」ではない**（§5.2 の裏面を
  commit message に明記）。**除外を足していない**——今日 `IsTextRow` と
  `StaffAffinity != null` が**同じ集合を指す**から動かないだけ。設定するのは
  `Staff.CreateTextRow`（DOWN）と `MeasureCollector` の lyrics 反転（UP）の 2 か所だけで、
  **どちらも text row にしか立たない**。⇒ **言語で綴れるスコアには両者を割る配置が無い。**
- ★★ **綴りは 3 つではなく 5 つだった**（引き継ぎは「ClassifySystem と他 2 つ」と書いていた）。
  `StaffSprings`（model 直読み）／`ClassifySystem`／`BuildStaffAnchorTables` の **2 か所**
  （`lastSpaceableGroup` と `lastSpaceableStaffY`）／`ComputeBetweenStavesEnd`。
  ⇒ ★ **「N 綴りある」と引き継がれた数も stale になる**（§5.3 の「引き継がれた数は測り直す」）。
  **数える前に grep する。**
- ★ **障害は引き継ぎの読みどおり 1 つ**（`StaffLayout` が property を運んでいない）で、
  **`Staff.StaffAffinity` は最初から在り、`Staff.cs` が自分で is_spaceable を引いていた**。
  `StaffLayout` に `int? StaffAffinity` を足し、**構築 8 か所**で model から運ぶ。
- ★★ **網は「値」ではなく「規則」でしか書けない**（§5.4）。snapshot も台帳も
  コーパス距離も**定義により無力**（出力が同一）。書いたのは
  `ALineIsNonSpaceableBecauseItDeclaresAnAffinity_NotBecauseOfItsKind`
  ＝ **普通の音楽譜に affinity UP を立てて、ばねが消えることを要求する**。
  **旧述語を戻して落ちることを実証済**（control 側は通り、affinity 側だけ落ちる）。
  ⚠️ **この配置は model からしか作れない。それが代理表現が生き延びた理由そのもの**——
  **コーパスが届かない場所に住んでいた。**
- ★ **移植で空になった引数はその場で落とした**（§5.2.1⑥）: `textRowStaves` が
  **7 つのシグネチャ**と `AnnotationLayoutContext.TextRowStaves` から消えた。
  集合自体は**読み手 1 つ**で生き残る（**別の問い**＝どの `ChordName` が独立行に乗っているか）。

**同じセッションの 2 本目**（**測定だけ・Core 差分ゼロ**）＝
**「top ばねの床」は 1 個の量ではなく 2 個だった**。台帳の 2 点の `why` と §1 を訂正した。

- ★★ **引き継がれた読みが誤りだった**（§5.3「引き継がれた数は測り直す」の**理由版**）。
  台帳は「⚠️ もう ossia の量ではない／1 個の量が 2 点に出ている」と書いていたが、
  **摂動で割ると 2 個**（⑴ ossia のインクが scale されていない ⑵ clef の字形 0.024000）。
- ★★ **一番効いた読みは「同じであってはならない数が同じ」だった**: Lily# は
  **両本とも力 0.214493**（九桁一致）を解く。LP は **0.212184 / 0.213548**。
  ⇒ **ossia の小さいインクがページに 1 度も届いていない**——残差ではなく**恒等の破れ**が住所を出した。
  ⚠️ **この形は「摂動して動かない」の親戚**（§5.3）で、**動かないのが 2 冊の差のほう**。
- ★ **摂動は 2 回やった**: 1 回目（`ClefG.Top` 4.8→6.0）は**両本とも圧縮 regime を出てしまい**
  （force が符号反転）読めなかった（§5.0 罠 7）。**+0.05 に落として regime を保った**のが 2 回目。
  ⇒ ★ **摂動幅は「効果が見える最小」ではなく「regime を出ない最大」で決める。**

**同じセッションの 5 本目**: `6c6be1af`（**glyph skyline＝アウトライン・ユーザー承認済**・
**snapshot 74 枚**・`LILYPOND-REF` **+74**）。§2C の clef 族が**丸ごと閉じた**。

- ★★★ **12 点が 1e-7 まで閉じ、3 点が exact**（ossia の頭 +0.024000・足 +0.010000 ×2）。
  **clef sliver 族**（`page.clef.first-staff-refpoint` −8.3e-5／`system.clef-bounded-distance`
  −8.3e-4）も、**`system.stretched-distance` の「未説明の 0.005＝フォント量」も消えた**
  ——⚠️ **後者は符頭ではなく clef だった**（符頭の skyline は**宣言により extent**）。
- ★★★ ⚠️ **一般則ではなく grob ごとの宣言**。危うく一律に当てるところだった——
  **LP の dump が止めた**（`NOTEHEAD ext=0.545 skyline=0.545` なのに outline は 0.544）。
  ⇒ ★ **「1 つの規則を見つけた」と思ったら、それが宣言で分岐していないか先に見る**（§5.2 へ汎化）。
- ⚠️ **総和は 2.557670 → 2.606964 と上がった**が、**上がったのは lyrics 3 点だけ**で
  **打ち消しの解除**（§5.3 へ汎化）。**exact は 137 → 140。**

**同じセッションの 3 本目**: `c2955ba3`（**ossia のインクを scale・ユーザー承認済**・
Core **+116 / −**・`LILYPOND-REF` **+3**・`LILYSHARP-OWN` **+1**／**snapshot 3 枚**）。

- ★★ **落ちたのは 6 件だけ**（台帳 3 点・snapshot 3 枚）で、**対の control 側は 1 つも動かない**。
  `page.ossia-pair...first-staff-refpoint` **+0.073104 → −0.001429**／
  `system.ossia-pair.compressed-distance` −0.009234 → **−0.000953**／
  `staff.ossia-pair...staff-staff-inside` −0.002309 → **−0.000238**。
- ★★ **合否は残差でなく事前に書いた falsifier で見た**: **2 本が同じ力を解かなくなること**。
  **0.212422 / 0.214493**（LP は 0.212184 / 0.213548・**ossia が小さいという順序も一致**）。
  ⇒ ★ **「同じであってはならない数が同じ」は残差より鋭い**（残差は他の誤りと打ち消し得るが、
  恒等の破れは打ち消されない）。
- ★ **1 force の法則が内部で閉じたことが「正しい理由で動いた」証拠**:
  **4 × 0.000238 = 0.000952 対 system の 0.000953**。**鎖の頭を動かしても比が生き残った。**
- ★ **`VerticalSkyline.Scale` は `LILYSHARP-OWN`**（LP には対応物が無い——LP は小さい stencil を
  作るだけ）。**消える条件も書いた**＝「メトリクスを譜ごとにその譜のサイズで読む日」。
- ★ **網は 2 本とも「修正前なら落ちる」ことを実演済**: ⑴ **比**（ossia の silhouette は
  full size のちょうど `OssiaScale` 倍・**値でなく比なのは全部が字形量だから**）
  ⑵ **falsifier 本体**（ossia 本の圧縮が control より小さいこと）。
- ⚠️ **対照の +0.024000 は付いてこない**（LP の clef は中央線の上 3.776000／Lily# は 3.800000）。
  **§2C 行きのまま。**
...
（第21・22セッションの節は落とした。経緯は commit message と台帳の `why`、
原則は §5 に汎化済み。）
...
**非ゼロで残っている台帳点は 36 点**（数は前回と同じ。**今回動いたのは ossia の 3 点だけで、
どれも非ゼロのまま小さくなった**——総和 −0.082027・**対の control 側は 1 つも動いていない**）:
