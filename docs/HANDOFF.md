...
⚠️ **要ユーザー判断で開いているものは前便どおり**（標·volta の 4 定数の削除・silhouette chordNames scalar・和音語彙 CHL 3 点・Nimbus Sans 同梱・CHANGELOG 版番号・SectionLabel の X・F【8】3・混在綴りの同名 section・標の箱そのもの・`@text` の縦・`bandUp` の死んだ配線の撤去）。★ **本便で 1 つ増えて、同じ便で 1 つ減った**——**整数小節 body の `%` が*音楽として*正しいのか**を起票し、**ユーザーが ⒥（診断を出す）を選んで閉じた**（⑺・§3）。**残りは前便からの持ち越しだけ。**

⚠️ **見つけて直していないもの**:
- **★★ `FlowsThroughBarAccounting` の厳しい gate が、percent body 472 件のうち 114 件を「値段の付かない項目」として opaque 枝へ落としている**（第282 実測・**未解決**）。**落ちる 114 件の理由は全部 `break` か tie**——**どちらも時間を占めないので長さを変えない**。★ **緩めたらどうなるかは毒で測ってある**: **899 冊のうち 5 冊で `lysc check` が変わる**（`Can't Take My Eyes Off You` 2 種・`Dream Come True`・`Holiday`・`I Love You`）。**内訳は「重複警告が 5 行消える」＋「`Holiday` で*相異なる*警告が 2 行消える」**（`Measure duration 8 / 9 exceeds time signature 4/4`）。⚠️⚠️ ★★ **その 2 行を「偽陽性が消えた」と読んではいけない**——**消える理由は、リピートの flow accounting が積算器をリセットするから**で、**同じ 8 は「小節線を書かずに `r1` を 7 つ並べた」書き方そのもの**（`Holiday.lys` の 13 行目冒頭が`r1 r1 r1 r1 r1 r1 r1 r2. b,,4`）。**リピートが無い同じ書き方は今も警告される**（`scratch/p282/gate-ctl.lys` で対照済み）。⇒ **gate を緩めるのは「直す」ではなく「隠す」かもしれない。****本当の問いは「小節線を省いた走りは警告すべきか、そこにリピートが続いたらリセットしてよいか」**で、これは measure validator の設計の問い＝**独立した便と、それ自身の証拠が要る**。★ **今日 opt-in（`ignoringZeroDurationMarks`）で逃がしてあるのは LYS2014 の 1 呼び出しだけ**で、**flow 側は厳しいまま**。**触る便は上の 5 冊 7 行を再現してから始めること**（毒は `FlowsThroughBarAccounting` の `when` を 1 つ緩めるだけ・掃きは `scratch/p282/sweep.ps1`）。⚠️ **`$env:` を `-Parallel` の中で立て消しすると答えが揺れる**（プロセス共有・実測で 5 冊と 4 冊に割れた）——**2 パスに分けて取ること。**
...
⇒ ★★★ **次の一手＝開いているものは無い**。**自律も、ユーザー待ちも、ゼロ**。**本便で起票した ⒧（整数小節 body の `%`）はユーザーが ⒥ を選んで同便で閉じた**（§3・⑺）。★ **次の便は §2 から選ぶことになる**——A〜H のどこから採るかは、そこの ▶ を読んで決めること。
...
**✅ 自律 ⑵（`magic_constants.csv` のラチェット）は本便で完済**（`895055cb`）。**ユーザーが「このセッションでやる方が有利なら着手して／不利なら着手するな」と委譲し、⑵ だけ着手した**——判断の根拠は **⒜ 手本（`ApproximationInventoryTests`）をこの便で*使って*確かめたばかり**・**⒝ 出力中立**（CSV もテストも絵に触れない）・**⒞ 放置すると毎便腐る**。**⑴ は着手しない**と答えた（出力が変わり設計から始まる便で、この便の文脈から割引を受けない）。⚠️ ★★ **⒝ の「ビルドが今動いている」も理由に挙げたが、それは*着手した直後に消えた***——**SAC が Release の `LilySharp.Core.dll` を塞いだ**（Debug と WSL で通した）。**perishable な資産を根拠にするなら、それが消えても仕事が終わることを先に確かめておくこと。**
...
終了時: **commit 4 本＋ハンドオフ 2 本**（この段落を足す 1 本を含む）・**origin 不動（未 push 65・⚠️ ユーザー指示「プッシュはしないで」は生きている）**・木 0・未追跡 0。**suite 6437 合格 / 0 失敗 / 4 skip / 合計 6441**（開始 6430 から **+11**＝新しい網だけ。内訳は ⒜ +5・⒝ +5・ラチェット +1）・**Windows Release と WSL ubuntu Release の両方で同数**、**触った 22 ファイルのハッシュが両 OS で一致**（＝**緑を見た木と commit した木が同じ**）。Core 0 エラー 0 警告。**台帳 740 点・exact 582・総和 24.714341137——全項不動**。**追跡コーパス 573 冊（不動）・snapshot 223 枚（不動・1 枚を承認のうえ再ベース、新規 0）**。**SVG 全数掃きは 1 回**（899 冊＝追跡 573 ＋ ユーザーの蔵書 326）: **動いた本 5**（追跡 3・ユーザー 2＝**先頭 `|` を書く本ちょうど 5 冊**）。**`lysc check` の全数掃きは 2 回**（同 899 冊）: 1 回目が**歌詞のみの本 2 冊の偽陽性**を捕まえ、2 回目は**追跡 573 冊で新 LYS2013 が 0**・**終了コード変化 0**。⚠️ **`git diff | sha256sum` の一致は取れない**（blob の EOL 混在・memory `lilysharp-mixed-eol-blobs`）——**触ったファイル自体のハッシュを比べること**。⚠️ ★★ **本便の最後で SAC が Release の `LilySharp.Core.dll` を塞いだ**（第 8 の出方＝「この便のビルドも通る保証は無い」）。**ラチェットの緑は Windows Debug と WSL ubuntu Release で取った**（どちらも 6437/0/4/6441）。⚠️⚠️ ★★ **`git add <削除済みパス>` が `did not match any files` で失敗し、*同じコマンドの他の引数ごと* staged されなかった**——**「生成器を消して置き換えが入っていない」壊れた commit を 1 本作った**（未 push・**同便で amend 1 回で `895055cb` に修復**。⚠️ **その壊れた側の SHA は書かない**——amend が孤児にした commit の名前は、書いた瞬間に *dead citation* で、`DeadCitationsDoNotGrow` の天井を押す）。⇒ **`git rm` したパスを `git add` に混ぜない**。**commit したら `git show --stat` でファイル数を読むこと**（読んだから気づいた）。

⚠️ **この「開始時／終了時」の対は第279 が置いてから第281 まで*その場で*更新され続けた**ので、**開始時の数（HEAD `d6725a3f`・未 push 59・6426）は第279 のもの、終了時の数（未 push 65・6437）は第281 のもの**。**第282 が §1 を書き換えるときにこの 1 対を 3 便ぶんに割ることはできなかった**ので、そのまま第281 の block に置いた。
