...
**⑴ Lily# のエンジン自身が使った段間の床を吐く**: ⒀ で**床の値は LP と一致**（≤0.31）と分かり、**残りは解いた長さ**（LP は床から +1.07〜+5.01 伸び、Lily# は 5/6 対で単純和の床を 0.59〜0.83 下回る）。**ただし単純和は真の床ではない**——実際は X を見た `VerticalSkyline.Distance` で、この本では上の段の下インク（tab の音部記号 x≈0.7）と下の段の上インク（mark の箱 x≈5.7）の **x が重ならない**ので真の床はもっと小さい。**やること**: ⒜ `PageLayouter.PositionSystemsOnPage` が 1 頁目の各対で使った **skyline 距離・spec の basic/min・解いた距離・力**を吐く（`scratch/p338/ZzP338cProbeTests.cs.txt`／`ZzP338dProbeTests.cs.txt` が雛形。private なので `DebugPageBreakingScoring` の隣に debug hook を足すか reflection）。⒝ ⒀ の表の「床」列を置き換えて、**Lily# が本当に自分の床を割っているのか、それとも床が小さすぎるのか**を決める。⒞ 前者なら solver（rod の当て方）、後者なら `VerticalSkyline.Distance` の X／padding（LP は `skyline-horizontal-padding` 1.0）。⒟ どちらでも台帳点は `system.staff-tab.inter-system-*` の型で LP と対 → 直す（**出力を変える＝承認事項**）。⚠️ **譜対そのものも 9 → 8.326 に圧縮されている**（LP 8.927）＝1 段の差 3.05 のうち 0.60 はこちら。別項として数える。
