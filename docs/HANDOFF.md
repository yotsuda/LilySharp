...
最終更新 第167セッション＝**▶ ⒯ を 3 便で進めた便。第1便が「小数リテラルを足す」（⒯⑸・
...
終了時 **HEAD `16f7253b`**・**未 push 26**（3 便のコミット後に計測。⚠️ **handoff commit は
自分自身を数えられない**ので、この数は次の人が見るときには 27）・suite **4620 passed /
0 failed / 4 skipped**（+45＝新しい網ちょうど）・**snapshot 0 動・台帳 514 点で不動**
...
- **⑽ 第165 ⑺① の宿題は未着手**: `GRAMMAR_STATUS.md` の「Multi-file projects 未実装」は
  **stale**（`using "file.lys"` は `Parser\UsingExpander.cs` に実在）。**文法ドキュメントを
  整理する便が落とすこと。**⚠️ 本便も `GRAMMAR_STATUS.md` を触っていない
  ——**小数リテラルと `@name(引数)` の記述も、その便が同時に見ること。**
  ...
  ~~⑴ `override` / part property の文字列パイプラインを型付きの値にする~~ —
     ...
     **だからこの便では着手しなかった**）。**5 つに割れる。まとめて 1 便に入れないこと**:
     ...
     ⒟ **式の文法そのもの（1 件）**: `@name(引数)` run（`Music:875`）は token を無差別に
        集めている＝**式が着地するならここ**。★ **型集合ではなく「式の文法」を決めてから**
        でないと発明になる。⇒ ▶ ⒯ の本丸だが、着手順としては最後。
        ⚠️ ★★ **第167 で分かった: この島は ⒯⑷ の 3 系統もまとめて抱えている**——
        `@finger.3` / `@bend.5` / `fig.3.5` は**内部の mark 名**で、綴りは全部この
        `@name(引数)`（§1 ⑺）。**ここを設計すると ⒯⑷ が 3 系統ぶん軽くなる。**
        ⚠️ ★★★ **同じ島に未修正の欠陥が 1 つ載っている**（第167 起票・監査 §3.3 末尾）＝
        **`@name.suffix` が点を黙って落とす**（`@feather.up`→`@featherup`・診断ゼロ・
        round-trip も壊れる）。**点を受けるか、受けないなら診断を出すか**を同時に決めること。
...
- **⒃ ★★★ 第3便＝小数リテラルを起票し、その「不可の理由」がユーザー指摘で全部崩れた**
  （`082ccbb1` 起票 → `b4d895b4` 訂正。**技術的な中身は ▶ ⒯⑸ に集約**）。
  **穴は本物**（LP の grob 値は日常的に小数・`= 0.45` が黙って `0` になる・
  Lily# 自身のテストが `force-hshift` を 1.5/0.5 で C# から作っている）。
  ⚠️⚠️ **しかし「素朴な字句規則は `@fig.6.4` を壊す」は 3 重に誤りだった**:
  ⑴ **`@fig.6` / `@fig.3.5` は廃止済みで今はパースしない**（実測 error＋round-trip False）。
     **正しい綴りは `@fig(3 5)`**＝A3 に数えた `@name(引数)` の run。
  ⑵ 根拠にした本は **`output\review\` の古い出力でコーパスではない**
     （コーパスは `audit\lp-regression\lys` 80 冊／フィクスチャ `LilySharp.Tests\Fixtures\test` 209 枚）。
  ⑶ `fig.3.5` は**内部の mark 名としては実在**（パーサが `@fig(3 5)` を正規化し
     `FiguredBassItem.ParseFigures` が読む＝死んでいない）。
  ★★ **教訓 2 つ**: ⓐ **「コーパスに実在する」と書く前に、そのディレクトリがコーパスかを見る**
  ——`.lys` は `scratch\`（gitignore）や `output\` にも大量にあり、**そこの本はもう通らない**。
  「corpus に訊く」の失敗は*訊き方*ではなく**訊く先**だった。
  ⓑ ★ **コード内コメントの「構文」記述に釣られた**——`CollectFiguredBass` の remark
  （`MeasureCollector.Annotations.cs:36`）が **「Syntax: @fig.6 (single), @fig.6.4 (two figures)」**
  と書いており、これが**廃止された綴りのまま**。⇒ ⚠️ **この remark は stale。直す便が直すこと**
  （内部 mark 名としては正しいので、**「内部表現であって構文ではない」と書き分ける**のが正解）。
  ⇒ ★ **実測しなおした結論**: コーパス 80＋フィクスチャ 209 の音楽本文で `数.数` の隣接は
  **`chordnames.lys:21` の `g2:m7.5-` ただ 1 つ**で、**それも衝突しない**
  （`m7` が識別子として一括で食われ `7` は `ScanNumber` に届かない）。
  **⇒ `\d+\.\d+` 字句規則に測れた衝突は 0 件。推奨は lexer の `DecimalLiteral`**（▶ ⒯⑸ ⒜）。
- **⒂ ★★ 第2便の終わりに「残りも続けて切るか」を判定し、切らないと決めた**（記録として残す）。
  **残る A3 7 件を現物で見たら 5 種類に割れた**——決定なしで進むのは **MIDI part render の
  1 件だけ**で、`tempo` は構造化した値の設計、`drummap` は文法を育てる話、`@name(引数)` は
  式の文法そのもの、metadata/font の回復 run 2 件は**型付けが何も買わない**。
  ⇒ ★ **ボトルネックは文脈ではなく決定**なので、便を跨いでも失うものが小さい。
  逆に **1/7 だけ消化すると「自分で名指した穴を半分開けたまま切る」**＝第164 の
  「カットには表が 2 つある」の再演になる。**分類は ▶ ⒯⑴ の ⒜〜⒠ に書いた。**
...
- **⑶ ★★★ 「文法の外」の線がファイルの置き場所で引かれていて、4 件を誤除外していた**
  ...
  `SharedRenderer.Overlays.cs:124`（`@bend.N` の半音数）・`FiguredBassItem.cs:144`（**`@fig(3 5)`**
  の数字。⚠️ **第3便訂正**——ここに最初「`@fig.6`」と書いたが**それは廃止済みの綴り**）は
