...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第245セッション＝**PC の再起動で途中に終わった第244 を閉じ、そのあとユーザーが 2 件報告した。⑴「五線の1段目と2段目が近すぎる」＝段間の床が*段と段*のあいだの量として書かれていた（LP はそれを*五線と五線*のあいだで言う）。⑵「`~B` でマークを消すつもりがキャップが消える」＝1 つの旗が 2 つの意味で読まれ、出荷されていたのは少数派のほう。**（3 commit＝`49f7af51` 第244 の §1 復元／`83709aed` 段間の床の枠／`82e45343` volta のチルダ／この行）⚠️ **台帳 580→583**（新点 3）／**総和 7.643880014→7.843880014**（新しい非ゼロ 1 つ＝`system.indent-floor.one-staff` の 0.200000）／**exact 488→490**／**count 113・非ゼロ 2 不動**／**追跡コーパス 572 不動**／**snapshot 222 は不動だが 3 枚を再ベース**（追加 0）。**6088→6095**（＋4 新網 ＋3 新台帳点）。
...
⇒ ★★★ **次の一手**: **下の「見つけて直していないもの」から選ぶ。本便の 2 島とも閉じている**（第1便＝報告・LP 実測 3 冊・台帳 3 点・移植・網 4 本／毒 3 種・掃き・目視・両 OS。第2便＝報告・文法・4 site の棚卸し・網 6 本／3 出力／毒 4 種・掃き 0 of 572・両 OS）。
