...
最終更新 第215セッション＝**ユーザーの「残債の返済を中断してリリースブロッカーを直す」で門の 2 択を閉じ、続けてリリース前の文法拡張＝リズム記法をユーザーと設計して同便で実装した便**（**実装 3 便＋決定 3 件**）。
⚠️ **既存の彫版は 1 冊も動いていない**——動いたのは ⒠ の SVG family 属性（snapshot 11 枚・属性のみ）と、**新スペルでしか到達できない新機能**（snapshot は新規 1 枚が増えただけ・台帳 529 点・総和 3.644109 で不動）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: ⒠・⒦ はユーザーの名指し（経緯は前版＝ARCHIVE 側 §1）。**リズム記法はユーザーの「このセッションで着手が有利なら着手」委任**で、⑴ 設計の全ニュアンス（LYS0016 の代償・`[` 衝突・LP 準拠の細部）が会話内にあり引継ぎで最も失われやすい ⑵ 両 OS 緑を実測済みの温態 ⑶ 設計判断が全て閉じていて独立に緑で commit できる——ので**有利**と判断した。

★ **開始時裏取り（§0 通し・経緯は前版）**: HEAD `45f9cf10`・未 push 5・suite 5596/0/4・CI 緑＝引継ぎと全一致。
終了時 **未追跡 0・作業ツリー項目 0・未 push 10**（**この行を書く commit を含めて 10**。
**数えたのは commit の*あと***）・suite **Windows 5621 / 0 / 4**・**Linux（この機械の WSL）5621 / 0 / 4**
——**両 OS 完全緑を維持**・**台帳 529 点・exact 454・総和 3.644109・count 110/112**（不動）・
**snapshot 219 枚**（＋1＝`test/rhythm-slashes`・既存 218 枚は ⒠ の属性のみ）・**Core 0 警告**。

★ **この便の値段**:

| 便 | 何が動いたか | 射程 |
|---|---|---|
| ⑴ ⒠ SVG の sans が名指しに（`f1676e5e`） | Core 1・snapshot 11 | 属性のみ（幾何 0）・§2F ✅ 済み |
| ⑵ ⒦ の決定＝未署名のまま出す | §3 に 1 行 | コード 0（§3 の 3 行目） |
| ⑶ リズム記法＝`/` スラッシュ音符＋裸 duration 反復（`c6b9fb32`） | Core 26 file・網 24 本 | **新スペルのみ**（既存 567 冊は字句的に到達不能） |
| ⑷ その docs・彩色・fixture（`f279565c`） | doc 4・tmLanguage 1・fixture+snapshot 1 | リリースの門と新機能のピン |

- **⑶ ★★★ 設計と実装の骨子**（決定 2 件は §3 の 1〜2 行目・網は `SlashNoteTests`＋`BareDurationTests`）:
  - **家は `q` の写し**: 共有 resolver `Music.BareDurations`（`ChordRepetitions` の鏡・文書順 fold・
    scope 境界同一）＋各 walker（collector／MIDI／MusicXML）が**自分の walk で記録した絶対綴り**を
    コピーする（相対フレームは進んでいるので**再解決は不可**——`q` と同じ理由・同じ二段構え）。
  - **LP 準拠は実測でピン**: `{ c'4 4 <d' f'>4 4 q4 }` と `{ c'4 r4 4 8 8 }` が明示綴りと
    **SVG バイト一致**（2.26.0・scratch/p216）。**和音は丸ごと反復**・休符透過。
  - **スラッシュの描画は既存配管**: `@notehead(slash)` の家（符頭・符尾・連桁・MusicXML の
    slash 名）が全部先に在り、**新設は「音高なし・中央線固定」だけ**。
  - **双子**: 裸 duration は**そのまま通す**（LP 自身が読む）。スラッシュは `\improvisationOn`＋
    clef 中央線音高で、**LP 側フレームだけ進めて既存の乖離補正機構に任せる**（`_lysClef` を
    part 頭で初期化する 1 行が新設。preset 由来 clef は見ない＝双子の線が 1 本ずれ得る・注記済み）。
  - **旧 LYS0016 のピン 4 本を新規則に書き換え**（DurationAdjacency 2・StrayItemToken 1・
    PhraseExtractor 1）。`("section","42")` は「数字は音楽になった」ので stray の網から外した
    （`c4` と同じ道＝music-needs-part＋反復先なしの validator が名指す）。
  - **perf（§7.9・計算を足した＝測った）**: 記録辞書 1 write／音符。alloc 最小-of-3 で
    **打鍵は桁まで不動**（fingstack1k 162.6・plain1k 45.2 MB）・全体 +0.4/+0.9 MB
    （2412.5/694.5 比 ≤0.13%）。
- **⑸ ★★★ 次に触るならここ**——**⓪ 歌詞文法 `sings` の設計判断**（このセッションでユーザーと
  設計済み・未起票・未実装）: 要件=歌詞は自分専用のメロディに定義側で結びつく（`lyrics ja sings vocal`）
  ／省略可（従来の均等割り独立行）／同メロディに複数トラック（同名=番・別名=言語）。
  **未決 3 点=キーワード綴り（sings/for/of）・unbound+`with lyrics` を即エラーにするか・
  0.3.0 の前か後か**。実装の山は「刷らないメロディの時刻に音節を置く」spacing 参加。
  **① タグを打つ**（0.3.0 の門は sings の判断以外すべて開いたまま: CHANGELOG 更新済み・
  両 OS 緑・未 push 10 を先に）。**残債は従来どおり**: ⒝ ペダル段間隔／⒞ CoPlaceToCodaWithLabels
  鏡像／⒟ §2 ▶ perf／⒡ 配管 6 site。

> ## ★★ この便の骨 1＝**「ユーザー判断待ち」は 1 種類ではなかった**
> ⒠（決めれば 1 便で閉じるコード）と ⒦（決めても資産が要る）は同じ棚に 10 便並んでいた。
> 委任が来た日、前者は即日閉じ、後者は選択肢を絞った 1 問で決定として閉じた。
> ⇒ **起票時に「判断だけで閉じる」か「ユーザーの資産が要る」かを書き分ける。**

> ## ★★★ この便の骨 2＝**新文法の最安の家は、10 便前の自分が建てていた**
> スラッシュ符頭は `@notehead(slash)` が、反復は `q` の resolver＋記録二段が、
> **実装前から全配管を持っていた**。この便の新規コードの大半は「既存の家を 1 kind 広げる」で、
> 発明はゼロ（`LILYSHARP-OWN` の新設 0 本・REF は music_embedded / improvisationOn / CHORD_REPETITION）。
> ⇒ **文法拡張の設計は「何を書くか」より先に「どの家が既に半分やっているか」を数える。**

> ## ★★ この便の骨 3＝**網を消費する変更は、消費した網の行き先まで書いて初めて閉じる**
> `c 4` の LYS0016 を機能に明け渡すとき、誤字クラスを 2 つに割った——時間が狂う類は bar check が
> 受け、音高書き漏らしは**黙って通るようになった**。後者は取り戻せないので §3 に「承知で決定」と
> 書き、GRAMMAR の当該節にも同じ 1 段落を残した。
> ⇒ **「エラー→機能」は互換では常に安全だが、網では常に負債**——負債の行き先を 3 箇所
> （§3・文法書・テストの remark）で同じ言葉にしておく。
