...
- ⚠️ ★★★ **新規＝`octave absolute` の本では、trailing の octave 記号 `'` `,` が診断ゼロで死んでいる**
  ...
  **相対モードでは C5 E5 G5 / C5 D5 E5 F5 と動くのに、`octave absolute` では
  どちらも無印と同じ C4 のまま**（`,` も同様）。**警告もエラーも 0。**
  ⇒ ★★★ **観測者は正典の 1 文だけ**——**`GRAMMAR.md` の Chord は
  「In 'octave absolute' mode … the trailing marks *still shift the whole group*」と
  明言している**（＝**文書が実装を反証している**形）。**その 1 文に実測を添えて印を付けた。**
  ⇒ ★★ **同じ位置の `(N)` は absolute でも効く**（`theme'(3)` → E4 F4 G4 A4）
  ＝**接尾辞そのものではなく、オクターブの移動だけが落ちている。**
  ⚠️ **ツリー 563 冊中 0 冊がこの綴りを書く**（`octave absolute` は 319 冊あるのに）
  ＝**falsifier になる本が無い**ので、**§5.4 の順序どおり「受理集合を網に → 黙る失敗に診断 → 型」**。
  **規則の形（動かす／error にする／absolute では無意味と宣言する）が決定。**
- ⚠️ ★★★ **新規＝`ParseList` の裸 `Advance()` は `using` に限らず*認識できない項を全部*落とす**
  （第183 実測・**未修理**。ユーザー決定でこの便は `using` だけに絞った）。
  **`Parser.Declarations.cs:146` の `else Advance();` と `ParseMusicBlock` の同じ行**が家で、
  **呼び手は form / score / section の 3 つだけ**＝島は小さい。**実測**: section 本体に stray identifier を
  1 つ置くと**4 文字失い、`m { … }` が `m …` に潰れる**——**診断ゼロ**。
  ⚠️ **一族の網 `AnnotationRoundTripTests.EveryBook_SpellsItselfBackOutOfItsTree` は在る**
  （既知の壊れた本 5 冊つき）が、**ディスクの本しか見られない**——**ツリー 563 冊のうち `using` を
  含むのは 4 冊で、4 冊ともコメント**（実測）。**この枝を踏む本が 1 冊も無い。**
  ⇒ ★ **着手するなら先に、section/form/score の item を読む消費者が全部「種別で肯定選択」して
  いるかを数え切ること**（第181 ⑶ の教訓＝**選択は否定形でなく肯定形**）。
