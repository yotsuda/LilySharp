...
最終更新 第362セッション（2026-09-10）＝**入り方はユーザー起票**（`scratch\ベースタブLy\tooLongChords.lys`＝「chords prog の section A が 2 小節あるのに警告が出ない。part 側に小節が足りないことを警告すべきだ。G7 を section B の先頭にはみ出すのではなく、section A を 2 小節にして表示すべきだ」）。**再現してから直した**（`lysc check`＝「No errors found.」・base の SVG は G7 と Cmaj7 が同じ bar 2 に並ぶ x=18.37／22.49・全 2 小節）。**病名は「同じ量の 2 綴り」ではなく「同じ集合の 2 軒が chords 行を数に入れていない」**——⑴ `CrossPartMeasureValidator.ValidatePartMajorSections` は親が `PartDeclarationSyntax` の section しか集めず（section-major の `ValidateSectionCrossPart` も `PartBlockSyntax` だけ・part 1 つ＋chords なら `parts.Count < 2` で数の比較の前に帰っていた）、⑵ `MeasureCollector.GetCanonicalSectionBars` も `PartMajorCells`＋`PartBlockSyntax` だけ＝canonical が 1 のまま melody が pad されず、chord 行は自分の 2 小節を B の先頭に置いていた。**製品＋番人＋CHANGELOG＋この HANDOFF を 1 commit（下 ⑸・snapshot 0＝承認ゲート無し）・push はユーザー**。**骨は 6**:

★★★ **⑴ 決定（起票のとおり・私も賛成）＝section の小節数は「その section を書く全 voice の最大」で、名前付き chords 行は voice の 1 つ**。lyrics 行は**入れない**（lyrics section が音楽より長いのは stacked verse＝`LyricsCollector` の auto-wrap の設計・`SectionState.ChordTrackCells` の remark に理由）。**逆向き（chords 行が part より短い）も警告する**（同じ食い違い・メッセージの尾だけ「the row writes no chord over the remaining bar(s)」・pad は無い＝行の残りに和音が無いだけ）。

★★★ **⑵ 形**: `SectionState.ChordTrackCells`（(section, track) → `chords track { section NAME {…} }` の内側 section・`CollectDefinitions` の `IsInsidePartMajorTrack` 分岐で `EnclosingChordTrackName` により記録）→ `GetCanonicalSectionBars` が `ChordNameCollector.CountSectionBars`（part-major）と `ChordNameCollector.CountBars`（section-major の名前付き `ChordPartBlockSyntax` 直下）を fold に足す（probe の `_walkMaxSourceRead` も同じ cell を読む＝RESUME の `CanonicalBarsMatch` は同じ関数を通るので自動で整合）。validator 側は `SectionVoice`（Label＝`part 'x'`／`chords 'x'`・IsChords・Bars・Span）で 2 つの pass の数の比較を `ReportBarCountMismatch` に 1 本化（part-major＝`sec.Parent is ChordPartBlockSyntax { PartName: not null }`／section-major＝`ChordPartBlockSyntax` 子を `chordVoices` に・part 1＋chords でも数の比較へ）。**LILYPOND-REF 0／LILYSHARP-OWN 0**（LP に対応物が無い＝Lily# の form/section 文法の話）。**式は足していない**: 集合に chords 行を足しただけ。

★★★ **⑶ 対・番人**: 起票本＝`(6,11) warning LYS2007: Section 'A' spans 1 bar(s) in part 'melody' but 2 in chords 'prog' — the shorter part is padded with rests to align`・layout 3 小節・SVG は Dm7 8.59／G7 18.37／Cmaj7 **24.76**（B の頭）・PNG 目視（`scratch/p363/tlc-head.png`）。番人 +6: `CrossPartMeasureValidationTests` 4 本（chords 長い→melody の section 名に anchor・一致→無音・chords 短い→track 側に anchor・section-major 綴り）＋`ChordPartSectionsTests` 2 本（0:Dm7 1:G7 2:Cmaj7・melody 3 小節＝part-major／section-major）。

★★ **⑷ 掃き 925 冊（base 56e62e94 Release＝`scratch\p363\wt-base`・`sweep363.ps1`・SVG バイト比較＋`check` の LYS2007 本文 grep）: MOVED 1＝起票本だけ**（追跡 599・ユーザー 325 は不動・snapshot 0）。**警告の増分 +3**（base 8 → head 11）: 起票本＋**逆向き 2 冊＝`LilySharp.Tests/Fixtures/test/rit-across-systems.lys` の B（melody 2・chords `Im7 VIImaj7` 1）と ユーザー `bench.lys` の Intro（bassline 3・chords 2）**——どちらも実際の食い違い（fixture は snapshot 不動＝警告だけ）。⚠️ **`lysc check`（単発も `--batch` も）は 1 行目にコード `LYS2007` を刷らない**（コード付き `Diagnostics:` は svg/layout/ly 側だけ）＝最初の grep が base 0／head 0 を出し、起票本の 1 件（陽性対照）が無いことで気付いた→ 本文の語で数え直し・スクリプトに注記。

★★★ **⑸ 数と射程**: **開始時**（`Session-Check` の `-Build -Test` は回していない＝起票再現を先にした・数は git と第361 末から）: HEAD **`56e62e94`**・未 push 8・`origin/master`＝`ba70753f`・木 clean・未追跡 0・`gh run list` **赤**（34416221197＝第361 ⑷⒝ の push 前の赤・以後 push 無し）・台帳 816・snapshot 249・追跡 `.lys` 599。solution build 0 エラー（増分・Core の警告は出力に無し）。**終了時**: **full `scratch/p363/run1.trx` 7571 / 1 / 4 / 7576（+6＝番人・赤 1＝`ApproximationInventoryTests.TheInventoryIsNotStale`＝`LILYSHARP_UPDATE_DOCS=1` で再生成・APPROXIMATIONS は行番号 1 行だけ動く）→ 再生成＋この HANDOFF の後に full `scratch/p363/run2.trx` **7572 / 0 / 4 / 7576**（RunInfos 無し）**・§7.5（作業ツリー・`git diff --numstat -- LilySharp.Core`）**Core `+` 142／REF 0／OWN 0**（Session-Check の `-DiffBase` は commit 済み差分を読むので作業ツリーでは 0 と出る）・台帳 816／snapshot 249／追跡 `.lys` 599 不動・ディスク 19698（p363 の staged 925 冊込み）。CHANGELOG（0.7.0 Diagnostics）。⚠️ `git worktree`: `scratch\p363\wt-base`（56e62e94）を足したまま・p362 のも残っている（`git worktree remove` で外す）。⚠️ scratch は **`p363`**（p362 は第361）。⚠️ §7 3.5: 第360 のブロックを `-Archive 360` で ARCHIVE の先頭へ（済）。⚠️ 記憶: `reference_lilysharp_check_batch_omits_diagnostic_code`。

✅ ★★★ **第 2 便（ユーザー「小節数が足りないとき、VS Code の quick fix で自動で小節線を足して、簡単に警告を除外できるように」）＝LYS2007 の quick fix**。**先に測った**: 空小節 `| |` は part でも chords でも 1 小節に数えられ LYS2001 には化けない（`scratch/p363/pad-probe.lys`＝melody `g2 g | |` で警告 0・layout 3 小節）。**形**: `LilySharpLanguageServer.BarCountFix.cs` の `BarCountPadding(tree, text, diagnostic)`＝メッセージから have/want を読み（`spans N bar(s) in … but M in`）、anchor（名前 token の span）から scope を引く（part-major＝`SectionDeclarationSyntax.Name`／section-major＝`PartBlockSyntax.PartName`（braces は body の `MusicBlockSyntax` のもの）／`ChordPartBlockSyntax.NameToken`）→ 本文の最後の項目の直後（空なら `{` の直後）に ` |`×n を挿す候補を作り、**再解析→`SemanticValidation.Run` で同じ anchor の LYS2007 が消える候補だけ出す**（n＝want−have と n+1: 末尾の小節が開いている `{ g2 g }`／`{ Dm7 }` は最初の `|` が閉じるだけなので 1 本余計に要る＝算術でなく実測で決める・実行は lightbulb 要求時だけ）。`GetCodeActions` は今まで **parse 診断（`doc.Tree.Diagnostics`）しか見ていなかった**→ `DocumentDiagnostics`（Problems panel と同じ集合）も読み、**範囲は span 始点でなく重なりで判定**（caret は squiggle の途中に居る）。title は「Add N bar line(s) to section A (| |)」＝bar の数でなく書く `|` の数（閉じ線込み）。番人 `Lsp/BarCountQuickFixTests` 6 本（part-major melody・開いた末尾・chords 短い×2・section-major の part block／chords block・不足無しで無音）。Lsp 179 本緑・**full `scratch/p363/run3.trx` 7578 / 0 / 4 / 7582（+6）**。CHANGELOG（Editor）。commit（Lsp 2＋Tests 1＋CHANGELOG＋この HANDOFF）→ `Deploy-Lsp.ps1` で配布（server だけ＝Reload 1 回）。push はユーザー（未 push 10）。

✅ ★★★★ **第 3 便（ユーザー「このセッションでやる方が有利なら着手」→ ⒝① は同じ regime の第 3〜5 の読み手で、p363 の probe 本・base worktree・掃きがそのまま効くので着手）＝exporter 3 軒の pad**。**先に測った**（`scratch/p363/pm-two-parts.lys`＝melody A 1 小節・bass A 2 小節）: LP 双子は melody 2 小節・bass 3 小節（B が bass の A の下）／MusicXML は P1 2 小節・P2 3 小節／MIDI は part 同士なら lane で揃う（B は tick 8）が chords 行だけが長い起票本では B が bar 2（tick 4）＝**読み手 5 軒（頁・validator・LP・MIDI・XML）で揃っていたのは頁と validator だけ**。**病名は「同じ量の 2 綴り」がもう 1 段**: 頁の canonical は `CountBarsInScope`（緑の構文数え＝`R1*4` は 1・`repeat unfold 13` は本体 1 回・phrase 参照は 0）、validator は `MeasureModel.Split`（意味数え）。**形**: `SectionBarCounts`（`Svg/Collector`）＝1 軒に **2 つの数え方を名前付きで**——`SemanticVoices(root)`（voice＝part-major cell／chords cell／section-major の part block・名前付き chords block／single-part shorthand・lyrics は voice でない・Label・Anchor・Bars＝Split・TrailingOpen＝構文の pendingMusic・PartMajor）と `BuildSemanticIndex`（name→canonical・container→voice・`Missing(container, out open)`）を **validator の part-major pass と LP／MIDI／XML が読む**／`CanonicalByNameSyntactic(root)` を**頁**が読む（perf: 緑 walk・第155 の red first-touch を避けるため。**頁の過小評価は remark に明記**＝最長 voice が展開のときだけ pad が足りない・観測例は未計数）。LP＝`PaddingBars(container)`（音楽: 素の `|` node を missing＋(open?1:0) 本→ `EmitMusicStream` の空小節規則が `s1 |` を書く／chords: `ChordBarMarker("s1")`＋`|`）を `AppendSection`・form 無し経路・chord track 経路の 4 箇所に。MIDI＝`PaddingTicks(voice)` を `PlaySectionByName`（part-major cell・bare-owner）と `PlaySection` の part block ループに（chords cell は lane 無し＝canonical にだけ効く）。XML＝`PadVoice(voice)`（素の `|` を `ProcessNode` に流す＝`AddSilentBar` の全休符小節）を `ProcessPartBlock`／`EmitPartMajorSection` に。⚠️ **第 1 カットは構文数えで pad して掃きが拾った**: ly MOVED 15 のうち 12 が誤り（canon-in-d の `repeat unfold 13` の後に `s1` 48 本・`R1*4` の後に 2 本・percent の後に 2 本＝`compare-leg3-ly.txt` 初回）→ 意味数えに切替。**掃き（base2＝366881b5 Release `scratch\p363\wt-base2`・`sweep363.ps1 -Kind svg|ly|midi|xml`）: svg MOVED 0 ／ ly 6 ／ xml 6 ／ midi 1**＝ly／xml の 6 冊は LYS2007 が鳴る 6 冊（11 件: feature-tour 4・grammar-tour 3・tooLongChords・bench・rit-across-systems・Untitled-3）そのもの・midi は起票本だけ（part 同士は lane で既に揃っていた＝byte 不変）。番人 `SectionVoicePaddingExportTests` 11 本（2 つの数え方の一致・open 判定・MMR／unfold／phrase は pad しない・LP×4・MIDI×2・XML×2）。⚠️ 名前の罠: `part a`／`chords p`／`section R` は音高・強弱・MMR に読まれて parse できない（テスト名は melody/prog/Rest に）。**full `scratch/p363/run4.trx` 7588 / 1 / 4 / 7593（+11・赤 1＝APPROXIMATIONS 再生成）→ 再生成＋この HANDOFF の後に full `run5.trx` **7589 / 0 / 4 / 7593**（RunInfos 無し）**・§7.5（作業ツリー）Core `+` 171＋新規 `SectionBarCounts.cs`／REF 0／OWN 0。CHANGELOG（MIDI, MusicXML and the LilyPond twin）。commit（Core 10＋Tests 1＋CHANGELOG＋APPROXIMATIONS＋この HANDOFF）→ `Deploy-Lsp.ps1`（validator が `SemanticVoices` を読むようになったので server を配布・Reload 1 回）。push はユーザー（未 push 11）。⚠️ `git worktree`: `scratch\p363\wt-base2` も足したまま。

✅ ★★★★ **第 4 便（ユーザー「有利なら着手」→ 拾い箱 ① は数え方のコードが温かく svg の base2 がそのまま使えるので着手）＝頁の canonical（緑の構文数え）に `R1*N`・repeat・phrase を教えた**。**先に測った**: 網 `SyntacticCanonical_MatchesSemantic_OnEveryNetBook`（fixture＋samples＋audit の全 section で構文数え＝意味数え）の初回は **97 section が不一致**、ほぼ全部「page 0 対 semantic N」＝phrase 参照だけで書かれた section を 0 と読む（MMR／repeat より phrase が主因）。**形**: `WalkBars`（緑）に `Rest` の slot 3（`*N`→ N−1 を先に足す）・`RepeatExpression`（tremolo は音符扱い／それ以外は body を別枠で歩き **閉じた小節だけ turns 倍・小節線の無い body は音符扱い・開いた尾は 1 回だけ持ち越し・入口では閉じない**＝過小はあっても過大にしない）・`VariableReference`（phrase 表 name→緑 body を `PhraseGreens(root)` で collect ごとに 1 回・cycle 番・**出口は閉じた body なら `confirmable=true`**＝直後の `|` は閉じの確認で空小節ではない・**開いた尾は閉じない**（`phrase p1 { c4 }` の DAG 32 個は 8 小節であって 32 ではない＝`ExpansionBudgetTests` が拾った）・入口は何もしない）。**予算**: 歩きの phrase 入口は `DefaultExpansionBudgetCap`（50,000）まで・小節数の上限も同じ（`R1*2000000000` の overflow 番）・**collector が予算超過（`ExpansionBudgetExceededAt`）なら頁は pad しない**（`R1*1000` を cap 10 で 11 小節に切った絵に 989 小節の spacer を足していた）。赤 oracle `WalkBarsRed` も同じ規則（phrase 表無し）。`CountBarsInScope(scope, out open, phrases)` overload・`CanonicalByNameSyntactic` が表を渡す。⚠️ **`confirmable` は今までどこも true にしておらず死んでいた**（今回 phrase／repeat の出口が使う）。**掃き（svg・base2 対 head）は 2 段階で拾った**: 第 1 カット（turn ごとに 1 小節）は **MOVED 4**＝tour 2 冊＋`audit/lpreg/slashprobe`（`repeat percent 2 { c16 d e f }`＝beat repeat を 1→4 小節に）＋`voltagrace-probe`（`grace { f8 } repeat volta 2 { b1 | }`＝grace の pending を入口で閉じて 2→3 小節）→ 規則を直して **MOVED 2**＝`showcase/grammar-tour`（41→52 小節＝A +2・C +5・D +4＝LYS2007 の数そのもの）・`test/feature-tour`（40→56）＝短い rh／melody が lh に揃う（snapshot 無し・承認ゲート無し）。**頁は in-music `repeat volta 2` を 2 小節に展開している**（voltagrace 2 小節＝MeasureModel の remark どおり）。網の残差 1＝`05-special-techniques` Main（`repeat percent 2 { c4 e g e } |`＝小節線の無い body は 1・意味数え 2＝意図した過小・網は理由付き許可表・**過大は必ず落とす**）。番人 +5（網 1＋repeat 4 形の Theory）・`EmptyMeasureValidatorTests` 3 本が phrase の出口規則の番人になっていた。**perf（ユーザー「今静かなので perf をとって」・Release・`scratch/p363/perf-phrases.ps1`＝重い側の本を生成して base2 対 head を交互 min-of-5 両順序・`editbench.txt`）**: 打鍵ベンチ `EditKeystrokeBench` 床（ms）plain1k 165／161 対 139／145・fingbeam 228／224 対 407／218・v2bow 227／294 対 310／240＝両順序で符号が揺れる＝ノイズ幅の内。**重い側（120 section × 2 part × 3 phrase 参照・40 phrase × 8 小節＝2880 小節/part）は memo 前 +8.4／+3.9%（対照 plain1k +4.1／−0.8%・DAG +1.9／+1.0%）＝縁だが両順序で正**→ **phrase 本体の数を (名前, 入口の confirmable) で memo**（`PhraseBarTable`＝本体の数は純関数・入口の pending は本体の最初の `|` が閉じるので影響なし）→ **memo 後 −10.4／−7.1%（対照 +1.8／+2.5%・DAG −1.0／−3.2%）＝コスト消滅・svg hash 一致**（`perf-phrases.txt`／`perf-phrases-memo.txt`）。CHANGELOG（Fixed）。**full `run6.trx` 7591 / 3 / 4 / 7598（赤 3＝`ExpansionBudgetTests`＝上の予算・上限・出口規則で閉じた）→ `run7.trx` 7594 / 0 / 4 / 7598（+5・RunInfos 無し）→ memo 後 `run8.trx` 7594 / 0 / 4 / 7598（3 分 37 秒＝run7 の 8 分半は機械側と確定）**。⚠️ run7 の所要 8 分 30 秒（run5 は 3 分 16 秒）は **機械側**: trx の per-test で font resolver・git 履歴・CLI batch など無関係なテストまで 2 倍で、静かな機械で `CollectResume…OnPerfBooks`＋font resolver の 2 本を計り直すと 49 秒（run5 では 54.5＋37.2）＝私の変更ではない。網 `SyntacticCanonical_…` 単体は 16 秒。§7.5（作業ツリー・commit 前）Core `+` 197（`MeasureCollector.Form.cs` 175・`SectionBarCounts.cs` 22）／REF 0／OWN 0。commit（Core 2＋Tests 1＋CHANGELOG＋この HANDOFF）→ `Deploy-Lsp.ps1`（preview の頁が変わる）。push はユーザー（未 push 12）。

⇒ ★★★ **⑹ 次の一手**: **⒜ push はユーザー**。**⒝ 拾い箱（この便が測って名指しただけ・未起票）**: ①~~頁の canonical は構文数え~~＝第 4 便で閉じた（残差は小節線の無い repeat body の過小 1 件・上）。②`rit-across-systems` の B は fixture 自身の食い違い（chords `Im7 VIImaj7` に `|` が無い）＝直すなら fixture 側（snapshot は動かない・警告だけ）。③XML の `repeat unfold`／`R1*N` は自前の小節数（unfold は繰り返し記号・MMR は 1 小節）＝pad の数え方（意味）と XML 自身の小節数が食い違う本では part 間の小節数は揃わない（`SemanticCount_…` テストの remark）。④`MeasureModel` と頁は in-music `repeat volta N` を N 回展開するが LP 双子は `\repeat volta` を書く＝双子は 1 回分の絵（既存・未計数）。**⒞ 第361 ⑺ の並び（`CreateLeftSkyline` remark・Dots.direction・drhs／bos／slash／ps2）と ⒟ 第358 ⑺ の並びはそのまま**。

## 以下は第361セッションの経緯

最終更新 第361セッション（2026-09-10）＝**入り方は第298〜第360 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。第360 ⑺ に「決定済み・未実装」は 0 だったので、**⒞ の拾い箱から ②「`DotGap` 0.3 対 LP 0.45（台帳点が要る・dcv／dfd の −0.15）」を選び、台帳点を先に開いてから移植した**。**測ったら病名は「同じ量の 2 綴り」**——描画側は第314 から `DotColumn.OffsetX`（頭のインク右端＋点 1 個幅 0.45・旗の押し出し込み）で置いていたのに、**予約側 3 軒（列の spacing 箱・keep-inside-line の reach・タイ輪郭）は定数 0.3・押し出し無し**で読んでいた。**そして移植が第 2 の欠陥を出した**（§5.0 の型どおり）: **wish の skyline と rod の skyline は LP では別の Separation_item**で、Dots は rod 側にしか居ない。**製品は作業ツリー＝承認待ち**（下 ⑹）。**骨は 6**:

★★★ **⑴ 台帳 5 点（`audit/lp-geometry/probes/dot-column-spacing.ly`・書く前に予測を書き、4 点は桁まで的中・1 点は最初のカットが外れて切り直した）**: **DCX**（dfd の小節＝`voice { c'4 c'4 } { b4. b8 }` 2/4）の col0→col1/4＝**3.908400**（cross-voice の rod＝0.1＋(1.3042 shift＋1.3042 head＋0.45 gap＋0.45 dot＋0.2 esw)＋0.1）→ Lily# **−0.150000**（＝0.45−0.3）／同書の col1/4→col3/8＝1.8（素の 8 分・対照）exact／**DCC**（点を外した `b2`）3.002245（声 1 の wish＝quarter-gap と同じ桁）exact／**DPF**（`voice { g8. s16 } { s16 g16 s8 }` 4/16＝線上の g の点は旗に押されて 2.5174）col0→col1/16＝**3.367400**（rod＝0.1＋(2.5174＋0.45＋0.2)＋0.1 が最短の素の ideal 2.4 を縛る）→ Lily# **−0.913200**（予約が 1.3042＋0.3・押し出し無し＝rod 2.4542）。⚠️ **DPF の最初のカット（16 分を 3/16 に）は何も観ず、反証条件が鳴った**: LP 4.301955＝2.4＋1.2·log2 3＝素の ideal が rod の上＝**付点音符→その半分の音価の対は shortest によらず必ずこの比**なので、rod が縛るには**対を shortest と同じ長さにする**しかない。／**DCW**（後述 ⑶・`<b' c'' d'' e''>4.@stemDown <f' g' a' b'>4.@stemDown` 6/8＝`audit/lp-regression/lys/dots.lys` bar 2 の絶対音高綴り）col A→col B＝**3.843400**。⚠️ **DCW も最初のカットは別の音楽だった**（`octave absolute` に相対綴り `<b c d e>''` を書いて b'' が 7 度上に飛んだ・LP 3.4542）——**双子の音高は LP dump の staff-position（7..10／4..7）と突き合わせてから数を記録した**。

★★★★ **⑵ 機構 1＝予約する点の列は描く点の列（`DotColumn.Reserved`）**: `ItemSkylineFactory.AddDots`・`SpacingRules.CalculateNoteheadRightExtent`・`ElementCoordinator` のタイ輪郭（＋スラーの extra 箱）が **`DotColumn.Reserved(item, noteValue, headInkRight)`**＝renderer と同じ `OffsetX` を、spacing 側の家（`StemSpacingInfo`／`LayoutUtilities.StemX`／`GetFlagBBox`）から組んだ stem／flag の支えで呼ぶ。行は `DotConfiguration.Resolve`（描画と同じ）。**`EngravingDefaults.DotGap` は退役**（`LpProvenanceTests` の remarks は歴史として残る・OWN 136→135）。**spacer／MMR の点は予約しない**（grob が無い・以前は placeholder の後ろに幻の点箱）。**休符の点は休符インク右端＋0.45**（以前は頭幅 1.3042＋0.3）。REF: `separation-item.cc:163-164`・`dot-column.cc:81-84`／`:103-109`／`:130-141`／`:252-257`・`output-lib.scm:692-704`。**OWN 1**＝**Dots.direction（声の向き）を読まない**（renderer は `VoiceDefaults` から取るが item が持たない・下向き旗の帯に沈んだ点だけ・観測者無し）。**対**: dfd 16.70＝LP 16.702・dcv 32.70＝32.701（前は各 −0.15）。

★★★★ **⑶ 機構 2＝wish の skyline は NoteColumn の要素だけ・rod は PaperColumn の全要素**（**移植の掃きで見つけた第 2 の欠陥**）: 予約の点を 0.45 にしたら **dots.lys bar 2／5 が +0.04／+0.06 → +0.19／+0.21 に*悪化***。LP 源: `spacing-interface.cc:36-97` の wish の skyline は left/right-items（NoteColumn）自身の `horizontal-skylines`（＋右列の conditional）で、**NoteColumn の `elements` は heads（`note-column.cc:133-156 add_head`）・stem（`:120-125 set_stem`）・flag（`rhythmic-column-engraver.cc:102`）だけ**。**Dots は `dot-column-engraver.cc:52-60` が DotColumn にしか入れず、PaperColumn の Separation_item（`paper-column-engraver.cc:246-261`＝全 item）＝rod にだけ届く**。DCW: wish の min は主頭→反転頭 2.7434（床 3.0434 < ideal 3.7042）・rod は点→反転頭 (1.7542＋0.45＋0.2)＋(1.2392＋0.1)＋0.1＝**3.8434 が縛る**。Lily# は 1 本の skyline を両方に使い、wish の min が点を持って ideal を min＋0.3＝4.0434 に上げていた（0.3 時代は 3.8934＝+0.05）。**形**: `ColumnParts` の `ColumnPart.NoteColumnMember`（Dots・半タイ＝false）＋`[Flags] ColumnElements { NoteColumn, ColumnOnly, Conditional }`＋`CreateWishRight/LeftSkylineAtColumn`（wish 右＝NoteColumn・wish 左＝NoteColumn＋Conditional＝`spacing-interface.cc:87-89`）。読み手 3 軒: `CalculateSkylineDistance`（wish）・`NoteColumnToBarlineFloorPair`（SkyMin は wish・Rod は列）・`ApplyCrossVoiceColumnSpacing`（same-voice の sky は wish・rod は列）。**dots.lys 5 小節が全部 LP の桁（9.227／8.448／8.372／9.123）**＝機構 1 で開いた分も、元から在った +0.04／+0.06 も閉じた。⚠️ `CreateLeftSkyline` の remark「LP の spring min は conditional を読まない・NOT CHANGED HERE」は**源と食い違う**（`:87-89` は右列の conditional を wish に merge する＝Lily# の今の読みが正）——remark は直していない（別便の起票へ）。

★★ **⑷ 付随 3 つ（全部 commit 済み・出力不変）**: ⒜ **ユーザーが 53 本を 10 本に畳んで push**（`origin/master`＝`ba70753f`・§1 の旧 hash は全部孤児・backup tag 無し・reflog は新 10 本だけ）→ `DeadCitationsDoNotGrow` 621 > 600＝前例どおり**天井を 621 に**（commit `cbd0ad00`・張り替えない・live 754）。⒝ **CI が push で赤**（34416221197）: 4 脚＝⒜／**extension-tests＝`exportBatchCore.test.ts` が Windows パスの literal を `duplicateStems` に渡し、ubuntu の `path.basename` は `\` で割らない**→ テストが `path.join` で組む（commit `b39f396d`・`npm test` 94・`tsc` 0）。⒞ **台帳 4 点を「開いた」commit** `829ed1c6`（前移植の残差を記録・出力不変）。

★★★ **⑸ 番人・目録**: `SpacingInvariantTests.CrossVoiceDotReach_FloorsTheNextColumnsSpring` を **LP の 3.3295 exact** に（DotGap 不足分の足し算を撤去）・APPROXIMATIONS／magic_constants 再生成（`LILYSHARP_UPDATE_DOCS=1`・Core の doc を触るたび）・`LpReferenceCitationTests`／`LpProvenanceTests` 緑・拡張 `npm test` 94／`tsc` 0。

★★★ **⑹ 数と射程・承認ゲート**: **開始時裏取り**（`tools\Session-Check.ps1 -Build -Test -Scratch p362`）: HEAD **`ba70753f`**（＝`origin/master`・**未 push 0**）・木 clean・未追跡 0・build 0 エラー／Core 0 警告・**full 7559 / 1 / 4 / 7564**（`scratch/p362/run1.trx`・赤 1＝`DeadCitationsDoNotGrow`）・`gh run list` **赤**（⑷⒝）・台帳 810・snapshot 249・追跡 `.lys` 599・ディスク 17236。**掃き 925 冊（base ba70753f Release＝`scratch/p362/wt-base`・`sweep362.ps1`・`compare2.txt`）: MOVED 55＝追跡 15（lp-regression 5＝`chord-tremolo`・`dot-rest-beam-trigger`・`dot-rest-horizontal-spacing`・`dots`・`laissez-vibrer-arpeggio`／lpreg 8／fixture 3＝`dot-cross-voice-spacing`・`dot-force-down`・`lv-meterchange`）＋ユーザー 40**（機構 1 だけなら 57＝`compare.txt`・機構 2 で LV 3 冊が加わり 5 冊が戻った）。**LV 2 冊も LP 側へ**: `laissez-vibrer-arpeggio` 23.65／39.80 → 23.25／39.00（LP 23.328／39.161＝+0.32 → **−0.08／−0.16**・残りは半タイの `extra-spacing-height (-0.5 . 0.5)` 未移植＝`AddSemiTies` の remark）・`lv-meterchange` 34.71 → 34.60（LP 34.471＝+0.24 → +0.13・別島）。**台帳 815 点全緑**（810 不動＋新 5＝residual 0）。**終了時**: **full `scratch/p362/run2.trx` 7562 / 3 / 4 / 7569（+5＝台帳 5 点・赤 3＝下の snapshot 3 枚だけ）**・build 0 エラー／Core 0 警告・**§7.5（作業ツリー・`git diff ba70753f -- LilySharp.Core`）Core `+` 274／LILYPOND-REF 10／LILYSHARP-OWN 1**（読み直し: REF＝`separation-item.cc:163-164`×2・`dot-column.cc:81-84`／`:229-232`・`spacing-interface.cc:36-97`・`note-column.cc:120-156`・`rhythmic-column-engraver.cc:102`・`dot-column-engraver.cc:52-60`・`paper-column-engraver.cc:246-261`・`define-grobs.scm:1278`（移動）。OWN 1＝⑵ の Dots.direction。式は 1 本も足していない: 予約側が描画側の `OffsetX` を読む・skyline の要素集合を 2 つに割った）・snapshot 249／追跡 `.lys` 599 不動・commit 3 本（`cbd0ad00`・`829ed1c6`・`b39f396d`）＋この HANDOFF＝未 push 4。⚠️ **承認ゲート**（出力が 55 冊動く・snapshot 3 枚再ベース＝`test/dot-cross-voice-spacing`・`test/dot-force-down`・`test/lv-meterchange`）: 第 1 便末は作業ツリー＝承認待ち（patch `scratch/p362/reserved-dot-column.patch`）→ **第 2 便で承認・commit 済み（下）**。⚠️ `LilySharp.Cli\bin` の Debug／Release は patch 入り。⚠️ `git worktree`: `scratch\p362\wt-base`（ba70753f）を足したまま（p361 のは外した）。⚠️ scratch は **`p362`**（p361 は第360）。⚠️ §7 3.5: 第359 のブロックを `-Archive 359` で ARCHIVE の先頭へ。

✅ ★★★ **第 2 便（ユーザー「リベースを承認する」）＝`scratch/p362/rebase-snapshots.ps1`（`LILYSHARP_UPDATE_SNAPSHOTS=1` で 247 本・env を消して再実行 247 全緑・**動いた snapshot は意図した 3 枚だけ**）→ full `scratch/p362/run3.trx` **7565 / 0 / 4 / 7569**（台帳 815／snapshot 249／追跡 `.lys` 599 不動）→ 製品 commit **`86788fe3`**（16 ファイル +535/−271＝Core 6＋Tests 2＋snapshot 3＋CHANGELOG＋台帳 JSON／probe＋APPROXIMATIONS／magic_constants）＋この HANDOFF。push はユーザー（未 push 6）。**

✅ ★★★ **第 3 便（ユーザー「このセッションでやる方が有利なら着手」→ ⒞① は p362 の道具（base 掃き 925 冊済み・pair.ps1・LP 双子の数）がそのまま使えるので着手）＝⒞① を測って移植した（製品 commit `cbfb6ef0`・snapshot 0＝承認不要）**。**予測は外れて反証条件が拾った**: 台帳 `semi-tie.lv-to-arpeggio-gap`（`probes/semi-tie-spacing.ly` LVA＝`laissez-vibrer-arpeggio` そのまま）の header は Y 帯（`extra-spacing-height`）を犯人と書いたが、**dump の LaissezVibrerTie X extent は頭右＋0.16..＋1.34（Lily# の曲線は ＋0.2..＋1.3）＝stencil が曲線の箱を線幅の半分（0.8×0.1/2＝0.04）だけ全辺に広げる**（`lookup.cc:483-516 bezier_sandwich` の `b.widen(0.5*thickness)`・`tie.cc:225-253 Tie::print`）。rod＝tie 右＋0.1＋0.1＋0.1＋1.3＝**4.2442 exact**・Lily# 移植前 **−0.040000**（予測どおりの桁）→ 移植後 0。**形**: `TieVariantEngraver.LineThickness`（0.8×line）＋`ExtraSpacingHeight`（0.5・`define-grobs.scm:2037`／`:2932`）を `AddSemiTies` の箱に（X ±0.04・Y ±(0.04＋0.5)）。**対**: `laissez-vibrer-arpeggio` 23.33／39.16＝LP 23.328／39.161（前は −0.08／−0.16）・`lv-meterchange` 34.60 不動（+0.13 は別島）。**掃き（head2→head3）: MOVED 2**＝`lp-regression/laissez-vibrer-arpeggio`・`lpreg/lvarp`（ユーザー 0・snapshot 0）。**full `scratch/p362/run4.trx` 7566 / 0 / 4 / 7570（+1＝台帳点）**・台帳 816・§7.5（`86788fe3`→HEAD）Core `+` 35／REF 6／OWN 0。Y の ±0.5 は観測者無し（この本では tie の帯が arpeggio の帯の内側）＝宣言の写し。

⇒ ★★★★ **⑺ 次の一手**: **⒜ push はユーザー**（未 push 8）。**~~⒝ 承認ゲート~~＝第 2 便で適用・commit 済み**（patch は `scratch/p362/reserved-dot-column.patch` に残骸として残る）。**第 4 便は着手しない**（同じ指示の 2 度目）: p362 の道具で測れる残りは ⒞②（remark の訂正だけ・出力不変）と ⒞③（model に Dots.direction を焼く＝観測者未計数）で、②は値打ち小・③は別便の設計。**⒞ この便の拾い箱**: ~~①`AddSemiTies` の `extra-spacing-height`~~＝第 3 便で閉じた（実体は線幅の半分だった）／②`CreateLeftSkyline` の「spring min は conditional を読まない」remark は源（`spacing-interface.cc:87-89`）と食い違う＝remark の訂正だけ（式は正しい）／③`Reserved` の OWN＝Dots.direction を item に焼く日（観測者は下向き旗の帯に沈む点の本＝未計数）／④第360 ⒞ の残り＝③drhs の臨時記号側 +0.19・④bos bar 1 の −0.05・⑤slash の +1.1・⑥ps2 の 0.017。**⒟ 第358 ⑺ の並び（旗 reach 島の分離・`time` 再印字＋全音符の +1.0）はそのまま**。
