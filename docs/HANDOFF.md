...
最終更新 第360セッション（2026-09-10）＝**入り方は第298〜第358 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言い・background job・口挟み 0）。第359 ⑹ に「決定済み・未実装」は 0、p360 の道具（隔離 VS Code）で閉じる残債も 0 だったので、**第333 から §1 の並びに載り続けた「`beam-over-stem` bar 2 の +0.33（2 声の 16 分と 8 分の対・未診断）」を p359 の LP 計器で診断した**。**測ったら病名は 3 つ＋付随 2 つ**で、全部移植した。**製品は第 2 便でユーザー承認（「リベースを承認する」）→ 再ベース 17 枚 → commit `ec7acf84`**（32 ファイル +1088/−712＝Core 7＋Tests 5（新規 `CrossVoiceColumnSpacingTests`）＋CHANGELOG＋APPROXIMATIONS／magic_constants＋snapshot 17）。patch の残骸は `scratch/p361/crossvoice-spacing.patch`。**骨は 7**:

★★★ **⑴ 計器**（`scratch/p361/lp/`）: `settings-wish.ly`＝NoteSpacing の **left-items／right-items の列の when**（PROBEWISH）・PaperColumn の horizontal-skylines（PROBECOLSKY）・Stem の X/Y extent／`settings-dots.ly`＝＋Dots／NoteHead の列枠 X extent／`settings-bar.ly`＝BarLine の列枠 extent。`pair.ps1`（p359 の写し・`-Exe` で base／head の lysc を差し替え・`-Springs`）。⚠️ **LP の `ideal-distances`／`minimum-distances` は after-line-breaking では列にも `ly:grob-original` にも空**（第358 ⑺ の言い分どおり）＝**ばねは wish の有無と列 skyline から手で組む**のが最短で、この便は全部それで割れた。Lily# 側は捨て計器 `scratch/p361/ZzP361ProbeTests.cs.txt`（CreateTimingSprings → ApplyCrossVoiceColumnSpacing → ApplySharedColumnReservations の 3 段のばねと、cross pass の対ごとの skyline/rod・shift を刷る）。

★★★★ **⑵ 機構 1＝LP の NoteSpacing wish は *Voice ごと*の鎖**（`note-spacing-engraver.cc:31-37` の `last_spacings_`／`last_spacing_` は engraver のメンバー＝1 Voice に 1 つ・parent で引くのは `\change Staff` のため）。**PROBEWISH（bos bar 2 `b8 b s2.`／`s16 d''4 s8. s2`）: 声 1 の b8 の right-items は自分の次の b8（9/8）だけ・声 2 の d''4 の wish は小節線（2 と 3）だけ** ⇒ **b8→d''4 と d''4→b8 は wish 無し**＝`musical_column_spacing` の `springs.empty()`（ideal＝素の fraction×duration_space＝1.2・min 0）で、**他声のインクは rod（`set_column_rods`＝列 skyline 距離 1.5042＋0.1＝1.6042 exact）でしか届かない**。Lily#: ⒜ **7efd0fbd（2026-08-09）の `prevStaves` 分岐が「map は Staff 単位」と読んで cross-voice 対を wish にしていた**（skyline min＋0.3＝1.8042）→ 削除・`CollectStaffIndicesAtIndex` も退役（`CreateTimingSprings` の引数 1 つ減）／⒝ `ApplyCrossVoiceColumnSpacing` は **cross-voice 対に rod だけ・same-voice の shift 対だけ sky＋headroom**。**2 冊目 `test/dot-cross-voice-spacing`（dcv）: 声 3 の cis2. の付点→声 2 の d8 は 3.3295＝0.1＋(0.652＋1.3774＋0.45＋0.45＋0.2)＋0.1＝rod**（Lily# は sky＋0.3 で 3.28）。**wish の skyline も自分の声の NoteColumn だけ**（`spacing-interface.cc:36-97`＝left/right-items の Separation_item）。

★★★ **⑶ 機構 2＝skip は wish の端点ではない**（grob を作らない＝`acknowledge_rhythmic_grob` に来ない）: `MeasureLayouter.ItemStartingAt` が spacer を返し、声 2 の `s8.` が d''4→b8 を wish にして頭幅 refinement +0.1042（LP は素の 1.2）→ **spacer は null**（`anyWish` と `wishLefts` は同じ事実になった）。

★★★★ **⑷ 機構 3＝`voice { }` で向きを返された梁の pure tip が反転していた**: `ResolveBeamStemDirections` は BeamDetector の音高由来の向き（`b8 b` は中線→下）で `PureBeamedStemTip = −6.75` を焼き、あとから `ResolveVoiceStemDirections` が向きだけ上に返して tip を残す→ **符尾の帯が (−6.75 .. 0.37)＝箱が頭の下・小節線への stem correction 0.1562（LP 0.1296＝(4−0.3724)/7×0.25）**＝bar 3（単声と同じ音楽）にも +0.026。→ `RebakePureBeamedTips`（BeamId ごと・forced 向きの構成員の unbeamed 帯の極値＝`stem.cc:399-418` の同向き stems の union）。**掃きで動いた `multivoice-*`／`drum-groove`／`voice-*` の主因はこれ**（梁の箱が頭の下に出ていた）。

★★ **⑸ 付随 2 つ**: ⒜ **Dots の `extra-spacing-width (0 . 0.2)`**（`define-grobs.scm:1278`・dcv の列 skyline 3.1295＝2.9295＋0.2・`test/dot-force-down`（dfd）の rod 3.908＝0.1＋3.5084＋0.2＋0.1・`dot-rest-horizontal-spacing`（drhs）の r16. の箱 2.3＝2.1＋0.2＝3 冊で確認）→ `ItemSkylineFactory.AddDots` の箱（`SpacingRules.DotsExtraSpacingWidthRight`）。**単独で 41 冊動く**（追跡 10・ユーザー 31）。／⒝ **wish の頭幅 refinement `left_head_end = g->extent(col, X)[RIGHT]` は*列枠＝衝突 shift 込み*で、merge_springs は wish の ideal を平均する**（台帳 TSU `a1`／`b1` の 2 度: 最初の対 7.042＝(6.058＋8.026)/2＝片方の全音符が 1 頭幅 shift・shift 盲目だと 6.058／`test/collision` の 2 分音符 2 度も同型）→ cross pass で same-voice 対に Σshift／N を足す。**これが無いと TSU の 6 小節が 1 段に収まり、台帳 `system.tuplet-bracket-up` が「page 0: 1 staff」の例外で落ちた**（第 1 の full で見つけた）。

★★★ **⑹ 数と射程**: **開始時裏取り**（`tools\Session-Check.ps1 -Build -Test -Scratch p361`）: HEAD **`7f3be23e`**・**未 push 44**・`origin/master`＝`cf6edb6f`・木 clean・未追跡 0・build 0 エラー／Core 0 警告・**full 7552 / 0 / 4 / 7556**（`scratch/p361/run1.trx`＝第359 終了時の赤 1 を再生成した数と一致）・`gh run list` 緑（最新 34286219497）・台帳 810・snapshot 249・追跡 `.lys` 599・ディスク 15705。**対**（raggedRight・LP 2.26.0）: bos bar 2／3 **20.93／20.63＝LP 20.928／20.628**（各 leg 1.6042／1.2000／17.0338 まで一致・bar 1 は 37.5 対 37.554＝前は +0.35）・dcv 32.55 対 32.701（−0.15＝DotGap 島）・dfd 16.55 対 16.702（同 −0.15）・tsu 23.25 対 23.443（−0.19）・coll（`test/collision` 2 声）10.40 対 11.086（−0.69・前は −1.38）。**掃き 924 冊（base 7f3be23e Release＝`scratch/p361/wt-base`・`sweep361.ps1`）: MOVED 70**＝機構 1〜3 だけで 29（追跡 26＝lp-regression 6・lpreg 8・fixture 12／ユーザー 3）・Dots esw で 41（`compare-nodots-head.txt`）・⒝ の shift refinement は最後の build（`compare.txt`＝70）に含めた。**snapshot 17 枚**（`beam-over-stem`・`beam-under-staves`・`cross-voice-accidental`・`dot-cross-voice-spacing`・`dot-force-down`・`drum-groove`・`multivoice-beam-collision`・`multivoice-crossing-collision`・`multivoice-tuplet-beams`・`tuplet-bracket-whole-notes`・`voice-dynamics-offbeat`・`voice-tuplet`＋⒝ で `cue-region-measure`・`multi-voice`・`multivoice-voice2-tuplet`・`multivoice-beams`・`collision`）。**番人 `CrossVoiceColumnSpacingTests` 6 本**（LP の 1.6042／1.2／17.0338／20.928／20.628 を釘・skip は端点でない・返された梁の tip）＋`CrossVoiceDotReach_FloorsTheNextColumnsSpring` を rod の形に書き換え（Lily# の rod＋(0.45−DotGap)＝LP 3.3295 を釘）＋`PercentRepeat` の slash 幅を 1 桁に（絶対 X の丸めで 1.17/1.18 が揺れる）。**終了時**: **full `scratch/p361/run3.trx` 7541 / 17 / 4 / 7562（+6＝`CrossVoiceColumnSpacingTests`・赤 17＝上の snapshot 17 枚だけ）**・**§7.5（作業ツリー・`git diff -- LilySharp.Core`）Core `+` 240／LILYPOND-REF 10／LILYSHARP-OWN 0**（読み直し: REF＝`note-spacing-engraver.cc:31-37`／`:81-91`・`spacing-interface.cc:36-97`・`spacing-spanner.cc:228-297`・`note-spacing.cc:46-77`・`spring.cc:101-129`・`define-grobs.scm:1278`・`stem.cc:399-418`／`:449-458`。OWN 0＝式は 1 本も足していない: 分岐の削除 2・rod だけの読み替え・箱の esw・tip の焼き直し・shift の平均）・台帳 810 不動（`LpGeometryLedgerTests` 全緑＝この regime を観る点は TSU 以外に無い）・snapshot 249／追跡 `.lys` 599 不動。⚠️ **scratch は `p361`**。⚠️ §7 3.5: 第358 のブロックを `-Archive 358` で ARCHIVE の先頭へ。⚠️ `LilySharp.Cli\bin` の Debug／Release は patch 入り。⚠️ `git worktree`: `scratch\p361\wt-base` を足したまま（`git worktree remove scratch\p361\wt-base` で外す）。

✅ ★★★ **第 2 便（ユーザー「リベースを承認する」）＝`scratch/p361/rebase-snapshots.ps1`（`LILYSHARP_UPDATE_SNAPSHOTS=1` で 247 本・env を消して再実行 247 全緑・**動いた snapshot は意図した 17 枚だけ**）→ full `scratch/p361/run4.trx` **7558 / 0 / 4 / 7562**（台帳 810／snapshot 249／追跡 `.lys` 599 不動）→ 製品 commit **`ec7acf84`**＋この HANDOFF。push はユーザー（未 push 47）。**

✅ ★★★ **第 3 便（ユーザー「このセッションでやる方が有利なら着手」→ ⒞① は p361 の計器と base exe がそのまま使えるので着手）＝⒞① を測って 2 つに割り、両方移植した（製品は作業ツリー＝承認待ち・patch `scratch/p361/collision-shift.patch`＝7 ファイル +157/−40・Core `+` 41／REF 3／OWN 0）**。**⑴ coll（2 分音符 2 度）の −0.69 は shift の量ではなかった**: 捨て計器 `ZzP361bProbeTests.cs.txt`（`ComputeVoiceOffsets` を刷る）で Lily# の shift は 1.3774＝LP と同じ（touch → −0.5 → `automatic_shift` が UP +0.5／DOWN −0.5 → `left_most` で 2×0.5×wid）。**欠けていたのは閉じばね（最後の列→小節線）の shift 込み refinement**（第 1 便 ⑸⒝ は列対だけに足していた・LP は最後の音の wish も小節線の列を名指す＝`note-spacing-engraver.cc:109-120`）→ `ApplyCrossVoiceColumnSpacing` の末尾で `columns[^1]` の Σshift/N を閉じばねに足す。**coll bar 10.40 → 11.08（LP 11.086）・小節線 18.63／29.71＝LP 18.629／29.715**。**⑵ tsu（全音符 2 度）の 1.5696 は LP の duration log と Lily# の分母の取り違え**: `note-collision.cc:191-193` の「distant_half_collide && up_ball_type <= 0＝wholes and longer は full」を `noteValue <= 0`（＝breve）と綴っていたので全音符が 0.4 の distant-half（2×0.4×1.962＝1.5696）を取っていた → `<= 1` で full 0.5＝**1.962＝LP**（PROBEHEADX: up voice の a1 が (1.962 . 3.924)）。**同じ取り違えが 2 つ隣に**: `:102-104` の「whole は merge しない」も `<= 0`（全音符同士が merge できた）／`:116-126` の「half と quarter は merge しない」を (1,2)＝whole/half と綴っていて half+quarter が `merge-differently-headed` で merge した（LP の regression texidoc「open head と merge できるのは 8th 以下」）→ (2,4)。**`NoteCollisionTests` の 2 本がその誤りを釘にしていた**ので書き換え（half+eighth は merge・half+quarter は Theory で never）。**tsu 23.44／23.45＝LP 23.443**。**番人 `CrossVoiceColumnSpacingTests` +2**（閉じばねの shift・全音符 2 度は full collide＝up が 1.962）。**掃き（`head3`）: この便だけで MOVED 8**（追跡 7＝`collision-seconds`・`collisions`・`dot-column-note-collision`・`dot-column-vertical-positioning`・`stclash`・fixture `collision`・`multi-voice`／ユーザー 1＝`voiceissue.lys`）・**snapshot 2 枚（`test/collision`・`test/multi-voice`）**・base からの累計 72。**full `scratch/p361/run5.trx` 7558 / 3 / 4 / 7565（+3＝番人・赤 3＝snapshot 2＋`CitationsThatNameNothing_DoNotGrow`＝REF 行に記号名が無かった→ 直して focused 86 本緑）**・台帳 810 不動。✅ **第 4 便（ユーザー「承認する」）＝`scratch/p361/rebase-snapshots2.ps1`（247 本・env を消して再実行 247 全緑・動いた snapshot は意図した 2 枚だけ）→ full `scratch/p361/run6.trx` **7561 / 0 / 4 / 7565**（台帳 810／snapshot 249／追跡 `.lys` 599 不動）→ 製品 commit **`5a4eb274`**（9 ファイル +216/−99）＋この HANDOFF。push はユーザー（未 push 50）。**

⇒ ★★★★ **⑺ 次の一手**: **⒜ push はユーザー**。**~~⒝ 承認ゲート~~＝第 2 便で適用・commit 済み**。**~~⒝′ 第 3 便の patch~~＝第 4 便で適用・commit 済み**。**「有利なら着手」の判定（第 4 便末）: 着手しない**——p361 の計器で測れる残りは ⒞②〜⑥（DotGap は EngravingDefaults の記録どおり台帳点を先に・drhs の臨時記号側・bar 1 の −0.05・slash・ps2 の 0.017）で、どれも観測者 1 冊か別 regime＝この便の道具の利が無い。

✅ ★★ **第 5 便（ユーザーの問い「lys フェンスの `score {}` は省略できない方が良いのでは」→ 私も賛成（暗黙 score は歌詞行・和音行・tab を*黙って*落とす／`GenerateAll` の single-voice fallback と別の「score 無しの絵」の 2 綴り目／例がそのまま `.lys` になる）→「着手して」）＝§3 の決定を書き換え（score はちょうど 1 個・0 も 2 以上も拒否）・`RenderSpecParser.ImpliedScore`／`SvgGenerator.GenerateForSpec` 退役・LSP の fence 分岐は score 0 個で「declares none. Write score { … }」・`RenderTextRequestTests` は暗黙 score 3 本 → 拒否 1 本＋「form が 2 つでも score があれば描く」1 本・CHANGELOG（Editor）・拡張はコメントのみ（`markdownFence.ts`）・`ws3/notes.md` の例に score を足した。**full `scratch/p361/run7.trx` 7560 / 0 / 4 / 7564（−1＝3 本退役 +2 本）・build 0 エラー**・commit **`5a76829e`**（Core 2＋Lsp 2＋Tests 1＋CHANGELOG＋拡張 1）＋この HANDOFF。**配布済み**（`Deploy-Lsp.ps1`＝`0.6.0+5a76829e`・95 ファイル byte 一致・manifest 不変＝**Reload は 1 回**・script 末尾の `Get-Process lilysharp-lsp` の 3 エラーは名前の空振りで無害）。push はユーザー（未 push 53）。**⒞ 拾い箱（この便が測って名指しただけ・未起票）**: ~~①衝突 shift の量~~＝第 3 便で閉じた（承認待ち）／②`DotGap` 0.3 対 LP 0.45（`EngravingDefaults` に記録済み・台帳点が要る・dcv／dfd の −0.15）／③drhs の r16.→cis'♯ +0.19（base から在る・臨時記号側）／④bos bar 1 の −0.05（16 分 4 つの梁の対）／⑤`slashprobe` の +1.1（percent の別島）／⑥ps2 の 0.017＝c2→e4 の wish 無し対が LP 2.066 対 素の 2.049（**LP 側が素の ideal より 0.017 広い理由は未特定**）。**⒟ 第358 ⑺ の並び（旗 reach 島の分離・`time` 再印字＋全音符の +1.0・⒞⒟⒠）はそのまま**。
