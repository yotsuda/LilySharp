...
終了時: **commit 6 本（`c47ebd4c`＝テスト 2 ファイルのコメント＝⑹／`0c445991`＝HANDOFF＋`HANDOFF-ARCHIVE.md`＝第330 の経緯を末尾へ逐語／`aa52e1c1`＝第 2 便＝⑺／`2f22dbfb`＝第 3 便＝⑻／`6408fc6f`＝第 4 便／第 5 便＝この HANDOFF＝⑼）・未 push 37・製品コード 0 変更・台帳／`.lys`（追跡）／snapshot 不動**。**実コーパス（git 管理外）は第 3 便で動いた**＝`@mark` 171 冊（原本 `backup-marks/`）＋手 4 冊（`backup-hand/`＝⒟ 3 冊の `.lys`・Endless Story の `.ly`）。（scratch に動いたのは `scratch/p332/` ＝`marks332.ps1`／`.csv`・`t6c/`（計器・`out-marks/` 171 冊・`out-marks-v1/`・`-v2/`・verify／report／detail）・`run1.trx`／`run2.trx`）。**full 7106 / 0 / 4 / 7110（`run2.trx`・§1 書き換え後・4 分 41 秒＝ビルド込み）**。

⇒ ★★★★ **次の一手**: ✅ **⑴ ⒞ は当てた（⑻）**。残る 179 札（94 冊）は手の 4 群（境界近傍 134・休符 120・`'` 落ち 42・section 名違い 55）＝`.lys` の section の切り方の問題。**⑵ ピックアップ（⑻⒟）の直し方をユーザーに訊く**: **(A) 割れた小節を前の section へ戻す**（`~Body_1 { … r2. b,,4\4 | }`＋`section Intro { repeat percent …`＝LP と同じ 1 小節・機械で当てられる＝census の「section 末尾の短い小節 ＋ 次 section 先頭の短い小節の和が拍子」の対）／**(B) `section Intro { partial 4 }` を書き、前 section 末尾の 3/4 休符を消す**（LP 風の pickup 彫版・機械で当てられるが休符 1 つが消える）／(C) 触らない。Disco Inferno（volta ending の頭 1/2）はどちらでも手。**⑶ T7 C の冊ごと判断（3c）の材料**: Rose Garden J＋K（20 小節・form main に無い）／She Bangs D2（8・`form unfold` だけ）／All Star Outro（2）／A Thousand Miles B3（10・form main に無い）＝それぞれ form に戻すか意図的省略か。**⑷ ✅ ⒟ は済（⑻⒝）**。**⑸ T6 ⒥ は 6b＝触らない（決定）**。★★★★ **⑹ 次便の入り口＝⑼ の製品欠陥「percent 反復の空小節を隠した音符の幅で組む」を LP の空小節 spacing で直す**（再現 `scratch/p332/t7/pc2.lys`・数字は ⑼・住所 `MeasureCollector.cs:3478` の展開と、その音符を読む spring 構築）。**LP 側の式**: 1 小節 body の `PercentRepeat`（`define-grobs.scm:2788-2803`）は **`springs-and-rods = ly:multi-measure-rest::set-spacing-rods`＝MMR と同じ rod**（Lily# の `R1` 経路がそのまま使える）／2 小節 body の `DoublePercentRepeat`（`:1290-1309`）は rod を持たない break-aligned Item で、空小節の幅は `lily/spacing-spanner.cc:478 Spacing_spanner::breakable_column_spacing` → `:515 standard_breakable_column_spacing`（音符列の無い 2 本の breakable column の間）／`lily/percent-repeat-iterator.cc`（2 回目以降は PercentEvent のみ＝音符は無い）。**射程は percent の本 113 冊のうち 2 小節 body（`Double`）の本**（1 小節 body は偶然一致）——**着手前に `.lys` で `Double` を数えること**。**門**: `pc2` が LP と同じ 8 小節 1 段＋percent 小節 7.5〜7.7 ss・structure332 の SigMatch が増えて減らない・snapshot の動きは LP 照合の上で承認。**⑺ 併せる側（カムフラージュ 6→13・未来予想図Ⅱ 8→16）は双子で再現しないので手書き `.ly` の設定の差＝製品ではない**（追わない）。**⑷ 以下は第330 の並び**: push 後 `gh run list`（`extension-tests` job 込み）／✅ T7 の C の残り 11 冊は ⑺ で判定済み（直しは全部ユーザー側＝form 4 冊・`.ly` 1 冊・ピックアップ 4 冊・既知 2 冊）／`audit/lpreg` 取り直し／§2 U8c・U8b・U8／§2 A/B/D/E／§2 C⑴／§2 G／`:|.|:` の幅 0.08／smart typing の小穴／第330 ⑸ の overlay 群。⚠️ **承認待ち・リリース 0.6.0 の bump／tag は第328 と同じ。**
...
- **T7. ▶ 起票（第321）＝絵の突き合わせ（286 冊全数）の結果＝族の表。計器は `scratch/p321/`（§1 ⑴）**
  ...
  **上の 388／382 は代用対（both の Lily# score を単譜の LP book と比べた 384 対）を含む数で、もう指標にしない**（第323 ⑶′⒜）。
  ...
  ★★★★ **B-eng の製品側の正体＝第332 ⑼（percent 反復の空小節の幅）**: LP 側の基線は `scratch/p332/structure332.csv`（KindMatched 209 / SigMatch 140 / 段違い 50 / 小節数違い 19）、break の位置まで一致して段割れが違う 27 冊は `scratch/p332/beng332.csv`、最小再現は `scratch/p332/t7/pc2.lys`（LP 8 小節 1 段・Lily# 2＋3＋3・空小節 7.5〜7.7 対 32.5 ss）。**次便の入り口**（§1 第332 次の一手 ⑹）。
