...
最終更新 第372セッション（2026-09-12）＝**入り方は第298〜第371 と同じ**（ユーザーは `docs/HANDOFF.md` を読んで着手せよとだけ言った。道具の指示は無し＝記憶どおり ripple ではなく pwsh MCP で作業した）。**第371 が「次の一手」として残した `staff→chord` の −0.851723138 は、engine ではなく*計器*の欠陥だった**——残差そのものが存在しなかった。**骨は 8**:
...
★★★ **⑺ 数**: **開始時**（`Session-Check -Build -Test -Scratch p373`）: HEAD **`18efc322`**・**未 push 0**（⚠️ **ユーザーが第368〜第371 の 28 本を 7 本に畳んで push 済み**）・木 clean・未追跡 0・build 0 エラー／Core 0 警告・**full `scratch/p373/run1.trx` 7782 / 1 / 4 / 7787**。⚠️ **赤 1 は squash 孤児**（`DeadCitationsDoNotGrow` 651 対天井 635）＝**記憶 `reference_lilysharp_squash_orphans_doc_citations` の形そのもの**。**孤児 28・引用されている 17・うち a-f が 2 文字未満 1 ⇒ 予測 +16 が実測と一致**・**畳む前後の tree が同一オブジェクト**（reflog に残る旧 tip と新 tip。⚠️ **その旧 SHA をここに書いてはいけない**——書いた瞬間 dead が 1 増えて、今上げたばかりの天井を割る）⇒ **天井を 651 へ**（commit `9f6df2a3`・remarks に旧 SHA は書かない）。⚠️⚠️ **`origin/master` の CI は今この赤で落ちている**（run 34663615384）＝**この commit を push すれば緑**。**終了時: full `scratch/p373/run6.trx` 7788 / 0 / 4 / 7792**（+5＝番人 1・台帳 4。HANDOFF／CHANGELOG／APPROXIMATIONS 込み・run5 は文書を入れる前の同じ数）・**台帳 825 点／OPEN 0／ss 非ゼロ 213／総和 27.896261602 → 24.517662441（−3.38 ss）／exact 647**・**snapshot 249（3 枚再ベース）／追跡 `.lys` 599**・commit 4 本（`9f6df2a3`＝天井／`a6d31bc6`＝計器と台帳 4 点／`d2e42b39`＝engine と snapshot 3 枚と新しい対＋この HANDOFF）。⚠️ scratch は **`p373`**。⚠️ §7 3.5: 第370 を `-Archive 370` で ARCHIVE へ（済）。

⇒ ★★★ **⑻ 次の一手**: **⒜ ユーザー決定 1 件＝`Dmaj7` の既定**（上 ⑷。`words` のままなら +0.337483977 は「決めた差」として台帳に残り続ける／`symbols` 相当を既定にすれば 0 になる。**LP の既定は三角**）。**⒝ 残った face の島**＝`Em7` の上付き数字が Nimbus Sans では 0.0277 高い（上 ⑹）——これは「どの sans を同梱するか」の島で、`page.chord-row.staff-to-chord-baseline` と同じ決定。**⒞ 第369 ⑸⒝ の並びはそのまま**（tab の wish の merge＝設計級）／第368 ⑺⒝ の並び（slash 箱 2 軒・chord quality 35 種・`raggedLastBottom`・第362 ⑹⒝②〜④・第361 ⑺⒞②③④⑥）もそのまま。**push はユーザー（未 push 4）**。
