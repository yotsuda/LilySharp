...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第238セッション＝**残債の返済を中断し、ユーザーが目で見ているリリースブロッカーを直した。歌詞の行が五線の音符を貫いており、5807 本・snapshot 222 枚・台帳 566 点は 1 つもそれを見ていなかった**（2 commit＝`65d5e048` 修正＋観測者 2 本／この行）。⚠️ **本便は engine を動かしている**（`LayoutEngine`／`LyricEngraver`）——にもかかわらず**台帳・snapshot・追跡コーパスは 1 バイトも動かない**。**それが症状ではなく、この便の主要な発見**（②）。
...
終了時: 本便 3 本（`65d5e048` 修正／`be5273fb` handoff／この行＝閉幕の訂正）＝**未 push 1**。⚠️⚠️ ★★ **ユーザーが本便の途中で push した**＝`git log -1 origin/master` が `be5273fb`＝**本便の commit を指した**（§0 の第201 の形）——**開始 0 ／ 第1便で 1 ／ 第2便で 2 ／ そこで push ／ 終了 1**。**両端（0 → 1）だけ見ると便数とも commit 数とも合わない**ので、この行が唯一の記録。⚠️ **`be5273fb` は push 済みなので amend せず、訂正を 3 本目として足した**（履歴の書き換えは第235〜237 が 3 か所で払った）。・未追跡 0/木 0・suite **Windows 5809/0/4・WSL/Linux 5809/0/4**（開始比 +2＝観測者 2 本。**両 OS とも `65d5e048` で実測**）・**台帳 566／ss 非ゼロ 110／総和 3.876038461／count 107・非ゼロ 2・snapshot 222・追跡コーパス 572＝完全不動**・Core 0 警告。**ref は 1 本も動かしていない**（§1 骨 4・第237）。⚠️ **修正 commit は push 前に 2 度 amend しており、置き換えられた 2 本は孤児になっている**——**その SHA はここにも書かない**（第236 の警告どおり。**孤児は名前を書いた瞬間に dead 引用になる**＝骨 4）。
