...
終了時: **commit 4 本**（**本体 `d6bf1e1b` ／ docs `b296ed7e` ／ 数を閉じる `4093ffe5` ／ 射程の取り直し（この commit）**）＝**未 push 33**（**この commit を含む。3 本目までの時点で 32 だったのを確かめてから書いた**）・木 0・未追跡 0。**suite 6684 合格 / 0 失敗 / 4 skip / 合計 6688**（開始 6674 から **+14**＝**本便の網 15 ケース − 番人から消えた 1 行**）。Core 0 エラー 0 警告。**台帳・追跡コーパス・snapshot は開始時と同じ**（748／588／190／24.714570188／158 中 2／`OPEN:` 0／581 冊／231 枚）。⚠️ **`docs/APPROXIMATIONS.md` も `audit/magic_constants.csv` も 1 行も動いていない**——**本便は定数を 1 つも足していない**（**足した `LILYPOND-REF` は*決定*の出所であって量ではない**）。
