...
**未 push 42**・テスト **3770 passed / 0 failed / 3 skipped**（**+17**）・
...
★★★ **ゲート ⑹ を決める**（**設計判断**＝`instrument` を exporter に読ませて双子に `clef` を書くか）。
**④'''' で 6 冊ぶんと確定**（tab 5 ＋ `fermata-down`）。**`clef` を書いている 2 冊だけ記譜譜が
一致している**ので、**書けば残り 6 冊も記譜譜は揃うはず**——⚠️ **これは予想であって実測ではない。**
★★ **そのあと tab の梁の高さ**（**要承認＝描画が動く**）。⚠️ **⑦ の「向きが逆」は ⑩ で消えた**ので、
**残っているのは高さだけ**（`tab-percent-repeat`: LP `1.50/1.87/3.00` 対 Lily# `1.80/2.39/1.80`・
**差は一定でない**）。**`SharedRenderer.Tab.cs` の「LP と同じく上を向く」というコメントは実測どおり正しい。**
