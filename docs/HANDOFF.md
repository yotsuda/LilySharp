...
### 1.1 第570セッション（2026-09-24・YT-DELL2）

同じ会話の続き。★ `-Start p570`（HEAD `9049fb78`・full 8987 / 0 / 2 / 8989・`-Archive 568`）＝HANDOFF の 450 KB 天井で便を切った（§1 の字数ではない・畳める §2/§3 は 0 件だった）。作業ツリーの `samples/*` はユーザーの手。

★★★ **⑴ 歌詞のメリスマを LP の `\lyricsto` に（ユーザー決定「LP に合わせて」・§3 に行・実装は第569 の末尾の `9049fb78`）**。`c'4( d e) f` に `la __ la` で Lily# は 2 つ目を e に置いていた（全音符が音節を取り、歌詞側の印だけがメリスマを作る・`__` も 1 音取る）。LP 実測（Lab `sessions/p569/melisma-assign.ly`＝各音節の列の `when`）: スラー 0／3/4・タイ 0／3/4・裸の `la __ lb lc ld` 0 1/4 1/2 3/4・`la _ lb lc` 0 1/2 3/4・小節をまたぐスラー 0 1/2 3/2。実装＝`LyricsCollector.BuildNoteIndices` が busy（`slurMelismaBusy`＝スラーの 2 音目〜閉じる音／`tieMelismaBusy`＝タイの着く音・休符はタイを切りスラーは跨ぐ）を印し、`LyricCollector` は busy でない音符にだけ音節を置き、busy は直前の音節のメリスマ（左寄せ・`MelismaEnd`）に。**`__` は線だけで音符を取らない・`_` と離れた `~` は 1 つ取る**。`LyricHyphen` の最後の延長線の連なりもスラーの閉じまで追う（旧＝開く音の次の 1 音まで）。旧綴りの本は LP の綴りへ: `audit/lpreg/lymelmel.lys`（`__ ~ ~`→`__`・描画 byte 同一）・台帳 LMS/LMN の Lily# 側（`mumum mum`＝LP のプローブと同じ語・2 点不動）・テスト 2 本。**追跡 40 冊で絵が動いたのはその双子 1 冊・snapshot 0**・コーパスに歌詞 0 冊。網 `LyricSlurMelismaTests` 6 本・毒 8 赤。文書: GRAMMAR の LyricSyllable・GRAMMAR_FOR_LLM・SYNTAX_REFERENCE（メリスマの段落と例）・CHANGELOG 0.8.0 Breaking。
  ⇒ 判定: R10 の残りは ⒜（aligned_side の 3 綴り＝大）だけ。次は ⒜ か、§1.0 の他の忠実度項目（R7〜R9・R11 の LP 双子／⒳⁶／⒡′／⒵⁴）。この便の文脈に依らない＝(c)。

## 以下は第569セッションの経緯

### 1.1 第569セッション（2026-09-24・YT-DELL2）

★ `-Start p569`（HEAD `0574ea2e`・full 8976 / 0 / 2 / 8978・`-Archive 567`）。§1.0 の忠実度に着手する前に**ユーザーが優先の作業を差し込んだ**＝文法の変更 1 件。作業ツリーの `samples/*` はユーザーの手（触らない）。

★★★ **⑴ `section ~A { … }` を廃止＝ラベルを隠す `~` は form の参照にだけ書く（ユーザー決定・§3 の行を差し替え）**。ユーザーの論拠: **part-major では `part p1 { section ~A }` と `part p2 { section A }` が同じ section の宣言＝性質の置き場がパートの数だけある**（`FormDeclarationValidator.BuildSectionIndex` は「どれか 1 つに `~` があれば勝ち」で辻褄を合わせていた）。デメリット（ユーザーが自分で挙げた）＝volta 用の短い section は全 form の参照に `~` を書く。**実コーパス 333 冊で測った**: `section ~X` は 101 冊・宣言 342 個、**全冊に form があり、342 個すべて参照 1 回・`~` で表示に反転した参照 0・form を 2 つ持つ本 0**＝デメリットは「書く場所が移るだけ」。移行コストと後方互換は考えない（ユーザー指示）。**エラー文は出す**（ユーザーの問い「後方互換の警告に見えるが親切か」→ 私の答え＝`form { ~A }` からの類推で*今後の*書き手も踏むので現在形の案内として残す・LYS0032 と同じ論拠）。
  実装（`ba73b3eb`）: `SectionLabelRule` から宣言の項を外した（shown = 参照に `~` が無い）＝collector の 4 形・twin の 4 箇所・LYS0012 が参照だけで問う。**LYS0033 `SectionDeclarationTilde`**＝4 つの section parser（top・part-major・lyric・chord track）が 1 つの helper `ReportDeclarationTilde` で報告し、**token は green の slot 1 に残す**（LYS0030 と同じ「報告して保持」＝`ToFullString() == source`）。`LabelHiddenByDefault` は削除。LSP の「宣言の `~` を跨ぐ」処理とそのテストを削除。網 `SectionDeclarationTildeTests` 16 本（旧 `SectionLabelDefaultTests` を git mv して書き直し）: 4 site で LYS0033 が 1 つだけ・span・byte 一致の復帰／**迷子の `~` はラベルを 1 つも変えない**（宣言が再び発言権を持てば赤）／参照だけの真理表を page と twin、section-major と part-major で。fixture 2 つ（`SectionBoundarySplitBarTests`・`audit/tabfingering/automatic.lys`）は `[1. ~E1]` に。**毒**（報告を殺す）で 4 赤を確認。full **8970 / 0 / 2 / 8972**（−6＝旧の反転表 22 ケースと LSP の 2 ケースを外し 16 を足した）。文書: GRAMMAR §5・GRAMMAR_FOR_LLM・GRAMMAR_STATUS・SYNTAX_REFERENCE・CHANGELOG 0.8.0 Breaking・拡張の CHANGELOG・`APPROXIMATIONS.md` 再生成。
  ⇒ **実コーパス 101 冊はユーザーの指示で移行した**（Lab に commit・100 冊＝`The Final Countdown.lys` はユーザーの未コミットの編集があるので木に置いたまま）: `sessions/p569/migrate.ps1`（宣言 342・参照 338＝同名宣言がヘッダとパートの 4 冊）。**照合＝旧 lysc で変換前・新 lysc で変換後の svg（data-pos 伏せ）と ly 202 ファイルが全一致**・陽性対照＝変換前を新 lysc に通すと 101/101 が変わり LYS0033 342 件。

★★★ **⑵ タブ譜の梁の高さ（ユーザー報告＝`The Final Countdown` の score "tab"・ユーザー決定・§3 に行）**。**なぜ LP は揃わないか**（ユーザーの問い）: 梁の理想の長さは本数で違い（`beamed-lengths` 3.26／3.5）、五線譜でそれが見えないのは calc_stem_info の「最上の梁は中央線より下に来ない」clamp（stem.cc:1235-1239）が低い音符の群を*全部*中央線まで伸ばすから。**タブの数字は中央から遠くない**ので clamp が効かず、21 小節（E 弦）で 16 分群と 8 分対が 1 段違った。⚠️ **双子（`lysc ly`）はタブを `Staff` で書き出す＝タブは双子の射程外**（未起票）。
  **1 度目（`35d53849`）は外した**: 向きごとに「中央に最も近い弦から 16 分が届く高さ」を下限にした＝全部 1 つの高さ。ユーザー「stem が長すぎる・beam がすべて水平」。⇒ **2 度目**: 下限は中央線に戻し、**タブの梁だけ 1 本梁の理想の長さを 2 本梁の値（3.5）にする**（`uniformBeamedLength`・既定の StemDetails の写し 1 つ）＝同じ弦の 8 分対と 16 分群が揃い、符尾は自分の数字から自然な長さ、弦をまたぐ群は傾く・弦が違えば高さも違う（五線譜で音高が違う小節と同じ）。**台帳は斜めの 3 点が EXACT に戻り、平らな 8 分群 2 点だけ ±0.373333（1 quant）**＝宣言した乖離。snapshot は作業前から 6 枚（目視＝8 分群が 1 quant 上がっただけ・記号は梁を避ける）。網 `TabBeamLengthTests` 3 本（揃う／弦で高さが違う／傾く＝後の 2 本は 1 度目の形に戻さない番人で、この毒では緑）・**毒（`uniformBeamedLength: false`）で 9 赤**。full **8973 / 0 / 2 / 8975**。
★★ **⑶ R10⒠⒣（§1.0 の方針どおり LP 忠実度）**: ⒠ `9eead2eb`＝通奏低音の数字がスカイラインに差し出す箱が LILYSHARP-OWN の中央寄せ 0.8（描いたインクより 0.4 左・段間の seed は半幅として 1.6）だった→`FigureXExtent`＝`[x, x+Width]`（LP 実測 (0 . 0.921869)＝Lab `sessions/p569/figbass-xext.ly`・Lily# の Width と一致）・4 読み手（行の積み・drop・譜表の予約・段間 seed）が 1 か所。台帳 0 点・snapshot 1 枚（`figbass-below-script` が僅かに上）・毒 5 赤。⚠️ LP は臨時記号を数字の*左*に置く（`6♯`＝(−0.8146 . 0.9219)）・Lily# は右＝既知の未移植のまま。⒣ `0669caa8`＝`TextOffsetInSpanner(fonts)`＝0.6 × em 比（LP 実測 font-size −3 で −0.424264・+2 で −0.755953＝Lab `dyn-yoffset.ly`）・出力不変・毒 2 赤。full **8980 / 0 / 2 / 8982**。
★★ **⑷ `chords` track の行が LP より 1.0 低い件＝原因は臨時記号の積み順だった**（`ba106399`・台帳を先に開けた `9e2c1f08`）。chordnames.lys の最後の和音 `<c'' ees'' ges'' bes''>` で、LP は行を bes'' の ♭ の上端（音符中心 +1.86）＋0.5 で持ち上げ、Lily# はその ♭ を行の X 窓（`[x, x+SymbolWidth]`）の外に置いていた＝和音が 1.5 右（LP の ♭ は bes→ees→ges の順に置かれ ees が bes の下に潜る・幅 1.667／Lily# は `StaggerEntries` の**発明した近接グループ**で 3 つを 1 群・高さ順のまま＝幅 1.929）。臨時記号なしの対照は LP 2.53・Lily# 2.59＝既知の面項のみ。直し＝LP の ape（音名でまとめる＝`add_accidental` のキー）・`ape_less`（要素数→上端）・`stagger_apes` のジグザグ。**`acc_less`（ナチュラルを符頭側）は ape の中だけ**＝ape をまたいで使っていたのも発明（LP 実測 `<b'! dis''>` は d'' の ♯ が符頭側＝既存テストの主張が逆だったので書き直し）。台帳 `chord.accidental.flat-stagger-span` を開けて（+0.262535）→ EXACT。既存の 2 個の列間隔 4 点は不動。snapshot 6 枚（LP の双子と目視で照合・chordnames の行は 3.90＝LP 3.85＋面項）。毒 9 赤。full **8981 / 0 / 2 / 8983**。
★ **⑸ R10⒞ と残差**: ⒞ `eae2a38a`＝`ResolveOverlaps`（音節の中心を半幅 0.6 の箱とみなし 1.7 未満なら右へ最大 2.0 ずらす＝LP に無い後処理）を撤去。止めても 8,983 本・snapshot・台帳が不動、32 分音符に 1 文字音節の負荷本でも全音節が列の中心のまま（rod と両端揃えで 1.7 を割らない）・コーパスに歌詞 0 冊＝観測者 0 を確かめてから削除。**音節の ink 幅の残差は測定の副作用**: 書体を固定しない LP はこの機械で serif＝`times.ttf`（`ly:font-config-get-font-file`）・C059 に固定すると Lily# と 6 桁一致（上の R10 行）。**melisma 割当は言語仕様の差**（LP はスラーを自動メリスマ・Lily# は歌詞側の印が音符を消費）＝ユーザー判断待ち。
  ⇒ 判定: R10 の残りは ⒜（aligned_side の 3 綴り＝大）と melisma（ユーザー判断）。次は ⒜ か、§1.0 の他の忠実度項目（R7〜R9・R11 の LP 双子／⒳⁶／⒡′／⒵⁴）。この便の文脈に依らない＝(c)。
...
- **R10 記号・歌詞の残り**＝⒜ `ArticulationEngraver` の aligned_side が 3 綴り（note 支持は箱の算術、chain/tie/slur は pointwise、`DynamicEngraver.SpannerOffsetY`）＋`on_line` の ledger parity／⒝ ✅ 第565 が閉じた（extender＝`h = lt×0.8`・padding は h・`minimum-length 1.5` を系の右端で切る・最後の符頭の右端まで・次の音節の左端 − h で切る・baseline の上に箱・`w < 1.5h` で消す＝手書き twin で実測一致・網 `LyricExtenderGeometryTests` 5 本。⚠️ 残差 2 つは別項: 音節の ink 幅は**第569 が測定の副作用と確定**（書体を固定しない LP はこの機械で serif を `times.ttf` に解決する＝`ly:font-config-get-font-file`・`LilyPond Serif`＝C059 に固定すると「a」1.365732・「i」0.785296 で Lily# と 6 桁一致＝Lab `sessions/p569/lyric-xext-pinned.ly`・双子は `lysc ly --pin-fonts` で比べる）／melisma 割当は**第570 がユーザー決定「LP に合わせて」で閉じた**（網 `LyricSlurMelismaTests`））／⒞ ✅ 第569 が閉じた（`ResolveOverlaps` を削除＝LP は音節を列の中心に置き rod で列を広げる・観測者 0 を確認して撤去）／⒟ ✅ 第567 が閉じた（起票は古かった: stacker が staff-padding 0.5 の床と 450 の順を持つ・engraver の dead な seed を aligned_side の床 `AlignedSideBaselineYUp` に＝出力不変。⚠️ `_"text"` は `^\markup`＝譜表の*上*に置かれている＝ユーザー判断の余地）／⒠ ✅ 第569 が閉じた（箱＝描いたインク `[x, x+Width]`＝`FigureXExtent`・LP 実測 (0 . 0.921869) と一致・4 読み手が 1 か所に・網 `FiguredBassTests` 4 本。⚠️ 臨時記号の左置きは未移植のまま）／⒡ ✅ 第567 が閉じた（全番号の右端＝最左の先頭音節 ink 左 − 1.0＝x-aligned-side ＋ `Stanza_number_align_engraver`・手書き双子で一致・網 `StanzaNumberAlignmentTests` 2 本。lead sheet は OWN のまま）／⒢ ✅ 第567 が閉じた（attached 行の baseline＝記号ごとの skyline ＋ 0.5 − ink 下端の max＝`RelatedStaffPadding`・双子で 0.013＝面項まで一致・網 `ChordNameStaffPaddingTests` 3 本・帯も 0.5・PagingSkylines の silhouette 腕も実 ink に＝台帳 `page.inline-chord.gap-first` −0.044541 に再ベース <!-- ledger: page.inline-chord.gap-first = -0.044541027 -->。⚠️ 残差だった `chords` track の行が LP より 1.0 低い件は第569 が閉じた＝原因は行ではなく*臨時記号の積み順*（`StaggerEntries` の発明した近接グループ→LP の ape＝音名・`ape_less`・`stagger_apes`）・台帳 `chord.accidental.flat-stagger-span` EXACT <!-- ledger: chord.accidental.flat-stagger-span = 0 -->）／⒣ ✅ 第569 が閉じた（`TextOffsetInSpanner(fonts)`＝0.6 × em 比・LP 実測 −0.424264／−0.755953 と一致・網 `DynamicTextOffsetTests`）。
...
| ★★★ **section のラベルを隠す `~` は form の参照にだけ書く＝`section ~A { … }` は LYS0033**（2026-09-24・第569・**ユーザー決定**。旧決定「宣言側の `~` でラベル既定をひっくり返す」（2026-08-31・第304・`73b47843`）を**覆した**＝part-major では各パートが同じ section を宣言するので性質の置き場が複数になる・実コーパスは宣言 342 個がどれも参照 1 回で反転の利用 0。デメリット＝全 form の参照に `~` を書く（実測で 2 回以上の参照 0）。エラー文は「後方互換」ではなく現在形の案内として残す） |
