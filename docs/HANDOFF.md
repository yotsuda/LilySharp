...
最終更新 第294セッション＝**§1 が名指していた §2 U4 の残債に着手し、着手前の測定で*残債が存在しない*ことを確定**。**続いて §2 U9 の原因を測り直して §2F ⑺ の島だと確定し、その島の検出器の欠けていた半分を建て、そのうえで*ユーザーの決定を得て島ごと閉じた*（`65224424`＋`ee426f62`）**——**開いていたのは第185 起票・第186 census 以来、109 便**。**commit 6 本**。**台帳 748／exact 588／ss 非ゼロ 190／総和 24.714570188・snapshot 230・追跡 `.lys` 580 は全部不動**——**この便はどの距離も絵も動かしていない。**

⚠️⚠️ ★★★ **⑴ 「タブの `as full` に連桁が無い」という残債は無かった**（§2 U4 の末尾。**§5.0「引き継がれた処方箋は、着手する前にその*診断*を 1 回測る」がそのまま効いた**）。**タブの連桁は主梁も副梁も切れ端も在り、LP の量子器を通っている**——**`TabBeamQuant.Compute` が `BeamScoringProblem` へ*弦の線*を stem position として渡し、TabStaff の 2 つの override（`Beam.beam-thickness 0.32`・`Beam.length-fraction 0.62`）で解く**。**実測 30 秒**（`scratch/p294/b16.lys`）: **タブ側 polygon 8 本＝主 4・副 4**、**`staff` を外したタブ*だけ*の本でも同じ**。⇒ ★★★ **反証は起票の 12 行上に最初から在った**——**同じ §2 U4 が「fixture 7 冊の主題（*連桁*・付点・…）は numbers では消えるので `as full` を明示した」と書いている**。⇒ ★★ **1 つの量について同じ節が 2 つのことを言っていたら、grep ではなく*その節を通しで*読む。**

⚠️⚠️ ★★★ **⑵ §2 U9 の「`GreenNode` の span の取り方」「第286 U3 と同じ族」も両方外れ**。**`GreenNode` は正しく、犯人は `ParsePostEvents` の並べ替え**——**記号（`~ ( ) [ ]`）の*後ろ*に書いた post-event が音符へ持ち上げられ、記号は queue から*その後ろ*へ再生される**ので、**木の順序が原文と逆**になり、**入れ替わった 2 ノードが両方とも嘘の住所を名乗る**。⇒ ★★ **U3 と見分ける問いは 1 つ**: **「幅は正しいが `Span` が trivia を食う」（`GreenNode`）か、「`Span` の取り方は正しいが、そのノードが木の別の場所に居る」（parser）か**。**症状はどちらも「data-pos が 1 手前」で同じ顔をしている。**

★★★ **⑶ RULES §5.0 が「2 つで 1 組」と書いていた検出器の、片方しか無かった**（`12599c12`）。**和の検査はコーパス 580 冊に在り、*各ノードの*検査は inline 2 本と狭い 3 ファイルにしか無く、本に 1 度も当たっていなかった**。**建てた 2 つの半分**: **⒜ `FullSpan` ⒝ `Span`（trivia を食っていないか）**。★★ **⒝ は「幅から導かない」ことが値打ち**——**ノード自身のテキストを切り出す書き方は同じ `GetLeadingTriviaWidth` から来るので、U3 の誤答と一緒にずれて*一致してしまう*。**動かないのは trivia の定義のほう。** ★ **毒**（戻した）: **`Span` を U3 の綴りに戻すと ⒝ が 569 冊で赤・和の掃きは緑。**

⚠️⚠️ ★★★ **⑷ この便は予測を 4 回外し、4 回とも*最初の run* が訂正した**——**書く前に測っていれば 1 回も要らなかった往復**。**⒳「2 つの一覧は違うはず」→ 同じ 11 冊**／**⒴「`Span` 側は 0 のはず」→ 同じ 11 冊**／**⒵「網の helper は最初の記号を見ればよい」→ 対の多くは `(` を先に開くので*最初*は動かない記号**（**any に直した**）／**Ⓐ「census の代償どおり 12 経路の書き換えが要る」→ 実際は 2 か所**。⇒ ★★ **remark を先に書くと、あとの測定が*確認*になって読み飛ばされる。網→毒→remark の順に書くこと。**

⚠️⚠️⚠️ ★★★★ **⑸ そして毒が*私の書いた網の嘘*を暴いた**——**2 本目の毒が 1 度目に緑で返った**。**`MusicXmlExporter.Export` は文書*モデル*を返すので `.ToString()` は型名**で、**同じ定数どうしを比べていた**（**6587 本が通る、落ちようのない比較**）。**`ToXml()` で直列化して初めて赤。** ⇒ ★★★ **RULES §5.4 の「検査器は落ちることを先に証明する」は、*自分が今日書いた網*にも当たる。** **緑を先に見た網は、緑が「一致」か「何も見ていない」か区別が付かない。**

★★★ **⑹ 直しは 1 行で、そのあとにコーパスが 2 つの帰結を出した**（**どちらも自分では思いつかず、掃きが名指した**）。**⒜ 容器の body は `Body.Items`＝*直下の子*を歩く**ので、**音符の中へ移った記号が見えない**（**実測 `tupnumss.lys` がスラー開きを落として警告し始めた**）——**第293 のリハーサル記号と同じ容器・同じ直下歩き・落ちるものだけが違う鏡像**。**⒝ 型フィルタは物が落ちる場所**——**`Rest`／`Arpeggio`／`Chord` の `Articulations` が記号の種別を名指していなかったので、緑の木が*どの accessor も渡さない*ノードを抱えた**（**実測 `empty-chord.lys` が MusicXML と LP 双子の両方からスラー閉じを無言で落とした・絵は正しいまま**）。

⚠️⚠️ ★★★ **⑺ 2026-08-16 の census の「代償」は請求書より大きかった**（§5.0「処方箋の否定形と肯定形を分けて読む」の 3 例目）。**⒜ 生存 12 経路のうち `TieTargetScanner`・`SlurPairingScanner` は*収集済み item* を読む＝下流で無関係**（動いたのは lookahead 2 か所）／**⒝ 死んでいた 6 経路は目を覚まし、しかも*一致した*——MIDI と MusicXML は 1630 冊バイト同一**／**⒞ `smartTyping.ts` は 1 行も要らない**（**両方の綴りが忠実になったので、打鍵補助がどちらに置こうと木は原文を写す**）。⇒ ★★ **「できない理由」が正しくても「だから何が要るか」は別に測る。**

★★ **⑻ 全数 A/B は 4 出力で取った**——**SVG だけの掃きは ⒝ に構造的に盲目**（**あの欠陥は exporter の中だけに居た**）。**最終 1645 冊・base `f2746535`・両側 Release・本ごとに続けて**: **svg MOVED 64／midi 0／xml 0／ly 69・`lysc check` の差 0・レンダー失敗 0**。★★★ **動いた 2 列は*確かめた*、仮定しなかった**（`scratch/p294/verify-moved.ps1`）: **svg 64 は `data-pos` だけが違う**（**伏せるとバイト同一＝1645 冊のインクが 1 点も動かない**）／**ly 69 は*並べ替えだけ***（**同じ文字・増減 0**＝`)` が双子から消える形を捕まえる検査）。

⚠️⚠️⚠️ ★★★★ **⑼ そして 4 出力*全部*が黙る 4 軒目が在った**（`ee426f62`・**§7 の perf 項を片付けている途中で出た**）。**`MeasureValidator.FlowsThroughBarAccounting` は repeat の body が bar accounting を通るかを*item の種別*で決める**ので、**音符の中へ移った記号のせいで body が「素の音符だけ」に見え、前は拒んでいた body を通すようになった**——**`repeat unfold 2 { c8(@accent d e f } g4 |` が LYS2001、同じ音楽の `c8@accent(` が LYS2006**。⚠️ **これは svg・midi・xml・ly の 4 出力すべてバイト同一で、しかもディスク 1645 冊にこの形の本が 1 冊も無い**——**掃きは何も言えず、`check` の比較を*網に*書いて初めて出た**。⇒ ★★★ **掃きの「0 冊」は、その掃きが見た*出力*についてしか言っていない**（RULES §5.0 に汎化）。
⚠️ **白名簿を*緩める*のは別の決定**（その remark が「472 の percent body のうち 114 がタイか `break` で strict gate を落ちる」と測って保留している）。**綴りの副作用で跨がないよう、白名簿に*音符の中も見せる*ほうへ倒した**（保守側）。⇒ ★★ **これで種別を列挙していた軒は 4 つ**（型フィルタ 3 ＋ 白名簿 1）。**grep するのは種別名ではなく「その位置を読む述語」**（RULES §5.0）。

⚠️⚠️ ★★★ **⑽ 毒の緑を「無関係」と読んで、コードの remark に書いた**（**同じ便で自分で訂正**）。**seed を外す毒の赤が tuplet の対 1 本だけ**だったので「上の平坦な歩きは seed を要らない」と書いたが、**cue と repeat の body も同じ関数を*直下の子*で通るのに case が無かった**＝**緑は「未記述」だった**。**対を 2 つ足して当て直すと赤は cue と repeat・平坦な対は緑**で、主張自体は正しかったが最初の run はそれを言っていない。⇒ ★★ **毒の緑を経路について読む前に、*その経路を踏む case が一覧に在るか*を数える**（RULES §5.4 に汎化）。

★★ **⑾ perf は測った**（§7.9＝「計算を足したなら訊かれる前に測る」。**`PeekMarkers` は打鍵経路で音符ごとに走る**）。**`audit/LilySharp.Probe alloc`・3 run の床・full render MB**（**base ／ 最初の commit ／ green 事前判定つき**）: **plain1k 471.0 / 471.4 / 471.0**・**fingbeam1k 961.5 / 961.8 / 961.5**・**slurscript1k 1253.1 / 1253.4 / 1253.3**。**打鍵の床は plain1k 26.1 で不動**。⇒ **host の*緑スロットの種別*だけを見て、記号が無ければ何も作らない**（red も iterator も 0）。**slurscript1k の +0.2 MB は「訊く費用」ではなく仕事そのもの**（スラーだらけの本）。⚠️ **この変更に対照本は存在しない**（全音符ノードを通る）ので「対照は動かなかった」は使えない主張（RULES §5.3）。

⚠️ **本便で新しく開いたもの**:
- **★★ タイとスラーは `data-pos` を 1 つも持たない＝プレビューで指せない**（**§2 U10・ユーザーの問いから出た**）。**snapshot 230 枚の `<path>` 56 本のうち住所を持つのは 6 本だけで、全部*装飾音符のスラー***。**U9 を閉じた今、`~` の上では*直前の住所*＝音符が光る**——**嘘ではないが筋は通っていない。**

⇒ ★★★ **次の一手＝ユーザーに訊く**。**§2 の ▶ は U8（装飾音符が注釈を 1 つも運ばない・射程はユーザーの本 0 冊・閉じる便は設計から）と U10（上・射程は広いがインク不動・閉じる前に「弓の住所は 1 つか 2 つか」の決定が要る）**。★ **読み手の目に一番近いのは U10**。★ **ユーザーの本で今 LYS4018 が出ているのは `Untitled-6.lys` の 1 冊だけ**（第288 便の `@rit`）。

★ **開始時裏取り**: HEAD `fba98ca1`・**未 push 19**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6578 / 0 / 4 / 6582**・台帳 **748／exact 588／ss 非ゼロ 190／総和 24.714570188／count 158 中 2・`OPEN:` 0**・追跡 `.lys` **580 冊**・snapshot **230 枚**・**CI は最後の push（`91650d7d`）で success**——**§1 の終了時の数と 1 つも違わなかった**（**`OPEN:` だけは数え方が書かれていなかったので §0 に足した＝10 例目。素朴な grep は 12 と答える**）。
終了時: **commit 6 本**（`12599c12` 検出器 ＋ `f2746535` docs ＋ `65224424` 島の本体 ＋ `741242f2` docs ＋ `ee426f62` §7 の残り ＋ **この 1 本**）＝**未 push 25**・木 0・未追跡 0。**suite 6589 合格 / 0 失敗 / 4 skip / 合計 6593**（開始 6582 から **+11**＝**検出器 1 ＋ 恒等の対 8 ＋ data-pos 2**）。Core 0 エラー 0 警告。**追跡 `.lys` 580 冊・snapshot 230 枚・台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188＝すべて不動。**
★★ **WSL ubuntu Release も同じ 6589 / 0 / 4 / 6593**。★ **`12599c12`・`f2746535`・`65224424`・`741242f2` の各木でも両脚で緑を取ってある**（**docs の木を回すのは形式ではない——`HandoffArchiveContinuityTests` が docs を読む**）。⚠️ **CI だけは未実行**（未 push 25）。

## 以下は第293セッションの経緯

最終更新 第293セッション＝**ユーザーが「残債の返済をいったんやめて、私に見えている問題を直して」と言い、実コーパスを掃いて出た 1 件を閉じた（1 件・commit 2 本、コードは `684d5a3c`）**＝**自分の walk を持つ容器の中に書かれたリハーサル記号が、黙って 1 文字も描かれていなかった**。**ユーザーの本 45 冊・120 文字が返り、失われたテキストは 0**。**台帳は 1 点も動いていない**（748／exact 588／ss 非ゼロ 190／総和 24.714570188・**この便の量はどの点も読んでいない**）。**snapshot は 229 → 230＝1 枚*追加*で、書き換えは 0 枚。**

★★★ **⑴ 欠陥は「印」ではなく「歩き」だった**。**リハーサルマークは、item が*音符*ではなく `ProcessMusicNode` の*statement アーム*で作られる唯一のマーク族**で、**そのアームは「書かれた音符の*兄弟*として walk が配った印」しか見ない**。**降りる walk は 1 つだけ**——**`MusicSitesLazy` は音符の*中*へ入るので `@mark` の子が flat list で音符の直後に並ぶ**。**自分の walk を持つ容器は body の*直下の子*しか配る**（`volta.Items`・`repeat.Body.Items`・`cue.Body.Items`・`tuplet.Body.Items`）ので、**インライン ending／tuplet／repeat／cue の中では site にならず、アームが 1 度も走らない。** ⇒ ★★ **「なぜこの印だけ消えるのか」の答えは、印の側に 1 つも無かった。**

★★ **⑵ 直す場所は既に現場に居た**。**`CollectArticulations` は同じ `MusicMarkSyntax` を見て `_markHostMeasure` を記録し、そのうえで build だけを上へ渡していた**——**`@rit`・`@text`・`@sustain`・すぐ隣の compound `@ottava` は全部この経路で作られている**。⇒ **直しは「隣にもう 1 本アームを足して、渡すのをやめる」**。**運ばれていたのは measure だけで、いま印そのものが同じ場所で作られる。**

⚠️⚠️ ★★★ **⑶ de-dupe を一緒に動かし忘れ、1 度目の掃きが 218 冊を動かした**。**statement アームの冒頭の `MusicMarkExistsAt` は自分自身への防御ではなく実務だった**——**1 つの part を staff と tab の 2 段に描く本（`staff bassline` ＋ `tab bassline`＝ユーザーの本の大半）は、その part の音符を*段ごとに 1 度ずつ*歩く**ので、**新しいアームは 1 つの書かれた印に 2 度届く**。**guard 無しの実測: 秋桜 と Take On Me が全文字を 2 度刷り、1 段の ポリリズム だけが正しかった**。⇒ ★★★ **「射程が広い」は「よく効いた」ではない**——**218 → 45 の差 173 冊は、全部こちらの欠陥だった**。⇒ ★★ **既存の guard を持つコードから build を動かすときは、*その guard が誰から守っていたか*を先に言うこと。「自分自身から」だと思ったら、たいてい外れている。**

⚠️⚠️ ★★★ **⑷ 集合で数えた射程 41 冊は、下限ですらなかった**。**「ソースに書かれたラベルの*集合*」と「SVG に出たテキストの*集合*」を比べていた**ので、**同じラベルを 2 か所に書く本（`Tell Her About It` の `Intro`／`A`／`B`）では、片方が消えていても集合は一致する**。**出現*回数*で数え直して 45 冊 120 文字**。⇒ ★★ **第290 の「族の 1 綴りで grep して族の射程を答えた」の*集合*版。在庫は、重複を潰さずに数えるまで在庫ではない。**

★★ **⑸ 毒は 2 本要り、効き方が違うことが設計**。**アームを外すと 5 つの容器ケース全部 ＋ staff/tab ＋ snapshot が赤**／**de-dupe を外すと repeat の 2 ケースと staff/tab と snapshot だけが赤で、volta・tuplet・cue の 3 ケースは緑**。⇒ ★★ **1 本の毒では番人の半分しか確かめられない**——**この修理は 2 つの主張の合成なので、番人も 2 つの主張として置いた。**

★ **⑹ 追跡コーパスは全盲だった**——**45 冊動いて snapshot は 1 枚も動かない**。**だから本を建てた**（`test/rehearsal-marks-inside-containers`・**4 文字のうち 1 つだけが top level・残り 3 つが tuplet と両 ending の中**）。⚠️ **その本の `tab melody` は飾りではない**——**1 段の本では ⑶ の毒が緑になる**ので、**staff ＋ tab でなければ de-dupe の主張が書けない**。

⚠️ **見つけて直していないもの**（前便から引き継いだ分に加えて）:
- **★ ✅ 2 便目でユーザーが決めた＝`repeat unfold N` の中の `@mark` は 1 度だけ刷る**（⑶ の位置 de-dupe の帰結・⑺ で*留めた*）。**LP は unfold を*展開*するので N 度刷る**。**ユーザーの実コーパスに「unfold の中の mark」は 0 冊**（`repeat percent` は body を 1 度しか歩かないので無関係）。**要ユーザー判断で §2 U7 に起票した。**
- **★ ✅ 2 便目でユーザーが決めた＝この族も「描かないなら位置つきで言う」**（`LYS4019`・⑻〜⑽）。**起票時の観測はこうだった**:（**両 exe の `lysc check` は 1603 冊バイト同一**）。**「描かないなら位置つきで言う」は `@!X` 族には在って、この族には無い**（§2 U6 の ⒜ が「無言で消える」を最悪と名指したのと同じ形）。
（以下、前便からの一覧はそのまま）

⇒ ★★★ **次の一手＝ユーザーに訊く**。**U7 の 2 問はユーザーが答えて閉じた**（下の 2 便目）。**§2 の ▶ は本便が開けた U8・U9 の 2 つ**——**どちらも射程はユーザーの本 0 冊（U8）／インク不動（U9）**。★ **次に読み手の目に触れそうなのは §2 U4 の残債＝タブの `as full` に連桁が無い**（**ユーザーは「符尾・連桁も含めて全部」と決めている**）。★ **ユーザーの本で今 LYS4018 が出ているのは `Untitled-6.lys` の 1 冊だけ**（第288 便の `@rit`）——**本便はその本の*絵*を動かしていない**（掃きの MOVED 45 に入っていない）。

★ **開始時裏取り**: HEAD `99410d46`・**未 push 15**・木 0・未追跡 0・Core 0 エラー 0 警告・**Windows Debug 6559 / 0 / 4 / 6563**・台帳 **748／exact 588／総和 24.714570188／162 中 164 count・`OPEN:` 0**・追跡 `.lys` **579 冊**・snapshot **229 枚**——**§1 の終了時の数と 1 つも違わなかった**。
終了時: **commit 4 本**（`684d5a3c` 1 便目の code ＋ `50467a14` その docs ＋ `b702e47f` 2 便目の code ＋ **この 1 本＝§1 の続きと §2 U7 の決着**）＝**未 push 19**・木 0・未追跡 0。**suite 6566 合格 / 0 失敗 / 4 skip / 合計 6570**（開始 6563 から **+7**＝**theory 5 ケース ＋ fact 1 ＋ snapshot 1**）。Core 0 エラー 0 警告。**追跡 `.lys` 580 冊**（**+1＝番人の本**）・**snapshot 230 枚**（**+1・書き換え 0 枚**）。**台帳 748 点／exact 588／ss 非ゼロ 190／総和 24.714570188＝すべて不動**（**この便はどの点が読む距離も動かしていない**）。
⚠️ **掃きは 2 回**（**母集団はディスク上の全 `.lys`＝ユーザーの実コーパス込み・base と head は*本ごとに続けて*・両側 Release・`sweep289p.ps1`**）: **⑴ de-dupe を足す前＝1602 冊 SAME 1380 / MOVED 222**（**ユーザー実コーパス 218**——**その 173 冊は二重描画**）／**⑵ 足したあと＝1603 冊 SAME 1554 / MOVED 49**（**ユーザー実コーパス 45 ＋ 本便の fixture 1 ＋ `scratch/p293` のプローブ 3。追跡 fixture・samples・audit は 0 冊**）。**どちらも `lysc check` の差 0・レンダー失敗 0。** ★★ **動いた 45 冊を*テキスト単位*で突き合わせた**: **GAINED 120／LOST 0**、**gained は全部リハーサルのラベル**（B2 13・B1 12・D 11・E 9・C 8・C1 7・A2 6・Intro2／Intro3／Solo／Interlude／B3 各 4…）——**他の文字は 1 つも増えず、1 つも消えていない。**
★★ **緑は*コード commit のあと*に取った**。★★ **WSL ubuntu Release も同じ 6566 / 0 / 4 / 6570**（**`684d5a3c` の木**・§0 の「この機械の Windows での緑でしかない」に両脚で答えた）。⚠️ **CI だけは未実行**（未 push 17）。

---

★ **同じ便の 2 便目＝ユーザーが §2 U7 の 2 問に答えた**（**⒜「unfold の中は 1 度だけにする」／⒝「消えるときは同じ扱いで言う」**）。**commit `b702e47f`。SVG は 1617 冊 1 枚も動かない**（**診断はインクを動かさない・それを掃きで示した**）。

★★ **⑺ ⒜ は*すでにそうなっていた*ので、変えずに*留めた***。**位置 de-dupe の帰結として `repeat unfold N` の中の `@mark` は 1 度しか刷らない**——**`AMarkInsideAnUnfoldedRepeat_IsPrintedOnce_NotOncePerUnfolding` が規則と乖離を述べる**。**LP は unfold を*展開*するので、engraver が見る頃には N 個の書かれた印になっていて N 度刷る**。**Lily# は「書かれたもの」を数える**（**staff ＋ tab で全文字が 2 度出るのを止めているのと同じ規則**）。⇒ ★ **決定は「どちらが正しいか」ではなく「どちらを我々の規則にするか」で、射程 0 冊だからこそユーザーのもの。**

⚠️⚠️ ★★★ **⑻ ⒝ の計器を先に作り、その計器が 1 度嘘をついた**。**「描かれたか」は絵から読める**——**リハーサルの箱の `data-pos` は印の `SourceStart` そのもの**なので、**書かれた `@mark` を 1 つずつ絵の上で引ける**。**1 度目は「37 冊 58 個が描かれていない」と出た**。**うち 56 個は `note~@mark("X")` の形**で、**この綴りでは印のノードの span が `@` ではなく `~` から始まる**——**つまり `data-pos` は書かれた位置の 1 手前**で、**集合比較が全部取りこぼしていた**。⇒ ★★★ **「欠陥が 56 個」と読んで着手していたら、正しく描けている本に 1 便を使っていた。** ⇒ ★★ **計器の当たりが多すぎるときは、まず*計器の座標系*を疑う**（第286 U3 の「行頭の音符が自分の住所を持っていなかった」の、*計測側*の版）。★ **許容 3 文字で数え直して 263 冊 2244 個中 2242 個が描かれている**——**残る 2 個は `@mark` が*そもそも構文として通らない*本**（`scratch/p206/m8`・`m9`）で、**既に 1 冊 3 行のエラーを出している。**

★★ **⑼ そこで「まだ消えうる形」を*当てて*数えた**（**掃きの 0 冊は形の不在を意味しない**——第192 の骨）。**当たったのは 3 つ**: **⒜ どの form も演奏しない `section`**／**⒝ どの score も描かない `part`**／**⒞ 装飾音符（`grace { }`）**。⚠️⚠️ ★★★ **⒞ は印の欠陥ではない**——**`CollectGraceNotes` は音高と音価しか読まない**ので、**装飾音符は*あらゆる注釈*を落とす**（**`@staccato` も `@text` も実測で消える**）。⇒ **この診断は「その穴を*聞こえるように*する」もので、穴を塞ぐのは別便**（**§2 に起票・番人 `AMarkOnAGraceNote_IsReported` が両方を述べているので、塞いだ人はテストに出迎えられる**）。

★★ **⑽ 診断は木と collect の*両方*を要る**（`ISharedCollectValidator.ValidateWith` に `SyntaxTree` が増えた・実装 10 個）。**報告すべき印は collect が*見なかった*もの**——**演奏されない section の印は「collector が落とした」のではなく「歩きがそこへ行っていない」**ので、**`MeasureCollector` の中の何も、それを名指せない**。**書かれた側は源から来るしかない。** ★ **collector 側は `EngravedRehearsalMarkPositions`＝*位置*であって item ではない**（**「そこに書かれた印は描かれたか」は位置の問い。何段が描こうと 1 位置は 1 印**）。

⚠️ **⒝ の限界を 1 行で**: **問うのは*最初の score* 1 つだけ**（`SemanticValidation.TryCollect`）。**後続の score だけが名指す part を持つ本は「描かれない」と言われる**——**その score については真で、その*ファイル*については誤解を招く**。**ディスク上にその形の本は 0 冊**。**出たら「score ごとに問う」が直しで、答えを弱めるほうではない**（コードにそう書いた）。

⚠️ **本便で新しく開いたもの**:
- **★★ 装飾音符は注釈を 1 つも運ばない**（⑼⒞・**`@mark`・`@staccato`・`@text` を実測**）。**`CollectGraceNotes` が音高と音価しか読まないため**。**射程はユーザーの本 0 冊**（**装飾音符に注釈を書く本が無い**）。**LYS4019 が今それを言う。**
- **★ `note~@mark("X")` の印は、自分の住所に `~` を含む**（⑻）。**`data-pos` が 1 手前になるので、クリック／ハイライトの的が tie を巻き込む**。**絵は正しい。**~~第286 U3 と同じ族。~~ ⚠️ **この「同じ族」は第294 が測って外れと確定**——**`GreenNode` ではなく `ParsePostEvents` の並べ替え＝§2F ⑺ の島**（訂正は §2 U9）。

★ **2 便目の裏取り**: **suite 6578 / 0 / 4 / 6582**（第 1 便の 6570 から **+12**＝**validator 11 ケース ＋ unfold の決定 1**）。**Core 0 エラー 0 警告。** **掃き 3 回目＝1617 冊・SVG SAME 1617 / MOVED 0・`check` の差は 4 冊＝本便が建てたプローブだけ**（**ユーザーの 326 冊は 1 冊も警告を得ない**）・**レンダー失敗 0**。**台帳不動。** **`docs/APPROXIMATIONS.md` は*再生成*（棚は 57 で不動・差分は 18 行の追加でずれた行番号 2 つだけ）。** ★★ **WSL ubuntu Release も同じ 6578 / 0 / 4 / 6582**（`b702e47f` の木）。
...
- **U10. ▶ タイとスラーは `data-pos` を 1 つも持たない＝プレビューで指せない**
  （**第294・ユーザーの問い「`~` をクリックしたらタイがハイライトされるべきじゃないの？」から出た**）。
  ★★★ **そのとおりで、そして今はどう転んでも光らない**——**通常のタイ・スラーは `Source(...)` スコープの
  外で描かれるので、`data-pos` を持つ要素が 1 つも無い**。**実測**: **snapshot 230 枚の `<path>` は 56 本、
  うち `data-pos` を持つのは 6 本だけで、その 6 本は全部*装飾音符のスラー***
  （**たまたま音符の Source スコープの中に入っている**）。
  ⇒ ★★ **U9 を閉じた今、`~` の上にカーソルを置くと*直前の住所*＝音符が光る**——**嘘ではないが筋は通っていない**。
  **筋を通すには弓の側に住所を持たせる**（`IDrawingContext.Source(int)` で包む）。
  ★ **射程はユーザーの本の*全部*に近い**（**タイもスラーも普通に書かれる**）——**ただしインクは動かない。
  増えるのは属性だけ**（`data-pos` は F2 丸めの座標と違って絵に出ない）。
  ⚠️ **閉じる前に決めることが 1 つ**: **弓は 2 つの音符に跨がる**ので、**住所は*開いた側*か*書かれた記号*か**。
  **`~` は 1 文字なので後者が素直**だが、**スラーは `(` と `)` の 2 か所**で、**1 本の弓に 2 つの住所が要る**
  （エディタ側は 1 要素 1 `data-pos` を仮定している——`extension.ts` の `sourcePositions()`）。
  ★ **これは §2F ⑺ とは独立**（あちらは住所が*間違っていた*、こちらは住所が*無い*）。
