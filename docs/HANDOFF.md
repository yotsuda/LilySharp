...
最終更新 第390セッション（2026-09-15〜16）＝**入り方はユーザー報告**（`docs/HANDOFF.md` と `scratch\ベースタブLy\bass.lys` を読ませて「clef が treble のまま・bass が正しいはず」）。**§0 の裏取りは走らせていない**（開始時 HEAD `e185b717`・木 clean で着手）。道具は pwsh MCP。**結果＝clef は音高を動かさない（commit `cc8d3cad`・snapshot 45 枚はユーザー承認のうえ再ベース）**。**骨は 4**:

★★★ **⑴ 報告の欠陥と設計の問い**: `staff bass melody` は `RenderSpecParser` が clef 語を読むのに collector が clef を part ヘッダからしか取らなかった（`staff treble|bass|alto melody` の SVG がバイト同一・双子は `\clef` 無し）。直すと staff 項目の clef が page の音高だけを動かし MIDI（60）・双子（`\relative c'`）と食い違った。さらに第196 の「途中 clef は anchor を付け替える」は octave を clef から・文字を直前の音から取るので `g'4 | clef bass b`＝B3／`c''4 | clef bass b`＝B2 に割れる。⇒ 選択肢を実測つきで提示 → **ユーザー決定「ヘッダの clef も音高を動かさない方が一貫」「手元コーパスの書き換えはコストに計上しない」**（§3 に行・第196 の行は撤回印）。

★★★ **⑵ 実装**: 相対 anchor＝`octave N` > instrument preset > 4（`InstrumentDefaults.AnchorOctave(explicit, preset)`・`DefaultAnchorOctave`・`GetDefaultOctave` は除去）。page（part 頭の 2 経路・section 境界の clef 復帰・途中 clef・cue clef）・MIDI・MusicXML・双子の 4 軒から clef→octave を外した。staff 項目の clef は `StaffSpec.WrittenClef` → `RenderSpec.WrittenClefOf` → collector の `_meta.Clef`（描画だけ）と双子の `\clef`。網 `ClefMovesNoPitchTests`（4 出力×clef を消した対照・陽性対照 `instrument cello`／`octave 3`）・旧 `ClefReanchorsFrameTests` は削除・旧規則を守っていた単体テスト 10 ファイルは `octave 3` 付与か期待値修正。

★★★ **⑶ 射程と書き換え**: 変更前の lysc（worktree `scratch/p391/base`＝`e185b717`）と head の `check --pitches --batch` を 938 冊（追跡 608＋実コーパス 330）で比較（`scratch/p391/pitch/`）→ **53 冊が動いた**（追跡 51・実コーパス 2＝`'Til They Take My Heart Away`・`がくふ`。実コーパスの大半は `octave absolute` で無影響）。49 冊はヘッダに ` octave 3`（`rewrite-log.csv`・原本は `rewrite-backup/`）、4 冊（`clef-change`・`cue-notes`・`mmr-clef-change-bound`・`cue-clef-manually`）は手で octave 記号 → **再掃きで 938 冊すべて音高一致**。snapshot 45 枚は HEAD との差が data-pos だけ（描画差 0）。**第 2 便（常設指示）**で `.lys` の外を追従: `TUTORIAL`・`GRAMMAR_FOR_LLM`・`SYNTAX_REFERENCE` の例と LSP の `TwinklePianoTemplate` に ` octave 3`（旧ソース×旧 lysc と新ソース×新 lysc で音高一致）・`MidiExporter` の古いコメント。

★★ **⑷ 数と次の一手**: **full `scratch/p391/clef6.trx` 8592 / 0 / 4 / 8596**（この §1 を書いた後・archive 済み。clef5 は 8591 / 1＝`ApproximationInventoryTests` がコメントの行ずれで古い→再生成）・台帳 851／総和 22.584727806 不変・snapshot 249（45 枚は data-pos）・§7.5（`-DiffBase e185b717`）Core `+` 100 行／REF 0／OWN 0＝**監査した**: 足した計算なし（clef の段の削除・staff 項目 clef の配線・`DefaultAnchorOctave`＝4 は LP `\relative` の既定 c' と同じ値だが住所は未確認で REF は書いていない）・perf: `WrittenClefOf` は binding ごとに score 項目を 1 回走査＝読んで数えた。HEAD＝`cc8d3cad`（＋第 2 便の未 commit: docs 3・LSP 1・MIDI コメント・APPROXIMATIONS・この §1）・未 push 27・§7 3.5: 第388 を `-Archive 388`（済）。★ **LSP の配布は要る**（描画とテンプレート）。★ **第 3 便（ユーザー「任せる」）**: 見つけた既存の欠陥＝`docs/TUTORIAL.md` を直した（`f148668c`）——冒頭 2 例は top-level の音楽で LYS0020（完全な楽譜に）・相対モードで `'` が積み上がる例 6 つ（E11・G0 など）・小節不足 2 つ。コード例ごとに `check --pitches` で無エラー・狙った音高を確認。⚠️ `HANDOFF.md` は 450 KB の門が近い。⇒ **次の一手**: 第389 ⑩ の残り（`grace-dot-flag-column`・`grace-unfold`）／cue clef の大きさ／第379 ⑺⑻。
...
| ⛔ **第390 で撤回（上の行）** 小節途中の `clef` は相対オクターブの anchor を付け替える（2026-08-17・第196） | 撤回の理由は上の行 |
