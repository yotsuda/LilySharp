...
★★★ **⑵ dump が答えた＝rod だった**（計器 `scratch/p393/lpdump/rods.ly`＋`run.ps1`・`-dinclude-settings`・**`ly:paper-column::get-rank` は 2.26.0 に無い**ので column は system 相対 X で名指す・**rod 標的が grob でないことがある**ので `ly:grob?` の門が要る）。**`minimum-distances` は全精度で読める**（`ideal-distances` の Spring は Scheme に getter が無く `ly:paper-column::print` の矢印 2 桁だけ＝**今回は要らなかった**）。⚠️ **`run.ps1`／`run-any.ps1` の 4 桁表示は*切り捨て***（第392 第 4 便で 2.6074 を 2.6073 と書き、網が 0.0001 で赤くなって気づいた。**4 桁を引用する前に生の値を見ること**）。**`grace-dot-flag-column` の rod は 1.7945（g8.・生 1.794455）・1.7386（f8.）・2.4559（d'8.）・1.9352（e'8.）・2.4559（d'16.）・2.3151（g16.）・1.9352（`g16. g16` の内側）で、描かれた gap は全部 max(spring, rod) に 4 桁一致**（g8. だけ spring 1.9386 が勝つ）。

★★★ **⑶ 住所と模型**: **LP** `separation-item.cc:47-68 set_distance`＝`padding(0.1) + 左 column の右 skyline と右 column の左 skyline の距離`・:120-190 `boxes` は各要素の **`pure_y_extent`**＋`extra-spacing-height`（Dots は `define-grobs.scm:1277` (-0.5 . 0.5)・esw :1278 (0.0 . 0.2)）・:92-110 で `skyline-vertical-padding`（PaperColumn 0.08）を**作るときに**足す。⇒ ★★★ **付点の箱は*描かれた段*ではなく*符頭自身の段*に立つ**（`RODDOTS`／`RODPCSKY` で実測: 線上の `g16.` は付点を 1 つ上の間に*描く*が、箱は g の線を中心に ±0.7343）。**だから `g16.` の 2.3151 が出る**——付点の箱の下の面取りと主音の頭の上の面取りが 0.0193 だけ*角と角で*触れ、差は一定 2.2151＝＋0.1。**Lily#** は `SpacingRules.GraceDotRod`（新）＝`ItemSkylineFactory.CreateGraceDotRightSkyline`（付点だけ・0.08 で padded）対 次の column の左 skyline（次が grace なら `CreateGraceLeftSkyline`・主音なら既存の `CreateLeftSkylineAtColumn`）。**島の他の部分は rod を足さなくてよい**——同じ箱に対し rod は 0.1＋0.08、spring の床は 0.3＋0.15 なので必ず床が勝つ（コメントに書いた）。

★★★ **⑷ 2 つ目の綴りと網**: **`DotColumn.ReservedForGrace`（新）は renderer の `DrawNote` が組む stem／flag support を*spacing 側で組み直す***——**spacing は drawing の前に走るので畳めない**。⇒ §7.7 の**差分の網** `GraceBodyValidatorTests.TheReservedGraceDotIsTheDrawnOne`＝**予約した付点 X と*ページが描いた* X を同じ 6 冊で比べる**（描いた側は既存 `AGraceDotClearsTheFlagOnlyWhenTheFlagIsOnItsRow` が LP に留めてある）。**主網** `GraceSpacingTests.ADottedGraceGap_TakesTheRodThroughItsDot`＝LP の 6 gap（1.9386・1.9386・2.4559・2.4559・2.3151・1.9352）。**毒**（`SpacingRules.Grace.cs` を stash）＝**rod の 4 行だけ赤・spring の 2 行と `ADottedGrace_IsDrawn` は緑**。⚠️ **網を書くとき Lily# の絶対オクターブで 1 つ高く綴って 3 行が赤くなった**——**`c'` は LP の `c''`**（`AGraceDotClearsTheFlagOnlyWhenTheFlagIsOnItsRow` のコメントが既にそう書いてある）。**そのオクターブでは LP も同じ答えを出す**ので、網の期待値ではなく綴りが誤りだった。

★★ **⑸ 射程と数**: 掃き 938 冊（`scratch/p393/sweep`・base＝`exe-base`＝`1236d365` の Debug を編集前に複製）で **1 冊**＝snapshot `test/grace-dot-flag-column` のみ（第391 が動かした `voltagrace-ctl4`・実コーパス `青い珊瑚礁` は不動）。**LP との突き合わせ**（`scratch/p393/kg/afterbar.ps1`・小節線右端から・LP／前／後）: 2 本目以降の小節線で **d'8. 主音 3.14／2.62／3.14・e'8. 2.62／2.62／2.62（不動）・d'16. 3.14／2.62／3.14・g16. 3.00／2.62／3.00・`g16. g16` は 2 つ目の grace 2.62／2.54／2.62 と主音 4.04／3.96／4.03**。⚠️ **`afterbar.ps1` の「after bar line k」は k+1 小節目**（最初の小節線は曲頭の反復線）。**終了時**: HEAD＝この §1 を書く commit（親＝コード commit `146e68ab`）・未 push 35・**full `scratch/p393/run4.trx` 8600 / 0 / 4 / 8604**（+7＝主網 6 行と差分の網 1 本・この §1 を書いた後・archive 済み。run3 は 8599 / 1＝承認前の snapshot）・台帳 851／総和 22.584727806 不変・snapshot 249（1 枚更新）・追跡 `.lys` 608・§7.5（対 `1236d365`）: **Core `+` 183 行／REF 4／OWN 0**＝足した計算 4 つ（dot column・boxes・set_distance・Dots の esh）に全部住所・perf: rod は**付点を持つ column のときだけ・次の 1 column に対してだけ**建てる＝読んで数えた・`audit/magic_constants.csv` は行番号＋新定数 1 行（`DotsExtraSpacingHeight`＝緑）・`docs/APPROXIMATIONS.md` は行番号だけ・§7 3.5: 第390 を `-Archive 390`（済）。★ **LSP は配布済み**（`tools/Deploy-Lsp.ps1`・`0.7.0+68fac536` を `yotsuda.lilysharp-0.5.0-win32-x64\server` へ・95 ファイル照合・**ユーザーに Reload Window を依頼済み**＝server だけなので 1 回でよい）。★ **第 2 便（常設指示）＝③ を LP に測って、コードには触らず止めた**（下の ③・`scratch/p393/lpdump` の計器が残っていたので着手し、**観測できる本が作れないところで止めた**）。

⚠️ **残した差**: ① **`e'8.` の gap は LP 1.9352 に対し Lily# 1.9386**——**これは rod ではなく spring の床**で、`GraceColumnRightReach` が旗を*平らな幅*で読む（LP は note column の skyline なので、高い `e'` の旗は主音の頭の段に届かない）。**網はこの行を外してある**（理由をコメントに明記）。★★ **第 3 便で LP の規則と Lily# 側の穴まで特定し、*着手せずに止めた*（下）。**② 第389 ⑩ ②（`grace-unfold` の最後の音→小節線 +0.72）。③ ★ **測った（第 2 便）＝フルサイズでも LP は付点の箱を*符頭自身の段*に置く**（計器 `lpdump/fullsize-dot-row.ly`＋`run-any.ps1`）: `g8.`（線・付点は 1 つ上に*描かれる*）の右 skyline は **2.4042 が y −5.5249〜−4.0749＝中心 −4.7999＝g の線**、**高さ 1.45（インク 0.45 ＋ esh 1.0）**。**LP の rod 2.5342 は符頭の段で計算したときだけ合う**（描かれた段なら 2.6042）。⇒ `AddDots` が*描かれた段*に置くのは LP と違う。⚠️ **観測できる本があるかは未測**——rod が効くのは行が詰まったときだけで、この本は LP が広く組んだので spring が勝つ（gap 5.00／6.75 対 rod 2.53）。`DotColumn` のコメントも「1 声では付点の wish が rod を上回るので corpus に出なかった」と書いている。⇒ **次の一手**: **① ＝ §2 U8 ⒝2 の一部として**（下）／③ の観測（詰まった本・cross-voice なら出やすい）／②／cue clef の大きさ／第379 ⑺⑻。

★★★ **⑹ 第 3 便（常設指示）＝① の LP 規則と Lily# 側の穴を特定し、*着手せずに止めた*。理由は「字面移植できないから」**。
**LP の床は 1 行**: `note-spacing.cc:78-83` ＝ `min_dist = max(0, skys[LEFT].distance(skys[RIGHT], 右 column の svp))`。**skys は保存された `horizontal-skylines`**（`spacing-interface.cc:37-97`）＝**NoteColumn の 0.15 で建て、距離で PaperColumn の 0.08 を足す**＝Lily# の `SkylineFloorPair` と同じ対（**移植済みの家が在る**）。
**実測（RODSKY）**: `e'8.` は旗 y 1.7032 に対し主音の帯が 1.3449 まで＝**0.358 離れ 0.08 では届かない**⇒床は頭だけ 1.4179、gap は rod 1.9352（LP）。`g8.`／`f8.` は旗が主音の段に**かかる**ので床 **1.9386**（LP）。⇒ **Lily# の 1.9386 は旗を平らな幅で読んだ帰結で、Y を見れば LP に一致する。**
⚠️⚠️ ★★★ **しかし*字面移植*するなら、grace 専用の skyline を書いてはいけない**——**LP は grace column を*普通の NoteColumn*として `Separation_item::boxes` に通す**ので、**Lily# の対応物は既に在る `ItemSkylineFactory.ColumnParts`（＝`boxes` の移植）**であって、**その脇に grace 用の箱組みを 1 つ建てるのは §7.7 の「同じ量の 2 つ目の綴り」そのもの**。**ユーザー指示（2026-09-16）＝「出力に基づいて発明するな・可能な限り LP の実装を字面移植せよ」。**
★★ **通すために要るもの 3 つと配線は §2 U8 ⒝2 に書いた**（`StemSpacingInfo` に grace の腕が無い／`ColumnParts` 族が full-size design を直読み／`NarrowToGraceTime` が符尾と梁を運ばない。配線＝`GraceNoteLayout.ColumnItemIndices`）。
⇒ ★★★ **つまり ① は「床を skyline にする」パッチではなく、§2 U8 ⒝2（普通の engraver に grace time を歩かせる）の一部**。**出力は grace を持つ本全部で動きうるので、独立した掃きと承認が要る＝この便の道具では短くならない別 regime。** ⇒ **第 3 便は着手しない**（判定の型＝「この便の道具が残っている宿題だけ着手する」）。

★★★ **⑺ 第 4 便（常設指示）＝③ を*観測できる本*まで作り、LP の規則は測れたが、移植は*取り下げた*（Core は無変更に戻した）。**
★ **着手の鍵**: **交差声部の対は wish が無く rod だけで床が決まる**（`ApplyCrossVoiceColumnSpacing` の doc・`spacing-interface.cc:36-97`）＝**行を詰めなくても rod が見える**（第 2 便の「別 regime」はこれで外れた）。本＝`lpdump/dot-row-crossvoice.ly`／`ks/verify/dot-row-crossvoice.lys`（下声部 `g2.`＝線・上声部 `c'8` が 3 ポジション上＝段の選び方で答えが変わる形）。道具＝`lpdump/run-any.ps1`・`exe-head`・`sweep2`・`kg/afterbar2.ps1`。
★★★ **LP の実測（これは残る）**: `RODPCSKY x=8.5849` の付点の箱は **2.4774 が y −5.805…−4.195 に平ら ⇒ 箱 −5.725…−4.275・高さ 1.45（インク 0.45 ＋ esh 0.5×2）・中心 −5.0＝g の線そのもの**（`RODDOTS` の*描かれた*付点は −5.5）。**LP の rod は 2.6074**（生 `2.6073999999999997`・素の対は 1.6042）。⇒ **`Separation_item::boxes` は pure Y に esh を足す**（`separation-item.cc:163`・`:168-183`・`define-grobs.scm:1277`）。**フルサイズでも grace でも同じ。**
⚠️ **Lily# の穴は 2 つ**: `AddDots` は箱を*描かれた段*に置き、**esh を足さない**（高さ 0.45）。
⚠️⚠️ ★★★ **だが「直すと 2.6074 になる」は*毒で反証***: `ItemSkylineFactory.cs` を stash し Core と Tests を `--no-incremental` で建て直しても `crossOnly min=2.6074`。⇒ **交差声部の床は変更前から LP と同じ**で、網 `ADottedLineNotesBox_StandsOnItsHeadsRow` は**変更の有無で同じ答え＝番人でない**（削除）。
⚠️⚠️ **一方でページは動く**（次の `c'` が **3.47 → 3.58**＝LP の位置）。**経路は未特定。**
⚠️⚠️⚠️ ★★★ **掃き 938 冊で 13 冊が動き、全部ユーザーの実コーパス**（`Kiss Me`・`Universe`・`奏（かなで）`・`瞳をとじて` ほか）。**`Universe.lys` は改行ごと動く**（`35.29 60.99 84.05 102.24` → `54.97 102.24` …）。⇒ **原因が言えないまま実コーパスを 13 冊動かす変更は出さない**（ユーザー指示「出力に基づいて発明するな・LP を字面移植せよ」）⇒ **`git checkout` で Core を戻した**（製品変更 0）。
★ **残したのは**: 上の LP 実測と **1.7944 → 1.7945 の訂正**（生 `1.794455`・コード commit の message は直せないので正しい桁はここ。原因は ⑵ の切り捨て）。
⇒ **③ 再開の順**: ⒜ **ページを動かしている経路を先に特定**（下の第 5 便で*単独の付点小節は除外できた*）⒝ LP の箱（pure Y ＋ esh）を*その*経路へ移植し、13 冊が LP の双子に近づくことを冊ごとに見る ⒞ **網は毒で赤くなることを先に確かめてから書く**（本便は番人でない網を 1 本書いた）。

★★ **⑻ 第 5 便＝⒜ を測り、*否定的な結果*（コードは無変更）。** 利＝**両 binary が残っていた**（`exe-head`／`exe-head2`）ので建て直しも掃き直しも無しで A/B できた。
**⑴ `lysc layout` の A/B**（`Universe.lys`・既定の score は `staff`＝**notation**）: **33 段 → 38 段**・`12,13,8` → `12,13,13`・**17〜40 小節が 4 小節/段 → 2 小節/段**＝改行が動いたのは確か。
**⑵ しかし単独の付点小節は動かない**: その書法の 1 小節（16/16）＋付点なしの対照（12/16 ＋ `r4`）＝`ks/verify/dotted-bar-width.lys` を両 binary で描くと**小節線 42.41 / 73.65 がバイト一致**。⇒ ★★★ **「箱が高い → rod が増える → 小節が広がる」は 1 小節では起きない。第 4 便の筋書きは輪郭としても裏が取れていない。**
