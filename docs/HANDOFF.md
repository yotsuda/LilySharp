...
---
## 1. 現在地 ← **毎セッション書き換える**
最終更新 第248セッション＝**ユーザーが残債の返済を中断させ、画面に見えている欠陥を*3 つ*直させた。3 つとも別の欠陥で、1 つ目を直すと 2 つ目が見え、2 つ目を直すと 3 つ目が見えた。**（3 commit＝`5fd5c770` staff ばね／`af693184` 行を鎖の要素に／`d2f8ae4d` anchor を system ごとに）⚠️ **台帳 583→606 は第247。本便は 606 不動／exact 498→502／総和 37.645494543→18.421820311／非ゼロ 135→131／count 117・非ゼロ 2 不動／snapshot 222 不動（再ベース 0）／追跡コーパス 572 不動。6128→6136。Windows も WSL Ubuntu Release も 6136/0/4。**

**① ★★★ 第1便＝`MultiStaffLayouter.StaffSprings` が LP の `append_system` を*隣接*として書いていた。** **LP は `is_spaceable` な要素*どうし*にばねを張り、あいだのものは `loose_lines` へ積む**（`page-layout-problem.cc:660-672`・`:919-925`）。**2 つの歩き方が一致するのは「2 五線のあいだに何も立たない」あいだだけ。** ⇒ **裸の `lyrics` 行は alignment の要素**（束縛歌詞は要素ですらない）なので **`staff / lyrics / staff` は spring を 1 本も持たない**。⇒ `CreatePages.OriginToChainEnd` が `ToLast` ではなく `ToFirst` に落ち、**system 間の床が 2 つの system の*最初の*五線のあいだに書かれた**。**報告の本では 1 段目どうしがぴったり 12.000000＝床は*効いていた*、相手を間違えて。** **描かれた 2 枚目と次 system の 1 枚目は 0.470000＝貫通。**

**② ★★★ 第2便＝`ClassifySystem` が「行の下に spaceable 五線が来たら `UnmodelledRow`」で run ごと捨てていた。** **それは `distribute_loose_lines` の*もう一方*の呼び出し**（`:936-939`＝1 system の 2 つの spaceable 位置を渡される）**で、run は run。** ⇒ **`BuildBetweenRowStaves` で run を保持し、`LyricEngraver.BuildChainPrefix` が末尾の行と同じように歩く。** ⚠️ **行の所属は (system, 行) の事実**——**hara-kiri は 1 ページの途中で run を移す**（ROWVH がその形）。**行だけで持ったら mid-system の verse step が 2.800000 → 4.600000 に戻った（実測して直した）。** ⚠️ **裸の行だけの本には*上側の族*も無かった**ので、`-1` の族と対称に合成し、閉じる端の門も広げた。

**③ ★★★ そして 6 点が閉じ、閉じ方が「双子と同じ数」だった。** `…between-staves{,.hara-kiri.first-system}.staff-to-lyric` **+1.472149 → −0.000100321**（束縛歌詞の双子は **−0.000100246**）／`…two-verse…` **+1.762110 → −0.000100660**（双子 **−0.000100585**）／**verse step 2 点 +1.800000 → exact**（双子は前から exact）。⚠️ **残る 1e-4 は双子の項の注記が既に説明している**（LP の glyph-outline `Skyline::distance` に対する lyric face）。**双子どうしの 7.5e-8 は 2 冊の音節の違い。** ⇒ ★★ **0 に押し込まない。**

**④ ★★★ 第3便＝`ResolveAnchor` が上側の族の anchor を `systemsArray[0]` から取っていた。** **`BuildStaffAnchorTables` の注記が自分でその簡略化を宣言し、射程まで書いていた**——「hara-kiri が system ごとに違う五線を殺し*かつ*非最終グループから block が下がるときだけ／fixture も台帳点も到達しない」。⚠️ ★★★ **和音行は hara-kiri 無しで到達する**——**1 つの system に在り別の system に無い行は、その下の五線を*帯ごと*動かす**。**anchor は system 原点からの距離**なので、**行の無い system は system 0 の frame で置かれていた。** **実測: 鎖は 0 / 5.175 / 7.975 / 12.033515 と解いていたのに、音節は*その全部より 1.895000 下*に描かれた**（1.895000＝和音行の帯）。**最後の詩は次の五線の上端の 0.160000 上＝descender が五線を貫通。いまは 2.060000。**

**⑤ ★★★ 3 便に共通する骨は「コメントが正しくてコードが違う」「コメントが自分の射程を間違える」。** ⑴ `StaffSprings` の注記は **60 便のあいだ**「consecutive SPACEABLE elements (:660-672 append_system)」と*正しく*書き、コードは隣接で対を作っていた。**住所も記号名も正しく、REF ラチェットも `LpProvenanceTests` も緑**。⑵ anchor の注記は **「no fixture reaches it」を*幾何の*言明のように持ち回っていた**——**それは fixture についての言明**（§5.3）。⇒ ★★★ **どちらも見つけたのは監査ではなく*ユーザーの画面*。572 冊も 222 snapshot も 6128 本の網も、3 つとも 1 度も綴らなかった。**

**⑥ ★★ 毒の赤は 3 便とも交わらない。** ⑴ walk を戻す → `ARowBetweenTwoStaves_DoesNotTakeTheSystemsFloor` だけ。⑵ 本便の移植を戻す → `OneScoreOnePage_TwoArrangements_TwoAnswers`・`TwoVersesBetweenTwoStaves_AreSteppedByTheAlignment`・`ABareRowAndASungVerse_PutTheirSyllablesInTheSamePlace(2)` の 3 本。⑶ anchor の 1 式を戻す → `LyricAnchorPerSystemTests` の 2 本だけ（他 6134 は緑）。⚠️ **掃きは ⑴⑵ が 1 of 572（`audit/lpreg/lyhygrace.lys`）、⑶ が 0 of 572**——**⑶ の 0 は本物**（**網を書き・毒で赤を見せ・そのあと掃いた**＝§5.0 ⑸ の順）。

**⑦ ★★ 正直に書く損失が 1 つある。** **`lyhygrace` の*あいだ*の行は、帯の 5.500000（LP 5.368569 と偶然 0.131 差）から鎖の 5.150000（0.219 差）へ動いた。** **LP から遠くなった。** ⇒ **動いたのは「その本で本物の機構が走るようになった」ことで、残るのは 2 つの名前の付いた量**（部屋 9.490000 対 LP 9.368569／閉じる最小 4.340000 対 4.000000）。**§5.2.1 ⑷「解かれない鎖が理想に座り、それがたまたま近い」の実例。** **その本の末尾の行は LP と exact（5.500000）。**

**⑧ ★★ 環境: この機械の Smart App Control が Lily# のバイナリをブロックする。** `VerifiedAndReputablePolicyState = 1`・**08-24 以降 147 回**（`LilySharp.Core.dll` 95／`LilySharp.Tests.dll` 50／`lysc.exe` 2）。⚠️ **本便の途中で Debug の `LilySharp.Tests.dll` が塞がれ、`dotnet test` が「テストなし」で*緑に見える*形で終わった**（`Skipping: ... 0x800711C7`）。⇒ ★★ **`-c Release` は通る**。**回避はもう 1 つ、WSL**（Linux には SAC が無い・`git apply` で working tree を持ち込める）。**「成功!」の行が出ないときは必ず `-v q` を外して `Skipping` を読むこと。**

⇒ ★★★ **次の一手**: **残債（`UnmodelledRow` の島）の残りは*部屋*だけ**（HANDOFF 3）。**⑵ ⑶ は第247・第248 が閉じた。** **鎖は行を正しく置くようになったが、行に与えられる*部屋*はまだ `MultiStaffLayouter.GetStaffHeight` の公称帯で、walk の答えではない**——**2 詩を裸の行 2 本で綴ると 14.970000、`sings` で綴ると 13.170000**（**音節のベースラインは両者完全一致**）。⚠️ **`TextRowVerseSpacing = 3.2` の注記が入口を書いている**：「The port is to take the band's height from the same walk, which wants the pair first」——**その pair はもう在る**（`StaffSprings` が consecutive spaceable で張る）。⚠️ **`…hara-kiri.system-gap` の +1.220090／+5.820090 は 1 ミクロンも動いていない**（**実測**——台帳が赤くならなかった）＝**system 間の部屋は別の島**。⚠️ **ユーザー判断が要る項は 4 つのまま**（テキスト行のインク profile／`staff / chords / lyrics` の診断／`make-colon-bar-line` の「LP に合わせる」の意味／**CHANGELOG＝版番号**）。

⚠️ **見つけて直していないもの**:
- **★★★ 行に与えられる*部屋*を walk から取る**（＝HANDOFF 3 の残り）。**行の*位置*は第248 が閉じた**が、**部屋は公称帯のまま**（上の ⇒）。**入口は `MultiStaffLayouter.GetStaffHeight` の `TextRowVerseSpacing` と、その注記が名指す「同じ walk から帯の高さを取る」。**
...
- **★★ `lyhygrace` の 2 つの残差**（⑦）＝**部屋 +0.121431 と閉じる最小 +0.340000**。**LP の実測は取ってある**（`page-vertical.ly` の `PROBEV` を `-dinclude-settings` で流す・本便の手順）。**台帳点にするなら probe を `audit/lp-geometry/probes/` に足すところから。**
...
終了時: **本便は 5 commit**（`5fd5c770`／`e216ec08` §1／`af693184`／`d2f8ae4d`／この行）・**未 push 12**・未追跡 0/木 0・**Windows 6136/0/4**（**Release で。Debug は SAC に塞がれている**＝⑧）・**WSL Ubuntu Release も 6136/0/4**・Core 0 警告（`--no-incremental`）・**台帳 606／exact 502／総和 18.421820311／非ゼロ 131／count 117・非ゼロ 2／snapshot 222（再ベース 0）／追跡コーパス 572**。⚠️ **§7.5 の数え上げ**（3 便合計）: `LilySharp.Core` の `+` **291 行**／**`LILYPOND-REF` 5**（`:660-672 append_system`・`:589 filter_dead_elements`・`:1173-1177 is_spaceable`・`:936-939 distribute_loose_lines`・`:896-914 find_system_offsets`）／**`LILYSHARP-OWN` 2**／**新しい数値定数 0**——**式は 1 つも足していない。足したのは「既にある walk にどの線を渡すか」だけ。** **分類は §7.6 ⒜**（3 便とも LP のループそのもの）**＋ ⒝ 1 件**（写す相手の無い decline）。
...
> と書いていた。**コードは隣接で対を作っていた。**
> **住所も記号名も正しく、`LpReferenceCitationTests` も `LpProvenanceTests` も緑。**
...
> anchor の注記は射程まで丁寧に書いていた——「hara-kiri が system ごとに違う五線を殺し
> *かつ*非最終グループから block が下がるときだけ」。**和音行が hara-kiri 無しで到達した。**
> ⇒ ★★ **原則**: **除外の射程を「今のコーパスが綴らない」で書いたら、
> それは*コーパスの*性質。幾何の言明として持ち回らない**（§5.3）。
> ⇒ ★ **書き方**: 「no fixture reaches it」ではなく
> **「到達するのは X という配置で、コーパスにその冊は無い」**と*配置*を名指す。
> **名指せないなら射程は分かっていない。**

> ## ★★★ 骨 3＝**閉じた点が双子の数と一致したら、それが「同じ walk を通った」の証拠**
> 本便の 6 点は **0 ではなく −0.000100321 / −0.000100660 に着地した**。
> **束縛歌詞の同名項が*前から*記録していた数と 7 桁一致**で、
> **その項の注記が残差を項ごとに説明済み。**
> ⇒ ★★ **判定法**: **残差を 0 と比べる前に、*同じ配置の別の綴り*が何を記録しているか見る。**
> **一致したら、残っているのは機構ではなく両者に共通の項**——**0 に押し込んではいけない。**

> ## ★★ 骨 4＝**「緑」が「走った」とは限らない。テスト数を読む**
> Smart App Control が Debug の `LilySharp.Tests.dll` を塞いだとき、
> **`dotnet test -v q` は何も出さずに終了コード 0 で返った**（`Skipping: … 0x800711C7`）。
> ⇒ ★★ **合否行が*出ない*のは合格ではない。** §0 は「ja-JP では `Passed!` が出ない」を
> 警告しているが、**「何も出ない」の警告は無かった。**
> ⇒ ★ **`成功!|失敗!` を grep するなら、0 件ヒットを失敗として扱うこと。**

---

## 以下は第247セッションの経緯
最終更新 第247セッション＝**第246 が「次便は*本を作るところから*始められる」と名指した `UnmodelledRow` の島を、本を作り・LP で測り・移植した。** ⇒ **移植は正しく効いたが、閉じたのは 1 点だけで、閉じなかった理由が*別の量*に移った**——**鎖は建つのに、渡される部屋が LP の 12.000000 に対して 17.702255／22.752175**。（5 commit＝`50c3a8bd` 4 冊と 20 点／`11d29db2` 移植と 3 点と網／`4cccfad6` §1 とアーカイブ移送／`66926c21` §0 の `exact` の数え方／この行＝次島の入口の絞り込み）⚠️ **台帳 583→606／exact 490→498／総和 7.843880014→37.645494543／count 113→117・非ゼロ 2 不動／snapshot 222 不動（再ベース 0）／追跡コーパス 572 不動。6104→6128。**

**① ★★★ 第1便＝4 冊。第246 の「専用の本が要る」はそのとおりで、しかも*2 度*要った。** `ROWB`＝**裸の `lyrics one` 行が 2 つの spaceable 五線の*あいだ*に立つ本**（全 system）、`ROWH`＝**同じ本で下五線を system 1 以降だけ黙らせ `removeEmpty all` を付けたもの**（＝**行は system 0 では*あいだ*・以降は*唯一の五線の下*）。⚠️ **`LYRB` では代われない**——**その歌詞は音符束縛で、Lily# の alignment の要素ですらないので旗に届かない。裸の行だけが届く。** **LP は綴りを読まない**（`LYRR` の恒等）ので、**LP 側は `LYRB` の数**。**LP の予測 8 つは全部当たった**: ROWB 4.027851／inside 9.000000／page 1 は 8 五線、**ROWH は 1 ページに 4.027851・5.500000・5.500001 の 3 つ**＝**LP は 1 つのスコアの 1 ページの中で system ごとに分岐する。**

**② ★★★ ところが `ROWB`/`ROWH` は*何も測らなかった*。Lily# 側の予測が外れ、外れが本題だった。** **`ROWH` の system 1..3 は最初から exact**——**解かれない鎖は `max(min, ideal)` に座り、12.000000 の部屋に緩い線が*1 本*なら LP の解も同じ 5.500000 に緩む**。**両engineが一致し、分岐は見えない。** ⇒ ★★★ **これは §5.2.1 ⑷「exact は『その regime では動かない』かもしれない」の新しい顔**で、**「欠陥を測る本」は*解が理想から外れる regime*に置かなければならない。** ⇒ **1 変数で外せた＝第2の詩**（`ROWV`/`ROWVH`）。**2 本の緩い線は 12.000000 に入らないので LP は負の力で解き、第1詩は自分のインク床 3.737890 まで落ちる**。**解かない engine は 5.500000 のまま**＝**+1.762110。ここで初めて読める。**

**③ ★★★ 第2便＝移植。`if (alignment.UnmodelledRow) return null;` は*メソッドを抜ける*ので、1 system の形が 20 system 分の鎖を消していた。** **`usable[]` に替えた**（+31 行）。**端を書くのに 2 つの system を読む**——**自分の最後の spaceable 五線と、ページ内に次があるならその system の*最初の* spaceable 五線と leading**——**なので落ちるのは k と、同ページなら k−1 の 2 つ**（第246 の読みどおり）。**次*ページ*は関係ない**（else 枝がページ端で閉じる）。**LILYPOND-REF は `page-layout-problem.cc:936-939 distribute_loose_lines`＝1 つの run を解く呼び出し。**⚠️ **decline 自体は今も LILYSHARP-OWN**（LP は必ず解く）——**移植したのはその*粒度*だけ**で、注記にそう書いた（§7.6 ⒝）。

**④ ★★★ 閉じたのは 1 点、幅は 1 ミクロン。** `lyrics.row.between-staves.hara-kiri.last-system` が **−0.000001 → +0.000000021（exact）**。**その system の run はページ*端*で閉じ、閉じばねは最後の行自身の descent（:1004-1013）——帯にはその項が無い。** **他は 1 つも動かない。**

**⑤ ★★★ そして動かない理由が「数」になった。ここが本便で一番の収穫。** **鎖は建っている**（solver の trace が 2 詩の inner system を spec のばねで解いている——verse step は 2.800000 で出る）**が、渡される部屋が 17.702255／22.752175 で、LP は両方 12.000000。** **締まっていない部屋に鎖は縮まない。** ⇒ **新点 2 つ** `lyrics.row.between-staves{,.two-verse}.hara-kiri.system-gap`＝**+1.220090／+5.820090**。⚠️ **さらに その部屋（17.702255）は*描かれた* gap（13.220090）とも一致しない**——**`BuildLooseChainEnds` は緩い線が解かれる*前*のページを読み、解いた結果ページが締まる。** **LP にこの問題は起きない**（緩い線はページのばね鎖に最初から居ない）。

**⑥ ★★ 帯の誤りは 1 つで、3 通りに測れる。** **verse step +1.800000**（帯は 2 詩を 4.600000 で積む・spec は 2.800000）／**system 内 +1.028706・+3.555642**／**system 間 +1.220090・+5.820090**。⚠️ **17.820090 − 13.220090 = 4.600000** ＝**第2詩がLily#のページに掛ける費用で、LP には 0**。**同じ 1 つの帯。**

**⑦ ★★★ revert で 2 件捕まえた——どちらも「緑を信じる前に赤を見せる」（§5.4）で。** ⒜ **注記に「移植が verse step を閉じた」と書いた**——**旗を戻して測ったら 2.800000 のまま**。**帯自身が system ごとに分岐していた**（**唯一の五線の下では正しく・2 五線のあいだでは誤る**）ので、**あれは移植の手柄ではない**。⒝ **新しい網の閾値 `reading > 5.5` は毒で*緑のまま*だった**（decline 側の読みが 5.5 + 最下位ビット）。⇒ **2 冊の差にした**（1.021e-6 対 −3.0e-10）。

**⑧ ★ 掃きは 0 of 572、そして 0 は本物。** **対照の毒（`BuildLooseChainEnds` を無条件 `return null`）が 7 of 572 を動かす**（第246 と同じ 7 冊）ので、**掃きはこの経路に届いている。追跡コーパスはこの形を 1 冊も綴らない。**

⇒ ★★★ **次の一手**: **`UnmodelledRow` の島は「粒度」が閉じ、残りは*部屋*と*帯*の 1 島**（HANDOFF 3＝**行を alignment の要素にする**）。**着手点は 2 つの system-gap 点と、`BuildLooseChainEnds` が読むページが「解く前」であること**（⑤）。⚠️ ★★ **第247 が最後に 1 つ*読んだだけ*で終えた手掛かり**（**測っていない＝仮説**・§5.2.1）: **`LyricReservationBelowSystem` の walk は k>0 に `NonStaffNonStaff.MinimumDistance`（2.8）を渡しており、字面上は正しい**。⇒ **+1.220090 と、詩 1 本あたり 4.600000 という増え方は、この歩幅の中には無さそう**——**ページばねの床（padding ＋ 次 system の up-ink ＋ この system の下向き予約）のどこか。** ⚠️ **この段落は読みであって観測ではない。次便はまず「予約の深さ」を 2 冊で*測って*から site を決めること**（**第247 は Lily# 側の予測を 8 つ中 3 つ外し、さらに 2 件を revert で捕まえている**——**この subsystem について、読みは当てにならない**）。⚠️ **ユーザー判断が要る項は 4 つのまま**（テキスト行のインク profile／`staff / chords / lyrics` の診断／`make-colon-bar-line` の「LP に合わせる」の意味／**CHANGELOG＝版番号**）。

⚠️ **見つけて直していないもの**（第246 から持ち越し・**`UnmodelledRow` の項は⑤⑥で*書き換えた***）:
- **★★★ 行を alignment の要素にする**（＝HANDOFF 3・**第243 起票の `UnmodelledRow` の島の*残り***）。**粒度は第247 が閉じた**（`usable[]`）。**残るのは帯**: ⑴ **部屋** `lyrics.row.between-staves{,.two-verse}.hara-kiri.system-gap` +1.220090／+5.820090 ⑵ **system 内** `…staff-staff-inside` +1.028706／+3.555642 ⑶ **verse step** `…two-verse.verse-step` +1.800000。**3 つは同じ帯**（⑥）。⚠️ **さらに `BuildLooseChainEnds` は*解く前*のページを読む**（描かれた gap 13.220090 に対し部屋 17.702255）——**LP は緩い線をページのばね鎖に入れないのでこの循環が無い。**
- **★★ 第244 の乖離の在処＝`MusicMarkEngraver` の rows-only の腕**（⒜ を落とせば `base + Padding` に戻り予約も落ちる）。**LP は 0.460000 上に置く**（実測・`8d6b7978`）。**ユーザー決定なので閉じない。**
- **★★★ 骨 9 ⒞ の残り＝テキスト行に*インクの profile* を与える島**（`SkylineBuilder.AddStaffToSkylines` はテキスト行で早期 return）。**第241 から名指され、`barnumber.rows-only.row-to-ink-bottom` の 2026-08-24 のユーザー決定と当たる。**
- **★★ 再現できていない報告＝1 小節に和音 4 つで和音名が横に重なる。** **第243 が 9 通り試して全部隙間は正**（最も狭くて 0.15）。⚠️ **`ChordNameEngraver.ClearOfPrevious` の抜け道**＝`if (!cur.UseTiming || !prev.UseTiming) return curX;`。⚠️ **第244 がその隣に第 2 の押し出しを足した**ので**実物が来たら両方見ること。当てずっぽうで直さない。**
- **★★ 第245 第2便の残り＝`IsSilent` は今も 6 site が各々読んでいる**（ページ 4・双子 2）。**畳むなら `FormAlternativeSyntax` に「ラベル」を解かせる**。**独立した便にはしないこと（§5.1）。次に volta か section label に触る便に括り付ける。**
...
★ **開始時裏取り**: HEAD `0574d845`・**未 push 2**・未追跡 0/木 0・**Windows 6104 合格 / 0 失敗 / 4 skip**・台帳 583／exact 490／総和 7.843880014／count 113・非ゼロ 2／snapshot 222／追跡コーパス 572・Core 0 警告。
終了時: **本便は 5 commit**（`50c3a8bd` 本と点／`11d29db2` 移植／`4cccfad6` §1 とアーカイブ移送／`66926c21` §0 の数え方／この行）・**未 push 7**・未追跡 0/木 0・**Windows 6128/0/4**・**WSL Ubuntu Release も 6128/0/4**（README が「この WSL にだけ出る」と書く `FontBlockCompletionTests` の 2 件は**出なかった**）・Core 0 警告（`--no-incremental`）・**台帳 606／exact 498／総和 37.645494543／count 117・非ゼロ 2／snapshot 222（再ベース 0）／追跡コーパス 572**。⚠️ **§7.5 の数え上げ**: `LilySharp.Core` の `+` **31 行**／**`LILYPOND-REF` 1**（`page-layout-problem.cc:936-939 distribute_loose_lines`）／**`LILYSHARP-OWN` 0**／**新しい数値定数 0**。**分類は §7.6 ⒝**——**LP には写す decline が無い**ので、**字面なのは「run ごとに 1 つ」の分岐だけ**。**字面にし切るには行を alignment の要素にする必要**があり、それが上の第1項。⚠️ **`audit/probe-out/*.csv` は生成物で git 管理外**、**`scratch/books572.txt` も同じ**（`git ls-files '*.lys'` で作り直せる）。

> ## ★★★ 骨 1＝**解かれない鎖は「解かれた答え」に座ることがある。欠陥を測る本は*解が理想から外れる regime*に置く**
> `ROWB`/`ROWH` は**正しく作られた対で、正しく何も測らなかった**。緩い線が **1 本**なら、
> **`max(min, ideal)` の理想と、12.000000 の部屋に解いた答えが、どちらも 5.500000。**
> **両engineが一致し、旗の分岐は 1 ミクロンしか見えない。**
> ⇒ ★★★ **判定法**: **点を開く前に「LP の解はこの本で理想から外れるか」を確かめる。**
> 外れないなら、**その本は decline を測れない**。本便は **第2 の詩**で外した
> （2 本の緩い線は 12.000000 に入らない ⇒ LP は負の力で解く ⇒ +1.762110 が読める）。
> ⚠️ **これは §5.2.1 ⑷「exact は正しいではなく、その regime では動かない」の*本の設計*版**
> ——あちらは既存の点について、こちらは**これから作る点**について。

> ## ★★★ 骨 2＝**移植が効いても答えが動かないなら、理由は消えていない。移った先を*数*で名指す**
> 本便の移植は**確かに効いた**（trace が per-system に鎖を建てている）のに、
> **台帳は 1 点しか動かなかった。** ここで止めれば「移植は無駄だった」になる。
> ⇒ **渡される部屋を読んだ**: **17.702255／22.752175 に対し LP は 12.000000。**
> **締まっていない部屋に鎖は縮まない。** ⇒ **点を 2 つ開いた**（system-gap）。
> ⇒ ★★ **原則**: **「閉じなかった」で終わらせない。閉じなかった*理由*が
> 前と同じか違うかを言う。** 本便は **「鎖が無い」→「部屋が違う」** に移っており、
> **それは進捗**（次の島の入口が 1 つ減っている）。
> ⚠️ ★★★ **副産物**: **その部屋は*描かれた* gap とも違った**（17.702255 対 13.220090）
> ——**`BuildLooseChainEnds` は解く前のページを読み、解いた結果ページが締まる。**
> **LP は緩い線をページのばね鎖に入れないので、この循環そのものが無い。**

> ## ★★ 骨 3＝**「移植が閉じた」と書く前に、旗を戻して同じ量を測る**
> 本便は注記に **「narrowing がこの verse step を 2.800000 にした」** と書いた。
> **旗を戻すと 2.800000 のまま**——**帯自身が system ごとに分岐していた**
> （**唯一の五線の下では正しく、2 五線のあいだでは 4.600000**）。
> ⇒ ★★ **判定法**: **移植の功績として書く量は、必ず revert で測り直す。**
> **同じ run で赤を見せられないなら、それは功績ではなく偶然。**
> ⚠️ **網の閾値も同じ検査で落ちた**: `reading > 5.5` は **毒で緑のまま**だった
> （decline 側の読みが 5.5 + 最下位ビット）。**2 冊の差にすると 1.021e-6 対 −3.0e-10。**
> ⇒ ★ **絶対値の閾値ではなく、対照との差で書く。**

> ## ★ 骨 4＝**復元したファイルは `--no-incremental` で建てる**
> 本便は移植を一度 revert して測り、**戻したあと増分ビルドで測って「回帰した」と読んだ。**
> **測っていたのは古い DLL。** §0 の `--no-incremental` は「0 警告を確かめるため」と
> 書いてあるが、**ファイルを*外から*書き戻した直後にも要る。**
