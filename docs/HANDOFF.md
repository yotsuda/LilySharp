...
- ★★★ **⒮⁶ 「建てない」の島の残り＝約 73,356 B／打鍵 1.60%**（第448 実測 92,144 から**第449 が閉じた
  ⒮⁵ の実測 18,788 を引いた値＝未再測**。次に census を回す便は、この数ではなく脚の出力で置き換えること）。
...
- ⚠️⚠️ **`docs/RULES.md` は 247,272 / 250,000 B・1,862 / 2,000 行＝天井の 98.9%**（第449 実測。今便ぶん 2 件は
  入ったが、**次便はもう入らないと見てよい**）。⇒ **第450 は先に §5 の実例を `RULES-CASES.md` へ出すこと**
  （第433 がやった形＝規則文を残し、経緯に `（→CASES 5.N-0MM）` を付けて appendix へ）。⚠️ **マーカーの
  無い項目は 128 軒 78,908 B**（第449 実測）で、**大きい 8 軒だけで約 13.5 KB**＝**そこから出せば 1 便ぶん空く**。

### 1.1 第449セッション（2026-09-21・YT-DELL2）

`/clear` 直後の新セッション。指示は「**HANDOFF を読んで着手**」で、**着手先は §1.0 ⒜ の ⒮⁵**（ユーザーが
選んだ・⒮⁵ と ⒮⁶ と ⒮″ と LP 双子を並べて訊いた）。★ **`-Start p449` の 1 コマンドで §0 が全部済んだ**
（HEAD `cb4401ff`・未 push 6・full `sessions/p449/run1.trx` 8774 / 0 / 3 / 8777・台帳 851 点／
総和 22.584727806・snapshot 249・追跡 `.lys` 609・`-Archive 447` も自動）。**裏取りは 1 つも赤を出さなかった**。

★★★ **⑴ 天井は機械が読めない形で詰まっていた。** `Fold-ClosedHandoffItems.ps1` は「folded 0」と言ったが、
HANDOFF は 444,003 / 450,000 B（残り 5,997）で**今便ぶんが入らない**。⇒ 1 行ずつバイトを数えたら
**§2 R. の `R13 プレビュー遅延の構造` が 25,495 B の*1 行***で、fold の条件（本文＝次の行以降）に
原理的に掛からなかった＝**「閉じた本文」は在ったが、本文*行*が 0 だった**。⚠️ **落とす前に
`git grep` でコードからの参照を数えた**（`R13⒜` 3 本・`R13⒩` 4 本・`R13⒫` 1 本）＝**その 3 つの主題は
§2 に残した**。落ちたのは経緯だけで、**開いている ⒝⒞⒠⒣⒤′ と「戻らない島」も §2 に残る**。
⇒ **420,907 B（残り 29,093）**。

★★★★ **⑵ 起票は当たっていて、当たった理由は「平均ではなく 0／1／多を数えた」から。** ⒮⁵ の処方箋
（`Take` に 2 本目の手を足す・呼び手 4 か所の仕上げ式も同時に）は**そのまま通った**。**A/B は parse
351,407 → 332,619 B／打鍵（−5.35%）・render は雑音の内（4,233,644 → 4,233,632）・合計 −18,788 B／打鍵
（−0.410%）**（231 冊 × 8 打鍵・Release・TC=0・**各側 2 回**）。⇒ ★★★ **census の予測 18,821 に対し実測
18,788＝99.8%（33 B 差）**。**第448 の render が 96.5% で*下*に外れたのと違い、今回は上下どちらにも外れていない。**
⚠️ **render が動かなかったことも別に主張していた**（予測 2「GreenRun は parser のみ」）＝**実測 12 B 差で当たり**
＝**この島は parse の側にしか無い**。

★★★ **⑶ 直したのは 1 箇所と、その仕上げ 4 か所。** `GreenRun.Take` が `first`／`second`／`more` の 3 段になり、
**list は*3 本目*で建つ**。仕上げは `Lexer.ScanTrivia`（`SyntaxTriviaList([first!, second])`）／`ParsePitch` の
octaveMarks／`ParseOptionalDuration` の付点／`ParseArticulations` の 4 つで、**どれも「2 本のとき」の腕が 1 本
増えただけ**＝**返す配列は前と同じ 2 要素**（だから減るのは List の object 32 B ＋ 最初の配列 56 B ＝ 88 B ちょうど）。

★★★ **⑷ 出力同一は 2 つとも**: ⑴ `rerender-ls -Compare`＝**絵が動いた本 0 / 81**（⚠️ **baseline が第437 の
HEAD で stale だったので、変更前の HEAD で取り直してから比べた**＝RULES §5.5）／⑵ **実コーパス全ページ
SHA-256＝5,824 行・0 行差**（baseline `p439/hashes-after.txt`）。

★★★ **⑸ 毒 11 本——11 本とも予測どおり赤。** `Take` の 3 段そのもの: 2 本目を捨てる **+455**／2 本目が 1 本目を
上書き **+454**／3 本目以降を捨てる **+275**。4 つの仕上げ: trivia が 2 本目を落とす **+36**／3 本以上で 2 本目を
落とす **+254**／2 本を逆順 **+2**／octave mark が 2 本目を落とす **+183**／逆順 **+3**／3 本以上で 2 本目を落とす
**+22**／2 つ目の付点を落とす **+12**／2 つ目の注釈を落とす **+59**。⇒ ★★ **「0.4% しか通らない腕」（毒 3・
毒 5・毒 9）にも観測者が居た**＝**尾は測れないほど小さくても、見ている本は在る**。

⚠️⚠️ ★★★★ **⑹ 毒のベースライン行が 1 赤を叫んだ——そして正体は*私が壊した*ものだった。** 毒の前の
ベースラインが **8773 / 1 / 3 / 8777**（開始時は 8774 / 0）。⇒ `ApproximationInventoryTests.TheInventoryIsNotStale`
＝**`docs/APPROXIMATIONS.md` は生成物で、`Parser.Music.cs` の行が :435 → :440 に動いた**（450 行・APPROX 60／
OWN 143／TOTAL 255 は不変＝**行番号だけ**）。**再生成（`LILYSHARP_UPDATE_DOCS=1`）で消えた**。
⇒ ★★★ **これは第448 が「毒スクリプトのベースライン行は飾りではない」と書いた罠の*別の宿主***——
あちらは使い捨ての `Zz` がスイートに混ざる形で、**こちらは*自分のコード変更*が生成物を stale にする形**。
**どちらも「毒のせいに見える赤」で、ベースライン行だけが言い分けられる。**
⚠️ ★★ **そのうえ再生成器は `APPROXIMATIONS.md` を*全 449 行 LF* で書く**（repo の他の md は CRLF・
`.gitattributes` は `*.lys` と snapshot svg にしか eol を与えていない）。**`core.autocrlf=true` が commit を
吸収するので `git diff` には出ない**＝**気づくのは `git status` の warning だけ**。CRLF に揃え直した。
⇒ **CLAUDE-OPERATIONS §1 の「改行を壊す 3 つの顔」に*4 つ目*（生成器）を足した。**

★ **⑺ 終了時**: commit 2 本（code `bba593e3`・docs 1 本＝**この文を含むので SHA は書かない**＝§5.4）。
**最終 full 8774 / 0 / 3 / 8777＝開始時と同値**・§7.5（対 `cb4401ff`）**Core '+' 53 行／REF 0／OWN 0**
（⚠️ **式も定数も 0＝§7.6 ⒟**。grep に出た 3 行は全部*註の中の実測値*）・**未 push 7**・台帳 851 点／
総和 22.584727806 不変・snapshot 249 枚不動・追跡 `.lys` 609・**棚卸しは行番号 1 つ・`magic_constants.csv` は差分なし**。
全文は Lab `sessions/p449/`。**push はユーザー**（Lab も）。
⚠️ **天井の残りは今便 23.1 KB を作って使った**（§2 R13 の 25,495 B の 1 行→ARCHIVE）。**`-Start p450` の
1 コマンドから入る**。
