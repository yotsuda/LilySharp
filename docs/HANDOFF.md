...
- ★★★ **cue の島は第176 で閉じた——ただし 1 形だけ残っていて、それは*別の場所の*残件と同じもの**
  （2026-08-15）。**`cue-context-design.md` §4 の禁止は全部実装済み**（LYS4012 が第176第1便・
  LYS4013/4014 は既存）。**§8 ⑵ の「`\cueClef` は今は足さない」は stale で、入っていた。**
  ⚠️ **残っている 1 形＝隣り合う cue の間を跨ぐスパン**（`cue { … } cue { … }`）。
  **両端とも `IsCue` 真**なので LYS4012 が見えないが、**LP は拒否する**（スラーは警告 2 本／
  タイは無警告でバイト同一 `B53141B8…`・第176 実測）。
  ⇒ ★★★ **同じ原因の食い違いが spacing 側にもう 1 つある**——
  `SpacingRules.CrossesVoiceBoundary` の remarks が
  **「`IsCue` は per-note flag であって region identity ではない」「goes away when: 刻印になったとき」**
  と**先に書いてあった**。**cue のフラグを「導出」から「刻印」に変えると 2 つ同時に閉じる。**
  **観測者はどちらも 0**（back-to-back の cue を書く本がディスクに 1 冊も無い＝2026-08-03/08-10/08-15 の 3 回）。
  ⚠️ **これは*締める*向きの残件**（いま黙って受理している綴りが 1 形ある）＝
  **§2F の他の項と向きが違う**。ただし**書く本が 0 なので急ぎではない**。
  ⚠️ **着手するなら item のレコードに触る便**（`NoteItem`/`ChordItem` の `IsCue`・
  読み手は 40 site 前後だが全部 bool 読みなので `IsCue => CueRegion != 0` で据え置ける）＝
  **出力が動きうる**（spacing 側が隣接 cue の対を refine し始める）ので**点が先・要承認**。
