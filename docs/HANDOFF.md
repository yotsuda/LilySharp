...
最終更新 第205セッション＝**描画バックエンド 2 軒の shaping を閉じた便**
（**実装 3 便**。**perf は測っていない＝走査を 1 本も足していないため**。§7.9 の
「足していない例」・commit ごとに理由を 1 行書いた）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **第204 の §1 ⑹ ⓪ が「次は 0.3.0 を出すかの判断」としていたので、
裏取りのあと*訊いた*。** **ユーザー決定は 1 件＝「⒜ 描画側の shaping 2 軒」**
（**選択肢は ⓪0.3.0 を出す／⓪' CHANGELOG を書いてから 0.3.0／⒜ shaping／⒝ perf の 4 つ**）。
⇒ ★★★ **0.3.0 を出すかは*まだ決まっていない*。次便も同じ設問から始まる**（下の ⑹）。
★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 525 点・ss 非ゼロ 99・
総和 3.644108828・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 21**・
**snapshot 218 枚**・suite **5446 passed / 0 failed / 4 skipped**・**コーパス 567 冊**・
**Core 0 警告**＝**引継ぎの数は全部合っていた**（**35 回連続**）。
⚠️ **`origin/master` は `9830873f`＝第204 より前の commit なので、便の途中の push は無かった**
（§0 の最後の確かめ方）。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 25**（**この行を書く commit を含めて 25**。
⚠️ **この行を*書いた時点*では作業ツリー項目 2**＝`docs/HANDOFF.md` と `docs/HANDOFF-ARCHIVE.md`）・
suite **5446 → 5449 passed / 0 failed / 4 skipped**（**+3＝観測者 3 本**）・
**台帳 525 点で不動**（ss 非ゼロ 99・総和 3.644108828・count 106/2 も不動）・
**snapshot 218 枚で不動＝中身も 1 枚も動いていない**・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Turmeric`）で通せた。**

> ## ★★★ この便の骨＝**探しに行ったら、記録に無いずれが「shaping ぶん」ではなく「face ぶん」で出た**
> **第201 が残した見立ては「⒜⒝ どちらも予約は正しく、ずれるのは shaping ぶん（kern・合字）で
> face ぶんではない」。** **⒜（PNG）はそのとおりだった。⒝（PDF）は違った。**
>
> | | 予約が測るファイル | 描画が出すファイル |
> |---|---|---|
> | `font { serif "TeX Gyre Schola" }` | 同梱 **Schola** | 同梱 Schola ✅ |
> | **`font { sans "TeX Gyre Schola" }`** | 同梱 **Schola** | **同梱 Heros** ❌ |
>
> ⇒ ★★★ **`TextFontMetrics` は名前を*その名前の*ファイルに解決する**（名前は face を指すのであって
> 書かれたスロットを指さない）が、**PDF の resolver は「*束ねられた* family の stand-in」を出していた。**
> ⇒ ★★ **これは第203 の骨と同じ形**——**記録された site を直すだけで終えていたら、
> 大きいほうを踏んだまま出荷していた。** **規則は `TextFontMetrics.TryBundledFamily` の 1 軒**で、
> **doc に「`TextFace` の 2 つの family 欄の*どちら*に答えているか」を書いた**
> ——**両側が食い違っていたのはまさにそこ**だから。

> ## ★★★ 2 つ目の骨＝**観測者の 1 本目は足りなかった。毒が緑のままだった**
> **cross-family の probe を「family を*束ね*から取る」毒で汚したら、*配置の網は緑のまま*だった。**
> ⇒ **理由は構造**: **配置は `ShapeRun` から来て、それは*描かれる* face が何であれ
> *測った* face を渡される。** ⇒ **span を見る網は shaping を pin するが face を pin しない。**
> ⇒ ★★ **ページの `/BaseFont` も読ませた。** **毒の下では
> `Collection: ["HUTCCG+TeX#20Gyre#20Heros"]` と*証拠付きで*落ちる。**
> ⚠️ ★★★ **「観測者を建てた」で止めず、*建てた観測者に毒を当てる*まで**——
> §5.0 の型だが、**この便は 1 本目が実際にすり抜けた**ので実例として残す。

> ## ★★ 3 つ目の骨＝**射程 0 冊を*測って*から、それでも移植した**
> **追跡 567 冊に `font` 宣言は 1 冊も無い**（**コメントを剥いでから数え・毒の 1 冊で計器の到達を確認**
> ＝§5.3 の「0 は『測れていない』の顔で出る」）。**ディスク 1119 冊の 10 冊は前の便の scratch probe。**
> ⇒ ★★★ **snapshot も無力**——**218 枚は全部 SVG で、PNG も PDF も 1 枚も無い。**
> **それがこの 2 軒が生き延びた理由そのもの**（`BackendKerningTests` の冒頭が 2026-08-03 にそう書いている）。
> ⇒ ★★ **だから網に入らない枝は*便外で 1 度だけ実測した***（下の値段の表）。

> ## ★★ 4 つ目の骨＝**§7.5 の機械的な数えが、また 1 件出した**
> **Core に 184 行足して `LILYPOND-REF` 0 本・`LILYSHARP-OWN` 0 本。** §7.5 の「0 本や 1 本なら監査対象」。
> **項ごとに §7.6 で分類したら、1 つを除いて全部 ⒟**（発明の削除／既存の家を指し直し）だった。
> ⇒ ★★★ **⒟ でなかったのは「PDF の stand-in」**——**LP には対応物が*無く、持てない***
> （`select_font` は必ず*ファイル*を得るのでこの設問が生じない）。
> **`embedded` を書かない限り第三者 face を埋め込まない＝*licence の決定が組版の帰結を生む*。**
> ⇒ **`LILYSHARP-OWN` に 4 点**（どの LP 行から外れたか／どちら向きに倒れるか／いつ消えるか／
> **観測者は無い、そして持てない**）。

★ **この便の値段**（**perf は測っていない**＝§7.9 の「足していない例」。
**走査を 1 本も足していない**——**予約側が既に解決していた face を描画側にも読ませ、
chain の 2 つ目の walk（`FirstAvailable`）を削っただけ。** **むしろ 1 描画あたりの割り付けが減った**
——**旧経路は `"family|size|style"` のキー文字列を組み、名指し face では
`SKTypeface.FromFamilyName` を 1 描画ごとに呼んでいた。** **pass も rebuild も新しい鍵も無い。**）:

| 便 | 何が動いたか | 射程（**追跡 567 冊／ディスク 1119 冊**） |
|---|---|---|
| ⑴ PNG を予約したファイルで・shaped に | Core 1 ファイル ＋ 網 1 本 | **0 冊**（**snapshot 0 枚＝PNG は 1 枚も無い**） |
| ⑵ PDF を同じ形に ＋ cross-family の face ずれ | Core 3 ファイル ＋ 網 2 本 | **0 冊**（**snapshot 0 枚＝PDF も 1 枚も無い**） |
| ⑶ §7.5 の監査が出した `LILYSHARP-OWN` | Core 2 ファイル ＋ 網 1 本（**注記のみ**） | **0 冊**（出力不変） |

- **⑴ ★★★ 第1便＝PNG**（`cefdc68a`）。**観測者を先に建てて赤を見た**——
  **`reserved -2.185172` に対し `drew 0.000000`**＝**`BackendKerningTests` の doc が予言していた
  「shaping を通さない backend は右辺にちょうど 0 を置く」の署名。**
  ⚠️ **probe が*同梱 family の名前*を書く理由**: **Georgia を名指す網は機械の持ち物で受かる/落ちる**が、
  **試したいのはフォントではなく「描画側が*予約が開いたファイル*に届くか」。**
  **`ResolvedFace.IsBundled` は「名前が*書かれたか*」であってどのファイルに着くかではない**ので、
  **`TeX Gyre Schola` と書けば*どの機械でも同じファイル*のまま名指し枝を通る。**
  **1 本目の assert がそれを pin する**（bundled に解決し始めたら「もう名指し枝を見ていない」と言って落ちる）。
  ⚠️ **`TextRole.SystemBrace` に明示分岐**——**`FirstAvailable` が特例にしていたので、
  分岐なしで消していたら serif の「{」を黙って描く退行だった。**
- **⑵ ★★★ 第2便＝PDF**（`753d67fe`）。上の骨 1・2。
  **3 分岐**（**同梱ファイル／`embedded` で本物の program／それ以外＝stand-in は unshaped のまま**）。
- **⑶ ★★ 第3便＝§7.5 の監査**（`5a7833c`・**注記のみ・出力不変**）。上の骨 4。
- **⑷ ★★ 便外で 1 度だけ実測した**（**網に入らない枝の代わり**・`scratch/p205/`）:
  **`named-face.lys`**（Georgia の title・**この機械に Georgia は在る**）で
  **PNG の title インク 1027 → 1029 px＝+0.1 ss・左端 1 px 外・音楽は不動**
  （**356 インク行のうち動いた 51 行は全部 title 帯**）／
  **`embed.lys`**（`font "Georgia" embedded`）で **PDF のテキスト配置 8 → 30 件**
  ＝**1 文字列 1 配置がクラスタごとになった**・**`/BaseFont` は `PBZCOJ+Georgia`。**
- **⑸ ★★ この便が見つけて動かさなかったもの**:
  ⇒ **`PngDrawingContext.FontCache.Dispose` は `SKTypeface.Default` を dispose しうる**
  （`GetTypeface` の `tf ??= SKTypeface.Default` が `_typefaces` に入り、borrow 集合に入らない）。
  **今は到達しない**（music/brace の family は必ずファイルから解決する）**ので触らなかった**が、
  **既存の欠陥であってこの便が作ったものではない。**
  ⇒ **`FontCache` の text 用の枝（`TryLoadBundledText`／`_borrowed`／`IsGenericTextFamily`）は
  この便から*到達しない***（**text は `TextFace` の扉から入るようになった**）。
  **消さずに残し、doc に「family 名の扉は music 用で、text が将来また来たときに
  Times New Roman ではなく測ったファイルに着くための備え」と書いた。**
- **⑹ ★★★ 次に触るならここ**——**⓪ まず 0.3.0**（**この便の着手はユーザーが「⒜ を先に」と
  決めたぶんで、その次は*まだ*「出すか」の判断**。**`Directory.Build.props` は `0.3.0`・
  タグは 1 つも無い・remote は `github.com/yotsuda/LilySharp`・`v0.3.0` を push すると
  `.github/workflows/release.yml` が test → CLI 4 RID（win/linux/osx-x64/osx-arm64・
  ReadyToRun・非 single-file）→ VSIX を回し、LICENSE と THIRD-PARTY-NOTICES 一式を同梱する。
  **Marketplace publish は意図的に手動のまま**）。**correctness の門は空のまま**
  （§2F に「決定済み・未実装」は無い。**§2F の `font "NAME"` の項はこの便で全部閉じた**）。
  ⚠️ **出す前に片付ける候補が 2 つ、どちらも 1 便**:
  **⒤ `CHANGELOG` がリポジトリに無い**（`release.yml` はリリース本文を生成せず assets を貼るだけなので、
  **0.3.0 はリリースノート無しで出る**）／**⒥ 未 push が 25 便ぶん**（**タグだけ先に push すると
  CI が古い木を建てる**ので、**`master` を先に push すること**）。
  ／**⒜ §2 の頭の ▶ perf**（第193 の地図のまま・門の外）／
  **⒝ ★ SVG の同梱 sans のずれ**（**和音記号は `font-family="sans-serif"` を出すので*読み手の* sans が
  描き、予約は同梱 Heros**。**同梱名を書けば閉じるが和音記号を持つ snapshot が全部動く**＝
  **出力についてのユーザー判断が要る**。`SvgDrawingContext.FamilyAttributeFor` の注記）／
  **⒞ ★ 配管の残り 6 site**（`TabStaffGeometry` 3・`FetaTextRun` 3。**測るのは同梱 face のまま**）。

---

## 以下は第204セッションの経緯

最終更新 第204セッション＝**印の em・face・そして「Ped.」の*機構*を LilyPond に合わせた便**
（**実装 5 便**。**perf は測っていない＝走査を 1 本も足していないため**。§7.9 の「足していない例」・
commit ごとに理由を 1 行書いた）。
ユーザーの目的は **0.3.0 を出すこと**で、優先は「**文法を完全にしてから出す**」。
★★★ **着手の理由を先に書く**: **第203 の §1 ⑷ ⓪ が「次は 0.3.0 を出すかの判断」としていたので、
裏取りのあと*訊いた*。** **ユーザー決定は 4 件**——
**⑴「⒞ mark の em/style を先にやれ」**（選択肢は ⓪0.3.0／⒞／⒜ shaping／⒝ perf）／
**⑵ snapshot 4 枚の再ベース承認**／**⑶ pedal グリフ移植と snapshot 2 枚の承認**（**便の途中で
「リリース前にもう少し直せ。有利なら着手、不利なら着手するな」の委任**——**有利と判断して着手**）／
**⑷ 最後に「ここで止める」**（**下の ⑹ の新事実を渡した結果**）。
★ **この便は §0 の開始時裏取りを*通しで*走らせた**——**台帳 521 点・ss 非ゼロ 98・
総和 3.609965521・count 点 106/うち非ゼロ 2**・**未追跡 0・作業ツリー項目 0**・**未 push 14**・
**snapshot 218 枚**・suite **5440 passed / 0 failed / 4 skipped**・**コーパス 567 冊**
＝**引継ぎの数は全部合っていた**（**34 回連続**。**第203 が直した「566 は stale」も 567 で一致**）。
 終了時 **未追跡 0・作業ツリー項目 0・未 push 21**（**この行を書く commit を含めて 21**。
⚠️ **この行を*書いた時点*では作業ツリー項目 1**＝`docs/HANDOFF.md`）・
suite **5440 → 5446 passed / 0 failed / 4 skipped**（**+6**＝**台帳 4 点 ＋ 網 2 本**）・
**台帳 521 → 525 点**（**ss 非ゼロ 98 → 99・総和 3.609965521 → 3.644108828**
——⚠️ **総和が増えたのは悪化ではない**。**新しい 4 点のうち 3 点は開いた便のうちに 0 へ閉じ**、
**残る 1 点（sostenuto の 1 device pixel）が「今まで一度も測られていなかった量」を可視化しただけ**。
README の「比較は同じ点集合の中でだけ意味を持つ」の 3 例目）・**count 点 106/2 は不動**・
**snapshot 218 枚で不動＝ただし*中身が* 6 枚動いた**（`test/navigation-marks`・
`test/nav-below-clears-lyrics`・`test/pedal-text`・`test/pedal-mixed`・**後二者は 2 度**。
**どちらの再ベースもユーザー承認済み**）・**Core は 0 警告**。
★★ **環境＝pwsh の MCP コンソールは 1 本（`Turmeric`）で通せた。**

> ## ★★★ この便の骨＝**「先に LP を測る」が、探していなかったものを 3 つ出した**
> **島は「plain-text mark の em が 2.8 か 2.2 か／描画が BoldItalic か italic か」**で、
> §5.2.1③ の順序どおり**プローブ → 台帳 → 移植**に割った。**答えは 2.2・italic**
> （`scm/define-grobs.scm:1898-1926` の `JumpScript` は `font-shape italic` だけを宣言し
> `font-series` も `font-size` も宣言しない ⇒ paper の `text-font-size`＝11pt/5pt＝2.2 ss）。
> **幅から推測したのではない**——**stencil 式が `C059-Italic 3.865234375` mm と*自分で*言う。**
>
> ⑴ ★★ **LP は「Fine」を*書かれた終端では印刷しない***。`lily/jump-engraver.cc:228-237` が
> `finalFineTextVisibility`（既定 `#f`）のとき **grob を suicide させる**。
> **1 回目の run の空の本は「壊れたプローブ」ではなく*測定*だった**（§5.3）。**JMN として残した。**
> ⑵ ★★★ **sustain pedal は LP ではテキストですらない**。`lily/sustain-pedal.cc:47-76` が
> **Emmentaler の `pedal.Ped` / `pedal..` を padding 0 で並べる**（ファイル自身のコメントが
> "we have no kerning"）。**この便の後半で移植して閉じた**（下の ⑷）。
> ⑶ ★★★ **Lily# の advance は em に*線形ではない***。**台帳の初稿は線形と仮定して分解し、
> 存在しない「face/rounding 項 +0.065」を発明していた**——**2.8 の読みを 2.2/2.8 で割ると
> 5.070423959・毒で測ると 5.019066141**（＝LP の bold-italic そのもの）。**Skia は自分の ppem で
> glyph ごとに丸める**ので**2 つの丸めはスケールと交換しない。** ⇒ **台帳 4 点は*毒で*分解した。**

> ## ★★★ 2 つ目の骨＝**射程が機構と合わなかったので、追いかけたら 4 腕のうち 3 腕が偽物だった**
> **移植を当てたら 1119 冊中 18 冊動き、うち 13 冊は plain-text mark を 1 つも持たない。**
> **持っていたのは `form main { Intro A1 … }`＝セクションラベル。**
> ⇒ **`LayoutEngine` の system 間シルエット `AddMarkBox` が、印の箱を*自分で*綴っていた**:
>
> | 腕 | 綴っていた | 正しい家 |
> |---|---|---|
> | **Tempo** | **text の advance を plain-text の em で・しかも*中央*寄せ** | `MetronomeMarkGeometry` の ink・**左**寄せ |
> | **箱ラベル**（Rehearsal/SectionLabel） | **2.8 BoldItalic** | 2.2 / 2.4 **Bold** ＋ markup 箱 padding 0.2 |
> | **記号**（segno/coda） | **手書きの 1.0** | 同じグリフに他所が与えている **1.2** |
> | plain text | ✅ 第203 が一軒へ寄せた唯一の腕 | — |
>
> ⇒ ★★★ **第203 の「size と style は一軒から来る」という注記が*site 全体*が移ったように読める**
> ——**移っていたのは 4 腕のうち 1 腕だけ。** §7.6 の「統一を主張するコメントは網羅性を保証しない」。
> ⇒ ★★ **見つけ方は「射程が機構と合わない」**。**18 冊のうち 13 冊が*機構を持たない*本**だった。
> **これは grep の的でも毒の 0 でもなく、*射程の内訳*が出した所見。**

> ## ★★ 3 つ目の骨＝**毒が「移植後の姿」を先に測った**
> **台帳点を開いた便で `PlainTextFontSize` と `TextStyleOf` に毒を当てた**ところ、
> **em だけ → 残差はちょうど「LP 自身の重み項」／em ＋ style → `fine` と `ds-al-coda` が
> `0.000000000`・`sostenuto` が 1 device pixel・`sustain` は不動**。
> ⇒ **移植の便は「何がどこまで閉じるか」を*知ってから*着手した**（承認を取りに行くときも同じ数を出せた）。

> ## ★★★ 4 つ目の骨＝**「機構の移植」は、算術がコードより先に閉じた**
> **`mark.pedal.width.sustain` は「em を移植しても閉じない」と*書いてから*開けた点**で、
> **後半にその機構を移植して 0 にした。** ★★ **1 手目は反証だった**——
> **`pedal.Ped` / `pedal..` / `pedal.*` が束ねたフォントに在るか。** 在った。**そして LILC の bbox が
> 3.192000000 / 0.280000000 / 1.555600000 ＝ LP の実測 3.472000000 と 1.555600000 を 9 桁で再現した**
> ——**コードを 1 行も書く前に。** ⇒ **移植は `Sustain_pedal::print` の字面移植だけで済んだ。**
> ⚠️ ★★ **観測者も機構を追う必要があった**——**`RenderedGeometry` は文字列を探していたので
> 移植の直後に "found 0" で落ちた。** ***大声で*落ちたのは良い版だが、
> 見張るべき修理を見られない読みは読みではない。** **今は描かれた*グリフ*を測る。**

★ **この便の値段**（**perf は測っていない**＝§7.9 の「足していない例」。
**走査を 1 本も足していない**——**定数を 2 つ動かし、既に在る家を読ませ、`MarkXExtent` に
アダプタを 1 本、pedal に一軒を建てただけ。** **予約側は `SustainPedalExtent` を通り、
グリフ列を作らずに同じ歩きで幅を返す**（**Skia の `MeasureText` を 4 文字のループに置き換えた**）。
**pass も rebuild も新しい鍵も無い。**）:

| 便 | 何が動いたか | 射程（**ディスク上 1119 冊**） |
|---|---|---|
| ⑴ プローブ ＋ 台帳 4 点 ＋ 観測者 | 新規 `.ly` 1 本・台帳 ＋4 点・網 ＋4 | **0 冊**（注記のみ・出力不変） |
| ⑵ シルエットの 4 腕を一軒へ | Core 3 ファイル | **11 冊**（**全部ユーザーの未追跡本・snapshot 0 枚**） |
| ⑶ em/style の移植 | Core 1 ファイル ＋ 網 ＋ snapshot 4 枚 | **5 冊**（**追跡 4 ＋ ユーザー本 1**） |
| ⑷ pedal をグリフで描く | 生成器 2 本 ＋ Core 4 ファイル ＋ snapshot 2 枚 | **2 冊**（**どちらも追跡 fixture**） |
| ⑸ pedal change の nudge | Core 1 ファイル ＋ 網 1 本 | **0 冊**（**毒でも 0＝site に到達する本が無い**） |

- **⑴ ★★★ 第1便＝先に測って台帳を開く**（`06cda89d`）。
  **`audit/lp-geometry/probes/jump-mark-em.ly`（11 冊・数秒）**——**JumpScript を「同じ文字列の
  italic TextScript」と対にして 15 桁一致を出し、bold-italic の対照で重みを*別に*測る。**
  **観測者は `RenderedGeometry.SoleMusicMarkReservedWidth`**——⚠️ **em・style・role を
  `MusicMarkEngraver` の一軒*から読む***（**2.8 と綴っていたら移植を観測できない網になる**・§5.2.1⑥）。
- **⑵ ★★★ 第2便＝シルエットの 4 腕**（`7951a74c`）。上の骨 2。
  ⚠️ **`MarkXExtent` に *アダプタ* を足しただけで 2 つ目のモデルは作っていない**
  （`LayoutEngine` は `MusicMarkLayout` を・重なり検査は `MusicMarkItem` を歩く。
  **その型の違いがこの site に独自の算術を持たせていた理由そのもの**）。
  ⚠️ **Tempo 腕は 100 ss の毒でも 1119 冊中 0 冊**＝**このディスクに到達する本は 1 冊も無い。**
- **⑶ ★★★ 第3便＝em/style の移植**（`6b877a97`・**ユーザー承認済み snapshot 4 枚**）。
  **差分は印の文字列だけ**——**4 枚とも `<text>` 以外の行は 0 行差**。
  ⚠️ ★★ **ラチェットを 1 本引退させ、置き換えを名指した**（§7.6 ⒟）: **第203 の
  「2.4 Bold より狭くしてはいけない」は、LP の答え（2.2 italic）が*どの文字列でも*それより
  狭いので正しい箱を禁じる**。⇒ `PlainTextMarks_AreSetAtLilyPondsOwnTextFontSize` に差し替えた。
- **⑷ ★★★ 第4便＝`Ped.` をグリフで描く**（`801daaf7`・**ユーザー承認済み snapshot 2 枚**）。上の骨 4。
  **差分は置換そのもの**——**`Ped.` の中心 14.73 は不動**（12.99 ＋ 3.472/2 ＝ 14.726）、
  **`pedal..` は 12.99 ＋ 3.192 ＝ 16.18 に継がれる**（LP の `add_at_edge` と同じ）、**他の行は 0 行差。**
- **⑸ ★★ 第5便＝pedal change の nudge**（`0f4a0d49`・**出力不変**）。
  **第203 が「族に関わらず `"Ped."` と綴る＝*文字列*の誤り」として名前だけ付けて残した項**で、
  **⑷ のあとは*機構*の誤りに悪化していた**（**`Sost. Ped.` はテキスト・`Ped.` はグリフ**）。
  **後続の engage を group から*見つける*ようにした。**
  ⚠️ **毒でも 0 冊＝この site に到達する本がディスクに 1 冊も無い**ので、
  **fixture ではなく網を建てた**（`PedalChange_ClearsTheWordThatFollowsIt`・sostenuto の本を作る）。
- **⑹ ★★★ 次に触るならここ**——**⓪ 0.3.0 を出すかの判断**（**`Directory.Build.props` は `0.3.0`・
  タグは 1 つも無い・`v0.3.0` を push すると `.github/workflows/release.yml` が
  test → CLI 4 RID → VSIX を回す**）。**correctness の門は空のまま**（§2F に「決定済み・未実装」は無い）。
  **この島の食い違いは 4 点とも閉じた**（残るのは sostenuto の 1 device pixel＝`text.width.*` の
  丸め項で、**この島のものではない**）。
  ／**⒜ 描画側の shaping 2 軒**（第201 の §1 ⑹ のまま・**観測者は今も 0 本**・門の外）／
  **⒝ §2 の頭の ▶ perf**（第193 の地図のまま）。
- **⑺ ★★★ 便の終わりに 2 件を測って、ユーザーが「やらない」と決めた**——
  **どちらも「既定の振る舞い」の食い違いで、名前・LP 住所・射程・費用が揃っている**:
  ⇒ **⒤ pedal スタイルの既定**。**Lily# は `Staff.PedalStyle = Bracket`・LP は
  `pedalSustainStyle='text` / `pedalSostenutoStyle='mixed`**（`ly/engraver-init.ly:895,904`）。
  **射程 1119 冊中 4 冊（全部 fixture・ユーザー本 0）。費用は定数 1 つ ＋ snapshot 4 枚。**
  ⚠️ **これは*欠陥*ではなく*好み***——**LP は 3 スタイルとも持ち Lily# も 3 つとも持つ。違うのは既定だけ**で、
  **LP の見た目が欲しい作者は今すでに `pedal text` の 1 語で得られる。**
  ⇒ **⒥ `fine` の可視性**。**LP は書かれた終端の「Fine」を印刷しない**（`jump-engraver.cc:228-237`）。
  **射程 1119 冊中 1 冊**（`test/navigation-marks` のみ。**`custom-text.lys` はコメント内で偽陽性**）。
  ⚠️⚠️ ★★★ **やらないと決めた根拠は、着手直前に出た 1 つの事実**——
  **Lily# の `fine` は*印だけ*で、再生に効かない**（`NavigationMarkType` を読むのは exporter・
  構文写像・collector だけ。**MIDI 側は D.S./D.C. を展開すらしない**）。
  **LP が自由に抑制できるのは `\fine` が抑制後も「演奏を終える」仕事を続けるから**で、
  **Lily# では抑制＝トークンの唯一の効果が消える。** ⇒ **逃げ道が必須になり**、
  **その逃げ道は安くない**——**`score` ヘッダはプロパティ 1 個の固定スロット
  （`transpose` 専用ゲート・`RenderDeclarationGreen`）なので、score 階層に足すのは
  green node とパーサの変更**。**パート属性なら 30 行だが、score 全体の規則をパートに置く近似になる。**
  ⇒ ★★ **追跡 1 冊の差のために新語彙を足す取引**なので見送った。**着手するならこの段落から。**
