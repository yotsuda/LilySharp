...
**閉じたもの**（**コード変更ゼロ・出力不変・snapshot 0 枚**）:
```
テキスト幅の book を作り点を 8 つ開く  84b0eedd  台帳 +8 点（+0.445214173）
```
...
```
⇒ **`OttavaBracketEngraver` は `InkX(...).Right` を `Advance(...)` に戻す**。**別 commit・snapshot 要承認**。

**未 push 119**（この引継ぎ commit まで）・テスト **3893 passed / 0 failed / 4 skipped**（**+8**）・
台帳 **416 点**（**ss 非ゼロ 87・総和 4.501145519**／**count 点 99・うち非ゼロ 2**）。
⚠️ **総和が 4.055931346 → 4.501145519 に増えたのは悪化ではない**——**今まで一度も測っていなかった量**
（テキスト幅）を 8 点足した分がそのまま可視化されただけ（README「比較は同じ点集合の中でのみ意味を持つ」）。
⚠️ **この行は書いた直後に stale になる**。§0 のとおり**開始時に必ず実測すること**。

## ▶ 次の一手

★★★ **テキスト幅の移植（この book が観測者を 8 つ用意した）**。**二成分・二段階**で:
```
⑴ 丸め   TextFontMetrics.AdvancePerEm を「グリフごとに round(adv_em × ppem)」に
          ppem = fontSize(ss) × output_scale(mm) × 1200/72   ⚠️ **サイズごとに違う**
          （2.2 は 64.434297 だが mark 2.4・lyric 2.469417 は別の ppem＝定数を 1 つ足す話ではない）
⑵ カーン  ペアの詰めは HarfBuzz（`SkiaSharp.HarfBuzz`）が要る。MeasureText では原理的に出ない
```
⚠️ **どのテキスト grob の幅も動く**（歌詞・和音記号・マーク・メトロノーム・ottava の線の隙間）
＝**snapshot が大量に動く。着手前にユーザー承認**。⚠️ **落とし穴**: ⑴ だけ入れると
`text.width.av` は残り、⑵ だけ入れると `text.width.n1` は残る。**8 点は片方ずつ潰せる形に並べてある**。
★★★ **ottava の `InkX` → `Advance` の訂正**（上記・**単独 commit**）。あわせて
`ottava.x.line-start-to-notehead` の LP 値が**線の stencil を読んでいる 0.05** も why に入れること
（Lily# 側は SVG の `x1` ＝幾何の端なので、**両側の定義がそもそも 0.05 ずれている**）。
