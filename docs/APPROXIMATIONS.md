...
### `LilySharp.Core/Svg/Layout/ElementCoordinator.cs`
- **:2371** observed by: NOTHING. There is no ledger point on a tab tie's width,
### `LilySharp.Core/Svg/Layout/FiguredBassGlyphRun.cs`
...
- **:1804** would be output-moving with no observer, which is the one move HANDOFF 5.0 forbids.
### `LilySharp.Core/Svg/Layout/PedalEngraver.cs`
...
- **:2477** LILYSHARP-OWN: no LilyPond counterpart — LilyPond itself hangs on the
- **:4599** voice last wrote. An undurated grace opens at an eighth — LILYSHARP-OWN, not a
...
### `LilySharp.Core/Svg/Layout/ElementCoordinator.cs`
- **:2360** LILYSHARP-OWN: no head extent on a tab, so the horizontal-distance term
- **:2409** ⚠️ LILYSHARP-OWN, and LilyPond cannot be asked: MEASURED on 2.26.0, all three
- **:3830** ⚠️ LILYSHARP-OWN: a phrasing slur's WRITTEN side ('.up'/'.down') is NOT honoured here, a
...
- **:921** ⚠️ LILYSHARP-OWN: SELECTING A GROB'S SIBLINGS OUT OF A SCORE-WIDE ARRAY. LilyPond
...
- **:826** (LILYSHARP-OWN: LilyPond never emits a system with no live staff at all).
### `LilySharp.Core/Svg/Layout/LayoutEngine.Pages.cs`
...
### `LilySharp.Core/Svg/Layout/OutsideStaffStacker.cs`
- **:1124** system would hand back marks placed without the labels under them. LILYSHARP-OWN
- **:3151** LILYSHARP-OWN: the SUPPORT entry cannot be passed on its far side.
