...
| `OWN` | 122 | LilyPond に対応物が無いと宣言しているもの (LILYSHARP-OWN) |
| **計** | **231** | |
...
| `LilySharp.Core/Svg/Layout/MusicMarkEngraver.cs` | 9 |
...
- **:492** ⚠️ NOT PORTED: courtesy parens are not counted, where LP's grob extent includes
...
- **:1131** ⚠️ NOT PORTED — the round-box form: LP draws each dash as a round_filled_box
- **:1136** whiteout −1) is not ported.
- **:1249** NOT ported; this takes the note's own stem direction. No book and
...
- **:2146** ⚠️ NOT PORTED — the per-type padding: the 2.25 is FLAT, but under the
...
- **:801** differ by half the bump whenever MinimumDy actually bites. Not ported here
...
- **:3267** same approximation the scripts' own remark records for the movers; the books that
...
### `LilySharp.Core/Rendering/SharedRenderer.Beams.cs`
- **:495** no observer; add the paren widths when a book brings one. The unpacked
- **:497** bare glyph width, which is exact for one and unobserved for many.
### `LilySharp.Core/Rendering/SharedRenderer.Marks.cs`
- **:1250** no observer reaches that branch yet.
...
- **:2151** 2.45; no ledger point or book observes a fermata where this floor
- **:2270** box per duration, which Lily# does not read for this purpose. No ledger point observes
...
- **:804** paging augment families. Unreachable today, so no observer CAN be built; the
### `LilySharp.Core/Svg/Layout/LyricHyphen.cs`
...
### `LilySharp.Core/Svg/Layout/MultiStaffLayouter.cs`
- **:4068** company on a row whose symbols sit closer than its spec's 1.0. No point measures
### `LilySharp.Core/Svg/Layout/MusicMarkEngraver.cs`
...
## OWN — LilyPond に対応物が無いと宣言しているもの (LILYSHARP-OWN)（122 件）
...
- **:519** prefix — LILYSHARP-OWN, a decided divergence (user decision
### `LilySharp.Core/Rendering/SharedRenderer.GraceNotes.cs`
- **:239** as for a notation grace. ⚠️ LILYSHARP-OWN, and knowingly so: LilyPond's TabStaff
### `LilySharp.Core/Rendering/SharedRenderer.Marks.cs`
- **:560** LILYSHARP-OWN sizes: the feel equation keeps the small chart-style note (1.6)
- **:1243** LP: ss × length-fraction × 0.81. LILYSHARP-OWN: length-fraction is
...
### `LilySharp.Core/Svg/Layout/LayoutUtilities.cs`
- **:827** (the chain's last node — see its LILYSHARP-OWN remark), the single-page path
...
- **:1659** ⚠️ LILYSHARP-OWN limit: a mid-line meter CHANGE is not a break-align column in
- **:1754** ⚠️ LILYSHARP-OWN, declared (user decision 2026-09-02, HANDOFF §3 第322): the
- **:1800** ⚠️ LILYSHARP-OWN, and it has to be: LilyPond never draws this sign here at all.
...
- **:586** ⚠️ LILYSHARP-OWN: THE CHAIN'S LAST NODE, WHICH IS NOT ALWAYS THE LAST STAFF.
