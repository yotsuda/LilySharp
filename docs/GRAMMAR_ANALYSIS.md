...
| `name = { ... }` | `phrase name { ... }` | キーワードで意図明確 |
| `\name` (変数参照) | `name` | プレフィックスなし、自然な記法 |
| `\name` (フレーズ参照) | `name` | プレフィックスなし、自然な記法 |
| `c-^` | `c@accent` | 名前で意味明確 |
...
// === @ = アノテーション（音符への付加情報） ===
c4@staccato@p@trill    // アーティキュレーション、ダイナミクス、装飾
...
### 完了
- [x] ダイナミクス表記を `\p` → `@p` に変更
- [x] ダイナミクスのフォントサイズを LilyPond に合わせて調整
- [x] セクション開始時のオクターブリセット実装
- [x] 楽器→clef/octave マッピングテーブル実装
- [x] パート定義に `instrument` / `octave` 属性追加

### 優先度: 高
- [ ] VS Code セマンティックハイライト（フレーズ、セクション、変数を色分け）
