...
⇒ ★★★ **第178セッション（同日）で cue は「領域として振る舞う」ところまで閉じた**——
**隣り合う 2 つの `cue { … }` が 2 声部であることを、診断（`e368f995`）と spacing（`f045872e`）の
...
- タイ／スラーが囲いの内外を跨ぐ → 既存 `LYS4010 UnpairedSlur` と同族。**新コード LYS4012**
  ...
  **締める向きだけが後から不可能になる**（`エラー → 警告 → 描く` は誰も壊さない）。
  ...
  ✔ **隣接 cue の間を跨ぐ 1 形も 2026-08-15・第178セッションで閉じた**（`e368f995`）。
  両端とも `IsCue` 真なので**フラグでは見えず**、**collector が領域の*最初の音符*に刻印を打つ**
  （`MusicItem.BeginsCueRegion`）ことで見えるようにした——**領域番号ではなく*縁*の印**なのは、
  番号だと collect resume の suffix splice が**別の walk で採番された tail** を貼り付けるから
  （縁の印は「その領域の中での自分の位置」だけの関数なので貼り付けても正しい）。
  **識別子が要る読み手は、既に歩いている所で導出する**：スラーは走査中に領域を数え、
  タイの結び先は*次の音符*なので「結び先が領域を開くか」がそのまま「別の領域か」になる。
  ⚠️ **エラー文は 3 種を区別する**（外→中／中→外／**cue から次の cue へ**）。
  ⇒ ★★★ **spacing 側も同じ刻印で閉じた**（`f045872e`）。**しかも先に測った**——
  `voice-boundary-spacing.ly` §F は**同じ 4 音で `\new CueVoice` が 1 個か 2 個かだけが違う対**で、
