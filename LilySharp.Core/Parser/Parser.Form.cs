...
internal sealed partial class Parser
{
    ...
    private GrandStaffRenderGreen ParseGrandStaffRender()
    {
        ...
        while (!Check(SyntaxKind.CloseBrace) && !Check(SyntaxKind.EndOfFile))
        {
            ...
            // A group inside a group — the piano inside the orchestra's bracket, the violins'
            // sub-bracket inside the strings — of any type, at any depth, drawn as WRITTEN: each
            // delimiter stands left of the one around it (user decision, session 376, "respect
            // the .lys the user wrote"; LilyPond draws every such nesting the same way).
            if (Check(SyntaxKind.GrandStaffKeyword) || Check(SyntaxKind.StaffGroupKeyword)
                || Check(SyntaxKind.ChoirStaffKeyword))
            {
                members.Add(ParseGrandStaffRender());
                continue;
            }
            ...
            _diagnostics.Error(
                ...
                + "'grandStaff', 'staffGroup' and 'choirStaff' groups and, between them, "
                + "'lyrics NAME' rows. "
            ...
            while (!Check(SyntaxKind.StaffKeyword) && !Check(SyntaxKind.LyricsKeyword)
                ...
                && !Check(SyntaxKind.GrandStaffKeyword) && !Check(SyntaxKind.StaffGroupKeyword)
                && !Check(SyntaxKind.ChoirStaffKeyword)
