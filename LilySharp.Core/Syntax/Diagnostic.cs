...
/// · <c>LYS2003</c> NoTimeSignature · <c>LYS2008</c> EmptyPlaceholderMeasure ·
/// <c>LYS2014</c> PercentBodyTooLong (2026-08-29: it named a picture Lily# invented — one
/// percent per repeated measure for a body of three or more whole measures — and once the
/// repetition became LilyPond's one slash over blank measures, it had nothing to warn
/// about) · <c>LYS8007</c> FontOneLinerRemoved ·
...
public static class DiagnosticCodes
{
    ...
    // ⚠️ LYS2014 (PercentBodyTooLong) IS RETIRED — session 286, and the number is not reused.
    // It warned that a `repeat percent` body of three or more whole measures earned a sign
    // that could not say what it repeated. That was true of the sign Lily# was drawing — one
    // percent per repeated measure — and of nothing else: LilyPond's iterator has two
    // equality tests and one else, and the else engraves ONE slash for the whole repetition
    // with its remaining measures blank. Lily# draws that now, so the warning has no subject.
