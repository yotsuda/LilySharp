...
- **行中の clef/key 変更**（4点・6.843242 ss）→ `e21acc22`
- **グリフメトリクス**（7点）→ `22673bbb`。LP と同じく `LILC` テーブルから読むようにした
- **行頭の key/time 変更**（4点・4.439007 ss）→ `72e48460`。
  **着手前に置いた4点の予測が桁まで的中**した（`COORDINATE_AUDIT.md` §4.7.3）
- **臨時記号 padding**（3点・0.334252 ss）→ `6f2305f2`。LP の `right-padding` 0.15 が抜けていた
- **`fills_measure` が rest を弾いていた**（1点・1.000000 ss）→ `1fb3a703`

### ★ 片側しか測っていない点は、欠陥に構造的に盲目になる

`1fb3a703` の教訓。スコア F（`r1 | r1`）は**閉じ側** `barline.prev.whole-rest` だけを持っていて
...
15点 4.592405 → 19点 11.435647（`813ec1f3` が**それまで測っていなかった**行中の発散を可視化）
→ 21点 4.747978（`e21acc22`＋MKA 2点）→ 4.738987（`22673bbb`）→ 0.338987（`72e48460`）
→ 0.022361（`6f2305f2`）→ 22点 0.022361（`1fb3a703`、足した点が exact で着地）
