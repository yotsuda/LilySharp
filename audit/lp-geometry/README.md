...
19 点中 **7 点が LP と厳密一致**、total |residual| = **11.435647 ss**。残る12点の内訳:

| residual | 原因 |
|---|---|
| +2.451 / −2.000 / +1.756 / −0.637 | **行中**の clef/key 変更に専用列が無い（`COORDINATE_AUDIT.md` §4.7.2） |
| −2.234 / −1.455 / −0.500 / −0.250 | **行頭**の key/time 変更が break-align 列に入っていない（§3.I・型の欠落） |
| −0.150 | 臨時記号→符頭の距離が Lily# 1.300 / LP 1.450 |
| +0.00002 | 符尾の符頭接続オフセットのメトリクス差 |
| −0.002 / −0.0013 | **OPEN** — 閉じ側の gap がわずかに狭い。未帰属 |

⚠️ **点を足すと total は増えうる。** 4.592405 → 11.435647 は悪化ではなく、
行中 clef/key という**それまで測っていなかった発散**を可視化した結果
（`84dc3a79` で4点追加）。**比較は同じ点集合の中でのみ意味を持つ。**
