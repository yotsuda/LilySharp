# A/B perf round 19 (session 120: loose change-column port + hemiola bare arm +
# add_rod blocking-force branch).
# Heavy sides this session created:
#   hairpingrand1k — grand staff (multi-staff): CollectStaffIndicesAtIndex now runs
#                    per measure in BOTH the layouter and the break gate, and every
#                    measure pays the ComputeLooseChangeHangs any-change scan.
...
$inputs = @(
  @{ Name='hairpingrand1k (grand staff; staff-index + hang-gate side)'; File='C:\MyProj\LilySharp\audit\lpreg\perf-hairpingrand1k.lys'; Hash=$true },
